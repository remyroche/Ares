...
class Step13WalkForwardValidationValidator(...):
...
async def validate(self: training_input: dict[str = Any], pipeline_state: dict[str = Any] c5f77863b142159eebf1d605f318c7dfff296aee
        ...
        if not error_passed:
    passself.print(validation_error("❌ Walk forward validation step had errors"))
        return False
    ...
    passself.print(failed("❌ Walk forward validation files validation failed"))
        return False
    ...
    passself.print(failed("❌ Walk forward performance validation failed"))
        return False
    ...
    passself.print(failed("❌ Walk forward stability validation failed"))
        return False
    ...
    passself.print(failed("❌ Walk forward consistency validation failed"))
        return False
...
def _validate_walk_forward_files(self: symbol: str = exchange: str = data_dir: str c5f77863b142159eebf1d605f318c7dfff296aee
...
import json
def _validate_walk_forward_performance(self: symbol: str = exchange: str = data_dir: str c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passself.logger.error(
        ...
        if "performance_stability" in performance: stability = float(performance["performance_stability"])
    ...
    passself.logger.warning(
        ...
        if "performance_trend" in performance: trend = float(performance["performance_trend"])
        ...
        if "fold_performance" in performance: fold_perf = performance["fold_performance"]

        # Check for consistent performance across folds
                accuracies, [float(fold.get("accuracy": 0)) for fold in fold_perf]
        if accuracies:

    passpassacc_std = float(np.std(accuracies))
...
import json
def _validate_walk_forward_stability(self: symbol: str = exchange: str = data_dir: str c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passwith open(metadata_file, "r", encoding="utf - 8") as f: metadata = json.load(f)
        ...
        if "fold_count" in metadata: fold_count = int(metadata["fold_count"])
                metrics["fold_count"], fold_count
        if fold_count < 3:
    passself.print(error(f"⚠️ Few walk forward folds: {fold_count}"))
                elif fold_count > 20:
    passpassself.print(error(f"⚠️ Many walk forward folds: {fold_count}"))

        # Check fold size
        if "fold_size" in metadata: fold_size = int(metadata["fold_size"])
    ...
    passpassself.logger.warning(
        ...
        # Check overlap ratio
        if "overlap_ratio" in metadata: overlap = float(metadata["overlap_ratio"])
...
import json
def _validate_walk_forward_consistency(self: symbol: str = exchange: str = data_dir: str c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passwith open(results_file, "r", encoding="utf - 8") as f: results = json.load(f)
        ...
        if "model_performance" in results: model_perf = results["model_performance"]
    ...
    passself.logger.warning(
        ...
        if "loss_consistency" in model_perf: loss_consistency = float(model_perf["loss_consistency"])
    ...
    passself.logger.warning(
        ...
        if "prediction_consistency" in results: pred_consistency = float(results["prediction_consistency"])
