...
class DataQualityMonitor:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.quality_thresholds = {'excellent': 0.95, 'good': 0.85, 'acceptable': 0.75, 'poor': 0.6, 'critical': 0.5}
    ...
    def _calculate_consistency(self, data: pd.DataFrame) -> float:
        ...
        try:
            ...
            if all((col in data.columns for col in ['open', 'high', 'low', 'close'])):
                price_consistency = ((data['high'] >= data['low']) & (data['high'] >= data['open']) & (data['high'] >= data['close']) & (data['low'] <= data['open']) & (data['low'] <= data['close'])).mean()
    ...
    def _calculate_validity(self, data: pd.DataFrame) -> float:
        ...
        try:
            ...
            if 'volume' in data.columns:
                volume_validity = ((data['volume'] >= 0) & (data['volume'] < 1000000000000.0)).mean()
    ...
    def _calculate_accuracy(self, data: pd.DataFrame) -> float:
        ...
        try:
            ...
            if 'close' in data.columns:
                ...
                iqr = q3 - q1
                outlier_ratio = ((data['close'] < q1 - 1.5 * iqr) | (data['close'] > q3 + 1.5 * iqr)).mean()
                accuracy_checks.append(1.0 - outlier_ratio)
            if 'close' in data.columns and len(data) > 1:
                returns = data['close'].pct_change().abs()
                extreme_moves = (returns > 0.5).mean()
    ...
    def _check_data_types(self, data: pd.DataFrame, step_name: str) -> list[str]:
        ...
        issues = []
        expected_types = {'timestamp': 'datetime64[ns]', 'open': 'float64', 'high': 'float64', 'low': 'float64', 'close': 'float64', 'volume': 'float64'}
    ...
    async def get_quality_summary(self) -> dict[str, Any]:
        ...
        return {'total_checks': len(self.quality_history), 'average_quality_score': np.mean(scores), 'min_quality_score': np.min(scores), 'max_quality_score': np.max(scores), 'quality_level_distribution': pd.Series(quality_levels).value_counts().to_dict(), 'recent_quality_trend': scores[-10:] if len(scores) >= 10 else scores, 'critical_issues_count': sum((1 for metrics in self.quality_history if metrics.quality_level == QualityLevel.CRITICAL)), 'poor_quality_count': sum((1 for metrics in self.quality_history if metrics.quality_level in [QualityLevel.CRITICAL, QualityLevel.POOR]))}

    async def generate_quality_report(self) -> dict[str, Any]:
        """Generate comprehensive quality report."""
        quality_summary = await self.get_quality_summary()
        return {'report_timestamp': datetime.now().isoformat(), 'quality_summary': quality_summary, 'compatibility_summary': {'total_checks': len(self.compatibility_history), 'compatible_count': sum((1 for m in self.compatibility_history if m.overall_compatible)), 'incompatible_count': sum((1 for m in self.compatibility_history if not m.overall_compatible))}, 'format_summary': {'total_checks': len(self.format_history), 'format_match_count': sum((1 for m in self.format_history if m.format_match)), 'format_mismatch_count': sum((1 for m in self.format_history if not m.format_match))}, 'index_summary': {'total_checks': len(self.index_history), 'valid_index_count': sum((1 for m in self.index_history if m.overall_valid)), 'invalid_index_count': sum((1 for m in self.index_history if not m.overall_valid))}, 'recent_issues': [{'step': f'Step {i + 1}', 'quality_score': metrics.overall_score, 'issues': metrics.issues, 'warnings': metrics.warnings} for i, metrics in enumerate(self.quality_history[-5:])]}
