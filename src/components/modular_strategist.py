...
class ModularStrategist(IStrategist):
    ...
    @handle_errors(
        ...
        default_return=None,
        context="modular_strategist_start"
    )
    ...
    @handle_errors(
        exceptions=(Exception,),
        default_return=None,
        context="modular_strategist_stop"
    )
    ...
    @handle_errors(
        exceptions=(Exception,),
        default_return={},
        context="get_strategy_performance"
    )
    ...
    async def _determine_leverage_cap(self, analysis_result: AnalysisResult) -> float:
        ...
        try:
            ...
            leverage_cap = base_leverage * leverage_multiplier * regime_multiplier * risk_multiplier
    ...
    async def _determine_max_notional_size(self, analysis_result: AnalysisResult) -> float:
    ...
    async def _calculate_risk_parameters(self, analysis_result: AnalysisResult) -> Dict[str, float]:
    ...
    async def _assess_market_conditions(self, analysis_result: AnalysisResult) -> Dict[str, Any]:
    ...
    async def _update_strategy_performance(self, strategy_result: StrategyResult) -> None:
        ...
        try:
            ...
            performance["avg_leverage"] = (
                (current_avg_leverage * (total_decisions - 1) + strategy_result.leverage_cap) / total_decisions
            )
            performance["avg_position_size"] = (
                (current_avg_position_size * (total_decisions - 1) + strategy_result.max_notional_size) / total_decisions
            )
