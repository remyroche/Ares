...
## 2026-05-07 - [Iterrows] Learning: Replaced slow Pandas iterrows loop with Numpy/Zip. Action: Extract columns to numpy arrays using a safe getter to prevent missing column KeyError, then zip the index and columns instead of `.iterrows()` to iterate over them for significant performance gain.
