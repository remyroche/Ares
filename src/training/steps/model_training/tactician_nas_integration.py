    ...
    def integrate_nas_model(self, 
        ...
        try:
            ...
            if X_train.shape[1] > 200:
                tprint_warning("⚠️ High feature count detected, using existing comprehensive feature selection pipeline...")
                ...
                tprint_success(f"✅ Existing pipeline: → {X_train.shape[1]} features")
                tprint_info(f"📊 Pipeline methods: {feature_selector.get_applied_methods()}")
                tprint_success(f"🎯 Final feature reduction to {X_train.shape[1]} features using existing comprehensive pipeline")
