...
import joblib # For saving/loading skopt results and other objects
...
class TrainingPipeline:
    ...
    def __init__(self):
        ...
        self.pipeline_progress_file = os.path.join(self.checkpoint_dir, CONFIG['PIPELINE_PROGRESS_FILE'])
        self.prepared_data_checkpoint_file = os.path.join(self.checkpoint_dir, CONFIG['PREPARED_DATA_CHECKPOINT_FILE'])
        self.walk_forward_reports_file = os.path.join(self.checkpoint_dir, CONFIG['WALK_FORWARD_REPORTS_FILE'])
        self.optimizer_checkpoint_file = os.path.join(self.checkpoint_dir, CONFIG['OPTIMIZER_CHECKPOINT_FILE'])
    ...
    async def run(self):
        ...
        try:
            ...
            self.logger.info(f"Resuming from last completed fold: {last_completed_fold}")

            # --- STAGE 1 & 2: Data Loading and Preparation (with checkpoint) ---
            self.logger.info("STAGE 1 & 2: Loading and preparing historical data (with checkpoint)...")
            full_prepared_data = pd.DataFrame()
            if os.path.exists(self.prepared_data_checkpoint_file):
                self.logger.info(f"Loading prepared data from checkpoint: {self.prepared_data_checkpoint_file}")
                full_prepared_data = pd.read_parquet(self.prepared_data_checkpoint_file)
            
            if full_prepared_data.empty:
                self.logger.info("Prepared data checkpoint not found or empty. Loading raw data and preparing from scratch.")
                ...
                if full_prepared_data.empty:
                    self.logger.error("Failed to prepare full dataset. Halting pipeline.")
                    return
                
                self.logger.info(f"Saving prepared data to checkpoint: {self.prepared_data_checkpoint_file}")
                full_prepared_data.to_parquet(self.prepared_data_checkpoint_file)
            else:
                self.logger.info(f"Loaded {len(full_prepared_data)} rows of prepared data from checkpoint.")
            ...
            for fold, (train_index, test_index) in enumerate(all_splits):
                if fold <= last_completed_fold:
                    self.logger.info(f"Skipping fold {fold + 1} as it was already completed.")
                    continue
                ...
                all_klines, all_agg_trades, all_futures = load_raw_data() # Re-load full raw data
                
                train_klines = all_klines.loc[train_data.index.min():train_data.index.max()]
                train_agg_trades = all_agg_trades.loc[train_data.index.min():train_data.index.max()]
                train_futures = all_futures.loc[train_data.index.min():train_data.index.max()]
                ...
                self.logger.info("Hyperparameters re-optimizing/resuming for this fold.")
            ...
            self.logger.error(f"An error occurred during the pipeline: {e}", exc_info=True)
    ...
    def _load_pipeline_progress(self):
        ...
        if os.path.exists(self.pipeline_progress_file):
            try:
                ...
                self.logger.warning(f"Error reading pipeline progress file: {e}. Starting fresh.")
    ...
    def _save_pipeline_progress(self, last_completed_fold: int, all_fold_reports: list):
        ...
        try:
            ...
            self.logger.info(f"Pipeline progress saved: Completed fold {last_completed_fold}")
        except Exception as e:
