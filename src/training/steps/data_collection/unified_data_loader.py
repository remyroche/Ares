...
class UnifiedDataLoader:
    ...
    async def load_unified_data(
        ...
        try:
            ...
            if not data_path.exists():
                self.logger.warning(
                    f"Data path does not exist: {data_path}, trying consolidated files..."
                )
                # Try to find consolidated files as fallback
                consolidated_path = (
                    Path(data_dir)
                    / f"features_{exchange.lower()}_{symbol}_consolidated.parquet"
                )
                if consolidated_path.exists():
                    ...
                    self.logger.error(
                        f"Data path does not exist and no consolidated file available: {consolidated_path}"
                    )
                    return None
            else:
                ...
                if not parquet_files:
                    # Try to find consolidated files as fallback
                    self.logger.warning(
                        f"No parquet files found in {data_path}, trying consolidated files..."
                    )
                    ...
                    if consolidated_path.exists():
                        self.logger.info(
                            f"📁 Found consolidated file: {consolidated_path}"
                        )
                        parquet_files = [str(consolidated_path)]
                    else:
                        ...
                        consolidated_1m_path = (
                            data_path.parent
                            / f"{symbol.lower()}_1m"
                            / f"features_{symbol.lower()}_1m_consolidated.parquet"
                        )
                        if consolidated_1m_path.exists():
                            self.logger.info(
                                f"📁 Found 1m consolidated file: {consolidated_1m_path}"
                            )
                            parquet_files = [str(consolidated_1m_path)]
                        else:
                            self.logger.error(
                                f"No parquet files found in {data_path} and no consolidated files available"
                            )
                            return None
            ...
            if start_date or end_date:
                ...
                data = await self._load_data_with_date_filters(
                    parquet_files, start_date, end_date, columns
                )
                if data is None or data.empty:
                    self.logger.error(
                        "No data loaded with date filters, trying fallback approach"
                    )
            ...
            if "timestamp" in data.columns:
                ...
                if duplicates_removed > 0:
                    self.logger.warning(
                        f"🧹 Removed {duplicates_removed} duplicate timestamps during data loading"
                    )

            if start_date or end_date:
                self.logger.info(
                    f"Successfully loaded {len(data)} rows from {len(parquet_files)} files"
                )
            else:
                self.logger.info(
                    f"Successfully loaded {len(data)} rows from {latest_file}"
                )
            return data
    ...
    async def _load_data_with_date_filters(
        self,
        ...
        try:
            self.logger.info(
                f"🎯 Loading data for date range: {start_date} to {end_date}"
            )
            ...
            if consolidated_files:
                self.logger.info(
                    f"📁 Found {len(consolidated_files)} consolidated files, using most recent"
                )
                # Use the most recent consolidated file - ensure it's a Path object
                latest_consolidated = max(
                    consolidated_files,
                    key=lambda x: Path(x).stat().st_mtime
                    if isinstance(x, str)
                    else x.stat().st_mtime,
                ...
                if data is not None and not data.empty:
                    ...
                    if not data.empty:
                        self.logger.info(
                            f"✅ Loaded {len(data)} rows from consolidated file"
                        )
                        return data
            ...
            self.logger.info(
                "📁 No consolidated files or insufficient data, loading from partitioned files"
            )
            ...
            for file_path in sorted(
                ...
                if file_data is not None and not file_data.empty:
                    ...
                    filtered_data = self._apply_date_filters(
                        file_data, start_date, end_date
                    )
                    if not filtered_data.empty:
                        ...
                        self.logger.info(
                            f"📊 Loaded {len(filtered_data)} rows from {file_name} (total: {total_rows})"
                        )

            if not all_data:
                self.logger.error(
                    "No data found in any files for the specified date range"
                )
                return None
            ...
            self.logger.info(
                f"✅ Efficiently loaded {len(data)} rows for date range {start_date} to {end_date}"
            )
            return data
    ...
    async def _load_data_file(
        self, file_path: Path, columns: Optional[List[str]] = None
    ) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            non_finite = (
                ~np.isfinite(data.select_dtypes(include=[np.number])).values
            ).sum()
            if non_finite > 0:
                print(
                    f"🔍 [DEBUG] UnifiedDataLoader._load_data_file - {Path(file_path).name}: {non_finite} non-finite values IMMEDIATELY after parquet read"
                )
                for col in data.select_dtypes(include=[np.number]).columns:
                    col_non_finite = (~np.isfinite(data[col])).sum()
                    if col_non_finite > 0:
                        print(
                            f"🔍 [DEBUG] UnifiedDataLoader._load_data_file - {Path(file_path).name} {col}: {col_non_finite} non-finite values"
                        )
                        ...
                        print(
                            f"🔍 [DEBUG] UnifiedDataLoader._load_data_file - {Path(file_path).name} {col}: Non-finite values at rows: {non_finite_rows[:10]}"
                        )
                        # Show the actual values
                        non_finite_values = data.loc[non_finite_mask, col].tolist()
                        print(
                            f"🔍 [DEBUG] UnifiedDataLoader._load_data_file - {Path(file_path).name} {col}: Non-finite values: {non_finite_values[:10]}"
                        )
            ...
            if data.index.name == "timestamp" or (
                hasattr(data.index, "name") and data.index.name == "timestamp"
            ):
                ...
                if non_finite_after > non_finite:
                    print(
                        f"🔍 [DEBUG] UnifiedDataLoader._load_data_file - {Path(file_path).name}: {non_finite_after - non_finite} NEW non-finite values introduced during timestamp conversion"
                    )
            elif (
                "timestamp" not in data.columns
                and hasattr(data.index, "dtype")
                and "datetime" in str(data.index.dtype)
            ):
            ...
            if len(data) > self.max_rows:
                self.logger.warning(
                    f"Data has {len(data)} rows, exceeding limit of {self.max_rows}"
                )
    ...
    def _fix_non_finite_values(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        print(
            f"🔧 [DEBUG] Fixing {non_finite_before} non-finite values using forward/backward fill (limit 6 consecutive)"
        )
        ...
        for col in numeric_columns:
            ...
            print(
                f"🔧 [DEBUG] Column '{col}': {original_non_finite} non-finite values to fix"
            )
            ...
            if non_finite_mask.any():
                ...
                diff = np.diff(
                    np.concatenate(([False], non_finite_mask.values, [False]))
                )
                ...
                for start, end in zip(start_indices, end_indices):
                    ...
                    if group_length > 6:
                        print(
                            f"⚠️ [WARNING] Column '{col}' has {group_length} consecutive non-finite values at indices {start}-{end-1} (exceeds 6-row limit)"
                        )
            ...
            if remaining_non_finite > 0:
                print(
                    f"⚠️ [WARNING] Column '{col}': {remaining_non_finite} non-finite values remain after forward/backward fill"
                )
                ...
                if col_data.dtype in [np.float64, np.float32]:
                    ...
                    if np.isfinite(col_mean):
                        ...
                        print(
                            f"🔧 [DEBUG] Column '{col}': Filled remaining {remaining_non_finite} values with column mean {col_mean}"
                        )
                    else:
                        print(
                            f"⚠️ [ERROR] Column '{col}': Cannot use mean fallback - column mean is also non-finite"
                        )
                else:
                    print(
                        f"⚠️ [ERROR] Column '{col}': Cannot apply fallback for non-float column"
                    )
            ...
            if final_non_finite == 0:
                print(
                    f"✅ [SUCCESS] Column '{col}': Successfully fixed all {original_non_finite} non-finite values"
                )
            elif final_non_finite < original_non_finite:
                print(
                    f"✅ [PARTIAL] Column '{col}': Reduced non-finite values from {original_non_finite} to {final_non_finite}"
                )
            else:
                print(
                    f"❌ [FAILED] Column '{col}': Non-finite values unchanged or increased ({final_non_finite})"
                )
        ...
        if non_finite_after == 0:
            print(
                f"✅ [SUCCESS] All non-finite values fixed! Reduced from {non_finite_before} to {non_finite_after}"
            )
        else:
            print(
                f"⚠️ [PARTIAL] {non_finite_after} non-finite values remain after fixing (was {non_finite_before})"
            )
    ...
    def _extract_metadata_from_path(self, file_path: Path) -> Dict[str, Any]:
        ...
        if root_idx is not None:
            ...
            if len(parts) > root_idx + 4:
                ...
                timeframe_value = (
                    dataset_part.split("_", 1)[1]
                    if "_" in dataset_part
                    else dataset_part
                )
        ...
        for part in parts:
            if part.startswith("year="):
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Invalid year partition value '{value}' in {file_path}"
                    )
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Invalid month partition value '{value}' in {file_path}"
                    )
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Invalid day partition value '{value}' in {file_path}"
                    )

        return metadata

    def _inject_partition_metadata(
        self, data: pd.DataFrame, file_path: Path
    ) -> pd.DataFrame:
        ...
        for column, value in metadata.items():
            ...
            value_to_assign = (
                str(value) if column in {"exchange", "symbol", "timeframe"} else value
            )
        ...
        if "day" not in data.columns or data["day"].isna().all():
            if "timestamp" in data.columns:
                ...
                if timestamp_series.dtype.kind in {"i", "u"}:
                    ts = pd.to_datetime(
                        timestamp_series, unit="ms", errors="coerce", utc=True
                    )
                else:
            ...
            if start_date:
                ...
                if isinstance(start_date, np.ndarray):
                    if start_date.size == 1:
                        ...
                        self.logger.warning(
                            f"Invalid start_date format: numpy array with {start_date.size} elements"
                        )
                        return data
            ...
            if end_date:
                ...
                if isinstance(end_date, np.ndarray):
                    if end_date.size == 1:
                        ...
                        self.logger.warning(
                            f"Invalid end_date format: numpy array with {end_date.size} elements"
                        )
                        return data
    ...
    async def validate_data_quality(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if negative_prices > 0:
                validation_results["issues"].append(
                    f"Found {negative_prices} negative price values"
                )
                validation_results["passed"] = False

            # Check OHLC consistency
            # Fast vectorized calculation
            open_ok = (data["low"] <= data["open"]) & (data["open"] <= data["high"])
            close_ok = (data["low"] <= data["close"]) & (data["close"] <= data["high"])
            ohlc_errors = (~(open_ok & close_ok)).sum()

            if ohlc_errors > 0:
                validation_results["warnings"].append(
                    f"Found {ohlc_errors} OHLC consistency errors"
                )
    ...
    def _robust_timestamp_conversion(self, timestamp_series: pd.Series) -> pd.Series:
        ...
        try:
            ...
            self.logger.warning(
                "Standard timestamp conversion failed, attempting corruption correction"
            )
    ...
    def _validate_timestamps(self, timestamps: pd.Series) -> bool:
        ...
        within_bounds = (
            (valid_timestamps >= min_date) & (valid_timestamps <= max_date)
        ).all()
        ...
        if len(valid_timestamps) < 10:
            ...
            reasonable_range = (
                (valid_timestamps >= pd.Timestamp("1990-01-01", tz="UTC"))
                & (valid_timestamps <= pd.Timestamp("2026-12-31", tz="UTC"))
            ).all()
            ...
            if timestamp_series.dtype.kind in ["i", "f"]:
                ...
                if len(sample_values) > 0:
                    ...
                    if max_val > 1e12:  # Values are in milliseconds
                        self.logger.info(
                            "Detected millisecond timestamps, converting to seconds"
                        )
                        try:
                            # Convert milliseconds to seconds
                            corrected_values = timestamp_series / 1000
                            converted = pd.to_datetime(
                                corrected_values, unit="s", utc=True
                            )
                            self.logger.debug(
                                f"Converted timestamps sample: {converted.head()}"
                            )
                            self.logger.debug(
                                f"Converted timestamps range: {converted.min()} to {converted.max()}"
                            )
                            validation_result = self._validate_timestamps(converted)
                            self.logger.debug(f"Validation result: {validation_result}")
                            if validation_result:
                                self.logger.info(
                                    "Successfully corrected millisecond timestamps"
                                )
                                return converted
                            else:
                                self.logger.warning(
                                    "Millisecond conversion produced invalid timestamps"
                                )
                        except Exception as e:
                    ...
                    # If values are in the range of microseconds (16+ digits)
                        ...
                        self.logger.info(
                            "Detected microsecond timestamps, converting to seconds"
                        )
                        try:
                            ...
                            if self._validate_timestamps(converted):
                                self.logger.info(
                                    "Successfully corrected microsecond timestamps"
                                )
                                return converted
                        except Exception as e:
                    ...
                    # If values are in the range of nanoseconds (19+ digits)
                        ...
                        self.logger.info(
                            "Detected nanosecond timestamps, converting to seconds"
                        )
                        try:
                            ...
                            if self._validate_timestamps(converted):
                                self.logger.info(
                                    "Successfully corrected nanosecond timestamps"
                                )
                                return converted
                        except Exception as e:
            ...
            if timestamp_series.dtype.kind in ["i", "f"]:
                ...
                for factor in [1000, 1000000, 1000000000]:
                    try:
                        ...
                        if self._validate_timestamps(converted):
                            self.logger.info(
                                f"Successfully corrected timestamp by dividing by {factor}"
                            )
                            return converted
                    except Exception:
                        continue
            ...
            try:
                ...
                if valid_mask.any():
                    ...
                    if len(valid_timestamps) > 1:
                        ...
                        if self._validate_timestamps(converted):
                            self.logger.info(
                                "Successfully interpolated missing timestamps"
                            )
                            return converted
