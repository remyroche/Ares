...
def build_threshold_sweep_configs(base_config: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # 4b) Support touch band sweep (how close price must be to support)
    # Tighter band -> fewer, more precise touches
    add_variant(
        "touch_band_tight_0.001",
        {"support_touch_band_pct": 0.0010}
    )
    # Medium band
    add_variant(
        "touch_band_medium_0.002",
        {"support_touch_band_pct": 0.0020}
    )
    # Wider band -> more touches, potentially noisier
    add_variant(
        "touch_band_wide_0.004",
        {"support_touch_band_pct": 0.0040}
    )
...
def save_sweep_results(
    ...
    csv_path = out_dir / f"support_break_threshold_sweep_{symbol}_{timestamp}.csv"
    analysis_path = out_dir / f"support_break_threshold_sweep_{symbol}_{timestamp}_analysis.json"
