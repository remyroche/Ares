...
class MonteCarloValidationStep(BaseValidationStep):
    ...
    def _initialize_step(self) -> None:
        ...
        self.mc_config = {
            "n_iterations": self.config.get("monte_carlo_iterations", 100),
            "test_size": self.config.get("monte_carlo_test_size", 0.2),
            "bootstrap": self.config.get("monte_carlo_bootstrap", True),
            "stratify": self.config.get("monte_carlo_stratify", True),
            "confidence_level": self.config.get("confidence_level", 0.95),
            "parallel_iterations": self.config.get("parallel_iterations", True)
        }
    ...
    def _validate_step_specific_inputs(
        ...
        # Check if we have sufficient data for Monte Carlo
    ...
    @handles_errors(
        ...
        context="monte carlo validation logic"
    )
    async def execute_logic(
        ...
        for iteration in range(self.mc_config["n_iterations"]):
            ...
            iteration_results = await self._run_single_simulation(
                models, X, y, iteration
            )
        ...
        result[f"{self.full_step_name}_results"] = {
            ...
            "model_distributions": self.model_distributions,
            "statistics": mc_statistics,
            "confidence_intervals": confidence_intervals,
            "configuration": self.mc_config
        }
        
        # Create summary
        result[f"{self.full_step_name}_summary"] = self._create_validation_summary({
            "model_results": mc_statistics,
            "overall_metrics": self._calculate_overall_mc_metrics(mc_statistics)
        })
    ...
    async def _run_single_simulation(
        ...
        if self.mc_config["bootstrap"]:
            ...
            if oob_indices:
                ...
                # Fallback to random split if no OOB samples
                X_train, X_test, y_train, y_test = train_test_split(
                    X_sample, y_sample,
                    test_size=self.mc_config["test_size"],
                    stratify=y_sample if self.mc_config["stratify"] else None,
            ...
            X_train, X_test, y_train, y_test = train_test_split(
                X, y,
                test_size=self.mc_config["test_size"],
                stratify=y if self.mc_config["stratify"] else None,
                random_state=iteration
            )
            X_sample = X_train
            y_sample = y_train
        
        simulation_result["sample_info"] = {
    ...
    def _update_distributions(self, iteration_results: Dict[str, Any]) -> None:
        ...
        for model_name, metrics in iteration_results["model_results"].items():
            ...
            if model_name not in self.model_distributions:
                self.model_distributions[model_name] = {
                    "accuracy": [],
                    "precision": [],
                    "recall": [],
                    "f1_score": []
            ...
            for metric_name in ["accuracy", "precision", "recall", "f1_score"]:
                if metric_name in metrics:
                    self.model_distributions[model_name][metric_name].append(
                        metrics[metric_name]
                    )
    ...
    def _calculate_overall_mc_metrics(
        self,
        mc_statistics: Dict[str, Dict[str, float]]
    ) -> Dict[str, float]:
        """Calculate overall Monte Carlo metrics."""
        metrics = {
            "n_models_validated": len(mc_statistics),
            "n_simulations": self.mc_config["n_iterations"],
            "avg_accuracy": [],
            "avg_f1": [],
    ...
    def _add_step_specific_summary(
        ...
        if overall.get("avg_f1", 0) > 0:
            summary["key_findings"].append(
                f"Average F1 score across {overall.get('n_simulations', 0)} simulations: {overall['avg_f1']:.3f}"
            )
        
        if overall.get("model_stability", 0) > 0.85:
            summary["key_findings"].append(
                f"High model stability across random samples: {overall['model_stability']:.3f}"
            )
        
        # Warnings
        if overall.get("avg_cv", 0) > 0.2:
            summary["warnings"].append(
                f"High coefficient of variation ({overall['avg_cv']:.3f}) indicates unstable performance"
            )
        
        # Recommendations
        if overall.get("n_simulations", 0) < 100:
            summary["recommendations"].append(
                "Consider increasing the number of Monte Carlo iterations for more robust estimates"
            )
