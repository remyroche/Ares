...
class LiveWaveletDemo:
    ...
    def __init__(self, config_path: str='src/config/live_wavelet_config.yaml') -> None:
    ...
    def _generate_demo_data(self) -> None:
        ...
        try:
            ...
            for i in range(n_points):
                ...
                self.demo_data.append({'timestamp': time.time() + i, 'open': open_price, 'high': high, 'low': low, 'close': price, 'volume': volume})
    ...
    def _create_market_data(self, index: int) -> dict:
        ...
        try:
            ...
            return {'price_data': pd.DataFrame([data_point]), 'volume_data': pd.DataFrame({'volume': [data_point['volume']]}), 'timestamp': data_point['timestamp'], 'symbol': 'BTCUSDT', 'exchange': 'BINANCE'}
        except Exception as e:
    ...
    def _log_signal_results(self, results: dict) -> None:
        ...
        try:
            ...
            if signal != 'hold':
                self.logger.info(f'📊 Signal: {signal.upper()} (confidence: {confidence:.2f}, energy: {energy:.4f}, entropy: {entropy:.4f}, time: {comp_time:.3f}s)')
        except Exception as e:
    ...
    def _log_performance_stats(self) -> None:
        ...
        try:
            ...
            if stats:
                ...
                self.logger.info(f'📈 Performance: Signals: {buy_count}B/{sell_count}S/{hold_count}H = Avg Confidence: {avg_confidence:.2f}, Avg Time: {avg_time:.3f}s')
        except Exception as e:
