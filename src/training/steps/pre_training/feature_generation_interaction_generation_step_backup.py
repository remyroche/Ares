...
- Cross-timeframe feature generation with 3x, 9x, 27x lookback ratios
...
- For each variant feature (base, volnorm, vwap, trend_adj), generates 3 additional timeframe versions
- Creates ratio-based interactions: feature_base / feature_3x, feature_base / feature_9x, feature_base / feature_27x
- Uses safe division with math validation and causality enforcement (.shift(1))
- Effectively multiplies feature count by ~4x after Phase 1 (1 base + 3 ratios per variant)
"""
...
class FeatureGenerationInteractionGenerationStep(BaseStep):
    ...
    async def _generate_cross_timeframe_features(
        ...
        """
        tprint_info("🔄 Generating cross-timeframe features with 3x, 9x, 27x lookback ratios")
    ...
    def _extract_variant_type(self, variant_col: str) -> str:
        ...
        if variant_col.endswith('_volnorm'):
            ...
            # Calculate VWAP
            typical_price = (ohlcv_data['high'] + ohlcv_data['low'] + ohlcv_data['close']) / 3
            vwap = (typical_price * ohlcv_data['volume']).rolling(window=20).sum() / ohlcv_data['volume'].rolling(window=20).sum()
    ...
    def _generate_extended_timeframe_feature(
        ...
        try:
            ...
            if variant_type == 'base':
                ...
                extended_feature = self._apply_volatility_normalization(extended_base_feature, ohlcv_data)
            elif variant_type == 'vwap':
                # Apply VWAP weighting
                extended_feature = self._apply_vwap_weighting(extended_base_feature, ohlcv_data)
            elif variant_type == 'trend_adj':
                # Apply trend adjustment
                extended_feature = self._apply_trend_adjustment(extended_base_feature, ohlcv_data)
            else:
                extended_feature = extended_base_feature
...
"""
    ...
    def _extract_variant_type(self, variant_col: str) -> str:
        ...
        if variant_col.endswith('_volnorm'):
            ...
            return 'base'_info.category.value if hasattr(feature_info.category, 'value') else str(feature_info.category)
                    except Exception:
                        # If we can't get feature info, fall back to name-based inference
                        pass
            except Exception as e:
                tprint_warning(f"Error getting category from feature bank: {e}")
    ...
    def _extract_variant_type(self, variant_col: str) -> str:
        ...
        if variant_col.endswith('_volnorm'):
            ...
            return 'base's, targets
                
            if len(targets) == 0:
                print("🔍 DEBUG: Targets DataFrame is empty!")
                return feature
    ...
    def _extract_variant_type(self, variant_col: str) -> str:
        ...
        if variant_col.endswith('_volnorm'):
            ...
            if len(features) > max_samples:
                print(f"🔍 DEBUG: Sampling {max_samples} from {len(features)} features")
    ...
    def _extract_variant_type(self, variant_col: str) -> str:
        ...
        if variant_col.endswith('_volnorm'):
            ...
            print(f"🔍 DEBUG: Exception in _get_consistent_sample: {e}")
            return feature
