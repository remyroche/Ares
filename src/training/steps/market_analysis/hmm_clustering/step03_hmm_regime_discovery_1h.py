...
class HMMRegimeDiscovery1H:
    ...
    async def execute(self, symbol: str, exchange: str, trading_timeframe: str, data_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if trading_timeframe != self.regime_timeframe:
                regime_labels_trading = await self._map_regimes_to_timeframe(data_1h, regime_labels_1h, symbol, exchange, trading_timeframe, data_dir)
            else:
                regime_labels_trading = regime_labels_1h
            transition_matrix = self._calculate_transition_matrix(regime_labels_1h)
            results = {'regime_model': hmm_model, 'regime_labels_1h': regime_labels_1h, 'regime_labels_trading': regime_labels_trading, 'regime_timeframe': self.regime_timeframe, 'trading_timeframe': trading_timeframe, 'regime_statistics': regime_stats, 'transition_matrix': transition_matrix, 'timestamp': datetime.now().isoformat()}
    ...
    async def _load_or_create_1h_data(self, symbol: str, exchange: str, data_dir: str) -> pd.DataFrame:
    ...
    async def _prepare_hmm_features(self, data: pd.DataFrame) -> np.ndarray:
        ...
        features_dict['sma_ratio_168'] = data['close'] / data['close'].rolling(168).mean()
...
from src.core.decorators.errors import handles_errors
    ...
    def _calculate_duration_stats(self, data: pd.DataFrame, regime: int) -> Dict[str, float]:
        ...
        if durations:
            return {'avg_duration_hours': np.mean(durations), 'min_duration_hours': np.min(durations), 'max_duration_hours': np.max(durations), 'std_duration_hours': np.std(durations)}
        else:
            return {}

    async def _map_regimes_to_timeframe(self, data_1h: pd.DataFrame, regime_labels_1h: np.ndarray, symbol: str, exchange: str, target_timeframe: str, data_dir: str) -> np.ndarray:
        """Map 1h regime labels to target trading timeframe."""
        self.logger.info(f'Mapping regimes from 1h to {target_timeframe}')
        regime_df = pd.DataFrame({'timestamp': data_1h['timestamp'], 'regime': regime_labels_1h[:len(data_1h)]})
    ...
    async def _save_results(self, results: Dict[str, Any], symbol: str, exchange: str, data_dir: str) -> None:
        ...
        np.save(output_dir / f'{exchange}_{symbol}_regime_labels_1h.npy', results['regime_labels_1h'])
        np.save(output_dir / f"{exchange}_{symbol}_regime_labels_{results['trading_timeframe']}.npy", results['regime_labels_trading'])
        save_dict = {'regime_statistics': results['regime_statistics'], 'transition_matrix': results['transition_matrix'].tolist(), 'regime_names': self.regime_names, 'hmm_params': self.hmm_params, 'timestamp': results['timestamp']}
        safe_json_dump(save_dict, output_dir / f'{exchange}_{symbol}_regime_analysis.json')
        self.logger.info(f'Saved regime analysis results to {output_dir}')

async def run_enhanced_regime_discovery(symbol: str, exchange: str, timeframe: str, data_dir: str, config: Dict[str, Any]) -> Dict[str, Any]:
...
if __name__ == '__main__':
    ...
    async def main() -> None:
        config = {'symbol': 'BTCUSDT', 'exchange': 'binance', 'timeframe': '5m', 'data_dir': 'data_cache'}
