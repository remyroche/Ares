...
def load_ev_decomposition_bundle(bundle_path, allow_unknown_version: bool = False, verbose: bool = True) -> EVDecompositionBundle:
    if verbose:
...
def _ensure_X(X_batch, feature_cols, verbose: bool = True):
    if verbose:
...
def predict_ev_components(bundle: EVDecompositionBundle, X_batch, regime_labels=None, row_ids=None, verbose: bool = True) -> dict:
    if verbose:
        tprint("Predicting with EV decomposition bundle models")
    X_df = _ensure_X(X_batch, bundle.feature_cols, verbose=verbose)
...
def compute_ev_risk(preds: dict, costs: float, cfg: PositionSizerConfig, verbose: bool = True):
    if verbose:
...
def gate_and_size(EV, Risk, cfg: PositionSizerConfig, optional_rank_context=None, alpha_score=None, verbose: bool = True):
    if verbose:
