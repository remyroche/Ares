...
CONFIG = {
    ...
    "log_progress_interval": 1000,  # Log progress every N rows
    "enable_timing": True,
...
class TimingTracker:
    ...
    def checkpoint(self, checkpoint_name: str):
        """Record a checkpoint within the current phase."""
        if self.current_phase and self.current_phase in self.checkpoints:
            if "checkpoints" not in self.checkpoints[self.current_phase]:
                self.checkpoints[self.current_phase]["checkpoints"] = {}
            self.checkpoints[self.current_phase]["checkpoints"][checkpoint_name] = time.time()
    ...
    def print_summary(self):
        ...
        for phase_name, phase_data in self.checkpoints.items():
            if "end" in phase_data:
                ...
                # Print checkpoints if any
                if "checkpoints" in phase_data:
                    for checkpoint_name, checkpoint_time in phase_data["checkpoints"].items():
                        checkpoint_duration = checkpoint_time - phase_data["start"]
                        print(f"    └─ {checkpoint_name}: {checkpoint_duration:.2f}s")
        print("="*60)
...
class MemoryTracker:
    ...
    def log_memory_usage(context: str = ""):
        ...
        if CONFIG.get("log_memory_usage", True):
            memory = MemoryTracker.get_memory_usage()
            print(f"💾 [MEMORY] {context}: RSS={memory['rss_mb']:.1f}MB, VMS={memory['vms_mb']:.1f}MB, {memory['percent']:.1f}%")
...
class ProgressTracker:
    def __init__(self, total_items: int, description: str = "Processing"):
        ...
        self.log_interval = CONFIG.get("log_progress_interval", 1000)
    ...
    def update(self, increment: int = 1):
        ...
        if (self.processed_items % self.log_interval == 0 or 
            current_time - self.last_log_time >= 30):  # Or every 30 seconds
            ...
            if self.processed_items > 0:
                ...
                print(f"📊 [PROGRESS] {self.description}: {self.processed_items}/{self.total_items} ({percentage:.1f}%) - {items_per_second:.1f} items/s - ETA: {eta_str}")
    ...
    def complete(self):
        ...
        print(f"✅ [PROGRESS] {self.description} completed: {self.processed_items} items in {elapsed_time:.2f}s ({items_per_second:.1f} items/s)")
...
class ParquetDatasetManager:
    ...
    def compact_dataset(
        self, base_dir: str, output_dir: str | None = None, compression: str = "snappy", min_rows_per_group: int | None = None, max_rows_per_file: int | None = None
    ) -> str | None:
...
class UnifiedDataConverter:
    ...
    async def execute(
        self, symbol: str, exchange: str, timeframe: str = "1m", data_dir: str = "data_cache", force_rerun: bool = False
    ) -> bool:
    ...
    async def _process_incremental_updates(
        self = symbol: str, exchange: str, timeframe: str, ) -> bool:,
    ...
    async def _backup_existing_data(
        self = symbol: str, exchange: str, timeframe: str, ) -> None:,
    ...
    async def _process_data_incrementally(
        self, klines_data: pd.DataFrame, symbol: str, exchange: str, timeframe: str, start_date: date | None = None
    ) -> bool:
        ...
        # Filter klines data for current date
                    date_mask = ((klines_data["year"] == current_date.year)
    ...
    async def _load_aggtrades_for_date(
        self = symbol: str, exchange: str, target_date: datetime.date, ) -> pd.DataFrame | None:,
    ...
    async def _merge_daily_data(
        self, daily_klines: pd.DataFrame, daily_aggtrades: pd.DataFrame | None, daily_futures: pd.DataFrame | None, symbol: str, exchange: str, timeframe: str, ) -> pd.DataFrame | None:
    ...
    async def _merge_daily_aggtrades(
        self = unified: pd.DataFrame, aggtrades_data: pd.DataFrame, ) -> pd.DataFrame:,
        ...
        # Group by kline timestamp and aggregate
                agg_stats = (aggtrades_data.groupby("kline_timestamp")
                    .agg(
                        {
                            "quantity": ["sum", "count"],
                            "price": ["mean", "min", "max"],
    ...
    async def _merge_daily_futures(
        self = unified: pd.DataFrame, futures_data: pd.DataFrame, ) -> pd.DataFrame:,
    ...
    async def _write_daily_partition(
        self, daily_data: pd.DataFrame, symbol: str, exchange: str, timeframe: str, target_date: datetime.date, base_dir: str, ) -> bool:
    ...
    async def _validate_unified_dataset(
        ...
        try: pdm = ParquetDatasetManager(logger=self.logger)
        except Exception as e:
        ...
        # Scan the dataset
        try: sample_data = pdm.scan_dataset(
                    base_dir=base_dir,
    ...
    async def _verify_unified_data_quality(
        ...
        # Check data quality
                        aggtrades_coverage = ((df["trade_volume"] > 0).sum() / len(df) * 100
        if len(df) > 0
                            else 0
                        )
                        futures_coverage = (df["funding_rate"].notna().sum() / len(df) * 100
        if len(df) > 0
                            else 0
                        )
    ...
    def get_unified_config_path(
        self = symbol: str, exchange: str, timeframe: str, ) -> str:,
    ...
    async def _load_klines_data(
        self = symbol: str, exchange: str, timeframe: str, ) -> pd.DataFrame | None:,
    ...
    def _convert_aggtrades_to_klines(
        self = aggtrades_df: pd.DataFrame, timeframe: str, ) -> pd.DataFrame | None:,
        ...
        # Resample and aggregate
            resampled = (aggtrades_df.resample(rule)
    ...
    async def _fill_missing_values(self, unified: pd.DataFrame) -> pd.DataFrame:
        ...
        # No trade data exists, fill with 0
                            ...
                            else: # Trade data exists = only fill the truly missing values
                                unified[col] = unified[col].fillna(0)
                                filled_columns.append(
                                    f"{col} ({missing_count} values - preserving trade data)",
                                )
                        else: # Non-trade data columns = fill normally
                            unified[col] = unified[col].fillna(0)
...
async def run_step(symbol: str, exchange: str, timeframe: str = "1m", data_dir: str = "data_cache", force_rerun: bool = False
    ...
    try:
        ...
        # Phase 2: Data Conversion Execution
        ...
        if success:
            ...
            if validate_step1_5_file:
                print("🔍 Running comprehensive file format validation...")
                validation_success = await _run_comprehensive_validation(symbol, exchange, timeframe, data_dir)
                
                if validation_success:
                    print("✅ Comprehensive file format validation passed")
                else:
                    print("⚠️ Comprehensive file format validation found issues")
            else:
                print("⚠️ Comprehensive file validation not available, skipping validation")
            
            timing_tracker.end_phase("Success_Processing")
        else:
            print("❌ [PHASE 3] Data conversion failed - skipping success processing")
        
        # Final phase: Cleanup and Summary
        ...
        print("="*80)
        if success:
            print("🎉 STEP 1.5: UNIFIED DATA CONVERTER - COMPLETED SUCCESSFULLY")
        else:
            print("💥 STEP 1.5: UNIFIED DATA CONVERTER - FAILED")
        print("="*80 + "\n")
        ...
        # Log timing information even on failure
        timing_tracker.end_phase("Step1_5_Total_Execution")
        timing_tracker.print_summary()
