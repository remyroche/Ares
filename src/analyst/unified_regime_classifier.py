...
class UnifiedRegimeClassifier:
    ...
    def __init__(self, config: dict[str, Any], exchange: str='UNKNOWN', symbol: str='UNKNOWN') -> None:
        ...
        self.volatility_percentile_threshold = self.config.get('volatility_percentile_threshold', 0.8)
        self.bb_width_volatility_threshold = self.config.get('bb_width_volatility_threshold', 0.045)
        self.logger.info({'msg': 'UnifiedRegimeClassifier configuration', 'n_states_configured': configured_states, 'n_states_enforced_min': self.n_states, 'min_data_points_default': self.config.get('min_data_points', 1000), 'thresholds': {'adx_sideways_threshold': self.adx_sideways_threshold, 'volatility_threshold': self.volatility_threshold, 'atr_normalized_threshold': self.atr_normalized_threshold, 'volatility_percentile_threshold': self.volatility_percentile_threshold, 'bb_width_volatility_threshold': self.bb_width_volatility_threshold}})
        ...
        self.sr_config = {'sr_breakout_predictor': {'enable_sr_breakout_tactics': True, 'sr_proximity_threshold': 0.02, 'breakout_confidence_threshold': 0.6, 'sr_detection_method': 'fractal', 'min_sr_strength': 0.3, 'max_sr_levels': 10, 'sr_lookback_periods': 100, 'volume_weight': 0.7, 'price_weight': 0.3, 'atr_multiplier': 1.5, 'breakout_confirmation_periods': 3, 'false_breakout_filter': True, 'use_optimized_params': True, 'strength_calculation': {'enable_enhanced_strength': True, 'touch_count_lookback': 50, 'bounce_rate_threshold': 0.02, 'isolation_distance_threshold': 0.05, 'age_decay_factor': 0.95}, 'dbscan_clustering': {'enable_dbscan_clustering': True, 'eps': 0.01, 'min_samples': 2, 'enable_noise_filtering': True}, 'feature_calculation': {'enable_comprehensive_features': True, 'strength_score_weights': {'touch_count': 0.3, 'total_volume': 0.2, 'level_age': 0.2, 'bounce_rate': 0.2, 'isolation_score': 0.1}}}}
        self.trained = False
    ...
    def _enable_numpy_rng_unpickle_compat(logger: logging.Logger = None) -> None:
        ...
        """
        if getattr(UnifiedRegimeClassifier._enable_numpy_rng_unpickle_compat, '_patched', False):
            return
        try:
            ...
            def _normalized_numpy_bitgen_ctor(bit_generator_name: Any, state: Any = None, *args, **kwargs) -> None:
                ...
                try:
                    ...
                    if logger is not None:
                        logger.debug("URC RNG ctor: failed to normalize name '%s': %s", name_candidate, _name_exc)
    ...
    async def _calculate_features(self, klines_df: pd.DataFrame, min_data_points: int = None) -> pd.DataFrame:
        ...
        if len(klines_df) < min_data_points:
            self.logger.warning(f'Insufficient data: {len(klines_df)} < {min_data_points}. Consider reducing min_data_points or collecting more data.')
            if len(klines_df) >= 200:
        ...
        features_df['atr_normalized'] = features_df['atr'] / (features_df['close'].diff().abs() + 1e-08)
        ...
        features_df['volatility_momentum'] = features_df['volatility_20'] - features_df['volatility_20'].shift(5)
        if self.sr_predictor and self.enable_sr_integration:
            ...
            features_df = self._add_basic_sr_features(features_df)
        technical_columns = ['rsi', 'macd', 'macd_signal', 'macd_histogram', 'bb_upper', 'bb_middle', 'bb_lower', 'bb_position', 'bb_width', 'atr', 'atr_normalized', 'adx', 'volatility_regime', 'sr_proximity', 'sr_strength', 'sr_zone_width', 'sr_cluster_count', 'sr_fibonacci_proximity', 'sr_elliott_proximity', 'sr_order_flow_imbalance', 'sr_enhanced_strength', 'sr_touch_count', 'sr_bounce_rate', 'sr_isolation_score']
        ...
        for col in price_features:
            if col in features_df.columns:
                features_df[col] = features_df[col].fillna(0)
        ratio_features = ['high_low_diff_ratio', 'close_open_diff_ratio', 'volume_ratio']
        for col in ratio_features:
            if col in features_df.columns:
                features_df[col] = features_df[col].fillna(1)
        vol_features = ['volatility_20', 'volatility_10', 'volatility_5', 'ewma_volatility_20']
        ...
        self.logger.info(f'✅ Calculated comprehensive features for {len(features_df)} periods (dropped {dropped_rows} rows due to NaN)')
        return features_df
    ...
    def _calculate_volatility_regime(self, features_df: pd.DataFrame) -> pd.Series:
        ...
        bb_width_high = features_df['bb_width'] > float(getattr(self, 'bb_width_volatility_threshold', 0.04)) if 'bb_width' in features_df.columns else False
            ...
            for i in range(50, len(features_df)):
                try:
                    ...
                    if sr_context:
                        ...
                        max_enhanced_strength = max(enhanced_support.get('max_strength', 0.5), enhanced_resistance.get('max_strength', 0.5))
    ...
    def _interpret_hmm_states(self, features_df: pd.DataFrame, state_sequence: np.ndarray) -> dict:
        ...
        for state in range(self.n_states):
            ...
            if is_sideways and abs(mean_return) < 0.0003:
                regime = 'SIDEWAYS'
            elif mean_return > 0.0005 and mean_adx > adx_sideways_threshold:
                regime = 'BULL'
            elif mean_return < -0.0005 and mean_adx > adx_sideways_threshold:
                regime = 'BEAR'
            elif is_sideways:
                if abs(mean_return) > 0.0002:
                ...
                regime = 'BEAR'
            state_analysis[state] = {'regime': regime, 'mean_return': mean_return, 'mean_volatility': mean_volatility, 'mean_adx': mean_adx, 'mean_atr_norm': mean_atr_norm, 'count': len(state_data)}
            self.logger.info(f'State {state}: {regime} (mean_return={mean_return:.4f}, mean_vol={mean_volatility:.4f}, mean_adx={mean_adx:.2f}, is_sideways={is_sideways})')
        state_to_regime_map = {state: data['regime'] for state, data in state_analysis.items() if isinstance(state, int)}
        ...
        for state, data in state_analysis.items():
            if state == 'state_to_regime_map':
                continue
            mapped_counts[data['regime']] = mapped_counts.get(data['regime'], 0) + int(data.get('count', 0))
        self.logger.info({'msg': 'HMM state mapping summary', 'n_states': self.n_states, 'unique_mapped_regimes': sorted(list({r for r in state_to_regime_map.values()})), 'mapped_counts': mapped_counts, 'thresholds': {'adx_sideways_threshold': adx_sideways_threshold, 'volatility_threshold': volatility_threshold, 'atr_normalized_threshold': atr_norm_threshold}})
    ...
    async def _calculate_enhanced_sr_levels(self, df_window: pd.DataFrame) -> dict:
        ...
        try:
            ...
            if len(df_window) < 5:
                return {'s1': 0, 's2': 0, 'r1': 0, 'r2': 0, 'pivot': 0, 'enhanced_strengths': {}, 'clustering_result': {}, 'fibonacci_levels': {}, 'elliott_wave_levels': {}, 'order_flow_analysis': {}}
            ...
            enhanced_strengths = {'support_strength': sr_context.get('support_strength', 0.5), 'resistance_strength': sr_context.get('resistance_strength', 0.5), 'enhanced_strength_support': sr_context.get('enhanced_strength_support', {}), 'enhanced_strength_resistance': sr_context.get('enhanced_strength_resistance', {})}
            return {'s1': s1, 's2': s2, 'r1': r1, 'r2': r2, 'pivot': pivot, 'enhanced_strengths': enhanced_strengths, 'clustering_result': sr_context.get('clustering_result', {}), 'fibonacci_levels': sr_context.get('fibonacci_levels', {}), 'elliott_wave_levels': sr_context.get('elliott_wave_levels', {}), 'order_flow_analysis': sr_context.get('order_flow_analysis', {}), 'support_levels': support_levels, 'resistance_levels': resistance_levels, 'sr_zone_width': sr_context.get('sr_zone_width', 0.0)}
        except Exception as e:
    ...
    async def _calculate_basic_pivots(self, df_window: pd.DataFrame) -> dict:
        ...
        try:
            ...
            s1 = 2 * pivot - high
            s2 = pivot - (high - low)
            return {'s1': s1, 's2': s2, 'r1': r1, 'r2': r2, 'pivot': pivot, 'enhanced_strengths': {'support_strength': 0.5, 'resistance_strength': 0.5}, 'clustering_result': {}, 'fibonacci_levels': {}, 'elliott_wave_levels': {}, 'order_flow_analysis': {}, 'support_levels': [], 'resistance_levels': [], 'sr_zone_width': 0.0}
        except Exception as e:
    ...
    async def _analyze_enhanced_volume_levels(self, df_window: pd.DataFrame) -> dict | None:
        ...
        try:
            ...
            analyzed_levels = {}
            if poc_level:
                analyzed_levels['poc'] = {'price': poc_level.get('price', current_price), 'volume': poc_level.get('volume', 0.0), 'age': poc_level.get('age', 0), 'touches': poc_level.get('touches', 0), 'strength': poc_level.get('strength', 0.5), 'volume_strength': poc_level.get('volume_strength', 0.5), 'touch_strength': poc_level.get('touch_strength', 0.5), 'age_strength': poc_level.get('age_strength', 0.5), 'enhanced_metrics': {'value_area_high': value_area.get('high', current_price * 1.02), 'value_area_low': value_area.get('low', current_price * 0.98), 'value_area_volume': value_area.get('volume', 0.0)}}
            for i, hvn in enumerate(hvns[:2]):
                level_name = 'hvn_primary' if i == 0 else 'hvn_secondary'
                analyzed_levels[level_name] = {'price': hvn.get('price', current_price), 'volume': hvn.get('volume', 0.0), 'age': hvn.get('age', 0), 'touches': hvn.get('touches', 0), 'strength': hvn.get('strength', 0.5), 'volume_strength': hvn.get('volume_strength', 0.5), 'touch_strength': hvn.get('touch_strength', 0.5), 'age_strength': hvn.get('age_strength', 0.5), 'enhanced_metrics': {'cluster_id': hvn.get('cluster_id', -1), 'isolation_score': hvn.get('isolation_score', 0.5), 'bounce_rate': hvn.get('bounce_rate', 0.0)}}
            imbalances = order_flow_analysis.get('imbalances', [])
            if imbalances:
                analyzed_levels['order_imbalances'] = {'count': len(imbalances), 'total_volume': sum((imb.get('volume', 0.0) for imb in imbalances)), 'average_size': np.mean([imb.get('size', 0.0) for imb in imbalances]) if imbalances else 0.0}
    ...
    def _analyze_basic_volume_levels(self, df_window: pd.DataFrame) -> dict | None:
        ...
        for i, (level_bin, level_volume) in enumerate(top_hvns.items()):
            ...
            touch_strength = min(touches / 10.0, 1.0)
            age_strength = min(age / len(df_window), 1.0)
            overall_strength = volume_strength * 0.5 + touch_strength * 0.3 + age_strength * 0.2
            level_name = 'poc' if i == 0 else 'hvn_secondary'
            analyzed_levels[level_name] = {'price': level_price, 'volume': level_volume, 'age': age, 'touches': touches, 'strength': overall_strength, 'volume_strength': volume_strength, 'touch_strength': touch_strength, 'age_strength': age_strength}
    ...
    async def _classify_enhanced_location(self, features_df: pd.DataFrame) -> list[str]:
        ...
        """
        self.logger.info('Classifying location with enhanced S/R analysis using SRBreakoutPredictor...')
        long_term_period = self.config.get('long_term_hvn_period', 720)
        short_term_period = self.config.get('short_term_pivot_period', 24)
        tolerance = self.config.get('level_tolerance', 0.01)
        min_level_touches = self.config.get('min_level_touches', 1)
        min_strength_threshold = self.config.get('min_strength_threshold', 0.3)
        if len(features_df) < long_term_period:
            self.logger.warning(f'Insufficient data for enhanced location classification. Need at least {long_term_period} rows, but only have {len(features_df)}. Returning all OPEN_RANGE labels.')
        ...
        for i in range(start_index, len(features_df)):
            ...
            for level in support_levels:
                if isinstance(level, dict):
                    ...
                    level_strength = 0.5
                    level_touches = 1
                if level_touches >= min_level_touches and level_strength >= min_strength_threshold and (abs(current_price - level_price) / current_price <= tolerance):
            ...
            if volume_levels:
                for level_name, level_data in volume_levels.items():
                    if level_name in ['poc', 'hvn_primary', 'hvn_secondary']:
                        if level_data.get('touches', 0) >= min_level_touches and level_data.get('strength', 0.5) >= min_strength_threshold and (abs(current_price - level_data['price']) / current_price <= tolerance):
        ...
        self.logger.info(f'Finished enhanced location classification. Found: {pd.Series(final_locations).value_counts().to_dict()}')
    ...
    def _classify_basic_location(self, features_df: pd.DataFrame) -> list[str]:
        ...
        short_term_pivot_period = self.config.get('short_term_pivot_period', 24)
        tolerance = self.config.get('level_tolerance', 0.01)
        min_level_touches = self.config.get('min_level_touches', 1)
        if len(features_df) < long_term_hvn_period:
            self.logger.warning(f'Insufficient data for location classification. Need at least {long_term_hvn_period} rows, but only have {len(features_df)}. Returning all OPEN_RANGE labels.')
            return ['OPEN_RANGE'] * len(features_df)
        locations = []
        ...
        for i in range(start_index, len(features_df)):
            ...
            for p_sup in pivot_supports:
                if abs(current_price_diff - p_sup) / (abs(current_price_diff) + 1e-08) <= tolerance:
                    loc_pivot = 'PIVOT_S'
                    break
            if not loc_pivot:
                for p_res in pivot_resistances:
                    if abs(current_price_diff - p_res) / (abs(current_price_diff) + 1e-08) <= tolerance:
                        loc_pivot = 'PIVOT_R'
                        break
            if volume_levels:
                for level_data in volume_levels.values():
                    if level_data['touches'] < min_level_touches:
                        continue
                    if abs(current_price_diff - level_data['price']) / (abs(current_price_diff) + 1e-08) <= tolerance:
                        hvn_type = 'SUPPORT' if current_price_diff > level_data['price'] else 'RESISTANCE'
        ...
        padding = ['OPEN_RANGE'] * start_index
        final_locations = padding + locations
        self.logger.info(f'Finished basic location classification. Found: {pd.Series(final_locations).value_counts().to_dict()}')
        return final_locations
    ...
    async def train_location_classifier(self, historical_klines: pd.DataFrame) -> bool:
        ...
        try:
            ...
            long_term_hvn_period = self.config.get('long_term_hvn_period', 720)
            if len(features_df) < long_term_hvn_period:
                self.logger.warning(f'Insufficient data for location classification. Need at least {long_term_hvn_period} rows, but only have {len(features_df)}. Skipping location classifier training.')
                return True
            location_labels = self._classify_location(features_df)
            if len(location_labels) != len(features_df):
                self.logger.error(f'Location labels length ({len(location_labels)}) does not match features length ({len(features_df)}). Skipping location classifier training.')
                return True
    ...
    async def train_basic_ensemble(self, historical_klines: pd.DataFrame) -> bool:
        ...
        try:
            ...
            state_sequence = self.hmm_model.predict(hmm_features_scaled)
            regime_labels = [self.state_to_regime_map.get(state, 'SIDEWAYS') for state in state_sequence]
    ...
    async def predict_regime(self, current_klines: pd.DataFrame) -> tuple[str, float, dict]:
        """
        ...
        try:
            ...
            additional_info = {'regime_confidence': regime_confidence, 'features_used': list(features_df.columns), 'prediction_time': datetime.now().isoformat()}
    ...
    async def predict_regime_and_location(self, current_klines: pd.DataFrame) -> tuple[str, str, float, dict]:
        """
        ...
        try:
            ...
            additional_info = {'regime_confidence': regime_confidence, 'location_confidence': location_confidence, 'features_used': list(features_df.columns), 'prediction_time': datetime.now().isoformat()}
    ...
    def load_models(self) -> bool:
        ...
        try:
            self.logger.info({'msg': 'UnifiedRegimeClassifier model directories', 'model_dir': self.model_dir, 'hmm_model_path': self.hmm_model_path, 'ensemble_model_path': self.ensemble_model_path, 'location_model_path': self.location_model_path, 'hierarchical_model_dir': getattr(self, '_hierarchical_model_dir', None)})
            loaded_any = False
            hmm_candidates = [self.hmm_model_path, os.path.join(getattr(self, '_hierarchical_model_dir', self.model_dir), 'unified_hmm_model.joblib')]
            ensemble_candidates = [self.ensemble_model_path, os.path.join(getattr(self, '_hierarchical_model_dir', self.model_dir), 'unified_ensemble_model.joblib')]
            location_candidates = [self.location_model_path, os.path.join(getattr(self, '_hierarchical_model_dir', self.model_dir), 'unified_location_model.joblib')]
            ...
            if location_path is not None:
                ...
                loaded_any = True
            encoder_candidates = [self.ensemble_model_path.replace('.joblib', '_encoder.joblib'), os.path.join(getattr(self, '_hierarchical_model_dir', self.model_dir), 'unified_ensemble_model_encoder.joblib')]
            enc_path = _first_existing(encoder_candidates)
            if enc_path is not None:
                self.basic_label_encoder = joblib.load(enc_path)
            location_encoder_candidates = [self.location_model_path.replace('.joblib', '_encoder.joblib'), os.path.join(getattr(self, '_hierarchical_model_dir', self.model_dir), 'unified_location_model_encoder.joblib')]
    ...
    async def classify_regimes(self, historical_klines: pd.DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            regime_features = features_df[['log_returns', 'volatility_20', 'volume_ratio', 'rsi', 'macd', 'macd_signal', 'macd_histogram', 'bb_position', 'bb_width', 'atr', 'adx', 'volatility_regime', 'volatility_acceleration']].fillna(0)
            regimes = []
            confidence_scores = []
            if self.basic_ensemble and self.basic_label_encoder:
                ...
                self.logger.info({'msg': 'Ensemble regime prediction summary', 'unique_regimes': unique_regimes, 'counts': counts, 'expected_min': self.n_states, 'total_records': int(len(regime_features)), 'thresholds': {'adx_sideways_threshold': getattr(self, 'adx_sideways_threshold', None), 'volatility_threshold': getattr(self, 'volatility_threshold', None), 'atr_normalized_threshold': getattr(self, 'atr_normalized_threshold', None), 'volatility_percentile_threshold': getattr(self, 'volatility_percentile_threshold', None)}})
                if len(unique_regimes) < self.n_states:
                    self.logger.warning(warning(f'Fewer regimes predicted ({len(unique_regimes)}) than expected ({self.n_states}). Consider increasing min_data_points or enhancing volatility features.'))
                ...
                confidence_scores = [0.8] * len(regimes)
                unique_regimes = list(sorted(set(regimes)))
                counts = {r: int((np.array(regimes) == r).sum()) for r in unique_regimes}
                self.logger.info({'msg': 'HMM regime prediction summary', 'unique_regimes': unique_regimes, 'counts': counts, 'expected_min': self.n_states, 'total_records': int(len(regime_features)), 'thresholds': {'adx_sideways_threshold': getattr(self, 'adx_sideways_threshold', None), 'volatility_threshold': getattr(self, 'volatility_threshold', None), 'atr_normalized_threshold': getattr(self, 'atr_normalized_threshold', None), 'volatility_percentile_threshold': getattr(self, 'volatility_percentile_threshold', None)}})
                if len(unique_regimes) < self.n_states:
                    self.logger.warning(warning(f'Fewer regimes predicted ({len(unique_regimes)}) than expected ({self.n_states}). Consider increasing min_data_points or enhancing volatility features.'))
            else:
            ...
            return {'regimes': regimes, 'confidence_scores': confidence_scores, 'locations': location_labels, 'total_records': len(features_df), 'regime_distribution': regime_distribution, 'location_distribution': dict(pd.Series(location_labels).value_counts())}
        except Exception as e:
    ...
    def get_system_status(self) -> dict[str, Any]:
        ...
        return {'trained': self.trained, 'last_training_time': self.last_training_time.isoformat() if self.last_training_time else None, 'hmm_model_loaded': self.hmm_model is not None, 'basic_ensemble_loaded': self.basic_ensemble is not None, 'location_classifier_loaded': self.location_classifier is not None, 'n_states': self.n_states, 'target_timeframe': self.target_timeframe, 'state_to_regime_map': self.state_to_regime_map}
