#!/usr/bin/env python3
"""
Simple Import Analyzer - Checks for dead code in src/utils/ directory.
...
class SimpleImportAnalyzer:
    """Simple analyzer to find dead code by checking import usage."""
    ...
    def _is_module_imported(self, module_name: str, imports: Set[str]) -> bool:
        ...
        # Check if any import starts with this module name
        for imp in imports:
    ...
    def print_report(self, results: Dict[str, Any]) -> None:
        ...
        print("\n" + "="*80)
        print("DEAD CODE ANALYSIS REPORT - src/utils/")
        print("="*80)
        ...
        for file_path in sorted(import_status.keys()):
            ...
            try:
                ...
                print(f"{status_text:10} | {rel_path} ({import_count} imports)")
                
                if imported_by:
                    print(f"             └─ Imported by:")
                    for importer in sorted(imported_by)[:5]:  # Show first 5
                        try:
                ...
                status_text = "IMPORTED" if is_imported else "DEAD CODE"
                print(f"{status_text:10} | {file_path} ({import_count} imports)")
        ...
        if most_imported:
            ...
            for file_path, status in most_imported[:5]:
                if status["import_count"] > 0:
                    try:
                        rel_path = Path(file_path).relative_to(Path.cwd())
                        print(f"  • {rel_path} ({status['import_count']} imports)")
                    except ValueError:
