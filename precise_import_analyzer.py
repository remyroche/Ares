#!/usr/bin/env python3
"""
Precise Import Analyzer - More accurate dead code detection.
Fixes false positives from directory/subdirectory mentions.
"""
...
class PreciseImportAnalyzer:
    """More precise analyzer to find dead code by checking actual module imports."""
        ...
        for utils_file in utils_files:
            ...
            for other_file, other_imports in all_imports.items():
                if other_file != file_path_str:
                    if self._is_module_imported_precise(module_name, other_imports):
    ...
    def _is_module_imported_precise(self, module_name: str, imports: Set[str]) -> bool:
        """
        Check if a module is imported with more precise logic.
        Only counts actual module imports, not directory references.
        """
        # Direct exact match
        ...
        # e.g., if module is "src.utils.common_operations" and we see "src.utils.common_operations.some_function"
        for imp in imports:
            if imp.startswith(module_name + '.'):
                return True
        
        # Check if this module is imported as a submodule
        # e.g., if module is "src.utils.common_operations" and we see "from src.utils import common_operations"
    ...
    def print_detailed_report(self, results: Dict[str, Any]) -> None:
        ...
        print("\n" + "="*80)
        print("PRECISE DEAD CODE ANALYSIS REPORT - src/utils/")
        print("="*80)
        ...
        print("\n" + "-"*80)
        print("DETAILED RESULTS WITH IMPORT EXAMPLES:")
        print("-"*80)
...
def main():
    ...
    analyzer = PreciseImportAnalyzer()
