...
class SurrogateOptimizationMonitor:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.alert_thresholds = {'surrogate_accuracy_threshold': 0.7, 'convergence_stall_threshold': 10, 'memory_usage_threshold': 0.8, 'training_time_threshold': 60.0, 'uncertainty_threshold': 0.5}
    ...
    def _calculate_surrogate_accuracy(self, metrics: OptimizationMetrics) -> float:
        ...
        if metrics.actual_score is None:
            return 0.0
        relative_error = abs(metrics.surrogate_score - metrics.actual_score) / (abs(metrics.actual_score) + 1e-08)
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        return {'total_trials': self.total_trials, 'expensive_evaluations': self.expensive_evaluations, 'surrogate_evaluations': self.surrogate_evaluations, 'expensive_evaluation_ratio': expensive_ratio, 'total_time': total_time, 'avg_surrogate_accuracy': avg_accuracy, 'best_score': best_score, 'convergence_rate': convergence_rate, 'avg_uncertainty': avg_uncertainty, 'avg_training_time': avg_training_time, 'avg_prediction_time': avg_prediction_time, 'alerts_count': len(self.alerts), 'performance_efficiency': self._calculate_performance_efficiency()}
    ...
    def _calculate_performance_efficiency(self) -> float:
        ...
        time_efficiency = max(0, 1.0 - avg_training_time / 60.0)
