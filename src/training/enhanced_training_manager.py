    ...
    async def _execute_pipeline_step(
        ...
        try:
            ...
            try:
                ...
                if is_fatal:
                    self.logger.exception(f"❌ {step_display_name} validator failed: {e} - stopping pipeline")
                    return False
    ...
    async def _execute_comprehensive_pipeline(
        ...
        try:
            ...
            if not hasattr(self, "force_rerun"):
                self.force_rerun = getattr(self, "enhanced_training_config", {}).get("force_rerun", False)
            ...
            if start_step == "step1_data_collection":
                ...
                try:
                    ...
                    if isinstance(df_input, pd.DataFrame):
                        ...
                        active = MetaLabelRelevanceEvaluator.evaluate_label_complementarity(
                            df_input, label_names
                        )
                        active_path = data_root / f"{exchange}_{symbol}_active_meta_labels.json"
                        _safe_json_write(active_path, {"active_labels": active})
                except Exception:
                    pass
                ...
                if not should_run_step11:
                    ...
                    with self._timed_step("Step 11: Final Parameters Optimization", step_times):
                        ...
                        if step11_validation and step11_validation.get("validation_passed", False):
                            ...
                            self.logger.error("❌ Step 11 validation failed - stopping pipeline")
                            return False
                ...
                if not should_run_step12:
                    ...
                    with self._timed_step("Step 12: Walk Forward Validation", step_times):
                        ...
                        if step12_validation and step12_validation.get("validation_passed", False):
                            ...
                            self.logger.error("❌ Step 12 validation failed - stopping pipeline")
                            return False
                ...
                if not should_run_step13:
                    ...
                    with self._timed_step("Step 13: Monte Carlo Validation", step_times):
                        ...
                        if step13_validation and step13_validation.get("validation_passed", False):
                            ...
                            self.logger.error("❌ Step 13 validation failed - stopping pipeline")
                            return False
                ...
                if not should_run_step14:
                    ...
                    with self._timed_step("Step 14: A/B Testing", step_times):
                        ...
                        if step14_validation and step14_validation.get("validation_passed", False):
                            ...
                            self.logger.error("❌ Step 14 validation failed - stopping pipeline")
                            return False
