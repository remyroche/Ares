...
from src.utils.parallel_processing_optimizer import parallel_feature_engineering, optimize_for_m1_mac
...
class VectorizedVolatilityRegimeModel:
    ...
    async def model_volatility_vectorized(
        ...
        try:
            ...
            self.logger.error(f"❌ Error in volatility modeling: {e}")
            return {}
...
class VectorizedCandlestickPatternAnalyzer:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"❌ Exception traceback: {e.__traceback__}")
            return False
...
class VectorizedSRDistanceCalculator:
    ...
    async def calculate_sr_distances(
        ...
        try:
            ...
            self.logger.error(f"❌ Error in S/R distance calculation: {e}")
            return {}
...
class VectorizedAdvancedFeatureEngineering:
        ...
        try:
            ...
            if self.enable_liquidity_analysis:
                try:
                    ...
                    self.logger.error(f"🚨 Error creating liquidity analyzer: {e}")
                    self.logger.error(f"🚨 Exception type: {type(e)}")
                    self.liquidity_analyzer = None
            else:
            ...
            if self.enable_candlestick_patterns:
                try:
                    ...
                    self.logger.error(f"🚨 Error creating candlestick analyzer: {e}")
                    self.logger.error(f"🚨 Exception type: {type(e)}")
                    self.candlestick_analyzer = None
            else:
            ...
            if self.enable_sr_distance:
                try:
                    ...
                    self.logger.error(f"🚨 Error creating S/R distance calculator: {e}")
                    self.logger.error(f"🚨 Exception type: {type(e)}")
                    self.sr_distance_calculator = None
            else:
            ...
            if self.enable_wavelet_transforms:
                try:
                    ...
                    self.logger.error(f"🚨 Error creating wavelet analyzer: {e}")
                    self.logger.error(f"🚨 Exception type: {type(e)}")
                    self.wavelet_analyzer = None
            else:
    ...
    def _calculate_volume_price_impact_vectorized(
        ...
        try:
            ...
            volume_price_impact = volume_price_impact.fillna(0)

            return volume_price_impact
    ...
    def _calculate_order_flow_imbalance_vectorized(
        ...
        try:
            ...
            order_flow_imbalance = order_flow_imbalance.fillna(0)
    ...
    def _handle_nan_values_basic(self, features: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"🚨 Error in basic NaN handling: {e}")
            return features
    ...
    def _log_multi_timeframe_summary(self, features: dict[str, Any], timeframes: list[str]) -> None:
        ...
        try:
            ...
            for tf in timeframes:
                tf_features = [f for f in features.keys() if f.endswith(f"_{tf}")]
    ...
    def _generate_simple_timeframe_features(
        ...
        try:
            ...
            self.logger.error(f"Error generating simple timeframe features for {timeframe}: {e}")
            return {}
    ...
    def _handle_nan_values_comprehensive(self, features: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"🚨 Error in comprehensive NaN handling: {e}")
    ...
    def _handle_nan_values_robust(self, features: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                result = self._handle_nan_values_comprehensive(valid_features)
                return result
            ...
            try:
                result = self._handle_nan_values_basic(valid_features)
                return result
            ...
            try:
                result = self._handle_nan_values_inline(valid_features)
                return result
            ...
            self.logger.error(f"🚨 All NaN handling methods failed: {e}")
            return features
    ...
    def _handle_nan_values_inline(self, features: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"🚨 Error in inline NaN handling: {e}")
            return features
    ...
    async def engineer_features(
        ...
        try:
            ...
            if not isinstance(price_data.index, pd.DatetimeIndex):
                ...
                if 'timestamp' in price_data.columns:
                    ...
                    try:
                        ...
                        self.logger.error(f"❌ Failed to create DatetimeIndex: {e}")
                        return {}
            ...
            if price_validation.get("preprocessing_applied"):
                ...
                self.logger.info(f"✅ Price data preprocessing completed:")
            ...
            self.logger.info(f"🔍 Input data validation before feature generation:")
            ...
            if price_data is not None and not price_data.empty:
                ...
                self.logger.error(f"❌ Price data is empty or None")
            ...
            if volume_data is not None and not volume_data.empty:
                ...
                self.logger.error(f"❌ Volume data is empty or None")
            ...
            feature_categories = {}
            for feature_name in sanitized.keys():
            ...
            try:
                self.logger.info(
                    f"🧾 Vectorized feature list ({len(sanitized)}): {sorted(list(sanitized.keys()))}"
                )
            except Exception as e:
    ...
    def _calculate_bid_ask_spread_vectorized(
        ...
        try:
            ...
            if all(
                ...
                spread_pct = spread_pct.replace([np.inf, -np.inf], np.nan).fillna(0.001)
            ...
            # Fallback to volatility-based proxy if aggtrades data not available
                ...
                # Clean up infinite and NaN values
                spread_proxy = spread_proxy.replace([np.inf, -np.inf], np.nan).fillna(
                    0.001
                )
    ...
    def _track_nan_origins(self, stage: str, data_dict: dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.error(f"🚨 Error in NaN tracking for {stage}: {e}")
    ...
    def _engineer_ohlcv_price_features_vectorized(
        ...
        try:
            ...
            self.logger.error(f"🚨 Error engineering OHLCV features: {e}")
            return {}
    ...
    def _engineer_adaptive_indicators_vectorized(
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(f"🔍 Successfully converted adaptive_period to int")
            except Exception as e:
                self.logger.warning(
            ...
            try:
                ...
                self.logger.debug(
                    f"🔍 Successfully converted adaptive_rsi_period to int"
                )
            except Exception as e:
                self.logger.warning(
    ...
    def _select_optimal_features_vectorized(
        ...
        try:
            ...
            if len(non_constant) < len(features):
                constant_features = [
                    col for col in features.keys() if col not in non_constant
                ]
        ...
        try:
            ...
            for key, value in features.items():
                ...
                if hasattr(value, '__await__') or asyncio.iscoroutine(value):
                    ...
                    continue
                elif hasattr(value, '__aiter__') or hasattr(value, '__anext__'):
                    removed_features.append(key)
                    self.logger.warning(f"⚠️ Skipping async iterator feature: {key}")
                    continue
                elif callable(value) and asyncio.iscoroutinefunction(value):
                    removed_features.append(key)
            ...
            self.logger.error(f"❌ Error ensuring pickle safety: {e}")
            return features
    ...
    def _resample_price_data(self, price_data: pd.DataFrame, timeframe: str) -> pd.DataFrame | None:
        ...
        try:
            ...
            self.logger.error(f"🚨 Error resampling price data to {timeframe}: {e}")
            return None
    ...
    def _resample_volume_data(self, volume_data: pd.DataFrame, timeframe: str) -> pd.DataFrame | None:
        ...
        try:
            ...
            self.logger.error(f"🚨 Error resampling volume data to {timeframe}: {e}")
            return None
    ...
    def _generate_cross_timeframe_features(self, price_data: pd.DataFrame, volume_data: pd.DataFrame | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            # Validate input data
            if close.isna().all() or close.std() == 0:
                self.logger.warning(f"⚠️ Invalid close data for cross-timeframe features")
                return features
    ...
    def _calculate_bollinger_bands(self, close: pd.Series, window: int, std: float) -> pd.Series | None:
        ...
        try:
            ...
            bb_position = (close - lower_band) / (upper_band - lower_band + 1e-8)
            return bb_position
        except Exception:
            return None
    ...
    async def _generate_regime_aware_features(self, price_data: pd.DataFrame, volume_data: pd.DataFrame | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"🚨 Error generating regime-aware features: {e}")
            return {}
    ...
    def _validate_and_clean_features(self, features: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"🚨 Error validating and cleaning features: {e}")
            return features
    ...
    def _generate_fallback_features(self, price_data: pd.DataFrame, volume_data: pd.DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"🚨 Error generating fallback features: {e}")
            return {}
    ...
    async def _engineer_difference_and_acceleration_features(
        ...
        try:
            ...
            try:
                ...
                accel_raw = [k for k in final_enhanced_features.keys() if "_accel_" in k and not k.endswith("_norm")]
                # Cross-timeframe raw diff features (e.g., rsi_diff_5m_1m)
                cross_time_raw = [
                    k for k in final_enhanced_features.keys()
            ...
            self.logger.error(f"🚨 Error engineering difference and acceleration features: {e}")
            return {}
    ...
    def _normalize_with_rolling_zscore(self, series: pd.Series, window: int = 20) -> pd.Series:
        ...
        try:
            ...
            self.logger.error(f"🚨 Error in rolling Z-score normalization: {e}")
    ...
    def _validate_difference_engineering_inputs(
        ...
        try:
            ...
            # Validate features
            if not features:
                raise ValueError("No features provided for enhancement")
            
            # Check for minimum data length
            if len(price_data) < 100:
                raise ValueError(f"Insufficient data length: {len(price_data)} < 100")
            ...
            self.logger.error(f"❌ Validation failed for difference engineering inputs: {e}")
            raise
    ...
    def _validate_enhanced_features(self, enhanced_features: dict[str, Any]) -> None:
        ...
        try:
            ...
            # Count feature types
            diff_features = [f for f in valid_features.keys() if '_diff_' in f]
            accel_features = [f for f in valid_features.keys() if '_accel_' in f]
            norm_features = [f for f in valid_features.keys() if '_norm' in f]
            interaction_features = [f for f in valid_features.keys() if '_x_' in f]
            cross_timeframe_features = [f for f in valid_features.keys() if 'diff_' in f and ('m_' in f or 'h_' in f)]
            ...
            self.logger.error(f"❌ Validation failed for enhanced features: {e}")
            raise
    ...
    def _log_feature_engineering_summary(
        ...
        try:
            ...
            diff_features = [f for f in valid_enhanced_features.keys() if '_diff_' in f]
            accel_features = [f for f in valid_enhanced_features.keys() if '_accel_' in f]
            norm_features = [f for f in valid_enhanced_features.keys() if '_norm' in f]
            interaction_features = [f for f in valid_enhanced_features.keys() if '_x_' in f]
            cross_timeframe_features = [f for f in valid_enhanced_features.keys() if 'diff_' in f and ('m_' in f or 'h_' in f)]
    ...
    def _generate_sr_levels(self, price_data: pd.DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"❌ Error generating S/R levels: {e}")
            return {}
        ...
        try:
            ...
            # Strategy 2: Resample to regular intervals for higher irregularity
                ...
                if aggregation_map:
                    ...
                    # Fallback: if no recognized columns, use last observation
