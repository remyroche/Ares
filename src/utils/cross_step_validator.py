...
class CrossStepValidator:
    ...
    def __init__(self):
        ...
        self.consistency_rules = {
            ...
            'volume_consistency': {
                'enabled': True,
                'max_volume_change_percentage': 1000.0,  # 1000% change
                'min_volume_threshold': 0.0
            },
            'price_range_validation': {
                'enabled': True,
                'max_price_change_percentage': 50.0,  # 50% change
            ...
            'data_shape_consistency': {
                'enabled': True,
                'max_row_loss_percentage': 10.0,  # 10% row loss
                'max_row_gain_percentage': 20.0   # 20% row gain
            }
        }
        ...
        self.step_specific_rules = {
            ...
            'sr_optimization': {
                'expected_new_columns': ['sr_levels', 'sr_strength'],
                'preserve_original_columns': True
            },
            'hmm_regime_discovery': {
                'expected_new_columns': ['regime_labels', 'regime_probabilities'],
                'regime_count_range': (2, 10)
            },
            'regime_data_splitting': {
                'expected_outputs': ['regime_data'],
                'preserve_data_integrity': True
            },
            'labeling': {
                'expected_new_columns': ['labels', 'label_confidence'],
                'label_value_range': (-1, 0, 1)  # -1, 0, 1 for sell, hold, buy
            },
            'feature_engineering': {
    ...
    def validate_step_transition(self, 
        ...
        try:
            ...
            consistency_checks = [
                self._check_timestamp_continuity,
                self._check_volume_consistency,
                self._check_price_range_validation,
                self._check_column_preservation,
                self._check_data_shape_consistency,
                self._check_step_specific_rules
            ]
            ...
            validation_result['consistency_score'] = self._calculate_consistency_score(
                total_issues, total_warnings, input_data, output_data
            )
            ...
            elif total_issues > 5:  # More than 5 issues
                validation_result['passed'] = False
    ...
    def _record_data_lineage(self, 
        ...
        lineage = DataLineage(
            ...
            data_quality_score=self.standards.validate_data_quality(output_data, 'unified').quality_score,
    ...
    def _check_timestamp_continuity(self, from_step: str, to_step: str, 
        ...
        if 'timestamp' not in input_data.columns or 'timestamp' not in output_data.columns:
            warnings.append({
                'type': 'timestamp_continuity',
                'severity': 'low',
                'message': 'Timestamp column not found in input or output data'
            })
            return issues, warnings
        ...
        if len(output_timestamps) < len(input_timestamps) * 0.9:  # More than 10% loss
            issues.append({
                'type': 'timestamp_continuity',
                'severity': 'medium',
                'message': f'Significant timestamp loss: {len(input_timestamps)} → {len(output_timestamps)}',
                'suggested_fix': 'Check for data filtering or aggregation issues'
            })
        
        # Check for new timestamps (should be minimal)
        new_timestamps = set(output_timestamps) - set(input_timestamps)
        if len(new_timestamps) > len(input_timestamps) * 0.05:  # More than 5% new timestamps
            warnings.append({
                'type': 'timestamp_continuity',
                'severity': 'low',
                'message': f'Unexpected new timestamps: {len(new_timestamps)} new timestamps found',
                'suggested_fix': 'Verify timestamp generation logic'
            })
        
        return issues, warnings
    
    def _check_volume_consistency(self, from_step: str, to_step: str,
                                input_data: pd.DataFrame, output_data: pd.DataFrame,
                                metadata: Optional[Dict[str, Any]]) -> Tuple[List[Dict], List[Dict]]:
        ...
        if input_volume_mean > 0:
            ...
            if volume_change_percentage > max_change:
                issues.append({
                    ...
                    'severity': 'high',
                    'message': f'Extreme volume change: {volume_change_percentage:.1f}%',
                    'suggested_fix': 'Check for volume calculation errors or data corruption'
                })
        ...
        if negative_volumes > 0:
            issues.append({
                'type': 'volume_consistency',
                'severity': 'critical',
                'message': f'Negative volumes detected: {negative_volumes} rows',
                'suggested_fix': 'Fix volume calculation or data source'
            })
        
        return issues, warnings
    
    def _check_price_range_validation(self, from_step: str, to_step: str,
                                    input_data: pd.DataFrame, output_data: pd.DataFrame,
                                    metadata: Optional[Dict[str, Any]]) -> Tuple[List[Dict], List[Dict]]:
        ...
        if missing_price_cols:
            warnings.append({
                'type': 'price_range_validation',
                'severity': 'low',
                'message': f'Missing price columns: {missing_price_cols}'
            })
            return issues, warnings
            ...
            if col in input_data.columns and col in output_data.columns:
                ...
                if input_price_mean > 0:
                    ...
                    if price_change_percentage > max_change:
                        issues.append({
                            'type': 'price_range_validation',
                            'severity': 'high',
                            'message': f'Extreme {col} price change: {price_change_percentage:.1f}%',
                            'suggested_fix': 'Check for price calculation errors or data corruption'
                        })
                ...
                if negative_prices > 0:
                    issues.append({
                        'type': 'price_range_validation',
                        'severity': 'critical',
                        'message': f'Negative {col} prices detected: {negative_prices} rows',
                        'suggested_fix': 'Fix price calculation or data source'
                    })
    ...
    def _check_column_preservation(self, from_step: str, to_step: str,
        ...
        if missing_critical:
            issues.append({
                'type': 'column_preservation',
                'severity': 'critical',
                'message': f'Critical columns removed: {missing_critical}',
                'suggested_fix': 'Ensure critical columns are preserved in step processing'
            })
        
        # Check for unexpected column removal
        ...
        if removed_columns and not self.consistency_rules['column_preservation']['allow_additional_columns']:
            warnings.append({
                'type': 'column_preservation',
                'severity': 'medium',
                'message': f'Columns removed: {list(removed_columns)}',
                'suggested_fix': 'Verify column removal is intentional'
            })
        
        return issues, warnings
    
    def _check_data_shape_consistency(self, from_step: str, to_step: str,
                                    input_data: pd.DataFrame, output_data: pd.DataFrame,
                                    metadata: Optional[Dict[str, Any]]) -> Tuple[List[Dict], List[Dict]]:
        ...
        if input_rows > 0:
            ...
            if row_loss_percentage > max_loss:
                issues.append({
                    'type': 'data_shape_consistency',
                    'severity': 'high',
                    'message': f'Excessive row loss: {row_loss_percentage:.1f}% ({input_rows} → {output_rows})',
                    'suggested_fix': 'Check for data filtering or processing errors'
                })
        
        # Check for excessive row gain
        if input_rows > 0:
            ...
            if row_gain_percentage > max_gain:
                warnings.append({
                    'type': 'data_shape_consistency',
                    'severity': 'medium',
                    'message': f'Unexpected row gain: {row_gain_percentage:.1f}% ({input_rows} → {output_rows})',
                    'suggested_fix': 'Verify data expansion logic'
                })
    ...
    def _check_step_specific_rules(self, from_step: str, to_step: str,
        ...
        if 'expected_columns' in rules:
            ...
            if missing_cols:
                issues.append({
                    'type': 'step_specific_rules',
                    'severity': 'high',
                    'message': f'Missing expected columns for {to_step}: {list(missing_cols)}',
                    'suggested_fix': f'Ensure {to_step} produces required columns'
                })
        
        # Check expected new columns
        if 'expected_new_columns' in rules:
            ...
            if missing_new_cols:
                warnings.append({
                    'type': 'step_specific_rules',
                    'severity': 'medium',
                    'message': f'Missing expected new columns for {to_step}: {list(missing_new_cols)}',
                    'suggested_fix': f'Verify {to_step} feature generation logic'
                })
        ...
        if 'min_rows' in rules:
            ...
            if len(output_data) < min_rows:
                issues.append({
                    'type': 'step_specific_rules',
                    'severity': 'high',
                    'message': f'Insufficient rows for {to_step}: {len(output_data)} < {min_rows}',
                    'suggested_fix': 'Check data filtering or processing logic'
                })
    ...
    def _calculate_consistency_score(self, total_issues: int, total_warnings: int,
        ...
        issue_penalty = total_issues * 10  # 10 points per issue
        
        # Deduct points for warnings
        warning_penalty = total_warnings * 2  # 2 points per warning
        
        # Deduct points for data shape changes
        if len(input_data) > 0:
    ...
    def get_consistency_summary(self) -> Dict[str, Any]:
        ...
        return {
            'total_steps_validated': len(self.data_lineage),
            'total_issues': total_issues,
            'critical_issues': critical_issues,
            'high_issues': high_issues,
            'consistency_score_avg': np.mean([lineage.data_quality_score for lineage in self.data_lineage]) if self.data_lineage else 100.0,
            'data_lineage_length': len(self.data_lineage)
        }
