...
class RegimeSpecificTrainingManager:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.tactician_models = {"lightgbm": None, "calibrated_logistic": None}
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                "✅ Regime-Specific Training Manager initialization completed",
            )
            return True

        except Exception as e:
            self.logger.error(
                f"❌ Regime-Specific Training Manager initialization failed: {e}",
            )
            return False
    ...
    async def execute_regime_training(
        ...
        try:
            ...
            regime_data = await self._separate_data_by_regime(
                training_data,
                regime_labels,
            )
            ...
            training_success = await self._train_regime_models(
                labeled_regime_data,
                symbol,
                exchange,
            )
            ...
            self.logger.info(
                "✅ Regime-specific training pipeline completed successfully",
            )
            return True
    ...
    async def _label_data_with_regimes(
        ...
        try:
            ...
            for i in range(len(training_data)):
                ...
                # Predict regime
                regime, confidence, details = self.regime_classifier.predict_regime(
                    window_data,
                )
    ...
    async def _separate_data_by_regime(
        ...
        try:
            ...
            for regime in set(regime_labels):
                ...
                regime_df = training_data_with_regimes[
                    training_data_with_regimes["regime"] == regime
                ]
    ...
    async def _label_with_triple_barrier(
    ...
    ) -> dict[str, RegimeTrainingData] | None:
        """
        ...
        try:
            ...
            for regime, data in regime_data.items():
                ...
                labels, features = await self._apply_triple_barrier_labeling(
                    data,
                    regime,
                )
                ...
                if labels is not None and features is not None:
                    ...
                    self.logger.info(
                        f"Labeled {regime} regime: {len(labels)} samples, {features.shape[1]} features",
                    )
    ...
    async def _apply_triple_barrier_labeling(
        ...
        try:
            ...
            self.logger.error(
                f"Error applying triple barrier labeling for {regime}: {e}",
            )
            return None, None
    ...
    def _apply_triple_barrier(self, data: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            for i in range(len(data)):
                ...
                # Calculate barriers
                profit_barrier = entry_price * (
                    1 + self.triple_barrier_config["profit_take"]
                )
                stop_barrier = entry_price * (
                    1 - self.triple_barrier_config["stop_loss"]
                )
        ...
        try:
            ...
            for regime, training_data in labeled_regime_data.items():
                ...
                if self.training_config.get("enable_autoencoder_training", True):
                    autoencoder_success = await self._train_autoencoder(
                        training_data,
                        regime,
                    )
                    if not autoencoder_success:
                        self.logger.warning(
                            f"Autoencoder training failed for {regime} regime",
                        )
                ...
                if self.training_config.get("enable_ml_training", True):
                    ...
                    if not ml_success:
                        self.logger.warning(
                            f"ML model training failed for {regime} regime",
                        )
    ...
    async def _train_autoencoder(
        ...
        try:
            ...
            self.trained_models[f"{regime}_autoencoder"] = ModelTrainingResult(
                ...
                regime=regime,
                performance_metrics={
                    "explained_variance": autoencoder.explained_variance_ratio_.sum(),
                },
                feature_importance={},
                training_time=0.0,
            )
    ...
    async def _train_ml_models(
        ...
        try:
            ...
            if len(X_binary) < 100:
                self.logger.warning(
                    f"Insufficient data for {regime} regime: {len(X_binary)} samples",
                )
                return False

            # Split data
            split_idx = int(
                len(X_binary) * (1 - self.training_config.get("test_size", 0.2)),
            )
            ...
            if regime in ["BULL", "BEAR", "SIDEWAYS"]:
                ...
                success = await self._train_high_data_regime_models(
                    X_train,
                    X_test,
                    y_train,
                    y_test,
                ...
                success = await self._train_low_data_regime_models(
                    X_train,
                    X_test,
                    y_train,
                    y_test,
    ...
    async def _train_high_data_regime_models(
        ...
        try:
            ...
            self.logger.info(
                f"Trained {len(analyst_models)} models for {regime} regime",
            )
            return True

        except Exception as e:
            self.logger.error(
                f"Error training high-data regime models for {regime}: {e}",
            )
            return False
    ...
    async def _train_low_data_regime_models(
        ...
        try:
            ...
            self.logger.error(
                f"Error training low-data regime models for {regime}: {e}",
            )
            return False
    ...
    async def _apply_calibration(
        ...
        try:
            ...
            calibration_curve = {
                ...
                "calibration_threshold": self.calibration_config[
    ...
    def get_training_summary(self) -> dict[str, Any]:
        ...
        try:
            summary = {
                ...
                "regimes_trained": list(
                    set([model.regime for model in self.trained_models.values()]),
                ),
                "model_types": list(
                    set(
                        [
                ...
                "calibration_status": all(
                    hasattr(model, "calibration_curve")
                    and model.calibration_curve is not None
                    for model in self.trained_models.values()
                ),
