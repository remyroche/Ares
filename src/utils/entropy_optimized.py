...
def rolling_entropy_numba(
    values: np.ndarray, window: int, n_bins: int = 10
) -> np.ndarray:
    """
...
def shannon_entropy_numba(values: np.ndarray, n_bins: int = 10) -> float:
    ...
    # 2. Count directly (avoid allocating bins array)
    counts = np.zeros(n_bins, dtype=np.int32)

    # Pre-compute inverse width for multiplication (faster than division)
    inv_width = 1.0 / bin_width

    for i in range(n):
        val = values[i]
        bin_idx = int((val - min_val) * inv_width)
        if bin_idx >= n_bins:
            bin_idx = n_bins - 1
        elif bin_idx < 0:
            bin_idx = 0
...
def lempel_ziv_complexity_numba(
    values: np.ndarray, normalize: bool = True
) -> np.ndarray:
    """
...
def calculate_trend_conviction_index_numba(
    entropy_values: np.ndarray, timestamps: np.ndarray
) -> np.ndarray:
    """
...
def calculate_drift_proxy_numba(
    current_prices: np.ndarray, specialist_prices: np.ndarray
) -> np.ndarray:
    """
    ...
    for i in prange(n):
        if specialist_prices[i] > 0:
            drift_proxy[i] = (
                current_prices[i] - specialist_prices[i]
            ) / specialist_prices[i]
        else:
            drift_proxy[i] = 0.0
...
def calculate_entropy_statistics_numba(
    entropy_values: np.ndarray, window: int = 10
...
def vectorized_entropy_features(
    ...
    if use_numba:
        ...
        if "entropy_contribution" in source_data.columns:
            ...
            tci_values = calculate_trend_conviction_index_numba(
                entropy_contribution, timestamps
            )
            features["trend_conviction_index"] = tci_values

            # Entropy statistics
            entropy_ma, entropy_std, entropy_zscore = (
                calculate_entropy_statistics_numba(entropy_contribution)
            )
        ...
        staleness_seconds, staleness_adjusted_drift = (
            calculate_staleness_features_numba(timestamps, last_update_time, volatility)
        )
        ...
        for window in [20, 40, 60, 100]:
            if window < len(source_data):
                entropy_values = returns.rolling(window).apply(
                    lambda x: (
                        shannon_entropy_numba(x.values, n_bins)
                        if len(x.dropna()) > 0
                        else 0
                    )
                )
        ...
        features["staleness_seconds"] = (
            source_data.index - pd.Timestamp(current_time, unit="s")
        ).total_seconds()
        ...
        if "entropy_contribution" in source_data.columns:
            ...
            time_diff = (
                source_data.index.to_series().diff().dt.total_seconds().fillna(60)
            )
