...
def check_model_artifact_structure(
    run_id: str, data_root: str = DEFAULT_DATA_ROOT
) -> Dict[str, bool]:
    ...
    meta_feature_contract_path = os.path.join(
        run_dir, "meta_oof", "meta_feature_contract.json"
    )
    if os.path.exists(meta_feature_contract_path):
        artifacts["meta_feature_contract"] = True
...
def check_model_loading(
    ...
    try:
        ...
        model_loader_path = os.path.join(
            project_root, "extreme_price_movements", "model_loader.py"
        )
        ...
        if found_run_id != run_id:
            result[
                "error"
            ] = f"Found run_id ({found_run_id}) differs from expected ({run_id})"
        ...
        try:
            ...
            result["details"]["spike_models_count"] = len(
                bundle.get("spike_models", {})
            )
            ...
            try:
                ...
                result["details"]["feature_parity"] = validate_live_feature_contract(
                    bundle,
                    strict=False,
                )
                result["details"][
                    ...
                    for kind, info in models.items():
                        ...
                        if model and hasattr(model, "calibration_state_"):
                            if not model.calibration_state_:
                                ...
                                result["details"][
                                    "calibration_integrity_error"
                                ] = f"Missing calibration state for {side}_{kind}"
                                break
            except Exception as e:
        ...
        try:
            ...
            result["details"]["bucket_params_keys"] = (
                list(bucket_params.keys()) if bucket_params else []
            )
        except Exception as e:
    ...
    success = (
        result["can_import_model_loader"]
        and result["can_find_latest_run_id"]
        and result["can_load_model_bundle"]
        and bool(result["details"].get("meta_feature_contract", False))
    )
...
def check_binance_connection(
    test_symbols: List[str] = DEFAULT_SYMBOLS,
) -> Tuple[bool, Dict[str, Any]]:
...
def check_local_data(
    data_root: str = DEFAULT_DATA_ROOT, test_symbols: List[str] = None
) -> Dict[str, Any]:
...
def print_summary(
    ...
    all_checks_passed = (
        artifacts_ok
        and model_loading_result.get("can_load_model_bundle", False)
        and model_loading_result.get("details", {}).get("meta_feature_contract", False)
        and binance_result.get("can_create_exchange", False)
    )
    ...
    if run_id:
        ...
        if all_run_ids:
            print(
                f"  All available run_ids: {', '.join(all_run_ids[:5])}{'...' if len(all_run_ids) > 5 else ''}"
            )
    ...
    print_status(
        "Native models directory", model_structure.get("native_models_dir", False)
    )
    ...
    print_status(
        ...
        model_structure.get("meta_feature_contract", False),
    )
    ...
    print_status(
        "Can import model_loader",
        model_loading_result.get("can_import_model_loader", False),
    )
    print_status(
        "Can find latest run_id",
        model_loading_result.get("can_find_latest_run_id", False),
    )
    print_status(
        "Can load model bundle",
        model_loading_result.get("can_load_model_bundle", False),
    )
    ...
    if model_loading_result.get("details"):
        ...
        if "meta_feature_contract" in details:
            print_status("Meta feature contract", details["meta_feature_contract"])

        if "calibration_integrity" in details:
            print_status(
                "Calibration integrity validator", details["calibration_integrity"]
            )
    ...
    print_status(
        "Can create exchange", binance_result.get("can_create_exchange", False)
    )
    ...
    if all_checks_passed:
        ...
        if not model_loading_result.get("details", {}).get(
            "meta_feature_contract", False
        ):
            print("    - Meta feature contract missing. Rerun train_meta.")
