...
from .hardware import (
    ...
    get_integrated_hardware_manager, WorkloadCategory, optimize_dataframe, optimize_array
)
...
class ArtifactManager:
    ...
    def save(self, data: Any, artifact_name: str, 
        ...
        """
        Save an artifact with automatic CSV generation and hardware optimization.
        
        This is the default save method used by BaseStep. It automatically:
        - Optimizes data with hardware acceleration
        ...
        """
        try:
            # Optimize data with hardware acceleration
            optimized_data = self.hardware_manager.process_data_with_optimization(
                data, WorkloadCategory.DATA_PROCESSING
            )
            
            # Apply additional DataFrame optimization if applicable
            if isinstance(optimized_data, pd.DataFrame):
                optimized_data = optimize_dataframe(optimized_data)
            elif isinstance(optimized_data, np.ndarray):
                optimized_data = optimize_array(optimized_data)
            ...
            if isinstance(optimized_data, pd.DataFrame) and len(optimized_data) < 2000:
                try:
                    csv_path = self._get_enhanced_path(self._current_step_name, artifact_name, "csv")
                    optimized_data.to_csv(csv_path, index=True)
            ...
            elif isinstance(optimized_data, pd.DataFrame) and len(optimized_data) >= 2000:
                self.logger.info(f"📊 Skipping CSV auto-save for {artifact_name} (rows >= 2000: {len(optimized_data)})")
    ...
    def get_artifact(self, artifact_name: str, 
        ...
        """
        Retrieve an artifact using multiple fallback mechanisms with hardware optimization.
        ...
        try:
            ...
            if artifact_path and artifact_path.exists():
                ...
                # Apply hardware optimization to retrieved data
                if data is not None:
                    data = self.hardware_manager.process_data_with_optimization(
                        data, WorkloadCategory.DATA_PROCESSING
                    )
                return data
    ...
    def reset_run(self) -> None:
        """Clear all artifacts and prepare storage for a new pipeline run with garbage collection optimization."""
        with self._lock:
            # Clear all storage
