...
Provides resilience patterns for handling transient failures, timeouts, and cascading
failures.
"""
    ...
    *,
    ...
    def sync_handler(func: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
        ...
        raise ServiceUnavailableError(
            ...
            details={"attempts": max_attempts, "last_error": str(last_exception)},
        )
    ...
    return uniform_wrapper(
        f"retry(max_attempts={max_attempts})", sync_handler, async_handler
    )
...
def circuit_breaker(
    *,
    ...
    return uniform_wrapper(
        f"circuit_breaker({breaker_name or 'auto'})", sync_handler, async_handler
    )
