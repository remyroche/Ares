	...
	def _load_artifact_from_path(self, path: Path) -> Any:
		...
		try:
			if path.suffix == '.parquet':
				...
				# Check if this is artifact metadata (matches structure generated by save() method)
				# Metadata files always have: artifact_name, artifact_type, file_path, file_size, timestamp
				is_metadata = (
					isinstance(json_data, dict) and
					'file_path' in json_data and
					'artifact_type' in json_data and
					'artifact_name' in json_data
				)
