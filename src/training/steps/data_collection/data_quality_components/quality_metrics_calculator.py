...
class QualityMetricsCalculator:
    ...
    def calculate_completeness_metrics(self, data: pd.DataFrame) -> dict[str, Any]:
        ...
        """
        metrics = {'total_rows': len(data), 'total_columns': len(data.columns), 'missing_values': {}, 'completeness_by_column': {}, 'overall_completeness': 0.0}
        for col in data.columns:
            missing_count = data[col].isna().sum()
            missing_ratio = missing_count / len(data) if len(data) > 0 else 0
            metrics['missing_values'][col] = {'count': int(missing_count), 'ratio': float(missing_ratio)}
    ...
    def calculate_consistency_metrics(self, data: pd.DataFrame) -> dict[str, Any]:
        ...
        """
        metrics = {'ohlc_consistency': {}, 'volume_consistency': {}, 'price_consistency': {}, 'overall_consistency': 0.0}
        if all((col in data.columns for col in ['open', 'high', 'low', 'close'])):
            ...
            metrics['ohlc_consistency'] = {'inconsistent_count': int(ohlc_inconsistent.sum()), 'inconsistent_ratio': float(ohlc_inconsistent.sum() / len(data)) if len(data) > 0 else 0, 'consistent_ratio': float(1 - ohlc_inconsistent.sum() / len(data)) if len(data) > 0 else 1}
    ...
    def calculate_timeliness_metrics(self, data: pd.DataFrame) -> dict[str, Any]:
        ...
        if isinstance(data.index, pd.DatetimeIndex):
            ...
            metrics['update_frequency'] = {'mean_interval_seconds': float(time_diffs.mean().total_seconds()), 'median_interval_seconds': float(time_diffs.median().total_seconds()), 'mode_interval_seconds': float(time_diffs.mode()[0].total_seconds()) if len(time_diffs.mode()) > 0 else None, 'std_interval_seconds': float(time_diffs.std().total_seconds())}
            ...
            metrics['gap_analysis'] = {'gap_count': int(len(gaps)), 'gap_ratio': float(len(gaps) / len(time_diffs)) if len(time_diffs) > 0 else 0, 'max_gap_hours': float(gaps.max().total_seconds() / 3600) if len(gaps) > 0 else 0, 'total_gap_hours': float(gaps.sum().total_seconds() / 3600) if len(gaps) > 0 else 0}
            age_penalty = min(metrics['data_age']['latest_data_age_hours'] / 24, 1.0)
    ...
    def calculate_validity_metrics(self, data: pd.DataFrame) -> dict[str, Any]:
        ...
        """
        metrics = {'data_type_validity': {}, 'range_validity': {}, 'format_validity': {}, 'overall_validity': 0.0}
        validity_scores = []
        expected_types = {'open': [np.number], 'high': [np.number], 'low': [np.number], 'close': [np.number], 'volume': [np.number]}
    ...
    def generate_quality_report(self, data: pd.DataFrame, symbol: str, exchange: str, include_recommendations: bool = True) -> dict[str, Any]:
        ...
        """
        report = {'metadata': {'symbol': symbol, 'exchange': exchange, 'timestamp': pd.Timestamp.now().isoformat(), 'data_shape': data.shape, 'date_range': {'start': str(data.index.min()) if isinstance(data.index, pd.DatetimeIndex) else None, 'end': str(data.index.max()) if isinstance(data.index, pd.DatetimeIndex) else None}}, 'metrics': {}, 'overall_score': 0.0, 'recommendations': []}
    ...
    def _generate_recommendations(self, metrics: dict[str, Any]) -> list[str]:
        ...
        if 'completeness' in metrics:
            if metrics['completeness']['overall_completeness'] < 0.95:
                recommendations.append(f"Data completeness is {metrics['completeness']['overall_completeness']:.1%}. Consider filling missing values or investigating data collection issues.")
        if 'consistency' in metrics:
            if metrics['consistency'].get('ohlc_consistency', {}).get('inconsistent_ratio', 0) > 0.001:
                recommendations.append('OHLC consistency issues detected. Review data source and validation logic.')
        if 'timeliness' in metrics:
            if metrics['timeliness'].get('data_age', {}).get('latest_data_age_hours', 0) > 24:
                recommendations.append('Data is more than 24 hours old. Consider updating to more recent data.')
        if 'validity' in metrics:
            if metrics['validity']['overall_validity'] < 0.95:
                recommendations.append('Data validity issues detected. Review data types and value ranges.')
