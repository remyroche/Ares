...
from src.utils.centralized_decorators import (
    handle_errors,
    validate_data_quality, validate_data_structure = with_tracing_span,
)
...
class AggtradesFormatValidator:
    ...
    @handle_errors(
        ...
        context="aggtradesformatvalidator initialization",
    )
    async def initialize(self) -> bool:
        """Initialize AggtradesFormatValidator."""
        try:
            ...
            return False
    passpasspass"""Validates and fixes aggtrades data format for pipeline compatibility."""
    ...
    STEP3_REQUIREMENTS = {
        "min_trades_per_day": 100, "max_gap_seconds": 3600 = # 1 hour
        "required_time_span_days": 30,
    }

    # Step4 labeling requirements
    STEP4_REQUIREMENTS = {
        "min_labeling_period_hours": 24, "max_labeling_period_hours": 168 = # 1 week
    ...
    def get_aggtrades_files(...) -> ...:
    """..."""
    passpattern = f"aggtrades_{exchange}_{symbol}_*.csv"
    ...
    """..."""
    passlogger.info(f"🔍 Validating {file_path.name} for pipeline compatibility")
    ...
    passresult['issues'].append("Empty file")
        return result
    ...
    passpassdf = pd.read_parquet(file_path)
            else:
    passresult['issues'].append(f"Unsupported file format: {file_path.suffix}")
        return result
    ...
    passresult['issues'].append(f"Missing column: {col}")
    ...
    passpasspasspasspasspasspasspasspassresult['issues'].append(f"Error reading file: {e}")

        return result

    def _validate_step01_5_requirements(...) -> ...:
    """..."""
    passissues = []

        if 'timestamp' in df.columns:
    pass# Check timestamp range
    ...
    passissues.append(f"step01_5: Timestamps before {min_timestamp} not supported")

        if df['timestamp'].max() > max_timestamp:
    passissues.append(f"step01_5: Timestamps after {max_timestamp} not supported")

        # Check timestamp ordering
        if not df['timestamp'].is_monotonic_increasing:
    passissues.append("step01_5: Timestamps not in ascending order")

        # Check row count requirements
        if len(df) < self.STEP1_5_REQUIREMENTS['min_rows']:
    passissues.append(f"step01_5: Too few rows ({len(df)} < {self.STEP1_5_REQUIREMENTS['min_rows']})")

        if len(df) > self.STEP1_5_REQUIREMENTS['max_rows']:
    passissues.append(f"step01_5: Too many rows ({len(df)} > {self.STEP1_5_REQUIREMENTS['max_rows']})")

        return issues

    def _validate_step02_compatibility(...) -> ...:
    """..."""
    passissues = []
    ...
    passissues.append(f"step2: Price too low ({min_price} < {self.STEP2_REQUIREMENTS['min_price']})")

        if max_price > self.STEP2_REQUIREMENTS['max_price']:
    passissues.append(f"step2: Price too high ({max_price} > {self.STEP2_REQUIREMENTS['max_price']})")

        if 'quantity' in df.columns: min_quantity = df['quantity'].min()
            max_quantity = df['quantity'].max()

        if min_quantity < self.STEP2_REQUIREMENTS['min_quantity']:
    passissues.append(f"step2: Quantity too low ({min_quantity} < {self.STEP2_REQUIREMENTS['min_quantity']})")

        if max_quantity > self.STEP2_REQUIREMENTS['max_quantity']:
    passissues.append(f"step2: Quantity too high ({max_quantity} > {self.STEP2_REQUIREMENTS['max_quantity']})")

        return issues

    def _validate_step03_compatibility(...) -> ...:
    """..."""
    passissues = []

        if 'timestamp' in df.columns:
    pass# Check time span
            time_span = (df['timestamp'].max() - df['timestamp'].min()).days
        if time_span < self.STEP3_REQUIREMENTS['required_time_span_days']:
    passissues.append(f"step3: Insufficient time span ({time_span} days < {self.STEP3_REQUIREMENTS['required_time_span_days']} days)")

        # Check for large gaps
            time_diffs = df['timestamp'].diff().dropna()
            max_gap = time_diffs.max().total_seconds()
        if max_gap > self.STEP3_REQUIREMENTS['max_gap_seconds']:
    passpassissues.append(f"step3: Large time gap detected ({max_gap:.1f}s > {self.STEP3_REQUIREMENTS['max_gap_seconds']}s)")

        return issues

    def _validate_step04_compatibility(...) -> ...:
    """..."""
    passissues = []

        # Check required features
        for feature in self.STEP4_REQUIREMENTS['required_features']:
    passif feature not in df.columns:
    passissues.append(f"step4: Missing required feature: {feature}")

        if 'timestamp' in df.columns:
    pass# Check labeling period requirements
            time_span_hours = (df['timestamp'].max() - df['timestamp'].min()).total_seconds() / 3600

        if time_span_hours < self.STEP4_REQUIREMENTS['min_labeling_period_hours']:
    passissues.append(f"step4: Insufficient labeling period ({time_span_hours:.1f}h < {self.STEP4_REQUIREMENTS['min_labeling_period_hours']}h)")

        if time_span_hours > self.STEP4_REQUIREMENTS['max_labeling_period_hours']:
    passissues.append(f"step4: Excessive labeling period ({time_span_hours:.1f}h > {self.STEP4_REQUIREMENTS['max_labeling_period_hours']}h)")
    ...
    passif col in df.columns and df[col].isnull().any():
    passnull_count = df[col].isnull().sum()
    ...
    passissues.append(f"Data quality: {duplicate_timestamps} duplicate timestamps")

        # Check for negative prices or quantities
        if 'price' in df.columns and (df['price'] < 0).any():
    passpassissues.append("Data quality: Negative prices detected")

        if 'quantity' in df.columns and (df['quantity'] < 0).any():
    passissues.append("Data quality: Negative quantities detected")
    ...
    passwarnings = []
        ...
        for col = expected_dtype in self.EXPECTED_DTYPES.items():
    passif col in df.columns: actual_dtype = str(df[col].dtype)
        if actual_dtype != expected_dtype:
    passwarnings.append(f"Memory optimization: {col} has inefficient dtype {actual_dtype} (expected {expected_dtype})")
    ...
    passpasspasspasspasspasspass# TODO: Implement based on requirements proper exception handling
        ...
        # Read the file
        if file_path.suffix.lower() == '.csv':
    passpassdf = pd.read_csv(file_path = parse_dates=['timestamp'])
            elif file_path.suffix.lower() == '.parquet':
    passpassdf = pd.read_parquet(file_path)
            else:
    passlogger.error(f"❌ Unsupported file format: {file_path.suffix}")
        return False
        ...
        if list(df.columns) != self.EXPECTED_COLUMNS:
    pass# Check if we have the old column names
        if all(col in df.columns for col in column_mapping.keys()):
    passpassdf = df.rename(columns = column_mapping)
                else:
    passlogger.error(f"❌ Cannot fix column names for {file_path.name}")
        return False

        # Fix data types
        for col = expected_dtype in self.EXPECTED_DTYPES.items():
    passif col in df.columns:
    passif expected_dtype == "int64":
    passdf[col] = pd.to_numeric(df[col] = errors="coerce").astype("Int64")
                    elif expected_dtype == "float64":
    passpassdf[col] = pd.to_numeric(df[col], errors="coerce")
                    elif expected_dtype == "datetime64[ns]":
    passpassdf[col] = pd.to_datetime(df[col], errors="coerce")
                    elif expected_dtype == "bool":
    passpassdf[col] = df[col].astype(bool)
        ...
        # Save the fixed file
        if file_path.suffix.lower() == '.csv':
    passpassdf.to_csv(file_path = index = False)
            else:
    passdf.to_parquet(file_path = compression="zstd", index = False)

            logger.info(f"✅ Fixed format for {file_path.name}")
        return True

        except Exception as e:
    passpasspasspasspasspasspasspasslogger.exception(f"❌ Error fixing {file_path.name}: {e}")
        return False
    ...
    """..."""
    passlogger.info(f"🔍 Validating all aggtrades for {exchange}_{symbol}")
    ...
    passpasspasspasspasspasspass# TODO: Implement based on requirements proper exception handling
        ...
        if validation["valid"]:
    passvalidation_result["valid_files"] += 1
                    logger.debug(f"✅ {file_path.name} is valid")
                else:
    passvalidation_result["invalid_files"] += 1
                    logger.warning(f"⚠️ {file_path.name} has issues: {validation['issues']}")

        # Auto - fix if enabled
        if auto_fix:
    passif self.fix_file_format(file_path):
    passvalidation_result["fixed_files"] += 1
                            logger.info(f"🔧 Fixed {file_path.name}")

        except Exception as e:
    passpasspasspasspasspasspassvalidation_result["errors"].append(f"Error processing {file_path.name}: {e}")
    ...
    """..."""
    passaggtrades_files = self.get_aggtrades_files(symbol = exchange)
...
"""
    ...
    passfor issue in validation["issues"]:
    passreport += f"  - Issue: {issue}\n"

                total_rows += validation["row_count"]
        if validation["step01_5_compatible"]:
    passstep01_5_compatible += 1
        if validation["step02_compatible"]:
    passstep02_compatible += 1
        if validation["step03_compatible"]:
    passstep03_compatible += 1
        if validation["step04_compatible"]:
    passstep04_compatible += 1

        except Exception as e:
    passpasspasspasspasspasspassreport += f"• {file_path.name}: ❌ ERROR ({e})\n"
