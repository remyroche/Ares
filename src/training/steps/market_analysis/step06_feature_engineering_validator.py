...
class Step6FeatureEngineeringValidator(BaseValidator):
    ...
    @validates()
    async def validate_step6_feature_engineering(self, symbol: str, exchange: str, data_dir: str, training_input: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not regime_features_dir.exists():
                self.logger.warning(f'⚠️ Regime features directory not found: {regime_features_dir}')
                return False
            ...
            for regime_dir in regime_dirs:
                ...
                if not feature_files:
                    self.logger.warning(f'⚠️ No feature files found for regime: {regime_name}')
                    continue
            ...
            error_context = {'step': 'step06_feature_engineering', 'symbol': symbol, 'exchange': exchange, 'data_dir': data_dir, 'error_type': type(e).__name__, 'error_message': str(e), 'timestamp': pd.Timestamp.now().isoformat()}
    ...
    async def _validate_feature_file(self, feature_file: Path, regime_name: str) -> bool:
        ...
        try:
            ...
            if len(feature_columns) < 5:
                self.logger.warning(f'⚠️ Insufficient features in {feature_file.name}: {len(feature_columns)} features')
                return False
            ...
            error_context = {'file': str(feature_file), 'regime': regime_name, 'error_type': type(e).__name__, 'error_message': str(e)}
    ...
    def validate_step_prerequisites(self, symbol: str, exchange: str, timeframe: str) -> Dict[str, Any]:
        ...
        try:
            step5_output_dir = Path('data/training/labeled_data')
            step5_files = list(step5_output_dir.glob(f'{exchange}_{symbol}_{timeframe}*labeled*.parquet'))
            if not step5_files:
                validation_result['validation_passed'] = False
                validation_result['errors'].append(f'Step 5 labeling output not found for {exchange}_{symbol}_{timeframe}')
            else:
                ...
                validation_result['details']['step5_files_found'] = len(step5_files)
                validation_result['details']['step5_files'] = [str(f) for f in step5_files]
        except Exception as e:
            validation_result['validation_passed'] = False
...
async def run_validator(training_input: Dict[str, Any], pipeline_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        step_result = await validator.validate_step6_feature_engineering(symbol, exchange, data_dir, training_input)
        ...
        return {'step_name': 'step06_feature_engineering', 'validation_passed': validation_passed, 'prerequisites': prereq_result, 'step_execution': step_result, 'outputs': output_result, 'warnings': prereq_result['warnings'] + output_result['warnings'], 'errors': prereq_result['errors'] + output_result['errors']}
    except Exception as e:
        error_context = {'step': 'step06_feature_engineering', 'symbol': training_input.get('symbol', 'UNKNOWN'), 'exchange': training_input.get('exchange', 'UNKNOWN'), 'error_type': type(e).__name__, 'error_message': str(e), 'timestamp': pd.Timestamp.now().isoformat()}
        logger.exception(f'❌ Step 6 validation failed: {error_context}')
        return {'step_name': 'step06_feature_engineering', 'validation_passed': False, 'error': str(e), 'error_context': error_context}
