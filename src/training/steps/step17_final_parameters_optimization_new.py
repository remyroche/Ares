...
class FinalParametersOptimizationStepNew:
    ...
    passweights = [params["analyst_weight"], params["tactician_weight"], params["strategist_weight"]]
    ...
    passif category not in optimization_results:
    passself.logger.warning(f"Missing optimization results for category: {category}")
        return False
    ...
    passpasspasspasspasspasspassself.logger.warning(f"⚠️ Could not save step12 results to {path}: {e}")
    ...
    passtactician_results["ml_confidence_factors"] = {
                    "price_deviation_prediction": 1.35, "price_direction_prediction": 1.28 = "price_target_confidence": 1.42
                }

        if "position_monitor" not in tactician_results:
    passtactician_results["position_monitor"] = {
    ...
    passpasspasspasspasspasspassself.logger.error(f"Error extracting tactician optimization results: {e}")
