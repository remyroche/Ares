...
class VectorBTSupportLevelGenerator(VectorBTFeatureGenerator):
    ...
    def _create_default_config(cls, level: int = 1, window: int = 20) -> FeatureConfig:
        return FeatureConfig(
            name=f"vectorbt_support_level_{level}_{window}",
    ...
    def _generate_feature(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        if data.empty:
            return pd.Series(dtype=float, index=data.index, name=f'vectorbt_support_level_{self.level}_{self.window}')
        ...
        return support_level.rename(f'vectorbt_support_level_{self.level}_{self.window}')
...
class VectorBTResistanceLevelGenerator(VectorBTFeatureGenerator):
    ...
    def _create_default_config(cls, level: int = 1, window: int = 20) -> FeatureConfig:
        return FeatureConfig(
            name=f"vectorbt_resistance_level_{level}_{window}",
            category=FeatureCategory.SUPPORT_RESISTANCE,
    ...
    def _generate_feature(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        if data.empty:
            return pd.Series(dtype=float, index=data.index, name=f'vectorbt_resistance_level_{self.level}_{self.window}')
        ...
        return resistance_level.rename(f'vectorbt_resistance_level_{self.level}_{self.window}')
...
class VectorBTPivotPointGenerator(VectorBTFeatureGenerator):
    ...
    def _create_default_config(cls, window: int = 20) -> FeatureConfig:
        return FeatureConfig(
            name=f"vectorbt_pivot_point_{window}",
            category=FeatureCategory.SUPPORT_RESISTANCE,
    ...
    def _generate_feature(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        if data.empty:
            return pd.Series(dtype=float, index=data.index, name=f'vectorbt_pivot_point_{self.window}')
        ...
        return pivot_point.rename(f'vectorbt_pivot_point_{self.window}')
...
class VectorBTFibonacciLevelGenerator(VectorBTFeatureGenerator):
    ...
    def _create_default_config(cls, level: float = 0.618, window: int = 20) -> FeatureConfig:
        return FeatureConfig(
            name=f"vectorbt_fibonacci_{level}_{window}",
            category=FeatureCategory.SUPPORT_RESISTANCE,
    ...
    def _generate_feature(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        if data.empty:
            return pd.Series(dtype=float, index=data.index, name=f'vectorbt_fibonacci_{self.level}_{self.window}')
        ...
        return fibonacci_level.rename(f'vectorbt_fibonacci_{self.level}_{self.window}')


class VectorBTVolumeWeightedSupportResistanceGenerator(VectorBTFeatureGenerator):
    ...
    def _generate_feature(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        if data.empty:
            return pd.Series(dtype=float, index=data.index, name=f'vectorbt_vw_sr_{self.window}')
        ...
        return vw_sr.rename(f'vectorbt_vw_sr_{self.window}')


class VectorBTSupportResistanceStrengthGenerator(VectorBTFeatureGenerator):
    ...
    def _create_default_config(cls, window: int = 20) -> FeatureConfig:
        return FeatureConfig(
            name=f"vectorbt_sr_strength_{window}",
            category=FeatureCategory.SUPPORT_RESISTANCE,
    ...
    def _generate_feature(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        if data.empty:
            return pd.Series(dtype=float, index=data.index, name=f'vectorbt_sr_strength_{self.window}')
        ...
        return sr_strength.rename(f'vectorbt_sr_strength_{self.window}')
...
class VectorBTDynamicSupportResistanceGenerator(VectorBTFeatureGenerator):
    ...
    def _generate_feature(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        return dynamic_sr.rename(f'vectorbt_dynamic_sr_{self.window}_{self.sensitivity}')
...
def create_vectorbt_support_resistance_generators() -> List[VectorBTFeatureGenerator]:
    ...
    for level in [1, 2, 3, 4, 5]:
        for window in [5, 10, 20]:
            generators.extend([
                VectorBTSupportLevelGenerator(level, window),
                VectorBTResistanceLevelGenerator(level, window),
            ])
    ...
    # Volume-weighted support/resistance
    for window in [5, 10, 20]:
        generators.append(VectorBTVolumeWeightedSupportResistanceGenerator(window))
    
    # Support/resistance strength
    for window in [5, 10, 20]:
        generators.append(VectorBTSupportResistanceStrengthGenerator(window))
