...
def run_walk_forward_analysis(full_df, params=None): # Changed default to None
    ...
    for i in range(num_windows):
        ...
        if testing_set.empty: continue
            
        report_lines.append(f"\n--- Window {i+1}/{num_windows} ---")
        report_lines.append(f"Testing:  {testing_set.index.min().date()} to {testing_set.index.max().date()}")
...
def run_monte_carlo_simulation(full_df, params=None): # Changed default to None
    ...
    report_lines.append(f"Simulating {MC_SIMULATIONS} equity curves by shuffling {len(trade_pnls)} trades...")
    ...
    report_lines.append(f"{CONFIDENCE_LEVEL*100}% Confidence Interval for Final Equity: ${lower_bound:,.2f} - ${upper_bound:,.2f}")
...
def plot_results(mc_curves, base_portfolio):
    ...
    for curve in mc_curves:
        fig.add_trace(go.Scatter(
            ...
            mode='lines', line=dict(color='rgba(173, 216, 230, 0.3)'),
            showlegend=False
        ), row=1, col=1)
    ...
    fig.add_trace(go.Scatter(
        x=list(range(len(drawdown))), y=drawdown,
