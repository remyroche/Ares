...
class CoarseOptimizer:
    ...
    def prune_features(self, top_n_percent: float = 0.5) -> list:
        ...
        features with the highest importance (using SHAP values).
        ...
        self.logger.info("🤖 Step 2: Training LightGBM model for feature importance...")
        ...
        if n_classes == 2:
            ...
            self.logger.info(f"   Configuring LightGBM for multi-class classification ({n_classes} classes)")
            lgb_model = lgb.LGBMClassifier(
                random_state=42,
        ...
        self.logger.info(f"   ✅ Model training completed in {model_duration:.2f} seconds")

        # Use SHAP for robust feature importance calculation
        self.logger.info("🔍 Step 3: Calculating SHAP values for feature importance...")
        ...
        if len(X) > sample_size:
            ...
            self.logger.info(f"   Using {sample_size} samples for SHAP computation (out of {len(X)} total)")
        else:
            X_sample = X
        ...
        self.logger.info(f"   ✅ SHAP computation completed in {shap_duration:.2f} seconds")
        ...
        if isinstance(shap_values, list):
            self.logger.info(f"   Multi-class classification detected: {len(shap_values)} classes")
        ...
        if hasattr(global_shap_values, '__len__'):
            ...
            importance_values = global_shap_values.tolist() if hasattr(global_shap_values, 'tolist') else list(global_shap_values)
            # Ensure all values are scalars, not lists
            importance_values = [float(val) if not isinstance(val, (list, np.ndarray)) else float(np.mean(val)) for val in importance_values]
            self.logger.info(f"   Processed {len(importance_values)} feature importance values")
        else:
            # Single scalar value
            importance_values = [float(global_shap_values)] * len(X.columns)
            self.logger.info(f"   Using single importance value for all {len(X.columns)} features")

        processing_duration = time.time() - processing_start_time
        self.logger.info(f"   ✅ SHAP processing completed in {processing_duration:.2f} seconds")

        # Create feature importance DataFrame
        self.logger.info("📈 Step 5: Creating feature importance ranking...")
        ranking_start_time = time.time()
        
        feature_importance = pd.DataFrame(
            list(zip(X.columns, importance_values)), columns=["feature", "importance"]
        )
        ...
        for i, (_, row) in enumerate(feature_importance.head(10).iterrows()):
            ...
            # Handle case where importance is a list (multi-class SHAP values)
            if isinstance(importance_val, list):
                importance_val = sum(importance_val)  # Sum across all classes
            else:
                importance_val = float(importance_val)
            self.logger.info(f"     {i+1:2d}. {feature_name:<30} {importance_val:.6f}")
        ...
        for i, (_, row) in enumerate(feature_importance.tail(10).iterrows()):
            ...
            if isinstance(importance_val, list):
                ...
                importance_val = float(importance_val)
            self.logger.info(f"     {len(feature_importance)-9+i:2d}. {feature_name:<30} {importance_val:.6f}")
        
        ranking_duration = time.time() - ranking_start_time
        self.logger.info(f"   ✅ Feature ranking completed in {ranking_duration:.2f} seconds")
        ...
        pruned_features = feature_importance.head(num_features_to_keep)["feature"].tolist()
        
        pruning_step_duration = time.time() - pruning_step_start_time
        self.logger.info(f"   ✅ Feature pruning completed in {pruning_step_duration:.2f} seconds")

        # Final summary
        total_duration = time.time() - pruning_start_time
        self.logger.info("🎉 FEATURE PRUNING PROCESS COMPLETED")
        self.logger.info(f"   Total duration: {total_duration:.2f} seconds")
        self.logger.info(f"   Features kept: {len(pruned_features)} out of {len(features)} ({len(pruned_features)/len(features)*100:.1f}%)")
        self.logger.info(f"   Features removed: {len(features) - len(pruned_features)}")
        self.logger.info(f"   Performance breakdown:")
        self.logger.info(f"     - Model training: {model_duration:.2f}s ({(model_duration/total_duration)*100:.1f}%)")
        self.logger.info(f"     - SHAP computation: {shap_duration:.2f}s ({(shap_duration/total_duration)*100:.1f}%)")
        self.logger.info(f"     - SHAP processing: {processing_duration:.2f}s ({(processing_duration/total_duration)*100:.1f}%)")
        self.logger.info(f"     - Feature ranking: {ranking_duration:.2f}s ({(ranking_duration/total_duration)*100:.1f}%)")
        self.logger.info(f"     - Feature pruning: {pruning_step_duration:.2f}s ({(pruning_step_duration/total_duration)*100:.1f}%)")
    ...
    def find_hyperparameter_ranges(
        ...
        study = optuna.create_study(
            direction="maximize", pruner=SuccessiveHalvingPruner()
        )
