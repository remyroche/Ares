...
class SRRegimeOptimizer:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.regime_performance_history: Dict[str, List[float]] = {'trending': [], 'ranging': [], 'transitional': []}
        self.regime_weights = {'trending': 1.0, 'ranging': 1.0, 'transitional': 1.0}
    ...
    @sr_error_handler(exceptions=(SROptimizationError, SRDataError), default_return=None, context='regime optimization', max_retries=2)
    async def optimize_regime_weights(self, market_data: pd.DataFrame, sr_levels: List[Dict[str, Any]], target_timeframe: str='15m') -> Optional[RegimeOptimizationResult]:
        ...
        try:
            ...
            regime_performance = await self._backtest_regime_performance(market_data, sr_levels, regime_sequence)
            ...
            result = RegimeOptimizationResult(regime_weights=self.regime_weights.copy(), regime_performance=regime_performance, optimization_score=self._calculate_optimization_score(regime_performance), backtest_periods=self.backtest_periods, adaptation_rate=self.adaptation_rate, memory_decay=self.memory_decay, total_samples=len(market_data))
    ...
    async def _backtest_regime_performance(self, market_data: pd.DataFrame, sr_levels: List[Dict[str, Any]], regime_sequence: List[str]) -> Dict[str, RegimePerformance]:
        ...
        try:
            ...
            for regime_type in ['trending', 'ranging', 'transitional']:
                ...
                performance = await self._calculate_regime_performance(regime_data, sr_levels, regime_type)
    ...
    async def _calculate_regime_performance(self, regime_data: pd.DataFrame, sr_levels: List[Dict[str, Any]], regime_type: str) -> RegimePerformance:
        ...
        try:
            ...
            return RegimePerformance(regime_type=regime_type, accuracy=accuracy, precision=precision, recall=recall, f1_score=f1_score, profit_factor=profit_factor, sharpe_ratio=sharpe_ratio, max_drawdown=max_drawdown, win_rate=win_rate, sample_count=len(interactions), confidence_interval=confidence_interval)
        except Exception as e:
            self.logger.error(f'Performance calculation failed for {regime_type}: {e}')
            return self._create_default_regime_performance(regime_type)

    async def _simulate_sr_interactions(self, regime_data: pd.DataFrame, sr_levels: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(len(regime_data) - 1):
                ...
                for level in sr_levels:
                    ...
                    if proximity < 0.02:
                        interaction = {'level_price': level_price, 'level_type': level_type, 'current_price': current_price, 'next_price': next_price, 'proximity': proximity, 'volume': regime_data['volume'].iloc[i], 'timestamp': regime_data.index[i]}
    ...
    async def _optimize_weights_from_performance(self, regime_performance: Dict[str, RegimePerformance]) -> Dict[str, float]:
        """Optimize regime weights based on performance."""
        try:
            optimized_weights = {}
            for regime_type, performance in regime_performance.items():
                performance_score = performance.accuracy * 0.3 + performance.f1_score * 0.3 + performance.profit_factor * 0.2 + performance.sharpe_ratio * 0.2
    ...
    def _calculate_optimization_score(self, regime_performance: Dict[str, RegimePerformance]) -> float:
        ...
        try:
            ...
            for regime_type, performance in regime_performance.items():
                ...
                regime_score = performance.accuracy * 0.4 + performance.f1_score * 0.3 + performance.profit_factor * 0.2 + performance.sharpe_ratio * 0.1
    ...
    def _create_default_regime_performance(self, regime_type: str) -> RegimePerformance:
        ...
        return RegimePerformance(regime_type=regime_type, accuracy=0.5, precision=0.5, recall=0.5, f1_score=0.5, profit_factor=1.0, sharpe_ratio=0.0, max_drawdown=0.0, win_rate=0.5, sample_count=0, confidence_interval=(0.0, 1.0))

    def _create_default_regime_performance_dict(self) -> Dict[str, RegimePerformance]:
        """Create default performance dictionary."""
        return {regime_type: self._create_default_regime_performance(regime_type) for regime_type in ['trending', 'ranging', 'transitional']}

    def _create_default_result(self) -> RegimeOptimizationResult:
        """Create default optimization result."""
        return RegimeOptimizationResult(regime_weights=self.regime_weights.copy(), regime_performance=self._create_default_regime_performance_dict(), optimization_score=0.5, backtest_periods=0, adaptation_rate=self.adaptation_rate, memory_decay=self.memory_decay, total_samples=0)
