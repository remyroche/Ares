...
class TradingPerformanceTracker(Protocol):
    ...
    @abstractmethod
    async def get_trade_history(self, limit: Optional[int] = None) -> List[TradeDecision]:
        """Get trade history."""
        ...
...
class TradingTactician(Protocol):
    ...
    @abstractmethod
    async def manage_positions(self, positions: List[PositionInfo]) -> List[TradeDecision]:
