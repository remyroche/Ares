...
def _cleanup_multiprocessing_resources() -> None:
...
def _generate_simple_hmm_report(exchange: str, symbol: str, timeframe: str, data_dir: str
) -> str:
...
def log_with_context(message: str, level: str = "info", context: str = "", **kwargs) -> None:
...
def print_and_log(message: str, level: str = "info", context: str = "", **kwargs) -> None:
...
def validate_required_artifacts(symbol: str, exchange: str, data_dir: str, timeframe: str
) -> dict[str, bool]:
...
def log_artifact_status(symbol: str, exchange: str, data_dir: str, timeframe: str, artifact_status: dict[str, bool], ):
...
class FeatureCache:
    ...
    def __init__(
        self, cache_dir: str = "data_cache/feature_cache", max_size_gb: float = 5.0, ttl_hours: int = 24
    ) -> None:
    ...
    def _save_metadata(self) -> None:
        """Save cache metadata."""
        try: with open(self.cache_metadata_file = "w") as f:
    ...
    def _generate_cache_key(
        self, symbol: str, exchange: str, timeframe: str, block_name: str, data_hash: str, ) -> str:
    ...
    def _remove_entry(self, key: str) -> None:
        ...
        try: cache_file = os.path.join(self.cache_dir, f"{key}.parquet")
    ...
    def get_cached_features(
        self, symbol: str, exchange: str, timeframe: str, block_name: str, data_hash: str, ) -> pd.DataFrame | None:
        """Retrieve cached features if available and valid."""
        if not HMM_OPTIMIZATION_CONFIG.get("enable_feature_caching", True):
        return None

        try: cache_key = self._generate_cache_key(
    ...
    def cache_features(
        self, features: pd.DataFrame, symbol: str, exchange: str, timeframe: str, block_name: str, data_hash: str, ) -> None:
...
class ModelCache:
    ...
    def __init__(
        self, cache_dir: str = "data_cache/model_cache", max_size_gb: float = 3.0, ttl_hours: int = 48
    ) -> None:
        self.cache_dir = cache_dir
        self.max_size_gb = max_size_gb
        self.ttl_seconds = ttl_hours * 3600
    ...
    def _generate_cache_key(
        self, symbol: str, exchange: str, timeframe: str, block_name: str, n_states: int, data_hash: str, config_hash: str, ) -> str:
    ...
    def _remove_entry(self, key: str) -> None:
        ...
        if key in self.metadata["entries"]:
        self.metadata["entries"][key]
        try: model_file = os.path.join(self.cache_dir, f"{key}_model.joblib")
    ...
    def get_cached_model(
        self, symbol: str, exchange: str, timeframe: str, block_name: str, n_states: int, data_hash: str, config_hash: str, ) -> tuple[Any, Any] | None:
    ...
    def cache_model(
        self, model: Any, scaler: Any, symbol: str, exchange: str, timeframe: str, block_name: str, n_states: int, data_hash: str, config_hash: str, ) -> None:
        ...
        if not os.path.exists(scaler_file) or os.path.getsize(scaler_file) == 0:
            ...
            print_and_log(
                "✅ Model and scaler files saved successfully", "debug", "ModelCache",
            )
...
@handle_data_processing_errors(default_return=pd.DataFrame())
def _select_block_features(full_df: pd.DataFrame, block: str, max_features: int
) -> pd.DataFrame:
...
def _fit_block_hmm_robust(X: pd.DataFrame, n_states: int, block_name: str = "unknown"
    ...
    try:
        ...
        if non_nan_mask.any():
            ...
            else: # If all values are NaN = use 0
                arr_clean[:, i] = 0.0
...
def _cluster_combinations(profile_df: pd.DataFrame, min_cluster_size: int, 5
) -> pd.Series:
...
@handle_data_processing_errors(default_return={})
def _state_feature_medians(X_block: pd.DataFrame, states: np.ndarray
...
def _name_states(block: str, medians: dict[int, dict[str, float]]) -> dict[int, str]:
    ...
    for s, feat in medians.items():
        ...
        elif block in {"momentum", "liquidity"}:
    ...
    for rank, (s, _sc) in enumerate(sorted_states):
        ...
        elif q < 0.25:  # 4 states, so 1/4, 0.25
...
def _process_single_block(blk: BlockConfig, blk_idx: int, total_blocks: int, block_features: dict[str, pd.DataFrame], features_df: pd.DataFrame, context: str, tf: str, symbol: str, exchange: str, ) -> tuple[str, bool, dict[str, Any], str, float]:
    """Process a single block for HMM training.
    This function is designed to be called in parallel.

    Returns: Tuple of (block_name = success, results_dict, message, processing_time)
    ...
    try:
        ...
        return blk.name, False, {}, f"exception: {e!s}", time.time() - blk_start_time
...
async def run_step(symbol: str, exchange: str = "BINANCE", data_dir: str = "data/training", timeframe: str = "1m", lookback_days: int | None = None, force: bool = False
    **kwargs: Any, ) -> bool:
    ...
    for tf in timeframes_to_process:
        ...
        try: success = await implement_hmm_regime_discovery(
                symbol=symbol
                exchange=exchange
                timeframe=tf
                data_dir=data_dir
        ...
        for cluster_id, count in cluster_stats.items():
                ...
                logger.exception(f"❌ Failed to generate composite clusters for {tf}: {e}")
                print_and_log(
        ...
        try: with open(report_path = "w") as f:
                        f.write(report)
    ...
    for tf in timeframes_to_process:
        ...
        with open(report_path, "w") as f:
                ...
                logger.exception(f"❌ ERROR details: {type(report_error).__name__}: {report_error!s}")
                import traceback
            ...
            logger.exception(f"❌ ERROR details: {type(e).__name__}: {e!s}")
            import traceback
            logger.exception(f"❌ ERROR traceback: {traceback.format_exc()}")
            print_and_log(
...
@handle_data_processing_errors(default_return=pd.Series([-1] * 1000))
def _merge_rare_combinations(combo_keys: pd.Series, profile_df: pd.DataFrame, min_count: int = 3, # Reduced from 5 to 3 to make penalty weaker, similarity_threshold: float = 0.65, # Reduced from 0.7 to 0.65 to make penalty weaker
    ...
    Returns: Tuple of (merged_combo_keys = merged_profile_df)
...
def _generate_archetype_descriptions(cluster_centroids: dict[int, list[float]], state_names: dict[str, dict[str, str]], block_posteriors: dict[str, pd.DataFrame], cluster_counts: dict[int, int], ) -> dict[int, str]:
...
def _compute_comprehensive_composite_metrics(profile_df: pd.DataFrame, labels: pd.Series, cluster_centroids: dict[int, list[float]], features_df: pd.DataFrame, block_states: dict[str, pd.Series], block_posteriors: dict[str, pd.DataFrame], cluster_series: pd.Series, state_names: dict[str, dict[str, str]], block_features: dict[str, list[str]], ) -> CompositeModelMetrics:
    ...
    if valid_mask.sum() > 1 and len(np.unique(labels[valid_mask])) > 1:
        try: silhouette = silhouette_score(profile_df[valid_mask], labels[valid_mask])
    ...
    cluster_persistence, dict.fromkeys(cluster_sizes.keys(), 1.0)
    cluster_volatility, dict.fromkeys(cluster_sizes.keys(), 0.1)
    ...
    regime_stability, dict.fromkeys(cluster_sizes.keys(), 0.8)
...
def _generate_state_name(block_name: str, state_id: int, original_name: str, meta: dict | None = None) -> str:
    ...
    try:
        ...
        if block_name_lower == "microstructure":
...
def _generate_simple_hmm_report(exchange: str, symbol: str, timeframe: str, data_dir: str
    ...
    try:
        ...
        if block_name in state_names and str(state_id) in state_names[block_name]:
                        ...
                        else: description = _generate_state_name(block_name, state_id, f"State {state_id}", meta)
        ...
        if block_name in state_names and str(dominant_state) in state_names[block_name]:
                        ...
                        else: dominant_desc = _generate_state_name(block_name, dominant_state, f"State {dominant_state}", meta)
        ...
        logger.exception(f"❌ ERROR in _generate_simple_hmm_report for {timeframe}: {e!s}")
        logger.exception(f"❌ ERROR traceback: {traceback.format_exc()}")
        return f"# HMM Regime Report\n\nError generating report: {e!s}\n\nTraceback: {traceback.format_exc()}"


def _generate_comprehensive_metrics_report(metrics: CompositeModelMetrics, symbol: str, timeframe: str, exchange: str
) -> str:
...
async def _calculate_and_integrate_hmm_features(symbol: str, exchange: str, timeframe: str, data_dir: str, hmm_output_dir: Path, ) -> None:
...
@auto_fix_data_quality_issues
async def run_step_enhanced(symbol: str, exchange: str = "BINANCE", data_dir: str = "data/training", timeframe: str = "1m", lookback_days: int | None = None, force_rerun: bool = False
    **kwargs: Any, ) -> bool:
...
async def implement_hmm_regime_discovery(symbol: str, exchange: str, timeframe: str, data_dir: str, force: bool = False
) -> bool:
