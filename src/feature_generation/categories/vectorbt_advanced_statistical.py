...
class VectorBTCVaRGenerator(VectorBTFeatureGenerator):
    ...
    def _generate_feature(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        # Use VectorBT for optimized rolling operations
        cvar = self._calculate_cvar_vectorbt(returns)
        
        return cvar.rename(f'vectorbt_cvar_{self.window}_{self.confidence_level}')
...
class VectorBTMaxDrawdownGenerator(VectorBTFeatureGenerator):
    ...
    def _generate_feature(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        # Use VectorBT for optimized rolling operations
        max_drawdown = self._calculate_max_drawdown_vectorbt(close)
        
        return max_drawdown.rename(f'vectorbt_max_drawdown_{self.window}')
...
class VectorBTRollingSkewnessKurtosisGenerator(VectorBTFeatureGenerator):
    ...
    def _generate_feature(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        # Use VectorBT for optimized rolling operations
        rolling_stats = self._calculate_rolling_stats_vectorbt(returns)
        
        return rolling_stats.rename(f'vectorbt_rolling_{self.stat_type}_{self.window}')
...
class VectorBTTrendPersistenceGenerator(VectorBTFeatureGenerator):
    """VectorBT-optimized trend persistence generator (run length and fraction of up bars)."""
    ...
    def _generate_feature(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        # Use VectorBT for optimized rolling operations
        trend_persistence = self._calculate_trend_persistence_vectorbt(returns)
        
        return trend_persistence.rename(f'vectorbt_trend_persistence_{self.window}')
