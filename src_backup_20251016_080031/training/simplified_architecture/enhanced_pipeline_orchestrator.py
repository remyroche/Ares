...
class PipelineResult:
    ...
    def failed_steps(self) -> List[str]:
        ...
        return [name for name, result in self.step_results.items() if result.status == StepStatus.FAILED]

    def to_dict(self) -> Dict[str, Any]:
        """Convert result to dictionary for serialization."""
        return {'status': self.status.value, 'start_time': self.start_time.isoformat() if self.start_time else None, 'end_time': self.end_time.isoformat() if self.end_time else None, 'duration': self.duration, 'execution_id': self.execution_id, 'pipeline_name': self.pipeline_name, 'pipeline_version': self.pipeline_version, 'failed_steps': self.failed_steps, 'errors': self.errors, 'warnings': self.warnings, 'metrics': self.metrics, 'artifacts': {k: str(v) for k, v in self.artifacts.items()}, 'step_results': {k: v.to_dict() for k, v in self.step_results.items()}}
...
class EnhancedPipelineOrchestrator:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {'status': self._current_execution.status.value, 'execution_id': self._current_execution.execution_id, 'start_time': self._current_execution.start_time.isoformat() if self._current_execution.start_time else None, 'duration': self._current_execution.duration, 'completed_steps': len([r for r in self._current_execution.step_results.values() if r.is_success]), 'total_steps': len(self.config.steps), 'failed_steps': self._current_execution.failed_steps, 'is_running': self._is_running}
    ...
    def _determine_execution_order(self, start_from_step: Optional[str]=None, stop_at_step: Optional[str]=None) -> List[StepConfiguration]:
        ...
        for step in self.config.steps:
            if step.enabled and step.name not in visited:
                visit(step.name)
        if start_from_step:
            start_index = next((i for i, step in enumerate(execution_order) if step.name == start_from_step), None)
            if start_index is not None:
                execution_order = execution_order[start_index:]
        if stop_at_step:
            stop_index = next((i for i, step in enumerate(execution_order) if step.name == stop_at_step), None)
            if stop_index is not None:
                execution_order = execution_order[:stop_index + 1]
        return execution_order

    async def _execute_steps_sequential(self, execution_order: List[StepConfiguration], checkpoint_interval: int) -> None:
    ...
    async def _execute_steps_parallel(self, execution_order: List[StepConfiguration], checkpoint_interval: int) -> None:
