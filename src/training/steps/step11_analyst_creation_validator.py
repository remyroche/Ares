...
class Step11AnalystCreationValidator:
    ...
    @handle_errors(
        ...
        context="step11analystcreationvalidator initialization",
    )
    async def initialize(self) -> bool:
        """Initialize Step11AnalystCreationValidator."""
        try:
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passself.logger.warning(
        ...
        if not regime_dirs:
    passpassself.logger.warning("⚠️ No regime - specific analyst model directories found")
        return False
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passpasspasspasspasspasspassself.logger.exception(f"❌ Step 11 validation failed: {e}")
        return False
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passimport joblib
        ...
        if model is None:
    passself.logger.warning(f"⚠️ Model file is empty: {model_file.name}")
        return False

        self.logger.info(f"✅ Model file validated: {model_file.name} ({file_size} bytes)")
        return True
        except Exception as e:
    passpasspasspasspasspasspassself.logger.warning(f"⚠️ Could not load model file {model_file.name}: {e}")
        return False

        except Exception as e:
    passpasspasspasspasspasspassself.logger.exception(f"❌ Error validating model file {model_file}: {e}")
        return False
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passpasspasspasspasspasspassself.logger.exception(f"❌ Error validating metadata file {metadata_file}: {e}")
        return False
...
def step11_analyst_creation_validator(...) -> ...:
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
        ...
        if result:
    passlogger.info("✅ Step 11: Analyst Creation validation passed")
        return True
        else:
    passlogger.warning("⚠️ Step 11: Analyst Creation validation failed")
        return False

    except Exception as e:
    passpasspasspasspasspasspasslogger.exception(f"❌ Step 11 validation failed: {e}")
        return False