...
from src.tactician.sr_data_integration_simple import create_sr_data_integration_simple
...
class SROptimizationStep:
            ...
            if self.sr_levels_manager:
                try:
                    ...
                    if market_data is not None:
                        ...
                        self.logger.info(
                            f"✅ Calculated SR levels: {
                                len(
                                    sr_levels_result.get(
                                        'support_levels',
                                        []))} support, {
                                len(
                                    sr_levels_result.get(
    ...
    async def _generate_detailed_optimization_reports(
        self, optimization_result: Any, sr_analysis_reports: dict[str, Any], sr_integration_analysis: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _analyze_sr_strength(self, market_data: pd.DataFrame, sr_context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            analysis = {
                "support_strength_distribution": {},
                "resistance_strength_distribution": {},
    ...
    async def _analyze_sr_proximity(self, market_data: pd.DataFrame, sr_context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            analysis = {"proximity_distribution": {}, "proximity_trends": {}, "proximity_volatility": {}}
            ...
            if "support_proximity" in sr_context:
                analysis["proximity_distribution"]["support"] = {
                    ...
                    "proximity_percentile": self._calculate_proximity_percentile(market_data, current_price, "support"),
                }

            if "resistance_proximity" in sr_context:
                analysis["proximity_distribution"]["resistance"] = {
                    "current_proximity": sr_context["resistance_proximity"],
    ...
    async def _analyze_sr_breakouts(self, market_data: pd.DataFrame, sr_context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            analysis = {"breakout_probability": {}, "breakout_confidence": {}, "breakout_volume_analysis": {}}
    ...
    def _calculate_proximity_percentile(
        self, market_data: pd.DataFrame, current_price: float, level_type: str
    ) -> float:
    ...
    async def _generate_method_effectiveness_report(self, optimization_result: Any) -> dict[str, Any]:
        ...
        try:
            ...
            report = {"method_performance": {}, "method_ranking": {}, "method_recommendations": {}}
        ...
        try:
            ...
            if comparison_metrics:
                ...
                analysis["level_analysis"] = {
                    "price_approach": {
                        "support_levels": comparison_metrics.get("price_vs_vwap", {})
                        .get("support_levels", {})
                        .get("price_count", 0),
                        "resistance_levels": comparison_metrics.get("price_vs_vwap", {})
                        .get("resistance_levels", {})
                        .get("price_count", 0),
                        "avg_strength": comparison_metrics.get("price_vs_vwap", {})
                        .get("support_levels", {})
                        .get("price_avg_strength", 0),
                        "avg_confidence": comparison_metrics.get("price_vs_vwap", {})
                        .get("support_levels", {})
                        .get("price_avg_confidence", 0),
                    },
                    "vwap_approach": {
                        "support_levels": comparison_metrics.get("price_vs_vwap", {})
                        .get("support_levels", {})
                        ...
                        "avg_strength": comparison_metrics.get("price_vs_vwap", {})
                        .get("support_levels", {})
                        .get("vwap_avg_strength", 0),
                        "avg_confidence": comparison_metrics.get("price_vs_vwap", {})
                        .get("support_levels", {})
                        .get("vwap_avg_confidence", 0),
                    },
                }
        ...
        try:
            ...
            if "level_quality" in analysis.get("performance_metrics", {}):
                ...
                if abs(price_score - vwap_score) < 0.05:
                    ...
                    insights.append(
                        f"Price approach outperforms VWAP approach (score: {
                            price_score:.3f} vs {
                            vwap_score:.3f})"
                    )
                else:
                    insights.append(
                        f"VWAP approach outperforms price approach (score: {
                            vwap_score:.3f} vs {
                            price_score:.3f})"
                    )
    ...
    async def _generate_integration_analysis_report(
        self, sr_analysis_reports: dict[str, Any], sr_integration_analysis: dict[str, Any]
    ) -> dict[str, Any]:
            ...
            for result in valid_results:
                ...
                if hasattr(result, "optimization_score"):
                    combined_result["performance_metrics"]["optimization_score"] = max(
                        combined_result["performance_metrics"]["optimization_score"], result.optimization_score
                    )
                ...
                if hasattr(result, "win_rate"):
                    combined_result["performance_metrics"]["win_rate"] = max(
                        combined_result["performance_metrics"]["win_rate"], result.win_rate
                    )
    ...
    async def _update_config_with_optimized_params(self, optimization_result: Any) -> bool:
        ...
        try:
            ...
            if hasattr(optimization_result, "method_weights"):
                ...
                ] = optimization_result.method_weights
            if hasattr(optimization_result, "strength_weights"):
                self.config["sr_detection_optimization"][
                    "optimized_strength_weights"
                ] = optimization_result.strength_weights
            if hasattr(optimization_result, "dbscan_params"):
            ...
            if hasattr(optimization_result, "timeframe_weights"):
                ...
                ] = optimization_result.timeframe_weights
            if hasattr(optimization_result, "advanced_params"):
                self.config["sr_detection_optimization"][
                    "optimized_advanced_params"
                ] = optimization_result.advanced_params
        ...
        try:
            ...
            self.logger.info(
                f"📊 Optimization score: {
                    comprehensive_report['optimization_summary']['optimization_score']:.4f}"
            )
            ...
            self.logger.info(
                f"📋 Total reports generated: {
                    comprehensive_report['optimization_summary']['total_reports_generated']}"
            )
            self.logger.info(
                f"🔗 SR analysis reports: {len(comprehensive_report['sr_analysis_summary']['analysis_reports'])}"
            )
            self.logger.info(
                f"📊 Detailed reports: {len(comprehensive_report['detailed_reports_summary']['report_types'])}"
            )
            self.logger.info("=" * 80)
...
@handle_errors(exceptions=(Exception,), default_return=False, context="step2_5_sr_optimization")
@secure_step_execution
