...
class FeatureEngineeringValidator:
    ...
    def _validate_feature_completeness(self, original_df: pd.DataFrame, features_df: pd.DataFrame, feature_config: dict[str, Any], result: ValidationResult, summary: dict[str, Any]) -> None:
    ...
    def _validate_feature_ranges(self, features_df: pd.DataFrame, result: ValidationResult, summary: dict[str, Any]) -> None:
    ...
    def _analyze_nan_propagation(self, original_df: pd.DataFrame, features_df: pd.DataFrame) -> dict[str, Any]:
        ...
        for column in features_df.columns:
            if column not in original_df.columns:
                ...
                if nan_percentage > 50:
                    excessive_nan_features.append({'feature': column, 'nan_percentage': float(nan_percentage), 'nan_count': int(feature_nan_counts[column])})
            ...
            try:
                ...
                if pd.api.types.is_numeric_dtype(expected_values):
                    ...
                    if close_matches:
                        ...
                        validation_results['failed_validations'].append({'feature': feature_name, 'max_difference': float(max_diff), 'sample_size': len(sample_indices)})
                else:
                    matches = (expected_values == actual_values).all()
                    if matches:
                        validation_results['validated_features'].append(feature_name)
                    else:
                        validation_results['failed_validations'].append({'feature': feature_name, 'type': 'non_numeric', 'match_percentage': float((expected_values == actual_values).mean() * 100)})
            except Exception as e:
                validation_results['skipped_features'].append({'feature': feature_name, 'error': str(e)})
        return validation_results

    def _validate_feature_dependencies(self, features_df: pd.DataFrame) -> dict[str, Any]:
        """Validate logical dependencies between features."""
        inconsistencies = []
        if all((col in features_df.columns for col in ['high', 'low', 'close'])):
            invalid_hlc = (features_df['high'] < features_df['low']) | (features_df['close'] > features_df['high']) | (features_df['close'] < features_df['low'])
            if invalid_hlc.any():
                inconsistencies.append({'type': 'ohlc_relationship', 'invalid_count': int(invalid_hlc.sum()), 'invalid_percentage': float(invalid_hlc.mean() * 100)})
        if 'returns' in features_df.columns and 'log_returns' in features_df.columns:
            ...
            if (log_return_diff > 0.001).any():
                inconsistencies.append({'type': 'return_consistency', 'max_difference': float(log_return_diff.max()), 'inconsistent_count': int((log_return_diff > 0.001).sum())})
        ...
        if len(ma_columns) >= 2:
            ...
            for ma1, ma2 in ma_pairs[:5]:
                std1 = features_df[ma1].std()
                std2 = features_df[ma2].std()
                if self._extract_ma_period(ma1) < self._extract_ma_period(ma2):
                    if std1 < std2 * 0.9:
                        inconsistencies.append({'type': 'ma_volatility', 'features': [ma1, ma2], 'std_values': [float(std1), float(std2)]})
        return {'inconsistent_features': inconsistencies, 'consistency_score': 1.0 - min(len(inconsistencies) * 0.1, 1.0)}

    def _validate_feature_relevance(self, features_df: pd.DataFrame) -> dict[str, Any]:
            ...
            for i in range(len(numeric_features)):
                for j in range(i + 1, len(numeric_features)):
                    ...
                    if abs(corr_value) > 0.95:
                        highly_correlated_pairs.append({'feature1': numeric_features[i], 'feature2': numeric_features[j], 'correlation': float(corr_value)})
        return {'zero_variance_features': zero_variance_features, 'constant_features': constant_features, 'highly_correlated_pairs': highly_correlated_pairs[:10], 'redundancy_score': len(highly_correlated_pairs) / max(len(numeric_features), 1)}

    def _check_feature_leakage(self, original_df: pd.DataFrame, features_df: pd.DataFrame) -> dict[str, Any]:
        ...
        for column in features_df.columns:
            if any((keyword in column.lower() for keyword in future_keywords)):
                if column not in ['target', 'label', 'y']:
                    potential_leakage.append({'feature': column, 'reason': 'suspicious_name', 'keywords_found': [k for k in future_keywords if k in column.lower()]})
    ...
    def _identify_nan_sources(self, feature_name: str, original_df: pd.DataFrame, features_df: pd.DataFrame) -> list[str]:
    ...
    def register_custom_calculation(self, feature_name: str, calculation_func: Callable[[pd.DataFrame], pd.Series]) -> None:
