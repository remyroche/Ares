...
class ComprehensiveSRTrainingPipeline:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # Output paths
        self.step7_output_path = config.get(
            "step7_output_path", "data/matrix_operations"
        )
        self.step2_5_output_path = config.get(
    ...
    async def execute_comprehensive_training(
        ...
        try:
            ...
            self.logger.info(
                f"🚀 Starting comprehensive SR training for {symbol} on {exchange}"
            )
            ...
            if not step7_success:
                self.logger.warning(
                    "⚠️ Step7 features not loaded, continuing with available features"
                )
            ...
            if not step2_5_success:
                self.logger.warning(
                    "⚠️ Step2_5 SR levels not loaded, continuing with available features"
                )

            # Step 3: Prepare comprehensive training data
            comprehensive_data = await self._prepare_comprehensive_training_data(
                training_data
            )
            ...
            self.logger.info(
                f"✅ Comprehensive SR training completed in {execution_time}"
            )
            ...
            self.logger.exception(f"❌ Comprehensive SR training failed: {e}")
    ...
    async def _load_step7_features(self) -> bool:
        ...
        try:
            ...
            success = await self.model_trainer.load_step7_features(
                self.step7_output_path
            )

            if success:
                self.step7_features_loaded = True
                self.logger.info(
                    f"✅ Step7 features loaded: {len(self.model_trainer.step7_features)} features"
                )
            else:
            ...
            self.logger.exception(f"❌ Error loading step07 features: {e}")
            return False
    ...
    async def _load_step2_5_sr_levels(self) -> bool:
        ...
        try:
            ...
            success = await self.model_trainer.load_step2_5_sr_levels(
                self.step2_5_output_path
            )

            if success:
                self.step2_5_sr_levels_loaded = True
                support_levels = self.model_trainer.step2_5_sr_levels.get(
                    "support_levels", []
                )
                resistance_levels = self.model_trainer.step2_5_sr_levels.get(
                    "resistance_levels", []
                )
                self.logger.info(
                    f"✅ Step2_5 SR levels loaded: {len(support_levels)} support, {len(resistance_levels)} resistance"
                )
            else:
            ...
            self.logger.exception(f"❌ Error loading step2_5 SR levels: {e}")
            return False
    ...
    async def _prepare_comprehensive_training_data(
        ...
        try:
            ...
            comprehensive_data = (
                await self.model_trainer._add_comprehensive_sr_features(training_data)
            )

            # Validate feature completeness
            missing_features = self.model_trainer.validate_feature_completeness(
                comprehensive_data
            )
            if missing_features:
            ...
            sr_features = [
                col for col in comprehensive_data.columns if "sr_" in col.lower()
            ]
            self.logger.info("📊 Comprehensive data prepared:")
            ...
            self.logger.exception(
                f"❌ Error preparing comprehensive training data: {e}"
            )
        ...
        try:
            ...
            for model_type in model_types:
                try:
                    ...
                    self.logger.exception(f"❌ Error training {model_type} model: {e}")

            return training_results

        except Exception as e:
            self.logger.exception(
                f"❌ Error training models with comprehensive features: {e}"
            )
            return {}
        ...
        try:
            ...
            for model_type, result in training_results.items():
                if "sr_feature_analysis" in result:
                    validation_results["sr_feature_analysis"][model_type] = result[
                        "sr_feature_analysis"
                    ]

                if "feature_importance" in result:
                    validation_results["model_performance"][model_type] = {
                        "direction_accuracy": result.get("direction_metrics", {}).get(
                            "accuracy", 0.0
                        ),
                        "profit_r2": result.get("profit_metrics", {}).get(
                            "r2_score", 0.0
                        ),
            ...
            output_file = (
                Path(self.training_output_path)
                / "comprehensive_sr_training_results.json"
            )
            ...
            self.logger.exception(f"❌ Error validating and saving results: {e}")
            return {"error": str(e)}
        ...
        try:
            return {
                ...
                "step2_5_sr_levels": {
                    ...
                    "support_levels": len(
                        self.model_trainer.step2_5_sr_levels.get("support_levels", [])
                    ),
                    "resistance_levels": len(
                        self.model_trainer.step2_5_sr_levels.get(
                            "resistance_levels", []
                        )
                    ),
                },
...
async def run_comprehensive_sr_training(
    ...
    return await pipeline.execute_comprehensive_training(
