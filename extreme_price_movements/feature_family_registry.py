...
def get_feature_family(feature_name: str) -> FeatureFamily:
    ...
    if registry_family in {"trend", "volatility", "curvature", "compression", "vol_of_vol", "persistence_dwell", "structure"}:
    ...
    if lowered.startswith(("loc_", "dist_", "zscore_")):
        return FeatureFamily.BOUNDED_GEOMETRY
    if any(token in lowered for token in ("pct", "ratio", "score", "entropy", "vol", "atr", "slope", "ret", "range", "compression", "trend")):
        return FeatureFamily.RISK_NORMALIZED_CONTINUOUS
