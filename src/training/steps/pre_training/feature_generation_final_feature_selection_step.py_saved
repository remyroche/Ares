...
class FeatureGenerationFinalFeatureSelectionStep(BaseStep):
    ...
    def _perform_multi_size_selection(self, features_df: pd.DataFrame, targets: pd.Series, config: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        # Determine trading mode (default: long)
        ...
        if 'target_long' in features_df.columns and 'target_short' in features_df.columns:
            # Select appropriate target based on trading mode
            ...
            # Log target statistics for selected mode
            target_col = target_cols[0]
            signals = (features_df[target_col] > 0).sum()
            tprint_info(f"📊 Target statistics: {target_col}={signals} signals ({signals/len(features_df)*100:.1f}%)")
        else:
            # Fall back to legacy target detection
            ...
            try:
                ...
                for size in feature_set_sizes:
                    ...
                    if result_key in batch_results.columns:
                        ...
                        feature_sets[f'selected_feature_dataframe_{actual_feature_count}'] = features_df[selected_features + target_cols].copy()
                        tprint_info(f"✅ Batch: {actual_feature_count} features (requested {size})")
        ...
        for size in feature_set_sizes:
            ...
            # Also create the corresponding dataframes with actual feature count in name
            actual_feature_count = len(selected_features)
            feature_sets[f'selected_feature_dataframe_{actual_feature_count}'] = features_df[selected_features + target_cols].copy()
    ...
    def _perform_cmi_aware_selection(self, features_df: pd.DataFrame, targets: pd.Series, 
        ...
        try:
            ...
            for size in feature_set_sizes:
                ...
                # Use actual feature count in dataset name
                actual_feature_count = len(selected_features)
                feature_sets[f'selected_feature_dataframe_{actual_feature_count}'] = features_df[selected_features + target_cols].copy()
                tprint_info(f"✅ CMI: {actual_feature_count} features (requested {size})")
    ...
    def _perform_standard_selection(self, features_df: pd.DataFrame, targets: pd.Series, 
        ...
        for size in feature_set_sizes:
            ...
            actual_feature_count = len(selected_features)
            feature_sets[f'selected_feature_dataframe_{actual_feature_count}'] = features_df[selected_features + target_cols].copy()
            tprint_info(f"✅ Standard: {actual_feature_count} features (requested {size})")
