...
class EfficientFeaturesDatabase:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.storage_format = self.db_config.get("storage_format", "pickle")  # pickle, parquet, hdf5
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"❌ Error initializing EfficientFeaturesDatabase: {e}")
            return False
    ...
    def _generate_database_name(
        ...
        db_name = f"{clean_symbol}_{clean_exchange}_{start_date}_{timestamp}_historical_data_with_precomputed_features"
        return db_name
        ...
        try:
            ...
            for filename in os.listdir(self.base_storage_dir):
                if filename.endswith(('.pkl', '.parquet', '.h5')) and 'precomputed_features' in filename:
                    ...
                    if len(parts) >= 6:  # Expected format: TOKEN_EXCHANGE_DATE_TIMESTAMP_historical_data_with_precomputed_features
                        symbol = parts[0]
                        exchange = parts[1]
                        date = parts[2]
                        timestamp = parts[3]
            ...
            self.logger.error(f"Error scanning existing databases: {e}")
            return []
    ...
    async def _get_database_info(self, db_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning(f"Error reading database info: {e}")
            return {}
    ...
    async def find_existing_database(
    ...
    ) -> Tuple[Optional[str], List[Tuple[pd.Timestamp, pd.Timestamp]]]:
        """
        ...
        try:
            ...
            for db_name, metadata in self.metadata_cache.items():
                if (metadata.get("symbol", "").upper() == clean_symbol and 
                    metadata.get("exchange", "").upper() == clean_exchange):
            ...
            self.logger.error(f"Error finding existing database: {e}")
    ...
    async def load_database(self, database_name: str) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.error(f"Error loading database {database_name}: {e}")
            return pd.DataFrame()
    ...
    async def update_database(
        ...
        try:
            if new_data.empty:
                self.logger.warning("No new data to update database")
                return True
            ...
            success = await self._save_database_with_timestamp_update(
                combined_data, symbol, exchange, existing_database_name
            )
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        symbols = set(meta.get("symbol") for meta in self.metadata_cache.values())
        exchanges = set(meta.get("exchange") for meta in self.metadata_cache.values())
    ...
    async def cleanup_old_databases(self, keep_latest_n: int = 5) -> None:
        ...
        try:
            ...
            deleted_count = 0
            for (symbol, exchange), databases in symbol_exchange_groups.items():
                # Sort by last modified date (newest first)
