...
class Step10TacticianEnsembleCreationValidator(BaseValidator):
    ...
    async def validate(
        ...
        if not error_passed:
            self.print(error("❌ Tactician ensemble creation step had errors"))
            return False

        # 2. Validate tactician ensemble files existence
        ensemble_files_passed = self._validate_tactician_ensemble_files(
            symbol,
            exchange,
            data_dir,
        )
        if not ensemble_files_passed:
            self.print(failed("❌ Tactician ensemble files validation failed"))
            return False

        # 3. Validate tactician ensemble diversity
        diversity_passed = self._validate_tactician_ensemble_diversity(
            symbol,
            exchange,
            data_dir,
        )
        if not diversity_passed:
            self.print(failed("❌ Tactician ensemble diversity validation failed"))
            return False

        # 4. Validate tactician ensemble performance
        performance_passed = self._validate_tactician_ensemble_performance(
            symbol,
            exchange,
            data_dir,
        )
        if not performance_passed:
            self.print(failed("❌ Tactician ensemble performance validation failed"))
            return False

        # 5. Validate tactician ensemble stability
        stability_passed = self._validate_tactician_ensemble_stability(
            symbol,
            exchange,
            data_dir,
        )
        if not stability_passed:
            self.print(failed("❌ Tactician ensemble stability validation failed"))
            return False
        ...
        if not outcome_passed:
            self.logger.warning(
                "⚠️ Tactician ensemble creation outcome is not favorable",
            )
            return False
    ...
    def _validate_tactician_ensemble_files(
        self,
        symbol: str,
        exchange: str,
        data_dir: str,
        ...
        try:
            ...
            if missing_files:
                self.logger.error(
                    f"❌ Missing tactician ensemble files: {missing_files}",
                )
                return False
            ...
            self.print(error("❌ Error validating tactician ensemble files: {e}"))
            return False

    def _validate_tactician_ensemble_diversity(
        self,
        symbol: str,
        exchange: str,
        data_dir: str,
        ...
        try:
            ...
            if os.path.exists(metadata_file):
                ...
                if "model_count" in metadata:
                    ...
                    if model_count < 3:
                        self.logger.warning(
                            f"⚠️ Few tactician models in ensemble: {model_count}",
                        )
                    elif model_count > 20:
                        self.logger.warning(
                            f"⚠️ Many tactician models in ensemble: {model_count}",
                        )
                ...
                if "model_types" in metadata:
                    ...
                    if len(unique_types) < 2:
                        self.logger.warning(
                            f"⚠️ Low tactician model type diversity: {len(unique_types)} types",
                        )
                ...
                if os.path.exists(weights_file):
                    ...
                    if weights:
                        ...
                        if weight_std < 0.01:  # Very uniform weights
                            self.logger.warning(
                                "⚠️ Very uniform tactician ensemble weights",
                            )
                        ...
                        if max_weight > 0.8:  # Single model dominates
                            self.logger.warning(
                                f"⚠️ Single tactician model dominates ensemble: {max_weight:.3f}",
                            )

                        if min_weight < 0.01:  # Very small weights
                            self.logger.warning(
                                f"⚠️ Very small tactician ensemble weights: {min_weight:.3f}",
                            )
            ...
            self.logger.exception(
                f"❌ Error during tactician ensemble diversity validation: {e}",
            )
            return False

    def _validate_tactician_ensemble_performance(
        self,
        symbol: str,
        exchange: str,
        data_dir: str,
        ...
        try:
            ...
            if os.path.exists(metadata_file):
                ...
                if "ensemble_accuracy" in metadata:
                    ...
                    if not acc_passed:
                        self.logger.error(
                            f"❌ Tactician ensemble accuracy too low: {ensemble_acc:.3f}",
                        )
                        return False
                ...
                if "ensemble_loss" in metadata:
                    ...
                    if not loss_passed:
                        self.logger.error(
                            f"❌ Tactician ensemble loss too high: {ensemble_loss:.3f}",
                        )
                        return False
                ...
                if "signal_prediction_accuracy" in metadata:
                    ...
                    if signal_acc < 0.6:
                        self.logger.warning(
                            f"⚠️ Low tactician ensemble signal prediction accuracy: {signal_acc:.3f}",
                        )
                ...
                if "individual_model_performance" in metadata:
                    ...
                    if (
                        poor_models > len(individual_perf) * 0.5
                    ):  # More than 50% poor models
                        self.logger.warning(
                            f"⚠️ Many poor performing tactician models: {poor_models}/{len(individual_perf)}",
                        )

                # Check ensemble vs individual performance
                if (
                    "ensemble_accuracy" in metadata
                    and "individual_model_performance" in metadata
                ):
                    ensemble_acc = metadata["ensemble_accuracy"]
                    individual_accs = [
                        p.get("accuracy", 0)
                        for p in metadata["individual_model_performance"]
                    ]
                    ...
                    if individual_accs:
                        ...
                        if ensemble_improvement < 0:
                            self.logger.warning(
                                "⚠️ Tactician ensemble performs worse than average individual model",
                            )
                        elif ensemble_improvement < 0.01:
                            self.logger.warning(
                                f"⚠️ Minimal tactician ensemble improvement: {ensemble_improvement:.3f}",
                            )
            ...
            self.logger.exception(
                f"❌ Error during tactician ensemble performance validation: {e}",
            )
            return False

    def _validate_tactician_ensemble_stability(
        self,
        symbol: str,
        exchange: str,
        data_dir: str,
        ...
        try:
            ...
            if os.path.exists(metadata_file):
                ...
                if "stability_score" in metadata:
                    ...
                    if stability_score < 0.7:
                        self.logger.warning(
                            f"⚠️ Low tactician ensemble stability: {stability_score:.3f}",
                        )
                ...
                if "prediction_variance" in metadata:
                    ...
                    if pred_variance > 0.1:
                        self.logger.warning(
                            f"⚠️ High tactician ensemble prediction variance: {pred_variance:.3f}",
                        )
                ...
                if "model_correlation" in metadata:
                    ...
                    if model_corr > 0.9:
                        self.logger.warning(
                            f"⚠️ High tactician model correlation: {model_corr:.3f}",
                        )
                ...
                if "robustness_score" in metadata:
                    ...
                    if robustness_score < 0.6:
                        self.logger.warning(
                            f"⚠️ Low tactician ensemble robustness: {robustness_score:.3f}",
                        )
            ...
            if os.path.exists(ensemble_file):
                try:
                    ...
                    if callable(getattr(ensemble, "predict", None)):
                        self.logger.info("✅ Tactician ensemble has predict method")
                    else:
                        self.logger.error(
                            "❌ Tactician ensemble missing predict method",
                        )
                        return False
                    ...
                    if hasattr(ensemble, "estimators_"):
                        ...
                        if estimator_count < 3:
                            self.logger.warning(
                                f"⚠️ Few tactician estimators in ensemble: {estimator_count}",
                            )
                    ...
                    if hasattr(ensemble, "weights_"):
                        ...
                        if weight_count < 3:
                            self.logger.warning(
                                f"⚠️ Few tactician weights in ensemble: {weight_count}",
                            )

                except Exception:
                    self.print(error("❌ Error loading tactician ensemble: {e}"))
                    return False
            ...
            self.logger.exception(
                f"❌ Error during tactician ensemble stability validation: {e}",
            )
            return False
...
if __name__ == "__main__":
    ...
    async def test_validator():
        ...
        pipeline_state = {
            "tactician_ensemble_creation": {"status": "SUCCESS", "duration": 700.5},
        }
