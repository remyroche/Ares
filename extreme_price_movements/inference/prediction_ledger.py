...
PREDICTION_LEDGER_DIAGNOSTIC_COLUMNS = [
    ...
    "global_auction_complete",
    "global_auction_completed_sides_json",
    "global_auction_incomplete_sides_json",
    "was_traded",
    "portfolio_decision",
...
class PredictionLedger:
    ...
    def mark_resolved(self, updates: pd.DataFrame) -> None:
        ...
        # align datatypes on the indexed DataFrames
        for col in key_cols:
            if col in old_idx.columns and col in upd_idx.columns:
                if pd.api.types.is_datetime64_any_dtype(
                    old_idx[col]
                ) and not pd.api.types.is_datetime64_any_dtype(upd_idx[col]):
                    upd_idx[col] = pd.to_datetime(upd_idx[col], utc=True)
                    # If the key column was used for the index, we must also update the index type
                    if old_idx.index.nlevels == 1:
                        upd_idx.index = pd.to_datetime(upd_idx.index, utc=True)
                    else:
                        upd_idx = upd_idx.reset_index(drop=True).set_index(key_cols, drop=False)
                elif pd.api.types.is_datetime64_any_dtype(
                    upd_idx[col]
                ) and not pd.api.types.is_datetime64_any_dtype(old_idx[col]):
                    old_idx[col] = pd.to_datetime(old_idx[col], utc=True)
                    if upd_idx.index.nlevels == 1:
                        old_idx.index = pd.to_datetime(old_idx.index, utc=True)
                    else:
                        old_idx = old_idx.reset_index(drop=True).set_index(key_cols, drop=False)
