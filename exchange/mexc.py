...
class MexcExchange(BaseExchange):
    ...
    async def get_historical_klines(
        ...
        try:
            ...
            logger.error(f"Error fetching historical klines from MEXC for {symbol}: {e}")
            return []
        ...
        try:
            ...
            try:
                ...
                while current_start < end_time_ms:
                    ...
                    logger.info(f"   📡 Fetching trades for {datetime.fromtimestamp(current_start / 1000)} to {datetime.fromtimestamp(current_end / 1000)}")
                        ...
                        async with session.get(url, params=params) as response:
                            if response.status == 200:
                                ...
                                if data and len(data) > 0:
                                    logger.info(f"   📈 Found {len(data)} aggregated trades for this hour")
                                    ...
                                    logger.info(f"   ⚠️ No trades found for this hour")
                            else:
                                text = await response.text()
                                logger.warning(f"   ⚠️ MEXC aggTrades API failed with status {response.status}: {text[:200]}")
            ...
            logger.info(f"   🔄 Falling back to CCXT fetch_trades with pagination")
            ...
            while since < end_time_ms and total_calls < max_calls:
                try:
                    ...
                    if not trades:
                        logger.info(f"   ⚠️ No more trades available at {datetime.fromtimestamp(since / 1000)}")
                        break
                    ...
                    for trade in trades:
                        ...
                        if start_time_ms <= trade_time <= end_time_ms:
                            formatted_trade = {
                                ...
                                "is_buyer_maker": trade.get('side', 'buy') == 'buy' and trade.get('takerOrMaker', 'taker') == 'maker'
                            }
                            filtered_trades.append(formatted_trade)
                    
                    all_trades.extend(filtered_trades)
                    logger.info(f"   📊 Call {total_calls + 1}/{max_calls}: Got {len(trades)} trades, filtered to {len(filtered_trades)} in range")
    ...
    async def get_historical_agg_trades(
        ...
        print(f"🔧 MEXC: Using synthetic data approach since API doesn't support historical trades")
        ...
        try:
            ...
            if klines:
                ...
                print(f"⚠️ MEXC: No klines available for synthetic data")
                return []
    ...
    async def get_historical_futures_data(
        ...
        try:
            ...
            logger.info(f"   🔍 Fetching historical futures data from {datetime.fromtimestamp(since / 1000)} to {datetime.fromtimestamp(end_time_ms / 1000)}")
            
            # Try direct HTTP requests to MEXC's official API endpoints
            try:
                logger.info(f"   🌐 Attempting direct HTTP request to MEXC Futures API")
                    ...
                    try:
                        ...
                        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
                            async with session.get(endpoint['url'], params=endpoint['params']) as response:
                                if response.status == 200:
                                    data = await response.json()
                                    logger.info(f"   📊 Got response from {endpoint['name']}: {len(str(data))} chars")
                                    ...
                                    if funding_data and len(funding_data) > 0:
                                        logger.info(f"   📈 Found {len(funding_data)} funding rate records from {endpoint['name']}")
                                        ...
                                        for item in funding_data:
                                            if isinstance(item, dict):
                                                formatted_item = {
                                                    ...
                                                    "funding_rate": item.get("fundingRate", item.get("rate", 0)),
                                                    "funding_time": item.get("fundingTime", item.get("time", 0)),
                                                    "next_funding_time": item.get("nextFundingTime", 0)
                                                }
                                                formatted_data.append(formatted_item)
                                        
                                        logger.info(f"   ✅ Successfully collected {len(formatted_data)} funding rate records from {endpoint['name']}")
                                        return formatted_data
                                    else:
                                        logger.warning(f"   ⚠️ {endpoint['name']} returned empty data")
                                else:
                                    text = await response.text()
                                    logger.warning(f"   ⚠️ {endpoint['name']} failed with status {response.status}: {text[:200]}")
                    except Exception as e:
            ...
            logger.info(f"   ℹ️ MEXC doesn't have direct funding rate endpoint. Skipping.")
            return []
            
        except Exception as e:
            logger.error(f"Error fetching historical futures data from MEXC for {symbol}: {e}")
            return []
    ...
    async def get_order_status(self, symbol: str, order_id: int):
        ...
        try:
            ...
            logger.error(f"Failed to get status for order {order_id} on MEXC {symbol}: {e}")
            return {"error": str(e)}
    ...
    async def get_position_risk(self, symbol: str = None):
        ...
        try:
            ...
            logger.error(f"Failed to get position risk from MEXC for {symbol or 'all symbols'}: {e}")
            return []
    ...
    async def get_open_orders(self, symbol: str = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to get open orders from MEXC for {symbol or 'all symbols'}: {e}")
            return []
    ...
    async def _get_historical_agg_trades_raw(
        ...
        try:
            ...
            logger.info(f"   🌐 Attempting to fetch MEXC data using multiple methods")
            
            # Method 1: Try MEXC's public API with different endpoints
            try:
                logger.info(f"   📡 Method 1: Trying MEXC public API endpoints")
                ...
                logger.info(f"   📡 Starting paginated download from {datetime.fromtimestamp(current_start / 1000)} to {datetime.fromtimestamp(end_time_ms / 1000)}")
                print(f"   📡 Starting paginated download from {datetime.fromtimestamp(current_start / 1000)} to {datetime.fromtimestamp(end_time_ms / 1000)}")
                logger.info(f"   📊 Target: Collect all trades between {datetime.fromtimestamp(current_start / 1000)} and {datetime.fromtimestamp(end_time_ms / 1000)}")
                print(f"   📊 Target: Collect all trades between {datetime.fromtimestamp(current_start / 1000)} and {datetime.fromtimestamp(end_time_ms / 1000)}")
                ...
                while current_start < end_time_ms and total_calls < max_calls:
                    ...
                    for endpoint in api_endpoints:
                        try:
                            ...
                            logger.info(f"   🔄 Call {total_calls}: Trying {endpoint['name']} from {datetime.fromtimestamp(current_start / 1000)}")
                            print(f"   🔄 Call {total_calls}: Trying {endpoint['name']} from {datetime.fromtimestamp(current_start / 1000)}")
                            ...
                            if response.status_code == 200:
                                ...
                                if data and len(data) > 0:
                                    logger.info(f"   📈 Found {len(data)} records from {endpoint['name']}")
                                    print(f"   📈 Found {len(data)} records from {endpoint['name']}")
                                    ...
                                    if endpoint['name'] == "MEXC Public aggTrades":
                                        ...
                                        for trade in data:
                                            if isinstance(trade, dict):
                                                trade_time = trade.get("T", trade.get("time", 0))
                                    ...
                                    elif endpoint['name'] == "MEXC Public klines (convert to trades)":
                                        ...
                                        for kline in data:
                                            if isinstance(kline, list) and len(kline) >= 6:
                                                ...
                                                formatted_trade = {
                                                    "a": int(open_time / 1000),  # Use timestamp as ID
                                                    "p": close_price,
                                                    "q": volume,
                                                    "T": open_time,
                                                    "m": False,
                                    ...
                                    logger.info(f"   ✅ Successfully processed {len(data)} records from {endpoint['name']}")
                                    print(f"   ✅ Successfully processed {len(data)} records from {endpoint['name']}")
                                    
                                    # Count how many trades were added to all_trades in this iteration
                                    trades_added = len([t for t in all_trades if t.get('T', 0) >= current_start])
                                    logger.info(f"   📊 Added {trades_added} trades within time range from {endpoint['name']}")
                                    print(f"   📊 Added {trades_added} trades within time range from {endpoint['name']}")
                                    ...
                                    if data:
                                        ...
                                        latest_timestamp = max(trade.get('T', trade.get('time', 0)) for trade in data if isinstance(trade, dict))
                                        ...
                                        if latest_timestamp > current_time_ms - 60000:  # If timestamp is within 1 minute of current time
                                            logger.info(f"   ✅ Latest timestamp {datetime.fromtimestamp(latest_timestamp / 1000)} is current time, reached end of historical data")
                                            print(f"   ✅ Latest timestamp {datetime.fromtimestamp(latest_timestamp / 1000)} is current time, reached end of historical data")
                                            break
                                        
                                        current_start = latest_timestamp + 1  # Start from 1ms after the last trade
                                        logger.info(f"   📈 Advanced timestamp to {datetime.fromtimestamp(current_start / 1000)}")
                                        print(f"   📈 Advanced timestamp to {datetime.fromtimestamp(current_start / 1000)}")
                                        
                                        # Check if we've reached the end time
                                        if current_start >= end_time_ms:
                                            logger.info(f"   ✅ Reached end time, stopping pagination")
                                        ...
                                        if len(data) < 5000:  # If we got fewer than the limit, we've reached the end
                                            logger.info(f"   ✅ Got {len(data)} records (less than limit), reached end of available data")
                                            print(f"   ✅ Got {len(data)} records (less than limit), reached end of available data")
                                            break
                                    else:
                                        # If no data, advance by 1 hour as fallback
                                        current_start += 3600000
                                        logger.info(f"   ⏭️ No data, advancing by 1 hour")
                                        print(f"   ⏭️ No data, advancing by 1 hour")
                                        ...
                                        if current_start > end_time_ms + 86400000:  # More than 24 hours past end time
                                            logger.info(f"   ⚠️ Advanced too far into future, stopping pagination")
                                            print(f"   ⚠️ Advanced too far into future, stopping pagination")
                                            break
                                ...
                                logger.warning(f"   ⚠️ {endpoint['name']} failed with status {response.status_code}")
                                print(f"   ⚠️ {endpoint['name']} failed with status {response.status_code}")
                                
                        except Exception as endpoint_error:
                            logger.warning(f"   ⚠️ {endpoint['name']} failed: {endpoint_error}")
                    ...
                    if not any_success:
                        current_start += 3600000
                        logger.warning(f"   ⚠️ All endpoints failed, advancing by 1 hour")
                        print(f"   ⚠️ All endpoints failed, advancing by 1 hour")
                    
                    await asyncio.sleep(0.2)  # Rate limiting
                
                if all_trades:
                    logger.info(f"   ✅ Successfully collected {len(all_trades)} aggregated trades from MEXC public APIs")
                    print(f"   ✅ Successfully collected {len(all_trades)} aggregated trades from MEXC public APIs")
                    logger.info(f"   📊 Pagination summary: {total_calls} API calls made, {len(all_trades)} total trades collected")
                    print(f"   📊 Pagination summary: {total_calls} API calls made, {len(all_trades)} total trades collected")
                    return all_trades
                else:
                    logger.warning(f"   ⚠️ No trades collected from MEXC public APIs")
                    print(f"   ⚠️ No trades collected from MEXC public APIs")
                            
            except Exception as http_error:
                logger.warning(f"Public API methods failed: {http_error}")
            
            # Method 2: Enhanced CCXT fallback with better error handling
            logger.info(f"   🔄 Method 2: Enhanced CCXT fallback with better error handling")
            print(f"   🔄 Method 2: Enhanced CCXT fallback with better error handling")
            ...
            try:
                ...
                logger.info(f"   🔄 Reinitialized exchange connection for CCXT fallback")
                ...
                while since < end_time_ms and total_calls < max_calls:
                    try:
                        ...
                        ccxt_methods = [
                            lambda: self.exchange.fetch_trades(symbol=market_id, since=since, limit=min(limit, 100)),
                            lambda: self.exchange.fetch_ohlcv(symbol=market_id, timeframe='1m', since=since, limit=100),
                        ...
                        for method_idx, method in enumerate(ccxt_methods):
                            try:
                                if method_idx == 0:  # fetch_trades
                                    trades = await method()
                                    if trades:
                                        logger.info(f"   📊 CCXT fetch_trades successful: {len(trades)} trades")
                                        print(f"   📊 CCXT fetch_trades successful: {len(trades)} trades")
                                        break
                                elif method_idx == 1:  # fetch_ohlcv (convert to trades)
                                    ohlcv_data = await method()
                                    if ohlcv_data:
                                        logger.info(f"   📊 CCXT fetch_ohlcv successful: {len(ohlcv_data)} candles")
                                        print(f"   📊 CCXT fetch_ohlcv successful: {len(ohlcv_data)} candles")
                                        ...
                                        for candle in ohlcv_data:
                                            if len(candle) >= 6:
                                                trades.append({
                                                    ...
                                                    'amount': float(candle[5]),  # volume
                                                    'side': 'buy' if candle[4] > candle[1] else 'sell',
                                                    'takerOrMaker': 'taker'
                                                })
                                        break
                                elif method_idx == 2:  # fetch_ticker (single data point)
                                    ticker = await method()
                                    if ticker:
                                        logger.info(f"   📊 CCXT fetch_ticker successful")
                                        print(f"   📊 CCXT fetch_ticker successful")
                                        # Create synthetic trade from ticker
                                        trades = [{
                                            'id': int(ticker.get('timestamp', since) / 1000),
                                            'timestamp': ticker.get('timestamp', since),
                                ...
                                logger.warning(f"   ⚠️ CCXT method {method_idx} failed: {method_error}")
                                continue
                        
                        if not trades:
                            logger.info(f"   ⚠️ No data available at {datetime.fromtimestamp(since / 1000)}")
                            ...
                            for trade in trades:
                                ...
                                if start_time_ms <= trade_time <= end_time_ms:
                                    formatted_trade = {
                                        ...
                                        "m": trade.get('side', 'buy') == 'buy' and trade.get('takerOrMaker', 'taker') == 'maker',  # is buyer maker
                ...
                if all_trades:
                    logger.info(f"   ✅ Successfully collected {len(all_trades)} trades from CCXT fallback")
                    return all_trades
                else:
                    logger.warning(f"   ⚠️ No trades collected from CCXT fallback")
            ...
            if not all_trades:
                logger.warning(f"   ⚠️ No trades collected from MEXC API, returning minimal dataset")
                ...
                logger.info(f"   ✅ Returning minimal dataset to allow download process to continue")
    ...
    async def get_historical_agg_trades_ccxt(
        ...
        try:
            ...
            result = await self.get_historical_agg_trades(symbol, start_time_ms, end_time_ms, limit)
            
            logger.info(f"✅ MEXC: get_historical_agg_trades_ccxt completed, returned {len(result)} trades")
            return result
