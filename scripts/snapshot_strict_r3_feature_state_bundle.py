    ...
    try:
        ...
        try:
            ...
            if "feature_rows" in tables:
                # ``feature_rows_ts`` is the append-order index used by the
                # state writer.  The former aggregate also counted every row
                # and distinct key on every hourly snapshot.  On the nested
                # state this is a full-table scan of a 100+ MB database and
                # can hold the live decision behind a bookkeeping receipt for
                # minutes.  Publication only needs the causal watermark: use
                # index-backed first/last lookups and retain the contract
                # identity from ``metadata``.  Cardinalities are diagnostic,
                # not a state-continuity gate, so leave them explicitly unset
                # rather than inventing approximate values.
                first_row = connection.execute(
                    "SELECT timestamp_ns FROM feature_rows "
                ...
                metadata.update(
                    {
                        "first_timestamp": (
                            pd.Timestamp(int(first_row[0]), tz="UTC").isoformat()
                            if first_row and first_row[0] is not None else None
                        ),
                        "last_timestamp": (
                            pd.Timestamp(int(last_row[0]), tz="UTC").isoformat()
                            if last_row and last_row[0] is not None else None
                        ),
                        "inventory_statistics_mode": "indexed_timestamp_bounds_only",
                    }
                )
            return metadata
        finally:
...
def _canonical_source_files(cache_dir: Path) -> tuple[list[Path], list[dict[str, str]]]:
    """Return source files after resolving only verified stale copy duplicates.

    Finder/manual copies occasionally leave ``" 2"`` beside a state whose
    canonical filename has already advanced.  Treating both as raw states
    makes a snapshot fail its one-watermark gate.  We may suppress the copy
    only when the NPZ metadata proves it is the same operator and strictly no
    newer than the canonical file; every other collision remains fail-closed.
    """
    files = sorted(
    ...
    for source in files:
        match = re.match(r"^(.*) ([2-9][0-9]*)$", source.stem)
        canonical = (
            source.with_name(match.group(1) + source.suffix) if match else None
        )
        if canonical is None or canonical not in available:
            kept.append(source)
            continue
        if source.suffix != ".npz":
            raise ValueError(f"ambiguous non-NPZ state copy: {source}")
        duplicate_meta = _npz_metadata(source)
        canonical_meta = _npz_metadata(canonical)
        identity = ("op", "name", "window", "symbol_order", "version")
        if (
            duplicate_meta.get("metadata_error")
            or canonical_meta.get("metadata_error")
            or any(duplicate_meta.get(key) != canonical_meta.get(key) for key in identity)
        ):
            raise ValueError(f"conflicting operator-state copy: {source}")
        duplicate_ts = pd.to_datetime(duplicate_meta.get("last_timestamp"), utc=True)
        canonical_ts = pd.to_datetime(canonical_meta.get("last_timestamp"), utc=True)
        if pd.isna(duplicate_ts) or pd.isna(canonical_ts) or duplicate_ts > canonical_ts:
            raise ValueError(f"non-stale operator-state copy: {source}")
        suppressed.append({
            "duplicate": source.relative_to(cache_dir).as_posix(),
            "canonical": canonical.relative_to(cache_dir).as_posix(),
            "duplicate_last_timestamp": duplicate_ts.isoformat(),
            "canonical_last_timestamp": canonical_ts.isoformat(),
        })
...
def main() -> None:
    ...
    source_files, suppressed_duplicates = _canonical_source_files(args.cache_dir)
    if not source_files:
    ...
    manifest = {
        ...
        **panel_receipt,
        ...
        "suppressed_stale_operator_state_copies": suppressed_duplicates,
