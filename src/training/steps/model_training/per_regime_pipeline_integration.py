...
class PerRegimePipelineIntegrator:
    ...
    def update_step_config_for_regime(self, step_name: str, config: Dict[str, Any], regime_id: Optional[int]=None) -> Dict[str, Any]:
    ...
    async def verify_regime_data_availability(self, symbol: str, exchange: str, timeframe: str, data_dir: str) -> bool:
        ...
        try:
            ...
            if not unified_file.exists():
                ...
                return False
            data = await self.regime_handler.load_unified_regime_data(symbol, exchange, timeframe, data_dir)
    ...
    def generate_per_regime_config_template(self) -> Dict[str, Any]:
        ...
        """
        template = {'per_regime_processing': True, 'regime_specific_params': {'regime_0': {'step05_labeling': {'time_barrier_minutes': 45, 'max_lookahead': 150, 'profit_take_multiplier': 0.003, 'stop_loss_multiplier': 0.0015}, 'step06_feature_engineering': {'lookback_periods': [10, 20, 50, 100, 200], 'emphasis': 'trend'}}, 'regime_1': {'step05_labeling': {'time_barrier_minutes': 30, 'max_lookahead': 100, 'profit_take_multiplier': 0.002, 'stop_loss_multiplier': 0.001}, 'step06_feature_engineering': {'lookback_periods': [7, 14, 30, 60], 'emphasis': 'balanced'}}, 'regime_2': {'step05_labeling': {'time_barrier_minutes': 20, 'max_lookahead': 75, 'profit_take_multiplier': 0.0015, 'stop_loss_multiplier': 0.0008}, 'step06_feature_engineering': {'lookback_periods': [5, 10, 20, 30], 'emphasis': 'mean_reversion'}}}, 'step_specific_settings': {'step04_regime_data_splitting': {'per_regime': False}, 'step17_final_parameters_optimization': {'per_regime': True, 'optimize_across_regimes': True}}}
        return template
...
async def integrate_per_regime_processing(pipeline_config: Dict[str, Any], steps_to_run: List[str]) -> Dict[str, Callable]:
