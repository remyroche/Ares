        ...
        if self.optimizer:
            try:
                ...
                for group_name, group_features in feature_groups.items():
                    if group_features:
                        periods = await self.optimizer.optimize_feature_group(data, group_features)
                        optimized_periods[group_name] = periods
                return {'optimized_periods': optimized_periods, 'feature_groups': feature_groups, 'method': 'diverse_lookback'}
            except Exception as e:
