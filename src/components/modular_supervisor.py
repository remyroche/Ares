...
class ModularSupervisor(ISupervisor):
    ...
    @handle_errors(
        ...
        default_return=None,
        context="modular_supervisor_start"
    )
    async def start(self) -> None:
        ...
        await self.event_bus.subscribe(
            ...
            self._handle_component_started
        )
        
        await self.event_bus.subscribe(
            EventType.COMPONENT_STOPPED,
            self._handle_component_stopped
        )
        
        self.logger.info("ModularSupervisor started")

    @handle_errors(
        exceptions=(Exception,),
        default_return=None,
        context="modular_supervisor_stop"
    )
    ...
    async def monitor_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_metrics = await self._calculate_performance_metrics(account_info)
    ...
    async def _handle_component_stopped(self, event) -> None:
        ...
        if component_name in self.system_health["components"]:
            ...
            self.system_health["components"][component_name]["stop_time"] = datetime.now()
    ...
    async def _calculate_performance_metrics(self, account_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics = {
                ...
                "utilization_rate": (total_balance - available_balance) / total_balance if total_balance > 0 else 0,
                "margin_utilization": (total_margin_balance - available_balance) / total_margin_balance if total_margin_balance > 0 else 0
            }
            
            # Get historical performance if available
            historical_performance = self.state_manager.get_state("historical_performance", {})
    ...
    async def _check_performance_alerts(self, performance_metrics: Dict[str, Any]) -> list:
        """Check for performance alerts"""
        alerts = []
    ...
    async def _calculate_risk_metrics(self, positions: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if positions and "positions" in positions:
                for position in positions["positions"]:
                    if position.get("size", 0) != 0:  # Active position
                        ...
                        risk_metrics["total_position_value"] += abs(position.get("notional", 0))
                        risk_metrics["total_unrealized_pnl"] += position.get("unrealizedPnl", 0)
    ...
    async def _monitor_component_health(self) -> None:
        ...
        try:
            ...
            for component_name, component_info in self.system_health["components"].items():
    ...
    async def _check_component_failures(self) -> None:
        ...
        try:
            failed_components = [
                name for name, info in self.system_health["components"].items()
