#!/usr/bin/env python3
"""Source-gate ablation for Stage108 path-grid labels.

This is a proxy-only label/execution alignment test. It keeps the Stage108
hard-binary target and economic proxy fixed, then applies decision-time source
gates to test whether excluding rebound/low-barrier pockets improves OOS
execution quality before any model training.
"""
...
DEFAULT_OUTPUT_DIR = Path("data_perp/reports/pathgrid_source_gate_stage111_v1")
DEFAULT_SOURCE_GATES = (
    "all",
    "exclude_rebound_mid",
    "exclude_rebound_or_lowbarrier",
    "exclude_rebound_or_lowbarrier_family",
...
def _gate_mask_frame(sources: dict[str, pd.Series]) -> dict[str, pd.Series]:
    all_mask = sources["all"].fillna(False).astype(bool)
    rebound = sources.get("rebound_mid", pd.Series(False, index=all_mask.index)).fillna(False).astype(bool)
    lowbarrier = sources.get("confirmed_lowbarrier_quality", pd.Series(False, index=all_mask.index)).fillna(False).astype(bool)
    lowbarrier_family = sources.get("confirmed_lowbarrier_family", pd.Series(False, index=all_mask.index)).fillna(False).astype(bool)
    confirmed_event = sources.get("confirmed_event_quality_family", pd.Series(False, index=all_mask.index)).fillna(False).astype(bool)
    loud = sources.get("loud_event", pd.Series(False, index=all_mask.index)).fillna(False).astype(bool)
    quiet = sources.get("quiet_mid", pd.Series(False, index=all_mask.index)).fillna(False).astype(bool)
    breakout = sources.get("clean_breakout_quality", pd.Series(False, index=all_mask.index)).fillna(False).astype(bool)
    run_loud = sources.get("run_entry_loud_event", pd.Series(False, index=all_mask.index)).fillna(False).astype(bool)
    return {
        "all": all_mask,
        "exclude_rebound_mid": all_mask & ~rebound,
        "exclude_rebound_or_lowbarrier": all_mask & ~(rebound | lowbarrier),
        "exclude_rebound_or_lowbarrier_family": all_mask & ~(rebound | lowbarrier_family),
        "exclude_rebound_or_confirmed_event_family": all_mask & ~(rebound | confirmed_event),
        "loud_event_no_rebound": loud & ~rebound,
        "quiet_mid_no_rebound": quiet & ~rebound,
        "clean_breakout_no_rebound": breakout & ~rebound,
        "run_entry_loud_event_no_rebound": run_loud & ~rebound,
    }


def _apply_source_gate(score: pd.Series, gate: pd.Series) -> pd.Series:
    rank = _rank_pct(score).fillna(0.5).reset_index(drop=True)
    gate = gate.fillna(False).astype(bool).reset_index(drop=True)
    gated = rank.copy()
    gated.loc[~gate] = -1.0 + 0.001 * rank.loc[~gate]
        ...
        rows.append(
            {
                ...
                "gate_frac": float(mask.mean()) if len(mask) else 0.0,
                "gate_mean_return_net": float(pd.to_numeric(selected.get("ret_net"), errors="coerce").mean())
                if len(selected)
                else float("nan"),
                "gate_bad_mae_1r_rate": float((pd.to_numeric(selected.get("mae_norm"), errors="coerce") >= 1.0).mean())
                if len(selected)
                else float("nan"),
                "gate_timeout_rate": float(pd.to_numeric(selected.get("is_timeout"), errors="coerce").mean())
                if len(selected)
                else float("nan"),
            }
        )
...
def _write_report(
    *,
    ...
    path = output_dir / "pathgrid_source_gate_ablation.md"
    monthly = period_rows[period_rows["period_type"].astype(str).eq("month")].copy() if not period_rows.empty else pd.DataFrame()
    lines = [
        "# Path-Grid Source Gate Ablation",
        "",
        "Scope: proxy-only source gates on Stage108 hard-binary path-grid labels. No model training, Optuna, or policy optimisation.",
        "",
        ...
        f"Source gates: `{', '.join(manifest['source_gates'])}`",
        "",
        "## Fit/Holdout Summary",
        "",
        _table(
        ...
        "",
        _table(
            source_profile.sort_values(["month", "candidate", "gate"]) if not source_profile.empty else source_profile,
            [
                "month",
                "candidate",
                "gate",
                "gate_rows",
                "gate_frac",
                "gate_mean_return_net",
                "gate_bad_mae_1r_rate",
                "gate_timeout_rate",
            ],
            limit=120,
        ),
...
def run_report(
    *,
    ...
    source_masks = _build_sources(frame, source_context, run_gap_hours=6.0)
    gate_masks_all = _gate_mask_frame(source_masks)
    missing_gates = sorted(set(source_gates).difference(gate_masks_all))
    if missing_gates:
        raise ValueError(f"Unknown source gates: {missing_gates}")
    ...
    for spec_idx, spec in enumerate(specs, start=1):
        ...
        for month in months:
            ...
            valid_gate_masks = {
                gate: mask.loc[valid_mask].copy().reset_index(drop=True)
                for gate, mask in gate_masks_all.items()
            ...
            for top_frac in top_fracs:
                selector_scores = {
                    "oracle_target_sort": valid_target["target_soft"].reset_index(drop=True),
                    "economic_ic_proxy_oos": proxy_score,
                }
                selector_features = {
                    "oracle_target_sort": "",
                    "economic_ic_proxy_oos": proxy_features,
                }
                for gate_name in source_gates:
                    if gate_name == "all":
                        continue
                    selector_name = f"proxy_source_gate_{gate_name}"
                    selector_scores[selector_name] = _apply_source_gate(proxy_score, valid_gate_masks[gate_name])
                    selector_features[selector_name] = proxy_features
    ...
    paths = {
        "period_rows": output_dir / "pathgrid_source_gate_period_rows.csv",
        "fit_holdout": output_dir / "pathgrid_source_gate_fit_holdout.csv",
        "confusion": output_dir / "pathgrid_source_gate_confusion.csv",
        "source_profile": output_dir / "pathgrid_source_gate_profile.csv",
        "selected_ledger": output_dir / "pathgrid_source_gate_selected_ledger.csv",
    ...
    manifest = {
        "scope": "pathgrid_source_gate_ablation",
    ...
    markdown = _write_report(
        ...
        manifest={**manifest, "outputs": {**manifest["outputs"], "markdown": str(output_dir / "pathgrid_source_gate_ablation.md")}},
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source-gates",
        type=lambda value: _parse_csv(value, DEFAULT_SOURCE_GATES),
        default=",".join(DEFAULT_SOURCE_GATES),
    )
...
def main() -> int:
    ...
    manifest = run_report(
        ...
        source_gates=list(args.source_gates),
