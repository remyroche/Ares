...
## 2026-05-18 - Replacing iterrows with Numpy iteration in Inference loops
Learning: Iterating over short timeseries data (like 5m OHLCV bars) inside performance-sensitive environments using pandas `iterrows()` incurs catastrophic boxing/unboxing overhead.
Action: Extracted index and target columns using `.to_numpy()` and used standard python `zip()` to iterate directly over the raw arrays, casting them natively to floats for downstream trailing stop processing. This avoids allocating a new pandas `Series` for every row and significantly improves OCO policy simulation speeds.
