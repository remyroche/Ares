...
def _standardize_index(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "timestamp" in df.columns:
        ...
        df = df.copy()
        # AFML FIX: Most crypto data uses milliseconds since epoch.
        # Check numeric magnitude to distinguish between seconds and milliseconds.
        idx_raw = df.index
        try:
            idx_numeric = pd.to_numeric(idx_raw, errors="coerce")
            if idx_numeric.notna().any():
                q = float(idx_numeric.dropna().quantile(0.5))
                if q > 1e12:  # Milliseconds magnitude
                    unit = "ms"
                elif q > 1e9:   # Seconds magnitude
                    unit = "s"
                else:
                    unit = "ms"
                idx = pd.to_datetime(idx_numeric, unit=unit, utc=True, errors="coerce")
            else:
                idx = pd.to_datetime(idx_raw, errors="coerce")
        except Exception:
            idx = pd.to_datetime(idx_raw, errors="coerce")
...
def get_enhanced_specialist_models_outputs(
    ...
    for step_name, _unused in enhanced_specialists.items():
        try:
            ...
            spec_store_path = None
            possible_suffixes = [
                step_name,
                step_name.replace('enhanced_', ''),
                step_name.replace('_step', ''),
                step_name.replace('enhanced_', '').replace('_step', ''),
                step_name.replace('ml_', '').replace('_step', ''),
                step_name.replace('enhanced_ml_', '').replace('_step', ''),
                # Add specific common patterns
                step_name.replace('ml_', '').replace('_regime_step', '_regime'),
                step_name.replace('enhanced_ml_', '').replace('_regime_step', '_regime'),
            ]
            
            base_prefix = f"{symbol}_{exchange}_{timeframe}_{direction}_"
            for suffix in possible_suffixes:
                p = Path("versioned_artifacts") / f"{base_prefix}{suffix}"
            ...
            if spec_store_path:
                store = VersionedArtifactStore(spec_store_path)
            else:
                # Fallback to the main store
                store = VersionedArtifactStore(Path("versioned_artifacts") / f"{symbol}_{exchange}_{timeframe}_{direction}_{model}")
            ...
            context = {
                ...
                "timeframe": timeframe,
                "direction": direction,
                "model": step_name,  # Use step_name as the model name for filtering
                "step_name": step_name,
            }
            ...
            for art_n in possible_art_names:
                try:
                    # Check if artifact name exists in store versions before loading
                    # and handle potential metadata/HDF5 mismatches
                    try:
                        versions = store.list_versions()
                    except Exception as ve:
                        log_warning(f" Could not list versions for {step_name}: {ve}")
                        continue

                    # If the specific version from metadata is missing, try to find it in the HDF5 keys
                    if not any(art_n in v for v in versions):
                        # Try to find any version starting with art_n in the physical file
                        try:
                            # This is a bit of a hack but necessary when metadata is out of sync
                            h5_path = Path(store.store_path) / "store.h5"
                            if h5_path.exists():
                                import h5py
                                with h5py.File(h5_path, 'r') as f:
                                    if 'versions' in f:
                                        physical_versions = list(f['versions'].keys())
                                        matching_physical = [v for v in physical_versions if art_n in v]
                                        if matching_physical:
                                            # Update versions list to include what's actually there
                                            versions = matching_physical
                        except Exception:
                            pass
                    ...
                    # Try to load the latest matching version
                    matching_v = sorted([v for v in versions if art_n in v])[-1]
                    
                    try:
                        view = store.get_view(matching_v)
                        specialist_data = view.materialize()
                        log_info(f" 📂 Loading {len(specialist_data.columns)} columns from '{matching_v}' | {symbol}/{exchange} [{timeframe}] {direction}/{step_name}")
                    except Exception as le:
                        log_warning(f" Load failed for {matching_v}: {le}")
                        continue

                    if specialist_data is not None and not getattr(specialist_data, "empty", True):
                        log_info(f" Successfully loaded {art_n} for {step_name}")
                        break
                except Exception as e:
                    log_warning(f" Error loading {art_n} from {spec_store_path}: {e}")
                    continue
            ...
            if specialist_data is not None and not getattr(specialist_data, "empty", True):
                ...
                pred_cols = [c for c in specialist_data.columns if c.startswith("predicted_") or "prediction" in c.lower()]
                prob_cols = [c for c in specialist_data.columns if c.startswith("prob_") or "probability" in c.lower()]
                conf_cols = [c for c in specialist_data.columns if c.startswith("confidence_") or "conf" in c.lower()]
                
                # Also include any 'score' columns as they are often used for diagnostics
                score_cols = [c for c in specialist_data.columns if "score" in c.lower() or "val" in c.lower()]
                
                all_cols = list(set(pred_cols + prob_cols + conf_cols + score_cols))
                ...
                if all_cols:
                    ...
                    if training_index is not None:
                        # AFML FIX: Most specialists in this framework follow the convention 
                        # of shift(1) + ffill to represent values available at the START of the bar
                        # and prevent lookahead. 
                        
                        # Replace 0.5 with NaN to allow proper ffilling of sparse signals
                        # 0.5 is used as a default 'no-event' value which blocks ffill.
                        before_block = before_block.replace(0.5, np.nan)
                        
                        # AFML FIX: Ffill BEFORE shift(1) to ensure the signal from the most recent 
                        # event is propagated to all subsequent bars before being lagged.
                        block = before_block.ffill().shift(1)
                        block = block.reindex(training_index, method="ffill")
                        
                        # Restore 0.5 as the baseline uncertainty value
                        block = block.fillna(0.5)
                    else:
                        block = before_block
