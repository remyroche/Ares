#!/usr/bin/env python3
"""Causal raw-feature gate sweep on label-smoke selected-row ledgers."""
...
DEFAULT_INPUT_DIR = Path("data_perp/reports/label_dual_target_execution_smoke_s49_allfeatures_selected_ledger_v1")
DEFAULT_OUTPUT_DIR = Path("data_perp/reports/label_dual_target_execution_smoke_s49_allfeatures_feature_gate_v1")
DEFAULT_FEATURE_CONTRAST_CSV = Path(
    "data_perp/reports/label_proxy_feature_gap_s49_w14_june_top005_targetoracle_v1/"
    "feature_contrast_missed_vs_false_positive.csv"
)
...
def _feature_names_from_contrast(path: Path, *, top_n: int) -> list[str]:
    if not path.exists():
        raise FileNotFoundError(path)
    frame = pd.read_csv(path)
    if "feature" not in frame.columns:
        raise ValueError(f"{path} must contain a 'feature' column")
    if "abs_rank_gap" not in frame.columns and "missed_minus_false_positive_rank" in frame.columns:
        frame = frame.copy()
        frame["abs_rank_gap"] = pd.to_numeric(
            frame["missed_minus_false_positive_rank"],
            errors="coerce",
        ).abs()
    if "abs_rank_gap" in frame.columns:
        frame = frame.sort_values("abs_rank_gap", ascending=False)
    features = [str(v) for v in frame["feature"].dropna().drop_duplicates().tolist()]
    return features[: int(top_n)] if top_n > 0 else features


def _load_gate_features(
    *,
    ...
    if (
        feature_contrast_csv is not None
        and str(feature_contrast_csv)
        and str(feature_contrast_csv) != "/dev/null"
        and feature_contrast_csv.exists()
        and feature_contrast_csv.stat().st_size > 0
    ):
        return _feature_names_from_contrast(feature_contrast_csv, top_n=top_n)
    if feature_list_csv is not None:
        return _read_feature_list(feature_list_csv, max_features=top_n if top_n > 0 else None)
    raise ValueError("Provide --gate-features, --feature-contrast-csv, or --feature-list-csv")
...
def _attach_features(
    ...
    *,
    ...
    frame = pd.DataFrame(
        {
            "__ts__": pd.to_datetime(ledger["timestamp"], errors="coerce"),
            "__symbol__": ledger["symbol"].astype(str),
        }
    )
    features, manifest = _load_feature_store_columns(
        frame,
    ...
    for feature in features.columns:
        out[feature] = pd.to_numeric(features[feature], errors="coerce").to_numpy()
    return out, manifest


def _feature_gate_candidates(
    fit: pd.DataFrame,
    *,
    features: list[str],
    ...
    out: list[tuple[str, Callable[[pd.DataFrame], pd.Series]]] = [
        ("no_feature_gate", lambda frame: pd.Series(True, index=frame.index)),
    ]
    score_median = (
        float(pd.to_numeric(fit["score"], errors="coerce").quantile(0.50))
    ...
    for feature in features:
        ...
        for q in quantiles:
            ...
            if math.isfinite(threshold):
                thresholds.append((float(q), threshold))
        for q, threshold in thresholds:
            for op in ("<=", ">="):
                name = f"{feature}{op}fit_q{int(q * 100)}({threshold:.6g})"
                if op == "<=":
                    gate = lambda frame, f=feature, t=threshold: pd.to_numeric(
                        frame[f],
                        errors="coerce",
                    ) <= t
                else:
                ...
                if include_score_combo and math.isfinite(score_median):
                    if op == "<=":
                        combo = lambda frame, f=feature, t=threshold, s=score_median: (
                            pd.to_numeric(frame[f], errors="coerce") <= t
                        ) & (pd.to_numeric(frame["score"], errors="coerce") >= s)
                    else:
                        combo = lambda frame, f=feature, t=threshold, s=score_median: (
                            pd.to_numeric(frame[f], errors="coerce") >= t
                        ) & (pd.to_numeric(frame["score"], errors="coerce") >= s)
                    out.append((f"{name}&score>=fit_q50({score_median:.6g})", combo))

                if include_rank_combo and {"selected_rank", "selected_rows"}.issubset(fit.columns):
                    if op == "<=":
                        rank_combo = lambda frame, f=feature, t=threshold: (
                            pd.to_numeric(frame[f], errors="coerce") <= t
                        ) & (
                    ...
                    out.append((f"{name}&selected_rank_frac<=0.75", rank_combo))
    return out
    ...
    *,
    ...
    for key, group in subset.groupby(group_cols, observed=True, dropna=False):
        ...
        if fit.empty or holdout.empty:
            continue
        for gate_name, gate_func in _feature_gate_candidates(
            fit,
...
def _write_markdown(output_dir: Path, summary: pd.DataFrame, manifest: dict[str, Any]) -> Path:
    path = output_dir / "label_selected_ledger_feature_gate.md"
    ...
    lines = [
        "# Label Selected-Ledger Raw-Feature Gate",
        "",
        "Scope: raw feature thresholds are derived from fit-month selected rows, then applied unchanged to the holdout month.",
        "",
...
def run_report(
    *,
    ...
    selected_features = _load_gate_features(
        explicit_features=gate_features,
        feature_contrast_csv=feature_contrast_csv,
        feature_list_csv=feature_list_csv,
        top_n=int(top_gate_features),
    )
    ledger, feature_manifest = _attach_features(
        ledger,
        feature_dir=feature_dir,
        gate_features=selected_features,
    )
    retained_features = [feature for feature in selected_features if feature in ledger.columns]
    summary = summarize_feature_gates(
        ledger,
    ...
    paths = {
        "summary": output_dir / "label_selected_ledger_feature_gate_summary.csv",
    ...
    manifest = {
        ...
        "feature_list_csv": str(feature_list_csv) if feature_list_csv else "",
        "feature_contrast_csv": str(feature_contrast_csv) if feature_contrast_csv else "",
        "fit_months": [str(v) for v in fit_months],
        "holdout_month": str(holdout_month),
        "top_fracs": [float(v) for v in top_fracs],
        "quantiles": [float(v) for v in quantiles],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--top-gate-features", type=int, default=20)
