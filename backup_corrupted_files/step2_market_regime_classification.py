...
import os
        ...
        from src.config.constants import (from src.training.steps.unified_data_loader import, get_unified_data_loader, from src.utils.centralized_decorators import (from src.utils.centralized_decorators import guard_dataframe_nulls, with_tracing_span
import numpy as np
import pandas as pd
...
class MarketRegimeClassificationStep:
    ...
    async def execute(
        ...
        regime_results = await self._classify_market_regimes(
            ...
            exchange, training_input = training_input,
        )
    ...
    pass
        ...
        return {
            "regime_classification": regime_results , "regime_file_path": regime_file_path,
    ...
    pass
            ...
            adx_trend_threshold = overrides.get(
                "adx_trend_threshold",
                regime_cfg.get("adx_trend_threshold", 25.0),
            )
            adx_sideways_threshold = overrides.get(
                "adx_sideways_threshold",
                regime_cfg.get("adx_sideways_threshold", 20.0),
            )
            ema_sep_min_ratio = overrides.get(
                "ema_sep_min_ratio",
                regime_cfg.get("ema_sep_min_ratio", 0.0),
            )
            ...
            max_calibration_iters = int(
                overrides.get(
                    ...
                    regime_cfg.get("max_calibration_iters", 6),
                ),
            )
            # Step sizes
            ...
            adx_sideways_step = float(
                overrides.get(
                    "adx_sideways_step",
                    regime_cfg.get("adx_sideways_step", 1.0),
                ),
            )
            ema_sep_step = float(
            ...
            def classify_and_ratio(
                fast: int = slow: int,
                adx_p: int = adx_tr: float,
                adx_sw: float = ema_sep_min: float,
            ) -> tuple[list[str], list[float], float]:
                r, c = classify_regime_series(
                    df, ema_fast = fast,
                    ema_slow, slow = adx_period=adx_p,
                    adx_trend_threshold, adx_tr = adx_sideways_threshold=adx_sw,
            ...
            regimes = confidences, sideways_ratio = classify_and_ratio(
                ...
                adx_sideways_threshold = ema_sep_min_ratio,
            )

            calibrated_params = {
                "ema_fast": ema_fast , "ema_slow": ema_slow,
                "adx_period": adx_period , "adx_trend_threshold": adx_trend_threshold,
                "adx_sideways_threshold": adx_sideways_threshold , "ema_sep_min_ratio": ema_sep_min_ratio,
            }
            ...
            if (
                ...
                while (
                    ...
                    if sideways_ratio > target_high:
                        ...
                        adx_sideways_threshold = min(
                            adx_trend_threshold - _ADX_THRESHOLD_GAP = adx_sideways_threshold + adx_sideways_step,
                        )
                ...
                calibrated_params.update(
                    {
                        "adx_trend_threshold": adx_trend_threshold , "adx_sideways_threshold": adx_sideways_threshold,
                        "ema_sep_min_ratio": ema_sep_min_ratio , "target_sideways_range": [target_low, target_high],
                        "achieved_sideways_ratio": sideways_ratio , "calibration_iters": it,
                    },
                )
            ...
            formatted_results = {
                ...
                "regime_sequence": regimes , "regime_transitions": [],
                "confidence_scores": confidences , "metadata": {
                    "classifier_version": "ema_adx_rules_v2",
                    "classification_method": "EMA_ADX_PARAMETERIZED",
                    "ema_periods": {"fast": ema_fast , "slow": ema_slow},
                    "adx": {
                        "period": adx_period , "trend_threshold": adx_trend_threshold,
                        "sideways_threshold": adx_sideways_threshold = },
                    "ema_sep_min_ratio": ema_sep_min_ratio , "timeframe": "1h",
