...
_FORBIDDEN_PREENTRY_TOKENS = (
    "mfe", "mae", "future", "forward", "outcome", "time_to_", "reversal",
    "final_peak", "target", "label", "realized", "realised", "exit_",
)
...
def catboost_resource_contract(
    ...
    effective_threads = requested_threads if unsafe_override else min(requested_threads, safe_thread_cap)
...
def capped_catboost_params(params: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    """Cap externally supplied CatBoost parameters for geometry-only refits."""
    requested_threads = int(params.get("thread_count", PathArchetypeConfig().catboost_thread_count))
    config = PathArchetypeConfig(catboost_thread_count=max(1, requested_threads))
    return _catboost_params_with_resource_limits(params, config=config), catboost_resource_contract(config)
...
def summarize_future_path(
    ...
    *,
    ...
    if path.size == 0 or bar_hours <= 0:
        ...
        for name in (
            "time_to_025r_h", "time_to_05r_h", "time_to_1r_h", "time_to_tp_h",
            "time_to_trailing_h", "time_to_stop_h", "mfe_before_mae",
            "mae_before_mfe", "time_to_first_meaningful_mfe_h",
            "time_to_90pct_peak_mfe_h", "peak_mfe_r", "peak_mfe_atr",
            "mfe_to_cost", "mfe_to_activation_distance", "early_late_ratio",
            "efficiency", "reversal_count", "final_return_r", "final_to_peak",
        ):
    ...
    trailing_trigger_r = take_profit_r if trailing_trigger_r is None else trailing_trigger_r
    out.update(
        {
            ...
            f"{prefix}time_to_tp_h": _first_time(path, path >= take_profit_r, bar_hours),
            f"{prefix}time_to_trailing_h": _first_time(
                path, path >= trailing_trigger_r, bar_hours
            ),
            f"{prefix}time_to_stop_h": _first_time(path, path <= -abs(stop_r), bar_hours),
...
def summarize_future_paths(
    ...
    **kwargs: Any,
    ...
    return pd.DataFrame(
        [summarize_future_path(path, **kwargs) for path in frame[path_column]], index=frame.index
    )
...
def validate_preentry_features(columns: Iterable[str]) -> tuple[str, ...]:
    ...
    if violations:
        raise ValueError("Path archetype classifier received non-pre-entry features: " + ", ".join(violations[:8]))
    return accepted


def configured_base_meta_preselection_universe(
    available_columns: Iterable[str], *, config_mapping: Mapping[str, Any] | None = None,
...
def _rank(values: np.ndarray) -> np.ndarray:
    return pd.Series(values).rank(method="average", na_option="keep").to_numpy(dtype=float)
...
def _relief_multisurf_proxy(values: np.ndarray, y: np.ndarray, neighbors: int) -> np.ndarray:
    """Small deterministic nearest-neighbour Relief/MultiSURF-style rescue."""
    n, p = values.shape
    if n < 4 or len(np.unique(y)) < 2:
        return np.zeros(p)
    scaled = (values - np.nanmedian(values, axis=0)) / np.maximum(np.nanstd(values, axis=0), 1e-6)
...
def _normalise(values: Mapping[str, float]) -> dict[str, float]:
    finite = np.array([value for value in values.values() if np.isfinite(value)], dtype=float)
    if not finite.size or np.ptp(finite) <= 1e-12:
        return {key: 0.0 for key in values}
    low, span = float(finite.min()), float(np.ptp(finite))
    return {key: (float(value) - low) / span if np.isfinite(value) else 0.0 for key, value in values.items()}
...
def _univariate_catboost_oos_logloss_gain(
    ...
    *,
    ...
    resource = catboost_resource_contract(config)
    effective_parallel_jobs = min(max(1, int(parallel_jobs)), int(resource["effective_thread_count"]))
    ...
    def score_column(item: tuple[int, str]) -> tuple[str, float]:
        ...
        for fold, train, valid, baseline in fold_specs:
            ...
            if not finite_train.size or np.ptp(finite_train) <= 1e-12:
                ...
                continue
            model = CatBoostClassifier(**_catboost_params_with_resource_limits(
                {
...
def fast_select_preentry_features(
    ...
    target: Sequence[Any],
    *, mandatory_features: Sequence[str] = (), warmup_mask: Sequence[bool] | None = None,
    config: PathArchetypeConfig = PathArchetypeConfig(),
) -> FastSelectorResult:
    ...
    eligible_rows = np.ones(len(features), dtype=bool) if warmup_mask is None else ~np.asarray(warmup_mask, dtype=bool)
    ...
    availability = {c: float(features.loc[eligible_rows, c].notna().mean()) for c in numeric}
    ...
    for col in mandatory:
        if availability.get(col, 0.0) <= config.availability_threshold:
            raise ValueError(f"Mandatory feature {col!r} fails strict >{config.availability_threshold:.0%} availability")
    if not available:
    ...
    sample_y = y[idx]
    raw_mi = {col: _mutual_information(raw_sample[:, j], sample_y) for j, col in enumerate(available)}
    ...
    if tuple(screened) == tuple(available):
        ...
        available_index = {column: position for position, column in enumerate(available)}
    ...
    candidates = list(dict.fromkeys(mandatory)) + [
        col for col in sorted(ranked_union.difference(mandatory), key=combined.get, reverse=True)
    ]
    candidates = candidates[: len(mandatory) + cap]
    selected, clusters = prune_spearman_clusters(features.iloc[idx], candidates, combined, mandatory, config.corr_threshold)
    score_frame = pd.DataFrame({"mi": pd.Series(mi), "catboost_proxy": pd.Series(proxy), "relief": pd.Series(relief)})
    score_frame["combined"] = pd.Series(combined)
    return FastSelectorResult(tuple(selected), tuple(candidates), tuple(mandatory), availability, score_frame, tuple(clusters), backend)


def prune_spearman_clusters(
    features: pd.DataFrame, candidates: Sequence[str], scores: Mapping[str, float],
    mandatory_features: Sequence[str] = (), threshold: float = 0.90,
    ...
    for group in groups.values():
        ...
        selected.extend(forced or [max(group, key=lambda col: scores.get(col, float("-inf")))])
...
def purged_chronological_folds(
    timestamps: Sequence[Any], *, label_end: Sequence[Any] | None = None,
    n_splits: int = 5, embargo: pd.Timedelta = pd.Timedelta(hours=24),
) -> list[PurgedFold]:
    ...
    if not ts.is_monotonic_increasing:
        raise ValueError("timestamps must already be chronological; sorting silently breaks row alignment")
    end = ts if label_end is None else pd.to_datetime(pd.Series(label_end), utc=True, errors="raise")
    ...
    for fold_id, valid in enumerate(chunks[1:]):
        ...
        keep = (end.iloc[prior] < start).to_numpy() & (ts.iloc[prior] < start - embargo).to_numpy()
        train = prior[keep]
        if len(train):
...
def multiclass_log_loss(y: Sequence[int], probabilities: np.ndarray, classes: Sequence[int] | None = None) -> float:
    y = np.asarray(y, dtype=int)
    proba = np.asarray(probabilities, dtype=float)
    class_order = np.arange(proba.shape[1]) if classes is None else np.asarray(classes, dtype=int)
...
def catboost_hpo_objective_components(
    ...
    fold_losses = [multiclass_log_loss(y[folds == f], p[folds == f]) for f in np.unique(folds) if np.any(folds == f)]
...
def multiclass_classification_diagnostics(
    y: Sequence[int], probabilities: np.ndarray, *, fold_ids: Sequence[int] | None = None,
    class_names: Sequence[Any] | None = None, ece_bins: int = 10,
) -> dict[str, Any]:
    ...
    precision = np.divide(np.diag(confusion), confusion.sum(axis=0), out=np.zeros(len(classes)), where=confusion.sum(axis=0) > 0)
    recall = np.divide(np.diag(confusion), support, out=np.zeros(len(classes)), where=support > 0)
    f1 = np.divide(2.0 * precision * recall, precision + recall, out=np.zeros(len(classes)), where=(precision + recall) > 0)
    ...
    if fold_ids is not None:
        ...
        for fold_id in sorted(int(value) for value in np.unique(all_folds) if int(value) >= 0):
            ...
            fold_precision = np.divide(np.diag(fold_conf), fold_conf.sum(axis=0), out=np.zeros(len(classes)), where=fold_conf.sum(axis=0) > 0)
            fold_recall = np.divide(np.diag(fold_conf), fold_support, out=np.zeros(len(classes)), where=fold_support > 0)
            fold_f1 = np.divide(2.0 * fold_precision * fold_recall, fold_precision + fold_recall, out=np.zeros(len(classes)), where=(fold_precision + fold_recall) > 0)
        ...
        report["temporal_stability"] = {
            ...
            "probability_drift_first_to_last": float(np.mean(np.abs(fold_means[-1] - fold_means[0]))) if len(fold_means) > 1 else 0.0,
        }
...
def _catboost_params(config: PathArchetypeConfig, params: Mapping[str, Any] | None) -> dict[str, Any]:
    base = {
        "loss_function": "MultiClass", "eval_metric": "MultiClass", "iterations": 3000,
        "od_wait": 150, "learning_rate": 0.03, "depth": 6, "l2_leaf_reg": 30.0,
        "random_strength": 1.0, "bagging_temperature": 1.0, "rsm": 0.8,
        "border_count": 64, "auto_class_weights": None, "bootstrap_type": "Bayesian",
        "grow_policy": "SymmetricTree",
        "random_seed": config.random_state, "verbose": False, "allow_writing_files": False,
    }
    ...
    if not config.legacy_allow_class_weights:
        if base.get("auto_class_weights") is not None:
            raise ValueError(
                ...
                "auto_class_weights must be None"
            )
        if base.get("class_weights") is not None:
            raise ValueError(
                "future-training CatBoost requires uniform sample weights; "
                "class_weights are not permitted"
            )
...
def path_archetype_probability_contract(
    ...
    *,
    ...
    entropy = -np.sum(
        values * np.log(np.clip(values, 1e-12, 1.0)), axis=1
    )
    ...
    output["adverse_probability_mass"] = output.loc[:, PATH_ARCHETYPE_ADVERSE_CLASSES].sum(axis=1)
    output["favorable_probability_mass"] = output.loc[:, PATH_ARCHETYPE_FAVORABLE_CLASSES].sum(axis=1)
    return output


def fit_purged_chronological_oof_catboost(
    features: pd.DataFrame, target: Sequence[Any], timestamps: Sequence[Any], *,
    label_end: Sequence[Any] | None = None, config: PathArchetypeConfig = PathArchetypeConfig(),
    ...
    folds = purged_chronological_folds(timestamps, label_end=label_end, n_splits=config.oof_folds, embargo=config.embargo)
    if staged_matrix_cache is not None and not staged_matrix_cache.supports(feature_columns):
        missing = sorted(set(feature_columns).difference(staged_matrix_cache.column_index))
        raise KeyError(f"Staged MDA cache is missing OOF features: {missing[:8]}")
    x = None if staged_matrix_cache is not None else _finite_matrix(features, feature_columns)
    ...
    for fold in folds:
        ...
        model.fit(
            train_matrix, y[fold.train_indices],
    ...
    if np.any(valid):
        diagnostics = multiclass_classification_diagnostics(
            y[valid], probabilities[valid], fold_ids=fold_ids[valid], class_names=classes,
        )
        diagnostics["fold_fit_reports"] = fold_fit_reports
    return OOFPathArchetypeResult(
        probabilities, fold_ids, fitted_folds, models, classes, tuple(feature_columns), diagnostics,
        staged_matrix_cache,
    )


def suggest_catboost_hpo_params(trial: Any) -> dict[str, Any]:
    """Sample only from the declared classifier search space."""
    return {
...
def optimize_purged_catboost_hpo(
    features: pd.DataFrame, target: Sequence[Any], timestamps: Sequence[Any], *,
    label_end: Sequence[Any] | None = None, config: PathArchetypeConfig = PathArchetypeConfig(),
    n_trials: int = 30, study_name: str | None = None, storage: str | None = None,
    search_iterations: int = 1500, search_od_wait: int = 100,
    ...
    known_trials = list(study.trials)
    ...
    def write_progress(*, current_trial: Any | None, status: str) -> None:
        ...
        _atomic_json_write(Path(progress_path), {
            ...
            "total_recorded_trial_count": int(len(known_trials)),
            "current_trial": (
                _hpo_trial_record(
                    latest_trial,
                ...
                if latest_trial is not None else None
            ),
            "current_params": (
        ...
        def report_fold(
            ...
            if trial.should_prune():
                raise optuna.TrialPruned(
                    f"median-pruned after purged fold {step + 1}"
                )

        oof = fit_purged_chronological_oof_catboost(
            features, target, timestamps, label_end=label_end, config=config, params=params,
            fold_callback=report_fold,
        )
        valid = oof.fold_ids >= 0
        if not np.any(valid):
            raise ValueError("Purged OOF fitting did not produce validation predictions")
    ...
    completed_before = len(known_trials)
    ...
    def stop_after_stagnation(study: Any, trial: Any) -> None:
        ...
        if (
            ...
            and len(known_trials) - 1 - last_improvement_position
            >= int(no_improvement_trials)
        ):
            study.stop()
    ...
    best_params = suggest_catboost_hpo_params(_FixedTrial(study.best_trial.params))
    ...
    if best_oof is None:
        best_oof = fit_purged_chronological_oof_catboost(
            features, target, timestamps, label_end=label_end, config=config, params=best_params,
        )
    trials = tuple(
        _hpo_trial_record(
            trial,
...
def _batched_permutation_losses(
    ...
    *,
    ...
    batch_size = min(
        len(candidates), max(1, int(max_batch_bytes) // per_matrix_bytes)
    )
...
def _screened_mda_candidates(
    ...
    *,
    ...
    if (
        not config.permutation_screening_enabled
        or candidate_limit >= len(nonmandatory)
    ):
    ...
    cutoff = float(screen_losses[ranked[candidate_limit - 1]]) if candidate_limit else None
    return candidates, True, cutoff


def staged_permutation_selection(
    features: pd.DataFrame, target: Sequence[Any], oof: OOFPathArchetypeResult, *,
    mandatory_features: Sequence[str] = (), stages: Sequence[int] = (150, 125, 100, 75),
    random_state: int = 20260722, config: PathArchetypeConfig | None = None,
    params: Mapping[str, Any] | None = None,
    ...
    if not cache.supports(columns):
        ...
        raise KeyError(f"Staged MDA cache is missing features: {missing_cache_columns[:8]}")
    CatBoostClassifier = _require_catboost()
    reused_models = {
        fold.fold_id: model for fold, model in zip(oof.folds, oof.models)
    }
    ...
    for stage_index, stage in enumerate(stages):
        if stage_index < len(completed):
            ...
            if int(checkpoint.get("stage_index", -1)) != stage_index or int(
                ...
                raise ValueError("MDA checkpoint stages do not match the current contract")
            ...
            if input_features != current or not selected_features or not isinstance(records, list):
        ...
        for fold in oof.folds:
            ...
            if can_reuse:
                ...
                prior_report = next(
                    (
                        dict(report) for report in prior_reports
                ...
                model.fit(
                    train_matrix, y[fold.train_indices], eval_set=(matrix, y[valid]),
                    early_stopping_rounds=int(fold_params["od_wait"]), verbose=False,
                )
                fit_seconds += time.perf_counter() - fit_started
                fit_calls += 1
                baseline_started = time.perf_counter()
                baseline = multiclass_log_loss(
                    y[valid], model.predict_proba(matrix), getattr(model, "classes_", None)
                )
        ...
        potential_screen = (
            config.permutation_screening_enabled
            and len(current) > keep + max(0, int(config.permutation_screen_margin))
        )
        if potential_screen:
        ...
        stability = {
            column: (
                1.0 / (1.0 + float(np.std(values)) / (abs(float(np.mean(values))) + 1e-6))
        ...
        current = list(dict.fromkeys(mandatory + sorted(
        ...
        stage_fields = {
            ...
            "stage_screened_out_count": int(len(stage_input_columns) - len(full_candidates)),
            ...
            "stage_selection_semantics": (
                ...
                if screened else "exact_full_mda"
            ),
...
class PathArchetypeClassifier:
    ...
    @classmethod
    def fit(
        cls, features: pd.DataFrame, path_archetypes: Sequence[Any], timestamps: Sequence[Any], *,
        label_end: Sequence[Any] | None = None, mandatory_features: Sequence[str] = (),
        config: PathArchetypeConfig = PathArchetypeConfig(), params: Mapping[str, Any] | None = None,
        run_permutation_selection: bool = False, warmup_mask: Sequence[bool] | None = None,
        config_mapping: Mapping[str, Any] | None = None, run_hpo_trials: int = 0,
        hpo_study_name: str | None = None, hpo_storage: str | None = None,
    ) -> "PathArchetypeClassifier":
        validate_preentry_features(features.columns)
        universe = configured_base_meta_preselection_universe(
            features.columns, config_mapping=config_mapping,
        )
        if not universe:
            raise ValueError("No configured base/meta preselection features are present in the training frame")
        mandatory = tuple(feature for feature in mandatory_features if feature in universe)
        ...
        selector = fast_select_preentry_features(
            features.loc[:, universe], path_archetypes, mandatory_features=mandatory,
            warmup_mask=warmup_mask, config=config,
        )
        ...
        mda_cache = (
            build_staged_permutation_matrix_cache(
                mda_features, timestamps, label_end=label_end, config=config,
            )
        ...
        selection_oof = fit_purged_chronological_oof_catboost(
            selection_oof_features, path_archetypes, timestamps, label_end=label_end,
            config=config, params=fitted_params, staged_matrix_cache=mda_cache,
        ...
        if run_permutation_selection:
            selected, permutation = staged_permutation_selection(
                mda_features, path_archetypes, selection_oof,
                mandatory_features=mandatory,
                stages=config.permutation_stages, random_state=config.random_state,
                config=config, params=fitted_params,
            )
            permutation_report = _json_ready(permutation.to_dict(orient="records"))
        if run_hpo_trials:
            hpo_result = optimize_purged_catboost_hpo(
                features.loc[:, selected], path_archetypes, timestamps,
                label_end=label_end, config=config, n_trials=run_hpo_trials,
                study_name=hpo_study_name, storage=hpo_storage,
            )
            fitted_params.update(hpo_result.best_params)
        oof = fit_purged_chronological_oof_catboost(
            features.loc[:, selected], path_archetypes, timestamps,
            label_end=label_end, config=config, params=fitted_params,
        )
        CatBoostClassifier = _require_catboost()
        ...
        report = _json_ready({
            ...
            "permutation_acceleration_contract": staged_permutation_acceleration_contract(config),
        ...
        return cls(tuple(selected), tuple(map(str, y_series.cat.categories)), model, selector, config, report)
    ...
    def predict_proba(self, preentry_features: pd.DataFrame) -> pd.DataFrame:
        ...
        values = np.asarray(
            self.model.predict_proba(_finite_matrix(preentry_features, self.feature_columns)),
            dtype=float,
        )
        if tuple(self.class_names) == MERGED_PATH_ARCHETYPE_CLASSES:
            ...
            numeric_classes = pd.to_numeric(
                pd.Series(model_classes), errors="coerce"
            ).notna().all()
            if numeric_classes:
                aligned[:, np.asarray(model_classes, dtype=int)] = values
            else:
                positions = {name: position for position, name in enumerate(self.class_names)}
        ...
        return pd.DataFrame(values, index=preentry_features.index, columns=self.class_names)

    def predict(self, preentry_features: pd.DataFrame) -> pd.Series:
        proba = self.predict_proba(preentry_features)
        return proba.loc[:, list(self.class_names)].idxmax(axis=1).rename("path_archetype")
