...
class TradingEngine:
    ...
    def __init__(self, config: TradingConfig, exchange_client: Any):
        ...
        # Signal generation pipelines (key format: "{symbol}_{direction}")
        self.signal_pipelines: Dict[str, SignalGenerationPipeline] = {}

        # Sizing components (key format: "{symbol}_{direction}")
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            # Register internal handlers before starting components so that
            # the DataStreamer sees all configured handlers when starting
    ...
    async def _initialize_signal_pipelines(self) -> None:
        ...
        try:
            ...
            # Determine directions to initialize
            directions_to_init = []
            if self.config.direction == "both":
                directions_to_init = ["long", "short"]
            else:
                directions_to_init = [self.config.direction]

            # Create a dedicated signal pipeline per symbol per direction
            for symbol in self.config.symbols:
                for direction in directions_to_init:
                    pipeline_key = f"{symbol}_{direction}"
                    if pipeline_key in self.signal_pipelines:
                        continue
                    ...
                    pipeline_cfg.direction = direction
                    ...
                    if pipeline is None:
                        self.logger.error(f"❌ Failed to initialize SignalGenerationPipeline for {pipeline_key}")
                        continue

                    self.signal_pipelines[pipeline_key] = pipeline
                    self.logger.info(f"✅ SignalGenerationPipeline initialized for {pipeline_key}")

        except Exception as exc:
            self.logger.error(f"❌ Failed to initialize signal pipelines: {exc}")
    
    async def _initialize_sizing_components(self) -> None:
        """Initialize Kelly-based sizing components for each symbol and direction.
        ...
        try:
            ...
            for symbol in self.config.symbols:
                for direction in directions_to_init:
                    ...
                    if pipeline_key in self.position_sizers:
                        continue
                    ...
                    try:
                        ...
                        self.logger.warning(f"⚠️ Failed to setup sizing components for {pipeline_key}: {exc}")
                        continue
                    ...
                    if position_sizer is None:
                        self.logger.warning(
                            f"⚠️ PositionSizer not initialized for {pipeline_key} - falling back to inline sizing"
                        )
                    else:
                        self.position_sizers[pipeline_key] = position_sizer

                    if leverage_manager is not None:
                        self.leverage_managers[pipeline_key] = leverage_manager
                    if risk_calculator is not None:
                        self.sizing_risk_calculators[pipeline_key] = risk_calculator
    ...
    async def get_trading_status(self) -> Dict[str, Any]:
        ...
        # Aggregate signal pipeline metrics per symbol and direction (if available)
        signal_status: Dict[str, Any] = {}
        for key, pipeline in self.signal_pipelines.items():
            try:
                signal_status[key] = pipeline.get_performance_metrics()
            except Exception as exc:
                self.logger.warning(f"⚠️ Failed to get signal metrics for {key}: {exc}")
                signal_status[key] = {"error": str(exc)}
    ...
    async def _on_kline_data(self, stream_data: StreamData) -> None:
        ...
        # Prepare input data once for all pipelines of this symbol
        ...
        # Iterate over all initialized pipelines for this symbol (e.g., long and short)
        # Pipelines are keyed by "{symbol}_{direction}"
        relevant_pipelines = [
            (k, p) for k, p in self.signal_pipelines.items()
            if k.startswith(f"{symbol}_")
        ]

        for pipeline_key, pipeline in relevant_pipelines:
            try:
                ...
                # (Shared monitor, any pipeline update is fine, ideally consistent)
                if (
                    self.regime_monitor is not None
                    and result is not None
                    and getattr(result, "regime_output", None) is not None
                ):
                    try:
                        await self.regime_monitor.update_regime_state(
                            regime_probabilities=result.regime_output.regime_probabilities,
                            confidence=result.regime_output.confidence,
                            market_conditions={
                                "symbol": symbol,
                ...
                self.logger.error(f"❌ Failed to generate or execute signal for {pipeline_key}: {exc}")
    ...
    async def _map_signal_to_trade_decision(
        ...
        try:
            ...
            # Determine direction context based on action
            # buy = Long Entry
            # sell = Short Entry
            # close = Exit (depends on open position, handled by order manager)

            # Note: For closing, we need price/quantity but SL/TP are irrelevant
            # For entries, we calculate SL/TP based on direction.

            if action == "buy": # Long Entry
                stop_loss = current_price * (1.0 - stop_loss_pct)
                take_profit = current_price * (1.0 + take_profit_pct)
            elif action == "sell": # Short Entry
                stop_loss = current_price * (1.0 + stop_loss_pct)
                take_profit = current_price * (1.0 - take_profit_pct)
            else: # Close (Exit)
                # SL/TP not used for market close, but populated for completeness
                stop_loss = 0.0
                take_profit = 0.0
            ...
            # Look up sizer by pipeline_key (e.g. "ETHUSDT_long") if provided
            sizer = None
            if pipeline_key:
                sizer = self.position_sizers.get(pipeline_key)
            else:
                # Fallback to symbol lookup if using old keys (legacy safety)
