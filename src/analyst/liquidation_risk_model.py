...
class LiquidationRiskModel:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.safe_leverage_multiplier: float = self.risk_config.get("safe_leverage_multiplier", 0.8)
        self.max_leverage: int = self.risk_config.get("max_leverage", 100)  # Increased for high leverage
        self.min_leverage: int = self.risk_config.get("min_leverage", 10)   # Increased minimum
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.info("✅ Liquidation Risk Model initialization completed successfully")
            return True
    ...
    def _validate_configuration(self) -> bool:
        ...
        try:
            ...
            if self.max_leverage < 10:
                self.logger.error("max_leverage must be at least 10 for high leverage trading")
                return False
                
            if self.min_leverage < 10:
                self.logger.error("min_leverage must be at least 10 for high leverage trading")
                return False
        ...
        try:
            ...
            if adverse_key in ml_predictions:
                ...
                if isinstance(adverse_probs, dict):
                    max_adverse_prob = max(adverse_probs.values()) if adverse_probs else 0.0
                else:
            ...
            self.logger.info(f"Extracted adverse risk: {max_adverse_prob:.3f} for {target_direction}")
    ...
    def _calculate_safe_leverage(
        ...
        try:
            ...
            safe_leverage = max(self.min_leverage, min(safe_leverage, self.max_leverage))
            
            self.logger.info(f"Calculated safe leverage: {safe_leverage}x (adverse risk: {adverse_risk:.3f})")
    ...
    def _calculate_liquidation_prices(
        ...
        try:
            ...
            self.logger.info(f"Calculated liquidation prices for {len(liquidation_prices)} leverage levels")
    ...
    def _classify_risk_level(self, adverse_risk: float) -> str:
        ...
        if adverse_risk <= 0.05:  # 5% adverse risk
            return "VERY_LOW"
        elif adverse_risk <= 0.1:  # 10% adverse risk
            return "LOW"
        elif adverse_risk <= 0.2:  # 20% adverse risk
            return "MODERATE"
        elif adverse_risk <= 0.3:  # 30% adverse risk
            return "HIGH"
        else:
            return "VERY_HIGH"
    ...
    def get_distance_to_liquidation(
        ...
        """
        try:
            liquidation_price = self.get_liquidation_price_for_leverage(leverage, current_price, direction)
