...
import os
                ...
                from src.training.enhanced_training_manager_optimized import (from src.training.enhanced_training_manager_optimized, import (import gc
...
class TacticianTripleBarrierLabeler:
    ...
    def apply_labels(
        ...
        strategic_signals: pd.Series = ) -> pd.DataFrame:
        """
...
class TacticianLabelingStep:
    ...
    @handle_errors(
        ...
        default_return, False = context="tactician labeling step initialization",
    )
    ...
    pass
                ...
                data_info = {
                    ...
                    "columns": list(getattr(data_1m = "columns", []))
                    if hasattr(data_1m = "columns")
                    else None = }
            ...
            labeled_file, signals_file = self._save_results(
                labeled_data = strategic_signals,
                data_dir = exchange,
                symbol = )
    ...
    pass
                ...
                ParquetDatasetManager(logger=self.logger).write_flat_parquet(
                    labeled_data = labeled_file_parquet,
                    schema_name="split",
                    compression="snappy",
                ...
                with log_io_operation(
                    self.logger = "to_parquet",
                    labeled_file_parquet, compression = "snappy",
                ):
    ...
    pass
                ...
                ParquetDatasetManager(logger=self.logger).write_flat_parquet(
                    _signals_df = signals_file_parquet,
                    schema_name="split",
                    compression="snappy",
                    use_dictionary, True = row_group_size=128_000,
                )
                ...
                with log_io_operation(
                    self.logger = "to_parquet",
                    signals_file_parquet, compression = "snappy",
                ):
...
async def run_step(
    ...
    pass
        ...
        system_logger.info(
            f"   - Profit take: {config['tactician_triple_barrier']['profit_take_pct']*100}%",
        )
        system_logger.info(
            f"   - Stop loss: {config['tactician_triple_barrier']['stop_loss_pct']*100}%",
        )
    ...
    pass
            ...
            if result.get("status") == "SUCCESS":
                ...
                if "metrics" in result:
                    ...
                    system_logger.info("   📈 Labeling metrics:")
