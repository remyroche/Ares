...
async def main():
    ...
    logger.info(f"  Enabled Optimizations: {optimization_summary['optimizations_enabled']}")
    ...
    for category, improvements in performance_expectations["computational_time_reduction"].items():
        logger.info(f"  {category}: {improvements['min']}-{improvements['max']}% reduction")
    ...
    if not await training_manager.initialize():
        logger.error("Failed to initialize training manager")
        return
    ...
    logger.info(f"Final memory usage: {final_snapshot['process_memory']['rss_mb']:.1f}MB")
    ...
    if memory_trends["status"] == "success":
        ...
        logger.info(f"Memory usage trends - Mean: {rss_stats['mean']:.1f}MB, "
    ...
    if hasattr(step_executor, 'get_execution_stats'):
        ...
        logger.info(f"  Parallel execution: {exec_stats.get('parallel_execution_enabled', False)}")
    ...
    logger.info(f"Optimizations Used: {list(optimization_summary['optimizations_enabled'].keys())}")
    logger.info(f"Memory Freed: {memory_freed:.1f}MB")
    logger.info(f"Memory Leak Status: {'Detected' if leak_results['leak_detected'] else 'Clean'}")
...
def demonstrate_individual_components():
    ...
    logger.info(f"Memory freed: {optimization_results['garbage_collection']['memory_freed_mb']:.1f}MB")
