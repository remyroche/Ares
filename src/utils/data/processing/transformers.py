...
try:
    from src.features_common.transforms.base_scaler import (
        BaseScaler,
        create_optimized_scaler,
    )
    from src.features_common.transforms.vectorbt_scaler import (
        VectorBTScaler,
        VectorBTBatchScaler,
    )
...
class DataStreamingManager:
    ...
    def __init__(
        ...
        should_log_init = not type(self)._init_log_emitted
        if should_log_init:
            self.logger.info(
                f"🚀 DataStreamingManager initialized (singleton): chunk_size={chunk_size}, memory_threshold={memory_threshold}"
            )
            type(self)._init_log_emitted = True
        else:
            self.logger.debug("DataStreamingManager initialization log suppressed")
        ...
        if should_log_init:
            try:
                ...
                self.logger.info(
                    f"⏱️ DataStreamingManager initialized in {duration:.3f}s"
                )
...
class DataTransformer:
    ...
    def _initialize_scalers(self):
        ...
        if not SCALER_AVAILABLE:
            self.logger.warning(
                "BaseScaler not available, using fallback implementations"
            )
            return
        ...
        try:
            ...
            self.scalers = {
                "zscore": (
                    create_optimized_scaler("zscore")
                    if create_optimized_scaler
                    else None
                ),
                "minmax": (
                    create_optimized_scaler("minmax")
                    if create_optimized_scaler
                    else None
                ),
                "robust": (
                    create_optimized_scaler("robust")
                    if create_optimized_scaler
                    else None
                ),
                "quantile": (
                    create_optimized_scaler("quantile")
                    if create_optimized_scaler
                    else None
                ),
            }
    ...
    def transform_features(
        self, data: pd.DataFrame, transformations: List[str] = None
    ) -> pd.DataFrame:
        ...
        for transformation in transformations:
            try:
                ...
                self.logger.warning(
                    f"Failed to apply transformation {transformation}: {e}"
                )

        self.logger.info(
            f"Applied {len(transformations)} transformations to {len(data.columns)} features"
        )
    ...
    def _apply_scaler_to_dataframe(
        self, data: pd.DataFrame, scaler: BaseScaler
    ) -> pd.DataFrame:
    ...
    def should_chunk_data(self, data: pd.DataFrame) -> bool:
        ...
        if current_memory > self.memory_threshold:
            self.logger.info(
                f"📊 Memory usage high ({current_memory:.2%}), chunking recommended"
            )
            return True
        if len(data) > self.chunk_size * 2:
            self.logger.info(
                f"📊 Large dataset ({len(data)} rows), chunking recommended"
            )
            return True
        memory_footprint = data.memory_usage(deep=True).sum() / 1024**3
        if memory_footprint > 1.0:
            self.logger.info(
                f"📊 Large memory footprint ({memory_footprint:.2f} GB), chunking recommended"
            )
            return True
        return False
    ...
    def _create_time_based_chunks(
        self, data: pd.DataFrame, preserve_order: bool
    ) -> Generator[pd.DataFrame, None, None]:
        ...
        while current_time < data["timestamp"].max():
            ...
            chunk_mask = (data["timestamp"] >= current_time) & (
                data["timestamp"] < chunk_end_time
            )
            chunk_data = data[chunk_mask].copy()
            if len(chunk_data) > 0:
                if chunk_index > 0 and self.overlap_size > 0:
                    overlap_start = current_time - timedelta(
                        seconds=self.overlap_size * 60
                    )
                    overlap_mask = (data["timestamp"] >= overlap_start) & (
                        data["timestamp"] < current_time
                    )
                    overlap_data = data[overlap_mask].copy()
                    if len(overlap_data) > 0:
                        chunk_data = pd.concat(
                            [overlap_data, chunk_data], ignore_index=True
                        )
                        chunk_data = chunk_data.drop_duplicates(
                            subset=["timestamp"], keep="last"
                        )
                self.logger.info(
                    f"📦 Created time-based chunk {chunk_index + 1}: {len(chunk_data)} rows, time range: {chunk_data['timestamp'].min()} to {chunk_data['timestamp'].max()}"
                )
                yield chunk_data
                chunk_index += 1
    ...
    def _create_size_based_chunks(
        ...
        for start_idx in range(0, total_rows, self.chunk_size):
            ...
            self.logger.info(
                f"📦 Created size-based chunk {chunk_index + 1}: {len(chunk_data)} rows (rows {start_idx}-{end_idx - 1})"
            )
            yield chunk_data
            chunk_index += 1
        ...
        try:
            for chunk in self.create_data_chunks(data):
                ...
                try:
                    ...
                    self.performance_metrics["total_rows_processed"] += len(
                        processed_chunk
                    )
                    current_memory = self.get_memory_usage()
                    ...
                    if current_memory > self.memory_threshold * 0.9:
                        self.logger.info(
                            "🧹 High memory usage, triggering garbage collection..."
                        )
                        gc.collect()
                except Exception as e:
            ...
            if combine_results and processed_chunks:
                ...
                if self.overlap_size > 0 and "timestamp" in processed_chunks[0].columns:
                    combined_data = self._combine_chunks_without_overlap(
                        processed_chunks
                    )
                else:
                    combined_data = pd.concat(processed_chunks, ignore_index=True)
                combined_data = self.standards.validate_data_quality(
                    combined_data, "unified"
                )
                self.logger.info(
                    f"✅ Dataset processing completed: {len(combined_data)} rows"
                )
                return combined_data
            else:
                self.logger.info(
                    f"✅ Dataset processing completed: {len(processed_chunks)} chunks"
                )
    ...
    def _combine_chunks_without_overlap(
            ...
            if "timestamp" in chunk.columns:
                ...
                if len(non_overlap_data) > 0:
                    combined_data = pd.concat(
                        [combined_data, non_overlap_data], ignore_index=True
                    )
            else:
    ...
    def _log_performance_summary(self) -> None:
        ...
        self.logger.info(
            f"   - Total processing time: {metrics['processing_time_total']:.2f}s"
        )
        if metrics["chunks_processed"] > 0:
            avg_chunk_time = (
                metrics["processing_time_total"] / metrics["chunks_processed"]
            )
            self.logger.info(
                f"   - Average chunk processing time: {avg_chunk_time:.2f}s"
            )
