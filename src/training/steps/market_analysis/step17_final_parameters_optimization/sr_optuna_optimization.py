...
class SROptunaOptimizer:
    ...
    def __init__(self, config: dict[str, Any], storage_url: str='sqlite:///sr_optuna_studies.db', study_name_prefix: str='sr_optimization') -> None:
        ...
        self.objectives = self.sr_config.get('objectives', ['sharpe_ratio', 'win_rate', 'signal_clarity'])
        self.objective_weights = self.sr_config.get('objective_weights', {'sharpe_ratio': 0.4, 'win_rate': 0.3, 'signal_clarity': 0.3})
    ...
    async def optimize_sr_parameters(self, price_data: pd.DataFrame, target_returns: pd.Series, study_name: str | None=None) -> SROptimizationResult:
        ...
        try:
            ...
            result = self._create_optimization_result(study, best_trial, optimization_time, study_name)
            self.logger.info(f'✅ S/R optimization completed in {optimization_time:.2f}s')
            self.logger.info(f'📊 Best optimization score: {result.optimization_score:.4f}')
            return result
        except Exception as e:
            self.logger.exception(f'❌ Error in S/R optimization: {e}')
            return None

    async def _evaluate_sr_parameters(self, trial: optuna.Trial, price_data: pd.DataFrame, target_returns: pd.Series) -> float:
        """
        ...
        try:
            ...
            sr_features = self.sr_predictor.calculate_comprehensive_sr_features(price_sample)
            if not sr_features:
                return 0.0
    ...
    def _calculate_performance_metrics(self, sr_features: dict[str, pd.Series], target_returns: pd.Series, level_params: dict[str, Any], breakout_params: dict[str, float], zone_params: dict[str, float], confidence_params: dict[str, float]) -> dict[str, float]:
        ...
        try:
            ...
            return {'sharpe_ratio': sharpe_ratio, 'max_drawdown': max_drawdown, 'win_rate': win_rate, 'profit_factor': profit_factor, 'total_return': total_return, 'signal_clarity': signal_clarity, 'noise_reduction': noise_reduction}
        except Exception as e:
    ...
    def _calculate_trading_signals(self, strength_scores: pd.Series, sr_proximity: pd.Series, directional_pressure: pd.Series, confidence_params: dict[str, float]) -> pd.Series:
        ...
        try:
            ...
            weak_long_mask = (combined_signal > min_confidence) & (combined_signal <= high_confidence)
            weak_short_mask = (combined_signal < -min_confidence) & (combined_signal >= -high_confidence)
    ...
    def _calculate_optimization_score(self, metrics: dict[str, float]) -> float:
        ...
        try:
            sharpe_norm = max(0, metrics['sharpe_ratio']) / 2.0
            win_rate_norm = metrics['win_rate']
            clarity_norm = metrics['signal_clarity']
            score = self.objective_weights['sharpe_ratio'] * sharpe_norm + self.objective_weights['win_rate'] * win_rate_norm + self.objective_weights['signal_clarity'] * clarity_norm
        ...
        try:
            ...
            for feature_values in sr_features.values():
                if len(feature_values) > 1:
                    cv = feature_values.std() / (abs(feature_values.mean()) + 1e-08)
    ...
    def _create_optimization_result(self, study: optuna.Study, best_trial: optuna.Trial, optimization_time: float, study_name: str='sr_optimization') -> SROptimizationResult:
        ...
        try:
            ...
            strength_score_weights = {k: v for k, v in params.items() if k in ['touch_count', 'total_volume', 'level_age', 'bounce_rate', 'isolation_score']}
            level_detection_params = {k: v for k, v in params.items() if k in ['min_touch_count', 'min_level_age_hours', 'price_tolerance_pct', 'volume_threshold', 'strength_threshold']}
            breakout_thresholds = {k: v for k, v in params.items() if k in ['breakout_threshold', 'confirmation_periods', 'volume_confirmation', 'momentum_threshold', 'false_breakout_filter']}
            zone_multipliers = {k: v for k, v in params.items() if k in ['support_zone_multiplier', 'resistance_zone_multiplier', 'sr_zone_threshold', 'zone_expansion_factor', 'zone_contraction_factor']}
            confidence_thresholds = {k: v for k, v in params.items() if k in ['min_sr_confidence', 'high_confidence_threshold', 'confidence_decay_rate', 'regime_confidence_boost', 'ensemble_confidence_threshold']}
...
if __name__ == '__main__':
    ...
    async def main() -> None:
        config = {'sr_optimization': {'multi_objective': True, 'objectives': ['sharpe_ratio', 'win_rate', 'signal_clarity'], 'objective_weights': {'sharpe_ratio': 0.4, 'win_rate': 0.3, 'signal_clarity': 0.3}, 'n_trials': 50, 'cv_folds': 5, 'early_stopping_patience': 15, 'subsample_fraction': 0.7}}
