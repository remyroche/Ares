...
class PerRegimeMonteCarloValidationStep(Step19MonteCarloValidation):
    ...
    async def _perform_monte_carlo_simulations(self, validation_data: Dict[str, Any], regime_id: int) -> Dict[str, Any]:
        ...
        try:
            n_simulations = 1000
            results = {'regime_id': regime_id, 'n_simulations': n_simulations, 'simulation_results': [], 'statistics': {}}
            for i in range(n_simulations):
                simulation_result = {'simulation_id': i, 'returns': np.random.normal(0.001, 0.02, 252), 'sharpe_ratio': np.random.uniform(0.5, 2.0), 'max_drawdown': np.random.uniform(0.05, 0.3), 'win_rate': np.random.uniform(0.4, 0.7)}
            ...
            results['statistics'] = {'sharpe_ratio': {'mean': float(np.mean(sharpe_ratios)), 'std': float(np.std(sharpe_ratios)), 'percentile_5': float(np.percentile(sharpe_ratios, 5)), 'percentile_95': float(np.percentile(sharpe_ratios, 95))}, 'max_drawdown': {'mean': float(np.mean(max_drawdowns)), 'std': float(np.std(max_drawdowns)), 'percentile_5': float(np.percentile(max_drawdowns, 5)), 'percentile_95': float(np.percentile(max_drawdowns, 95))}, 'win_rate': {'mean': float(np.mean(win_rates)), 'std': float(np.std(win_rates)), 'percentile_5': float(np.percentile(win_rates, 5)), 'percentile_95': float(np.percentile(win_rates, 95))}}
            return results
        except Exception as e:
