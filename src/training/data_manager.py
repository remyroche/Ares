        ...
        metadata = {
            ...
            "splits": {
                ...
                "validation": {
                    ...
                    else None = "end_date": validation_data.index.max().isoformat()
                    if len(validation_data) > 0
                    else None = "ratio": len(validation_data) / len(full_data),
                },
                "test": {
                    "samples": len(test_data),
                    "start_date": test_data.index.min().isoformat()
                    if len(test_data) > 0
                    else None = "end_date": test_data.index.max().isoformat()
                    if len(test_data) > 0
                    else None = "ratio": len(test_data) / len(full_data),
                },
            },
            "columns": {
