...
class BaseEnsemble:
    ...
    def __init__(self, config: dict, ensemble_name: str, enable_m1_optimizations: bool = True) -> None:
        ...
        self.sequence_features = ['close', 'volume', 'ADX', 'MACD_HIST', 'ATR', 'volume_delta', 'autoencoder_reconstruction_error', 'Realized_Volatility', 'Volatility_Regime_Numeric', 'Hour_Sin', 'Hour_Cos', 'DayOfWeek_Sin', 'DayOfWeek_Cos', 'VROC', 'OBV_Divergence', 'Buy_Sell_Pressure_Ratio', 'Order_Flow_Imbalance', 'Price_Momentum', 'Volatility_Momentum', 'RSI_MACD_Divergence', 'Volume_Price_Divergence', 'distance_to_sr', 'sr_strength', 'sr_type', 'price_position', 'momentum_5', 'momentum_10', 'volume_ratio', 'volatility']
...
def train_ensemble(self, historical_features: pd.DataFrame, historical_targets: pd.Series | None = None) -> None:
    ...
    def _validate_ensemble_state(self) -> bool:
        ...
        try:
            ...
            self.logger.exception(f'{self.ensemble_name}: Error validating ensemble state: {e}')
            return False
        ...
        try:
            ...
            for i in range(len(meta_input_pca)):
                try:
                    ...
                    predictions.append({'prediction': pred_result['prediction'], 'confidence': pred_result['confidence']})
                except Exception as e:
            ...
            self.logger.exception(f'{self.ensemble_name}: Error in historical prediction: {e}')
            return pd.DataFrame()
    ...
    def check_ensemble_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            if not self.config:
                ...
                status = 'degraded'
            health_report = {'status': status, 'ensemble_name': self.ensemble_name, 'trained': self.trained, 'num_base_models': len(self.models) if self.models else 0, 'has_meta_learner': self.meta_learner is not None, 'has_scaler': self.meta_feature_scaler is not None, 'has_encoder': self.label_encoder is not None, 'has_pca': self.pca is not None, 'issues': issues, 'timestamp': pd.Timestamp.now().isoformat()}
            if status == 'healthy':
    ...
    def save_model(self, path: str) -> None:
        ...
        try:
            model_data = {'models': self.models, 'meta_learner': self.meta_learner, 'pca': self.pca, 'meta_feature_scaler': self.meta_feature_scaler, 'label_encoder': self.label_encoder, 'trained': self.trained, 'best_meta_params': self.best_meta_params, 'ensemble_weights': self.ensemble_weights}
    ...
    def _extract_pivot_levels(self, sr_analyzer: Any, features_df: pd.DataFrame | None = None) -> dict[str, list[float]]:
        ...
        try:
            ...
            return {'supports': list(set(supports)), 'resistances': list(set(resistances))}
        except Exception as e:
    ...
    def _extract_hvn_levels(self, sr_analyzer: Any, features_df: pd.DataFrame | None = None) -> dict[str, list[float]]:
        ...
        try:
            supports: list[float] = []
            resistances: list[float] = []
            if features_df is not None and len(features_df) >= 720:
    ...
    def _calculate_sr_distances(self, sr_features: pd.DataFrame, row_idx: int, current_price: float, pivot_levels: dict, hvn_levels: dict, current_location: str | None = None) -> None:
        ...
        try:
            ...
            sr_features.iloc[row_idx, sr_features.columns.get_loc('is_pivot_support')] = 1.0 if current_location and 'PIVOT_S' in current_location else 0.0
            sr_features.iloc[row_idx, sr_features.columns.get_loc('is_pivot_resistance')] = 1.0 if current_location and 'PIVOT_R' in current_location else 0.0
            sr_features.iloc[row_idx, sr_features.columns.get_loc('is_hvn_support')] = 1.0 if current_location and 'HVN_SUPPORT' in current_location else 0.0
            sr_features.iloc[row_idx, sr_features.columns.get_loc('is_hvn_resistance')] = 1.0 if current_location and 'HVN_RESISTANCE' in current_location else 0.0
            sr_features.iloc[row_idx, sr_features.columns.get_loc('is_confluence')] = 1.0 if current_location and 'CONFLUENCE' in current_location else 0.0
            sr_features.iloc[row_idx, sr_features.columns.get_loc('is_open_range')] = 1.0 if current_location == 'OPEN_RANGE' else 0.0
        except Exception as e:
        ...
        try:
            ...
            for i in range(len(df)):
                try:
                    ...
                    sr_features.iloc[i, sr_features.columns.get_loc('distance_to_sr')] = 1.0
                    sr_features.iloc[i, sr_features.columns.get_loc('sr_strength')] = 0.0
                    sr_features.iloc[i, sr_features.columns.get_loc('sr_type')] = 0.5
                except Exception as e:
                    self.logger.warning(f'Error calculating simplified SR features for row {i}: {e}')
                    continue
        except Exception as e:
        ...
        try:
            ...
            for feature in spread_features:
                if feature in df.columns:
                    if feature in ['spread_liquidity', 'price_impact']:
                        normalized_df[f'{feature}_bps'] = df[feature] * 10000.0
                    normalized_df[f'{feature}_z_score'] = self._calculate_rolling_z_score(df[feature], self.normalization_windows['medium'])
                    normalized_df[f'{feature}_pct_change'] = df[feature].pct_change()
                    normalized_df[f'{feature}_change'] = df[feature].diff().fillna(0)
            liquidity_features = ['volume_liquidity', 'liquidity_percentile', 'liquidity_health']
            for feature in liquidity_features:
                if feature in df.columns:
                    if feature in ['volume_liquidity']:
                        normalized_df[f'{feature}_log'] = np.log1p(np.abs(df[feature]))
                    normalized_df[f'{feature}_z_score'] = self._calculate_rolling_z_score(df[feature], self.normalization_windows['medium'])
                    if feature not in ['liquidity_percentile']:
                        normalized_df[f'{feature}_change'] = df[feature].diff().fillna(0)
            ...
            for feature in ofi_features:
                if feature in df.columns:
                    ...
                    normalized_df[f'{feature}_z_score'] = self._calculate_rolling_z_score(normalized_df[f'{feature}_bounded'], self.normalization_windows['medium'])
            ...
            if 'vwap' in df.columns and 'close' in df.columns:
                mid_price = df['close']
                normalized_df['vwap_deviation'] = (df['vwap'] - mid_price) / (mid_price + 1e-08)
                normalized_df['vwap_deviation_z_score'] = self._calculate_rolling_z_score(normalized_df['vwap_deviation'], self.normalization_windows['medium'])
            if 'large_order_ratio' in df.columns:
                normalized_df['large_order_ratio_bounded'] = np.clip(df['large_order_ratio'], 0, 1)
                normalized_df['large_order_ratio_z_score'] = self._calculate_rolling_z_score(normalized_df['large_order_ratio_bounded'], self.normalization_windows['medium'])
            ...
            for feature in volatility_features:
                if feature in df.columns:
                    ...
                    normalized_df[f'{feature}_z_score'] = self._calculate_rolling_z_score(normalized_df.get(f'{feature}_log', df[feature]), self.normalization_windows['medium'])
                    normalized_df[f'{feature}_pct_change'] = df[feature].pct_change()
            ...
            for feature in momentum_features:
                if feature in df.columns:
                    normalized_df[f'{feature}_z_score'] = self._calculate_rolling_z_score(df[feature], self.normalization_windows['medium'])
                    normalized_df[f'{feature}_acceleration'] = df[feature].diff(3).fillna(0)
