...
class Step17ProbabilisticBayesianOptimization:
    ...
    async def _prepare_optimization_features(
        self, market_data: pd.DataFrame, historical_data: pd.DataFrame
    ) -> np.ndarray:
                    ...
                    for objective, solution in best_solutions.items():
                        if objective in ["total_profit", "win_rate", "sharpe_ratio"]:
                            summary["recommendations"].append(
                                f"Use {model_type} parameters for {objective}: " f"{solution['value']:.4f}"
                            )
                ...
                for param_name, param_value in params.items():
                    if isinstance(param_value, (int, float)):
                        ...
                        param_uncertainty[param_name] = {
                            ...
                            "confidence_interval": [param_value - uncertainty_range, param_value + uncertainty_range],
                        }
        ...
        for model_type, uncertainty in analysis.get("uncertainty_estimates", {}).items():
            if uncertainty:
                high_uncertainty_params = [
                    param for param, data in uncertainty.items() if data.get("uncertainty", 0) > 0.1
                ]
    ...
    async def _generate_final_report(
        self, analysis_results: Dict[str, Any], application_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if performance_validation.get("improvements_detected"):
            report["optimization_summary"]["performance_improvements"] = performance_validation.get(
                "improvement_metrics", {}
            )
