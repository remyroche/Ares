...
class WalkForwardValidator:
    ...
    def generate_walk_forward_windows(self, data: pd.DataFrame) -> List[WalkForwardWindow]:
        ...
        while train_end + timedelta(days=self.test_period_days) <= end_date:
            ...
            if self.adaptive_windows:
                window_params = self._adjust_window_for_volatility(
    ...
    def _adjust_window_for_volatility(self, data: pd.DataFrame, 
                                    train_start: datetime, 
                                    train_end: datetime) -> Optional[Dict[str, datetime]]:
        """Adjust window size based on market volatility."""
        
        # Calculate volatility in the training period
        train_data = data[train_start:train_end]
        returns = train_data['close'].pct_change().dropna()
        volatility = returns.std()
        
        if volatility > self.volatility_threshold:
            # High volatility - use shorter windows
            new_train_days = int(self.train_period_days * 0.5)
            new_test_days = int(self.test_period_days * 0.5)
        else:
            # Normal volatility - use standard windows
            return None
            
        return {
            'train_start': train_end - timedelta(days=new_train_days),
            'test_end': train_end + timedelta(days=new_test_days)
        }
    
    async def validate_model(self, model_trainer: Callable, 
                           data: pd.DataFrame,
        ...
        if self.regime_aware and regime_labels is not None:
            results = await self._validate_regime_aware(
                model_trainer, data, regime_labels, windows
            )
        else:
            results = await self._validate_standard(
                model_trainer, data, windows
            )
    ...
    async def _validate_standard(self, model_trainer: Callable,
                               data: pd.DataFrame,
                               windows: List[WalkForwardWindow]) -> List[Dict[str, Any]]:
    ...
    def _validate_single_window(self, model_trainer: Callable,
                              data: pd.DataFrame,
                              window: WalkForwardWindow) -> Dict[str, Any]:
        ...
        try:
            ...
            train_metrics = self._calculate_metrics(
                train_data, train_predictions, 'train'
            )
            test_metrics = self._calculate_metrics(
                test_data, test_predictions, 'test'
            )
    ...
    async def _validate_regime_aware(self, model_trainer: Callable,
                                   data: pd.DataFrame,
        ...
        for window in windows:
            ...
            for regime in ['bull', 'bear', 'sideways']:
                regime_result = await self._validate_regime_window(
                    model_trainer, 
    ...
    async def _validate_regime_window(self, model_trainer: Callable,
        ...
        if len(train_regime_data) < self.min_samples_per_regime:
            return {
                'regime': regime,
                'error': f'Insufficient {regime} training samples: {len(train_regime_data)}',
                'success': False
            }
        
        if len(test_regime_data) < 10:  # Minimum test samples
            return {
                'regime': regime,
                'error': f'Insufficient {regime} test samples: {len(test_regime_data)}',
                'success': False
            }
        ...
        try:
            ...
            # Calculate metrics
            train_metrics = self._calculate_metrics(
                train_regime_data, train_predictions, f'train_{regime}'
            )
            test_metrics = self._calculate_metrics(
                test_regime_data, test_predictions, f'test_{regime}'
            )
            ...
            return {
                'regime': regime,
                'train_samples': len(train_regime_data),
                'test_samples': len(test_regime_data),
                'train_metrics': train_metrics,
                'test_metrics': test_metrics,
                'degradation': degradation,
                'success': True
    ...
    def _calculate_metrics(self, data: pd.DataFrame, 
        ...
        # Simple return calculation (would be more complex in practice)
        ...
        metrics = {
            ...
            f'{prefix}_win_rate': (strategy_returns > 0).mean(),
            f'{prefix}_total_return': strategy_returns.sum(),
            f'{prefix}_volatility': strategy_returns.std()
        }
    ...
    def _calculate_degradation(self, train_metrics: Dict[str, float],
        ...
        degradation['overall'] = (degradation['sharpe_degradation'] + 
                                 degradation['win_rate_degradation']) / 2
    ...
    def _analyze_validation_results(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'successful_windows': sum(1 for r in results if r.get('success', False)),
                ...
                for result in results:
                    if 'regime_results' in result:
                        ...
                        if regime_result.get('success', False):
                            regime_degradations.append(
                                regime_result['degradation']['overall']
                            )
                
                if regime_degradations:
                    analysis['regime_analysis'][regime] = {
                        'avg_degradation': np.mean(regime_degradations),
        ...
        analysis['validation_passed'] = (
            ...
            analysis['overfitting_windows'] / max(analysis['successful_windows'], 1) < 0.3
        )
...
# Example model trainer for testing
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        # regime_labels = np.load("regime_labels.npy")
