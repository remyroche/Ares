...
class QualityResult:
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {'passed': self.passed, 'issue_count': len(self.issues), 'warning_count': len(self.warnings), 'metrics': self.metrics, 'issues': self.issues[:5], 'warnings': self.warnings[:5]}
...
class EnhancedDataQualityValidator:
    ...
    def _validate_correlations(self, df: pd.DataFrame, result: QualityResult) -> None:
        """Validate correlations between numeric columns, excluding OHLCV columns which are expected to be highly correlated."""
        numeric_columns = df.select_dtypes(include=[np.number]).columns
        
        # Exclude OHLCV columns from correlation analysis as they are expected to be highly correlated
            ...
            for i in range(len(corr_matrix.columns)):
                for j in range(i + 1, len(corr_matrix.columns)):
                    ...
                    if abs(corr_value) > self.thresholds.max_correlation_threshold:
                        high_corr_pairs.append({'col1': corr_matrix.columns[i], 'col2': corr_matrix.columns[j], 'correlation': corr_value})
...
class UnifiedDataQualityValidator(EnhancedDataQualityValidator):
    ...
    def _validate_unified_structure(self, df: pd.DataFrame, result: QualityResult) -> None:
    ...
    def _validate_data_consistency(self, df: pd.DataFrame, result: QualityResult) -> None:
        ...
        if 'exchange' in df.columns:
            ...
            if len(exchange_counts) > 1:
                ...
                if cv > 0.5:
                    issues.append({'type': 'uneven_exchange_distribution', 'exchange_counts': exchange_counts.to_dict(), 'coefficient_of_variation': cv})
        if 'symbol' in df.columns:
            ...
            if len(symbol_counts) > 1:
                ...
                cv = std_count / mean_count if mean_count > 0 else 0
                if cv > 0.5:
                    issues.append({'type': 'uneven_symbol_distribution', 'symbol_counts': symbol_counts.to_dict(), 'coefficient_of_variation': cv})
