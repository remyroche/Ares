...
def test_data_collection_config_exposes_locator_paths(tmp_path: Path) -> None:
...
def test_data_collection_pipeline_uses_locator_directories(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "src.training.steps.data_collection.sub_pipeline.UNIFIED_COMPONENTS_AVAILABLE",
        False,
        raising=False,
    )
