...
class UnifiedRegimeIntelligenceValidator:
    ...
    def _validate_configuration(self) -> bool:
        ...
        try:
            ...
            if self.model_performance_threshold <= 0 or self.model_performance_threshold > 1:
                self.logger.error("Invalid model performance threshold")
                return False

            if self.artifact_completeness_threshold <= 0 or self.artifact_completeness_threshold > 1:
        ...
        try:
            ...
            for tf in timeframes:
                ...
                if os.path.exists(hmm_file):
                    ...
                    if not hmm_data.empty and "composite_cluster_id" in hmm_data.columns:
                        hmm_files_found += 1
            ...
            validation_results["intensity_data_completeness"] = intensity_files_found / max(len(timeframes), 1)
    ...
    async def validate_training_process(self, training_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if training_data and "feature_tensor" in training_data:
                ...
                if isinstance(feature_tensor, torch.Tensor) and feature_tensor.shape[0] > 0:
                    validation_results["sequence_creation"] = True

            # Check label encoding
            if training_data and "labels" in training_data:
                labels = training_data["labels"]
                if isinstance(labels, dict) and all(k in labels for k in ["regime", "transition", "tpsl"]):
    ...
    async def validate_artifacts(self, artifacts_dir: str) -> bool:
        ...
        try:
            ...
            validation_results["label_encoders"] = encoders_found >= 2  # At least 2 out of 3
            ...
            if validation_results["overall_score"] >= self.artifact_completeness_threshold:
                self.logger.info(
    ...
    @handle_errors(
        exceptions=(Exception,),
        default_return=False,
        context="predictions validation",
    )
    ...
    async def run_comprehensive_validation(
        ...
        try:
            ...
            checks = [
                ...
                ("Predictions", await self.validate_predictions(model, test_data or {})),
                ("S/R Integration", await self.validate_sr_integration(model)),
            ]
