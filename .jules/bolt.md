...
## 2026-05-09 - [Inference Loop] Learning: Using `.iterrows()` to simulate OCO policies on 5-minute bars in `_evaluate_oco_policy` is very slow due to Pandas Series instantiation. Action: Replaced `.iterrows()` with `numpy` arrays via `.to_numpy()` and indexed iteration (`bars_index[i]`, etc.), improving loop performance significantly for large inputs.
