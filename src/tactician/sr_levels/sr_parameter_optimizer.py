...
class SRParameterOptimizer:
    ...
    def _get_parameter_ranges(self) -> Dict[str, Tuple[float, float]]:
        ...
        return {'price_action_weight': (0.1, 0.4), 'momentum_weight': (0.1, 0.3), 'trend_strength_weight': (0.1, 0.3), 'volume_weight': (0.1, 0.3), 'volume_surge_multiplier': (1.5, 3.0), 'volume_confirmation_threshold': (1.2, 2.0), 'volatility_weight': (0.05, 0.2), 'high_volatility_breakout_boost': (0.05, 0.25), 'low_volatility_consolidation_boost': (0.05, 0.2), 'level_strength_weight': (0.1, 0.3), 'touch_count_weight': (0.2, 0.4), 'age_decay_factor': (0.9, 0.99), 'proximity_threshold': (0.001, 0.005), 'proximity_decay_rate': (1.5, 3.0), 'min_breakout_probability': (0.1, 0.3), 'max_breakout_probability': (0.7, 0.9)}

    @handles_errors(exceptions=(ValueError, AttributeError), default_return=None, context='optimize SR parameters')
    @traced(span_name='SROptimizer.optimize')
    async def optimize_parameters(self, market_data: pd.DataFrame, sr_levels: List[Dict[str, Any]], validation_data: Optional[pd.DataFrame]=None) -> OptimizationResult:
        """
        ...
        try:
            ...
            study = optuna.create_study(direction='maximize', study_name='sr_parameter_optimization')

            def objective(trial: Any) -> None:
                return self._optimization_objective(trial, market_data, sr_levels)
            study.optimize(objective, n_trials=self.n_trials, show_progress_bar=True)
            best_params = study.best_params
            best_value = study.best_value
            optimized_params = self._create_parameter_set(best_params)
            if validation_data is not None:
                validation_score = await self._validate_parameters(optimized_params, validation_data, sr_levels)
                self.logger.info(f'Validation score: {validation_score:.4f}')
            backtest_metrics = await self._calculate_backtest_metrics(optimized_params, market_data, sr_levels)
            result = OptimizationResult(best_parameters=optimized_params, optimization_score=best_value, backtest_metrics=backtest_metrics, n_trials=len(study.trials), best_trial=study.best_trial.number, optimization_time=(datetime.now() - start_time).total_seconds())
    ...
    def _optimization_objective(self, trial: optuna.Trial, market_data: pd.DataFrame, sr_levels: List[Dict[str, Any]]) -> float:
        """Objective function for parameter optimization."""
        params = {}
        for param_name, (min_val, max_val) in self.param_ranges.items():
            params[param_name] = trial.suggest_float(param_name, min_val, max_val)
        weight_params = ['price_action_weight', 'momentum_weight', 'trend_strength_weight', 'volume_weight', 'volatility_weight']
        weight_sum = sum((params[p] for p in weight_params))
        for p in weight_params:
            params[p] /= weight_sum
        param_set = self._create_parameter_set(params)
        probabilities = self._calculate_sr_probabilities(market_data, sr_levels, param_set)
        score = self._evaluate_probability_accuracy(market_data, sr_levels, probabilities)
        return score

    def _calculate_sr_probabilities(self, market_data: pd.DataFrame, sr_levels: List[Dict[str, Any]], params: SRParameterSet) -> Dict[str, Dict[str, float]]:
        ...
        for i in range(len(market_data)):
            ...
            probs = self._calculate_point_probabilities(market_data, i, nearest_support, nearest_resistance, params)
            probabilities[timestamp] = probs
        return probabilities

    def _calculate_point_probabilities(self, market_data: pd.DataFrame, idx: int, nearest_support: Optional[Dict[str, Any]], nearest_resistance: Optional[Dict[str, Any]], params: SRParameterSet) -> Dict[str, float]:
        ...
        components = {}
        price_action_score = self._calculate_price_action_score(market_data, idx, params)
        components['price_action'] = price_action_score * params.price_action_weight
        momentum_score = self._calculate_momentum_score(market_data, idx, params)
        components['momentum'] = momentum_score * params.momentum_weight
        trend_score = self._calculate_trend_strength_score(market_data, idx, params)
        components['trend'] = trend_score * params.trend_strength_weight
        volume_score = self._calculate_volume_score(market_data, idx, params)
        components['volume'] = volume_score * params.volume_weight
        volatility_score = self._calculate_volatility_score(market_data, idx, params)
        components['volatility'] = volatility_score * params.volatility_weight
        if nearest_support or nearest_resistance:
            proximity_score = self._calculate_proximity_score(current_price, nearest_support, nearest_resistance, params)
            components['proximity'] = proximity_score
        combined_score = sum(components.values())
        if combined_score > 0.6:
            breakout_prob = min(params.max_breakout_probability, params.default_probability + combined_score * 0.5)
            rebounce_prob = params.default_probability * 0.7
            consolidation_prob = 1 - breakout_prob - rebounce_prob
        elif combined_score < 0.4:
            rebounce_prob = min(params.max_breakout_probability, params.default_probability + (1 - combined_score) * 0.5)
        ...
        return {'breakout': breakout_prob / total, 'rebounce': rebounce_prob / total, 'consolidation': consolidation_prob / total, 'combined_score': combined_score}

    def _calculate_price_action_score(self, market_data: pd.DataFrame, idx: int, params: SRParameterSet) -> float:
        """Calculate price action component score."""
        if idx < 20:
            return 0.5
        recent_returns = market_data['close'].iloc[idx - 10:idx].pct_change()
        bullish_candles = (market_data['close'].iloc[idx - 10:idx] > market_data['open'].iloc[idx - 10:idx]).sum()
        acceleration = recent_returns.diff().mean()
        score = 0.5
        score += 0.2 * (bullish_candles / 10 - 0.5)
        score += 0.3 * np.tanh(acceleration * 100)
        return np.clip(score, 0, 1)

    def _calculate_momentum_score(self, market_data: pd.DataFrame, idx: int, params: SRParameterSet) -> float:
    ...
    def _calculate_trend_strength_score(self, market_data: pd.DataFrame, idx: int, params: SRParameterSet) -> float:
    ...
    def _calculate_volume_score(self, market_data: pd.DataFrame, idx: int, params: SRParameterSet) -> float:
    ...
    def _calculate_volatility_score(self, market_data: pd.DataFrame, idx: int, params: SRParameterSet) -> float:
        """Calculate volatility component score."""
        if idx < 20:
            return 0.5
        high_low = market_data['high'].iloc[idx - 14:idx] - market_data['low'].iloc[idx - 14:idx]
        high_close = np.abs(market_data['high'].iloc[idx - 14:idx] - market_data['close'].iloc[idx - 15:idx - 1].values)
        low_close = np.abs(market_data['low'].iloc[idx - 14:idx] - market_data['close'].iloc[idx - 15:idx - 1].values)
    ...
    def _calculate_proximity_score(self, current_price: float, nearest_support: Optional[Dict[str, Any]], nearest_resistance: Optional[Dict[str, Any]], params: SRParameterSet) -> float:
    ...
    def _evaluate_probability_accuracy(self, market_data: pd.DataFrame, sr_levels: List[Dict[str, Any]], probabilities: Dict[str, Dict[str, float]]) -> float:
    ...
    async def _validate_parameters(self, params: SRParameterSet, validation_data: pd.DataFrame, sr_levels: List[Dict[str, Any]]) -> float:
        """Validate parameters on out-of-sample data."""
        probabilities = self._calculate_sr_probabilities(validation_data, sr_levels, params)
        score = self._evaluate_probability_accuracy(validation_data, sr_levels, probabilities)
        return score

    async def _calculate_backtest_metrics(self, params: SRParameterSet, market_data: pd.DataFrame, sr_levels: List[Dict[str, Any]]) -> Dict[str, float]:
        """Calculate detailed backtest metrics."""
        probabilities = self._calculate_sr_probabilities(market_data, sr_levels, params)
        metrics = {'accuracy': 0, 'precision_breakout': 0, 'precision_rebounce': 0, 'recall_breakout': 0, 'recall_rebounce': 0, 'profit_factor': 0, 'sharpe_ratio': 0, 'max_drawdown': 0, 'signal_count': 0}
    ...
    def _create_parameter_set(self, params: Dict[str, float]) -> SRParameterSet:
        ...
        return SRParameterSet(**{k: v for k, v in params.items() if k in SRParameterSet.__dataclass_fields__})
    ...
    def save_optimized_parameters(self, filepath: str) -> None:
        ...
        with open(filepath, 'w') as f:
            json.dump({'parameters': params_dict, 'optimization_history': [{'score': r.optimization_score, 'metrics': r.backtest_metrics, 'timestamp': datetime.now().isoformat()} for r in self.optimization_history[-10:]]}, f, indent=2)
