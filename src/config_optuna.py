...
class EnsembleParameters:
    ...
    def __post_init__(self) -> None:
        if self.regime_specific_weights is None:
            self.regime_specific_weights = {'BULL_TREND': 1.2, 'BEAR_TREND': 0.8, 'SIDEWAYS_RANGE': 0.9, 'HIGH_IMPACT_CANDLE': 0.6, 'SR_ZONE_ACTION': 1.1}
...
class MarketRegimeParameters:
    ...
    def __post_init__(self) -> None:
        if self.regime_specific_constraints is None:
            self.regime_specific_constraints = {'bull': {'tp_multiplier_range': [2.5, 5.0], 'sl_multiplier_range': [1.2, 2.5], 'position_size_range': [0.1, 0.25]}, 'bear': {'tp_multiplier_range': [2.0, 4.5], 'sl_multiplier_range': [1.0, 2.2], 'position_size_range': [0.08, 0.2]}, 'sideways': {'tp_multiplier_range': [1.5, 3.0], 'sl_multiplier_range': [0.8, 1.8], 'position_size_range': [0.06, 0.15]}}
...
class SROptimizationParameters:
    ...
    def __post_init__(self) -> None:
        ...
        if self.objective_weights is None:
            self.objective_weights = {'sharpe_ratio': 0.4, 'win_rate': 0.3, 'signal_clarity': 0.3}

    def get_strength_score_weights(self) -> dict[str, float]:
        """Get strength score weights as a dictionary."""
        return {'touch_count': self.touch_count_weight, 'total_volume': self.total_volume_weight, 'level_age': self.level_age_weight, 'bounce_rate': self.bounce_rate_weight, 'isolation_score': self.isolation_score_weight}

    def get_level_detection_params(self) -> dict[str, Any]:
        """Get level detection parameters as a dictionary."""
        return {'min_touch_count': self.min_touch_count, 'min_level_age_hours': self.min_level_age_hours, 'price_tolerance_pct': self.price_tolerance_pct, 'volume_threshold': self.volume_threshold, 'strength_threshold': self.strength_threshold}

    def get_breakout_thresholds(self) -> dict[str, float]:
        """Get breakout thresholds as a dictionary."""
        return {'breakout_threshold': self.breakout_threshold, 'confirmation_periods': self.confirmation_periods, 'volume_confirmation': self.volume_confirmation, 'momentum_threshold': self.momentum_threshold, 'false_breakout_filter': self.false_breakout_filter}

    def get_zone_multipliers(self) -> dict[str, float]:
        """Get zone multipliers as a dictionary."""
        return {'support_zone_multiplier': self.support_zone_multiplier, 'resistance_zone_multiplier': self.resistance_zone_multiplier, 'sr_zone_threshold': self.sr_zone_threshold, 'zone_expansion_factor': self.zone_expansion_factor, 'zone_contraction_factor': self.zone_contraction_factor}

    def get_confidence_thresholds(self) -> dict[str, float]:
        """Get confidence thresholds as a dictionary."""
        return {'min_sr_confidence': self.min_sr_confidence, 'high_confidence_threshold': self.high_confidence_threshold, 'confidence_decay_rate': self.confidence_decay_rate, 'regime_confidence_boost': self.regime_confidence_boost, 'ensemble_confidence_threshold': self.ensemble_confidence_threshold}
...
class HyperparameterOptimizationConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.objective_weights is None:
            self.objective_weights = {'accuracy': 0.4, 'f1_score': 0.4, 'precision': 0.2}
...
PARAMETER_SEARCH_SPACES = {'confidence_thresholds': {'base_entry_threshold': {'min': 0.5, 'max': 0.9, 'type': 'float'}, 'analyst_confidence_threshold': {'min': 0.6, 'max': 0.9, 'type': 'float'}, 'tactician_confidence_threshold': {'min': 0.7, 'max': 0.95, 'type': 'float'}, 'sr_zone_threshold': {'min': 0.6, 'max': 0.9, 'type': 'float'}}, 'volatility_parameters': {'target_volatility': {'min': 0.1, 'max': 0.25, 'type': 'float'}, 'volatility_lookback_period': {'min': 10, 'max': 50, 'type': 'int'}, 'volatility_multiplier': {'min': 0.5, 'max': 2.0, 'type': 'float'}}, 'ensemble_parameters': {'analyst_weight': {'min': 0.2, 'max': 0.6, 'type': 'float'}, 'tactician_weight': {'min': 0.2, 'max': 0.5, 'type': 'float'}, 'strategist_weight': {'min': 0.2, 'max': 0.5, 'type': 'float'}}, 'risk_management_parameters': {'max_portfolio_risk': {'min': 0.1, 'max': 0.25, 'type': 'float'}, 'max_single_position': {'min': 0.1, 'max': 0.25, 'type': 'float'}, 'max_drawdown': {'min': 0.15, 'max': 0.35, 'type': 'float'}}, 'market_regime_parameters': {'regime_lookback_period': {'min': 30, 'max': 100, 'type': 'int'}, 'regime_volatility_threshold': {'min': 0.01, 'max': 0.05, 'type': 'float'}, 'regime_trend_threshold': {'min': 0.005, 'max': 0.02, 'type': 'float'}, 'sr_zone_multiplier': {'min': 0.8, 'max': 1.5, 'type': 'float'}}, 'sr_optimization_parameters': {'touch_count_weight': {'min': 0.1, 'max': 0.5, 'type': 'float'}, 'total_volume_weight': {'min': 0.1, 'max': 0.4, 'type': 'float'}, 'level_age_weight': {'min': 0.1, 'max': 0.4, 'type': 'float'}, 'bounce_rate_weight': {'min': 0.1, 'max': 0.4, 'type': 'float'}, 'isolation_score_weight': {'min': 0.05, 'max': 0.3, 'type': 'float'}, 'min_touch_count': {'min': 2, 'max': 10, 'type': 'int'}, 'min_level_age_hours': {'min': 1, 'max': 48, 'type': 'int'}, 'price_tolerance_pct': {'min': 0.1, 'max': 2.0, 'type': 'float'}, 'volume_threshold': {'min': 0.5, 'max': 2.0, 'type': 'float'}, 'strength_threshold': {'min': 0.3, 'max': 0.8, 'type': 'float'}, 'breakout_threshold': {'min': 0.6, 'max': 0.9, 'type': 'float'}, 'confirmation_periods': {'min': 1, 'max': 5, 'type': 'int'}, 'volume_confirmation': {'min': 1.2, 'max': 3.0, 'type': 'float'}, 'momentum_threshold': {'min': 0.1, 'max': 0.5, 'type': 'float'}, 'false_breakout_filter': {'min': 0.1, 'max': 0.3, 'type': 'float'}, 'support_zone_multiplier': {'min': 0.8, 'max': 1.5, 'type': 'float'}, 'resistance_zone_multiplier': {'min': 0.8, 'max': 1.5, 'type': 'float'}, 'sr_zone_threshold': {'min': 0.6, 'max': 0.9, 'type': 'float'}, 'zone_expansion_factor': {'min': 1.0, 'max': 2.0, 'type': 'float'}, 'zone_contraction_factor': {'min': 0.5, 'max': 1.0, 'type': 'float'}, 'min_sr_confidence': {'min': 0.5, 'max': 0.8, 'type': 'float'}, 'high_confidence_threshold': {'min': 0.7, 'max': 0.9, 'type': 'float'}, 'confidence_decay_rate': {'min': 0.1, 'max': 0.5, 'type': 'float'}, 'regime_confidence_boost': {'min': 0.1, 'max': 0.3, 'type': 'float'}, 'ensemble_confidence_threshold': {'min': 0.6, 'max': 0.9, 'type': 'float'}}}
...
def get_optuna_config() -> dict[str, Any]:
    ...
    """
    return {'confidence_thresholds': asdict(DEFAULT_CONFIDENCE_THRESHOLDS), 'volatility_parameters': asdict(DEFAULT_VOLATILITY_PARAMETERS), 'ensemble_parameters': asdict(DEFAULT_ENSEMBLE_PARAMETERS), 'risk_management_parameters': asdict(DEFAULT_RISK_MANAGEMENT_PARAMETERS), 'market_regime_parameters': asdict(DEFAULT_MARKET_REGIME_PARAMETERS), 'sr_optimization_parameters': asdict(DEFAULT_SR_OPTIMIZATION_PARAMETERS), 'hyperparameter_optimization': asdict(DEFAULT_HYPERPARAMETER_OPTIMIZATION_CONFIG)}
...
def update_parameter_value(param_path: str, new_value: Any) -> bool:
    ...
    try:
        ...
        section_map: dict[str, Any] = {'confidence_thresholds': DEFAULT_CONFIDENCE_THRESHOLDS, 'volatility_parameters': DEFAULT_VOLATILITY_PARAMETERS, 'ensemble_parameters': DEFAULT_ENSEMBLE_PARAMETERS, 'risk_management_parameters': DEFAULT_RISK_MANAGEMENT_PARAMETERS, 'market_regime_parameters': DEFAULT_MARKET_REGIME_PARAMETERS, 'sr_optimization_parameters': DEFAULT_SR_OPTIMIZATION_PARAMETERS, 'hyperparameter_optimization': DEFAULT_HYPERPARAMETER_OPTIMIZATION_CONFIG}
...
def create_optimization_study_config(study_name: str, optimization_type: str='sr_parameters', multi_objective: bool=True) -> dict:
    """
    ...
    """
    config = {'study_name': study_name, 'optimization_type': optimization_type, 'multi_objective': multi_objective, 'storage_url': 'sqlite:///optuna_studies.db', 'sampler': 'tpe', 'pruner': 'hyperband', 'load_if_exists': True}
    if optimization_type == 'sr_parameters':
        config.update({'objectives': ['sharpe_ratio', 'win_rate', 'signal_clarity'], 'objective_weights': {'sharpe_ratio': 0.4, 'win_rate': 0.3, 'signal_clarity': 0.3}, 'n_trials': 100, 'timeout_minutes': 120})
    return config