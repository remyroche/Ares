...
if old_tbm in content:
    content = content.replace(old_tbm, new_tbm)
    print("Replaced tbm_outcomes_atr_nb successfully.")
else:
    print("Could not find exact old_tbm string. Replacing with regex...")
    pattern = r"@njit\(cache=True, fastmath=True\)\ndef tbm_outcomes_atr_nb\((.*?)\) -> Tuple\[np.ndarray, np.ndarray, np.ndarray\]:\n(.*?)    return tp_first, sl_first, timeout"
    if re.search(pattern, content, re.DOTALL):
        content = re.sub(pattern, new_tbm, content, flags=re.DOTALL)
        print("Replaced tbm_outcomes_atr_nb using regex.")
    else:
        print("Could not find old_tbm with regex.")
...
pattern = r"def compute_tbm_outcomes_per_symbol\((.*?)\) -> Tuple\[np.ndarray, np.ndarray, np.ndarray\]:\n(.*?)return out_tp, out_sl, out_to"
if re.search(pattern, content, re.DOTALL):
    content = re.sub(pattern, new_compute, content, flags=re.DOTALL)
    print("Replaced compute_tbm_outcomes_per_symbol using regex.")
else:
    print("Could not find compute_tbm_outcomes_per_symbol with regex.")
