...
def _materialize_slice(symbols: list[str], start: pd.Timestamp, end: pd.Timestamp) -> tuple[dict[str, pd.DataFrame], object]:
    panel, source_map = _make_panel(
        symbols, start, end, allow_minute_fallback=False,
    )
...
def main() -> None:
    ...
    if args.state_in is None:
        panel, source_map = _materialize_slice(symbols, start, end)
        loaded_start = start
        bootstrap = True
    else:
        ...
        if end == prior_end:
            ...
            update, source_map = _materialize_slice(symbols, loaded_start, end)
