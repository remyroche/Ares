...
from src.training.comprehensive_sr_training_pipeline import (
    ComprehensiveSRTrainingPipeline,
)
...
async def test_sr_breakout_predictor_features():
    ...
    try:
        features = await sr_predictor.extract_ml_features(market_data, current_price)

        print(f"✅ SRBreakoutPredictor extracted {len(features)} features")
        print(f"   - Proximity features: {len([f for f in features if 'proximity' in f])}")
        print(f"   - Strength features: {len([f for f in features if 'strength' in f])}")
        print(f"   - Level features: {len([f for f in features if 'level' in f])}")
        print(f"   - Advanced features: {len([f for f in features if any(keyword in f for keyword in ['fibonacci', 'elliott', 'order_flow'])])}")
...
async def test_comprehensive_training_pipeline():
    ...
    try:
        ...
        print("✅ Feature summary generated:")
...
async def test_feature_completeness():
    ...
    try:
        ...
        print("✅ SR feature analysis completed:")
        ...
        if missing_features:
            print("⚠️ Missing features by category:")
