...
class SRLevelAnalyzer:
    ...
    @handle_data_processing_errors(
        default_return=([], [], [], []), context="detect_peaks_and_troughs"
    )
    def _detect_peaks_and_troughs(self, prices: pd.Series):
        ...
        resistance_indices, _ = find_peaks(
            prices, prominence=prominence_abs, width=self.config["peak_width"]
        )
        ...
        return (
            resistance_levels,
            resistance_timestamps,
            support_levels,
            support_timestamps,
        )

    @handle_data_processing_errors(default_return=[], context="group_levels")
    def _group_levels(self, levels, timestamps, level_type: str, current_price: float):
        ...
        for level, ts in sorted_levels_info:
            if not current_group:
                ...
                tolerance_abs = (
                    last_grouped_level_price * self.config["level_tolerance_pct"]
                )
    ...
    @handle_data_processing_errors(default_return=[], context="assess_strength")
    def _assess_strength(
        self, sr_levels: list, prices: pd.Series, volumes: pd.Series, current_timestamp
    ):
        """
        ...
        for level_info in sr_levels:
            try:
                ...
                age_days = (
                    (current_timestamp - last_tested).days
                    if current_timestamp and last_tested
                    else 0
                )
                ...
                for touch_ts in level_info["touches"]:
                    ...
                    try:
                        ...
                        start_idx = max(
                            0, idx - self.config["volume_lookback_window"] // 2
                        )
                        end_idx = min(
                            len(volumes) - 1,
                            idx + self.config["volume_lookback_window"] // 2,
                        )
                        total_volume_at_touches += volumes.iloc[
                            start_idx : end_idx + 1
                        ].sum()
                    except KeyError:
                ...
                volume_factor = (
                    (total_volume_at_touches / (num_touches * avg_daily_volume))
                    if num_touches > 0 and avg_daily_volume > 0
                    else 0
                )
                ...
                recency_factor = max(
                    0, 1 - (age_days / self.config["max_age_days"])
                )  # 1 for recent, 0 for old

                # Combine factors (weights can be adjusted)
                strength_score = (
                    (num_touches * 0.4) + (volume_factor * 0.3) + (recency_factor * 0.3)
                )
                ...
                if (
                    level_price > current_timestamp
                ):  # Assuming current_timestamp is actually current_price
                ...
                self.logger.error(
                    f"Error assessing strength for level {level_info}: {e}"
                )
                continue
    ...
    def analyze(self, historical_data: pd.DataFrame):
        ...
        """
        if not all(
            col in historical_data.columns for col in ["Close", "High", "Low", "Volume"]
        ):
            raise ValueError(
                "Historical data must contain 'Close', 'High', 'Low', 'Volume' columns."
            )
        if not isinstance(historical_data.index, pd.DatetimeIndex):
            raise ValueError("Historical data index must be a DatetimeIndex.")

        close_prices = historical_data["Close"]
        # Removed unused variables high_prices and low_prices
        volumes = historical_data["Volume"]
        current_timestamp = historical_data.index[
            -1
        ]  # Use the latest timestamp as current
        ...
        grouped_resistances = self._group_levels(
            resistance_levels,
            resistance_timestamps,
            "Resistance",
            close_prices.iloc[-1],
        )
        grouped_supports = self._group_levels(
            support_levels, support_timestamps, "Support", close_prices.iloc[-1]
        )

        # 3. Assess strength
        assessed_resistances = self._assess_strength(
            grouped_resistances, close_prices, volumes, current_timestamp
        )
        assessed_supports = self._assess_strength(
            grouped_supports, close_prices, volumes, current_timestamp
        )

        # Combine and sort by price
        all_sr_levels = sorted(
            assessed_resistances + assessed_supports, key=lambda x: x["level_price"]
        )
...
if __name__ == "__main__":
    ...
    base_price[120:130] = np.maximum(
        base_price[120:130], 2050 + np.random.randn(10) * 2
    )

    # Create a resistance level around 2150
    base_price[80:90] = np.minimum(base_price[80:90], 2150 + np.random.randn(10) * 2)
    base_price[150:160] = np.minimum(
        base_price[150:160], 2150 + np.random.randn(10) * 2
    )

    close_prices = pd.Series(
        base_price + np.random.randn(data_points) * 2, index=dates, name="Close"
    )
    high_prices = close_prices + np.random.rand(data_points) * 5
    low_prices = close_prices - np.random.rand(data_points) * 5
    volumes = pd.Series(
        np.random.randint(1000, 50000, data_points), index=dates, name="Volume"
    )
    ...
    if sr_levels:
        for level in sr_levels:
            print(
                f"Level: {level['level_price']:.2f} | Type: {level['type']} | "
    ...
    if sr_levels:
        ...
        print(
            f"  All Touch Timestamps: {[ts.strftime('%Y-%m-%d') for ts in first_level['touches']]}"
        )
