...
class EnhancedDataQualityManager:
    ...
    async def comprehensive_quality_check(
        ...
        """
        logger.info(
            f"🔍 Starting comprehensive quality check for {exchange}_{symbol}_{timeframe}"
        )
        ...
        try:
            ...
            if check_gaps and self.gap_detector:
                ...
                if gap_results.get("gaps"):
                    ...
                    results["recommendations"].append(
                        "Data gaps detected - consider filling them"
                    )

                    # Step 2: Fill gaps if requested
                    if fill_gaps and self.gap_filler:
                        fill_results = await self._fill_data_gaps(
                            symbol, exchange, timeframe, gap_results["gaps"]
                        )
                        results["gaps_filled"] = fill_results.get("filled_gaps", [])

                        if fill_results.get("success"):
                            logger.info(
                                f"✅ Successfully filled {len(fill_results['filled_gaps'])} gaps"
                            )
                        else:
            ...
            if validate_format and self.validator:
                format_results = await self._validate_data_format(
                ...
                if format_results.get("issues"):
                    logger.warning(
                        f"⚠️ Found {len(format_results['issues'])} format issues"
                    )
                    results["recommendations"].append(
                        "Data format issues detected - consider fixing them"
                    )

            # Step 4: Check data completeness for step03/step04 requirements
            completeness_results = await self._check_step3_step4_completeness(
                symbol, exchange, timeframe
            )
            ...
            if not completeness_results.get("ready"):
                results["recommendations"].append(
                    "Data not ready for step03/step04 - additional data needed"
                )

            logger.info(
                f"✅ Comprehensive quality check completed for {exchange}_{symbol}_{timeframe}"
            )
            return results
    ...
    async def _fill_data_gaps(
        self, symbol: str, exchange: str, timeframe: str, gaps: List[Dict]
    ) -> Dict[str, Any]:
    ...
    async def _check_step3_step4_completeness(
        ...
        try:
            ...
            unified_path = (
                self.data_cache_path / "unified" / exchange.lower() / symbol / timeframe
            )
    ...
    async def get_data_for_step3_step4(
        ...
        """Get data ready for step03 and step04, ensuring all gaps are filled and
        quality is validated."""
        logger.info(
            f"📊 Preparing data for step03/step04: {exchange}_{symbol}_{timeframe}"
        )
        ...
        try:
            ...
            if not completeness_results.get("ready", False):
                logger.warning(
                    "⚠️ Data not ready for step03/step04, attempting to fix..."
                )

                # Try to use step01 and step1_5 components to get missing data
                fix_results = await self._fix_missing_data_for_steps(
                    symbol, exchange, timeframe
                )
    ...
    async def _fix_missing_data_for_steps(
        ...
        try:
            logger.info(
                "🔄 Attempting to fix missing data using step01/step1_5 components..."
            )
...
async def ensure_data_quality(
    ...
    """Convenience function to ensure data quality for a given
    symbol/exchange/timeframe."""
