...
class TacticianEnsembleCreationStep:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Error initializing Tactician Ensemble Creation Step: {e}")
            raise
    ...
    async def execute(self, training_input: Dict[str, Any], pipeline_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            ensemble_results = await self._create_tactician_ensemble(tactician_models, symbol, exchange)
            
            # Save ensemble model
            ensemble_models_dir = f"{data_dir}/tactician_ensembles"
            os.makedirs(ensemble_models_dir, exist_ok=True)
            
            ensemble_file = f"{ensemble_models_dir}/{exchange}_{symbol}_tactician_ensemble.pkl"
            with open(ensemble_file, 'wb') as f:
                pickle.dump(ensemble_results, f)
            
            # Save ensemble summary
            summary_file = f"{data_dir}/{exchange}_{symbol}_tactician_ensemble_summary.json"
            with open(summary_file, 'w') as f:
                json.dump(ensemble_results, f, indent=2)
            
            self.logger.info(f"✅ Tactician ensemble creation completed. Results saved to {ensemble_models_dir}")
    ...
    async def _create_tactician_ensemble(self, models: Dict[str, Any], symbol: str, exchange: str) -> Dict[str, Any]:
        """
        ...
        """
        try:
            self.logger.info(f"Creating tactician ensemble for {symbol} on {exchange}...")
            ...
            simple_averaging_ensemble = await self._create_simple_averaging_ensemble(model_list, model_names, accuracies, symbol, exchange)
            ensemble_results["simple_averaging"] = simple_averaging_ensemble
            
            # 2. Weighted Averaging Ensemble
            weighted_averaging_ensemble = await self._create_weighted_averaging_ensemble(model_list, model_names, accuracies, symbol, exchange)
            ensemble_results["weighted_averaging"] = weighted_averaging_ensemble
            
            # 3. Voting Ensemble
            voting_ensemble = await self._create_voting_ensemble(model_list, model_names, symbol, exchange)
            ensemble_results["voting"] = voting_ensemble
            
            # 4. Stacking Ensemble
            stacking_ensemble = await self._create_stacking_ensemble(model_list, model_names, symbol, exchange)
    ...
    async def _create_simple_averaging_ensemble(self, models: List[Any], model_names: List[str], 
                                              accuracies: List[float], symbol: str, exchange: str) -> Dict[str, Any]:
                ...
                def predict_proba(self, X):
                    ...
                    for model in self.models:
                        try:
                            ...
                            pred_proba = np.zeros((len(pred), 3))  # Assuming 3 classes: -1, 0, 1
                            for i, p in enumerate(pred):
                                pred_proba[i, p + 1] = 1.0  # +1 to convert from [-1,0,1] to [0,1,2]
    ...
    async def _create_weighted_averaging_ensemble(self, models: List[Any], model_names: List[str], 
                                                accuracies: List[float], symbol: str, exchange: str) -> Dict[str, Any]:
    ...
    async def _create_voting_ensemble(self, models: List[Any], model_names: List[str], 
    ...
    async def _create_stacking_ensemble(self, models: List[Any], model_names: List[str], 
                                      symbol: str, exchange: str) -> Dict[str, Any]:
