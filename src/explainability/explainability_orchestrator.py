...
class ExplainabilityOrchestrator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.explanation_timeout = self.explain_config.get("explanation_timeout", 30)  # seconds
    ...
    async def register_model(
        ...
        try:
            ...
            # Store model
            self.registered_models[model_type][model_name] = {
                'model': model,
                'training_data': training_data,
                'initialized': False
            }
            
            # Initialize explainers if training data is available
    ...
    async def explain_model_prediction(
        ...
        try:
            ...
            # Explain prediction with timeout
            try:
                explanation = await asyncio.wait_for(
                    explainer.explain_prediction(model, features, feature_names, prediction),
                    timeout=self.explanation_timeout
                )
                
                if explanation:
                    # Add market conditions to metadata
    ...
    async def start_trade_decision_trace(
        self,
        decision_id: str,
        decision_type: str,
        market_conditions: Optional[Dict[str, Any]] = None
        ...
        try:
            ...
            trace = await self.decision_tracer.start_decision_trace(
                decision_id, decision_type, market_conditions
            )
    ...
    async def add_explanation_to_trace(
        self,
        decision_id: str,
        model_type: str,
        explanation: Any
        ...
        try:
            ...
            success = await self.decision_tracer.add_model_explanation(
                decision_id, model_type, explanation
            )
    ...
    async def finalize_trade_decision_trace(
        self,
        decision_id: str,
        final_decision: Any,
        confidence: float
    ) -> Optional[TradeDecisionTrace]:
        ...
        try:
            ...
            trace = await self.decision_tracer.finalize_decision_trace(
                decision_id, final_decision, confidence
            )
    ...
    async def explain_complete_trading_decision(
        self,
        decision_id: str,
        decision_type: str,
        market_data: pd.DataFrame,
        tactician_features: Optional[Tuple[np.ndarray, List[str]]] = None,
        hmm_features: Optional[Tuple[np.ndarray, List[str]]] = None,
        sr_features: Optional[Tuple[np.ndarray, List[str]]] = None,
        analyst_features: Optional[Tuple[np.ndarray, List[str]]] = None,
        ...
        try:
            ...
            trace = await self.start_trade_decision_trace(
                decision_id, decision_type, market_conditions
            )
            ...
            # Explain each model if features are provided
            ...
            if tactician_features is not None:
                ...
                explanation = await self.explain_model_prediction(
                    'tactician', 'main', features, feature_names, market_conditions=market_conditions
                )
                if explanation:
            ...
            if hmm_features is not None:
                ...
                explanation = await self.explain_model_prediction(
                    'hmm', 'main', features, feature_names, market_conditions=market_conditions
                )
                if explanation:
            ...
            if sr_features is not None:
                ...
                explanation = await self.explain_model_prediction(
                    'sr', 'main', features, feature_names, market_conditions=market_conditions
                )
                if explanation:
            ...
            if analyst_features is not None:
                ...
                explanation = await self.explain_model_prediction(
                    'analyst', 'main', features, feature_names, market_conditions=market_conditions
                )
                if explanation:
            ...
            # Finalize trace
            final_trace = await self.finalize_trade_decision_trace(
                decision_id, final_decision, confidence
            )
    ...
    async def get_decision_trace_summary(self, decision_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            # Try to find in active traces first
            ...
            summary = {
                ...
                'explanations_available': {
                    'tactician': trace.tactician_explanation is not None,
                    'hmm': trace.hmm_explanation is not None,
                    'sr': trace.sr_explanation is not None,
                    'analyst': trace.analyst_explanation is not None
                },
                'top_contributing_factors': trace.top_contributing_factors[:5],  # Top 5
                'risk_factors_count': len(trace.risk_factors),
                'opportunity_factors_count': len(trace.opportunity_factors)
            }
    ...
    async def _load_decision_trace(self, decision_id: str) -> Optional[TradeDecisionTrace]:
        ...
        try:
            ...
            trace = TradeDecisionTrace(
                ...
                final_decision=trace_data['final_decision'],
                confidence=trace_data['confidence'],
                top_contributing_factors=trace_data.get('top_contributing_factors', []),
                risk_factors=trace_data.get('risk_factors', []),
                opportunity_factors=trace_data.get('opportunity_factors', []),
                market_conditions=trace_data.get('market_conditions', {}),
    ...
    async def get_model_explanation_summary(
        self,
        model_type: str,
        model_name: str,
        explanation: Any
        ...
        return {
            model_type: {
                model_name: {
                    ...
                    'has_training_data': model_info['training_data'] is not None
                }
                for model_name, model_info in models.items()
            }
            for model_type, models in self.registered_models.items()
        }
