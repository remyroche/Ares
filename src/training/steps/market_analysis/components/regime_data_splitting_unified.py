"""
Unified Regime Data Splitting Component.

This module provides a streamlined, robust implementation of regime data splitting
with comprehensive error handling, enhanced reporting, and prevention of silent failures.

KEY IMPROVEMENTS:
- Single unified implementation (eliminates redundancy)
- Comprehensive error handling with explicit failure modes
- Enhanced reporting with detailed metrics
- Silent failure prevention with validation checkpoints
...
class UnifiedRegimeDataSplittingComponent(BaseMarketAnalysisComponent):
    """
    Unified Regime Data Splitting Component.
    
    Provides a single, robust implementation with comprehensive error handling,
    enhanced reporting, and prevention of silent failures.
    """
    
    def __init__(self, config: Optional[ComponentConfig] = None):
        """Initialize the unified regime data splitting component."""
        super().__init__(config)
        self.logger = system_logger.getChild('UnifiedRegimeDataSplitting')
    ...
    async def execute(self, data: Any, pipeline_state: Dict[str, Any]) -> ComponentResult:
        """
        Execute unified regime data splitting with comprehensive error handling.
        ...
        self.logger.info('✂️ Starting Unified Regime Data Splitting')
        ...
        try:
            ...
            self.logger.info(f'✅ Unified Regime Data Splitting completed: {self.metrics.regime_count} regimes processed')
            ...
            self.logger.error(f'❌ Unified Regime Data Splitting failed: {e}')
            import traceback
            ...
            artifacts = {
                'regime_data_splitting_result': {
                    ...
                    'metadata': {
                        ...
                        'component_version': 'unified_v1.0'
                    }
                },
                'regime_splitting_report': {
...
async def execute_unified_regime_data_splitting(
    ...
    """Execute unified regime data splitting with comprehensive error handling."""
    try:
        # Convert config to ComponentConfig
        ...
        result = await component.execute(training_input.get('data'), pipeline_state)
        
        # Convert ComponentResult to dictionary format
        return {
            'success': result.success,
            'step04_regime_data_splitting_completed': result.success,
            'regime_data': result.artifacts.get('regime_data_splitting_result', {}),
            'regime_splitting_report': result.artifacts.get('regime_splitting_report', {}),
            'regime_validation_results': result.artifacts.get('regime_validation_results', {}),
        ...
        return {
            ...
            'error_message': f"Unified regime data splitting failed: {str(e)}",
            'execution_time': 0.0
        }