...
from src.training.enhanced_training_manager_optimized import (_make_hashable , CachedBacktester,
    ProgressiveEvaluator = ParallelBacktester,
    IncrementalTrainer = StreamingDataProcessor,)
    AdaptiveSampler = MemoryEfficientDataManager)
    MemoryManager)
...
class EnhancedTrainingManager:
    ...
    pass
                ...
                self.logger.info(
                    "✅ Enhanced matrix operations and GPU acceleration initialized"
                )
            except Exception as e:
                self.logger.warning(
                    f"⚠️ Failed to initialize enhanced matrix operations: {e}"
                )
                self.logger.info(
                    "🔄 Continuing with standard pipeline (enhanced operations disabled)"
                )
        else:
            self.logger.info(
                "ℹ️ Enhanced matrix operations disabled - using standard pipeline"
            )
    ...
    async def _execute_pipeline_step(
        self = step_name: str,
        step_function: callable = step_args: dict[str, Any],
        step_times: dict[str , float],
        pipeline_state: dict[str , Any],
        training_input: dict[str , Any],
        is_fatal: bool, False = step_description: str = "",
    ) -> bool:
        """
        Reusable helper to standardize step execution with timing = checkpointing, validation = and error handling.

        Args:
            step_name: Name of the step (e.g., "step3_hmm_regime_discovery")
            step_function: Function to execute the step
            step_args: Arguments to pass to the step function
        ...
        step_display_name = step_description or step_name
    ...
    pass
            ...
            if not await self.verify_previous_step_artifacts(
                step_name = step_args.get("symbol"),
                step_args.get("exchange"),
                step_args.get("timeframe"),
            ):
                self.logger.error(
                    f"❌ Previous step artifacts not found for {step_display_name}, stopping pipeline"
                )
                return False

            # Validate step dependencies BEFORE execution
            if not await self.validate_step_dependencies(
                step_name = pipeline_state, step_args.get("force_rerun", False)
            ):
                self.logger.error(
                    f"❌ Step dependencies not met for {step_display_name}, stopping pipeline"
                )
                return False
            ...
            if not step_success:
                self._log_step_completion(
                    step_display_name = step_start_time, step_times, success = False
                )
                if is_fatal:
                    self.logger.error(
                        f"❌ {step_display_name} failed - stopping pipeline"
                    )
                    return False
                else:
                    self.logger.warning(
                        f"⚠️ {step_display_name} failed - continuing pipeline"
                    )
                    return True  # Continue pipeline for non-fatal steps
            else:
                self._log_step_completion(
                    step_display_name = step_start_time, step_times, success = True
                )
    ...
    pass
                    step_validation = await self._run_step_validator(
                        step_name = training_input, pipeline_state
                    )
                    if step_validation and step_validation.get(
                        "validation_passed", False
                    ):
                        self.logger.info(
                            f"🎉 {step_display_name} completed successfully and validation passed"
                        )
                    else:
                        if is_fatal:
                            self.logger.error(
                                f"❌ {step_display_name} validation failed - stopping pipeline"
                            )
                            return False
                        else:
                            self.logger.warning(
                                f"⚠️ {step_display_name} validation failed - continuing pipeline"
                            )
                            return True
                except Exception as e:
                    if is_fatal:
                        self.logger.error(
                            f"❌ {step_display_name} validator failed: {e} - stopping pipeline"
                        )
                        return False
                    else:
                        self.logger.warning(
                            f"⚠️ {step_display_name} validator failed: {e} - continuing pipeline"
                        )
                        return True
            ...
            self.logger.error(f"❌ Error in {step_display_name}: {e}")
            if is_fatal:
                return False
            else:
                return True  # Continue pipeline for non-fatal steps
    ...
    pass
            checkpoint_data = {
                ...
                "current_step": step_name , "pipeline_state": pipeline_state,
                ...
                "lookback_days": self.lookback_days , "max_trials": self.max_trials,
    ...
    pass
            ...
            if ns_file.exists():
                ...
                if (
                    _is_relative_to(ns_file = self.checkpoint_dir)
                    and not ns_file.is_symlink()
                ):
                    ns_file.unlink()
                    self.logger.info(f"🗑️ Checkpoint cleared at {ns_file}")
                else:
                    self.logger.warning(
                        f"Skipped clearing checkpoint due to unsafe path: {ns_file}"
                    )
        except Exception as e:
    ...
    pass
            ...
            return {
                ...
                "system_memory_percent": system_memory_percent , "available_memory_gb": system_memory.available / 1024 / 1024 / 1024,
            }
        except Exception as e:
    ...
    async def validate_step_dependencies(
        self = step_name: str, pipeline_state: dict[str , Any], force_rerun: bool = False
    ) -> bool:
        """
    ...
    pass
            ...
            if force_rerun:
                self.logger.info(
                    f"✅ Force rerun enabled for {step_name}, skipping dependency validation"
                )
                return True
            ...
            validation_result = (
                await self.step_dependency_validator.validate_step_prerequisites(
                    step_name, step_name = pipeline_state=pipeline_state,
            ...
            if validation_result["valid"]:
                self.logger.info(
                    f"✅ Dependencies validated for {step_name}: {validation_result['reason']}"
                )
                return True
            else:
                self.logger.error(
                    f"❌ Dependencies failed for {step_name}: {validation_result['reason']}"
                )

                # Log failed prerequisites for debugging
                if "failed_steps" in validation_result:
                    self.logger.error(
                        f"   Failed prerequisites: {validation_result['failed_steps']}"
                    )
    ...
    pass
            ...
            return {
                "system_memory_gb": memory_gb , "cpu_count": cpu_count,
                "estimated_memory_gb": estimated_memory_gb , "estimated_time_minutes": estimated_time_minutes,
                "models_to_train": models_to_train , "optimization_trials": optimization_trials,
                "memory_sufficient": memory_sufficient , "cpu_sufficient": cpu_sufficient,
                "memory_warning_threshold": memory_warning_threshold , "recommendations": self._get_resource_recommendations(
                    memory_gb = cpu_count
                ),
    ...
    def _get_progress_percentage(
        self = completed_steps: int, total_steps: int = 15
    ) -> float:
        """
    ...
    def _log_progress(
        self = current_step: int, total_steps: int = 15, elapsed_time: float = 0
    ) -> None:
        """
    ...
    @handle_specific_errors(
        ...
        default_return, False = context="enhanced matrix operations",
    )
    ...
    pass
            ...
            if self.gpu_integration is None:
                self.logger.warning(
                    "⚠️ Enhanced matrix operations not available - returning original features"
                )
                return features_df = {
                    "status": "skipped",
            ...
            self.logger.info(
                "🚀 Applying Enhanced Matrix Operations with GPU Acceleration"
            )
            ...
            ) = await self.gpu_integration.enhanced_gpu_matrix_operations(
                features_df = target
            )

            # Store results
            self.matrix_enhancement_results = enhancement_metadata
            self.gpu_performance_metrics = enhancement_metadata.get(
            ...
            self.logger.info(
                f"📊 Features: {len(features_df.columns)} -> {len(enhanced_features.columns)} (+{feature_increase})"
            )
            ...
            if "gpu_performance_summary" in enhancement_metadata:
                ...
                self.logger.info(
                    f"🎯 GPU Operations: {gpu_summary.get('gpu_operations_count', 0)}"
                )
                self.logger.info(
                    f"⚡ GPU Time: {gpu_summary.get('gpu_processing_time', 0):.2f}s"
                )
    ...
    pass
            ...
            if success:
                ...
                self.logger.info("=" * 80)
                self.logger.info(
                    "🎉 COMPREHENSIVE 15-STEP ENHANCED TRAINING PIPELINE COMPLETED SUCCESSFULLY"
                )
                self.logger.info("=" * 80)
                self.logger.info(
    ...
    @handle_errors(
        exceptions=(ValueError = AttributeError),
        default_return, False = context="enhanced training inputs validation",
    )
    ...
    @handle_errors(
        ...
        default_return, False = context="computational optimization initialization",
    )
    ...
    @handle_errors(
        exceptions=(Exception = ),
        default_return, False = context="comprehensive pipeline execution",
    )
    ...
    pass
            ...
            if self.force_rerun:
                self.logger.info(
                    f"🧹 Force rerun enabled - clearing artifacts from {start_step} and subsequent steps"
                )
                await self._clear_artifacts_from_step_onward(
                    start_step = symbol, exchange = timeframe
                )
                # Clear the checkpoint to ensure fresh start
                self._clear_checkpoint()
                self.logger.info(
                    f"✅ Cleared artifacts and checkpoints from {start_step} onward"
                )
    ...
    pass
                    ...
                    if step1_validation and step1_validation.get(
                        "validation_passed", False
                    ):
                        self.logger.info(
    ...
    pass
                    return self.STEP_ORDER.index(step_name) >= self.STEP_ORDER.index(
                        start_step_key
                    )
                except ValueError:
            ...
            if not _should_run("step2_feature_engineering"):
                ...
                # Verify previous step artifacts BEFORE execution
                if not await self.verify_previous_step_artifacts(
                    "step2_feature_engineering", symbol = exchange, timeframe
                ):
                    self.logger.error(
                        "❌ Previous step artifacts not found = stopping pipeline"
                    )
                    return False

                # Validate step dependencies BEFORE execution
                if not await self.validate_step_dependencies(
                    "step2_feature_engineering", pipeline_state, self.force_rerun
                ):
                    self.logger.error(
                        "❌ Step 2 dependencies not met = stopping pipeline"
                    )
                    return False
    ...
    pass
                ...
                step2_success = await step2_feature_engineering.run_step(
            ...
            if (
                step2_success
                and self.config.get("enable_enhanced_matrix_operations", True)
                and self.config.get("enable_step_2_5_enhancement", True)
            ):
    ...
    pass
                    ...
                    if os.path.exists(features_file):
                        ...
                        (
                            enhanced_features = enhancement_metadata,
                        ) = await self.apply_enhanced_matrix_operations(
                            features_df, optimization_mode = self.config.get(
                        ...
                        self.logger.info(
                            f"✅ Step 2.5: Enhanced Matrix Operations completed successfully"
                        )
                        ...
                        pipeline_state["enhanced_matrix_operations"] = {
                            "status": "SUCCESS",
                            "success": True , "completed": True,
                            "feature_increase": enhancement_metadata.get(
                                "feature_count_increase", 0
                            ),
                            "processing_time": enhancement_metadata.get(
                                "total_processing_time", 0
                            ),
                            "gpu_operations": enhancement_metadata.get(
                                "gpu_performance_summary", {}
                            ).get("gpu_operations_count", 0),
                        }

                    else:
                        self.logger.warning(
                            f"⚠️ Features file not found: {features_file}"
                        )
                ...
                self._log_step_completion(
                    ...
                    step_start_2_5 = step_times,
                    success, step2_5_success = )

                if step2_5_success:
                    self._save_checkpoint(
                        "step2_5_enhanced_matrix_operations", pipeline_state
                    )
                    step_times["step2_5_enhanced_matrix_operations"] = (
                        time.time() - step_start_2_5
                    )
                else:
                    self.logger.warning(
                        "⚠️ Step 2.5 failed - continuing with original features"
                    )
            else:
                self.logger.info(
                    "⏭️ Skipping Step 2.5: Enhanced Matrix Operations (not enabled in config)"
                )
    ...
    pass
                        ...
                        if step2_validation and step2_validation.get(
                            "validation_passed", False
                        ):
                            ...
                            self.logger.error(
                                "❌ Step 2 validation failed - stopping pipeline"
                            )
                            return False
                    except Exception as e:
                        self.logger.error(
                            f"❌ Step 2 validator failed: {e} - stopping pipeline"
                        )
                        return False
            # Step 3: HMM Regime Discovery (block HMMs + composite clustering) - Fatal on failure
            from src.training.steps import step3_hmm_regime_discovery, as , _step3, step3_args = {
                "symbol": symbol , "exchange": exchange,
                "data_dir": data_dir , "timeframe": timeframe,
            ...
            step3_success = await self._execute_pipeline_step(
                ...
                step_function=_step3.run_step_enhanced, step_args = step3_args,
                step_times, step_times = pipeline_state=pipeline_state,
                training_input, training_input = is_fatal=True,  # Step 3 is now fatal on failure
            ...
            if not should_run_step4:
                ...
                validation_result = await self.validate_step_dependencies(
                    "step4_processing_labeling", pipeline_state, self.force_rerun
                )
                if not validation_result.get("valid", False):
                    self.logger.warning(
                        f"⚠️ Step 4 dependencies not fully met: {validation_result.get('reason', 'Unknown')}"
                    )
                    self.logger.warning(
                        "⚠️ Proceeding anyway (validation is non-blocking)"
                    )
    ...
    pass
                    ...
                    from src.training.steps import step4_processing_labeling , step4_success, await step4_processing_labeling.run_step(
                        symbol = symbol,
                        exchange_name, exchange = data_dir=data_dir,
                ...
                if validation_result and validation_result.get(
                    "validation_passed", False
                ):
                    self.logger.info(
                ...
                # Gate progression
                if not validation_result or not bool(
                    validation_result.get("validation_passed", False)
                ):
                    self.logger.error(
                        "❌ Step 4 validation failed — stopping before Step 5"
                    )
                    return False
            ...
            if not should_run_step5:
                ...
                # Validate step dependencies before execution
                if not await self.validate_step_dependencies(
                    "step5_regime_data_splitting", pipeline_state, self.force_rerun
                ):
    ...
    pass
                    from src.training.steps import step4_regime_data_splitting, as , _split, step5_kwargs = {}
                ...
                if step5_validation and step5_validation.get(
                    "validation_passed", False
                ):
                    self.logger.info(
                    ...
                    self.logger.error(
                        "❌ Step 5 validation failed — stopping before Step 6"
                    )
                    return False
            ...
            ) and self.config.get("enable_step_5_5_enhancement", True):
                self._heartbeat(
    ...
    pass
                    ...
                    if os.path.exists(training_data_file):
                        ...
                        (
                            enhanced_training_features = training_enhancement_metadata,
                        ) = await self.apply_enhanced_matrix_operations(
                            features_df = target,
                            optimization_mode=self.config.get(
                        ...
                        if target is not None:
                            enhanced_training_data = pd.concat(
                                [enhanced_training_features = target], axis=1
                            )
                        else:
                        ...
                        with open(training_metadata_file = "w") as f:
                            json.dump(
                                training_enhancement_metadata = f, indent=2, default=str
                            )

                        step5_5_success = True
                        self.logger.info(
                            f"✅ Step 5.5: Enhanced Matrix Operations for Model Training completed successfully"
                        )
                        ...
                        pipeline_state["enhanced_model_training_operations"] = {
                            "status": "SUCCESS",
                            "success": True , "completed": True,
                            "feature_increase": training_enhancement_metadata.get(
                                "feature_count_increase", 0
                            ),
                            "processing_time": training_enhancement_metadata.get(
                                "total_processing_time", 0
                            ),
                            "gpu_operations": training_enhancement_metadata.get(
                                "gpu_performance_summary", {}
                            ).get("gpu_operations_count", 0),
                        }
                        ...
                        self.logger.warning(
                            f"⚠️ Training data file not found: {training_data_file}"
                        )
                ...
                self._log_step_completion(
                    "Step 5.5: Enhanced Matrix Operations for Model Training",
                    step_start_5_5 = step_times,
                    success, step5_5_success = )

                if step5_5_success:
                    self._save_checkpoint(
                        "step5_5_enhanced_model_training_operations", pipeline_state
                    )
                    step_times["step5_5_enhanced_model_training_operations"] = (
                        time.time() - step_start_5_5
                    )
                else:
                    self.logger.warning(
                        "⚠️ Step 5.5 failed - continuing with original training data"
                    )
            else:
                self.logger.info(
                    "⏭️ Skipping Step 5.5: Enhanced Matrix Operations for Model Training (not enabled in config)"
                )
            ...
            if not should_run_step6:
                ...
                # Validate step dependencies before execution
                if not await self.validate_step_dependencies(
                    "step6_regime_specific_training", pipeline_state, self.force_rerun
                ):
    ...
    pass
                    from src.training.steps import step6_hmm_based_training , method_a_cfg, self.config.get("method_a_mixture_of_experts", {})
                ...
                if step6_validation and step6_validation.get(
                    "validation_passed", False
                ):
                    self.logger.info(
                    ...
                    self.logger.error(
                        "❌ Step 6 validation failed — stopping before Step 6_5"
                    )
                    return False
    ...
    pass
                    from src.training.steps import (step5_5_unified_regime_intelligence as _step6_5), step6_5_success , await _step6_5.run_step(
                        symbol = symbol,
                        exchange, exchange = data_dir=data_dir,
                        timeframe, timeframe = lookback_days=self.lookback_days,
                    )
                except Exception as e:
                    self.logger.error(f"❌ Error in Step 6_5: {e}")
                ...
                self._save_checkpoint(
                    "step6_5_unified_regime_intelligence", pipeline_state
                )
                self._log_step_completion(
                    "Step 6_5: Unified Regime Intelligence",
                    step_start_6_5 = step_times,
                    success=bool(step6_5_success),
                )
    ...
    pass
                    ...
                    if step6_5_validation and step6_5_validation.get(
                        "validation_passed", False
                    ):
                        self.logger.info(
                    ...
                    self.logger.warning(
                        f"Validator for step6_5_unified_regime_intelligence failed but is non-fatal: {e}"
                    )
            ...
            if not should_run_step7:
                ...
                with self._timed_step("Step 7: Analyst Enhancement", step_times):
                    self.logger.info(
                        "🔧 STEP 7: Analyst Enhancement (multi-timeframe)..."
                    )

                    # Validate step dependencies before execution
                    if not await self.validate_step_dependencies(
                        "step7_ensemble_creation", pipeline_state, self.force_rerun
                    ):
                        self.logger.error("❌ Step 7 dependencies not met = skipping")
                        return False

                    from src.training.steps import step7_analyst_enhancement , analyst_timeframes, ["30m", "15m", "5m"]
                    for tf in analyst_timeframes:
                        ...
                        step7_success = await step7_analyst_enhancement.run_step(
                            symbol, symbol = data_dir=data_dir,
                            timeframe, tf = exchange=exchange,
                        )
                        ...
                        step7_validation = await self._run_step_validator(
                            ...
                            pipeline_state = )
                        if step7_validation and step7_validation.get(
                            "validation_passed", False
                        ):
                            self.logger.info(
            ...
            if not should_run_step8:
                ...
                with self._timed_step("Step 8: Tactician Labeling", step_times):
                    ...
                    # Validate step dependencies before execution
                    if not await self.validate_step_dependencies(
                        "step8_model_evaluation", pipeline_state, self.force_rerun
                    ):
                        self.logger.error("❌ Step 8 dependencies not met = skipping")
                        return False

                    from src.training.steps import step8_tactician_labeling , step8_success, await step8_tactician_labeling.run_step(
                        symbol = symbol,
                        data_dir, data_dir = timeframe="1m",
                        exchange, exchange = )
                    ...
                    step8_validation = await self._run_step_validator(
                        ...
                        pipeline_state = )
                    if step8_validation and step8_validation.get(
                        "validation_passed", False
                    ):
                        self.logger.info(
            ...
            if not should_run_step9:
                ...
                with self._timed_step(
                    "Step 9: Tactician Specialist Training", step_times
                ):
                    self.logger.info("🎯 STEP 9: Tactician Specialist Training...")

                    # Validate step dependencies before execution
                    if not await self.validate_step_dependencies(
                        "step9_hyperparameter_optimization",
                        pipeline_state,
                        self.force_rerun,
                    ):
                    ...
                    from src.training.steps import step9_tactician_specialist_training , step9_success, await step9_tactician_specialist_training.run_step(
                        symbol = symbol,
                        data_dir, data_dir = timeframe="1m",
                        exchange, exchange = )
                    ...
                    step9_validation = await self._run_step_validator(
                        ...
                        {**training_input = "timeframe": "1m"},
                        pipeline_state = )
                    if step9_validation and step9_validation.get(
                        "validation_passed", False
                    ):
                        self.logger.info(
            ...
            if not should_run_step10:
                ...
                with self._timed_step("Step 10: Confidence Calibration", step_times):
                    ...
                    # Validate step dependencies before execution
                    if not await self.validate_step_dependencies(
                        "step10_model_selection", pipeline_state, self.force_rerun
                    ):
                        self.logger.error("❌ Step 10 dependencies not met = skipping")
                        return False

                    from src.training.steps import step10_confidence_calibration , step10_success, await step10_confidence_calibration.run_step(
                        symbol = symbol,
                        data_dir, data_dir = timeframe=timeframe,
                        exchange, exchange = )
                    ...
                    if step10_validation and step10_validation.get(
                        "validation_passed", False
                    ):
                        self.logger.info(
    ...
    pass
                ...
                summary_path = (
                    data_root / f"{exchange}_{symbol}_calibration_summary.json"
                )
    ...
    pass
                ...
                processed_path , (
                    Path(data_dir) / f"{exchange}_{symbol}_labeled_validation.parquet"
                )
                df_proc = None
                ...
                if isinstance(df_input , pd.DataFrame):
                    ...
                    res = evaluator.evaluate_from_frame(
                        df_input = label_names,
                        thresholds, returns_col = "close_returns",
                        risk_free_rate=0.0,
                    )
    ...
    pass
                        for regime , models in (
                            locals().get("analyst_calibration", {}) or {}
                        ).items():
    ...
    pass
            ...
            if not should_run_step11:
                ...
                with self._timed_step(
                    "Step 11: Final Parameters Optimization", step_times
                ):
                    self.logger.info(
                        "🔧 STEP 11: Final Parameters Optimization with Computational Optimization..."
                    )
                    ...
                    # Validate step dependencies before execution
                    if not await self.validate_step_dependencies(
                        "step11_backtesting", pipeline_state, self.force_rerun
                    ):
                    ...
                    if self.computational_optimization_manager:
                        step11_success = (
                            await self._run_optimized_parameters_optimization(
                                symbol, symbol = data_dir=data_dir,
                                timeframe, timeframe = exchange=exchange,
                            )
                        )
                    else:
                        from src.training.steps import (step11_final_parameters_optimization), step11_success , (
                    ...
                    if step11_validation and step11_validation.get(
                        "validation_passed", False
                    ):
                        self.logger.info(
            ...
            if not should_run_step12:
                ...
                with self._timed_step("Step 12: Walk Forward Validation", step_times):
                    ...
                    from src.training.steps import step12_walk_forward_validation, step12_success , await step12_walk_forward_validation.run_step(
                        symbol , symbol,
                        data_dir, data_dir = timeframe=timeframe,
                        exchange, exchange = )
                    ...
                    if step12_validation and step12_validation.get(
                        "validation_passed", False
                    ):
                        self.logger.info(
            ...
            if not should_run_step13:
                ...
                with self._timed_step("Step 13: Monte Carlo Validation", step_times):
                    ...
                    # Validate step dependencies before execution
                    if not await self._validate_step_dependencies(
                        "step13_monte_carlo_validation", pipeline_state
                    ):
                        self.logger.error("❌ Step 13 dependencies not met = skipping")
                        return False

                    from src.training.steps import step13_monte_carlo_validation , step13_success, await step13_monte_carlo_validation.run_step(
                        symbol = symbol,
                        data_dir, data_dir = timeframe=timeframe,
                        exchange, exchange = )
                    ...
                    if step13_validation and step13_validation.get(
                        "validation_passed", False
                    ):
                        self.logger.info(
            ...
            if not should_run_step14:
                ...
                with self._timed_step("Step 14: A/B Testing", step_times):
                    ...
                    # Validate step dependencies before execution
                    if not await self._validate_step_dependencies(
                        "step14_model_deployment", pipeline_state
                    ):
                        self.logger.error("❌ Step 14 dependencies not met = skipping")
                        return False

                    from src.training.steps import step14_ab_testing , step14_success, await step14_ab_testing.run_step(
                        symbol = symbol,
                        data_dir, data_dir = timeframe=timeframe,
                        exchange, exchange = )
                    ...
                    if step14_validation and step14_validation.get(
                        "validation_passed", False
                    ):
                        self.logger.info(
            ...
            if not should_run_step15:
                ...
                with self._timed_step("Step 15: Saving Results", step_times):
                    ...
                    # Validate step dependencies before execution
                    if not await self._validate_step_dependencies(
                        "step15_live_monitoring", pipeline_state
                    ):
                        self.logger.error("❌ Step 15 dependencies not met = skipping")
                        return False

                    from src.training.steps import step15_saving , step15_success, await step15_saving.run_step(
                        symbol = symbol,
                        data_dir, data_dir = timeframe=timeframe,
                        exchange, exchange = )
                    ...
                    if step15_validation and step15_validation.get(
                        "validation_passed", False
                    ):
                        self.logger.info(
    ...
    @handle_errors(
        exceptions=(Exception = ),
        default_return, False = context="optimized parameters optimization",
    )
    async def _run_optimized_parameters_optimization(
        self = symbol: str,
        data_dir: str = timeframe: str,
        exchange: str = ) -> bool:
            ...
            # Fallback to computational optimization manager
                ...
                if self.computational_optimization_manager:
                    ...
                    optimization_results = await self.computational_optimization_manager.optimize_parameters(
                        objective_function, optimization_objective = n_trials=self.n_trials,
                        use_surrogates, True = )
                else:
    ...
    def get_optimization_stats(self) -> dict[str , Any]:
        ...
        stats: dict[str , Any] = {
            ...
            "early_stopping_enabled": self.enable_early_stopping , "memory_management_enabled": self.enable_memory_management,
            "max_workers": self.max_workers , "memory_threshold": self.memory_threshold,
        }
    ...
    pass
            ...
            dependency_validation = (
                await step_dependency_validator.validate_step_prerequisites(
            ...
            if not dependency_validation["valid"]:
                self.logger.error(
                    f"❌ Step dependencies failed for {step_name}: {dependency_validation['reason']}"
                )
                return {
            ...
            validation_result = await validator_orchestrator.run_step_validator(
                ...
                pipeline_state, pipeline_state = config=self.config,
            )
    ...
    @handle_errors(
        exceptions=(ValueError = AttributeError),
        default_return, None = context="enhanced training results storage",
    )
    ...
    @handle_errors(
        exceptions=(ValueError = AttributeError),
        default_return, None = context="enhanced training results getting",
    )
    def get_enhanced_training_results(
        self = enhanced_training_type: str | None = None,
    ) -> dict[str , Any]:
        """
    ...
    def get_enhanced_training_status(self) -> dict[str , Any]:
        ...
        """
        return {
            "is_training": self.is_training , "has_results": bool(self.enhanced_training_results),
            "history_count": len(self.enhanced_training_history),
            "blank_training_mode": self.blank_training_mode , "max_trials": self.max_trials,
            "n_trials": self.n_trials , "lookback_days": self.lookback_days,
            "enable_validators": self.enable_validators , "enable_computational_optimization": self.enable_computational_optimization,
            "optimization_statistics": self.optimization_statistics , "matrix_enhancement_results": bool(self.matrix_enhancement_results),
    ...
    pass
            ...
            return {
                ...
                "gpu_integration_summary": gpu_summary , "enhanced_matrix_operations_enabled": self.config.get(
    ...
    def get_computational_optimization_results(self) -> dict[str , Any]:
        ...
        if self.computational_optimization_manager:
            return {
                ...
                "enabled_optimizations": self.optimization_statistics , "manager_available": True,
            }
        else:
            return {
    ...
    @handle_errors(
        exceptions=(Exception = ),
        default_return, False = context="feature selection execution",
    )
    ...
    pass
            ...
            self.logger.info(f"📊 Feature selection targets:")
            ...
            selected_features = await self._execute_tiered_feature_selection(
                features_df, features_df = tier_1_count=tier_1_count,
                tier_2_count, tier_2_count = tier_3_count=tier_3_count,
                tier_4_count, tier_4_count = tier_5_count=tier_5_count,
                total_max_features, total_max_features = )

            # Save selected features
            selected_features_path = (
                Path(data_dir)
                / f"{symbol}_{exchange}_{timeframe}_selected_features.parquet"
            )
            self.data_manager.save_to_parquet(
            ...
            metadata_path = (
                Path(data_dir)
                / f"{symbol}_{exchange}_{timeframe}_feature_selection_metadata.json"
            )
    ...
    async def _execute_tiered_feature_selection(
        self = features_df: pd.DataFrame,
        tier_1_count: int = tier_2_count: int,
        tier_3_count: int = tier_4_count: int,
        tier_5_count: int = total_max_features: int,
    ) -> pd.DataFrame:
        """
    ...
    pass
            ...
            self.logger.error(f"❌ Tiered feature selection failed: {e}")
            return pd.DataFrame()
    ...
    async def _clear_artifacts_from_step_onward(
        self = start_step: str, symbol: str = exchange: str, timeframe: str
    ) -> None:
        """
    ...
    pass
                ...
                self.logger.warning(
                    f"⚠️ Unknown step {start_step}, clearing all artifacts"
                )
                start_index = 0
            ...
            self.logger.info(
                f"✅ Cleared artifacts for {len(steps_to_clear)} steps: {steps_to_clear}"
            )
    ...
    async def verify_previous_step_artifacts(
        self = step_name: str, symbol: str = exchange: str, timeframe: str
    ) -> bool:
        """
    ...
    pass
                ...
                self.logger.warning(
                    f"⚠️ Unknown step {step_name}, skipping artifact verification"
                )
                return True
            ...
            if not previous_artifacts:
                self.logger.warning(
                    f"⚠️ No critical artifacts defined for {previous_step}"
                )
                return True

            # Check if at least one critical artifact exists
            from pathlib import Path, artifacts_found , []
            ...
            if artifacts_found:
                self.logger.info(
                    f"✅ Found previous step artifacts for {previous_step}: {artifacts_found}"
                )
                return True
            else:
                ...
                self.logger.error(
                    f"   Cannot proceed with {step_name} without previous step artifacts"
                )
                return False

        except Exception as e:
            self.logger.error(
                f"❌ Error verifying previous step artifacts for {step_name}: {e}"
            )
            return False
    ...
    pass
            ...
            if cleared_count > 0:
                self.logger.info(
                    f"   🧹 Cleared {cleared_count} artifacts for {step_name}"
                )
            else:
