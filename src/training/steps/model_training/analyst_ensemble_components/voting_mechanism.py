...
class VotingMechanism:
    ...
    @handles_errors(exceptions=(Exception,), default_return={}, context='voting ensemble creation')
    async def create_voting_ensemble(self, models: Dict[str, Dict[str, Any]], voting_types: List[str]) -> Dict[str, Any]:
        ...
        for voting_type in voting_types:
            if voting_type == 'hard':
                ...
                ensemble = await self._create_weighted_soft_voting_ensemble(base_models, models)
            else:
    ...
    async def _create_hard_voting_ensemble(self, base_models: List[tuple]) -> Optional[VotingClassifier]:
    ...
    async def _create_soft_voting_ensemble(self, base_models: List[tuple]) -> Optional[VotingClassifier]:
        ...
        try:
            ...
            for name, model in base_models:
                if hasattr(model, 'predict_proba'):
                    ...
                    self.logger.warning(f"Model {name} doesn't support predict_proba, excluding from soft voting")
            if not proba_models:
    ...
    async def _create_weighted_soft_voting_ensemble(self, base_models: List[tuple], models_info: Dict[str, Dict[str, Any]]) -> Optional[VotingClassifier]:
    ...
    async def create_dynamic_voting_ensemble(self, models: Dict[str, Dict[str, Any]], features: pd.DataFrame) -> Optional[Any]:
        ...
        self.logger.info(f'Created dynamic voting ensemble with {len(base_models)} models')
