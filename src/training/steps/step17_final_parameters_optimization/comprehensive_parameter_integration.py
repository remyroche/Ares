...
class ComprehensiveParameterIntegration:
    ...
    async def _validate_all_applied_parameters(self, application_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_steps = len(
                [r for r in application_results.get("parameters_applied", {}).values() if r.get("success")]
            )
            ...
            validation["validation_metrics"] = {
                ...
                "overall_validation_score": (
                    sum([v.get("validation_score", 0) for v in validation["step_validation"].values()])
                    / len(validation["step_validation"])
                    if validation["step_validation"]
                    else 0
                ),
            }
    ...
    def _log_parameter_application_to_mlflow(self, application_results: Dict[str, Any]):
        ...
        try:
            ...
            if validation:
                ...
                mlflow.log_metric(
                    "overall_validation_score",
                    validation.get("validation_metrics", {}).get("overall_validation_score", 0),
                )
    ...
    def _generate_integration_recommendations(self) -> List[str]:
        ...
        if self.parameter_validation:
            failed_validations = [
                step for step, status in self.parameter_validation.items() if not status.get("validation_passed", False)
            ]
    ...
    async def _store_integration_results(self, integration_report: Dict[str, Any]):
        ...
        try:
            ...
            metadata = {
                ...
                "total_steps_integrated": len(
                    integration_report.get("integration_status", {}).get("parameters_applied", {})
                ),
