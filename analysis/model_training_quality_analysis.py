...
class ModelTrainingQualityAnalyzer:
        ...
        for loss_col in loss_columns:
            ...
            if len(values) > 5:
                ...
                if is_converged:
                    ...
                    stability_score = max(0, 100 - later_std * 1000) if later_std is not None else 0
    ...
    def _create_training_visualizations(self):
        ...
        try:
            ...
            if self.training_data is not None:
                ...
                if loss_cols:
                    for _i, col in enumerate(loss_cols[:2]):  # Plot first 2 loss metrics
    ...
    def save_report(self, filename="model_training_quality_report.txt"):
        ...
        with open(filename, "w") as f:
            ...
            f.writelines(f"{metric}: convergence={analysis['convergence_score']:.1f}, stability={analysis['stability_score']:.1f}\n" for metric, analysis in training_metrics.items())
            f.write("\n")

            # Model performance
            model_performance = self.report.get("model_performance", {})
            f.write("MODEL PERFORMANCE:\n")
            f.writelines(f"{metric}: {analysis['final_value']:.4f} (score: {analysis['score']:.1f})\n" for metric, analysis in model_performance.items())
            f.write("\n")
...
def main():
    ...
    for path in data_paths:
        if os.path.exists(path) and analyzer.load_training_data(path):
            data_loaded = True
            break
