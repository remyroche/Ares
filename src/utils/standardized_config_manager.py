...
class StandardizedConfigManager:
    ...
    def save_config(self, config_type: str, config_name: str, config: Dict[str, Any]) -> bool:
    ...
    def get_standardized_paths(self, exchange: str, symbol: str) -> Dict[str, Path]:
        """Get standardized paths for data and model storage."""
        return {
...
def get_standardized_config(step_name: str, config_overrides: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Get standardized configuration for a pipeline step."""
    # Create a default base config if loading fails
    try:
        base_config = config_manager.load_config("pipeline", "default")
...
def validate_step_config(step_name: str, step_config: Dict[str, Any]) -> bool:
    """Validate step configuration has required keys."""
