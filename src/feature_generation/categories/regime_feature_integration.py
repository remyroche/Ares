...
class RegimeFeatureIntegration(FeatureGenerator):
    ...
    def _generate_comprehensive_indicators(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        NOTE: Only includes features NOT already generated by other categories:
        - momentum.py: RSI, MACD already covered
        - volatility.py: Bollinger Bands, ATR already covered
        - trend.py: MA, EMA already covered
        - volume.py: VWAP already covered
        - oscillator.py: CCI already covered
        - advanced_statistical.py: Hurst exponent, Skew/Kurtosis already covered
        
        This method focuses on:
        - MA/EMA slopes (trend strength/direction)
        - Specific rolling std dev windows
        - Price momentum (1-5 bars)
        ...
        try:
            ...
            # 10. MA slope - 3-5 bars (trend strength) - NOT in trend.py
            for window in [3, 4, 5]:
                if len(data) >= window:
                    if self.vectorbt_rolling_optimizer:
                        try:
            ...
            # 11. EMA slope - 3-5 bars (trend direction) - NOT in trend.py
            for window in [3, 4, 5]:
                if len(data) >= window:
            ...
            # 14. Rolling std dev - 5-20 bars (specific windows for regime detection)
            # Note: volatility.py has rolling std but may not have these specific windows
            ...
            # 21. Price momentum - 1-5 bars (specific short-term momentum)
            # Note: momentum.py has momentum but may not have these specific 1-5 bar windows
            ...
            # 28. Rolling z-score of returns - 5-10 bars - NOT in other categories
            for window in [5, 10]:
                if len(returns) >= window:
            ...
            # 29. EWMA volatility - 10-20 bars - NOT in volatility.py (they use standard rolling)
            for window in [10, 20]:
                if len(returns) >= window:
