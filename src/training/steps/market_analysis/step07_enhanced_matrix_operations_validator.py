...
class Step7EnhancedMatrixOperationsValidator(BaseValidator):
    ...
    def validate_step_prerequisites(self, symbol: str, exchange: str, timeframe: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not step06_files:
                ...
                validation_result['errors'].append(f'Step 06 feature engineering output not found for {exchange}_{symbol}_{timeframe}')
            else:
            ...
            if not matrix_ops_dir.exists():
                validation_result['warnings'].append(f'Matrix operations directory does not exist: {matrix_ops_dir}')
    ...
    def validate_step_output(self, symbol: str, exchange: str, timeframe: str) -> dict[str, Any]:
        ...
        try:
            ...
            expected_files = [f'{exchange}_{symbol}_{timeframe}_matrix_operations_config.json', f'{exchange}_{symbol}_{timeframe}_matrix_operations_results.json', f'{exchange}_{symbol}_{timeframe}_matrix_operations_summary.json']
            missing_files = []
            existing_files = []
            ...
            if missing_files:
                validation_result['validation_passed'] = False
                validation_result['errors'].extend([f'Missing matrix operations file: {f}' for f in missing_files])
            else:
                validation_result['details']['files_found'] = len(existing_files)
                validation_result['details']['files'] = existing_files
            if existing_files:
                file_validations = self._validate_file_contents(output_dir, symbol, exchange, timeframe)
    ...
    def _validate_file_contents(self, output_dir: Path, symbol: str, exchange: str, timeframe: str) -> list[dict[str, Any]]:
    ...
    def _validate_config_file(self, config_file: Path) -> dict[str, Any]:
        ...
        try:
            ...
            required_fields = ['enable_gpu_acceleration', 'enable_sparse_optimizations', 'enable_memory_optimization', 'enable_parallel_processing', 'condition_number_threshold', 'min_eigenvalue_threshold', 'correlation_threshold', 'memory_threshold_gb', 'batch_size', 'max_iterations', 'tolerance', 'data_shape', 'numeric_columns', 'operations']
            ...
            if 'operations' in config:
                expected_operations = ['correlation_analysis', 'condition_number_check', 'eigenvalue_analysis', 'singular_value_decomposition', 'matrix_rank_analysis']
    ...
    def _validate_summary_file(self, summary_file: Path) -> dict[str, Any]:
        ...
        try:
            ...
            required_fields = ['timestamp', 'symbol', 'exchange', 'timeframe', 'operations_performed', 'data_shape', 'numeric_columns']
    ...
    def get_validation_summary(self, symbol: str, exchange: str, timeframe: str) -> dict[str, Any]:
        ...
        return {'step_name': 'step07_enhanced_matrix_operations', 'symbol': symbol, 'exchange': exchange, 'timeframe': timeframe, 'prerequisites_validation': prerequisites, 'output_validation': output, 'overall_validation_passed': prerequisites['validation_passed'] and output['validation_passed'], 'total_warnings': len(prerequisites['warnings']) + len(output['warnings']), 'total_errors': len(prerequisites['errors']) + len(output['errors'])}