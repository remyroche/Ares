...
class TacticianModelsTrainingStep:
    ...
    async def train_tactician_models(
        ...
        **kwargs
        ...
        try:
            ...
            tprint_info("🔧 Optimizing Tactician training data with VectorBT utilities...")
            optimized_training_data = self.optimize_training_data(training_data)
            
            # Generate additional VectorBT features for Tactician models
            enhanced_training_data = self.generate_tactician_vectorbt_features(optimized_training_data, feature_columns)
            ...
            if new_features:
                tprint_success(f"✅ Added {len(new_features)} Tactician VectorBT-generated features: {new_features[:5]}{'...' if len(new_features) > 5 else ''}")
            ...
            # Prepare training data with validated features (using enhanced data with VectorBT features)
    ...
    def optimize_training_data(self, training_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if memory_savings > 0:
                tprint_success(f"✅ Tactician data optimization completed: {memory_savings:.1f}% memory savings")
            else:
                tprint_success("✅ Tactician data optimization completed")
            
            return optimized_data
            
        except Exception as e:
            tprint_warning(f"⚠️ Tactician data optimization failed: {e}, returning original data")
            return training_data

    def generate_tactician_vectorbt_features(self, training_data: pd.DataFrame, 
                                           feature_columns: List[str]) -> pd.DataFrame:
        """
        Generate additional features using VectorBT rolling operations for Tactician models.
        ...
        try:
            tprint_info("🔧 Generating VectorBT features for Tactician models...")
            ...
            for col in numeric_columns:
                if col in training_data.columns:
                    try:
                        # Generate rolling features optimized for Tactician timing decisions
                        rolling_mean_10 = self.rolling_optimizer.rolling_mean(training_data[col], window=10)
                        rolling_std_10 = self.rolling_optimizer.rolling_std(training_data[col], window=10)
                        ...
                        # Add new features
                        enhanced_data[f'{col}_tactician_rolling_mean_10'] = rolling_mean_10
                        enhanced_data[f'{col}_tactician_rolling_std_10'] = rolling_std_10
                        enhanced_data[f'{col}_tactician_rolling_mean_30'] = rolling_mean_30
                        enhanced_data[f'{col}_tactician_rolling_std_30'] = rolling_std_30
                        
                        # Generate rolling momentum and volatility features
                        rolling_max_10 = self.rolling_optimizer.rolling_max(training_data[col], window=10)
                        rolling_min_10 = self.rolling_optimizer.rolling_min(training_data[col], window=10)
                        
                        enhanced_data[f'{col}_tactician_rolling_max_10'] = rolling_max_10
                        enhanced_data[f'{col}_tactician_rolling_min_10'] = rolling_min_10
                        enhanced_data[f'{col}_tactician_rolling_range_10'] = rolling_max_10 - rolling_min_10
                        
                        # Generate rolling correlation with volume if it's a price column
                                ...
                                if other_col != col and 'volume' in other_col.lower():
                                    try:
                                        rolling_corr = self.rolling_optimizer.rolling_corr(
                                            training_data[col], training_data[other_col], window=10
                                        )
                                        enhanced_data[f'{col}_{other_col}_tactician_rolling_corr_10'] = rolling_corr
                                    except Exception as e:
            ...
            tprint_success(f"✅ Generated Tactician VectorBT features: {vectorbt_ops} operations completed")
            
            return enhanced_data
            
        except Exception as e:
            tprint_warning(f"⚠️ Tactician VectorBT feature generation failed: {e}, returning original data")
            return training_data
