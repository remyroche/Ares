...
def compute_trade_path_quality_metrics(
    ...
    df["realized_profit_consistency"] = np.clip(
        df["final_ret"] / (df["mae"] + eps), -ratio_cap, ratio_cap
    )
    df["trajectory_smoothness"] = np.clip(
        df["final_ret"].abs() / (df["path_length"] + eps), 0.0, 1.0
    )

    # Convert to numpy arrays for Numba
    ratio = df["mfe_mae_ratio"].to_numpy(dtype=float)
    realized_profit_consistency = df["realized_profit_consistency"].to_numpy(
        dtype=float
    )
...
def run_lgbm_mask_generation_pipeline(
    ...
    if not combined_global_registry.empty:
        ...
        top_final = select_final_regimes(combined_global_registry, combined_mask_map, top_n=10)
...
class MaskAssessor:
    ...
    def _compute_subset_auc(self, X, fwd_ret, mask, folds) -> Tuple[float, float, np.ndarray]:
        ...
        score, coverage = self._compute_oof_learnability_score(
