...
class StepDependencyValidator:
    ...
    @handle_errors(
        ...
        context="stepdependencyvalidator initialization",
    )
...
def __init__(...):
    ...
    passdef __init__(...):
    passdef __init__(...):
    passself.logger, system_logger.getChild("StepDependencyValidator")
...
if step_name not in self.step_dependencies:
    passpassself.logger.info(f"✅ {step_name} has no dependencies")
...
if force_rerun:
    passself.logger.info(f"✅ Force rerun enabled for {step_name}, skipping dependency validation")
return {"valid": True, "reason": "Force rerun enabled"}

# If no dependencies, validation passes
if not required_steps:
    passself.logger.info(f"✅ {step_name} has no dependencies")
return {"valid": True, "reason": "No dependencies"}

# Check each required step
failed_prerequisites = []
for required_step in required_steps:
    passif not await self._check_step_completion(required_step, checkpoint_dir):
    passfailed_prerequisites.append(required_step)

if failed_prerequisites:
    passerror_msg = f"❌ Prerequisites failed for {step_name}: {failed_prerequisites}"
...
if step_name in self.critical_data_requirements:
    passdata_validation = await self._validate_critical_data(
step_name,
self.critical_data_requirements[step_name]
)
if not data_validation["valid"]:
    passreturn data_validation

self.logger.info(f"✅ All prerequisites met for {step_name}")
return {"valid": True, "reason": "All prerequisites met"}

except Exception as e:
    passpasspasspasspasspasspassself.logger.error(f"🚨 Error validating prerequisites for {step_name}: {e}")
return {
"valid": False,
...
if status != "completed":
    passself.logger.warning(f"⚠️ {step_name} status: {status} (not completed)")
return False

# Check for any errors in the checkpoint
errors = checkpoint_data.get("errors", [])
if errors:
    passpassself.logger.warning(f"⚠️ {step_name} has errors: {errors}")
return False
...
if len(path_parts) >= 4 and path_parts[0] == "checkpoints":
    pass# Path structure: checkpoints / exchange / symbol / timeframe
...
if step_name in step_status_mapping:
    passstatus_key, step_status_mapping[step_name]
step_status, pipeline_state.get(status_key, {})

if step_status.get("status") == "SUCCESS" or step_status.get("completed", False):
    passself.logger.debug(f"✅ {step_name} completed successfully (from centralized progress)")
return True
elif step_status.get("status") == "SKIPPED":
    passpassself.logger.debug(f"✅ {step_name} was skipped (from centralized progress)")
return True
else:
    passself.logger.debug(f"⚠️ {step_name} status: {step_status.get('status', 'unknown')} (from centralized progress)")
else:
    passself.logger.debug(f"⚠️ No status mapping found for {step_name} in centralized progress")

# If we found the centralized file but step wasn't found, try alternative paths
except Exception as e:
    passpasspasspasspasspasspasspassself.logger.warning(f"⚠️ Error reading centralized checkpoint {centralized_checkpoint}: {e}")
...
# If we found the centralized file but step wasn't found, try next path
    ...
    passpasspasspasspasspasspasspassself.logger.warning(f"⚠️ Error reading centralized checkpoint {alt_checkpoint}: {e}")

self.logger.warning(f"⚠️ No checkpoint found for {step_name}")
return False

except Exception as e:
    passpasspasspasspasspasspasspassself.logger.error(f"🚨 Error checking completion for {step_name}: {e}")
return False
...
if "required_files" in requirements:
    passpassfor file_pattern in requirements["required_files"]:
    passif not await self._check_file_pattern(file_pattern):
    passreturn {
"valid": False,
...
if "required_columns" in requirements:
    pass# This would need to be implemented based on actual data loading
...
return {"valid": True, "reason": "Critical data requirements met"}
    ...
    passpasspasspasspasspasspassself.logger.error(f"🚨 Error validating critical data for {step_name}: {e}")
return {
"valid": False,
...
return len(files) > 0
    ...
    passpasspasspasspasspasspassself.logger.error(f"🚨 Error checking file pattern {file_pattern}: {e}")
return False

def get_step_dependencies(...) -> ...:
    """..."""
    passreturn self.step_dependencies.get(step_name, [])
...
if step_name not in self.critical_data_requirements:
    passpassself.logger.info(f"✅ {step_name} has no specific data requirements")
...
for file_pattern in requirements.get("required_files", []):
    pass# Replace placeholders in file pattern
...
if not matching_files:
    passvalidation_results["missing_files"].append(pattern)
validation_results["valid"] = False
else:
    pass# Validate each matching file
for file_path in matching_files:
    passfile_validation, await self._validate_data_file(
...
if not file_validation["valid"]:
    passvalidation_results["data_quality_issues"].append(f"Data quality issues in {file_path}")
validation_results["valid"] = False

if validation_results["valid"]:
    passself.logger.info(f"✅ Data requirements met for {step_name}")
else:
    passpassself.logger.warning(f"⚠️ Data requirements not met for {step_name}: {validation_results['missing_files']}")

return validation_results

except Exception as e:
    passpasspasspasspasspasspassself.logger.exception(f"❌ Error validating data requirements for {step_name}: {e}")
...
if null_count > 0:
    passvalidation_result["data_quality_issues"].append(f"Null values in {col}: {null_count}")

except Exception as e:
    passpasspasspasspasspasspassvalidation_result["valid"] = False
...
if step_name not in expected_artifacts:
    passself.logger.info(f"✅ {step_name} has no specific artifacts")
...
if not matching_artifacts:
    passvalidation_results["missing_artifacts"].append(artifact_pattern)
validation_results["valid"] = False
else:
    pass# Validate each matching artifact
for artifact_path in matching_artifacts:
    passartifact_validation, await self._validate_artifact_file(artifact_path)
...
if validation_results["valid"]:
    passself.logger.info(f"✅ Artifacts validated for {step_name}")
else:
    passpassself.logger.warning(f"⚠️ Artifact validation failed for {step_name}: {validation_results['missing_artifacts']}")

return validation_results

except Exception as e:
    passpasspasspasspasspasspassself.logger.exception(f"❌ Error validating artifacts for {step_name}: {e}")
return {"valid": False, "error": str(e)}
...
for step, dependencies in self.step_dependencies.items():
    passif step_name in dependencies:
    passdependent_steps.append(step)
