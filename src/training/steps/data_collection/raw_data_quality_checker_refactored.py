...
class RawDataQualityChecker:
    ...
    def _get_default_config(self) -> dict[str, Any]:
        ...
        return {'critical_thresholds': {'min_rows': 100, 'max_missing_ratio': 0.5, 'max_duplicate_ratio': 0.1, 'max_negative_prices': 0.0, 'max_zero_volume_ratio': 0.9}, 'warning_thresholds': {'high_missing_ratio': 0.1, 'high_duplicate_ratio': 0.01, 'irregular_interval_ratio': 0.05, 'high_zero_volume_ratio': 0.5}, 'integrity_checks': {'check_ohlc_consistency': True, 'check_negative_values': True, 'check_extreme_movements': True, 'check_time_gaps': True, 'check_for_market_gaps': True}, 'score_weights': {'completeness': 0.25, 'consistency': 0.25, 'timeliness': 0.2, 'validity': 0.2, 'accuracy': 0.1}}
    ...
    def validate_raw_data(self, data: pd.DataFrame, symbol: str, exchange: str, auto_fix: bool=False, timeframe: str | None=None) -> tuple[dict[str, Any], pd.DataFrame]:
        ...
        """
        self.logger.info(f'🔍 Starting raw data validation for {symbol} on {exchange}...')
        results = {'validation_passed': True, 'critical_issues': [], 'warnings': [], 'data_quality_score': 0.0, 'symbol': symbol, 'exchange': exchange, 'timestamp': datetime.now().isoformat(), 'data_shape': data.shape, 'detailed_analysis': {}}
        ...
        if auto_fix and (not results['validation_passed']):
            ...
            processed_data, fix_results = self.validate_and_fix_data_quality_issues(data, symbol, exchange)
    ...
    def validate_and_fix_data_quality_issues(self, data: pd.DataFrame, symbol: str, exchange: str) -> tuple[pd.DataFrame, dict[str, Any]]:
        ...
        """
        fix_results = {'fixes_applied': [], 'fixes_failed': [], 'data_modified': False}
