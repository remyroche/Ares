...
def _scope(cfg: dict[str, Any] | None, scope: str | None = None) -> str:
    value = str(scope or (cfg or {}).get("strict_feature_parity_scope", "symbol")).lower()
...
def _contract_from_bundle(model_bundle: dict[str, Any] | None) -> FeatureTransformContract | None:
    if not isinstance(model_bundle, dict):
        return None
    ...
    if isinstance(contract, FeatureTransformContract):
        return contract
    inner = model_bundle.get("bundle")
    if isinstance(inner, dict) and isinstance(inner.get("feature_transform_contract"), FeatureTransformContract):
...
def _manifest_from_bundle(model_bundle: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    if isinstance(manifest, dict):
        return manifest
    inner = model_bundle.get("bundle")
    if isinstance(inner, dict) and isinstance(inner.get("feature_transform_manifest"), dict):
...
def validate_model_bundle_transform_contract(
    ...
    report.contract_hash = getattr(contract, "contract_hash", None) if contract is not None else None
    ...
    if isinstance(manifest, dict) and state_hash and str(manifest.get("contract_hash") or "") != state_hash:
    ...
    if contract is not None:
        ...
        if cfg_market and contract_market and cfg_market != contract_market:
            report.global_errors.append("feature_transform_contract_market_mode_mismatch")
        try:
        ...
        missing_stats = [c for c in transformable if c not in getattr(contract, "per_column_stats", {})]
        if missing_stats:
            report.global_errors.append(
                "feature_transform_contract_missing_stats:" + ",".join(missing_stats[:50])
            )

    if report.global_errors and _strict(cfg):
        report.ok = False
        tprint(f"FEATURE PARITY: global refusal {report.global_errors[:5]}")
        raise FeatureParityError("Feature transform contract parity failed", report.asdict())
    if contract is None:
        raise FeatureParityError("Feature transform contract is missing", report.asdict())
    tprint(
...
def validate_required_features_against_contract(
    ...
    if strict and (missing or never_requested):
        raise FeatureParityError(
            ...
            {
                "global_errors": [
                    *([f"missing_from_contract:{','.join(missing[:50])}"] if missing else []),
                    *([f"never_transform_requested:{','.join(never_requested[:50])}"] if never_requested else []),
                ]
            },
        )
...
def validate_transform_stat_completeness(
    ...
    for feature in sorted(required):
        ...
        if str(getattr(contract, "transform_config", {}).get("impute", "median")).lower() == "median":
            keys.append("median")
    ...
    if errors and strict:
        raise FeatureParityError(
            ...
            {"global_errors": ["missing_or_invalid_transform_stats"], "details": errors[:100]},
        )


def _add_symbol_error(report: FeatureParityReport, symbol: str, category: str, value: Any) -> None:
...
def validate_raw_history_sufficiency(
    ...
    if not isinstance(close, pd.DataFrame) or close.empty or not isinstance(close.index, pd.DatetimeIndex):
    ...
    min_ratio = float((cfg or {}).get("feature_parity_min_raw_history_finite_ratio", 0.95))
    warmup = int(getattr(contract, "required_warmup_hours", 0) or 0)
    lookbacks = getattr(contract, "required_lookback_hours_by_feature", {}) or {}
    required = [str(k) for k in (required_feature_keys or []) if str(k)]
    required_hours = max([warmup, *[int(lookbacks.get(k, 0) or 0) for k in required], 1])
    ...
    for symbol in symbols:
        if symbol not in close.columns:
            _add_symbol_error(report, symbol, "insufficient_history", {"reason": "missing_close_column"})
            continue
        ...
        finite_ratio = float(np.isfinite(window.to_numpy(dtype=np.float64)).mean()) if len(window) else 0.0
    ...
    return apply_feature_parity_scope(report.asdict(), symbols, report.scope, strict=strict_b)
...
def validate_raw_feature_availability(
    ...
    end_ts = pd.Timestamp(end_ts)
    require_current = bool((cfg or {}).get("feature_parity_require_current_timestamp", True))
    required = [str(k) for k in (required_feature_keys or []) if str(k)]
...
def validate_final_model_matrix(
    ...
    if not np.isfinite(values).all():
        bad_cols = [
            ...
            if not np.isfinite(X_float[col].to_numpy(dtype=np.float32, copy=False)).all()
        ]
        report["global_errors"].append("model_matrix_nonfinite")
        report["nonfinite_features"] = bad_cols[:100]
        if strict:
            raise FeatureParityError("Final model matrix contains non-finite values", report)
    return X_float
