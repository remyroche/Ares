...
class ImprovedAutoencoderPreprocessor:
    ...
    def __init__(self, use_robust_scaling=True, outlier_threshold=3.0):
        ...
        self.logger.info(
            f"Preprocessor initialized with robust_scaling={use_robust_scaling}, outlier_threshold={outlier_threshold}",
        )
    ...
    def transform(self, X: pd.DataFrame) -> np.ndarray:
        if not self.is_fitted:
            self.logger.error(
                "Preprocessor must be fitted before transform can be called.",
            )
    ...
    def _remove_extreme_outliers(self, X: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            lower_clipped_count = np.sum(lower_bound > X)
            upper_clipped_count = np.sum(upper_bound < X)
            if lower_clipped_count > 0 or upper_clipped_count > 0:
                self.logger.debug(
                    f"Clipping {lower_clipped_count} lower-bound outliers and {upper_clipped_count} upper-bound outliers.",
                )
...
class RobustAutoencoder:
    ...
    def __init__(
        ...
        self.logger.info(
            f"RobustAutoencoder initialized with input_dim={input_dim}, encoding_dim={self.encoding_dim}, dropout_rate={dropout_rate}",
        )
    ...
    def fit(
        ...
        self.logger.info(
            f"Starting autoencoder training for {epochs} epochs with batch size {batch_size}.",
        )
        try:
...
class AutoencoderFeaturePipeline:
    ...
    def generate(self, features_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.info(
                f"Selected {clean_features.shape[1]} features for autoencoder",
            )
            ...
            reconstruction_error = np.mean(
                (clean_features.values - reconstructed) ** 2,
                axis=1,
            )
            result_df["autoencoder_reconstruction_error"] = reconstruction_error

            self.logger.info(
                f"Autoencoder feature generation completed. Added {encoded_features.shape[1] + 1} new features.",
            )
            return result_df
    ...
    def _select_and_clean_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            valid_cols = missing_counts[
                missing_counts / len(df) < missing_threshold
            ].index.tolist()
            ...
            selected_df = (
                df[finite_cols].fillna(method="ffill").fillna(method="bfill").fillna(0)
            )
    ...
    def get_pipeline_info(self) -> dict[str, Any]:
        ...
        try:
            return {
                ...
                "dropout_rate": self.autoencoder.dropout_rate
                if self.autoencoder
                else None,
            }
        except Exception as e:
...
class AutoencoderFeatureGenerator:
    ...
    def generate_features(self, features_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.info("✅ Autoencoder feature generation completed")
            self.logger.info(f"📊 Output features shape: {result_df.shape}")
            self.logger.info(
                f"📊 Added autoencoder features: {[col for col in result_df.columns if 'autoencoder' in col]}",
            )
