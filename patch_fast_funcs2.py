...
def patch_file():
    ...
    content = content.replace("""    elif func == _numba_rolling_median:
        ...
        return apply_to_matrix_parallel(df, up_down_return_mass_ratio_nb_parallel, *args)""", """    elif func == _numba_rolling_median:
        return apply_to_matrix_parallel(df, _numba_rolling_median_parallel, *args)""")

    content = content.replace("""    elif func == realized_vol_nb:""", """    elif func == bars_since_flip_nb:
        return apply_to_matrix_parallel(df, bars_since_flip_nb_parallel, *args)
    elif func == consecutive_bars_nb:
        return apply_to_matrix_parallel(df, consecutive_bars_nb_parallel, *args)
    elif func == up_down_semivol_ratio_nb:
