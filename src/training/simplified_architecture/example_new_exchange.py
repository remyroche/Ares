...
class BybitDataSource(BaseExchangeDataSource):
    ...
    async def fetch_data(self, symbol: str, start: datetime, end: datetime) -> pd.DataFrame:
        ...
        data = pd.DataFrame({'timestamp': pd.date_range(start, end, freq='1H')[:hours], 'open': np.random.randn(hours) * 8 + 40000, 'high': np.random.randn(hours) * 8 + 40500, 'low': np.random.randn(hours) * 8 + 39500, 'close': np.random.randn(hours) * 8 + 40000, 'volume': np.random.randint(500, 5000, hours)})
...
class CatBoostModel(IModel):
    ...
    def __init__(self, catboost_model: Any) -> None:
...
class CatBoostTrainer(BaseModelTrainer):
    ...
    def _get_default_hyperparameters(self) -> Dict[str, Any]:
        ...
        return {'iterations': 100, 'learning_rate': 0.05, 'depth': 6, 'loss_function': 'Logloss', 'verbose': False, 'random_state': 42}
