...
def extract_training_metadata(config: Dict[str, Any]) -> Dict[str, Any]:
    """Extract training metadata from configuration.
...
def log_bot_version_to_mlflow(run_id: Optional[str] = None) -> None:
    """Log bot version to MLflow run.
...
def log_training_metadata_to_mlflow(
    ...
    """Log training metadata to MLflow run.
...
def log_model_with_metadata(
    ...
    """Log model with enhanced metadata to MLflow.
...
def log_artifacts_with_metadata(
    ...
    """Log artifacts with enhanced metadata to MLflow.
...
def log_metrics_with_metadata(
    ...
    """Log metrics with enhanced metadata to MLflow.
...
def log_params_with_metadata(
    ...
    """Log parameters with enhanced metadata to MLflow.
...
@handle_errors(default_return=Dict[str, Any], context="mlflow_utils.get_run_with_bot_version")
def get_run_with_bot_version(run_id: str) -> Dict[str, Any]:
    """Get run information with bot version.
    
    Args:
        run_id: MLFlow run ID
        
    Returns:
        Dictionary with run information and metadata
    """
...
@handle_errors(default_return=Dict[str, Any], context="mlflow_utils.get_enhanced_run_metadata")
def get_enhanced_run_metadata(run_id: str) -> Dict[str, Any]:
    ...
    Returns:
        Dictionary with enhanced run metadata
    """
    client = mlflow.tracking.MlflowClient()
    run = client.get_run(run_id)
...
@handle_errors(default_return=bool, context="mlflow_utils.validate_run_metadata")
def validate_run_metadata(run_id: str) -> bool:
    """Validate that a run has all required metadata.
...
@handle_errors(default_return=str, context="mlflow_utils.ensure_enhanced_mlflow_run")
def ensure_enhanced_mlflow_run(
    config: Dict[str, Any],
    experiment_name: Optional[str] = None,
    run_name: Optional[str] = None
) -> str:
    """Ensure an enhanced MLflow run exists with all required metadata.
