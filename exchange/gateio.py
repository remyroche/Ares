...
class GateioExchange(BaseExchange):
    ...
    async def get_klines(self, symbol: str, interval: str, limit: int = 1000) -> list[dict[str, Any]]:
        ...
        try:
            ...
            ohlcv = await self.exchange.fetch_ohlcv(market_id, timeframe=interval, limit=limit)
            logger.info(f"📊 GATEIO: CCXT Response: Received {len(ohlcv) if ohlcv else 0} klines")
            ...
            logger.info(f"✅ GATEIO: get_klines completed, returned {len(result)} klines")
            return result
        except Exception as e:
            logger.error(f"❌ GATEIO: Error fetching klines from Gate.io for {symbol}: {e}")
    ...
    async def get_historical_klines(
        ...
        logger.info(f"📊 Parameters: interval={interval}, start_time_ms={start_time_ms}, end_time_ms={end_time_ms}, limit={limit}")
        ...
        try:
            ...
            while since < end_time_ms:
                call_count += 1
                logger.info(f"📡 GATEIO: CCXT Call {call_count}: Fetching klines from {datetime.fromtimestamp(since / 1000)}")
                logger.info(f"🔧 GATEIO: CCXT Parameters: market_id={market_id}, timeframe={interval}, since={since}, limit={limit}")
                ...
                if not ohlcv:
                    logger.info(f"⚠️ GATEIO: No more klines available at {datetime.fromtimestamp(since / 1000)}")
                    break
                ...
                logger.info(f"📈 GATEIO: Added {len(klines)} klines, total: {len(all_klines)}")
                
                # Update since for next iteration
                if len(ohlcv) > 0:
                    since = ohlcv[-1][0] + 1  # Next timestamp after the last kline
                    logger.info(f"⏭️ GATEIO: Advancing to {datetime.fromtimestamp(since / 1000)}")
                else:
                    logger.info(f"⏭️ GATEIO: No klines, stopping")
                    break
                    
                # Add small delay to respect rate limits
                await asyncio.sleep(0.1)
                
            logger.info(f"✅ GATEIO: get_historical_klines completed, returned {len(all_klines)} klines")
            return all_klines
        except Exception as e:
            logger.error(f"❌ GATEIO: Error fetching historical klines from Gate.io for {symbol}: {e}")
            logger.error(f"🔍 GATEIO: Full error details: {type(e).__name__}: {str(e)}")
            return []
    ...
    async def get_historical_agg_trades(
        ...
        try:
            ...
            try:
                logger.info(f"   🌐 Attempting direct HTTP request to Gate.io historical trades API")
                
                # Define base headers for Gate.io API
                base_headers = {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
                    'Timestamp': str(int(datetime.now().timestamp() * 1000))  # Always required
                }
                ...
                logger.info(f"   ⚠️ Gate.io public API has limited historical access, using CCXT fallback")
                logger.info(f"   📊 Requesting data from {datetime.fromtimestamp(since / 1000)} to {datetime.fromtimestamp(end_time_ms / 1000)}")
                logger.info(f"   ⏰ This is likely beyond Gate.io's public API limits (10,000 points ago)")
            ...
            while since < end_time_ms and total_calls < max_calls:
                try:
                    logger.info(f"   📡 CCXT Call {total_calls + 1}/{max_calls}: Fetching trades from {datetime.fromtimestamp(since / 1000)}")
                    logger.info(f"   🔧 CCXT Parameters: symbol={market_id}, since={since}, limit={min(limit, 100)}")
                    ...
                    logger.info(f"   📊 CCXT Response: Received {len(trades) if trades else 0} trades")
                    ...
                    if trades:
                        ...
                        logger.info(f"   ⏭️ Advancing to {datetime.fromtimestamp(since / 1000)}")
                    else:
                        since += 86400000  # Advance by 1 day if no trades
                        logger.info(f"   ⏭️ No trades, advancing by 1 day to {datetime.fromtimestamp(since / 1000)}")
            ...
            logger.info(f"✅ GATEIO: get_historical_agg_trades completed, returned {len(all_trades)} trades")
            return all_trades
            
        except Exception as e:
            logger.error(f"❌ GATEIO: Error fetching historical trades from Gate.io for {symbol}: {e}")
            logger.error(f"🔍 GATEIO: Full error details: {type(e).__name__}: {str(e)}")
            return []
    ...
    async def get_historical_futures_data(
        ...
        try:
            ...
            try:
                logger.info(f"   🌐 Attempting direct HTTP request to Gate.io public API")
                            ...
                            async with session.get(endpoint['url'], params=endpoint['params']) as response:
                                if response.status == 200:
                                    ...
                                    if data and len(data) > 0:
                                        logger.info(f"   📈 Found {len(data)} funding rate records from {endpoint['name']}")
                                        ...
                                        for item in data:
                                            if isinstance(item, dict):
                                                formatted_item = {
                                                    "symbol": item.get("contract", symbol),
                                                    "funding_rate": item.get("funding_rate", item.get("rate", 0)),
                                                    "funding_time": item.get("funding_time", item.get("time", 0)),
                                    ...
                                    if response.status == 401:
                                        logger.info(f"   ℹ️ Skipping futures data - authentication required but not available")
                                        return []
                    except Exception as e:
            ...
            logger.info(f"   🔄 Falling back to CCXT for funding rates")
            try:
            ...
            logger.error(f"Error fetching historical futures data from Gate.io for {symbol}: {e}")
            return []
    ...
    async def get_order_status(self, symbol: str, order_id: int):
        ...
        try:
            ...
            logger.error(f"Failed to get status for order {order_id} on Gate.io {symbol}: {e}")
            return {"error": str(e)}
    ...
    async def get_position_risk(self, symbol: str = None):
        ...
        try:
            ...
            logger.error(f"Failed to get position risk from Gate.io for {symbol or 'all symbols'}: {e}")
            return []
    ...
    async def get_open_orders(self, symbol: str = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to get open orders from Gate.io for {symbol or 'all symbols'}: {e}")
            return []
    ...
    async def get_historical_agg_trades_ccxt(
        ...
        logger.info(f"📊 Parameters: start_time_ms={start_time_ms}, end_time_ms={end_time_ms}, limit={limit}")
        ...
        try:
            ...
            logger.info(f"✅ GATEIO: get_historical_agg_trades_ccxt completed, returned {len(result)} trades")
            return result
        except Exception as e:
    ...
    async def get_historical_klines_ccxt(
        ...
        try:
            ...
            logger.info(f"✅ GATEIO: get_historical_klines_ccxt completed, returned {len(result)} klines")
            return result
        except Exception as e:
