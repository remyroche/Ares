    ...
    async def _log_step1_artifacts_and_report(
        ...
        try:
            ...
            report_data = create_detailed_step_report(
                ...
                errors_encountered=(
                    [] if pipeline_state.get("data_collection_completed", False) else ["Data collection failed"]
                ),
            )
                ...
                if os.path.exists(file_path):
                    ...
                    try:
                        ...
                        if validation_result.passed:
                            self.logger.info(
                                f"✅ {file_name} quality check passed (score: {
    ...
    async def _validate_downloaded_data(self, symbol: str, exchange: str, timeframe: str, data_dir: str) -> bool:
        ...
        try:
            ...
            for file_name in expected_files:
                ...
                if os.path.exists(file_path):
                    ...
                    try:
                        ...
                        if validation_result.passed:
                            self.logger.info(
                                f"✅ {file_name} validation passed (quality score: {
                                    validation_result.quality_score:.2f})"
                            )
                        else:
                            self.logger.warning(
            ...
            if validation_results:
                ...
                overall_quality_score = sum(result.quality_score for result in validation_results) / len(
                    validation_results
                )
                self.logger.info(
                    f"📊 Overall validation: {
                        'PASSED' if overall_passed else 'FAILED'} (avg quality score: {
