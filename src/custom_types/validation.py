...
@staticmethod def validate_type(value: Any, expected_type: Any, context: str = "") -> T: """ Validate that a value matches the expected type.  Args: value: The value to validate expected_type: The expected type context: Context for error messages  Returns: The validated value  Raises: RuntimeTypeError: If validation fails """ if not TypeValidator._check_type(value, expected_type): raise RuntimeTypeError(expected_type, value, context)
    return value

@staticmethod def _check_type(value: Any, expected_type: Any) -> bool: """Check if value matches expected type.""" origin = get_origin(expected_type)
