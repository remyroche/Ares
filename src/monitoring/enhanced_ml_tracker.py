...
class ModelComparisonReport:
    ...
    # Ensemble analysis
    ensemble_effectiveness: dict[str, float]  # How much ensemble improves over individuals
...
class EnhancedMLTracker:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.enable_real_time_tracking = self.tracker_config.get("enable_real_time_tracking", True)
        self.enable_ensemble_analysis = self.tracker_config.get("enable_ensemble_analysis", True)
        self.enable_model_comparison = self.tracker_config.get("enable_model_comparison", True)
        self.performance_window_days = self.tracker_config.get("performance_window_days", 7)
        self.min_predictions_for_analysis = self.tracker_config.get("min_predictions_for_analysis", 50)
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"❌ Enhanced ML Tracker initialization failed: {e}")
            return False
    ...
    async def _initialize_storage(self) -> None:
        ...
        try:
            ...
            storage_backend = self.config.get("monitoring", {}).get("storage_backend", "sqlite")
            ...
            self.logger.info(f"ML tracker storage backend '{storage_backend}' initialized")
            
        except Exception as e:
            self.logger.error(f"Failed to initialize ML tracker storage: {e}")
            raise
    ...
    async def _create_ml_tracking_tables(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to create ML tracking tables: {e}")
            raise
    ...
    async def _load_historical_data(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to load historical ML data: {e}")
    ...
    async def _initialize_performance_analysis(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to initialize performance analysis: {e}")
            raise
        ...
        **kwargs
        ...
        try:
            ...
            record = ModelPredictionRecord(
                ...
                prediction_type=kwargs.get("prediction_type", PredictionType.REGRESSION),
            ...
            self.logger.debug(f"Tracked prediction {prediction_id} for model {model_id}")
            
            return prediction_id
            
        except Exception as e:
            self.logger.error(f"Failed to track model prediction: {e}")
            return ""
    ...
    async def _store_prediction_record(self, record: ModelPredictionRecord) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to store prediction record: {e}")
            raise
        ...
        try:
            ...
            if len(predictions) > 2:
                ...
                for pred_record in individual_predictions:
                    if pred_record.prediction < lower_bound or pred_record.prediction > upper_bound:
            ...
            self.logger.error(f"Failed to track ensemble performance: {e}")
            return ""
    ...
    async def _store_ensemble_record(self, record: EnsemblePerformanceRecord) -> None:
        ...
        try:
            data = {
                ...
                "individual_predictions_json": json.dumps([asdict(p, default=str) for p in record.individual_predictions]),
            ...
            self.logger.error(f"Failed to store ensemble record: {e}")
            raise
    ...
    async def record_actual_outcome(
        ...
        try:
            ...
            if prediction_id in self.prediction_records:
                ...
                self.logger.warning(f"Prediction {prediction_id} not found for outcome recording")
                return False
            
        except Exception as e:
            self.logger.error(f"Failed to record actual outcome: {e}")
            return False
    ...
    async def _update_prediction_outcome(self, record: ModelPredictionRecord) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to update prediction outcome in database: {e}")
            raise
    ...
    async def generate_model_performance_analysis(
        ...
        try:
            ...
            if len(model_predictions) < self.min_predictions_for_analysis:
                self.logger.warning(f"Insufficient predictions for model {model_id} analysis")
                return None
            ...
            # Directional accuracy
            directional_correct = sum([1 for p in model_predictions if p.directional_accuracy])
            ...
            up_prediction_accuracy = (
                sum([1 for p in up_predictions if p.directional_accuracy]) / len(up_predictions)
                if up_predictions else 0.0
            )
            down_prediction_accuracy = (
                sum([1 for p in down_predictions if p.directional_accuracy]) / len(down_predictions)
                if down_predictions else 0.0
            )
            
            # Confidence calibration
            confidence_calibration = await self._calculate_confidence_calibration(model_predictions)
            
            # Feature stability
            feature_stability_score = await self._calculate_feature_stability(model_predictions)
            
            # Performance trend
            performance_trend, recent_change = await self._analyze_performance_trend(model_predictions)
            ...
            analysis = ModelPerformanceAnalysis(
                ...
                overconfidence_score=max(0, confidence_calibration - directional_accuracy),
                underconfidence_score=max(0, directional_accuracy - confidence_calibration),
                feature_stability_score=feature_stability_score,
                most_important_features=await self._get_most_important_features(model_predictions),
            ...
            self.logger.error(f"Failed to generate performance analysis for model {model_id}: {e}")
            return None
    ...
    async def _calculate_confidence_calibration(
        ...
        try:
            ...
            self.logger.error(f"Failed to calculate confidence calibration: {e}")
            return 0.5
        ...
        try:
            ...
            for pred in predictions:
                if pred.feature_importance:
                    ...
                    ranking = {feat: idx for idx, (feat, _) in enumerate(sorted_features)}
            ...
            for feature in all_features:
                ranks = [ranking.get(feature, len(all_features)) for ranking in feature_rankings]
    ...
    async def _analyze_performance_trend(
        ...
        try:
            ...
            for i in range(window_size, len(sorted_preds)):
                ...
                accuracy = sum([1 for p in window if p.directional_accuracy]) / len(window)
    ...
    async def _get_most_important_features(
        ...
        try:
            ...
            self.logger.error(f"Failed to get most important features: {e}")
            return []

    async def _store_performance_analysis(self, analysis: ModelPerformanceAnalysis) -> None:
    ...
    async def _periodic_performance_analysis(self) -> None:
        ...
        try:
            while True:
                ...
                active_models = set(record.model_id for record in self.prediction_records.values())
            ...
            self.logger.error(f"Error in periodic performance analysis: {e}")
    ...
    async def _periodic_model_comparison(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Error in periodic model comparison: {e}")
    ...
    async def generate_model_comparison_report(
        ...
        try:
            ...
            if len(qualified_models) < 2:
                self.logger.warning("Insufficient models for comparison")
                return None
            ...
            report = ModelComparisonReport(
                ...
                performance_ranking=[(model, metrics["overall_score"]) for model, metrics in performance_ranking],
                stability_ranking=[(model, metrics["stability_score"]) for model, metrics in stability_ranking],
                efficiency_ranking=[(model, metrics["efficiency_score"]) for model, metrics in model_metrics.items()],
                best_accuracy=best_accuracy,
                best_precision=(best_accuracy[0], best_accuracy[1]["directional_accuracy"]),  # Simplified
                best_recall=(best_accuracy[0], best_accuracy[1]["directional_accuracy"]),
                ...
                regime_generalists=[],
                model_recommendations=await self._generate_model_recommendations(model_metrics),
                ensemble_recommendations=[],
                retraining_recommendations=await self._generate_retraining_recommendations(model_metrics)
            )
            ...
            self.logger.error(f"Failed to generate model comparison report: {e}")
            return None
    ...
    async def _calculate_comparison_metrics(
        ...
        try:
            ...
            directional_correct = sum([1 for p in predictions if p.directional_accuracy])
            ...
            metrics = {
                ...
                "efficiency_score": directional_correct / len(predictions),  # Simplified
        ...
        try:
            ...
            for model_id, metrics in best_models:
                if metrics["overall_score"] > 0.7:
                    recommendations.append(f"Increase allocation to {model_id} (score: {metrics['overall_score']:.2f})")
            ...
            for model_id, metrics in worst_models:
                if metrics["overall_score"] < 0.4:
                    recommendations.append(f"Consider reducing allocation to {model_id} (score: {metrics['overall_score']:.2f})")
            
            return recommendations
            
        except Exception as e:
            self.logger.error(f"Failed to generate model recommendations: {e}")
            return []
        ...
        try:
            ...
            for model_id, metrics in model_metrics.items():
                if metrics["directional_accuracy"] < 0.5:
                    ...
                    recommendations.append(f"Retrain {model_id} - high prediction variance")
            
            return recommendations
            
        except Exception as e:
            self.logger.error(f"Failed to generate retraining recommendations: {e}")
            return []
    ...
    async def _store_comparison_report(self, report: ModelComparisonReport) -> None:
        ...
        try:
            data = {
                ...
                "best_model": report.performance_ranking[0][0] if report.performance_ranking else "",
            ...
            self.logger.error(f"Failed to store comparison report: {e}")
            raise
    ...
    async def get_tracking_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            stats.update({
                ...
                "cache_size": len(self.model_performance_cache) + len(self.ensemble_performance_cache),
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to cleanup Enhanced ML Tracker: {e}")
...
async def setup_enhanced_ml_tracker(config: dict[str, Any]) -> EnhancedMLTracker | None:
    ...
    try:
        ...
        system_logger.error(f"Failed to setup Enhanced ML Tracker: {e}")
        return None