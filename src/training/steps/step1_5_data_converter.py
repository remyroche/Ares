...
enhanced_decorators = PipelineStandards.safe_import(
    "src.utils.enhanced_data_quality_decorators", None
)
...
if centralized_decorators is None:
    ...
    secure_klines_download_operation = (
        centralized_decorators.secure_klines_download_operation
    )
...
if enhanced_decorators is None:
    ...
    validate_memory_optimized_data_quality = (
        enhanced_decorators.validate_memory_optimized_data_quality
    )
...
class TimingTracker:
    ...
    def end_phase(self, phase_name: str) -> None:
        if phase_name in self.checkpoints and "end" not in self.checkpoints[phase_name]:
            ...
            print(
                f"⏱️  [TIMING] Phase '{phase_name}' completed in {duration:.2f} seconds"
            )
...
class UnifiedDataConverter:
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=False, context="unified data conversion"
    )
    async def execute(
        self,
        symbol: str,
        ...
        try:
            ...
            if unified_exists:
                if force_rerun:
                    ...
                    inc_ok = await self._process_incremental_updates(
                        symbol, exchange, timeframe
                    )
                    if inc_ok:
            ...
            infra_ok = await self._setup_future_infrastructure(
                symbol, exchange, timeframe
            )
            if not infra_ok:
            ...
            verify_ok = await self._verify_unified_data_quality(
                symbol, exchange, timeframe
            )
            if not verify_ok:
            ...
            try:
                ...
                self.logger.info(
                    "🔍 Running comprehensive Step1.5 data quality validation..."
                )
                ...
                if validation_result["validation_passed"]:
                    self.logger.info(
                        "✅ Comprehensive Step1.5 data quality validation passed"
                    )
                else:
                    self.logger.warning(
                        f"⚠️ Comprehensive Step1.5 data quality validation found {len(validation_result['issues'])} issues:"
                    )
                    for issue in validation_result["issues"][:5]:  # Show first 5 issues
                        self.logger.warning(f"   - {issue}")
                    if len(validation_result["issues"]) > 5:
                        self.logger.warning(
                            f"   ... and {len(validation_result['issues']) - 5} more issues"
                        )
                ...
                self.logger.warning(
                    f"⚠️ Comprehensive Step1.5 data quality validation failed: {e} - continuing anyway"
                )
    ...
    async def _run_enhanced_quality_validation(
    ...
    async def _process_incremental_updates(
        ...
        try:
            ...
            klines_dates: set[date] = set(
                map(
                    date.fromordinal,
                    map(lambda d: d.toordinal(), klines_data["date"].unique()),
                )
            )
            ...
            return await self._process_data_incrementally(
        ...
        try:
            ...
            while current_date <= max_date:
                try:
                    ...
                    unified = await self._merge_daily_data(
                        daily_klines,
                        daily_aggtrades,
                        daily_futures,
                        symbol,
                    ...
                    if unified is not None and not unified.empty:
                        success = await self._write_daily_partition(
                            unified, symbol, exchange, timeframe, current_date, base_dir
                        )
                        if success:
                            total_rows_processed += len(unified)
                            self.logger.info(
                                f"   ✅ Processed {len(unified)} kline rows for {current_date}"
                            )
                        else:
                            self.logger.error(
    ...
    async def _load_aggtrades_for_date(
        self, symbol: str, exchange: str, target_date: date
        ...
        try:
            ...
            if dfs:
                ...
                self.logger.info(
                    f"✅ Loaded {len(combined)} aggtrades rows for {target_date_str}"
                )
                return combined
            return None
        except Exception as e:
    ...
    async def _load_futures_for_date(
        ...
        try:
            ...
            if dfs:
                ...
                combined = combined.sort_values("timestamp").reset_index(drop=True)
                self.logger.info(
                    f"✅ Loaded {len(combined)} futures rows for {target_date_str}"
                )
                return combined
            return None
        except Exception as e:
    ...
    async def _merge_daily_aggtrades(
        ...
        try:
            ...
            agg["kline_timestamp"] = pd.to_datetime(
                agg["timestamp"], unit="ms", utc=True
            )
            agg["kline_timestamp"] = (
                agg["kline_timestamp"].dt.floor("1min").astype("int64") // 10**6
            )
            agg_stats = (
                agg.groupby("kline_timestamp")
                .agg(
                    {
                        "quantity": ["sum", "count"],
                        "price": ["mean", "min", "max"],
    ...
    async def _merge_daily_futures(
        ...
        try:
            ...
            if pd.api.types.is_datetime64_any_dtype(df["timestamp"]):
                df["timestamp"] = (df["timestamp"].astype(np.int64) // 10**6).astype(
                    "int64"
                )
    ...
    async def _verify_unified_data_quality(
        ...
        try:
            ...
            if quality_issues:
                ...
                return False
            self.logger.info(
                "✅ Data quality verification passed - all data types present"
            )
            return True
        except Exception as e:
    ...
    async def _load_klines_data(
        ...
        try:
            ...
            parquet_file = self.standards.generate_file_name(
            ...
            if os.path.exists(parquet_path):
                ...
                if validation_result.passed:
                    self.logger.info(
                        f"   ✅ Loaded {len(df)} klines rows (quality score: {validation_result.quality_score:.2f})"
                    )
                else:
                    self.logger.warning(
                        f"   ⚠️ Loaded {len(df)} klines rows but validation found issues"
                    )
    ...
    async def _download_klines_data(
        ...
        try:
            ...
            pattern = os.path.join(
                self.data_cache_dir, f"klines_{exchange}_{symbol}_{timeframe}_*.csv"
            )
            klines_files = sorted(glob.glob(pattern))

            if not klines_files:
                self.logger.warning(
                    f"⚠️ No klines files found after download: {pattern}"
                )
                return None
            ...
            for fp in klines_files:
                try:
                    ...
                    if not df.empty:
                        frames.append(df)
                    self.logger.debug(
                        f"📊 Loaded {len(df)} rows from {os.path.basename(fp)}"
                    )
                except Exception as e:
            ...
            if validation_result.passed:
                self.logger.info(
                    f"✅ Downloaded data validation passed (quality score: {validation_result.quality_score:.2f})"
                )
            else:
            ...
            out_file = self.standards.generate_file_name(
                "klines", exchange, symbol, timeframe
            )
    ...
    async def _verify_and_calculate_missing_columns(
        self, unified: pd.DataFrame, symbol: str, exchange: str, timeframe: str
    ) -> pd.DataFrame:
        ...
        try:
            self.logger.info(
                "🔍 Step 1.5 Enhancement: Verifying and calculating missing columns..."
            )
            ...
            missing_info = column_verifier.verify_missing_columns(
                unified, data_type="unified"
            )

            # Log verification results
            if missing_info["verification_passed"]:
                self.logger.info(
                    "✅ Column verification passed - all required columns present"
                )
            else:
                self.logger.warning(
                    f"⚠️ Column verification found missing required columns: {missing_info['missing_required']}"
                )
            ...
            for category, missing_optional in missing_info["missing_optional"].items():
                if missing_optional:
                    ...
                    self.logger.info(
                        f"📊 {category}: {len(missing_optional)} missing, {len(can_calculate)} can be calculated"
                    )

            # Calculate missing columns if any can be calculated
            has_calculable = any(
                len(can_calc) > 0 for can_calc in missing_info["can_calculate"].values()
            )

            if has_calculable:
                self.logger.info("🔄 Calculating missing columns...")
                enhanced_unified = column_verifier.calculate_missing_columns(
                    unified, missing_info
                )
                ...
                if new_columns:
                    self.logger.info(
                        f"✅ Successfully calculated {len(new_columns)} new columns: {list(new_columns)}"
                    )
            ...
            self.logger.exception(
                f"❌ Error during column verification and calculation: {e}"
            )
            self.logger.warning(
                "⚠️ Continuing with original data without column enhancements"
            )
            return unified
...
# ----------------------------------------------------------------------------
@handle_errors(
    exceptions=(Exception,), default_return=False, context="step1_5_data_converter"
)
@secure_data_processing
@prevent_data_leakage
@resource_monitor
...
async def run_step(
    ...
    try:
        ...
        print("=" * 80)
        print(
            "🎉 STEP 1.5: UNIFIED DATA CONVERTER - COMPLETED SUCCESSFULLY"
            if success
            else "💥 STEP 1.5: UNIFIED DATA CONVERTER - FAILED"
        )
        print("=" * 80 + "\n")
        return success
    except Exception as e:
...
if __name__ == "__main__":
    ...
    async def _main() -> None:
        ...
        print(
            "✅ Step 1.5: Data Converter completed successfully"
            if ok
            else "❌ Step 1.5: Data Converter failed"
        )
        import gc
