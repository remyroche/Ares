    ...
    async def wait_if_needed(self, request_weight=1, endpoint=None):
        ...
        if endpoint and endpoint in self.endpoint_limits:
            ...
            if limit_info["request_count"] >= limit_info["requests_per_minute"]:
                ...
                if wait_time > 0:
                    logger.info(f"Endpoint rate limit hit for {endpoint}, waiting {wait_time:.1f} seconds...")
        ...
        if self.current_weight + request_weight > self.weight_limit * 0.9:  # Increased to 90%
...
class BinanceExchange:
    ...
    @handle_network_operations(
        max_retries=5, default_return=None, context="binance_request"
    )
    async def _request(
        self,
        method: str,
        ...
        for attempt in range(max_retries):
            try:
                ...
                async with session.request(
                    method.upper(), endpoint, params=params, headers=headers, timeout=10
                ) as response:
                    ...
                    if e.status == 429:  # Rate limit
                        ...
                        if retry_after:
                            ...
                            try:
                                ...
                                if 'retryAfter' in response_data:
                                    retry_after = int(response_data['retryAfter'] / 1000)  # Convert from ms to seconds
                                else:
    ...
    # --- REST API Functions ---
    @retry_on_rate_limit()
    @handle_network_operations(max_retries=3, default_return=[], context="get_klines")
    async def get_klines(
        self, symbol: str, interval: str, limit: int = 500
    ) -> List[Dict[str, Any]]:
        """Get kline/candlestick data for a symbol."""
        logger.info(f"🔧 Starting get_klines request:")
        ...
        try:
            ...
            request_duration = time.time() - start_time
            logger.error(f"💥 Error in get_klines after {request_duration:.2f} seconds: {e}")
    ...
    @retry_on_rate_limit()
    @handle_network_operations(max_retries=3, default_return=[], context="get_historical_klines")
    async def get_historical_klines(
        self, symbol: str, interval: str, start_time_ms: int, end_time_ms: int, limit: int = 1000
    ) -> List[Dict[str, Any]]:
        """Get historical kline/candlestick data for a symbol within a time range with pagination."""
        logger.info(f"🔧 Starting get_historical_klines request:")
        logger.info(f"   Symbol: {symbol}")
        logger.info(f"   Interval: {interval}")
        ...
        try:
            while current_start_time < end_time_ms:
                ...
                logger.info(f"✅ Received {len(response)} klines (total: {len(all_klines)})")
                ...
                if len(response) < limit:
                    logger.info(f"Received {len(response)} klines (less than {limit}), stopping")
                    break
            ...
            request_duration = time.time() - start_time
            logger.info(f"⏱️  All API requests completed in {request_duration:.2f} seconds")
            logger.info(f"✅ Successfully received {len(all_klines)} total historical klines")
            return all_klines
                
        except Exception as e:
            request_duration = time.time() - start_time
            logger.error(f"💥 Error in get_historical_klines after {request_duration:.2f} seconds: {e}")
            logger.error(f"   Error type: {type(e).__name__}")
            logger.error("Full traceback:")
            logger.error(traceback.format_exc())
            raise
    ...
    @handle_network_operations(
        max_retries=3, default_return=[], context="get_position_risk"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return=[], context="get_open_orders"
    )
    ...
    @retry_on_rate_limit()
    @handle_errors(
        exceptions=(Exception,), default_return=None, context="close_all_positions"
    )
    ...
    @retry_on_rate_limit()
    @handle_errors(
        exceptions=(Exception,), default_return=None, context="cancel_all_orders"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return=[], context="get_historical_agg_trades"
    )
    async def get_historical_agg_trades(
        ...
        while current_start_time < end_time_ms and iteration_count < max_iterations:
            ...
            try:
                ...
                if not trades:
                    system_logger.info(f"No more trades found for {symbol} at time {current_start_time}")
                    break
                ...
                if len(trades) < limit:
                    system_logger.info(f"Reached end of trade data for {symbol}, got {len(trades)} trades")
                    break
        ...
        if iteration_count >= max_iterations:
            system_logger.warning(f"Reached max iterations ({max_iterations}) for {symbol} agg trades")
    ...
    async def get_historical_agg_trades_ccxt(
        ...
        try:
            ...
            exchange = ccxt.binance({
                ...
                'secret': self._api_secret.decode('utf-8') if self._api_secret else None,
            ...
            system_logger.info(f"CCXT: Fetching agg trades for {symbol} from {start_time} to {end_time}")
            ...
            if trades:
                system_logger.info(f"CCXT Debug: First trade timestamp: {trades[0].get('T')}")
                system_logger.info(f"CCXT Debug: Last trade timestamp: {trades[-1].get('T')}")
                system_logger.info(f"CCXT Debug: Number of unique timestamps: {len(set(t.get('T') for t in trades))}")
            ...
            system_logger.info(f"CCXT: Retrieved {len(formatted_trades)} aggregated trades for {symbol}")
            ...
            system_logger.error(f"CCXT Error fetching historical agg trades for {symbol}: {e}")
            return []
    ...
    async def get_historical_futures_data(
        ...
        while current_fr_start < end_time_ms:
            ...
            try:
                ...
                rates = await self._request("GET", fr_endpoint, params, request_weight=1)
                if not rates:
                    break
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return={}, context="get_exchange_info"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(max_retries=3, default_return={}, context="get_ticker")
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return={}, context="get_funding_rate"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return={}, context="get_open_interest"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return=[], context="get_all_orders"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return=[], context="get_trade_history"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return={}, context="get_income_history"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return={}, context="get_commission_rate"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return=[], context="get_adl_quantile"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return={}, context="get_force_orders"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return={}, context="get_position_side_dual"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return={}, context="change_position_side_dual"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return={}, context="get_multi_assets_margin"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return={}, context="change_multi_assets_margin"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return=0.0, context="get_account_balance"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return=0.0, context="get_spot_balance"
    )
    ...
    @retry_on_rate_limit()
    @handle_network_operations(
        max_retries=3, default_return=False, context="transfer_to_spot"
    )
    ...
    # --- WebSocket Handlers ---
    @handle_errors(
        exceptions=(Exception,), default_return=None, context="websocket_handler"
    )
    ...
    async def _process_depth_message(self, data: Dict[str, Any]):
        ...
        try:
            ...
            system_logger.error(
                f"Error processing depth message: {e}. Data: {data}", exc_info=True
            )
    ...
    async def _process_trade_message(self, data: Dict[str, Any]):
        ...
        try:
            ...
            system_logger.error(
                f"Error processing trade message: {e}. Data: {data}", exc_info=True
            )
    ...
    async def _process_kline_message(self, data: Dict[str, Any]):
        ...
        try:
            ...
            system_logger.error(
                f"Error processing kline message: {e}. Data: {data}", exc_info=True
            )

    @handle_errors(
        exceptions=(Exception,), default_return=None, context="start_kline_socket"
    )
    async def start_kline_socket(self, symbol: str, interval: str):
        ...
        await self._websocket_handler(
            url, self._process_kline_message, f"Kline ({symbol})"
        )

    @handle_errors(
        exceptions=(Exception,), default_return=None, context="start_depth_socket"
    )
    async def start_depth_socket(self, symbol: str):
        ...
        url = f"{self.WS_BASE_URL}/ws/{stream_name}"
        await self._websocket_handler(
            url, self._process_depth_message, f"Depth ({symbol})"
        )

    @handle_errors(
        exceptions=(Exception,), default_return=None, context="start_trade_socket"
    )
    async def start_trade_socket(self, symbol: str):
        ...
        url = f"{self.WS_BASE_URL}/ws/{stream_name}"
        await self._websocket_handler(
            url, self._process_trade_message, f"Trade ({symbol})"
        )

    @handle_errors(
        exceptions=(Exception,), default_return=None, context="start_user_data_socket"
    )
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=None, context="start_mark_price_socket"
    )
    async def start_mark_price_socket(self, symbol: str):
        ...
        url = f"{self.WS_BASE_URL}/ws/{stream_name}"
        await self._websocket_handler(
            url, self._process_mark_price_message, f"Mark Price ({symbol})"
        )
    ...
    async def start_funding_rate_socket(self, symbol: str):
        ...
        url = f"{self.WS_BASE_URL}/ws/{stream_name}"
        await self._websocket_handler(
            url, self._process_funding_rate_message, f"Funding Rate ({symbol})"
        )
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=None, context="close_session"
    )
