...
class RegimeDataSplittingStep:
    ...
    @handle_errors(
        ...
        default_return={"success": False, "error": "Execution failed"},
        context="step8_execution",
    )
    async def execute(self) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"✅ Successfully created unified dataset with {
                    len(unique_clusters)} HMM composite regime labels"
            )
    ...
    async def _log_step8_artifacts_and_report(self, unified_data: pd.DataFrame, summary: dict[str, Any]) -> None:
