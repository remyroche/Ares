...
class EnhancedHMMBasedTrainingStep:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.profit_feature_engine = ProfitBasedFeatureEngineering(
            profit_column="potential_profit_pct", use_numba=True, memory_efficient=True
        )
    ...
    async def _load_regime_specific_data(self, symbol: str, data_dir: str, regime: str) -> pd.DataFrame:
    ...
    async def _engineer_regime_features(self, regime_data: pd.DataFrame, regime: str) -> pd.DataFrame:
        ...
        try:
            ...
            if self.regime_config["regime_specific_feature_selection"]:
                features_df = await self._apply_regime_specific_feature_selection(features_df, regime)
    ...
    async def _optimize_regime_hyperparameters(self, regime_features: pd.DataFrame, regime: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.regime_config["regime_specific_hyperparameters"]:
                ...
                optimized_params = await self._optimize_params_for_regime(regime_features, regime_params, regime)
    ...
    async def _optimize_params_for_regime(
        ...
        try:
            ...
            return {
                "learning_rate": float(
                    regime_params.get("learning_rate", [0.05])[
                        0 if len(regime_params.get("learning_rate", [])) == 1 else 1
                    ]
                ),
                "max_depth": int(
                    regime_params.get("max_depth", [6])[0 if len(regime_params.get("max_depth", [])) == 1 else 1]
                ),
                "n_estimators": int(
                    regime_params.get("n_estimators", [200])[
                        0 if len(regime_params.get("n_estimators", [])) == 1 else 1
                    ]
                ),
                "regime": str(regime),
            }
    ...
    async def _validate_regime_model(
        ...
        try:
            ...
            if self.regime_config["regime_specific_validation"]:
                ...
                validation_results = await self._perform_default_validation(regime_model, regime_features)
    ...
    @handle_errors(
        exceptions=(ValueError, TypeError, MemoryError), default_return=None, context="enhanced_data_preparation"
    )
    async def prepare_enhanced_data(
        self, data: pd.DataFrame, timeframe: str, regime_key: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if has_profit and self.enable_multi_output:
            try:
                ...
                self.logger.info(
                    f"✅ Enhanced feature selection completed: {
                        selected_features.shape[1]} features selected"
                )
                self.logger.info(
                    f"   - Autoencoder features: {
                        metadata.get(
                            'stages',
                            {}).get(
                            'stage0_autoencoder',
                            {}).get(
                            'autoencoder_features_added',
                            0)}"
                )
                self.logger.info(
                    f"   - Regularization applied: {
                        metadata.get(
                            'stages',
                            {}).get(
                            'stage6_regularization',
                            {}).get(
    ...
    async def train_enhanced_model(
        self, prepared_data: Dict[str, Any], model_name: str = "enhanced_model"
    ) -> Dict[str, Any]:
        ...
        if prepared_data["has_multi_output"] and self.multi_output_trainer:
            ...
            y = (
                prepared_data["single_target"].values
                if "single_target" in prepared_data
                else np.random.choice([0, 1], size=len(X))
            )
    ...
    async def _train_single_output_model(
        self, features: pd.DataFrame, target: pd.Series, timeframe: str, regime_key: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                "feature_importance": (
                    dict(zip(features.columns, final_model.feature_importances_))
    ...
    async def train_enhanced_regime_specific_models(
        ...
        for regime_key, regime_info in regime_data.items():
            ...
            prepared_data = self.prepare_enhanced_data(all_regime_data, timeframe, regime_key)

            # Train enhanced model
            model_result = await self.train_enhanced_model(prepared_data, f"enhanced_{timeframe}_{regime_key}")
    ...
    def save_enhanced_models(self, results: Dict[str, Any], save_path: str) -> None:
    ...
    def load_enhanced_models(self, model_name: str, load_path: str) -> None:
    ...
    async def _apply_regime_specific_feature_selection(self, features_df: pd.DataFrame, regime: str) -> pd.DataFrame:
...
async def run_enhanced_step(
    ...
    try:
        ...
        # Load configuration
        config = {"enable_multi_output": enable_multi_output, "HMM_LM": method_a_mixture_of_experts or {}}
        ...
        results = await enhanced_trainer.train_enhanced_model(prepared_data, f"enhanced_{symbol}_1m")
