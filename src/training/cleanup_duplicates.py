...
def identify_backup_files(directory: Path) -> List[Path]:
    ...
    backup_patterns = ['*backup*', '*_backup*', '*_old*', '*_deprecated*', '*_temp*', '*_tmp*', '*_copy*', '*_original*']
    backup_files = []
...
def generate_cleanup_report(training_dir: Path) -> Dict[str, any]:
    ...
    report = {'duplicate_files': duplicate_files_str, 'backup_files': [str(f) for f in identify_backup_files(training_dir)], 'enhanced_versions': {k: [str(f) for f in v] for k, v in identify_enhanced_versions(training_dir).items()}, 'large_files': [(str(f), lines) for f, lines in analyze_file_sizes(training_dir)]}
    return report
...
def main() -> None:
    ...
    if report['large_files']:
        ...
        print('-' * 60)
        for f, lines in report['large_files'][:10]:
