...
def _sanitize_quality_array(values: np.ndarray, *, fallback: float = 0.5) -> np.ndarray:
    ...
    if np.any(bad):
        tprint(
            f"WARNING: quality contains {int(bad.sum())} non-finite values; replacing with fallback={float(fallback):.3f}"
        )
    arr = np.nan_to_num(
        arr, nan=float(fallback), posinf=float(fallback), neginf=float(fallback)
    )
...
def _resolve_barrier_conflict_15m(
    *,
    side: str,
    bar_open: float,
    tp_price: float,
    ...
    hits: Dict[str, float] = {
        "TP": abs(bar_open - tp_price),
        "SL": abs(bar_open - sl_price),
    }
...
def _refine_ambiguous_labels_with_intrabar(
    ...
    if symbols_needing_intrabar:
        ...
        def _preload_symbol(sym: str):
            ...
            return sym, load_or_download_fn(
                sym, start_ts, end_ts, allow_download=allow_download
            )

        preload_results = Parallel(
            n_jobs=min(n_jobs, len(symbols_needing_intrabar)), prefer="threads"
    ...
    def _process_symbol(sym: str):
        ...
        end_idx = np.searchsorted(idx_ns, idx_ns + horizon_ns, side="right").astype(
            np.int64, copy=False
        )
        ...
        end_full = min(
            idx[ambiguous_j_list[-1][0]] + pd.Timedelta(hours=float(h)), idx[-1]
        )

        if sym in preloaded:
            hf_full = preloaded[sym]
            hf_full = hf_full.loc[
                (hf_full.index >= start_full) & (hf_full.index <= end_full)
            ]
        else:
            hf_full = load_or_download_fn(
                sym, start_full, end_full, allow_download=allow_download
            )
        ...
        for (
            ...
            if any_hit.size:
                ...
                if bool(hit_tp_arr[k0] and hit_sl_arr[k0]):
                    d_tp = (
                        abs(float(c15[k0]) - float(h15[k0]))
                        if side == "long"
                        else abs(float(c15[k0]) - float(l15[k0]))
                    )
                    d_sl = (
                        abs(float(c15[k0]) - float(l15[k0]))
                        if side == "long"
                        else abs(float(c15[k0]) - float(h15[k0]))
                    )
...
def _load_panel_from_store(
    cfg: Dict[str, Any]
) -> Tuple[Optional[Dict[str, pd.DataFrame]], List[str]]:
    ...
    try:
        ...
        market_basket = apply_hardcoded_universe_exclusions(
            list(cfg.get("market_basket", []))
        )
        all_syms = apply_hardcoded_universe_exclusions(
            list(set(margin_symbols + market_basket))
        )
        ...
        if os.path.exists(ohlcv_dir):
            for name in os.listdir(ohlcv_dir):
                ...
                sym = (
                    ...
                    else (name if os.path.isdir(full) else None)
                )
            ...
            if ohlcv_syms:
                ...
                tprint(
                    f"Augmented symbol universe from local ohlcv (USDT-only): +{len(set(ohlcv_syms))} symbols"
                )
        all_syms = apply_hardcoded_universe_exclusions(
        ...
        # Cap: honour explicit override; otherwise use 1/3 of the live margin universe size
        ...
        for i, sym in enumerate(train_syms, 1):
            ...
            _last_gc_ts = _maybe_collect_gc(
                last_gc_ts=_last_gc_ts, mem_threshold_mb=6144.0, min_interval_s=8.0
            )
        if not dfs:
            tprint(
                f"Panel store load returned no usable symbols. store.ohlcv_dir={store.ohlcv_dir}"
            )
            return None, []
        panel = to_panel(dfs)
        ...
        if n_total > MAX_ROWS:
            tprint(
                f"Capping TBM panel at {MAX_ROWS} rows (random entry masking for different parts of history & symbols)..."
            )
            ...
            if len(rows) > MAX_ROWS:
                ...
                keep_df = pd.DataFrame(
                    keep_arr, index=close_frame.index, columns=close_frame.columns
                )
                for k in panel:
...
def _build_tbm_learnability_report_rows(
    ...
    for idx, config_id in enumerate(config_ids):
        ...
        bucket_metrics = (
            detail.get("bucket_metrics", {}) if isinstance(detail, dict) else {}
        )
        regime_metrics = (
            detail.get("regime_metrics", {}) if isinstance(detail, dict) else {}
        )
        vol_metrics = (
            detail.get("vol_quintile_metrics", {}) if isinstance(detail, dict) else {}
        )
        bucket_h_metrics = (
            detail.get("bucket_horizon_metrics", {}) if isinstance(detail, dict) else {}
        )
        ...
        if bucket_rows:
            ic_payoff_vals = np.array(
                [_safe_float(v.get("ic_payoff"), np.nan) for v in bucket_rows],
                dtype=float,
            )
            ic_label_vals = np.array(
                [_safe_float(v.get("ic_label"), np.nan) for v in bucket_rows],
                dtype=float,
            )
            n_vals = np.array(
                [_safe_float(v.get("n"), np.nan) for v in bucket_rows], dtype=float
            )
...
def _estimate_tbm_cache_size_bytes(
    layer1_cache: Dict[str, Any], layer2_cache: Dict[str, Any]
) -> int:
    total = 0
    ...
    try:
        ...
        for item in entries:
            ...
            if "quality_file" in item and item["quality_file"]:
                ...
                qual = pd.DataFrame(
                    0.5, index=lbl.index, columns=lbl.columns, dtype=np.float32
                )
...
# IO helpers
# ---------------------------
def _read_symbol_parquet_file(
    f: Path, columns: Optional[List[str]] = None
...
def load_panel(panel_path: Path) -> Dict[str, pd.DataFrame]:
    ...
    if panel_path.is_dir():
        ...
        if not has_ohlcv:
            raise ValueError(
                f"Panel directory {panel_path} does not contain OHLCV columns"
            )
        return to_panel(dfs)
    ...
    if {"timestamp", "symbol", "open", "high", "low", "close", "volume"}.issubset(
        ...
        piv = {}
        for k in ["open", "high", "low", "close", "volume"]:
            piv[k] = panel_df.pivot(
                index="timestamp", columns="symbol", values=k
            ).sort_index()
        return piv
...
def _get_barrier_atr_frame(artifacts: RunArtifacts) -> pd.DataFrame:
    ...
    if "atr_pct_raw" in feats and isinstance(feats["atr_pct_raw"], pd.DataFrame):
        ...
        "atr_pct" in feats
        and isinstance(feats["atr_pct"], pd.DataFrame)
        and not _is_standardized_like_atr(feats["atr_pct"])
    ):
        src = feats["atr_pct"]
    else:
        src = _compute_panel_atr_pct(artifacts.panel)
        tprint(
            "[atr_source] Using panel-derived ATR% for barriers (feature atr_pct looks normalized or missing)."
        )
...
def align_artifacts(
    ...
    if "atr_pct" not in f_out:
        ...
        tprint(
            "[align_artifacts] WARNING atr_pct appears normalized (contains negatives); barriers will use atr_pct_raw."
        )
...
def _derive_sl_from_tp(
    tp: pd.DataFrame, atr: pd.DataFrame, cfg: Dict[str, Any]
) -> pd.DataFrame:
    """Fast SL derivation from cached TP arrays for sl_method=tp_pct."""
    sl = float(cfg.get("sl_as_tp_pct", 0.5)) * tp
    sl = _apply_caps(
        sl,
        float(cfg.get("sl_abs_lo_pct", 0.005)),
        float(cfg.get("sl_abs_hi_pct", 0.08)),
    )
...
def _strict_walk_forward_probe_auc(
    ...
    try:
        events = pd.DataFrame(
            {
                "event_id": np.arange(len(y), dtype=np.int64),
                "symbol": np.repeat("ALL", len(y)),
                "t0": pd.to_datetime(ts, utc=True, errors="coerce"),
                "t1": pd.to_datetime(ts, utc=True, errors="coerce")
        ...
        cand = [
            p
            for p in bundle["consumer_plans"]["barrier_search"]
            if p.fit_idx.size > 0 and p.predict_idx.size > 0
        ]
        if not cand:
...
def _regime_multiplier(
    atr_pct: pd.DataFrame, model: str, mix_weight: float = 0.5
) -> pd.DataFrame:
    ...
    shock = (
        atr_pct.pct_change(fill_method=None)
        .abs()
...
def build_barriers(
    ...
    tp_regime = _regime_multiplier(
        atr, cfg.get("tp_regime_model", "none"), cfg.get("mix_weight", 0.5)
    )
    sl_regime = _regime_multiplier(
        atr,
        cfg.get("sl_regime_model", cfg.get("tp_regime_model", "none")),
        cfg.get("mix_weight", 0.5),
    )
    ...
    if tp_method == "atr_mult":
        ...
        tp_raw = pd.DataFrame(
            float(cfg["tp_abs_pct"]), index=atr.index, columns=atr.columns
        )
        ...
        if med is None:
            ...
            series_cache[med_key] = med.astype(np.float32)
        atrn_tp = (
            cfg["k_tp"] * (atr / (med + EPS)) * float(cfg.get("tp_base_pct", 0.02))
        )
        abs_tp = pd.DataFrame(
            float(cfg.get("tp_abs_pct", cfg.get("tp_base_pct", 0.02))),
            index=atr.index,
    ...
    if sl_method == "tp_pct":
        ...
        sl = pd.DataFrame(
            float(cfg["sl_abs_pct"]), index=atr.index, columns=atr.columns
        )
    else:
    ...
    out_stats = {
        ...
        "bound_saturation": float(
            (
                ...
                | (tp.values >= float(cfg.get("tp_abs_hi_pct", 0.08) - 1e-9))
            ).mean()
        ),
    }
    if dyn_stats:
...
def make_quantile_basis(artifacts: RunArtifacts, basis: str) -> pd.DataFrame:
    ...
    for k in ["atr_pct", "trend_snr", "vol_z", "rsi"]:
        if k == "atr_pct":
            ...
            continue
        parts.append(
            (x - x.rolling(72, min_periods=8).median())
            / (x.rolling(72, min_periods=8).std() + EPS)
        )
    if parts:
...
def choose_feature_matrix(
    artifacts: RunArtifacts, max_features: int = 20
    ...
    if not selected:
        raise ValueError(
            "No configured test_feature_keys available in features for TBM comparison"
        )
...
def get_feature_matrix_cache(
    ...
    panel_flat_idx = pd.MultiIndex.from_product(
        [panel_idx, panel_cols], names=["ts", "symbol"]
    )
    global_aligner = (
        pd.Index(X_flat.index).get_indexer(panel_flat_idx).astype(np.int64, copy=False)
    )
...
def _build_equidistant_grid(
    ...
    *,
    ...
    if not spacing_valid:
        tprint(
            f"[grid_meta] spacing violation detected lo={lo_f:.6f} hi={hi_f:.6f} grid={out}"
        )
    return out, {
...
def _build_param_grid(lo: float, hi: float) -> Tuple[List[float], Dict[str, Any]]:
    ...
    if len(g) < 4:
        ...
        tprint(
            f"[grid_meta] relaxed grid due to low cardinality lo={lo:.6f} hi={hi:.6f} -> n={len(g)}"
        )
    return g, meta
...
def _build_sl_biased_grid(runtime_floor: float) -> Tuple[List[float], Dict[str, Any]]:
    ...
    p_mid = (
        np.linspace(mid_lo, mid_hi, 10, dtype=np.float64)
        if mid_hi > mid_lo + EPS
    ...
    if len(out) < 6 and sl_hi > sl_lo + EPS:
        out = sorted(
            set(
                round(float(v), 6)
                for v in np.linspace(sl_lo, sl_hi, 6, dtype=np.float64).tolist()
            )
        )
    if len(out) > 15:
        interior = out[1:-1]
...
def per_slice_metrics(
    events: pd.DataFrame, pred: np.ndarray, slice_col: str
...
def evaluate_config(
    ...
    if sl_ratio < sl_ratio_floor:
        return _empty_result(
            ...
            cfg_id,
            0,
            reason=f"cheap_gate_sl_as_tp_too_low:{sl_ratio:.3f}<{sl_ratio_floor:.3f}",
    ...
    if sl_ratio > 0 and (1.0 / sl_ratio) < (min_rr * 0.7):
        return _empty_result(
            ...
            cfg_id,
            0,
            reason=f"cheap_gate_rr_too_low:{1.0/sl_ratio:.2f}<{min_rr:.2f}",
            stage2_rescore=stage2_rescore,
        )
    ...
    _idx_flat = pd.MultiIndex.from_product(
        [_panel_idx, _panel_cols], names=["ts", "symbol"]
    )
        ...
        for side in ["long", "short"]:
            ...
            if str(cfg.get("sl_method", "tp_pct")) == "tp_pct":
                ...
                key1_tp = (
                    ...
                    _compact_cfg_signature(
                        tp_barrier_cfg, tuple(tp_barrier_cfg.keys())
                    ),
                )
                if key1_tp not in layer1_cache:
                    tp_df, _sl_df, geom_stats, dyn_h = build_barriers(
                        artifacts, cfg, h, side
                    )
                ...
                if atr_cached is None:
                    atr_cached = (
                        _get_barrier_atr_frame(artifacts)
                        .shift(1)
                        .clip(lower=1e-6)
                        .bfill(limit=1)
                        .astype(np.float32)
                ...
                if key1 not in layer1_cache:
                    tp_df, sl_df, geom_stats, dyn_h = build_barriers(
                        artifacts, cfg, h, side
                    )
            ...
            if key2 not in layer2_cache:
                ...
                if keep_global_list:
                    ...
                    total_tp = sum(
                        len(np.where(lbl_arr[np.where(m.ravel())[0]] == 2)[0])
                ...
                if panel_15m_for_labeling is not None:
                    ...
                    valid_15m_mask = np.isfinite(
                        _tp_15m.to_numpy(dtype=np.float32, copy=False)
                    )

                    amb_before_5m = int(
                        (
                            valid_15m_mask
                            & (lbl_15m.to_numpy(dtype=np.int8, copy=False) == OUT_SL)
                            & (qual_15m.to_numpy(dtype=np.float32, copy=False) < 0.5)
                        ).sum()
                    )
                    tprint(
            ...
            if len(df) > 0:
                ...
                if (
                    timeout_count > 1000
                ):  # If too many timeouts, keep only non-timeouts for now
    ...
    events["quality"] = _sanitize_quality_array(
        events["quality"].to_numpy(dtype=np.float32, copy=False)
    )
    ...
    for _i, _k in enumerate(_u_keys):
        ...
        _side, _h = _k.split("|")
        _funnel_rows.append(
            {"side": _side, "h": int(_h), "n_raw": _n_raw, "n_rr": _n_rr}
        )
    if _funnel_rows:
        _funnel_rows = sorted(_funnel_rows, key=lambda r: (str(r["side"]), int(r["h"])))
        _parts = [
            f"{r['side']}_H{r['h']}: {r['n_raw']:,}→{r['n_rr']:,}" for r in _funnel_rows
        ]
    ...
    if np.any(_cand_mask):
        ...
        local_bucket_map = {
            k: v[event_panel_idx][_cand_mask] for k, v in bucket_map.items()
        }
    else:
    ...
    tprint(
        f"[eval:{cfg_id}] candidate_prefilter_kept={len(events):,}/{pre_candidate_n:,}"
    )
    ...
    if target_cell_filter:
        ...
        if events.empty:
            return _empty_result(
                ...
                cfg_id,
                0,
                f"no_events_for_cell_{_bkt}_H{_h}",
                stage2_rescore=stage2_rescore,
            )
        ...
        bucket_masks = {
            b: (events["bucket"] == b)
            for b in (
                ["MR_long", "MR_short", "TF_long", "TF_short", "Candidate", "Global"]
            )
        }
    ...
    if atr_flat is None:
        atr_flat = np.ascontiguousarray(
            atr_source.to_numpy(dtype=np.float32, copy=False).ravel()
        )
    ...
    if atr_roll is None:
        atr_roll = (
            atr_source.rolling(24 * 14, min_periods=24).median().astype(np.float32)
        )
        eval_cache["atr_roll_14d"] = atr_roll
        gc.collect()
    atr_roll_flat = eval_cache.get("atr_roll_flat")
    if atr_roll_flat is None:
        atr_roll_flat = np.ascontiguousarray(
            atr_roll.to_numpy(dtype=np.float32, copy=False).ravel()
        )
    ...
    ratio = np.divide(
        atr,
        roll + EPS,
        out=np.ones_like(atr, dtype=np.float32),
        where=np.isfinite(roll),
    )
    ...
    regime = np.where(
        ratio < 0.85, "low_vol", np.where(ratio > 1.15, "high_vol", "medium_vol")
    )
    ...
    if basis_frame_key not in eval_cache:
        eval_cache[basis_frame_key] = make_quantile_basis(
            artifacts, quant_basis
        ).astype(np.float32)
        gc.collect()
    basis_flat_key = f"quant_basis_flat::{quant_basis}"
    basis_flat = eval_cache.get(basis_flat_key)
    if basis_flat is None:
        basis_flat = np.ascontiguousarray(
            eval_cache[basis_frame_key].to_numpy(dtype=np.float32, copy=False).ravel()
        )
    ...
    if cfg.get("use_quantile_filter", False):
        ...
        _qb_key = (
            _index_cache_key(pd.Index(ts_vals_for_grp)),
            str(quant_basis),
            round(lo, 6),
            round(hi, 6),
        )
    ...
    _last_gc_ts = _maybe_collect_gc(
        last_gc_ts=_last_gc_ts, mem_threshold_mb=8192.0, min_interval_s=5.0
    )
    ...
    if _agg_sl_to_tp > 5.0:
        tprint(
            f"[eval:{cfg_id}] EARLY_EXIT sl_to_tp={_agg_sl_to_tp:.2f}x > 5.0 — skipping learnability metrics"
        )
        return _empty_result(
            cfg,
            cfg_id,
            full_n,
            reason=f"sl_to_tp_too_high:{_agg_sl_to_tp:.2f}x",
            stage2_rescore=stage2_rescore,
        )
    ...
    _DEFAULT_CELL_METRICS = {
        ...
        "tp_mean": float("nan"),
        "sl_mean": float("nan"),
        "barrier_ratio": float("nan"),
        "h_eff_mean": float("nan"),
        "h_eff_p90": float("nan"),
        "payoff_mean": float("nan"),
        "max_timeout_threshold": float("nan"),
        "ok": False,
        "ic_payoff": float("nan"),
        "ic_label": float("nan"),
        "auc_label": float("nan"),
        ...
        "er_tp": float("nan"),
        "er_sl": float("nan"),
        "tp_over_sl": float("nan"),
        "er_tp_top_decile": float("nan"),
        "er_sl_top_decile": float("nan"),
        "dir_superiority_top_decile": float("nan"),
        "auc_bound": float("nan"),
        "auc_vol_low": float("nan"),
        "auc_vol_mid": float("nan"),
        "auc_vol_high": float("nan"),
        "brier": float("nan"),
        "ece": float("nan"),
        "monotonicity": float("nan"),
        "ic_std_time": float("nan"),
        "ic_std_asset": float("nan"),
    }
    ...
    for (b, h), g in events.groupby(["bucket", "horizon"], observed=False):
        ...
        ok = (
            (n_eval_kept >= min_raw)
            and (tp_hit_kept >= min_tp_hit)
            and (timeout_kept <= max_timeout_h)
        )
        pass_cells += int(ok)
        sl_hit_kept = float((g["label"] == OUT_SL).mean())
        bind_cell = tp_hit_kept + sl_hit_kept
        balance_cell = min(tp_hit_kept, sl_hit_kept) / max(
            max(tp_hit_kept, sl_hit_kept), EPS
        )
        ...
        bucket_h_metrics[(b, h)] = {
            ...
            "barrier_ratio": round(
                float((g["sl"].mean() + 0.003) / max(g["tp"].mean(), EPS)), 4
            ),
    ...
    tprint(
        f"[eval:{cfg_id}] Step 4 (Quantile/Filter/Buckets) took {time.perf_counter() - _t2:.2f}s"
    )
    ...
    if bool(cfg.get("use_neg_mass_renorm", True)):
        ...
        cell_ids = (
            events["bucket"].astype(str) + "_H" + events["horizon"].astype(str)
        ).values
    ...
    tprint(
        f"[eval:{cfg_id}] TRACE: finished format fm, shape={fm.shape if hasattr(fm, 'shape') else 'cached'}"
    )
    ...
    oof_flip_rate = (
        (float(flipped_cells) / float(modeled_cells)) if modeled_cells > 0 else 0.0
    )
    tprint(
    ...
    for _bname, _bg in events.groupby("bucket", observed=False):
        ...
        _bucket_top10_thresh[str(_bname)] = (
            float(np.quantile(_bp, 0.90)) if len(_bp) >= 10 else float("inf")
        )
    ...
    for (b, h), g in events.groupby(["bucket", "horizon"], observed=False):
        ...
        n_pos = int(y_tp_cell.sum())
        n_neg = len(y_tp_cell) - n_pos
        ...
        tp_top_cell = (
            float(y_tp_cell[top_mask_cell].mean())
            if top_mask_cell.any()
            else float("nan")
        )
        tp_rest_cell = (
            float(y_tp_cell[~top_mask_cell].mean())
        ...
        tp_sep_cell = (
            (tp_top_cell - tp_rest_cell)
            if not (math.isnan(tp_top_cell) or math.isnan(tp_rest_cell))
            else 0.0
        )
        bucket_h_metrics[(b, h)]["ic_payoff"] = round(float(cell_ic), 5)
        bucket_h_metrics[(b, h)]["ic_label"] = round(float(cell_ic_label), 5)
        bucket_h_metrics[(b, h)]["auc_label"] = (
            round(auc_cell, 4) if not math.isnan(auc_cell) else 0.5
        )
        ...
        if not math.isnan(_base_rate) and _base_rate > 0.0 and n_pos > 0 and n_neg > 0:
            ...
            _ap_lift = float("nan")
        bucket_h_metrics[(b, h)]["base_rate"] = (
            round(_base_rate, 5) if not math.isnan(_base_rate) else float("nan")
        )
        bucket_h_metrics[(b, h)]["ap_lift"] = (
            round(_ap_lift, 4) if not math.isnan(_ap_lift) else float("nan")
        )
        ...
        _er_tp = (
            float(_payoff_cell[_tp_mask_cell].mean())
            if _tp_mask_cell.sum() > 0
            else float("nan")
        )
        _er_sl = (
            float(_payoff_cell[_sl_mask_cell].mean())
        ...
        _cell_top10_thresh = (
            float(np.quantile(p_cell, 0.90)) if len(p_cell) >= 10 else float("inf")
        )
        ...
        _er_tp_top = (
            float(_payoff_cell[_tp_top_dec].mean())
            if _tp_top_dec.sum() > 0
            else float("nan")
        )
        _er_sl_top = (
            float(_payoff_cell[_sl_top_dec].mean())
            if _sl_top_dec.sum() > 0
            else float("nan")
        )
        if (
            not (math.isnan(_er_tp_top) or math.isnan(_er_sl_top))
            and abs(_er_sl_top) > EPS
        ):
            _dir_sup = _er_tp_top / abs(_er_sl_top)
        else:
            _dir_sup = float("nan")
        bucket_h_metrics[(b, h)]["er_tp_top_decile"] = (
            round(_er_tp_top, 6) if not math.isnan(_er_tp_top) else float("nan")
        )
        bucket_h_metrics[(b, h)]["er_sl_top_decile"] = (
            round(_er_sl_top, 6) if not math.isnan(_er_sl_top) else float("nan")
        )
        bucket_h_metrics[(b, h)]["dir_superiority_top_decile"] = (
            round(_dir_sup, 4) if not math.isnan(_dir_sup) else float("nan")
        )
    ...
    for (b, h), g in events.groupby(["bucket", "horizon"], observed=False):
        ...
        if bound_mask.sum() >= 10:
            ...
            n_pos_b = int(y_bound.sum())
            n_neg_b = len(y_bound) - n_pos_b
            ...
            auc_bound_cell = float("nan")
        bucket_h_metrics[(b, h)]["auc_bound"] = (
            round(auc_bound_cell, 4) if not math.isnan(auc_bound_cell) else float("nan")
        )
        ...
        if len(p_cell) >= 20:
            ...
            if _n_bins_mono >= 2:
                _dec_cell = pd.qcut(
                    pd.Series(p_cell), _n_bins_mono, labels=False, duplicates="drop"
                )
                _pay_cell = g["payoff"].values.astype(np.float32, copy=False)
                _pay_by_dec = (
                    pd.DataFrame({"d": _dec_cell, "p": _pay_cell})
                    .groupby("d")["p"]
                    .mean()
                )
                _mono_cell = (
                    float((np.diff(_pay_by_dec.values) >= 0).mean())
                    if len(_pay_by_dec) > 1
                    else float("nan")
                )
            else:
            ...
            _ece_cell = float("nan")
            _mono_cell = float("nan")
        bucket_h_metrics[(b, h)]["brier"] = (
            round(_brier_cell, 5) if not math.isnan(_brier_cell) else float("nan")
        )
        bucket_h_metrics[(b, h)]["ece"] = (
            round(_ece_cell, 5) if not math.isnan(_ece_cell) else float("nan")
        )
        bucket_h_metrics[(b, h)]["monotonicity"] = (
            round(_mono_cell, 4) if not math.isnan(_mono_cell) else float("nan")
        )
        ...
        try:
            ...
            for _m in pd.unique(_months):
                ...
                if _mm.sum() >= 5:
                    _ic_by_month.append(
                        _safe_spearman(_p_cell_f[_mm], _pay_cell_f[_mm])
                    )
            _ic_std_time_cell = (
                float(np.std(_ic_by_month)) if len(_ic_by_month) >= 2 else float("nan")
            )
        except Exception:
        ...
        try:
            ...
            _ic_std_asset_cell = (
                float(np.std(_ic_by_sym)) if len(_ic_by_sym) >= 2 else float("nan")
            )
        except Exception:
            _ic_std_asset_cell = float("nan")
        bucket_h_metrics[(b, h)]["ic_std_time"] = (
            round(_ic_std_time_cell, 5)
            if not math.isnan(_ic_std_time_cell)
            else float("nan")
        )
        bucket_h_metrics[(b, h)]["ic_std_asset"] = (
            round(_ic_std_asset_cell, 5)
    ...
    cell_ics = [
        ...
        if not math.isnan(v["ic_payoff"])
    ]
    cell_ic_labels = [
        v["ic_label"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("ic_label", float("nan")))
    ]
    cell_aucs = [
        v["auc_label"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("auc_label", float("nan")))
    ]
    cell_aucs_bound = [
        v["auc_bound"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("auc_bound", float("nan")))
    ]
    cell_tp_seps = [
        v["tp_sep_top10"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("tp_sep_top10", float("nan")))
    ]
    cell_timeouts = [v["timeout"] for v in bucket_h_metrics.values()]
    cell_sl_to_tp_vals = [
        v["sl_to_tp"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("sl_to_tp", float("nan")))
    ]
    cell_ap_lifts = [
        v["ap_lift"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("ap_lift", float("nan")))
    ]
    cell_tp_over_sls = [
        v["tp_over_sl"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("tp_over_sl", float("nan")))
    ]
    cell_dir_sups = [
        v["dir_superiority_top_decile"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("dir_superiority_top_decile", float("nan")))
    ]
    cell_barrier_ratios = [
        v["barrier_ratio"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("barrier_ratio", float("nan")))
    ]
    ...
    min_cell_auc_bound = (
        float(np.min(cell_aucs_bound)) if cell_aucs_bound else float("nan")
    )
    median_cell_auc_bound = (
        float(np.median(cell_aucs_bound)) if cell_aucs_bound else float("nan")
    )
    ...
    timeout_range = (
        float(np.max(cell_timeouts) - np.min(cell_timeouts))
        if len(cell_timeouts) > 1
        else 0.0
    )
    ...
    median_cell_dir_superiority = (
        float(np.median(cell_dir_sups)) if cell_dir_sups else float("nan")
    )
    p50_cell_sl_to_tp = (
        float(np.median(cell_sl_to_tp_vals)) if cell_sl_to_tp_vals else float("nan")
    )
    p90_cell_sl_to_tp = (
        float(np.quantile(np.asarray(cell_sl_to_tp_vals, dtype=np.float64), 0.90))
        if cell_sl_to_tp_vals
        else float("nan")
    )
    max_barrier_ratio = (
        float(np.max(cell_barrier_ratios)) if cell_barrier_ratios else float("nan")
    )
    ...
    if target_cell_filter:
        ...
        min_cell_ic_label = median_cell_ic_label = (
            0.0 if math.isnan(_ic_lbl_v) else _ic_lbl_v
        )
    ...
    _tp_vals_agg = (
        events.loc[events["label"] == OUT_TP, "payoff"].to_numpy(
    ...
    _sl_vals_agg = (
        events.loc[events["label"] == OUT_SL, "payoff"].to_numpy(
            dtype=np.float32, copy=False
        )
        if len(events)
        else np.array([], dtype=np.float32)
    ...
    edge = float(
        math.log(
            max(tp_hit_agg, EPS)
            * max(tp_med_agg, EPS)
            / max(sl_hit_agg * sl_med_agg, EPS)
        )
    )
    # Use neutral payoff-edge contribution when TP/SL sample support is too thin.
    payoff_edge = (
        edge if (_tp_count >= _min_edge_count and _sl_count >= _min_edge_count) else 0.0
    )
    ...
    _cell_briers = [
        v["brier"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("brier", float("nan")))
    ]
    _cell_eces = [
        v["ece"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("ece", float("nan")))
    ]
    _cell_monos = [
        v["monotonicity"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("monotonicity", float("nan")))
    ]
    _cell_ic_std_times = [
        v["ic_std_time"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("ic_std_time", float("nan")))
    ]
    _cell_ic_std_assets = [
        v["ic_std_asset"]
        for v in bucket_h_metrics.values()
        if not math.isnan(v.get("ic_std_asset", float("nan")))
    ]
    ...
    ic_std_time_median = (
        float(np.median(_cell_ic_std_times)) if _cell_ic_std_times else float("nan")
    )
    ic_std_asset_median = (
        float(np.median(_cell_ic_std_assets)) if _cell_ic_std_assets else float("nan")
    )
    ...
    _target_h = (
        int(target_cell_filter[1])
        if target_cell_filter
        else int(cfg.get("horizon_base", 4))
    )
    _target_b = str(target_cell_filter[0]) if target_cell_filter else None
    _lw = _resolve_learnability_weights(
        cfg, target_horizon=_target_h, target_bucket=_target_b
    )

    _auc_raw = (
        median_cell_auc_bound
        if not math.isnan(median_cell_auc_bound)
        else (median_cell_auc if not math.isnan(median_cell_auc) else 0.5)
    )
    ...
    if not math.isnan(median_cell_tp_over_sl) and median_cell_tp_over_sl > 0.0:
        _tp_over_sl_score = float(
            np.tanh(math.log(max(median_cell_tp_over_sl, EPS)) / 0.35)
        )
    ...
    _ic_std_asset = (
        0.0 if math.isnan(ic_std_asset_median) else float(ic_std_asset_median)
    )
    _ic_std_target = float(cfg.get("stage2_ic_std_target", 0.12))
    _instability_pen = (
        _lw.w_ic_std_time
        * max(0.0, (_ic_std_time - _ic_std_target) / max(_ic_std_target, EPS)) ** 2
    )
    _instability_asset_pen = (
        _lw.w_ic_std_asset
        * max(0.0, (_ic_std_asset - _ic_std_target) / max(_ic_std_target, EPS)) ** 2
    )
    _coverage_pen = _lw.w_coverage * max(
        0.0,
        (float(cfg.get("stage2_target_coverage", 0.45)) - coverage)
        / max(float(cfg.get("stage2_target_coverage", 0.45)), EPS),
    )
    _timeout_pen = _lw.w_timeout * max(
        0.0, timeout_agg - float(cfg.get("stage2_timeout_target", 0.55))
    )
    _bind_pen = _lw.w_bind * max(
        0.0, float(cfg.get("stage2_bind_target", 0.35)) - bind_agg
    )
    ...
    rr_floor = float(
        _rr_floor_map.get(
            _target_h, float(cfg.get("stage2_rr_keep_floor_default", 0.55))
        )
    )
    ...
    _horizon_cons_pen = _lw.w_horizon_consistency * _horizon_consistency_penalty(
        bucket_h_metrics, cfg
    )
    ...
    if probe_enabled:
        (
            ...
            probe_n_train,
            probe_n_val,
        ) = _strict_walk_forward_probe_auc(
    ...
    if target_cell_filter:
        ...
        _prod_aligned_meta = (
            cfg.get("prod_aligned_tp", {})
            if isinstance(cfg.get("prod_aligned_tp", {}), dict)
            else {}
        )
        _tp_min_tradeable = float(
            _prod_aligned_meta.get(
                "tp_min_tradeable", cfg.get("prod_adm_tradeable_tp_min", 0.015)
            )
        )
        _prod_tp_lo = float(
            cfg.get("prod_tp_abs_lo_pct", cfg.get("tp_abs_lo_pct", 0.005))
        )
        _prod_sl_lo = float(
            cfg.get("prod_sl_abs_lo_pct", cfg.get("sl_abs_lo_pct", 0.005))
        )
        _prod_gates = ProdGates(
            ...
            tp_floor_bind_max_cell=float(
                cfg.get(
                    "prod_adm_floor_bind_cell_max",
                    cfg.get("prod_adm_tp_floor_bind_max_cell", 0.35),
                )
            ),
            tp_floor_bind_max_agg=float(
                cfg.get(
                    "prod_adm_floor_bind_agg_max",
                    cfg.get("prod_adm_tp_floor_bind_max_agg", 0.20),
                )
            ),
            ...
            enforce_tradeable_tp_lo=bool(
                cfg.get("prod_adm_enforce_tradeable_tp_lo", True)
            ),
        ...
        _tp_eff_arr = (
            events["tp"].to_numpy(dtype=np.float32, copy=False)
            if "tp" in events.columns
            else np.array([], dtype=np.float32)
        )
        _tp_eff_arr = (
            _tp_eff_arr[np.isfinite(_tp_eff_arr)] if _tp_eff_arr.size else _tp_eff_arr
        )
        _tp_eff_p50 = (
            float(np.quantile(_tp_eff_arr, 0.50)) if _tp_eff_arr.size else float("nan")
        )
        _tp_eff_p75 = (
            float(np.quantile(_tp_eff_arr, 0.75)) if _tp_eff_arr.size else float("nan")
        )
        _tp_eff_p90 = (
            float(np.quantile(_tp_eff_arr, 0.90)) if _tp_eff_arr.size else float("nan")
        )
        ...
        _tradeable_rule_p50 = bool(
            np.isfinite(_tp_eff_p50) and (_tp_eff_p50 >= _tradeable_min_lo)
        )
        ...
        prod_admissibility["admissible_tier0"] = bool(
            prod_admissibility.get("admissible_tier0", False) and _tradeable_ok
        )
        ...
        if not bool(prod_admissibility.get("admissible_tier0", False)):
            tprint(
                f"[PROD_ADMISSIBILITY:{cfg_id}] FAIL "
                + " | ".join(prod_admissibility.get("failures", []))
            )

        _pa_agg = (
            prod_admissibility.get("aggregates", {})
            if isinstance(prod_admissibility, dict)
            else {}
        )
        ...
        _floor_dom_pen = float(
            cfg.get("floor_dominance_penalty_weight_agg", 0.25)
        ) * max(_tp_fb_agg, 0.0) + float(
            cfg.get("floor_dominance_penalty_weight_max", 0.25)
        ) * max(
            _tp_fb_max, 0.0
        )
    ...
    summary = {
        ...
        "ece": ece,
        "monotonicity": mono,
        "min_cell_brier": round(min_cell_brier, 5)
        if not math.isnan(min_cell_brier)
        else float("nan"),
        "min_cell_ece": round(min_cell_ece, 5)
        if not math.isnan(min_cell_ece)
        else float("nan"),
        "min_cell_mono": round(min_cell_mono, 4)
        if not math.isnan(min_cell_mono)
        else float("nan"),
        ...
        "min_cell_auc": round(min_cell_auc, 4)
        if not math.isnan(min_cell_auc)
        else float("nan"),
        "median_cell_auc": round(median_cell_auc, 4)
        if not math.isnan(median_cell_auc)
        else float("nan"),
        "min_cell_auc_bound": round(min_cell_auc_bound, 4)
        if not math.isnan(min_cell_auc_bound)
        else float("nan"),
        "median_cell_auc_bound": round(median_cell_auc_bound, 4)
        if not math.isnan(median_cell_auc_bound)
        else float("nan"),
        ...
        "median_cell_ap_lift": round(median_cell_ap_lift, 4)
        if not math.isnan(median_cell_ap_lift)
        else float("nan"),
        "min_cell_tp_over_sl": round(min_cell_tp_over_sl, 4)
        if not math.isnan(min_cell_tp_over_sl)
        else float("nan"),
        "median_cell_tp_over_sl": round(median_cell_tp_over_sl, 4)
        if not math.isnan(median_cell_tp_over_sl)
        else float("nan"),
        "min_cell_dir_superiority": round(min_cell_dir_superiority, 4)
        if not math.isnan(min_cell_dir_superiority)
        else float("nan"),
        "median_cell_dir_superiority": round(median_cell_dir_superiority, 4)
        if not math.isnan(median_cell_dir_superiority)
        else float("nan"),
        "effective_pos_mass": float(effective_pos_mass),
        "effective_neg_mass": float(effective_neg_mass),
        "prod_admissible_tier0": bool(
            prod_admissibility.get("admissible_tier0", False)
        ),
        ...
        "tp_eff_tradeable_rule_p50": bool(
            _pa_agg.get("tp_eff_tradeable_rule_p50", False)
        ),
        "tp_eff_tradeable_rule_tail": bool(
            _pa_agg.get("tp_eff_tradeable_rule_tail", False)
        ),
        "prod_aligned_tp": cfg.get("prod_aligned_tp", {}),
    }
    ...
    detail = {
        ...
        "bucket_horizon_metrics": {
            f"{k[0]}_H{k[1]}": v for k, v in bucket_h_metrics.items()
        },
    ...
    if collect_weights:
        weights_df = events[
            ["ts", "symbol", "side", "horizon", "bucket", "label", "payoff"]
        ].copy()
...
def _atr_pct_samples_for_prod_universe(
    ...
    for m in bucket_masks.values():
        if isinstance(m, pd.DataFrame):
            union_mask |= m.reindex(index=close.index, columns=close.columns).fillna(
                False
            )
...
def _apply_prod_aligned_tp_centering(
    ...
    *,
    ...
    if atr_samples.size == 0:
        tprint(
            "[prod_aligned_tp] WARNING no ATR% samples in production universe; skipping TP centering override"
        )
        return cfgs, {}
    ...
    if not ladder:
        ladder = [
            {
                "tp_base_pct": float(aligned["tp_base_pct"]),
                "tp_eff_targets": {},
                "tp_eff_bands": {},
                "q": q,
                "alpha": alpha,
    ...
    for c in cfgs:
        ...
        grid_tp_base = float(
            c.get("tp_base_pct", c.get("tp_abs_pct", float(aligned["tp_base_pct"])))
        )
        ...
        if preserve_sl_axis or not bool(runtime.get("prod_sl_tp_wide_enable", True)):
            ...
            sup_add = float(
                runtime.get(
                    "prod_sl_tp_superiority_add",
                    c2.get("prod_sl_tp_superiority_add", 0.0075),
                )
            )
            if sup_add > 0.02:
                tprint(
            ...
            policy = SLTPPolicy(
                ...
                drop_on_violation=bool(
                    runtime.get("prod_sl_tp_drop_on_violation", True)
                ),
            )
            tp_eff_ref = float(
            ...
            out.extend(
                expand_configs_wide_sl_tp_additive_superiority(
                    c2, tp_eff=tp_eff_ref, policy=policy
                )
            )
...
def base_param_template(cfg_runtime: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    tbm_defaults = get_tbm_optimizer_defaults(
    ...
    return {
        ...
        "tbm_prod_aligned_tp_enable": bool(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "tbm_prod_aligned_tp_enable", True
            )
        ),
        "tbm_prod_aligned_fee_pct_total": float(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "tbm_prod_aligned_fee_pct_total", 0.003
            )
        ),
        ...
        "tbm_prod_aligned_q": float(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "tbm_prod_aligned_q", 0.25
            )
        ),
        "tbm_prod_aligned_alpha": float(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "tbm_prod_aligned_alpha", 0.45
            )
        ),
        "tbm_prod_aligned_margin_mult": float(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "tbm_prod_aligned_margin_mult", 4.0
            )
        ),
        "tbm_prod_aligned_hard_min_tp": float(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "tbm_prod_aligned_hard_min_tp", 0.02
            )
        ),
        "tbm_prod_aligned_inflate": float(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
        ...
        "prod_adm_tradeable_tp_min": float(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "prod_adm_tradeable_tp_min", 0.015
            )
        ),
        "prod_adm_tradeable_tp_min_lo": float(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "prod_adm_tradeable_tp_min_lo", 0.012
            )
        ),
        "prod_adm_tradeable_tp_min_hi": float(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "prod_adm_tradeable_tp_min_hi", 0.022
            )
        ),
        "prod_adm_floor_bind_agg_max": float(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "prod_adm_floor_bind_agg_max", 0.20
            )
        ),
        "prod_adm_floor_bind_cell_max": float(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "prod_adm_floor_bind_cell_max", 0.35
            )
        ),
        # Wide SL/TP ladder policy under additive TP superiority rule.
        "prod_sl_tp_wide_enable": bool(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "prod_sl_tp_wide_enable", True
            )
        ),
        "prod_sl_tp_superiority_add": float(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "prod_sl_tp_superiority_add", 0.0075
            )
        ),
        "prod_sl_tp_drop_on_violation": bool(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "prod_sl_tp_drop_on_violation", True
            )
        ),
        ...
        "oof_auc_flip_enable": bool(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "oof_auc_flip_enable", True
            )
        ),
        "oof_auc_flip_threshold": float(
            (cfg_runtime if cfg_runtime is not None else CFG).get(
                "oof_auc_flip_threshold", 0.50
            )
        ),
    }
...
def _filter_tradeability_guard(
    cfgs: List[Dict[str, Any]], cfg_runtime: Optional[Dict[str, Any]] = None
) -> List[Dict[str, Any]]:
    if not cfgs:
        return cfgs
...
def stage2_grids_from_stage1(
    winners: List[Dict[str, Any]], max_per_substage: int = 24
) -> List[Dict[str, Any]]:
    ...
    for base in winners:
        ...
        if tp_method == "atr_norm":
            ...
            _win_idx = min(
                range(len(_win_ladder)), key=lambda i: abs(_win_ladder[i] - base_win)
            )
            _win_neighbours = sorted(
                set(_win_ladder[max(0, _win_idx - 1) : _win_idx + 2])
            )
    ...
    for base in stage2a:
        ...
        for sl_pct in [
            ...
            c.update(
                {
                    "mode": f"{base.get('mode','stage2')}_sl",
                    "sl_method": "tp_pct",
                    "sl_as_tp_pct": float(sl_pct),
                }
            )
            stage2b.append(c)
...
_PROMOTE_MIN_AP_LIFT: float = (
    1.20  # AP / base_rate — model must lift precision above random
)
_PROMOTE_MIN_TP_OVER_SL: float = (
    1.30  # E[r|TP] / abs(E[r|SL]) — 30% payoff edge required to survive 50bps fees
)
_PROMOTE_MAX_SL_TO_TP: float = 3  # SL-hit / TP-hit ratio cap — informational only; fee_ev gate is the binding constraint
_MAX_BARRIER_RATIO: float = (
    1.0  # SL-mean / TP-mean cap — ensures SL isn't too high compared to TP
)
_AVG_AUC_THRESHOLD: float = (
    0.54  # Minimum average AUC for the selected set of geometries
)
...
_ROUND_TRIP_FEE: float = (
    0.003  # 0.3% round-trip fee (entry + exit) applied to both TP and SL legs
)
_PROMOTE_MIN_FEE_EV: float = (
    0.0  # fee-adjusted EV must be > 0: tp_hit*(tp_mean-fee) - sl_hit*(sl_mean+fee) > 0
)
...
_FEASIBLE_TIERS: List[
    Tuple[float, float, float, float, float, float, float, float, float, float, float]
] = [
    (
        _PROMOTE_MIN_COVERAGE,
        _PROMOTE_MIN_BIND,
        _PROMOTE_MAX_TIMEOUT,
        _PROMOTE_MIN_TP_SEP,
        _PROMOTE_MIN_AUC,
        _PROMOTE_MIN_AP_LIFT,
        _PROMOTE_MIN_TP_OVER_SL,
        _PROMOTE_MAX_ECE,
        _PROMOTE_MAX_BRIER,
        _PROMOTE_MIN_EFFECTIVE_POS,
        _PROMOTE_MIN_EFFECTIVE_NEG,
    ),  # Tier 0
    (
        _PROMOTE_MIN_COVERAGE,
        _PROMOTE_MIN_BIND,
    ...
    for tier, (
        ...
        if auc_min > 0.0:
            ...
            if auc_col in df.columns:
                mask = mask & (
                    pd.to_numeric(df[auc_col], errors="coerce").fillna(0.0) >= auc_min
                )
        ...
        if ece_max < 999.0:
            _ece_col = (
                "min_cell_ece"
                if "min_cell_ece" in df.columns
                else ("ece" if "ece" in df.columns else None)
            )
            if _ece_col:
                mask = mask & (
                    pd.to_numeric(df[_ece_col], errors="coerce").fillna(999.0)
                    <= ece_max
                )
        if brier_max < 999.0:
            _br_col = (
                "min_cell_brier"
                if "min_cell_brier" in df.columns
                else ("brier" if "brier" in df.columns else None)
            )
            if _br_col:
                mask = mask & (
                    pd.to_numeric(df[_br_col], errors="coerce").fillna(999.0)
                    <= brier_max
                )
        if eff_pos_min > 0.0 and "effective_pos_mass" in df.columns:
            mask = mask & (
                pd.to_numeric(df["effective_pos_mass"], errors="coerce").fillna(0.0)
                >= eff_pos_min
            )
        if eff_neg_min > 0.0 and "effective_neg_mass" in df.columns:
            mask = mask & (
                pd.to_numeric(df["effective_neg_mass"], errors="coerce").fillna(0.0)
                >= eff_neg_min
            )
        ...
        if not result.empty:
            if tier > 0:
                tprint(
                    f"[feasible_set] Tier {tier} used (relaxed learnability gates): {len(result)} configs"
                )
            return result, tier
    return df, -1
...
def _param_distance(cfg_a: Dict[str, Any], cfg_b: Dict[str, Any]) -> float:
    ...
    d_win = (
        abs(
            float(cfg_a.get("base_atr_window", 672))
            - float(cfg_b.get("base_atr_window", 672))
        )
        / 168.0
    )
    ...
    d_sl = (
        abs(
            float(cfg_a.get("sl_as_tp_pct", 0.5))
            - float(cfg_b.get("sl_as_tp_pct", 0.5))
        )
        / 0.1
    )
    d_tp = (
        abs(
            float(cfg_a.get("tp_base_pct", 0.01))
            - float(cfg_b.get("tp_base_pct", 0.01))
        )
        / 0.002
    )
...
def _robust_minmax(
    values: np.ndarray, p_lo: float = 5.0, p_hi: float = 95.0, eps: float = 1e-12
) -> np.ndarray:
...
def _diverse_subset(
    ...
    if max_configs <= 0 or feasible_df is None or feasible_df.empty:
        return (
            feasible_df.head(0)
            if isinstance(feasible_df, pd.DataFrame)
            else pd.DataFrame()
        )
    ...
    score_col = (
        "stage_score"
        if "stage_score" in work_df.columns
        else ("stage2_score" if "stage2_score" in work_df.columns else "stage1_score")
    )
    auc_col = (
        "min_cell_auc_bound"
        if "min_cell_auc_bound" in work_df.columns
        else ("min_cell_auc" if "min_cell_auc" in work_df.columns else None)
    )
    if auc_col is None:
    ...
    work_df["_cid"] = work_df.get(
        "config_id", pd.Series(range(len(work_df)), index=work_df.index)
    ).astype(str)
    _tuple_cols = [
        c for c in ("k_tp", "tp_anchor", "sl_as_tp_pct") if c in work_df.columns
    ]
    if len(_tuple_cols) < 3:
        _tuple_cols = [
            c for c in ("k_tp", "tp_base_pct", "sl_as_tp_pct") if c in work_df.columns
        ]
    work_df["_stable_id"] = work_df["_cid"]
    if _tuple_cols:
        work_df["_stable_id"] = (
            work_df[_tuple_cols].round(8).astype(str).agg("|".join, axis=1)
        )

    dedupe_cols = list(_tuple_cols)
    if dedupe_cols:
        work_df = work_df.sort_values(
            ["_stage_score", "_auc_metric", "_stable_id"],
    ...
    pool_df = work_df[
        np.isfinite(work_df["_auc_metric"]) & (work_df["_auc_metric"] >= auc_floor)
    ].copy()
    auc_floor_relaxed = False
    if pool_df.empty:
        pool_df = work_df[
            np.isfinite(work_df["_auc_metric"]) & (work_df["_auc_metric"] >= 0.54)
        ].copy()
    ...
    if "pr_auc_lift" in pool_df.columns:
        econ_mask = econ_mask & (
            pd.to_numeric(pool_df["pr_auc_lift"], errors="coerce").fillna(-np.inf) > 0.0
        )
    if "edge" in pool_df.columns:
        econ_mask = econ_mask & (
            pd.to_numeric(pool_df["edge"], errors="coerce").fillna(-np.inf) > 0.0
        )
    ...
    param_cols = [
        c for c in ("k_tp", "tp_anchor", "sl_as_tp_pct") if c in pool_df.columns
    ]
    if len(param_cols) < 3:
        param_cols = [
            c for c in ("k_tp", "tp_base_pct", "sl_as_tp_pct") if c in pool_df.columns
        ]
    if len(param_cols) < 3:
    ...
    stage_score_norm = _robust_minmax(
        pool_df["_stage_score"].to_numpy(dtype=np.float64, copy=False),
        p_lo=5.0,
        p_hi=95.0,
    )
    ...
    selected_metrics[seed_pos] = (
        1.0,
        float(stage_score_norm[seed_pos]),
        float(lambda_eff * stage_score_norm[seed_pos] + (1.0 - lambda_eff) * 1.0),
    )
    ...
    if remaining:
        ...
        dmin_all[rem_idx0] = np.minimum(
            dmin_all[rem_idx0], _dist_to_one(rem_idx0, seed_pos)
        )
    ...
    while remaining and len(selected) < max_configs:
        ...
        utility = (
            lambda_eff * stage_score_norm[rem_idx] + (1.0 - lambda_eff) * novelty_norm
        )
        ...
        selected_metrics[pick_pos] = (
            float(novelty_raw[pick_loc]),
            float(stage_score_norm[pick_pos]),
            float(utility[pick_loc]),
        )
        selected.append(pick_pos)
        remaining.remove(pick_pos)
        if remaining:
            rem_idx2 = np.asarray(remaining, dtype=int)
            dmin_all[rem_idx2] = np.minimum(
                dmin_all[rem_idx2], _dist_to_one(rem_idx2, pick_pos)
            )

    selected_df = pool_df.iloc[np.asarray(selected, dtype=int)].copy()
    selected_df["sl_anchor_pct"] = pd.to_numeric(
        selected_df.get("tp_base_pct", np.nan), errors="coerce"
    ) * pd.to_numeric(selected_df.get("sl_as_tp_pct", np.nan), errors="coerce")
    if enable_span_repair:
        pool_aug = pool_df.copy()
        pool_aug["sl_anchor_pct"] = pd.to_numeric(
            pool_aug.get("tp_base_pct", np.nan), errors="coerce"
        ) * pd.to_numeric(pool_aug.get("sl_as_tp_pct", np.nan), errors="coerce")
    ...
    selected_df["_cid"] = (
        selected_df["_cid"].fillna(selected_df.get("config_id", "")).astype(str)
    )
    if "_stable_id" not in selected_df.columns:
        selected_df["_stable_id"] = selected_df["_cid"]
    selected_df["_stable_id"] = (
        selected_df["_stable_id"].fillna(selected_df["_cid"]).astype(str)
    )
    if "_stage_score" not in selected_df.columns:
        selected_df["_stage_score"] = pd.to_numeric(
            selected_df.get(score_col, np.nan), errors="coerce"
        )
    if "_auc_metric" not in selected_df.columns:
        selected_df["_auc_metric"] = pd.to_numeric(
            selected_df.get(auc_col, np.nan), errors="coerce"
        )
    ...
    for r in selected_df.itertuples(index=False):
        ...
        if not _row.empty:
            ...
            _nov, _scn, _util = selected_metrics.get(
                _pos, (float("nan"), float("nan"), float("nan"))
            )
        tprint(
    ...
    selected_df.attrs["grr_pool_df"] = pool_df.drop(
        columns=["sl_anchor_pct"], errors="ignore"
    )
    return selected_df
...
def _rank_by_learnability(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df is None or df.empty:
        return df
    _s2 = pd.to_numeric(
        df.get("stage2_score", pd.Series(np.nan, index=df.index)), errors="coerce"
    )
    _s1 = pd.to_numeric(
    ...
    _min_auc = (
        ...
        if "min_cell_auc_bound" in df.columns
            ...
            (df["min_cell_auc"].fillna(0.0) * 1000).round() / 1000
            if "min_cell_auc" in df.columns
            else pd.Series(0.0, index=df.index)
        )
    )
    _min_sep = (
        df["min_cell_tp_sep"].fillna(0.0)
        if "min_cell_tp_sep" in df.columns
        else pd.Series(0.0, index=df.index)
    )
    _disp = (
        df["cell_dispersion"].fillna(999.0)
        if "cell_dispersion" in df.columns
        else pd.Series(999.0, index=df.index)
    )
    _to_r = (
        df["timeout_range"].fillna(999.0)
        if "timeout_range" in df.columns
        else pd.Series(999.0, index=df.index)
    )
    _ece = (
        df["min_cell_ece"].fillna(999.0)
        if "min_cell_ece" in df.columns
        else (
            df["ece"].fillna(999.0)
    ...
    _brier = (
        df["min_cell_brier"].fillna(999.0)
        if "min_cell_brier" in df.columns
        else (
            df["brier"].fillna(999.0)
            if "brier" in df.columns
            else pd.Series(999.0, index=df.index)
        )
    )
    _ic_t = (
        df["ic_std_time"].fillna(999.0)
        if "ic_std_time" in df.columns
        else pd.Series(999.0, index=df.index)
    )
    _ic_a = (
        df["ic_std_asset"].fillna(999.0)
        if "ic_std_asset" in df.columns
        else pd.Series(999.0, index=df.index)
    )
    return (
        df.assign(
...
def promote_stage1(stage1_results: pd.DataFrame, top_k: int = 10) -> List[str]:
    ...
    _to_r_col = (
        stage1_results["timeout_range"].fillna(999.0)
        if "timeout_range" in stage1_results.columns
        else pd.Series(0.0, index=stage1_results.index)
    )
    struct_mask = (
        (stage1_results["hard_gate"] == True)
        & (stage1_results["pass_cells"] == stage1_results["total_cells"])
        & (_to_r_col <= _MAX_TIMEOUT_RANGE)
    )
...
def _build_per_cell_feasible_sets(
    ...
    _to_r_col = (
        out_df["timeout_range"].fillna(999.0)
        if "timeout_range" in out_df.columns
        else pd.Series(0.0, index=out_df.index)
    )
    struct_mask = (
        (out_df["hard_gate"] == True)
        & (out_df["pass_cells"] == out_df["total_cells"])
        & (_to_r_col <= _MAX_TIMEOUT_RANGE)
    )
    ...
    if base_df.empty:
        base_df = out_df.copy()
        tprint(
            f"[per_cell] WARNING: using all {len(base_df)} configs as fallback (no structural filter passed)"
        )

    result: Dict[str, pd.DataFrame] = {}

    _base_rows = base_df[
        ["config_id", "coverage", "cell_dispersion", "timeout_range"]
    ].copy()
    _cid_bh = {
        str(cid): (
            details.get(str(cid), {}).get("bucket_horizon_metrics", {})
            if isinstance(details.get(str(cid), {}), dict)
            else {}
        )
    ...
    for cell_key in _CELL_KEYS:
        ...
        for row in _base_rows.itertuples(index=False):
            ...
            coverage_cell = float(
                getattr(row, "coverage", 0.0)
            )  # config-level coverage as proxy
            ...
            if not any(math.isnan(v) for v in [_tp_h, _sl_h, _tp_m, _sl_m]):
                fee_ev_cell = _tp_h * (_tp_m - _ROUND_TRIP_FEE) - _sl_h * (
                    _sl_m + _ROUND_TRIP_FEE
                )
            else:
            ...
            _eff_pos_cell = (
                (_n_cell * _base_rate_cell)
                if math.isfinite(_base_rate_cell)
                else float("nan")
            )
            _eff_neg_cell = (
                (_n_cell * max(0.0, 1.0 - _base_rate_cell))
                if math.isfinite(_base_rate_cell)
                else float("nan")
            )
            ...
            cell_rows.append(
                {
                    ...
                    "ok": ok_cell,
                    "min_cell_ap_lift": ap_lift_cell
                    if not math.isnan(ap_lift_cell)
                    else 0.0,
                    "min_cell_tp_over_sl": tp_over_sl_cell
                    if not math.isnan(tp_over_sl_cell)
                    else 0.0,
                    "min_cell_dir_superiority": dir_sup_cell
                    if not math.isnan(dir_sup_cell)
                    else 0.0,
                    "barrier_ratio": barrier_ratio_cell
                    if not math.isnan(barrier_ratio_cell)
                    else 999.0,
                    "fee_ev": fee_ev_cell,
                    "monotonicity": mono_cell,
                    "min_cell_brier": cell_m.get("brier", float("nan")),
                    "min_cell_ece": cell_m.get("ece", float("nan")),
                    "effective_pos_mass": _eff_pos_cell
                    if math.isfinite(_eff_pos_cell)
                    else 0.0,
                    "effective_neg_mass": _eff_neg_cell
                    if math.isfinite(_eff_neg_cell)
                    else 0.0,
                }
            )
        ...
        if "min_cell_auc" in diverse.columns:
            while (
                len(diverse) > 3 and diverse["min_cell_auc"].mean() < _AVG_AUC_THRESHOLD
            ):
        ...
        avg_auc = (
            diverse["min_cell_auc"].mean()
            if "min_cell_auc" in diverse.columns and not diverse.empty
            else 0.0
        )
        tprint(
...
def _per_bucket_metrics_from_details(
    ...
    timeout_range = (
        float(np.max(timeout_vals) - np.min(timeout_vals))
        if len(timeout_vals) > 1
        else 0.0
    )
    bind_vals = [b for b in binds if not math.isnan(b)]
    bind_min = float(np.min(bind_vals)) if bind_vals else float("nan")
    sl_to_tp_max = (
        float(np.max([s for s in sl_to_tps if not math.isnan(s)]))
        if any(not math.isnan(s) for s in sl_to_tps)
        else float("nan")
    )
    barrier_max = (
        float(np.max([b for b in barrier_rs if not math.isnan(b)]))
        if any(not math.isnan(b) for b in barrier_rs)
        else float("nan")
    )
    ...
    for c in cells:
        ...
        if not any(math.isnan(v) for v in [_tp_h, _sl_h, _tp_m, _sl_m]):
            fee_evs.append(
                _tp_h * (_tp_m - _ROUND_TRIP_FEE) - _sl_h * (_sl_m + _ROUND_TRIP_FEE)
            )
    ...
    return {
        ...
        "min_cell_auc": round(_safe_min(aucs), 4)
        if not math.isnan(_safe_min(aucs))
        else float("nan"),
        "median_cell_auc": round(_safe_median(aucs), 4)
        if not math.isnan(_safe_median(aucs))
        else float("nan"),
        "min_cell_auc_bound": round(_safe_min(aucs_b), 4)
        if not math.isnan(_safe_min(aucs_b))
        else float("nan"),
        "min_cell_tp_sep": round(_safe_min(tp_seps), 5)
        if not math.isnan(_safe_min(tp_seps))
        else 0.0,
        "min_cell_ap_lift": round(_safe_min(ap_lifts), 4)
        if not math.isnan(_safe_min(ap_lifts))
        else 0.0,
        "min_cell_tp_over_sl": round(_safe_min(tp_over), 4)
        if not math.isnan(_safe_min(tp_over))
        else 0.0,
        "min_cell_ece": round(_safe_min(eces), 5)
        if not math.isnan(_safe_min(eces))
        else float("nan"),
        "min_cell_brier": round(_safe_min(briers), 5)
        if not math.isnan(_safe_min(briers))
        else float("nan"),
        "bind": round(bind_min, 4) if not math.isnan(bind_min) else 0.0,
        "timeout_rate": round(_safe_min(timeouts), 4)
        if not math.isnan(_safe_min(timeouts))
        else 1.0,
        "sl_to_tp": round(sl_to_tp_max, 4) if not math.isnan(sl_to_tp_max) else 999.0,
        "barrier_ratio": round(barrier_max, 4)
        if not math.isnan(barrier_max)
        else 999.0,
...
def _build_per_bucket_feasible_sets(
    ...
    for bucket_name in TBM_BUCKET_NAMES:
        ...
        if feasible.empty:
            result[bucket_name] = pd.DataFrame()
            tprint(
                f"[per_bucket] {bucket_name}: no configs passed feasibility gates (tier={tier})"
            )
            continue
        ...
        diverse = _diverse_subset(
            ...
            details,
            min_distance=min_distance,
            max_configs=max_configs_per_bucket,
...
def _print_winning_geometry_summary(
    ...
    tprint(sep)
    tprint(
        f"WINNING GEOMETRY SUMMARY  (top {top_k}  ranked: health_gates → min_auc → min_tp_sep → cell_dispersion → timeout_range → min_auc_bound)"
    )
    tprint(sep)
    ...
    for rank, (_, row) in enumerate(top.iterrows(), 1):
        ...
        # Degeneracy flags
        flags = []
        if row.get("flag_degenerate_timeout", False):
            flags.append("TIMEOUT!")
        if row.get("flag_degenerate_sl", False):
            flags.append("SL_DOM!")
        if row.get("flag_degenerate_tp", False):
            flags.append("TP_LOW!")
        ...
        _auc_str = (
            f"{med_auc:.4f}"
            if not (isinstance(med_auc, float) and math.isnan(med_auc))
            else "n/a"
        )
        _min_auc_str = (
            f"{min_auc:.4f}"
            if not (isinstance(min_auc, float) and math.isnan(min_auc))
            else "n/a"
        )
        ...
        _med_auc_b_str = (
            f"{_med_auc_b:.4f}"
            if not (isinstance(_med_auc_b, float) and math.isnan(_med_auc_b))
            else "n/a"
        )
        _min_auc_b_str = (
            f"{_min_auc_b:.4f}"
            if not (isinstance(_min_auc_b, float) and math.isnan(_min_auc_b))
            else "n/a"
        )
        tprint(
        ...
        if bh:
            ...
            for cell_name in sorted(bh.keys()):
                m = bh[cell_name]
                cell_flags = ""
                if m.get("timeout", 1.0) > 0.85:
                    cell_flags += "T"
                if m.get("sl_hit", 0.0) > 0.60:
                    cell_flags += "S"
                if m.get("tp_hit", 0.0) < 0.05:
                    cell_flags += "P"
    ...
    if per_cell_grids:
        tprint(
            "PER-CELL FEASIBLE SET SUMMARY  (bucket × horizon → diverse geometry grid)"
        )
        hdr_c = (
            f"  {'Cell':<18} {'n_configs':>9}  {'k_tp values':<30}  {'sl values':<20}  "
        ...
        for cell_key in sorted(per_cell_grids.keys()):
            ...
            _cfgs = [
                details.get(c, {}).get("config", {}) for c in _cids if c in details
            ]
            _k_vals = sorted(
                set(
                    float(c.get("k_tp", float("nan")))
                    for c in _cfgs
            ...
            _sl_vals = sorted(
                set(
                    float(c.get("sl_as_tp_pct", float("nan")))
                    for c in _cfgs
                    if isinstance(c, dict)
                )
            )
    ...
    hdr2 = (
        f"  {'mode':<24} {'n':>4}  {'timeout':>8}  {'tp_hit':>7}  "
    ...
    for mode, g in out_df.groupby("mode"):
        _min_auc_best = (
            g["min_cell_auc"].max() if "min_cell_auc" in g.columns else float("nan")
        )
        _min_sep_best = (
            g["min_cell_tp_sep"].max()
            if "min_cell_tp_sep" in g.columns
            else float("nan")
        )
        _to_range_best = (
            g["timeout_range"].min() if "timeout_range" in g.columns else float("nan")
        )
        bind_mean = g["bind"].mean() if "bind" in g.columns else float("nan")

        def _f(v: Any) -> str:
            return (
                f"{v:.4f}" if not (isinstance(v, float) and math.isnan(v)) else "  n/a"
            )
...
class TBMObjective:
    ...
    def __init__(
        ...
        tuple_eval_cache: Optional[
            Dict[Any, Tuple[float, Dict[str, Any], Dict[str, Any]]]
        ] = None,
    ):
        ...
        _close = (
            self.artifacts.panel.get("close", pd.DataFrame())
            if isinstance(self.artifacts, RunArtifacts)
            else pd.DataFrame()
        )
        _ctx = {
            ...
            "use_quantile_filter": bool(
                self.runtime_cfg.get("use_quantile_filter", False)
            ),
            ...
            "evaluate_under_prod_floor": bool(
                self.runtime_cfg.get("evaluate_under_prod_floor", False)
            ),
            "bars": int(len(_close.index)),
            "symbols": int(len(_close.columns)),
        }
        self.eval_context_hash = hashlib.sha1(
            json.dumps(_ctx, sort_keys=True).encode("utf-8")
        ).hexdigest()[:12]
        self.cache_hits = 0
        self.cache_misses = 0
    ...
    def __call__(self, trial: optuna.Trial) -> float:
        ...
        _cached = (
            self.eval_cache.get(_r_key, None)
            if hasattr(self.eval_cache, "get")
            else None
        )
        if _cached is not None:
            ...
            try:
                _atr_src = _get_barrier_atr_frame(self.artifacts).replace(
                    [np.inf, -np.inf], np.nan
                )
                _med = (
                    _atr_src.rolling(_atr_win, min_periods=24)
                    .median()
                    .fillna(_atr_src.median())
                )
                _ratio = (
                    (_atr_src / (_med + EPS))
                ...
                _atr_norm_mean_ref = float(
                    np.clip(np.nanmedian(_ratio) if _ratio.size else 1.0, 0.5, 2.0)
                )
                try:
        ...
        if not tp_anchor_grid:
            tp_anchor_grid, tp_anchor_meta = _build_param_grid(
                _TP_ANCHOR_LO, _TP_ANCHOR_HI
            )
        if not tp_anchor_grid:
        ...
        _cell_tag = (
            f"{self.target_cell[0]}_H{self.target_cell[1]}"
            if self.target_cell
            else "global"
        )
        _tuple_key = (
        ...
        try:
            ...
            if tp_anchor_eff < _TP_ANCHOR_LO or tp_anchor_eff > _TP_ANCHOR_HI:
                self.tuple_eval_cache[_tuple_key] = (
                    -1.0,
                    {
                        "config_id": config_id(c),
                        "stage_score": -1.0,
                        "hard_gate": False,
                        "k_tp": k_tp,
                        "tp_anchor": tp_anchor,
                        "sl_as_tp_pct": sl_as_tp,
                    },
                    {"config": dict(c)},
                )
                return -1.0
            ...
            self._avg_eval_s = (
                _dur if self._avg_eval_s <= 0 else (0.9 * self._avg_eval_s + 0.1 * _dur)
            )
            ...
            _m_auc = float(
                _safe_float(res.get("median_cell_auc", float("nan")), float("nan"))
            )
            _m_ic_snr = float(
                _safe_float(res.get("ic_snr", float("nan")), float("nan"))
            )
            _m_tp_sep = float(
                _safe_float(res.get("median_cell_tp_sep", float("nan")), float("nan"))
            )
            _m_dir_sup = float(
                _safe_float(
                    res.get("median_cell_dir_superiority", float("nan")), float("nan")
                )
            )
            _m_payoff = float(
                _safe_float(res.get("payoff_edge", float("nan")), float("nan"))
            )
            ...
            trial.set_user_attr(
                "derived_tp_clipped_high", bool(derived_tp_clipped_high)
            )
...
def run(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        tprint(
            f"[MEM_INIT] RSS={_rss_mb:.0f}MB  system_available={_sys_avail_mb:.0f}MB"
        )
    except ImportError:
        tprint(
            f"[MEM_INIT] RSS={_rss_mb:.0f}MB  (psutil unavailable for system RAM check)"
        )
    ...
    if bool(args.perps):
        ...
        runtime_cfg["data_root"] = _append_suffix(
            runtime_cfg.get("data_root", "../data"), "_perp"
        )
    ...
    if args.features:
        ...
        if features is None:
            raise ValueError(
                "Could not auto-detect features. Please provide --features path."
            )
    ...
    cache_sig = _tbm_cache_signature(
        artifacts, horizons=horizons, lookback_years=int(args.lookback_years)
    )
    ...
    weights_path = (
        Path(args.weights_output)
        if args.weights_output
        else output_path.with_suffix(".weights.parquet")
    )
    weights_writer = None
    ...
    def write_weights_streaming(weights_df: pd.DataFrame) -> None:
        ...
        if weights_writer is None:
            ...
            weights_writer = pq.ParquetWriter(
                weights_path, schema, compression="snappy"
            )
    ...
    for bkt, hor in canonical_cells:
        ...
        grid_sl_full, _ = _build_sl_biased_grid(
            float(runtime_cfg.get("tbm_sl_as_tp_pct_min", 0.3))
        )
        ...
        n_phase1 = int(
            min(max(1, runtime_cfg.get("tbm_sl_shrink_phase1_trials", 30)), n_trials_s1)
        )
        ...
        obj_s1 = TBMObjective(
            ...
            grid_overrides={
                "k_tp": grid_k_tp,
                "tp_anchor": grid_tp_anchor,
                "sl_as_tp_pct": grid_sl_full,
            },
        ...
        if n_phase1 < n_trials_s1 and obj_s1.trial_results:
            ...
            _score_col = (
                "stage_score"
                if "stage_score" in phase1_df.columns
                else (
                    "stage2_score"
                    if "stage2_score" in phase1_df.columns
                    else "stage1_score"
                )
            )
            feasible_df = phase1_df[(phase1_df.get("hard_gate", False) == True)].copy()
            if (
                not feasible_df.empty
                and "sl_as_tp_pct" in feasible_df.columns
                and _score_col in feasible_df.columns
            ):
                m_all = float(
                    pd.to_numeric(feasible_df[_score_col], errors="coerce")
                    .dropna()
                    .median()
                )
                _grp_total = (
                    phase1_df.groupby("sl_as_tp_pct", observed=True)
                    .size()
                    .rename("n_total")
                    .reset_index()
                    if "sl_as_tp_pct" in phase1_df.columns
                    else pd.DataFrame(columns=["sl_as_tp_pct", "n_total"])
                )
                _grp = (
                    feasible_df.groupby("sl_as_tp_pct", observed=True)[_score_col]
                    .agg(["count", "median"])
                    .reset_index()
                )
                ...
                _grp["n_total"] = (
                    pd.to_numeric(_grp.get("n_total", 0), errors="coerce")
                    .fillna(0)
                    .astype(int)
                )
                _grp["removed"] = False
                ...
                _judge = (_grp["n_total"] >= n_total_min) | (
                    _grp["n_obs"] >= n_feas_min
                )
                bad_vals = set(
                    _grp.loc[_judge & (_grp["m_v"] < m_all), "sl_as_tp_pct"].tolist()
                )
                kept_vals = [v for v in sl_grid_phase2 if float(v) not in bad_vals]
                if len(kept_vals) < keep_min and not _grp.empty:
                    top_vals = _grp.sort_values(
                        ["m_v", "n_obs"], ascending=[False, False]
                    )["sl_as_tp_pct"].tolist()
                    kept_vals = sorted(set([float(v) for v in top_vals[:keep_min]]))
                sl_grid_phase2 = (
                    kept_vals if len(kept_vals) >= keep_min else sl_grid_phase2
                )
                _grp["removed"] = _grp["sl_as_tp_pct"].apply(
                    lambda x: bool(float(x) not in set(sl_grid_phase2))
                )
                    ...
                    sl_shrink_rows.append(
                        {
                            ...
                            "grid_after": json.dumps(
                                [float(x) for x in sl_grid_phase2]
                            ),
                        }
                    )
                tprint(
                    f"[sl_shrink] {bkt}_H{hor} phase1_trials={len(phase1_df)} sl_before={len(grid_sl_full)} sl_after={len(sl_grid_phase2)}"
                )
        ...
        if n_phase1 < n_trials_s1:
            ...
            study_s2.optimize(
                obj_s2, n_trials=(n_trials_s1 - n_phase1), n_jobs=_optuna_n_jobs()
            )
        ...
        _score_col = (
            "stage_score"
            if "stage_score" in _cell_df.columns
            else (
                "stage2_score" if "stage2_score" in _cell_df.columns else "stage1_score"
            )
        )
        _best_val = "N/A"
    ...
    out_df["_all_cells_pass"] = (out_df["pass_cells"] == out_df["total_cells"]).astype(
        int
    )
    out_df["_to_range_ok"] = (
        (out_df["timeout_range"].fillna(999.0) <= _MAX_TIMEOUT_RANGE).astype(int)
        if "timeout_range" in out_df.columns
        else 1
    )
    ...
    _cov_ok = (
        (out_df["coverage"] >= _PROMOTE_MIN_COVERAGE).astype(int)
        if "coverage" in out_df.columns
        else pd.Series(1, index=out_df.index)
    )
    _bind_ok = (
        (out_df["bind"] >= _PROMOTE_MIN_BIND).astype(int)
        if "bind" in out_df.columns
        else pd.Series(1, index=out_df.index)
    )
    _to_ok = (
        (out_df["timeout_rate"] <= _PROMOTE_MAX_TIMEOUT).astype(int)
        if "timeout_rate" in out_df.columns
        else pd.Series(1, index=out_df.index)
    )
    _sep_ok = (
        (out_df["min_cell_tp_sep"].fillna(0.0) >= _PROMOTE_MIN_TP_SEP).astype(int)
        if "min_cell_tp_sep" in out_df.columns
        else pd.Series(1, index=out_df.index)
    )
    _auc_col = (
        "min_cell_auc_bound"
        if "min_cell_auc_bound" in out_df.columns
        else ("min_cell_auc" if "min_cell_auc" in out_df.columns else None)
    )
    _auc_ok = (
        (out_df[_auc_col].fillna(0.0) >= _PROMOTE_MIN_AUC).astype(int)
        if _auc_col
        else pd.Series(1, index=out_df.index)
    )
    _ece_ok = (
        out_df.get(
            "min_cell_ece", out_df.get("ece", pd.Series(0.0, index=out_df.index))
        ).fillna(999.0)
        <= _PROMOTE_MAX_ECE
    ).astype(int)
    _brier_ok = (
        out_df.get(
            "min_cell_brier", out_df.get("brier", pd.Series(0.0, index=out_df.index))
        ).fillna(999.0)
        <= _PROMOTE_MAX_BRIER
    ).astype(int)
    _epos_ok = (
        out_df.get("effective_pos_mass", pd.Series(0.0, index=out_df.index)).fillna(0.0)
        >= _PROMOTE_MIN_EFFECTIVE_POS
    ).astype(int)
    _eneg_ok = (
        out_df.get("effective_neg_mass", pd.Series(0.0, index=out_df.index)).fillna(0.0)
        >= _PROMOTE_MIN_EFFECTIVE_NEG
    ).astype(int)
    out_df["_health_flags"] = (
        _cov_ok
        + _bind_ok
        + _to_ok
        + _sep_ok
    ...
    _min_auc_col = (
        (out_df[_min_auc_source].fillna(0.0) * 1000).round() / 1000
        if _min_auc_source in out_df.columns
        else pd.Series(0.0, index=out_df.index)
    )
    ...
    _min_auc_b_col = (
        out_df["min_cell_auc_bound"].fillna(0.0)
        if "min_cell_auc_bound" in out_df.columns
        else _min_auc_col
    )
    _score_s = pd.to_numeric(
        out_df.get("stage_score", pd.Series(np.nan, index=out_df.index)),
        errors="coerce",
    )
    _score_s2 = pd.to_numeric(
        out_df.get("stage2_score", pd.Series(np.nan, index=out_df.index)),
        errors="coerce",
    )
    _score_s1 = pd.to_numeric(
        out_df.get("stage1_score", pd.Series(-np.inf, index=out_df.index)),
        errors="coerce",
    ).fillna(-np.inf)
    _score_col = _score_s.where(
        _score_s.notna(), _score_s2.where(_score_s2.notna(), _score_s1)
    ).fillna(-np.inf)
    out_df = (
        out_df.assign(
    ...
    if sl_shrink_rows:
        pd.DataFrame(sl_shrink_rows).to_csv(
            output_path.with_name("tbm_sl_shrink_report.csv"), index=False
        )
    ...
    if weights_writer is not None:
        ...
        tprint(
            f"Saved sample weights (streaming): {weights_path} (total_rows={total_weights_written:,})"
        )
    ...
    if not out_df.empty:
        ...
        _to_r_ok = (
            out_df["timeout_range"].fillna(999.0) <= _MAX_TIMEOUT_RANGE
            if "timeout_range" in out_df.columns
            else pd.Series(True, index=out_df.index)
        )
        _base_mask = (
            (out_df["hard_gate"] == True)
            & _to_r_ok
        ...
        _grid_sl, _ = _build_sl_biased_grid(
            float(runtime_cfg.get("tbm_sl_as_tp_pct_min", 0.3))
        )
        _tuple_cols = [
            c for c in ("k_tp", "tp_anchor", "sl_as_tp_pct") if c in out_df.columns
        ]
        if len(_tuple_cols) < 3:
            _tuple_cols = [
                c
                for c in ("k_tp", "tp_base_pct", "sl_as_tp_pct")
                if c in out_df.columns
            ]
        _n_unique_tuples = (
            int(out_df.drop_duplicates(subset=_tuple_cols).shape[0])
            if _tuple_cols
            else int(len(out_df))
        )
        ...
        for _col in ("k_tp", "tp_anchor", "sl_as_tp_pct"):
            if _col in out_df.columns:
                ...
                tprint(
                    f"[grr_coverage] {_col}_counts={{{', '.join([f'{k}:{int(v)}' for k,v in _vc.items()])}}}"
                )

        _global_diverse_k = int(
            np.clip(int(runtime_cfg.get("tbm_global_diverse_k", 8)), 6, 10)
        )
        _diverse_pool = _diverse_subset(
            _candidate_set,
            details,
            min_distance=1.0,
            max_configs=_global_diverse_k,
        ...
        if _diverse_pool.empty and len(_best_pool) > 0:
            _pr_s = pd.to_numeric(
                _best_pool.get(
                    "pr_auc_lift", pd.Series(np.nan, index=_best_pool.index)
                ),
                errors="coerce",
            )
            _ed_s = pd.to_numeric(
                _best_pool.get("edge", pd.Series(np.nan, index=_best_pool.index)),
                errors="coerce",
            )
            tprint(
        ...
        try:
            _dist_cols = [
                c
                for c in ("k_tp", "tp_anchor", "sl_as_tp_pct")
                if c in _diverse_pool.columns
            ]
            if len(_dist_cols) >= 3 and len(_diverse_pool) >= 2:
                _x = (
                    _diverse_pool[_dist_cols]
                ...
                if _nn:
                    tprint(
                        f"[grr_coverage] winner_nearest_dist min={float(np.min(_nn)):.4f} mean={float(np.mean(_nn)):.4f}"
                    )
        except Exception:
            pass
        tprint(
            f"Global diverse pool: {len(_diverse_pool)} configs after diversity filter"
        )
        ...
        _score_col = (
            "stage_score"
            if "stage_score" in _best_source.columns
            else (
                "stage2_score"
                if "stage2_score" in _best_source.columns
                else "stage1_score"
            )
        )
        ...
        if isinstance(best_params, dict):
            ...
            save_best_params_csv(
                ...
                metadata={
                    ...
                    "config_id": best_cid,
                    "label": "best_stage1_not_authoritative",
                },
            )
            tprint(
        ...
        _grid_rows = []
        _fallback_window = (
            int(best_params.get("base_atr_window", 840))
            if isinstance(best_params, dict)
            else 840
        )
        for cell_key, cell_df in per_cell_grids.items():
            ...
            for rank_i, crow in enumerate(cell_df.itertuples(index=False), 1):
                ...
                if not isinstance(cfg_i, dict):
                    continue
                bh_i = (
                    _d_c.get("bucket_horizon_metrics", {})
                    if isinstance(_d_c, dict)
                    else {}
                )
                ...
                _grid_rows.append(
                    {
                        ...
                        "base_atr_window": int(
                            cfg_i.get("base_atr_window", _fallback_window)
                        ),
                        "tp_base_pct": float(cfg_i.get("tp_base_pct", float("nan"))),
                        "tp_abs_lo_pct": float(
                            cfg_i.get("tp_abs_lo_pct", float("nan"))
                        ),
                        "sl_abs_lo_pct": float(
                            cfg_i.get("sl_abs_lo_pct", float("nan"))
                        ),
                        "mode": str(cfg_i.get("mode", ""))
                        .split("_2A")[0]
                        .split("_refine")[0],
                        "stage2_score": float(
                            getattr(crow, "stage2_score", float("nan"))
                        ),
                        "stage1_score": float(
                            getattr(crow, "stage1_score", float("nan"))
                        ),
                        "probe_auc": float(getattr(crow, "probe_auc", float("nan"))),
                        "probe_auc_bound": float(
                            getattr(crow, "probe_auc_bound", float("nan"))
                        ),
                        "probe_status": str(getattr(crow, "probe_status", "")),
                        "probe_n_train": int(
                            _safe_float(getattr(crow, "probe_n_train", 0), 0)
                        ),
                        "probe_n_val": int(
                            _safe_float(getattr(crow, "probe_n_val", 0), 0)
                        ),
                        "cell_auc": float(cell_m_i.get("auc_label", float("nan"))),
                        "cell_auc_bound": float(
                            cell_m_i.get("auc_bound", float("nan"))
                        ),
                        "cell_tp_sep": float(
                            cell_m_i.get("tp_sep_top10", float("nan"))
                        ),
                        "cell_ap_lift": float(cell_m_i.get("ap_lift", float("nan"))),
                        "cell_tp_over_sl": float(
                            cell_m_i.get("tp_over_sl", float("nan"))
                        ),
                        "cell_sl_to_tp": float(cell_m_i.get("sl_to_tp", float("nan"))),
                        "cell_bind": float(cell_m_i.get("bind", float("nan"))),
                        "cell_timeout": float(
                            cell_m_i.get("timeout_kept", float("nan"))
                        ),
                        "cell_brier": float(cell_m_i.get("brier", float("nan"))),
                        "cell_ece": float(cell_m_i.get("ece", float("nan"))),
                        "cell_monotonicity": float(
                            cell_m_i.get("monotonicity", float("nan"))
                        ),
                        "cell_ic_std_time": float(
                            cell_m_i.get("ic_std_time", float("nan"))
                        ),
                        "cell_ic_std_asset": float(
                            cell_m_i.get("ic_std_asset", float("nan"))
                        ),
                        "cell_ic_payoff": float(
                            cell_m_i.get("ic_payoff", float("nan"))
                        ),
                        "cell_ic_label": float(cell_m_i.get("ic_label", float("nan"))),
                        "cell_barrier_ratio": float(
                            cell_m_i.get("barrier_ratio", float("nan"))
                        ),
        ...
        for _ck in _CELL_KEYS:
            ...
            tprint(
                f"[grid] Cell {_ck} missing from per_cell_grids — emitting fallback row from global best"
            )
            _grid_rows.append(
                {
                    "cell_key": _ck,
                    ...
                    "base_atr_window": int(
                        _fb_params.get("base_atr_window", _fallback_window)
                    ),
                    "tp_base_pct": float(_fb_params.get("tp_base_pct", float("nan"))),
                    "tp_abs_lo_pct": float(
                        _fb_params.get("tp_abs_lo_pct", float("nan"))
                    ),
                    "sl_abs_lo_pct": float(
                        _fb_params.get("sl_abs_lo_pct", float("nan"))
                    ),
                    "mode": str(_fb_params.get("mode", ""))
                    .split("_2A")[0]
                    .split("_refine")[0],
                    ...
                    "cell_timeout": float("nan"),
                    "cell_bind": float("nan"),
                    "cell_ap_lift": float("nan"),
                    "cell_tp_over_sl": float("nan"),
                    "cell_brier": float("nan"),
                    "cell_ece": float("nan"),
                    "cell_monotonicity": float("nan"),
                    "cell_ic_std_time": float("nan"),
                    "cell_ic_std_asset": float("nan"),
                    "cell_ic_payoff": float("nan"),
                    "cell_ic_label": float("nan"),
        ...
        if _grid_rows:
            ...
            _grid_df = _grid_df.sort_values("rank").drop_duplicates(
                subset=_param_cols, keep="first"
            )
            if len(_grid_df) < _n_grid_before:
                tprint(
                    f"[cleanup] Deduplicated geometry grid: {_n_grid_before} -> {len(_grid_df)} rows"
                )
            ...
            for _bkt in TBM_BUCKET_NAMES:
                ...
                for cell_key, cell_group in _bdf_valid.groupby("cell_key"):
                    _cell_stage2 = pd.to_numeric(
                        cell_group.get(
                            "stage2_score", pd.Series(np.nan, index=cell_group.index)
                        ),
                        errors="coerce",
                    )
                    _cell_aucb = pd.to_numeric(
                        cell_group.get(
                            "cell_auc_bound", pd.Series(np.nan, index=cell_group.index)
                        ),
                        errors="coerce",
                    ).fillna(0.0)
                    _cell_sep = pd.to_numeric(
                        cell_group.get(
                            "cell_tp_sep", pd.Series(np.nan, index=cell_group.index)
                        ),
                        errors="coerce",
                    ).fillna(0.0)
                    _cell_ece = pd.to_numeric(
                        cell_group.get(
                            "cell_ece", pd.Series(np.nan, index=cell_group.index)
                        ),
                        errors="coerce",
                    ).fillna(999.0)
                    _cell_brier = pd.to_numeric(
                        cell_group.get(
                            "cell_brier", pd.Series(np.nan, index=cell_group.index)
                        ),
                        errors="coerce",
                    ).fillna(999.0)
                    ...
                    for _rank_i, _row in _cell_ranked.iterrows():
                        ...
                        _cell_payload["cell_brier"] = float(
                            _row.get("cell_brier", float("nan"))
                        )
                        _cell_payload["cell_bind"] = float(
                            _row.get("cell_bind", float("nan"))
                        )
                        _cell_payload["cell_timeout"] = float(
                            _row.get("cell_timeout", float("nan"))
                        )
                        _cell_payload["cell_ic_std_time"] = float(
                            _row.get("cell_ic_std_time", float("nan"))
                        )
                        _cell_payload["cell_ic_std_asset"] = float(
                            _row.get("cell_ic_std_asset", float("nan"))
                        )
                        _cell_payload["stage2_score"] = float(
                            _row.get("stage2_score", float("nan"))
                        )
                        _cell_payload["cell_score"] = float(_row["_score"])
                        per_cell_rows.append(_cell_payload)

                _bdf_agg["_score"] = pd.to_numeric(
                    _bdf_agg.get(
                        "stage2_score", pd.Series(np.nan, index=_bdf_agg.index)
                    ),
                    errors="coerce",
                ).fillna(-np.inf)
                ...
                _bkt_row: Dict[str, Any] = {
                    ...
                    "tp_abs_lo_pct": float(
                        _bkt_winner.get("tp_abs_lo_pct", float("nan"))
                    ),
                    "sl_abs_lo_pct": float(
                        _bkt_winner.get("sl_abs_lo_pct", float("nan"))
                    ),
                    "tp_abs_hi_pct": float(_bkt_cfg.get("tp_abs_hi_pct", float("nan")))
                    if isinstance(_bkt_cfg, dict)
                    else float("nan"),
                    "sl_abs_hi_pct": float(_bkt_cfg.get("sl_abs_hi_pct", float("nan")))
                    if isinstance(_bkt_cfg, dict)
                    else float("nan"),
                    "tp_method": str(_bkt_cfg.get("tp_method", "atr_norm"))
                    if isinstance(_bkt_cfg, dict)
                    else "atr_norm",
                    "sl_method": str(_bkt_cfg.get("sl_method", "tp_pct"))
                    if isinstance(_bkt_cfg, dict)
                    else "tp_pct",
                    "mode": str(_bkt_winner.get("mode", ""))
                    .split("_2A")[0]
                    .split("_refine")[0],
                    "horizon_base": int(_bkt_cfg.get("horizon_base", 4))
                    if isinstance(_bkt_cfg, dict)
                    else 4,
                    "horizon_scaling": str(_bkt_cfg.get("horizon_scaling", "sqrt"))
                    if isinstance(_bkt_cfg, dict)
                    else "sqrt",
    ...
    if not out_df.empty:
        _print_winning_geometry_summary(
            out_df, details, per_cell_grids=per_cell_grids, top_k=5
        )
...
def _build_prod_aligned_reports(
    ...
    for r in out_df.itertuples(index=False):
        ...
        cfg_meta = (
            d.get("prod_aligned_tp", _r.get("prod_aligned_tp", {}))
            if isinstance(d, dict)
            else {}
        )

        targets = (
            cfg_meta.get("tp_eff_targets", {}) if isinstance(cfg_meta, dict) else {}
        )
        ...
        rows_trade.append(
            {
                ...
                "tp_eff_p50_prod": float(
                    agg.get("tp_eff_p50_prod", _r.get("tp_eff_p50_prod", float("nan")))
                ),
                "tp_eff_p75_prod": float(
                    agg.get("tp_eff_p75_prod", _r.get("tp_eff_p75_prod", float("nan")))
                ),
                "tp_eff_p90_prod": float(
                    agg.get("tp_eff_p90_prod", _r.get("tp_eff_p90_prod", float("nan")))
                ),
                "tp_eff_tradeable_rule_p50": bool(
                    agg.get(
                        "tp_eff_tradeable_rule_p50",
                        _r.get("tp_eff_tradeable_rule_p50", False),
                    )
                ),
                "tp_eff_tradeable_rule_tail": bool(
                    agg.get(
                        "tp_eff_tradeable_rule_tail",
                        _r.get("tp_eff_tradeable_rule_tail", False),
                    )
                ),
                "tp_eff_tradeable_ok": bool(
                    agg.get("tp_eff_tradeable_ok", _r.get("tp_eff_tradeable_ok", False))
                ),
                "tp_floor_bind_prod_agg": float(
                    _r.get("tp_floor_bind_prod_agg", float("nan"))
                ),
                "max_cell_tp_floor_bind_prod": float(
                    _r.get("max_cell_tp_floor_bind_prod", float("nan"))
                ),
                "tp_hit_agg": float(_r.get("tp_hit_rate", float("nan"))),
                "sl_to_tp_agg": float(_r.get("sl_to_tp", float("nan"))),
                "tp_over_sl_median_cell": float(
                    _r.get("median_cell_tp_over_sl", float("nan"))
                ),
                ...
                "missing_cells": int(
                    max(
                        0,
                        int(_safe_float(_r.get("total_cells", 0)))
                        - int(_safe_float(_r.get("pass_cells", 0))),
                    )
                ),
            }
        )
        ...
        if not bool(_r.get("prod_admissible_tier0", False)):
            if fails:
                for f in fails:
                    rows_rej.append(
                        {"config_id": cid, "bucket": bucket, "reason": str(f)}
                    )
            else:
                rows_rej.append(
                    {
                        "config_id": cid,
                        "bucket": bucket,
                        "reason": "unknown_prod_admissibility_failure",
                    }
                )
    ...
    if rows_div:
        ...
        grp = (
            div_df.groupby("bucket", observed=True)
            .agg(
                unique_tp_base_pct=(
                    "tp_base_pct",
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--features",
        default=None,
        help="Path to features directory (auto-detected from data_root if not set)",
    )
    p.add_argument(
        "--panel",
        default=None,
    ...
    p.add_argument(
        "--stage2", dest="stage2", action="store_true", help="Run stage2 validation"
    )
    p.add_argument(
        "--no-stage2",
        dest="stage2",
        action="store_false",
        help="Disable stage2 validation",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=24,
        help="Max configs per Stage2 substage (hierarchical cap)",
    )
    p.add_argument(
        "--lookback-years", type=int, default=2, help="Years of history to keep"
    )
    p.add_argument(
        "--weights-output",
        default="",
        help="Optional sample-weights parquet output path",
    )
    p.add_argument(
        "--tbm-cache-max-mb",
        type=int,
