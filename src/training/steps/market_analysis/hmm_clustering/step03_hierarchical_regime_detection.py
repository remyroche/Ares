...
class HierarchicalRegimeDetector:
    ...
    def _detect_regimes_at_timeframe(self, data: pd.DataFrame, timeframe: str) -> Dict[str, Any]:
        ...
        if len(resampled_data) < 100:
            return {'regimes': np.zeros(len(resampled_data)), 'n_regimes': 1, 'confidence': 0.0, 'timeframe': timeframe}
    ...
    def _extract_timeframe_features(self, data: pd.DataFrame, timeframe: str) -> np.ndarray:
        ...
        features.extend([returns.mean(), returns.std(), returns.skew(), returns.kurtosis(), returns.rolling(20).std().mean()])
        volume = data['volume']
