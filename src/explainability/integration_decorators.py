...
class ExplainabilityIntegration:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Model registry for automatic registration
        self.model_registry = {}
        
    def explainable_prediction(
        self,
        model_type: str,
        model_name: str = "main",
        feature_extractor: Optional[Callable] = None,
        decision_tracer: bool = True
    ):
            ...
            async def wrapper(*args, **kwargs):
                try:
                    ...
                    if feature_extractor:
                        features, feature_names = await self._extract_features(
                            feature_extractor, *args, **kwargs
                        )
                    
                    # Get prediction
                    prediction = await func(*args, **kwargs)
                    
                    # Generate explanation if features are available
                    explanation = None
                    if features is not None and feature_names is not None:
                        explanation = await self.orchestrator.explain_model_prediction(
                            model_type, model_name, features, feature_names, prediction
                        )
                    
                    # Add explanation to result if available
                    if explanation:
            ...
            async def wrapper(*args, **kwargs):
                try:
                    ...
                    if self.auto_trace_decisions:
                        trace = await self.orchestrator.start_trade_decision_trace(
                    ...
                    # Add explanations to trace
                    if trace and model_features:
                        for model_type, (features, feature_names) in model_features.items():
                            explanation = await self.orchestrator.explain_model_prediction(
                                model_type, "main", features, feature_names
                            )
                            if explanation:
                                await self.orchestrator.add_explanation_to_trace(
                    ...
                    if trace:
                        ...
                        final_trace = await self.orchestrator.finalize_trade_decision_trace(
                            decision_id, decision, confidence
                        )
                        
                        # Add trace to decision result
    ...
    def auto_register_model(
        ...
        def decorator(func):
            ...
            async def wrapper(*args, **kwargs):
                try:
                    # Get model instance (assume it's the first argument or 'self')
                    model = None
                    ...
                    if model and self.auto_register_models:
                        ...
                        if model_key not in self.model_registry:
                            await self.orchestrator.register_model(
        ...
        **kwargs
        ...
        try:
            ...
            if isinstance(result, tuple) and len(result) == 2:
                ...
                # Assume dict contains 'features' and 'feature_names'
    ...
    def _extract_confidence(self, decision: Any) -> float:
        ...
        try:
            if isinstance(decision, dict):
                # Look for confidence in various keys
...
def explainable_tactician_prediction(model_name: str = "main", feature_extractor: Optional[Callable] = None):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            explainable_func = integration.explainable_prediction(
                'tactician', model_name, feature_extractor
            )(func)
...
def explainable_hmm_prediction(model_name: str = "main", feature_extractor: Optional[Callable] = None):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            integration = get_explainability_integration(config)
            explainable_func = integration.explainable_prediction(
                'hmm', model_name, feature_extractor
            )(func)
            return await explainable_func(*args, **kwargs)
        return wrapper
    return decorator
...
def explainable_sr_prediction(model_name: str = "main", feature_extractor: Optional[Callable] = None):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            integration = get_explainability_integration(config)
            explainable_func = integration.explainable_prediction(
                'sr', model_name, feature_extractor
            )(func)
            return await explainable_func(*args, **kwargs)
        return wrapper
    return decorator
...
def explainable_analyst_prediction(model_name: str = "main", feature_extractor: Optional[Callable] = None):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            integration = get_explainability_integration(config)
            explainable_func = integration.explainable_prediction(
                'analyst', model_name, feature_extractor
            )(func)
            return await explainable_func(*args, **kwargs)
        return wrapper
    return decorator
...
def explainable_trading_decision(
    decision_type: str,
    model_types: List[str] = None,
    feature_extractors: Optional[Dict[str, Callable]] = None
):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            integration = get_explainability_integration(config)
            explainable_func = integration.explainable_decision(
                decision_type, model_types, feature_extractors
            )(func)
            return await explainable_func(*args, **kwargs)
        return wrapper
    return decorator
...
class FeatureExtractor:
    ...
    @staticmethod
    def from_dataframe(columns: List[str], preprocessing: Optional[Callable] = None):
    ...
    @staticmethod
    def from_dict(key_mapping: Dict[str, str]):
        """Create feature extractor from dictionary."""
        async def extractor(*args, **kwargs):
            try:
                # Find dict in arguments
                data_dict = None
                for arg in args:
    ...
    @staticmethod
    def custom(extractor_func: Callable):
