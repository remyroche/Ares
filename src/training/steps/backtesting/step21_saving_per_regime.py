...
class PerRegimeSavingStep(Step21Saving):
    ...
    async def execute_per_regime_saving(self, symbol: str, exchange: str, timeframe: str, data_dir: str, force_rerun: bool = False, regime_id: Optional[int]=None, regime_context: Optional[Any]=None, per_regime: bool = True) -> bool:
        ...
        try:
            ...
            saving_results = await self._apply_regime_saving(all_regime_results, regime_config, regime_id)
            if saving_results is None:
                self.logger.error(f'❌ Failed saving for regime {regime_id}')
                return False
            success = await self._save_regime_saving_results(saving_results, symbol, exchange, timeframe, data_dir, regime_id)
            if success:
    ...
    async def _load_all_regime_results(self, symbol: str, exchange: str, timeframe: str, data_dir: str, regime_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            all_results = {}
            step_files = ['feature_engineering', 'matrix_operations', 'feature_selection', 'hmm_training', 'regime_intelligence', 'analyst_creation', 'parameters_optimization']
    ...
    def _get_regime_saving_config(self, regime_id: int) -> Dict[str, Any]:
        ...
        base_config = {'save_models': True, 'save_parameters': True, 'save_metadata': True, 'save_performance_metrics': True, 'save_analyst_data': True, 'save_intelligence_data': True, 'save_optimization_results': True, 'create_regime_summary': True}
        if regime_id <= 2:
            return {**base_config, 'saving_strategy': {'emphasis': 'trend_data', 'compression_level': 'medium', 'include_visualizations': True}, 'saving_parameters': {'model_format': 'pickle', 'metadata_format': 'json', 'performance_format': 'json', 'include_feature_importance': True, 'include_trend_analysis': True}, 'file_organization': {'create_regime_subfolder': True, 'separate_model_files': True, 'include_timestamp': True}}
        elif regime_id >= 5:
            return {**base_config, 'saving_strategy': {'emphasis': 'volatility_data', 'compression_level': 'high', 'include_visualizations': True}, 'saving_parameters': {'model_format': 'pickle', 'metadata_format': 'json', 'performance_format': 'json', 'include_risk_metrics': True, 'include_volatility_analysis': True}, 'file_organization': {'create_regime_subfolder': True, 'separate_model_files': True, 'include_timestamp': True}}
        else:
            return {**base_config, 'saving_strategy': {'emphasis': 'balanced_data', 'compression_level': 'medium', 'include_visualizations': True}, 'saving_parameters': {'model_format': 'pickle', 'metadata_format': 'json', 'performance_format': 'json', 'include_balanced_analysis': True, 'include_ensemble_data': True}, 'file_organization': {'create_regime_subfolder': True, 'separate_model_files': True, 'include_timestamp': True}}

    async def _apply_regime_saving(self, all_regime_results: Dict[str, Any], regime_config: Dict[str, Any], regime_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            results = {'regime_id': regime_id, 'saving_strategy': regime_config.get('saving_strategy', {}), 'saving_parameters': regime_config.get('saving_parameters', {}), 'saved_components': {}, 'saving_metadata': {}, 'file_paths': {}}
            if regime_config.get('save_models', True):
                model_results = await self._save_regime_models(all_regime_results, regime_config, regime_id)
                if model_results:
                    results['saved_components']['models'] = model_results
            if regime_config.get('save_parameters', True):
                parameter_results = await self._save_regime_parameters(all_regime_results, regime_config, regime_id)
                if parameter_results:
                    results['saved_components']['parameters'] = parameter_results
            if regime_config.get('save_metadata', True):
                metadata_results = await self._save_regime_metadata(all_regime_results, regime_config, regime_id)
                if metadata_results:
                    results['saved_components']['metadata'] = metadata_results
            if regime_config.get('save_performance_metrics', True):
                performance_results = await self._save_regime_performance_metrics(all_regime_results, regime_config, regime_id)
                if performance_results:
                    results['saved_components']['performance_metrics'] = performance_results
            if regime_config.get('save_analyst_data', True):
                analyst_results = await self._save_regime_analyst_data(all_regime_results, regime_config, regime_id)
                if analyst_results:
                    results['saved_components']['analyst_data'] = analyst_results
            if regime_config.get('save_intelligence_data', True):
                intelligence_results = await self._save_regime_intelligence_data(all_regime_results, regime_config, regime_id)
            ...
            if regime_config.get('save_optimization_results', True):
                optimization_results = await self._save_regime_optimization_results(all_regime_results, regime_config, regime_id)
                if optimization_results:
                    results['saved_components']['optimization_results'] = optimization_results
            if regime_config.get('create_regime_summary', True):
                summary_results = await self._create_regime_summary(all_regime_results, regime_config, regime_id)
                if summary_results:
    ...
    async def _save_regime_models(self, all_regime_results: Dict[str, Any], regime_config: Dict[str, Any], regime_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not models:
                ...
                return None
            model_results = {'saved_models': list(models.keys()), 'model_count': len(models), 'model_types': [model_data.get('model_type', 'unknown') for model_data in models.values()], 'model_performances': {name: model_data.get('accuracy', 0.0) for name, model_data in models.items()}, 'saving_timestamp': datetime.now().isoformat()}
    ...
    async def _save_regime_parameters(self, all_regime_results: Dict[str, Any], regime_config: Dict[str, Any], regime_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not optimization_results:
                ...
                return None
            parameter_results = {'optimized_parameters': {}, 'parameter_count': 0, 'optimization_types': list(optimization_results.keys()), 'saving_timestamp': datetime.now().isoformat()}
    ...
    async def _save_regime_metadata(self, all_regime_results: Dict[str, Any], regime_config: Dict[str, Any], regime_id: int) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            metadata_results = {'regime_id': regime_id, 'regime_characteristics': self._extract_regime_characteristics(regime_id), 'processing_timeline': self._extract_processing_timeline(all_regime_results), 'data_statistics': self._extract_data_statistics(all_regime_results), 'component_versions': self._extract_component_versions(), 'saving_timestamp': datetime.now().isoformat()}
    ...
    def _extract_regime_characteristics(self, regime_id: int) -> Dict[str, Any]:
        ...
        """
        if regime_id <= 2:
            return {'market_type': 'trending', 'volatility_level': 'low', 'trend_strength': 'high', 'mean_reversion': 'low'}
        elif regime_id >= 5:
    ...
    def _extract_data_statistics(self, all_regime_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            Data statistics
        """
        statistics = {'total_steps': len(all_regime_results), 'step_names': list(all_regime_results.keys()), 'data_sizes': {}}
        for step_name, step_results in all_regime_results.items():
            if isinstance(step_results, dict):
    ...
    def _extract_component_versions(self) -> Dict[str, str]:
        ...
        Returns:
            Component versions
        """
        return {'per_regime_saving': '1.0.0', 'regime_continuity_manager': '1.0.0', 'pipeline_standards': '1.0.0', 'saving_timestamp': datetime.now().isoformat()}

    async def _save_regime_performance_metrics(self, all_regime_results: Dict[str, Any], regime_config: Dict[str, Any], regime_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not performance_metrics:
                ...
                return None
            performance_results = {'performance_metrics': performance_metrics, 'step_count': len(performance_metrics), 'overall_performance': self._calculate_overall_performance(performance_metrics), 'saving_timestamp': datetime.now().isoformat()}
    ...
    def _calculate_overall_performance(self, performance_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            overall_performance = {'total_steps': len(performance_metrics), 'step_performances': {}, 'average_performance': 0.0}
            all_scores = []
    ...
    async def _save_regime_analyst_data(self, all_regime_results: Dict[str, Any], regime_config: Dict[str, Any], regime_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not created_analysts:
                ...
                return None
            analyst_results = {'created_analysts': list(created_analysts.keys()), 'analyst_count': len(created_analysts), 'analyst_types': [analyst.get('analyst_type', 'unknown') for analyst in created_analysts.values()], 'analyst_specializations': [analyst.get('specialization', 'unknown') for analyst in created_analysts.values()], 'saving_timestamp': datetime.now().isoformat()}
    ...
    async def _save_regime_intelligence_data(self, all_regime_results: Dict[str, Any], regime_config: Dict[str, Any], regime_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not intelligence_components:
                ...
                return None
            intelligence_results = {'intelligence_components': list(intelligence_components.keys()), 'component_count': len(intelligence_components), 'intelligence_strategy': regime_intelligence.get('intelligence_strategy', {}), 'performance_metrics': regime_intelligence.get('performance_metrics', {}), 'saving_timestamp': datetime.now().isoformat()}
    ...
    async def _save_regime_optimization_results(self, all_regime_results: Dict[str, Any], regime_config: Dict[str, Any], regime_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not optimization_results:
                self.logger.warning(f'⚠️ No optimization results found for regime {regime_id}')
                return None
            optimization_saving_results = {'optimization_types': list(optimization_results.keys()), 'optimization_count': len(optimization_results), 'optimization_strategy': parameters_optimization.get('optimization_strategy', {}), 'performance_metrics': parameters_optimization.get('performance_metrics', {}), 'saving_timestamp': datetime.now().isoformat()}
    ...
    async def _create_regime_summary(self, all_regime_results: Dict[str, Any], regime_config: Dict[str, Any], regime_id: int) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            regime_summary = {'regime_id': regime_id, 'regime_characteristics': self._extract_regime_characteristics(regime_id), 'processing_summary': {'total_steps_processed': len(all_regime_results), 'steps_completed': list(all_regime_results.keys()), 'processing_status': 'completed'}, 'performance_summary': self._create_performance_summary(all_regime_results), 'component_summary': self._create_component_summary(all_regime_results), 'recommendations': self._generate_regime_recommendations(regime_id, all_regime_results), 'created_at': datetime.now().isoformat()}
    ...
    def _create_performance_summary(self, all_regime_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            performance_summary = {'overall_performance': 0.0, 'step_performances': {}, 'best_performing_step': None, 'worst_performing_step': None}
            step_scores = {}
            for step_name, step_results in all_regime_results.items():
                if isinstance(step_results, dict):
                    scores = []
    ...
    def _create_component_summary(self, all_regime_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            component_summary = {'total_components': len(all_regime_results), 'component_types': {}, 'data_sizes': {}}
            for step_name, step_results in all_regime_results.items():
    ...
    def _generate_regime_recommendations(self, regime_id: int, all_regime_results: Dict[str, Any]) -> List[str]:
        ...
        try:
            recommendations = []
            if regime_id <= 2:
                recommendations.extend(['This is a trending regime - focus on trend-following strategies', 'Consider longer lookback periods for trend analysis', 'Monitor for trend reversals and adjust accordingly'])
            elif regime_id >= 5:
                recommendations.extend(['This is a volatile regime - use conservative position sizing', 'Focus on mean-reversion strategies', 'Implement strict risk management controls'])
            else:
                recommendations.extend(['This is a balanced regime - use adaptive strategies', 'Monitor regime transitions closely', 'Balance trend-following and mean-reversion approaches'])
    ...
    def _calculate_saving_metadata(self, saved_components: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            return {'total_components_saved': len(saved_components), 'component_types': list(saved_components.keys()), 'saving_timestamp': datetime.now().isoformat(), 'saving_status': 'completed'}
        except Exception as e:
            self.logger.error(f'❌ Error calculating saving metadata: {e}')
            return {'saving_status': 'error'}

    async def _save_regime_saving_results(self, saving_results: Dict[str, Any], symbol: str, exchange: str, timeframe: str, data_dir: str, regime_id: int) -> bool:
