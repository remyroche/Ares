...
class LinearConfidenceScaler:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Load confidence scaling parameters
    ...
    def calculate_linear_confidence_multiplier(
        ...
        # If confidence is below minimum threshold, use minimum multiplier
        if confidence < self.confidence_min_threshold:
            ...
            normalized_confidence = (confidence - self.confidence_min_threshold) / (
                self.confidence_max_threshold - self.confidence_min_threshold
            )
            base_multiplier = (
                self.confidence_min_multiplier + 
                (self.confidence_max_multiplier - self.confidence_min_multiplier) * 
    ...
    def _calculate_intensity_multiplier(self, intensity: float) -> float:
        ...
        # Linear scaling from low to high intensity
        if intensity < 0.5:
            # Below average intensity - reduce multiplier
            intensity_multiplier = self.low_intensity_reduction + (1.0 - self.low_intensity_reduction) * (intensity / 0.5)
        else:
            # Above average intensity - boost multiplier
    ...
    def _calculate_reliability_multiplier(self, reliability: float) -> float:
        ...
        # Reliability acts as a confidence boost/reduction factor
        # High reliability (0.8+) gets slight boost, low reliability (0.5-) gets reduction
        if reliability >= 0.8:
            reliability_multiplier = 1.0 + (reliability - 0.8) * 0.5  # Up to 1.1x boost
        elif reliability <= 0.5:
            reliability_multiplier = 0.7 + (reliability / 0.5) * 0.3  # Down to 0.7x reduction
        else:
            reliability_multiplier = 1.0  # Neutral for medium reliability
    ...
    def calculate_position_size_multiplier(
        ...
        """
        # Get base confidence multiplier
        confidence_multiplier = self.calculate_linear_confidence_multiplier(
            confidence, intensity, reliability
        )
        ...
        # Apply intensity position multiplier
    ...
    def should_enter_trade(
        ...
        # Check profit confidence threshold
        ...
        intensity_acceptable = intensity >= 0.3  # Minimum intensity threshold
        
        # All conditions must be met
        should_enter = all([
            min_confidence_met,
            profit_confidence_met,
            risk_acceptable,
            intensity_acceptable
        ])
        
        reasoning = {
            "should_enter": should_enter,
            "confidence_met": min_confidence_met,
            "profit_confidence_met": profit_confidence_met,
            "risk_acceptable": risk_acceptable,
            "intensity_acceptable": intensity_acceptable,
            "confidence_score": confidence,
            "profit_confidence_score": profit_confidence,
            "risk_score": risk_score,
            "intensity_score": intensity,
            "confidence_multiplier": self.calculate_linear_confidence_multiplier(confidence, intensity),
            "position_multiplier": self.calculate_position_size_multiplier(confidence, intensity, 1.0, risk_score),
            "leverage_multiplier": self.calculate_leverage_multiplier(confidence, intensity, 1.0, risk_score)
        }
