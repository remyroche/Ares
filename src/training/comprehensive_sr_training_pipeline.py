...
class ComprehensiveSRTrainingPipeline:
    ...
    async def execute_comprehensive_training(self, training_data: pd.DataFrame, symbol: str, exchange: str, timeframe: str) -> dict[str, Any]:
        ...
        try:
            ...
            training_results = await self._train_models_with_comprehensive_features(comprehensive_data, symbol, exchange, timeframe)
            ...
            return {'success': True, 'execution_time': execution_time.total_seconds(), 'step7_features_loaded': step7_success, 'step2_5_sr_levels_loaded': step2_5_success, 'training_results': training_results, 'validation_results': validation_results, 'comprehensive_data_info': {'total_features': len(comprehensive_data.columns), 'sr_features': len([col for col in comprehensive_data.columns if 'sr_' in col.lower()]), 'data_shape': comprehensive_data.shape}}
    ...
    async def _prepare_comprehensive_training_data(self, training_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.exception(f'❌ Error preparing comprehensive training data: {e}')
            return training_data

    async def _train_models_with_comprehensive_features(self, comprehensive_data: pd.DataFrame, symbol: str, exchange: str, timeframe: str) -> dict[str, Any]:
        ...
        try:
            ...
            if direction_column not in comprehensive_data.columns:
                ...
                comprehensive_data[direction_column] = 1
            ...
            for model_type in model_types:
                try:
                    ...
                    model_result = await self.model_trainer.train_multi_output_model(features, direction_target, profit_target, f'{model_type}_{symbol}')
                    if model_result:
            ...
            self.logger.exception(f'❌ Error training models with comprehensive features: {e}')
            return {}
    ...
    async def _validate_and_save_results(self, training_results: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            validation_results = {'models_trained': len(training_results), 'model_types': list(training_results.keys()), 'sr_feature_analysis': {}, 'feature_completeness': {}, 'model_performance': {}}
    ...
    def get_comprehensive_feature_summary(self) -> dict[str, Any]:
        ...
        try:
            return {'step7_features': {'loaded': self.step7_features_loaded, 'count': len(self.model_trainer.step7_features), 'features': self.model_trainer.step7_features}, 'step2_5_sr_levels': {'loaded': self.step2_5_sr_levels_loaded, 'support_levels': len(self.model_trainer.step2_5_sr_levels.get('support_levels', [])), 'resistance_levels': len(self.model_trainer.step2_5_sr_levels.get('resistance_levels', []))}, 'sr_feature_columns': {'count': len(self.model_trainer.sr_feature_columns), 'columns': self.model_trainer.sr_feature_columns}}
        except Exception as e:
