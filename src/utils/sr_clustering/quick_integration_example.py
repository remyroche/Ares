...
def demonstrate_quick_integration():
    ...
    tprint(f"✅ Market data: {len(market_data)} rows")
    tprint(f"✅ Existing SR levels: {len(existing_sr_levels)} levels")
    ...
    if SR_CLUSTERING_AVAILABLE:
        ...
        tprint(f"✅ Enhanced SR levels: {len(enhanced_sr_levels.clusters)} clusters")
        ...
        if training_result.get('status') == 'success':
            ...
            tprint("✅ SR quality predictions generated")
            ...
            if optimized_weights:
                ...
                tprint(f"   Optimized weights: {optimized_weights}")
            else:
            ...
            tprint("⚠️  SR quality prediction failed, using default weights")
    else:
    ...
    if SR_CLUSTERING_AVAILABLE:
        ...
        if len(enhanced_data) > 0:
            ...
            tprint(f"✅ Enhanced training data: {len(enhanced_data)} samples")
            ...
            if available_features:
                tprint(enhanced_data[available_features].head())
    ...
    if SR_CLUSTERING_AVAILABLE and len(enhanced_data) > 0:
        ...
        if training_result.get('status') == 'success':
            ...
            tprint("✅ ML models trained successfully")
            ...
            tprint(f"   Classification Accuracy: {accuracy:.3f}")
            tprint(f"   Regression R²: {r2_score:.3f}")
            ...
            tprint(f"❌ ML model training failed: {error_msg}")
    else:
    ...
    if SR_CLUSTERING_AVAILABLE:
        ...
        tprint("✅ Integration successful!")
        tprint("   • SR clustering system integrated")
        tprint("   • Weight optimization enabled")
        tprint("   • Enhanced training data created")
        tprint("   • ML models trained with SR quality features")
        tprint("   • Trading signals generated")
        
        # Show what would be passed to next steps
        tprint(f"\n🎯 Enhanced output for next steps:")
        tprint(f"   • Enhanced SR levels with quality scores")
        tprint(f"   • Enhanced training data with SR features")
        tprint(f"   • Trained ML models with SR quality integration")
        tprint(f"   • Trading signals with confidence scores")
        tprint(f"   • Optimized weights for continuous learning")
        ...
        tprint("❌ Integration failed - SR clustering not available")
...
def show_integration_code_changes():
    ...
    tprint("📝 File: src/training/steps/data_collection/data_preparation/step02_5_sr_optimization.py")
