...
class TrendReporter:
    ...
    def generate_trend_report(self, project_name: str = "default",
        ...
        if project_name not in history:
            msg = f"No history found for project: {project_name}"
        ...
        if not filtered_points:
            msg = f"No data points found in the last {days} days"
            raise ValueError(msg)
    ...
    def _analyze_trends(self, data_points: list[TrendPoint]) -> TrendAnalysis:
        ...
        # Analyze each metric
        metric_trends = {}
        for metric_name in data_points[0].metrics:
    ...
    def clear_history(self, project_name: str | None = None) -> None:
        """
