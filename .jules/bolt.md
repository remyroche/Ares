...
## 2026-06-18 - Vectorized DataFrame Updates with Pandas
Learning: Using `.iterrows()` along with `.loc` or `pd.concat` to update or append rows in a Pandas DataFrame scales terribly ((N^2)$) and causes huge bottlenecks during inference, where prediction ledgers may contain tens of thousands of rows.
Action: Completely replace row-wise update loops with vectorized index logic. For large sets of updates, deduplicate keys via `upd_idx[~upd_idx.index.duplicated(keep='last')]`, calculate the set of matching indices using `index.intersection()`, use bulk `.loc` for assigning values, and use a single bulk `pd.concat()` for new rows to drop execution times by up to 60x. Pre-fill any missing columns dynamically using `pd.NA` to ensure dtype safety.
