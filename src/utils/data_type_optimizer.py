...
def optimize_dataframe_dtypes(df: pd.DataFrame, 
    ...
    logger.info(f"🔧 Optimizing data types - Initial memory: {initial_memory / 1024**2:.2f} MB")
    ...
    for col in df.columns:
        if df[col].dtype == 'object':
            if df[col].isin([True, False, 1, 0, 'True', 'False', '1', '0']).all():
                optimized_df[col] = df[col].map({'True': True, 'False': False, '1': True, '0': False, 1: True, 0: False}).astype('bool')
    
    final_memory = optimized_df.memory_usage(deep=True).sum()
    memory_reduction = (initial_memory - final_memory) / initial_memory
    
    logger.info(f"🔧 Data type optimization complete:")
...
def optimize_feature_engineering_pipeline(df: pd.DataFrame, 
                                        stage: str = "input") -> pd.DataFrame:
    """
    ...
    if stage == "input":
        ...
        return optimize_dataframe_dtypes(df, target_memory_reduction=0.3, preserve_categorical=True)
    
    elif stage == "intermediate":
        # For intermediate calculations, be more aggressive
        return optimize_dataframe_dtypes(df, target_memory_reduction=0.6, preserve_categorical=False)
