...
class TradeTracker:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.enable_feature_importance_tracking = self.tracking_config.get('enable_feature_importance_tracking', True)
        self.enable_decision_path_tracking = self.tracking_config.get('enable_decision_path_tracking', True)
        self.enable_model_behavior_tracking = self.tracking_config.get('enable_model_behavior_tracking', True)
                ...
                for prediction in trade.ensemble_decision.individual_predictions:
                    ...
                    for feature_imp in prediction.feature_importance:
                        ...
                        if regime and feature_imp.regime != regime:
                            continue
                        feature_importance_data.append({'trade_id': trade.trade_id, 'timestamp': trade.timestamp, 'model_type': feature_imp.model_type, 'timeframe': feature_imp.timeframe, 'regime': feature_imp.regime, 'feature_name': feature_imp.feature_name, 'importance_score': feature_imp.importance_score, 'importance_rank': feature_imp.importance_rank})
        if not feature_importance_data:
            return {}
        df = pd.DataFrame(feature_importance_data)
        analysis = {'total_features_tracked': len(df['feature_name'].unique()), 'top_features': df.groupby('feature_name')['importance_score'].mean().nlargest(10).to_dict(), 'feature_stability': df.groupby('feature_name')['importance_score'].std().to_dict(), 'model_performance_by_feature': {}}
        for model in df['model_type'].unique():
            model_data = df[df['model_type'] == model]
            analysis['model_performance_by_feature'][model] = {'top_features': model_data.groupby('feature_name')['importance_score'].mean().nlargest(5).to_dict(), 'feature_count': len(model_data['feature_name'].unique())}
        return analysis
    ...
    def get_decision_path_analysis(self) -> dict[str, Any]:
        ...
        if not decision_paths:
            return {}
        analysis = {'total_decisions': len(decision_paths), 'common_decision_steps': {}, 'decision_weights_distribution': {}, 'decision_thresholds_analysis': {}}
        all_steps = []
        ...
        if all_weights:
            analysis['decision_weights_distribution'] = {'mean': np.mean(all_weights), 'std': np.std(all_weights), 'min': np.min(all_weights), 'max': np.max(all_weights)}
        return analysis
    ...
    def get_regime_analysis_summary(self) -> dict[str, Any]:
        ...
        if not regime_data:
            return {}
        analysis = {'total_regime_analyses': len(regime_data), 'regime_distribution': {}, 'regime_confidence_stats': {}, 'regime_transition_analysis': {}}
        ...
        analysis['regime_confidence_stats'] = {'mean': np.mean(confidences), 'std': np.std(confidences), 'min': np.min(confidences), 'max': np.max(confidences)}
