...
class ErrorPreventionSystem:
    ...
    def _get_prevention_recommendation(self, category: str, pattern: str) -> str:
        ...
        recommendations = {
            'data_quality': {
                'duplicate timestamps': 'apply_data_quality_fixer',
                'non-monotonic datetime index': 'sort_and_fix_index',
                'index is not DatetimeIndex': 'convert_to_datetime_index',
                'DataFrame is ambiguous': 'fix_dataframe_boolean_operations'
            },
            'data_structure': {
                'dict object has no attribute columns': 'validate_dataframe_structure',
                'Could not find DataFrame': 'ensure_dataframe_in_pipeline_state',
                'No data available': 'check_data_availability',
                'object has no attribute': 'validate_object_attributes'
            },
            'module_imports': {
                'No module named': 'use_graceful_module_handler',
                'cannot import name': 'create_fallback_implementation',
                'name.*is not defined': 'fix_variable_references'
            },
            'validation': {
                'validation_results': 'ensure_validation_results_attribute',
                'metrics.*is not defined': 'fix_metrics_reference',
                'CONFIG.*is not defined': 'import_config_module'
            }
        }
    ...
    def apply_preventive_fixes(self, step_name: str, data: Any, recommendations: List[str]) -> Tuple[Any, Dict[str, Any]]:
        ...
        fix_report = {
            ...
            'applied_fixes': [],
            'fixes_failed': [],
            'original_issues': len(recommendations)
        }
    ...
    def validate_step_input(self, step_name: str, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Check DataFrame quality if present
    ...
    def create_step_safety_wrapper(self, step_class, step_name: str):
        """
        ...
        class SafeStepWrapper(step_class):
            ...
            async def execute(self, training_input: Dict[str, Any], pipeline_state: Dict[str, Any]) -> Dict[str, Any]:
                ...
                if not input_validation['valid']:
                    ...
                    return {
                        'success': False,
                        'error': f"Input validation failed: {input_validation['issues']}",
                        'step_name': self.safe_step_name
                    }
                ...
                try:
                    ...
                    if output_analysis['risk_level'] == 'HIGH':
                        ...
                        # Apply preventive fixes if data is present
                        if 'dataframe' in result or 'validated_data' in result:
                            data = result.get('dataframe') or result.get('validated_data')
                            if isinstance(data, pd.DataFrame):
                                fixed_data, fix_report = self.error_prevention.apply_preventive_fixes(
                                    self.safe_step_name, data, output_analysis['recommendations']
                                )
