...
from src.utils.function_call_monitor import (
    monitor_comprehensive, monitor_standard, monitor_basic,
    get_function_call_monitor, log_function_call_summary
)
from src.utils.function_validation_framework import (
    validate_function_entry, validate_function_output,
    get_function_validator
)
from src.utils.enhanced_error_handler import (
    handle_errors_with_tracking, get_error_handler, log_error_summary
)
...
class Step01ComprehensiveMonitoring:
    ...
    def _validate_environment(self) -> None:
        ...
        required_modules = [
            'pandas', 'numpy', 'src.config', 'src.utils.logger', 
            'src.utils.error_handler', 'src.training.steps.data_downloader',
            'src.utils.enhanced_mlflow_integration', 'src.utils.centralized_decorators'
        ]
    ...
    async def execute(self, training_input: Dict[str, Any], pipeline_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Add comprehensive monitoring results to pipeline state
        ...
        # Log artifacts and reports with comprehensive monitoring
        await self._log_comprehensive_step1_artifacts_and_report(training_input, pipeline_state)
        
        # Log final comprehensive monitoring summary
        self._log_comprehensive_summary()
    ...
    async def _run_comprehensive_data_collection(self, training_input: Dict[str, Any], data_dir: str) -> bool:
        ...
        try:
            ...
            # Try to use existing downloader with comprehensive monitoring
        ...
        try:
            ...
            for file_name in expected_files:
                ...
                if os.path.exists(file_path):
                    ...
                    # Validate file with comprehensive monitoring
    ...
    async def _generate_comprehensive_mock_data(self, symbol: str, exchange: str, timeframe: str, data_dir: str) -> bool:
        ...
        try:
            ...
            # Create and save klines DataFrame
            ...
            # Create and save aggtrades DataFrame
    ...
    async def _run_comprehensive_quality_check(self, symbol: str, exchange: str, timeframe: str, data_dir: str) -> bool:
        ...
        try:
            ...
            # Use the same validation logic as data validation
    ...
    async def _log_comprehensive_detailed_data_extract(self, symbol: str, exchange: str, timeframe: str, data_dir: str) -> None:
        ...
        try:
            ...
            files_to_check = [
                ('Klines', os.path.join(data_dir, klines_file)),
                ('Aggtrades', os.path.join(data_dir, aggtrades_file))
            ]
    ...
    async def _log_comprehensive_step1_artifacts_and_report(self, training_input: Dict[str, Any], pipeline_state: Dict[str, Any]) -> None:
        ...
        try:
            ...
            execution_metadata = {
                ...
                'comprehensive_monitoring_summary': pipeline_state.get('comprehensive_monitoring_summary', {})
            }
            ...
            metrics_calculated = {
                ...
                'function_calls_monitored': monitoring_summary.get('function_calls', {}).get('total_calls', 0),
                'function_call_success_rate': monitoring_summary.get('function_calls', {}).get('success_rate', 0.0),
                'validation_checks_performed': monitoring_summary.get('validation', {}).get('total_checks', 0),
                'validation_success_rate': monitoring_summary.get('validation', {}).get('success_rate', 0.0),
                'errors_handled': monitoring_summary.get('errors', {}).get('total_errors', 0),
                'error_recovery_rate': monitoring_summary.get('errors', {}).get('recovery_rate', 0.0)
            }
            
            # Create comprehensive report data
            report_data = {
                'step_name': 'step01_comprehensive_monitoring',
                ...
                'comprehensive_monitoring_results': monitoring_summary
            }
    ...
    def _get_comprehensive_monitoring_summary(self) -> Dict[str, Any]:
        ...
        return {
            'function_calls': function_monitor.get_call_summary(),
            'validation': {
                'total_checks': len(function_validator.validation_rules),
                'success_rate': 100.0  # Placeholder - would need to track validation results
            },
...
@validate_function_output('data_collection')
@handle_errors_with_tracking(fallback=True)
async def run_comprehensive_step01(
    symbol: str,
    exchange: str,
    timeframe: str = "1m",
    ...
    **kwargs: Any
    ...
    try:
        ...
        # Initialize step with comprehensive monitoring
        config = {
            'SYMBOL': symbol,
            'EXCHANGE': exchange,
            'TIMEFRAME': timeframe,
        ...
        if success:
            ...
            # Export comprehensive monitoring reports
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            
            # Export function call monitoring report
