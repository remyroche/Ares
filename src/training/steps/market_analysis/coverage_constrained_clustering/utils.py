...
def extract_regime_summary_vectors(
    ...
    """Build 4D feature vector per regime with robust fallbacks.

    Prefers 'features' sub-dict (means) produced by HMM component. Falls back to
    light-weight proxies (sample_count, percentage, regime_id) when absent.
    Applies weights and scaling per dimension for separation.
    """
    regime_keys = list(regime_characteristics.keys())
    X: List[List[float]] = []

    for key in regime_keys:
        char = regime_characteristics.get(key) or {}
        feat = char.get("features") or {}

        # Primary: use normalized dimensional features if available
        volume_feature = float(feat.get("volume_ratio", char.get("sample_count", 0)))
        volatility_feature = float(feat.get("volatility_20", char.get("percentage", 0)))
        momentum_feature = float(feat.get("momentum_20", char.get("regime_id", 0)))
        trend_feature = float(feat.get("trend_score", (char.get("regime_id", 0) * char.get("sample_count", 0))))

        # Simple normalizations for fallbacks
        if "features" not in char:
            volume_feature = volume_feature / 1000.0
            volatility_feature = volatility_feature / 100.0
            momentum_feature = momentum_feature / 10.0
            trend_feature = trend_feature / 10000.0
    ...
    if not X:
        X = [[i, 0.0, 0.0, 0.0] for i in range(len(regime_keys))]
