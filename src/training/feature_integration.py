...
from src.core.decorators.errors import handles_errors
    ...
    async def integrate_features(self, historical_data: pd.DataFrame, market_data: pd.DataFrame, order_flow_data: pd.DataFrame | None=None) -> pd.DataFrame:
        ...
        try:
            if not self.is_initialized:
                self.logger.error(initialization_error('Feature integration manager not initialized'))
    ...
    def get_liquidity_feature_summary(self, data: pd.DataFrame) -> dict[str, Any]:
        ...
        try:
            liquidity_features = ['volume_liquidity', 'price_impact', 'spread_liquidity', 'liquidity_regime', 'liquidity_percentile', 'kyle_lambda', 'amihud_illiquidity', 'order_flow_imbalance', 'large_order_ratio', 'vwap', 'volume_roc', 'volume_ma_ratio', 'liquidity_stress']
            available_features = [f for f in liquidity_features if f in data.columns]
            summary = {'total_liquidity_features': len(available_features), 'available_liquidity_features': available_features, 'liquidity_feature_coverage': len(available_features) / len(liquidity_features), 'feature_statistics': {}}
            for feature in available_features:
                if feature in data.columns:
                    feature_data = data[feature].dropna()
                    if not feature_data.empty:
                        summary['feature_statistics'][feature] = {'mean': feature_data.mean(), 'std': feature_data.std(), 'min': feature_data.min(), 'max': feature_data.max(), 'missing_pct': data[feature].isna().sum() / len(data) * 100}
            return summary
        except Exception as e:
