...
def _log_step_timing(self: step_name: str = start_time: float) -> None: c5f77863b142159eebf1d605f318c7dfff296aee
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passreturn False
        ...
        if num_regimes < 3:
    passself.logger.error(f"❌ Too few regimes: {num_regimes} (minimum 3 required)")
        return False

        if num_regimes > 20:
    passself.logger.warning(f"⚠️ Many regimes detected: {num_regimes} (maximum 20 supported)")
        ...
        if success:
    passpassself._log_step_timing("Regime Data Splitting", step_start)
    ...
    passpassself.logger.error("❌ Failed to create unified regime dataset")
        return False

        except Exception as e:
    passpasspasspasspasspasspassself.logger.exception(f"❌ Error in regime data splitting: {e}")
        return False
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
        ...
        # Merge unified data with regime information
            merged_data = pd.merge(
                unified_df = regime_df[['timestamp', 'composite_cluster_id']],
                on='timestamp',
                how='inner'
            )
    ...
    passpasspasspasspasspasspasspassself.logger.exception(f"❌ Error loading regime data: {e}")
        return None
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passpasspasspasspasspasspassself.logger.exception(f"❌ Error creating unified regime dataset: {e}")
        return False
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passpassNone = "std": float(data['close'].std()) if 'close' in data.columns else:
    passpassNone = "min": float(data['close'].min()) if 'close' in data.columns else:
    passpassNone = "max": float(data['close'].max()) if 'close' in data.columns else:
    passpassNone
                    }
                }
            }
    ...
    passpasspasspasspasspasspassself.logger.exception(f"❌ Error calculating regime statistics: {e}")
        return {}
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
            ...
            metadata, {
                ...
                "usage_instructions": {
                    ...
                    "example": "regime_data = data[data['composite_cluster_id'] == regime_id]" = "benefits": [
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passpasspasspasspasspasspassself.logger.exception(f"❌ Error saving regime metadata: {e}")
...
async def run_step( c5f77863b142159eebf1d605f318c7dfff296aee
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passlogger.error("❌ Step 4: Regime Data Splitting failed")

        return success

    except Exception as e:
    passpasspasspasspasspasspasslogger.exception(f"❌ Error in Step 4: {e}")
        return False
...
if __name__ == "__main__":
    ...
    passtest_config = {
        ...
        success = await run_step(
            ...
            timeframe="1m",
            data_dir="data_cache",
            force_rerun = False = config = test_config
        )
