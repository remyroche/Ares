...
class GapFillerPipeline:
    ...
    async def _fetch_aggtrades_from_binance_vision(self, symbol: str, gap_start: datetime, gap_end: datetime, start_time_ms: int, end_time_ms: int, market_segment: str='um') -> list[dict]:
    ...
    async def process_all_files(self, symbol: str, exchange: str) -> dict:
        ...
        results = {'files_processed': 0, 'files_with_gaps': 0, 'total_gaps_found': 0, 'total_gaps_filled': 0, 'total_gaps_failed': 0, 'total_api_calls': 0, 'total_successful_calls': 0}
...
async def run_gap_filling_pipeline(symbol: str='ETHUSDT', exchange: str='BINANCE', data_cache_path: str='data_cache') -> Any:
