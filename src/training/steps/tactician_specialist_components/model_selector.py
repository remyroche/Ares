...
class ModelSelector:
    ...
    async def select_best_models(
        ...
        model_scores = await self._calculate_model_scores(
            models, validation_results, tactic_config
        )
        
        # Filter by minimum threshold
        min_threshold = tactic_config.get(
            "min_confidence",
        ...
        qualified_models = {
            ...
            if validation_results[name].get(self.selection_metric, 0) >= min_threshold
        }
        
        if not qualified_models:
            self.logger.warning(
                f"No models meet minimum threshold of {min_threshold}"
            )
            # Return best model even if below threshold
            best_model_name = max(model_scores.items(), key=lambda x: x[1])[0]
            return {best_model_name: models[best_model_name]}
        
        # Select top models with diversity consideration
        selected_models = await self._select_diverse_models(
            models,
            qualified_models,
            validation_results
        )
    ...
    async def _calculate_model_scores(
        self,
        models: Dict[str, Any],
        validation_results: Dict[str, Dict[str, float]],
        tactic_config: Dict[str, Any]
    ...
    async def _select_diverse_models(
        self,
        models: Dict[str, Any],
        ...
        # Add additional models considering diversity
        ...
        self.logger.info(
            f"Selected {len(selected)} models from {len(models)} candidates"
        )
    ...
    async def _calculate_diversity_score(
        ...
        # Simple diversity based on model type
    ...
    @handles_errors(
        exceptions=(Exception,),
        default_return={},
        context="model ranking"
    )
    async def rank_models_by_tactic(
        self,
        ...
        for tactic in all_tactics:
            ...
            # Collect all models for this tactic
    ...
    @handles_errors(
        exceptions=(Exception,),
        default_return={},
        context="optimal model combination"
    )
    async def select_optimal_combination(
        self,
        models: Dict[str, Any],
        ...
        for model_info in model_values:
            ...
            selected_combination[tactic].append({
