...
from ..sr_strength_optimizer import SRLevelIdentifier
import numpy as np
import logging
import time
...
class OptimizedSRMethod(BaseSRMethod):
    ...
    def identify_levels(self, market_data: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        return [{'price': level.price, 'strength': level.strength, 'type': level.type, 'metadata': {'touches': level.touch_count, 'age_bars': level.age_bars, 'bounce_ratio': level.avg_bounce_ratio}} for level in strong_levels]
...
class ClassicalSRMethod(BaseSRMethod):
    ...
    def _calculate_pivot_points(self, data: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        r2 = pp + (high - low)
        s2 = pp - (high - low)
        levels.extend([{'price': pp, 'strength': 0.7, 'type': 'pivot', 'metadata': {'pivot_type': 'pp'}}, {'price': r1, 'strength': 0.6, 'type': 'resistance', 'metadata': {'pivot_type': 'r1'}}, {'price': s1, 'strength': 0.6, 'type': 'support', 'metadata': {'pivot_type': 's1'}}, {'price': r2, 'strength': 0.5, 'type': 'resistance', 'metadata': {'pivot_type': 'r2'}}, {'price': s2, 'strength': 0.5, 'type': 'support', 'metadata': {'pivot_type': 's2'}}])
        return levels
    ...
    def _find_swing_points(self, data: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for i in range(window, len(data) - window):
            if all(data['high'].iloc[i] >= data['high'].iloc[i - window:i]) and all(data['high'].iloc[i] >= data['high'].iloc[i + 1:i + window + 1]):
                levels.append({'price': data['high'].iloc[i], 'strength': 0.7, 'type': 'resistance', 'metadata': {'swing_type': 'high', 'index': i}})
        for i in range(window, len(data) - window):
            if all(data['low'].iloc[i] <= data['low'].iloc[i - window:i]) and all(data['low'].iloc[i] <= data['low'].iloc[i + 1:i + window + 1]):
                levels.append({'price': data['low'].iloc[i], 'strength': 0.7, 'type': 'support', 'metadata': {'swing_type': 'low', 'index': i}})
        return levels
    ...
    def _find_psychological_levels(self, data: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for i in range(-3, 4):
            ...
            if level_price > 0:
                ...
                levels.append({'price': level_price, 'strength': 0.5, 'type': level_type, 'metadata': {'psychological': True}})
        return levels
...
class VolumeProfileSRMethod(BaseSRMethod):
    ...
    def identify_levels(self, market_data: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        bins = np.linspace(market_data['low'].min(), market_data['high'].max(), self.n_bins + 1)
        ...
        for i in range(1, self.n_bins - 1):
            if volume_profile[i] > threshold and volume_profile[i] > volume_profile[i - 1] and (volume_profile[i] > volume_profile[i + 1]):
                ...
                levels.append({'price': price, 'strength': strength, 'type': level_type, 'metadata': {'volume_node': True, 'volume_ratio': volume_profile[i] / volume_profile.mean()}})
        return levels
...
class FractalSRMethod(BaseSRMethod):
    ...
    def identify_levels(self, market_data: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for i in range(2, len(market_data) - 2):
            if market_data['high'].iloc[i] > market_data['high'].iloc[i - 1] and market_data['high'].iloc[i] > market_data['high'].iloc[i - 2] and (market_data['high'].iloc[i] > market_data['high'].iloc[i + 1]) and (market_data['high'].iloc[i] > market_data['high'].iloc[i + 2]):
                touches = self._count_level_touches(market_data, market_data['high'].iloc[i], 'resistance', i)
                if touches >= 2:
                    levels.append({'price': market_data['high'].iloc[i], 'strength': min(touches / 10, 1.0), 'type': 'resistance', 'metadata': {'fractal': True, 'touches': touches}})
        for i in range(2, len(market_data) - 2):
            if market_data['low'].iloc[i] < market_data['low'].iloc[i - 1] and market_data['low'].iloc[i] < market_data['low'].iloc[i - 2] and (market_data['low'].iloc[i] < market_data['low'].iloc[i + 1]) and (market_data['low'].iloc[i] < market_data['low'].iloc[i + 2]):
                touches = self._count_level_touches(market_data, market_data['low'].iloc[i], 'support', i)
                if touches >= 2:
                    levels.append({'price': market_data['low'].iloc[i], 'strength': min(touches / 10, 1.0), 'type': 'support', 'metadata': {'fractal': True, 'touches': touches}})
        return levels
        ...
        for i in range(start_idx + 1, len(data)):
            if level_type == 'resistance':
                if abs(data['high'].iloc[i] - level_price) / level_price < threshold:
                    touches += 1
            elif abs(data['low'].iloc[i] - level_price) / level_price < threshold:
                touches += 1
        return touches
...
class EnsembleSRPredictor:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.methods = [OptimizedSRMethod(config), ClassicalSRMethod(config), VolumeProfileSRMethod(config), FractalSRMethod(config)]
    ...
    def _load_method_weights(self) -> Dict[str, float]:
        ...
        default_weights = {'optimized_strength': 0.35, 'classical_ta': 0.25, 'volume_profile': 0.25, 'fractal': 0.15}
        try:
            weights_file = os.path.join(self.config.get('model_save_path', 'models'), 'ensemble_sr_weights.json')
    ...
    @handles_errors(exceptions=(ValueError, AttributeError), default_return=[], context='ensemble SR prediction')
    @traced(span_name='EnsembleSR.predict')
    async def identify_ensemble_levels(self, market_data: pd.DataFrame) -> List[EnsembleSRLevel]:
        """
    ...
    def _combine_levels(self, all_levels: Dict[str, List[Dict]]) -> List[EnsembleSRLevel]:
        ...
        for method_name, levels in all_levels.items():
            for level in levels:
                tagged_levels.append({'method': method_name, 'price': level['price'], 'strength': level['strength'], 'type': level.get('type', 'unknown'), 'metadata': level.get('metadata', {})})
    ...
    def _create_ensemble_level(self, cluster: List[Dict]) -> EnsembleSRLevel:
        ...
        metadata = {'cluster_size': len(cluster), 'methods_agreed': list(method_votes.keys()), 'price_spread': max((l['price'] for l in cluster)) - min((l['price'] for l in cluster))}
        return EnsembleSRLevel(price=avg_price, strength=ensemble_confidence, type=level_type, method_votes=method_votes, ensemble_confidence=ensemble_confidence, metadata=metadata)

    def _calculate_ensemble_confidence(self, cluster: List[Dict], method_votes: Dict[str, float]) -> float:
        ...
        consistency_score = 1 - min(price_std * 100, 1)
        ensemble_confidence = agreement_score * 0.4 + avg_strength * 0.4 + consistency_score * 0.2
        return min(ensemble_confidence, 1.0)

    def optimize_method_weights(self, historical_data: pd.DataFrame, actual_reversals: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        for method in self.methods:
            ...
            for reversal in actual_reversals:
                ...
                if matching_levels:
                    closest = min(matching_levels, key=lambda x: abs(x['price'] - reversal_price))
        ...
        if total_score > 0:
            optimized_weights = {method: score / total_score for method, score in avg_scores.items()}
        else:
            optimized_weights = {method: 1.0 / len(self.methods) for method in avg_scores.keys()}
    ...
    def _save_optimized_weights(self, weights: Dict[str, float]) -> None:
        ...
        try:
            ...
            with open(weights_file, 'w') as f:
                json.dump({'weights': weights, 'timestamp': datetime.now().isoformat()}, f, indent=2)
