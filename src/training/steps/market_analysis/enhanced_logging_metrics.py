...
class EnhancedPipelineLogger:
    ...
    def __init__(self, name: str='market_analysis', log_dir: str='log') -> None:
    ...
    def log_feature_quality(self, step_name: str, data: pd.DataFrame, feature_columns: List[str]=None) -> None:
    ...
    def log_regime_quality(self, step_name: str, regimes: pd.Series, regime_labels: List[str]=None) -> None:
    ...
    def log_step6_metrics(self, step_name: str, feature_engine_results: Dict[str, Any]) -> None:
    ...
    def log_step7_metrics(self, step_name: str, matrix_results: Dict[str, Any]) -> None:
    ...
    def log_progress(self, step_name: str, progress: float, message: str='') -> None:
    ...
    def log_issue(self, step_name: str, issue_type: str, message: str, severity: str='warning') -> None:
    ...
    def _calculate_feature_quality_metrics(self, data: Union[pd.DataFrame, Dict[str, Any]], feature_columns: List[str]) -> FeatureQualityMetrics:
        ...
        try:
            ...
            total_issues = metrics.nan_features + metrics.constant_features + metrics.high_correlation_pairs + metrics.low_variance_features + metrics.infinite_values + metrics.duplicate_features
    ...
    def _save_metrics_to_file(self) -> None:
        ...
        try:
            metrics_data = {'correlation_id': self.correlation_id, 'pipeline_start_time': self.pipeline_start_time.isoformat() if self.pipeline_start_time else None, 'pipeline_end_time': self.pipeline_end_time.isoformat() if self.pipeline_end_time else None, 'step_metrics': {}}
            for step_name, metrics in self.step_metrics.items():
