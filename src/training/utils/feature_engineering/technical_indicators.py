...
class TechnicalIndicatorCalculator:
    ...
    def calculate_price_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in [5, 10, 20, 50]:
            ...
            features[f'ema_{period}'] = self._ema_numba(data['close'].values, period)
        for period in [5, 10, 20, 50]:
            ma_col = f'sma_{period}'
            if ma_col in features:
                features[f'close_to_sma_{period}'] = data['close'] / features[ma_col] - 1
        return features
    ...
    def calculate_momentum_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in [6, 14, 28]:
            features[f'rsi_{period}'] = self._rsi_numba(data['close'].values, period)
        if len(data) >= 26:
        ...
        if len(data) >= 14:
            slowk, slowd = talib.STOCH(data['high'].values, data['low'].values, data['close'].values, fastk_period=14, slowk_period=3, slowd_period=3)
    ...
    def calculate_volatility_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(data) >= 14:
            features['atr_14'] = talib.ATR(data['high'].values, data['low'].values, data['close'].values, timeperiod=14)
        for period in [20, 50]:
            ...
            features[f'bb_position_{period}'] = (data['close'] - features[f'bb_lower_{period}']) / (features[f'bb_width_{period}'] + 1e-10)
        if 'atr_14' in features:
            ...
            features['kc_position'] = (data['close'] - features['kc_lower']) / (features['kc_upper'] - features['kc_lower'] + 1e-10)
        return features
    ...
    def calculate_volume_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(data) >= 14:
            features['mfi_14'] = talib.MFI(data['high'].values, data['low'].values, data['close'].values, data['volume'].values, timeperiod=14)
    ...
    def calculate_all_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        all_features = pd.concat([price_features, momentum_features, volatility_features, volume_features], axis=1)
