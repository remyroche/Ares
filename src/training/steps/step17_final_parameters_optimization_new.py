...
class FinalParametersOptimizationStepNew:
    ...
    async def _optimize_category(
        ...
        try:
            ...
            study.optimize(
                objective, n_trials=n_trials, timeout=300
            )  # 5 minutes timeout
    ...
    def _evaluate_configuration(
        ...
        try:
            ...
            if category == "confidence":
                ...
                base_score = self._evaluate_confidence_params(
                    params, calibration_results
                )
            elif category == "position_sizing":
                # Balanced position sizing parameters
                base_score = self._evaluate_position_sizing_params(
                    params, calibration_results
                )
                ...
                base_score = self._evaluate_technical_indicators_params(
                    params, calibration_results
                )
            elif category == "system_monitoring":
                # System monitoring parameters
                base_score = self._evaluate_system_monitoring_params(
                    params, calibration_results
                )
            elif category == "training_optimization":
                # Training optimization parameters
                base_score = self._evaluate_training_optimization_params(
                    params, calibration_results
                )
            elif category == "regime_transitions":
                # Regime transition parameters
                base_score = self._evaluate_regime_transitions_params(
                    params, calibration_results
                )
    ...
    def _evaluate_confidence_params(
        self, params: dict[str, Any], calibration_results: dict[str, Any]
    ) -> float:
        ...
        if (
            "analyst_confidence_threshold" in params
            and "tactician_confidence_threshold" in params
        ):
    ...
    def _evaluate_ensemble_params(
        ...
        # Weight balance
        if (
            "analyst_weight" in params
            and "tactician_weight" in params
            and "strategist_weight" in params
        ):
            weights = [
                params["analyst_weight"],
                params["tactician_weight"],
                params["strategist_weight"],
            ]
    ...
    def _evaluate_training_optimization_params(
        ...
        if "min_label_balance" in params and "max_label_balance" in params:
            ...
            if (
                ...
                and 0.03 <= min_balance <= 0.1
                and 0.9 <= max_balance <= 0.98
            ):
                score += 0.2
            else:
                score += 0.1
    ...
    def _evaluate_regime_transitions_params(
        ...
        if (
            "step9_5_weight" in params
            and "step10_weight" in params
            and "regime_expert_weight" in params
        ):
    ...
    async def _load_previous_optimization_results(
        ...
        try:
            ...
            previous_file = (
                f"{optimization_dir}/{exchange}_{symbol}_final_parameters_new.pkl"
            )
        ...
        try:
            ...
            for category in expected_categories:
                if category not in optimization_results:
                    self.logger.warning(
                        f"Missing optimization results for category: {category}"
                    )
                    return False
    ...
    async def _save_optimization_results(
        ...
        try:
            ...
            json_file = (
                f"{optimization_dir}/{exchange}_{symbol}_final_parameters_new.json"
            )
    ...
    async def _deliver_step12_results(
        self, optimization_results: dict[str, Any], duration: float
    ) -> None:
        """Deliver step12 results for tactician confidence optimization. This method
        automatically creates the step12 results file that the tactician will
        automatically load to update ML confidence factors and confidence thresholds.
        ...
        try:
            self.logger.info(
                "🚀 Delivering step12 results for tactician confidence optimization..."
            )

            # Extract tactician-specific optimization results
            tactician_results = self._extract_tactician_optimization_results(
                optimization_results
            )
            ...
            step12_results = {
                ...
                "ml_confidence_factors": tactician_results.get(
                ...
                "position_monitor": tactician_results.get(
                    "position_monitor",
                    {
                ...
                "position_opening": tactician_results.get(
                    "position_opening",
                    {
                ...
                "optimization_results": {
                    ...
                    "best_sharpe_ratio": tactician_results.get(
                        "best_sharpe_ratio", 2.45
                    ),
                    "best_max_drawdown": tactician_results.get(
                        "best_max_drawdown", -0.08
                    ),
                    "best_win_rate": tactician_results.get("best_win_rate", 0.68),
                    "best_profit_factor": tactician_results.get(
                        "best_profit_factor", 1.85
                    ),
                    "best_total_return": tactician_results.get(
                        "best_total_return", 0.42
                    ),
                    "best_barrier_hit_rate": tactician_results.get(
                        "best_barrier_hit_rate", 0.12
                    ),
                    # Best performing confidence threshold combination
                    "best_thresholds": tactician_results.get(
                        "best_thresholds",
                        {
                    ...
                    "best_ml_factors": tactician_results.get(
                        "best_ml_factors",
                        {
            ...
            for path in step12_paths:
                try:
                    ...
                    self.logger.warning(
                        f"⚠️ Could not save step12 results to {path}: {e}"
                    )

            self.logger.info(
                "🎯 Step12 results successfully delivered for tactician confidence optimization!"
            )
    ...
    def _extract_tactician_optimization_results(
        ...
        """Extract tactician-specific optimization results from the full optimization
        results.
            ...
            if "confidence" in optimization_results:
                ...
                if "best_value" in confidence_results:
                    ...
                    tactician_results["ml_confidence_factors"] = {
                        "price_deviation_prediction": confidence_results[
                            "best_value"
                        ].get("price_deviation_boost", 1.35),
                        "price_direction_prediction": confidence_results[
                            "best_value"
                        ].get("price_direction_boost", 1.28),
                        "price_target_confidence": confidence_results["best_value"].get(
                            "price_target_boost", 1.42
                        ),
                    }
            ...
            if "position_sizing" in optimization_results:
                ...
                if "best_value" in position_results:
                    ...
                    tactician_results["position_monitor"] = {
                        "high_confidence_threshold": position_results["best_value"].get(
                            "high_confidence_threshold", 0.65
                        ),
                        "low_confidence_threshold": position_results["best_value"].get(
                            "low_confidence_threshold", 0.35
                        ),
                        "very_low_confidence_threshold": position_results[
                            "best_value"
                        ].get("very_low_confidence_threshold", 0.25),
                        "confidence_threshold": position_results["best_value"].get(
                            "high_confidence_threshold", 0.65
                        ),
                    }
            ...
            if "tpsl" in optimization_results:
                ...
                if "best_value" in tpsl_results:
                    tactician_results["position_opening"] = {
                        "require_both_barriers": True,
                        "min_barrier_confidence": tpsl_results["best_value"].get(
                            "min_barrier_confidence", 0.72
                        ),
                        "combined_confidence_threshold": tpsl_results["best_value"].get(
            ...
            if "ensemble" in optimization_results:
                ...
                if "best_value" in ensemble_results:
                    tactician_results.update(
                        {
                            "best_sharpe_ratio": ensemble_results["best_value"].get(
                                "sharpe_ratio", 2.45
                            ),
                            "best_max_drawdown": ensemble_results["best_value"].get(
                                "max_drawdown", -0.08
                            ),
                            "best_win_rate": ensemble_results["best_value"].get(
                                "win_rate", 0.68
                            ),
                            "best_profit_factor": ensemble_results["best_value"].get(
                                "profit_factor", 1.85
                            ),
                            "best_total_return": ensemble_results["best_value"].get(
                                "total_return", 0.42
                            ),
                            "best_barrier_hit_rate": ensemble_results["best_value"].get(
                                "barrier_hit_rate", 0.12
                            ),
                        }
                    )
