...
class TacticianSpecialistTrainingStep:
    ...
    async def _enhance_training_data_with_sr_context(
        ...
        # Interpolate to all data points
                    full_feature = (feature_series.reindex(enhanced_data.index)
    ...
    async def execute(
        ...
        try: feat_cols = training_input.get(
                                    "model_feature_columns",
                                ) or training_input.get("feature_columns")
                                label_col, training_input.get("label_column", "label")
        ...
        try: with open(labeled_file_pickle = "rb") as f:
                            labeled_data = pickle.load(f),
        except Exception:
                        pass
            else:
                msg = ("Tactician labeled data not found: "
        ...
        try: shape = getattr(labeled_data, "shape", None)
        ...
        self.logger.warning(
                ...
                enhanced_labeled_data = (await self._enhance_training_data_with_sr_context(
                        labeled_data,
                        symbol,
                        current_timeframe,
                    )
    ...
    async def _train_tactician_models(
        ...
        try: best_cb = await self._hpo_catboost(X_train_ens, X_test_ens, y_train, y_test)
        if best_cb:
    ...
    async def _hpo_catboost(
        self, X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.Series, y_test: pd.Series, ) -> dict[str, Any] | None:
