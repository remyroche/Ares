...
class CausalSurpriseDetector:
    ...
    def compute_regime_specific_reliability(
        ...
        reliability_matrix = pd.DataFrame(
            index=errors_aligned.columns, columns=posteriors_aligned.columns
        ).astype(float)
    ...
    def _compute_specialist_zone_levels(
        ...
        for col in surprise_df.columns:
            levels = pd.cut(
                surprise_df[col], bins=zone_bins, labels=zone_labels
            ).astype(float)
    ...
    def _compute_specialist_zone_scores(
        self, zone_levels: pd.DataFrame, surprise_df: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def generate_causal_events(
        ...
        try:
            ...
            if regime_posteriors is not None:
                ...
                adaptive_threshold = rolling_threshold.fillna(event_threshold).clip(
                    lower=event_threshold
                )
            ...
            if regime_vol is not None:
                ...
                if (
                    ...
                    aligned_vol = (
                        regime_vol.reindex(scores.index).fillna(0)
                        if isinstance(regime_vol, pd.Series)
            ...
            if self.verbose:
                ...
                tprint_info("   - Event threshold: Adaptive (Quantile 98%)")
    ...
    def compute_reliability_metrics(
        ...
        try:
            ...
            return {"specialists": specialist_metrics, "detector": detector_metrics}
