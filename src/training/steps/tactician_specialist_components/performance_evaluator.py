...
class PerformanceEvaluator:
    ...
    async def evaluate_all_specialists(
        ...
        # Evaluate each regime's specialists
        for regime_id, regime_models in all_models.items():
            self.logger.info(f"Evaluating specialists for regime {regime_id}...")
            
            regime_eval = await self._evaluate_regime_specialists(
                regime_models, test_data, regime_id
            )
            
            evaluation_results["regime_evaluations"][regime_id] = regime_eval
        
        # Compare tactics across regimes
        evaluation_results["tactic_comparisons"] = await self._compare_tactics_across_regimes(
            evaluation_results["regime_evaluations"]
        )
        ...
        evaluation_results["performance_summary"] = self._create_performance_summary(
            evaluation_results
        )
        
        return evaluation_results
    
    async def _evaluate_regime_specialists(
        self,
        ...
        for tactic_name, tactic_models in regime_models.items():
            tactic_eval = await self._evaluate_tactic(
                tactic_name, tactic_models, X_test, y_test
            )
            
            regime_evaluation["tactics"][tactic_name] = tactic_eval
            
            # Track best performing tactic
        ...
        # Calculate regime-level metrics
        regime_evaluation["regime_metrics"] = self._aggregate_tactic_metrics(
            regime_evaluation["tactics"]
        )
        
        return regime_evaluation
    
    async def _evaluate_tactic(
        self,
        tactic_name: str,
        models: Any,
        X_test: pd.DataFrame,
        ...
        if isinstance(models, dict):
            # Multiple models for the tactic
            all_metrics = []
            
            for model_name, model in models.items():
                metrics = await self._evaluate_single_model(
                    model, X_test, y_test, f"{tactic_name}_{model_name}"
                )
            ...
            # Single model for the tactic
            metrics = await self._evaluate_single_model(
                models, X_test, y_test, tactic_name
            )
            tactic_evaluation["model_metrics"]["single"] = metrics
            tactic_evaluation["average_metrics"] = metrics
        
        # Calculate stability metrics
        tactic_evaluation["stability_metrics"] = await self._calculate_stability_metrics(
    ...
    async def _calculate_stability_metrics(
        ...
        for start_idx in range(0, len(X_test) - self.evaluation_window + 1, self.sliding_window_step):
            ...
            # Evaluate on window
            if isinstance(models, dict):
                # Use first model for stability analysis
        ...
        if window_metrics:
            ...
            # Calculate temporal stability (trend)
    ...
    def _aggregate_tactic_metrics(
        self,
        tactics_evaluation: Dict[str, Dict[str, Any]]
    ) -> Dict[str, float]:
        ...
        aggregated = {
            ...
            "avg_recall": [],
            "avg_f1": [],
            "n_tactics": len(tactics_evaluation),
            "n_meeting_thresholds": 0
        }
    ...
    async def _compare_tactics_across_regimes(
        self,
        regime_evaluations: Dict[str, Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for tactic in all_tactics:
            ...
            # Collect metrics from each regime
    ...
    def _calculate_overall_metrics(
        ...
        overall = {
            "total_regimes": len(regime_evaluations),
            "total_tactics": 0,
            "overall_precision": [],
            "overall_recall": [],
            "overall_f1": [],
    ...
    def _create_performance_summary(
        ...
        if overall.get("best_regime") and overall.get("best_tactic"):
            summary["highlights"].append(
                f"Best performance: {overall['best_tactic']} in regime {overall['best_regime']} "
                f"(F1: {overall['best_score']:.3f})"
            )
        
        if overall.get("overall_precision_mean", 0) > self.performance_thresholds["min_precision"]:
            summary["highlights"].append(
                f"Overall precision meets threshold: {overall['overall_precision_mean']:.3f}"
            )
        
        # Warnings
        if overall.get("overall_recall_mean", 0) < self.performance_thresholds["min_recall"]:
            summary["warnings"].append(
                f"Low overall recall: {overall.get('overall_recall_mean', 0):.3f}"
            )
        
        # Recommendations
        if overall.get("overall_f1_std", 0) > 0.1:
            summary["recommendations"].append(
                "High variance in performance across regimes - consider regime-specific tuning"
            )
