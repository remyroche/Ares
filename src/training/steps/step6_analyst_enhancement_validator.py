...
class Step6HMMBasedEnhancementValidator(BaseValidator):
    ...
    async def validate(
        ...
        try:
            error_passed, error_metrics = self._validate_error_absence(step_result)
        ...
        try:
            outcome_passed, outcome_metrics = self._validate_outcome_favorability(
                step_result,
            )
        ...
        successful_phases = sum(1 for v in validation_phases.values() if v)
    ...
    def _validate_performance_improvement(
        ...
        try:
            ...
            if enhanced_accuracies and isinstance(original_metrics.get("accuracy"), (int, float)):
    ...
    def _validate_error_absence(self, step_result: Any) -> tuple[bool, dict[str, Any]]:
        """Basic error absence check for the step result."""
        metrics: dict[str, Any] = {}
        if isinstance(step_result, dict):
            has_error = bool(step_result.get("error")) or step_result.get("status") == "FAILED"
            metrics = {"has_error": has_error, "status": step_result.get("status")}
            return (not has_error), metrics
        return True, {"info": "no structured step_result"}

    def _validate_outcome_favorability(self, step_result: Any) -> tuple[bool, dict[str, Any]]:
        """Check whether outcome metrics indicate favorability (placeholder)."""
        if isinstance(step_result, dict):
            metrics = step_result.get("metrics", {})
            # Consider presence of any positive metric as favorable placeholder
            favorable = any(
                isinstance(v, (int, float)) and v >= 0 for v in metrics.values()
            ) if isinstance(metrics, dict) else True
            return favorable, {"metrics_present": bool(metrics)}
        return True, {"info": "no structured step_result"}
