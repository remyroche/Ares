    ...
    def normalize_non_price_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if 'funding_rate' in df.columns:
                normalized_df['funding_rate_z_score'] = self._calculate_rolling_z_score(df['funding_rate'], self.normalization_windows['medium'])
                normalized_df['funding_rate_change'] = df['funding_rate'].diff(3).fillna(0)
                normalized_df['funding_rate_acceleration'] = normalized_df['funding_rate_change'].diff(2).fillna(0)
