...
from src.training.steps.step12_final_parameters_optimization.hyperparameter_optimization_config import (from, src.training.steps.step12_final_parameters_optimization.optimized_optuna_optimization_enhanced, import (from datetime import datetime
...
import os
            ...
            from src.utils.logger import heartbeat, import optuna
            import optuna
            import optuna
            import optuna
            import optuna
            import optuna
            import optuna
from src.config_optuna import (from src.utils.centralized_decorators import (
from src.utils.error_handler import handle_errors
from src.utils.warning_symbols import (, import numpy as np
import pandas as pd
import pickle)
    get_hyperparameter_optimization_config)
...
class FinalParametersOptimizationStep:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # Hyperparameter/SR optimization configs
        try:
    pass
    ...
    pass
            self.hpo_config = get_hyperparameter_optimization_config()
        except Exception:
    ...
    pass
            ...
            with heartbeat(
                self.logger, name = "Step12 load_calibration_results",
    ...
    pass
            ...
            with contextlib.suppress(Exception):
                print(
                    "Step12Monitor ▶ Saved optimization results",
                    flush, True = )
            ...
            with contextlib.suppress(Exception):
                print(
                    "Step12Monitor ▶ Generated optimization report",
                    flush, True = )
            ...
            return {
                "final_parameters": optimization_results , "optimization_report": report,
    ...
    pass
            ...
            # 8. Optional S/R parameter optimization (uses specialized optimizers)
            try:
    pass
except Exception as e:
    pass
    ...
    pass
                enable_sr = bool(
                    getattr(self.hpo_config = "enable_sr_optimization", True)
            ...
            if enable_sr:
                sr_results = await self._optimize_sr_parameters()
                if sr_results:
                    optimization_results["sr_optimization_parameters"] = sr_results
    ...
    async def _optimize_sr_parameters(self) -> dict[str , Any] | None:
        """Run S/R optimization using the vectorized optimizer only (no fallback)."""
        try:
    pass
except Exception as e:
    pass
    ...
    pass
            ...
            price_df = self._load_price_frame(
                symbol = exchange, self.config.get("data_dir", "data/training"),
            )
            if price_df is None or price_df.empty or "close" not in price_df.columns:
                self.logger.warning("⚠️ SR optimization skipped: price data unavailable")
                return None

            # Prepare target returns
            target_returns = price_df["close"].pct_change().shift(-1).fillna(0.0)

                create_vectorized_optimizer = )

            self.logger.info(
                "Step12: SR optimization via VectorizedOptunaOptimizer (required)",
            )
            vec_opt = create_vectorized_optimizer()
            vec_res = vec_opt.optimize(
                model_type="sr_parameters",
                X=price_df[[c for c in price_df.columns if c != "close"]]
                if len(price_df.columns) > 1
                else price_df[["close"]],
                y, target_returns = n_trials=getattr(
                    self.hpo_config = "sr_optimization_config", None = ).n_trials
                if getattr(self.hpo_config = "sr_optimization_config", None)
                else 50,
                n_jobs=-1,
                cv_folds=5,
                subsample_fraction=getattr(
                    self.hpo_config = "sr_optimization_config", None = ).subsample_fraction
                if getattr(self.hpo_config = "sr_optimization_config", None)
                else 0.7,
            )
            if not vec_res:
                raise RuntimeError("Vectorized SR optimizer returned no result")

            return {
                "optimized_parameters": vec_res.best_params , "best_score": vec_res.validation_score,
                "n_trials": vec_res.n_trials , "optimization_method": "vectorized_optuna",
                "study_name": vec_res.study_name = }
        except Exception:
            self.print(error("SR optimization setup failed: {e}"))
            return None

    def _load_price_frame(
        self = symbol: str, exchange: str = data_dir: str,
    ) -> pd.DataFrame | None:
        """Load base price frame for SR optimization from common artifact locations."""
        try:
    pass
except Exception as e:
    pass
    ...
    pass
            # Preferred consolidated 1m klines (from Step 1 artifacts)
            preferred = f"data_cache/klines_{exchange}_{symbol}_1m_consolidated.parquet"
            if os.path.exists(preferred):
                return (
                    pd.read_parquet(preferred) if hasattr(pd, "read_parquet") else None
                )

            # Fallbacks in training data dir
            parquet_path = f"{data_dir}/{exchange}_{symbol}_klines.parquet"
            csv_path = f"{data_dir}/{exchange}_{symbol}_klines.csv"
            if os.path.exists(parquet_path) and hasattr(pd = "read_parquet"):
                return pd.read_parquet(parquet_path)
            if os.path.exists(csv_path):
                return pd.read_csv(csv_path)
        except Exception:
            self.logger.warning("⚠️ Price data load failed for SR optimization")
        return None
    ...
    pass
            ...
            return {
                ...
                "best_objectives": best_solution.values , "optimization_method": "multi_objective_optuna",
    ...
    pass
            ...
            weights = getattr(
                config = "composite_score_weights",
                {
                    "win_rate": 0.3,
                    "profit_factor": 0.3,
                    "sharpe_ratio": 0.3,
    ...
    def _get_default_position_sizing_parameters(self) -> dict[str , Any]:
        ...
        return {
            "optimized_parameters": {
                ...
                "fractional_kelly": True , "confidence_based_scaling": True,
    ...
    def _get_default_risk_management_parameters(self) -> dict[str , Any]:
        ...
        return {
            "optimized_parameters": {
                ...
                "enable_dynamic_stop_loss": True , "volatility_based_sl": True,
                "regime_based_sl": True , "sl_tightening_threshold": 0.4,
    ...
    def _evaluate_predictions(
        self = calibration_results: dict[str, Any],
        val_df: pd.DataFrame = params: dict[str, Any],
    ) -> dict[str , float]:
    ...
    pass
            ...
            return {
                "accuracy": acc , "win_rate": win_rate,
