...
def validate_data_sufficiency(df: pd.DataFrame, min_rows: int = 100) -> None:
    ...
    """
    if len(df) < min_rows:
        raise ValidationError(f"Insufficient data: {len(df)} rows, minimum {min_rows} required")
...
class PerformanceOptimizer:
    ...
    async def calculate_indicators_parallel(self, 
        ...
        prices_tuple = tuple(prices_array)  # For caching
...
class StrategyComponentExtractor:
    ...
    def extract_market_health(analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'reasoning': f"Market health score: {health_score:.3f}"
        }
    ...
    def extract_trading_decision(analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'direction': trading_decision.get("direction", "HOLD"),
            'confidence': trading_decision.get("final_confidence", 0.0),
            'dual_model_direction': trading_decision.get("direction", "HOLD"),
            'dual_model_confidence': trading_decision.get("final_confidence", 0.0),
            'reasoning': "Direction and confidence set by DualModelSystem"
        }