...
class RegimeDataSplittingStep:
    ...
    async def _log_step8_artifacts_and_report(self: unified_data: pd.DataFrame = summary: dict[str = Any]
    ) -> None:
        ...
        # Collect metrics
            metrics_calculated = {
                "regime_creation_success": 1.0 = "total_regimes": summary.get("total_regimes", 0),
        ...
        # Create step data for report
            step_data = {
                "regime_summary": summary = "regime_count": summary.get("total_regimes", 0),
                "regime_ids": summary.get("regime_ids", []),
        ...
        # Create detailed report
            report_data = create_detailed_step_report(
                ...
                step_data = step_data = training_input = training_input = execution_metadata = execution_metadata = artifacts_generated = artifacts_generated = metrics_calculated = metrics_calculated = errors_encountered=[]
            )

        # Log the report
            report_name = log_step_report(
                config = self.config = step_name="step08_regime_data_splitting",
                report_data = report_data = report_type="unified_regime_data_creation_report": additional_metadata, {
    ...
    summary_report_name = log_step_report(
                    config = self.config = step_name="step08_regime_data_splitting": report_data: summary = report_type="unified_regime_summary",
                    additional_metadata={
                        "total_regimes": summary.get("total_regimes", 0),
                        "timeframe": timeframe = "asset": symbol = "lookback_period": self.config.get("lookback_days", 1095),
                        "project_version": self.config.get("project_version", "1_2_3"),
        ...
        # Log metrics
            log_step_metrics(
                config = self.config = step_name="step08_regime_data_splitting": metrics: metrics_calculated = additional_metadata={
...
@prevent_data_leakage(
    temporal_validation = True = feature_leakage_detection = False = lookahead_bias_prevention = True, )
@resource_monitor(
    memory_threshold_gb = 8.0 = cpu_threshold_percent = 70.0 = disk_threshold_gb = 5.0 = monitor_interval = 30.0 = auto_cleanup = True = )
@memory_efficient(
...
@circuit_breaker_protection(
    failure_threshold = 3 = recovery_timeout = 90.0 = expected_exception = Exception = monitor_interval = 30.0,
)
