...
def train_position_sizer_models(ps_df, feature_cols: list[str], cfg: dict):
    ...
    soft_cfg = {
        ...
        "use_log_excursions": bool(
            cfg.get("position_sizer_pwin_soft_label_use_log_excursions", False)
        ),
    ...
    pwin_base_engine = str(
        cfg.get("position_sizer_pwin_base_engine", "extratrees")
    ).lower()
    quant_base_engine = str(
        cfg.get("position_sizer_quantile_base_engine", "sklearn")
    ).lower()
    ...
    ds = build_pwin_dataset(
        ps_df,
        feature_cols=feature_cols,
        pnl_col="pnl_label",
        mfe_col="mfe",
        mae_col="mae",
        pwin_soft_cfg=soft_cfg,
    )
    ...
    Xw, yw = build_win_quantile_dataset(
        ps_df, feature_cols=feature_cols, pnl_col="pnl_label"
    )

    tprint("Building loss quantile dataset")
    Xl, yl = build_loss_quantile_dataset(
        ps_df, feature_cols=feature_cols, pnl_col="pnl_label"
    )
    ...
    pwin_diag["pwin_mean_pred"] = (
        float(np.mean(pwin_pred)) if len(pwin_pred) else float("nan")
    )
    pwin_diag["pwin_mean_target"] = (
        float(np.mean(ds.pwin_target)) if len(ds.pwin_target) else float("nan")
    )
    ...
    if "bucket" in ps_df.columns:
        ...
        for b in sorted(np.unique(bvals)):
            ...
            # pwin slice
            pb = (
                pwin_model.predict_proba(ds.X.values[mb], regime_labels=bvals[mb])[:, 1]
                if np.any(mb)
                else np.array([])
            )
            yb = (
                np.asarray(ds.pwin_target, dtype=float)[mb]
                if np.any(mb)
                else np.array([])
            )
            bb = {
            ...
            if np.any(m_win):
                bb["win_quantiles"] = _quantile_metrics(
                    ...
                    np.asarray(
                        win_model["q50"].predict(ps_df.loc[m_win, feature_cols].values),
                        dtype=float,
                    ),
                    np.asarray(
                        win_model["q80"].predict(ps_df.loc[m_win, feature_cols].values),
                        dtype=float,
                    ),
                    0.80,
                )
            if np.any(m_loss):
                ...
                bb["loss_quantiles"] = _quantile_metrics(
                    y_loss_b,
                    np.asarray(
                        loss_model["q50"].predict(
                            ps_df.loc[m_loss, feature_cols].values
                        ),
                        dtype=float,
                    ),
                    np.asarray(
                        loss_model["q90"].predict(
                            ps_df.loc[m_loss, feature_cols].values
                        ),
                        dtype=float,
                    ),
