...
class TransitionSeqDataset(Dataset):
    ...
    def __init__(self, samples: list[dict[str, Any]], numeric_dim: int, label_index: list[str]) -> None:
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        return {'hmm_ids': _to_tensor(hmm_ids, torch.long), 'x_num': _to_tensor(X_num, torch.float32), 'y_ret': _to_tensor(y_ret, torch.float32), 'y_hmm': _to_tensor(y_hmm, torch.long), 'y_path': _to_tensor(np.array(y_path), torch.long), 'y_ttpt': _to_tensor(np.array(y_ttpt), torch.float32)}
...
class SmallTransformer(pl.LightningModule if pl else nn.Module):
    ...
    def training_step(self, batch: dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        if self.focal_gamma > 0.0:
            ...
            with torch.no_grad():
                p = torch.softmax(pred_path, dim=-1).gather(1, y_path.view(-1, 1)).clamp_min(1e-06).squeeze()
