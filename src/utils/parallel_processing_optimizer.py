...
class MacM1ParallelOptimizer:
    ...
    def __init__(self, 
        ...
        if self.is_m1_mac:
            ...
            self.memory_limit_mb = min(memory_limit_mb * 2, 8192)  # Up to 8GB per worker
        
        logger.info(f"🔧 Initialized MacM1ParallelOptimizer:")
    ...
    def _merge_chunks(self, chunks: List[pd.DataFrame]) -> pd.DataFrame:
        ...
        logger.debug(f"🔗 Merged {len(chunks)} chunks into DataFrame with {len(merged_df)} rows")
        return merged_df
    ...
    def parallel_apply(self, 
        ...
        if len(df) < self.chunk_size * 2:
            ...
            logger.debug("📊 Dataset too small for parallel processing, using sequential")
    ...
    def parallel_feature_engineering(self, 
                                   df: pd.DataFrame, 
        ...
        logger.info(f"🔧 Parallel feature engineering with {len(feature_funcs)} functions")
        ...
        for i, func in enumerate(feature_funcs):
            logger.info(f"   Processing function {i+1}/{len(feature_funcs)}: {func.__name__}")
        ...
        logger.info(f"✅ Parallel feature engineering completed")
        return final_result
    
    def parallel_rolling_operations(self, 
                                  df: pd.DataFrame, 
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if asyncio.iscoroutinefunction(func):
                logger.debug(f"⏭️ Skipping parallel processing for async function: {func.__name__}")
