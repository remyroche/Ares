# Strict-R3 current-v5 stack; sealed BCF/current dual-MC1 inference challenger

## Canonical long-only inference handover

**Updated:** 2026-08-17  
**Active executable schema:** `strict_r3_inference_bundle_v6_robust21_mc1_d2_rich_policy_v1`  
**Scope:** Kraken perpetuals, long side only  
**Readiness:** current-v5 remains the order-capable control; the dual BCF/current chain is wired into the sealed exchange-free inference producer  
**Order-capable control bundle:** `config/strict_r3_inference_bundle_long_20260801_v59_close_reporting_threshold_lineage.json`  
**Dual inference successor:** `config/strict_r3_inference_overlay_long_20260801_v60_bcf_current_dual_mc1.json`  
**Active execution bundle:** `config/strict_r3_kraken_live_execution_v57_v67_rich_policy.json`

## Canonical short-base research contract (P0/F90)

The active executable stack remains **long-only**. P0/F90 is the canonical
short base model for the present research generation: every downstream short
Geometry/K9, consensus, trust, MC1/BCF, policy and portfolio experiment must
consume its strict-prequential output rather than retrain or substitute the
base. It has no Kraken, admission, portfolio, or live-order authority until
the downstream side-aware stack has independently passed its OOS gates.

| Item | Frozen short contract |
|---|---|
| Name | P0/F90 (software target ID: `P1_policy_bps`) |
| Candidate/feature inputs | target-free, entry-executable short candidates; F90 is 90 fields selected from 115 fields passing the ≥90% target-free coverage gate |
| Label | exact completed 1-minute canonical-policy net; decision-time entry, 12-hour timeout, 100-bps cost once; invalid paths are excluded from fitting |
| Learner | LightGBM LambdaRank, exact decision timestamp × short-side queries, K32, linear gains, uniform weighting |
| Target grades | policy-net bps cut at −400, −200, 0, +100, +200 and +400 |
| Confirmation | October–December 2024, trained through September: rank IC 0.1522; positive IC in 87.59% of queries; Top-1/2/4 uplift +101.19/+86.53/+77.98 bps versus the same query mean |
| Strict OOS replication | Three 3-month 2025 folds: weighted within-timestamp policy IC 0.1221 / 0.1469 / 0.1178; positive IC in 79.76% / 81.52% / 76.22% of timestamps. Global policy-net Top-1% was +91.09 / +81.41 / +127.90 bps. The per-timestamp Top-1/2 outcome varies by fold, so this remains a relative ranker rather than an admission rule. |
| Interpretation | a portable **relative** opportunity ranker, not an absolute expected-value or admission model; downstream conversion layers alone may establish admission |

The ordered fields, source hashes, training semantics, confirmation evidence,
and the mandatory 20-trial no-improvement HPO stagnation patience are sealed in
`config/strict_r3_short_p0_f90_base_v1.json`.  Every downstream short model
must use strict-prequential P0/F90 predictions and a short-specific frozen
Geometry/K9 identity; it must never reuse long field indices, long K9 state,
or long calibration maps.

The first full short CMI/LambdaRank consensus funnel completed on 2026-08-21.
**No head passed its incremental ensemble all-tail/worst-month promotion gate.**
Therefore there is currently no canonical short consensus or BCF layer. The
valid short continuation is explicitly labelled `base_only`: it keeps P0/F90,
the frozen Geometry/K9 representation, same-model conversion and downstream
trust/MC1 research, but it cannot imply that an absent residual head has a
neutral or positive contribution. A future consensus may only be added after
a new predeclared strict-OOS gate passes.

### Completed side-aware continuation — research verdict

The requested long-equivalent downstream funnel was completed for the frozen
P0/F90 short score, using one stable short Geometry/K9 identity and only
strictly prequential score/map inputs. It does **not** promote a short
executable stack:

| Layer | Frozen short result | Advancement decision |
|---|---|---|
| Geometry/K9 | Oct--Dec 2024 definition, 64-tree encoder, K9 fit on 100,000 equal-month rows; bundle SHA `2c93f1690ef36a780f2aa7b5c32b125937827f2e760913ddc3d68b65e24c9262`; 33 stable aggregate fields only | required representation; retained |
| Consensus / BCF | Ten ensemble-first CMI/LambdaRank residual-head searches; none passed the all-tail and worst-month gate | no consensus; BCF unavailable by design |
| Raw 28-day Cell-day map | Matched rich-policy portfolio: 306 accepted trades, +74.36 net bps/trade; 2025 +77.82 and 2026 +58.24 | positive but too sparse / intermittent for promotion |
| R5 9-month posterior trust | 39 portfolio trades, +74.32 net bps/trade; February 2026 −100.02 bps/trade | reject as too sparse and non-portable |
| MC1-d2 mapper | 734 portfolio trades, +41.90 net bps/trade; 2025 +69.38 but 2026 −4.26, with March--May and July negative | challenger only; reject for promotion |
| Rich policy | Selected from 2024 only with the same smooth-capital-protection branch as long; 38 executed trials of 48 ceiling, stopped after exactly 20 stale trials; exact H12/15-minute paths and same chronological auction | frozen research policy, not a short live policy |

All three mapper arms use mapped EV >= +50 bps **before** any future policy
path is materialised, then use the same short-directional rich-policy exit and
two-concurrent/two-per-timestamp/one-per-asset auction. The full receipt is
`docs/SHORT_P0_F90_SIDE_AWARE_PIPELINE_AUDIT_20260821.md`; machine-readable
full-stack results are under
`data_perp/artifacts/strict_r3_short_p0_mapped_rich_policy_smooth_oos_2025_2026_20260821_v1`.

Therefore the canonical short contract remains **P0/F90 base-only research**.
No short inference, admission, portfolio, Kraken, or live-order configuration
is authorised by these results. Long contracts below remain unchanged.

## 0. Current operational authority and BCF challenger status

The only order-capable stack remains the current-v5 chain bound by the v59/v67
contracts above. The v60 overlay is an exchange-free inference successor: it
uses the same target-free candidates, 120-field contract, frozen Geometry/K9,
rich SimplePolicyOptimiser parent and Adaptive Exit V1 overlay, but replaces
the single-map admission/auction stage with a hash-bound dual confirmation.
Any v40 reference below is historical lineage only; it is not an authority to
score or submit an order.

The following research challenger is documented here so it can be reproduced
without being confused with the active live trader:

```text
BCF MC1 expected EV >= 30 bps
AND current-v5 MC1 expected EV >= 30 bps
→ common constrained portfolio
→ priority = BCF MC1 expected EV
→ optimized SimplePolicyOptimiser parent exit
```

It was selected from a frozen 2026 20-arm agreement/priority grid, then tested
once on 2025 without retuning. It is **not order-enabled** for two separate
reasons:

1. Exact Kraken one-minute data at `decision + 5 minutes` exists for only 9 of
   its 3,792 2026 portfolio entries (0.24%; none in June or July).  The
   reported historical economics therefore cannot yet be claimed to hold at a
   five-minute live fill delay.
2. Its historical data still lack a sufficiently broad exact five-minute
   Kraken latency archive. The dual scorer itself is now materialised and
   wired into the normal inference cycle, but order authority remains with the
   current-v5 control until that forward/latency gate is explicitly promoted.

The challenger receipts are
`agents/receipts/20260817_strict_r3_bcf_current_v5_agreement_blend_v1.json`
and `agents/receipts/20260817_strict_r3_bcf_current_dual_mc1_live_materialisation_v1.json`.
Its detailed technical handover is
`docs/BCF_CURRENT_V5_DUAL_MAPPER_CHALLENGER_HANDOVER_20260817.md`.

The absence of a full five-minute historical Kraken path is a validation-data
gap, not a negative result. It must be closed with an exact Kraken minute
archive or a separately labelled latency proxy before the challenger receives
...
The user explicitly authorized live execution and the 2026-08-17 rich-policy
transition. `config/strict_r3_kraken_live_execution_v57_v67_rich_policy.json`
is the order-capable execution contract. It binds the v59 inference bundle,
the exact frozen rich-policy artifact, runtime-code hashes, and
`config/strict_r3_kraken_live_activation_authorization_20260817_v22_rich_policy.json`.
The manual override covers only incomplete forward-evidence count gates; it
does not override freshness, point-in-time feature identity, admission
identity, long-only scope, leverage/margin limits, execution-friction checks,
protective-stop installation, or exit replay. No previous-hour signal may be
resubmitted after its sealed receipt exists.
...
creates no new entry, while existing rich-policy/Adaptive Exit V1
...
evidence only. New live decisions use the sealed v59/v67 top-30 routing
contract; earlier schemas remain immutable historical reconciliation evidence.
...
After the common portfolio auction, the live executor fetches the
contemporaneous bid/ask and visible book for the intended contract count. It
computes the following final executable-EV safety value:
...
`raw_expected_gross_bps` is the mapped MC1 policy-net estimate plus its
historical 100-bps policy cost, so this cost is neither silently omitted nor
deducted twice. The impact term is the full-size buy VWAP impact from the live
book. A favourable execution-delay gap is not credited.

Under active v112, a selected auction winner is submitted only when this
value is at least **+30 bps**. This is a final veto, never a re-ranking step:
the live book cannot promote a lower-ranked candidate or displace the auction
winner. Observed spread and absolute decision-open gap remain telemetry rather
than independent vetoes. The book is still technically mandatory for sizing
and installation of the full-position VWAP-targeted reduce-only stop. Exchange
failure, invalid/partial fill handling, or inability to install that stop also
fails closed because a live position cannot safely be created.
...
→ current-v5 frozen MC1_d2 absolute expected-net map
→ BCF same-model scorer + BCF-native MC1 absolute expected-net map
→ dual admission: both MC1 maps ≥ +30 bps
→ common auction admitted rows by BCF MC1 expected net
→ constrained portfolio
→ final non-reranking execution-adjusted EV ≥ +30 bps
→ executable sizing and immediate full-position protective stop
→ SimplePolicyOptimiser parent exit
→ Adaptive Exit V1 activation-only overlay
```

Robust-21 is still telemetry only. The current-v5 and BCF MC1 maps are not
numerically blended: the current-v5 map is a required confirmation while the
BCF-native map supplies the auction priority. Both must be available and at
least +30 bps; neither can manufacture an admission alone. The BCF priority
replaces `final_score` only inside the dual successor’s common auction.
...
Future-path completeness is not a candidate criterion. Cross-sectional
features are computed before candidate eligibility filters, from the
point-in-time market universe. After the portfolio auction, live
spread/delay/impact form one non-reranking executable-EV ≥ +30-bps safety
veto. The visible book remains required for executable sizing and immediate
stop protection. Missing required technical inputs or insufficient visible
depth to protect the full position fail closed for that row.

For every new live entry, v112 stores the complete public ticker response and
every bid/ask level returned by Kraken at preflight, alongside the bounded
stop-calculation snapshot. For every confirmed market/policy exit it captures
the same public ticker and returned book depth **after** the reduce-only fill;
this preserves exit evidence without delaying the close. Exchange-detected
protective fills receive the equivalent detection-time snapshot. These are
immutable receipt/state audit fields only and cannot affect entry, ranking,
or exit decisions.
...
### 3.8 Robust-21 telemetry, current MC1, and the dual BCF/current successor
...
The sealed v60 inference successor runs a separate BCF-native scorer against
the frozen August BCF monthly bundle and its own preceding-42-day same-model
reserve. Its ten residual-head outputs form BCF-native agreement features; a
separate BCF MC1 depth-2 map then estimates expected policy net bps. It is not
the current-v5 mapper applied to a BCF score.

```text
bcf_admit     = bcf_mc1_expected_net_bps >= +30
current_admit = mc1_d2_expected_net_bps >= +30
dual_admit    = bcf_admit AND current_admit
auction order = bcf_mc1_expected_net_bps descending
```

Missing BCF/current score, reserve, map output, routed current score, or
feature-complete base row rejects the candidate. The BCF map therefore adds
confirmation and priority; it cannot create a current-v5 admission by itself.
The rich SimplePolicyOptimiser parent plus Adaptive Exit V1 activation overlay
is retained unchanged after the auction.
...
Active dual-successor files:

- `config/strict_r3_inference_overlay_long_20260801_v89_bcf_current_dual_admission_only_execution.json`
- `config/strict_r3_kraken_live_execution_v87_v109_bcf_current_dual_admission_only_execution.json`
- `config/strict_r3_kraken_live_activation_authorization_20260818_v17_bcf_current_dual_admission_only_execution.json`
- `config/strict_r3_bcf_current_dual_mc1_portfolio_challenger_v1.json`
- `scripts/score_strict_r3_bcf_forward.py`
- `scripts/admit_strict_r3_bcf_current_dual_forward.py`
- `extreme_price_movements/strict_r3_bcf_mc1_mapper.py`
- `data_perp/artifacts/strict_r3_bcf_mc1_d2_canonical_long_20260801_native_v1/`

The first immutable full-cycle receipt is
`data_perp/artifacts/strict_r3_bcf_current_dual_fullcycle_smoke_20260817T050000Z_v5/run_manifest.json`.
It validated all target-free, same-model-reserve, Geometry/K9, policy-cost,
current-map availability, BCF-map identity, dual-admission, and common-auction
checks. It had zero admissions at that hour because both maps were below the
frozen threshold—not because any stack component was skipped.

### 3.9 Portfolio auction and sizing

Only admitted rows enter the auction. Under current-v5 control they are ordered
by `final_score`; under the v60 dual successor they are ordered by
`bcf_mc1_expected_net_bps` after both +30-bps checks pass.
...
Dual successor contract:

- `config/strict_r3_bcf_current_dual_mc1_portfolio_challenger_v1.json`

### 3.10 Frozen rich SimplePolicyOptimiser exit

The active exit is the **frozen rich SimplePolicyOptimiser challenger**, not
the predecessor three-parameter parent. It was selected on 2024 development
only (48 sequential trials; January–June calibration and July–December
selection) and was evaluated without retuning on 2025 and 2026. It keeps the
same 12-hour timeout, 15-minute source-aligned policy convention, and a
one-time 100-bps historical cost convention.

| Element | Frozen value |
|---|---:|
| Stop ATR transform | 1.25 × ATR, power 1.30, multiplier 4.3797493 |
| Stop absolute bounds | 0.60% floor; 5.00% cap |
| TP ATR transform | 1.25 × ATR, power 0.70, multiplier 1.00 |
| Trailing activation | 1.0474796 × transformed TP; min 0.50× / 0.75% floor / 2.00% cap |
| Activation schedule | decay starts after 16 bars, 32-bar half-life |
| Giveback | fixed 0.08897237 × transformed TP distance |
| Capital protection | activates at 3.0× transformed TP; regression 0.7612365; 10-bps minimum lock |
| Fast-adverse guard | enabled; theta 1.5868990, min MAE 1.0 ATR, min speed 0.45, p65 severity |
| Timeout / historical cost | 12 hours / 100 bps once |
| Live bar convention | only completed post-fill one-minute bars; policy distances retain the frozen 15-minute ATR anchor |

Artifact and implementation:

- `data_perp/artifacts/strict_r3_rich_policy_hpo_long_20260817_v1/frozen_challenger.json`
  (`e72db968478889ef1e87d1b737a0d4995e23af2f0a29ddf594139e7fdda66d4a`)
- `extreme_price_movements/strict_r3_rich_policy.py`
- `scripts/run_strict_r3_rich_policy_hpo.py`
- `extreme_price_movements/inference/strict_r3_live_execution.py`

The source-aligned research comparison held the score/portfolio selection
fixed. Relative to the predecessor exit, the rich policy improved 2025 from
+123.96 to **+127.81 bps/trade** (1,103 to 1,434 exits) and 2026 Jan–Jul from
+80.57 to **+91.25 bps/trade** (944 to 1,204 exits), while reducing the
reported maximum drawdowns. These are policy research results, not a new
full-stack MC1 re-selection; live execution is governed by the exact frozen
artifact above.

### 3.11 Adaptive Exit V1

Adaptive Exit V1 is **not a standalone exit policy**. It sits on top of the
frozen rich policy and has authority only as a bounded multiplier of that
policy's trailing activation. Its own activation schedule, stop, giveback,
capital protection, fast-adverse guard, timeout, entry, and cost convention
remain unchanged.

For the active rich branch, the sealed Adaptive V1 result is converted to a
relative activation multiplier at entry and applied to the rich policy's
activation base. The one-minute controller never recomputes ML output on a
minute tick: it only applies the persisted multiplier to completed post-fill
bars. The F4 controller abstains when its disagreement exceeds the frozen p80
threshold; missing or unsupported inputs use an exact 1.0 multiplier. A fresh
15-minute re-evaluation during an open position is not yet part of v67 and
must be separately versioned before introduction.
...
- `scripts/admit_strict_r3_mc1_forward.py` — Robust-21 telemetry and MC1_d2
  authority.
- `scripts/score_strict_r3_bcf_forward.py` — BCF same-model scorer with its
  own prior-42-day reserve; never consumes held-window ranks or outcomes.
- `scripts/admit_strict_r3_bcf_current_dual_forward.py` — fail-closed dual
  +30-bps confirmation and BCF-EV priority handoff.
- `extreme_price_movements/strict_r3_mc1_mapper.py` — frozen MC1 mapper.
- `extreme_price_movements/strict_r3_bcf_mc1_mapper.py` — BCF-native ten-head
  agreement feature derivation and BCF MC1 map.
...
6. the frozen rich-policy stop transform, activation schedule, giveback,
   capital-protection and fast-adverse state, timeout, and one-time cost,
   followed by the Adaptive Exit V1 activation multiplier and bundle identity.

Adaptive Exit V1 is always described as an activation-only modulator of the
frozen rich policy. If its inputs or bundle are unavailable, if it abstains,
or if it has not yet been evaluated, the email explicitly identifies the rich
policy's unmodified activation as the fallback; no adaptive value is inferred
or silently imputed.
...
Under the active dual-admission live successor, common explicit rejection
states are: incomplete base contract, below top-30% base route, missing MC1
component, either BCF or current-v5 MC1 EV below +30 bps, duplicate symbol,
entry-rate limit, concurrency limit, margin cap, final execution-adjusted EV
below +30 bps, or a technical inability to size and immediately protect a live
exchange position. Observed spread and price gap remain telemetry; only the
explicit execution-adjusted-EV calculation is a post-auction veto. Missing
Adaptive inputs fall back to the parent exit rather than blocking entry.
...
| Exit behavior | frozen rich SimplePolicy policy and Adaptive Exit bundle |

## 8. Canonical identities

| Component | Identity |
|---|---|
| Active inference bundle v59 | `1cc7587ec9dbf000b0bf3983ea139b0bafdc0e2d5a13424d6070759342ab1e3d` |
...
| Frozen rich policy winner | `e72db968478889ef1e87d1b737a0d4995e23af2f0a29ddf594139e7fdda66d4a` |
| Portfolio policy | `cffb9e9fe3ff1f3509fe421227a0cd04944e12303049ca276f481f93b8b13cf4` |
| Kraken execution contract v67 | `config/strict_r3_kraken_live_execution_v57_v67_rich_policy.json` (order authority true; code hashes sealed within) |
| Live authorization v22/rich policy | `4ba27a62bc3c8398649c1023a01afd882a330123b5bd6dc70b2dd48af671e727` |
