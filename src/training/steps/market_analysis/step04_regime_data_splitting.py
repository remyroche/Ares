...
class RegimeDataSplittingStep(BaseStep):
    ...
    def _initialize_step(self) -> None:
        ...
        if abs(total_ratio - 1.0) > 0.001:
            self.logger.warning(
                f"Split ratios sum to {total_ratio}, normalizing to 1.0"
            )
    ...
    async def execute_logic(
        ...
        split_data = self._split_data(
            unified_data,
            regime_labels,
            stratify=self.stratify_by_regime
        )
    ...
    def validate_outputs(self, pipeline_state: Dict[str, Any]) -> Tuple[bool, list]:
        ...
        if all(key in pipeline_state for key in ["train_data", "val_data", "test_data"]):
            ...
            if total_len == 0:
                ...
                # Check if splits roughly match expected ratios
        ...
        if "regime_statistics" in pipeline_state:
            ...
            for regime_id, stats in regime_stats.items():
                if stats["count"] < self.min_regime_samples:
                    self.logger.warning(
                        f"Regime {regime_id} has only {stats['count']} samples "
                        f"(minimum: {self.min_regime_samples})"
                    )
    ...
    def _analyze_regime_distribution(self, regime_labels: np.ndarray) -> Dict[str, Any]:
        ...
        for regime, count in zip(unique_regimes, counts):
            regime_stats[int(regime)] = {
                ...
                "sufficient_samples": count >= self.min_regime_samples
            }
    ...
    def _split_data(
        ...
        self.logger.info(
            f"✅ Split data: Train={len(train_indices)}, "
    ...
    def _log_regime_stats(self, regime_stats: Dict[str, Any]) -> None:
        ...
        for regime_id, stats in regime_stats.items():
            if isinstance(stats, dict):
                self.logger.info(
                    f"   Regime {regime_id}: {stats['count']} samples "
                    f"({stats['percentage']:.1f}%) - "
                    f"{'✅' if stats['sufficient_samples'] else '⚠️ Low samples'}"
                )
        
        if "total_transitions" in regime_stats:
            self.logger.info(
                f"   Total transitions: {regime_stats['total_transitions']}"
            )
            self.logger.info(
                f"   Avg regime duration: {regime_stats['avg_regime_duration']:.1f} samples"
            )
    ...
    def _generate_split_reports(
        ...
        summary_lines = [
            "Data Split Summary",
            "=" * 40,
            f"Total samples: {sum(len(df) for df in split_data.values() if isinstance(df, pd.DataFrame))}",
            "",
            "Split sizes:"
        ]
        ...
        for split_name in ["train", "val", "test"]:
            if split_name in split_data:
                ...
                detail_lines.extend([
                    ...
                    f"  Shape: {df.shape}",
                    f"  Date range: {df.index.min()} to {df.index.max()}" if hasattr(df.index, 'min') else "",
                    f"  Features: {sum(1 for col in df.columns if col.startswith('feature_'))}",
                    f"  Original columns: {sum(1 for col in df.columns if not col.startswith('feature_'))}"
                ])
    ...
    def get_produced_outputs(self) -> list:
        ...
        return [
            "unified_data", "train_data", "val_data", "test_data",
            "split_indices", "regime_statistics", "split_reports"
        ]
