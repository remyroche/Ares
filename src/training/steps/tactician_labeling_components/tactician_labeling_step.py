...
class TacticianLabelingStep(BaseStep):
    ...
    def _initialize_step(self) -> None:
        ...
        # Initialize tactician configuration
    ...
    def _initialize_tactician_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "analyst_signal_requirement": True,
    ...
    def validate_inputs(
        ...
        # Check for required analyst ensemble data
    ...
    @handles_errors(
        ...
        context="tactician labeling logic"
    )
    async def execute_logic(
        ...
        for regime_id, regime_info in regime_data.items():
            ...
            regime_market_data = self._get_regime_market_data(
            ...
            if len(regime_market_data) < self.tactician_config["min_regime_samples"]:
                self.logger.warning(
                    ...
                    f"({len(regime_market_data)} < {self.tactician_config['min_regime_samples']})"
                )
                continue
            
            # Get regime-specific analyst predictions
            regime_analyst_predictions = await self._get_regime_analyst_predictions(
                regime_id, analyst_ensembles, regime_market_data
            )
            
            # Apply regime-specific labeling
            regime_labeled = await self._apply_regime_labeling(
                regime_id,
                regime_market_data,
                regime_analyst_predictions,
                regime_info
            )
        ...
        # Combine labeled data across regimes
        ...
        if self.tactician_config["enable_quality_filters"]:
            ...
            combined_labeled_data = await self.quality_filter.apply_global_filters(
                combined_labeled_data
            )
        
        # Optimize precision thresholds
        if self.tactician_config["enable_high_precision_mode"]:
            self.logger.info("🎯 Optimizing precision thresholds...")
            precision_results = await self.precision_optimizer.optimize_thresholds(
                combined_labeled_data,
                labeling_results
            )
        else:
        ...
        # Add labeling summary
        result["tactician_labeling_summary"] = self._create_labeling_summary(
            labeled_data, labeling_results, barrier_results
        )
        
        return result
    
    async def _apply_regime_labeling(
        self,
        regime_id: str,
        ...
        try:
            ...
            regime_barriers = await self.barrier_calculator.calculate_regime_barriers(
                regime_data,
                regime_info,
                self.tactician_config["barrier_combinations"]
            )
            
            # Apply regime-specific quality filters
            if self.tactician_config["regime_specific_quality_filters"]:
                quality_filters = await self.quality_filter.get_regime_filters(
                    regime_id, regime_data
                )
                filtered_data = await self.quality_filter.apply_regime_filters(
                    regime_data, quality_filters
                )
            else:
            ...
            if self.tactician_config["regime_specific_precision"]:
                precision_thresholds = await self.precision_optimizer.get_regime_thresholds(
                    regime_id, filtered_data, analyst_predictions
                )
            else:
                precision_thresholds = {
                    "min_signal_strength": self.tactician_config["min_signal_strength"],
                    "precision_threshold": self.tactician_config["precision_threshold"]
                }
            
            # Apply labeling strategy
            labeled_data = await self.labeling_strategy.apply_triple_barrier_labeling(
                filtered_data,
                regime_barriers,
                analyst_predictions,
                precision_thresholds,
            ...
            labeling_stats = self._calculate_labeling_statistics(
                labeled_data,
                filtered_data,
                len(regime_data)
            )
            ...
            self.logger.info(
                f"  ✅ Regime {regime_id} labeling complete: "
                f"{labeling_stats['labeled_samples']} labeled, "
                f"{labeling_stats['positive_rate']:.2%} positive rate"
            )
    ...
    async def _get_regime_analyst_predictions(
        self,
        regime_id: str,
        ...
        if regime_id in analyst_ensembles:
            ...
            # Use cross-regime ensemble if available
            if "cross_regime" in analyst_ensembles:
    ...
    def _combine_regime_labeled_data(
        self,
    ...
    def _calculate_labeling_statistics(
        self,
        ...
        stats = {
            ...
            "filtering_rate": 1 - (len(filtered_data) / original_size) if original_size > 0 else 0,
            "labeling_rate": len(labeled_data) / len(filtered_data) if len(filtered_data) > 0 else 0,
        }
        ...
        if 'label' in labeled_data.columns:
            ...
            stats.update({
                ...
                "positive_rate": label_counts.get(1, 0) / total_labeled if total_labeled > 0 else 0,
                "negative_rate": label_counts.get(0, 0) / total_labeled if total_labeled > 0 else 0,
            })
            
            # Multi-class statistics if applicable
    ...
    def _create_labeling_summary(
        ...
        summary = {
            "total_regimes": len(labeled_data),
            "successfully_labeled_regimes": len(labeling_results),
            "total_samples": sum(len(df) for df in labeled_data.values()),
            "regime_statistics": {},
            "average_metrics": {
                "filtering_rate": [],
                "positive_rate": [],
        ...
        for regime_id, results in labeling_results.items():
            if "statistics" in results:
                ...
                summary["regime_statistics"][regime_id] = {
                    "samples": stats.get("labeled_samples", 0),
    ...
    def validate_outputs(self, pipeline_state: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        required_outputs = [
            ...
            "tactician_labeling_results",
            "tactician_barrier_results",
            "tactician_labeling_summary"
        ]
    ...
    def get_required_inputs(self) -> List[str]:
        ...
        return [
            ...
            "market_data",
            "regime_data",
            "step13_analyst_ensemble_creation_completed"
        ]
