...
class DynamicWeighter:
    ...
    def __init__(self, config):
        ...
        self.missed_opportunity_penalty = self.config.get("missed_opportunity_penalty", -0.5)
        self.incorrect_penalty = self.config.get("incorrect_penalty", -1.5)
        self.correct_reward = self.config.get("correct_reward", 1.0)

    def run_daily_adjustment(self, ensemble_orchestrator, prediction_history: pd.DataFrame):
        """
        ...
        recent_history = prediction_history[prediction_history['timestamp'] >= start_date]

        if recent_history.empty:
            self.logger.warning("No recent prediction history found. Skipping weight adjustment.")
            return
    ...
    def _adjust_ensemble_weights(self, ensemble, history):
        ...
        if max_abs_score == 0: return # Avoid division by zero
        ...
        for model_name, score in normalized_scores.items():
            ...
            adjusted_weights[model_name] = max(0.01, current_weight * (1 + adjustment_factor)) # Ensure weight doesn't go to 0

        total_adjusted_weight = sum(adjusted_weights.values())
        final_weights = {k: v / total_adjusted_weight for k, v in adjusted_weights.items()}

        self.logger.info(f"Updated weights for {ensemble.ensemble_name} from {current_weights} to {final_weights}")
