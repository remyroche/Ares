...
vectorized_feature_engineering = PipelineStandards.safe_import(
    "src.training.steps.vectorized_advanced_feature_engineering", None
)
    ...
    try:
        ...
        if regime_data is not None:
            ...
            if "composite_cluster_id" in regime_data.columns:
                ...
                if regime_columns:
                    merged_data = merged_data.merge(regime_data[regime_columns], on="timestamp", how="left")
        ...
        if labeled_data is not None:
            ...
            if label_columns:
                merged_data = merged_data.merge(labeled_data[["timestamp"] + label_columns], on="timestamp", how="left")
        ...
        features_df = await _enhanced_integration_with_vectorized_features(
            features_df, feature_engineer, symbol, exchange, timeframe
        )
...
def _create_basic_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    features["vwap"] = (data["close"] * data["volume"]).rolling(window=20).sum() / data["volume"].rolling(
        window=20
    ).sum()
...
def _create_rolling_window_features(features: pd.DataFrame) -> pd.DataFrame:
    ...
    system_logger.info(
        f"✅ Created {len(windows) *
                                    7 +
                                    (len(windows) *
                                     6 if 'vwap_returns' in features.columns else 0) +
                                    (3 *
                                     5 if 'vwap_momentum_20' in features.columns else 0)} rolling window features"
    )
    return features
...
def _add_regime_aware_features(features: pd.DataFrame, data: pd.DataFrame) -> pd.DataFrame:
    ...
    for regime in data["regime"].unique():
        if pd.notna(regime):
            ...
            if "vwap_returns" in features.columns:
                features[f"regime_{regime}_vwap_returns_mean"] = (
                    features["vwap_returns"].where(regime_mask).rolling(20).mean()
                )
                features[f"regime_{regime}_vwap_volatility_mean"] = (
                    features["vwap_volatility_20"].where(regime_mask).rolling(20).mean()
                )
                features[f"regime_{regime}_vwap_momentum_mean"] = (
                    features["vwap_momentum_20"].where(regime_mask).rolling(20).mean()
                )
...
def _enhance_hmm_features(features: pd.DataFrame, regime_data: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        system_logger.info(
            f"✅ Enhanced features with HMM feature enhancer: {len(enhanced_features.columns)} total features"
        )
...
def _add_statistical_features(features: pd.DataFrame) -> pd.DataFrame:
    ...
    # Z-score features
    features["returns_zscore"] = (features["returns"] - features["returns"].rolling(20).mean()) / features[
        "returns"
    ].rolling(20).std()
    features["volume_zscore"] = (features["volume"] - features["volume"].rolling(20).mean()) / features[
        "volume"
    ].rolling(20).std()

    # VWAP Z-score features
    if "vwap_returns" in features.columns:
        features["vwap_returns_zscore"] = (
            features["vwap_returns"] - features["vwap_returns"].rolling(20).mean()
        ) / features["vwap_returns"].rolling(20).std()
        features["vwap_momentum_zscore"] = (
            features["vwap_momentum_20"] - features["vwap_momentum_20"].rolling(20).mean()
        ) / features["vwap_momentum_20"].rolling(20).std()

    return features


async def _add_sr_features(features: pd.DataFrame, market_data: pd.DataFrame, config: dict[str, Any]) -> pd.DataFrame:
    ...
    try:
        ...
        existing_sr_features = [
            col
            for col in features.columns
            if any(
                keyword in col.lower() for keyword in ["sr_", "support", "resistance", "pivot", "breakout", "proximity"]
            )
        ]
        ...
        if existing_sr_features:
            system_logger.info(
                f"⚠️ Found {
                    len(existing_sr_features)} existing SR features, will enhance rather than replace"
            )
        ...
        context_features = {
            ...
            "sr_enhanced_support_strength": (
                np.mean([level.get("enhanced_strength", 0.5) for level in sr_context.get("support_levels", [])])
                if sr_context.get("support_levels")
                else 0.5
            ),
            "sr_enhanced_resistance_strength": (
                np.mean([level.get("enhanced_strength", 0.5) for level in sr_context.get("resistance_levels", [])])
                if sr_context.get("resistance_levels")
                else 0.5
            ),
            # Clustering features
            ...
            "sr_order_flow_hvns": len(
                sr_context.get("order_flow_analysis", {}).get("volume_profile", {}).get("high_volume_nodes", [])
            ),
...
async def _add_sr_aware_feature_selection(
    features: pd.DataFrame, market_data: pd.DataFrame, config: dict[str, Any]
) -> pd.DataFrame:
    ...
    try:
        ...
        features["sr_combined_strength"] = (
            sr_context.get("support_strength", 0.5) + sr_context.get("resistance_strength", 0.5)
        ) / 2
        ...
        if sr_zone_width > 0 and current_price > 0:
            zone_position_pct = (
                (current_price - sr_context.get("nearest_support", current_price)) / current_price / sr_zone_width
            )
        else:
        ...
        features["sr_trend_pct"] = (market_data["close"].iloc[-1] - market_data["close"].iloc[-20]) / market_data[
            "close"
        ].iloc[-20]
...
async def _add_sr_optimization_features(
    ...
    try:
        ...
        if optimized_params:
            ...
            features["sr_optimized_strength_weights"] = np.mean(
                list(optimized_params.get("strength_weights", {}).values())
            )
            features["sr_optimized_dbscan_eps"] = optimized_params.get("dbscan_params", {}).get("eps", 0.01)
            features["sr_optimized_dbscan_min_samples"] = optimized_params.get("dbscan_params", {}).get(
                "min_samples", 3
            )
            features["sr_optimized_fibonacci_sensitivity"] = optimized_params.get("advanced_params", {}).get(
                "fibonacci_sensitivity", 0.7
            )
            features["sr_optimized_elliott_confidence"] = optimized_params.get("advanced_params", {}).get(
                "elliott_confidence_threshold", 0.6
            )
            features["sr_optimized_order_flow_threshold"] = optimized_params.get("advanced_params", {}).get(
...
async def _enhanced_integration_with_vectorized_features(
    ...
    try:
        ...
        additional_features = await feature_engineer.engineer_features(
            price_data=features[["open", "high", "low", "close"]], volume_data=features[["volume"]]
        )
...
async def _save_feature_artifacts(
    features_result: Dict[str, Any], symbol: str, exchange: str, timeframe: str, data_dir: str
) -> bool:
