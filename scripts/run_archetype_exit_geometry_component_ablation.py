    ...
    for bundle in bundles:
        ...
        for arch_value, group in bundle.rows.groupby(arch_col, sort=True, dropna=False):
            ...
            if geometry is not None:
                selected_geometry = dict(geometry[0])
                if override_fields is not None:
                    selected_geometry = {
                        key: value
                        for key, value in selected_geometry.items()
                        if key in override_fields
                    }
                local.update(selected_geometry)
...
def main() -> None:
    ...
    # Every component arm preserves the exact candidate ledger and portfolio
    # admission contract.  Only the named exit parameter(s) may vary by the
    # pre-existing side/archetype label; this avoids turning the archetype into
    # a hidden trade-admission gate.
    component_sets = {
        "side_archetype_sl_only_parent_evcurve": {"sl_mult", "sl_abs_cap_pct"},
        "side_archetype_activation_only_parent_evcurve": {
            "trailing_activation_mult", "trailing_activation_cap_pct",
            "trailing_activation_decay_half_life_bars",
            "trailing_activation_decay_start_bars",
            "trailing_activation_min_mult",
        },
        "side_archetype_protection_only_parent_evcurve": {
            "capital_protect_mfe_mult", "capital_protect_regression_frac",
            "capital_protect_lock_frac", "capital_protect_min_lock_bps",
        },
        "side_archetype_atr_shape_only_parent_evcurve": {"atr_power", "atr_multiplier"},
        "side_archetype_giveback_only_parent_evcurve": {"giveback_beta"},
        "side_archetype_trailing_power_only_parent_evcurve": {"trailing_power"},
    }
    component_ledgers = {
        arm: _materialize_geometry_ledger(
            bundles=bundles,
            parent_params=parent_params,
            archetype_geometry=archetype_geometry,
            mode="side_archetype",
        ...
        for arm, fields in component_sets.items()
    }
    ...
    # `simulate_and_score` can reject an entry during its internal execution
    # sanitisation when a particular policy has a stricter executable-price
    # path.  That must never become an apparent exit-policy advantage.  Score
    # every arm on the common path-valid population only; the subsequent
    # portfolio replay may still differ naturally because exits free capacity.
    common_source_ids: set[int] | None = None
    for ledger in ledgers.values():
        if "__source_row_id" not in ledger.columns:
            raise ValueError("Prepared candidate identity missing from replay ledger")
        ids = set(pd.to_numeric(ledger["__source_row_id"], errors="coerce").dropna().astype(int))
        common_source_ids = ids if common_source_ids is None else common_source_ids & ids
    if not common_source_ids:
        raise ValueError("No common path-valid candidates across replay arms")
    ledgers = {
        arm: ledger.loc[
            pd.to_numeric(ledger["__source_row_id"], errors="coerce").isin(common_source_ids)
    ...
    for arm, ledger in component_ledgers.items():
        ledger.to_parquet(ledger_dir / f"{arm}_candidates.parquet", index=False)
    ...
    manifest = {
        ...
        "common_path_valid_rows": int(len(common_source_ids)),
