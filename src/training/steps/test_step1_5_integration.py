...
class TestStep1_5Integration(unittest.TestCase):
    ...
    def test_secure_klines_download_integration(self, mock_download) -> None:
        ...
        with patch("glob.glob") as mock_glob, patch("pandas.read_csv") as mock_read_csv, patch(
            "pandas.concat"
        ) as mock_concat, patch("pandas.DataFrame.to_parquet") as mock_to_parquet:
            # Mock file discovery
            mock_glob.return_value = [
                f"data_cache/klines_{self.exchange}_{self.symbol}_{self.timeframe}_2024-01.csv"
            ]
        ...
        mock_download.assert_called_once_with(
            symbol=self.symbol, exchange_name=self.exchange, interval=self.timeframe
        )

    @patch("src.training.steps.step1_5_data_converter.download_all_data_with_consolidation")
    def test_security_validation_integration(self, mock_download) -> None:
    ...
    @patch("src.training.steps.step1_5_data_converter.download_all_data_with_consolidation")
    def test_data_quality_validation_integration(self, mock_download) -> None:
        ...
        # The step should handle quality issues; behavior depends on decorators

    @patch("src.training.steps.step1_5_data_converter.download_all_data_with_consolidation")
    def test_error_handling_integration(self, mock_download) -> None:
    ...
    @patch("src.training.steps.step1_5_data_converter.download_all_data_with_consolidation")
    def test_resource_monitoring_integration(self, mock_download) -> None:
