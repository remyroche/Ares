...
class EnhancedRegimePredictor:
    ...
    def predict_regime_changes(self, features: pd.DataFrame, hmm_probs: np.ndarray, hmm_states: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            regime_changes = self._detect_regime_changes_multi_signal(hmm_states, regime_stability, regime_entropy)
            transition_probs = self._calculate_transition_probabilities(hmm_probs, regime_changes)
            confidence_scores = self._calculate_prediction_confidence(regime_stability, regime_entropy, transition_probs)
            if self.persistence_model:
                persistence_adjustments = self._apply_persistence_model(regime_changes, hmm_states)
                confidence_scores *= persistence_adjustments
            predictions = self._create_prediction_events(regime_changes, hmm_states, transition_probs, confidence_scores)
            high_confidence_predictions = [pred for pred in predictions if pred['confidence'] >= self.confidence_threshold]
            self.logger.info(f'✅ Predicted {len(high_confidence_predictions)} high-confidence regime changes')
            return {'success': True, 'predictions': high_confidence_predictions, 'all_predictions': predictions, 'regime_stability': regime_stability.tolist(), 'regime_entropy': regime_entropy.tolist(), 'transition_probabilities': transition_probs.tolist(), 'confidence_scores': confidence_scores.tolist()}
        ...
        try:
            ...
            for i in range(len(regime_changes)):
                if regime_changes[i] and i < len(hmm_probs) - 1:
                    ...
                    transition_probs[i] = min(max_change * 5, 1.0)
    ...
    def _calculate_prediction_confidence(self, stability: np.ndarray, entropy: np.ndarray, transition_probs: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            for i in range(len(stability)):
                ...
                confidence = stability_confidence * 0.4 + (1 - entropy_penalty) * 0.3 + transition_boost * 0.3
        ...
        try:
            ...
            for i in range(len(regime_changes)):
                if regime_changes[i] and i < len(hmm_states) - 1:
                    event = {'timestamp_index': i, 'from_state': int(hmm_states[i]), 'to_state': int(hmm_states[i + 1]), 'transition_probability': float(transition_probs[i]), 'confidence': float(confidence_scores[i]), 'prediction_type': 'regime_change', 'prediction_horizon': 1, 'prediction_metadata': {'method': 'enhanced_multi_signal', 'signals_used': ['state_transition', 'stability', 'entropy', 'acceleration']}}
                    events.append(event)
            return events
        except Exception as e:
    ...
    def fit_persistence_model(self, regime_sequence: np.ndarray) -> bool:
        ...
        try:
            ...
            if best_distribution:
                ...
                self.persistence_model['statistics'] = {'mean_duration': float(np.mean(durations)), 'median_duration': float(np.median(durations)), 'std_duration': float(np.std(durations)), 'min_duration': int(np.min(durations)), 'max_duration': int(np.max(durations))}
    ...
    def get_model_summary(self) -> Dict[str, Any]:
        ...
        summary = {'persistence_model': None, 'adaptive_boundaries': None, 'configuration': {'stability_threshold': self.stability_threshold, 'min_persistence': self.min_persistence, 'entropy_percentile': self.entropy_percentile, 'confidence_threshold': self.confidence_threshold}}
        if self.persistence_model:
            summary['persistence_model'] = {'distribution_type': self.persistence_model.get('distribution_type'), 'mean_duration': self.persistence_model.get('mean_duration'), 'statistics': self.persistence_model.get('statistics', {})}
        if self.regime_boundaries:
            summary['adaptive_boundaries'] = {'n_boundaries': len(self.regime_boundaries.labels_) if hasattr(self.regime_boundaries, 'labels_') else 0, 'eps': self.regime_boundaries.eps, 'min_samples': self.regime_boundaries.min_samples}
        return summary