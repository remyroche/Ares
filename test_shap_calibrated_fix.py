#!/usr/bin/env python3
"""
Script de test pour vérifier que la correction SHAP avec CalibratedClassifierCV fonctionne correctement
"""
...
def test_shap_with_calibrated_model():
    ...
    try:
        ...
        calibrated_model.fit(X[:80], y[:80])
        ...
        try:
            ...
            if hasattr(calibrated_model, '__class__') and 'CalibratedClassifierCV' in str(type(calibrated_model)):
                ...
                if hasattr(calibrated_model, 'estimators_'):
                    # Pour CalibratedClassifierCV, les estimateurs sont dans estimators_
