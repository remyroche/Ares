...
async def initialize_health_monitoring():
    ...
    try:
        ...
        if sentinel:
            ...
            logger.error("❌ Failed to initialize Sentinel")
        ...
        logger.info(f"✅ Registered {len(health_checker.component_checkers)} components for health monitoring")
...
async def periodic_health_checks():
    ...
    while True:
        try:
            ...
            overall_status = health_summary.get("summary", {}).get("overall_status", "unknown")
            overall_score = health_summary.get("summary", {}).get("overall_score", 0)
            
            if overall_status == "healthy":
                logger.info(f"💚 System healthy - Score: {overall_score}")
            elif overall_status == "warning":
                logger.warning(f"⚠️ System warning - Score: {overall_score}")
            elif overall_status in ["degraded", "critical"]:
                logger.error(f"🔴 System {overall_status} - Score: {overall_score}")
            ...
            for component_name, health_data in components.items():
                ...
                if status not in ["healthy"]:
                    ...
                    logger.warning(f"⚠️ Component {component_name}: {status} - {issues}")
...
def integrate_health_check_with_component(component_instance: Any, component_type: str) -> Any:
    """
    ...
    try:
        ...
        logger.error(f"❌ Failed to integrate health checks with {component_type}: {e}")
        return component_instance
...
async def get_component_health_report(component_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if status == "error":
            report["recommendations"].append("Immediate attention required - component is non-functional")
        elif status == "critical":
            report["recommendations"].append("Critical issues detected - investigate immediately")
        elif status == "degraded":
            report["recommendations"].append("Performance degraded - monitor closely")
        elif status == "warning":
            report["recommendations"].append("Minor issues detected - schedule maintenance")
        elif health_score < 80:
            report["recommendations"].append("Health score below optimal - review configuration")
        ...
        logger.error(f"Error generating health report for {component_name}: {e}")
        return {
            "component": component_name,
            "error": str(e),
            "timestamp": time.time()
...
async def generate_system_health_dashboard() -> Dict[str, Any]:
    ...
    try:
        ...
        component_reports = {}
        for component_name in health_checker.component_checkers.keys():
            component_reports[component_name] = await get_component_health_report(component_name)
            
        # Create dashboard data
        dashboard = {
            "system_overview": {
                "status": health_summary.get("summary", {}).get("overall_status", "unknown"),
                "score": health_summary.get("summary", {}).get("overall_score", 0),
                "uptime": health_summary.get("summary", {}).get("system_uptime", 0),
                "component_count": health_summary.get("summary", {}).get("component_count", 0),
                "timestamp": time.time()
            },
            "resource_usage": {
            ...
            "component_status": {
                "healthy": health_summary.get("health", {}).get("system", {}).get("healthy_components", 0),
                "critical": health_summary.get("health", {}).get("system", {}).get("critical_components", 0),
                "degraded": health_summary.get("health", {}).get("system", {}).get("degraded_components", 0)
            },
...
async def demo_health_monitoring():
    ...
    health_summary = await health_checker.get_health_summary()
    print(f"Overall Status: {health_summary.get('summary', {}).get('overall_status', 'unknown')}")
    print(f"Health Score: {health_summary.get('summary', {}).get('overall_score', 0)}/100")
