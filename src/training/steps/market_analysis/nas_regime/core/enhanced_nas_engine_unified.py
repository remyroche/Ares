"""
Enhanced NAS Engine - Unified System Version
...
@dataclass
class NASConfig:
    """Configuration for NAS engine (legacy compatibility)."""
...
class EnhancedNASEngine:
    ...
    def __init__(self, config: Optional[NASConfig] = None):
        """Initialize the Enhanced NAS Engine using unified system."""
        ...
        tprint_info("🚀 Enhanced NAS Engine initialized using unified system")
        tprint_info(f"   Architecture type: {ArchitectureType.NEURAL.value}")
        tprint_info(f"   Search strategy: {SearchStrategy.ENHANCED_BAYESIAN.value}")
    ...
    def _initialize_unified_system(self):
        ...
        try:
            ...
            # Configure for neural architecture search
            unified_config.search_config.architecture_type = ArchitectureType.NEURAL
            unified_config.search_config.search_strategy = SearchStrategy.ENHANCED_BAYESIAN
            ...
            unified_config.optimization_config.objectives = ['accuracy', 'efficiency', 'complexity']
            unified_config.optimization_config.objective_weights = [0.4, 0.3, 0.3]
            ...
            unified_config.evaluation_config.architecture_type = ArchitectureType.NEURAL
            unified_config.evaluation_config.evaluation_types = ['economic_significance', 'trading_viability']
            
            # Configure regime detection
            unified_config.regime_detection_config.architecture_type = ArchitectureType.NEURAL
            ...
            tprint_success("✅ Unified system initialized for NAS")
    ...
    def search(self, search_space: Dict[str, Any], objective_function: Callable) -> Dict[str, Any]:
        ...
        """
        try:
            tprint_info("🔍 Starting neural architecture search...")
            ...
            tprint_success(f"✅ Neural architecture search completed in {legacy_result['execution_time']:.2f}s")
...
def create_enhanced_nas_engine(config: Optional[NASConfig] = None) -> EnhancedNASEngine:
    """Create an enhanced NAS engine with specified configuration."""
    return EnhancedNASEngine(config)

def quick_nas_search(search_space: Dict[str, Any], 
                    objective_function: Callable,
                    max_iterations: int = 100) -> Dict[str, Any]:
    """Quick NAS search using default configuration."""
    config = NASConfig(max_iterations=max_iterations)
