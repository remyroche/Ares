...
from ...training.steps.data_collection.data_preparation.sr_strength_optimizer import SRStrengthOptimizer
from .enhanced_sr_optimization_20250913_1422 import EnhancedSROptimizer
from .enhanced_sr_confluence_20250913_1422 import EnhancedSRConfluenceDetector
from .enhanced_sr_validation_20250913_1422 import EnhancedSRValidator
from .enhanced_sr_detection_20250913_1422 import EnhancedSRDetector
from .sr_breakout_predictor_enhanced_20250913_1422 import SRBreakoutPredictor
from .sr_context_aware_calculator_20250913_1422 import ContextAwareSRCalculator
from .sr_data_integration_20250913_1422 import SRDataIntegration
from .sr_ensemble_predictor_20250913_1422 import SREnsemblePredictor
from .sr_parameter_optimizer_20250913_1422 import SRParameterOptimizer
from .sr_performance_monitor_20250913_1422 import SRPerformanceMonitor
from .sr_weight_optimizer_20250913_1422 import SRWeightOptimizer
from .sr_levels_manager_20250913_1422 import SRLevelsManager
from ...utils.logger import system_logger
import logging
import time
...
class SRComprehensiveIntegration:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.config = config
        self.logger = logger
        self.components = {'strength_optimizer': None, 'enhanced_optimizer': None, 'confluence_detector': None, 'validator': None, 'detector': None, 'breakout_predictor': None, 'context_calculator': None, 'data_integration': None, 'ensemble_predictor': None, 'parameter_optimizer': None, 'performance_monitor': None, 'weight_optimizer': None, 'levels_manager': None}
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            initialization_order = [('detector', self._init_detector), ('strength_optimizer', self._init_strength_optimizer), ('breakout_predictor', self._init_breakout_predictor), ('context_calculator', self._init_context_calculator), ('confluence_detector', self._init_confluence_detector), ('validator', self._init_validator), ('enhanced_optimizer', self._init_enhanced_optimizer), ('data_integration', self._init_data_integration), ('ensemble_predictor', self._init_ensemble_predictor), ('parameter_optimizer', self._init_parameter_optimizer), ('performance_monitor', self._init_performance_monitor), ('weight_optimizer', self._init_weight_optimizer), ('levels_manager', self._init_levels_manager)]
            ...
            critical_success = all((self.initialization_status.get(comp, False) for comp in critical_components))
    ...
    async def _init_context_calculator(self) -> bool:
        ...
        try:
            self.components['context_calculator'] = ContextAwareSRCalculator(self.config)
    ...
    async def detect_sr_levels(self, market_data: pd.DataFrame, timeframe: str='1m', use_ensemble: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            results = {'support_levels': [], 'resistance_levels': [], 'confluence_zones': [], 'breakout_predictions': {}, 'context_analysis': {}, 'validation_results': {}, 'performance_metrics': {}, 'metadata': {'timestamp': datetime.now().isoformat(), 'data_points': len(market_data), 'timeframe': timeframe, 'components_used': []}}
            if self.components['detector']:
                try:
                    detection_result = await self.components['detector'].detect_levels(market_data, timeframe)
            ...
            if self.components['strength_optimizer'] and results['support_levels']:
                try:
                    optimized_levels = await self.components['strength_optimizer'].optimize_strength(results['support_levels'] + results['resistance_levels'], market_data)
            ...
            if self.components['context_calculator']:
                try:
                    context_result = await self.components['context_calculator'].calculate_context(market_data, results['support_levels'], results['resistance_levels'])
            ...
            if self.components['confluence_detector']:
                try:
                    confluence_result = await self.components['confluence_detector'].detect_confluence(results['support_levels'], results['resistance_levels'], market_data)
            ...
            if self.components['breakout_predictor']:
                try:
                    breakout_result = await self.components['breakout_predictor'].predict_breakouts(market_data, results['support_levels'], results['resistance_levels'])
            ...
            if use_ensemble and self.components['ensemble_predictor']:
                try:
                    ensemble_result = await self.components['ensemble_predictor'].predict(market_data, results)
            ...
            if self.components['validator']:
                try:
                    validation_result = await self.components['validator'].validate_levels(results['support_levels'], results['resistance_levels'], market_data)
            ...
            if self.components['performance_monitor']:
                try:
                    performance_metrics = await self.components['performance_monitor'].analyze_performance(results, market_data)
            ...
            self.logger.info(f"✅ S/R detection completed: {len(results['support_levels'])} support, {len(results['resistance_levels'])} resistance levels found")
            return results
        except Exception as e:
            self.logger.exception(f'❌ S/R detection failed: {e}')
            return {'error': str(e), 'support_levels': [], 'resistance_levels': [], 'metadata': {'timestamp': datetime.now().isoformat(), 'error': True}}

    async def optimize_parameters(self, market_data: pd.DataFrame, validation_data: Optional[pd.DataFrame]=None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            optimized_params = {'detection_params': {}, 'strength_params': {}, 'confluence_params': {}, 'breakout_params': {}, 'weight_params': {}, 'metadata': {'timestamp': datetime.now().isoformat(), 'data_points': len(market_data), 'optimizers_used': []}}
            if self.components['parameter_optimizer']:
                try:
                    param_result = await self.components['parameter_optimizer'].optimize(market_data, validation_data)
            ...
            if self.components['weight_optimizer']:
                try:
                    weight_result = await self.components['weight_optimizer'].optimize_weights(market_data, validation_data)
            ...
            return {'error': str(e), 'metadata': {'timestamp': datetime.now().isoformat(), 'error': True}}

    async def integrate_data(self, sr_results: Dict[str, Any], market_data: pd.DataFrame, additional_data: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        """
        ...
        try:
            if self.components['data_integration']:
                return await self.components['data_integration'].integrate(sr_results, market_data, additional_data)
            else:
    ...
    async def update_levels(self, market_data: pd.DataFrame, current_price: float, volume: Optional[float]=None) -> Dict[str, Any]:
        """
        ...
        try:
            if self.components['levels_manager']:
                return await self.components['levels_manager'].update_levels(market_data, current_price, volume)
            else:
