...
class HMMRegimeDiscoveryStep:
    ...
    @validate_pipeline_step(
        step_name="hmm_regime_discovery", validation_level="CRITICAL", enable_rollback=True, max_retries=2
    )
    ...
    async def execute(self, training_input: dict[str, Any], pipeline_state: dict[str, Any]) -> dict[str, Any]:
        ...
        if PSUTIL_AVAILABLE:
            ...
            self.logger.info(
                f"💾 Initial memory usage: {
                    initial_memory.percent:.1f}% ({
                    initial_memory.used /
                    1024**3:.1f}GB / {
                    initial_memory.total /
                    1024**3:.1f}GB)"
            )
        else:
            ...
            if "metrics" in regime_results and "reports" in regime_results:
                ...
                report_name = log_step_report(
                    ...
                    additional_metadata={
                        ...
                        "reports_generated": (
                            list(regime_results["reports"].keys()) if "reports" in regime_results else []
                        ),
                    },
                )
    ...
    @handle_errors(exceptions=(Exception,), default_return=False, context="data_quality_validation")
    ...
    async def _prepare_hmm_features(self, df: Any) -> Any:
        ...
        try:
            ...
            features["volume_price_trend_ratio"] = (
                features["volume_price_trend"] / features["volume_price_trend"].rolling(20).mean()
            )
    ...
    async def _perform_simple_regime_discovery(self, features: Any) -> dict[str, Any]:
        ...
        try:
            ...
            for i in range(total_periods):
                ...
                volume_ratio.iloc[i] if hasattr(volume_ratio, "iloc") else volume_ratio[i]
    ...
    def _interpret_hmm_states(self, features: Any, state_sequence: Any, state_probs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            for state in unique_states:
                ...
                self.logger.info(
                    f"   State {state} → {regime_name}: {
                        len(state_data)} periods ({
                        state_char['percentage']:.1f}%)"
                )
    ...
    def _map_state_to_regime(self, state_char: dict[str, Any]) -> str:
        ...
        try:
            ...
            state_char.get("volume_ratio_10_mean", 1)
            state_char.get("rsi_mean", 50)
            state_char.get("adx_mean", 25)
    ...
    def _detect_regime_changes_advanced(
        self, hmm_probs: np.ndarray, hmm_states: np.ndarray, threshold: float = 0.1, min_persistence: int = 3
    ) -> dict[str, Any]:
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=np.zeros(0, dtype=float), context="calculate_transition_confidence"
    )
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=np.zeros(0, dtype=float), context="calculate_regime_strength"
    )
    ...
    def _create_regime_change_events(
        self, transitions: np.ndarray, states: np.ndarray, confidence: np.ndarray, strength: np.ndarray
    ) -> list[dict[str, Any]]:
    ...
    def _extract_regime_characteristics(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if "price_momentum_10" in features.columns and "volatility_20" in features.columns:
                characteristics["momentum_volatility_ratio"] = features["price_momentum_10"] / (
    ...
    async def _enhance_regime_analysis_with_sr(
        self, regime_results: dict[str, Any], sr_context: dict[str, Any], market_data: pd.DataFrame
    ) -> dict[str, Any]:
        """Enhance regime analysis with SR context."""
        try:
    ...
    async def _create_sr_regime_features(
        self, regime_states: list[int], sr_context: dict[str, Any], market_data: pd.DataFrame
    ) -> dict[str, Any]:
        """Create SR-aware regime features."""
        try:
            features = {}
...
async def run_step(
    symbol: str, exchange: str, timeframe: str = "1m", data_dir: str = None, force_rerun: bool = False, **kwargs: Any
) -> bool:
    ...
    def _calculate_cluster_quality_metrics(
        ...
        try:
            ...
            metrics["cluster_balance"] = (
                metrics["std_cluster_size"] / metrics["mean_cluster_size"] if metrics["mean_cluster_size"] > 0 else 0
            )
    ...
    def _analyze_composite_clusters(
        self, features: Any, hmm_states: Any, cluster_labels: Any, cluster_metrics: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _create_composite_cluster_dataframe(
        self, features: Any, hmm_states: Any, cluster_labels: Any, composite_analysis: dict[str, Any]
    ) -> Any:
        ...
        try:
            ...
            for cluster_id, char in composite_analysis.get("cluster_characteristics", {}).items():
                ...
                df.loc[cluster_mask, "market_condition"] = composite_analysis.get("market_conditions", {}).get(
                    cluster_id, "unknown"
                )
    ...
    def _create_intensity_dataframe(
        ...
        try:
            ...
            for cluster_id in unique_clusters:
                ...
                intensity_df.loc[cluster_mask, "volatility_intensity"] = self._calculate_volatility_intensity(
                    features, cluster_mask
                )
                intensity_df.loc[cluster_mask, "momentum_intensity"] = self._calculate_momentum_intensity(
                    features, cluster_mask
                )
                intensity_df.loc[cluster_mask, "volume_intensity"] = self._calculate_volume_intensity(
                    features, cluster_mask
                )
    ...
    def _create_meta_information(
        ...
        try:
            ...
            meta = {
                ...
                "reports_summary": {"total_reports": len(reports), "report_types": list(reports.keys())},
                "feature_summary": {
                    "total_features": len(composite_analysis.get("feature_importance", {})),
                    "top_features": sorted(
                        composite_analysis.get("feature_importance", {}).items(), key=lambda x: x[1], reverse=True
                    )[:10],
                },
            }
    ...
    def _determine_market_condition(self, cluster_char: dict[str, Any]) -> str:
        ...
        try:
            ...
            cluster_char.get("feature_means", {}).get("volume_ratio_10", 1)
            cluster_char.get("feature_means", {}).get("rsi", 50)
    ...
    def _should_run_optimization(
        self, symbol: str, exchange: str, timeframe: str, data_dir: str, force_rerun: bool
    ) -> bool:
    ...
    async def _run_automatic_optimization(
        ...
        try:
            ...
            optimization_config = self._get_optimization_config()
            optimization_config.get("optimization_settings", {})
            auto_config = optimization_config.get("automatic_optimization", {})
            ...
            optimization_results = optimizer.optimize(
                ...
                study_name=f"{
                    auto_config.get(
                        'study_name_prefix',
                        'auto_optimization')}_{symbol}_{exchange}_{timeframe}",
            )
    ...
    def _get_optimization_config(self) -> Dict[str, Any]:
        ...
        return {
            "automatic_optimization": {"enabled": True, "max_trials": 50, "timeout_minutes": 30, "force_rerun_days": 7},
    ...
    async def _save_optimization_results(
        self, optimization_results: Dict[str, Any], symbol: str, exchange: str, timeframe: str, data_dir: str
    ) -> None:
        """Save optimization results."""
