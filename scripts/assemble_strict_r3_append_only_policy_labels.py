...
LABEL_COLUMNS = (
    "candidate_id", "side_name", "policy_path_valid", "policy_gross_bps", "policy_net_bps",
    "policy_exit_bar_15m", "policy_exit_reason", "policy_entry_price",
    "policy_exit_price", "policy_label_available_ts", "policy_cost_bps",
)
ECONOMIC_COLUMNS = tuple(
    column for column in LABEL_COLUMNS if column not in {"candidate_id", "side_name"}
)
...
def _require_side(frame: pd.DataFrame, *, side: str, purpose: str) -> pd.DataFrame:
    """Require a single explicit side; retain only legacy long compatibility."""
    result = frame.copy()
    if "side_name" not in result:
        if side != "long":
            raise ValueError(f"{purpose} lacks explicit short-side lineage")
        result["side_name"] = "long"
    observed = result["side_name"].astype(str).str.strip().str.lower()
    if not observed.isin(("long", "short")).all() or not observed.eq(side).all():
        raise ValueError(f"{purpose} must be side-local {side}")
...
def assemble_append_only_labels(
    *, immutable_prefix: pd.DataFrame, refreshed_labels: pd.DataFrame,
    target_free_population: pd.DataFrame, side: str = "long",
) -> tuple[pd.DataFrame, dict[str, object]]:
    prefix = _require_side(
        immutable_prefix, side=side, purpose="immutable prefix",
    ).loc[:, LABEL_COLUMNS].copy()
    refreshed = _require_side(
        refreshed_labels, side=side, purpose="refreshed labels",
    ).loc[:, LABEL_COLUMNS].copy()
    target = _require_side(
        target_free_population, side=side, purpose="target-free population",
    ).loc[:, ["candidate_id", "side_name"]].copy()
    for frame, name in (
...
def main() -> None:
    ...
    if str(policy_payload.get("side") or "").strip().lower() != args.side:
        raise ValueError("policy JSON side must match --side")
    prefix = pd.read_parquet(args.immutable_prefix)
    refreshed = pd.read_parquet(args.refreshed_labels)
    target_columns = ["candidate_id", "side_name"]
