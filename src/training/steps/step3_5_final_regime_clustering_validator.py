...
from src.utils.enhanced_validation_decorators import smart_validation_cache, validate_step3_5_comprehensive
...
class Step3_5FinalRegimeClusteringValidator(BaseValidator):
    ...
    async def validate_step3_5_final_regime_clustering(
        ...
        try:
            ...
            if not final_regime_dir.exists():
                self.logger.warning(f"⚠️ Final regime clustering directory not found: {final_regime_dir}")
                return False
    ...
    async def _validate_analysis_report(self, analysis_report: Path) -> bool:
        ...
        try:
            ...
            if missing_fields:
                self.logger.warning(f"⚠️ Missing required fields in analysis report: {missing_fields}")
                return False
            ...
            error_context = {"file": str(analysis_report), "error_type": type(e).__name__, "error_message": str(e)}
    ...
    async def _validate_characteristics_file(self, characteristics_file: Path) -> bool:
        ...
        try:
            ...
            for regime_id, characteristics in characteristics_data.items():
                ...
                if missing_basic:
                    self.logger.warning(f"⚠️ Missing basic characteristics for regime {regime_id}: {missing_basic}")
                    return False
            ...
            error_context = {"file": str(characteristics_file), "error_type": type(e).__name__, "error_message": str(e)}
    ...
    def validate_step_prerequisites(self, symbol: str, exchange: str, timeframe: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if param_optimization_file.exists():
                file_valid, file_metrics = self.validate_file_exists(
                    str(param_optimization_file), "parameter optimization results"
                )
                if not file_valid:
                    validation_result["warnings"].append(
                        f"Parameter optimization file validation failed: {param_optimization_file}"
                    )
            else:
                validation_result["warnings"].append(
