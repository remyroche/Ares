...
class MetaLabelingSystem:
    ...
    def _detect_entry_signals(
        ...
        try:
            ...
            current_price = data["close"].iloc[-1]
            vwap = rolling_mean(data["close"], window=20) if VECTORBT_AVAILABLE and len(data) > 1000 else data["close"].rolling(window=20).mean().iloc[-1]
            ...
            volume_spike = (
                volume_data["volume"].iloc[-1]
                / volume_rolling_mean(data["volume"], window=20) if VECTORBT_AVAILABLE and len(data) > 1000 else data["volume"].rolling(window=20).mean().iloc[-1]
            ...
            # Calculate order book imbalance from bid/ask volume if available
