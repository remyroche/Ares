...
class AsyncOrderExecutor:
    ...
    async def _execute_twap(self, request: ExecutionRequest, result: ExecutionResult) -> bool:
        ...
        """
        try:
            num_slices = max(1, int(request.time_limit / 60))
    ...
    async def _execute_iceberg(self, request: ExecutionRequest, result: ExecutionResult) -> bool:
        ...
        """
        try:
            visible_quantity = request.quantity * 0.1
    ...
    def get_performance_metrics(self) -> dict[str, Any]:
        ...
        """
        try:
            return {'total_executions': self.total_executions, 'successful_executions': self.successful_executions, 'failed_executions': self.failed_executions, 'success_rate': self.successful_executions / self.total_executions if self.total_executions > 0 else 0.0, 'total_volume_executed': self.total_volume_executed, 'average_slippage': self.total_slippage / self.total_executions if self.total_executions > 0 else 0.0, 'active_executions': len(self.active_executions), 'execution_history_size': len(self.execution_history)}
        except Exception as e:
