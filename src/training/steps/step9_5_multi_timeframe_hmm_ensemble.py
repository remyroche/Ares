...
class RegimeSpecificMultiTimeframeEnsemble:
    ...
    async def _load_regime_specific_data(
        ...
        try:
            ...
            self.logger.info(
                f"✅ Loaded {
                    len(unified_data)} samples with {
                    unified_data['composite_cluster_id'].nunique()} regimes"
            )
            return unified_data
        ...
        try:
            ...
            if "composite_cluster_id" in tf_data.columns:
                ...
                if len(regime_data) < self.regime_config["min_regime_samples"]:
                    self.logger.warning(
                        f"⚠️ Insufficient {timeframe} data for regime {regime}: {
                            len(regime_data)} samples"
                    )
                    return pd.DataFrame()
