...
async def export_database(db_path: str = "data/ares_local_db.sqlite"):
    ...
    try:
        ...
        if export_path:
            ...
            print(f"\n📋 Next steps:")
            print(f"   1. Copy the export file to your trading computer")
            print(f"   2. Run: python scripts/database_migration.py import {export_path}")
        else:
...
async def validate_file(file_path: str):
    ...
    try:
        ...
        print(f"   Checksum valid: {'✅' if validation_result['checksum_valid'] else '❌'}")
        print(f"   Database valid: {'✅' if validation_result['database_valid'] else '❌'}")
        
        if validation_result['checksum']:
            print(f"   Checksum: {validation_result['checksum']}")
        if validation_result['file_size']:
            print(f"   File size: {validation_result['file_size'] / 1024 / 1024:.2f} MB")
...
def print_usage():
    ...
    print("  python scripts/database_migration.py import data/migrations/trading_export_20231201_143022.sqlite")
    print("")
    print("  # Validate a migration file")
    print("  python scripts/database_migration.py validate data/migrations/trading_export_20231201_143022.sqlite")
    print("")
