        ...
        async def wrapper(*args, **kwargs) -> bool:
            ...
            try:
                if not regime_continuity_manager.regime_metadata:
                    success = await regime_continuity_manager.initialize_regime_continuity(symbol, exchange, timeframe, data_dir)
                    if not success:
                        logger.error(f'❌ Failed to initialize regime continuity for {step_name}')
                        return False
                if per_regime_required and regime_aware:
                    return await _execute_per_regime_step(func, step_name, symbol, exchange, timeframe, data_dir, args, kwargs)
                else:
                    return await _execute_standard_step(func, step_name, symbol, exchange, timeframe, data_dir, args, kwargs)
...
async def _execute_per_regime_step(func: Callable, step_name: str, symbol: str, exchange: str, timeframe: str, data_dir: str, args: tuple, kwargs: dict) -> bool:
    ...
    try:
        ...
        for regime_id in regime_ids:
            ...
            try:
                await regime_continuity_manager.update_step_status(step_name, regime_id, RegimeStatus.IN_PROGRESS)
                ...
                if not context:
                    ...
                    await regime_continuity_manager.update_step_status(step_name, regime_id, RegimeStatus.FAILED, error_message='No context found')
                    continue
                regime_kwargs = kwargs.copy()
                regime_kwargs.update({'regime_id': regime_id, 'regime_context': context, 'per_regime': True})
                result = await func(*args, **regime_kwargs)
                if result:
                    regime_results[regime_id] = result
                    successful_regimes += 1
                    await regime_continuity_manager.update_step_status(step_name, regime_id, RegimeStatus.COMPLETED, metadata={'result': str(result) if result else None})
                    logger.info(f'✅ Completed regime {regime_id} for {step_name}')
                else:
                    await regime_continuity_manager.update_step_status(step_name, regime_id, RegimeStatus.FAILED, error_message='Step function returned False')
        ...
        if regime_results:
            await _aggregate_regime_results(step_name, regime_results, symbol, exchange, timeframe, data_dir)
...
async def _execute_standard_step(func: Callable, step_name: str, symbol: str, exchange: str, timeframe: str, data_dir: str, args: tuple, kwargs: dict) -> bool:
...
async def _aggregate_regime_results(step_name: str, regime_results: Dict[int, Any], symbol: str, exchange: str, timeframe: str, data_dir: str) -> None:
