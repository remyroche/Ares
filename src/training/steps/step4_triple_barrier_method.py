...
class TripleBarrierMethodStep:
    ...
    def _initialize_components(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                "✅ Optimized triple barrier labeler initialized successfully"
            )
        except ImportError as e:
            self.logger.warning(
                f"⚠️ Could not import OptimizedTripleBarrierLabeling: {e}"
            )
    ...
    async def execute_triple_barrier_method(
        ...
        self.logger.info(
            f"🚀 Executing Triple Barrier Method for {symbol} on {exchange}"
        )

        try:
            # Load data from previous steps
            unified_data_path = (
                Path(data_dir) / "unified" / exchange / symbol / timeframe
            )
    ...
    async def _log_step4_artifacts_and_report(
        ...
        try:
            ...
            metrics_calculated = {
                ...
                "labeled_samples": (
                    len(result_data[result_data["triple_barrier_label"].notna()])
                    if result_data is not None
                    and "triple_barrier_label" in result_data.columns
                    else 0
                ),
                "label_distribution": (
                    result_data["triple_barrier_label"].value_counts().to_dict()
                    if result_data is not None
                    and "triple_barrier_label" in result_data.columns
                    else {}
                ),
            ...
            step_data = {
                ...
                "data_shape": (
                    list(result_data.shape) if result_data is not None else []
                ),
                "label_columns": (
                    list(result_data.columns) if result_data is not None else []
                ),
            }
            ...
            if result_data is not None:
                artifact_name = log_step_dataframe_with_standardized_name(
                    ...
                    additional_metadata={
                        ...
                        "label_distribution": (
                            result_data["triple_barrier_label"].value_counts().to_dict()
                            if "triple_barrier_label" in result_data.columns
                            else {}
                        ),
                        "asset": symbol,
                        "lookback_period": self.config.get("lookback_days", 1095),
    ...
    async def _apply_optimized_triple_barrier(
        ...
        try:
            ...
            profit_take_multiplier = self.config.get("triple_barrier", {}).get(
                "profit_take_multiplier", 0.002
            )
            stop_loss_multiplier = self.config.get("triple_barrier", {}).get(
                "stop_loss_multiplier", 0.001
            )
            time_barrier_minutes = self.config.get("triple_barrier", {}).get(
                "time_barrier_minutes", 30
            )
            max_lookahead = self.config.get("triple_barrier", {}).get(
                "max_lookahead", 100
            )
            ...
            self.logger.info(
                f"✅ Generated {len(labels)} triple barrier labels with profit tracking"
            )
            ...
            if len(labeled_data) > 0:
                long_profits = labeled_data[labeled_data["label"] == 1][
                    "potential_profit_pct"
                ]
                short_profits = labeled_data[labeled_data["label"] == -1][
                    "potential_profit_pct"
                ]

                self.logger.info("💰 Profit tracking statistics:")
                self.logger.info(
                    f"   - LONG positions avg profit: {long_profits.mean():.4f} ({long_profits.std():.4f} std)"
                )
                self.logger.info(
                    f"   - SHORT positions avg profit: {short_profits.mean():.4f} ({short_profits.std():.4f} std)"
                )
                self.logger.info(
                    f"   - Overall avg profit: {labeled_data['potential_profit_pct'].mean():.4f}"
                )
    ...
    async def _apply_basic_triple_barrier(
        ...
        try:
            self.logger.warning(
                "⚠️ Using basic triple barrier implementation with profit tracking"
            )
            ...
            self.logger.info(
                f"✅ Generated {len(labels)} basic triple barrier labels with profit tracking"
            )
            self.logger.info(f"   - Long positions: {(labels == 1).sum()}")
            self.logger.info(f"   - Short positions: {(labels == -1).sum()}")
            self.logger.info(f"   - Hold signals: {(labels == 0).sum()}")
            self.logger.info(
                f"   - Filtered samples: {filtered_count} (from {original_count})"
            )

            # Log profit statistics
            if len(result_data) > 0:
                long_profits = result_data[result_data["label"] == 1][
                    "potential_profit_pct"
                ]
                short_profits = result_data[result_data["label"] == -1][
                    "potential_profit_pct"
                ]

                self.logger.info("💰 Basic profit tracking statistics:")
                self.logger.info(
                    f"   - LONG positions avg profit: {long_profits.mean():.4f}"
                )
                self.logger.info(
                    f"   - SHORT positions avg profit: {short_profits.mean():.4f}"
                )
                self.logger.info(
                    f"   - Overall avg profit: {result_data['potential_profit_pct'].mean():.4f}"
                )
    ...
    def _create_enhanced_labels(self, data: pd.DataFrame) -> pd.DataFrame:
        """Create enhanced labels that include profit information alongside direction
        labels.
        ...
        try:
            ...
            enhanced_data["profit_weighted_label"] = (
                enhanced_data["label"] * enhanced_data["potential_profit_pct"]
            )
            ...
            enhanced_data["risk_adjusted_profit"] = enhanced_data[
                "potential_profit_pct"
            ].abs()
            ...
            if max_profit > 0:
                enhanced_data["signal_confidence"] = (
                    enhanced_data["potential_profit_pct"].abs() / max_profit
                )
            else:
            ...
            self.logger.info(
                f"   - Profit categories: {enhanced_data['profit_category'].value_counts().to_dict()}"
            )
            self.logger.info(
                f"   - Direction-profit combinations: {enhanced_data['direction_profit_label'].value_counts().to_dict()}"
            )
            self.logger.info(
                f"   - Average signal confidence: {enhanced_data['signal_confidence'].mean():.4f}"
            )
            self.logger.info(
                f"   - Risk-adjusted profit range: {enhanced_data['risk_adjusted_profit'].min():.4f} to {enhanced_data['risk_adjusted_profit'].max():.4f}"
            )
