...
Provides a tiny, typed utility that wraps sync/async functions consistently, preserving
...
def ensure_async(func: Callable[P, R]) -> Callable[P, Union[R, Callable[..., R]]]:
    """Ensure a function is async, wrapping sync functions in an async wrapper.

    This is useful for decorators that need to work with async operations but want to
...
def ensure_sync(func: Callable[P, R]) -> Callable[P, R]:
    ...
    def sync_wrapped(*args: P.args, **kwargs: P.kwargs) -> R:
        ...
        try:
            ...
            raise RuntimeError(
                f"Cannot call async function {func.__name__} synchronously from async context"
            )
        except RuntimeError:
