...
class MLDynamicTargetPredictor:
    ...
    def predict_dynamic_targets(
        ...
        try:
            ...
            tp_multiplier = np.clip(tp_multiplier, self.min_tp_multiplier, self.max_tp_multiplier)
            sl_multiplier = np.clip(sl_multiplier, self.min_sl_multiplier, self.max_sl_multiplier)
            ...
            return {
                ...
                "prediction_confidence": self._calculate_prediction_confidence(tp_model_name, sl_model_name)
            }
    ...
    def _extract_prediction_features(
        ...
        features.append(technical_analysis_data.get('close', 0) / current_atr if current_atr > 0 else 0)
        ...
        if not current_features.empty:
            ...
            indicator_features = [
                'ADX', 'MACD_HIST', 'rsi', 'stoch_k', 'bb_bandwidth',
                'volume_delta', 'OBV', 'CMF', 'position_in_range'
            ]
    ...
    def _predict_single_target(
        ...
        if model is None or scaler is None:
            ...
            return (self.fallback_tp_multiplier if target_type == "tp" 
    ...
    def _get_entry_price(self, signal_type: str, technical_analysis_data: Dict[str, Any]) -> Optional[float]:
    ...
    def _get_fallback_targets(
        ...
        if technical_analysis_data:
            ...
            return {
                "take_profit": entry_price + (current_atr * self.fallback_tp_multiplier),
    ...
    def _prepare_training_data(self, historical_data: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        training_datasets = {}
        signal_types = ["SR_FADE_LONG", "SR_FADE_SHORT", "SR_BREAKOUT_LONG", "SR_BREAKOUT_SHORT"]
        ...
        for signal_type in signal_types:
            ...
            if len(signal_data) > 0:
                ...
                signal_data = self._calculate_optimal_multipliers(signal_data, signal_type)
    ...
    def _calculate_prediction_confidence(self, tp_model_name: str, sl_model_name: str) -> float:
    ...
    def get_model_status(self) -> Dict[str, Any]:
        ...
        status = {
            "models_trained": sum(1 for model in self.models.values() if model is not None),
