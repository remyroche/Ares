...
class HybridNASTASRegimeDetector:
    ...
    def __init__(self, config: HybridRegimeConfig):
        ...
        self.logger.info(f"   TAS Weight: {config.tas_config.get('weight', 0.5)}")
        self.logger.info(f"   NAS Weight: {config.nas_config.get('weight', 0.5)}")
    ...
    def detect_regimes(self,
        ...
        try:
            ...
            regime_predictions, cluster_metrics = self._perform_clustering(combined_features)

            # Step 5: Calculate regime probabilities and transitions
