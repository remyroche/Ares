...
class FeatureIntegrationManager:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.enable_liquidity_features = self.feature_config.get("enable_liquidity_features", True)
        self.enable_advanced_features = self.feature_config.get("enable_advanced_features", True)
        ...
        try:
            ...
            if self.advanced_feature_engineering:
                advanced_features = await self._add_advanced_features(
                    historical_data, market_data, order_flow_data
                )
    ...
    async def _add_advanced_features(
        ...
        try:
            ...
            advanced_features = await self.advanced_feature_engineering.engineer_features(
    ...
    async def _add_liquidity_features(
        ...
        try:
            ...
            liquidity_features['price_impact'] = liquidity_features['price_impact'].rolling(20).mean()
            
            # Amihud illiquidity
            liquidity_features['amihud_illiquidity'] = np.abs(price_changes) / volume
            liquidity_features['amihud_illiquidity'] = liquidity_features['amihud_illiquidity'].rolling(20).mean()
            ...
            liquidity_features['liquidity_stress'] = self._calculate_liquidity_stress(liquidity_features)
    ...
    def _calculate_liquidity_stress(self, liquidity_features: Dict[str, pd.Series]) -> pd.Series:
        """Calculate liquidity stress score."""
        try:
            stress_factors = []
            
            # Volume-based stress
            volume_liquidity = liquidity_features.get('volume_liquidity', pd.Series(1.0))
            ...
            amihud_illiquidity = liquidity_features.get('amihud_illiquidity', pd.Series(0.0))
            amihud_stress = pd.Series(0.1, index=amihud_illiquidity.index)
            amihud_stress[amihud_illiquidity > 0.01] = 0.9
            amihud_stress[(amihud_illiquidity > 0.005) & (amihud_illiquidity <= 0.01)] = 0.5
            ...
            return pd.Series(0.0, index=liquidity_features.get('volume_liquidity', pd.Series()).index)
    ...
    def get_liquidity_feature_summary(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            liquidity_features = [
                'volume_liquidity', 'price_impact', 'spread_liquidity',
                'liquidity_regime', 'liquidity_percentile', 'kyle_lambda',
                'amihud_illiquidity', 'order_flow_imbalance', 'large_order_ratio',
                'vwap', 'volume_roc', 'volume_ma_ratio', 'liquidity_stress'
            ]
            ...
            summary = {
                ...
                "liquidity_feature_coverage": len(available_features) / len(liquidity_features),
                "feature_statistics": {}
            }
