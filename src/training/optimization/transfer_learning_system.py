...
class ProblemSimilarityDetector:
    ...
    def calculate_similarity(
        ...
        weights = self.config.get(
            "similarity_weights", {"feature": 0.4, "structural": 0.4, "domain": 0.2}
        )
...
class KnowledgeTransferManager:
    ...
    def transfer_knowledge(
        ...
        for i, (source_problem, similarity) in enumerate(
            zip(source_problems, similarity_scores)
        ):
...
class MetaLearner:
    ...
    def predict_optimal_strategy(
        ...
        strategy = list(set([example["strategy"] for example in self.training_data]))[
            strategy_idx
        ]
...
class TransferLearningOptimizer:
    ...
    def optimize_with_transfer(
        ...
        if self.enable_transfer:
            similar_problems_with_scores = self.knowledge_manager.find_similar_problems(
                problem_signature, self.similarity_threshold, self.max_source_problems
            )
    ...
    def _create_optimization_config_with_transfer(
        self, adapted_knowledge: Dict[str, Any], problem_signature: ProblemSignature
    ) -> Dict[str, Any]:
