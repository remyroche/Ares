...
class MLTargetUpdater:
    ...
    def __init__(self, 
        ...
        self.update_interval_seconds = self.config.get("update_interval_seconds", 300)  # 5 minutes
        self.min_time_between_updates = self.config.get("min_time_between_updates_seconds", 60)  # 1 minute
        self.confidence_threshold_for_update = self.config.get("confidence_threshold_for_update", 0.6)
        self.max_target_change_percent = self.config.get("max_target_change_percent", 0.25)  # 25% max change
        self.enable_stop_loss_updates = self.config.get("enable_stop_loss_updates", True)
        self.enable_take_profit_updates = self.config.get("enable_take_profit_updates", True)
        self.trailing_stop_enabled = self.config.get("trailing_stop_enabled", True)
        
        # Risk management settings
        self.max_sl_distance_from_entry = self.config.get("max_sl_distance_from_entry", 0.05)  # 5%
        self.min_tp_distance_from_current = self.config.get("min_tp_distance_from_current", 0.01)  # 1%
    ...
    async def start_monitoring(self):
        ...
        try:
            ...
            self.logger.error(f"Error in target updater monitoring loop: {e}", exc_info=True)
        finally:
            self.is_running = False
    ...
    async def _get_current_market_data(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            ticker = await self.exchange.get_ticker(self.state_manager.get_state("trade_symbol", "BTCUSDT"))
    ...
    def _determine_signal_type(self, position: Dict[str, Any]) -> Optional[str]:
        ...
        if original_signal in ["SR_FADE_LONG", "SR_FADE_SHORT", "SR_BREAKOUT_LONG", "SR_BREAKOUT_SHORT"]:
    ...
    async def _get_ml_targets(self, 
                            signal_type: str, 
    ...
    def _evaluate_target_update(self, 
        ...
        tp_change_percent = abs(new_tp - current_tp) / current_tp if current_tp > 0 else 0
        sl_change_percent = abs(new_sl - current_sl) / current_sl if current_sl > 0 else 0
        
        # Check if changes are within acceptable limits
        if tp_change_percent > self.max_target_change_percent:
            update_decision["reasons"].append(f"TP change too large: {tp_change_percent:.2%}")
            new_tp = current_tp  # Don't update
        
        if sl_change_percent > self.max_target_change_percent:
            update_decision["reasons"].append(f"SL change too large: {sl_change_percent:.2%}")
        ...
        if direction == "long":
            ...
            if new_tp > current_tp and tp_distance_from_current < self.min_tp_distance_from_current:
    ...
    async def _execute_target_update(self, 
                                   position: Dict[str, Any],
    ...
    def get_update_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_update_time": max(self.last_update_time.values()) if self.last_update_time else None,
