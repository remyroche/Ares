...
class FeatureGenerationPeriodLookbackOptimizationStep(BaseStep):
    ...
    def _load_generated_features(self, config: Dict[str, Any]) -> Optional[Any]:
        ...
        try:
                ...
                if labeled_data is not None:
                    ...
                    try:
                        ...
                        if meta_labeled is not None and not getattr(meta_labeled, 'empty', True):
                            ...
                            # Copy meta-label columns (including directional binary labels)
                            meta_label_cols = [
                                'binary_label', 
                                'binary_label_long',
                                'binary_label_short',
            ...
            if generated_features is not None and labeled_data is not None:
                ...
                # Priority: regression targets first, then classifier targets
                priority_target_columns = [
                    'target_long',          # Primary regression target for longs
                    'target_short',         # Primary regression target for shorts
                    'target_long_fused',
                    'target_short_fused',
    ...
    def _find_target_column(self, features, config: Optional[Dict[str, Any]] = None) -> Optional[str]:
        """Find the target column generated by feature_generation_labeling_integration_step.
        
        Target selection priority:
        - model_type="regressor" (default): Prefer target_long/target_short (continuous)
        - model_type="classifier": Prefer binary_label_long/binary_label_short (binary)
        
        Direction is respected: long direction uses *_long targets, short uses *_short.
        """
        try:
            ...
            if config is None:
                config = {}
            direction = config.get('direction', getattr(self, '_target_direction', 'long'))
            ...
            # Build prioritized candidate list based on model_type and direction
            candidates: List[str] = []
            
            if model_type == 'classifier':
                # For classifiers: prefer binary_label_long/short, fallback to regressor targets
                tprint_info(f"🔧 Model type: classifier (preferring binary classification targets for {direction} direction)")
                if direction == 'long':
                    candidates = [
                ...
                # For regressors (default): prefer target_long/short (continuous values)
            ...
            for candidate in candidates:
                if candidate in features.columns:
                    tprint(f"🎯 Found target column: {candidate}")
                    return candidate
            
            # Priority 2: Exact matches for analyst-specific targets
            analyst_targets = [
            ...
            # Priority 3: Legacy labeling step targets
    ...
    def _fuzzy_find_target_column(self, features) -> Optional[str]:
        ...
        try:
            ...
            # Prefer regression targets over classification targets
            target_patterns = {
                # Highest priority: regression targets
                'target_long': 1.0,  # Primary regressor target for longs
                'target_short': 1.0,  # Primary regressor target for shorts
                ...
                'analyst_entry': 0.75,
                'analyst_exit': 0.75,
                'analyst_direction': 0.75,
                'analyst_signal': 0.75,
                
                # Medium priority patterns (labeling outputs)
                'price_target_vol_normalized': 0.7,  # Legacy name (deprecated)
                'volatility_labels': 0.7,  # Legacy name
                'direction_confidence': 0.7,
                'opportunity_asymmetry': 0.7,
                'overall_opportunity': 0.7,
                'profit_label': 0.7,
                'target_label': 0.7,
                'label': 0.6,
