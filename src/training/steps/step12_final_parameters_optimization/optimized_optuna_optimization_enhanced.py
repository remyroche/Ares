...
from src.utils.warning_symbols import failed, success, warning
...
class VectorizedOptunaOptimizer:
    ...
    def __init__(
        ...
        self.logger.info(f"🚀 Vectorized Optuna Optimizer initialized")
    ...
    def _vectorized_data_preparation(self, data_hash: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        try:
            ...
            for i, trial in enumerate(trials):
                try:
                    ...
                    strength_scores = self._vectorized_feature_generation(X_batch, params)
                    
                    # Vectorized signal calculation
                    signals = self._vectorized_signal_calculation(
                        strength_scores, 
                        params["min_sr_confidence"], 
                        params["high_confidence_threshold"]
                    )
                    
                    # Vectorized performance calculation
                    performance = self._vectorized_performance_calculation(signals, y_batch)
    ...
    def optimize(
        ...
        try:
                ...
                try:
                    ...
                    if model_type == "sr_parameters":
                        ...
                        if custom_objective:
                            ...
                            raise ValueError("Custom objective function required")
                    
                    # Traditional ML model optimization
                    return self._evaluate_ml_model_vectorized(trial, model_type, X_np, y_np, cv_folds, subsample_fraction)
            ...
            result = VectorizedOptimizationResult(
                ...
                test_score=0.0,  # Would be calculated separately
                overfitting_score=0.0,  # Would be calculated separately
                generalization_gap=0.0,  # Would be calculated separately
                vectorized_scores=np.array([t.value for t in study.trials if t.value is not None]),
                batch_performance=np.array([t.value for t in study.trials if t.value is not None]),
            ...
            self.logger.error(f"❌ Error in vectorized optimization: {e}")
            return None
    ...
    def _evaluate_ml_model_vectorized(self, trial: optuna.Trial, model_type: str, X: np.ndarray, y: np.ndarray, cv_folds: int, subsample_fraction: float) -> float:
