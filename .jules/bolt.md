...
## 2026-06-09 - Vectorizing O(N) Pandas row-by-row DataFrame updates
Learning: Updating a Pandas DataFrame row-by-row inside an `.iterrows()` loop using `.loc` for existing keys and `.concat` for new rows creates a severe $O(N^2)$ performance bottleneck due to Pandas' internal overhead for series extraction and mutation.
Action: Replace `.iterrows()` loops when applying updates from one DataFrame to another. Instead, strictly use vectorized operations: find common indices via `upd_idx.index.intersection(old_idx.index)` for bulk `.loc` updates, and append new indices via a single `pd.concat()`. This reduces operations from $O(N^2)$ to $O(1)$. Ensure deduplication of `upd_idx` before updating to avoid 'cannot reindex' errors, and align schemas by adding missing columns using `pd.NA`.
