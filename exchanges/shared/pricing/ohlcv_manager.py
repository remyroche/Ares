...
class OHLCVManager:
    ...
    def calculate_technical_indicators(
        ...
        data = self.get_cached_candles(symbol, timeframe)
        if not data:
            return {}
        ...
        for indicator in indicators:
            if indicator.lower() == "sma_20":
                results[indicator] = self._calculate_sma(closes, 20)
            elif indicator.lower() == "ema_20":
                results[indicator] = self._calculate_ema(closes, 20)
            elif indicator.lower() == "rsi_14":
                results[indicator] = self._calculate_rsi(closes, 14)
            elif indicator.lower() == "macd":
                results[indicator] = self._calculate_macd(closes)
            elif indicator.lower() == "bollinger_bands":
                results[indicator] = self._calculate_bollinger_bands(closes, 20, 2)
    ...
    def _calculate_bollinger_bands(self, prices: List[float], period: int = 20, std_dev: float = 2) -> List[float]:
