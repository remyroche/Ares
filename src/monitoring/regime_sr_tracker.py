...
class RegimeSRTracker:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.enable_regime_tracking = self.tracker_config.get("enable_regime_tracking", True)
        self.enable_sr_tracking = self.tracker_config.get("enable_sr_tracking", True)
        self.enable_performance_analysis = self.tracker_config.get("enable_performance_analysis", True)
        
        # Detection parameters
        self.regime_detection_interval = self.tracker_config.get("regime_detection_interval", 60)  # seconds
        self.sr_update_interval = self.tracker_config.get("sr_update_interval", 300)  # seconds
        self.min_regime_duration = self.tracker_config.get("min_regime_duration", 30)  # minutes
        self.sr_touch_threshold = self.tracker_config.get("sr_touch_threshold", 0.002)  # 0.2%
        ...
        # Current state
        self.current_regime: dict[str, RegimeDetection] = {}  # symbol_timeframe -> detection
        self.active_sr_levels: dict[str, list[SupportResistanceLevel]] = {}  # symbol_timeframe -> levels
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"❌ Regime and S/R Tracker initialization failed: {e}")
            return False
    ...
    async def _initialize_storage(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Regime/SR tracker storage backend '{storage_backend}' initialized")
    ...
    async def _create_regime_sr_tables(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to create regime/SR tables: {e}")
            raise
    ...
    async def _load_historical_data(self) -> None:
        ...
        try:
            ...
            if hasattr(self, 'storage_manager'):
                ...
                regime_results = await self.storage_manager.execute_query(query, (cutoff_date,))
                ...
                self.logger.info(f"Loaded {len(regime_results)} regime detections and {len(sr_results)} S/R levels")
    ...
    async def _initialize_detection_algorithms(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to initialize detection algorithms: {e}")
            raise
    ...
    async def detect_current_regime(
        ...
        try:
            ...
            detection = RegimeDetection(
                ...
                transition_probability=await self._calculate_transition_probabilities(regime_indicators)
            )
            ...
            if key in self.current_regime:
                ...
                if previous_detection.current_regime != regime_type:
                    ...
                    # Update duration
                    duration = (detection.timestamp - previous_detection.timestamp).total_seconds() / 60.0
            ...
            self.logger.debug(f"Detected regime {regime_type.value} for {symbol} {timeframe} (confidence: {confidence:.2f})")
            
            return detection
            
        except Exception as e:
            self.logger.error(f"Failed to detect regime for {symbol} {timeframe}: {e}")
            return None

    async def _get_market_data(self, symbol: str, timeframe: str) -> pd.DataFrame | None:
        ...
        try:
            ...
            if cache_key in self.price_history:
                ...
                if not data.empty and data.index[-1] >= datetime.now() - timedelta(minutes=5):
                    return data
            ...
            self.logger.error(f"Failed to get market data for {symbol} {timeframe}: {e}")
            return None

    async def _fetch_market_data(self, symbol: str, timeframe: str) -> pd.DataFrame | None:
    ...
    async def _calculate_regime_indicators(self, data: pd.DataFrame) -> dict[str, float]:
        ...
        try:
            ...
            if len(data) >= 20:
                ...
                indicators["price_change_1h"] = returns.tail(4).sum() if len(returns) >= 4 else 0.0
                indicators["price_change_4h"] = returns.tail(16).sum() if len(returns) >= 16 else 0.0
                indicators["price_change_24h"] = returns.tail(96).sum() if len(returns) >= 96 else 0.0
                
                # Trend strength score
                ema_short = data['close'].ewm(span=10).mean()
                ema_long = data['close'].ewm(span=30).mean()
                trend_ratio = (ema_short.iloc[-1] - ema_long.iloc[-1]) / ema_long.iloc[-1]
            ...
            if 'volume' in data.columns and len(data) >= 20:
                ...
                indicators["volume_24h"] = data['volume'].tail(96).sum() if len(data) >= 96 else 0.0
            ...
            if len(data) >= 20:
                ...
                if len(atr) >= 50:
                    atr_percentile = stats.percentileofscore(atr.tail(50), atr.iloc[-1]) / 100.0
            ...
            self.logger.error(f"Failed to calculate regime indicators: {e}")
            return {}

    async def _classify_regime(self, indicators: dict[str, float]) -> tuple[RegimeType, float]:
        ...
        try:
            ...
            # Sideways detection
            sideways_score = 0.0
            if abs(indicators.get("price_change_24h", 0)) < 0.015:  # Less than 1.5% movement
                sideways_score += 0.3
            if indicators.get("trend_strength_score", 0) < 0.3:
                sideways_score += 0.3
            if 40 < indicators.get("rsi", 50) < 60:
                sideways_score += 0.2
            if indicators.get("bollinger_position", 0.5) > 0.3 and indicators.get("bollinger_position", 0.5) < 0.7:
                sideways_score += 0.2
    ...
    async def _calculate_transition_probabilities(self, indicators: dict[str, float]) -> dict[str, float]:
        ...
        try:
            ...
            self.logger.error(f"Failed to calculate transition probabilities: {e}")
            return {}
    ...
    async def _handle_regime_transition(
        ...
        try:
            ...
            duration = (current_detection.timestamp - previous_detection.timestamp).total_seconds() / 60.0
            ...
            self.logger.info(f"Regime transition: {previous_detection.current_regime.value} → {current_detection.current_regime.value}")
            
        except Exception as e:
            self.logger.error(f"Failed to handle regime transition: {e}")
    ...
    async def _store_regime_detection(self, detection: RegimeDetection) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to store regime detection: {e}")
            raise
    ...
    async def _store_regime_transition(self, transition: RegimeTransition) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to store regime transition: {e}")
            raise
        ...
        try:
            ...
            for price, strength in pivot_levels:
                level = await self._create_sr_level(
                    symbol, timeframe, SRLevelType.PIVOT, 
                    price, strength, current_price
                )
            ...
            if 'volume' in market_data.columns:
                ...
                for price, strength in volume_levels:
                    level = await self._create_sr_level(
                        symbol, timeframe, SRLevelType.VOLUME_PROFILE,
                        price, strength, current_price
                    )
                    sr_levels.append(level)
            ...
            for price, strength in psychological_levels:
                level = await self._create_sr_level(
                    symbol, timeframe, SRLevelType.PSYCHOLOGICAL,
                    price, strength, current_price
                )
                sr_levels.append(level)
            ...
            self.logger.debug(f"Identified {len(sr_levels)} S/R levels for {symbol} {timeframe}")
            
            return sr_levels
            
        except Exception as e:
            self.logger.error(f"Failed to identify S/R levels for {symbol} {timeframe}: {e}")
            return []
    ...
    async def _find_pivot_levels(self, data: pd.DataFrame) -> list[tuple[float, float]]:
        ...
        try:
            ...
            self.logger.error(f"Failed to find pivot levels: {e}")
            return []

    async def _find_volume_levels(self, data: pd.DataFrame) -> list[tuple[float, float]]:
        ...
        try:
            ...
            self.logger.error(f"Failed to find volume levels: {e}")
            return []

    async def _find_psychological_levels(self, current_price: float) -> list[tuple[float, float]]:
        ...
        try:
            ...
            self.logger.error(f"Failed to find psychological levels: {e}")
            return []

    async def _cluster_price_levels(self, prices: list[float]) -> list[tuple[float, int]]:
        ...
        try:
            ...
            threshold = sorted_prices[0] * self.sr_detection_params["clustering_threshold"]
            ...
            self.logger.error(f"Failed to cluster price levels: {e}")
            return []
    ...
    async def _create_sr_level(
        ...
        try:
            ...
            self.logger.error(f"Failed to create S/R level: {e}")
    ...
    async def _store_sr_level(self, level: SupportResistanceLevel) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to store S/R level: {e}")
            raise
    ...
    async def _periodic_regime_detection(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Error in periodic regime detection: {e}")
    ...
    async def _periodic_sr_update(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Error in periodic S/R update: {e}")
    ...
    async def analyze_regime_performance(
        ...
        try:
            ...
            self.logger.error(f"Failed to analyze regime performance for {regime_type.value}: {e}")
            return None

    async def get_current_regime(self, symbol: str, timeframe: str) -> RegimeDetection | None:
        """Get current regime for symbol/timeframe."""
        key = f"{symbol}_{timeframe}"
        return self.current_regime.get(key)

    async def get_active_sr_levels(self, symbol: str, timeframe: str) -> list[SupportResistanceLevel]:
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to cleanup Regime and S/R Tracker: {e}")
...
async def setup_regime_sr_tracker(config: dict[str, Any]) -> RegimeSRTracker | None:
    ...
    try:
        ...
        system_logger.error(f"Failed to setup Regime and S/R Tracker: {e}")
        return None