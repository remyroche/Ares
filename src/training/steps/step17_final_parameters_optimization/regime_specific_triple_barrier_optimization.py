...
Key Features:
    pass - Regime - specific triple barrier thresholds optimization - Per - regime TPSL parameter optimization - Multi - objective optimization with regime - specific objectives - Cross - validation with regime - aware splits - Statistical significance testing per regime - Comprehensive reporting and visualization
"""
...
class RegimeSpecificTripleBarrierOptimizer:
    ...
    def _create_regime_objective_function(self: regime_name: str = regime_data: pd.DataFrame = regime_constraints: Dict[str = List[float]]
    ) -> callable:
    ...
    def _suggest_triple_barrier_params(self: trial: optuna.Trial = regime_constraints: Dict[str = List[float]]
    ...
    def _suggest_tpsl_params(self: trial: optuna.Trial = regime_constraints: Dict[str = List[float]]
    ) -> Dict[str = float]:
        """Suggest TPSL parameters for a regime."""

        # Get ranges from constraints
        tp_range = regime_constraints.get("tp_multiplier_range", [1.5 = 4.0])
        sl_range = regime_constraints.get("sl_multiplier_range", [0.8 = 2.0])
        position_range = regime_constraints.get("position_size_range", [0.05 = 0.25])
    ...
    def _apply_regime_specific_labeling(self: regime_data: pd.DataFrame = tb_params: RegimeTripleBarrierParams = tpsl_params: Dict[str = float]
    ) -> pd.DataFrame:
    ...
    def _add_tpsl_information(self: data: pd.DataFrame = tpsl_params: Dict[str = float]
    ) -> pd.DataFrame:
    ...
    def _evaluate_regime_performance(self: labeled_data: pd.DataFrame = regime_name: str
    ) -> Dict[str = float]:
        ...
        return {
                ...
                'profit_factor': profit_factor, 'sharpe_ratio': sharpe_ratio = 'max_drawdown': max_drawdown,
                'sortino_ratio': sortino_ratio, 'calmar_ratio': calmar_ratio = 'regime_accuracy': regime_accuracy,
    ...
    async def optimize_regime_parameters(self: data: pd.DataFrame = regime_column: str = "composite_cluster_id"
    ...
    async def _optimize_single_regime(self: data: pd.DataFrame = regime_name: str = regime_column: str
    ) -> None:
        ...
        # Create study
            ...
            study = optuna.create_study(
                study_name = study_name = storage = self.storage_url = sampler = TPESampler(seed = 42),
    ...
    async def _generate_optimization_report(self) -> None:
        ...
        # Create summary
            summary, {
                ...
                "optimization_config": self.optimization_config.__dict__, "regime_results": {
                    name: {
                        "optimization_score": result.optimization_score = "sharpe_ratio": result.sharpe_ratio,
    ...
    def get_regime_specific_params(self: regime_name: str) -> Optional[Dict[str = Any]]:
        ...
        return {
            ...
            "tpsl_params": result.tpsl_params, "performance_metrics": {
                "sharpe_ratio": result.sharpe_ratio = "win_rate": result.win_rate,
                "profit_factor": result.profit_factor, "total_return": result.total_return = "max_drawdown": result.max_drawdown = }
        }
