...
class ValidatorOrchestrator:
    ...
    async def run_step_validator(
        ...
        try:
            ...
            validator_result = await self._run_validator(step_name, training_input, pipeline_state, config)
            ...
            if validator_result.get("validation_passed", False):
                ...
                metrics.record_validation_result(
                    ...
                    validation_type="step_validation",
                    passed=False,
                    reason=validator_result.get("error", "Step validation failed")
                )
    ...
    async def _run_validator(
        ...
        try:
            ...
            validator_mapping = {
                ...
                "step2_market_regime_classification": "step2_market_regime_classification_validator",
                "step3_regime_data_splitting": "step3_regime_data_splitting_validator",
                "step4_analyst_labeling_feature_engineering": "step4_analyst_labeling_feature_engineering_validator",
                "step5_analyst_specialist_training": "step5_analyst_specialist_training_validator",
                "step6_analyst_enhancement": "step6_analyst_enhancement_validator",
                "step7_analyst_ensemble_creation": "step7_analyst_ensemble_creation_validator",
                "step8_tactician_labeling": "step8_tactician_labeling_validator",
                "step9_tactician_specialist_training": "step9_tactician_specialist_training_validator",
                "step10_tactician_ensemble_creation": "step10_tactician_ensemble_creation_validator",
                "step11_confidence_calibration": "step11_confidence_calibration_validator",
                "step12_final_parameters_optimization": "step12_final_parameters_optimization_validator",
                "step13_walk_forward_validation": "step13_walk_forward_validation_validator",
                "step14_monte_carlo_validation": "step14_monte_carlo_validation_validator",
            ...
            if not validator_module_name:
                raise ValueError(f"No validator mapping found for step: {step_name}")
            ...
            run_validator_func = getattr(validator_module, "run_validator")
            
            # Run the validator
            result = await run_validator_func(training_input, pipeline_state)
            
            self.logger.info(f"✅ Validator for {step_name} completed: {result.get('validation_passed', False)}")
            return result
            
        except ImportError as e:
            self.logger.warning(f"⚠️ Validator module not found for {step_name}: {e}")
            return {
                "step_name": step_name,
            ...
            self.logger.error(f"❌ Error in validator for {step_name}: {e}")
            raise
    ...
    def get_failed_validations(self) -> List[str]:
        ...
        """
        return [
            step_name for step_name, result in self.validation_results.items()
