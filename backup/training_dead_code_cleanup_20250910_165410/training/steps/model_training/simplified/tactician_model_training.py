...
class TacticianModelTrainer:
    """Tactician model trainer with advanced HPO and tactical features."""
    ...
    async def train_tactician_models(
        ...
        **kwargs
        ...
        """Train all Tactician models with advanced HPO."""
        ...
        self.logger.info(f"✅ Tactician model training completed in {execution_time:.2f}s")
        self.logger.info(f"📊 Overall performance: {results.overall_performance}")
        
        return results
    
    def _validate_data(self, data: pd.DataFrame) -> None:
        """Validate input data for Tactician training."""
    ...
    async def _train_tactician_models_internal(
        ...
        **kwargs
        ...
        """Internal Tactician model training logic."""
        ...
        # Prepare tactical features
        tactical_data = await self._prepare_tactical_features(data)
        
        # Train individual models with advanced HPO
        individual_results = {}
        model_performance = {}
        hpo_results = {}
        
        for model_type in self.config.model_types:
            self.logger.info(f"🔄 Training {model_type.value} with advanced HPO...")
            
            try:
                # Create training config for this model type
                training_config = self._create_tactical_training_config(model_type)
                
                # Perform advanced HPO if enabled
                if self.config.enable_advanced_hpo:
                    hpo_result = await self._advanced_hyperparameter_optimization(
        ...
        if self.config.enable_multi_objective_optimization:
            multi_objective_results = await self._multi_objective_optimization(tactical_data)
        
        # Perform walk-forward validation if enabled
        walk_forward_results = []
        if self.config.enable_walk_forward_validation:
            walk_forward_results = await self._walk_forward_validation(tactical_data)
        
        # Generate tactical insights
        tactical_insights = await self._generate_tactical_insights(
    ...
    async def _prepare_tactical_features(self, data: pd.DataFrame) -> pd.DataFrame:
        """Prepare tactical features for training."""
        
        tactical_data = data.copy()
        
        # Add regime awareness features
        if self.config.enable_regime_awareness:
            tactical_data = self._add_regime_features(tactical_data)
        ...
        self.logger.info(f"📊 Prepared tactical features: {len(tactical_data.columns)} total columns")
    ...
    async def _advanced_hyperparameter_optimization(
        self, 
        data: pd.DataFrame, 
        model_type: TacticianModelType,
        training_config: ModelTrainingConfig
    ) -> Dict[str, Any]:
        """Perform advanced hyperparameter optimization for tactical models."""
        
        self.logger.info(f"🔬 Starting advanced HPO for {model_type.value}...")
        
        # Create study with tactical-specific objectives
        study = optuna.create_study(
            directions=['maximize', 'minimize', 'maximize'],  # sharpe, drawdown, win_rate
            study_name=f"{self.config.tactician_name}_{model_type.value}_advanced_hpo"
        )
        
        # Define multi-objective function
        def objective(trial):
            return self._tactical_objective_function(trial, data, model_type, training_config)
        ...
        # Get best trial
        best_trial = study.best_trials[0]  # First objective (sharpe ratio)
        
        result = {
            'best_params': best_trial.params,
            'best_score': best_trial.values[0],
            'all_scores': [trial.values for trial in study.trials],
            'study': study
        }
        
        self.logger.info(f"✅ Advanced HPO completed for {model_type.value}")
        self.logger.info(f"📊 Best score: {result['best_score']:.4f}")
        
        return result
    
    def _tactical_objective_function(
        self, 
        trial: optuna.Trial, 
        data: pd.DataFrame, 
        model_type: TacticianModelType,
        ...
        """Tactical objective function for multi-objective optimization."""
        
        try:
            # Get hyperparameters
            params = self._suggest_tactical_hyperparameters(trial, model_type)
            
            # Create and train model
            training_config.model_params = params
            trainer = GeneralModelTrainer(training_config)
            
            # Quick training for HPO
            X, y = trainer._prepare_data(data)
            X_train, X_val, _, y_train, y_val, _ = trainer._split_data(X, y)
            
            model = trainer.model_factory.create_model(training_config.model_type, params)
            ...
            sharpe_ratio = self._calculate_sharpe_ratio(y_val, y_pred)
            ...
            self.logger.error(f"Error in tactical objective function: {e}")
            return 0.0, 1.0, 0.0  # Worst case values
    
    def _suggest_tactical_hyperparameters(self, trial: optuna.Trial, model_type: TacticianModelType) -> Dict[str, Any]:
        """Suggest hyperparameters for tactical models."""
        ...
        elif model_type in [TacticianModelType.ENTRY_TIMING, TacticianModelType.EXIT_TIMING]:
            return {
    ...
    def _calculate_sharpe_ratio(self, y_true: pd.Series, y_pred: np.ndarray) -> float:
        """Calculate Sharpe ratio for tactical performance."""
    ...
    def _calculate_max_drawdown(self, y_true: pd.Series, y_pred: np.ndarray) -> float:
        """Calculate maximum drawdown."""
        
        try:
            if len(y_pred) > 1:
                cumulative = np.cumsum(y_pred)
    ...
    def _calculate_win_rate(self, y_true: pd.Series, y_pred: np.ndarray) -> float:
        """Calculate win rate."""
    ...
    async def _multi_objective_optimization(self, data: pd.DataFrame) -> Dict[str, Any]:
        """Perform multi-objective optimization across all models."""
        
        self.logger.info("🔬 Starting multi-objective optimization...")
        
        # This would implement NSGA-II or similar multi-objective algorithm
    ...
    async def _walk_forward_validation(self, data: pd.DataFrame) -> List[Dict[str, Any]]:
        """Perform walk-forward validation for tactical models."""
        ...
        for i in range(self.config.walk_forward_windows - 1):
            ...
            for model_type in self.config.model_types:
                try:
                    training_config = self._create_tactical_training_config(model_type)
                    trainer = GeneralModelTrainer(training_config)
                    model_result = await trainer.train_model(train_data)
                    
                    # Evaluate on validation window
                    X_val, y_val = trainer._prepare_data(val_data)
                    val_metrics = await trainer._evaluate_model(model_result.trained_model, X_val, y_val)
                    
                    window_results[model_type.value] = val_metrics
                    
                except Exception as e:
                    self.logger.error(f"Error in walk-forward validation for {model_type.value}: {e}")
                    continue
        ...
        self.logger.info(f"✅ Walk-forward validation completed: {len(walk_forward_results)} windows")
        
        return walk_forward_results
    
    async def _generate_tactical_insights(
        self, 
        ...
        if 'close' in data.columns:
            price_volatility = data['close'].rolling(20).std().mean()
            insights['market_conditions']['volatility'] = price_volatility
            insights['market_conditions']['regime'] = 'High Volatility' if price_volatility > 0.02 else 'Low Volatility'
