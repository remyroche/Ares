...
sr_breakout_predictor = PipelineStandards.safe_import(
    "src.tactician.sr_breakout_predictor", None
)
...
if enhanced_mlflow is None:
    ...
    log_step_dataframe_with_standardized_name = (
        lambda *args, **kwargs: "fallback_dataframe"
    )
    log_step_report = lambda *args, **kwargs: "fallback_report"
    log_step_artifact_with_standardized_name = (
        lambda *args, **kwargs: "fallback_artifact"
    )
else:
...
class HMMRegimeDiscoveryStep:
    ...
    def _initialize_components(self) -> None:
        ...
        if sr_breakout_predictor is not None:
            try:
                ...
                sr_config["sr_breakout_predictor"] = sr_config.get(
                ...
                self.logger.warning(
                    f"⚠️ Could not initialize SR Breakout Predictor: {e}"
                )
    ...
    async def initialize(self) -> None:
        ...
        if hasattr(self, "sr_predictor"):
            try:
                ...
                self.logger.warning(
                    f"⚠️ Failed to initialize SR Breakout Predictor: {e}"
                )
    ...
    async def execute(
        ...
        if PSUTIL_AVAILABLE:
            ...
            self.logger.info(
                f"💾 Initial memory usage: {initial_memory.percent:.1f}% ({initial_memory.used / 1024**3:.1f}GB / {initial_memory.total / 1024**3:.1f}GB)"
            )
        else:
            self.logger.info(
        ...
        try:
            ...
            self.logger.info(
                f"⏱️ Data Quality Validation completed in {data_quality_elapsed:.2f} seconds"
            )
            ...
            self.logger.info(
                f"⏱️ Data Loading and Preparation completed in {data_loading_elapsed:.2f} seconds"
            )
            ...
            if not data_loaded.get("success", False):
                ...
                pipeline_state["hmm_regime_discovery_completed"] = False
                pipeline_state["regime_discovery_error"] = (
            ...
            optimized_params = await self._run_automatic_optimization(
            ...
            if optimized_params:
                ...
                self.logger.warning(
                    "⚠️ Parameter optimization failed, using default parameters"
                )
                pipeline_state["optimization_used"] = False

            optimization_elapsed = time.time() - optimization_start
            self.logger.info(
                f"⏱️ Parameter Optimization completed in {optimization_elapsed:.2f} seconds"
            )
            ...
            self.logger.info(
                f"⏱️ HMM Regime Discovery completed in {hmm_elapsed:.2f} seconds"
            )
            ...
            if regime_results.get("success", False):
                ...
                pipeline_state["regime_states"] = regime_results.get(
                    "regime_states", []
                )
                pipeline_state["regime_transitions"] = regime_results.get(
                    "regime_transitions", {}
                )
                ...
                await self._log_step3_artifacts_to_mlflow(
                    regime_results, training_input
                )
                ...
                sr_context = await self._get_sr_context_for_regime_analysis(
                    data_loaded["data"], current_price
                )
                ...
                self.logger.info(
                    f"⏱️ SR Context Analysis completed in {sr_elapsed:.2f} seconds"
                )
            ...
            self.logger.exception(
                f"❌ Unexpected error during HMM regime discovery: {e}"
            )
            pipeline_state["hmm_regime_discovery_completed"] = False
        ...
        self.logger.info(
            f"   - Data Loading and Preparation: {data_loading_elapsed:.2f}s"
        )
        ...
        if PSUTIL_AVAILABLE:
            ...
            self.logger.info(
                f"💾 Memory usage: {memory_usage.percent:.1f}% ({memory_usage.used / 1024**3:.1f}GB / {memory_usage.total / 1024**3:.1f}GB)"
            )
    ...
    async def _log_step3_artifacts_to_mlflow(
        self, regime_results: dict[str, Any], training_input: dict[str, Any]
    ) -> None:
        """Log step 3 artifacts to MLflow with enhanced metadata and standardized
        naming."""
        try:
            symbol = training_input.get("symbol", "ETHUSDT")
            exchange = training_input.get("exchange", "BINANCE")
            ...
            if "composite_df" in regime_results:
                ...
                artifact_name = log_step_dataframe_with_standardized_name(
                    ...
                    additional_metadata={
                        ...
                        "regime_count": (
                            len(composite_df.get("composite_cluster_id", []).unique())
            ...
            if "intensity_df" in regime_results:
                ...
                artifact_name = log_step_dataframe_with_standardized_name(
                    ...
                    additional_metadata={
                        ...
                        "intensity_features": [
                            col for col in intensity_df.columns if "intensity" in col
                        ],
                        "timeframe": timeframe,
                    },
                )
            ...
            if "metrics" in regime_results and "reports" in regime_results:
                ...
                report_name = log_step_report(
                    ...
                    additional_metadata={
                        ...
                        "composite_clusters": regime_results["metrics"].get(
                            "composite_clusters", 0
                        ),
                        "reports_generated": (
                            list(regime_results["reports"].keys())
                            if "reports" in regime_results
                            else []
                        ),
                    },
                )
                ...
                if numeric_metrics:
                    log_step_metrics(
                        ...
                        additional_metadata={
                            ...
                            "hmm_states": metrics.get("hmm_states", 0),
                ...
                log_step_model(
                    ...
                    additional_metadata={
                        ...
                        "covariance_type": getattr(
                            hmm_model, "covariance_type", "unknown"
                        ),
            ...
            self.logger.info(
                "✅ Step 3 artifacts logged to MLflow with standardized naming successfully"
            )
    ...
    def _log_regime_discovery_results(self, regime_results: dict[str, Any]) -> None:
        ...
        self.logger.info(
            f"📈 Total periods analyzed: {metrics.get('total_periods', 0):,}"
        )
        self.logger.info(
            f"🔄 Unique regimes discovered: {metrics.get('unique_regimes', 0)}"
        )
        ...
        if regime_distribution:
            ...
            for regime, count in regime_distribution.items():
                ...
                self.logger.info(
                    f"   - {regime}: {count:,} periods ({percentage:.1f}%)"
                )
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=False, context="data_quality_validation"
    )
    async def _ensure_data_quality(self, training_input: dict[str, Any]) -> bool:
        ...
        if not self.data_quality_manager:
            self.logger.warning(
                "⚠️ Data quality manager not available, proceeding without quality check"
            )
        ...
        try:
            ...
            self.logger.info(
                f"🎯 Validating data quality for {symbol} on {exchange} ({timeframe})..."
            )
    ...
    async def _fix_missing_data(self, training_input: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"🔄 Fixing missing data for {symbol} on {exchange} ({timeframe})..."
            )
            ...
            try:
                ...
                if step1_5_success:
                    self.logger.info(
                        "✅ Step1_5 data conversion completed successfully"
                    )
                else:
    ...
    async def _load_and_prepare_data(
        ...
        """Load and prepare data for HMM regime discovery with standardized
        validation."""
        try:
            symbol = training_input.get("symbol", "ETHUSDT")
            exchange = training_input.get("exchange", "BINANCE")
            ...
            if validation_result.passed:
                self.logger.info(
                    f"✅ Data validation passed (quality score: {validation_result.quality_score:.2f})"
                )
            else:
            ...
            self.logger.info(
                f"✅ Klines data loaded: {len(df):,} rows, {len(df.columns)} columns"
            )
    ...
    async def _prepare_hmm_features(self, df: Any) -> Any:
        """Prepare comprehensive features for HMM regime discovery including momentum,
        S/R, volume, and volatility."""
        try:
            ...
            features["volatility_10"] = (
                df["close"].pct_change().rolling(window=10).std()
            )
            ...
            features["volatility_momentum"] = features["volatility_20"] - features[
                "volatility_20"
            ].shift(5)
            ...
            features["volume_price_trend"] = (df["close"] - df["close"].shift(1)) * df[
                "volume"
            ]
            features["volume_price_trend_ratio"] = (
                features["volume_price_trend"]
                / features["volume_price_trend"].rolling(20).mean()
            )
            ...
            features["distance_to_support"] = (
                df["close"] - features["support_1"]
            ) / df["close"]
            features["distance_to_resistance"] = (
                features["resistance_1"] - df["close"]
            ) / df["close"]
            ...
            features["price_vs_sma20"] = (df["close"] - features["sma_20"]) / features[
                "sma_20"
            ]
            features["price_vs_sma50"] = (df["close"] - features["sma_50"]) / features[
                "sma_50"
            ]
            ...
            self.logger.info(
                f"   - Removed rows: {removed_rows:,} ({removed_rows/initial_rows*100:.1f}%)"
            )
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=pd.Series(), context="calculate_rsi"
    )
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=pd.Series(), context="calculate_macd"
    )
    def _calculate_macd(
        self, prices: Any, fast: int = 12, slow: int = 26, signal: int = 9
    ) -> Any:
        """Calculate MACD (Moving Average Convergence Divergence)."""
        self.logger.debug(
            f"Calculating MACD (fast={fast}, slow={slow}, signal={signal})..."
        )
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=pd.Series(), context="calculate_atr"
    )
    ...
    def _calculate_bollinger_bands(
        self, prices: Any, window: int = 20, num_std: float = 2
    ) -> Any:
        """Calculate Bollinger Bands."""
        self.logger.debug(
            f"Calculating Bollinger Bands (window={window}, std={num_std})..."
        )
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=pd.Series(), context="calculate_adx"
    )
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=None, context="log_feature_categories"
    )
        ...
        try:
            ...
            for col in features.columns:
                if "momentum" in col.lower():
                    ...
                    sr_term in col.lower()
                    for sr_term in ["support", "resistance", "pivot", "sr_", "bb_"]
                ):
                    feature_categories["support_resistance"].append(col)
                elif any(
                    tech_term in col.lower()
                    for tech_term in ["rsi", "macd", "adx", "atr", "sma", "ema"]
                ):
            ...
            for category, cols in feature_categories.items():
                if cols:
                    self.logger.info(
                        f"   - {category.capitalize()}: {len(cols)} features"
                    )
    ...
    async def _perform_hmm_regime_discovery(
        self, training_input: dict[str, Any], data: Any
    ) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"📊 Features prepared: {len(features.columns)} features, {len(features)} samples"
            )
            ...
            for col in features.columns:
                ...
                if len(series) > 0:
                    self.logger.info(
                        f"   - {col}: mean={series.mean():.6f}, std={series.std():.6f}, min={series.min():.6f}, max={series.max():.6f}"
                    )
            ...
            try:
                ...
                HMM_AVAILABLE = False
                self.logger.warning(
                    "⚠️ hmmlearn not available, falling back to simple regime detection"
                )
    ...
    async def _perform_hmmlearn_regime_discovery(self, features: Any) -> dict[str, Any]:
        """Perform HMM regime discovery using hmmlearn library with 20-cluster composite
        approach."""
        try:
            from hmmlearn import hmm
            ...
            composite_features = self._create_composite_features(
                features, hmm_state_sequence, hmm_state_probs
            )

            # Scale composite features
            composite_scaler = StandardScaler()
            composite_features_scaled = composite_scaler.fit_transform(
                composite_features
            )
            ...
            self.logger.info(
                "🎯 Phase 4: Enhanced regime analysis and interpretation..."
            )
            ...
            self.logger.info(
                f"✅ Composite HMM regime discovery completed successfully"
            )
            self.logger.info(
                f"📊 HMM States: {n_hmm_states}, Composite Clusters: {n_clusters}"
            )
            self.logger.info(
                f"📈 Cluster Quality - Silhouette: {cluster_metrics['silhouette_score']:.4f}"
            )
    ...
    async def _perform_simple_regime_discovery(self, features: Any) -> dict[str, Any]:
        ...
        try:
            ...
            for i in range(total_periods):
                vol = (
                    volatility.iloc[i] if hasattr(volatility, "iloc") else volatility[i]
                )
                ...
                if (i + 1) % progress_interval == 0:
                    ...
                    self.logger.info(
                        f"📊 Regime classification progress: {progress:.1f}% ({i + 1:,}/{total_periods:,})"
                    )
    ...
    def _interpret_hmm_states(
        self, features: Any, state_sequence: Any, state_probs: Any
    ) -> dict[str, Any]:
        ...
        try:
            ...
            for state in unique_states:
                ...
                self.logger.info(
                    f"   State {state} → {regime_name}: {len(state_data)} periods ({state_char['percentage']:.1f}%)"
                )
    ...
    def _calculate_regime_transitions(self, regimes: List[str]) -> dict[str, Any]:
        ...
        self.logger.info(
            f"✅ Transition matrix calculated for {len(transitions)} regimes"
        )
        return transitions
    ...
    @handle_errors(
        ...
        default_return={
            "success": False,
            "error": "Enhanced regime change detection failed",
        },
    ...
    def _detect_regime_changes_advanced(
        ...
        try:
            self.logger.info(
                "🔍 Detecting regime changes using advanced probability-based approach..."
            )
            ...
            self.logger.info(
                f"✅ Detected {len(regime_changes)} regime changes with advanced method"
            )
    ...
    def _apply_persistence_filter(
        self, transitions: np.ndarray, states: np.ndarray, min_persistence: int
    ) -> np.ndarray:
    ...
    def _calculate_adaptive_regime_boundaries(
        ...
        """Calculate adaptive regime boundaries using clustering of regime
        characteristics."""
        try:
            self.logger.info("🔧 Calculating adaptive regime boundaries...")
            ...
            if regime_features.empty:
                self.logger.warning(
                    "⚠️ No regime characteristics available for boundary calculation"
                )
                return {}
            ...
            self.logger.info(
                f"✅ Calculated {len(unique_boundaries)} adaptive regime boundaries"
            )
            ...
            self.logger.exception(
                f"❌ Error calculating adaptive regime boundaries: {e}"
            )
            return {}
    ...
    def _extract_regime_characteristics(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if (
                "volume_ratio_10" in features.columns
                and "price_momentum_10" in features.columns
            ):
    ...
    @handle_errors(
        exceptions=(Exception,), default_return={}, context="model_regime_persistence"
    )
    def _model_regime_persistence(self, regime_sequence: np.ndarray) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"✅ Modeled regime persistence with {best_distribution} distribution"
            )
    ...
    async def _get_sr_context_for_regime_analysis(
        ...
        try:
            if not hasattr(self, "sr_predictor") or self.sr_predictor is None:
                self.logger.warning(
                    "⚠️ SR predictor not available, skipping SR context analysis"
                )
                return {}
            ...
            self.logger.info(
                f"✅ SR context analysis completed: {len(sr_context)} context elements"
            )
            return sr_context
    ...
    async def _enhance_regime_analysis_with_sr(
        ...
        try:
            ...
            sr_regime_features = await self._create_sr_regime_features(
                regime_results.get("regime_states", []), sr_context, market_data
            )
            ...
            if (
                hasattr(self, "sr_predictor")
                and self.sr_predictor
                and self.sr_predictor.reporting_enabled
            ):
    ...
    async def _create_sr_regime_features(
        ...
        try:
            ...
            for regime in set(regime_states):
                ...
                if len(regime_data) > 0:
                    ...
                    features["sr_proximity_by_regime"][f"regime_{regime}"] = {
                        "support_proximity": regime_sr_context.get(
                            "support_proximity", 1.0
                        ),
                        "resistance_proximity": regime_sr_context.get(
                            "resistance_proximity", 1.0
                        ),
                    }

                    features["sr_strength_by_regime"][f"regime_{regime}"] = {
                        "support_strength": regime_sr_context.get(
                            "support_strength", 0.5
                        ),
                        "resistance_strength": regime_sr_context.get(
            ...
            self.logger.info(
                f"✅ Created SR-aware regime features for {len(set(regime_states))} regimes"
            )
            return features
...
async def run_step(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        if result.get("hmm_regime_discovery_completed", False):
            ...
            if result.get("optimization_used", False):
                logger.info(
                    "🔧 Automatic parameter optimization completed successfully"
                )
                if result.get("optimized_params"):
                    logger.info(
                        f"📊 Optimized parameters applied: {list(result['optimized_params'].keys())}"
                    )
            else:
                logger.warning(
                    "⚠️ Parameter optimization failed, using default parameters"
            ...
            if result.get("regime_states"):
                ...
                logger.info(
                    f"📊 Discovered {unique_regimes} unique regimes across {total_periods:,} periods"
                )
    ...
    def _create_composite_features(
        self, features: Any, hmm_states: Any, hmm_probs: Any
    ) -> Any:
        """Create composite features combining HMM states with original features."""
        try:
            ...
            composite_df["hmm_state_entropy"] = -np.sum(
                hmm_probs * np.log(hmm_probs + 1e-10), axis=1
            )
            ...
            for feature in key_features:
                if feature in composite_df.columns:
                    composite_df[f"{feature}_x_hmm_state"] = (
                        composite_df[feature] * composite_df["hmm_state"]
                    )
                    composite_df[f"{feature}_x_hmm_entropy"] = (
                        composite_df[feature] * composite_df["hmm_state_entropy"]
                    )

            # Add rolling statistics for HMM states
            composite_df["hmm_state_persistence"] = self._calculate_persistence(
                hmm_states
            )
            composite_df["hmm_state_transitions"] = self._calculate_transitions(
                hmm_states
            )

            self.logger.info(
                f"✅ Created composite features: {len(composite_df.columns)} total features"
            )
            return composite_df
    ...
    def _calculate_cluster_quality_metrics(
        self, features_scaled: Any, cluster_labels: Any, kmeans_model: Any
    ) -> dict[str, Any]:
        ...
        try:
            ...
            metrics["cluster_balance"] = (
                metrics["std_cluster_size"] / metrics["mean_cluster_size"]
                if metrics["mean_cluster_size"] > 0
                else 0
            )
    ...
    @handle_errors(
        exceptions=(Exception,), default_return={}, context="analyze_composite_clusters"
    )
        ...
        try:
            ...
            for cluster_id in unique_clusters:
                ...
                cluster_char = {
                    ...
                    "hmm_state_distribution": self._calculate_hmm_state_distribution(
                        cluster_hmm_states
                    ),
                    "feature_means": {},
                    "feature_stds": {},
                    "dominant_hmm_state": self._get_dominant_hmm_state(
                        cluster_hmm_states
                    ),
                }
            ...
            analysis["hmm_state_distribution"] = self._calculate_hmm_state_distribution(
                hmm_states
            )
            ...
            analysis["cluster_stability"] = self._calculate_cluster_stability(
                cluster_labels, cluster_metrics
            )

            self.logger.info(
                f"✅ Composite cluster analysis completed for {len(unique_clusters)} clusters"
            )
            return analysis
    ...
    async def _generate_comprehensive_reports(
        ...
        try:
            ...
            reports["cluster_quality"] = self._generate_cluster_quality_report(
                cluster_metrics
            )

            # 2. Cluster Characteristics Report
            reports["cluster_characteristics"] = (
                self._generate_cluster_characteristics_report(composite_analysis)
            )

            # 3. Market Conditions Report
            reports["market_conditions"] = self._generate_market_conditions_report(
                composite_analysis
            )

            # 4. Feature Importance Report
            reports["feature_importance"] = self._generate_feature_importance_report(
                composite_analysis
            )

            # 5. HMM State Analysis Report
            reports["hmm_state_analysis"] = self._generate_hmm_state_analysis_report(
                hmm_states, composite_analysis
            )

            # 6. Temporal Analysis Report
            reports["temporal_analysis"] = self._generate_temporal_analysis_report(
                cluster_labels, features
            )

            # 7. Recommendations Report
            reports["recommendations"] = self._generate_recommendations_report(
                cluster_metrics, composite_analysis
            )
    ...
    def _create_composite_cluster_dataframe(
        ...
        try:
            ...
            for cluster_id, char in composite_analysis.get(
                "cluster_characteristics", {}
            ).items():
                cluster_mask = cluster_labels == cluster_id
                ...
                df.loc[cluster_mask, "market_condition"] = composite_analysis.get(
                    "market_conditions", {}
                ).get(cluster_id, "unknown")

            # Add intensity scores
            df["cluster_intensity"] = self._calculate_cluster_intensity(
                cluster_labels, composite_analysis
            )

            # Add stability metrics
            df["cluster_stability"] = self._calculate_cluster_stability_scores(
                cluster_labels, composite_analysis
            )

            self.logger.info(
                f"✅ Created composite cluster DataFrame: {len(df)} rows, {len(df.columns)} columns"
            )
            return df
    ...
    def _create_intensity_dataframe(
        ...
        try:
            ...
            for cluster_id in unique_clusters:
                cluster_mask = cluster_labels == cluster_id
                cluster_char = composite_analysis.get(
                    "cluster_characteristics", {}
                ).get(cluster_id, {})

                # Calculate various intensity metrics
                intensity_df.loc[cluster_mask, "cluster_intensity"] = cluster_char.get(
                    "size", 0
                ) / len(features)
                intensity_df.loc[cluster_mask, "volatility_intensity"] = (
                    self._calculate_volatility_intensity(features, cluster_mask)
                )
                intensity_df.loc[cluster_mask, "momentum_intensity"] = (
                    self._calculate_momentum_intensity(features, cluster_mask)
                )
                intensity_df.loc[cluster_mask, "volume_intensity"] = (
                    self._calculate_volume_intensity(features, cluster_mask)
                )
            ...
            self.logger.info(
                f"✅ Created intensity DataFrame: {len(intensity_df)} rows, {len(intensity_df.columns)} columns"
            )
            return intensity_df
    ...
    @handle_errors(
        exceptions=(Exception,), default_return={}, context="create_meta_information"
    )
        ...
        try:
            ...
            meta = {
                ...
                "composite_analysis_summary": {
                    "total_clusters": len(
                        composite_analysis.get("cluster_characteristics", {})
                    ),
                    "hmm_states": len(
                        composite_analysis.get("hmm_state_distribution", {})
                    ),
                    "market_conditions": len(
                        composite_analysis.get("market_conditions", {})
                    ),
                },
                "reports_summary": {
                ...
                "feature_summary": {
                    "total_features": len(
                        composite_analysis.get("feature_importance", {})
                    ),
                    "top_features": sorted(
    ...
    def _determine_market_condition(self, cluster_char: dict[str, Any]) -> str:
        ...
        try:
            ...
            volume_ratio = cluster_char.get("feature_means", {}).get(
                "volume_ratio_10", 1
            )
    ...
    def _calculate_feature_importance(
        self, features: Any, cluster_labels: Any
    ) -> dict[str, float]:
                ...
                if col in features.columns:
                    ...
                    if total_var > 0:
                        ...
                        for cluster_id in np.unique(cluster_labels):
                            ...
                            between_cluster_var += (
                                cluster_size
                                * (cluster_mean - features[col].mean()) ** 2
                            )
    ...
    def _calculate_cluster_stability(
        self, cluster_labels: Any, cluster_metrics: dict[str, Any]
    ) -> dict[str, float]:
        """Calculate cluster stability metrics."""
        try:
            stability = {
                ...
                "mean_distance_to_center": cluster_metrics.get(
                    "mean_distance_to_center", 0
                ),
            }
            return stability
        except Exception:
    ...
    def _calculate_cluster_intensity(
        self, cluster_labels: Any, composite_analysis: dict[str, Any]
    ) -> Any:
    ...
    def _calculate_volatility_intensity(
        self, features: Any, cluster_mask: Any
    ) -> float:
    ...
    def _generate_cluster_quality_report(self, cluster_metrics: dict[str, Any]) -> str:
        ...
        try:
            ...
            report.append(
                f"- **Silhouette Score**: {cluster_metrics.get('silhouette_score', 0):.4f}"
            )
            report.append(
                f"- **Calinski-Harabasz Score**: {cluster_metrics.get('calinski_harabasz_score', 0):.2f}"
            )
            report.append(
                f"- **Davies-Bouldin Score**: {cluster_metrics.get('davies_bouldin_score', 0):.4f}"
            )
            report.append(f"- **Inertia**: {cluster_metrics.get('inertia', 0):.2f}")
            report.append("")
            report.append(f"## Cluster Distribution")
            report.append(
                f"- **Min Cluster Size**: {cluster_metrics.get('min_cluster_size', 0)}"
            )
            report.append(
                f"- **Max Cluster Size**: {cluster_metrics.get('max_cluster_size', 0)}"
            )
            report.append(
                f"- **Mean Cluster Size**: {cluster_metrics.get('mean_cluster_size', 0):.1f}"
            )
            report.append(
                f"- **Cluster Balance**: {cluster_metrics.get('cluster_balance', 0):.4f}"
            )
    ...
    def _generate_cluster_characteristics_report(
        self, composite_analysis: dict[str, Any]
    ) -> str:
        ...
        try:
            ...
            for cluster_id, char in composite_analysis.get(
                ...
                report.append(
                    f"- **Size**: {char.get('size', 0)} ({char.get('percentage', 0):.1f}%)"
                )
                report.append(
                    f"- **Dominant HMM State**: {char.get('dominant_hmm_state', 'unknown')}"
                )
                report.append(
                    f"- **Market Condition**: {composite_analysis.get('market_conditions', {}).get(cluster_id, 'unknown')}"
                )
                report.append("")
    ...
    def _generate_hmm_state_analysis_report(
        self, hmm_states: Any, composite_analysis: dict[str, Any]
    ) -> str:
    ...
    def _generate_temporal_analysis_report(
        self, cluster_labels: Any, features: Any
    ) -> str:
        ...
        try:
            ...
            report.append(
                f"- **Transition Rate**: {transitions / len(cluster_labels) * 100:.2f}%"
            )
    ...
    def _generate_recommendations_report(
        self, cluster_metrics: dict[str, Any], composite_analysis: dict[str, Any]
    ) -> str:
        ...
        try:
            ...
            if silhouette < 0.2:
                report.append(
                    "- **Low Silhouette Score**: Consider reducing number of clusters or improving feature engineering"
                )
            elif silhouette > 0.5:
                report.append(
                    "- **Good Silhouette Score**: Clusters are well-separated"
                )
            ...
            if balance > 0.5:
                report.append(
                    "- **Unbalanced Clusters**: Consider adjusting clustering parameters for better balance"
                )
            ...
            if feature_importance:
                ...
                report.append(
                    f"- **Most Important Feature**: {top_feature[0]} (importance: {top_feature[1]:.4f})"
                )
    ...
    def _should_run_optimization(
        ...
        self.logger.info(
            "🔄 Step 3 optimization: Always running parameter optimization"
        )
        return True
    ...
    async def _run_automatic_optimization(
        ...
        try:
            ...
            try:
                ...
                from optimize_hmm_regime_parameters import (
                    HMMRegimeOptimizer,
                    identify_market_condition_columns,
                )
            ...
            feature_data = await self._load_feature_data_for_optimization(
                symbol, exchange, timeframe, data_dir
            )
            ...
            self.logger.info(
                f"📊 Optimization data: {len(feature_data)} samples, {len(feature_columns)} features"
            )
            ...
            optimization_results = optimizer.optimize(
                ...
                timeout=auto_config.get("timeout_minutes", 30)
    ...
    async def _load_feature_data_for_optimization(
        ...
        try:
            ...
            self.logger.info(
                "📂 Feature file not found, creating basic features from raw data"
            )
            ...
            self.logger.exception(
                f"❌ Error loading feature data for optimization: {e}"
            )
            return None

    async def _create_basic_features(self, data: pd.DataFrame) -> pd.DataFrame:
        """Create basic features for optimization if Step 2 features are not
        available."""
        ...
        try:
            ...
            if "volume" in features.columns:
                features["volume_ratio_10"] = (
                    features["volume"] / features["volume"].rolling(10).mean()
                )
    ...
    async def _save_optimization_results(
        ...
        try:
            ...
            results_file = (
                optimization_dir
                / f"{exchange}_{symbol}_{timeframe}_optimization_results.json"
            )
    ...
    async def _generate_optimization_report(
        self, optimizer: Any, symbol: str, exchange: str, timeframe: str, data_dir: str
    ) -> None:
        """Generate optimization report."""
        ...
        try:
            ...
            report_file = (
                optimization_dir
                / f"{exchange}_{symbol}_{timeframe}_optimization_report.md"
            )
    ...
    def _apply_optimized_parameters(self, optimized_params: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if "similarity_threshold" in optimized_params:
                self.config["similarity_threshold"] = optimized_params[
                    "similarity_threshold"
                ]
            if "coherence_threshold" in optimized_params:
                self.config["coherence_threshold"] = optimized_params[
                    "coherence_threshold"
                ]
            if "differentiation_threshold" in optimized_params:
                self.config["differentiation_threshold"] = optimized_params[
...
if __name__ == "__main__":
    ...
    async def main() -> None:
        ...
        if len(sys.argv) >= 4:
            ...
            print(
                "Usage: python step3_hmm_regime_discovery.py <symbol> <exchange> <timeframe> [data_dir] [force_rerun]"
            )
            print(
                "Example: python step3_hmm_regime_discovery.py ETHUSDT BINANCE 1m data_cache true"
            )
            return
