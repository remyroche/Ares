...
class BayesianProbabilityUpdater:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.price_targets = [f'{i * 0.1:.1f}%' for i in range(1, 21)]
        self.prior_alpha = self.bayesian_config.get('prior_alpha', 1.0)
        self.prior_beta = self.bayesian_config.get('prior_beta', 1.0)
    ...
    async def update_probability(self, target: str, regime: str, success: bool, confidence: float = 1.0, metadata: Dict[str, Any]=None) -> Optional[BayesianUpdate]:
        ...
        try:
            ...
            confidence_interval = self._calculate_credible_interval(new_alpha, new_beta, self.confidence_level)
    ...
    def _calculate_credible_interval(self, alpha: float, beta: float, confidence_level: float) -> Tuple[float, float]:
        ...
        try:
            ...
            for target in self.price_targets:
                ...
                if model['evidence_count'] >= min_evidence:
                    ...
                    confidence_interval = self._calculate_credible_interval(alpha, beta, self.confidence_level)
                    confidence = self._calculate_bayesian_confidence(alpha, beta)
                    probabilities[target] = {'probability': probability, 'confidence_interval': confidence_interval, 'confidence': confidence, 'evidence_count': model['evidence_count'], 'last_updated': model['last_updated']}
            return {'regime': regime, 'probabilities': probabilities, 'total_targets': len(probabilities), 'min_evidence': min_evidence, 'timestamp': datetime.now()}
        except Exception as e:
        ...
        try:
            ...
            for observation in observations:
                ...
                if target and success is not None:
                    update_result = await self.update_probability(target, regime, success, confidence, metadata)
                    if update_result:
    ...
    async def compare_regime_probabilities(self, regime1: str, regime2: str, target: str = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            if regime1 not in self.regime_names or regime2 not in self.regime_names:
                ...
                return None
            comparison_results = {'regime1': regime1, 'regime2': regime2, 'target_comparisons': {}, 'overall_comparison': {}, 'timestamp': datetime.now()}
            ...
            for target_name in targets_to_compare:
                ...
                if model1['evidence_count'] >= self.min_evidence and model2['evidence_count'] >= self.min_evidence:
                    ...
                    significance = self._test_probability_difference(model1['alpha'], model1['beta'], model2['alpha'], model2['beta'])
                    comparison_results['target_comparisons'][target_name] = {'regime1_probability': prob1, 'regime2_probability': prob2, 'difference': prob1 - prob2, 'relative_difference': (prob1 - prob2) / prob2 if prob2 > 0 else 0, 'statistical_significance': significance, 'regime1_evidence': model1['evidence_count'], 'regime2_evidence': model2['evidence_count']}
            if comparison_results['target_comparisons']:
                ...
                comparison_results['overall_comparison'] = {'avg_difference': np.mean(differences), 'significant_differences': sum(significant_differences), 'total_comparisons': len(differences), 'significance_rate': sum(significant_differences) / len(differences) if differences else 0}
            return comparison_results
        except Exception as e:
            self.logger.error(f'Error comparing regimes {regime1} and {regime2}: {e}')
            return None

    def _test_probability_difference(self, alpha1: float, beta1: float, alpha2: float, beta2: float) -> bool:
    ...
    def get_bayesian_summary(self) -> Dict[str, Any]:
        ...
        summary = {'system_status': 'active', 'regime_count': len(self.regime_names), 'target_count': len(self.price_targets), 'total_models': len(self.regime_names) * len(self.price_targets), 'prior_parameters': {'alpha': self.prior_alpha, 'beta': self.prior_beta}, 'confidence_level': self.confidence_level, 'min_evidence': self.min_evidence, 'regime_summaries': {}}
        for regime in self.regime_names:
            regime_models = self.bayesian_models[regime]
            evidence_counts = [model['evidence_count'] for model in regime_models.values()]
            summary['regime_summaries'][regime] = {'total_evidence': sum(evidence_counts), 'avg_evidence': np.mean(evidence_counts), 'max_evidence': max(evidence_counts), 'min_evidence': min(evidence_counts), 'models_with_sufficient_evidence': sum((1 for count in evidence_counts if count >= self.min_evidence))}
        return summary