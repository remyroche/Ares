...
class RegimeAwareConfidenceCalibrationStep:
    ...
    @handles_errors(exceptions=(Exception,), default_return={'status': 'FAILED', 'error': 'Execution failed'}, context='confidence calibration step execution')
    async def execute(self, training_input: dict[str, Any], pipeline_state: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
                            ...
                            for model_file in os.listdir(regime_path):
                                if model_file.endswith(('.pkl', '.joblib')):
                                    ...
                                    try:
                                        ...
                                        self.logger.warning(f'⚠️ Failed to load model {model_file}: {e}')
            ...
            if os.path.exists(tactician_models_dir):
                ...
                with contextlib.suppress(Exception):
                    self.logger.info(f'Tactician models loaded: count={len(tactician_models)}')
            ...
            if os.path.exists(tactician_ensembles_dir):
                ...
                with heartbeat(self.logger, name='Step11 load_tactician_ensembles', interval_seconds=60.0):
                    ...
                    for ensemble_file in os.listdir(tactician_ensembles_dir):
                        candidate_path = os.path.join(tactician_ensembles_dir, ensemble_file)
                        if ensemble_file.endswith('_tactician_ensemble.pkl') and candidate_path != model_path:
                            try:
            ...
            try:
                ...
                self.logger.warning(f'⚠️ Could not augment validation frame with 1m meta-labels: {_ce}')
            with contextlib.suppress(Exception):
                self.logger.info(f"Validation frame loaded: shape={getattr(generic_val, 'shape', None)}")
            calibration_results: dict[str, Any] = {}
            self.logger.info('Step11: Calibrating analyst models...')
            analyst_calibration = await self._calibrate_regime_aware_analyst_models(analyst_models, analyst_ensembles, generic_val, data_dir, exchange, symbol)
            calibration_results['analyst_models'] = analyst_calibration
            with contextlib.suppress(Exception):
                pass
            self.logger.info('Step11: Calibrating tactician models...')
            tactician_calibration = await self._calibrate_regime_aware_tactician_models(tactician_models, tactician_ensembles, generic_val, data_dir, exchange, symbol)
            calibration_results['tactician_models'] = tactician_calibration
            with contextlib.suppress(Exception):
                pass
            self.logger.info('Step11: Calibrating analyst ensembles...')
            analyst_ensemble_calibration = await self._calibrate_analyst_ensembles(analyst_ensembles, generic_val, data_dir, exchange, symbol)
            ...
            tactician_ensemble_calibration = await self._calibrate_tactician_ensembles(tactician_ensembles, generic_val)
            ...
            try:
                ...
                summary_counts = {'analyst_models': sum((len(v or {}) for v in calibration_results.get('analyst_models', {}).values())), 'tactician_models': len(calibration_results.get('tactician_models', {})), 'analyst_ensembles': len(calibration_results.get('analyst_ensembles', {})), 'tactician_ensembles': len(calibration_results.get('tactician_ensembles', {}))}
                            ...
                            if isinstance(models, dict):
                                for name, res in models.items():
                                    if isinstance(res, dict) and 'metrics' in res:
                                        acc_map[name] = float(res.get('metrics', {}).get('accuracy', 0.0))
                    except Exception as e:
            ...
            return {'calibration_results': calibration_results, 'calibration_file': calibration_file, 'duration': 0.0, 'status': 'SUCCESS'}
        except Exception as e:
    ...
    async def _calibrate_analyst_models(self, models: dict[str, dict[str, Any]], ensembles: dict[str, Any], generic_val: pd.DataFrame | None, data_dir: str, exchange: str, symbol: str) -> dict[str, Any]:
        ...
        for regime_name, regime_models in models.items():
            ...
            for model_name, model_data in regime_models.items():
                try:
                    base_model = model_data if hasattr(model_data, 'predict_proba') else model_data.get('model', None) if isinstance(model_data, dict) else None
                    ...
                    regime_res[model_name] = {'calibrated_model': calibrator, 'metrics': {'accuracy': acc, 'f1': f1}, 'base_metrics': base_metrics, 'calibration_method': 'isotonic_prefit', 'regime': regime_name}
                    with contextlib.suppress(Exception):
                        self.logger.info({'msg': 'calibration_model_metrics', 'regime': regime_name, 'model': model_name, 'base': base_metrics, 'calibrated': {'accuracy': float(acc), 'f1': float(f1)}})
                except Exception as e:
                    self.logger.warning(f'Calibration failed for analyst model {model_name} in {regime_name}: {e}')
            results[regime_name] = regime_res
        return results

    async def _calibrate_tactician_models(self, models: dict[str, Any], ensembles: dict[str, Any], generic_val: pd.DataFrame | None) -> dict[str, Any]:
        results: dict[str, Any] = {}
        ...
        for model_name, model_data in models.items():
            try:
                ...
                acc = accuracy_score(y_val, calibrator.predict(X_val))
                f1 = f1_score(y_val, calibrator.predict(X_val), average='weighted')
                results[model_name] = {'calibrated_model': calibrator, 'metrics': {'accuracy': acc, 'f1': f1}, 'base_metrics': base_metrics, 'calibration_method': 'isotonic_prefit'}
                with contextlib.suppress(Exception):
                    self.logger.info({'msg': 'calibration_tactician_model_metrics', 'model': model_name, 'base': base_metrics, 'calibrated': {'accuracy': float(acc), 'f1': float(f1)}})
            except Exception as e:
                self.logger.warning(f'Calibration failed for tactician model {model_name}: {e}')
        return results

    async def _calibrate_analyst_ensembles(self, ensembles: dict[str, Any], generic_val: pd.DataFrame | None, data_dir: str, exchange: str, symbol: str) -> dict[str, Any]:
        results: dict[str, Any] = {}
        for regime_name, regime_ensembles in ensembles.items():
            ...
            try:
                ...
                acc = accuracy_score(y_val, calibrator.predict(X_val))
                f1 = f1_score(y_val, calibrator.predict(X_val), average='weighted')
                results[regime_name] = {'calibrated_ensemble': calibrator, 'metrics': {'accuracy': acc, 'f1': f1}, 'base_metrics': base_metrics, 'calibration_method': 'isotonic_prefit'}
                with contextlib.suppress(Exception):
                    self.logger.info({'msg': 'calibration_analyst_ensemble_metrics', 'regime': regime_name, 'base': base_metrics, 'calibrated': {'accuracy': float(acc), 'f1': float(f1)}})
            except Exception as e:
                self.logger.warning(f'Calibration failed for analyst ensemble in {regime_name}: {e}')
        return results

    async def _calibrate_tactician_ensembles(self, ensembles: dict[str, Any], generic_val: pd.DataFrame | None) -> dict[str, Any]:
        results: dict[str, Any] = {}
        ...
        for ensemble_type, ensemble_data in ensembles.items():
            ...
            try:
                ...
                acc = accuracy_score(y_val, calibrator.predict(X_val))
                f1 = f1_score(y_val, calibrator.predict(X_val), average='weighted')
                results[ensemble_type] = {'calibrated_ensemble': calibrator, 'metrics': {'accuracy': acc, 'f1': f1}, 'base_metrics': base_metrics, 'calibration_method': 'isotonic_prefit'}
                with contextlib.suppress(Exception):
                    self.logger.info({'msg': 'calibration_tactician_ensemble_metrics', 'type': ensemble_type, 'base': base_metrics, 'calibrated': {'accuracy': float(acc), 'f1': float(f1)}})
            except Exception as e:
    ...
    def _summarize_calibration(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        summary['analyst_models'] = {regime: {name: data.get('metrics', {}) for name, data in models.items()} for regime, models in analyst.items()}
        tact_models = results.get('tactician_models', {})
        summary['tactician_models'] = {name: data.get('metrics', {}) for name, data in tact_models.items()}
        analyst_ens = results.get('analyst_ensembles', {})
        summary['analyst_ensembles'] = {regime: data.get('metrics', {}) for regime, data in analyst_ens.items()}
    ...
    def _calculate_base_metrics(self, model: Any, X_val: pd.DataFrame, y_val: pd.Series) -> dict[str, float]:
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                self.logger.warning(f'Could not calculate base metrics for {type(model).__name__}: {e}')
            return {}
...
async def _calibrate_regime_aware_analyst_models(self, models: dict[str, dict[str, Any]], ensembles: dict[str, Any], generic_val: pd.DataFrame | None, data_dir: str, exchange: str, symbol: str) -> dict[str, Any]:
    ...
    try:
        ...
        has_regime_specific_structure = any((isinstance(regime_models, dict) and any((isinstance(model_data, dict) for model_data in regime_models.values())) for regime_models in models.values()))
        if has_regime_specific_structure:
            ...
            for regime_name, regime_models in models.items():
                ...
                if regime_val is not None and len(regime_val) >= self.regime_config['min_regime_samples']:
                    regime_calibrated = await self._calibrate_regime_models(regime_models, regime_name, regime_val)
                    regime_calibration_results[regime_name] = regime_calibrated
                    if self.regime_config['regime_specific_logging']:
                        self._log_regime_specific_metrics(regime_name, {'models_calibrated': len(regime_calibrated), 'validation_samples': len(regime_val), 'regime': regime_name}, 'analyst_calibration')
                else:
            ...
            regime_calibration_results = await self._calibrate_analyst_models(models, ensembles, generic_val, data_dir, exchange, symbol)
...
async def _calibrate_regime_aware_tactician_models(self, models: dict[str, Any], ensembles: dict[str, Any], generic_val: pd.DataFrame | None, data_dir: str, exchange: str, symbol: str) -> dict[str, Any]:
        ...
        if has_regime_specific_structure:
            ...
            for regime_name, regime_models in models.items():
                ...
                if regime_val is not None and len(regime_val) >= self.regime_config['min_regime_samples']:
                    ...
                    regime_calibration_results[regime_name] = regime_calibrated
                    if self.regime_config['regime_specific_logging']:
                        self._log_regime_specific_metrics(regime_name, {'models_calibrated': len(regime_calibrated), 'validation_samples': len(regime_val), 'regime': regime_name}, 'tactician_calibration')
                else:
                    self.logger.warning(f'⚠️ Insufficient validation data for regime {regime_name}')
        else:
            self.logger.info('🔄 Using traditional calibration (no regime structure)')
            regime_calibration_results = await self._calibrate_tactician_models(models, ensembles, generic_val)
...
async def _calibrate_regime_models(self, regime_models: dict[str, Any], regime_name: str, validation_data: pd.DataFrame) -> dict[str, Any]:
    ...
    try:
        ...
        for model_name, model_data in regime_models.items():
            try:
                calibrated_model = await self._apply_regime_calibration(model_data, model_name, regime_name, validation_data)
                ...
                calibrated_models[model_name] = model_data
        return calibrated_models
    except Exception as e:
        self.logger.error(f'❌ Error calibrating models for regime {regime_name}: {e}')
        raise

async def _apply_regime_calibration(self, model_data: dict[str, Any], model_name: str, regime_name: str, validation_data: pd.DataFrame) -> dict[str, Any]:
    """Apply calibration to a specific model for a specific regime."""
    try:
        model = model_data.get('model')
        if model is None:
            return model_data
        feature_columns = [col for col in validation_data.columns if col not in ['timestamp', 'exchange', 'symbol', 'timeframe', 'composite_cluster_id']]
