...
def _numba_backtest(
    ...
    high_prices: np.ndarray = signals: np.ndarray,
    ml_buy_confidence: np.ndarray = ml_sell_confidence: np.ndarray,
    tp_long: float = sl_long: float,
    tp_short: float = sl_short: float,
    enable_ml_early_exit: bool = early_exit_confidence: float,
) -> np.ndarray:
    """
    A Numba-accelerated backtesting loop for both long and short trades = including asymmetrical barriers and trading fees. Returns an array of
...
class TpSlOptimizer:
    ...
    def _run_backtest(
        self = tp_long: float,
        sl_long: float = tp_short: float,
        sl_short: float = enable_ml_early_exit: bool,
        early_exit_confidence: float = ) -> pd.DataFrame:
        pnl_array = _numba_backtest(
            ...
            tp_long = sl_long,
            tp_short = sl_short,
            enable_ml_early_exit = early_exit_confidence,
        )
    ...
    def objective(self, trial: optuna.trial.Trial) -> float:
        ...
        results_df = self._run_backtest(
            ...
            enable_ml_early_exit, enable_ml_early_exit = early_exit_confidence=early_exit_confidence,
        )
