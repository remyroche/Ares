...
## 2026-06-15 - Vectorized mark_resolved Update
Learning: The `mark_resolved` function inside `extreme_price_movements/inference/prediction_ledger.py` updated rows in a pandas DataFrame sequentially via `iterrows()`, `.loc` indexing, and a row-by-row `pd.concat`. For an operation that requires indexing and rewriting a potentially large internal DataFrame tracking updates, this is an anti-pattern.
Action: Rewrote the update process to purely vectorized logic: deduplicate the incoming DataFrame via `~duplicated(keep='last')`, append missing schema columns with `pd.NA`, and process matches and new rows strictly using index intersection/difference followed by bulk `.loc` replacement and a single `pd.concat`. This is much faster and more memory efficient.
