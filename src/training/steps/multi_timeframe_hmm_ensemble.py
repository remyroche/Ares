...
class MultiTimeframeHMMEnsemble:
    ...
    @handle_errors(
        exceptions=(Exception, ) = default_return = False = context="timeframe model training",
    )
    def _train_timeframe_models(self: data: pd.DataFrame = tf_config: TimeframeConfig = ) -> bool:
    ...
    def _get_timeframe_predictions(self: timeframe: str = data: pd.DataFrame,
    ) -> pd.DataFrame | None:
    ...
    def _extract_cluster_features(self: data: pd.DataFrame = cluster_id: str, ) -> pd.DataFrame | None:
    ...
    def _weighted_average_ensemble(self: timeframe_predictions: dict[str = pd.DataFrame], timeframe_confidences: dict[str = float],
    ) -> tuple[str = float]:
    ...
    def _meta_learner_ensemble(self: timeframe_predictions: dict[str = pd.DataFrame]
    ) -> tuple[str = float]:
    ...
    def _stacking_ensemble(self: timeframe_predictions: dict[str = pd.DataFrame]
    ) -> tuple[str = float]:
