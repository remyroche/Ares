...
class ErrorReporter:
    ...
    def generate_report(self, target_path: str | Path | None = None) -> ErrorReport:
        """
    ...
    def _generate_summary(self, errors: list[dict]) -> ErrorSummary:
        ...
        # Count files
        files_with_errors = len({e.get("file_path", "") for e in errors if e.get("type") == "error"})
        files_with_warnings = len({e.get("file_path", "") for e in errors if e.get("type") == "warning"})
        total_files = len({e.get("file_path", "") for e in errors})
    ...
    def _categorize_errors(self, errors: list[dict]) -> list[ErrorCategory]:
        ...
        for category_name, category_errors in category_groups.items():
            ...
            files_affected = len({e.get("file_path", "") for e in category_errors})
    ...
    def _analyze_files(self, errors: list[dict], target_path: str | Path | None = None) -> list[FileErrorSummary]:
    ...
    def export_report(self, report: ErrorReport, format: str = "json", output_path: str | Path | None = None) -> str:
        """
