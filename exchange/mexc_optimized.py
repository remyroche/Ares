...
class MexcExchangeOptimized(BaseExchange):
    ...
    async def _get_historical_agg_trades_raw(
        ...
        try:
            ...
            print(
                f"🔍 DEBUG: Time range: {datetime.fromtimestamp(since / 1000)} to {datetime.fromtimestamp(end_time_ms / 1000)}",
            )
    ...
    async def get_historical_agg_trades(
        ...
        print(
            f"🔍 DEBUG: Optimized MEXC get_historical_agg_trades completed in {end_time - start_time:.2f} seconds",
        )
