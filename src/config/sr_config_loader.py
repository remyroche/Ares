...
class SRConfigLoader:
    ...
    def _config_to_dict(self, config: SROptimizationConfig) -> Dict[str, Any]:
        ...
        return {'sr_detection_optimization': {'n_trials': config.n_trials, 'cv_folds': config.cv_folds, 'test_size': config.test_size, 'optimization_timeout': config.optimization_timeout, 'performance_thresholds': {'min_sr_validation_score': config.performance_thresholds.min_sr_validation_score, 'min_bounce_rate': config.performance_thresholds.min_bounce_rate, 'max_false_breakout_rate': config.performance_thresholds.max_false_breakout_rate, 'min_volume_confirmation': config.performance_thresholds.min_volume_confirmation, 'min_level_detection_accuracy': config.performance_thresholds.min_level_detection_accuracy}, 'timeframe_config': {tf: {'touch_threshold': tf_config.touch_threshold, 'bounce_threshold': tf_config.bounce_threshold, 'breakout_threshold': tf_config.breakout_threshold, 'min_touches': tf_config.min_touches, 'volume_spike_threshold': tf_config.volume_spike_threshold, 'fractal_period': tf_config.fractal_period, 'pivot_period': tf_config.pivot_period} for tf, tf_config in config.timeframe_configs.items()}, 'parameter_ranges': {'fractal_weight': list(config.parameter_ranges.fractal_weight), 'volume_weight': list(config.parameter_ranges.volume_weight), 'pivot_weight': list(config.parameter_ranges.pivot_weight), 'atr_weight': list(config.parameter_ranges.atr_weight), 'touch_count_weight': list(config.parameter_ranges.touch_count_weight), 'total_volume_weight': list(config.parameter_ranges.total_volume_weight), 'level_age_weight': list(config.parameter_ranges.level_age_weight), 'bounce_rate_weight': list(config.parameter_ranges.bounce_rate_weight), 'isolation_score_weight': list(config.parameter_ranges.isolation_score_weight), 'timeframe_weights': {'tf_15m_weight': list(config.parameter_ranges.tf_15m_weight), 'tf_1h_weight': list(config.parameter_ranges.tf_1h_weight), 'tf_4h_weight': list(config.parameter_ranges.tf_4h_weight), 'tf_1d_weight': list(config.parameter_ranges.tf_1d_weight)}, 'fibonacci_sensitivity': list(config.parameter_ranges.fibonacci_sensitivity), 'elliott_confidence_threshold': list(config.parameter_ranges.elliott_confidence_threshold), 'order_flow_hvn_threshold': list(config.parameter_ranges.order_flow_hvn_threshold)}}, 'caching': config.caching_config, 'error_handling': config.error_handling_config, 'memory_management': config.memory_management_config}
