...
class DataFlowVisitor(ast.NodeVisitor):
    ...
    def _calculate_data_flow_complexity(self, file_path: Path) -> int:
        ...
        try:
            ...
            # Simple complexity calculation based on variable interactions
            lines = content.split("\n")
            complexity = 0
    ...
    def _calculate_validation_coverage(self) -> dict[str, float]:
        ...
        total_variables = len(self.file_variables)
        validated_variables = 0
        
        for file_path, variables in self.file_variables.items():
            file_validations = self.validations.get(file_path, [])
            for var_name in variables:
                if any(v.variable == var_name for v in file_validations):
                    validated_variables += 1
        
        coverage = (validated_variables / total_variables * 100) if total_variables > 0 else 0
