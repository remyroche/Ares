...
class PrecomputedFeaturesManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.price_difference_features = {
            "price_change", "price_momentum", "gap_size", "dist_to_resistance",
            "dist_to_support", "price_return", "log_return"
        }
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"❌ Failed to initialize PrecomputedFeaturesManager: {e}")
            return False
    ...
    def generate_feature_name(self, category: str, timeframe: str, name: str) -> str:
        ...
        if category not in self.feature_categories:
            raise ValueError(f"Invalid category: {category}. Valid categories: {list(self.feature_categories.keys())}")
            
        if timeframe not in self.timeframes:
            raise ValueError(f"Invalid timeframe: {timeframe}. Valid timeframes: {self.timeframes}")
    ...
    def parse_feature_name(self, feature_name: str) -> Tuple[str, str, str]:
        ...
        if category not in self.feature_categories:
            raise ValueError(f"Invalid category in feature name: {category}")
            
        if timeframe not in self.timeframes:
            raise ValueError(f"Invalid timeframe in feature name: {timeframe}")
    ...
    async def retrieve_features(
        ...
        try:
            ...
            df = self.db_manager.query_api.query_data_frame(query=query, org=self.db_manager.org)
    ...
    def _ensure_price_differences(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for col in df_copy.columns:
                ...
                try:
                    ...
                    if category == "price" and any(price_feat in name for price_feat in self.price_difference_features):
                        ...
                        continue
                    elif category == "price" and any(abs_feat in name for abs_feat in ["open", "high", "low", "close"]):
                        # Convert absolute prices to percentage changes
                        if name.endswith("_close"):
                            df_copy[col] = df_copy[col].pct_change()
                        elif name.endswith("_open"):
                            df_copy[col] = df_copy[col].pct_change()
                        elif name.endswith("_high") or name.endswith("_low"):
            ...
            self.logger.error(f"Error ensuring price differences: {e}")
            return df
    ...
    def _apply_feature_filters(
        ...
        try:
            ...
            self.logger.error(f"Error applying feature filters: {e}")
            return df
    ...
    async def _create_feature_metadata_tables(self):
        ...
        try:
            ...
            self.logger.error(f"Error creating feature metadata tables: {e}")
    ...
    async def _store_feature_metadata(
        ...
        try:
            ...
            for feature_name in feature_names:
                try:
                    ...
                    record = {
                        ...
                        "description": self.feature_categories.get(category, "Unknown category")
                    }
            ...
            self.logger.error(f"Error storing feature metadata: {e}")

    def get_available_features(self, category: Optional[str] = None, timeframe: Optional[str] = None) -> List[str]:
        """
