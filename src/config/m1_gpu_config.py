...
def validate_m1_config(config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if gpu_config["gpu_memory_fraction"] <= 0 or gpu_config["gpu_memory_fraction"] > 1:
        ...
        if matrix_config["min_matrix_size_for_gpu"] <= 0:
            raise ValueError("min_matrix_size_for_gpu must be positive")
        if matrix_config["min_batch_size_for_gpu"] <= 0:
            raise ValueError("min_batch_size_for_gpu must be positive")
        if matrix_config["max_gpu_memory_usage"] <= 0 or matrix_config["max_gpu_memory_usage"] > 1:
            raise ValueError("max_gpu_memory_usage must be between 0 and 1")
