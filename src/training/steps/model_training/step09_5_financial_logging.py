...
"""
Financial metrics logging for Step09.5_5.
...
logger = system_logger.getChild('Step095Financiallogging')

class Step095FinancialloggingFinancialLogger:
    """Independent financial metrics logger for Step09.5_5 with enhanced regime logging."""
    ...
    def log_step_execution(self, *args, data: Optional[pd.DataFrame] = None, **kwargs) -> bool:
        """
        Log comprehensive financial metrics for Step09.5_5 execution with enhanced regime validation.
        ...
        try:
            ...
            if validate_and_log_regime_data:
                validation_success = validate_and_log_regime_data(
                    ...
                    step_name="Step09.5_5",
                    data=data,
                    regime_column='composite_cluster_id'
                )
                
                if not validation_success:
                    logger.error("🚨 Regime validation failed for Step09.5_5")
                    return False
            
            # Log step start
            self.financial_logger.log_step_start("Step09.5_5", self.symbol, self.exchange, self.timeframe)
    ...
    def _log_with_standard_method(self, *args, **kwargs) -> bool:
        ...
        with financial_metrics_context(
            ...
            try:
                ...
                self.financial_logger.log_step_end("Step09.5_5", self.symbol, self.exchange, self.timeframe, success=True)
                
                return True
                
            except Exception as e:
                self.financial_logger.log_step_end("Step09.5_5", self.symbol, self.exchange, self.timeframe, success=False, error_message=str(e))
                logger.error(f"Failed to log financial metrics: {e}")
                return False
    ...
    def _log_created_file_paths(self) -> None:
        ...
        try:
            ...
            logger.info("📁 File paths logged for Step09.5_5")
        except Exception as e:
            logger.warning(f"Could not log file paths: {e}")

# Enhanced Step095Financiallogging Financial Logger with Regime-Aware Decorator Support
class EnhancedStep095FinancialloggingFinancialLogger(Step095FinancialloggingFinancialLogger):
    """Enhanced Step095Financiallogging Financial Logger with automatic regime-aware logging decorator support."""
