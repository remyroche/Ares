...
## $(date +%Y-%m-%d) - [DataFrame Updates] Learning: DataFrame index intersections are massively faster than iterrows
Action: For bulk updates based on keys in pandas, replace `.iterrows()` with `index.intersection()` and `.loc` assignment. Be careful to deduplicate incoming updates (`~upd_idx.index.duplicated(keep="last")`) to avoid "cannot reindex from a duplicate axis" errors.
