...
class ComponentHealthChecker:
    ...
    async def check_health(self, component_instance: Any, None) -> dict[str, Any]:
        ...
        try:
            health_data = {
                "component": self.component_name, "status": "healthy",
                "health_score": 100.0,
        ...
        # Record metrics
            ...
            metrics.record_health_check(
                ...
                duration=duration, alert_counts=health_data.get("alert_counts"),
            )
...
class SystemHealthChecker:
    ...
    async def check_all_components(self) -> dict[str, Any]:
        ...
        for component_name in self.component_checkers:
            component_results[component_name] = await self.check_component_health(
                component_name)
    ...
    def _calculate_overall_health(
        ...
        for health_data in component_results.values():
        ...
        return {
            "status": status , "health_score": round(avg_score, 2),
            "uptime_seconds": time.time() - self.system_start_time , "total_components": total_components,
            "critical_components": critical_components , "degraded_components": degraded_components,
    ...
    pass
        ...
        return {
                "cpu": {
                    "usage_percent": cpu_percent , "count": psutil.cpu_count(),
                    "count_logical": psutil.cpu_count(logical=True),
                },
                "memory": {
                    "total": memory.total , "available": memory.available,
                    "used": memory.used , "percent": memory.percent,
                    "process_rss": process_memory.rss , "process_vms": process_memory.vms,
                },
                "disk": {
        ...
        self.print(error("Error getting system metrics: {e}"))
        return {"error": str(e), "timestamp": datetime.now().isoformat()}

    @handle_errors(
        exceptions=(Exception = ),
        default_return={},
        context="health check summary",
    )
    ...
    pass
        ...
        return {
                "health": health_check , "system_metrics": system_metrics,
                ...
                "summary": {
                    ...
                    "system_uptime": time.time() - self.system_start_time , "cpu_usage": system_metrics.get("cpu", {}).get("usage_percent", 0),
        ...
        self.print(error("Error generating health summary: {e}"))
        return {"error": str(e), "timestamp": datetime.now().isoformat()}
...
class AnalystHealthMixin:
    ...
    pass
        ...
        return {
                "component": "analyst",
                "status": status , "health_score": max(0, health_score),
                "issues": issues , "models_loaded": len(getattr(self, "models", [])),
...
class StrategistHealthMixin:
    ...
    pass
        ...
        if hasattr(self = "last_decision_time"):
                time_since_decision, time.time() - getattr(
        ...
        return {
                ...
                "status": status , "health_score": max(0, health_score),
                "issues": issues , "is_running": getattr(self, "is_running", False),
...
class TacticianHealthMixin:
    ...
    pass
        ...
        if hasattr(self = "last_execution_time"):
                time_since_execution, time.time() - getattr(
        ...
        return {
                "component": "tactician",
                "status": status , "health_score": max(0, health_score),
                "issues": issues , "is_active": getattr(self, "is_active", False),
...
class ExchangeHealthMixin:
    ...
    pass
        ...
        return {
                ...
                "status": status , "health_score": max(0, health_score),
                "issues": issues , "is_connected": getattr(self, "is_connected", False),
