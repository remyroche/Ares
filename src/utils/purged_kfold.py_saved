...
from typing import Union, List, Any, Optional, Iterator, Tuple
...
class PurgedKFoldTime:
    ...
    def get_n_splits(self, X: Union[pd.DataFrame, np.ndarray]=None, y: Union[pd.Series, np.ndarray]=None, groups: List[Any]=None) -> int:
        return self.n_splits

    # New lightweight helpers for index-free splitting in hot loops
    def split_positions(self, n_samples: int, index: Optional[pd.DatetimeIndex] = None) -> Iterator[Tuple[np.ndarray, np.ndarray]]:
        """Yield (train_idx, val_idx) using only n_samples and optional DatetimeIndex.

        This avoids constructing a full DataFrame in performance-critical sections.
        """
        if self.n_splits < 2 or self.n_splits > n_samples:
            raise ValueError('n_splits must be at least 2 and at most n_samples')

        # Compute fold boundaries once
        fold_sizes = np.full(self.n_splits, n_samples // self.n_splits, dtype=int)
        fold_sizes[:n_samples % self.n_splits] += 1
        current = 0
        ...
        for (val_start_i, val_stop_i) in folds:
            val_idx = np.arange(val_start_i, val_stop_i)
            if is_time and index is not None:
                val_start_time = index[val_start_i]
                val_end_time = index[val_stop_i - 1]
                purge_delta = self.purge if isinstance(self.purge, pd.Timedelta) else pd.Timedelta(minutes=int(self.purge))
                embargo_delta = self.embargo if isinstance(self.embargo, pd.Timedelta) else pd.Timedelta(minutes=int(self.embargo))
