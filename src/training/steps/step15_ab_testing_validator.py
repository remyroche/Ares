...
class Step15ABTestingValidator(BaseValidator):
    ...
    def _validate_ab_testing_files(
        ...
        try:
            ...
            if missing_files:
                self.print(missing("❌ Missing A/B testing files: {missing_files}"))
                return False
            ...
            self.print(error("❌ Error validating A/B testing files: {e}"))
            return False
        ...
        try:
            ...
            if os.path.exists(results_file):
                ...
                if "power" in results:
                    power = results["power"]
                    if power < 0.8:
                        self.print(error("⚠️ Low statistical power: {power:.3f}"))
    ...
    def _validate_ab_performance_comparison(
        ...
        try:
            ...
            if os.path.exists(performance_file):
                ...
                if "group_a_performance" in performance:
                    ...
                    if "accuracy" in group_a:
                        ...
                        if acc_a < 0.5:
                            self.print(error("⚠️ Low group A accuracy: {acc_a:.3f}"))
                ...
                if "group_b_performance" in performance:
                    ...
                    if "accuracy" in group_b:
                        ...
                        if acc_b < 0.5:
                            self.print(error("⚠️ Low group B accuracy: {acc_b:.3f}"))
            ...
            self.logger.exception(
                f"❌ Error during A/B testing performance comparison validation: {e}",
            )
            return False
