...
class AggtradesFormatValidator:
    ...
    def validate_file_format(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if list(df.columns) != self.EXPECTED_COLUMNS:
                result["issues"].append(f"Invalid columns: expected {self.EXPECTED_COLUMNS}, found {list(df.columns)}")
    ...
    def _validate_step3_compatibility(self, df: pd.DataFrame) -> List[str]:
        ...
        if "timestamp" in df.columns:
            ...
            if time_span < self.STEP3_REQUIREMENTS["required_time_span_days"]:
                issues.append(
                    f"step03: Insufficient time span ({time_span} days < {
                        self.STEP3_REQUIREMENTS['required_time_span_days']} days)"
                )
            ...
            if max_gap > self.STEP3_REQUIREMENTS["max_gap_seconds"]:
                issues.append(
                    f"step03: Large time gap detected ({
                        max_gap:.1f}s > {
                        self.STEP3_REQUIREMENTS['max_gap_seconds']}s)"
                )
    ...
    def _validate_step4_compatibility(self, df: pd.DataFrame) -> List[str]:
        ...
        if "timestamp" in df.columns:
            ...
            if time_span_hours < self.STEP4_REQUIREMENTS["min_labeling_period_hours"]:
                issues.append(
                    f"step04: Insufficient labeling period ({
                        time_span_hours:.1f}h < {
                        self.STEP4_REQUIREMENTS['min_labeling_period_hours']}h)"
                )

            if time_span_hours > self.STEP4_REQUIREMENTS["max_labeling_period_hours"]:
                issues.append(
                    f"step04: Excessive labeling period ({
                        time_span_hours:.1f}h > {
                        self.STEP4_REQUIREMENTS['max_labeling_period_hours']}h)"
                )
    ...
    def validate_all_aggtrades(self, symbol: str, exchange: str, auto_fix: bool = True) -> Dict[str, Any]:
