...
class MLRegimeTransitionDetector:
    ...
    def __init__(self, config: Dict[str, Any]=None) -> None:
        ...
        self.model_types = self.config.get('model_types', ['random_forest', 'gradient_boosting', 'logistic_regression', 'neural_network'])
    ...
    def train_transition_models(self, data: pd.DataFrame, regimes: np.ndarray) -> Dict[str, Any]:
        ...
        """
        training_results = {'models_trained': [], 'model_performance': {}, 'feature_importance': {}, 'training_summary': {}}
        ...
        for model_type in self.model_types:
            try:
                model_results = self._train_single_model(model_type, X_train, X_test, y_train, y_test)
    ...
    def _train_single_model(self, model_type: str, X_train: np.ndarray, X_test: np.ndarray, y_train: np.ndarray, y_test: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': True, 'model': model, 'scaler': scaler, 'performance': performance, 'feature_importance': feature_importance}
        except Exception as e:
            return {'success': False, 'error': str(e), 'model': None, 'scaler': None, 'performance': {}, 'feature_importance': {}}
    ...
    def _initialize_model(self, model_type: str) -> None:
        ...
        if model_type == 'random_forest':
            ...
            return GradientBoostingClassifier(n_estimators=100, max_depth=6, learning_rate=0.1, random_state=42)
    ...
    def _calculate_model_performance(self, y_true: np.ndarray, y_pred: np.ndarray, y_pred_proba: Optional[np.ndarray]=None) -> Dict[str, float]:
    ...
    def _calculate_feature_importance(self, model: Any, n_features: int) -> Dict[str, float]:
    ...
    def _generate_training_summary(self, training_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = {'models_trained': len(training_results['models_trained']), 'best_model': training_results['best_model'], 'best_performance': training_results['model_performance'].get(training_results['best_model'], {}), 'feature_importance_top_10': {}}
    ...
    def predict_transitions(self, data: pd.DataFrame, regimes: np.ndarray, model_type: Optional[str]=None) -> Dict[str, Any]:
        ...
        return {'transition_predictions': transition_predictions, 'transition_probabilities': transition_probabilities, 'model_used': model_type, 'confidence_scores': transition_probabilities if transition_probabilities is not None else transition_predictions}

    def save_models(self, filepath: str) -> bool:
        """Save trained models to file."""
        try:
            model_data = {'models': self.models, 'scalers': self.scalers, 'model_performance': self.model_performance, 'feature_importance': self.feature_importance, 'config': self.config}
...
if __name__ == '__main__':
    ...
    config = {'model_types': ['random_forest', 'gradient_boosting', 'logistic_regression'], 'transition_window': 5, 'prediction_horizon': 3}
