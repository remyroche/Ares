...
class CachedBacktester:
    ...
    def __init__(self, market_data: pd.DataFrame, config: ComputationalOptimizationConfig):
    ...
    def _run_simplified_backtest(self, params: Dict[str, Any]) -> float:
        ...
        sma_short_values = self.technical_indicators.get(f'sma_{sma_short}', 
                                                       self.technical_indicators['sma_20'])
        sma_long_values = self.technical_indicators.get(f'sma_{sma_long}', 
        ...
        for i in range(1, len(signals)):
            if (sma_short_values[i] > sma_long_values[i] and 
                sma_short_values[i-1] <= sma_long_values[i-1] and
                rsi_values[i] < rsi_threshold):
                signals[i] = 1  # Buy signal
            elif (sma_short_values[i] < sma_long_values[i] and 
                  sma_short_values[i-1] >= sma_long_values[i-1]):
...
class ProgressiveEvaluator:
    ...
    def __init__(self, full_data: pd.DataFrame, config: ComputationalOptimizationConfig):
    ...
    def _evaluate_subset(self, subset_data: pd.DataFrame, params: Dict[str, Any]) -> float:
        ...
        for i in range(1, len(signals)):
            if sma_short_values.iloc[i] > sma_long_values.iloc[i] and \
               sma_short_values.iloc[i-1] <= sma_long_values.iloc[i-1]:
                signals[i] = 1
            elif sma_short_values.iloc[i] < sma_long_values.iloc[i] and \
                 sma_short_values.iloc[i-1] >= sma_long_values.iloc[i-1]:
                signals[i] = -1
...
class IncrementalTrainer:
    ...
    def __init__(self, base_model_config: Dict[str, Any], config: ComputationalOptimizationConfig):
...
class ComputationalOptimizationManager:
    ...
    def get_optimization_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            'cache_hits': len(self.cached_backtester.cache) if self.cached_backtester else 0,
            'memory_usage': psutil.virtual_memory().percent,
            'surrogate_evaluations': len(self.surrogate_optimizer.expensive_evaluations) if self.surrogate_optimizer else 0,
        }
