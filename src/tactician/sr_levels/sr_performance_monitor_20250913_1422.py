...
class SRPerformanceMonitor:
    ...
    def _update_category_performance(self, prediction: SRPrediction, outcome: SROutcome) -> None:
    ...
    async def _check_performance_alerts(self) -> None:
        ...
        if len(recent_matches) < 50:
            return
        correct = sum((1 for pred, outcome in recent_matches if pred.predicted_outcome == outcome.actual_outcome))
    ...
    async def _send_performance_alert(self, accuracy: float) -> None:
        ...
        self.logger.warning(f'⚠️ S/R Performance Alert: Accuracy dropped to {accuracy:.1%} (threshold: {self.alert_threshold:.1%})')
        if self.alert_callback:
            try:
                await self.alert_callback({'type': 'sr_performance_degradation', 'accuracy': accuracy, 'threshold': self.alert_threshold, 'timestamp': datetime.now().isoformat()})
    ...
    def calculate_performance_metrics(self) -> PerformanceMetrics:
        ...
        correct = sum((1 for pred, outcome in matches if pred.predicted_outcome == outcome.actual_outcome))
        ...
        strong_accuracy = sum((1 for p, o in strong_matches if p.predicted_outcome == o.actual_outcome)) / len(strong_matches) if strong_matches else 0.0
        weak_accuracy = sum((1 for p, o in weak_matches if p.predicted_outcome == o.actual_outcome)) / len(weak_matches) if weak_matches else 0.0
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        report = {'timestamp': datetime.now().isoformat(), 'overall_metrics': asdict(metrics), 'sample_size': len(self.matched_results), 'prediction_count': len(self.predictions), 'outcome_count': len(self.outcomes), 'match_rate': len(self.matched_results) / len(self.predictions) if self.predictions else 0, 'accuracy_trend': self._calculate_accuracy_trend(), 'performance_by_hour': self._analyze_performance_by_hour(), 'confidence_calibration': self._analyze_confidence_calibration(), 'optimization_recommendations': self._generate_recommendations(metrics)}
        return report
    ...
    def _calculate_accuracy_trend(self) -> List[Dict[str, Any]]:
        ...
        for day in sorted(daily_performance.keys()):
            ...
            trend.append({'date': day.isoformat(), 'accuracy': stats['correct'] / stats['total'] if stats['total'] > 0 else 0, 'sample_size': stats['total']})
        return trend
    ...
    def _analyze_performance_by_hour(self) -> Dict[int, float]:
        ...
        return {hour: stats['correct'] / stats['total'] if stats['total'] > 0 else 0 for hour, stats in hourly_performance.items()}
    ...
    def _analyze_confidence_calibration(self) -> Dict[str, Any]:
        ...
        for bucket in sorted(confidence_buckets.keys()):
            ...
            if stats['total'] > 0:
                ...
                calibration[f'{bucket:.1f}'] = {'expected': bucket, 'actual': actual_accuracy, 'samples': stats['total']}
        calibration_errors = [abs(data['expected'] - data['actual']) for data in calibration.values()]
        avg_calibration_error = np.mean(calibration_errors) if calibration_errors else 0
        return {'buckets': calibration, 'average_calibration_error': avg_calibration_error, 'is_well_calibrated': avg_calibration_error < 0.1}

    def _generate_recommendations(self, metrics: PerformanceMetrics) -> List[str]:
        """Generate recommendations based on performance."""
        recommendations = []
        if metrics.overall_accuracy < 0.5:
            recommendations.append('Overall accuracy is below 50%. Consider re-optimizing S/R parameters.')
        if metrics.breakout_accuracy < 0.4:
            recommendations.append('Breakout predictions are underperforming. Consider adjusting momentum and volatility weights.')
        if metrics.rebounce_accuracy < 0.4:
            recommendations.append('Rebounce predictions are weak. Review bounce ratio thresholds and touch requirements.')
        if metrics.strong_level_accuracy < metrics.weak_level_accuracy:
            recommendations.append('Strong levels performing worse than weak levels. Strength calculation may need recalibration.')
        if metrics.sharpe_ratio < 0.5:
            recommendations.append('Low Sharpe ratio indicates poor risk-adjusted returns. Consider more selective entry criteria.')
        worst_method = None
        worst_accuracy = 1.0
        ...
        if worst_method and worst_accuracy < 0.4:
            recommendations.append(f"Method '{worst_method}' is underperforming ({worst_accuracy:.1%}). Consider adjusting its weight in the ensemble.")
        for regime, accuracy in metrics.accuracy_by_volatility.items():
            if accuracy < 0.45:
                recommendations.append(f'Poor performance in {regime} volatility ({accuracy:.1%}). Review context-aware adjustments for this regime.')
