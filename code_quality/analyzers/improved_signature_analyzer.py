...
class FunctionCall(NamedTuple):
    ...
    caller_type: str | None = None  # 'attribute', 'name', 'other'
...
class ImprovedSignatureAnalyzer:
    """Enhanced signature analyzer with better false positive filtering."""

    # Common built-in functions to exclude
    PYTHON_BUILTINS = set(dir(builtins))
    
    # Common method names that are likely object methods, not functions
    COMMON_METHODS = {
        # List methods
        'append', 'extend', 'insert', 'remove', 'pop', 'clear', 'index', 'count',
        'sort', 'reverse', 'copy',
        # Dict methods
        'get', 'set', 'items', 'keys', 'values', 'update', 'setdefault',
        'popitem', 'fromkeys',
        # String methods
        'split', 'join', 'strip', 'lstrip', 'rstrip', 'upper', 'lower',
        'capitalize', 'title', 'find', 'rfind', 'replace', 'startswith', 'endswith',
        'format', 'encode', 'decode', 'isdigit', 'isalpha', 'isalnum',
        # Set methods
        'add', 'discard', 'union', 'intersection', 'difference', 'symmetric_difference',
        # File/Path methods
        'read', 'write', 'close', 'open', 'exists', 'mkdir', 'rmdir', 'unlink',
        'is_file', 'is_dir', 'resolve', 'absolute',
        # DataFrame/Series methods (pandas)
        'loc', 'iloc', 'at', 'iat', 'head', 'tail', 'info', 'describe', 'mean',
        'sum', 'min', 'max', 'std', 'var', 'median', 'mode', 'rolling', 'groupby',
        'merge', 'concat', 'pivot', 'melt', 'dropna', 'fillna', 'interpolate',
        'resample', 'asfreq', 'shift', 'diff', 'pct_change', 'cumsum', 'cumprod',
        'to_csv', 'to_json', 'to_excel', 'to_sql', 'to_dict', 'to_list',
        # Numpy methods
        'reshape', 'flatten', 'ravel', 'transpose', 'swapaxes', 'squeeze',
        'astype', 'tolist', 'tobytes',
        # Datetime methods
        'now', 'today', 'utcnow', 'fromtimestamp', 'strftime', 'strptime',
        'isoformat', 'date', 'time', 'timestamp', 'weekday', 'isoweekday',
        # Logger methods
        'debug', 'info', 'warning', 'error', 'exception', 'critical', 'log',
        'getChild', 'addHandler', 'removeHandler', 'setLevel',
        # Other common methods
        'fit', 'predict', 'transform', 'fit_transform', 'score',  # sklearn
        'forward', 'backward', 'zero_grad', 'step',  # torch
        'compile', 'match', 'search', 'findall',  # re
        'dumps', 'loads', 'dump', 'load',  # json/pickle
        'create', 'save', 'delete', 'filter', 'all', 'first', 'last',  # ORM
    }
    
    # Common external library prefixes
    EXTERNAL_MODULES = {
        'np', 'pd', 'plt', 'torch', 'tf', 'cv2', 'sklearn', 'scipy',
        'requests', 'json', 'pickle', 'os', 'sys', 'time', 'datetime',
        'pathlib', 'logging', 'warnings', 're', 'math', 'random',
        'itertools', 'functools', 'collections', 'typing', 'dataclasses',
        'asyncio', 'aiohttp', 'flask', 'django', 'fastapi', 'sqlalchemy',
        'celery', 'redis', 'boto3', 'google', 'azure', 'yaml', 'toml',
    ...
    def __init__(self, config: CodeQualityConfig):
        ...
        self.imported_names = defaultdict(set)  # Track imports per file
        self.class_methods = defaultdict(set)  # Track class methods per file
        self.class_names = defaultdict(set)  # Track class names per file
    ...
    def _analyze_file_signatures(self, file_path: str) -> None:
        """Analyze function signatures in a single file with import tracking."""
        try:
            ...
            self._collect_class_methods(tree, file_path)
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    self.imported_names[file_path].add(name)
                    # Also add the base module name
                    self.imported_names[file_path].add(alias.name.split('.')[0])
            elif isinstance(node, ast.ImportFrom):
                if node.module:
                    # Add the module itself
                    self.imported_names[file_path].add(node.module.split('.')[0])
                for alias in node.names:
                    name = alias.asname if alias.asname else alias.name
                    if name != '*':  # Don't add wildcard imports
                        self.imported_names[file_path].add(name)

    def _collect_class_methods(self, tree: ast.AST, file_path: str) -> None:
        """Collect all methods defined in classes."""
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                for item in node.body:
                    if isinstance(item, ast.FunctionDef | ast.AsyncFunctionDef):
                        self.class_methods[file_path].add(item.name)

    def _walk_tree(self, tree: ast.AST, file_path: str, class_context: str | None = None) -> None:
        """Walk the AST tree to collect functions and calls."""
        # First, collect all class names in this file
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                self.class_names[file_path].add(node.name)
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                for item in node.body:
                    if isinstance(item, ast.FunctionDef | ast.AsyncFunctionDef):
                        signature = self._extract_function_signature(
                            item, file_path, is_method=True, class_name=node.name
                        )
                ...
                if call and self._should_analyze_call(call, file_path):
    ...
    def _should_analyze_call(self, call: FunctionCall, file_path: str) -> bool:
        """Determine if a function call should be analyzed."""
        # Skip built-in functions
        if call.function_name in self.PYTHON_BUILTINS:
            return False
        
        # Skip common method names when called as methods
        if call.is_method_call and call.function_name in self.COMMON_METHODS:
            return False
        
        # Skip imported names (external libraries)
        if call.function_name in self.imported_names[file_path]:
            return False
        
        # Skip if it starts with a known external module prefix
        if any(call.function_name.startswith(f"{mod}.") for mod in self.EXTERNAL_MODULES):
            return False
    ...
    def _detect_signature_changes(self) -> None:
        ...
        for functions in self.functions_by_file.values():
            for func in functions:
                # Skip methods for cross-file signature comparison
    ...
    def _detect_compatibility_issues(self) -> None:
        ...
        for functions in self.functions_by_file.values():
            for func in functions:
                # Store both regular functions and methods
                function_definitions[func.name] = func

        # Collect all class names across all files
        all_class_names = set()
        for class_names in self.class_names.values():
        ...
        for file_path, calls in self.function_calls_by_file.items():
            for call in calls:
                if call.function_name in function_definitions:
                    ...
                    # Only report as missing if it's not a method call, class instantiation, or imported function
                    if (not call.is_method_call and 
                        call.function_name not in self.class_methods[file_path] and
                        call.function_name not in all_class_names and
                        call.function_name not in self.imported_names[file_path]):
    ...
    def _check_call_compatibility(self, call: FunctionCall, func_def: FunctionSignature) -> list[str]:
        ...
        # Skip checking for methods with 'self' parameter
        args = func_def.args
        if func_def.is_method and args and args[0] == 'self':
            args = args[1:]  # Remove 'self' from consideration

        # Check positional arguments
        if len(call.args) > len(args):
            if not func_def.vararg:
                issues.append(f"Too many positional arguments: {len(call.args)} provided, {len(args)} expected")

        # Check keyword arguments
        defined_args = set(args)
        ...
        required_args = []
        defaults = func_def.defaults
        if func_def.is_method and defaults:
            defaults = defaults[1:]  # Adjust for 'self' parameter
            
        for i, (arg, default) in enumerate(zip(args, defaults or [None] * len(args), strict=False)):
            if default is None:
    ...
    def _detect_missing_functions(self) -> None:
        """Already handled in _detect_compatibility_issues."""
        pass
    ...
    def _is_special_function(self, func: FunctionSignature) -> bool:
        ...
        special_names = {"main", "__main__", "__init__", "__str__", "__repr__", 
                        "__enter__", "__exit__", "__call__", "__getattr__", "__setattr__",
                        "__getitem__", "__setitem__", "__len__", "__iter__", "__next__",
                        "setUp", "tearDown", "setUpClass", "tearDownClass"}
        special_prefixes = ["test_", "_test", "bench_", "benchmark_"]
        special_decorators = {"pytest", "test", "main", "cli", "route", "task", 
                            "api", "endpoint", "property", "staticmethod", "classmethod"}

        # Check function name
        if func.name in special_names:
            return True
            
        if any(func.name.startswith(prefix) for prefix in special_prefixes):
            return True

        # Check decorators
        for decorator in func.decorators:
            if any(special in str(decorator).lower() for special in special_decorators):
                return True

        # Methods are often called dynamically
        if func.is_method:
            return True
