...
class ProfitBasedFeatureEngineering:
    ...
    def apply_all_features(self, data: pd.DataFrame, feature_categories: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    def _apply_risk_reward_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data[f"{self.profit_column}_sharpe"] = np.where(rolling_std > 0, rolling_mean / rolling_std, 0.0)
        ...
        sortino_ratio = np.where(downside_std > 0, rolling_mean / downside_std, 0.0)
    ...
    def _apply_momentum_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data[f"{self.profit_column}_momentum_ratio"] = np.where(long_momentum != 0, short_momentum / long_momentum, 1.0)
    ...
    def _apply_volatility_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in [20, 50]:
            ...
            vol_ratio = np.where(historical_vol > 0, current_vol / historical_vol, 1.0)
        ...
        data[f"{self.profit_column}_volatility_surprise"] = np.where(
            expected_vol > 0, (realized_vol - expected_vol) / expected_vol, 0.0
        )
    ...
    def _apply_volume_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.volume_column in data.columns:
            ...
            volume_z_score = np.where(volume_std > 0, (volume - volume_mean) / volume_std, 0.0)
    ...
    def _apply_rolling_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for window in windows:
            if self.use_numba:
                ...
                data[f"{self.profit_column}_rolling_mean_{window}"] = series.rolling(
            ...
            # Rolling quantiles
            data[f"{self.profit_column}_rolling_q25_{window}"] = series.rolling(window=window, min_periods=1).quantile(
                0.25
            )
            data[f"{self.profit_column}_rolling_q75_{window}"] = series.rolling(window=window, min_periods=1).quantile(
                0.75
            )
    ...
    def select_features(
        ...
        if method == "correlation":
            ...
            try:
                ...
                if numerical_features:
                    mi_scores = mutual_info_regression(data[numerical_features].fillna(0), data[self.profit_column])
...
def benchmark_profit_feature_engineering(data: pd.DataFrame) -> Dict[str, float]:
    ...
    numba_features = len(
        [col for col in result_numba.columns if "potential_profit_pct" in col and col != "potential_profit_pct"]
    )
    ...
    python_features = len(
        [col for col in result_python.columns if "potential_profit_pct" in col and col != "potential_profit_pct"]
    )
