...
class EnsembleCreator:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.info("✅ Ensemble Creator initialization completed successfully")
            return True
    ...
    async def create_ensemble(
        ...
        try:
            ...
            final_ensemble = await self._apply_multi_stage_hpo(
                ensemble_result, ensemble_name
            )
            
            # Step 6: Evaluate and store ensemble
            evaluation_results = await self._evaluate_ensemble(final_ensemble, ensemble_name)
        ...
        try:
            ...
            for timeframe, features in ensemble_data["features"].items():
                if self.enhanced_coarse_optimizer:
                    ...
                    feature_importance = await self.enhanced_coarse_optimizer.analyze_feature_importance(
                        features, ensemble_data["targets"][timeframe]
                    )
                    
                    # Keep only important features
                    important_features = [
                        feature for feature, importance in feature_importance.items()
                        if importance > self.ensemble_config.feature_importance_threshold
                    ]
                    
                    pruned_data["features"][timeframe] = features[important_features]
                    pruned_features.extend(important_features)
                    
                    self.logger.info(f"Pruned {len(features.columns) - len(important_features)} features for {timeframe}")
            ...
            for timeframe, model in ensemble_data["models"].items():
                if timeframe in pruned_data["predictions"]:
                    ...
                    performance = self._calculate_model_performance(predictions, targets)
                    ...
                    if performance < self.ensemble_config.model_performance_threshold:
                        ...
                        self.logger.info(f"Removed underperforming model for {timeframe} (performance: {performance:.3f})")
            
            # Step 3: Correlation-based pruning
            if len(pruned_data["predictions"]) > 1:
                correlation_matrix = self._calculate_prediction_correlations(pruned_data["predictions"])
                pruned_data = self._remove_correlated_models(pruned_data, correlation_matrix)
                ...
                for j, model2 in enumerate(correlation_matrix.columns):
                    if i < j:  # Avoid duplicate pairs
                        ...
                        if correlation > self.ensemble_config.correlation_threshold:
                            ...
                            self.logger.info(f"Removing correlated model {model2} (correlation: {correlation:.3f})")
            ...
            for timeframe, model in ensemble_data["models"].items():
                if hasattr(model, 'set_params'):
                    ...
                    try:
                        ...
                        self.logger.warning(f"Could not apply regularization to {timeframe} model: {e}")
    ...
    async def _create_optimized_ensemble(
        ...
        try:
            self.logger.info(f"🎯 Creating optimized ensemble '{ensemble_name}' using coarse optimization")
            ...
            optimization_result = await self.enhanced_coarse_optimizer.optimize_ensemble(
                ensemble_creation_data
            )
    ...
    async def _apply_multi_stage_hpo(
        ...
        try:
            self.logger.info(f"🎯 Applying multi-stage HPO for ensemble '{ensemble_name}'")
            
            if not self.multi_stage_hpo:
                self.logger.warning("Multi-Stage HPO not available, skipping fine-tuning")
                return ensemble_result
            ...
            if hpo_result:
                ...
                ensemble_result["ensemble"] = hpo_result.get("optimized_ensemble", ensemble_result["ensemble"])
                ensemble_result["hpo_metrics"] = hpo_result.get("hpo_metrics", {})
                
                self.logger.info(f"✅ Multi-stage HPO completed for ensemble '{ensemble_name}'")
            else:
                self.logger.warning(f"Multi-stage HPO failed for ensemble '{ensemble_name}', using original ensemble")
