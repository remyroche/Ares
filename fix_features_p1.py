...
# In `compute_features_hourly`, build requested_feature_set if `requested_feature_keys` is not provided
# and conditionally compute features.
...
new_function = """def compute_features_hourly(panel, mkt_gates, cfg, requested_feature_keys=None):
    ...
    if requested_feature_keys is None:
        ...
        # Also need vp_ features if in requested feature keys, but if `requested_feature_keys` is None it means everything.
        # But we want to explicitly list all keys that should be built.
