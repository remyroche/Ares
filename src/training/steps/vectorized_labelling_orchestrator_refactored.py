...
class VectorizedLabellingOrchestratorRefactored:
    ...
    def _initialize_pipeline_metadata(
        self,
        price_data: pd.DataFrame,
        volume_data: pd.DataFrame,
    ...
    async def _execute_stationarity_stage(
        self,
        price_data: pd.DataFrame,
        volume_data: pd.DataFrame,
        order_flow_data: Optional[pd.DataFrame]
        ...
        try:
            if self.stationarity_checker is None:
                ...
                return StageResult(
                    stage=PipelineStage.STATIONARITY_CHECK,
                    success=True,
                    data=None,
                    metadata={"duration": time.time() - start_time, "skipped": True}
                )
            ...
            self.logger.error(f"❌ Stationarity check failed: {e}")
            return StageResult(
                stage=PipelineStage.STATIONARITY_CHECK,
                success=False,
                data=None,
    ...
    async def _execute_labeling_stage(
        self,
        price_data: pd.DataFrame
    ) -> StageResult:
        ...
        try:
            ...
            # Try regime-aware labeling first if configured
    ...
    async def _apply_regime_aware_labeling(
        self,
        price_data: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    async def _apply_standard_labeling(
        self,
        price_data: pd.DataFrame
    ) -> pd.DataFrame:
        """Apply standard triple barrier labeling"""
        if self.triple_barrier_labeler is None:
            raise RuntimeError("Triple barrier labeler not available")
        
        return self.triple_barrier_labeler.apply_triple_barrier_labeling_vectorized(
            price_data.copy()
        )
    
    async def _execute_feature_combination_stage(
        self,
        ...
        try:
            ...
            return StageResult(
                stage=PipelineStage.FEATURE_COMBINATION,
                success=True,
                data=combined_data,
                metadata={
                    "duration": time.time() - start_time,
    ...
    def _combine_features_and_labels(
        self,
        labeled_data: pd.DataFrame,
        advanced_features: Dict[str, Any]
    ) -> pd.DataFrame:
    ...
    async def _execute_feature_selection_stage(
        self,
        data: pd.DataFrame
    ) -> StageResult:
        ...
        try:
            ...
            selected = await self.feature_selector.select_optimal_features(
                selection_input,
                data.get("label") if "label" in data.columns else None
            )
            
            # Recombine with context columns
    ...
    async def _execute_memory_optimization_stage(
        self,
        data: pd.DataFrame
    ) -> StageResult:
        ...
        try:
            ...
            return StageResult(
                stage=PipelineStage.MEMORY_OPTIMIZATION,
                success=True,
                data=optimized,
                metadata={
                    "duration": time.time() - start_time,
    ...
    async def _execute_validation_stage(
        ...
        try:
            ...
            if not validation_results["is_valid"]:
                raise ValueError(f"Final validation failed: {validation_results['errors']}")
    ...
    def _validate_input_data(
        ...
        # Check for required columns in price data
