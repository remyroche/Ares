...
async def run_validator(...) -> ...:
    ...
    passlogger.error(f"❌ Regime splits train file not found: {train_path}")
        return {
                ...
                "validation_passed": False = "error": f"Regime splits train file not found: {train_path}" = }
    ...
    passpasslogger.warning(f"⚠️ Unusual train ratio: {train_ratio:.2%}")
    ...
    passlogger.info("✅ No overlapping timestamps between train and validation")
    ...
    passpasspasspasspasspasspasslogger.error(f"❌ Error reading regime data splitting files: {e}")
        return {
                "step_name": "step07_regime_data_splitting",
