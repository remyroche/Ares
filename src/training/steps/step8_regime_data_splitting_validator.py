...
async def run_validator(
    ...
    try:
        ...
        train_path = (
            Path(data_dir)
            / "training"
            / f"{exchange}_{symbol}_{timeframe}_regime_splits_train.parquet"
        )
        validation_path = (
            Path(data_dir)
            / "training"
            / f"{exchange}_{symbol}_{timeframe}_regime_splits_validation.parquet"
        )
        ...
        if not validation_path.exists():
            logger.error(
                f"❌ Regime splits validation file not found: {validation_path}"
            )
            return {
                "step_name": "step7_regime_data_splitting",
                "validation_passed": False,
        ...
        if validation_file_size == 0:
            logger.error(
                f"❌ Regime splits validation file is empty: {validation_path}"
            )
            return {
                "step_name": "step7_regime_data_splitting",
                "validation_passed": False,
...
from src.core.decorators import handles_errors
            ...
            missing_validation_columns = [
                col for col in required_columns if col not in validation_data.columns
            ]

            if missing_train_columns:
                logger.error(
                    f"❌ Missing required columns in train data: {missing_train_columns}"
                )
                return {
                    "step_name": "step7_regime_data_splitting",
                    "validation_passed": False,
            ...
            if missing_validation_columns:
                logger.error(
                    f"❌ Missing required columns in validation data: {missing_validation_columns}"
                )
                return {
                    "step_name": "step7_regime_data_splitting",
                    "validation_passed": False,
            ...
            logger.info(
                f"✅ Validation label distribution: {validation_label_counts.to_dict()}"
            )

            # Check regime distribution
            train_regime_counts = train_data["composite_cluster_id"].value_counts()
            validation_regime_counts = validation_data[
                "composite_cluster_id"
            ].value_counts()

            logger.info(
                f"✅ Train regime distribution: {train_regime_counts.to_dict()}"
            )
            logger.info(
                f"✅ Validation regime distribution: {validation_regime_counts.to_dict()}"
            )
            ...
            if (
                "timestamp" in train_data.columns
                ...
                if overlap:
                    logger.warning(
                        f"⚠️ Found {len(overlap)} overlapping timestamps between train and validation"
                    )
                else:
                    logger.info(
                        "✅ No overlapping timestamps between train and validation"
                    )
