...
class SidewaysRangeEnsemble(BaseEnsemble):
    ...
    def _train_base_models(self, aligned_data: pd.DataFrame, y_encoded: np.ndarray):
        ...
        try:
            ...
            self.models["clustering"] = KMeans(n_clusters=self.model_config["kmeans_clusters"], random_state=42, n_init=10).fit(cluster_features)
        except Exception as e: self.logger.error(f"KMeans training failed: {e}")
        ...
        svm_params = self._tune_hyperparameters(SVC, self._get_svm_search_space, X_sample, y_sample)
        self.models["svm"] = self._train_with_smote(SVC(**svm_params, random_state=42), X_flat, y_encoded)
    ...
    def _get_meta_features(self, df: pd.DataFrame, is_live: bool = False, **kwargs) -> pd.DataFrame or dict:
        ...
        if is_live:
            ...
            if self.models.get("clustering"): meta['price_cluster'] = self.models["clustering"].predict(current_row_flat[['close', 'volume', 'ATR']])[0]
            if self.models.get("order_flow_lgbm"): meta['of_lgbm_prob'] = np.max(self.models["order_flow_lgbm"].predict_proba(current_row_of))
            if self.models.get("svm"): meta['svm_prob'] = np.max(self.models["svm"].predict_proba(current_row_flat))
            meta.update(current_row_flat.iloc[0].to_dict())
            return meta
        else:
            meta_df = pd.DataFrame(index=df.index)
            if self.models.get("clustering"): meta_df['price_cluster'] = self.models["clustering"].predict(X_flat[['close', 'volume', 'ATR']])
            if self.models.get("order_flow_lgbm"): meta_df['of_lgbm_prob'] = np.max(self.models["order_flow_lgbm"].predict_proba(X_of), axis=1)
            if self.models.get("svm"): meta_df['svm_prob'] = np.max(self.models["svm"].predict_proba(X_flat), axis=1)
