...
def _worker(args: argparse.Namespace) -> None:
    ...
    spectral_parents = (
        _frozen_spectral_parent_keys(args.spectral_contract)
        if spectral_requested else []
    )
...
def main() -> None:
    ...
    failing = audit.loc[
        audit["missing_mismatch_rows"].gt(0)
        | audit["rows_over_0p01_percent"].gt(0)
    ]
    manifest = {
        "schema": "strict_r3_partitioned_feature_state_parity_v1",
