...
class ModelProbabilityGenerator:
    ...
    def generate_price_action_probabilities(self, model: Any, X_test: np.ndarray, y_test: np.ndarray, market_data: pd.DataFrame, model_type: str='classification', **kwargs) -> dict[str, float]:
        """
        ...
        try:
            ...
            probabilities = {'triple_barrier_probability': self._calculate_triple_barrier_probability(calculator, model, X_test, market_data, **kwargs), 'direction_probability': self._calculate_direction_probability(calculator, model, X_test, y_test, **kwargs), 'magnitude_probability': self._calculate_magnitude_probability(calculator, model, X_test, market_data, **kwargs), 'barrier_avoidance_probability': self._calculate_barrier_avoidance_probability(calculator, model, X_test, market_data, **kwargs)}
            ...
            self.logger.exception(f'Error generating probability outputs: {e}')
            return self._get_default_probabilities(model_type)

    def _calculate_triple_barrier_probability(self, calculator: ClassificationProbabilityCalculator | RegressionProbabilityCalculator, model: Any, X_test: np.ndarray, market_data: pd.DataFrame, **kwargs) -> float:
        ...
        try:
            ...
            if isinstance(calculator, ClassificationProbabilityCalculator):
                return calculator.calculate_triple_barrier_probability(model, X_test, market_data, profit_target, stop_loss, volatility_window)
            return calculator.calculate_triple_barrier_probability(model, X_test, market_data, profit_target, stop_loss)
        except Exception as e:
            self.logger.exception(f'Error calculating triple barrier probability: {e}')
            return 0.5

    def _calculate_direction_probability(self, calculator: ClassificationProbabilityCalculator | RegressionProbabilityCalculator, model: Any, X_test: np.ndarray, y_test: np.ndarray, **kwargs) -> float:
    ...
    def _calculate_magnitude_probability(self, calculator: ClassificationProbabilityCalculator | RegressionProbabilityCalculator, model: Any, X_test: np.ndarray, market_data: pd.DataFrame, **kwargs) -> float:
        """Calculate magnitude probability."""
        try:
            threshold_factor = kwargs.get('threshold_factor', 0.8)
            return calculator.calculate_magnitude_probability(model, X_test, market_data, threshold_factor)
        except Exception as e:
            self.logger.exception(f'Error calculating magnitude probability: {e}')
            return 0.5

    def _calculate_barrier_avoidance_probability(self, calculator: ClassificationProbabilityCalculator | RegressionProbabilityCalculator, model: Any, X_test: np.ndarray, market_data: pd.DataFrame, **kwargs) -> float:
        """Calculate barrier avoidance probability."""
        try:
            adverse_threshold = kwargs.get('adverse_threshold', 0.01)
            return calculator.calculate_barrier_avoidance_probability(model, X_test, market_data, adverse_threshold)
        except Exception as e:
    ...
    def _get_default_probabilities(self, model_type: str) -> dict[str, float]:
        ...
        return {'triple_barrier_probability': 0.5, 'direction_probability': 0.5, 'magnitude_probability': 0.5, 'barrier_avoidance_probability': 0.5, 'generation_timestamp': datetime.now().isoformat(), 'model_type': model_type, 'note': 'Default probabilities due to calculation error'}
    ...
    def validate_probabilities(self, probabilities: dict[str, float]) -> bool:
        ...
        """
        required_keys = ['triple_barrier_probability', 'direction_probability', 'magnitude_probability', 'barrier_avoidance_probability']
    ...
    def generate_ensemble_probabilities(self, models: list, model_types: list, X_test: np.ndarray, y_test: np.ndarray, market_data: pd.DataFrame, weights: list | None=None, **kwargs) -> dict[str, float]:
        ...
        try:
            if len(models) != len(model_types):
                msg = 'Number of models must match number of model types'
            ...
            self.logger.exception(f'Error generating ensemble probabilities: {e}')
            return self._get_default_probabilities('ensemble')

    def generate_calibrated_probabilities(self, model: Any, X_test: np.ndarray, y_test: np.ndarray, market_data: pd.DataFrame, model_type: str='classification', calibration_method: str='isotonic', **kwargs) -> dict[str, float]:
        """
        ...
        """
        try:
            probabilities = self.generate_price_action_probabilities(model, X_test, y_test, market_data, model_type, **kwargs)
...
def generate_model_probabilities(model: Any, X_test: np.ndarray, y_test: np.ndarray, market_data: pd.DataFrame, model_type: str='classification', **kwargs) -> dict[str, float]:
    """
    ...
    return generator.generate_price_action_probabilities(model, X_test, y_test, market_data, model_type, **kwargs)