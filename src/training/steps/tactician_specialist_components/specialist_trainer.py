...
class SpecialistTrainer:
    ...
    @handles_errors(
        ...
        default_return={},
        context="tactic model training"
    )
    async def train_tactic_models(
        self,
        tactic_name: str,
    ...
    async def _train_single_model(
        ...
        if model_type == "lightgbm" and LIGHTGBM_AVAILABLE:
            return await self._train_lightgbm(
                X_train, y_train, X_val, y_val, tactic_name
            )
        elif model_type == "xgboost" and XGBOOST_AVAILABLE:
            return await self._train_xgboost(
                X_train, y_train, X_val, y_val, tactic_name
            )
        elif model_type == "random_forest":
            return await self._train_random_forest(
                X_train, y_train, X_val, y_val, tactic_name
            )
    ...
    @handles_errors(
        exceptions=(Exception,),
        default_return=None,
        context="tactic ensemble creation"
    )
    async def create_tactic_ensemble(
        self,
        tactic_name: str,
        ...
        try:
            ...
            self.logger.info(
                f"Created ensemble for {tactic_name} with {len(models)} models"
            )
