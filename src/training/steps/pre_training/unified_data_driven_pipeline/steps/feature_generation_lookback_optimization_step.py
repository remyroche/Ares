...
class FeatureGenerationLookbackOptimizationStep(BasePreTrainingComponent):
    ...
    async def execute(self,
        ...
        try:
            ...
            if component_result.success:
                self.logger.info(f"✅ Lookback optimization completed successfully with {component_result.metadata.get('optimized_lookbacks', 0)} optimized lookbacks")
            else:
                self.logger.error(f"❌ Lookback optimization failed: {component_result.error_message}")
