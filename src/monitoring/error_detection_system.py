...
class ErrorDetectionSystem:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.enable_anomaly_detection = self.detection_config.get("enable_anomaly_detection", True)
        self.enable_predictive_alerts = self.detection_config.get("enable_predictive_alerts", True)
        self.enable_email_alerts = self.detection_config.get("enable_email_alerts", False)
        self.enable_slack_alerts = self.detection_config.get("enable_slack_alerts", False)
        
        # Detection parameters
        self.monitoring_interval = self.detection_config.get("monitoring_interval", 30)  # seconds
        self.anomaly_sensitivity = self.detection_config.get("anomaly_sensitivity", 0.95)  # percentile
        self.min_data_points = self.detection_config.get("min_data_points", 100)
        self.lookback_window_hours = self.detection_config.get("lookback_window_hours", 24)
        ...
        self.metric_baselines: dict[str, dict[str, float]] = {}  # metric -> {mean, std, percentiles}
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"❌ Error Detection System initialization failed: {e}")
            return False
    ...
    async def _initialize_storage(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Error detection storage backend '{storage_backend}' initialized")
    ...
    async def _create_error_detection_tables(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to create error detection tables: {e}")
            raise
        ...
        try:
            ...
            for rule_name, rule_config in default_rules.items():
                rule = AlertRule(
                    ...
                    evaluation_window_minutes=rule_config.get("evaluation_window_minutes", 5),
    ...
    async def _add_default_critical_rules(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to add default critical rules: {e}")
    ...
    async def _load_historical_metrics(self) -> None:
        ...
        try:
            if hasattr(self, 'storage_manager'):
                cutoff_date = datetime.now() - timedelta(hours=self.lookback_window_hours)
                ...
                results = await self.storage_manager.execute_query(query, (cutoff_date,))
    ...
    async def _calculate_metric_baselines(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Calculated baselines for {len(self.metric_baselines)} metrics")
            
        except Exception as e:
            self.logger.error(f"Failed to calculate metric baselines: {e}")
        ...
        try:
            ...
            if self.enable_slack_alerts:
                ...
                if not self.slack_webhook:
                    ...
                    self.logger.warning("Slack alerts disabled due to missing webhook URL")
    ...
    async def record_error_event(
        ...
        **kwargs
        ...
        try:
            ...
            impact_score = await self._calculate_error_impact(severity, category, component)
            ...
            self.logger.warning(f"Recorded {severity.value} error: {error_message} (ID: {error_id})")
    ...
    async def _calculate_error_impact(
        ...
        try:
            ...
            self.logger.error(f"Failed to calculate error impact: {e}")
            return 0.5
    ...
    async def _check_error_alerts(self, error_event: ErrorEvent) -> None:
        ...
        try:
            ...
            if error_event.severity in [AlertSeverity.CRITICAL, AlertSeverity.EMERGENCY]:
            ...
            self.logger.error(f"Failed to check error alerts: {e}")
...
"""
    ...
    async def _check_error_rate_spike(self, error_event: ErrorEvent) -> None:
        ...
        try:
            ...
            recent_errors = [
                err for err in self.error_events.values()
                if err.timestamp >= recent_cutoff and err.category == error_event.category
            ]
            
            if len(recent_errors) >= 5:  # 5 errors in 10 minutes
                await self._trigger_error_rate_alert(error_event.category, len(recent_errors))
            
        except Exception as e:
            self.logger.error(f"Failed to check error rate spike: {e}")

    async def _trigger_error_rate_alert(self, category: ErrorCategory, error_count: int) -> None:
...
"""
        ...
        **kwargs
        ...
        try:
            ...
            if std == 0:
                ...
                deviation_score = abs(current_value - mean) / std
            ...
            if deviation_score >= 2.0:  # 2 standard deviations
                ...
                anomaly_type = await self._classify_anomaly_type(metric_name, current_value, baseline)
                
                # Calculate percentile rank
                percentile_rank = stats.percentileofscore(
                    [baseline["min"], baseline["max"]], current_value
                ) / 100.0
                ...
                anomaly = AnomalyDetection(
                    ...
                    potential_causes=await self._identify_potential_causes(metric_name, current_value, baseline)
                )
                ...
                self.logger.warning(f"Anomaly detected in {metric_name}: {current_value} (expected: {mean:.2f})")
    ...
    async def _classify_anomaly_type(
        ...
        try:
            ...
            self.logger.error(f"Failed to classify anomaly type: {e}")
            return AnomalyType.PERFORMANCE_DEGRADATION
    ...
    async def _identify_potential_causes(
        ...
        try:
            ...
            self.logger.error(f"Failed to identify potential causes: {e}")
    ...
    async def _store_anomaly_detection(self, anomaly: AnomalyDetection) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to store anomaly detection: {e}")
            raise
    ...
    async def _check_anomaly_alerts(self, anomaly: AnomalyDetection) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to check anomaly alerts: {e}")
...
"""
    ...
    async def _monitor_system_health(self) -> None:
        ...
        try:
            while True:
                ...
                cutoff_time = datetime.now() - timedelta(hours=self.lookback_window_hours)
                ...
                # Check for anomalies
                if self.enable_anomaly_detection:
                    await self.detect_anomaly("cpu_usage_percent", metrics.cpu_usage_percent)
                    await self.detect_anomaly("memory_usage_percent", metrics.memory_usage_percent)
                    await self.detect_anomaly("error_rate_percent", metrics.error_rate_percent)
                    await self.detect_anomaly("response_time_ms", metrics.response_time_ms)
    ...
    async def _update_metric_baselines(self) -> None:
        ...
        try:
            ...
            if self.health_metrics_history:
                metrics_data = {
                    "cpu_usage_percent": [m.cpu_usage_percent for m in self.health_metrics_history],
                    "memory_usage_percent": [m.memory_usage_percent for m in self.health_metrics_history],
                    "error_rate_percent": [m.error_rate_percent for m in self.health_metrics_history],
                    "response_time_ms": [m.response_time_ms for m in self.health_metrics_history]
                }
            ...
            self.logger.error(f"Failed to update metric baselines: {e}")
    ...
    async def _evaluate_alert_rules(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Error in alert rule evaluation: {e}")

    async def _evaluate_rule(self, rule: AlertRule, metrics: SystemHealthMetrics) -> None:
    ...
    async def _handle_rule_trigger(self, rule: AlertRule, metric_value: float) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to handle rule trigger: {e}")
...
"""
            ...
            if rule.notify_email and self.enable_email_alerts:
                await self._send_email_alert(f"Alert: {rule.rule_name}", alert_message, rule.email_recipients)
            ...
            self.logger.error(f"Failed to send rule alert: {e}")

    async def _send_email_alert(self, subject: str, message: str, recipients: list[str] = None) -> None:
        """Send email alert."""
        try:
    ...
    async def get_detection_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            stats.update({
                "active_error_events": len([e for e in self.error_events.values() if not e.is_resolved]),
                "total_error_events": len(self.error_events),
                "active_anomalies": len([a for a in self.anomaly_detections.values() if not a.auto_resolved]),
                "total_anomalies": len(self.anomaly_detections),
                "active_alert_rules": len([r for r in self.alert_rules.values() if r.is_enabled]),
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to cleanup Error Detection System: {e}")


# Setup function for integration
async def setup_error_detection_system(config: dict[str, Any]) -> ErrorDetectionSystem | None:
    """
    ...
    try:
        ...
        system_logger.error(f"Failed to setup Error Detection System: {e}")
        return None