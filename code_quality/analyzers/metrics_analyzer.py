...
class MetricsVisitor(ast.NodeVisitor):
    ...
    def _calculate_halstead_metrics(self, node: ast.AST) -> dict[str, float]:
        ...
        for child in ast.walk(node):
            # Count operators
            if isinstance(child, ast.Add | ast.Sub | ast.Mult | ast.Div | ast.Mod | ast.Pow | ast.LShift | ast.RShift | ast.BitOr | ast.BitXor | ast.BitAnd | ast.FloorDiv | ast.And | ast.Or | ast.Not | (ast.Eq | ast.NotEq | ast.Lt | ast.LtE | ast.Gt | ast.GtE | ast.Is | ast.IsNot | ast.In | ast.NotIn)):
