...
await self.load_levels()
    ...
    return True
            ...
            self.logger.error(f"❌ Failed to initialize SR Levels Manager: {e}")
    return False
...
support_levels.append(level)
    ...
    self.logger.info(f"✅ Detected {len(support_levels)} support levels using {method} method")
                    ...
                    self.logger.error(f"❌ Error detecting support levels with {method} method: {e}")
...
resistance_levels.append(level)
    ...
    self.logger.info(f"✅ Detected {len(resistance_levels)} resistance levels using {method} method")
                    ...
                    self.logger.error(f"❌ Error detecting resistance levels with {method} method: {e}")
...
"method_used": method
            ...
            self.logger.error(f"❌ Error calculating SR levels with {method} method: {e}")
# Restore original method on error
    self.sr_predictor.sr_detection_method = original_method
...
"strength": nearest_support.strength if nearest_support else None,
"proximity": support_proximity,
"touch_count": nearest_support.touch_count if nearest_support else None,
"age_hours": nearest_support.age_hours if nearest_support else None,
"bounce_rate": nearest_support.bounce_rate if nearest_support else None,
"quality_score": nearest_support.calculate_quality_score() if nearest_support else None
} if nearest_support else None,
"nearest_resistance": {
"price": nearest_resistance.price if nearest_resistance else None,
"strength": nearest_resistance.strength if nearest_resistance else None,
"proximity": resistance_proximity,
"touch_count": nearest_resistance.touch_count if nearest_resistance else None,
"age_hours": nearest_resistance.age_hours if nearest_resistance else None,
"bounce_rate": nearest_resistance.bounce_rate if nearest_resistance else None,
"quality_score": nearest_resistance.calculate_quality_score() if nearest_resistance else None
} if nearest_resistance else None,
"all_levels": {
"support_count": len(self.support_levels),
"resistance_count": len(self.resistance_levels),
"total_count": len(self.support_levels) + len(self.resistance_levels)
}
}
