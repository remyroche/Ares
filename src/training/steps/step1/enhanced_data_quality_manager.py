...
class EnhancedDataQualityManager:
            ...
            for gap in gaps:
                try:
                    if gap["type"] == "aggtrades":
                        ...
                        # Fill klines gap
                        success = await self.gap_filler.fill_klines_gap(symbol, exchange, timeframe, gap["date"])
                        if success:
                            filled_gaps.append(gap)

                    elif gap["type"] == "futures":
                        # Fill futures gap
                        success = await self.gap_filler.fill_futures_gap(symbol, exchange, gap["date"])
                        if success:
                            filled_gaps.append(gap)
        ...
        try:
            ...
            for file_path in klines_files:
                try:
                    ...
                    metrics[f"klines_{file_path.name}"] = {"valid": False, "error": str(e)}
    ...
    async def get_data_for_step3_step4(
        self, symbol: str, exchange: str, timeframe: str = "1m", force_refresh: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "klines_path": str(
                    self.data_cache_path / f"klines_{exchange}_{symbol}_{timeframe}_consolidated.parquet"
                ),
...
async def ensure_data_quality(
    symbol: str, exchange: str, timeframe: str = "1m", data_cache_path: str = "data_cache"
) -> Dict[str, Any]:
