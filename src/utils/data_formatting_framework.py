...
class DataFormattingFramework:
    ...
    def _log_formatting_operation(self, original_data: pd.DataFrame, formatted_data: pd.DataFrame, target_format: DataFormat) -> None:
        ...
        operation = {'timestamp': datetime.now().isoformat(), 'target_format': target_format.value, 'original_shape': original_data.shape, 'formatted_shape': formatted_data.shape, 'columns_changed': list(set(original_data.columns) - set(formatted_data.columns) | set(formatted_data.columns) - set(original_data.columns))}
    ...
    def normalize_timestamps(self, data: pd.DataFrame, timestamp_column: str='timestamp', target_format: str='unix_seconds') -> pd.DataFrame:
    ...
    def get_formatting_report(self, data: pd.DataFrame, target_format: DataFormat) -> Dict[str, Any]:
        ...
        Returns:
            Formatting report
        """
        report = {'timestamp': datetime.now().isoformat(), 'data_shape': data.shape, 'target_format': target_format.value, 'current_validation': self.validate_data_format(data, target_format), 'formatting_operations': self.format_history[-10:] if self.format_history else []}
        if target_format in self.standard_formats:
            format_spec = self.standard_formats[target_format]
            report['format_comparison'] = {'required_columns': format_spec['required_columns'], 'current_columns': list(data.columns), 'missing_columns': list(set(format_spec['required_columns']) - set(data.columns)), 'extra_columns': list(set(data.columns) - set(format_spec['required_columns']))}
        return report
