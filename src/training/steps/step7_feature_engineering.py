# src/training/steps/step7_feature_engineering.py
...
@handle_errors(
    ...
    context="step7_feature_engineering",
)
async def run_step(
    symbol: str,
    exchange: str,
    ...
    **kwargs: Any,
    ...
    """Step 7: Advanced Feature Engineering (AFTER regimes are known).
    
    This step creates advanced features after HMM regime discovery, allowing for
    regime-specific feature engineering and optimization.
    ...
    """
    logger = system_logger.getChild("Step7FeatureEngineering")
    
    logger.info("=" * 80)
    logger.info("🚀 STEP 7: Advanced Feature Engineering (After Regime Discovery)")
    logger.info("=" * 80)
    ...
    try:
        # Import the feature engineering logic from step6
        from .step6_feature_engineering import run_step as step6_feature_engineering
        
        # Execute the feature engineering with regime-aware context
        result = await step6_feature_engineering(
            symbol=symbol,
            exchange=exchange,
            timeframe=timeframe,
            data_dir=data_dir,
            force_rerun=force_rerun,
            regime_aware=True,  # New parameter to indicate regime-aware processing
            **kwargs
        )
        
        if result:
            logger.info("✅ Step 7: Advanced Feature Engineering completed successfully")
            logger.info("📊 Features created with regime-aware optimization")
            return True
        else:
            logger.error("❌ Step 7: Advanced Feature Engineering failed")
            return False
            
    except Exception as e:
        logger.exception(f"❌ Unexpected error in Step 7: {e}")
        return False


# Import the rest of the step6_feature_engineering content for reuse
from .step6_feature_engineering import *