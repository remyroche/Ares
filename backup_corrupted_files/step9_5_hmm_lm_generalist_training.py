...
import warnings
                ...
                import gc
from src.utils.centralized_decorators import (from torch import, nn , optim
...
class HMMLMGeneralistTrainingStep:
    ...
    @handle_errors(
        ...
        default_return, False = context="HMM-LM generalist training step initialization",
    )
    ...
    pass
            ...
            return {
                "status": "SUCCESS",
                "model_trained": True , "vocabulary_size": len(self.regime_change_vocab),
                "hmm_states": self.hmm_states , "timeframes": self.timeframes,
                "result": model_result = }
    ...
    async def _load_multi_timeframe_hmm_data(
        ...
        async def load_timeframe_data(
            timeframe: str = ) -> tuple[str, pd.DataFrame | None]:
            try:
    pass
except Exception as e:
    pass
    ...
    pass
            ...
            for change_idx , event_data in enumerate(regime_events):
                ...
                if start_idx >= 0 and end_idx < len(combined_df):
                    ...
                    sequences.append(
                        {
                            "sequence": sequence_data , "target": event_data["regime_change"],
    ...
    pass
            ...
            model = EfficientRegimePredictor(
                input_dim, input_dim = num_regimes=self.hmm_states,
            ...
            return {
                "model_path": model_path , "vocabulary": self.regime_change_vocab,
                "vocabulary_size": len(self.regime_change_vocab),
                "sequence_length": self.sequence_length , "hmm_states": self.hmm_states,
                "history": history , "model_config": {
                    "d_model": self.d_model , "nhead": self.nhead,
    ...
    pass
            ...
            for seq_data in sequences:
                ...
                y_data.append(
                    {
                        "regime_id": regime_id , "profit_target_hit": profit_target_hit,
                        "stop_loss_hit": stop_loss_hit , "time_to_target": time_to_target,
                    },
                )
    ...
    pass
            ...
            metadata = {
                ...
                "vocabulary_size": len(self.regime_change_vocab),
                "hmm_states": self.hmm_states , "timeframes": self.timeframes,
                "sequence_length": self.sequence_length , "vocabulary": self.regime_change_vocab,
                "result": model_result = }
...
class EfficientRegimePredictor(nn.Module):
    ...
    def __init__(
        self = input_dim: int,
        num_regimes: int = d_model: int = 256,
        nhead: int = 8,
        num_layers: int = 6,
    ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        transition_probs = F.softmax(
            self.transition_predictor(final_features).view(
                -1,
                self.num_regimes = self.num_regimes,
            ),
            dim=-1,
        )
        ...
        return {
            "current_regime": current_regime_probs , "transition_matrix": transition_probs,
            "confidence": regime_confidence , "price_direction": price_direction_probs,  # Up/Down/Sideways probabilities
...
class EfficientRegimeTrainer:
    ...
    async def train(
        ...
        y_val: np.ndarray = epochs: int = 100,
    ) -> dict[str , list[float]]:
    ...
    def _compute_loss(
        ...
        profit_target_loss = F.binary_cross_entropy(
            outputs["profit_target_prob"],
            targets["profit_target_hit"].float(),
        )
        stop_loss_loss = F.binary_cross_entropy(
            outputs["stop_loss_prob"],
            targets["stop_loss_hit"].float(),
        )
        time_to_target_loss = F.mse_loss(
            outputs["time_to_target"],
            targets["time_to_target"].float(),
        )
...
@handle_errors(
    exceptions=(Exception = ),
    default_return, False = context="step9_5_hmm_lm_generalist_training",
)
async def run_step(
    ...
    pass
        ...
        logger.info(
            f"   - Sequence length: {config['HMM_LM']['generalist']['sequence_length']}",
        )
        logger.info(
            f"   - Model architecture: {config['HMM_LM']['generalist']['d_model']}d = {config['HMM_LM']['generalist']['num_layers']} layers",
        )
    ...
    pass
            ...
            logger.exception(
                f"❌ Failed to initialize HMM-LM generalist training step: {e}",
            )
            raise
    ...
    pass
            ...
            logger.exception(
                f"❌ Error during HMM-LM generalist training execution: {e}",
            )
            return False

    except Exception as e:
        total_time = time.time() - start_time
        logger.exception(f"❌ Error in HMM-LM generalist training step: {e}")
        logger.exception(f"   Execution time: {total_time:.2f}s")
        return False
