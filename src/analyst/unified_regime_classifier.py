...
class UnifiedRegimeClassifier:
    ...
    def _analyze_sr_levels(self, price_data: pd.DataFrame) -> dict[str, Any]:
        """
        Analyze support and resistance levels from price data.
        ...
        try:
            ...
            # Find pivot points using scipy.signal.find_peaks
            ...
            high_peaks, _ = find_peaks(
                highs,
                height=np.percentile(highs, 75),
                distance=20,
            )
            resistance_prices = highs[high_peaks]

            # Find troughs (support levels)
            low_peaks, _ = find_peaks(
                -lows,
                height=-np.percentile(lows, 25),
                distance=20,
            )
            support_prices = lows[low_peaks]

            # Cluster nearby levels using DBSCAN
            if len(resistance_prices) > 0:
                resistance_clusters = DBSCAN(eps=0.01, min_samples=2).fit(
                    resistance_prices.reshape(-1, 1),
                )
                unique_clusters = set(resistance_clusters.labels_)

                for cluster_id in unique_clusters:
                    if cluster_id != -1:  # Skip noise points
                        cluster_prices = resistance_prices[
                            resistance_clusters.labels_ == cluster_id
                        ]
                        avg_price = np.mean(cluster_prices)
                        strength = len(cluster_prices)  # More touches = stronger level

                        sr_levels["resistance_levels"].append(
                            {
                                "price": float(avg_price),
                                "strength": int(strength),
                                "touches": int(strength),
                                "type": "resistance",
                            },
                        )

            if len(support_prices) > 0:
                support_clusters = DBSCAN(eps=0.01, min_samples=2).fit(
                    support_prices.reshape(-1, 1),
                )
                unique_clusters = set(support_clusters.labels_)

                for cluster_id in unique_clusters:
                    if cluster_id != -1:  # Skip noise points
                        cluster_prices = support_prices[
                            support_clusters.labels_ == cluster_id
                        ]
                        avg_price = np.mean(cluster_prices)
                        strength = len(cluster_prices)  # More touches = stronger level
            ...
            # Calculate level strengths and last tested times
            for level in sr_levels["resistance_levels"] + sr_levels["support_levels"]:
                ...
                distance_factor = 1.0 / (
                    1.0 + abs(current_price - level_price) / current_price
                )
                level["strength"] = int(level["strength"] * distance_factor)

                # Find last time level was tested
                if level_type == "resistance":
                    touches = price_data[price_data["high"] >= level_price * 0.995]
                else:
                    touches = price_data[price_data["low"] <= level_price * 1.005]

                if len(touches) > 0:
                    level["last_tested"] = (
                        touches.index[-1].isoformat()
                        if hasattr(touches.index[-1], "isoformat")
                        else str(touches.index[-1])
                    )
                else:
                    level["last_tested"] = "unknown"

            return sr_levels

        except Exception as e:
            self.logger.error(f"Error analyzing S/R levels: {e}")
            return {
                "support_levels": [],
                "resistance_levels": [],
                "level_strengths": {},
