...
from ...utils.logger import system_logger
from .enhanced_sr_detection_20250913_1422 import EnhancedSRDetector, SRLevel
from .enhanced_sr_validation_20250913_1422 import EnhancedSRValidator, ValidationResult, BacktestResult
from .enhanced_sr_confluence_20250913_1422 import ConfluenceLevel
import numpy as np
import logging
import time
...
class EnhancedSROptimizer:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.performance_thresholds = self.optimization_config.get('performance_thresholds', {'min_optimization_score': 0.7, 'min_detection_accuracy': 0.6, 'min_validation_accuracy': 0.6, 'min_confluence_quality': 0.5})
    ...
    async def optimize_sr_detection(self, market_data: pd.DataFrame, multi_timeframe_data: Optional[Dict[str, pd.DataFrame]]=None) -> Optional[EnhancedOptimizationResult]:
        ...
        try:
            ...
            optimized_parameters = await self._optimize_parameters(detected_levels, validation_results, backtest_result)
    ...
    async def _optimize_parameters(self, detected_levels: List[SRLevel], validation_results: List[ValidationResult], backtest_result: Optional[BacktestResult]) -> Dict[str, Any]:
        """Optimize S/R detection parameters."""
        try:
            self.logger.info('🔧 Optimizing S/R detection parameters...')
            current_performance = self._analyze_current_performance(detected_levels, validation_results, backtest_result)
    ...
    def _analyze_current_performance(self, detected_levels: List[SRLevel], validation_results: List[ValidationResult], backtest_result: Optional[BacktestResult]) -> Dict[str, float]:
        """Analyze current S/R detection performance."""
        try:
            performance = {'detection_rate': len(detected_levels) / 1000.0 if detected_levels else 0.0, 'avg_strength': np.mean([level.strength for level in detected_levels]) if detected_levels else 0.0, 'avg_validation_score': np.mean([r.validation_score for r in validation_results]) if validation_results else 0.0, 'avg_bounce_rate': np.mean([r.bounce_rate for r in validation_results]) if validation_results else 0.0, 'avg_false_breakout_rate': np.mean([r.false_breakout_rate for r in validation_results]) if validation_results else 0.0}
            if backtest_result:
                performance.update({'sharpe_ratio': backtest_result.sharpe_ratio, 'win_rate': backtest_result.win_rate, 'profit_factor': backtest_result.profit_factor, 'max_drawdown': backtest_result.max_drawdown})
            return performance
        except Exception as e:
    ...
    def _generate_parameter_suggestions(self, performance: Dict[str, float]) -> List[Dict[str, Any]]:
        ...
        try:
            suggestions = []
            base_params = {'min_touches': 3, 'touch_proximity_threshold': 0.002, 'min_strength': 0.6, 'volume_spike_threshold': 1.5, 'fractal_period': 5, 'pivot_period': 10}
            if performance.get('detection_rate', 0) < 0.1:
                suggestions.append({**base_params, 'min_touches': 2, 'touch_proximity_threshold': 0.003, 'min_strength': 0.5})
            if performance.get('avg_false_breakout_rate', 0) > 0.4:
                suggestions.append({**base_params, 'min_touches': 4, 'touch_proximity_threshold': 0.0015, 'min_strength': 0.7})
            if performance.get('avg_bounce_rate', 0) < 0.5:
                suggestions.append({**base_params, 'bounce_threshold': 0.0008, 'breakout_threshold': 0.003})
            for _ in range(5):
    ...
    def _calculate_performance_metrics(self, detected_levels: List[SRLevel], validation_results: List[ValidationResult], backtest_result: Optional[BacktestResult], confluence_levels: List[ConfluenceLevel]) -> Dict[str, float]:
        ...
        try:
            ...
            if backtest_result:
                optimization_score = (optimization_score + (backtest_result.win_rate + min(backtest_result.sharpe_ratio / 2, 0.5) + min(backtest_result.profit_factor / 3, 0.5)) / 3) / 2
            metrics.update({'optimization_score': optimization_score, 'detection_accuracy': detection_accuracy, 'validation_accuracy': validation_accuracy, 'confluence_quality': confluence_quality})
            return metrics
        except Exception as e:
            self.logger.warning(f'Performance metrics calculation failed: {e}')
            return {'optimization_score': 0.0, 'detection_accuracy': 0.0, 'validation_accuracy': 0.0, 'confluence_quality': 0.0}
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            summary = {'total_optimizations': len(self.optimization_results), 'latest_optimization': {'timestamp': latest_result.timestamp, 'optimization_score': latest_result.optimization_score, 'detection_accuracy': latest_result.detection_accuracy, 'validation_accuracy': latest_result.validation_accuracy, 'confluence_quality': latest_result.confluence_quality, 'detected_levels': len(latest_result.detected_levels), 'confluence_levels': len(latest_result.confluence_levels), 'optimization_time': latest_result.optimization_time}, 'best_optimization': {'optimization_score': best_result.optimization_score, 'detection_accuracy': best_result.detection_accuracy, 'validation_accuracy': best_result.validation_accuracy, 'confluence_quality': best_result.confluence_quality, 'market_regime': best_result.market_regime} if best_result else None, 'performance_thresholds': self.performance_thresholds}
            return summary
        except Exception as e:
