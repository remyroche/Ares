...
## 2026-06-03 - Replacing iterrows() with zip() on to_numpy() arrays in inference loops
Learning: Iterating over pandas DataFrames in performance-critical sections like inner inference loops using `iterrows()` is incredibly slow because of the continuous boxing and unboxing of pandas Series objects.
Action: Replace `iterrows()` with `zip()` operating on the underlying NumPy arrays (e.g., `zip(df.index.to_numpy(), df["colA"].to_numpy(), ...)`). When doing so, use `.get("col", pd.Series(np.nan, index=df.index)).to_numpy()` to safely handle potentially missing columns and guarantee array length alignment.
