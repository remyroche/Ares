...
class MonteCarloValidationStep:
    ...
    async def execute(
        ...
        try:
            ...
            from src.training.steps.step7_monte_carlo_validation import (
                run_step as mc_run_step,
            )
            ...
            validation_file = (
                f"{validation_dir}/{exchange}_{symbol}_monte_carlo_validation.pkl"
            )
            ...
            self.logger.info(
                f"✅ Monte Carlo validation completed. Results saved to {validation_dir}",
            )
