...
def create_sample_data():
    ...
    data['target'] = data['close'].pct_change(5).shift(-5).fillna(0)  # 5-period future returns
...
def demonstrate_enhanced_detector():
    ...
    feature_columns = [
        'ema20_slope', 'sma50_slope', 'market_depth_change', 
        'market_depth_returns', 'market_depth_imbalance', 'volatility_20_change',
        'rsi_momentum'
    ]
    ...
    if "implementation_analysis" in results:
        ...
        if "properly_lagged_features" in impl:
            print(f"   ✅ {len(impl['properly_lagged_features'])} features with proper lagging")
        if "potentially_problematic_features" in impl:
            print(f"   ⚠️ {len(impl['potentially_problematic_features'])} features need review")
...
def compare_with_old_detector():
    ...
    print(f"   • Legitimate features detected: {len(results.get('legitimate_features', []))}")
...
if __name__ == "__main__":
    ...
    print(f"💡 The enhanced detector reduces false positives while maintaining detection accuracy.")
