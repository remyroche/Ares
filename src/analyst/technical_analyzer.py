...
class TechnicalAnalyzer:
    ...
    @handle_data_processing_errors(
        default_return=np.nan,
        context="calculate_williams_r"
    )
    def _calculate_williams_r(self, df: pd.DataFrame, period: int = 14) -> float:
        """Calculates Williams %R momentum oscillator."""
        if len(df) < period:
            self.logger.warning(f"Insufficient data for Williams %R calculation. Need {period}, got {len(df)}")
            return np.nan
        
        try:
            willr = df.ta.willr(length=period)
            return willr.iloc[-1] if not willr.empty and not pd.isna(willr.iloc[-1]) else np.nan
        except Exception as e:
    ...
    @handle_data_processing_errors(
        default_return=np.nan,
        context="calculate_cci"
    )
    def _calculate_cci(self, df: pd.DataFrame, period: int = 20) -> float:
        """Calculates Commodity Channel Index (CCI)."""
        if len(df) < period:
            self.logger.warning(f"Insufficient data for CCI calculation. Need {period}, got {len(df)}")
            return np.nan
        
        try:
            cci = df.ta.cci(length=period)
            return cci.iloc[-1] if not cci.empty and not pd.isna(cci.iloc[-1]) else np.nan
        except Exception as e:
    ...
    @handle_data_processing_errors(
        default_return=np.nan,
        context="calculate_mfi"
    )
    def _calculate_mfi(self, df: pd.DataFrame, period: int = 14) -> float:
        """Calculates Money Flow Index (MFI) - Volume-weighted RSI."""
        if len(df) < period:
            self.logger.warning(f"Insufficient data for MFI calculation. Need {period}, got {len(df)}")
            return np.nan
        
        try:
            mfi = df.ta.mfi(length=period)
            return mfi.iloc[-1] if not mfi.empty and not pd.isna(mfi.iloc[-1]) else np.nan
        except Exception as e:
    ...
    @handle_data_processing_errors(
        default_return=np.nan,
        context="calculate_roc"
    )
    def _calculate_roc(self, df: pd.DataFrame, period: int = 10) -> float:
        """Calculates Rate of Change (ROC) momentum indicator."""
        if len(df) < period:
            self.logger.warning(f"Insufficient data for ROC calculation. Need {period}, got {len(df)}")
            return np.nan
        
        try:
            roc = df.ta.roc(length=period)
            return roc.iloc[-1] if not roc.empty and not pd.isna(roc.iloc[-1]) else np.nan
        except Exception as e:
    ...
    @handle_data_processing_errors(
        default_return={},
        context="calculate_parabolic_sar"
    )
    def _calculate_parabolic_sar(self, df: pd.DataFrame) -> Dict:
        ...
        if len(df) < 20:  # Need sufficient data for PSAR
            self.logger.warning(f"Insufficient data for Parabolic SAR calculation. Need 20, got {len(df)}")
            return {}
        ...
        try:
            ...
            current_psar = psar['PSARl_0.02_0.2'].iloc[-1] if not pd.isna(psar['PSARl_0.02_0.2'].iloc[-1]) else psar['PSARs_0.02_0.2'].iloc[-1]
            current_price = df['close'].iloc[-1]
            ...
            return {
                ...
                "trend": trend,
                "distance_to_psar": abs(current_price - current_psar) / current_price if current_price != 0 else 0
            }
        except Exception as e:
    ...
    @handle_data_processing_errors(
        default_return={},
        context="calculate_ichimoku_cloud"
    )
    def _calculate_ichimoku_cloud(self, df: pd.DataFrame) -> Dict:
        ...
        if len(df) < 52:  # Need sufficient data for Ichimoku
            self.logger.warning(f"Insufficient data for Ichimoku calculation. Need 52, got {len(df)}")
            return {}
        ...
        try:
            ...
            # Get latest values
            tenkan_sen = ichimoku['ITS_9'].iloc[-1] if 'ITS_9' in ichimoku.columns else np.nan
            kijun_sen = ichimoku['IKS_26'].iloc[-1] if 'IKS_26' in ichimoku.columns else np.nan
            senkou_span_a = ichimoku['ISA_9'].iloc[-1] if 'ISA_9' in ichimoku.columns else np.nan
            senkou_span_b = ichimoku['ISB_26'].iloc[-1] if 'ISB_26' in ichimoku.columns else np.nan
            
            # Determine cloud position
            cloud_top = max(senkou_span_a, senkou_span_b) if not (pd.isna(senkou_span_a) or pd.isna(senkou_span_b)) else np.nan
            cloud_bottom = min(senkou_span_a, senkou_span_b) if not (pd.isna(senkou_span_a) or pd.isna(senkou_span_b)) else np.nan
            
            # Determine trend signals
            above_cloud = current_price > cloud_top if not pd.isna(cloud_top) else False
            below_cloud = current_price < cloud_bottom if not pd.isna(cloud_bottom) else False
    ...
    @handle_data_processing_errors(
        default_return={},
        context="calculate_supertrend"
    )
    def _calculate_supertrend(self, df: pd.DataFrame, period: int = 10, multiplier: float = 3.0) -> Dict:
        """Calculates SuperTrend indicator."""
        if len(df) < period:
            self.logger.warning(f"Insufficient data for SuperTrend calculation. Need {period}, got {len(df)}")
            return {}
        ...
        try:
            ...
            return {
                ...
                "distance_to_supertrend": abs(current_price - current_supertrend) / current_price if current_price != 0 else 0
            }
        except Exception as e:
    ...
    def _calculate_donchian_channels(self, df: pd.DataFrame, period: int = 20) -> Dict:
        ...
        if len(df) < period:
            self.logger.warning(f"Insufficient data for Donchian Channels calculation. Need {period}, got {len(df)}")
            return {}
        ...
        try:
            ...
            return {
                ...
                "near_upper": channel_position > 0.8 if channel_position is not None else False,
                "near_lower": channel_position < 0.2 if channel_position is not None else False
            }
        except Exception as e:
    ...
    @handle_data_processing_errors(
        default_return={},
        context="calculate_atr_bands"
    )
    def _calculate_atr_bands(self, df: pd.DataFrame, period: int = 14, multiplier: float = 2.0) -> Dict:
        """Calculates ATR-based volatility bands."""
        if len(df) < period:
            self.logger.warning(f"Insufficient data for ATR Bands calculation. Need {period}, got {len(df)}")
            return {}
        ...
        try:
            ...
            current_price = df['close'].iloc[-1]
            upper_value = upper_band.iloc[-1] if not pd.isna(upper_band.iloc[-1]) else np.nan
            lower_value = lower_band.iloc[-1] if not pd.isna(lower_band.iloc[-1]) else np.nan
            ...
            return {
                ...
                "middle": middle_value,
                "lower": lower_value,
                "above_upper": current_price > upper_value if not pd.isna(upper_value) else False,
                "below_lower": current_price < lower_value if not pd.isna(lower_value) else False
            }
        except Exception as e:
    ...
    @handle_data_processing_errors(
        default_return={},
        context="calculate_vwap_bands"
    )
    def _calculate_vwap_bands(self, df: pd.DataFrame, period: int = 20, multiplier: float = 2.0) -> Dict:
        """Calculates VWAP-based bands."""
        if len(df) < period:
            self.logger.warning(f"Insufficient data for VWAP Bands calculation. Need {period}, got {len(df)}")
            return {}
        ...
        try:
            ...
            return {
                ...
                "above_upper": current_price > upper_value if not pd.isna(upper_value) else False,
                "below_lower": current_price < lower_value if not pd.isna(lower_value) else False,
                "distance_from_vwap": (current_price - vwap_value) / vwap_value if not pd.isna(vwap_value) and vwap_value != 0 else 0
            }
        except Exception as e:
