...
def generate_risk_regime_features(
    ...
    # 7. Volume Coefficient of Variation (CV) (New)
    # Rolling Std / Rolling Mean
    vol_cv_window = int(config.get("risk_volume_cv_window", 24))
    ...
    if volume is not None:
        ...
        if use_ewm:
            ...
            # CV
            vol_mean = volume.ewm(span=vol_cv_window, adjust=False).mean()
            vol_std = volume.ewm(span=vol_cv_window, adjust=False).std()
        else:
            volatility_of_volume = log_vol_change.rolling(window=vol_vol_window, min_periods=vol_vol_window).std()
            # CV
            vol_mean = volume.rolling(window=vol_cv_window, min_periods=vol_cv_window).mean()
            vol_std = volume.rolling(window=vol_cv_window, min_periods=vol_cv_window).std()

        rolling_volume_kurtosis = log_vol_change.rolling(window=vol_kurt_window, min_periods=vol_kurt_window).kurt()

        # Coefficient of Variation
        volume_cv = vol_std / vol_mean.replace(0, np.nan)

    else:
        volatility_of_volume = pd.Series(0.0, index=df.index)
        rolling_volume_kurtosis = pd.Series(0.0, index=df.index)
        volume_cv = pd.Series(0.0, index=df.index)
