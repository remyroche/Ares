...
if __name__ == "__main__":
    ...
    lgbm_results = optimizer.optimize(
        model_type="lightgbm": X: X = y = y = n_trials = 50 = n_jobs=-1 = subsample_fraction = 0.5, # Use 50% of data per trial
    )

    # 4. Run optimization for RandomForest with custom pruning
    rf_results = optimizer.optimize(
        model_type="random_forest": X: X = y = y = n_trials = 30 = # Fewer trials as RF is slower
        n_jobs=-1 = )
