...
class TripleBarrierMethodStep:
    ...
    async def execute_triple_barrier_method(self, symbol: str, exchange: str, timeframe: str, data_dir: str='data_cache', force_rerun: bool=False) -> bool:
        ...
        try:
            ...
            await self._log_step4_artifacts_and_report(symbol, exchange, timeframe, data_dir, result_data, output_path)
            return True
        except Exception as e:
            self.logger.exception(f'❌ Error in triple barrier method: {e}')
            return False

    async def _log_step4_artifacts_and_report(self, symbol: str, exchange: str, timeframe: str, data_dir: str, result_data: pd.DataFrame, output_path: Path) -> None:
        ...
        try:
            ...
            artifacts_generated = [str(output_path), f'{exchange}_{symbol}_{timeframe}_triple_barrier_metrics.json']
            metrics_calculated = {'triple_barrier_success': 1.0, 'total_samples': len(result_data) if result_data is not None else 0, 'labeled_samples': len(result_data[result_data['triple_barrier_label'].notna()]) if result_data is not None and 'triple_barrier_label' in result_data.columns else 0, 'label_distribution': result_data['triple_barrier_label'].value_counts().to_dict() if result_data is not None and 'triple_barrier_label' in result_data.columns else {}}
            training_input = {'symbol': symbol, 'exchange': exchange, 'timeframe': timeframe, 'data_dir': data_dir}
            step_data = {'output_path': str(output_path), 'data_shape': list(result_data.shape) if result_data is not None else [], 'label_columns': list(result_data.columns) if result_data is not None else []}
            report_data = create_detailed_step_report(step_name='step04_5_triple_barrier_method', step_data=step_data, training_input=training_input, execution_metadata=execution_metadata, artifacts_generated=artifacts_generated, metrics_calculated=metrics_calculated, errors_encountered=[])
            report_name = log_step_report(config=self.config, step_name='step04_5_triple_barrier_method', report_data=report_data, report_type='triple_barrier_method_report', additional_metadata={'triple_barrier_success': True, 'timeframe': timeframe, 'asset': symbol, 'lookback_period': self.config.get('lookback_days', 1095), 'project_version': self.config.get('project_version', '1.0.0')})
            self.logger.info(f'✅ Logged triple barrier method report: {report_name}')
            if result_data is not None:
                artifact_name = log_step_dataframe_with_standardized_name(config=self.config, step_name='step04_5_triple_barrier_method', df=result_data, artifact_type='triple_barrier_labels', additional_metadata={'artifact_type': 'triple_barrier_labels', 'dataframe_shape': list(result_data.shape), 'label_distribution': result_data['triple_barrier_label'].value_counts().to_dict() if 'triple_barrier_label' in result_data.columns else {}, 'asset': symbol, 'lookback_period': self.config.get('lookback_days', 1095), 'project_version': self.config.get('project_version', '1.0.0'), 'timeframe': timeframe})
                self.logger.info(f'✅ Logged triple barrier labels: {artifact_name}')
            log_step_metrics(config=self.config, step_name='step04_5_triple_barrier_method', metrics=metrics_calculated, additional_metadata={'metrics_type': 'triple_barrier_performance', 'timeframe': timeframe, 'asset': symbol, 'lookback_period': self.config.get('lookback_days', 1095), 'project_version': self.config.get('project_version', '1.0.0')})
...
from src.core.decorators.errors import handles_errors
    ...
    def _create_enhanced_labels(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            enhanced_data['profit_category'] = pd.cut(enhanced_data['potential_profit_pct'], bins=profit_bins, labels=profit_labels, include_lowest=True)
            enhanced_data['direction_profit_label'] = enhanced_data.apply(lambda row: f"{('LONG' if row['label'] == 1 else 'SHORT')}_{row['profit_category']}", axis=1)
...
async def run_step(symbol: str, exchange: str, timeframe: str, data_dir: str='data_cache', force_rerun: bool=False, config: Optional[Dict[str, Any]]=None) -> bool:
    ...
    if config is None:
        config = {}
    step_config = {'SYMBOL': symbol, 'EXCHANGE': exchange, 'TIMEFRAME': timeframe, 'DATA_DIR': data_dir, 'triple_barrier': {'profit_take_multiplier': 0.002, 'stop_loss_multiplier': 0.001, 'time_barrier_minutes': 30, 'max_lookahead': 100}, **config}
