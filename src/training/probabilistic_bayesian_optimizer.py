...
This module provides Bayesian optimization specifically designed for probabilistic
models that output probability distributions, confidence intervals, and uncertainty
estimates. It optimizes both model hyperparameters and probabilistic output calibration.
"""
...
class ProbabilisticBayesianOptimizer:
    ...
    def evaluate_probabilistic_metrics(
        ...
        if "calibration" in self.config.objectives:
            metrics["calibration"] = self._calculate_calibration_score(
                y_true, y_pred_proba
            )
        ...
        if "discrimination" in self.config.objectives:
            metrics["discrimination"] = self._calculate_discrimination_score(
                y_true, y_pred_proba
            )
    ...
    def optimize(
        ...
        self.logger.info(
            f"Starting probabilistic Bayesian optimization for {self.model_type}"
        )
        self.logger.info(f"Objectives: {self.config.objectives}")
        self.logger.info(f"Number of trials: {self.config.n_trials}")
        self.logger.info(
            f"Objective weights: 50% total_profit, 25% win_rate, 25% sharpe_ratio"
        )

        # Create objective function
        objective = self.create_objective_function(
            X, y, model_factory, validation_split
        )
        ...
        if best_trial:
            return {
                ...
                "objective_values": dict(
                    zip(self.config.objectives, best_trial.values)
                ),
    ...
    def _log_mlflow_experiment(
        self, study_name: str, best_params: Dict[str, Any], best_values: List[float]
    ):
    ...
    def plot_optimization_results(self, save_path: Optional[str] = None):
        ...
        try:
            ...
            fig, axes = plt.subplots(
                ...
                figsize=(5 * len(self.config.objectives), 5),
            )
...
if __name__ == "__main__":
    # Example usage
    config = ProbabilisticOptimizationConfig(
        objectives=["calibration", "sharpness", "discrimination"], n_trials=50, n_jobs=1
    )
