...
class DataSchema:
    ...
    def validate_dataframe(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        Returns:
            Validation results
        """
        results = {'valid': True, 'errors': [], 'warnings': [], 'missing_columns': [], 'extra_columns': [], 'type_mismatches': [], 'constraint_violations': []}
...
class EnhancedOutlierHandler:
    ...
    def __init__(self, raise_errors: bool=True, log_details: bool=True) -> None:
        ...
        self.standard_schemas = {'klines': DataSchema(name='klines', required_columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'], data_types={'timestamp': 'int64', 'open': 'float64', 'high': 'float64', 'low': 'float64', 'close': 'float64', 'volume': 'float64'}, constraints={'open': {'min': 0, 'not_null': True}, 'high': {'min': 0, 'not_null': True}, 'low': {'min': 0, 'not_null': True}, 'close': {'min': 0, 'not_null': True}, 'volume': {'min': 0, 'not_null': True}}), 'features': DataSchema(name='features', required_columns=['timestamp'], optional_columns=[], data_types={'timestamp': 'int64'}, constraints={'timestamp': {'not_null': True}}), 'labels': DataSchema(name='labels', required_columns=['timestamp', 'label'], data_types={'timestamp': 'int64', 'label': 'object'}, constraints={'timestamp': {'not_null': True}, 'label': {'not_null': True}})}
        self.detection_methods = {'zscore': self._detect_zscore_outliers, 'iqr': self._detect_iqr_outliers, 'isolation_forest': self._detect_isolation_forest_outliers, 'local_outlier_factor': self._detect_lof_outliers, 'mahalanobis': self._detect_mahalanobis_outliers}
    ...
    def create_custom_schema(self, name: str, required_columns: list[str], optional_columns: list[str]=None, data_types: dict[str, str]=None, constraints: dict[str, dict[str, Any]]=None) -> DataSchema:
    ...
    def get_schema_info(self, schema_name: str) -> dict[str, Any]:
        ...
        schema = self.standard_schemas[schema_name]
        return {'name': schema.name, 'required_columns': list(schema.required_columns), 'optional_columns': list(schema.optional_columns), 'all_columns': list(schema.all_columns), 'data_types': schema.data_types, 'constraints': schema.constraints}
    ...
    def get_outlier_report(self) -> dict[str, Any]:
        ...
        return {'timestamp': datetime.now().isoformat(), 'total_outlier_groups': len(self.outlier_history), 'severity_distribution': severity_counts, 'column_distribution': column_counts, 'method_distribution': method_counts, 'recent_outliers': [{'column': o.column, 'count': len(o.indices), 'severity': o.severity.value, 'method': o.method, 'timestamp': o.timestamp.isoformat()} for o in self.outlier_history[-10:]]}
