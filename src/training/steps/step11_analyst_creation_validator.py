...
class Step11AnalystCreationValidator:
    ...
    def validate_step11_analyst_creation(
        ...
        try:
            ...
            if not regime_dirs:
                self.logger.warning(
                    "⚠️ No regime-specific analyst model directories found"
                )
                return False
            ...
            for regime_dir in regime_dirs:
                ...
                self.logger.info(
                    f"📊 Validating analyst models for regime: {regime_name}"
                )
    ...
    def _validate_analyst_model(self, model_file: Path, regime_name: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    f"✅ Model file validated: {model_file.name} ({file_size} bytes)"
                )
                return True
            except Exception as e:
                self.logger.warning(
