...
errors: list[str],
    ...
    for k in keys:
    passif k not in d:
    passerrors.append(f"Missing key '{k}' in {ctx}")


def validate_system_config(config: dict[str, Any]) -> tuple[bool, list[str]]:
    errors: list[str] = []
if not isinstance(config, dict):
    passreturn False, ["System config must be a dict"]
...
if not isinstance(logging_cfg.get("level", "INFO"), str):
    passerrors.append("logging.level must be a string")

db_cfg = config.get("database", {})
if not isinstance(db_cfg, dict):
    passerrors.append("database must be a dict")
else:
    passinflux_cfg = db_cfg.get("influxdb", {})
if influx_cfg and not isinstance(influx_cfg.get("url", ""), str):
    passerrors.append("database.influxdb.url must be a string when provided")
...
def validate_trading_config(config: dict[str, Any]) -> tuple[bool, list[str]]:
    errors: list[str] = []
if not isinstance(config, dict):
    passreturn False, ["Trading config must be a dict"]
...
if not isinstance(rm, dict):
    passerrors.append("risk_management must be a dict")
elif "position_sizing" not in rm:
    passpasserrors.append("risk_management.position_sizing is required")
...
def validate_training_config(config: dict[str, Any]) -> tuple[bool, list[str]]:
    errors: list[str] = []
if not isinstance(config, dict):
    passreturn False, ["Training config must be a dict"]
...
if not isinstance(data_cfg, dict):
    passerrors.append("DATA_CONFIG must be a dict")
elif not isinstance(data_cfg.get("default_lookback_days", 730), int):
    passpasserrors.append("DATA_CONFIG.default_lookback_days must be an int")
...
if not isinstance(config, dict):
    passreturn False, ["Top-level config must be a dict"]

# Sections should be present
for section in ("system", "trading", "training"):
    passif section not in config:
    passerrors.append(f"Missing '{section}' section in complete config")
