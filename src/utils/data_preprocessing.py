...
def regularize_timestamps(
    ...
    try:
        ...
        if irregular_ratio > 0.0001:  # If more than 0.01% irregular intervals (more sensitive)
            logger.info(f"🔄 Regularizing timestamps (irregular ratio: {irregular_ratio:.3f})")
            ...
            logger.info(f"✅ Regularized timestamps: {len(processed_data)} rows with {freq} intervals")

        return processed_data

    except Exception as e:
        logger.error(f"🚨 Error regularizing timestamps: {e}")
        return data
...
def preprocess_data_for_multi_timeframe(
    ...
    try:
        ...
        logger.error(f"🚨 Error preprocessing data for multi-timeframe: {e}")
...
def validate_and_fix_data_quality(
    ...
    try:
        ...
        logger.info(f"✅ Data quality validation completed: {len(validation_results['issues_fixed'])} issues fixed")
