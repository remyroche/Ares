...
class EnhancedSRValidator:
    ...
    def _validate_single_level(self, level: Any, market_data: pd.DataFrame, level_id: int) -> Optional[ValidationResult]:
    ...
    def _get_validation_period(self, level: Any, market_data: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            validation_length = min(max(self.min_validation_period, len(market_data) - start_idx), self.max_validation_period)
                ...
                if level_type == 'support':
                    if abs(current_row['low'] - level_price) <= threshold:
                        ...
                        if next_row['high'] > level_price + threshold:
                            bounces += 1
                elif abs(current_row['high'] - level_price) <= threshold:
                    total_touches += 1
                    ...
                    if current_row['close'] < level_price - threshold:
                        ...
                        for j in range(i + 1, min(i + 6, len(data))):
                            if data.iloc[j]['close'] > level_price + threshold:
                                false_breakouts += 1
                                break
                elif current_row['close'] > level_price + threshold:
                    total_breakouts += 1
                    for j in range(i + 1, min(i + 6, len(data))):
                        if data.iloc[j]['close'] < level_price - threshold:
                            false_breakouts += 1
    ...
    def _calculate_volume_confirmation_rate(self, level: Any, data: pd.DataFrame) -> float:
        ...
        try:
            ...
            for i, row in data.iterrows():
                ...
                elif abs(row['high'] - level_price) <= threshold:
                    total_touches += 1
                    if row['volume'] > volume_ma.iloc[i] * self.volume_spike_threshold:
                        volume_confirmations += 1
            ...
            for i in range(len(data) - 1):
                ...
                if level_type == 'support':
                    ...
                    if bounce_ratio > 0:
                        bounce_ratios.append(bounce_ratio)
            if bounce_ratios:
                return {'avg': np.mean(bounce_ratios), 'max': np.max(bounce_ratios), 'min': np.min(bounce_ratios), 'std': np.std(bounce_ratios)}
            else:
            ...
            for i, row in data.iterrows():
                if level_type == 'support':
                    if row['close'] < level_price - threshold:
                        failures += 1
                elif row['close'] > level_price + threshold:
                    failures += 1
            return failures
        except Exception as e:
    ...
    def _calculate_validation_score(self, bounce_rate: float, false_breakout_rate: float, volume_confirmation_rate: float, touch_count: int, failure_count: int, statistical_significance: float) -> float:
        """Calculate overall validation score for S/R level."""
        try:
            weights = {'bounce_rate': 0.3, 'false_breakout_rate': 0.2, 'volume_confirmation': 0.2, 'touch_count': 0.15, 'statistical_significance': 0.15}
            normalized_touches = min(touch_count / 10.0, 1.0)
            failure_penalty = min(failure_count * 0.1, 0.3)
            score = weights['bounce_rate'] * bounce_rate + weights['false_breakout_rate'] * (1.0 - false_breakout_rate) + weights['volume_confirmation'] * volume_confirmation_rate + weights['touch_count'] * normalized_touches + weights['statistical_significance'] * statistical_significance - failure_penalty
    ...
    def _calculate_trading_performance(self, validation_results: List[ValidationResult], market_data: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            ...
            for result in validation_results:
                if result.validation_score > 0.6:
                    ...
                    level_data = market_data[(market_data.index >= market_data.index[0]) & (market_data.index <= market_data.index[-1])]
                    if len(level_data) > 0:
                        if result.level_type == 'support':
                            exit_price = level_data['high'].max() * 0.99
                            ...
                            exit_price = level_data['low'].min() * 1.01
                            ...
                            if exit_price < stop_loss:
                                ...
                                trades.append(-0.02)
            if not trades:
                return {'sharpe_ratio': 0.0, 'max_drawdown': 0.0, 'win_rate': 0.0, 'profit_factor': 0.0, 'total_trades': 0, 'successful_trades': 0, 'failed_trades': 0}
            ...
            if np.std(trades_array) > 0:
                sharpe_ratio = np.mean(trades_array) / np.std(trades_array) * np.sqrt(252)
            else:
                sharpe_ratio = 0.0
            ...
            return {'sharpe_ratio': sharpe_ratio, 'max_drawdown': max_drawdown, 'win_rate': win_rate, 'profit_factor': profit_factor, 'total_trades': len(trades), 'successful_trades': successful_trades, 'failed_trades': failed_trades, 'avg_return': np.mean(trades_array), 'volatility': np.std(trades_array)}
        except Exception as e:
