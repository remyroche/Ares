    ...
    async def _perform_full_conversion(self, symbol: str, exchange: str, timeframe: str, data_dir: str) -> bool:
        ...
        try:
            ...
            if unified_data.empty:
                ...
                return False
            quality_result = await self.quality_validator.validate_unified_data_quality(unified_data, 'unified_data_final')
            if not quality_result.passed:
...
async def run_enhanced_step1_5(training_input: Dict[str, Any], pipeline_state: Dict[str, Any], config: Optional[Step1_5Config]=None) -> Dict[str, Any]:
    """
...
if __name__ == '__main__':
    ...
    async def main() -> None:
        ...
        try:
            ...
            tprint('=' * 60)
            tprint('ENHANCED STEP1_5 EXECUTION RESULTS')
            tprint('=' * 60)
            tprint(f"Data conversion completed: {result['data_conversion_completed']}")
            tprint(f"Quality check passed: {result['quality_check_passed']}")
            tprint('=' * 60)
        except Exception as e:
            tprint(f'❌ Enhanced Step1_5 execution failed: {e}')
    asyncio.run(main())