...
from training.enhanced_optimization_orchestrator import EnhancedOptimizationOrchestrator
...
class ConfigurationUsageExample:
    ...
    def validate_configuration(self) -> bool:
        ...
        try:
            ...
            if not hpo_config["multi_objective"]["enabled"] and \
               not hpo_config["bayesian_optimization"]["enabled"] and \
               not hpo_config["adaptive_optimization"]["enabled"]:
                logger.warning("All optimization types are disabled")
            
            # Validate computational optimization
            comp_config = self.comp_config
            if not comp_config["caching"]["enabled"] and \
               not comp_config["parallelization"]["enabled"]:
    ...
    def print_configuration_summary(self):
        ...
        print(f"  Multi-Objective: {'✅ Enabled' if hpo_config['multi_objective']['enabled'] else '❌ Disabled'}")
        ...
        print(f"  Bayesian: {'✅ Enabled' if hpo_config['bayesian_optimization']['enabled'] else '❌ Disabled'}")
        ...
        print(f"  Adaptive: {'✅ Enabled' if hpo_config['adaptive_optimization']['enabled'] else '❌ Disabled'}")
        if hpo_config['adaptive_optimization']['enabled']:
            regimes = list(hpo_config['adaptive_optimization']['regime_specific_constraints'].keys())
        ...
        print(f"  Caching: {'✅ Enabled' if comp_config['caching']['enabled'] else '❌ Disabled'}")
        ...
        print(f"  Parallelization: {'✅ Enabled' if comp_config['parallelization']['enabled'] else '❌ Disabled'}")
        ...
        print(f"  Memory Management: {'✅ Enabled' if comp_config['memory_management']['enabled'] else '❌ Disabled'}")
        if comp_config['memory_management']['enabled']:
            print(f"    Memory Threshold: {comp_config['memory_management']['memory_threshold']*100}%")
            print(f"    Cleanup Frequency: {comp_config['memory_management']['cleanup_frequency']}")
        
        print(f"  Early Stopping: {'✅ Enabled' if comp_config['early_stopping']['enabled'] else '❌ Disabled'}")
    ...
    def demonstrate_multi_objective_config(self):
        ...
        if multi_obj_config["enabled"]:
            ...
            weighted_score = sum(
                example_scores[obj] * weights[obj] 
                for obj in objectives
            )
            
            print(f"\n📊 Example weighted score calculation:")
            for obj in objectives:
                print(f"  {obj}: {example_scores[obj]} × {weights[obj]} = {example_scores[obj] * weights[obj]:.3f}")
            ...
            for space_name, space_config in search_spaces.items():
                ...
                for param_name, param_config in space_config.items():
                    if isinstance(param_config, dict) and "low" in param_config:
                        print(f"    {param_name}: {param_config['low']} to {param_config['high']} ({param_config['type']})")
    ...
    def demonstrate_computational_config(self):
        ...
        if memory_config["enabled"]:
            ...
            print(f"  Cleanup frequency: {memory_config['cleanup_frequency']} operations")
        else:
        ...
        if progressive_config["enabled"]:
            ...
            for i, stage in enumerate(stages, 1):
                print(f"    Stage {i}: {stage['data_ratio']*100}% data, weight {stage['weight']}")
        else:
    ...
    async def demonstrate_optimization_usage(self):
        ...
        try:
            ...
            print(f"  Sampling strategy: {self.hpo_config['bayesian_optimization']['sampling_strategy']}")
            print(f"  Max trials: {self.hpo_config['bayesian_optimization']['max_trials']}")
        ...
        try:
            ...
            print(f"  Caching: {'Enabled' if self.comp_config['caching']['enabled'] else 'Disabled'}")
            print(f"  Parallelization: {'Enabled' if self.comp_config['parallelization']['enabled'] else 'Disabled'}")
            print(f"  Memory management: {'Enabled' if self.comp_config['memory_management']['enabled'] else 'Disabled'}")
    ...
    def demonstrate_configuration_modification(self):
        ...
        print(f"  Cache size: 1000 → {modified_comp_config['caching']['max_cache_size']}")
        print(f"  Cache TTL: 3600s → {modified_comp_config['caching']['cache_ttl']}s")
        print(f"  Max workers: 8 → {modified_comp_config['parallelization']['max_workers']}")
