...
def main() -> None:
    ...
    dual_mode = current.get(
        "portfolio_policy_schema", pd.Series("", index=current.index)
    ).astype(str).eq("strict_r3_bcf_current_dual_mc1_portfolio_v1").any()
    admission_field = (
        "dual_bcf_current_admitted_ge_30bps" if dual_mode
        else "mc1_d2_admitted_ge_50bps"
    )
    admitted = current[admission_field].fillna(False).astype(bool)
    ...
    checks.update({
        ...
        "mc1_admission_contract": (
            (~accepted | admitted).all()
            and (
                not dual_mode
                or current.loc[accepted, "bcf_mc1_admitted_ge_30bps"].fillna(False).all()
            )
        ),
