...
Provides decorators for enforcing authentication and permission policies in a framework-
agnostic way.
"""
...
def requires_role(
    *roles: str,
    ...
    def sync_handler(func: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
        ...
        if not has_required_roles:
            raise AuthorizationError(
                f"Missing required role(s): {', '.join(roles)}",
    ...
    return uniform_wrapper(
        f"requires_role({role_desc}{','.join(roles)})", sync_handler, async_handler
    )
    ...
    *permissions: str,
    ...
    def sync_handler(func: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
        ...
        if not has_required_perms:
            raise AuthorizationError(
                f"Missing required permission(s): {', '.join(permissions)}",
...
def owner_only(
    ...
    def sync_handler(func: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
        ...
        if not resource:
            raise ValueError("No resource to check ownership")
...
def rate_limit(
    *,
    ...
    return uniform_wrapper(
        f"rate_limit({calls}/{period}s)", sync_handler, async_handler
    )
