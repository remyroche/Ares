...
class EarlyStageOptimizer:
    ...
    async def optimize_sr_parameters(
        self, data: pd.DataFrame, optimization_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Optimize SR (Stationarity and Randomness) parameters for data
        preprocessing."""
    ...
    async def run_regime_specific_triple_barrier_optimization(
        ...
        """Run regime-specific triple barrier optimization through the early-stage
        optimizer."""
        ...
        try:
            ...
            optimization_results = (
                await self.regime_optimizer.optimize_regime_specific_parameters(
                    regime_data, optimization_config
                )
            )
    ...
    async def _optimize_single_regime_barrier(
        ...
        objective = self._create_regime_barrier_objective(
            regime_name, regime_data, regime_params
        )
    ...
    def _create_regime_barrier_objective(
        self, regime_name: str, regime_data: pd.DataFrame, regime_params: Dict[str, Any]
    ):
