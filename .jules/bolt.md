...
## 2026-06-24 - [Prediction Ledger] Vectorizing sequential row updates
Learning: Updating rows inside a DataFrame using `iterrows()` paired with `.loc[key, col] = value` and `pd.concat` for inserts scales exceptionally poorly (O(N^2) behavior). Pandas Series instantiation combined with single-cell index lookups creates a severe bottleneck.
Action: Use index manipulation (`index.intersection()` and `index.difference()`) along with bulk `.loc[common_idx]` assignments and a single `pd.concat` for missing indices. This vectorized approach processes the ledger update 20x faster.
