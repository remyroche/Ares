...
class BaseEnsemble:
    """
    his script all child ensembles to train highly optimized and robust models.
    """
    ...
    def train_ensemble(self, historical_features: pd.DataFrame, historical_targets: pd.Series):
        ...
        for col in self.sequence_features + self.flat_features:
            if col not in historical_features.columns: historical_features[col] = 0.0
    ...
    def get_prediction(self, current_features: pd.DataFrame, **kwargs) -> dict:
        ...
        if not self.trained: return {"prediction": "HOLD", "confidence": 0.0}
    ...
    def _tune_hyperparameters(self, model_class, search_space_func, X, y, n_trials=25):
        ...
        if not self.tune_base_models:
            return {} # Return default params if tuning is disabled
    ...
    def _get_svm_search_space(self, trial):
        return {
            ...
            'probability': True # Must be true to get probabilities
        }
