...
class EnhancedTrainingManager:
        ...
        try:
            ...
            if not step_success:
                ...
                if is_fatal:
                    ...
                    self.logger.warning(f"⚠️ {step_display_name} failed - continuing pipeline")
                ...
                try:
                    ...
                    if is_fatal:
                        self.logger.error(f"❌ {step_display_name} validator failed: {e} - stopping pipeline")
                        return False
                    else:
                        self.logger.warning(f"⚠️ {step_display_name} validator failed: {e} - continuing pipeline")
                        return True
    ...
    async def _execute_comprehensive_pipeline(
        ...
        try:
            ...
            try:
                ...
                processed_path = Path(data_dir) / f"{exchange}_{symbol}_labeled_validation.parquet"
                df_proc = None
            ...
            try:
                ...
                if not reliability:
                    ...
                    try:
                        for regime, models in (locals().get("analyst_calibration", {}) or {}).items():
    ...
    async def _run_step_validator(
        ...
        try:
            ...
            dependency_validation = await step_dependency_validator.validate_step_prerequisites(
