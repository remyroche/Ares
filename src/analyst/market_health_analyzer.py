...
class GeneralMarketAnalystModule:
    ...
    def get_market_health_score(self, klines_df: pd.DataFrame) -> float:
        ...
        try:
            ...
            self.logger.info(f"Market health score: {health_score:.2f} "
    ...
    @handle_data_processing_errors(
        default_return=0.0,
        context="calculate_volatility_score"
    )
    ...
    @handle_data_processing_errors(
        default_return=0.0,
        context="calculate_volume_score"
    )
    ...
    @handle_data_processing_errors(
        default_return=0.0,
        context="calculate_liquidity_score"
    )
    def _calculate_liquidity_score(self, df: pd.DataFrame) -> float:
        ...
        try:
            ...
            if avg_price > 0:
                ...
                score = min(100.0, liquidity_ratio * 1000)  # Adjust multiplier as needed
                return max(0.0, min(100.0, score))
            else:
                return 0.0
    ...
    @handle_data_processing_errors(
        default_return={},
        context="get_volatility_metrics"
    )
    def _get_volatility_metrics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "atr_ratio": (atr.iloc[-1] / atr.mean()) if not atr.empty and atr.mean() > 0 else 0.0
            }
        except Exception as e:
    ...
    def _get_trend_metrics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "price_vs_sma_20": ((current_price / current_sma_20) - 1) * 100 if current_sma_20 > 0 else 0.0,
                "price_vs_sma_50": ((current_price / current_sma_50) - 1) * 100 if current_sma_50 > 0 else 0.0
            }
        except Exception as e:
