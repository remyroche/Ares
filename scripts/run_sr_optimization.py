...
class SROptimizationRunner:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not validate_sr_optimization_config(self.sr_config):
            raise ValueError("Invalid S/R optimization configuration")
    ...
    def prepare_sample_data(self, n_samples: int = 2000) -> tuple[pd.DataFrame, pd.Series]:
        ...
        self.logger.info(f"   Price range: {price_data['close'].min():.2f} - {price_data['close'].max():.2f}")
        self.logger.info(f"   Returns range: {target_returns.min():.4f} - {target_returns.max():.4f}")
    ...
    async def run_optimization(
        ...
        study_name = study_name or f"sr_optimization_sr_parameters"
        try:
    ...
    def analyze_results(self) -> Dict[str, Any]:
        ...
        if not self.optimization_results:
            raise ValueError("No optimization results to analyze")
    ...
    def _assess_overfitting(self, result: OptimizationResult) -> Dict[str, Any]:
        ...
        overfitting_assessment = {
            ...
            "overfitting_severity": "low" if result.overfitting_score < 0.05 else 
                                  "medium" if result.overfitting_score < 0.1 else "high",
            "generalization_quality": "good" if result.generalization_gap < 0.05 else
                                    "acceptable" if result.generalization_gap < 0.1 else "poor",
            "recommendations": []
        }
    ...
    def _generate_recommendations(self, result: OptimizationResult) -> list[str]:
        ...
        if result.validation_score < 0.6:
            recommendations.append("Consider increasing the number of optimization trials")
        ...
        if result.overfitting_score > 0.1:
            recommendations.append("Increase regularization in strength score calculation")
            recommendations.append("Reduce model complexity by limiting parameter ranges")
        
        # Parameter-specific recommendations
        best_params = result.best_params
        
        # Check if weights are balanced
        weight_params = [k for k in best_params.keys() if "weight" in k]
        if weight_params:
            weights = [best_params[k] for k in weight_params]
            weight_std = np.std(weights)
            if weight_std > 0.2:
                recommendations.append("Consider rebalancing strength score weights for more stable performance")
        
        # Check confidence thresholds
        if best_params.get("min_sr_confidence", 0) < 0.6:
            recommendations.append("Consider increasing minimum confidence threshold for better signal quality")
        
        if best_params.get("high_confidence_threshold", 0) > 0.85:
            recommendations.append("High confidence threshold may be too restrictive, consider lowering")
    ...
    def export_parameters(self, output_path: str = "optimized_sr_parameters.json") -> str:
        ...
        export_data = {
            ...
            "strength_score_weights": {
                k: v for k, v in self.optimization_results.best_params.items()
                if "weight" in k
            },
            "level_detection_params": {
                k: v for k, v in self.optimization_results.best_params.items()
                if k in ["min_touch_count", "min_level_age_hours", "price_tolerance_pct", 
                        "volume_threshold", "strength_threshold"]
            },
            "breakout_thresholds": {
                k: v for k, v in self.optimization_results.best_params.items()
                if k in ["breakout_threshold", "confirmation_periods", "volume_confirmation",
                        "momentum_threshold", "false_breakout_filter"]
            },
            "zone_multipliers": {
                k: v for k, v in self.optimization_results.best_params.items()
                if k in ["support_zone_multiplier", "resistance_zone_multiplier", "sr_zone_threshold",
                        "zone_expansion_factor", "zone_contraction_factor"]
            },
            "confidence_thresholds": {
                k: v for k, v in self.optimization_results.best_params.items()
                if k in ["min_sr_confidence", "high_confidence_threshold", "confidence_decay_rate",
                        "regime_confidence_boost", "ensemble_confidence_threshold"]
            }
        }
    ...
    def print_comprehensive_report(self, analysis: Dict[str, Any]):
        ...
        print(f"   Generalization Quality: {overfitting['generalization_quality'].title()}")
        ...
        categories = {
            "Strength Score Weights": [k for k in best_params.keys() if "weight" in k],
            "Level Detection": [k for k in best_params.keys() if k in ["min_touch_count", "min_level_age_hours", "price_tolerance_pct", "volume_threshold", "strength_threshold"]],
            "Breakout Thresholds": [k for k in best_params.keys() if k in ["breakout_threshold", "confirmation_periods", "volume_confirmation", "momentum_threshold", "false_breakout_filter"]],
            "Zone Multipliers": [k for k in best_params.keys() if k in ["support_zone_multiplier", "resistance_zone_multiplier", "sr_zone_threshold", "zone_expansion_factor", "zone_contraction_factor"]],
            "Confidence Thresholds": [k for k in best_params.keys() if k in ["min_sr_confidence", "high_confidence_threshold", "confidence_decay_rate", "regime_confidence_boost", "ensemble_confidence_threshold"]]
        }
...
async def main():
    ...
    try:
        ...
        # Export parameters
        param_file = runner.export_parameters(f"{args.output_dir}/optimized_sr_parameters.json")
