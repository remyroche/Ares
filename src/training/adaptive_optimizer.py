...
class MarketRegime:
    ...
    def __init__(self, name: str, volatility: float, trend_strength: float, regime_type: str, optimal_params: dict[str, Any]) -> None:
        self.name = name
...
class AdaptiveOptimizer:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.regime_detection_config = config.get('regime_detection', {'lookback_window': 50, 'volatility_threshold': 0.02, 'trend_threshold': 0.01})
    ...
    def _get_regime_optimal_params(self, regime_type: str, features: dict[str, float]) -> dict[str, Any]:
    ...
    def _calculate_regime_confidence(self, features: dict[str, float], regime_type: str) -> float:
    ...
    def _update_regime_performance(self, regime_name: str, results: dict[str, Any]) -> None:
        ...
        self.regime_performance[regime_name].append({'timestamp': pd.Timestamp.now(), 'score': results.get('best_score', 0), 'params': results.get('best_params', {})})

    def get_regime_insights(self) -> dict[str, Any]:
        """Get insights about regime performance."""
        insights = {'current_regime': self.current_regime.name if self.current_regime else None, 'regime_performance': {}, 'optimal_regime_params': {}}
        for regime_name, performance_history in self.regime_performance.items():
            if performance_history:
                scores = [p['score'] for p in performance_history]
                insights['regime_performance'][regime_name] = {'avg_score': np.mean(scores), 'best_score': np.max(scores), 'num_optimizations': len(performance_history)}
...
class RegimeSpecificOptimizer:
    ...
    def _get_regime_constraints(self, regime: MarketRegime) -> dict[str, Any]:
        ...
        if regime.regime_type == 'trending':
            return {'tp_multiplier_range': (2.0, 5.0), 'sl_multiplier_range': (1.0, 2.5), 'position_size_range': (0.08, 0.25)}
        if regime.regime_type == 'ranging':
            return {'tp_multiplier_range': (1.5, 3.0), 'sl_multiplier_range': (0.8, 1.5), 'position_size_range': (0.05, 0.15)}
        if regime.regime_type == 'volatile':
            return {'tp_multiplier_range': (3.0, 6.0), 'sl_multiplier_range': (1.5, 3.0), 'position_size_range': (0.03, 0.12)}
        return {'tp_multiplier_range': (1.5, 4.0), 'sl_multiplier_range': (1.0, 2.0), 'position_size_range': (0.05, 0.2)}
    ...
    def _evaluate_regime_parameters(self, params: dict[str, Any], market_data: pd.DataFrame) -> float:
