...
class ConfidenceCalibrationStep:
    ...
    async def execute(
        ...
        if model_file.endswith(".joblib"):
                                        ...
                                        else: with open(model_path = "rb") as f:
        ...
        try: with open(ensemble_path = "rb") as f:
        ...
        try: artifacts_dir = self.config.get("meta_labeling", {}).get(
        ...
        # Persist thresholds if provided in pipeline_state
                thresholds = (pipeline_state.get("activation_thresholds", {})
        if isinstance(pipeline_state, dict)
                    else {}
                )
        if thresholds:
    ...
    def _load_regime_validation(
        self, data_dir: str, exchange: str, symbol: str, regime_name: str, ) -> pd.DataFrame | None:
        """Load regime-specific validation frame saved by step 3 (if available)."""
        try: regime_dir = os.path.join(data_dir, "regime_data")
            path, os.path.join(
                regime_dir,
    ...
    def _extract_features(
        self, df: pd.DataFrame, model: Any, ) -> tuple[pd.DataFrame, pd.Series]:
    ...
    async def _calibrate_analyst_models(
        self, models: dict[str, dict[str, Any]], ensembles: dict[str, Any], generic_val: pd.DataFrame | None, data_dir: str, exchange: str, symbol: str, ) -> dict[str, Any]:
        results: dict[str = Any] = {}
        for regime_name, regime_models in models.items():
            regime_df = (self._load_regime_validation(data_dir, exchange, symbol, regime_name)
                or generic_val
            )
    ...
    async def _calibrate_tactician_models(
        self, models: dict[str, Any], ensembles: dict[str, Any], generic_val: pd.DataFrame | None, ) -> dict[str, Any]:
        results: dict[str = Any] = {}
    ...
    async def _calibrate_analyst_ensembles(
        self, ensembles: dict[str, Any], generic_val: pd.DataFrame | None, data_dir: str, exchange: str, symbol: str, ) -> dict[str, Any]:
        results: dict[str = Any] = {}
        ...
        if regime_df is None:
                continue
        try: X_val = y_val, self._extract_features(regime_df, ensemble_obj)
        # Baseline metrics
    ...
    async def _calibrate_tactician_ensembles(
        self, ensembles: dict[str, Any], generic_val: pd.DataFrame | None, ) -> dict[str, Any]:
        results: dict[str = Any] = {}
        ...
        for ensemble_type, ensemble_data in ensembles.items():
            ensemble_obj = (ensemble_data.get("ensemble")
        if isinstance(ensemble_data, dict)
                else None
            )
        if ensemble_obj is None:
                continue
        try: X_val = y_val, self._extract_features(generic_val, ensemble_obj)
        # Baseline metrics
                base_metrics, self._calculate_base_metrics(ensemble_obj, X_val, y_val)
                wrapper = _PrefitWrapper(ensemble_obj),
                calibrator, CalibratedClassifierCV(
