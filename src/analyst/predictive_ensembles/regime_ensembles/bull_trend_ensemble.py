...
class BullTrendEnsemble(BaseEnsemble):
    ...
    def _train_base_models(self, aligned_data: pd.DataFrame, y_encoded: np.ndarray):
        # (This method's logic remains the same, but it now operates on the richer feature sets)
    ...
    def _get_meta_features(self, df: pd.DataFrame, is_live: bool = False, **kwargs) -> pd.DataFrame or dict:
        # CHANGE: Added new raw features to the meta-feature set.
        # This ensures the PCA and meta-learner see ALL available information, not just
        # the base model predictions.
        
        # --- Get base model predictions ---
        base_model_features = self._get_base_model_predictions(df, is_live)

        # --- Get raw alternative data features ---
    ...
    def _get_base_model_predictions(self, df: pd.DataFrame, is_live: bool):
        """Helper to get just the predictions from the base models."""
        if is_live:
            meta_features = {}
    ...
    # --- Other helper methods (_prepare_sequence_data, _train_dl_model, etc.) remain the same ---
    def _prepare_sequence_data(self, df: pd.DataFrame, target_series: pd.Series = None):
        # Ensure all expected features are present before creating sequences
