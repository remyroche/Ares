...
class BacktestingReportGenerator:
    ...
    def _assess_step_quality(self, step_name: str, step_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'errors' in step_results and step_results['errors']:
            quality_flags.append({'type': 'ERROR', 'message': f'Step {step_name} has errors', 'details': step_results['errors']})
            quality_score -= 30
        if 'performance_metrics' in step_results:
            perf = step_results['performance_metrics']
            if 'sharpe_ratio' in perf and perf['sharpe_ratio'] < 1.0:
                quality_flags.append({'type': 'WARNING', 'message': f"Low Sharpe ratio in {step_name}: {perf['sharpe_ratio']:.2f}", 'details': {'sharpe_ratio': perf['sharpe_ratio']}})
                quality_score -= 10
        if 'regime_performance' in step_results:
    ...
    def _generate_step_recommendations(self, step_name: str, step_results: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        if 'performance_metrics' in step_results:
            perf = step_results['performance_metrics']
            if 'sharpe_ratio' in perf and perf['sharpe_ratio'] < 1.0:
                recommendations.append({'category': 'PERFORMANCE', 'priority': 'HIGH', 'recommendation': f'Improve Sharpe ratio in {step_name} - consider parameter optimization or feature engineering', 'action': 'Review model parameters and feature selection'})
        if 'regime_performance' in step_results:
            for regime, regime_data in step_results['regime_performance'].items():
                if 'regime_return' in regime_data and regime_data['regime_return'] < 0:
    ...
    def _generate_step_troubleshooting(self, step_name: str, step_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        troubleshooting = {'common_issues': [], 'debugging_steps': [], 'log_locations': []}
        if step_name == 'walk_forward_validation':
            troubleshooting['common_issues'].extend(['Data gaps in time series', 'Insufficient training data', 'Overfitting in validation'])
            troubleshooting['debugging_steps'].extend(['Check data continuity', 'Verify minimum training window', 'Review validation metrics'])
        elif step_name == 'monte_carlo_validation':
            troubleshooting['common_issues'].extend(['High variance in results', 'Poor convergence', 'Memory issues with large simulations'])
            troubleshooting['debugging_steps'].extend(['Increase simulation count gradually', 'Check random seed consistency', 'Monitor memory usage'])
    ...
    def _assess_data_quality(self, step_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        quality_metrics = {'completeness': 100, 'consistency': 100, 'accuracy': 100, 'issues': []}
...
def generate_step_report(step_name: str, step_results: Dict[str, Any], symbol: str, timeframe: str, data_dir: str, output_file: Optional[str]=None) -> Dict[str, Any]:
    """Generate detailed report for a specific backtesting step."""
...
class ComprehensiveReporter:
    ...
    def _generate_backtesting_results(self, pipeline_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        backtesting_results = {'overall_performance': {}, 'walk_forward_results': {}, 'monte_carlo_results': {}, 'ab_testing_results': {}, 'performance_summary': {}}
    ...
    def _generate_regime_analysis(self, pipeline_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        regime_analysis = {'regime_identification': {}, 'regime_performance': {}, 'regime_transitions': {}, 'regime_stability': {}, 'regime_recommendations': []}
    ...
    def _generate_model_performance_analysis(self, pipeline_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        model_analysis = {'model_accuracy': {}, 'model_confidence': {}, 'feature_importance': {}, 'model_comparison': {}, 'model_recommendations': []}
        for result_type, results in pipeline_results.items():
            if results and isinstance(results, dict):
    ...
    def _generate_risk_analysis(self, pipeline_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        risk_analysis = {'portfolio_risk': {}, 'regime_risk': {}, 'model_risk': {}, 'liquidity_risk': {}, 'concentration_risk': {}, 'risk_recommendations': []}
        for result_type, results in pipeline_results.items():
            if results and isinstance(results, dict):
    ...
    def _generate_data_quality_report(self) -> Dict[str, Any]:
        ...
        data_files = [f'aggtrades_{self.exchange}_{self.symbol}_consolidated.parquet', f'volume_{self.exchange}_{self.symbol}_consolidated.parquet']
        quality_report = {'data_files_status': {}, 'data_quality_metrics': {}, 'data_issues': []}
            ...
            if safe_file_exists(file_path):
                try:
                    ...
                    if quality_metrics['missing_values'] > 0:
                        quality_report['data_issues'].append({'file': file_name, 'issue': 'Missing values detected', 'count': quality_metrics['missing_values']})
                    if quality_metrics['duplicate_records'] > 0:
                        quality_report['data_issues'].append({'file': file_name, 'issue': 'Duplicate records detected', 'count': quality_metrics['duplicate_records']})
                except Exception as e:
                    quality_report['data_files_status'][file_name] = 'ERROR'
                    quality_report['data_issues'].append({'file': file_name, 'issue': f'Error reading file: {e}', 'count': 0})
            else:
    ...
    def _generate_recommendations(self, pipeline_results: Dict[str, Any], logger_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if len(quality_flags) > 5:
            recommendations.append({'category': 'Quality', 'priority': 'HIGH', 'title': 'Address Quality Flags', 'description': f'Found {len(quality_flags)} quality flags. Review and address quality issues.', 'action': 'Review quality flags in the report and implement fixes'})
        step_times = logger_data.get('step_times', {})
        if step_times:
            slowest_step = max(step_times.items(), key=lambda x: x[1])
            if slowest_step[1] > 300:
                recommendations.append({'category': 'Performance', 'priority': 'MEDIUM', 'title': 'Optimize Slow Step', 'description': f'Step "{slowest_step[0]}" took {slowest_step[1]:.2f} seconds.', 'action': f'Consider optimizing the {slowest_step[0]} step for better performance'})
        data_issues = self._generate_data_quality_report().get('data_issues', [])
        if data_issues:
            recommendations.append({'category': 'Data Quality', 'priority': 'HIGH', 'title': 'Fix Data Quality Issues', 'description': f'Found {len(data_issues)} data quality issues.', 'action': 'Review and fix data quality issues before running backtesting'})
        performance_metrics = logger_data.get('performance_metrics', {})
        ...
        if step_times:
            ...
            for step, time_taken in step_times.items():
                if time_taken > avg_time * 2:
                    bottlenecks.append({'type': 'SLOW_STEP', 'step': step, 'time_taken': time_taken, 'severity': 'HIGH' if time_taken > avg_time * 3 else 'MEDIUM'})
        if performance_metrics:
    ...
    def _analyze_validation_step(self, results: Any, step_name: str) -> Dict[str, Any]:
        ...
        if results is None:
            return {'status': 'NOT_EXECUTED', 'message': f'{step_name} validation was not executed'}
    ...
    def _extract_performance_metrics(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metrics = {}
        performance_keys = ['total_return', 'sharpe_ratio', 'win_rate', 'max_drawdown', 'total_trades', 'avg_trade_return', 'profit_factor', 'volatility', 'var_95', 'calmar_ratio', 'sortino_ratio', 'information_ratio']
    ...
    def _calculate_overall_performance_summary(self, backtesting_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = {'best_performing_method': None, 'worst_performing_method': None, 'average_sharpe_ratio': 0.0, 'average_return': 0.0, 'consistency_score': 0.0}
        all_metrics = []
    ...
    def _analyze_regime_stability(self, regime_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        stability = {'regime_persistence': {}, 'transition_frequency': {}, 'regime_volatility': {}, 'stability_score': 0.0}
        ...
        if 'regime_performance' in regime_analysis:
            for regime, performance in regime_analysis['regime_performance'].items():
                if 'regime_return' in performance and performance['regime_return'] < 0:
                    recommendations.append({'type': 'REGIME_PERFORMANCE', 'regime': regime, 'issue': f'Negative returns in {regime}', 'recommendation': f'Review strategy for {regime} regime or consider regime-specific adjustments'})
    ...
    def _compare_model_performance(self, model_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        comparison = {'best_model': None, 'model_rankings': [], 'performance_gaps': {}, 'consistency_analysis': {}}
        models = []
    ...
    def _generate_model_recommendations(self, model_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if 'model_accuracy' in model_analysis:
            for model_name, accuracy in model_analysis['model_accuracy'].items():
                if accuracy < 0.6:
                    recommendations.append({'type': 'MODEL_ACCURACY', 'model': model_name, 'issue': f'Low accuracy: {accuracy:.2%}', 'recommendation': f'Consider retraining {model_name} or adjusting hyperparameters'})
    ...
    def _calculate_liquidity_risk(self, pipeline_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        liquidity_risk = {'volume_analysis': {}, 'spread_analysis': {}, 'execution_risk': {}, 'liquidity_score': 0.0}
        for result_type, results in pipeline_results.items():
            if results and isinstance(results, dict):
    ...
    def _calculate_concentration_risk(self, pipeline_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        concentration_risk = {'position_concentration': {}, 'regime_concentration': {}, 'time_concentration': {}, 'concentration_score': 0.0}
        for result_type, results in pipeline_results.items():
            if results and isinstance(results, dict):
    ...
    def _generate_risk_recommendations(self, risk_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if 'portfolio_risk' in risk_analysis:
            ...
            if 'var_95' in portfolio_risk and portfolio_risk['var_95'] > 0.05:
                recommendations.append({'type': 'PORTFOLIO_RISK', 'issue': f"High VaR: {portfolio_risk['var_95']:.2%}", 'recommendation': 'Consider reducing position sizes or improving risk management'})
