...
## 2025-05-14 - [Index Alignment] Learning: [Vectorized Index Match Types] Action: [When refactoring `.iterrows()` with `index.intersection()`, always explicitely cast index columns (e.g., timestamps to UTC datetime64) before `.set_index()` to prevent silent empty intersections due to type mismatch.]
