...
class HMMRegimeAnalyzer:
    ...
    async def analyze(self, features: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': True, 'n_states': self.n_regimes, 'regime_labels': states, 'regime_probabilities': state_probs, 'transition_matrix': self.hmm_model.transmat_, 'model_score': score, 'means': self.hmm_model.means_, 'covariances': self.hmm_model.covars_}
        except ImportError:
...
class RegimeCharacterizer:
    ...
    async def characterize(self, features: pd.DataFrame, hmm_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for regime in range(n_regimes):
            ...
            if np.any(mask):
                ...
                regime_stats = {'count': int(np.sum(mask)), 'percentage': float(np.mean(mask) * 100), 'duration_stats': self._calculate_duration_stats(regime_labels, regime)}
    ...
    def _analyze_transitions(self, regime_labels: np.ndarray, transition_matrix: Optional[np.ndarray]) -> Dict[str, Any]:
        ...
        for i in range(len(regime_labels) - 1):
            if regime_labels[i] != regime_labels[i + 1]:
                transitions.append({'from': int(regime_labels[i]), 'to': int(regime_labels[i + 1]), 'index': i})
        trans_freq = {}
        for trans in transitions:
            key = f"{trans['from']}_to_{trans['to']}"
            trans_freq[key] = trans_freq.get(key, 0) + 1
        return {'total_transitions': len(transitions), 'transition_rate': len(transitions) / len(regime_labels) if len(regime_labels) > 0 else 0, 'frequencies': trans_freq, 'matrix': transition_matrix.tolist() if transition_matrix is not None else None}