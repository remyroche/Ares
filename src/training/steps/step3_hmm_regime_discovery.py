...
BLOCKS: List[BlockConfig] = [
    # 1. MOMENTUM BLOCK - "How fast is the market moving?"
    ...
    # 2. VOLATILITY BLOCK - "How chaotic is the market?"
    ...
    # 3. VOLUME BLOCK - "How much conviction is behind the move?"
    BlockConfig("volume", 5, 4),  # 5 states for volume patterns
    # Features: volume_*, vwap_*, volume_zscore, volume_ratio_*, trade_*
    
    # 4. MARKET_MICROSTRUCTURE BLOCK - "What is the order flow telling us?"
    BlockConfig("market_microstructure", 4, 3),  # 4 states for microstructure patterns
    # Features: order_flow_*, trade_*, bid_ask_*, spread_*, market_impact
    
    # 5. SUPPORT_RESISTANCE BLOCK - "Where is the price in relation to the map?" and "Is the map changing?"
    BlockConfig("support_resistance", 3, 2),  # 3 states for SR patterns
    # Features: sr_score (position relative to map), delta_sr_score (map changes)
]
...
def _assign_block(feature_name: str) -> str:
    """
    Assign a feature to the appropriate expert block based on its characteristics.
    
    This function categorizes features into expert blocks for HMM training, where each block
    represents a unique, non-redundant dimension of market information:
    ...
    - SUPPORT_RESISTANCE: "Where is the price in relation to the map?" and "Is the map changing?"
    
    This expert panel approach ensures each block provides distinct, non-overlapping insights
    into market behavior, preventing redundancy and improving HMM regime detection.
    """
    ...
    volatility_patterns = [
        ...
        "chaos", "dispersion", "spread", "range", "deviation", "noise", "turbulence"
    ]
    ...
    volume_patterns = [
        ...
        "conviction", "participation", "commitment", "strength"
    ]
    ...
    microstructure_patterns = [
        # Order flow features
        "order_flow", "order_imbalance", "order_book", "bid_ask", "spread",
        "market_impact", "slippage", "liquidity", "depth", "resilience",
        
        # Trade microstructure
        "trade_size", "trade_frequency", "trade_interval", "trade_clustering",
        "trade_aggression", "trade_direction", "trade_flow",
        
        # Market quality
        "market_quality", "efficiency", "friction", "noise", "signal"
    ]
    
    for pattern in microstructure_patterns:
        if pattern in feature_name_lower:
...
def _select_block_features(
    ...
    """
    Select and prepare features for a specific expert block.
    
    Each block represents a unique market expert providing non-redundant insights:
    - MOMENTUM: Speed and direction of market movement
    - VOLATILITY: Market chaos and uncertainty
    - VOLUME: Conviction and participation behind moves
    - MARKET_MICROSTRUCTURE: Order flow and market structure
    - SUPPORT_RESISTANCE: Price position relative to key levels and level changes
    """
    ...
    logger.info(f"🔍 Available features for {block} expert:")
    ...
    logger.info(f"📊 Selected {len(cols)} features for {block} expert: {cols}")

    if not cols:
        logger.warning(f"⚠️ No features found for {block} expert")
    ...
    if X.empty:
        logger.warning(f"⚠️ No features remaining for {block} expert after dropping constants")
        return pd.DataFrame(index=full_df.index)

    # Apply correlation pruning with expert-specific thresholds
    if block == "momentum":
        correlation_threshold = 0.98  # Less aggressive for momentum expert to preserve diversity
    else:
        correlation_threshold = 0.95  # Standard threshold for other experts
    
    logger.info(f"🔧 Using correlation threshold {correlation_threshold} for {block} expert")
    ...
    if X.empty:
        logger.warning(f"⚠️ No features remaining for {block} expert after correlation pruning")
        return pd.DataFrame(index=full_df.index)
    ...
    logger.info(f"✅ Final {block} expert: {len(X.columns)} features")
    return X
...
def _fit_block_hmm_robust(
    ...
    """
    Fit HMM model for a specific market expert with robust training and quality validation.

    Each expert provides a unique, non-redundant perspective on market behavior:
    - MOMENTUM: Speed and direction of market movement
    - VOLATILITY: Market chaos and uncertainty  
    - VOLUME: Conviction and participation behind moves
    - MARKET_MICROSTRUCTURE: Order flow and market structure
