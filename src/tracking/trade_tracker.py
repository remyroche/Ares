...
class TradeTracker:
    """
    Comprehensive trade tracking system with model ensemble data,
    regime analysis, feature importance, decision path, and model behavior monitoring.
    """
    ...
    async def record_trade(
        ...
        """Record a new trade with all associated data."""
        try:
            # Generate trade ID
    ...
    def _update_performance_metrics(self, trade_record: TradeRecord) -> None:
        """Update performance metrics with trade data."""
    ...
    async def _track_model_performance(self, trade_record: TradeRecord) -> None:
        """Track model performance for a trade."""
    ...
    def get_model_performance_summary(self) -> Dict[str, Any]:
        """Get summary of model performance across all trades."""
        summary = {}
        ...
        for model_type, history in self.model_performance_history.items():
            ...
            avg_consistency = mean([h["prediction_consistency"] for h in history])
            avg_stability = mean([h["feature_importance_stability"] for h in history])
            avg_drift = mean([h["prediction_drift"] for h in history])
    ...
    def get_feature_importance_analysis(
        ...
        for item in feature_importance_data:
            ...
            feature_scores[feature_name].append(item["importance_score"])

        # Calculate top features
        top_features = {}
        for feature_name, scores in feature_scores.items():
            top_features[feature_name] = mean(scores)

        # Sort by importance and take top 10
        sorted_features = sorted(top_features.items(), key=lambda x: x[1], reverse=True)
        top_features = dict(sorted_features[:10])

        # Calculate feature stability (standard deviation)
        feature_stability = {}
        for feature_name, scores in feature_scores.items():
            if len(scores) > 1:
                feature_stability[feature_name] = stdev(scores)
            else:
                feature_stability[feature_name] = 0.0

        # Group by model type
        model_performance_by_feature = {}
        for item in feature_importance_data:
            model_type = item["model_type"]
            if model_type not in model_performance_by_feature:
                model_performance_by_feature[model_type] = {
                    "top_features": {},
                    "feature_count": 0
                }

        # Calculate top features per model
        for model_type in model_performance_by_feature:
            model_data = [item for item in feature_importance_data if item["model_type"] == model_type]
            model_feature_scores = {}
            
            for item in model_data:
                feature_name = item["feature_name"]
                if feature_name not in model_feature_scores:
                    model_feature_scores[feature_name] = []
                model_feature_scores[feature_name].append(item["importance_score"])

            # Calculate average scores and take top 5
            model_top_features = {}
            for feature_name, scores in model_feature_scores.items():
                model_top_features[feature_name] = mean(scores)

            sorted_model_features = sorted(model_top_features.items(), key=lambda x: x[1], reverse=True)
            model_performance_by_feature[model_type]["top_features"] = dict(sorted_model_features[:5])
            model_performance_by_feature[model_type]["feature_count"] = len(model_feature_scores)

        analysis = {
            "total_features_tracked": len(feature_scores),
            "top_features": top_features,
            "feature_stability": feature_stability,
            "model_performance_by_feature": model_performance_by_feature,
        }
    ...
    def get_decision_path_analysis(self) -> Dict[str, Any]:
        ...
        analysis["common_decision_steps"] = dict(step_counts.most_common(10))
        ...
        if all_weights:
            analysis["decision_weights_distribution"] = {
                ...
                "std": stdev(all_weights) if len(all_weights) > 1 else 0.0,
                "min": min(all_weights),
                "max": max(all_weights),
            }
    ...
    def get_regime_analysis_summary(self) -> Dict[str, Any]:
        """Get summary of regime analysis across trades."""
        ...
        regime_counts = Counter(regime_types)
        analysis["regime_distribution"] = dict(regime_counts)
        ...
        if transition_probs:
            analysis["regime_transition_analysis"] = {
                "mean_transition_probability": mean(transition_probs),
    ...
    async def cleanup_old_records(self, max_age_days: int = 30) -> int:
        """Clean up old trade records to manage memory usage."""
