...
def load_missing_dates():
    """Load missing dates from the file generated by identify_deleted_aggtrades.py"""
    try:
    pass
    ...
    pass
        with open("missing_aggtrades_dates.txt", "r") as f:
    ...
    pass
        ...
        for col , expected_dtype in expected_dtypes.items():
            if str(df[col].dtype) != expected_dtype:
                print(f"❌ Invalid dtype for {col} in {file_path}")
...
def fix_parquet_format(file_path: str) -> bool:
    ...
    pass
        ...
        print(f"❌ Error fixing format for {file_path}: {e}")
        return False
    ...
    pass
        ...
        if success:
            ...
            if not validate_parquet_format(parquet_path):
                print(f"🔧 Fixing format for {date_str}")
                if fix_parquet_format(parquet_path):
                    print(f"✅ Format fixed for {date_str}")
                else:
                    print(f"❌ Failed to fix format for {date_str}")
                    return False
            else:
                print(f"✅ Format validation passed for {date_str}")
        else:
...
async def main():
    """Main function to download missing aggtrades data"""
    global shutdown_requested
