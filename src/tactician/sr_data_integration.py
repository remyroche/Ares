...
This module integrates S/R backtesting validation with proper data access patterns from
ares_launcher, including lookback period management and data loading. It ensures the S/R
system uses the same data sources and configurations as the main trading system.
"""
...
class SRDataIntegration:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.logger = (
            system_logger.getChild("SRDataIntegration") if system_logger else None
        )
        ...
        self.lookback_days = self.data_config.get(
        ...
        try:
            ...
            for timeframe in self.timeframes:
                if not await self._check_timeframe_data_availability(timeframe):
                    if self.logger:
                        self.logger.warning(
                            f"⚠️ Data not available for {timeframe}, attempting download..."
                        )
                    ...
                    if not await self._download_timeframe_data(timeframe):
                        if self.logger:
                            self.logger.error(
                                f"❌ Failed to obtain data for {timeframe}"
                            )
                        return False
    ...
    async def _check_timeframe_data_availability(self, timeframe: str) -> bool:
        ...
        try:
            ...
            if self.logger:
                self.logger.debug(
                    f"Data availability check failed for {timeframe}: {e}"
                )
            return False
        ...
        try:
            ...
            if DATA_DOWNLOADER_AVAILABLE and download_all_data_with_consolidation:
                success = await download_all_data_with_consolidation(
                    symbol=self.symbol, exchange_name=self.exchange, interval=timeframe
                )
                ...
                if self.logger:
                    self.logger.warning(
                        f"⚠️ Data downloader not available for {timeframe}"
                    )
                return False
    ...
    async def get_market_data(
        ...
        try:
            ...
            if not force_reload and cache_key in self._data_cache:
                ...
                if (
                    last_load and (datetime.now() - last_load).total_seconds() < 3600
                ):  # 1 hour cache
                    if self.logger:
            ...
            if data is not None and len(data) > 0:
                ...
                if self.logger:
                    self.logger.info(
                        f"📊 Loaded {len(data)} data points for {timeframe} ({actual_lookback_days} days lookback)"
                    )
    ...
    async def _load_timeframe_data(
        self, timeframe: str, lookback_days: int
    ) -> Optional[pd.DataFrame]:
    ...
    async def get_multi_timeframe_data(
        ...
        try:
            ...
            if self.logger:
                self.logger.info(
                    f"📊 Loading multi-timeframe data for {len(timeframes)} timeframes"
                )
    ...
    def get_lookback_period_for_timeframe(self, timeframe: str) -> int:
        ...
        try:
            ...
            if self.logger:
                self.logger.error(
                    f"❌ Failed to get lookback period for {timeframe}: {e}"
                )
        ...
        try:
            ...
            if "timestamp" in data.columns:
                ...
                if large_gaps.sum() > len(data) * 0.1:  # More than 10% gaps
                    if self.logger:
                        self.logger.warning(
                            f"⚠️ Large data gaps detected in {timeframe}"
                        )
    ...
    async def cleanup_cache(self) -> None:
        ...
        try:
            ...
            if self.logger:
                self.logger.info(
                    f"✅ Cache cleanup completed, removed {len(keys_to_remove)} entries"
                )
    ...
    try:
        ...
        if lookback_days is None:
            try:
                ...
                # Fallback if TRAINING_MODES is not available
                lookback_days = DEFAULT_LOOKBACK_DAYS
