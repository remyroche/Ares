...
class TripleBarrierMethodStep:
    ...
    async def _log_step4_artifacts_and_report(
        self, symbol: str, exchange: str, timeframe: str, data_dir: str, result_data: pd.DataFrame, output_path: Path
    ) -> None:
        ...
        try:
            ...
            metrics_calculated = {
                ...
                "label_distribution": (
                    result_data["label"].value_counts().to_dict()
                    if result_data is not None and "label" in result_data.columns
                    else {}
                ),
            }
            ...
            if result_data is not None:
                artifact_name = log_step_dataframe_with_standardized_name(
                    ...
                    additional_metadata={
                        ...
                        "label_distribution": (
                            result_data["label"].value_counts().to_dict() if "label" in result_data.columns else {}
                        ),
                        "asset": symbol,
                        "lookback_period": self.config.get("lookback_days", 1095),
                        "project_version": self.config.get("project_version", "1.0.0"),
    ...
    def _create_enhanced_labels(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            enhanced_data["profit_category"] = pd.cut(
                enhanced_data["potential_profit_pct"], bins=profit_bins, labels=profit_labels, include_lowest=True
            )

            # Create combined direction-profit labels
            enhanced_data["direction_profit_label"] = enhanced_data.apply(
                lambda row: f"{'LONG' if row['label'] == 1 else 'SHORT'}_{row['profit_category']}", axis=1
            )
            ...
            self.logger.info(
                f"   - Risk-adjusted profit range: {
                    enhanced_data['risk_adjusted_profit'].min():.4f} to {
                    enhanced_data['risk_adjusted_profit'].max():.4f}"
            )
