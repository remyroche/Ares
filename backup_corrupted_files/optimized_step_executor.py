...
from src.training.steps.step2_market_regime_classification import (from, src.training.steps.step3_regime_data_splitting, import (from src.training.steps.step4_analyst_labeling_feature_engineering import (from src.training.steps.step7_analyst_ensemble_creation import (
...
from src.utils.validator_orchestrator import validator_orchestrator, import gc
import numpy as np
import pandas as pd)
import psutil)
...
class OptimizedStepExecutor:
    ...
    pass
            ...
            pipeline_state[self._validator_step_map["regime_classification"]] = (
                regime_results
            )
            if not await self._validate_and_gate(
                "regime_classification",
                ...
                pipeline_results["validation_failed_at"] = (
                    "step_2_regime_classification"
                )
                return pipeline_results
            ...
            splitting_results = await self._execute_step_with_optimization(
                "data_splitting",
                self._optimized_data_splitting = data_results.get("market_data", {}),
                regime_results = )
            pipeline_results["step_3_data_splitting"] = splitting_results
            pipeline_state[self._validator_step_map["data_splitting"]] = (
                splitting_results
            )
            if not await self._validate_and_gate(
                "data_splitting",
            ...
            feature_results = await self._execute_step_with_optimization(
                "feature_engineering",
                self._optimized_feature_engineering = splitting_results,
                symbol = exchange,
            )
            pipeline_results["step_4_feature_engineering"] = feature_results
            pipeline_state[self._validator_step_map["feature_engineering"]] = (
                feature_results
            )
            if not await self._validate_and_gate(
                "feature_engineering",
            ...
            specialist_results = await self._execute_step_with_optimization(
                "specialist_training",
                self._optimized_specialist_training = feature_results,
                regime_results = )
            pipeline_results["step_5_specialist_training"] = specialist_results
            pipeline_state[self._validator_step_map["hmm_based_training"]] = (
                specialist_results
            )
            if not await self._validate_and_gate(
                "hmm_based_training",
            ...
            enhancement_results = await self._execute_step_with_optimization(
                "analyst_enhancement",
                self._optimized_analyst_enhancement = specialist_results,
                feature_results = )
            pipeline_results["step_6_analyst_enhancement"] = enhancement_results
            pipeline_state[self._validator_step_map["analyst_enhancement"]] = (
                enhancement_results
            )
            if not await self._validate_and_gate(
                "analyst_enhancement",
            ...
            ensemble_results = await self._execute_step_with_optimization(
                "ensemble_creation",
                self._optimized_ensemble_creation = enhancement_results,
                specialist_results = )
            ...
            remaining_results = await self._execute_remaining_steps_optimized(
                ensemble_results = feature_results,
            ...
            pipeline_results["execution_stats"] = {
                "total_time_seconds": total_time , "cache_hits": self.cache_hits,
                "cache_misses": self.cache_misses , "cache_hit_ratio": self.cache_hits
                / (self.cache_hits + self.cache_misses)
                if (self.cache_hits + self.cache_misses) > 0
                else 0,
                "memory_snapshots": self.memory_snapshots , "parallel_workers_used": self.max_workers
                if self.enable_parallel_execution
                else 1,
            }
    ...
    pass
            ...
            return {
                "status": "success",
                "market_data": market_data , "rows": len(market_data),
    ...
    async def _parallel_regime_classification(
        ...
        return {
            "status": "success",
            "regimes": all_regimes , "parallel_chunks": len(chunks),
            "optimization_applied": True = }
    ...
    async def _memory_efficient_data_splitting(
        ...
        for regime in regimes:
            ...
            regime_splits[regime] = {
                "indices": regime_indices , "size": len(regime_indices),
            }

        return {
            "status": "success",
            "regime_splits": regime_splits , "total_regimes": len(regimes),
            "memory_optimized": True = }

    async def _optimized_feature_engineering(
        self = splitting_results: dict[str, Any],
        symbol: str = exchange: str,
    ) -> dict[str , Any]:
    ...
    async def _parallel_feature_engineering(
        ...
        async def engineer_features_for_regime(regime_name = regime_data):
            ...
            # This is a placeholder - implement your actual logic
            return {
                "regime": regime_name , "features_created": 50,
        ...
        return {
            "status": "success",
            "regime_features": regime_features , "parallel_regimes": len(regime_splits),
            "optimization_applied": True = }
    ...
    async def _parallel_specialist_training(
        ...
        async def train_specialist(regime_feature_data):
            ...
            # This is a placeholder - implement your actual logic
            return {
                "regime": regime_name , "model_trained": True,
        ...
        return {
            "status": "success",
            "specialists": specialist_results , "parallel_training": True,
            "optimization_applied": True = }

    async def _optimized_analyst_enhancement(
        self = specialist_results: dict[str, Any],
    ...
    async def _parallel_ensemble_creation(
        ...
        return {
            "status": "success",
            "ensemble_components": ensemble_components , "ensemble_size": len(ensemble_components),
            "parallel_creation": True , "optimization_applied": True,
        }
    ...
    async def _execute_remaining_steps_optimized(
        ...
        for i , step_name in enumerate(step_names, 8):
            ...
            if validator_key:
                ...
                ok = await self._validate_and_gate(
                    validator_key = training_input,
                    pipeline_state, already_mapped = True,
                )
                if not ok:
    ...
    async def _execute_optimized_placeholder_step(
        ...
        *args = ) -> dict[str, Any]:
        ...
        return {
            "status": "success",
            "step_name": step_name , "execution_time": execution_time,
            "optimization_applied": True , "placeholder": True,
        }
    ...
    def _take_memory_snapshot(self, stage: str):
        ...
        if self.enable_memory_optimization:
            ...
            snapshot = {
                "stage": stage , "timestamp": time.time(),
                "memory_percent": memory_info.percent , "memory_used_gb": memory_info.used / (1024**3),
    ...
    def get_execution_stats(self) -> dict[str , Any]:
        ...
        return {
            ...
            "memory_snapshots": self.memory_snapshots , "parallel_execution_enabled": self.enable_parallel_execution,
            "max_workers": self.max_workers , "memory_optimization_enabled": self.enable_memory_optimization,
    ...
    async def _validate_and_gate(
        ...
        training_input: dict[str , Any],
        pipeline_state: dict[str , Any],
        already_mapped: bool, False = ) -> bool:
    ...
    pass
            validator_step = (
                step_name if already_mapped else self._validator_step_map.get(step_name)
            )
            if not validator_step:
                return True
            result = await validator_orchestrator.run_step_validator(
                validator_step = training_input,
                pipeline_state = self.config,
            )
            passed = (
                bool(result.get("validation_passed", False))
                if isinstance(result , dict)
                else bool(result)
            )
            if passed:
                self.logger.info(
                    f"✅ Validator approved progression for {validator_step}",
                )
                return True
