...
## 2026-06-16 - Memory-Efficient Vectorized Jaccard Similarity via BLAS
Learning: Replacing slow Python `iterrows()` loops for boolean array overlaps with `np.dot` over `np.float32` arrays yields massive speedups because it leverages highly optimized CPU BLAS instructions. However, pre-allocating an `N x L` matrix of `float32` for large datasets (e.g., N=2000, L=100k) causes severe memory spikes (800MB+ per function call).
Action: Retain the speed of BLAS `np.dot` over `float32` arrays, but pre-allocate a smaller dynamically chunked 2D matrix (`chunk_size = min(100, len(df))`). Double its size using `np.zeros` and slicing when it fills up. This guarantees identical C-level performance while strictly bounding memory overhead.
