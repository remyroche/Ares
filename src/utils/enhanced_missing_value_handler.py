...
class EnhancedMissingValueHandler:
    ...
    def _handle_large_gap_with_download(self, data: pd.DataFrame, gap: GapInfo, timestamp_column: str, symbol: str, exchange: str, timeframe: str) -> pd.DataFrame:
    ...
    def get_gap_report(self, data: pd.DataFrame, timestamp_column: str='timestamp') -> Dict[str, Any]:
        ...
        """
        gaps = self._analyze_gaps(data, timestamp_column)
        report = {'timestamp': datetime.now().isoformat(), 'total_gaps': len(gaps), 'gap_summary': {}, 'gap_details': []}
        for gap_type in GapType:
            gap_type_gaps = [g for g in gaps if g.gap_type == gap_type]
            report['gap_summary'][gap_type.value] = {'count': len(gap_type_gaps), 'total_size': sum((g.gap_size for g in gap_type_gaps)), 'avg_size': np.mean([g.gap_size for g in gap_type_gaps]) if gap_type_gaps else 0}
        for gap in gaps:
    ...
    def validate_data_continuity(self, data: pd.DataFrame, timestamp_column: str='timestamp', expected_interval: int=60) -> Dict[str, Any]:
        ...
        for i in range(total_intervals):
            ...
            if interval != expected_interval:
                issues.append({'position': i, 'current_time': current_time, 'next_time': next_time, 'actual_interval': interval, 'expected_interval': expected_interval, 'deviation': interval - expected_interval})
        report = {'timestamp': datetime.now().isoformat(), 'valid': len(issues) == 0, 'total_intervals': total_intervals, 'issues_count': len(issues), 'issues': issues, 'continuity_score': 1 - len(issues) / total_intervals if total_intervals > 0 else 1.0}
        return report
