...
class ModelOptimizationTarget:
    ...
    hyperparameter_ranges: dict[str, tuple]
...
class ProbabilisticModelIntegrator:
    ...
    def _initialize_model_targets(self) -> dict[str, ModelOptimizationTarget]:
        ...
        return {'tactician': ModelOptimizationTarget(model_type='tactician', model_name='enhanced_prediction_integrator', optimization_objectives=['calibration', 'sharpness', 'discrimination'], hyperparameter_ranges={'barrier_system': {'upper_barrier_multiplier': (0.3, 0.8), 'lower_barrier_multiplier': (0.1, 0.5), 'confidence_threshold': (0.6, 0.9), 'precision_threshold': (0.7, 0.95)}, 'prediction_calibration': {'calibration_method': ['isotonic', 'sigmoid', 'platt'], 'calibration_cv_folds': (3, 10), 'uncertainty_estimation': ['ensemble', 'gaussian', 'conformal']}}, calibration_methods=['isotonic', 'sigmoid', 'platt'], uncertainty_methods=['ensemble', 'gaussian', 'conformal']), 'analyst': ModelOptimizationTarget(model_type='analyst', model_name='ensemble_predictor', optimization_objectives=['calibration', 'sharpness', 'discrimination', 'regime_accuracy'], hyperparameter_ranges={'regime_detection': {'regime_threshold': (0.5, 0.8), 'regime_confidence_threshold': (0.6, 0.9), 'regime_transition_smoothing': (0.1, 0.5)}, 'prediction_calibration': {'calibration_method': ['isotonic', 'sigmoid', 'platt', 'temperature'], 'calibration_cv_folds': (5, 15), 'uncertainty_estimation': ['ensemble', 'gaussian', 'conformal', 'mc_dropout']}}, calibration_methods=['isotonic', 'sigmoid', 'platt', 'temperature'], uncertainty_methods=['ensemble', 'gaussian', 'conformal', 'mc_dropout'])}
    ...
    def _prepare_tactician_optimization_data(self, market_data: pd.DataFrame, historical_predictions: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
    ...
    async def _apply_tactician_optimization_results(self, results: dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.exception(f'Error applying Tactician optimization results: {e}')

    async def _apply_analyst_optimization_results(self, results: dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.exception(f'Error applying Analyst optimization results: {e}')

    async def run_comprehensive_optimization(self, market_data: pd.DataFrame, historical_predictions: pd.DataFrame) -> dict[str, Any]:
        ...
        results = {}
        try:
            tactician_results = await self.optimize_tactician_model(market_data, historical_predictions)
            results['tactician'] = tactician_results
        except Exception as e:
            self.logger.exception(f'Tactician optimization failed: {e}')
            results['tactician'] = {'error': str(e)}
        try:
            analyst_results = await self.optimize_analyst_model(market_data, historical_predictions)
            results['analyst'] = analyst_results
        except Exception as e:
            self.logger.exception(f'Analyst optimization failed: {e}')
        ...
        for model_type, result in results.items():
            ...
            if 'error' in result:
                ...
                if 'calibration' in best_solutions:
                    ...
                    summary['recommendations'].append(f"Use {calib_params.get('calibration_method', 'default')} calibration for {model_type}")
        return summary

    def get_optimization_status(self) -> dict[str, Any]:
        """Get the current status of all optimizations."""
        status = {'optimizers_created': list(self.optimizers.keys()), 'optimization_history': self.optimization_history, 'model_performance': self.model_performance, 'recommendations': []}
        for model_type, history in self.optimization_history.items():
            if 'best_solutions' in history:
                ...
                if 'calibration' in best_solutions:
                    ...
                    if calib_score > 0.1:
                        status['recommendations'].append(f'{model_type.capitalize()} calibration needs improvement (score: {calib_score:.3f})')
...
async def main() -> None:
    ...
    config = {'optimization': {'n_trials': 50, 'n_jobs': 1, 'early_stopping_patience': 10, 'sampler_type': 'tpe'}}
    integrator = ProbabilisticModelIntegrator(config)
    market_data = pd.DataFrame({'close': np.random.randn(1000).cumsum() + 100, 'volume': np.random.uniform(1000, 10000, 1000), 'high': np.random.randn(1000).cumsum() + 101, 'low': np.random.randn(1000).cumsum() + 99})
    historical_predictions = pd.DataFrame({'prediction_accuracy': np.random.uniform(0.5, 0.9, 1000), 'regime_prediction': np.random.uniform(0, 1, 1000)})
    results = await integrator.run_comprehensive_optimization(market_data, historical_predictions)
