...
class DILauncher:
    ...
    def _create_paper_trading_config(self, symbol: str, exchange: str) -> dict[str, Any]:
        ...
        return {'mode': 'paper_trading', 'symbol': symbol, 'exchange': {'name': exchange, 'testnet': True, 'paper_trading': True}, 'state': {'persistence': 'memory', 'backup_enabled': False}, 'performance': {'tracking_enabled': True, 'reporting_interval': 60}, 'use_modular_components': True}

    def _create_live_trading_config(self, symbol: str, exchange: str) -> dict[str, Any]:
        """Create configuration for live trading mode."""
        return {'mode': 'live_trading', 'symbol': symbol, 'exchange': {'name': exchange, 'testnet': False, 'paper_trading': False}, 'state': {'persistence': 'database', 'backup_enabled': True}, 'performance': {'tracking_enabled': True, 'reporting_interval': 30}, 'use_modular_components': True}
    ...
    def get_status(self) -> dict[str, Any]:
        """Get launcher status."""
        return {'is_running': self.is_running, 'components': list(self.system_components.keys()), 'config': self.config}