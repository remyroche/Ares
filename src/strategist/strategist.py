import asyncio
import time
from typing import Dict, Any, Optional  # Ensure Union is imported
import pandas as pd  # Added for pd.isna

from src.utils.logger import system_logger
from src.config import settings
from src.utils.state_manager import StateManager
from exchange.binance import BinanceExchange  # Import if needed for type hinting
...
class Strategist:
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=None, context="strategist_start"
    )
        ...
        while True:
            try:
                ...
                if (
                    ...
                    self.logger.info(
                        "New analyst intelligence detected. Running strategy formulation."
                    )
                ...
                self.logger.error(
                    f"An error occurred in the Strategist loop: {e}", exc_info=True
                )
    ...
    async def formulate_strategy(self, analyst_intelligence: Dict[str, Any]):
        ...
        try:
            ...
            tech_analysis = analyst_intelligence.get(
                "technical_signals", {}
            )  # Corrected key

            # Determine positional bias using new detailed data
            positional_bias = self._determine_positional_bias(
                market_regime, tech_analysis
            )
            ...
            vol_targeting_multiplier = self._get_volatility_targeting_multiplier(
                analyst_intelligence
            )
            adjusted_leverage_cap = self._apply_volatility_targeting(
                leverage_cap, vol_targeting_multiplier
            )
            adjusted_notional_size = self._apply_volatility_targeting(
                max_notional_size, vol_targeting_multiplier
            )
            ...
            self.logger.info(
                f"Strategy formulated. Bias: {positional_bias}, Leverage Cap: {adjusted_leverage_cap}x"
            )
    ...
    def _determine_leverage_cap(
        self, liquidation_risk: float, confidence: float
    ) -> int:
    ...
    def _get_volatility_targeting_multiplier(self, analyst_intelligence: Dict) -> float:
        ...
        try:
            ...
            self.logger.debug(
                f"Volatility targeting multiplier ({vol_method}): {multiplier}"
            )
            return multiplier
    ...
    def _apply_volatility_targeting(
        self, base_value: float, vol_multiplier: float
    ) -> float:
    ...
    def _determine_positional_bias(
        self, market_regime: str, tech_analysis: Dict
    ) -> str:
        """Determines the trading bias using a combination of regime, technicals, and enhanced indicators."""

        regime_bias_map = self.config.get(
            "regime_to_bias_map", {"BULL_TREND": "LONG", "BEAR_TREND": "SHORT"}
        )
        ...
        long_weight = sum(
            weight for bias, weight in bias_weights.items() if bias == "LONG"
        )
        short_weight = sum(
            weight for bias, weight in bias_weights.items() if bias == "SHORT"
        )
