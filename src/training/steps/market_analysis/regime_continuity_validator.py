...
class RegimeContinuityValidator:
    ...
    def __init__(self, config: Dict[str, Any]=None) -> None:
        ...
        self.validation_config = self.config.get('regime_continuity_validation', {'min_regime_coverage': 0.8, 'max_regime_failure_rate': 0.2, 'min_data_continuity': 0.9, 'max_temporal_gaps': 0.1, 'correlation_threshold': 0.7, 'feature_consistency_threshold': 0.8})

    @traced(span_name='validate_step_continuity')
    async def validate_step_continuity(self, step_name: str, symbol: str, exchange: str, timeframe: str, data_dir: str) -> ContinuityValidationResult:
    ...
    async def _validate_regime_coverage(self, step_name: str, symbol: str, exchange: str, timeframe: str, data_dir: str) -> Dict[str, Any]:
        ...
        try:
            if not regime_continuity_manager.regime_metadata:
                return {'is_valid': False, 'issues': ['No regime metadata available'], 'coverage_rate': 0.0}
            total_regimes = len(regime_continuity_manager.regime_metadata)
            if step_name not in regime_continuity_manager.step_contexts:
                return {'is_valid': False, 'issues': [f'No step contexts found for {step_name}'], 'coverage_rate': 0.0}
            ...
            return {'is_valid': len(issues) == 0, 'issues': issues, 'coverage_rate': coverage_rate, 'failure_rate': failure_rate, 'total_regimes': total_regimes, 'completed_regimes': completed_regimes, 'failed_regimes': failed_regimes}
        except Exception as e:
            self.logger.error(f'❌ Error validating regime coverage: {e}')
            return {'is_valid': False, 'issues': [f'Coverage validation error: {str(e)}'], 'coverage_rate': 0.0}

    @traced(span_name='validate_data_continuity')
    async def _validate_data_continuity(self, step_name: str, symbol: str, exchange: str, timeframe: str, data_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not regime_files:
                return {'is_valid': False, 'issues': [f'No regime-specific outputs found for {step_name}'], 'continuity_rate': 0.0}
            ...
            return {'is_valid': len(issues) == 0, 'issues': issues, 'continuity_rate': continuity_rate, 'aggregated_data_points': aggregated_data_points, 'regime_data_points': total_regime_data_points, 'regime_files_count': len(regime_files)}
        except Exception as e:
            self.logger.error(f'❌ Error validating data continuity: {e}')
            return {'is_valid': False, 'issues': [f'Data continuity validation error: {str(e)}'], 'continuity_rate': 0.0}

    @traced(span_name='validate_temporal_consistency')
    async def _validate_temporal_consistency(self, step_name: str, symbol: str, exchange: str, timeframe: str, data_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if expected_diff is None:
                return {'is_valid': True, 'warnings': [f'Unknown timeframe: {timeframe}'], 'temporal_gaps': 0.0}
            gap_threshold = expected_diff * 2
            ...
            return {'is_valid': len(warnings) == 0, 'warnings': warnings, 'temporal_gaps': temporal_gaps, 'total_gaps': len(gaps), 'total_intervals': len(time_diffs)}
        except Exception as e:
            self.logger.error(f'❌ Error validating temporal consistency: {e}')
            return {'is_valid': False, 'warnings': [f'Temporal consistency validation error: {str(e)}'], 'temporal_gaps': 1.0}
    ...
    async def _validate_feature_consistency(self, step_name: str, symbol: str, exchange: str, timeframe: str, data_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(regime_feature_files) < 2:
                return {'is_valid': True, 'warnings': [], 'consistency_score': 1.0}
            regime_features = {}
            ...
            return {'is_valid': len(warnings) == 0, 'warnings': warnings, 'consistency_score': consistency_score, 'total_features': len(all_features), 'common_features': len(common_features), 'regime_count': len(regime_features)}
        except Exception as e:
            self.logger.error(f'❌ Error validating feature consistency: {e}')
            return {'is_valid': False, 'warnings': [f'Feature consistency validation error: {str(e)}'], 'consistency_score': 0.0}

    @traced(span_name='validate_regime_metadata_consistency')
    async def _validate_regime_metadata_consistency(self, step_name: str, symbol: str, exchange: str, timeframe: str, data_dir: str) -> Dict[str, Any]:
        ...
        try:
            if not regime_continuity_manager.regime_metadata:
                return {'is_valid': False, 'warnings': ['No regime metadata available'], 'metadata_consistency': 0.0}
            missing_metadata = []
            ...
            metadata_consistency = 1.0 - len(warnings) * 0.1
            return {'is_valid': len(warnings) == 0, 'warnings': warnings, 'metadata_consistency': max(0.0, metadata_consistency), 'total_regimes': len(regime_continuity_manager.regime_metadata), 'missing_metadata_count': len(missing_metadata)}
        except Exception as e:
            self.logger.error(f'❌ Error validating metadata consistency: {e}')
            return {'is_valid': False, 'warnings': [f'Metadata consistency validation error: {str(e)}'], 'metadata_consistency': 0.0}

    def _generate_recommendations(self, issues: List[str], warnings: List[str], validation_score: float) -> List[str]:
    ...
    async def validate_pipeline_continuity(self, symbol: str, exchange: str, timeframe: str, data_dir: str, steps: Optional[List[str]]=None) -> Dict[str, Any]:
        ...
        try:
            ...
            for step_name in steps:
                result = await self.validate_step_continuity(step_name, symbol, exchange, timeframe, data_dir)
            ...
            pipeline_result = {'symbol': symbol, 'exchange': exchange, 'timeframe': timeframe, 'validated_at': datetime.now().isoformat(), 'overall_score': overall_score, 'total_steps': len(steps), 'validated_steps': total_steps, 'step_results': {name: {'is_valid': result.is_valid, 'validation_score': result.validation_score, 'issues_count': len(result.issues), 'warnings_count': len(result.warnings)} for name, result in step_results.items()}, 'pipeline_issues': all_issues, 'pipeline_warnings': all_warnings, 'pipeline_recommendations': pipeline_recommendations}
            ...
            return {'error': str(e), 'overall_score': 0.0, 'validated_at': datetime.now().isoformat()}

    async def _save_validation_results(self, validation_results: Dict[str, Any], symbol: str, exchange: str, timeframe: str, data_dir: str) -> None:
