    ...
    def enhance_features(self, features: pd.DataFrame, hmm_regime: Optional[str] = None) -> pd.DataFrame:
        ...
        if self.feature_enhancement and hmm_regime:
            try:
                ...
                self.logger.info(
                    f"Enhanced features for regime {hmm_regime}: quality={
                        regime_quality:.3f}, stability={
                        regime_stability:.3f}"
                )
...
class CombinedFractionalSystem:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.fractional_labeler = FractionalTripleBarrierLabeling(fractional_config=self.config.get("labeling", {}))

        self.fractional_feature_generator = FractionalFeatureGenerator(config=self.config.get("differentiation", {}))

        self.hmm_integration = HMMFractionalIntegration(config=self.config.get("hmm_integration", {}))
    ...
    async def process_data(
        self, price_data: pd.DataFrame, volume_data: pd.DataFrame, hmm_regime: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"✅ Combined processing complete: {len(enhanced_features.columns)} features, {len(enhanced_features)} samples"
            )
    ...
    def _calculate_performance_metrics(
        self, features: pd.DataFrame, labels: pd.DataFrame, hmm_regime: Optional[str], processing_time: float
    ) -> Dict[str, Any]:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            feature_qualities = [
                p["metrics"].get("feature_quality", 0)
                for p in self.performance_history
                if "feature_quality" in p["metrics"]
            ]
...
def get_combined_fractional_config(
    ...
    return {
        ...
        "differentiation": differentiation_config
        or {
            "default_d": 0.5,
            "window": 100,
            "threshold": 1e-5,
        ...
        "hmm_integration": hmm_integration_config
        or {"feature_enhancement": True, "quality_tracking": True, "regime_metrics_enabled": True},
    }
