...
This module provides comprehensive validation for engineered features, including value
range checks, NaN propagation analysis, and feature correctness verification.
"""
...
class FeatureEngineeringValidator:
    ...
    def validate_engineered_features(
        ...
        if relevance_check["zero_variance_features"]:
            result.warnings.append(
                ValidationIssue(
                    ...
                    details={
                        "features": relevance_check["zero_variance_features"][:10]
                    },
                )
            )
        ...
        result.quality_score = max(
            0, 1 - (critical_issues * 0.2 + warning_issues * 0.05)
        )
    ...
    def _validate_feature_ranges(
                ...
                if out_of_range_low > 0 or out_of_range_high > 0:
                    out_of_range_features[column] = {
                        ...
                        "out_of_range_percentage": float(
                            (out_of_range_low + out_of_range_high) / len(col_data) * 100
                        ),
                    }
        ...
        if out_of_range_features:
            ...
            max_out_of_range_pct = max(
                f["out_of_range_percentage"] for f in out_of_range_features.values()
            )
            severity = (
                DataQualityLevel.CRITICAL
                if max_out_of_range_pct > 10
    ...
    def _analyze_nan_propagation(
        self, original_df: pd.DataFrame, features_df: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        for column in features_df.columns:
            if column not in original_df.columns:  # New feature
                ...
                potential_sources = self._identify_nan_sources(
                    column, original_df, features_df
                )
        ...
        return {
            ...
            "nan_increase_ratio": float(
                feature_nan_counts.sum() / max(original_nan_counts.sum(), 1)
            ),
        }
    ...
    def _validate_feature_calculations(
        ...
        sample_indices = np.random.choice(
            len(features_df), size=min(sample_size, len(features_df)), replace=False
        )
            ...
            try:
                ...
                if pd.api.types.is_numeric_dtype(expected_values):
                    ...
                    if close_matches:
                        ...
                        max_diff = np.nanmax(
                            np.abs(expected_values.values - actual_values.values)
                        )
                        ...
                        validation_results["failed_validations"].append(
                            {
                                ...
                                "match_percentage": float(
                                    (expected_values == actual_values).mean() * 100
                                ),
                            }
                        )
    ...
    def _validate_feature_dependencies(
        ...
        ma_columns = [
            col for col in features_df.columns if "ma_" in col or "ema_" in col
        ]
        if len(ma_columns) >= 2:
    ...
    def _validate_feature_relevance(self, features_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "redundancy_score": len(highly_correlated_pairs)
            / max(len(numeric_features), 1),
        }
                    ...
                    potential_leakage.append(
                        {
                            ...
                            "keywords_found": [
                                k for k in future_keywords if k in column.lower()
                            ],
                        }
                    )
    ...
    def _identify_feature_type(self, feature_name: str) -> Optional[str]:
        ...
        if "return" in feature_lower and "log" in feature_lower:
            ...
            return "macd"
        elif "stochastic" in feature_lower or "stoch" in feature_lower:
            return "stochastic"
        elif "bollinger" in feature_lower:
            return "bollinger_position"
        elif "volume_ratio" in feature_lower:
            return "volume_ratio"
        elif "z_score" in feature_lower or "zscore" in feature_lower:
            return "z_score"
        elif "normalized" in feature_lower:
            return "normalized"
        elif "scaled" in feature_lower and (
            "min" in feature_lower or "max" in feature_lower
        ):
            return "min_max_scaled"
        elif "corr" in feature_lower:
            return "correlation"
        elif "percentile" in feature_lower or "pctl" in feature_lower:
            return "percentile_rank"

        return None

    def _identify_nan_sources(
        self, feature_name: str, original_df: pd.DataFrame, features_df: pd.DataFrame
    ) -> List[str]:
    ...
    def _register_standard_calculations(self) -> None:
        ...
        self.feature_calculations["log_returns"] = lambda df: np.log1p(
            df["close"].pct_change()
        )

        # Price ratios
        self.feature_calculations["high_low_ratio"] = lambda df: df["high"] / df["low"]
        self.feature_calculations["close_open_ratio"] = (
            lambda df: df["close"] / df["open"]
        )

        # Volume features
        self.feature_calculations["volume_ma_ratio"] = (
    ...
    def register_custom_calculation(
        self, feature_name: str, calculation_func: Callable[[pd.DataFrame], pd.Series]
    ) -> None:
