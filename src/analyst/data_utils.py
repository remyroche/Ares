...
def load_futures_data(filename):
    """Loads futures data (funding rates) from a CSV file."""
    if not os.path.exists(filename):
...
def calculate_volume_profile(klines_df: pd.DataFrame, num_bins: int = 100):
    ...
    if max_price == min_price:  # Handle flat market
        return {
            ...
            "lvn_levels": [],
            "volume_in_bins": pd.Series([klines_df["Volume"].sum()], index=[min_price]),
        }
    ...
    hvn_indices, _ = find_peaks(
        volume_profile.values, prominence=volume_profile.max() * 0.1, width=3
    )  # 10% prominence, min 3 bins wide
    ...
    lvn_indices, _ = find_peaks(
        -volume_profile.values, prominence=volume_profile.max() * 0.05, width=3
    )  # 5% prominence for troughs
...
def create_dummy_data(filename, data_type, num_records=1000, start_date="2023-01-01"):
    ...
    if data_type == "klines":
        ...
        df.rename(
            columns={"p": "price", "q": "quantity", "m": "is_buyer_maker"}, inplace=True
        )
