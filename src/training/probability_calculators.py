...
class ClassificationProbabilityCalculator(BaseProbabilityCalculator):
    ...
    def calculate_direction_probability(self, model: Any, X_test: np.ndarray, y_test: np.ndarray) -> float:
        """
    ...
    def calculate_magnitude_probability(self, model: Any, X_test: np.ndarray, market_data: pd.DataFrame, threshold_factor: float=0.8) -> float:
        ...
        try:
            ...
            if 'close' in market_data.columns:
                ...
                avg_actual_magnitude = 0.01
            magnitude_accuracy = np.mean(predicted_magnitude >= avg_actual_magnitude * threshold_factor)
...
def get_probability_calculator(model_type: str) -> Union[ClassificationProbabilityCalculator, RegressionProbabilityCalculator]:
    ...
    if model_type.lower() in ['regression', 'regressor', 'reg']:
