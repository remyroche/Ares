...
class IntelligentFeatureCache:
    ...
    def __init__(self, cache_dir): str, "data_cache/feature_cache",
...
def _generate_cache_key(self, function_name): str = args: tuple = kwargs: dict) -> str:
        ...
        key_data = {
            "function": function_name , "args": self._make_pickle_safe(args),
...
def _make_pickle_safe(self, obj):
        ...
        return f"<async_function_{obj.__name__}>"
        return obj
...
def _evict_least_used(self, target_memory_mb: float):
        """
...
def _save_to_disk(self, cache_key): str = data: Any = metadata: dict):
        """
...
def set(self, cache_key): str = data: Any = metadata: dict | None = None):
        """
        Store data in cache.
        ...
        data_size_mb = metadata["size_mb"]
        current_memory = self._get_memory_usage_mb()

        if current_memory + data_size_mb > self.max_memory_mb:
        self._evict_least_used(self.max_memory_mb * 0.8)  # Evict to 80% of max
...
def get_stats(self) -> dict:
        ...
        return {
            "memory_usage_mb": memory_usage , "disk_usage_mb": disk_usage,
            "memory_cache_size": len(self.memory_cache),
            "hit_count": self.hit_count , "miss_count": self.miss_count,
            "hit_rate": hit_rate , "eviction_count": self.eviction_count,
