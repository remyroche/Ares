...
class TradeConditionsMonitor:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.enable_detailed_logging = self.monitor_config.get("enable_detailed_logging", True)
        self.enable_feature_analysis = self.monitor_config.get("enable_feature_analysis", True)
        self.enable_model_tracking = self.monitor_config.get("enable_model_tracking", True)
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"❌ Trade Conditions Monitor initialization failed: {e}")
            return False
    ...
    async def _initialize_timeframe_cache(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to initialize timeframe cache: {e}")
            raise
    ...
    async def _load_historical_data(self) -> None:
        ...
        try:
            ...
            if self.storage_manager:
                ...
                self.logger.info(f"Loaded {len(recent_decisions)} recent trade decisions")
    ...
    async def record_trade_decision(
        ...
        """
        try:
            self.logger.info(f"Recording trade decision: {decision_context.decision_id}")
    ...
    async def _store_decision_in_db(self, decision_context: TradeDecisionContext) -> None:
        ...
        try:
            ...
            for timeframe, features in decision_context.timeframe_features.items():
                ...
                await self.storage_manager.insert_data("timeframe_features", feature_data)
            ...
            for ensemble in decision_context.ensemble_predictions:
                for prediction in ensemble.individual_predictions:
                    prediction_data = {
                        ...
                        "prediction_details": json.dumps(asdict(prediction), default=str)
                    }
                    
                    await self.storage_manager.insert_data("model_predictions", prediction_data)
            
        except Exception as e:
            self.logger.error(f"Failed to store decision in database: {e}")
            raise

    async def _log_decision_details(self, decision_context: TradeDecisionContext) -> None:
        """Log detailed decision information for debugging."""
        try:
            details = [
                f"🎯 Trade Decision Details:",
            ...
            if decision_context.nearby_sr_levels:
                details.append(f"   Nearby S/R Levels: {len(decision_context.nearby_sr_levels)}")
                for sr in decision_context.nearby_sr_levels[:3]:  # Top 3
                    details.append(f"     - {sr.level_type.value}: ${sr.price:.4f} (strength: {sr.strength:.2f})")
            
            # Model predictions summary
            details.append(f"   Ensemble Predictions: {len(decision_context.ensemble_predictions)}")
            for ensemble in decision_context.ensemble_predictions:
                details.append(f"     - {ensemble.regime_type.value}: {ensemble.aggregated_prediction:.4f} (consensus: {ensemble.consensus_level:.2f})")
            
            # Multi-timeframe features
            details.append(f"   Timeframes Analyzed: {list(decision_context.timeframe_features.keys())}")
            
            self.logger.info("\n".join(details))
            
        except Exception as e:
            self.logger.error(f"Failed to log decision details: {e}")
    ...
    async def _store_execution_in_db(self, execution: TradeExecution) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to store execution in database: {e}")
            raise
    ...
    async def _analyze_execution_quality(self, execution: TradeExecution) -> None:
        ...
        try:
            ...
            if execution.slippage is not None:
                slippage_factor = max(0, 1 - abs(execution.slippage) / 0.01)  # Normalize to 1% slippage
                quality_factors.append(slippage_factor)
            
            # Execution speed factor (faster is better)
            if execution.execution_time_ms > 0:
                speed_factor = max(0, 1 - execution.execution_time_ms / 5000)  # Normalize to 5 seconds
            ...
            if quality_factors:
                execution.execution_quality_score = sum(quality_factors) / len(quality_factors)
            
            # Log quality analysis
            self.logger.debug(f"Execution quality for {execution.execution_id}: {execution.execution_quality_score:.2f}")
            
        except Exception as e:
            self.logger.error(f"Failed to analyze execution quality: {e}")
    ...
    async def _store_outcome_in_db(self, outcome: TradeOutcome) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to store outcome in database: {e}")
            raise
    ...
    async def _perform_post_trade_analysis(self, outcome: TradeOutcome) -> None:
        ...
        try:
            ...
            if not decision:
                self.logger.warning(f"No decision context found for trade {outcome.trade_id}")
                return
            
            # Analyze prediction accuracy
            if outcome.pnl_percentage is not None and decision.final_prediction is not None:
                ...
                outcome.prediction_accuracy = 1.0 if predicted_positive == actual_positive else 0.0
            ...
            self.logger.error(f"Failed to perform post-trade analysis: {e}")
        ...
        try:
            ...
            if outcome.pnl_percentage and outcome.pnl_percentage > 0:
                ...
                if decision.final_confidence > 0.8:
                    outcome.what_worked.append("High prediction confidence was justified")
            
            # Analyze what failed
            if outcome.pnl_percentage and outcome.pnl_percentage < 0:
                outcome.what_failed.append("Trade was unprofitable")
                if decision.regime_confidence < 0.6:
                    outcome.what_failed.append("Low regime confidence should have been avoided")
                if outcome.max_drawdown > 0.05:  # 5%
                    outcome.what_failed.append("Excessive drawdown experienced")
            
            # Generate suggestions
            if decision.risk_score > 0.7 and outcome.pnl_percentage and outcome.pnl_percentage < 0:
                suggestions.append("Consider avoiding trades with high risk scores")
            
            if decision.final_confidence < 0.6:
                suggestions.append("Require higher prediction confidence for trade entry")
            
            if len(decision.ensemble_predictions) < 3:
                suggestions.append("Use more ensemble models for better prediction diversity")
        ...
        try:
            ...
            for tf in timeframes:
                ...
                if tf_data is not None and not tf_data.empty:
                    ...
                    tf_features = await self._calculate_timeframe_features(tf_data, tf, timestamp)
                    features[tf] = tf_features
            
            return features
            
        except Exception as e:
            self.logger.error(f"Failed to get multi-timeframe features: {e}")
            return {}
    ...
    async def _get_timeframe_data(
        ...
        try:
            ...
            if cache_key in self.timeframe_cache[timeframe]:
                ...
                if not data.empty and data.index[-1] >= timestamp - timedelta(minutes=int(timeframe.replace('m', ''))):
                    return data
            ...
            self.logger.error(f"Failed to get timeframe data for {symbol} {timeframe}: {e}")
            return None
    ...
    async def _fetch_timeframe_data(
        ...
        try:
            ...
            self.logger.error(f"Failed to fetch timeframe data: {e}")
            return None
    ...
    async def _calculate_timeframe_features(
        ...
        try:
            ...
            if len(data) >= 20:  # Minimum for Bollinger Bands
                ...
                features.bollinger_position = (latest['close'] - features.bollinger_lower) / (features.bollinger_upper - features.bollinger_lower)
            ...
            if len(data) >= 20:
                ...
                features.volume_ratio = latest['volume'] / features.volume_sma if features.volume_sma > 0 else 1.0
            ...
            self.logger.error(f"Failed to calculate timeframe features: {e}")
        ...
        try:
            ...
            if self.trade_outcomes:
                profitable_trades = sum(
                    1 for outcome in self.trade_outcomes.values() 
                    if outcome.pnl_percentage and outcome.pnl_percentage > 0
                )
                stats["win_rate"] = profitable_trades / len(self.trade_outcomes)
                
                avg_pnl = np.mean([
                    outcome.pnl_percentage for outcome in self.trade_outcomes.values()
    ...
    async def generate_monitoring_report(self, days: int = 7) -> dict[str, Any]:
        ...
        try:
            ...
            # Filter recent data
            recent_decisions = {
                k: v for k, v in self.trade_decisions.items()
                if v.timestamp >= cutoff_date
            }
            
            recent_executions = {
                k: v for k, v in self.trade_executions.items()
                if v.timestamp >= cutoff_date
            }
            
            recent_outcomes = {
                k: v for k, v in self.trade_outcomes.items()
            ...
            if recent_outcomes:
                pnl_values = [
                    outcome.pnl_percentage for outcome in recent_outcomes.values()
                    if outcome.pnl_percentage is not None
                ]
            ...
            if recent_decisions:
                ...
                if model_predictions:
                    ...
                    report["model_analysis"] = {
                        ...
                        "model_distribution": model_df["model_type"].value_counts().to_dict()
                    }
            
            # Regime analysis
            if recent_decisions:
                regimes = [decision.current_regime.value for decision in recent_decisions.values()]
                regime_counts = pd.Series(regimes).value_counts()
                
                report["regime_analysis"] = {
                    "regime_distribution": regime_counts.to_dict(),
                    "most_common_regime": regime_counts.index[0] if not regime_counts.empty else None
                }
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to cleanup Trade Conditions Monitor: {e}")