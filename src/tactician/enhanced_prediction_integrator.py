...
class TacticianEnhancedPredictionIntegrator:
    ...
    predictions similar to the Analyst but with smaller price deviations and higher
    confidence.
    ...
    def _load_tactician_config(self) -> None:
        ...
        self.ml_confidence_factors = {
            "price_deviation_prediction": ml_config.get(
                "price_deviation_prediction"
            ),  # Will be ML model output
            "price_direction_prediction": ml_config.get(
                "price_direction_prediction"
            ),  # Will be ML model output
            "price_target_confidence": ml_config.get(
    ...
    def _initialize_tactician_models(self) -> dict[str, Any]:
        ...
        for prediction_type in self.prediction_types:
            models[prediction_type] = {
                ...
                "ml_confidence_factor": self.ml_confidence_factors.get(
                    prediction_type, 1.0
                ),
            }
    ...
    async def generate_tactician_predictions(
        ...
        try:
            ...
            upper_barrier, lower_barrier = (
                self.barrier_calculator.calculate_dynamic_barriers(timeframe)
            )
            ...
            self.logger.info(
                f"🎯 Generated {len(tactician_predictions)} Tactician enhanced predictions for {timeframe}"
            )
    ...
    async def _generate_enhanced_prediction(
        ...
        try:
            ...
            base_prediction = self._extract_analyst_prediction(
                analyst_predictions, prediction_type
            )
            ...
            self.logger.error(
                error(f"❌ Error generating {prediction_type} prediction: {e}")
            )
            return None

    def _extract_analyst_prediction(
        self, analyst_predictions: dict[str, Any], prediction_type: str
        ...
        try:
            ...
            return self._create_synthetic_prediction(
                prediction_type, analyst_predictions
            )

        except Exception as e:
            self.logger.warning(
                warning(
                    f"⚠️ Could not extract {prediction_type} from Analyst predictions: {e}"
                )
            )
            return None

    def _create_synthetic_prediction(
        self, prediction_type: str, analyst_predictions: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if prediction_type == "price_deviation_prediction":
                ...
                synthetic_value = (
                    base_confidence  # Base confidence, ML model will enhance
                )
            else:
    ...
    def _calculate_precision_score(
        self, enhanced_confidence: float, market_data: pd.DataFrame, timeframe: str
    ) -> float:
        ...
        try:
            ...
            for prediction_type in self.prediction_types:
                if prediction_type in tactician_predictions:
                    ...
                    analyst_pred = self._extract_analyst_prediction(
                        analyst_predictions, prediction_type
                    )
                    ...
                    if analyst_pred:
                        ...
                        if ml_confidence_factor > 1.0:
                            expected_confidence = (
                                analyst_confidence * ml_confidence_factor * 0.8
                            )
                            if tactician_confidence >= expected_confidence:
                                validation_results["enhancements"].append(
                                    f"{prediction_type}: ML model confidence enhancement"
                                )
                                total_score += 1.0
                            else:
                                validation_results["issues"].append(
                                    f"{prediction_type}: Insufficient ML model confidence enhancement"
                                )
                        else:
                            # ML model indicates no enhancement needed
                            validation_results["enhancements"].append(
                                f"{prediction_type}: ML model baseline confidence"
                            )
                            total_score += 0.5
                        ...
                        if prediction_type == "price_deviation_prediction":
                            ...
                            if abs(tactician_value) <= abs(analyst_value):
                                ...
                                validation_results["issues"].append(
                                    f"{prediction_type}: Deviation not smaller than Analyst"
                                )
                        else:
                        ...
                        validation_results["issues"].append(
                            f"{prediction_type}: No Analyst prediction available"
                        )
                else:
                    validation_results["issues"].append(
                        f"{prediction_type}: Missing Tactician prediction"
                    )
        ...
        try:
            for prediction_type, factor in new_factors.items():
                if prediction_type in self.ml_confidence_factors:
                    ...
                    self.logger.info(
                        f"Updated ML confidence factor for {prediction_type}: {factor}"
                    )
                else:
                    self.logger.warning(
                        f"Unknown prediction type for ML confidence factor: {prediction_type}"
                    )
        except Exception as e:
            self.logger.error(f"Error updating ML confidence factors: {e}")

    def load_step12_ml_confidence_factors(
        self, step12_results_path: str = None
    ) -> bool:
        """Automatically load ML confidence factors from step12 results. This method is
        called automatically when step12 completes.
        ...
        try:
            ...
            if step12_results_path and Path(step12_results_path).exists():
                ...
                if not step12_results:
                    self.logger.warning(
                        "No step12 results found, using default ML confidence factors"
                    )
                    return False
            ...
            if "ml_confidence_factors" in step12_results:
                ...
                for prediction_type in self.ml_confidence_factors:
                    if prediction_type in ml_factors:
                        self.ml_confidence_factors[prediction_type] = ml_factors[
                            prediction_type
                        ]
                        self.logger.info(
                            f"Loaded ML confidence factor for {prediction_type}: {ml_factors[prediction_type]}"
                        )
                    else:
                        self.logger.warning(
                            f"Missing ML confidence factor for {prediction_type} in step12 results"
                        )

                # Also update the models
                for prediction_type, model_data in self.tactician_models.items():
                    if prediction_type in self.ml_confidence_factors:
                        model_data["ml_confidence_factor"] = self.ml_confidence_factors[
                            prediction_type
                        ]

                self.logger.info(
                    "✅ Successfully loaded ML confidence factors from step12 results"
                )
                return True
    ...
    def get_prediction_summary(
        self, tactician_predictions: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        try:
            summary = {
                ...
                "prediction_types": {},
                "timeframe": tactician_predictions.get("metadata", {}).get(
                    "timeframe", "unknown"
                ),
            }
