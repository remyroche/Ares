...
class TripleBarrierOptimizer:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.param_ranges = {'profit_multiplier': self.optim_config.get('profit_range', (0.001, 0.005)), 'stop_multiplier': self.optim_config.get('stop_range', (0.0005, 0.0025)), 'time_barrier': self.optim_config.get('time_range', (10, 60))}
    ...
    async def optimize_barriers(self, market_data: pd.DataFrame, regime_labels: Optional[pd.Series]=None) -> Dict[str, BarrierOptimizationResult]:
        ...
        try:
            ...
            if self.regime_specific and regime_labels is not None:
                results = await self._optimize_regime_specific_barriers(market_data, regime_labels)
            else:
    ...
    async def _optimize_regime_specific_barriers(self, market_data: pd.DataFrame, regime_labels: pd.Series) -> Dict[str, BarrierOptimizationResult]:
    ...
    async def _optimize_global_barriers(self, market_data: pd.DataFrame) -> BarrierOptimizationResult:
        ...
        study = optuna.create_study(direction='maximize', study_name='triple_barrier_global')
    ...
    def _barrier_objective(self, trial: optuna.Trial, data: pd.DataFrame, regime: str) -> float:
        ...
        profit_mult = trial.suggest_float('profit_multiplier', self.param_ranges['profit_multiplier'][0], self.param_ranges['profit_multiplier'][1])
        stop_mult = trial.suggest_float('stop_multiplier', self.param_ranges['stop_multiplier'][0], self.param_ranges['stop_multiplier'][1])
        time_barrier = trial.suggest_int('time_barrier', self.param_ranges['time_barrier'][0], self.param_ranges['time_barrier'][1])
    ...
    def _calculate_barrier_metrics(self, data: pd.DataFrame, profit_mult: float, stop_mult: float, time_barrier: int) -> Dict[str, float]:
        ...
        if len(active_returns) == 0:
            return {'sharpe_ratio': 0, 'win_rate': 0, 'profit_factor': 0, 'max_drawdown': 0}
    ...
    def get_optimized_config(self, results: Dict[str, BarrierOptimizationResult], current_regime: Optional[str]=None) -> Dict[str, float]:
        ...
        return {'profit_take_multiplier': result.optimal_profit_multiplier, 'stop_loss_multiplier': result.optimal_stop_multiplier, 'time_barrier_minutes': result.optimal_time_barrier, 'binary_classification': True}