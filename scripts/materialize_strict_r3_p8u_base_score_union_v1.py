#!/usr/bin/env python3
"""Build an immutable target-free union of disjoint P8U Base OOS receipts.

This lineage adapter only copies the five declared Base inference fields from
exactly one previously sealed monthly receipt.  It is deliberately unable to
join labels, outcomes, path fields, MC1, admission, or portfolio state.
"""
...
REQUIRED = (*IDENTITY, "base_score", "base_rank_ts")
PROHIBITED_TOKENS = ("policy_", "label_", "target_", "outcome_", "path_")
SCHEMA = "strict_r3_p8u_base_targetfree_score_union_v1"
...
def _source_for_month(roots: tuple[Path, ...], month: pd.Timestamp) -> Path:
    ...
    candidates = [root / "scheme=tail_linear_125" / "target_free_scores" / token for root in roots]
    found = [path for path in candidates if path.is_file()]
    if len(found) != 1:
        raise AssertionError(
            f"{month:%Y-%m}: expected exactly one Base receipt across {[str(item) for item in candidates]}, found {len(found)}"
        )
    return found[0]
...
def _target_free_score(path: Path, month: pd.Timestamp) -> pd.DataFrame:
    ...
    if missing:
        raise AssertionError(f"{path}: missing Base score columns {missing}")
    leaked = sorted(
        column for column in frame.columns
        if column not in IDENTITY and any(token in column.lower() for token in PROHIBITED_TOKENS)
    )
    if leaked:
        raise AssertionError(f"{path}: non-target-free Base score columns {leaked}")
    ...
    for column in ("base_score", "base_rank_ts"):
...
def main() -> None:
    ...
    parser.add_argument("--source-roots", required=True, help="comma-separated disjoint immutable Base score roots")
    ...
    for month in months:
        ...
        audit.append({
            ...
            "source": str(source), "source_sha256": _sha256([source]), "target_free": True,
        })
    ...
    _write_json_exclusive(out / "run_manifest.json", {
        "schema": SCHEMA,
        "scope": "offline target-free Base score union; no training, labels, Meta, MC1, admission, portfolio, inference, live, or exchange mutation",
        "identity_contract": "each month is copied unchanged from exactly one strict-OOF F72 tail-linear-125 Base receipt",
        "sources": [str(root) for root in roots], "source_sha256": _sha256(used),
        "months": [f"{month:%Y-%m}" for month in months], "audit": audit,
        "correctness": {"single_source_per_month": True, "all_scores_target_free": True,
                        "no_label_or_path_columns": True, "all_identities_long_only_unique": True},
    })
    print(out)
