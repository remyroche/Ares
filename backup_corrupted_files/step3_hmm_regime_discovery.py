...
from src.training.steps.unified_data_loader import UnifiedDataLoader, get_unified_data_loader
...
class HMMRegimeAnalyzer:
    ...
    pass
            ...
            if "volume" in price_df.columns:
                volume_features["volume_ratio"] = (
                    price_df["volume"] / price_df["volume"].rolling(window=20).mean()
                )
            ...
            if features.empty:
                msg = "No valid features after preprocessing"
                raise ValueError(msg)

            self.logger.info(
                f"Created {len(features.columns)} features with {len(features)} samples",
            )
            return features
    ...
    def fit_hmm_model(self, features: pd.DataFrame, timeframe: str) -> GMMHMM | None:
    ...
    pass
            ...
            self.logger.info(
                f"Fitted HMM model for {timeframe} with {self.config['n_components']} components",
            )
            return model

        except Exception as e:
            self.logger.exception(f"Error fitting HMM model for {timeframe}: {e}")
            return None
    ...
    pass
            if timeframe not in self.models:
                msg = f"No fitted model found for {timeframe}"
                raise ValueError(msg)
            ...
            results = pd.DataFrame({"state": states, "timestamp": features.index})
            ...
            self.logger.info(
                f"Predicted regimes for {timeframe}: {len(results)} samples",
            )
            return results

        except Exception as e:
            self.logger.exception(f"Error predicting regimes for {timeframe}: {e}")
            return pd.DataFrame()
...
class RegimeClusterAnalyzer:
    ...
    pass
            ...
            if not combined_data:
                msg = "No valid regime data to cluster"
                raise ValueError(msg)
            ...
            prob_cols = [
                ...
                if col.startswith("state_") and col.endswith("_prob")
            ]
            if not prob_cols:
                msg = "No state probability columns found"
                raise ValueError(msg)
            ...
            self.logger.exception(f"Error in composite clustering: {e}")
            return pd.DataFrame()
...
BLOCKS: list[BlockConfig] = [
    ...
    BlockConfig(
        ...
        6,
        3,
    ),  # 6 states for granular momentum detection, target 3 features after filtering
...
class FeatureCache:
    ...
    pass
            ...
            print_and_log(
                f"Error retrieving cached features: {e}",
                "warning",
                "FeatureCache",
            )
            return None
...
class ModelCache:
    ...
    pass
            ...
            print_and_log(
                f"Failed to load model cache metadata: {e}",
                "warning",
                "ModelCache",
            )
            self.metadata = {"entries": {}, "total_size_gb": 0.0}
    ...
    pass
            ...
            print_and_log(
                f"Failed to save model cache metadata: {e}",
                "warning",
                "ModelCache",
            )
    ...
    pass
            ...
            print_and_log(
                f"Error retrieving cached model: {e}",
                "warning",
                "ModelCache",
            )
            return None
    ...
    pass
            ...
            if not os.path.exists(model_file) or os.path.getsize(model_file) == 0:
                msg = f"Model file was not created or is empty: {model_file}"
                raise ValueError(
                    msg,
                )
            if not os.path.exists(scaler_file) or os.path.getsize(scaler_file) == 0:
                msg = f"Scaler file was not created or is empty: {scaler_file}"
                raise ValueError(
                    msg,
                )

            print_and_log(
                "✅ Model and scaler files saved successfully",
                "debug",
                "ModelCache",
            )
...
def create_basic_features(price_df: pd.DataFrame, order_flow_data: pd.DataFrame | None = None) -> pd.DataFrame:
    """
    ...
    pass
        ...
        fe_config = {
            ...
            "vectorized_advanced_features": {
                ...
                "enable_sr_distance": True,  # Enable for HMM to provide support/resistance features
    ...
    pass
                ...
                features_dict = new_loop.run_until_complete(
                    fe.engineer_features(price_df, vol_df, order_flow_data),
                )
                ...
                if not features_dict:
                    ...
                    msg = "Advanced feature engineering failed to produce any features"
                    raise RuntimeError(
                        msg,
                    )
        ...
        logger.exception(f"❌ Critical error in advanced feature engineering: {e}")
        msg = f"Advanced feature engineering failed: {e}"
...
def _assign_block(feature_name: str) -> str:
    ...
    # CRITICAL: Exclude raw data and basic transformations (EXACT matches only)
    # Avoid excluding engineered features like 'volume_imbalance', 'volume_returns', etc.
    raw_data_exact = {
        "open",
        "high",
        "low",
        ...
        "date",  # Time data
        "returns",
        "log_returns",  # Basic transformations
        "price",
        "price_raw",  # Raw price data
        "bid",
        "ask",
        "bid_volume",
        "ask_volume",  # Raw order book
        "trade_id",
        "trade_time",  # Raw trade data
    }

    if feature_name_lower in raw_data_exact:
    ...
    momentum_patterns = [
        ...
        "volume_momentum_oscillator",
        "price_momentum_diff",
        "price_momentum_accel",
        "price_momentum_norm",
        "volume_weighted_momentum_diff",
        "volume_weighted_momentum_accel",
        "volume_weighted_momentum_norm",
        "volume_momentum_diff",
        "volume_momentum_accel",
        ...
        "rsi_diff_3m_1m_norm",
        "rsi_diff_5m_1m_norm",
        "rsi_diff_10m_3m_norm",
        "momentum_5_diff_3m_1m",
        "momentum_10_diff_5m_1m",
        "momentum_20_diff_10m_3m",
        "momentum_5_diff_3m_1m_norm",
        "momentum_10_diff_5m_1m_norm",
        "momentum_20_diff_10m_3m_norm",
        # IMPORTANT: Moving Average Slopes and MACD features
        "ema20_slope",
        "sma50_slope", 
        "ema_slope",
        "sma_slope",
    ...
    volatility_patterns = [
        ...
        "bbands",
        "bollinger",
        # IMPORTANT: Bollinger Band features
        "bb_zscore",
        "bb_upper",
        "bb_lower", 
        "bb_width",
    ...
    volume_patterns = [
        ...
        "vwap",
        "vwap_deviation",
        # Liquidity and spread (volume-related microstructure)
        "bid_ask_spread",
        "liquidity",
        ...
        "trade_volume_change",
        "trade_count_returns",
        "trade_volume_returns",
        "trade_to_order_ratio",
        # Volume correlation and impact features
        "price_volume_correlation",
        "volume_price_impact",
        "price_impact",
        "volume_price_divergence",
        "high_volume_price_impact",
        "low_volume_price_impact",
    ...
    sr_patterns = [
        ...
        "support_strength",
        "resistance_strength",
        "support_clarity_factor",
        "resistance_clarity_factor",
        # Add strength-weighted SR position features
        "sr_position_strength_weighted",
        "sr_position_momentum_adjusted",
        "sr_position_volatility_adjusted",
        "sr_position_trend",
        "sr_position_volatility",
        "effective_support_level",
        "effective_resistance_level",
        "effective_level_strength",
        "distance_to_effective_support",
        "distance_to_effective_resistance",
        "support_resistance_strength_ratio",
    ]
...
def _select_block_features(
    ...
    if X.empty:
        logger.warning(
            f"⚠️ No features remaining for {block} block after dropping constants",
        )
        return pd.DataFrame(index=full_df.index)
    ...
    if block == "momentum":
        correlation_threshold = (
            0.98  # Less aggressive for momentum block to preserve diversity
        )
    else:
        correlation_threshold = 0.95  # Standard threshold for other blocks

    logger.info(
        f"🔧 Using correlation threshold {correlation_threshold} for {block} block",
    )
    ...
    if X.empty:
        logger.warning(
            f"⚠️ No features remaining for {block} block after correlation pruning",
        )
        return pd.DataFrame(index=full_df.index)
    ...
    if max_features and len(X.columns) > max_features:
        ...
        logger.info(
            f"📊 Limiting to top {max_features} features by variance: {top_features}",
        )
        X = X[top_features]
...
def _fit_block_hmm_robust(
...
) -> tuple[GMMHMM | None, StandardScaler | None]:
    """
    ...
    pass
        ...
        X.columns[non_constant_mask].tolist()
    ...
    pass
        ...
        system_logger.error(
            f"🚨 Block {block_name}: Critical error in _fit_block_hmm_robust: {e}",
        )
        return None, None
...
def _posteriors(model: GMMHMM, X: np.ndarray) -> np.ndarray:
    ...
    pass
        ...
        if np.any(np.isnan(posteriors)) or np.any(np.isinf(posteriors)):
            system_logger.warning(
                "🚨 Posterior probabilities contain NaN/Inf, cleaning...",
            )
...
def _build_combination_profiles(
    ...
    if not block_states:
        ...
        # Transpose and join
        joined_keys = ["|".join(map(str, row)) for row in zip(*key_parts, strict=False)]
    ...
    for combo, idx in combination_keys.groupby(combination_keys).groups.items():
        ...
        for gamma in block_posteriors.values():
            if len(idx) == 0:
                continue
...
def _cluster_combinations(
    ...
    CONCENTRATION_TARGET_MIN = (
        0.75  # Reduced from 85% to 75% to allow more diverse regimes
    )
    CONCENTRATION_TARGET_MAX = (
        0.90  # Reduced from 95% to 90% to preserve more nuanced states
    )
    SIMILARITY_THRESHOLD = 0.80  # Keep original threshold for balanced merging
    ...
    while len(current_clusters) > TARGET_ARCHETYPES:
        ...
        for i, cluster1 in enumerate(current_clusters):
            for _j, cluster2 in enumerate(current_clusters[i + 1 :], i + 1):
...
def _name_states(block: str, medians: dict[int, dict[str, float]]) -> dict[int, str]:
    ...
    for s, feat in medians.items():
        ...
        elif block in ("momentum", "support_resistance"):
    ...
    for rank, (s, _sc) in enumerate(sorted_states):
        q = rank / max(1, n - 1)
...
async def run_step(
    ...
    **kwargs: Any,
    ...
    print_and_log(
        "🚀 Starting HMM Regime Discovery step",
        "info",
    ...
    print_and_log(
        "🔧 Initializing data sharing manager",
        "info",
        "Step3.HMMRegimeDiscovery",
    )
    ...
    print_and_log(
        "🔧 Initializing UnifiedDataLoader",
        "info",
        "Step3.HMMRegimeDiscovery",
    )
    ...
    pass
            ...
            logger.exception(f"🚨 Error in HMM regime discovery for {tf}: {e}")
            print_and_log(
    ...
    pass
                ...
                features_dict = load_features_for_step(
                    symbol,
                    exchange,
                    data_dir,
                    f"Step3.{tf}",
                ...
                if len(features_df) != len(price_df):
                    logger.warning(
                        f"⚠️ Feature count mismatch: {len(features_df)} features vs {len(price_df)} prices",
                    )
                ...
                logger.exception(f"❌ Failed to load features for {tf}: {e}")
                print_and_log(
    ...
    pass
                    ...
                    with Parallel(
                        n_jobs=parallel_block_n_jobs,
                        backend=parallel_block_backend,
                    ...
                    logger.exception(f"❌ Parallel block processing failed: {e}")
    ...
    pass
                ...
                logger.info(
                    f"🎯 Selected top {len(top_clusters)} clusters out of {len(unique_clusters)} total clusters",
                )
                logger.info(f"   Top clusters: {top_clusters}")
                logger.info(
                    f"   Cluster frequencies: {cluster_counts.head(20).to_dict()}",
                )
                ...
                logger.info(f"💾 Saved composite intensity -> {intensity_out_path}")
                ...
                composite_meta = {
                    ...
                    "total_clusters": int(
                        composite_df["composite_cluster_id"].nunique(),
                    ),
                    "total_combinations": int(
                        composite_df["combination_key"].nunique(),
                    ),
                    "cluster_distribution": composite_df["composite_cluster_id"]
                    .value_counts()
                    .to_dict(),
                    "blocks": [
                ...
                cluster_stats = (
                    composite_df["composite_cluster_id"].value_counts().sort_index()
                )
                logger.info(f"📊 Composite cluster distribution for {tf}:")
                for cluster_id, count in cluster_stats.items():
                    percentage = (count / len(composite_df)) * 100
                    logger.info(
                        f"   Cluster {cluster_id}: {count} samples ({percentage:.1f}%)",
                    )
    ...
    pass
                ...
                report_lines = []
                report_lines.append("HMM Regime Discovery Report")
                ...
                report_path = os.path.join(
                    data_dir,
                    f"{exchange}_{symbol}_hmm_regime_report_{tf}.txt",
                )
    ...
    pass
                ...
                logger.exception(f"❌ Failed to save basic HMM meta for {tf}: {e}")
                print_and_log(
            ...
            logger.exception(f"❌ Error processing timeframe {tf}: {e}")
            continue
    ...
    pass
            ...
            for req_file in required_files:
                ...
                logger.info(
                    f"  📄 {req_file}: {'✅ EXISTS' if exists else '❌ MISSING'}",
                )
    ...
    pass
                ...
                report_path = os.path.join(
                    reports_dir,
                    f"{exchange}_{symbol}_hmm_regime_summary_{tf}_{timestamp}.md",
                )
                ...
                logger.exception(
                    f"❌ ERROR details: {type(report_error).__name__}: {str(report_error)}",
                )
            ...
            logger.exception(f"❌ ERROR in HMM regime analysis for {tf}: {e}")
    ...
    if final_validation_passed:
        ...
        if os.path.exists(reports_dir):
            ...
            if report_files:
                logger.info("📊 HMM Regime Analysis Reports generated:")
                print_and_log(
                    "📊 HMM Regime Analysis Reports generated:",
                    "info",
                    "Step1_7.HMMRegimeDiscovery",
                )
...
def _generate_archetype_descriptions(
    ...
    pass
        ...
        logger.exception(f"❌ Error generating archetype descriptions: {e}")
        return {}
...
def _compute_comprehensive_composite_metrics(
    ...
    cluster_persistence = {k: 1.0 for k in cluster_sizes}
    cluster_volatility = {k: 0.1 for k in cluster_sizes}
    ...
    market_condition_distribution = {
        k: {"trending": 0.5, "ranging": 0.5} for k in cluster_sizes
    }
    regime_stability = {k: 0.8 for k in cluster_sizes}
    ...
    missing_features_by_cluster = {k: [] for k in cluster_sizes}
...
def _generate_state_name(
    ...
    pass
        ...
        if meta and "state_names" in meta:
            ...
            if (
                block_name_lower in state_names
                and str(state_id) in state_names[block_name_lower]
            ):
        ...
        if block_name_lower == "volatility":
            volatility_names = {
                0: "Low & Stable Vol",
                1: "Moderate Vol",
                2: "High & Choppy Vol",
        ...
        if block_name_lower == "support_resistance":
            sr_names = {
                0: "Very Near Support",
                1: "Near Support",
                2: "Approaching Support",
...
def _generate_simple_hmm_report(
    ...
    logger.info(
        f"🔧 DEBUG: _generate_simple_hmm_report called with exchange={exchange}, symbol={symbol}, timeframe={timeframe}, data_dir={data_dir}",
    )
    try:
    pass
except Exception as e:
    ...
    pass
        ...
        if blocks:
            ...
            report_lines.append(
                "> **Note**: Regime detection has been simplified to focus on core market dynamics:",
            )
            for block in blocks:
                report_lines.append(
            ...
            report_lines.append(
                "- **Momentum**: Price trend and momentum patterns (6 states)",
            )
            report_lines.append(
                "- **Volatility**: Market volatility and dispersion (4 states)",
            )
            report_lines.append(
                "- **Volume**: Trading volume and flow analysis (4 states)",
            )
            report_lines.append(
                "- **Support/Resistance**: Price level proximity and strength (6 states)",
            )
            report_lines.append("")
            report_lines.append(
                "*Liquidity and market microstructure blocks have been removed to simplify regime detection and improve stability.*",
            )
            report_lines.append("")
        ...
        if intensity_df is not None and "composite_cluster_id" in cluster_df.columns:
            ...
            for block in blocks:
                ...
                # Create table header
                report_lines.append(
                    "| State | Description | % of Time | Observations |",
                )
                report_lines.append(
                    "|-------|-------------|-----------|--------------|",
                )
                ...
                if state_col in intensity_df.columns:
                    ...
                    for state_id in range(n_states):
                        ...
                        if (
                            block_name in state_names
                            and str(state_id) in state_names[block_name]
                        ):
                            description = state_names[block_name][str(state_id)]
                        else:
                            description = _generate_state_name(
                                block_name,
                                state_id,
                                f"State {state_id}",
                                meta,
                        ...
                        report_lines.append(
                            f"| {state_id} | {description} | {percentage:.2f}% | {count:,} |",
                        )
            ...
            for block in blocks:
                ...
                if state_col in intensity_df.columns:
                    ...
                    # Find dominant state
                    dominant_state = (
                        state_counts.index[0] if len(state_counts) > 0 else None
                    )
                    dominant_percentage = (
                        (state_counts.iloc[0] / total_obs * 100)
                        if len(state_counts) > 0 and total_obs > 0
                        else 0
                    )
                    ...
                    if dominant_state is not None:
                        if (
                            block_name in state_names
                            and str(dominant_state) in state_names[block_name]
                        ):
                            dominant_desc = state_names[block_name][str(dominant_state)]
                        else:
                            dominant_desc = _generate_state_name(
                                block_name,
                                dominant_state,
                        ...
                        report_lines.append(
                            f"- **{block_name.title()}**: Dominant state is {dominant_desc} ({dominant_percentage:.1f}% of time)",
                        )

                    # Calculate state diversity
                    state_diversity = len(state_counts)
                    report_lines.append(
                        f"  - State diversity: {state_diversity} distinct states",
                    )
                    ...
                    report_lines.append(
                        f"  - Concentration: {concentration_desc} ({dominant_percentage:.1f}% in top state)",
                    )
                    report_lines.append("")
            ...
            report_lines.append("")
            report_lines.append(
                "*Note: Intensity data not available for detailed state distribution analysis.*",
            )
            report_lines.append("")
    ...
    pass
            ...
            for regime_id in sorted(cluster_counts.index):
                ...
                if (
                    ...
                    if likely_transitions:
                        report_lines.append("- **Most Likely Transitions**:")
                ...
                if block_states_df is not None:
                    ...
                    if regime_mask.sum() > 0:
                        ...
                        report_lines.append("**Block State Characteristics:**")
                ...
                report_lines.append("**Market Condition Interpretation:**")
        ...
        if "composite_cluster_id" in cluster_df.columns:
            ...
            for regime_id, count in cluster_counts.items():
                if regime_id >= 0:  # Skip noise cluster
                    ...
                    if any(
                        ...
                        "sideways" in better_desc_lower
                        or "neutral" in better_desc_lower
                    ):
            ...
            if total_valid_obs > 0:
                ...
                report_lines.append(
                    f"- **Sideways/Neutral**: {sideways_count/total_valid_obs*100:.1f}% of observations ({sideways_count:,} out of {total_valid_obs:,})",
                )
                report_lines.append(
                    f"- **Downtrend**: {downtrend_count/total_valid_obs*100:.1f}% of observations ({downtrend_count:,} out of {total_valid_obs:,})",
                )
                report_lines.append(
                    f"- **Uptrend**: {uptrend_count/total_valid_obs*100:.1f}% of observations ({uptrend_count:,} out of {total_valid_obs:,})",
                )
                report_lines.append("")

                report_lines.append("**⚠️ Important Note on Training Data Period:**")
                report_lines.append(
                    "The HMM analysis reflects market conditions during the **training data period** (historical data used to train the model), not necessarily the current market conditions. The predominance of sideways/neutral and downtrend regimes in this analysis indicates that during the training period, the market was primarily in consolidation or bearish phases.",
                )
                report_lines.append("")
                report_lines.append("**Recent Market Context:**")
                report_lines.append(
                    "Based on recent ETHUSDT price data, the market may be in a different phase than the training period. This suggests that:",
                )
                report_lines.append(
                    "1. The current market conditions may differ significantly from the training period",
                )
                report_lines.append(
                    "2. The model may need retraining with more recent data to capture current market dynamics",
                )
                report_lines.append(
                    "3. The last period (recent data) likely shows more uptrends than downtrends, but this analysis reflects the historical training period",
                )
                report_lines.append("")
                report_lines.append("**Trading Implications:**")
                report_lines.append(
                    "- Use regime-specific strategies based on current market conditions",
                )
                report_lines.append(
                    "- Consider retraining the HMM model with recent data for better current market representation",
                )
                report_lines.append(
                    "- Monitor regime transitions for market condition changes",
                )
                report_lines.append(
                    "- The model's historical bias toward sideways/downtrend conditions may not reflect current bullish market dynamics",
                )
                report_lines.append("")
        ...
        logger.info(
            f"✅ DEBUG: _generate_simple_hmm_report completed successfully for {timeframe}",
        )
        return "\n".join(report_lines)

    except Exception as e:
        logger.exception(
            f"❌ ERROR in _generate_simple_hmm_report for {timeframe}: {str(e)}",
        )
        logger.exception(f"❌ ERROR traceback: {traceback.format_exc()}")
        return f"# HMM Regime Report\n\nError generating report: {str(e)}\n\nTraceback: {traceback.format_exc()}"

def _generate_comprehensive_metrics_report(
    metrics: CompositeModelMetrics,
    symbol: str,
    timeframe: str,
    exchange: str,
) -> str:
    ...
    report_lines.append("COMPOSITE MODEL METRICS REPORT")
    report_lines.append(
...
async def _calculate_and_integrate_hmm_features(
    ...
    pass
        ...
        if not clusters_file.exists():
            logger.warning(
                "⚠️ HMM clusters file not found - skipping HMM feature integration",
            )
            return
        ...
        if not all(f.exists() for f in step2_files):
            logger.warning(
                "⚠️ Step2 features not found - HMM features will be integrated later",
            )
            return
        ...
        for split_file in step2_files:
            ...
            aligned_intensity = (
                intensity_df.reindex(step2_df.index)
            ...
            logger.info(
                f"✅ Added {len(hmm_features_added)} HMM features to {split_name} split",
            )
        ...
        logger.exception(f"🚨 Error integrating HMM features: {e}")
    ...
    **kwargs: Any,
    ...
    pass
        ...
        if not symbol or not exchange or not data_dir:
            logger.error(
                "❌ Missing required parameters: symbol, exchange, or data_dir",
            )
            return False
        ...
        meta_file = (
            output_dir / f"{exchange}_{symbol}_hmm_composite_meta_{timeframe}.json"
        )

        if clusters_file.exists() and meta_file.exists() and not force_rerun:
            logger.info(
                f"✅ HMM regime discovery results already exist for {exchange}_{symbol}_{timeframe}",
            )
        ...
        logger.info(
            f"✅ Created {len(features_df.columns)} features for HMM regime discovery",
        )
        ...
        for tf in timeframes_to_process:
            ...
            for blk_name, X_blk in block_features.items():
                logger.info(
                    f"🎯 Training HMM for block '{blk_name}' with {blk.n_states} states",
                )
                ...
                logger.info(
                    f"✅ Block '{blk_name}': {len(np.unique(states))} unique states",
                )
            ...
            block_out_path = (
                output_dir / f"{exchange}_{symbol}_hmm_block_states_{tf}.parquet"
            )
            _persist_dataframe(block_df, block_out_path)
            ...
            composite_out_path = (
                output_dir / f"{exchange}_{symbol}_hmm_composite_clusters_{tf}.parquet"
            )
            _persist_dataframe(composite_df, composite_out_path)
            ...
            intensity_out_path = (
                output_dir / f"{exchange}_{symbol}_hmm_composite_intensity_{tf}.parquet"
            )
            _persist_dataframe(intensity_df, intensity_out_path)
            ...
            meta_info = {
                ...
                "total_states": sum(
                    len(np.unique(states)) for states in block_states.values()
                ),
            ...
            meta_out_path = (
                output_dir / f"{exchange}_{symbol}_hmm_composite_meta_{tf}.json"
            )
        ...
        await _calculate_and_integrate_hmm_features(
            symbol,
            exchange,
            timeframe,
            data_dir,
...
async def implement_hmm_regime_discovery(
    ...
    pass
        ...
        output_dir = Path(data_dir) / "hmm_regimes"
        clusters_file = (
            output_dir / f"{symbol}_{exchange}_{timeframe}_composite_clusters.parquet"
        )
        ...
        # Load data for the specific timeframe (including order flow data)
        data_result = await loader.load_unified_data(
            symbol, 
            exchange, 
            timeframe, 
        ...
        if data_result is None or data_result["unified_data"] is None or data_result["unified_data"].empty:
            logger.error(f"❌ No unified data found for {symbol} on {exchange}")
            return False

        data = data_result["unified_data"]
        order_flow_data = data_result.get("order_flow_data")
        
        if order_flow_data is not None and not order_flow_data.empty:
            logger.info(f"✅ Loaded {len(order_flow_data)} order flow features")
        else:
            logger.warning("⚠️ No order flow data available")
        ...
        # Pass order flow data to feature engineering
        features_df = create_basic_features(price_df, order_flow_data=order_flow_data)
        ...
        logger.exception(f"🚨 Error in HMM regime discovery implementation: {e}")
        return False
