...
class BasicReturnsEngineer:
    ...
    def _add_basic_returns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.info(f"✅ Feature engineering complete: {len(featured_df.columns) - len(df.columns)} new features added")
            ...
            # This is critical to avoid losing newly added feature columns
            all_original_cols = set(df.columns)
            all_featured_cols = set(featured_df.columns)
            new_cols = all_featured_cols - all_original_cols
            if new_cols:
                self.logger.debug(f"✅ Preserving {len(new_cols)} new feature columns: {sorted(new_cols)}")
