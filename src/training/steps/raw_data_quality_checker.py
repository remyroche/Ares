...
class RawDataQualityChecker:
    ...
    def handle_validation_errors(func):
        ...
        def wrapper(self, data: pd.DataFrame, *args, **kwargs):
            try:
                ...
                self.logger.error(f"❌ {func.__name__}: Validation error: {e}")
    ...
    def validate_raw_data(
        self, data: pd.DataFrame, symbol: str, exchange: str, auto_download_missing: bool = False
    ...
    def _validate_data_structure(
        ...
        if not isinstance(data.index, pd.DatetimeIndex):
            self.logger.warning(f"⚠️ Data does not have datetime index, attempting to fix...")
            data = self._fix_datetime_index(data, results)
            if data is None:
                results["critical_issues"].append("Failed to create datetime index from data")
                return False
            else:
                self.logger.info(f"✅ Successfully created datetime index")
    ...
    def _fix_datetime_index(self, data: pd.DataFrame, results: Dict[str, Any]) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            try:
                if data.index.dtype == 'object':
                    ...
                    if not timestamps.isna().all():
                        fixed_data = data.copy()
                        fixed_data.index = timestamps
                        self.logger.info(f"✅ Created datetime index from existing index")
                        return fixed_data
            except Exception as e:
            ...
            self.logger.error(f"❌ Failed to create datetime index: {e}")
            return None
    ...
    def preprocess_irregular_intervals(self, data: pd.DataFrame, method: str = "forward_fill") -> pd.DataFrame:
        ...
        self.logger.info(f"🔧 Using frequency: {freq} (expected interval: {expected_interval})")
        
        if method == "forward_fill":
            # Resample to regular intervals and forward-fill missing values
            resampled = data.resample(freq).ffill()
            self.logger.info(f"✅ Forward-filled {len(resampled) - len(data)} missing intervals")
            return resampled
            
        elif method == "interpolate":
            # Resample and interpolate missing values
            resampled = data.resample(freq).interpolate(method='time')
            self.logger.info(f"✅ Interpolated {len(resampled) - len(data)} missing intervals")
            return resampled
            
        elif method == "resample":
            # Simple resampling without filling
            resampled = data.resample(freq).last()
            self.logger.info(f"✅ Resampled to regular intervals, removed {len(data) - len(resampled)} irregular intervals")
            return resampled
    ...
    def _handle_missing_data_download(
        ...
        try:
            ...
            self.logger.error(f"❌ Error in missing data download process: {e}")
            download_summary["download_errors"] += 1
            return data, download_summary
                ...
                if downloaded_data is not None and not downloaded_data.empty:
                    ...
                    if start_time and end_time:
                        ...
                        if not filtered_data.empty:
                            ...
                            self.logger.warning(f"⚠️ No data found in downloaded data for specified time range")
                            return None
                    else:
    ...
    def _load_downloaded_data(self, symbol: str, exchange: str, timeframe: str) -> Optional[pd.DataFrame]:
        """
    ...
    def validate_and_preprocess_data(self, data: pd.DataFrame, symbol: str, exchange: str, auto_preprocess: bool = True) -> tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        if needs_preprocessing and auto_preprocess:
            ...
            validation_results["preprocessing_applied"] = {
                ...
                "preprocessed_quality_score": preprocessed_validation.get("data_quality_score", 0),
