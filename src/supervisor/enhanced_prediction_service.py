...
from src.core.decorators import handles_errors
    ...
    def _calculate_step11_calibrated_confidence(
        ...
        try:
            ...
            self.logger.debug(
                f"Step 11 calibration for {model_name}: base={base_confidence:.3f}, "
    ...
    def _validate_price_action_probabilities(self, price_action_probabilities: Dict[str, Any], model_name: str) -> bool:
        ...
        try:
            ...
            if barrier_avoidance < triple_barrier * 0.5:
                self.logger.warning(
                    warning(
                        f"⚠️ Barrier avoidance probability ({barrier_avoidance}) seems too low relative to triple barrier ({triple_barrier}) for model {model_name}"
                    )
                )
                return False
    ...
    async def verify_all_models_probability_outputs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_models = (
                verification_results["summary"]["total_analyst_models"]
                + verification_results["summary"]["total_tactician_models"]
            )
            models_with_probabilities = (
                verification_results["summary"]["analyst_models_with_probabilities"]
                + verification_results["summary"]["tactician_models_with_probabilities"]
            )

            verification_results["summary"]["all_models_verified"] = (
                total_models > 0 and total_models == models_with_probabilities
            )
