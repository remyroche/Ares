...
and loading them back. It ensures consistent model data structure across all training
steps.
"""
...
def save_multi_output_model_with_probabilities(
    model_data: Dict[str, Any], model_path: str, save_format: str = "joblib"
) -> Dict[str, Any]:
    ...
    try:
        ...
        if multi_output_trainer and multi_output_models:
            ...
            market_data = model_data.get(
                "market_data",
                pd.DataFrame(
                    {"close": np.random.randn(100), "volume": np.random.randn(100)}
                ),
            )
        ...
        standardized_model_data = {
            ...
            "ensemble_weights": (
                multi_output_trainer.ensemble_weights if multi_output_trainer else None
            ),
            "calibrators": (
                multi_output_trainer.calibrators if multi_output_trainer else None
            ),
            "price_action_probabilities": price_action_probabilities,
            "training_date": model_data.get(
                "training_date", datetime.now().isoformat()
        ...
        if model_type == "multi_output":
            ...
            for field in required_fields:
                if field not in model_data:
                    logger.warning(
                        f"Missing required field in multi-output model: {field}"
                    )
...
def validate_model_probabilities(model_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if model_type == "multi_output":
            ...
            if (
                not hasattr(multi_output_trainer, "is_trained")
                or not multi_output_trainer.is_trained
            ):
...
def update_model_probabilities(
    model_path: str, new_probabilities: Dict[str, float], save_format: str = "joblib"
) -> bool:
...
def generate_and_save_model_probabilities(
    ...
    try:
        ...
        price_action_probabilities = (
            probability_generator.generate_price_action_probabilities(
        ...
        if not probability_generator.validate_probabilities(price_action_probabilities):
            ...
            price_action_probabilities = (
                probability_generator._get_default_probabilities(model_type)
            )
