...
This step performs feature vectorization as part of the unified data-driven pipeline
...
class FeatureGenerationVectorizationStep:
    """Vectorization step that calls the consolidated pipeline."""
    ...
    async def execute(self,
                     ...
                     custom_overrides: Optional[Dict[str, Any]] = None) -> VectorizationResult:
        """Execute vectorization step using consolidated pipeline."""
        
        self.logger.info("🔢 Starting vectorization step using consolidated pipeline")
        
        try:
            # Call the consolidated pipeline runner
            result = await run_vectorization_step(
                data=data,
                symbol=symbol,
                timeframe=timeframe,
                direction=direction,
            ...
            # Convert result to VectorizationResult
            vectorization_result = VectorizationResult(
                success=result['success'],
                vectorized_features=result.get('vectorized_features', pd.DataFrame()),
                vectorization_metadata=result.get('vectorization_metadata', {}),
            ...
            if vectorization_result.success:
                self.logger.info(f"✅ Vectorization completed successfully with {len(vectorization_result.vectorized_features.columns)} vectorized features")
            else:
                self.logger.error(f"❌ Vectorization failed: {vectorization_result.error_message}")
            
            return vectorization_result
            
        except Exception as e:
            self.logger.error(f"❌ Vectorization step failed with exception: {e}")
