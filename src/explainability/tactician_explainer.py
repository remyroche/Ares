...
class TacticianExplainer(BaseExplainer):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.feature_groups = {
            'market_conditions': ['volatility', 'volume', 'price_momentum', 'trend_strength'],
            'technical_indicators': ['rsi', 'macd', 'bb_position', 'atr', 'adx'],
            'regime_factors': ['regime_probability', 'regime_confidence', 'regime_transition_prob'],
            'sr_factors': ['sr_proximity', 'sr_strength', 'sr_breakout_probability'],
            'risk_factors': ['liquidation_risk', 'correlation_risk', 'volatility_risk'],
            'scenario_factors': ['scenario_probability', 'scenario_confidence', 'scenario_dominance']
        }
        
    async def explain_scenario_prediction(
        self,
        scenario_predictor: Any,
        ...
        try:
            ...
            if self.lime_explainer is not None:
                lime_explanation = self._calculate_lime_explanation(
                    self.lime_explainer, scenario_predictor, features, feature_names
                )
            ...
            explanation = ExplanationResult(
                ...
                metadata={
                    'model_type': 'scenario_predictor',
                    'feature_groups': self.feature_groups,
                    'explanation_type': 'scenario_prediction'
                }
            )
    ...
    async def explain_position_sizing(
        ...
        try:
            ...
            if self.lime_explainer is not None:
                lime_explanation = self._calculate_lime_explanation(
                    self.lime_explainer, position_sizer, features, feature_names
                )
            ...
            # Create explanation result
            explanation = ExplanationResult(
                model_name="Tactician_PositionSizing",
                ...
                confidence=confidence,
                metadata={
                    'model_type': 'position_sizer',
                    'feature_groups': self.feature_groups,
                    'explanation_type': 'position_sizing'
                }
            )
    ...
    async def explain_leverage_decision(
        self,
        leverage_sizer: Any,
        market_data: pd.DataFrame,
        features: np.ndarray,
        ...
        try:
            ...
            if self.lime_explainer is not None:
                lime_explanation = self._calculate_lime_explanation(
                    self.lime_explainer, leverage_sizer, features, feature_names
                )
            ...
            explanation = ExplanationResult(
                ...
                confidence=confidence,
                metadata={
                    'model_type': 'leverage_sizer',
                    'feature_groups': self.feature_groups,
                    'explanation_type': 'leverage_decision'
                }
            )
    ...
    async def explain_prediction(
        ...
        try:
            ...
            if model_type == 'scenario_predictor':
                return await self.explain_scenario_prediction(
                    model, pd.DataFrame(), features, feature_names, prediction
                )
            elif model_type == 'position_sizer':
                return await self.explain_position_sizing(
                    model, pd.DataFrame(), features, feature_names, prediction
                )
            elif model_type == 'leverage_sizer':
                return await self.explain_leverage_decision(
                    model, pd.DataFrame(), features, feature_names, prediction
                )
            else:
                # Generic explanation
                return await self._explain_generic_model(
    ...
    async def _explain_generic_model(
        self,
        model: Any,
        features: np.ndarray,
        feature_names: List[str],
    ...
    def _calculate_prediction_confidence(self, prediction: Any, shap_values: Optional[np.ndarray]) -> float:
        ...
        try:
            ...
            if shap_values is not None:
                # Higher confidence if SHAP values are more consistent
            ...
            if isinstance(prediction, (int, float)):
                # For numerical predictions, confidence based on magnitude
                if abs(prediction) > 0.1:
                    confidence += 0.1
            elif isinstance(prediction, dict):
                # For dictionary predictions, confidence based on key consistency
