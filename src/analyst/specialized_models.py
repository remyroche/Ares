...
class SpecializedModels:
    ...
    def __init__(self, config):
        ...
        self.global_config = config # Store global config to access other sections like sr_proximity_pct
    ...
    def get_sr_interaction_signal(self, current_price: float, sr_levels: list, current_atr: float):
        ...
        proximity_multiplier = self.global_config['BEST_PARAMS'].get("proximity_multiplier", 
        ...
        for level in sr_levels:
            if level["current_expectation"] in ["Very Strong", "Strong", "Moderate"]: # Only consider relevant levels
                ...
                if abs(current_price - level_price) <= tolerance_abs:
                    distance_pct = abs(current_price - level_price) / current_price if current_price > 0 else float('inf')
    ...
    def get_high_impact_candle_signal(self, klines_df: pd.DataFrame):
        ...
        min_data_required = max(volume_sma_period, atr_sma_period) + 1 # Need at least this many candles for SMA + current

        if klines_df.empty or len(klines_df) < min_data_required:
            return {"is_high_impact": False, "reason": f"Insufficient data ({len(klines_df)} candles). Need at least {min_data_required}."}
        ...
        prev_close = prev_candles['close'].iloc[-1] if not prev_candles.empty else current_close
