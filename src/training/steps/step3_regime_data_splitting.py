...
class RegimeDataSplittingStep:
    ...
    async def execute(self, training_input: Dict[str, Any], pipeline_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not os.path.exists(regime_file_path):
                raise FileNotFoundError(f"Regime classification file not found: {regime_file_path}")
            
            with open(regime_file_path, 'r') as f:
                regime_results = json.load(f)
            
            # Split data by regimes
            regime_data = await self._split_data_by_regimes(historical_data, regime_results, symbol, exchange)
            ...
            summary_file = f"{data_dir}/{exchange}_{symbol}_regime_splitting_summary.json"
            with open(summary_file, 'w') as f:
                json.dump(splitting_summary, f, indent=2)
            
            self.logger.info(f"✅ Regime data splitting completed. Results saved to {regime_data_dir}")
    ...
    async def _split_data_by_regimes(self, data: pd.DataFrame, regime_results: Dict[str, Any], 
                                   symbol: str, exchange: str) -> Dict[str, pd.DataFrame]:
        """
        ...
        try:
            ...
            if not regime_sequence:
                raise ValueError("No regime sequence found in regime classification results")
            
            # Ensure data and regime sequence have the same length
            if len(data) != len(regime_sequence):
                self.logger.warning(f"Data length ({len(data)}) doesn't match regime sequence length ({len(regime_sequence)})")
            ...
            for regime in regime_data:
                ...
                if 'timestamp' in regime_data[regime].columns:
                    regime_data[regime]['timestamp'] = pd.to_datetime(regime_data[regime]['timestamp'])
                    regime_data[regime] = regime_data[regime].set_index('timestamp')
                
                self.logger.info(f"Regime '{regime}': {len(regime_data[regime])} records")
            
            # Handle high-data vs low-data regimes
            high_data_regimes = ['BULL', 'BEAR', 'SIDEWAYS']
            low_data_regimes = ['SUPPORT_RESISTANCE', 'CANDLES', 'HUGE_CANDLE', 'SR_ZONE_ACTION']
            ...
            self.logger.info(f"High-data regimes: {list(regime_categories['high_data'].keys())}")
            self.logger.info(f"Low-data regimes: {list(regime_categories['low_data'].keys())}")
            self.logger.info(f"Other regimes: {list(regime_categories['other'].keys())}")
