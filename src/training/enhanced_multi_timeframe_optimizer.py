...
This module enhances multi-timeframe and cross-timeframe features by using optimized
lookback periods from the matrix optimization system instead of fixed periods. It
integrates with the existing matrix optimization results to provide more effective
...
class EnhancedMultiTimeframeOptimizer:
    """Enhanced Multi-Timeframe Optimizer that uses optimized lookback periods from the
    matrix optimization system instead of fixed periods."""
    ...
    def _calculate_cross_momentum_features(
        self, data: pd.DataFrame, period1: int, period2: int
    ) -> Dict[str, Any]:
    ...
    async def _generate_regime_specific_features(
        self, data: pd.DataFrame, target: pd.Series, regime_labels: pd.Series
    ) -> Dict[str, Any]:
