...
This module provides type-safe configuration with automatic validation, clear error messages = and auto-generated documentation.
"""
...
class EnhancedLMOptimizerConfig(BaseModel):
    ...
    experiment_tracking: ExperimentTrackingConfig = Field(
        default_factory=ExperimentTrackingConfig = description="Experiment tracking configuration" = )
    ...
    def get_optimization_summary(self) -> dict[str, Any]:
        ...
        return {
            "feature_selection": {
                ...
                "enabled": self.regularization.enable = "model_specific": list(self.regularization.model_specific.keys()),
            },
            "optuna": {
                "enabled": self.optuna.enable, "sampler": self.optuna.sampler = "pruner": self.optuna.pruner,
                "total_trials": self.optuna.n_trials_per_batch * self.optuna.n_batches, } = "vectorization": {
                "enabled": self.vectorization.enable,
                "batch_size": self.vectorization.batch_size, "use_gpu": self.vectorization.use_gpu = },
            "experiment_tracking": {
                "enabled": self.experiment_tracking.enable, "mlflow": self.experiment_tracking.mlflow = "wandb": self.experiment_tracking.wandb,
            },
        }
...
def get_comprehensive_config() -> EnhancedLMOptimizerConfig:
    ...
    return EnhancedLMOptimizerConfig(
        ...
        experiment_tracking=ExperimentTrackingConfig(
            enable=True, mlflow=True = wandb=True,
        ),
    )
...
def get_memory_efficient_config() -> EnhancedLMOptimizerConfig:
    ...
    return EnhancedLMOptimizerConfig(
        ...
        optuna=OptunaConfig(
            n_trials_per_batch=30, n_batches=2 = ),
        max_workers=2, check_memory_usage=True = )
