...
This module analyzes the relevance of different timeframes for high leverage trading
(10x-100x) and optimizes the ensemble configuration accordingly.
"""
...
class TimeframeRelevanceAnalyzer:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # High leverage trading parameters
        self.leverage_config = config.get(
        ...
        self.analysis_config = config.get(
            "timeframe_analysis",
            {
    ...
    async def analyze_timeframe_relevance(
        ...
        """
        self.logger.info(
            f"🎯 Starting timeframe relevance analysis for {symbol} on {exchange}"
        )
        self.logger.info(
            f"💰 Analyzing for leverage range: {leverage_range[0]}x - {leverage_range[1]}x"
        )
        ...
        # 4. Optimize ensemble configuration
    ...
    async def _analyze_single_timeframe(
        self, data: pd.DataFrame, timeframe: str, leverage_range: Tuple[int, int]
    ) -> dict[str, Any]:
        ...
        if "close" in data.columns:
            ...
            analysis["volatility_metrics"] = {
                ...
                "volatility_regime_stability": self._calculate_volatility_stability(
                    returns
                ),
            }

        # Calculate leverage-specific metrics
        analysis["leverage_metrics"] = await self._calculate_leverage_metrics(
            data, leverage_range
        )
        ...
        for timeframe, data in data_dict.items():
            if "close" in data.columns and len(data) > 0:
                ...
                volatility_data[timeframe] = {
                    ...
                    "rolling_vol": returns.rolling(20).std().mean() * np.sqrt(252),
                    "vol_regime_changes": self._count_volatility_regime_changes(
                        returns
                    ),
                }
            ...
            for j, tf2 in enumerate(timeframes[i + 1 :], i + 1):
                if tf1 in volatility_data and tf2 in volatility_data:
                    ...
                    corr = self._calculate_volatility_correlation(
                        data_dict[tf1], data_dict[tf2]
                    )
                    vol_correlations[f"{tf1}_vs_{tf2}"] = corr

        return {
            "timeframe_volatilities": volatility_data,
            "volatility_correlations": vol_correlations,
            "volatility_regime_analysis": self._analyze_volatility_regimes(
                volatility_data
            ),
        }

    async def _analyze_signal_quality(
        self, data_dict: dict[str, pd.DataFrame], leverage_range: Tuple[int, int]
    ) -> dict[str, Any]:
        ...
        for timeframe, data in data_dict.items():
            ...
            # Calculate signal quality metrics
            quality_metrics = await self._calculate_signal_quality_metrics(
                data, timeframe, leverage_range
            )
        ...
        return {
            "timeframe_signal_quality": signal_quality,
            "signal_decay_analysis": signal_decay_analysis,
            "optimal_signal_horizon": self._calculate_optimal_signal_horizon(
                signal_quality
            ),
        }

    async def _optimize_ensemble_configuration(
        ...
        for timeframe, analysis in timeframe_analysis.items():
            ...
            signal_quality = (
                signal_analysis.get("timeframe_signal_quality", {})
                .get(timeframe, {})
            ...
            volatility_factor = 1.0 - min(
                analysis["volatility_metrics"].get("daily_volatility", 0.5), 1.0
            )
            leverage_factor = analysis["leverage_metrics"].get(
        ...
        if total_score > 0:
            ...
            for timeframe, weight in weights.items():
                ...
                normalized_weight = max(
                    self.analysis_config["ensemble_weight_min"],
                    min(normalized_weight, self.analysis_config["ensemble_weight_max"]),
                )
        ...
        return {
            ...
            "weight_justification": self._generate_weight_justification(
                timeframe_analysis, normalized_weights
            ),
            "ensemble_performance_estimate": self._estimate_ensemble_performance(
                timeframe_analysis, normalized_weights
            ),
        }
    ...
    async def _generate_recommendations(
        ...
        if weights:
            recommendations["ensemble_recommendations"] = {
                "primary_timeframes": [tf for tf, w in weights.items() if w > 0.2],
                "secondary_timeframes": [
                    tf for tf, w in weights.items() if 0.1 <= w <= 0.2
                ],
                "excluded_timeframes": [
                    tf for tf in timeframe_analysis.keys() if tf not in weights
                ],
                "weight_distribution": weights,
            }

        # Risk management recommendations
        recommendations["risk_management_recommendations"] = {
            "max_leverage": self._calculate_safe_leverage(analysis_results),
            "position_sizing": self._calculate_position_sizing(analysis_results),
            "stop_loss_recommendations": self._calculate_stop_loss_recommendations(
                analysis_results
            ),
        }
        ...
        recommendations["general_recommendations"] = [
            "Consider reducing ensemble complexity for high leverage trading",
            "Focus on timeframes with stable volatility patterns",
            "Implement dynamic position sizing based on volatility regime",
            "Use regime-specific stop-loss levels",
        ]
    ...
    async def _calculate_leverage_metrics(
        self, data: pd.DataFrame, leverage_range: Tuple[int, int]
    ) -> dict[str, Any]:
        """Calculate leverage-specific metrics."""
        ...
        # Calculate max safe leverage (inverse of volatility)
        ...
        # Calculate leverage volatility ratio
    ...
    async def _calculate_signal_metrics(self, data: pd.DataFrame) -> dict[str, Any]:
        ...
        # In practice, you'd calculate actual signal metrics based on your signal generation logic
        ...
        if "close" in data.columns and len(data) > 100:
            ...
            # Signal persistence (how long signals last)
            ...
            metrics["overall_quality"] = (
                metrics["signal_stability"] * 0.4
                + metrics["signal_persistence"] * 0.3
                + 0.3  # Placeholder for signal accuracy
            )
    ...
    def _calculate_signal_persistence(self, returns: pd.Series) -> float:
        ...
        return min(max_persistence / 10.0, 1.0)  # Normalize to 0-1
    ...
    async def _calculate_relevance_score(self, analysis: dict[str, Any]) -> float:
        ...
        # Extract metrics
        volatility = analysis["volatility_metrics"].get("daily_volatility", 0.5)
        leverage_efficiency = analysis["leverage_metrics"].get(
            "leverage_efficiency", 0.0
        )
        ...
        vol_score = max(
            0.0, 1.0 - volatility / self.leverage_config["volatility_threshold"]
        )
    ...
    async def _calculate_signal_quality_metrics(
        self, data: pd.DataFrame, timeframe: str, leverage_range: Tuple[int, int]
    ) -> dict[str, Any]:
        ...
        # Overall quality
        metrics["overall_quality"] = (
            (1.0 - metrics["signal_decay_rate"]) * 0.4
            + metrics["signal_noise_ratio"] * 0.3
            + metrics["signal_consistency"] * 0.3
        )
    ...
    def _calculate_signal_decay_rate(self, returns: pd.Series) -> float:
        ...
        # Decay rate is the rate at which autocorrelation decreases
        if autocorr_1 > 0:
    ...
    def _calculate_signal_noise_ratio(self, returns: pd.Series) -> float:
        """Calculate signal-to-noise ratio."""
        # Signal is the trend component, noise is the random component
    ...
    def _calculate_signal_consistency(self, returns: pd.Series) -> float:
        """Calculate signal consistency."""
        # Measure how consistent the directional signals are
        direction = np.sign(returns)
        consistency = (
            direction.rolling(10)
    ...
    def _analyze_signal_decay(self, signal_quality: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "timeframe_decay_rates": decay_rates,
            "optimal_holding_period": self._calculate_optimal_holding_period(
                decay_rates
            ),
        }

    def _calculate_optimal_holding_period(
    ...
    def _calculate_optimal_signal_horizon(
        ...
        for timeframe, metrics in signal_quality.items():
            ...
            # Signal horizon is inversely related to decay rate
            horizon = int(5.0 / (1.0 + decay_rate))
            horizons[timeframe] = max(1, min(horizon, 20))

        return horizons

    def _generate_weight_justification(
        self, timeframe_analysis: dict[str, Any], weights: dict[str, float]
    ) -> dict[str, str]:
        ...
        for timeframe, weight in weights.items():
            if timeframe in timeframe_analysis:
                ...
                if weight > 0.3:
                    justifications[timeframe] = (
                        f"High relevance ({relevance_score:.2f}) and low volatility ({volatility:.3f})"
                    )
                elif weight > 0.15:
                    justifications[timeframe] = (
                        f"Moderate relevance ({relevance_score:.2f}) with acceptable volatility ({volatility:.3f})"
                    )
                else:
                    justifications[timeframe] = (
                        f"Lower weight due to relevance score ({relevance_score:.2f}) and volatility ({volatility:.3f})"
                    )
    ...
    def _estimate_ensemble_performance(
        ...
        return {
            ...
            "estimated_relevance": avg_relevance,
            "risk_adjusted_score": avg_relevance / (1.0 + avg_volatility),
        }
    ...
    def _calculate_position_sizing(
        ...
        return {
            ...
            "volatility_adjusted_size": base_position_size * (1.0 - avg_volatility),
            "max_position_size": min(1.0, base_position_size * 1.5),
        }

    def _calculate_stop_loss_recommendations(
        self, analysis_results: dict[str, Any]
    ) -> dict[str, float]:
    ...
    async def _save_analysis_results(
        self, results: dict[str, Any], symbol: str, exchange: str
    ):
    ...
    def get_optimized_ensemble_config(
        ...
        filepath = Path(
            f"data/timeframe_analysis/{exchange}_{symbol}_timeframe_analysis.json"
        )

        if not filepath.exists():
            self.logger.warning(
                f"⚠️ No timeframe analysis found for {symbol} on {exchange}"
            )
            return {}
