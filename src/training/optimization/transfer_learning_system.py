...
class ProblemSimilarityDetector:
    ...
    def calculate_similarity(self, problem1: ProblemSignature, problem2: ProblemSignature) -> float:
        ...
        feature_similarity = self._calculate_feature_similarity(problem1.feature_vector, problem2.feature_vector)
        ...
        weights = self.config.get('similarity_weights', {'feature': 0.4, 'structural': 0.4, 'domain': 0.2})
        return weights['feature'] * feature_similarity + weights['structural'] * structural_similarity + weights['domain'] * domain_similarity
    ...
    def _calculate_feature_similarity(self, features1: np.ndarray, features2: np.ndarray) -> float:
        ...
        try:
            ...
            euclidean_dist = euclidean_distances(features1.reshape(1, -1), features2.reshape(1, -1))[0, 0]
...
class KnowledgeTransferManager:
    ...
    def find_similar_problems(self, target_problem: ProblemSignature, similarity_threshold: float=0.7, max_results: int=5) -> list[tuple[OptimizationHistory, float]]:
        """Find problems similar to the target problem."""
        similarities = []
        for history in self.optimization_history:
            similarity = self.similarity_detector.calculate_similarity(target_problem, history.problem_signature)
            if similarity >= similarity_threshold:
                similarities.append((history, similarity))
        similarities.sort(key=lambda x: x[1], reverse=True)
        return similarities[:max_results]

    def transfer_knowledge(self, target_problem: ProblemSignature, source_problems: list[OptimizationHistory], similarity_scores: list[float]) -> TransferKnowledge:
        ...
        transfer_knowledge = TransferKnowledge(source_problem_id=','.join([p.problem_id for p in source_problems]), target_problem_id=target_problem.problem_id, similarity_score=np.mean(similarity_scores), transferred_models=transferred_models, transferred_hyperparameters=transferred_hyperparameters, transferred_strategies=transferred_strategies, adaptation_weights=adaptation_weights, transfer_timestamp=time.time(), transfer_effectiveness=0.0)
        self.transfer_knowledge.append(transfer_knowledge)
        self._save_history()
        return transfer_knowledge

    def adapt_transferred_knowledge(self, transfer_knowledge: TransferKnowledge, target_parameter_space: dict[str, Any]) -> dict[str, Any]:
    ...
    def update_transfer_effectiveness(self, transfer_knowledge: TransferKnowledge, optimization_performance: dict[str, Any]) -> None:
...
class MetaLearner:
    ...
    def extract_problem_features(self, problem_signature: ProblemSignature) -> np.ndarray:
        ...
        features = [problem_signature.dimensionality, problem_signature.constraint_count, problem_signature.noise_level, problem_signature.complexity_score, len(problem_signature.parameter_bounds)]
        features.extend(problem_signature.feature_vector)
        return np.array(features)

    def add_training_example(self, problem_signature: ProblemSignature, strategy_used: str, hyperparameters: dict[str, Any], performance: float) -> None:
        """Add a training example for meta-learning."""
        features = self.extract_problem_features(problem_signature)
        training_example = {'features': features, 'strategy': strategy_used, 'hyperparameters': hyperparameters, 'performance': performance}
    ...
    def train_meta_models(self) -> None:
        ...
        try:
            ...
            self.logger.exception(f'Error training meta-models: {e}')
...
class TransferLearningOptimizer:
    ...
    def optimize_with_transfer(self, objective_function: Callable, parameter_space: dict[str, Any], problem_metadata: dict[str, Any]=None) -> dict[str, Any]:
        """Optimize with transfer learning capabilities."""
        problem_signature = self._create_problem_signature(objective_function, parameter_space, problem_metadata)
        similar_problems = []
        if self.enable_transfer:
            similar_problems_with_scores = self.knowledge_manager.find_similar_problems(problem_signature, self.similarity_threshold, self.max_source_problems)
            if similar_problems_with_scores:
                ...
                transfer_knowledge = self.knowledge_manager.transfer_knowledge(problem_signature, similar_problems, similarity_scores)
                adapted_knowledge = self.knowledge_manager.adapt_transferred_knowledge(transfer_knowledge, parameter_space)
                optimization_config = self._create_optimization_config_with_transfer(adapted_knowledge, problem_signature)
            else:
        ...
        if similar_problems and self.enable_transfer:
            self.knowledge_manager.update_transfer_effectiveness(transfer_knowledge, optimization_results)
        self.meta_learner.add_training_example(problem_signature, optimization_config.get('strategy', 'default'), optimization_config.get('hyperparameters', {}), optimization_results.get('best_score', 0.0))
        optimization_history = OptimizationHistory(problem_id=problem_signature.problem_id, problem_signature=problem_signature, parameter_space=parameter_space, objective_function=self._hash_function(objective_function), optimization_results=optimization_results, surrogate_models=optimization_results.get('surrogate_models', {}), best_parameters=optimization_results.get('best_parameters', {}), best_score=optimization_results.get('best_score', 0.0), convergence_history=optimization_results.get('convergence_history', []), training_time=optimization_results.get('training_time', 0.0), completion_timestamp=time.time())
        self.knowledge_manager.add_optimization_history(optimization_history)
        return optimization_results

    def _create_problem_signature(self, objective_function: Callable, parameter_space: dict[str, Any], problem_metadata: dict[str, Any]=None) -> ProblemSignature:
        ...
        return ProblemSignature(problem_id=problem_id, dimensionality=dimensionality, parameter_bounds=parameter_bounds, objective_type='minimization', constraint_count=constraint_count, noise_level=0.0, complexity_score=complexity_score, feature_vector=feature_vector, metadata=problem_metadata or {})

    def _generate_problem_id(self, objective_function: Callable, parameter_space: dict[str, Any]) -> str:
    ...
    def _create_feature_vector(self, parameter_space: dict[str, Any]) -> np.ndarray:
        ...
        for param_config in parameter_space.values():
            if isinstance(param_config, dict):
                if 'min' in param_config and 'max' in param_config:
                    features.extend([param_config['min'], param_config['max'], param_config['max'] - param_config['min']])
                elif 'choices' in param_config:
                    features.extend([len(param_config['choices']), min(param_config['choices']), max(param_config['choices'])])
            elif isinstance(param_config, list | tuple) and len(param_config) == 2:
                features.extend([param_config[0], param_config[1], param_config[1] - param_config[0]])
        return np.array(features)
    ...
    def _create_optimization_config_with_transfer(self, adapted_knowledge: dict[str, Any], problem_signature: ProblemSignature) -> dict[str, Any]:
        """Create optimization configuration using transferred knowledge."""
        return {'strategy': adapted_knowledge.get('strategies', {}).get('strategy', 'default'), 'hyperparameters': adapted_knowledge.get('hyperparameters', {}), 'surrogate_models': adapted_knowledge.get('models', {}), 'warm_start': True, 'transfer_learning': True}

    def _create_default_optimization_config(self, problem_signature: ProblemSignature) -> dict[str, Any]:
        """Create default optimization configuration."""
        return {'strategy': 'default', 'hyperparameters': {'learning_rate': 0.1, 'exploration_balance': 0.3, 'uncertainty_threshold': 0.1}, 'warm_start': False, 'transfer_learning': False}

    def _run_optimization(self, objective_function: Callable, parameter_space: dict[str, Any], optimization_config: dict[str, Any]) -> dict[str, Any]:
        """Run the actual optimization."""
        return {'best_parameters': {}, 'best_score': 0.0, 'convergence_history': [], 'training_time': 0.0, 'surrogate_models': {}, 'strategy_used': optimization_config.get('strategy', 'default'), 'hyperparameters_used': optimization_config.get('hyperparameters', {})}