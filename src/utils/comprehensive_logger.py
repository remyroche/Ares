...
class ComprehensiveLogger:
    ...
    def log_global(self, message: str, level: str = "INFO"):
        ...
        """
        if self.global_logger:
            log_method = getattr(self.global_logger, level.lower(), self.global_logger.info)
            log_method(message)
    ...
    def log_launcher_start(self, mode: str, symbol: str = None, exchange: str = None):
        ...
        self.log_system_info(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def log_launcher_end(self, exit_code: int = 0):
        ...
        self.log_system_info(f"End time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        self.log_system_info("=" * 80)
    ...
    def log_session_summary(self):
        ...
        if self.global_logger:
            ...
            self.global_logger.info(f"Session started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            self.global_logger.info(f"Global log file: ares_global_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
