...
## 2024-05-18 - [Pandas Loop Perf] Learning: Replacing pandas iterrows() with zip() iteration over numpy arrays (using to_numpy()) is crucial for loop performance in critical inference loops, resulting in nearly 100x speedups. Action: Always use zip() with underlying numpy arrays instead of iterrows() for fast data extraction in loops on large DataFrames.
