#!/usr/bin/env python3
"""
Production HDP-HMM Clustering Test with Artifact Manager
Uses real market data from artifact manager and proper feature generation
"""
...
print("=" * 80)
print("Production HDP-HMM Clustering Test with Artifact Manager")
print("=" * 80)
...
artifact_manager.set_context(
    symbol="ETHUSDT",
    exchange="binance",
    information="hdp_hmm_production",
    timeframe="1h"
)
...
if feature_data is not None:
    ...
    # Load raw market data using KlinesParquetManager
    klines_manager = KlinesParquetManager()
    
    # Load recent ETHUSDT data (larger dataset)
    df = klines_manager.read_data(
        symbol="ETHUSDT",
        interval="1h",
        start_date="2023-01-01",  # Extended date range
        end_date="2024-12-31"
    )
    ...
    # Generate regime features for the dataset in chunks
    print("   🔄 Generating regime features for dataset in chunks...")
    regime_integrator = RegimeFeatureIntegration()
    
    # Process data in chunks to generate features for each chunk
    ...
    try:
        ...
        artifact_manager.save(
            ...
            {"generation_method": "RegimeFeatureIntegration", "source": "raw_klines"}
        )
...
if n_features > 20:
    pca_components = min(15, n_features // 3)  # Use 1/3 of features or max 15
elif n_features > 10:
    pca_components = min(10, n_features // 2)  # Use 1/2 of features or max 10
else:
...
configs = [
    HDPHMMConfig(
        alpha=2.0, kappa=20.0, gamma=2.0,  # Lower kappa for more regime changes
        n_iterations=100, max_states=10,
        enable_pca=True, pca_components=pca_components,
        min_samples_required=50
    ),
    HDPHMMConfig(
        alpha=3.0, kappa=30.0, gamma=3.0,
        n_iterations=150, max_states=15,
        enable_pca=True, pca_components=pca_components,
        min_samples_required=50
    ),
    HDPHMMConfig(
        alpha=4.0, kappa=40.0, gamma=4.0,
        n_iterations=200, max_states=20,
        enable_pca=True, pca_components=pca_components,
        min_samples_required=50
    )
]
...
print("\n" + "=" * 80)
print("✅ Best HDP-HMM Clustering Result (Auto-Tuned)")
print("=" * 80)
