#!/usr/bin/env python3
"""Working multi-specialist analysis with available data."""
...
def analyze_available_specialists() -> Dict[str, Dict[str, Any]]:
    """Analyze all available specialist models."""
    
    print("🚀 MULTI-SPECIALIST OPTIMIZATION ANALYSIS")
    print("=" * 60)
...
def analyze_specialist_working(specialist_name: str, store_name: str) -> Dict[str, Any]:
    """Working analysis of a single specialist."""
...
def analyze_working_data(df: pd.DataFrame, specialist_name: str) -> Dict[str, Any]:
    """Working data analysis with all metrics."""
    ...
    pred_cols = [col for col in df.columns if any(x in col.lower() for x in ['force_', 'regime_', 'momentum_', 'volatility_', 'liquidity_', 'breakout_', 'path_', 'smc_', 'reversion_', 'prediction', 'confidence'])]
    
    print(f"   🎯 Target columns: {target_cols}")
    print(f"   🔮 Prediction columns: {pred_cols}")
    
    if not target_cols and not pred_cols:
        print(f"   ⚠️ No target or prediction columns found")
        return analysis
    
    # Handle different data structures
    if target_cols and pred_cols:
        # Standard case: both targets and predictions
    ...
    elif pred_cols and len(pred_cols) >= 2:
        # Case with multiple prediction columns (like SMC)
        pred_col = pred_cols[0]
        predictions = df[pred_col]
        ...
        analysis['prediction_column'] = pred_col
        analysis['target_column'] = 'synthetic_from_prediction'
        analysis['synthetic_target'] = True
    ...
    # 1. Binary output conversion - REQUIREMENT 3
    ...
    # 4. Feature orthogonality - REQUIREMENT 2
    feature_cols = [col for col in df.columns if col not in [analysis.get('target_column', ''), analysis.get('prediction_column', ''), 'timestamp']]
        ...
        for j, spec2 in enumerate(specialist_names[i+1:], i+1):
            ...
            try:
                ...
                if views1 and views2:
                    ...
                    pred1_cols = [col for col in data1.columns if any(x in col.lower() for x in ['force_', 'regime_', 'momentum_', 'volatility_', 'liquidity_', 'breakout_', 'path_', 'smc_', 'reversion_', 'prediction', 'confidence'])]
                    pred2_cols = [col for col in data2.columns if any(x in col.lower() for x in ['force_', 'regime_', 'momentum_', 'volatility_', 'liquidity_', 'breakout_', 'path_', 'smc_', 'reversion_', 'prediction', 'confidence'])]
...
def generate_working_report(analyses: Dict[str, Dict[str, Any]], cross_analysis: Dict[str, Any]) -> str:
    """Generate working optimization report."""
    
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    
    report = f"""# Multi-Specialist Model Optimization Report
...
"""
    ...
    for specialist, analysis in analyses.items():
        ...
        report += f"| {specialist} | {samples} | {features} | {binary} | {mi:.4f} | {auc_str} | {acc_str} | {features} | {req_met}/3 |\n"
...
"""
    ...
    for specialist, analysis in analyses.items():
        ...
        synthetic = analysis.get('synthetic_target', False)
        
        if is_binary:
            status = "✅ BINARY"
            if synthetic:
                method = f"Synthetic target ({threshold:.4f})"
            else:
...
def main():
    """Main working analysis function."""
    
    print("🚀 STARTING MULTI-SPECIALIST OPTIMIZATION ANALYSIS")
    print("=" * 70)
    
    # Analyze all specialists
    all_analyses = analyze_available_specialists()
    ...
    print("\n📝 GENERATING MULTI-SPECIALIST OPTIMIZATION REPORT...")
    report = generate_working_report(all_analyses, cross_analysis)
    
    # Save report
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    report_path = Path("outcomes") / f"multi_specialist_optimization_{timestamp}.md"
    
    with open(report_path, 'w') as f:
        f.write(report)
    
    print(f"✅ MULTI-SPECIALIST REPORT SAVED: {report_path}")
    
    # Print final summary
    print("\n" + "=" * 70)
    print("🎯 MULTI-SPECIALIST OPTIMIZATION SUMMARY")
    print("=" * 70)
    ...
    print(f"\n🎯 NEXT STEPS:")
    print(f"   1. Review multi-specialist report for detailed recommendations")
    ...
    print(f"\n🚀 MULTI-SPECIALIST OPTIMIZATION FRAMEWORK READY FOR PRODUCTION")
