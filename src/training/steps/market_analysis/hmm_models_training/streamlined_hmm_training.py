    ...
    def __init__(self, config: Optional[HMMTrainingConfig] = None):
        ...
        if config is None:
            ...
            if not hasattr(config, 'model_types') or len(config.model_types) == 0:
                config.model_types = ["logistic_regression", "lightgbm", "random_forest"]
...
def create_streamlined_hmm_training(config: Optional[HMMTrainingConfig] = None) -> StreamlinedHMMTrainingStep:
    """
...
def execute_streamlined_hmm_training(
    X: np.ndarray,
    y: np.ndarray,
    regime_labels: np.ndarray,
