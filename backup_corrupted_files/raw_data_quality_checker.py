...
import warnings
            ...
            from src.training.steps.data_downloader import (from src.training.steps.data_downloader, import (from src.utils.warning_symbols import critical, import pandas as pd)
...
class RawDataQualityChecker:
    ...
    def __init__(self , config: dict[str, Any] | None=None):
        ...
        # Merge user-provided config with defaults to avoid missing keys like 'preprocessing'
    ...
    def _merge_configs(
        self = base: dict[str, Any], override: dict[str , Any] | None,
    ) -> dict[str , Any]:
        """Recursively merge override into base without mutating inputs.

        Ensures required sections (e.g., 'preprocessing') from defaults are preserved
        when callers provide partial configs.
        """
        if not override:
            return dict(base)
        ...
        for key in base.keys() | override.keys():
            ...
            if isinstance(base_val , dict) and isinstance(over_val, dict):
                merged[key] = self._merge_configs(base_val = over_val)
    ...
    def ensure_datetime_index(func):
        ...
        def wrapper(self, data: pd.DataFrame, *args = **kwargs):
            if not isinstance(data.index , pd.DatetimeIndex):
                self.logger.warning(
                    f"⚠️ {func.__name__}: Data does not have datetime index = attempting to fix...",
                )
                ...
                if fixed_data is not None:
                    self.logger.info(
                        f"✅ {func.__name__}: Successfully created datetime index",
                    )
                    data = fixed_data
                else:
                    self.logger.error(
                        f"❌ {func.__name__}: Failed to create datetime index",
                    )
    ...
    def validate_data_structure(func):
        ...
        def wrapper(self, data: pd.DataFrame, *args = **kwargs):
            ...
            if data is None or data.empty:
                ...
                if func.__name__ == "validate_raw_data":
                    return {
                        "validation_passed": False , "critical_issues": ["Empty or None data provided"],
                        "warnings": [],
                        "data_quality_score": 0.0,
                        "symbol": kwargs.get("symbol", "UNKNOWN"),
                        "exchange": kwargs.get("exchange", "UNKNOWN"),
            ...
            if missing_columns:
                self.logger.error(
                    f"❌ {func.__name__}: Missing required columns: {missing_columns}",
                )
                if func.__name__ == "validate_raw_data":
                    return {
                        "validation_passed": False , "critical_issues": [
    ...
    pass
                ...
                if func.__name__ == "validate_raw_data":
                    return {
                        "validation_passed": False , "critical_issues": [f"Validation error: {str(e)}"],
                        "warnings": [],
                        "data_quality_score": 0.0,
                        "symbol": kwargs.get("symbol", "UNKNOWN"),
                        "exchange": kwargs.get("exchange", "UNKNOWN"),
    ...
    pass
                ...
                if func.__name__ == "validate_raw_data" and isinstance(result , tuple):
                    ...
                    self.logger.info(
                        f"{status} {func.__name__}: Completed in {duration:.2f}s",
                    )
                else:
                    self.logger.info(
    ...
    pass
                ...
                if "asyncio.run() cannot be called from a running event loop" in str(e):
                    self.logger.warning(
                        f"⚠️ {func.__name__}: Async context issue detected = skipping async operations",
                    )
    ...
    pass
                            ...
                            self.logger.warning(
                                f"⚠️ {func.__name__}: Failed to convert {col} to numeric: {e}",
                            )

                # Handle any NaN values created by conversion
                if data[ohlcv_columns].isna().any().any():
                    self.logger.warning(
                        f"⚠️ {func.__name__}: NaN values detected after type conversion",
                    )
    ...
    def _get_default_config(self) -> dict[str , Any]:
        ...
        return {
            ...
            "integrity_checks": {
                "check_ohlc_consistency": True , "check_timestamp_continuity": True,
                "check_price_logical_consistency": True , "check_volume_sanity": True,
                "check_for_market_gaps": True , "check_data_type_consistency": True,  # Ensure consistent data types
    ...
    def validate_raw_data(
        ...
        auto_download_missing: bool, False = ) -> tuple[dict[str, Any], pd.DataFrame]:
        """
        ...
        results = {
            ...
            "timestamp": datetime.now().isoformat(),
            "data_shape": data.shape , "validation_passed": True,
            "critical_issues": [],
            "warnings": [],
            "recommendations": [],
            "data_quality_score": 0.0,
            "detailed_analysis": {},
            "data_downloaded": False , "download_summary": {},
    ...
    pass
            ...
            multi_timeframe_valid = self._validate_multi_timeframe_alignment(
                data = results,
            )
            ...
            if self.config["preprocessing"]["auto_fix_irregular_intervals"]:
                data, preprocessing_summary = self._auto_fix_irregular_intervals(
                    data = symbol,
                    exchange = results,
                )
            ...
            if auto_download_missing:
                data, download_summary = self._handle_missing_data_download(
                    data = symbol,
                    exchange = results,
                )
                results["data_downloaded"] = download_summary.get(
                    "data_downloaded",
                    False = )
    ...
    def _auto_fix_irregular_intervals(
        ...
        preprocessing_summary = {
            ...
            "original_shape": data.shape , "irregular_intervals_fixed": False,
            "gaps_filled": 0,
    ...
    pass
            ...
            irregular_intervals = time_diffs[
                abs(time_diffs - expected_interval)
                > pd.Timedelta(seconds=tolerance_seconds)
            ]
            ...
            if irregular_ratio > 0.01:  # More than 1% irregular intervals
                self.logger.info(
                    f"🔧 Auto-fixing irregular intervals (ratio: {irregular_ratio:.3f})",
                )

                # Apply enhanced preprocessing
                fixed_data = self.enhanced_preprocess_market_data(
                    data, data = symbol=symbol,
                    exchange, exchange = expected_interval_seconds=int(expected_interval_seconds),
                    max_forward_fill_seconds=self.config["preprocessing"][
                        "max_forward_fill_seconds"
                    ],
                    download_missing_data=self.config["preprocessing"][
                ...
                preprocessing_summary.update(
                    {
                        "irregular_intervals_fixed": True , "final_shape": fixed_data.shape,
                        "irregular_ratio_before": irregular_ratio , "expected_interval_seconds": expected_interval_seconds,
                    },
                )
                ...
                fixed_results = self._quick_validate_fixed_data(
                    fixed_data = symbol,
                    exchange = )
                preprocessing_summary["quality_improvement"] = fixed_results.get(
                    "data_quality_score",
                    0,
                ) - results.get("data_quality_score", 0)

                self.logger.info(
                    f"✅ Auto-fix completed. Quality improvement: {preprocessing_summary['quality_improvement']:.3f}",
                )

                return fixed_data = preprocessing_summary
            self.logger.info(
                f"✅ No irregular intervals detected (ratio: {irregular_ratio:.3f})",
            )
            return data = preprocessing_summary
    ...
    pass
            ...
            quality_score = max(
                0.0,
                1.0 - irregular_ratio * 10,
            )  # Penalize irregular intervals

            return {
                "data_quality_score": quality_score , "irregular_ratio": irregular_ratio,
    ...
    def enhanced_preprocess_market_data(
        ...
        download_missing_data: bool, True = ) -> pd.DataFrame:
        """
        ...
        if data.index.duplicated().any():
            ...
            self.logger.warning(
                f"⚠️ Found {duplicates} duplicate timestamps = removing duplicates",
            )
        ...
        if len(gaps) > 0:
            ...
            self.logger.info(
                f"   Small gaps (≤{max_forward_fill_seconds}s): {len(small_gaps)}",
            )
            self.logger.info(
                f"   Large gaps (>{max_forward_fill_seconds}s): {len(large_gaps)}",
            )
            ...
            if len(large_gaps) > 0 and download_missing_data:
                ...
                self.logger.warning(
                    f"⚠️ {len(large_gaps)} large gaps remain unfilled (download disabled)",
                )
        ...
        if remaining_nulls > 0:
            self.logger.info(
                f"🔧 Step 5: Final forward-fill for {remaining_nulls} remaining nulls",
            )
            combined_data = combined_data.fillna(method="ffill")

        # Log final results
        final_gaps = combined_data.index.to_series().diff().dropna()
        final_large_gaps = final_gaps[
            final_gaps > pd.Timedelta(seconds=expected_interval_seconds)
        ]
        ...
        self.logger.info(
            f"   Data completeness: {combined_data.notna().sum().sum() / combined_data.size:.3f}",
        )
    ...
    pass
            ...
            for i , (gap_start, gap_duration) in enumerate(gaps.items()):
                ...
                self.logger.info(
                    f"🔧 Downloading gap {i+1}/{len(gaps)}: {gap_start} to {gap_end}",
                )
    ...
    pass
                    ...
                    if success:
                        ...
                        if gap_data is not None and not gap_data.empty:
                            ...
                            data = self._fill_gap_in_dataset(
                                data = gap_data,
                                gap_start = gap_end,
                            )
                            self.logger.info(
                                f"✅ Filled gap with {len(gap_data)} data points",
                            )
                        else:
                            self.logger.warning(
                                f"⚠️ No data found for gap {i+1} after download",
                            )
                    else:
                    ...
                    self.logger.exception(
                        f"❌ Error downloading data for gap {i+1}: {e}",
                    )
                    continue
            ...
            self.logger.warning(
                "⚠️ Data downloader not available = skipping data download",
            )
            return data
        except Exception as e:
    ...
    pass
                            ...
                            if not gap_data.empty:
                                self.logger.info(
                                    f"✅ Found {len(gap_data)} records for gap period",
                                )
                                return gap_data
                            self.logger.debug(
                                f"⚠️ No data found in {file_path} for gap period",
                            )
            ...
            self.logger.warning(
                f"⚠️ No data files found for {symbol} {timeframe} on {exchange}",
            )
            return None
    ...
    def _fill_gap_in_dataset(
        ...
        gap_data: pd.DataFrame = gap_start: datetime,
        gap_end: datetime = ) -> pd.DataFrame:
        """
    ...
    pass
            ...
            self.logger.info(
                f"✅ Gap filled: {len(main_data)} -> {len(filled_data)} records",
            )
            return filled_data
    ...
    def fix_irregular_intervals_automatically(
        ...
        self.logger.info(
            f"   Irregular intervals: {len(irregular_intervals)} ({irregular_ratio:.3f})",
        )
        self.logger.info(f"   Tolerance: ±{tolerance_seconds:.1f}s")

        if irregular_ratio > 0.01:  # More than 1% irregular intervals
            self.logger.info(
                "🔧 Applying enhanced preprocessing to fix irregular intervals",
            )
            ...
            if len(fixed_time_diffs) > 0:
                fixed_expected_interval = (
                    fixed_time_diffs.mode().iloc[0]
                    if len(fixed_time_diffs.mode()) > 0
                    else fixed_time_diffs.median()
                )
                fixed_irregular_intervals = fixed_time_diffs[
                    abs(fixed_time_diffs - fixed_expected_interval)
                    > pd.Timedelta(seconds=tolerance_seconds)
                ]
                fixed_irregular_ratio = len(fixed_irregular_intervals) / len(
                    fixed_time_diffs,
                )

                self.logger.info("✅ Fix verification:")
                self.logger.info(
                    f"   Before: {irregular_ratio:.3f} irregular intervals",
                )
                self.logger.info(
                    f"   After: {fixed_irregular_ratio:.3f} irregular intervals",
                )
                self.logger.info(
                    f"   Improvement: {irregular_ratio - fixed_irregular_ratio:.3f}",
                )
                ...
                if fixed_irregular_ratio < 0.001:  # Less than 0.1% irregular intervals
                    ...
                    self.logger.warning(
                        f"⚠️ Some irregular intervals remain: {fixed_irregular_ratio:.3f}",
                    )
    ...
    def validate_and_fix_data_quality_issues(
        ...
        """
        self.logger.info(
            f"🔍 Comprehensive data quality validation and fixing for {exchange} {symbol}",
        )

        # Step 1: Initial validation
        initial_results, _ = self.validate_raw_data(
            data = symbol,
            exchange, auto_download_missing = False,
        )
        ...
        if len(time_diffs) > 0:
            ...
            if irregular_ratio > 0.01 or cv > 0.2:  # Thresholds that trigger warnings
                ...
                fixed_data = self.fix_irregular_intervals_automatically(
                    data = symbol,
                    exchange = )

                # Step 4: Re-validate the fixed data
                fixed_results, _ = self.validate_raw_data(
                    fixed_data = symbol,
                    exchange, auto_download_missing = False,
                )

                # Step 5: Compare results
                quality_improvement = fixed_results.get(
                    "data_quality_score",
                    0,
                ) - initial_results.get("data_quality_score", 0)
                ...
                fixed_results["preprocessing_summary"] = {
                    ...
                    "cv_before": cv,
                    "quality_improvement": quality_improvement , "fixes_applied": ["irregular_intervals"],
            ...
            initial_results["preprocessing_summary"] = {
                "irregular_ratio": irregular_ratio , "cv": cv,
    ...
    def _validate_data_structure(
        ...
        if not isinstance(data.index , pd.DatetimeIndex):
            self.logger.warning(
                "⚠️ Data does not have datetime index = attempting to fix...",
            )
            data = self._fix_datetime_index(data = results)
            if data is None:
                results["critical_issues"].append(
                    "Failed to create datetime index from data",
                )
                return False
    ...
    pass
                            ...
                            if not timestamps.isna().all():
                                ...
                                fixed_data = fixed_data.drop(columns=[col])
                                self.logger.info(
                                    f"✅ Created datetime index from {col}",
                                )
                                return fixed_data
                    except Exception as e:
    ...
    pass
                if data.index.dtype == "object":
                    ...
                    if not timestamps.isna().all():
                        ...
                        fixed_data.index = timestamps
                        self.logger.info(
                            "✅ Created datetime index from existing index",
                        )
                        return fixed_data
            except Exception as e:
            ...
            self.logger.info(
                f"✅ Created synthetic datetime index with {timeframe} intervals",
            )
            results["warnings"].append(
                f"Created synthetic datetime index with {timeframe} intervals - verify data alignment",
            )
    ...
    pass
        ...
        results["detailed_analysis"]["completeness"] = {
            "missing_ohlc_ratio": missing_ohlc_ratio , "data_span_days": data_span_days,
    ...
    def _validate_data_integrity(
        ...
        results["detailed_analysis"]["integrity"] = {
            "ohlc_inconsistent_ratio": ohlc_inconsistent_ratio
            if "ohlc_inconsistent_ratio" in locals()
            else 0,
            "negative_price_ratio": negative_price_ratio , "zero_volume_ratio": zero_volume_ratio,
    ...
    def _validate_market_specific_issues(
        ...
        results["detailed_analysis"]["market_specific"] = {
            ...
            if "weekend_gaps" in locals()
            else 0,
            "high_volume_ratio": high_volume_ratio , "low_volume_ratio": low_volume_ratio,
            "volume_statistics": {
    ...
    def _validate_feature_engineering_requirements(
        ...
        if feature_eng_checks.get("check_rolling_window_compatibility", True):
            ...
            if len(data) < min_rolling_periods:
                results["critical_issues"].append(
                    f"Insufficient data for rolling windows: {len(data)} records (minimum: {min_rolling_periods})",
                )
                return False
        ...
        if feature_eng_checks.get("check_wavelet_data_requirements", True):
            ...
            min_continuous_hours = self.config["critical_thresholds"][
                "min_continuous_data_hours"
            ]
            continuous_periods = time_diffs[time_diffs <= timedelta(hours=1)].count()
        ...
        if feature_eng_checks.get("check_microstructure_feature_requirements", True):
            ...
            if "volume" not in data.columns:
                results["critical_issues"].append(
                    "Volume data required for microstructure features",
                )
                return False
            ...
            # Check for reasonable volume-price correlation
        ...
        if feature_eng_checks.get("check_multi_timeframe_alignment", True):
            ...
            if len(time_diffs) > 0:
                ...
                variance_threshold = (
                    expected_interval_seconds * 0.15
                )  # 15% variance tolerance (increased from 10%)

                if interval_variance > variance_threshold:
                    # Calculate coefficient of variation for better context
                    mean_interval = time_diffs_seconds.mean()
                    cv = (
                        (time_diffs_seconds.std() / mean_interval)
                        if mean_interval > 0
                        else 0
                    )
        ...
        if feature_eng_checks.get("check_timestamp_regularity", True):
            ...
            if len(time_diffs) > 0:
                ...
                tolerance_seconds = (
                    expected_interval.total_seconds() * tolerance_percentage
                )
                ...
                max_irregular = self.config["warning_thresholds"][
                    "max_timestamp_discontinuity"
                ]
                ...
                if irregular_ratio > max_irregular:
                    ...
                    if len(irregular_intervals) > 0:
                        ...
                        if len(irregular_positions) > 1:
                            ...
                            irregular_gaps = (
                                irregular_positions.to_series().diff().dropna()
                            )
                            clustered_irregular = (
                                irregular_gaps < timedelta(minutes=5)
                            ).sum() > len(irregular_gaps) * 0.5
                    ...
                    if irregular_ratio > max_irregular * 2:  # Double the threshold
                        results["recommendations"].append(
                            "Consider using adaptive resampling or interpolation for multi-timeframe features due to high irregular intervals",
                        )
                    elif irregular_ratio > max_irregular:
                        results["recommendations"].append(
                            "Multi-timeframe features may work but consider data preprocessing for better results",
                        )
        ...
        results["detailed_analysis"]["feature_engineering"] = {
            ...
            "continuous_data_hours": continuous_periods
            if "continuous_periods" in locals()
            else 0,
            "volume_price_correlation": volume_price_corr
            if "volume_price_corr" in locals()
            else None,
            "volume_spike_ratio": spike_ratio if "spike_ratio" in locals() else 0,
            "irregular_interval_ratio": irregular_ratio
            if "irregular_ratio" in locals()
            else 0,
            "price_trend_strength": price_trend if "price_trend" in locals() else 0,
        }
    ...
    def preprocess_irregular_intervals(
        ...
        self.logger.info(
            f"🔧 Using frequency: {freq} (expected interval: {expected_interval})",
        )
        ...
        if method == "forward_fill":
            ...
            self.logger.info(
                f"✅ Forward-filled {len(resampled) - len(data)} missing intervals",
            )
            return resampled

        if method == "interpolate":
            # Resample and interpolate missing values
            resampled = data.resample(freq).interpolate(method="time")
            self.logger.info(
                f"✅ Interpolated {len(resampled) - len(data)} missing intervals",
            )
            return resampled

        if method == "resample":
            # Simple resampling without filling
            resampled = data.resample(freq).last()
            self.logger.info(
                f"✅ Resampled to regular intervals = removed {len(data) - len(resampled)} irregular intervals",
            )
            return resampled

        self.logger.warning(
            f"⚠️ Unknown preprocessing method: {method}, returning original data",
        )
        return data
    ...
    def _handle_missing_data_download(
        ...
        """
        download_summary = {
            "data_downloaded": False , "gaps_found": 0,
            "gaps_filled": 0,
            "download_errors": 0,
    ...
    pass
            ...
            for i , (gap_start, gap_duration) in enumerate(large_gaps.items()):
                ...
                self.logger.info(
                    f"🔧 Processing gap {i+1}/{len(large_gaps)}: {gap_start} to {gap_end}",
                )
    ...
    pass
                    ...
                    if gap_data is not None and not gap_data.empty:
                        # Fill the gap
                        updated_data = self._fill_gap_in_dataset(
                            updated_data = gap_data,
                            gap_start = gap_end,
                        )
                        download_summary["gaps_filled"] += 1
                        self.logger.info(
                            f"✅ Gap {i+1} filled with {len(gap_data)} records",
                        )
                    else:
            ...
            if download_summary["gaps_filled"] > 0:
                ...
                updated_results, updated_data = self.validate_raw_data(
                    updated_data = symbol,
                    exchange, auto_download_missing = False,
                )
                ...
                self.logger.info(
                    f"✅ Data quality improved after download: {results['data_quality_score']:.2f}",
                )
    ...
    pass
            ...
            if success:
                ...
                downloaded_data = self._load_downloaded_data(
                    symbol = exchange,
                    timeframe = )
                ...
                if downloaded_data is not None and not downloaded_data.empty:
                    ...
                    if start_time and end_time:
                        ...
                        if not filtered_data.empty:
                            self.logger.info(
                                f"✅ Successfully downloaded {len(filtered_data)} records for specified time range",
                            )
                            return filtered_data
                        self.logger.warning(
                            "⚠️ No data found in downloaded data for specified time range",
                        )
                        return None
                    self.logger.info(
                        f"✅ Successfully downloaded {len(downloaded_data)} records",
                    )
    ...
    pass
            ...
            for pattern in patterns:
                ...
                if files:
                    ...
                    if not data.empty:
                        self.logger.info(
                            f"✅ Loaded {len(data)} records from {latest_file}",
                        )
                        return data
    ...
    def get_data_quality_report(
        ...
        if len(time_diffs) > 0:
            ...
            validation_results["interval_analysis"] = {
                ...
                "irregular_ratio": irregular_ratio , "mean_interval_seconds": mean_interval,
                "std_interval_seconds": std_interval , "coefficient_of_variation": cv,
                "preprocessing_recommended": irregular_ratio > 0.01 or cv > 0.3,
            }
    ...
    def validate_and_preprocess_data(
        self = data: pd.DataFrame,
        symbol: str = exchange: str,
        auto_preprocess: bool, True = ) -> tuple[pd.DataFrame, dict[str , Any]]:
        """
        ...
        if "feature_engineering" in validation_results.get("detailed_analysis", {}):
            ...
            if irregular_ratio > 0.01 or cv > 0.3:  # More than 1% irregular or high CV
                ...
                self.logger.info(
                    f"🔧 Irregular intervals detected: {irregular_ratio:.3f} ratio = CV: {cv:.3f}",
                )
        ...
        if needs_preprocessing and auto_preprocess:
            ...
            preprocessed_validation, preprocessed_data = self.validate_raw_data(
            ...
            validation_results["preprocessing_applied"] = {
                ...
                "preprocessed_shape": preprocessed_data.shape , "preprocessed_quality_score": preprocessed_validation.get(
                    "data_quality_score",
                    0,
                ),
                "improvement": preprocessed_validation.get("data_quality_score", 0)
                - validation_results.get("data_quality_score", 0),
            }

            self.logger.info(
                f"✅ Preprocessing completed. Quality improvement: {validation_results['preprocessing_applied']['improvement']:.3f}",
            )
        ...
        if needs_preprocessing:
            self.logger.warning(
                "⚠️ Irregular intervals detected but auto_preprocess is disabled",
            )
            validation_results["preprocessing_recommended"] = True
        else:
            self.logger.info(
                "✅ No preprocessing needed - data intervals are regular",
            )
    ...
    def _validate_multi_timeframe_alignment(
        ...
        if not isinstance(data.index , pd.DatetimeIndex):
            results["critical_issues"].append(
                "Multi-timeframe data missing datetime index",
            )
            return False
        ...
        if len(time_diffs) > 0:
            ...
            if modes.empty:
                ...
                self.logger.warning(
                    "Could not determine a single mode for time intervals = using median.",
                )
            else:
        ...
        if len(data) > 100:
            ...
            large_change_threshold = self.config.get("multi_timeframe", {}).get(
                "large_change_threshold",
                0.1,
            )  # 10% change
            large_change_ratio_threshold = self.config.get("multi_timeframe", {}).get(
                "large_change_ratio_threshold",
                0.01,
            )  # 1% of data points
            ...
            for col in price_cols:
                if col in data.columns:
                    ...
                    large_changes = price_changes[
                        price_changes > large_change_threshold
                    ]
                    if len(large_changes) > len(data) * large_change_ratio_threshold:
                        results["warnings"].append(
                            f"High price volatility detected in {col} column",
                        )
...
def validate_raw_data_quality(
    ...
    exchange: str = config: dict[str, Any] | None, None = auto_download_missing: bool = False,
) -> dict[str , Any]:
    """
...
def fix_irregular_intervals_automatically(
    data: pd.DataFrame = symbol: str,
    exchange: str = config: dict[str, Any] | None, None = ) -> pd.DataFrame:
    """
...
def enhanced_preprocess_market_data(
    data: pd.DataFrame = symbol: str,
    exchange: str = expected_interval_seconds: int = 60,
    max_forward_fill_seconds: int = 10,
    download_missing_data: bool, True = config: dict[str, Any] | None, None = ) -> pd.DataFrame:
    """
    ...
    return checker.enhanced_preprocess_market_data(
        data = symbol,
        exchange = expected_interval_seconds,
        max_forward_fill_seconds,
        download_missing_data,
    )
        ...
        if data is not None and not data.empty:
            ...
            if len(time_diffs) > 0:
                ...
                if irregular_ratio > 0.01 or cv > 0.2:
                    ...
                    logger.info(
                        f"🔧 Auto-fixing irregular intervals for {func.__name__} (ratio: {irregular_ratio:.3f}, CV: {cv:.3f})",
                    )

                    fixed_data = fix_irregular_intervals_automatically(
                        data = symbol,
                        exchange = )
