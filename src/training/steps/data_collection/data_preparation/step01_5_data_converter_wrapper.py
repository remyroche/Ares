...
class DataConverterStep(BaseStep):
    ...
    @handles_errors(exceptions=(Exception,), default_return={'success': False}, context='data converter execution')
    ...
    def _extract_conversion_parameters(self, training_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {'symbol': training_input.get('symbol', 'ETHUSDT'), 'exchange': training_input.get('exchange', 'BINANCE'), 'timeframe': training_input.get('timeframe', '1m'), 'data_dir': training_input.get('data_dir'), 'force_rerun': training_input.get('force_rerun', False)}
    ...
    def _update_pipeline_state(self, pipeline_state: Dict[str, Any], params: Dict[str, Any], conversion_success: bool) -> None:
        ...
        pipeline_state.update({'unified_data_path': unified_path, 'unified_config_path': unified_config_path, 'unified_data_ok': conversion_success})
