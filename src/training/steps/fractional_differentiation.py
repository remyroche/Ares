...
class FractionalDifferentiation:
    ...
    def fractional_diff(self, series: pd.Series, preserve_original: bool = True) -> pd.Series:
        ...
        if len(series) < self.window:
            ...
            self.logger.warning(
                f"Series too short for fractional diff, using simple diff: {
                    len(series)} < {
                    self.window}"
            )
    ...
    def batch_fractional_diff(
        self, data: pd.DataFrame, columns: Optional[list[str]] = None, exclude_columns: Optional[list[str]] = None
    ) -> pd.DataFrame:
        """
...
class FractionalFeatureGenerator:
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=pd.DataFrame(), context="fractional_feature_generator.generate_features"
    )
