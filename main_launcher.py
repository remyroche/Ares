...
async def run_trading_bot(exchange_name: str, symbol: str, is_paper_trade: bool):
    ...
    system_logger.info(f"Initializing trading bot for {symbol} on {exchange_name} (Paper: {is_paper_trade}).")
    ...
    if not exchange_config:
        system_logger.critical(f"Exchange '{exchange_name}' not found in config. Exiting.")
        return
...
if __name__ == "__main__":
    try:
        ...
        system_logger.critical(f"A critical error occurred in the main launcher: {e}", exc_info=True)
        sys.exit(1)
