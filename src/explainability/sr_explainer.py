...
class SRExplainer(BaseExplainer):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.feature_groups = {
            'price_features': ['close', 'open', 'high', 'low', 'price_momentum', 'price_volatility'],
            'volume_features': ['volume', 'volume_ratio', 'volume_momentum', 'volume_at_level'],
            'level_features': ['level_price', 'level_age', 'touch_count', 'bounce_rate', 'isolation_score'],
            'technical_indicators': ['rsi', 'macd', 'bb_position', 'atr', 'adx', 'stochastic'],
            'market_structure': ['support_levels_count', 'resistance_levels_count', 'level_density'],
            'breakout_features': ['breakout_probability', 'breakout_confidence', 'time_to_breakout'],
            'quality_features': ['level_strength', 'level_confidence', 'level_reliability']
        }
        
        # SR level types
        self.level_types = ['support', 'resistance', 'dynamic_support', 'dynamic_resistance']
        
    async def explain_level_detection(
        self,
        sr_model: Any,
        market_data: pd.DataFrame,
        features: np.ndarray,
        feature_names: List[str],
        detected_levels: Optional[List[Dict[str, Any]]] = None
    ) -> ExplanationResult:
        ...
        try:
            ...
            # Get detected levels if not provided
            ...
            if self.lime_explainer is not None:
                lime_explanation = self._calculate_lime_explanation(
                    self.lime_explainer, sr_model, features, feature_names
                )
            ...
            explanation = ExplanationResult(
                model_name="SR_LevelDetection",
                prediction=detected_levels,
    ...
    async def explain_breakout_prediction(
        self,
        sr_model: Any,
        market_data: pd.DataFrame,
        features: np.ndarray,
        feature_names: List[str],
        breakout_prediction: Optional[Dict[str, Any]] = None
    ) -> ExplanationResult:
        ...
        try:
            ...
            # Get breakout prediction if not provided
            ...
            # Create explanation result
            explanation = ExplanationResult(
                model_name="SR_BreakoutPrediction",
                prediction=breakout_prediction,
                feature_names=feature_names,
                feature_values=features,
                shap_values=shap_values,
                lime_explanation=lime_explanation,
                ...
                confidence=confidence,
                metadata={
                    'model_type': 'breakout_prediction',
                    'feature_groups': self.feature_groups,
                    'level_types': self.level_types,
    ...
    async def explain_level_quality(
        self,
        sr_model: Any,
        market_data: pd.DataFrame,
        features: np.ndarray,
        ...
        try:
            ...
            # Get quality score if not provided
    ...
    async def explain_strength_calculation(
        self,
        sr_model: Any,
        market_data: pd.DataFrame,
        features: np.ndarray,
        ...
        try:
            ...
            # Get strength score if not provided
            ...
            # Create explanation result
            explanation = ExplanationResult(
                model_name="SR_LevelStrength",
                prediction=strength_score,
                feature_names=feature_names,
                feature_values=features,
                shap_values=shap_values,
                lime_explanation=lime_explanation,
    ...
    async def explain_prediction(
        ...
        try:
            ...
            if model_type == 'level_detection':
                return await self.explain_level_detection(
                    model, pd.DataFrame(), features, feature_names, prediction
                )
            elif model_type == 'breakout_prediction':
                return await self.explain_breakout_prediction(
                    model, pd.DataFrame(), features, feature_names, prediction
                )
            elif model_type == 'level_quality':
                return await self.explain_level_quality(
                    model, pd.DataFrame(), features, feature_names, prediction
                )
            elif model_type == 'level_strength':
                return await self.explain_strength_calculation(
                    model, pd.DataFrame(), features, feature_names, prediction
                )
            else:
                # Generic explanation
                return await self._explain_generic_sr_model(
    ...
    async def _explain_generic_sr_model(
        self,
        model: Any,
        features: np.ndarray,
        feature_names: List[str],
    ...
    def _calculate_detection_confidence(
        ...
        try:
            ...
            # Adjust based on number of detected levels
            if detected_levels:
                # More levels might indicate better detection
                level_count = len(detected_levels)
                if level_count > 0:
                    confidence += min(0.3, level_count * 0.05)
                
                # Check for level quality indicators
    ...
    def _calculate_breakout_confidence(
        self,
        breakout_prediction: Dict[str, Any],
        shap_values: Optional[np.ndarray]
    ) -> float:
    ...
    def _calculate_quality_confidence(
        ...
        try:
            ...
            # Adjust based on quality score magnitude
            if isinstance(quality_score, (int, float)):
                # Higher quality scores indicate more confidence
    ...
    def _calculate_strength_confidence(
        ...
        try:
            ...
            # Adjust based on strength score magnitude
            if isinstance(strength_score, (int, float)):
                # Higher strength scores indicate more confidence
    ...
    def get_level_type_importance(self, explanation: ExplanationResult) -> Dict[str, float]:
        ...
        try:
            ...
            # This would need to be implemented based on how level types are encoded
            # in the features. For now, return empty dict.
