...
def _as_float32_series(values: Any, index: pd.Index, default: float = np.nan) -> pd.Series:
...
def materialize_live_source_regime_features(
    ...
    *,
    ...
    original_symbols = pd.Index(out.index.astype(str), name=None)
    work = out.copy().reset_index(drop=True)
    work["__symbol__"] = original_symbols.to_numpy(dtype=object, copy=False)
    ...
    try:
        ...
        for col in list(COMPONENT_COLS):
            if col in components.columns:
                source_values[f"{SOURCE_REGIME_PREFIX}{col}{SOURCE_REGIME_SUFFIX}"] = components[col]
        for col in list(ARCHETYPE_COLS):
            if col in archetypes.columns:
                source_values[f"{SOURCE_REGIME_PREFIX}{col}{SOURCE_REGIME_SUFFIX}"] = archetypes[col]
        for col in family_scores.columns:
            source_values[f"{SOURCE_REGIME_PREFIX}{col}{SOURCE_REGIME_SUFFIX}"] = family_scores[col]
...
def load_live_ae_gmm_state_payload(data_root: str, run_id: str) -> dict[str, Any]:
    ...
    for path in candidates:
        ...
        return {
            ...
            "manifest": manifest,
            "input_feature_columns": [str(c) for c in state.get("feature_columns", []) or []],
            "generated_feature_columns": list(AE_GMM_FEATURE_COLUMNS),
        }
    return {}


def live_ae_gmm_input_feature_columns(payload: Mapping[str, Any] | None) -> list[str]:
...
def materialize_live_ae_gmm_features(
    ...
    *,
    ...
    required = {
        str(col)
        for col in (required_columns or [])
        if str(col) in AE_GMM_FEATURE_SET
    }
    if not required:
        return features
    ...
    x_base = out.reindex(columns=input_columns)
    try:
        generated = transform_ae_gmm_features(
            x_base,
            dict(state),
            index=out.index,
        ).reindex(columns=sorted(required), fill_value=0.0)
    except Exception as exc:
        tprint(
    ...
    for col in generated.columns:
        out[col] = pd.to_numeric(generated[col], errors="coerce").astype(
            np.float32,
            copy=False,
        )
    finite_counts = {
...
def _quantile_label(edges: list[float], value: float, prefix: str) -> str:
    ...
    clean = np.asarray([float(v) for v in (edges or []) if np.isfinite(float(v))], dtype=np.float64)
    if clean.size == 0:
        return f"{prefix}__missing"
...
def load_meta_reliability_prior_payload(data_root: str, run_id: str) -> dict[str, Any]:
    root = Path(str(data_root or "data_perp")) / "artifacts" / str(run_id) / "policy_params"
    candidates = [
        root / "meta_reliability_priors.json",
        Path(str(data_root or "data_perp")) / "artifacts" / str(run_id) / "meta_reliability_priors.json",
    ]
    for path in candidates:
        if path.exists():
...
def apply_live_meta_reliability_priors(
    ...
    *,
    ...
    def _group_values(prefix: str, arch: str, band: str) -> dict[str, float]:
        ...
        values = groups.get(exact_key) or groups.get(fallback_arch_key) or groups.get(fallback_band_key)
    ...
    for symbol in out.index:
        ...
        score = _safe_float(pred.get("base_pred", out.at[symbol, "score"] if "score" in out.columns else np.nan), np.nan)
        arch = ""
        ...
        margin = score - cutoff if np.isfinite(score) and np.isfinite(cutoff) else np.nan
        ...
        out.at[symbol, "base_margin_to_cutoff"] = np.float32(margin if np.isfinite(margin) else 0.0)

        for prefix, band in (("rel_rankband", rank_band), ("rel_marginband", margin_band)):
            vals = _group_values(prefix, arch, band)
            rows = _safe_float(vals.get("rows"), 0.0)
            clean = _safe_float(vals.get("clean_rate"), _safe_float(global_prior.get("clean_rate"), 0.0))
            bad = _safe_float(vals.get("bad_mae_rate"), _safe_float(global_prior.get("bad_mae_rate"), 0.0))
            timeout = _safe_float(vals.get("timeout_rate"), _safe_float(global_prior.get("timeout_rate"), 0.0))
            dirty = _safe_float(vals.get("dirty_positive_rate"), _safe_float(global_prior.get("dirty_positive_rate"), 0.0))
            exec_mean = _safe_float(vals.get("exec_margin_mean"), _safe_float(global_prior.get("exec_margin_mean"), 0.0))
            edge = clean - bad - 0.50 * timeout + exec_mean
            out.at[symbol, f"{prefix}_rows_log1p"] = np.float32(np.log1p(max(rows, 0.0)))
...
def reliability_prior_payload_from_training_frame(
    ...
    *,
    ...
    if "source_tag" in work.columns:
        ...
        arch = side + "__unknown"
    clean = pd.to_numeric(work.get("clean_exec", work.get("clean_exec_label", 0.0)), errors="coerce").fillna(0.0).clip(0.0, 1.0)
    bad = pd.to_numeric(work.get("full_path_bad_mae_1r", 0.0), errors="coerce").fillna(0.0).clip(0.0, 1.0)
    timeout = pd.to_numeric(work.get("timeout", 0.0), errors="coerce").fillna(0.0).clip(0.0, 1.0)
    dirty = pd.to_numeric(work.get("dirty_positive", 0.0), errors="coerce").fillna(0.0).clip(0.0, 1.0)
    exec_margin = pd.to_numeric(work.get("exec_margin", 0.0), errors="coerce").fillna(0.0)

    global_cutoff = float(score.loc[selected & score.notna()].min()) if bool((selected & score.notna()).any()) else float(score.quantile(0.70))
    ...
    if not np.isfinite(global_std) or global_std <= 1e-12:
        global_std = 1.0
    tmp = pd.DataFrame({"side": side, "arch": arch, "score": score, "selected": selected})
    ...
    for (side_value, arch_value), group in tmp.groupby(["side", "arch"], dropna=False):
        ...
        cutoff = float(selected_scores.min()) if len(selected_scores) else (float(scores.quantile(0.70)) if len(scores) else global_cutoff)
    ...
    score_edges = [
        float(v)
        for v in np.unique(score.dropna().quantile(np.linspace(0.0, 1.0, 6)[1:-1]).to_numpy(dtype=np.float64))
    ]
    ...
    margin_edges = [
        float(v)
        for v in np.unique(margin.dropna().quantile(np.linspace(0.0, 1.0, 6)[1:-1]).to_numpy(dtype=np.float64))
    ]
    rank_band = pd.Series(
    ...
    def _aggregate(prefix: str, band_col: str, side_key: str | None = None, arch_key: str | None = None) -> dict[str, dict[str, float]]:
        groups: dict[str, dict[str, float]] = {}
        group_cols = ["side", "arch", band_col]
        for (side_value, arch_value, band_value), group in stat_work.groupby(group_cols, dropna=False):
            rows = float(len(group))
            weight = rows / (rows + float(shrinkage_k))
            clean_rate = float(group["clean"].mean()) if rows else global_stats["clean_rate"]
            bad_rate = float(group["bad"].mean()) if rows else global_stats["bad_mae_rate"]
            timeout_rate = float(group["timeout"].mean()) if rows else global_stats["timeout_rate"]
            dirty_rate = float(group["dirty"].mean()) if rows else global_stats["dirty_positive_rate"]
            exec_mean = float(group["exec"].mean()) if rows else global_stats["exec_margin_mean"]
            vals = {
                "rows": rows,
                "clean_rate": weight * clean_rate + (1.0 - weight) * global_stats["clean_rate"],
                "bad_mae_rate": weight * bad_rate + (1.0 - weight) * global_stats["bad_mae_rate"],
                "timeout_rate": weight * timeout_rate + (1.0 - weight) * global_stats["timeout_rate"],
                "dirty_positive_rate": weight * dirty_rate + (1.0 - weight) * global_stats["dirty_positive_rate"],
                "exec_margin_mean": weight * exec_mean + (1.0 - weight) * global_stats["exec_margin_mean"],
            }
        ...
        for (side_value, band_value), group in stat_work.groupby(["side", band_col], dropna=False):
            rows = float(len(group))
            weight = rows / (rows + float(shrinkage_k))
            groups[f"{prefix}|{side_value}|*|{band_value}"] = {
                "rows": rows,
                "clean_rate": weight * float(group["clean"].mean()) + (1.0 - weight) * global_stats["clean_rate"],
                "bad_mae_rate": weight * float(group["bad"].mean()) + (1.0 - weight) * global_stats["bad_mae_rate"],
                "timeout_rate": weight * float(group["timeout"].mean()) + (1.0 - weight) * global_stats["timeout_rate"],
                "dirty_positive_rate": weight * float(group["dirty"].mean()) + (1.0 - weight) * global_stats["dirty_positive_rate"],
                "exec_margin_mean": weight * float(group["exec"].mean()) + (1.0 - weight) * global_stats["exec_margin_mean"],
            }
