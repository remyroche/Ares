...
def test_exclude_recent_days_configuration():
    ...
    ), f"Expected DEFAULT_EXCLUDE_RECENT_DAYS to be 2, got {DEFAULT_EXCLUDE_RECENT_DAYS}"
    print(
        f"✅ DEFAULT_EXCLUDE_RECENT_DAYS constant is correctly set to {DEFAULT_EXCLUDE_RECENT_DAYS}"
    )
    ...
    ), f"Expected exclude_recent_days in DATA_CONFIG to be 2, got {exclude_recent_days}"
    print(
        f"✅ exclude_recent_days in DATA_CONFIG is correctly set to {exclude_recent_days}"
    )
...
def test_lookback_calculation():
    ...
    end_cutoff = (
        now - timedelta(days=exclude_recent_days) if exclude_recent_days > 0 else now
    )
    ...
    print(
        f"📅 End cutoff (excludes last {exclude_recent_days} days): {end_cutoff.strftime('%Y-%m-%d %H:%M:%S')}"
    )
    print(
        f"📅 Start cutoff (lookback {lookback_days} days): {start_cutoff.strftime('%Y-%m-%d %H:%M:%S')}"
    )
    print(
        f"📅 Total data window: {start_cutoff.strftime('%Y-%m-%d')} to {end_cutoff.strftime('%Y-%m-%d')}"
    )

    # Verify that the end cutoff is indeed 2 days before now
    expected_end_cutoff = now - timedelta(days=2)
    assert (
        abs((end_cutoff - expected_end_cutoff).total_seconds()) < 1
    ), f"End cutoff should be 2 days before now"

    # Verify that the total window is correct
    total_window_days = (end_cutoff - start_cutoff).days
    assert (
        total_window_days == lookback_days
    ), f"Total window should be {lookback_days} days, got {total_window_days}"
...
def test_blank_vs_full_mode():
    ...
    full_end_cutoff = now - timedelta(
        days=exclude_recent_days
    )  # Same exclude_recent_days
    ...
    print(
        f"   Data window: {blank_start_cutoff.strftime('%Y-%m-%d')} to {blank_end_cutoff.strftime('%Y-%m-%d')}"
    )
    ...
    print(f"   Exclude recent days: {exclude_recent_days}")
    print(
        f"   Data window: {full_start_cutoff.strftime('%Y-%m-%d')} to {full_end_cutoff.strftime('%Y-%m-%d')}"
    )

    # Verify that both modes use the same exclude_recent_days
    assert (
        blank_end_cutoff == full_end_cutoff
    ), "Both modes should have the same end cutoff"
    ...
    assert (
        abs(start_date_diff - lookback_diff) <= 1
    ), f"Start date difference should equal lookback difference (got {start_date_diff}, expected {lookback_diff})"
...
def main():
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print(
            "🎉 All tests passed! The exclude_recent_days functionality is working correctly."
        )
        print(
            "✅ The lookback period will always exclude the last 2 days, regardless of mode."
        )
