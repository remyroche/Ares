...
class TacticianSpecialistTrainingStep(BaseStep):
    ...
    def _initialize_specialist_config(self) -> Dict[str, Any]:
        ...
        return {'model_types': ['lightgbm', 'xgboost', 'neural_network', 'random_forest'], 'enable_sr_integration': True, 'regime_specific_training': True, 'min_regime_samples': 500, 'validation_split': 0.2, 'test_split': 0.1, 'tactics': {'breakout': {'enabled': True, 'min_confidence': 0.8}, 'reversal': {'enabled': True, 'min_confidence': 0.85}, 'continuation': {'enabled': True, 'min_confidence': 0.75}, 'range_bound': {'enabled': True, 'min_confidence': 0.7}}, 'min_precision': 0.85, 'min_recall': 0.1, 'min_f1_score': 0.2, 'feature_engineering': {'use_technical_indicators': True, 'use_market_microstructure': True, 'use_regime_features': True, 'use_sr_features': True}, 'hyperparameter_optimization': True, 'feature_selection': True, 'ensemble_specialists': True, 'timeframes': ['1m', '5m', '15m'], 'primary_timeframe': '1m'}
    ...
    @handles_errors(exceptions=(Exception,), default_return={'success': False}, context='tactician specialist training logic')
    async def execute_logic(self, training_input: Dict[str, Any], pipeline_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for regime_id, regime_info in regime_data.items():
            ...
            regime_training_data = self._get_regime_training_data(training_data, regime_info)
            if len(regime_training_data) < self.specialist_config['min_regime_samples']:
                self.logger.warning(f"Regime {regime_id} has insufficient samples ({len(regime_training_data)} < {self.specialist_config['min_regime_samples']})")
                continue
            regime_tactics_config = await self.regime_tactics.determine_tactics(regime_id, regime_info, regime_training_data)
            regime_specialists = await self._train_regime_specialists(regime_id, regime_training_data, regime_tactics_config)
        ...
        if len(all_specialist_models) > 1:
            ...
            cross_regime_specialists = await self._create_cross_regime_specialists(all_specialist_models, training_data)
        ...
        evaluation_results = await self.performance_evaluator.evaluate_all_specialists(all_specialist_models, training_data)
        ...
        result['specialist_training_summary'] = self._create_training_summary(all_specialist_models, all_training_results, evaluation_results)
        return result

    async def _train_regime_specialists(self, regime_id: str, training_data: pd.DataFrame, tactics_config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for tactic_name, tactic_config in tactics_config.items():
                ...
                tactic_models = await self.specialist_trainer.train_tactic_models(tactic_name, X_train, y_train, X_val, y_val, self.specialist_config['model_types'], regime_id)
                tactic_validation = await self._evaluate_tactic_models(tactic_models, X_test, y_test, tactic_config)
                best_models = await self.model_selector.select_best_models(tactic_models, tactic_validation, tactic_config)
                if best_models:
                    specialist_models[tactic_name] = best_models
                    training_results[tactic_name] = {'models_trained': len(tactic_models), 'models_selected': len(best_models), 'training_samples': len(X_train)}
                    validation_results[tactic_name] = tactic_validation
            self.logger.info(f'  ✅ Trained {len(specialist_models)} specialist tactics for regime {regime_id}')
            return {'models': specialist_models, 'training_results': training_results, 'validation_results': validation_results}
        except Exception as e:
            self.logger.error(f'Failed to train specialists for regime {regime_id}: {str(e)}')
            return None

    async def _create_cross_regime_specialists(self, regime_specialists: Dict[str, Dict[str, Any]], full_training_data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for tactic in all_tactics:
                ...
                for regime_id, regime_models in regime_specialists.items():
                    if tactic in regime_models:
                        tactic_models.extend([(f'{regime_id}_{model_name}', model) for model_name, model in regime_models[tactic].items()])
                if tactic_models:
                    ensemble = await self.specialist_trainer.create_tactic_ensemble(tactic, tactic_models)
            ...
            for tactic, models in cross_regime_models.items():
                validation = await self._evaluate_tactic_models(models, X_test, y_test, {'min_confidence': 0.7})
                validation_results[tactic] = validation
            return {'models': cross_regime_models, 'training_results': {'type': 'cross_regime_ensemble', 'n_regimes': len(regime_specialists), 'tactics': list(cross_regime_models.keys())}, 'validation_results': validation_results}
        except Exception as e:
            self.logger.error(f'Failed to create cross-regime specialists: {str(e)}')
            return None

    def _prepare_training_data(self, labeled_data: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.specialist_config['feature_engineering']['use_regime_features']:
            if 'regime_id' in training_data.columns:
                regime_dummies = pd.get_dummies(training_data['regime_id'], prefix='regime')
                training_data = pd.concat([training_data, regime_dummies], axis=1)
        return training_data

    def _get_regime_training_data(self, training_data: pd.DataFrame, regime_info: Dict[str, Any]) -> pd.DataFrame:
    ...
    def _split_training_data(self, data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    def _extract_features_labels(self, data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    async def _evaluate_tactic_models(self, models: Dict[str, Any], X_test: pd.DataFrame, y_test: pd.Series, tactic_config: Dict[str, Any]) -> Dict[str, Dict[str, float]]:
        ...
        for model_name, model in models.items():
            try:
                ...
                metrics = {'accuracy': accuracy_score(y_test, y_pred), 'precision': precision_score(y_test, y_pred, average='weighted', zero_division=0), 'recall': recall_score(y_test, y_pred, average='weighted', zero_division=0), 'n_predictions': len(y_pred), 'positive_rate': (y_pred > 0).mean() if hasattr(y_pred, 'mean') else 0}
                ...
                evaluation_results[model_name] = {'error': str(e), 'meets_requirements': False}
        return evaluation_results

    def _create_training_summary(self, all_models: Dict[str, Dict[str, Any]], training_results: Dict[str, Dict[str, Any]], evaluation_results: Dict[str, Any]) -> Dict[str, Any]:
        """Create a summary of the specialist training process."""
        summary = {'total_regimes': len(all_models), 'total_specialists': sum((len(tactics) for tactics in all_models.values())), 'regime_summaries': {}, 'best_performers': {}, 'average_metrics': {'precision': [], 'recall': [], 'accuracy': []}}
        for regime_id, regime_models in all_models.items():
            regime_summary = {'tactics': list(regime_models.keys()), 'total_models': sum((len(models) if isinstance(models, dict) else 1 for models in regime_models.values())), 'training_info': training_results.get(regime_id, {})}
                    ...
                    for tactic, metrics in regime_eval.items():
                        if isinstance(metrics, dict) and 'precision' in metrics:
                            if metrics['precision'] > best_score:
                                ...
                                best_tactic = tactic
                    if best_tactic:
                        summary['best_performers'][regime_id] = {'tactic': best_tactic, 'precision': best_score}
        return summary
    ...
    def validate_outputs(self, pipeline_state: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        """
        errors = []
        required_outputs = ['tactician_specialist_models', 'specialist_training_results', 'specialist_validation_results', 'specialist_evaluation_results', 'specialist_training_summary']
    ...
    def get_required_inputs(self) -> List[str]:
        ...
        return ['tactician_labeled_data', 'regime_data', 'features', 'step14_tactician_labeling_completed']

    def get_produced_outputs(self) -> List[str]:
        """Get list of outputs produced by this step."""
        return ['tactician_specialist_models', 'specialist_training_results', 'specialist_validation_results', 'specialist_evaluation_results', 'specialist_training_summary']
