...
class ErrorMapper:
    ...
    def register_mapping(
        self, exception_type: Type[Exception], mapper: Callable[[Exception], AppError]
    ) -> None:
        ...
        for exc_type, mapper in self._exception_map.items():
            if isinstance(exc, exc_type):
                try:
                    ...
                    logger.error(
                        f"Error mapping exception {exc}: {mapping_error}", exc_info=True
                    )
                    break
    ...
    def to_cli_output(self, error: AppError) -> Dict[str, Any]:
        ...
        if error.is_client_error:
            color = color_map["client"]
        else:
            color = color_map["server"]
...
def register_exception_mapping(
    ...
    mapper: Union[Callable[[Exception], AppError], Type[AppError]],
) -> None:
    """Register a custom exception mapping.
