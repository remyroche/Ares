...
class EnsembleClusteringRegimeDetector:
    ...
    def ensemble_regime_detection(self, features: np.ndarray, method_weights: Optional[Dict[str, float]] = None) -> Tuple[np.ndarray, Dict[str, Any]]:
        """
        Perform ensemble regime detection using HMM + K-means + DBSCAN.
        ...
        # HMM clustering
        hmm_results = self._hmm_clustering_vectorized(features_scaled)
        ensemble_results['hmm'] = hmm_results
        
        # K-means clustering
        kmeans_results = self._kmeans_clustering_vectorized(features_scaled)
        ensemble_results['kmeans'] = kmeans_results
        
        # DBSCAN clustering
        dbscan_results = self._dbscan_clustering_vectorized(features_scaled)
        ensemble_results['dbscan'] = dbscan_results
        
        # 3. Calculate method weights based on quality scores
        quality_weights = self._calculate_quality_weights(ensemble_results)
        
        # 4. Weighted consensus voting
        consensus_regimes = self._weighted_consensus_voting_vectorized(
            ensemble_results, quality_weights, method_weights
        )
        
        # 5. Calculate ensemble confidence scores
        confidence_scores = self._calculate_ensemble_confidence_vectorized(
        ...
        final_results = {
            ...
            'ensemble_quality': self._calculate_ensemble_quality(features_scaled, consensus_regimes)
        }
