...
def run_live_cycle():
    ...
    meta_models = trained_bundle["meta_models"]
    ...
    if candidates and ts_sig in mkt_gates.index:
        ...
        if total_wt > 0:
            ...
            for sym, side, score in final_orders:
                # Wallet Sharing (Req 9)
                w_alloc = gross_cap * (abs(score) / total_wt)
                # We interpret "w_alloc" as percentage of equity to deploy?
                # Usually engine manages equity. Here we just log position.
                # In simulation, we assume full fill.
                ...
                state.set_position(sym, pos)
                tprint(f"ENTRY {side} {sym} @ {entry_px} (score={score:.4f}, w={w_alloc:.4f})")
