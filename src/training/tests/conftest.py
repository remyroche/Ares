...
def sample_market_data() -> pd.DataFrame:
    ...
    data = pd.DataFrame({
        ...
        "high": price * (1 + np.random.uniform(0, 0.005, n_samples)),
        "low": price * (1 - np.random.uniform(0, 0.005, n_samples)),
        "close": price,
...
@pytest.fixture
def sample_regime_labels(sample_features) -> np.ndarray:
...
@pytest.fixture
def sample_pipeline_state(sample_features, sample_regime_labels) -> Dict[str, Any]:
    ...
    return {
        ...
        "regime_characteristics": {
            "regime_0": {
                "count": 350,
                "percentage": 35.0,
                "volatility_20_mean": 0.01,
                "returns_mean": 0.0001
            },
            "regime_1": {
                "count": 350,
                "percentage": 35.0,
                "volatility_20_mean": 0.015,
                "returns_mean": -0.0001
            },
            "regime_2": {
                "count": 300,
                "percentage": 30.0,
                "volatility_20_mean": 0.02,
                "returns_mean": 0.0002
            }
        }
    }
...
@pytest.fixture
def sample_training_input(test_data_dir) -> Dict[str, Any]:
...
class MockStep:
    ...
    def __init__(self, step_name: str, outputs: Dict[str, Any]):
...
def mock_steps() -> Dict[str, MockStep]:
    ...
    return {
        "01_data_collection": MockStep(
            "data_collection",
            {"raw_market_data": "path/to/data.parquet"}
        ),
        "02_data_reading": MockStep(
            "data_reading",
            {"validated_data": pd.DataFrame()}
        ),
        "03_hmm_regime_discovery": MockStep(
            "hmm_regime_discovery",
            {"regime_labels": np.array([0, 1, 2, 0, 1])}
        )
    }