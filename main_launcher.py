...
def main():
    ...
    subparsers = parser.add_subparsers(
        dest="mode", required=True, help="Operating mode"
    )
    ...
    backtest_parser = subparsers.add_parser(
        "backtest", help="Run the bot in backtesting mode."
    )
    backtest_parser.add_argument(
        "type",
        choices=["soft", "deep"],
        help="soft: Standard backtest. deep: In-depth validation.",
    )

    # --- Trade Mode ---
    trade_parser = subparsers.add_parser(
        "trade", help="Run an individual trading bot for a specific market."
    )
    trade_parser.add_argument(
        "--symbol", type=str, required=True, help="The trading symbol (e.g., BTCUSDT)."
    )
    trade_parser.add_argument(
        "--exchange", type=str, required=True, help="The exchange name from config."
    )
    trade_parser.add_argument(
        "type", choices=["paper", "live"], help="paper or live trading."
    )

    args = parser.parse_args()
    system_logger.info(f"Starting Ares in mode: '{args.mode}'")
...
async def run_global_manager():
    ...
    system_logger.info("Initializing Global Portfolio Manager...")
...
async def run_trading_bot(exchange_name: str, symbol: str, is_paper_trade: bool):
    ...
    system_logger.info(
        f"Initializing trading bot for {symbol} on {exchange_name} (Paper: {is_paper_trade})."
    )
    ...
    exchange_config = next(
        (
            item
            for item in settings.get("exchanges", [])
    ...
    if not exchange_config:
        system_logger.critical(
            f"Exchange '{exchange_name}' not found in config. Exiting."
        )
        return
    ...
    local_supervisor = LocalSupervisor(
        exchange_client, state_manager, firestore_manager
    )
    ares_pipeline = AresPipeline(
        exchange_client, state_manager, firestore_manager, local_supervisor
    )
    ...
    system_logger.info(f"Bot for {symbol} is now running.")
...
if __name__ == "__main__":
    try:
        ...
        system_logger.info("Ares shutdown requested by user.")
        sys.exit(0)
    except Exception as e:
        system_logger.critical(
            f"A critical error occurred in the main launcher: {e}", exc_info=True
        )
        sys.exit(1)
