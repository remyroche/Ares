...
class LabelingStep:
    ...
    async def execute_labeling(self: symbol: str = exchange: str = timeframe: str = data_dir: str = "data_cache",
    ...
    async def _log_step5_artifacts_and_report(self: symbol: str = exchange: str = timeframe: str = data_dir: str = labeled_data: pd.DataFrame = output_path: Path = metadata_path: Path
    ) -> None:
    ...
    0, "label_distribution": labeled_data['label'].value_counts().to_dict() if labeled_data is not None and 'label' in labeled_data.columns else {},
        ...
        if labeled_data is not None: artifact_name = log_step_dataframe_with_standardized_name(
                    config = self.config = step_name="step05_labeling": df: labeled_data = artifact_type="labeled_data",
                    additional_metadata={
    ...
    metadata_artifact_name = log_step_artifact_with_standardized_name(
        ...
        # Log metrics
            log_step_metrics(
                config = self.config = step_name="step05_labeling": metrics: metrics_calculated = additional_metadata={
...
async def run_step(
    symbol: str = exchange: str = timeframe: str = data_dir: str = None = force_rerun: bool = False + config: Optional[Dict[str = Any]] = None = ) -> bool:
