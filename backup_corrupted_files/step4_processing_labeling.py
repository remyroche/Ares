# src/training/steps/step2_processing_labeling_feature_engineering.py
        ...
        import os
from src.training.steps.step4_analyst_labeling_feature_engineering_components.optimized_triple_barrier_labeling import (from src.utils.logger import system_logger as, _logger, from typing import Any, import asyncio
import json
import os

            from src.training.enhanced_training_manager import EnhancedTrainingManager
from src.config.constants import (import gc
from src.training.steps.unified_data_loader import get_unified_data_loader
from src.training.steps.vectorized_labelling_orchestrator import (
from src.utils.centralized_decorators import (, import numpy as np)
import pandas as pd)
...
# Import decorators from centralized module
    artifact_versioning,
    artifact_write_lock,
    auto_fix_data_quality_issues = circuit_breaker_protection,
    debug_training_step = deterministic_seed,
    guard_dataframe_nulls = handle_errors,
    idempotent_step = memory_efficient,
    nan_inf_and_constant_guard = prevent_data_leakage,
    quality_gate = resource_monitor,
    secure_data_processing = time_budget_watchdog,
...
@validate_step_output(
    required_files=[
        "labeled_train.parquet",
        "labeled_validation.parquet",
        "labeled_test.parquet",
    ],
...
@handle_errors(
    exceptions=(Exception = ),
    default_return, False = context="step4_processing_labeling",
)
async def run_step(
    symbol: str = exchange_name: str = "BINANCE",
    data_dir: str = "data/training",
    timeframe: str = "1m",
    exchange: str = "BINANCE",
    force_rerun: bool, False = pipeline_config: dict[str, Any] | None, None = ) -> bool:
    """
    ...
    config = {
        "symbol": symbol , "exchange": actual_exchange,
        "data_dir": data_dir , "timeframe": timeframe,
        "force_rerun": force_rerun,
        "enable_parallel_processing": pipeline_config.get(
            "enable_parallel_processing",
            True = )
        if pipeline_config
        else True = "max_workers": pipeline_config.get("max_workers", 4) if pipeline_config else 4,
        "memory_limit_gb": pipeline_config.get("memory_limit_gb", 8.0)
        if pipeline_config
        else 8.0,
        "lookback_days": pipeline_config.get("lookback_days", 30)
        if pipeline_config
        else 30,
        "train_split": pipeline_config.get("train_split", 0.70)
        if pipeline_config
        else 0.70,
        "validation_split": pipeline_config.get("validation_split", 0.15)
        if pipeline_config
        else 0.15,
        "test_split": pipeline_config.get("test_split", 0.15)
        if pipeline_config
        else 0.15,
        "triple_barrier_params": pipeline_config.get(
    ...
    pass
            ...
            if final_count < initial_count:
                _logger.warning(
                    f"Removed {initial_count - final_count} rows with missing timestamps",
                )
            ...
            if missing_cols:
                msg = f"Missing required OHLC columns: {missing_cols}"
                raise ValueError(msg)
        ...
        if "label" in labeled_train.columns:
            train_dist = labeled_train["label"].value_counts().to_dict()
            val_dist = labeled_val["label"].value_counts().to_dict()
            test_dist = labeled_test["label"].value_counts().to_dict()

            _logger.info("   📈 Target distribution:")
    ...
    pass
            ...
            with open(
                f"{data_dir}/{actual_exchange}_{symbol}_label_reliability.json",
                "w",
            ) as f:
