...
class TacticianModelsTrainingStep:
    ...
    async def _apply_analyst_filtering(
        ...
        **kwargs
        ...
        try:
            ...
            # Calculate sample weights - prioritize tactician entry labeler weights if available
            sample_weights = await self._get_tactician_entry_weights(enhanced_data, **kwargs)
            if sample_weights is None:
                # Fallback to Analyst confidence weights
    ...
    async def _get_tactician_entry_weights(self, training_data: pd.DataFrame, **kwargs) -> Optional[np.ndarray]:
        """
        Get sample weights from tactician entry labeler if available.
        
        This method looks for sample weights generated by the tactician entry labeler
        in the pipeline state or artifacts from previous steps.
        """
        try:
            tprint_info("🔍 Looking for tactician entry labeler sample weights...")
            
            # Check if sample weights are available in pipeline state
            pipeline_state = kwargs.get('pipeline_state', {})
            
            # Look for tactician entry labeler artifacts
            tactician_artifacts = None
            if 'tactician_entry_labeler' in pipeline_state:
                tactician_artifacts = pipeline_state['tactician_entry_labeler']
            elif 'multi_horizon_labeling_result' in pipeline_state:
                tactician_artifacts = pipeline_state['multi_horizon_labeling_result']
            
            # Also check for sample weights directly in pipeline state
            if tactician_artifacts is None and 'sample_weights' in pipeline_state:
                tactician_artifacts = pipeline_state
            
            if tactician_artifacts and isinstance(tactician_artifacts, dict):
                # Check for sample weights in the artifacts
                sample_weights = tactician_artifacts.get('sample_weights')
                if sample_weights is not None:
                    if isinstance(sample_weights, pd.Series):
                        tprint_success(f"✅ Found tactician entry labeler sample weights: {len(sample_weights)} samples")
                        tprint_info(f"   → Weight range: {sample_weights.min():.3f} - {sample_weights.max():.3f}")
                        return sample_weights.values
                    elif isinstance(sample_weights, np.ndarray):
                        tprint_success(f"✅ Found tactician entry labeler sample weights: {len(sample_weights)} samples")
                        tprint_info(f"   → Weight range: {sample_weights.min():.3f} - {sample_weights.max():.3f}")
                        return sample_weights
                    elif isinstance(sample_weights, dict) and 'enabled' in sample_weights:
                        tprint_info("ℹ️ Found tactician entry labeler weight metadata but not actual weights")
                        tprint_info(f"   → Weight stats: min={sample_weights.get('min_weight', 'N/A')}, max={sample_weights.get('max_weight', 'N/A')}")
            
            # Check if there's a labeled data file with sample weights
            labeled_data_path = tactician_artifacts.get('labeled_data_file') if tactician_artifacts else None
            if labeled_data_path and os.path.exists(labeled_data_path):
                try:
                    import pandas as pd
                    labeled_data = pd.read_parquet(labeled_data_path)
                    if 'sample_weights' in labeled_data.columns:
                        sample_weights = labeled_data['sample_weights']
                        tprint_success(f"✅ Loaded tactician entry labeler sample weights from file: {len(sample_weights)} samples")
                        tprint_info(f"   → Weight range: {sample_weights.min():.3f} - {sample_weights.max():.3f}")
                        return sample_weights.values
                except Exception as e:
                    tprint_warning(f"⚠️ Failed to load sample weights from file: {e}")
            
            tprint_info("ℹ️ No tactician entry labeler sample weights found, will use Analyst weights")
            return None
            
        except Exception as e:
            tprint_warning(f"⚠️ Error looking for tactician entry labeler weights: {e}")
            return None
