...
class ColumnVerifier:
    ...
    def verify_missing_columns(self, df: pd.DataFrame, data_type: str = "unified") -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"✅ Column verification completed. Verification passed: {
                    missing_info['verification_passed']}"
            )
            return missing_info
    ...
    def _check_calculation_feasibility(self, df: pd.DataFrame, category: str, missing_columns: list[str]) -> list[str]:
        ...
        if category == "price_returns":
            ...
            if "close" in df.columns and "volume" in df.columns:
                can_calculate.extend(
                    [
                        col
                        for col in missing_columns
                        if col in ["vwap", "vwap_return", "price_vwap_ratio", "price_vwap_deviation"]
                    ]
                )
            ...
            if "volume" in df.columns:
                can_calculate.extend(
                    [col for col in missing_columns if col in ["volume_return", "volume_ma", "volume_ratio"]]
                )
    ...
    def _calculate_vwap_features(self, df: pd.DataFrame, missing_vwap: list[str]) -> pd.DataFrame:
        ...
        if "vwap" in missing_vwap and "close" in df.columns and "volume" in df.columns:
            calculated["vwap"] = (df["close"] * df["volume"]).rolling(window=20).sum() / df["volume"].rolling(
                window=20
            ).sum()
    ...
    def scan_dataset(
        ...
        try:
            if ignore_hidden_temp and os.path.isdir(base_dir):
                ...
                dataset = (
                    ds.dataset(file_paths, format="parquet") if file_paths else ds.dataset(base_dir, format="parquet")
                )
            else:
                ...
                if self.logger:
                    self.logger.info(
                        f"Scan read: rows={
                            len(df)}, cols={
                            len(
                                df.columns)}, bytes≈{nbytes}, filters={
                            bool(filters)}, columns_pruned={
                            columns is not None}"
                    )
            return df
    ...
    async def _process_incremental_updates(self, symbol: str, exchange: str, timeframe: str) -> bool:
        try:
            ...
            klines_dates: set[date] = set(
                map(date.fromordinal, map(lambda d: d.toordinal(), klines_data["date"].unique()))
            )
            ...
            while current_date <= max_date:
                try:
                    ...
                    unified = await self._merge_daily_data(
                        daily_klines, daily_aggtrades, daily_futures, symbol, exchange, timeframe
                    )
        ...
        try:
            ...
            if "trade_volume" in unified.columns and "volume" in unified.columns:
                unified["volume_ratio"] = (
                    (unified["trade_volume"] / unified["volume"]).replace([np.inf, -np.inf], 0).fillna(0)
                )
            return unified
        except Exception as e:
        ...
        try:
            ...
            if os.path.exists(parquet_path):
                ...
                if validation_result.passed:
                    self.logger.info(
                        f"   ✅ Loaded {
                            len(df)} klines rows (quality score: {
    ...
    async def _download_klines_data(self, symbol: str, exchange: str, timeframe: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(download_all_data_with_consolidation):  # type: ignore
                # type: ignore
                ok = await download_all_data_with_consolidation(
                    symbol=symbol, exchange_name=exchange, interval=timeframe
                )
            else:
                ok = download_all_data_with_consolidation(
                    symbol=symbol, exchange_name=exchange, interval=timeframe
                )  # type: ignore
            ...
            if validation_result.passed:
                self.logger.info(
                    f"✅ Downloaded data validation passed (quality score: {
                        validation_result.quality_score:.2f})"
                )
            else:
    ...
    async def _verify_and_calculate_missing_columns(
        ...
        try:
            ...
            if missing_info["verification_passed"]:
                ...
                self.logger.warning(
                    f"⚠️ Column verification found missing required columns: {
            ...
            for category, missing_optional in missing_info["missing_optional"].items():
                if missing_optional:
                    ...
                    self.logger.info(
                        f"📊 {category}: {
                            len(missing_optional)} missing, {
                            len(can_calculate)} can be calculated"
                    )
