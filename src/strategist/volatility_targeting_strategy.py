...
class VolatilityTargetingStrategy:
    ...
    def __init__(self, config: Optional[VolatilityTargetingConfig] = None):
        self.config = config or VolatilityTargetingConfig()
        self.logger = system_logger.getChild('VolatilityTargeting')
        self.logger.info(f"Initialized volatility targeting strategy with target: {self.config.target_volatility:.1%}")
    ...
    def calculate_position_multiplier(
        ...
        if current_volatility <= 0 or pd.isna(current_volatility):
            self.logger.warning("Invalid volatility value, returning neutral multiplier")
            return 1.0
        ...
        if self.config.regime_adjustment:
            regime_factor = self._calculate_regime_factor(price_data, current_volatility)
        ...
        multiplier = np.clip(multiplier, self.config.min_leverage, self.config.max_leverage)
    ...
    def calculate_volatility(self, price_data: pd.DataFrame) -> float:
        ...
        elif method == VolatilityMethod.ADAPTIVE:
    ...
    def _calculate_ewma_volatility(self, returns: pd.Series) -> float:
        ...
        ewma_vol = returns.ewm(span=self.config.lookback_period).std().iloc[-1] * np.sqrt(252)
    ...
    def _calculate_parkinson_volatility(self, price_data: pd.DataFrame) -> float:
        ...
        return parkinson_vol if not pd.isna(parkinson_vol) else self.config.target_volatility

    def _calculate_adaptive_volatility(self, returns: pd.Series, price_data: pd.DataFrame) -> float:
        ...
        return adaptive_vol if not pd.isna(adaptive_vol) else self.config.target_volatility
    ...
    def _calculate_regime_factor(self, price_data: pd.DataFrame, current_volatility: float) -> float:
    ...
    def _calculate_kelly_factor(self, price_data: pd.DataFrame) -> float:
        """Calculate Kelly criterion enhancement factor."""
        if len(price_data) < self.config.lookback_period:
            return 1.0
    ...
    def generate_portfolio_allocation(
        ...
        if base_weights is None:
            base_weights = {asset: 1.0 / len(assets_data) for asset in assets_data.keys()}
        ...
        self.logger.info(f"Generated volatility-targeted allocation: {adjusted_weights}")
