...
class MetadataTracker:
    ...
    async def create_metadata(self, training_input: Dict[str, Any], pipeline_state: Dict[str, Any], saved_artifacts: Dict[str, Any], version_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metadata = {'version': version_info, 'training_input': self._sanitize_training_input(training_input), 'artifacts': saved_artifacts, 'created_at': datetime.now().isoformat(), 'metadata_version': '1.0'}
    ...
    async def _track_lineage(self, pipeline_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lineage = {'pipeline_steps': [], 'data_sources': [], 'transformations': [], 'model_dependencies': {}}
        for key in sorted(pipeline_state.keys()):
            if key.endswith('_completed') and pipeline_state[key]:
                step_name = key.replace('_completed', '')
                lineage['pipeline_steps'].append({'step': step_name, 'completed': True, 'timestamp': pipeline_state.get(f'{step_name}_timestamp', 'unknown')})
        if 'data_sources' in pipeline_state:
            ...
            if hasattr(data, 'index'):
                lineage['data_sources'].append({'type': 'market_data', 'start_date': str(data.index.min()), 'end_date': str(data.index.max()), 'rows': len(data)})
        transformation_keys = ['feature_engineering', 'labeling', 'regime_discovery', 'enhancement', 'calibration']
        for key in transformation_keys:
            for state_key in pipeline_state:
                if key in state_key and 'results' in state_key:
                    lineage['transformations'].append({'type': key, 'step': state_key})
    ...
    async def _track_data_statistics(self, pipeline_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'regime_data' in pipeline_state:
            ...
            stats['regime_statistics'] = {'n_regimes': len(regime_data), 'regime_names': list(regime_data.keys())}
    ...
    async def _track_performance_metrics(self, pipeline_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metrics = {'training_metrics': {}, 'validation_metrics': {}, 'test_metrics': {}, 'best_models': {}}
        ...
        validation_keys = ['step16_confidence_calibration_results', 'step18_walk_forward_validation_results', 'step19_monte_carlo_validation_results']
    ...
    async def _track_pipeline_execution(self, pipeline_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        execution = {'completed_steps': [], 'failed_steps': [], 'execution_times': {}, 'total_duration': None}
