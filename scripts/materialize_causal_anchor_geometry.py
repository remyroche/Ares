#!/usr/bin/env python3
"""Materialise target-free Anchor Discovery state and forward-path labels.

This is an offline producer.  It consumes archived 15-minute OHLCV and
strictly-prior OI observations, builds causal anchors one symbol at a time,
and writes three immutable tables: resolved source events, active-state tape,
and target-free entry snapshots.  It never imports live code or calls an
exchange.
"""
...
from extreme_price_movements.causal_anchor_geometry import AnchorGeometryConfig, CausalAnchorGeometryEngine
...
DEFAULT_OUTPUT = ROOT / "data_perp/artifacts/causal_anchor_geometry_2025_train_2026_score_20260831_v1"
...
def _worker(payload: tuple[str, dict[pd.Timestamp, list[dict[str, object]]], str, str, str, str, str, dict[str, object]]) -> dict[str, object]:
    ...
    try:
        ...
        config = AnchorGeometryConfig(**config_raw)
        # OI is optional causal context.  A malformed or absent sidecar must
        # never discard an otherwise complete price-based anchor candidate;
        # preserve the failure provenance and emit missing OI/Kalman-OI fields.
        try:
            oi, oi_rows = _strict_prior_oi(bars, _oi_path(Path(oi_root_raw), symbol), max_staleness_bars=4)
            oi_status = "strict_prior_available" if oi_rows else "unavailable_or_stale"
            oi_exception = None
        except Exception as exc:  # narrowly scoped to an optional sidecar
            oi = pd.Series(np.nan, index=bars.index, dtype="float64")
            oi_rows = 0
            oi_status = "sidecar_read_failed_optional"
            oi_exception = f"{type(exc).__name__}: {exc}"
        ...
        return {
            ...
            "oi_status": oi_status, "oi_exception": oi_exception,
...
def main() -> None:
    ...
    config_raw: dict[str, object] = json.loads(args.config_json.resolve().read_text(encoding="utf-8")) if args.config_json else {}
    config = AnchorGeometryConfig(**config_raw)
    ...
    events = _concat(parts, "events", output / "anchor_events.parquet")
    snapshots = _concat(parts, "snapshots", output / "anchor_snapshots.parquet")
    states = _concat(parts, "states", output / "anchor_state_tape.parquet")
    manifest: dict[str, Any] = {
        "schema": "causal-anchor-geometry-v1",
        "scope": "offline Anchor Discovery research only; no live/canonical/policy/execution mutation or exchange calls",
        "inputs": {
        ...
        "causality": {
            "event_detection": "completed 15m bars only; confirmed pivots become anchors only at confirmation time",
            "oi": "strictly-prior hourly OI, max four 15m bars stale; absent OI leaves optional fields missing",
            "kalman": "forward local-linear filters only with fixed ex-ante covariance; no retrospective smoother",
            "labels": "future 8h barrier/path labels are attached only after resolution and never enter state/snapshot fields",
            "snapshots": "target-free candidate identities are preserved; unavailable anchor data never removes a candidate",
        },
        "outputs": {"events": len(events), "snapshots": len(snapshots), "states": len(states), "symbols": len(symbols)},
