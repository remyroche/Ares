...
class Monitoring:
    def __init__(
        self,
        db_manager: Union[SQLiteManager, InfluxDBManager, None],
    ...
    def record_heartbeat(self):
        ...
        if self.db_manager:
            ...
            asyncio.create_task(
                self.db_manager.set_document("monitoring", "heartbeat", heartbeat_data)
            )
        else:
            self.logger.warning(
                "DB Manager not initialized, cannot record heartbeat to DB."
            )
    ...
    def record_trade(self, trade_data: dict):
        ...
        if self.db_manager:
            ...
            self.logger.warning(
                "DB Manager not initialized, cannot record trade to DB."
            )
    ...
    def record_error(self, error_message: str):
        ...
        if self.db_manager:
            ...
            self.logger.warning(
                "DB Manager not initialized, cannot record error to DB."
            )
    ...
    def record_performance_metrics(self, metrics: dict):
        ...
        if self.db_manager:
            asyncio.create_task(
                self.db_manager.add_document("performance", performance_data)
            )
        else:
            self.logger.warning(
                "DB Manager not initialized, cannot record performance metrics to DB."
            )
    ...
    def _log_to_file(self, data: dict):
        ...
        try:
            ...
            self.logger.error(
                f"Failed to write to monitoring log file {self.log_file}: {e}",
                exc_info=True,
            )
