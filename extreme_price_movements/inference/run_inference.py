...
def _evaluate_oco_policy(
    ...
    try:
        ...
        def _get_arr(col: str) -> np.ndarray:
            if col in bars.columns:
                return bars[col].to_numpy()
            return np.full(len(bars), np.nan)

        bar_ts_arr = bars.index.to_numpy()
        open_arr = _get_arr("open")
        high_arr = _get_arr("high")
        low_arr = _get_arr("low")
        close_arr = _get_arr("close")

        for bar_ts, _o, _h, _l, _c in zip(
