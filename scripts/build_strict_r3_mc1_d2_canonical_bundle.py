...
def main() -> None:
    ...
    if config_side != args.side:
        raise ValueError("MC1 champion configuration side must match --side")
    ...
    columns = [
        ...
        "policy_path_valid", "policy_net_bps", "side_name", *FEATURES,
    ]
    ...
    if not observed_side.isin(("long", "short")).all() or not observed_side.eq(args.side).all():
        raise ValueError(
            "MC1 ledger must be side-local and match --side; "
            f"observed={observed_side.value_counts(dropna=False).to_dict()}",
        )
    source = source.loc[
        # A fitted mapper may consume only labels fully resolved before its
        # activation boundary.  ``<`` also makes this builder agree with the
        # daily strict-prequential scorer used for OOS validation.
    ...
    identity = hashlib.sha256(
        (sha(args.champion_config) + sha(args.ledger) + cutoff.isoformat() + args.side).encode()
    ).hexdigest()[:20]
    manifest = {
        "schema": SCHEMA, "contract": CONTRACT, "bundle_id": identity,
        "side": args.side, "fit_cutoff": cutoff.isoformat(),
