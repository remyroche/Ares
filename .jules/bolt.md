...
## 2026-06-25 - Cautious Formatting of Large Files
Learning: While `ruff format` and `ruff check --fix` are excellent tools, running them indiscriminately on large, legacy files like `ridge_position_sizer.py` can produce massive, unintended file-wide diffs (thousands of lines). This pollutes the Git history and violates the 'under 100 lines' rule for Bolt optimizations.
Action: For targeted performance optimizations, avoid running file-wide auto-formatters unless strictly necessary. If linters must be passed, only fix the lines explicitly touched by the optimization. Revert any accidental full-file formats using `git checkout` or `git restore` before committing.
