...
def _side_state(side: str, *, rows: int = 4) -> dict[str, object]:
    return {
        "oof_fold_ids": np.zeros(rows, dtype=np.int16),
        "fold_provenance": [
            {
                "fold": 0,
                "fold_month": "2026-05",
...
def _role_result(
    ...
    *,
    ...
    target = (
        ...
        else np.resize(np.array([0.0, 1.0]), len(prediction))
        if task == "binary"
        else np.linspace(0.0, 1.0, len(prediction))
    )
    if len(target) != len(prediction):
        raise ValueError("test role target length must match prediction length")
    if len(prediction) % 2:
        raise ValueError("test roles require an equal long/short row count")
    return {
        "role_name": role_name,
        "task_kind": task,
        "target": target,
        ...
        "side_results": {
            "long": _side_state("long", rows=len(prediction) // 2),
            "short": _side_state("short", rows=len(prediction) // 2),
        },
    }
...
def test_peak_bundle_retains_all_rows_and_shared_event_prediction(
    ...
    assert (
        manifest["research_ablation_prediction_columns"]
        == gate["research_ablation"]["prediction_columns"]
    )
    assert (
        manifest["production_prediction_columns"]
        == gate["production_promotion"]["prediction_columns"]
    )
...
def test_peak_q80_quality_gate_withholds_materially_miscalibrated_slice() -> None:
    ...
    passing_gate = _promotion_contract(
        "peak_mfe_12h_atr",
        {
            MEANINGFUL_EVENT_ROLE: generic,
            "peak_mfe_12h_atr.conditional_mean": generic,
            "peak_mfe_12h_atr.conditional_q80": report(0.80),
        },
    )
    assert (
        passing_gate["component_quality"]["conditional_q80"]["status"]
        == "RESEARCH_ABLATION_CANDIDATE_Q80"
    )
    assert (
        "pred_peak_mfe_if_hit_q80_atr"
        in passing_gate["research_ablation"]["prediction_columns"]
    )


def test_peak_bundle_persists_q80_when_signed_quality_gate_passes(
    tmp_path: Path,
) -> None:
    rows_per_side = 100
    rows = 2 * rows_per_side
    timestamp = pd.date_range("2026-05-01", periods=rows, freq="h", tz="UTC")
    side = np.repeat(["long", "short"], rows_per_side)
    symbol = np.resize(["AAA/USD:USD", "BBB/USD:USD"], rows)
    labels = pd.DataFrame(
        {
            "__ts__": timestamp,
            "__symbol__": symbol,
            ...
            "archetype": "base",
            "gmm_representation_available": 1.0,
            "__path_auxiliary_target_valid__": 1.0,
            "__meaningful_mfe_reached_12h__": 1.0,
            "__time_to_first_meaningful_mfe_hours_12h__": 2.0,
            "__peak_mfe_atr_12h__": 1.0,
            "__mae_before_meaningful_mfe_atr_12h__": 0.5,
            "__bars_to_adverse_extreme_before_mfe_12h__": 1.0,
            "__bars_to_confirmed_adverse_trough__": 1.0,
            "__future_slope_atr_per_hour_12h__": 0.1,
            "__label_end_ts__": timestamp + pd.Timedelta(hours=13),
        }
    )
    q80_target = np.tile(np.r_[np.full(80, 0.5), np.full(20, 2.0)], 2)
    roles = {
        MEANINGFUL_EVENT_ROLE: _role_result(
            MEANINGFUL_EVENT_ROLE, [0.8] * rows, task="binary"
        ),
        "peak_mfe_12h_atr.conditional_mean": _role_result(
            "peak_mfe_12h_atr.conditional_mean", [1.0] * rows
        ),
        "peak_mfe_12h_atr.conditional_q80": _role_result(
            "peak_mfe_12h_atr.conditional_q80",
            [1.0] * rows,
            task="quantile",
            target_values=q80_target.tolist(),
        ),
    }
    manifest = _persist_head(
        labels,
    ...
    assert gate["component_quality"]["conditional_q80"]["status"] == (
        "RESEARCH_ABLATION_CANDIDATE_Q80"
    )
    assert (
        "pred_peak_mfe_if_hit_q80_atr"
        in gate["research_ablation"]["prediction_columns"]
    )
    assert (
        manifest["research_ablation_prediction_columns"]
        == gate["research_ablation"]["prediction_columns"]
