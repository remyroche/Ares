...
class HMMFeatureEnhancer:
    """Enhances HMM features with additional derived features for Step 5
    compatibility."""
    ...
    def enhance_hmm_features(self, features_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.info(
                f"✅ Enhanced HMM features: {enhanced_df.shape[1]} total features"
            )
            return enhanced_df
    ...
    def _add_regime_transition_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if "composite_cluster_id" in df.columns:
                df["regime_persistence"] = self._calculate_regime_persistence(
                    df["composite_cluster_id"]
                )
                df["regime_transition_count"] = self._calculate_regime_transitions(
                    df["composite_cluster_id"]
                )
                df["regime_volatility"] = self._calculate_regime_volatility(
                    df["composite_cluster_id"]
                )
            ...
            if state_columns:
                ...
                df["state_stability"] = self._calculate_state_stability(
                    df[state_columns]
                )
    ...
    def _add_regime_stability_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if "composite_cluster_id" in df.columns:
                ...
                df["regime_consistency_5"] = (
                    df["composite_cluster_id"]
                    .rolling(5)
                    .apply(lambda x: len(x.unique()) == 1, raw=False)
                    .astype(float)
                )

                df["regime_consistency_10"] = (
                    df["composite_cluster_id"]
                    .rolling(10)
                    .apply(lambda x: len(x.unique()) == 1, raw=False)
                    .astype(float)
            ...
            if state_columns:
                ...
                df["state_prob_volatility"] = (
                    df["dominant_state_prob"].rolling(10).std()
                )

                # State probability trend
                df["state_prob_trend"] = (
                    df["dominant_state_prob"].rolling(5).mean()
                    - df["dominant_state_prob"].rolling(20).mean()
                )
    ...
    def _add_regime_interaction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if (
                "composite_cluster_id" in df.columns
                and "momentum_strength" in df.columns
            ):
                df["regime_momentum_interaction"] = (
                    df["composite_cluster_id"] * df["momentum_strength"]
                )
                df["regime_momentum_divergence"] = df["momentum_strength"] - df.groupby(
                    "composite_cluster_id"
                )["momentum_strength"].transform("mean")

            # Regime-volatility interactions
            if (
                "composite_cluster_id" in df.columns
                and "volume_volatility" in df.columns
            ):
                df["regime_volatility_interaction"] = (
                    df["composite_cluster_id"] * df["volume_volatility"]
                )
                df["regime_volatility_divergence"] = df[
                    "volume_volatility"
                ] - df.groupby("composite_cluster_id")["volume_volatility"].transform(
                    "mean"
                )

            # Regime-liquidity interactions
            if "composite_cluster_id" in df.columns and "liquidity_score" in df.columns:
                df["regime_liquidity_interaction"] = (
                    df["composite_cluster_id"] * df["liquidity_score"]
                )
                df["regime_liquidity_divergence"] = df["liquidity_score"] - df.groupby(
                    "composite_cluster_id"
                )["liquidity_score"].transform("mean")
            ...
            if len(state_columns) >= 2:
                ...
                for i, col1 in enumerate(
                    state_columns[:3]
                ):  # Limit to first 3 to avoid explosion
    ...
    def _add_missing_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if len(available_ohlcv) >= 4:  # Need at least OHLC
                ...
                # ADX (if not present)
                if "adx" not in df.columns and all(
                    col in df.columns for col in ["high", "low", "close"]
                ):
                    df["adx"] = self._calculate_adx(df["high"], df["low"], df["close"])

                # CCI (if not present)
                if "cci" not in df.columns and all(
                    col in df.columns for col in ["high", "low", "close"]
                ):
                    df["cci"] = self._calculate_cci(df["high"], df["low"], df["close"])

                # MFI (if not present)
                if "mfi" not in df.columns and all(
                ...
                # ATR (if not present)
                if "atr" not in df.columns and all(
                    col in df.columns for col in ["high", "low", "close"]
                ):
    ...
    def _add_regime_enhanced_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if (
                "momentum_strength" in df.columns
                and "composite_cluster_id" in df.columns
            ):
                df["regime_enhanced_momentum"] = df["momentum_strength"] * (
                    1 + df["dominant_state_prob"] * 0.5
                )

            # Regime-enhanced volatility
            if (
                "volume_volatility" in df.columns
                and "composite_cluster_id" in df.columns
            ):
                df["regime_enhanced_volatility"] = df["volume_volatility"] * (
                    1 + df["state_uncertainty"] * 0.3
                )

            # Regime-enhanced liquidity
            if "liquidity_score" in df.columns and "composite_cluster_id" in df.columns:
                df["regime_enhanced_liquidity"] = df["liquidity_score"] * (
                    1 + df["regime_consistency_10"] * 0.2
                )
            ...
            if (
                "momentum_strength" in df.columns
                and "regime_momentum_divergence" in df.columns
            ):
                df["regime_momentum_extreme"] = (
                    np.abs(df["regime_momentum_divergence"])
                    > df["regime_momentum_divergence"].rolling(20).std() * 2
                )
