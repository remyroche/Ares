...
# Import centralized decorators and utilities
try:
...
def failed(msg): return f"⚠️ {msg}"
def missing(msg): return f"🔍 {msg}"
...
class FinalParametersOptimizationStep:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        self.config = config
        self.logger = system_logger.getChild('FinalParametersOptimizationStep') if system_logger else create_fallback_logger()
    ...
    def _validate_environment(self) -> None:
        ...
        required_modules = ['pandas', 'numpy', 'optuna', 'sklearn']
        missing_modules = []
    ...
    def print(self, message: str) -> None:
        """Print method for compatibility with existing code."""
    ...
    async def initialize(self) -> None:
        ...
        self.logger.info('📋 Step 17 Configuration:')
    ...
    async def _optimize_volatility_parameters_advanced(self, calibration_results: dict[str, Any], previous_results: dict[str, Any] | None) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info('Step17: Starting Optuna study for volatility parameters')
    ...
    async def _optimize_position_sizing_advanced(self, calibration_results: dict[str, Any], previous_results: dict[str, Any] | None) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info('Step17: Starting Optuna study for position sizing parameters')
            study = optuna.create_study(direction='maximize')
    ...
    async def _optimize_risk_management_advanced(self, calibration_results: dict[str, Any], previous_results: dict[str, Any] | None) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info('Step17: Starting Optuna study for risk management parameters')
            study = optuna.create_study(direction='maximize')
    ...
    async def _optimize_ensemble_parameters(self, calibration_results: dict[str, Any], previous_results: dict[str, Any] | None) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info('Step17: Starting Optuna study for ensemble parameters')
            study = optuna.create_study(direction='maximize')
    ...
    async def _optimize_regime_specific_parameters(self, calibration_results: dict[str, Any], previous_results: dict[str, Any] | None) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info('Step17: Starting Optuna study for regime-specific parameters')
            study = optuna.create_study(direction='maximize')
    ...
    async def _optimize_timing_parameters(self, calibration_results: dict[str, Any], previous_results: dict[str, Any] | None) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info('Step17: Starting Optuna study for timing parameters')
            study = optuna.create_study(direction='maximize')
    ...
    def _select_best_pareto_solution(self, pareto_front: list) -> Any:
        ...
        try:
            ...
            weights = {'win_rate': 0.25, 'profit_factor': 0.25, 'sharpe_ratio': 0.25, 'max_drawdown': 0.1, 'enhanced_prediction_performance': 0.15}
            best_solution = None
            best_score = -float('inf')
            
            for solution in pareto_front:
                if solution.values and len(solution.values) >= 5:
    ...
    def _evaluate_predictions(self, calibration_results: dict[str, Any], val_df: pd.DataFrame, params: dict[str, Any]) -> dict[str, float]:
        ...
        try:
            if val_df is None or len(val_df) == 0:
                # Return default metrics if no validation data available
...
async def run_step(symbol: str, exchange: str='BINANCE', data_dir: str='data/training', force_rerun: bool=False, **kwargs) -> bool:
    ...
    Args:
        ...
        force_rerun: Whether to force rerun optimization
