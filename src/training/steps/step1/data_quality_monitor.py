...
class DataQualityMonitor:
    ...
    async def start_monitoring(
        self, symbols: List[str], exchanges: List[str], timeframes: List[str], interval_seconds: int = 300
    ) -> bool:
        ...
        while self.monitoring_active:
            try:
                ...
                self.performance_metrics["average_check_duration"] = (
                    current_avg * (total_checks - 1) + duration
                ) / total_checks
    ...
    async def _evaluate_quality_results(
        self, quality_results: Dict[str, Any], symbol: str, exchange: str, timeframe: str
    ) -> None:
                ...
                if "timestamp" in df.columns:
                    ...
                    if hours_old > self.quality_thresholds["data_freshness_hours"]:
                        alert = DataQualityAlert(
                            ...
                            details={
                                "hours_old": hours_old,
                                "threshold": self.quality_thresholds["data_freshness_hours"],
    ...
    async def _check_data_completeness(
        self, symbol: str, exchange: str, timeframe: str, quality_results: Dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            if not step3_step4_ready:
                ...
                alert = DataQualityAlert(
                    ...
                    timeframe=timeframe,
                    timestamp=datetime.now(),
                    details={"missing_requirements": missing_for_steps, "step3_step4_ready": step3_step4_ready},
                )
                await self._generate_alert(alert)
                ...
                if isinstance(file_metric, dict) and "row_count" in file_metric:
                    ...
                    if row_count < self.quality_thresholds["min_data_rows"]:
                        alert = DataQualityAlert(
                            ...
                            timeframe=timeframe,
                            timestamp=datetime.now(),
                            details={"row_count": row_count, "min_required": self.quality_thresholds["min_data_rows"]},
                        )
                        await self._generate_alert(alert)
