...
class ParameterOptimizationStep:
    ...
    def _initialize_components(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                "✅ Parameter optimization components initialized successfully"
            )

        except Exception as e:
            self.logger.error(
                f"❌ Failed to initialize parameter optimization components: {e}"
            )
            raise
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"📋 Optimization configuration loaded: {len(optimization_config)} parameters"
            )
    ...
    async def execute(self) -> bool:
        ...
        try:
            self.logger.info(
                "🎯 Starting parameter optimization for HMM regime discovery..."
            )
            ...
            clustering_optimization = await self._optimize_clustering_parameters(
                data_loaded["data"]
            )

            # Step 4: Perform feature engineering parameter optimization
            feature_optimization = await self._optimize_feature_parameters(
                data_loaded["data"]
            )
            ...
            self.logger.info(
                f"✅ Parameter optimization completed successfully in {execution_time:.2f}s"
            )
    ...
    async def _load_and_validate_data(self) -> dict[str, Any]:
        ...
        try:
            self.logger.info(
                "📊 Loading and validating data for parameter optimization..."
            )
            ...
            self.logger.info(
                f"✅ Data loaded and validated: {len(df):,} rows, {len(features.columns)} features"
            )
    ...
    async def _prepare_features_for_optimization(
        ...
        try:
            ...
            self.logger.info(
                f"✅ Features prepared: {len(optimization_features.columns)} features"
            )
    ...
    @handle_errors(
        exceptions=(Exception,), default_return={}, context="optimize_hmm_parameters"
    )
    @resource_monitor
    @secure_data_processing
    async def _optimize_hmm_parameters(self, data: pd.DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"✅ HMM parameters optimized: {optimal_components} components"
            )
    ...
    async def _optimize_clustering_parameters(
        ...
        try:
            ...
            self.logger.info(
                f"✅ Clustering parameters optimized: {optimal_clusters} clusters"
            )
            return optimization_result
        ...
        try:
            ...
            if combined_result["hmm_optimization"]:
                combined_params.update(
                    combined_result["hmm_optimization"].get("best_parameters", {})
                )

            if combined_result["clustering_optimization"]:
                combined_params.update(
                    combined_result["clustering_optimization"].get(
                        "best_parameters", {}
                    )
                )
            ...
            if combined_result["feature_optimization"]:
                combined_params.update(
                    combined_result["feature_optimization"].get("best_parameters", {})
                )
    ...
    async def _generate_optimization_reports(
        ...
        try:
            ...
            summary_report = {
                "optimization_summary": optimization_results.get(
                    "optimization_summary", {}
                ),
                "combined_parameters": optimization_results.get(
                    "combined_parameters", {}
                ),
            ...
            self.logger.info("=" * 60)
            self.logger.info(
                f"🔧 HMM Components: {optimization_results.get('combined_parameters', {}).get('n_components', 'N/A')}"
            )
            self.logger.info(
                f"🎯 Clusters: {optimization_results.get('combined_parameters', {}).get('n_clusters', 'N/A')}"
            )
            self.logger.info(
                f"📈 Momentum Window: {optimization_results.get('combined_parameters', {}).get('momentum_window', 'N/A')}"
            )
            self.logger.info(
                f"📊 Volatility Window: {optimization_results.get('combined_parameters', {}).get('volatility_window', 'N/A')}"
            )
            self.logger.info(
                f"📋 Recommendations: {len(optimization_results.get('recommendations', []))}"
            )
            self.logger.info("=" * 60)
