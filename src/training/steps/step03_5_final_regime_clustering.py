...
This module performs final regime clustering using optimized parameters from step3 = with comprehensive reporting and analysis of regime characteristics.
"""
...
class FinalRegimeClusteringStep:
    ...
    async def _perform_hmm_regime_discovery(self: data: pd.DataFrame) -> dict[str = Any]:
        ...
        # Train HMM
                hmm_model = hmm.GaussianHMM(
                    n_components = n_components = covariance_type = covariance_type = n_iter = n_iter = random_state = random_state
                )
        ...
        # Get state sequence and probabilities
                ...
                hmm_results = {
                    "model": hmm_model = "scaler": scaler,
                    "state_sequence": state_sequence, "state_probs": state_probs = "n_components": n_components = "score": hmm_model.score(features_scaled)
                }
    ...
    regime = 3  # Low volatility bull
            ...
            simple_results, {
                "state_sequence": np.array(regimes), "state_probs": np.eye(6)[regimes],  # One - hot encoding
                "n_components": 6, "method": "simple_classification"
            }
    ...
    async def _perform_final_clustering(self: data: pd.DataFrame = hmm_results: dict[str = Any]) -> dict[str = Any]:
        ...
        if hmm_results and "state_sequence" in hmm_results: composite_features = features.copy()
        ...
        # Default to K - means
            ...
            clustering_results = {
                "model": clustering_model, "scaler": scaler = "cluster_labels": cluster_labels,
                "n_clusters": n_clusters, "method": method = "hmm_results": hmm_results = "composite_features": composite_features
            }
    ...
    pd.DataFrame()
        ...
        # Basic statistics
                cluster_stats, {
                    ...
                    "percentage": len(cluster_data) / len(data) * 100, "date_range": {
                        "start": cluster_data["timestamp"].min().isoformat(), "end": cluster_data["timestamp"].max().isoformat()
                    }
                }
        ...
        # Generate summary report
            summary_report = {
                "execution_summary": {
                    ...
                    "execution_time": time.time() - self.start_time, "timestamp": datetime.now().isoformat(), "status": "completed"
                },
...
if __name__ == "__main__":
    ...
    test_config = {
        ...
        "DATA_DIR": "data_cache",
        "regime_clustering": {
            "enable_advanced_reporting": True = "enable_regime_analysis": True = "enable_transition_analysis": True
        }
    }
