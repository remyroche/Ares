...
## 2026-06-21 - Replace pandas iterrows with index intersection for fast dataframe updates
Learning: Iterating over DataFrame rows sequentially using `iterrows()` to update another DataFrame based on key columns is extremely slow.
Action: A vectorized approach using `index.intersection()` to update existing rows and `index.difference()` coupled with `pd.concat` to append new rows provides a significant performance boost (e.g. from ~1.5s to ~0.03s for 10000 rows). Also deduplicate updates using `upd_idx = upd_idx[~upd_idx.index.duplicated(keep="last")]` to avoid `ValueError: cannot reindex on an axis with duplicate labels`. Remember to use `pd.NA` for missing columns to maintain compatibility.
## 2026-06-21 - Pandas Index Intersection Data Type Alignment
Learning: When using `index.intersection()` for vectorized Pandas index matching, type mismatches in index columns (e.g., string vs datetime) silently result in an empty intersection. This occurs notably with timestamps where one dataframe uses parsed datetimes and the other uses unparsed strings.
Action: Explicitly ensure datatypes on key columns are strongly aligned before setting indices. If a key column is datetime in one dataframe and string in the other, parse the strings to `datetime64[ns, UTC]` via `pd.to_datetime(..., utc=True)` before performing `set_index` and `.intersection()`.
