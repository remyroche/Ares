...
class ExplanationVisualizer:
    ...
    def visualize_shap_values(
        ...
        try:
            ...
            if isinstance(explanation.shap_values, dict):
                # Multi-model case - create subplots
    ...
    def _create_multi_model_shap_plot(
        self,
        explanation: ExplanationResult,
        max_features: int,
    ...
    def _plot_shap_subplot(self, ax, explanation: ExplanationResult) -> None:
    ...
    def _plot_lime_subplot(self, ax, explanation: ExplanationResult) -> None:
    ...
    def _plot_feature_importance_subplot(self, ax, explanation: ExplanationResult) -> None:
    ...
    def _plot_prediction_summary_subplot(self, ax, explanation: ExplanationResult) -> None:
...
Feature Importance: {'Yes' if explanation.feature_importance is not None else 'No'}
    ...
    def _plot_feature_values_subplot(self, ax, explanation: ExplanationResult) -> None:
        ...
        try:
            ...
            feature_data = feature_data[:10]  # Top 10
    ...
    def _plot_metadata_subplot(self, ax, explanation: ExplanationResult) -> None:
...
class DecisionTraceVisualizer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.style_config = self.viz_config.get("style", {})
        self.colors = self.style_config.get("colors", {
            'tactician': '#FF6B6B',    # Red
            'hmm': '#4ECDC4',          # Teal
            'sr': '#45B7D1',           # Blue
            'analyst': '#96CEB4',      # Green
            'risk': '#FF8C94',         # Light Red
            'opportunity': '#98D8C8',  # Light Green
    ...
    def visualize_decision_trace(
        ...
        try:
            ...
            # 2. Model contributions (top right)
            ax2 = fig.add_subplot(gs[0, 1])
            self._plot_model_contributions_subplot(ax2, trace)
            
            # 3. Top contributing factors (middle left)
            ax3 = fig.add_subplot(gs[1, 0])
            self._plot_top_factors_subplot(ax3, trace)
            
            # 4. Risk vs Opportunity factors (middle right)
            ax4 = fig.add_subplot(gs[1, 1])
            ...
            # 6. Decision timeline (bottom right)
            ax6 = fig.add_subplot(gs[2, 1])
    ...
    def _plot_decision_summary_subplot(self, ax, trace: TradeDecisionTrace) -> None:
...
• Opportunity Factors: {len(trace.opportunity_factors)}
    ...
    def _plot_model_contributions_subplot(self, ax, trace: TradeDecisionTrace) -> None:
        """Plot model contributions subplot."""
        try:
            models = ['Tactician', 'HMM', 'SR', 'Analyst']
            explanations = [
                trace.tactician_explanation,
                trace.hmm_explanation,
                trace.sr_explanation,
                trace.analyst_explanation
            ]
            
            # Count models with explanations
    ...
    def _plot_top_factors_subplot(self, ax, trace: TradeDecisionTrace) -> None:
    ...
    def _plot_risk_opportunity_subplot(self, ax, trace: TradeDecisionTrace) -> None:
    ...
    def _plot_market_conditions_subplot(self, ax, trace: TradeDecisionTrace) -> None:
    ...
    def _plot_decision_timeline_subplot(self, ax, trace: TradeDecisionTrace) -> None:
