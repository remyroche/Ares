...
class AnalystEnsembleCreationStep(BaseStep):
    ...
    def _initialize_ensemble_config(self) -> Dict[str, Any]:
        ...
        return {
            "aggregation_methods": ["voting", "weighted", "stacking", "blending"],
            "voting_types": ["hard", "soft", "weighted_soft"],
            "weight_optimization": True,
            "dynamic_weighting": True,
            "regime_aware_aggregation": True,
    ...
    def validate_inputs(
        ...
        # Check for required enhanced analyst models
    ...
    @handles_errors(
        ...
        context="analyst ensemble creation logic"
    )
    async def execute_logic(
        ...
        for regime_id, regime_models in enhanced_models.items():
            ...
            if len(regime_models) < self.ensemble_config["min_models_for_ensemble"]:
                self.logger.warning(
                    ...
                    f"minimum {self.ensemble_config['min_models_for_ensemble']} required"
                )
                continue
        ...
        # Optimize ensemble weights globally
        if self.ensemble_config["weight_optimization"]:
            self.logger.info("⚖️ Optimizing ensemble weights...")
            optimized_weights = await self.weight_optimizer.optimize_weights(
                regime_ensembles,
                features
            )
        ...
        result["ensemble_summary"] = self._create_ensemble_summary(
            regime_ensembles,
            regime_ensemble_metrics
        )
    ...
    async def _create_regime_ensemble(
        ...
        """
        try:
            # Prepare models for aggregation
            prepared_models = self._prepare_models_for_aggregation(regime_models)
            
            # Check model diversity
            diversity_score = await self._calculate_model_diversity(
                prepared_models,
                regime_features
            )
            ...
            if diversity_score < self.ensemble_config["ensemble_diversity_threshold"]:
                self.logger.warning(
                    f"Low model diversity ({diversity_score:.3f}) for regime {regime_id}"
                )
            ...
            if "voting" in self.ensemble_config["aggregation_methods"]:
                voting_ensemble = await self.voting_mechanism.create_voting_ensemble(
                    prepared_models,
                    self.ensemble_config["voting_types"]
                )
                ensembles["voting"] = voting_ensemble
            
            # 2. Weighted ensemble
            if "weighted" in self.ensemble_config["aggregation_methods"]:
                weighted_ensemble = await self.ensemble_aggregator.create_weighted_ensemble(
                    prepared_models,
                    regime_features
                )
            ...
            if "stacking" in self.ensemble_config["aggregation_methods"]:
                stacking_ensemble = await self.ensemble_aggregator.create_stacking_ensemble(
                    prepared_models,
                    regime_features,
                    self.ensemble_config["meta_learner_type"]
                )
                ensembles["stacking"] = stacking_ensemble
            
            # 4. Blending ensemble
            if "blending" in self.ensemble_config["aggregation_methods"]:
                blending_ensemble = await self.ensemble_aggregator.create_blending_ensemble(
                    prepared_models,
                    regime_features
                )
            ...
            # Evaluate ensembles
            metrics = await self.ensemble_evaluator.evaluate_ensembles(
                ensembles,
                regime_features
            )
            ...
            self.logger.info(
                f"✅ Created {len(ensembles)} ensemble types for regime {regime_id}, "
                f"best: {best_ensemble_type} (accuracy: {metrics[best_ensemble_type]['accuracy']:.4f})"
            )
            
            return {
                "ensemble": ensembles,
                "metrics": metrics,
                "best_type": best_ensemble_type,
    ...
    async def _create_cross_regime_ensemble(
        ...
        """
        try:
            # Select best ensemble from each regime
            best_regime_models = {}
            ...
            meta_ensemble = await self.ensemble_aggregator.create_meta_ensemble(
                best_regime_models,
                features
            )
            
            # Evaluate meta-ensemble
            metrics = await self.ensemble_evaluator.evaluate_single_ensemble(
                meta_ensemble,
                features
            )
            
            return {
                "ensemble": {"meta": meta_ensemble},
                "metrics": {"meta": metrics},
                "best_type": "meta",
                "regime_models": list(best_regime_models.keys())
            }
    ...
    def _prepare_models_for_aggregation(
        self,
        regime_models: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for model_name, model_info in regime_models.items():
            if isinstance(model_info, dict) and "model" in model_info:
                prepared[model_name] = {
                    ...
                    "performance": model_info.get("validation_accuracy", 0.5)
                }
        
        return prepared
    
    async def _calculate_model_diversity(
        self,
        models: Dict[str, Any],
        features: pd.DataFrame
    ) -> float:
    ...
    def _create_ensemble_summary(
        ...
        summary = {
            "total_regimes": len(regime_ensembles),
            "total_ensemble_types": 0,
            "best_performers": {},
            "average_metrics": {
                "accuracy": [],
                "f1_score": [],
    ...
    def get_required_inputs(self) -> List[str]:
        ...
        return [
            ...
            "features",
            "step12_analyst_enhancement_completed"
        ]
