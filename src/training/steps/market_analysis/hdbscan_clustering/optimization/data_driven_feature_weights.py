...
try:
    ...
    BOHB_AVAILABLE = True
    logging.info("✅ BOHB optimizer loaded for feature weight optimization")
...
class DataDrivenFeatureWeightOptimizer:
    ...
    def _optimize_with_tpe(self, 
        ...
        if BOHB_AVAILABLE:
            try:
                # Create BOHB configuration for feature weight optimization
                bohb_config = BOHBConfig(
                    ...
                    enable_cv=True,
                    enable_oof_stacking=False,  # Not needed for feature weights
                    seed=42
                )

                # Define multi-fidelity objective function for BOHB
                def bohb_objective(params: Dict[str, Any], resource: int = None) -> float:
                    """Multi-fidelity objective function for BOHB feature weight optimization."""
                    try:
                        ...
                        if resource and resource < 3:
                            ...
                            for group in self.config.feature_groups:
                                if group in feature_groups:
                                    limited_weights[f'weight_{group}'] = params.get(f'weight_{group}', 1.0 / len(feature_groups))
                            return objective(limited_weights, resource)
                        else:
                ...
                if result.success:
                    ...
                    logger.info("✅ BOHB feature weight optimization completed successfully")
        ...
        if BAYESIAN_TPE_AVAILABLE:
            ...
            raise RuntimeError("No optimizers available for feature weight optimization")
