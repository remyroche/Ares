...
class RegimeModelsTrainingComponent(BaseMarketAnalysisComponent):
    ...
    async def execute(self, data: pd.DataFrame, pipeline_state: Dict[str, Any]) -> ComponentResult:
        ...
        try:
            ...
            if model_predictions:
                # Use regime_probs index since predictions are only for rows with regime probabilities
                # Get the valid regime probs index (rows where regime probs are not all NaN)
                regime_prob_cols = [col for col in protected_data.columns if col.startswith('regime_') and col.endswith('_prob')]
                # Filter rows where at least one regime prob column is not NaN
                ...
                tprint(f"📊 [REGIME_MODELS] Valid regime rows: {len(valid_regime_idx)} out of {len(protected_data)}")
                tprint(f"📊 [REGIME_MODELS] Model predictions length: {len(list(model_predictions.values())[0])}")
                ...
                tprint(f"📊 [REGIME_MODELS] Predictions index: {len(predictions_df)} rows matching regime probabilities", color="cyan")
                # Save to HDF5
            ...
            tprint("💾 [REGIME_MODELS] ========== STARTING ARTIFACT SAVING ==========")
            tprint(f"💾 [REGIME_MODELS] Symbol: {symbol}, Exchange: {exchange}, Timeframe: {timeframe}")
            tprint(f"💾 [REGIME_MODELS] Model predictions available: {len(model_predictions) if model_predictions else 0}")
            tprint(f"💾 [REGIME_MODELS] Trained models available: {len(trained_models) if trained_models else 0}")
            ...
            try:
                ...
                tprint("💾 [REGIME_MODELS] Creating artifact saver step...")
                saver_step = _ArtifactSaverStep(
                    "regime_models_training_saver",
                    use_versioned_artifacts=True
                )
                tprint("💾 [REGIME_MODELS] Setting context for artifact saver...")
                ...
                tprint(f"💾 [REGIME_MODELS] Context set: {symbol}/{exchange} [{timeframe}] long/regime")
                
                # Combine all model predictions into a single DataFrame
                if model_predictions:
                    # Use the same valid regime index as before
                    regime_prob_cols = [col for col in protected_data.columns if col.startswith('regime_') and col.endswith('_prob')]
                    valid_regime_mask = protected_data[regime_prob_cols].notna().any(axis=1)
                    valid_regime_idx = protected_data[valid_regime_mask].index
                    predictions_df = pd.DataFrame(model_predictions, index=valid_regime_idx)
    ...
    async def _generate_training_reports(self, results: Dict[str, Any], execution_time: float) -> None:
        ...
        try:
            import os
            # Ensure Path is available in this scope
            from pathlib import Path
            ...
            tprint(f"✅ [REGIME_MODELS] Training reports generated in {outcomes_dir}", color="green")
            
        except Exception as e:
            tprint(f"❌ [REGIME_MODELS] Failed to generate training reports: {e}", color="red")
    ...
    async def _generate_csv_report(self, results: Dict[str, Any], outcomes_dir: Path, timestamp: str) -> None:
        ...
        try:
            ...
            models_result = results.get('regime_models_training_result', {})
            models = models_result.get('models', {})
            model_metrics = models_result.get('model_metrics', {})
            metadata = models_result.get('metadata', {})
            ...
            for model_name, model_data in models.items():
                if isinstance(model_data, dict):
                    ...
                    csv_lines.append(f"accuracy,{accuracy},performance,{model_name}")
                    csv_lines.append(f"f1_score,{f1_score},performance,{model_name}")
                        ...
                        if isinstance(report, dict):
                            for class_name, class_metrics in report.items():
                                if isinstance(class_metrics, dict):
                                    ...
                                    csv_lines.append(f"precision_{class_name},{precision},performance,{model_name}")
                                    csv_lines.append(f"recall_{class_name},{recall},performance,{model_name}")
                                    csv_lines.append(f"f1_{class_name},{f1},performance,{model_name}")
            
            # Training metadata
            csv_lines.append(f"training_time_seconds,{execution_time},metadata,training")
            csv_lines.append(f"n_samples_trained,{metadata.get('data_shape', [0, 0])[0]},metadata,training")
            csv_lines.append(f"n_features_used,{len(metadata.get('feature_names', []))},metadata,training")
            csv_lines.append(f"n_regimes,{metadata.get('n_regimes', 0)},metadata,training")
            
            # Hardware optimization metrics
            hw_opt = metadata.get('hardware_optimization', {})
            if hw_opt:
                csv_lines.append(f"hardware_optimization_enabled,{hw_opt.get('enabled', False)},metadata,hardware")
                csv_lines.append(f"workload_type,{hw_opt.get('workload_type', 'N/A')},metadata,hardware")
            
            # Lookahead protection
            la_prot = metadata.get('lookahead_protection', {})
            if la_prot:
                csv_lines.append(f"lookahead_protection_enabled,{la_prot.get('enabled', False)},metadata,security")
            
            # Write CSV file
            csv_filename = f"regime_models_training_report_{timestamp}.csv"
            csv_path = outcomes_dir / csv_filename
            
            with open(csv_path, 'w', encoding='utf-8') as f:
                f.write("\n".join(csv_lines))
            
            tprint(f"✅ [REGIME_MODELS] CSV report saved: {csv_path}", color="green")
            
        except Exception as e:
            tprint(f"❌ [REGIME_MODELS] Failed to generate CSV report: {e}", color="red")
    ...
    async def _generate_markdown_report(self, results: Dict[str, Any], outcomes_dir: Path, timestamp: str) -> None:
        ...
        try:
            ...
            md_lines.append(f"- **Models Trained**: {len(models)}")
            md_lines.append(f"- **Features Used**: {len(metadata.get('feature_names', []))}")
            md_lines.append(f"- **Training Time**: {execution_time:.2f} seconds")
            md_lines.append(f"- **Regimes Detected**: {metadata.get('n_regimes', 0)}")
            md_lines.append("")
            
            # Model Performance Table
            md_lines.append("## Model Performance")
            md_lines.append("| Model | Accuracy | F1-Score | Status |")
            md_lines.append("|-------|----------|---------|--------|")
            ...
            for model_name, model_data in models.items():
                if isinstance(model_data, dict):
                    accuracy = model_data.get('accuracy', 'N/A')
                    f1_score = model_data.get('f1_score', 'N/A')
                    status = "✅ Trained" if accuracy != 'N/A' else "❌ Failed"
                    
                    md_lines.append(f"| {model_name} | {accuracy} | {f1_score} | {status} |")
            
            md_lines.append("")
            
            # Training Configuration
            md_lines.append("## Training Configuration")
            md_lines.append(f"- **Hardware Optimization**: {'Enabled' if metadata.get('hardware_optimization', {}).get('enabled') else 'Disabled'}")
            md_lines.append(f"- **Lookahead Protection**: {'Enabled' if metadata.get('lookahead_protection', {}).get('enabled') else 'Disabled'}")
            md_lines.append(f"- **Centralized Config**: {'Enabled' if hasattr(self, 'config_manager') else 'Disabled'}")
            md_lines.append("")
            
            # Data Statistics
            md_lines.append("## Data Statistics")
            data_shape = metadata.get('data_shape', [0, 0])
            md_lines.append(f"- **Training Samples**: {data_shape[0] if len(data_shape) > 0 else 'N/A'}")
            md_lines.append(f"- **Features**: {data_shape[1] if len(data_shape) > 1 else 'N/A'}")
            md_lines.append(f"- **Test Samples**: {metadata.get('test_shape', [0, 0])[0] if 'test_shape' in metadata else 'N/A'}")
            md_lines.append("")
            
            # Feature Information
            if metadata.get('feature_names'):
                md_lines.append("## Features Used")
                feature_names = metadata.get('feature_names', [])
                for i, feature in enumerate(feature_names[:20]):  # Show first 20 features
                    md_lines.append(f"{i+1}. {feature}")
                if len(feature_names) > 20:
                    md_lines.append(f"... and {len(feature_names) - 20} more features")
            md_lines.append("")
            
            # Footer
            md_lines.append("---")
            md_lines.append("*Report generated by Regime Models Training Component*")
            
            # Write markdown file
            md_filename = f"regime_models_training_report_{timestamp}.md"
            md_path = outcomes_dir / md_filename
            
            with open(md_path, 'w', encoding='utf-8') as f:
                f.write("\n".join(md_lines))
            
            tprint(f"✅ [REGIME_MODELS] Markdown report saved: {md_path}", color="green")
            
        except Exception as e:
            tprint(f"❌ [REGIME_MODELS] Failed to generate markdown report: {e}", color="red")
