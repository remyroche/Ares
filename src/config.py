...
class Settings(BaseSettings):
    ...
    trading_environment: Literal["LIVE", "TESTNET", "PAPER"] = Field(
        default="PAPER", env="TRADING_ENVIRONMENT"
    )
    initial_equity: float = Field(default=100.0, env="INITIAL_EQUITY")
    trade_symbol: str = Field(default="ETHUSDT", env="TRADE_SYMBOL")
    exchange_name: str = Field(
        default="BINANCE", env="EXCHANGE_NAME"
    )
    timeframe: str = Field(default="15m", env="TIMEFRAME")

    # --- Binance Credentials (loaded from .env) ---
    binance_live_api_key: Optional[str] = Field(
        default=None, env="BINANCE_LIVE_API_KEY"
    )
    binance_live_api_secret: Optional[str] = Field(
        default=None, env="BINANCE_LIVE_API_SECRET"
    )
    binance_testnet_api_key: Optional[str] = Field(
        default=None, env="BINANCE_TESTNET_API_KEY"
    )
    binance_testnet_api_secret: Optional[str] = Field(
        default=None, env="BINANCE_TESTNET_API_SECRET"
    )
    ...
    def binance_api_key(self) -> Optional[str]:
        return (
            ...
            else self.binance_testnet_api_key
        )
    ...
    def binance_api_secret(self) -> Optional[str]:
        return (
            ...
            if self.is_live_mode
            else self.binance_testnet_api_secret
        )
    ...
    def check_keys_for_environment(cls, v, values):
        if v == "LIVE" and (
            not values.get("binance_live_api_key")
            or not values.get("binance_live_api_secret")
        ):
            raise ValueError(
                "For LIVE environment, BINANCE_LIVE_API_KEY and BINANCE_LIVE_API_SECRET must be set."
            )
        if v == "TESTNET" and (
            not values.get("binance_testnet_api_key")
            or not values.get("binance_testnet_api_secret")
        ):
            raise ValueError(
                "For TESTNET environment, BINANCE_TESTNET_API_KEY and BINANCE_TESTNET_API_SECRET must be set."
            )
...
try:
    settings = Settings()
    print(
        f"Configuration loaded successfully. Trading Environment: {settings.trading_environment}"
    )
...
ARES_VERSION = "1.0.0"  # Used for model versioning
LOG_LEVEL = "INFO"
...
CONFIG: Dict[str, Any] = {
    ...
    "INFLUXDB_BUCKET": os.getenv(
        "INFLUXDB_BUCKET", "ares_market_data"
    ),  # Moved inside CONFIG
    ...
    # --- Email Configuration ---
    "email_config": {
        "enabled": settings.email_sender_address is not None
        and settings.email_recipient_address is not None,
    ...
    "analyst": {
        ...
        # Global Meta-Learner Configuration
        "global_meta_learner": {
            "n_estimators": 100,
            "learning_rate": 0.1,
            "num_leaves": 31,
            "verbose": -1,
            "objective": "multiclass",  # For classification (BUY, SELL, HOLD)
            "num_class": 3,  # Number of classes for BUY, SELL, HOLD
        },
        "overall_confidence_threshold": 0.55,  # Minimum confidence for a directional signal
    },
    ...
    # --- Risk Allocator Component ---
    "risk_allocator": {
        "portfolio_risk_pct": 0.01,  # 1% risk per trade
        "max_capital_allocation_decrease_pct": 0.75,
        "max_capital_allocation_increase_pct": 1.0,
        "risk_allocation_lookback_days": 30,
        "initial_allocated_capital_multiplier": 1.0,
    },
    # --- Sentinel Component ---
    "sentinel": {
        ...
        "model_deviation_threshold": 0.5,
        "trade_anomaly_threshold": 0.1,
        "market_health_threshold": 0.3,
    ...
    "MODEL_TRAINING": {
        ...
        "regularization": {"l1_alpha": 0.01, "l2_alpha": 0.001, "dropout_rate": 0.2},
        ...
        "feature_pruning": {
            "top_n_percent": 0.66,  # Keep 66% of features
        },
        "model_types": {
            "lightgbm": {
                "enabled": True,
...
CONFIG["klines_filename"] = (
    f"data_cache/{CONFIG['exchange_name']}_{CONFIG['trading_symbol']}_{CONFIG['trading_interval']}_{CONFIG['lookback_years']}y_klines.csv"
)
CONFIG["agg_trades_filename"] = (
    f"data_cache/{CONFIG['exchange_name']}_{CONFIG['trading_symbol']}_{CONFIG['lookback_years']}y_aggtrades.csv"
)
CONFIG["futures_filename"] = (
    f"data_cache/{CONFIG['exchange_name']}_{CONFIG['trading_symbol']}_futures_{CONFIG['lookback_years']}y_data.csv"
)
CONFIG["prepared_data_filename"] = (
    f"data_cache/{CONFIG['exchange_name']}_{CONFIG['trading_symbol']}_{CONFIG['trading_interval']}_{CONFIG['lookback_years']}y_prepared_data.csv"
)
