...
class FinalParametersOptimizationStep:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error initializing Final Parameters Optimization Step: {e}",
            )
            raise
        ...
        try:
            ...
            if not os.path.exists(calibration_file):
                raise FileNotFoundError(
                    f"Calibration results not found: {calibration_file}",
                )
            ...
            optimization_results = await self._optimize_final_parameters(
                calibration_results,
                symbol,
                exchange,
            )
            ...
            self.logger.info(
                f"✅ Final parameters optimization completed. Results saved to {optimization_dir}",
            )
    ...
    async def _optimize_final_parameters(
        self,
        ...
        try:
            self.logger.info(
                f"Optimizing final parameters for {symbol} on {exchange}...",
            )
            ...
            confidence_thresholds = await self._optimize_confidence_thresholds(
                calibration_results,
                symbol,
                exchange,
            )
            ...
            volatility_importance = await self._optimize_volatility_importance(
                calibration_results,
                symbol,
                exchange,
            )
            ...
            market_health_importance = await self._optimize_market_health_importance(
                calibration_results,
                symbol,
                exchange,
            )
            ...
            position_sizing_params = await self._optimize_position_sizing_parameters(
                calibration_results,
                symbol,
                exchange,
            )
            ...
            risk_management_params = await self._optimize_risk_management_parameters(
                calibration_results,
                symbol,
                exchange,
            )
    ...
    async def _optimize_confidence_thresholds(
        self,
        calibration_results: dict[str, Any],
        symbol: str,
        exchange: str,
    ...
    async def _optimize_volatility_importance(
        self,
        calibration_results: dict[str, Any],
        symbol: str,
        exchange: str,
        ...
        try:
            ...
            def objective(trial):
                # Define parameter ranges
                volatility_weight = trial.suggest_float(
                    "volatility_weight",
                    0.1,
                    0.9,
                    step=0.1,
                ...
                # Simulate performance evaluation
                score = self._evaluate_volatility_importance(
                    volatility_weight,
                    volatility_threshold,
                )
    ...
    async def _optimize_market_health_importance(
        self,
        calibration_results: dict[str, Any],
        symbol: str,
        exchange: str,
        ...
        try:
            ...
            def objective(trial):
                # Define parameter ranges
                market_health_weight = trial.suggest_float(
                    "market_health_weight",
                    0.1,
                    0.9,
                    step=0.1,
                )
                market_health_threshold = trial.suggest_float(
                    "market_health_threshold",
                    0.5,
                    0.9,
                    step=0.05,
                )
                ...
                # Simulate performance evaluation
                score = self._evaluate_market_health_importance(
                    market_health_weight,
                    market_health_threshold,
                )
    ...
    async def _optimize_position_sizing_parameters(
        self,
        calibration_results: dict[str, Any],
        symbol: str,
        exchange: str,
        ...
        try:
            ...
            def objective(trial):
                ...
                kelly_fraction = trial.suggest_float(
                    "kelly_fraction",
                    0.1,
                    0.5,
                    step=0.05,
                ...
                # Simulate performance evaluation
                score = self._evaluate_position_sizing_parameters(
                    base_position_size,
                    max_position_size,
                    kelly_fraction,
                )
    ...
    async def _optimize_risk_management_parameters(
        self,
        calibration_results: dict[str, Any],
        symbol: str,
        exchange: str,
        ...
        try:
            ...
            def objective(trial):
                ...
                take_profit_multiplier = trial.suggest_float(
                ...
                # Simulate performance evaluation
                score = self._evaluate_risk_management_parameters(
                    stop_loss_multiplier,
                    take_profit_multiplier,
                    max_drawdown_threshold,
                )
    ...
    def _evaluate_confidence_thresholds(
        self,
        analyst_threshold: float,
        tactician_threshold: float,
        ...
        try:
            ...
            score = (
                analyst_threshold * 0.4
                + tactician_threshold * 0.3
    ...
    def _evaluate_volatility_importance(
        self,
    ...
    def _evaluate_market_health_importance(
        self,
        market_health_weight: float,
        market_health_threshold: float,
    ) -> float:
    ...
    def _evaluate_position_sizing_parameters(
        self,
    ...
    def _evaluate_risk_management_parameters(
        self,
        ...
        try:
            # Simulate performance evaluation
            score = (take_profit_multiplier / stop_loss_multiplier) * (
                1 - max_drawdown_threshold
            )
            return score
