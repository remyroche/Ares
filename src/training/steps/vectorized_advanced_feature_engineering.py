...
Implements sophisticated market microstructure features, regime detection, and adaptive
indicators for improved prediction accuracy with vectorized operations.
"""
...
from src.utils.common_operations import generate_cache_key as common_generate_cache_key
...
from src.utils.parallel_processing_optimizer import optimize_for_m1_mac
        ...
        try:
            ...
            if len(returns) >= 16:
                ...
                trend_strength = returns.rolling(window=16, min_periods=1).apply(
                    lambda x: (
                        np.corrcoef(x, np.arange(len(x)))[0, 1] if len(x) > 1 else 0
                    ),
                    raw=True,
                )
...
class VectorizedAdvancedFeatureEngineering:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.enable_fractional_diff = FEATURE_OPTIMIZATION_CONFIG.get(
            "enable_fractional_differentiation", True
        )
        if self.enable_fractional_diff:
            fractional_config = FEATURE_OPTIMIZATION_CONFIG.get(
                "fractional_diff_config", {}
            )
            ...
            self.logger.info(
                "✅ Initialized fractional differentiation feature generator"
            )
        ...
        try:
            ...
            memory_bytes, FEATURE_OPTIMIZATION_CONFIG.get(
                "joblib_memory_bytes", 1024 * 1024 * 1024
            )
            memory_compress, FEATURE_OPTIMIZATION_CONFIG.get(
                "joblib_memory_compress", 3
            )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    "✅ Profit-based feature engineering initialized successfully"
                )
            except Exception as e:
                self.logger.warning(
                    f"⚠️ Failed to initialize profit-based feature engineering: {e}"
                )
                self.profit_feature_engineer = None
    ...
    def _calculate_price_impact_vectorized(
        ...
        """Calculate price impact using vectorized operations with improved NaN
        handling."""
        try:
    ...
    def _calculate_volume_price_impact_vectorized(
        ...
        """Calculate volume-price impact using vectorized operations with improved NaN
        handling."""
        try:
            if "close" not in price_data.columns or "volume" not in volume_data.columns:
                return pd.Series(0, index=price_data.index)
    ...
    def _calculate_order_flow_imbalance_vectorized(
        ...
        """Calculate order flow imbalance using vectorized operations with improved NaN
        handling."""
        try:
            if "close" not in price_data.columns or "volume" not in volume_data.columns:
                return pd.Series(0, index=price_data.index)
    ...
    def _handle_nan_values_basic(self, features: dict[str, Any]) -> dict[str, Any]:
        """Basic NaN handling for features when comprehensive method is not
        available."""
        try:
            cleaned_features = {}
    ...
    def _log_multi_timeframe_summary(
        self, features: dict[str, Any], timeframes: list[str]
    ) -> None:
    ...
    async def engineer_features(
        ...
        """Engineer advanced features for improved prediction accuracy using vectorized
        operations.
            ...
            if "funding_rate" in price_data.columns:
                ...
                with np.errstate(divide="ignore", invalid="ignore"):
                    features["funding_rate_returns"] = (
                        (fr.pct_change()).replace([np.inf, -np.inf], np.nan).fillna(0)
                    )
            ...
            if "volume_ratio" in price_data.columns:
                ...
                with np.errstate(divide="ignore", invalid="ignore"):
                    features["volume_ratio_returns"] = (
                        (vr.pct_change()).replace([np.inf, -np.inf], np.nan).fillna(0)
                    )
            ...
            if "trade_count" in price_data.columns:
                ...
                with np.errstate(divide="ignore", invalid="ignore"):
                    features["trade_count_returns"] = (
                        (tc.pct_change()).replace([np.inf, -np.inf], np.nan).fillna(0)
                    )
                    context_features_count += 2
            # trade_volume
            if "trade_volume" in price_data.columns:
                ...
                with np.errstate(divide="ignore", invalid="ignore"):
                    features["trade_volume_returns"] = (
                        (tv.pct_change()).replace([np.inf, -np.inf], np.nan).fillna(0)
                    )
                    context_features_count += 2
        ...
        if self.enable_fractional_diff and self.fractional_feature_generator:
            try:
                ...
                fractional_features = (
                    self.fractional_feature_generator.generate_features(combined_data)
                )
                ...
                self.logger.info(
                    f"🔍 Generated {len(frac_diff_features)} fractional differentiation features"
                )
                if frac_diff_features:
                    self.logger.info(
                        f"🔍 Fractional differentiation feature names: {list(frac_diff_features.keys())}"
                    )
                    features.update(frac_diff_features)
                    self.logger.info(
                        f"🔍 Total features after fractional differentiation: {len(features)}"
                    )
                else:
                    self.logger.warning(
                        "⚠️ No fractional differentiation features generated"
                    )
            except Exception as e:
                self.logger.warning(
                    f"⚠️ Fractional differentiation feature generation failed: {e}"
                )
        else:
        ...
        if (
            self.profit_feature_engineer
            and "potential_profit_pct" in price_data.columns
        ):
            ...
            try:
                ...
                profit_features = self.profit_feature_engineer.apply_all_features(
                    profit_data
                )
                ...
                if profit_feature_columns:
                    profit_feature_dict = {
                        col: profit_features[col] for col in profit_feature_columns
                    }
                    self.logger.info(
                        f"🔍 Generated {len(profit_feature_dict)} profit-based features"
                    )
                    self.logger.info(
                        f"🔍 Profit feature names: {list(profit_feature_dict.keys())}"
                    )
                    selected_features.update(profit_feature_dict)
                    self.logger.info(
                        f"🔍 Total features after profit-based: {len(selected_features)}"
                    )
                else:
            ...
            self.logger.info(
                "ℹ️ Profit-based feature engineering not available or profit data not present"
            )
        ...
        if self.enable_difference_acceleration_features:
            ...
            for key, value in sanitized.items():
                if hasattr(value, "__await__"):
                    self.logger.warning(
                        f"⚠️ Skipping coroutine feature in sanitized: {key}"
                    )
                    continue
            ...
            enhanced_features = (
                await self._engineer_difference_and_acceleration_features(
                    valid_sanitized, price_data
                )
            )
            ...
            self.logger.info(
                f"🔍 Generated {len(enhanced_features)} difference and acceleration features"
            )
            if enhanced_features:
                self.logger.info(
                    f"🔍 Difference/acceleration feature names: {list(enhanced_features.keys())}"
                )
                ...
                for key, value in enhanced_features.items():
                    if hasattr(value, "__await__"):
                        self.logger.warning(
                            f"⚠️ Skipping coroutine feature before merging: {key}"
                        )
                        coroutine_count += 1
                        continue
                ...
                if coroutine_count > 0:
                    self.logger.info(
                        f"⚠️ Filtered out {coroutine_count} coroutine features before merging"
                    )

                sanitized.update(valid_enhanced_features)
            self.logger.info(
                f"🔍 Total features after difference/acceleration: {len(sanitized)}"
            )
            ...
            self.logger.info(
                "ℹ️ Difference and acceleration features disabled by configuration"
            )
        ...
        for feature_name in sanitized:
            if "potential_profit_pct" in feature_name.lower():
                feature_categories["profit_based"] = (
                    feature_categories.get("profit_based", 0) + 1
                )
            elif "wavelet" in feature_name.lower():
                feature_categories["wavelet"] = feature_categories.get("wavelet", 0) + 1
            elif (
                "momentum" in feature_name.lower()
                or "rsi" in feature_name.lower()
                or "macd" in feature_name.lower()
            ):
                feature_categories["momentum"] = (
                    feature_categories.get("momentum", 0) + 1
                )
            elif "volatility" in feature_name.lower():
                feature_categories["volatility"] = (
                    feature_categories.get("volatility", 0) + 1
                )
            elif "correlation" in feature_name.lower():
                feature_categories["correlation"] = (
                    feature_categories.get("correlation", 0) + 1
                )
            elif (
                "volume" in feature_name.lower() or "liquidity" in feature_name.lower()
            ):
                feature_categories["liquidity"] = (
                    feature_categories.get("liquidity", 0) + 1
                )
            elif (
                "candlestick" in feature_name.lower()
                or "pattern" in feature_name.lower()
            ):
                feature_categories["candlestick"] = (
                    feature_categories.get("candlestick", 0) + 1
                )
            elif (
                "microstructure" in feature_name.lower()
                or "impact" in feature_name.lower()
            ):
                feature_categories["microstructure"] = (
                    feature_categories.get("microstructure", 0) + 1
                )
            elif (
                "sr" in feature_name.lower()
                or "support" in feature_name.lower()
                or "resistance" in feature_name.lower()
            ):
                feature_categories["sr_distance"] = (
                    feature_categories.get("sr_distance", 0) + 1
                )
            elif "meta" in feature_name.lower():
                feature_categories["meta_labeling"] = (
                    feature_categories.get("meta_labeling", 0) + 1
                )
            elif "timeframe" in feature_name.lower():
                feature_categories["multi_timeframe"] = (
                    feature_categories.get("multi_timeframe", 0) + 1
                )
            else:
    ...
    async def _get_wavelet_features_with_caching(
        ...
        try:
            ...
            cache_key = self.wavelet_cache.generate_cache_key(
                ...
                {
                    "volume_data_shape": (
                        volume_data.shape if volume_data is not None else None
                    ),
                },
            )
            ...
            self.logger.exception(
                f"🚨 Error getting wavelet features with caching: {e}"
            )
            return {}
    ...
    async def _engineer_microstructure_features_vectorized(
        ...
        try:
            ...
            self.logger.info(
                f"🔍 Added price_impact feature, total features: {len(features)}"
            )
            ...
            self._track_nan_origins(
                "volume_price_impact",
                {"volume_price_impact": volume_price_impact},
            )
            features["volume_price_impact"] = volume_price_impact
            self.logger.info(
                f"🔍 Added volume_price_impact feature, total features: {len(features)}"
            )
            ...
            self._track_nan_origins(
                ...
                {"bid_ask_spread_returns": bas_returns},
            )
            ...
            try:
                if order_flow_data is not None:
                    ...
                    if "depth_near" in df.columns and "depth_far" in df.columns:
                        ...
                        slope = near - far
                        # Use shift(1) to avoid NaN in first row
                        features["depth_profile_slope_proxy"] = (
                            slope - slope.shift(1)
                        ).fillna(0)
    ...
    def _calculate_bid_ask_spread_vectorized(
        ...
        """Calculate bid-ask spread using aggtrades data for accurate spread
        estimation."""
        try:
            if "close" not in price_data.columns:
            ...
            self.logger.warning(
                f"⚠️ Error calculating bid-ask spread from aggtrades: {e}"
            )
    ...
    def _track_nan_origins(self, stage: str, data_dict: dict[str, Any]) -> None:
        """Track NaN values throughout the feature engineering pipeline to identify
        origins."""
        try:
            nan_report = {}
            total_nans = 0
            ...
            for name, data in data_dict.items():
                ...
                if hasattr(data, "__await__"):
                    self.logger.warning(
                        f"⚠️ Skipping coroutine in NaN tracking for {stage}.{name}"
                    )
                    continue
                ...
                if isinstance(data, pd.Series):
                    ...
                    if nan_count > 0:
                        nan_report[name] = {
                            ...
                            "first_nan_index": (
                                data.index[data.isna()].tolist()[:5]
                                if nan_count > 0
                                else []
                            ),
                            "sample_values": (
                                data.dropna().head(3).tolist()
    ...
    def _engineer_ohlcv_price_features_vectorized(
        ...
        """Engineer basic OHLCV-based technical indicators using vectorized
        operations."""
        try:
            features = {}
            ...
            features["price_efficiency_20"] = abs(close - close.shift(20)) / (
                close.rolling(20).apply(lambda x: np.sum(np.abs(np.diff(x))))
            )
            ...
            # Chaikin money flow
            features["chaikin_money_flow"] = (clv * volume).rolling(
                20
            ).sum() / volume.rolling(20).sum()
    ...
    def _engineer_adaptive_indicators_vectorized(
        ...
        try:
            ...
            # Use shift(1) to avoid NaN in first row
            features["adaptive_sma_slope"] = (
                adaptive_sma - adaptive_sma.shift(1)
            ).fillna(0)
    ...
    def _select_optimal_features_vectorized(
        ...
        try:
            ...
            non_constant = variance[variance > 1e-8].index.tolist()

            if len(non_constant) < len(features):
                constant_features = [col for col in features if col not in non_constant]
            ...
            return {col: features[col] for col in non_constant if col in features}
    ...
    async def _engineer_multi_timeframe_features_vectorized(
        ...
        try:
            ...
            if matrix_results and self._should_use_optimized_features():
                self.logger.info(
                    "🚀 Using enhanced multi-timeframe optimizer with optimized lookback periods"
                )
                ...
                enhanced_optimizer = EnhancedMultiTimeframeOptimizer(
                    config, matrix_results
                )
                ...
                if self._should_generate_comprehensive_features():
                    ...
                    comprehensive_features = (
                        await comprehensive_optimizer.generate_comprehensive_features(
                            data=price_data,
                            target=self._create_target_variable(price_data),
                            regime_labels=self._get_regime_labels(),
                        )
                    ...
                    self.logger.info(
                        f"✅ Generated {len(comprehensive_features)} comprehensive features"
                    )
                ...
                self.logger.info(
                    f"✅ Generated {len(features)} optimized multi-timeframe features using enhanced system"
                )

            else:
                self.logger.info(
                    "⚠️ Using traditional multi-timeframe features (no matrix optimization results available)"
                )
            ...
            self.logger.error(
                f"❌ Error in enhanced multi-timeframe feature engineering: {e}"
            )
    ...
    def _should_generate_comprehensive_features(self) -> bool:
        ...
        generate_comprehensive = (
            os.getenv("GENERATE_COMPREHENSIVE_FEATURES", "true").lower() == "true"
        )
    ...
    async def _generate_traditional_multi_timeframe_features(
        ...
        try:
            ...
            processed_price, processed_volume, processed_order_flow = (
            ...
            processed_price = optimize_feature_engineering_pipeline(
                processed_price, stage="input"
            )
            processed_volume = optimize_feature_engineering_pipeline(
                processed_volume, stage="input"
            )
            if processed_order_flow is not None:
                processed_order_flow = optimize_feature_engineering_pipeline(
                    processed_order_flow, stage="input"
                )
            ...
            for tf in timeframes:
                try:
                    ...
                    tf_volume = (
                        self._resample_volume_data(processed_volume, tf)
                    ...
                    if tf_price is not None and not tf_price.empty:
                        ...
                        self.logger.debug(
                            f"🔍 {tf} timeframe - Price data shape: {tf_price.shape}"
                        )
                        self.logger.debug(
                            f"🔍 {tf} timeframe - Price data NaN count: {tf_price.isna().sum().sum()}"
                        )
                        if tf_volume is not None and not tf_volume.empty:
                            self.logger.debug(
                                f"🔍 {tf} timeframe - Volume data shape: {tf_volume.shape}"
                            )
                            self.logger.debug(
                                f"🔍 {tf} timeframe - Volume data NaN count: {tf_volume.isna().sum().sum()}"
                            )
                        ...
                        if len(tf_price) < min_required_data:
                            self.logger.info(
                                f"ℹ️ Skipping {tf} timeframe - insufficient data: {len(tf_price)} rows (minimum required: {min_required_data})"
                            )
                            continue

                        # Generate features for this timeframe using existing comprehensive generator
                        tf_features = self._generate_timeframe_features(
                            tf_price, tf_volume, tf
                        )
                        ...
                        if tf != "1m" and tf_features:
                            ...
                            for feature_name, feature_series in tf_features.items():
                                if isinstance(feature_series, pd.Series):
                                    ...
                                    aligned_feature = feature_series.reindex(
                                        processed_price.index, method="ffill"
                                    )
                                    aligned_feature = aligned_feature.fillna(
                                        method="bfill"
                                    ).fillna(0)
                            ...
                            self.logger.debug(
                                f"🔍 Aligned {tf} features to 1-minute data index"
                            )
                        ...
                        if tf_features:
                            ...
                            self.logger.info(
                                f"✅ Generated {len(tf_features)} features for {tf} timeframe"
                            )
                            self.logger.debug(
                                f"🔍 {tf} features: {list(tf_features.keys())}"
                            )
                        else:
                            self.logger.info(
                                f"ℹ️ No features generated for {tf} timeframe - insufficient data quality"
                            )
                    else:
                        self.logger.info(
                            f"ℹ️ Skipping {tf} timeframe - no data available after resampling"
                        )

                except Exception as e:
                    self.logger.warning(
                        f"⚠️ Failed to generate features for {tf} timeframe: {e}"
                    )
                    continue

            # Generate additional cross-timeframe features
            cross_timeframe_features = await self._generate_cross_timeframe_features(
                features, processed_price
            )
            ...
            try:
                regime_features = await self._generate_regime_aware_features(
                    processed_price, processed_volume
                )
                if isinstance(regime_features, dict):
                    features.update(regime_features)
                else:
                    self.logger.warning(
                        f"⚠️ Regime features not a dict: {type(regime_features)}"
                    )
            except Exception as e:
            ...
            if features:
                ...
                optimized_features_df = optimize_feature_engineering_pipeline(
                    features_df, stage="output"
                )
                features = optimized_features_df.to_dict("series")
                self.logger.info(
                    f"✅ Generated {len(features)} traditional multi-timeframe features total"
                )
            ...
            self.logger.error(
                f"❌ Error in traditional multi-timeframe feature engineering: {e}"
            )
            return {}
    ...
    def _ensure_pickle_safe_features(self, features: dict[str, Any]) -> dict[str, Any]:
        """Ensure all features are pickle-safe by removing any coroutines or async
        objects."""
        try:
            ...
            if removed_features:
                self.logger.info(
                    f"✅ Removed {len(removed_features)} non-pickle-safe features: {removed_features}"
                )
    ...
    def _resample_price_data(
        ...
        try:
            ...
            regularized_data = self._handle_irregular_time_intervals(
                price_data, timeframe
            )

            # Use optimized resampler
            resampled_data = self.optimized_resampler.resample_optimized(
                regularized_data, timeframe
            )

            if resampled_data.empty:
                self.logger.warning(
                    f"⚠️ Resampling to {timeframe} resulted in empty data"
                )
                return None
    ...
    def _resample_volume_data(
        self, volume_data: pd.DataFrame, timeframe: str
    ) -> pd.DataFrame | None:
        ...
        try:
            ...
            if resampled_data.empty:
                self.logger.warning(
                    f"⚠️ Resampling volume to {timeframe} resulted in empty data"
                )
                return None
            ...
            self.logger.exception(
                f"🚨 Error resampling volume data to {timeframe}: {e}"
            )
            return None
    ...
    def _generate_timeframe_features(
        ...
        try:
            ...
            if price_data.empty or len(price_data) < min_required_data:
                self.logger.info(
                    f"ℹ️ Insufficient data for {timeframe} timeframe: {len(price_data)} rows (minimum required: {min_required_data})"
                )
                return features
            ...
            open_price = (
                open_price.fillna(method="ffill").fillna(method="bfill").fillna(0)
            )
            ...
            if len(close) >= min_periods:
                ...
                returns = (
                    returns.fillna(method="ffill").fillna(method="bfill").fillna(0)
                )
                volatility = returns.rolling(vol_window, min_periods=min_periods).std()
                volatility = (
                    volatility.fillna(method="ffill").fillna(method="bfill").fillna(0)
                )
            ...
            if (
                ...
                if len(volume) >= min_periods and volume.var() > 1e-12:
                    volume_ma = volume.rolling(
                        vol_window, min_periods=min_periods
                    ).mean()
                    volume_ma = (
                        volume_ma.fillna(method="ffill")
                        .fillna(method="bfill")
                        .fillna(0)
                    )
                    ...
                    volume_ratio = (
                        volume_ratio.fillna(method="ffill")
                        .fillna(method="bfill")
                        .fillna(1)
                    )
            ...
            if len(close) >= min_periods:
                ...
                high_roll = (
                    high_roll.fillna(method="ffill")
                    .fillna(method="bfill")
                    .fillna(close)
                )
                low_roll = (
                    low_roll.fillna(method="ffill").fillna(method="bfill").fillna(close)
                )

                # Safety check for division by zero
                price_range = high_roll - low_roll
                price_position = (close - low_roll) / (
                    price_range + 1e-8
                )  # Avoid division by zero
                price_position = (
                    price_position.fillna(method="ffill")
                    .fillna(method="bfill")
                    .fillna(0.5)
                )
            ...
            for name, feature in features.items():
                if isinstance(feature, pd.Series):
                    ...
                    feature = (
                        feature.fillna(method="ffill").fillna(method="bfill").fillna(0)
                    )
            ...
            self.logger.debug(
                f"✅ Generated {len(valid_features)} valid features for {timeframe} timeframe"
            )
            return valid_features
    ...
    def _generate_cross_timeframe_features(
        ...
        try:
            ...
            if price_data.empty or len(price_data) < 100:  # Need sufficient data
                self.logger.warning(
                    f"⚠️ Insufficient data for cross-timeframe features: {len(price_data)} rows"
                )
                return features
            ...
            if (
                volume_data is not None
                and isinstance(volume_data, pd.DataFrame)
                and not volume_data.empty
                and "volume" in volume_data.columns
            ):
                volume = volume_data["volume"].astype(float)
                        ...
                        for tf2 in timeframes[i + 1 : 4]:
                            if tf1 < len(volume) and tf2 < len(volume):
                                ...
                                vol_momentum = volume.pct_change(
                                    tf1
                                ) - volume.pct_change(tf2)
                                if vol_momentum.var() > 1e-12:
                                    features[f"volume_momentum_{tf1}m_{tf2}m"] = (
                                        vol_momentum
                                    )
            ...
            for k_period in [5, 10, 14]:
                for d_period in [3, 5, 7]:
                    if k_period < len(close) and d_period < len(close):
                        stoch_1 = self._calculate_stochastic(
                            high, low, close, k_period, d_period
                        )
                        stoch_2 = self._calculate_stochastic(
                            high, low, close, k_period * 2, d_period * 2
                        )

                        if stoch_1 is not None and stoch_2 is not None:
                            stoch_diff = stoch_1 - stoch_2
                            if stoch_diff.var() > 1e-12:
                                features[f"stoch_diff_{k_period}_{d_period}"] = (
                                    stoch_diff
                                )
            ...
            for name, feature in features.items():
                if isinstance(feature, pd.Series):
                    ...
                    if feature.var() > 1e-12 and not feature.isna().all():
                        ...
                        self.logger.debug(
                            f"⚠️ Skipping constant cross-timeframe feature: {name}"
                        )
                else:
                    valid_features[name] = feature
    ...
    def _calculate_bollinger_bands(
        self, close: pd.Series, window: int, std: float
    ) -> pd.Series | None:
    ...
    def _calculate_stochastic(
        ...
        try:
            ...
            k_percent = 100 * (
                (close - lowest_low) / (highest_high - lowest_low + 1e-8)
            )
                ...
                if hmm_files:
                    ...
                    if "composite_cluster_id" in hmm_data.columns:
                        ...
                        if len(hmm_data) == len(price_data):
                            ...
                            features["regime_stability"] = (
                                self._calculate_regime_stability(cluster_ids)
                            )
                            features["regime_transition"] = (
                                self._calculate_regime_transitions(cluster_ids)
                            )

                            self.logger.debug(
                                f"✅ Generated {len(features)} regime-aware features"
                            )
                        else:
                            self.logger.warning(
                                "⚠️ HMM data length doesn't match price data length"
                            )
                    else:
                        self.logger.debug(
                            "ℹ️ No HMM regime data found, skipping regime-aware features"
                        )
    ...
    def _validate_and_clean_features(self, features: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
                ...
                try:
                    if isinstance(feature_value, pd.Series):
                        ...
                        if nan_ratio > 0.8:  # More than 80% NaN (increased from 50%)
                            self.logger.warning(
                                f"⚠️ Skipping feature {feature_name} with {nan_ratio:.2%} NaN values"
                            )
                            continue

                        # Fill remaining NaN values
                        feature_value = (
                            feature_value.fillna(method="ffill")
                            .fillna(method="bfill")
                            .fillna(0)
                        )
                        ...
                        if feature_value.var() == 0:
                            constant_count += 1
                            self.logger.warning(
                                f"🚨 BUG: Constant feature detected: {feature_name}"
                            )
                            self.logger.warning(
                                f"🚨 BUG: All values: {feature_value.iloc[:5].tolist()}... (first 5)"
                            )
                            self.logger.warning(
                                f"🚨 BUG: Unique values: {feature_value.nunique()}"
                            )
                            self.logger.warning(
                                f"🚨 BUG: Min/Max: {feature_value.min()}/{feature_value.max()}"
                            )
                        ...
                        if (
                            feature_value.var() < 1e-10
                        ):  # Increased threshold from 1e-12
                            constant_count += 1
                            self.logger.warning(
                                f"🚨 BUG: Near-constant feature detected: {feature_name} (variance: {feature_value.var()})"
                            )
                        ...
                        if feature_name in cleaned_features:
                            duplicate_count += 1
                            self.logger.debug(
                                f"⚠️ Skipping duplicate feature {feature_name}"
                            )
                            continue
            ...
            if duplicate_count > 0:
                self.logger.info(
                    f"🔍 Removed {duplicate_count} duplicate features during cleaning"
                )

            if constant_count > 0:
                self.logger.warning(
                    f"🚨 BUG SUMMARY: Removed {constant_count} constant features due to calculation bugs"
                )
    ...
    def _ensure_pickle_safe_features(self, features: dict[str, Any]) -> dict[str, Any]:
        """Ensure features are pickle-safe by removing any async objects or
        coroutines."""
        try:
            safe_features = {}
            removed_count = 0
            ...
            for feature_name, feature_value in features.items():
                try:
                    ...
                    if hasattr(feature_value, "__await__") or hasattr(
                        feature_value, "send"
                    ):
                        self.logger.warning(
                            f"⚠️ Removing async object from feature {feature_name}"
                        )
                        removed_count += 1
                        continue
                    ...
                    if isinstance(feature_value, (list, tuple, dict)):
                        ...
                        if hasattr(feature_value, "__iter__") and any(
                            hasattr(item, "__await__") for item in feature_value
                        ):
                            self.logger.warning(
                                f"⚠️ Removing feature {feature_name} containing async objects"
                            )
                            removed_count += 1
                            continue
                    ...
                    self.logger.warning(
                        f"⚠️ Error checking pickle safety for feature {feature_name}: {e}"
                    )
                    continue
    ...
    def _generate_fallback_features(
        ...
        try:
            ...
            if (
                ...
                features["volume_ratio"] = volume / (
                    volume.rolling(20, min_periods=1).mean() + 1e-8
                )
    ...
    def _calculate_regime_stability(self, cluster_ids: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            for i in range(1, len(cluster_ids)):
                ...
                stability[i] = 1.0 / (
                    unique_clusters + 1
                )  # Higher stability, fewer unique clusters
    ...
    async def _generate_meta_labels_vectorized(
        ...
        try:
            ...
            if "close" in price_data.columns:
                ...
                features["volatility_regime"] = pd.Series(
                    volatility_categories.astype(float), index=price_data.index
                )
    ...
    async def _engineer_difference_and_acceleration_features(
        self, features: dict[str, Any], price_data: pd.DataFrame
    ) -> dict[str, Any]:
        """Engineer difference and acceleration features with proper normalization and
        ...
        try:
            ...
            for feature_name, feature_value in features.items():
                ...
                for period in periods:
                    if len(feature_series) > period:
                        ...
                        diff_normalized = self._normalize_with_rolling_zscore(
                            diff_series, window=20
                        )

                        # Store both raw and normalized differences
                        enhanced_features[f"{feature_name}_diff_{period}"] = diff_series
                        enhanced_features[f"{feature_name}_diff_{period}_norm"] = (
                            diff_normalized
                        )

                        # Generate acceleration features for high-priority candidates
                        if feature_name in acceleration_candidates and priority in [
                            "high",
                            "medium",
                        ]:
                            if period in (1, 3) and len(diff_series) > 1:
                                ...
                                accel_normalized = self._normalize_with_rolling_zscore(
                                    accel_series, window=20
                                )
                                enhanced_features[f"{feature_name}_accel_{period}"] = (
                                    accel_series
                                )
                                enhanced_features[
                                    f"{feature_name}_accel_{period}_norm"
                                ] = accel_normalized
            ...
            for key, value in enhanced_features.items():
                if hasattr(value, "__await__"):
                    self.logger.warning(
                        f"⚠️ Skipping coroutine feature in enhanced_features: {key}"
                    )
                    continue
                valid_enhanced_features[key] = value
            ...
            try:
                interaction_features = await self._generate_interaction_features(
                    valid_enhanced_features, features, price_data
                )
                if isinstance(interaction_features, dict):
                    ...
                    for key, value in interaction_features.items():
                        if hasattr(value, "__await__"):
                            self.logger.warning(
                                f"⚠️ Skipping coroutine feature from interaction generation: {key}"
                            )
                            coroutine_count += 1
                            continue
                    ...
                    if coroutine_count > 0:
                        self.logger.info(
                            f"⚠️ Filtered out {coroutine_count} coroutine features from interaction generation"
                        )

                    enhanced_features.update(valid_interaction_features)
                else:
                    self.logger.warning(
                        f"⚠️ Interaction features not a dict: {type(interaction_features)}"
                    )
            except Exception as e:
            ...
            for key, value in features.items():
                if hasattr(value, "__await__"):
                    self.logger.warning(
                        f"⚠️ Skipping coroutine feature in features: {key}"
                    )
                    continue
                valid_features[key] = value
            ...
            if isinstance(cross_timeframe_features, dict):
                ...
                for key, value in cross_timeframe_features.items():
                    if hasattr(value, "__await__"):
                        self.logger.warning(
                            f"⚠️ Skipping coroutine feature from cross-timeframe generation: {key}"
                        )
                        coroutine_count += 1
                        continue
                ...
                if coroutine_count > 0:
                    self.logger.info(
                        f"⚠️ Filtered out {coroutine_count} coroutine features from cross-timeframe generation"
                    )

                enhanced_features.update(valid_cross_timeframe_features)
            else:
                self.logger.warning(
                    f"⚠️ Cross-timeframe features not a dict: {type(cross_timeframe_features)}"
                )
            ...
            for key, value in enhanced_features.items():
                if hasattr(value, "__await__"):
                    self.logger.warning(
                        f"⚠️ Skipping coroutine feature in final output: {key}"
                    )
                    coroutine_count += 1
                    continue
            ...
            if coroutine_count > 0:
                self.logger.info(
                    f"⚠️ Filtered out {coroutine_count} coroutine features from final output"
                )
            ...
            try:
                ...
                accel_raw = [
                    k
                    for k in final_enhanced_features
                    if "_accel_" in k and not k.endswith("_norm")
                ]
                ...
                kept_keys = set()
                for raw_key in (
                    list(kept_accel_raw) + list(kept_cross_raw) + list(kept_diff_raw)
                ):
                ...
                self.logger.info(
                    ...
                    f"accel<=%d, diff<=%d, cross-time<=%d (priority-aware)"
                    % (max_accel, max_diff, max_cross_time)
                )
            ...
            self.logger.info(
                f"✅ Generated {len(capped_features)} difference and acceleration features (after caps)"
            )
            return capped_features

        except Exception as e:
            self.logger.exception(
                f"🚨 Error engineering difference and acceleration features: {e}"
            )
            return {}
    ...
    def _normalize_with_rolling_zscore(
        self, series: pd.Series, window: int = 20
    ) -> pd.Series:
    ...
    async def _generate_interaction_features(
        ...
        """Generate comprehensive interaction features between difference/acceleration
        features.
        ...
        try:
            ...
            if not isinstance(enhanced_features, dict):
                self.logger.error(
                    f"🚨 Enhanced features is not a dict: {type(enhanced_features)}"
                )
                return {}
            ...
            if not valid_features:
                self.logger.warning(
                    "⚠️ No valid features found for interaction generation"
                )
                return {}
            ...
            momentum_features = [
                f for f in all_feature_names if "momentum" in f.lower()
            ]
            volume_features = [f for f in all_feature_names if "volume" in f.lower()]
            volatility_features = [
                f for f in all_feature_names if "volatility" in f.lower()
            ]
            ...
            bb_features = [
                ...
                if "bb" in f.lower() or "bollinger" in f.lower()
            ]
            ...
            high_value_combinations = [
                ...
                *[
                    (f1, f2)
                    for f1 in momentum_features[:5]
                    for f2 in volume_features[:3]
                ],
                ...
                *[
                    (f1, f2)
                    for f1 in volatility_features[:5]
                    for f2 in volume_features[:2]
                ],
            ...
            for feature_name in all_feature_names[
                :10
            ]:  # Use first 10 features for polynomial
                if feature_name in valid_features:
                    polynomial_combinations.append(
            ...
            for feat1_name, feat2_name in selected_combinations:
                if feat1_name in valid_features and feat2_name in valid_features:
                    ...
                    if hasattr(feat1, "__await__") or hasattr(feat2, "__await__"):
                        self.logger.warning(
                            f"⚠️ Skipping interaction for coroutine features: {feat1_name}, {feat2_name}"
                        )
                        continue
                    ...
                    interaction_features[f"{interaction_name}_norm"] = (
                        self._normalize_with_rolling_zscore(interaction_mult, window=20)
                    )

                    # 2. Division interaction (with safety check)
                    if (feat2_series != 0).any():
                        interaction_div = feat1_series / (feat2_series + 1e-8)
                        interaction_features[f"{interaction_name}_div"] = (
                            interaction_div
                        )
                        interaction_features[f"{interaction_name}_div_norm"] = (
                            self._normalize_with_rolling_zscore(
                                interaction_div, window=20
                            )
                        )

            self.logger.info(
                f"✅ Generated {len(interaction_features)} comprehensive interaction features (capped to {MAX_INTERACTION_PAIRS} pairs)"
            )
    ...
    async def _generate_cross_timeframe_features(
        ...
        try:
            ...
            for key, value in features.items():
                if hasattr(value, "__await__"):
                    self.logger.warning(
                        f"⚠️ Skipping coroutine feature in cross-timeframe: {key}"
                    )
                    coroutine_count += 1
                    continue
                valid_features[key] = value
            ...
            if not valid_features:
                self.logger.warning(
                    "⚠️ No valid features found for cross-timeframe generation"
                )
                return {}
            ...
            for (
                ...
                if feature_name in valid_features:
                    ...
                    if hasattr(feature_value, "__await__"):
                        self.logger.warning(
                            f"⚠️ Skipping coroutine feature for cross-timeframe: {feature_name}"
                        )
                        continue
                    ...
                    if isinstance(feature_value, (np.ndarray, list)):
                        feature_series = pd.Series(
                            feature_value, index=price_data.index
                        )
                    else:
                        feature_series = feature_value
                    ...
                    if len(feature_series) > max(long_period, short_period):
                        ...
                        # Normalize
                        cross_diff_norm = self._normalize_with_rolling_zscore(
                            cross_diff, window=20
                        )

                        # Additional validation to ensure normalized feature is not a coroutine
                        if hasattr(cross_diff_norm, "__await__"):
                            self.logger.warning(
                                f"⚠️ Skipping coroutine normalized feature for cross-timeframe: {output_name}"
                            )
                            continue

                        # Store features
                        cross_timeframe_features[output_name] = cross_diff
                        cross_timeframe_features[f"{output_name}_norm"] = (
                            cross_diff_norm
                        )
            ...
            for key, value in cross_timeframe_features.items():
                if hasattr(value, "__await__"):
                    self.logger.warning(
                        f"⚠️ Skipping coroutine feature in final cross-timeframe output: {key}"
                    )
                    coroutine_count += 1
                    continue
            ...
            if coroutine_count > 0:
                self.logger.info(
                    f"⚠️ Filtered out {coroutine_count} coroutine features from final cross-timeframe output"
                )

            self.logger.info(
                f"✅ Generated {len(final_cross_timeframe_features)} cross-timeframe features"
            )
    ...
    def _validate_difference_engineering_inputs(
        ...
        try:
            ...
            for feature_name, feature_value in features.items():
                if not isinstance(feature_value, (pd.Series, np.ndarray, list)):
                    self.logger.warning(
                        f"Feature {feature_name} is not a supported type: {type(feature_value)}"
                    )

            self.logger.info(
                f"✅ Validated {len(features)} features for difference engineering"
            )

        except Exception as e:
            self.logger.exception(
                f"❌ Validation failed for difference engineering inputs: {e}"
            )
            raise
    ...
    def _validate_enhanced_features(self, enhanced_features: dict[str, Any]) -> None:
        ...
        try:
            ...
            for key, value in enhanced_features.items():
                if hasattr(value, "__await__"):
                    self.logger.warning(
                        f"⚠️ Skipping coroutine feature in validation: {key}"
                    )
                    continue
                valid_features[key] = value
            ...
            cross_timeframe_features = [
                f for f in valid_features if "diff_" in f and ("m_" in f or "h_" in f)
            ]
            ...
            for feature_name, feature_value in valid_features.items():
                if isinstance(feature_value, pd.Series):
                    ...
                    if nan_ratio > 0.1:  # More than 10% NaN
                        self.logger.warning(
                            f"⚠️ Feature {feature_name} has {nan_ratio:.2%} NaN values"
                        )
                    ...
                    if inf_count > 0:
                        self.logger.warning(
                            f"⚠️ Feature {feature_name} has {inf_count} infinite values"
                        )
            ...
            self.logger.info(
                f"  - Cross-timeframe features: {len(cross_timeframe_features)}"
            )
    ...
    def _log_feature_engineering_summary(
        self, all_features: dict[str, Any], enhanced_features: dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            for key, value in all_features.items():
                if hasattr(value, "__await__"):
                    self.logger.warning(
                        f"⚠️ Skipping coroutine feature in all_features: {key}"
                    )
                    continue
            ...
            cross_timeframe_features = [
                f
                for f in valid_enhanced_features
                if "diff_" in f and ("m_" in f or "h_" in f)
            ]
            ...
            try:
                ...
                self.logger.info(
                    "📏 Caps: acceleration<=10, difference<=25, cross-timeframe<=50 (priority-aware)"
                )
            except Exception:
                pass
            ...
            try:
                ...
                self.logger.debug(
                    "ℹ️ psutil not available, skipping memory usage logging"
                )
            except Exception as e:
    ...
    def _generate_sr_levels(self, price_data: pd.DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            if daily_data.empty:
                self.logger.warning(
                    "⚠️ No daily data available for S/R level generation"
                )
                return {}
            ...
            resistance_levels = sorted(
                {level for level in resistance_levels if level > 0}
            )
            ...
            self.logger.info(
                f"✅ Generated {len(support_levels)} support levels and {len(resistance_levels)} resistance levels"
            )
            return sr_levels
    ...
    def _handle_irregular_time_intervals(
        ...
        """Handle irregular time intervals gracefully for multi-timeframe feature
        generation.
        ...
        try:
            ...
            if not isinstance(data.index, pd.DatetimeIndex):
                self.logger.warning(
                    f"⚠️ Data does not have DatetimeIndex for {timeframe} timeframe"
                )
                return data
            ...
            irregular_intervals = time_diffs[
                abs(time_diffs - expected_interval) > pd.Timedelta(seconds=30)
            ]
            ...
            self.logger.info(
                f"🔧 Handling irregular time intervals for {timeframe} timeframe (irregularity: {irregular_ratio:.3f})"
            )
            ...
            if irregular_ratio < 0.15:  # Less than 15% irregular
                ...
                regularized_data = regularized_data.fillna(method="ffill").fillna(
                    method="bfill"
                )
            ...
            resampled_data = resampled_data.fillna(method="ffill").fillna(
                method="bfill"
            )

            self.logger.debug(
                f"🔧 Applied resampling for {timeframe} timeframe (shape: {resampled_data.shape})"
            )
            return resampled_data

        except Exception as e:
            self.logger.warning(
                f"⚠️ Error handling irregular time intervals for {timeframe}: {e}"
            )
            return data

    def _validate_input_data(
        self, data: pd.DataFrame, min_records: int = 10
    ) -> Optional[str]:
