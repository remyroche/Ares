...
class MarketRegimeClassificationStep:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.info("Market Regime Classification Step initialized successfully")
            
        except Exception as e:
            self.logger.error(f"Error initializing Market Regime Classification Step: {e}")
            raise
    ...
    async def execute(self, training_input: Dict[str, Any], pipeline_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            regime_results = await self._classify_market_regimes(historical_data, symbol, exchange)
            ...
            self.logger.info(f"✅ Market regime classification completed. Results saved to {regime_file_path}")
            ...
            self.logger.error(f"❌ Error in Market Regime Classification: {e}")
            return {
                "status": "FAILED",
                "error": str(e),
                "duration": 0.0
    ...
    async def _classify_market_regimes(self, data: pd.DataFrame, symbol: str, exchange: str) -> Dict[str, Any]:
        """
        ...
        """
        try:
            self.logger.info(f"Classifying market regimes for {symbol} on {exchange}...")
            ...
            if missing_columns:
                raise ValueError(f"Missing required columns for regime classification: {missing_columns}")
            ...
            if isinstance(regime_results, dict):
                ...
                if "confidence_scores" in regime_results:
                    formatted_results["confidence_scores"] = regime_results["confidence_scores"]
            
            self.logger.info(f"Regime classification completed. Found {len(formatted_results['regime_distribution'])} distinct regimes")
