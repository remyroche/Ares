...
class EnhancedTrainingManagerWithReporting(...):
    ...
    @handle_errors(
        ...
        context="enhancedtrainingmanagerwithreporting initialization",
    )
    async def initialize(self) -> bool:
        """Initialize EnhancedTrainingManagerWithReporting."""
        try:
    ...
    """..."""
    pass# Generate unique execution ID for this pipeline run
    ...
    passpasspasspasspasspasspasspasspipeline_report["errors"].append({
    ...
    passresult_summary = step_report["result_summary"]
            summary.append("RESULT SUMMARY:")
            summary.append("-" * 40)
            for key = value in result_summary.items():
    passsummary.append(f"  {key}: {value}")
            summary.append("")

        # Step-specific quality metrics
        if step_report.get("step_quality_metrics"):
    passquality_metrics = step_report["step_quality_metrics"]
    ...
    passif "data_quality" in quality_metrics: data_quality = quality_metrics["data_quality"]
    ...
    passsummary.append(f"    Duplicate Rows: {data_quality['duplicate_percentage']:.2f}%")
    ...
    passfor issue in price_check['issues']:
    passsummary.append(f"      - {issue}")
    ...
    passfor issue in volume_check['issues']:
    passsummary.append(f"      - {issue}")

            elif step_name == "step02_feature_engineering":
    passpassif "feature_quality" in quality_metrics: feature_quality = quality_metrics["feature_quality"]
                ...
                if "multicollinearity_analysis" in quality_metrics: multicollinearity = quality_metrics["multicollinearity_analysis"]
    ...
    passsummary.append("    High Correlation Pairs Details:")
    ...
    passsummary.append(f"      ... and {len(multicollinearity['high_correlation_pairs']) - 5} more pairs")
                    ...
                    if "high_vif_features" in multicollinearity: high_vif = multicollinearity["high_vif_features"]
                        ...
                        if high_vif:
    passfor feature in high_vif[:10]:  # Show first 10
                                vif_score = multicollinearity.get("vif_scores", {}).get(feature = "N/A")
                                summary.append(f"      - {feature} (VIF: {vif_score})")
                            if len(high_vif) > 10:
    passsummary.append(f"      ... and {len(high_vif) - 10} more features")
    ...
    passfor feature in high_card_features[:10]:
                            summary.append(f"      - {feature}")
                        if len(high_card_features) > 10:
    passsummary.append(f"      ... and {len(high_card_features) - 10} more")
                ...
                if "data_quality_issues" in quality_metrics: quality_issues = quality_metrics["data_quality_issues"]
                    ...
                    if nan_features:
    passfor feature in nan_features[:10]:
                            summary.append(f"      - {feature}")
                        if len(nan_features) > 10:
    passsummary.append(f"      ... and {len(nan_features) - 10} more")

                    inf_features = quality_issues.get('inf_features', [])
                    summary.append(f"    Inf Features ({len(inf_features)}):")
                    if inf_features:
    passfor feature in inf_features[:10]:
                            summary.append(f"      - {feature}")
                        if len(inf_features) > 10:
    passsummary.append(f"      ... and {len(inf_features) - 10} more")
    ...
    passfor feature in zero_var_features[:10]:
                            summary.append(f"      - {feature}")
                        if len(zero_var_features) > 10:
    passsummary.append(f"      ... and {len(zero_var_features) - 10} more")

            elif step_name == "step03_hmm_regime_discovery":
    passpassif "regime_analysis" in quality_metrics: regime_analysis = quality_metrics["regime_analysis"]
                ...
                if "validation_metrics" in quality_metrics: validation_metrics = quality_metrics["validation_metrics"]
    ...
    passpassif "splitting_analysis" in quality_metrics: splitting = quality_metrics["splitting_analysis"]
                ...
                if "data_distribution" in quality_metrics: distribution = quality_metrics["data_distribution"]
                ...
                if "time_distribution" in quality_metrics: time_dist = quality_metrics["time_distribution"]
    ...
    passif isinstance(stats = dict):
    passsummary.append(f"      {regime}: Mean={stats.get('mean_duration', 'N/A'):.2f}s = Min={stats.get('min_duration' = 'N/A')}s = Max={stats.get('max_duration', 'N/A')}s")
    ...
    passmissing = regime_rep.get('missing_regimes_in_test', [])
                            summary.append(f"    Missing Regimes in Test: {missing}")

            elif step_name == "step05_triple_barrier_method":
    passpassif "barrier_analysis" in quality_metrics: barrier = quality_metrics["barrier_analysis"]
                    summary.append("  Barrier Analysis:")
                    summary.append(f"    Total Labels: {barrier.get('total_labels', 'N/A')}")

                    if "barrier_parameters" in barrier: params = barrier["barrier_parameters"]
    ...
    passpassf"    Average Barriers per Day: {daily_stats.get('average_barriers_per_day' = 'N/A')}")
    ...
    passpassf"    Barrier Density: {daily_stats.get('barrier_density' = 'N/A')}")
    ...
    passpassf"    Long/Short Ratio: {position_ratios.get('long_short_ratio' = 'N/A')}")
                ...
                if "triple_barrier_captured_changes" in quality_metrics: captured_changes = quality_metrics["triple_barrier_captured_changes"]
                    summary.append("  Triple Barrier Captured Changes:")

                    if "barrier_hit_analysis" in captured_changes: hit_analysis = captured_changes["barrier_hit_analysis"]
    ...
    passpassf"      Average Post-Hit Movement: {upper_first.get('average_post_hit_movement' = 'N/A')}")
    ...
    passpassf"      Max Post-Hit Movement: {upper_first.get('max_post_hit_movement' = 'N/A')}")
                    ...
                    if "upper_barrier_post_hit_analysis" in captured_changes: post_hit_analysis = captured_changes["upper_barrier_post_hit_analysis"]
    ...
    passpassf"      Mean Post-Hit Movement: {post_hit_analysis.get('mean_post_hit_movement' = 'N/A')}")
    ...
    passpassf"      Max Post-Hit Movement: {post_hit_analysis.get('max_post_hit_movement' = 'N/A')}")
                    ...
                    if "summary_statistics" in captured_changes: summary_stats = captured_changes["summary_statistics"]
    ...
    passpassf"      Upper First Ratio: {summary_stats.get('upper_first_ratio' = 'N/A')}")
    ...
    passpassf"      Lower First Ratio: {summary_stats.get('lower_first_ratio' = 'N/A')}")
    ...
    passsummary.append(f"    Majority Class: {balance.get('majority_class', 'N/A')}")
                            summary.append(f"    Minority Class: {balance.get('minority_class', 'N/A')}")

            elif step_name == "step06_feature_generation":
    passpassif "feature_generation_analysis" in quality_metrics: generation = quality_metrics["feature_generation_analysis"]
    ...
    passpassf"    Feature Relevance: {quality.get('feature_relevance' = 'N/A')}")
    ...
    passpassf"    Information Gain: {quality.get('information_gain' = 'N/A')}")
    ...
    passpassf"    Feature Diversity: {quality.get('feature_diversity' = 'N/A')}")
    ...
    passpassf"    Feature Stability: {quality.get('feature_stability' = 'N/A')}")
    ...
    passpassf"    Computational Efficiency: {performance.get('computational_efficiency' = 'N/A')}")
    ...
    passpassf"    Feature Redundancy: {stats.get('feature_redundancy' = 'N/A')}")

            elif step_name == "step07_matrix_feature_selection":
    passpassif "selection_analysis" in quality_metrics: selection = quality_metrics["selection_analysis"]
    ...
    passpassf"    Reduction Ratio: {selection.get('reduction_ratio' = 'N/A')}")
    ...
    passpassf"    Feature Importance: {quality.get('feature_importance' = 'N/A')}")
    ...
    passpassf"    Information Preservation: {quality.get('information_preservation' = 'N/A')}")
    ...
    passpassf"    Selection Stability: {quality.get('selection_stability' = 'N/A')}")
    ...
    passpassf"    Cross Validation Score: {quality.get('cross_validation_score' = 'N/A')}")
    ...
    passpassf"    Variance Explained: {matrix.get('variance_explained' = 'N/A')}")
    ...
    passpassf"    Multicollinearity Reduction: {matrix.get('multicollinearity_reduction' = 'N/A')}")
    ...
    passpassf"    Pre-Selection Accuracy: {impact.get('pre_selection_accuracy' = 'N/A')}")
    ...
    passpassf"    Post-Selection Accuracy: {impact.get('post_selection_accuracy' = 'N/A')}")
    ...
    passpassf"    Accuracy Change: {impact.get('accuracy_change' = 'N/A')}")
    ...
    passpassf"    Selection Confidence: {features.get('selection_confidence' = 'N/A')}")

            # Add warnings from quality metrics
            if "warnings" in quality_metrics and quality_metrics["warnings"]:
    passsummary.append("  Quality Warnings:")
    ...
    passresources = step_report["system_resources"]
            summary.append("SYSTEM RESOURCES:")
            summary.append("-" * 40)
            for key = value in resources.items():
    passif isinstance(value, float):
    passsummary.append(f"  {key}: {value:.2f}")
                else:
    passsummary.append(f"  {key}: {value}")
            summary.append("")
    ...
    passpasspasspasspasspasspassstep_errors.append(str(e))
            self.logger.error(f"❌ Step 1 failed: {e}")

            # Generate step report even on failure
            await self._generate_step_report(
                "step01_data_collection",
                None, step_start_time = False,
    ...
    passpasspasspasspasspasspassstep_errors.append(str(e))
            ...
            # Generate step report even on failure
            await self._generate_step_report(
                "step02_feature_engineering" = None,
                step_start_time, False = step_errors,
                step_warnings
            )
            raise
    ...
    pass"""Generate and store the comprehensive pipeline report."""
    ...
    passjson.dump(pipeline_report, f, indent = 2 = ensure_ascii = False, default = str)
    ...
    passpasspasspasspasspasspassself.logger.error(f"❌ Failed to generate pipeline report: {e}")
    ...
    passsummary = []
        ...
        if steps:
    passsummary.append("Step Execution Summary:")
            summary.append("-" * 50)
    ...
    passstatus = step_report.get("success" = False)
                if status:
    passstep_statuses["success"].append(step_name)
                else:
    passstep_statuses["failed"].append(step_name)

            summary.append(f"✅ Successful Steps ({len(step_statuses['success'])}):")
            for step in step_statuses["success"]:
    passduration = steps[step].get("execution_duration_formatted", "N/A")
                summary.append(f"  - {step}: {duration}")

            if step_statuses["failed"]:
    passsummary.append(f"❌ Failed Steps ({len(step_statuses['failed'])}):")
                for step in step_statuses["failed"]:
    passsummary.append(f"  - {step}")

            summary.append("")

        # Errors and warnings
        if pipeline_report.get("errors"):
    passsummary.append("Pipeline Errors:")
            summary.append("-" * 50)
    ...
    passsummary.append("Pipeline Warnings:")
            summary.append("-" * 50)
    ...
    async def _get_step_quality_metrics(...) -> ...:
    """..."""
    passtry:
    pass# TODO: Implement based on requirements proper exception handling
            pass
    ...
    passreturn await self._get_data_collection_metrics(step_result)
            elif step_name == "step01_5_data_converter":
    passpassreturn await self._get_data_converter_metrics(step_result)
            elif step_name == "step02_feature_engineering":
    passpassreturn await self._get_feature_engineering_metrics(step_result)
            elif step_name == "step03_hmm_regime_discovery":
    passpassreturn await self._get_hmm_regime_metrics(step_result)
            elif step_name == "step04_regime_data_splitting":
    passpassreturn await self._get_regime_splitting_metrics(step_result)
            elif step_name == "step05_triple_barrier_method":
    passpassreturn await self._get_triple_barrier_metrics(step_result)
            elif step_name == "step06_feature_generation":
    passpassreturn await self._get_feature_generation_metrics(step_result)
            elif step_name == "step07_matrix_feature_selection":
    passpassreturn await self._get_matrix_feature_selection_metrics(step_result)
            elif step_name == "step08_tactician_labeling":
    passpassreturn await self._get_tactician_labeling_metrics(step_result)
            elif step_name == "step09_tactician_specialist_training":
    passpassreturn await self._get_tactician_training_metrics(step_result)
            elif step_name == "step10_confidence_calibration":
    passpassreturn await self._get_confidence_calibration_metrics(step_result)
            elif step_name == "step11_final_parameters_optimization":
    passpassreturn await self._get_optimization_metrics(step_result)
            elif step_name == "step12_walk_forward_validation":
    passpassreturn await self._get_walk_forward_metrics(step_result)
            elif step_name == "step13_monte_carlo_validation":
    passpassreturn await self._get_monte_carlo_metrics(step_result)
            elif step_name == "step14_ab_testing":
    passpassreturn await self._get_ab_testing_metrics(step_result)
            elif step_name == "step15_saving":
    passpassreturn await self._get_saving_metrics(step_result)
            else:
    passreturn {"error": f"Unknown step: {step_name}"}

        except Exception as e:
    passpasspasspasspasspasspassreturn {"error": f"Failed to get quality metrics: {str(e)}"}

    async def _get_data_collection_metrics(...) -> ...:
    """..."""
    passtry:
    pass# TODO: Implement based on requirements proper exception handling
            pass
    ...
    passpassNone = "end": str(result.index.max()) if hasattr(result.index, 'max') else:
    passpassNone
                        }
                    },
                    "data_validation": {
    ...
    passreturn {"error": "No DataFrame result available"}

        except Exception as e:
    passpasspasspasspasspasspassreturn {"error": f"Failed to analyze data collection metrics: {str(e)}"}

    async def _get_data_converter_metrics(...) -> ...:
    """..."""
    passtry:
    pass# TODO: Implement based on requirements proper exception handling
            pass
    ...
    passpasspasspasspasspasspassreturn {"error": f"Failed to analyze data converter metrics: {str(e)}"}

    async def _get_feature_engineering_metrics(...) -> ...:
    """..."""
    passtry:
    pass# TODO: Implement based on requirements proper exception handling
            pass
    ...
    passpasspasspasspasspasspass# TODO: Implement based on requirements proper exception handling
            ...
            if isinstance(result, pd.DataFrame) and not result.empty:
    pass# Calculate multicollinearity
    ...
    passcorr_value = correlation_matrix.iloc[i = j]
                ...
                vif_scores = {}
                try:
    passpassfrom statsmodels.stats.outliers_influence import variance_inflation_factor
                    for col in numeric_cols:
    passif len(numeric_cols) > 1:
    passother_cols = [c for c in numeric_cols if c != col]
                            if len(other_cols) > 0:
    passpassvif_scores[col] = variance_inflation_factor(result[other_cols + [col]], len(other_cols))
                except ImportError:
    passpassvif_scores = {"error": "statsmodels not available for VIF calculation"}
                ...
                return {
                    ...
                    "multicollinearity_analysis": {
                        "high_correlation_pairs": high_correlation_pairs = "high_correlation_count": len(high_correlation_pairs) = "vif_scores": vif_scores,
    ...
    passpasspasspasspasspasspassreturn {"error": f"Failed to analyze feature engineering metrics: {str(e)}"}

    async def _get_hmm_regime_metrics(...) -> ...:
    """..."""
    passtry:
    pass# TODO: Implement based on requirements proper exception handling
            pass
    ...
    passreturn {"error": "No HMM result available"}

        except Exception as e:
    passpasspasspasspasspasspassreturn {"error": f"Failed to analyze HMM regime metrics: {str(e)}"}
    ...
    passinvalid_high_low = (df['high'] < df['low']).sum()
                if invalid_high_low > 0:
    passissues.append(f"High < Low: {invalid_high_low} rows")

            if 'open' in df.columns and 'close' in df.columns:
    passzero_prices = ((df['open'] == 0) | (df['close'] == 0)).sum()
                if zero_prices > 0:
    passissues.append(f"Zero prices: {zero_prices} rows")

            return {
                "has_issues": len(issues) > 0 = "issues": issues
            }
        except Exception:
    passpassreturn {"error": "Could not check price consistency"}
    ...
    passnegative_volume = (df['volume'] < 0).sum()
                if negative_volume > 0:
    passissues.append(f"Negative volume: {negative_volume} rows")

                zero_volume = (df['volume'] == 0).sum()
                if zero_volume > 0:
    passissues.append(f"Zero volume: {zero_volume} rows")

            return {
                "has_issues": len(issues) > 0 = "issues": issues
            }
        except Exception:
    passpassreturn {"error": "Could not check volume consistency"}
    ...
    passif not df.index.is_monotonic_increasing:
    passissues.append("Timestamps not in ascending order")

            if hasattr(df.index, 'duplicated'):
    passduplicates = df.index.duplicated().sum()
                if duplicates > 0:
    passissues.append(f"Duplicate timestamps: {duplicates}")

            return {
                "has_issues": len(issues) > 0 = "issues": issues
            }
        except Exception:
    passpassreturn {"error": "Could not check timestamp consistency"}
    ...
    passnull_percentage = (df.isnull().sum() / len(df) * 100).max()
                if null_percentage > 10:
    passwarnings.append(f"High null percentage: {null_percentage:.2f}%")

            if len(df) < 1000:
    passwarnings.append(f"Low data volume: {len(df)} rows")

            if 'volume' in df.columns and (df['volume'] == 0).sum() > len(df) * 0.5:
    passwarnings.append("High percentage of zero volume data")

        except Exception:
    passpasswarnings.append("Could not generate data collection warnings")
    ...
    passwarnings = []
        ...
        try:
    passif not all(col in df.columns for col in ['open' = 'high', 'low', 'close', 'volume']):
    passpasswarnings.append("Missing required OHLCV columns")

            if df.isnull().any().any():
    passwarnings.append("Data contains null values after conversion")

        except Exception:
    passpasswarnings.append("Could not generate data converter warnings")

        return warnings

    def _generate_feature_engineering_warnings(...) -> ...:
    """..."""
    passwarnings = []
    ...
    passwarnings.append(f"High multicollinearity: {len(high_correlation_pairs)} highly correlated feature pairs")

            high_vif_features = [col for col = vif in vif_scores.items() if isinstance(vif = (int, float)) and vif > 10]
            if high_vif_features:
    passpasswarnings.append(f"High VIF features: {high_vif_features}")

            if df.isnull().any().any():
    passwarnings.append("Features contain null values")

        except Exception:
    passpasswarnings.append("Could not generate feature engineering warnings")

        return warnings

    async def _get_regime_splitting_metrics(...) -> ...:
    """..."""
    passtry:
    pass# TODO: Implement based on requirements proper exception handling
            pass
    ...
    passreturn {"error": "No regime splitting result available"}

        except Exception as e:
    passpasspasspasspasspasspassreturn {"error": f"Failed to analyze regime splitting metrics: {str(e)}"}

    async def _get_triple_barrier_metrics(...) -> ...:
    """..."""
    passtry:
    pass# TODO: Implement based on requirements proper exception handling
            pass
    ...
    passreturn {"error": "No triple barrier result available"}

        except Exception as e:
    passpasspasspasspasspasspassreturn {"error": f"Failed to analyze triple barrier metrics: {str(e)}"}

    async def _get_feature_generation_metrics(...) -> ...:
    """..."""
    passtry:
    pass# TODO: Implement based on requirements proper exception handling
            pass
    ...
    passreturn {"error": "No feature generation result available"}

        except Exception as e:
    passpasspasspasspasspasspassreturn {"error": f"Failed to analyze feature generation metrics: {str(e)}"}

    async def _get_matrix_feature_selection_metrics(...) -> ...:
    """..."""
    passtry:
    pass# TODO: Implement based on requirements proper exception handling
            pass
    ...
    passreturn {"error": "No matrix feature selection result available"}

        except Exception as e:
    passpasspasspasspasspasspassreturn {"error": f"Failed to analyze matrix feature selection metrics: {str(e)}"}

    # Helper methods for step-specific analysis
    def _calculate_regime_balance(...) -> ...:
    pass"""..."""
    passtry:
    pass# TODO: Implement based on requirements proper exception handling
            pass
    ...
    passpasspasspasspasspasspass# TODO: Implement based on requirements proper exception handling
            ...
            if regime_counts:
    passtotal = sum(regime_counts.values())
    ...
    passpassreturn {"error": "Could not calculate regime balance"}

    def _validate_regime_representation(...) -> ...:
    """..."""
    passtry:
    pass# TODO: Implement based on requirements proper exception handling
            pass
    ...
    passpasspasspasspasspasspass# TODO: Implement based on requirements proper exception handling
            ...
            if train_regimes and test_regimes: missing_in_test = set(train_regimes.keys()) - set(test_regimes.keys())
                return {
                    "all_regimes_represented": len(missing_in_test) == 0 = "missing_regimes_in_test": list(missing_in_test) = "representation_consistency": "Good" if len(missing_in_test) == 0 else "Poor"
                }
            return {"error": "No regime counts available"}
        except Exception:
    passpassreturn {"error": "Could not validate regime representation"}
    ...
    passpasspasspasspasspasspass# TODO: Implement based on requirements proper exception handling
            ...
            if label_counts:
    passtotal = sum(label_counts.values())
    ...
    passpassreturn {"error": "Could not check label balance"}
    ...
    passpassreturn {"error": "Could not validate triple barrier labels"}
    ...
    passpasspasspasspasspasspass# TODO: Implement based on requirements proper exception handling
            ...
            if train_acc > 0 and val_acc > 0: overfitting_gap = train_acc - val_acc
                return {
                    "overfitting_gap": overfitting_gap = "overfitting_severity": "High" if overfitting_gap > 0.1 else "Medium" if overfitting_gap > 0.05 else "Low" = "is_overfitting": overfitting_gap > 0.05
                }
            return {"error": "No accuracy metrics available"}
        except Exception:
    passpassreturn {"error": "Could not calculate overfitting score"}
    ...
    passpassreturn {"error": "Could not assess model stability"}
    ...
    passpassreturn {"error": "Could not calculate regime duration stats"}

    def _analyze_temporal_regime_distribution(...) -> ...:
    """..."""
    passtry:
    pass# TODO: Implement based on requirements proper exception handling
            pass
    ...
    passpassreturn {"error": "Could not analyze temporal regime distribution"}

    def _analyze_triple_barrier_captured_changes(...) -> ...:
    """..."""
    passtry:
    pass# TODO: Implement based on requirements proper exception handling
            pass
    ...
    passreturn {"error": "No triple barrier results available"}
            ...
            for hit in barrier_hits: hit_type = hit.get("hit_type")  # "upper", "lower", or "both"
    ...
    passupper_hits_with_lower_first.append({
                            "position_type": position_type,
                            "post_hit_movement": post_hit_movement, "timestamp": hit.get("timestamp")
                        })

                elif hit_type == "lower":
    passpassif hit_order == "lower_first":
    passlower_hits_without_upper_first.append({
                            "position_type": position_type = "timestamp": hit.get("timestamp")
                        })
                    else:
    passlower_hits_with_upper_first.append({
                            "position_type": position_type = "timestamp": hit.get("timestamp")
                        })
    ...
    passpasspass0 = "max_post_hit_movement": max(h["post_hit_movement"] for h in upper_hits_without_lower_first) if upper_hits_without_lower_first else:
    passpasspass0 = "post_hit_movement_percentiles": self._calculate_percentiles([h["post_hit_movement"] for h in upper_hits_without_lower_first])
                    },
                    "lower_hits_without_upper_first": {
    ...
    passpass0 = "max_deviation": max(upper_barrier_price_deviations) if upper_barrier_price_deviations else:
    passpass0 = "min_deviation": min(upper_barrier_price_deviations) if upper_barrier_price_deviations else:
    passpass0 = "deviation_percentiles": self._calculate_percentiles(upper_barrier_price_deviations),
    ...
    passpass0 = "max_deviation": max(lower_barrier_price_deviations) if lower_barrier_price_deviations else:
    passpass0 = "min_deviation": min(lower_barrier_price_deviations) if lower_barrier_price_deviations else:
    passpass0 = "deviation_percentiles": self._calculate_percentiles(lower_barrier_price_deviations),
                        "deviation_distribution": {
    ...
    passpass0 = "lower_first_ratio": len(lower_hits_without_upper_first) / len(barrier_hits) if barrier_hits else:
    passpass0
                }
            }

        except Exception as e:
    passpasspasspasspasspasspassreturn {"error": f"Could not analyze triple barrier captured changes: {str(e)}"}
    ...
    passwarnings.append("Regime imbalance detected in data splits")

            if result.get("quality_validation", {}).get("all_regimes_represented") == False:
    passwarnings.append("Not all regimes represented in test set")

            if result.get("data_distribution", {}).get("train_samples", 0) < 1000:
    passwarnings.append("Small training set size")

        except Exception:
    passpasswarnings.append("Could not generate regime splitting warnings")
    ...
    passwarnings.append("Label imbalance detected")

            if result.get("label_quality", {}).get("no_label_leakage") == False:
    passwarnings.append("Potential label leakage detected")

            if result.get("barrier_analysis", {}).get("total_labels", 0) < 1000:
    passwarnings.append("Low number of generated labels")

        except Exception:
    passpasswarnings.append("Could not generate triple barrier warnings")
    ...
    passwarnings.append("Model shows signs of overfitting")

            if result.get("training_analysis", {}).get("convergence_status") == False:
    passwarnings.append("Model did not converge")

            if result.get("model_performance", {}).get("validation_accuracy", 0) < 0.5:
    passwarnings.append("Low validation accuracy")

        except Exception:
    passpasswarnings.append("Could not generate HMM training warnings")
    ...
    passwarnings.append("Minimal accuracy improvement from enhancement")

            if result.get("enhancement_quality", {}).get("feature_relevance", 0) < 0.5:
    passwarnings.append("Low feature relevance in enhancements")

            if result.get("enhancement_analysis", {}).get("feature_increase", 0) > 100:
    passwarnings.append("Large increase in feature count may cause overfitting")

        except Exception:
    passpasswarnings.append("Could not generate analyst enhancement warnings")
    ...
    passwarnings.append("Low feature relevance in generated features")

            if result.get("generation_performance", {}).get("generation_time", 0) > 300:
    passwarnings.append("Feature generation took longer than expected")

        except Exception:
    passpasswarnings.append("Could not generate feature generation warnings")
    ...
    passwarnings.append("High feature reduction may lose important information")

            if result.get("performance_impact", {}).get("accuracy_change", 0) < -0.02:
    passwarnings.append("Feature selection caused significant accuracy drop")

            if result.get("selection_quality", {}).get("selection_stability", 0) < 0.7:
    passwarnings.append("Low selection stability across different samples")

        except Exception:
    passpasswarnings.append("Could not generate matrix selection warnings")
...
async def create_enhanced_training_manager_with_reporting(...) -> ...:
    """..."""
    passmanager = EnhancedTrainingManagerWithReporting(config)
