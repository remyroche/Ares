...
class BaseStep(ABC):
    ...
    @handles_errors(exceptions=(Exception,), default_return={'success': False}, context='step initialization')
    ...
    @handles_errors(exceptions=(Exception,), default_return={'success': False}, context='step execution')
    async def execute(self, training_input: Dict[str, Any], pipeline_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f'✅ {self.full_step_name} completed successfully in {self.execution_duration:.2f}s')
    ...
    def _record_input_info(self, training_input: Dict[str, Any], pipeline_state: Dict[str, Any]) -> None:
        ...
        self.input_data_info = {'training_input_keys': list(training_input.keys()), 'pipeline_state_keys': list(pipeline_state.keys()), 'timestamp': datetime.now().isoformat()}

    def _record_output_info(self, result: Dict[str, Any]) -> None:
        """Record information about output data."""
        self.output_data_info = {'result_keys': list(result.keys()), 'new_keys': [k for k in result.keys() if k not in self.input_data_info.get('pipeline_state_keys', [])], 'timestamp': datetime.now().isoformat()}

    def _create_failure_result(self, pipeline_state: Dict[str, Any], reason: str, errors: list) -> Dict[str, Any]:
    ...
    async def _generate_step_report(self, result: Dict[str, Any]) -> None:
        ...
        report = {'step_info': {'step_number': self.step_number, 'step_name': self.step_name, 'full_step_name': self.full_step_name}, 'execution_info': {'status': self.execution_status, 'start_time': self.start_time, 'end_time': self.end_time, 'duration': self.execution_duration, 'errors': self.execution_errors}, 'data_info': {'input': self.input_data_info, 'output': self.output_data_info}, 'timestamp': datetime.now().isoformat()}
