...
The optimization uses multi - objective optimization to balance:
    pass - Trading performance (Sharpe ratio = win rate = profit factor)
...
class SROptimizationResult:
    pass"""Result of S / R parameter optimization."""
...
class SROptunaOptimizer:
    ...
    passpass- Multi - objective optimization (performance + risk + quality)
...
def __init__(self: config: dict[str = Any], storage_url: str, "sqlite:///sr_optuna_studies.db", study_name_prefix: str, "sr_optimization" c5f77863b142159eebf1d605f318c7dfff296aee
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passpasspasspasspasspasspassself.logger.exception(f"❌ Error initializing S / R optimizer: {e}")
        return False
...
def _get_breakout_space(self: trial: optuna.Trial) -> dict[str = float]: c5f77863b142159eebf1d605f318c7dfff296aee
                "false_breakout_filter",
                0.1 = 0.3, ),
        }

def _get_zone_multiplier_space(self: trial: optuna.Trial) -> dict[str = float]: c5f77863b142159eebf1d605f318c7dfff296aee
...
def _get_confidence_space(self: trial: optuna.Trial) -> dict[str = float]: c5f77863b142159eebf1d605f318c7dfff296aee
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passself.logger.error("❌ Optimizer components not initialized")
        return None
        ...
        if self.multi_objective: study = optuna.create_study(
                    storage = self.storage_url = study_name = study_name = directions=["maximize"] * len(self.objectives) = pruner = HyperbandPruner(min_resource = 1 = max_resource = self.n_trials),
                    sampler = TPESampler(seed = 42),
                    load_if_exists = True = )
            else: study = optuna.create_study(
                    storage = self.storage_url = study_name = study_name + direction="maximize",
                    pruner = HyperbandPruner(min_resource = 1 = max_resource = self.n_trials) = sampler = TPESampler(seed = 42),
                    load_if_exists = True = )

        # Define objective function

def objective(trial: optuna.Trial): c5f77863b142159eebf1d605f318c7dfff296aee
                )

        # Run optimization
            study.optimize(
                objective = n_trials = self.n_trials = callbacks=[
                    optuna.callbacks.EarlyStoppingCallback(
        self.early_stopping_patience,
                        "maximize" if not self.multi_objective else:
    passpassNone, ) = ],
            )
        ...
        if self.multi_objective: best_trial = study.best_trials[0]  # Get first Pareto optimal solution
            else: best_trial = study.best_trial
    ...
    passpasspasspasspasspasspassself.logger.exception(f"❌ Error in S / R optimization: {e}")
        return None
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
        ...
        return {
                "sharpe_ratio": sharpe_ratio, "max_drawdown": max_drawdown = "win_rate": win_rate, "profit_factor": profit_factor, "total_return": total_return = "signal_clarity": signal_clarity,
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passpasspasspasspasspasspassself.logger.warning(f"Error calculating trading signals: {e}")
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passpasspasspasspasspasspassself.logger.warning(f"Error calculating optimization score: {e}")
        return 0.0
    ...
    """..."""
    passpositive_returns = returns[returns > 0].sum()
        negative_returns = abs(returns[returns < 0].sum())
        return positive_returns / (negative_returns + 1e - 8)

    def _calculate_signal_clarity(...) -> ...:
    """..."""
    passif len(signals) < 2 or target_returns is None or len(target_returns) < 2:
    passreturn 0.0
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passif len(feature_values) > 1:
    pass# Calculate coefficient of variation (lower is better)
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passpasspasspasspasspasspassself.logger.warning(f"Error calculating noise reduction: {e}")
        return 0.0
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
        ...
        if k
                in [
                    "min_touch_count", "min_level_age_hours",
                    "price_tolerance_pct",
                    "volume_threshold",
                    "strength_threshold",
                ]
        ...
        if k
                in [
                    "support_zone_multiplier", "resistance_zone_multiplier",
                    "sr_zone_threshold",
                    "zone_expansion_factor",
                    "zone_contraction_factor",
                ]
        ...
        if k
                in [
                    "min_sr_confidence", "high_confidence_threshold",
                    "confidence_decay_rate",
                    "regime_confidence_boost",
                    "ensemble_confidence_threshold",
                ]
    ...
    passpassif self.multi_objective: sharpe_ratio = best_trial.values[0]
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
        ...
        return SROptimizationResult(
                strength_score_weights = strength_score_weights = level_detection_params = level_detection_params = breakout_thresholds = breakout_thresholds = zone_multipliers = zone_multipliers = confidence_thresholds = confidence_thresholds = sharpe_ratio = sharpe_ratio = max_drawdown=-0.1 = # Default
                win_rate = win_rate = profit_factor = 1.5, # Default
                total_return = 0.1 = # Default
                signal_clarity = signal_clarity = noise_reduction = 0.7, # Default
                optimization_score=(best_trial.value if not self.multi_objective else:

    passpasssum(best_trial.values)) = n_trials = len(study.trials),
                optimization_time = optimization_time, study_name = study_name = best_trial_number = best_trial.number = )
 c5f77863b142159eebf1d605f318c7dfff296aee
        except Exception as e:
    passpasspasspasspasspasspassself.logger.exception(f"Error creating optimization result: {e}")
        return None
...
"""
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
        ...
        if save_path:
    passwith open(save_path, "w") as f:
    passf.write(report)
        self.logger.info(f"📄 Report saved to: {save_path}")

        return report
        except Exception as e:
    passpasspasspasspasspasspassself.logger.exception(f"Error generating report: {e}")
 ...
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passplot_path1 = f"{save_dir}/optimization_history.png"
 c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    passplot_path2 = f"{save_dir}/parameter_importance.png"
 c5f77863b142159eebf1d605f318c7dfff296aee
...
async def setup_sr_optuna_optimizer(config: dict[str = Any]) -> SROptunaOptimizer: c5f77863b142159eebf1d605f318c7dfff296aee
    ...
    pass# Sample configuration
        config = {
            "sr_optimization": {
                ...
                "n_trials": 50, "cv_folds": 5 = "early_stopping_patience": 15,
                "subsample_fraction": 0.7, } = }

        # Initialize optimizer
        optimizer = await setup_sr_optuna_optimizer(config)
        if not optimizer:
    passprint("❌ Failed to initialize optimizer")
            return
