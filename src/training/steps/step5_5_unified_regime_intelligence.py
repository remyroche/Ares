...
class MultiTimeframeHMMEncoder(nn.Module):
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.hmm_embeddings = nn.ModuleDict(
            {
                tf: nn.Embedding(
                    num_embeddings=self.hmm_states_per_tf,
                    embedding_dim=self.d_model // len(self.timeframes),
                )
    ...
    def forward(
        ...
        # Generate outputs
        regime_logits = self.regime_classifier(pooled) if self.regime_classifier is not None else torch.empty(0)
        intensity_logits = self.intensity_predictor(pooled) if self.intensity_predictor is not None else torch.empty(0)
...
class UnifiedRegimeIntelligenceStep:
    ...
    def _calculate_multi_timeframe_alignment(
        ...
        try:
            ...
            alignment_scores = []
            for i in range(len(next(iter(tf_intensities.values())))):
            ...
            return pd.Series(alignment_scores, index=next(iter(tf_intensities.values())).index)

        except Exception as e:
            self.logger.exception(f"🚨 Error calculating multi-timeframe alignment: {e}")
            return pd.Series(0, index=next(iter(tf_intensities.values())).index)
    ...
    def _calculate_regime_synchronization(
        ...
        try:
            ...
            sync_series = pd.Series(sync_scores, index=next(iter(tf_intensities.values())).index)
    ...
    async def _create_regime_transition_features(
        self, hmm_data: dict[str, pd.DataFrame], base_index: pd.DatetimeIndex, ) -> pd.DataFrame:
    ...
    async def _create_sequences(
        ...
        try:
            ...
            for i in range(self.sequence_length, len(base_index)):
                ...
                future_regimes = (hmm_data["1m"].iloc[i : i + 5]["composite_cluster_id"].values)
                ...
                transition_detected = self._detect_intensity_transition(
                    intensity_data=intensity_data, current_idx=i, window_start=window_start, window_end=window_end,
                )
        ...
        try:
            ...
            for cluster_id in current_intensities:
                if cluster_id in previous_intensities:
                    change = (current_intensities[cluster_id]
                        - previous_intensities[cluster_id]
                    )
        ...
        try:
            ...
            if "features" in train_data and len(train_data["features"]) > 0:
                ...
                pruned_features, pruning_metadata = pruning_manager.prune_for_step6_5_unified_regime(
                    features_df=features_df, target=target_series,
                )
    ...
    def predict(
        self, hmm_states: dict[str, np.ndarray], features: np.ndarray, ) -> dict[str, Any]:
...
async def run_step(symbol: str, exchange: str = "BINANCE", timeframe: str = "1m", training_config: dict[str, Any] | None = None, force_rerun: bool = False
    ...
    try:
        ...
        if successful_phases >= 4:  # At least 4 out of 5 phases successful
            logger.info(
                "✅ Step 5_5: Unified Regime Intelligence completed successfully",
            )
            logger.info(f"   Success rate: {successful_phases/total_phases*100:.1f}%")
        else:
            logger.error("❌ Step 5_5: Unified Regime Intelligence failed")
