...
class OptimizationManager:
    ...
    @handles_errors(exceptions=(Exception,), default_return={'best_params': {}, 'best_score': -np.inf}, context='hyperparameter optimization')
    async def optimize_hyperparameters(self, objective_function: Callable, param_space: Dict[str, Any], market_data: pd.DataFrame, n_trials: Optional[int]=None) -> Dict[str, Any]:
        ...
        try:
            return await self._optimize_with_optuna(objective_function, param_space, market_data, n_trials)
        except ImportError:
            self.logger.warning('Optuna not available, using grid search')
            return await self._optimize_with_grid_search(objective_function, param_space, market_data, n_trials)

    async def _optimize_with_optuna(self, objective_function: Callable, param_space: Dict[str, Any], market_data: pd.DataFrame, n_trials: int) -> Dict[str, Any]:
        """Optimize using Optuna."""
        import optuna
        study = optuna.create_study(direction='maximize', pruner=optuna.pruners.MedianPruner() if self.enable_early_stopping else None)

        def optuna_objective(trial: Any) -> None:
            params = {}
            ...
            return score
        study.optimize(optuna_objective, n_trials=n_trials, n_jobs=self.max_workers if self.enable_parallelization else 1)
        best_trial = study.best_trial
        return {'best_params': best_trial.params, 'best_score': best_trial.value, 'n_trials': len(study.trials), 'optimization_history': [{'params': t.params, 'score': t.value} for t in study.trials if t.value is not None]}

    async def _optimize_with_grid_search(self, objective_function: Callable, param_space: Dict[str, Any], market_data: pd.DataFrame, n_trials: int) -> Dict[str, Any]:
        ...
        return {'best_params': best_params, 'best_score': best_score, 'n_trials': len(history), 'optimization_history': history}

    def _generate_param_grid(self, param_space: Dict[str, Any], max_combinations: int) -> List[Dict[str, Any]]:
        """Generate parameter grid for search."""
        import itertools
        param_lists = {}
        for param_name, param_config in param_space.items():
            if param_config['type'] == 'int':
                param_lists[param_name] = np.linspace(param_config['low'], param_config['high'], min(5, param_config.get('n_samples', 5)), dtype=int).tolist()
            elif param_config['type'] == 'float':
                param_lists[param_name] = np.linspace(param_config['low'], param_config['high'], min(5, param_config.get('n_samples', 5))).tolist()
            elif param_config['type'] == 'categorical':
                param_lists[param_name] = param_config['choices']
        all_combinations = list(itertools.product(*param_lists.values()))
        param_names = list(param_lists.keys())
        param_grid = [dict(zip(param_names, combo)) for combo in all_combinations]
    ...
    @handles_errors(exceptions=(Exception,), default_return={'sharpe': 0, 'returns': 0, 'max_drawdown': 0}, context='backtesting')
    async def backtest_strategy(self, strategy_params: Dict[str, Any], market_data: pd.DataFrame, initial_capital: float=10000) -> Dict[str, float]:
        ...
        results = {'total_return': total_return, 'sharpe_ratio': sharpe_ratio, 'max_drawdown': max_drawdown, 'win_rate': (portfolio_returns > 0).mean(), 'avg_return': portfolio_returns.mean(), 'volatility': portfolio_returns.std()}
        self.result_cache[cache_key] = results
        return results

    def _generate_signals(self, market_data: pd.DataFrame, strategy_params: Dict[str, Any]) -> pd.Series:
    ...
    async def optimize_portfolio_weights(self, returns_matrix: pd.DataFrame, optimization_method: str='sharpe') -> Dict[str, float]:
        ...
        if optimization_method == 'equal_weight':
            ...
            def neg_sharpe(weights: Union[List[float], np.ndarray]) -> None:
            ...
            def portfolio_variance(weights: Union[List[float], np.ndarray]) -> None:
            ...
            weights = np.ones(n_assets) / n_assets
        weight_dict = {asset: weight for asset, weight in zip(returns_matrix.columns, weights)}
        return weight_dict
