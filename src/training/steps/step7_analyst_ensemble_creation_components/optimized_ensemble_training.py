...
def train_and_evaluate_worker_ray(
    ...
    X_val_ref: ray.ObjectRef | None = None,
    y_val_ref: ray.ObjectRef | None = None,
    ...
    X_val, y_val = (
        (ray.get(X_val_ref), ray.get(y_val_ref)) if X_val_ref else (None, None)
    )
    ...
    def _cached_train(model_instance, X_tr, y_tr, X_va, y_va):
        try:
            if (
                model_name in ["lightgbm", "xgboost"]
            ...
            score = (
                accuracy_score(y_va, model_instance.predict(X_va))
                if X_va is not None
                else None
            )
            return {
...
class HighPerformanceEnsembleManager:
    ...
    def __init__(self):
        ...
        if not ray.is_initialized():
            ...
            self.logger.info(
                f"Ray initialized successfully. Dashboard URL: {ray.dashboard_url}",
            )
    ...
    @handle_errors(
        ...
        default_return=[],
        context="high performance ensemble manager base model training",
    )
    def train_base_models(
        ...
        """
        self.logger.info(
            f"Starting parallel training for {len(model_configs)} models using Ray...",
        )
        ...
        tasks = [
            train_and_evaluate_worker_ray.remote(
                config,
                X_train_ref,
                y_train_ref,
                X_val_ref,
        ...
        for res in results:
            if res["error"]:
                self.logger.error(
                    f"Training failed for {res['model_name']}: {res['error']}",
                )
            else:
                self.logger.info(
                    f"Successfully trained {res['model_name']}. Validation Accuracy: {res['validation_accuracy']:.4f}",
                )
    ...
    @handle_errors(
        exceptions=(Exception,),
        default_return=None,
        context="high performance ensemble manager stacking ensemble creation",
    )
    def create_stacking_ensemble(
        ...
        estimators = [
            (res["model_name"], res["model"]) for res in trained_models if res["model"]
        ]

        if len(estimators) < 2:
            self.logger.warning(
                "Cannot create an ensemble with fewer than 2 valid base models.",
            )
            return None
        ...
        cv = TimeSeriesSplit(n_splits=cv_folds, test_size=int(len(X_train) * 0.2))
        self.logger.info("✅ Using TimeSeriesSplit for leak-proof cross-validation")
...
if __name__ == "__main__":
    ...
    X, y = (
        pd.DataFrame(np.random.randn(2000, 25)),
    ...
    try:
        ...
        base_models = manager.train_base_models(
            MODEL_CONFIGS,
            X_train,
            y_train,
            X_val,
        ...
        start_time = time.time()
        cached_models = manager.train_base_models(
            MODEL_CONFIGS,
            X_train,
            y_train,
            X_val,
        ...
        if base_models:
            stacking_model = manager.create_stacking_ensemble(
                base_models,
                X_train,
                y_train,
            )
