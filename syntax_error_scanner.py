...
class SyntaxErrorScanner:
    ...
    def scan_file(self, file_path: str) -> list[str]:
        ...
        try:
            ...
            logger.exception(f"Error scanning {file_path}: {e}")
    ...
    def parse_error_line(self, error_line: str) -> tuple[str, str, str]:
        ...
        if ":" in error_line:
            ...
            error_message = parts[1].strip() if len(parts) >= 2 else error_line
        else:
            error_message = error_line

        # Extract file info
        file_match = re.search(r'File "([^"]+)"', error_line)
        file_info = file_match.group(1) if file_match else "Unknown file"
...
def main():
    ...
    scanner.generate_report("syntax_error_report.txt")
