...
class EnhancedScenarioBasedPredictor:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.decision_thresholds = {'profit_zone_combined': scenario_config.get('profit_zone_combined_threshold', 0.6), 'risk_zone_combined': scenario_config.get('risk_zone_combined_threshold', 0.2), 'exit_risk_threshold': scenario_config.get('exit_risk_threshold', 0.5), 'neutral_threshold': scenario_config.get('neutral_threshold', 0.3), 'confidence_threshold': scenario_config.get('confidence_threshold', 0.7), 'profit_risk_ratio': scenario_config.get('profit_risk_ratio_threshold', 2.0), 'scenario_dominance': scenario_config.get('scenario_dominance_threshold', 0.4)}
        self.technical_indicators = {'RSI': {'lookback_period': scenario_config.get('rsi_lookback_period', 14), 'overbought_threshold': scenario_config.get('rsi_overbought_threshold', 70), 'oversold_threshold': scenario_config.get('rsi_oversold_threshold', 30)}, 'MACD': {'fast_period': scenario_config.get('macd_fast_period', 12), 'slow_period': scenario_config.get('macd_slow_period', 26), 'signal_period': scenario_config.get('macd_signal_period', 9)}, 'Bollinger_Bands': {'lookback_period': scenario_config.get('bb_lookback_period', 20), 'std_dev': scenario_config.get('bb_std_dev', 2.0), 'squeeze_threshold': scenario_config.get('bb_squeeze_threshold', 0.2)}, 'SMA': {'short_period': scenario_config.get('sma_short_period', 10), 'long_period': scenario_config.get('sma_long_period', 30)}, 'EMA': {'short_period': scenario_config.get('ema_short_period', 10), 'long_period': scenario_config.get('ema_long_period', 30)}, 'ATR': {'lookback_period': scenario_config.get('atr_lookback_period', 14)}, 'Stochastic': {'k_period': scenario_config.get('stoch_k_period', 14), 'd_period': scenario_config.get('stoch_d_period', 3), 'overbought': scenario_config.get('stoch_overbought', 80), 'oversold': scenario_config.get('stoch_oversold', 20)}, 'ADX': {'lookback_period': scenario_config.get('adx_lookback_period', 14), 'threshold': scenario_config.get('adx_threshold', 25)}, 'CCI': {'lookback_period': scenario_config.get('cci_lookback_period', 14), 'constant': scenario_config.get('cci_constant', 0.015)}}
    ...
    @handles_errors
    def prepare_scenario_targets(self, X: np.ndarray, market_data: pd.DataFrame, base_price_column: str='close') -> np.ndarray:
        """
        ...
        try:
            ...
            for i in range(len(X)):
                scenario = self._determine_first_scenario(prices[i:], i, self.time_limit_minutes)
                scenario_labels.append(scenario)
            return np.array(scenario_labels)
        except Exception as e:
            self.logger.exception(f'❌ Scenario labeling failed: {e}')
            return np.full(len(X), len(self.scenarios) - 1)

    def _determine_first_scenario(self, future_prices: np.ndarray, current_index: int, time_limit: int) -> int:
        """
        ...
        try:
            ...
            for scenario_id in range(len(self.scenarios) - 1):
                scenario = self.scenarios[scenario_id]
                if self._scenario_triggered(look_ahead_prices, current_price, scenario):
                    return scenario_id
            return len(self.scenarios) - 1
        except Exception as e:
    ...
    def _scenario_triggered(self, prices: np.ndarray, current_price: float, scenario: dict[str, Any]) -> bool:
        """
    ...
    def _analyze_enhanced_scenario_probabilities(self, probabilities: np.ndarray) -> dict[str, Any]:
        ...
        try:
            ...
            risk_reward_ratio = profit_zone_prob / (risk_zone_prob + 1e-08)
            profit_risk_difference = profit_zone_prob - risk_zone_prob
            max_prob = max(probabilities)
            scenario_dominance = max_prob / (sum(probabilities) + 1e-08)
            zone_distribution = {'profit_zones': len(profit_zone_probs), 'risk_zones': len(risk_zone_probs), 'profit_probabilities': profit_zone_probs, 'risk_probabilities': risk_zone_probs}
            return {'profit_zone_probability': profit_zone_prob, 'risk_zone_probability': risk_zone_prob, 'neutral_probability': neutral_prob, 'dominant_zone': dominant_zone, 'risk_reward_ratio': risk_reward_ratio, 'profit_risk_difference': profit_risk_difference, 'scenario_dominance': scenario_dominance, 'zone_distribution': zone_distribution, 'max_probability': max_prob, 'probability_entropy': -np.sum(probabilities * np.log(probabilities + 1e-08))}
        except Exception as e:
            self.logger.exception(f'❌ Enhanced scenario analysis failed: {e}')
            return {'profit_zone_probability': 0.0, 'risk_zone_probability': 0.0, 'neutral_probability': 1.0, 'dominant_zone': 'neutral', 'risk_reward_ratio': 0.0, 'profit_risk_difference': 0.0, 'scenario_dominance': 0.0, 'zone_distribution': {'profit_zones': 0, 'risk_zones': 0, 'profit_probabilities': [], 'risk_probabilities': []}, 'max_probability': 0.0, 'probability_entropy': 0.0}
    ...
    def _calculate_enhanced_confidence(self, probabilities: np.ndarray) -> float:
        ...
        """
        try:
            entropy = -np.sum(probabilities * np.log(probabilities + 1e-08))
    ...
    def get_enhanced_configuration_summary(self) -> dict[str, Any]:
        ...
        """
        return {'scenarios': self.scenarios, 'time_limit_minutes': self.time_limit_minutes, 'model_config': self.model_config, 'decision_thresholds': self.decision_thresholds, 'technical_indicators': self.technical_indicators, 'feature_config': self.feature_config, 'is_trained': self.is_trained, 'model_performance': self.model_performance, 'feature_importance': self.feature_importance, 'n_scenarios': len(self.scenarios)}