...
async def run_optimisation_pipeline(symbol, exchange, timeframe, data_dir, **config):
    ...
    def log_calibration_outcomes(calibration_results: dict):
    ...
    def log_optimization_outcomes(optimization_results: dict):
    ...
    async def execute_optimisation_step(step_name: str, step_func, *args, **kwargs):
        ...
        global step_count, error_count, warning_count
            ...
            if result:
                ...
                if step_name == "Confidence Calibration" and isinstance(result, dict):
                    ...
                    if calibration_results:
                        log_calibration_outcomes(calibration_results)
                    ...
                    if optimization_results:
                        log_optimization_outcomes(optimization_results)
    ...
    try:
        ...
        if config.get('confidence_calibration', True):
            ...
            try:
                ...
                confidence_calibrator = PerRegimeConfidenceCalibrationStep(config)
        ...
        if config.get('parameter_optimization', True):
            ...
            try:
                ...
                param_optimizer = FinalParametersOptimizationStepNew(config)
        ...
        if config.get('post_validation', True):
            ...
            # load_and_analyze_saved_results(symbol, exchange, data_dir)  # Function moved inside async context
        ...
        if config.get('save_summary', True):
            ...
            # Save summary using centralized reporting system
            summary_data = {
                'symbol': symbol,
                'exchange': exchange,
                'timeframe': timeframe,
