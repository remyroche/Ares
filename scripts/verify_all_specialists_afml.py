...
async def run_verification():
    ...
    config = {
        ...
        'timeframe': '15m',
        'direction': 'long',
        'is_batch_run': True,  # Avoid saving intermediate artifacts
    ...
    tprint_info(f"🚀 Starting Final Verification of 12 AFML-Hardened Specialists...")
    ...
    for step_class, name in specialists:
        ...
        try:
            ...
            if output.get('success'):
                ...
                tprint_success(f"✅ {name} completed: AUC={metrics.get('auc', 0):.3f}, MI={metrics.get('mi_score', 0):.4f}")
            else:
    ...
    print("\n" + "="*50)
    print("FINAL SPECIALIST VERIFICATION REPORT")
    print("="*50)
    print(df_results.to_string(index=False))
    print("="*50)
    
    # Save report
    df_results.to_csv("artifacts/final_specialist_verification_report.csv", index=False)
