...
class PaperTradingReporter:
    ...
    async def record_trade(
        ...
        try:
            ...
            ml_confidence_obj = MLConfidenceScores(
                ...
                meta_learner_confidence=ml_confidence.get(
                    "meta_learner_confidence",
                    0.0,
                ),
                individual_model_confidences=ml_confidence.get(
    ...
    async def _update_performance_metrics(
        ...
        try:
            ...
            closed_trades = [
                t for t in self.trade_records if t.trade_status == "closed"
            ]
            ...
            if closed_trades:
                ...
                total_proceeds = sum(
                    t.pnl_analysis.total_proceeds for t in closed_trades
                )

                # Win rate
                profitable_trades = [
                    t for t in closed_trades if t.pnl_analysis.net_pnl > 0
                ]
    ...
    def _calculate_sharpe_ratio(self) -> float:
        ...
        try:
            ...
            for i in range(1, len(self.trade_records)):
                ...
                curr_pnl = sum(
                    t.pnl_analysis.net_pnl for t in self.trade_records[: i + 1]
                )
    ...
    async def update_trade(
        ...
        try:
            ...
            if "close_timestamp" in update_data:
                trade_record.close_timestamp = datetime.fromisoformat(
                    update_data["close_timestamp"],
                )
            ...
            if "pnl_analysis" in update_data:
                ...
                trade_record.pnl_analysis.absolute_pnl = pnl_data.get(
                    "absolute_pnl",
                    0.0,
                )
                trade_record.pnl_analysis.percentage_pnl = pnl_data.get(
                    "percentage_pnl",
                    0.0,
                )
                trade_record.pnl_analysis.realized_pnl = pnl_data.get(
                    "realized_pnl",
                    0.0,
                )
    ...
    def get_trade(self, trade_id: str) -> DetailedTradeRecord | None:
    ...
    def get_trade_history(
        ...
        try:
            ...
            if start_time:
                filtered_records = [
                    t for t in filtered_records if t.timestamp >= start_time
                ]

            if end_time:
                filtered_records = [
                    t for t in filtered_records if t.timestamp <= end_time
                ]
    ...
    def get_portfolio_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            total_value = sum(
                t.position_sizing.absolute_size
            ...
            for trade in self.trade_records:
                if trade.trade_status == "open":
                    ...
                    symbol_positions[symbol]["quantity"] += (
                        trade.position_sizing.absolute_size
                    )
                    symbol_positions[symbol]["value"] += (
                        trade.position_sizing.absolute_size
                        * trade.pnl_analysis.total_cost
                    )
    ...
    async def generate_detailed_report(
        ...
        try:
            ...
            self.print(error("Error generating report: {e}"))
    ...
    async def _generate_analysis(self) -> dict[str, Any]:
        ...
        try:
            ...
            for trade in self.trade_records:
                ...
                trade_types[trade_type_key]["avg_confidence"] += (
                    trade.ml_confidence_scores.ensemble_confidence
                )
            ...
            confidence_scores = [
                t.ml_confidence_scores.ensemble_confidence for t in self.trade_records
            ]
    ...
    def _calculate_distribution(self, values: list[float]) -> dict[str, int]:
        ...
        try:
            ...
            for i in range(10):
                ...
                distribution[bin_key] = len(
                    [v for v in values if bin_start <= v < bin_end],
                )
        ...
        try:
            ...
            for trade in report_data.get("trade_records", []):
                record = {
                    ...
                    "absolute_size": trade.get("position_sizing", {}).get(
                        "absolute_size",
                    ),
                    "portfolio_percentage": trade.get("position_sizing", {}).get(
                        "portfolio_percentage",
                    ),
                    "absolute_pnl": trade.get("pnl_analysis", {}).get("absolute_pnl"),
                    "percentage_pnl": trade.get("pnl_analysis", {}).get(
                        "percentage_pnl",
                    ),
            ...
            self.print(error(f"Error converting to DataFrame: {e}"))
            return pd.DataFrame()
