...
def prepare_market_features(
    ...
    try:
        ...
        try:
            from .balanced_feature_extractor import extract_balanced_features, create_unified_config
            
            # Create unified configuration for comprehensive feature extraction
            balanced_config = create_unified_config()
            
            # Map feature categories to balanced extractor categories
            category_mapping = {
                'oscillator': 'technical',
                'momentum': 'momentum', 
                'trend': 'trend',
                'price': 'price',
                'volume': 'volume',
            ...
            # Enable only the categories we need
            enabled_categories = []
            for category in feature_config.feature_categories:
                if category in category_mapping:
                    mapped_category = category_mapping[category]
                    if mapped_category not in enabled_categories:
                        enabled_categories.append(mapped_category)
            
            # Update config to use only our categories
            from .balanced_feature_extractor import FeatureCategory
            balanced_config.enabled_categories = [
                getattr(FeatureCategory, cat.upper()) for cat in enabled_categories
            ]
            
            if verbose:
                tprint_debug(f"📊 [SHARED_FEATURES] Extracting features for categories: {enabled_categories}")
            
            # Extract balanced features
            result = extract_balanced_features(
                data=market_data,
                config=balanced_config,
                labels=None
            )
            
            if result.features is None or result.features.size == 0:
                if verbose:
                    tprint_warning("⚠️ [SHARED_FEATURES] No features generated by balanced extractor, falling back to basic features")
                ...
                features_df = pd.DataFrame(
                    ...
                    index=market_data.index[:len(result.features)]
                )
