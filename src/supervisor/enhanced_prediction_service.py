...
def _safe_load_model(filepath: Path, logger: logging.Logger) -> Any:
    ...
    try:
        ...
        expected_dirs = [Path('models').resolve(), Path('src/models').resolve(), Path('/app/models').resolve()]
...
class EnhancedPredictionService:
    ...
    async def _load_analyst_ml_models(self) -> None:
        ...
        try:
            ...
            analyst_model_types = ['hmm_profit', 'analyst_profit', 'calibrated', 'optimized', 'validated', 'monte_carlo', 'ab_tested']
    ...
    async def _load_tactician_ml_models(self) -> None:
        ...
        try:
            ...
            tactician_model_types = ['tactician_profit', 'tactician_specialist', 'calibrated', 'optimized', 'validated', 'monte_carlo', 'ab_tested']
...
from src.core.decorators.errors import handles_errors
    ...
    @performance_monitor
    async def get_calibrated_confidence_scores(self, market_data: pd.DataFrame, regime_info: Dict[str, Any], symbol: str, exchange: str) -> Dict[str, Dict[str, float]]:
        """
        ...
        try:
            ...
            tactician_scores = await self._get_tactician_calibrated_confidence(market_data, regime_info, symbol, exchange)
    ...
    async def _get_analyst_calibrated_confidence(self, market_data: pd.DataFrame, regime_info: Dict[str, Any], symbol: str, exchange: str) -> Dict[str, float]:
            ...
            for model_type, models in self.analyst_ml_models.items():
                for model_name, model_data in models.items():
                    try:
                        ...
                        calibrated_confidence = self._calculate_step11_calibrated_confidence(price_action_probabilities, model_calibration, model_name, 'analyst')
    ...
    async def _get_tactician_calibrated_confidence(self, market_data: pd.DataFrame, regime_info: Dict[str, Any], symbol: str, exchange: str) -> Dict[str, float]:
            ...
            for model_type, models in self.tactician_ml_models.items():
                for model_name, model_data in models.items():
                    try:
                        ...
                        calibration_data = self.calibration_results.get(calibration_key, {})
                        model_calibration = calibration_data.get('model_calibrations', {}).get(f'{model_type}_{model_name}', {})
                        calibrated_confidence = self._calculate_step11_calibrated_confidence(price_action_probabilities, model_calibration, model_name, 'tactician')
                        if calibrated_confidence is not None:
    ...
    def _calculate_step11_calibrated_confidence(self, price_action_probabilities: Dict[str, Any], model_calibration: Dict[str, Any], model_name: str, model_type: str) -> Optional[float]:
        """
        ...
        try:
            ...
            base_confidence = triple_barrier_prob * triple_barrier_weight + direction_prob * direction_weight + magnitude_prob * magnitude_weight + barrier_avoidance_prob * barrier_avoidance_weight
            ...
            self.logger.debug(f'Step 11 calibration for {model_name}: base={base_confidence:.3f}, reliability={reliability_score:.3f}, performance={performance_ratio:.3f}, weights={optimized_weights}, final={final_confidence:.3f}')
    ...
    def _validate_price_action_probabilities(self, price_action_probabilities: Dict[str, Any], model_name: str) -> bool:
        ...
        """
        try:
            required_probabilities = ['triple_barrier_probability', 'direction_probability', 'magnitude_probability', 'barrier_avoidance_probability']
            ...
            if magnitude < 0.3 or magnitude > 0.9:
                self.logger.warning(warning(f'⚠️ Magnitude probability ({magnitude}) outside reasonable range [0.3, 0.9] for model {model_name}'))
                return False
    ...
    async def verify_all_models_probability_outputs(self) -> Dict[str, Any]:
        ...
        """
        try:
            verification_results = {'analyst_models': {}, 'tactician_models': {}, 'summary': {'total_analyst_models': 0, 'total_tactician_models': 0, 'analyst_models_with_probabilities': 0, 'tactician_models_with_probabilities': 0, 'all_models_verified': False}}
            for model_type, models in self.analyst_ml_models.items():
                ...
                for model_name, model_data in models.items():
                    ...
                    verification_results['analyst_models'][model_type][model_name] = {'has_probability_outputs': has_probabilities, 'probability_keys': list(model_data.get('price_action_probabilities', {}).keys()) if has_probabilities else []}
            ...
            for model_type, models in self.tactician_ml_models.items():
                ...
                for model_name, model_data in models.items():
                    has_probabilities = self._verify_model_probability_outputs(model_data, f'{model_type}_{model_name}')
                    verification_results['tactician_models'][model_type][model_name] = {'has_probability_outputs': has_probabilities, 'probability_keys': list(model_data.get('price_action_probabilities', {}).keys()) if has_probabilities else []}
    ...
    def get_service_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'service_name': 'Enhanced Prediction Service', 'is_initialized': self.is_initialized, 'analyst_models_loaded': analyst_model_count, 'tactician_models_loaded': tactician_model_count, 'analyst_model_types': list(self.analyst_ml_models.keys()), 'tactician_model_types': list(self.tactician_ml_models.keys()), 'calibration_results_loaded': len(self.calibration_results), 'optimization_results_loaded': len(self.optimization_results), 'entry_threshold': self.entry_threshold, 'max_confidence_threshold': self.max_confidence_threshold, 'probability_requirements': self._get_probability_requirements_info()}
        except Exception as e:
    ...
    def _get_probability_requirements_info(self) -> Dict[str, Any]:
        ...
        return {'probability_outputs_location': 'ML models in steps 6-14 of enhanced_training_manager', 'required_probabilities': {'triple_barrier_probability': 'Probability of reaching profit target without hitting stop-loss', 'direction_probability': 'Probability of price moving in predicted direction', 'magnitude_probability': 'Probability of price moving by expected magnitude', 'barrier_avoidance_probability': 'Probability of avoiding adverse price movements'}, 'model_structure': {'expected_format': 'price_action_probabilities dict in model_data', 'example': {'triple_barrier_probability': 0.75, 'direction_probability': 0.8, 'magnitude_probability': 0.65, 'barrier_avoidance_probability': 0.7}}, 'calibration_requirements': {'step11_calibration': 'Model performance vs actual reliability data', 'step12_optimization': 'Optimized weights for probability components', 'calibration_data_structure': {'reliability_score': 'Historical reliability of model predictions', 'performance_ratio': 'Actual vs expected performance ratio', 'calibration_factor': 'Overall calibration adjustment', 'confidence_bias': 'Systematic bias adjustment', 'step12_optimized_weights': {'triple_barrier_weight': 'Optimized weight for triple barrier probability', 'direction_weight': 'Optimized weight for direction probability', 'magnitude_weight': 'Optimized weight for magnitude probability', 'barrier_avoidance_weight': 'Optimized weight for barrier avoidance probability'}}}}