    ...
    def required_input_features(self) -> list[str]:
        ...
        if self.residual_representation_state:
            ...
            representation_outputs.update(
                str(name)
                for name in self.residual_representation_state.get(
                    "output_columns", []
                )
            )
        for model in list(self.residual_recognizer.side_models.values()) + list(
            ...
            # PCA outputs are generated by _apply_residual_representation before
            # the recognizer runs. They are not observable feature-store inputs.
            required.extend(
                name
        ...
        injected = {
            str(self.residual_recognizer.config.side_col),
            str(self.residual_recognizer.config.archetype_col),
        }
        return list(
            dict.fromkeys(
                str(name)
                for name in required
                if str(name) and str(name) not in injected
            )
        )
