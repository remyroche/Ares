...
from src.utils.warning_symbols import warning, error, critical
import re
...
class LookaheadBiasDetector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.warning_threshold = self.config.get("warning_threshold", 50)  # Max suspicious features before warning
        ...
        try:
            ...
            if timestamp_col and timestamp_col in features_df.columns:
                self._check_temporal_alignment(
                    features_df, target_series, timestamp_col, results
                )
            ...
            if feature_engineering_code:
                self._analyze_implementation(feature_engineering_code, features_df, results)
            ...
            self.logger.error(f"Error in lookahead bias detection: {e}")
            results["error"] = str(e)
            return results
    ...
    def _check_temporal_alignment(
        ...
        try:
            ...
            # Check if features and target have same timestamps
    ...
    def _check_rolling_feature_timing(
        self, features_df: pd.DataFrame, timestamps: pd.Series, results: Dict[str, Any]
    ) -> None:
    ...
    def _check_rolling_window_issues(
        ...
        # Legitimate lagging indicators that suggest proper implementation
        ...
        inherently_lagged_patterns = [
            "slope", "returns", "change", "diff", "momentum", "acceleration"
        ]
        
        # Features that should be investigated but may be legitimate
        suspicious_features = []
        potentially_legitimate_features = []
        
        # Enhanced legitimate patterns for common technical indicators
        enhanced_legitimate_patterns = [
            "lag", "shift", "prev", "diff", "change", "slope", "returns", "pct_change",
            "impact", "correlation", "spread", "ratio", "zscore", "upper", "lower",
            "momentum", "acceleration", "volatility", "std", "atr", "rsi", "macd",
            "bb_", "sma_", "ema_", "vwap", "obv", "mfi", "cci", "williams", "stoch"
        ]
        ...
        for col in features_df.columns:
            ...
            is_common_technical_indicator = any([
                "_" in col_lower and col_lower.split("_")[0] in ["sma", "ema", "bb", "rsi", "macd", "atr", "cci", "mfi"],
                any(pattern in col_lower for pattern in ["spread", "ratio", "zscore", "impact", "correlation"]),
        ...
        if potentially_legitimate_features:
            ...
            self.logger.info(f"✅ Found {len(potentially_legitimate_features)} features with legitimate lagging patterns")
    ...
    def _identify_lagging_type(self, feature_name: str) -> str:
        ...
        elif "returns" in feature_lower or "pct_change" in feature_lower:
    ...
    def _generate_recommendations(self, results: Dict[str, Any]) -> None:
        ...
        if "implementation_analysis" in results:
            ...
            if "potentially_problematic_features" in impl_analysis:
                problematic_count = len(impl_analysis["potentially_problematic_features"])
        ...
        if results["feature_correlations"]:
            high_corr_features = [
                feat for feat, corr in results["feature_correlations"].items()
                if abs(corr) > 0.8
            ]
        ...
        if not results["critical_issues"] and not results["warnings"]:
            ...
            recommendations.append("🔍 Continue monitoring with enhanced detection system")
        elif results["warnings"] and not results["critical_issues"]:
            recommendations.append("⚠️ Minor issues detected - review and address as needed")
            recommendations.append("📈 Consider implementing automated lagging validation")
    ...
    def validate_train_test_split(
        ...
        results = {"split_valid": True, "issues": [], "recommendations": []}
    ...
    def add_lagging_to_features(
        self, features_df: pd.DataFrame, lag_periods: int = 1
    ) -> pd.DataFrame:
        """
    ...
    def _analyze_implementation(
        ...
        try:
            ...
            for pattern_name, pattern in lagging_patterns.items():
                ...
                if matches:
                    implementation_analysis["lagging_patterns_found"].append({
                        ...
                        "matches": matches,
                        "lag_periods": [int(m) if m.isdigit() else 1 for m in matches]
                    })
            ...
            self.logger.warning(f"Could not analyze implementation: {e}")
            results["implementation_analysis"] = {"error": str(e)}
    ...
    def _check_feature_lagging_in_code(
        ...
        Returns:
            String describing the lagging type if found, None otherwise
        """
        feature_lower = feature_name.lower()
        ...
        feature_patterns = [
            rf'features\[["\']{re.escape(feature_name)}["\']\]',
    ...
    def _extract_base_feature_name(self, feature_name: str) -> str:
        ...
        suffixes = [
            r'_diff_\d+', r'_diff', r'_change', r'_returns', r'_slope',
            r'_lag_\d+', r'_lag', r'_shift_\d+', r'_shift',
            r'_pct_change', r'_momentum', r'_acceleration'
        ]
    ...
    def _is_base_feature(self, feature_name: str) -> bool:
        ...
        base_features = [
            ...
            'ema_', 'sma_', 'rsi', 'macd', 'bb_', 'atr', 'stoch_',
            'funding_rate', 'bid_ask_spread', 'market_depth'
        ]
...
def detect_lookahead_bias(
    ...
    return detector.detect_feature_lookahead_bias(
        features_df, target_series, timestamp_col
    )
...
def validate_temporal_split(
    ...
    return detector.validate_train_test_split(
        X_train, X_test, y_train, y_test, timestamp_col
    )


def apply_feature_lagging(
    features_df: pd.DataFrame, lag_periods: int = 1
) -> pd.DataFrame:
    """
