...
class PerformanceOptimizer:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.performance_window = self.optimizer_config.get("performance_window", 24)  # 24 hours
        self.min_trades_for_optimization = self.optimizer_config.get("min_trades_for_optimization", 10)
    ...
    async def _should_optimize(self, trading_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if len(self.performance_history) >= 5:
                recent_sharpe = np.mean([h["metrics"]["sharpe_ratio"] for h in self.performance_history[-5:]])
                ...
                if current_sharpe < recent_sharpe * 0.9:
                    self.logger.info(f"📉 Performance degradation detected: {current_sharpe:.3f} vs {recent_sharpe:.3f}")
                    return True
            ...
            last_optimization = datetime.fromisoformat(self.optimization_results[-1]["timestamp"])
    ...
    async def _detect_regime_change(self) -> bool:
        ...
        try:
            ...
            recent_volatility = np.mean([h["metrics"]["volatility"] for h in self.performance_history[-5:]])
            historical_volatility = np.mean([h["metrics"]["volatility"] for h in self.performance_history[-10:-5]])
            ...
            recent_returns = np.mean([h["metrics"]["total_return"] for h in self.performance_history[-5:]])
            historical_returns = np.mean([h["metrics"]["total_return"] for h in self.performance_history[-10:-5]])
            
            if (recent_returns > 0 and historical_returns < 0) or (recent_returns < 0 and historical_returns > 0):
                return True
    ...
    def _calculate_calmar_ratio(self, returns: List[float]) -> float:
        ...
        try:
            ...
            max_dd = self._calculate_max_drawdown([1 + sum(returns[:i+1]) for i in range(len(returns))])
    ...
    def _calculate_performance_trend(self) -> str:
        ...
        try:
            ...
            recent_sharpe = np.mean([h["metrics"]["sharpe_ratio"] for h in self.performance_history[-5:]])
            historical_sharpe = np.mean([h["metrics"]["sharpe_ratio"] for h in self.performance_history[-10:-5]])
            
            if recent_sharpe > historical_sharpe * 1.1:
                return "improving"
            elif recent_sharpe < historical_sharpe * 0.9:
                return "declining"
            else:
                return "stable"
        except Exception:
...
class MarketRegimeDetector:
    ...
    def _classify_regime(self, volatility: float, trend: str, volume_profile: str) -> str:
        """Classify market regime."""
        if trend == "bullish" and volume_profile == "high":
            return "bull_high_vol" if volatility > 0.02 else "bull_low_vol"
        elif trend == "bearish" and volume_profile == "high":
...
class ParameterOptimizer:
    ...
    async def optimize_parameters(self, trading_data: Dict[str, Any], regime_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            def objective(trial):
                params = {
                    "entry_confidence_threshold": trial.suggest_float("entry_confidence_threshold", 0.5, 0.9),
                    "stop_loss_threshold": trial.suggest_float("stop_loss_threshold", 0.01, 0.05),
                    "take_profit_threshold": trial.suggest_float("take_profit_threshold", 0.02, 0.08),
                ...
                performance = self._simulate_performance(trading_data, params, regime_info)
                return performance
    ...
    def _simulate_performance(self, trading_data: Dict[str, Any], params: Dict[str, Any], regime_info: Dict[str, Any]) -> float:
...
class StrategyAdapter:
    ...
    async def adapt_strategy(self, optimization_result: Dict[str, Any], regime_info: Dict[str, Any]) -> Dict[str, Any]:
        """Adapt trading strategy based on optimization results."""
        try:
            ...
            adaptations = {
                ...
                "risk_management_updates": self._get_risk_management_updates(optimization_result),
                "position_sizing_updates": self._get_position_sizing_updates(optimization_result)
            }
    ...
    def _get_risk_management_updates(self, optimization_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _get_position_sizing_updates(self, optimization_result: Dict[str, Any]) -> Dict[str, Any]:
