#!/usr/bin/env python3
"""
Standalone test for LightGBM + Featuretools + ALE generator
"""
...
def test_lightgbm_import():
    """Test importing the LightGBM generator directly."""
    print("🧪 Testing LightGBM generator import...")
    
    try:
        # Import directly without going through the pipeline
        sys.path.append(str(Path(__file__).parent / "src" / "training" / "steps" / "pre_training" / "unified_data_driven_pipeline" / "enhanced_components"))
        
        from lightgbm_featuretools_generator import (
...
def test_lightgbm_generator():
    """Test the LightGBM generator functionality."""
    print("\n🧪 Testing LightGBM generator functionality...")
...
def test_catboost_generator():
    """Test the CatBoost generator functionality."""
    print("\n🧪 Testing CatBoost generator functionality...")
    ...
    try:
        ...
        # Configure generator for CatBoost
        config = LightGBMFeatureToolsConfig(
            model_type='catboost',
            max_features_to_select=30,  # Limit for testing
        ...
        print(f"✅ CatBoost test completed:")
        print(f"   - Features generated: {result.n_features_generated}")
        print(f"   - Features selected: {result.n_features_selected}")
        print(f"   - Generation time: {result.generation_time:.3f}s")
...
def main():
    """Main test function."""
    print("🚀 Testing LightGBM + Featuretools + ALE Generator (Standalone)")
    print("=" * 70)
    
    try:
        # Test 1: Import test
        success1 = test_lightgbm_import()
        
        if not success1:
            print("\n❌ Import test failed - cannot proceed")
            return 1
        
        # Test 2: LightGBM functionality
        success2 = test_lightgbm_generator()
        
        # Test 3: CatBoost functionality
        success3 = test_catboost_generator()
        ...
        if success1 and success2 and success3:
            ...
            print("   - Implementation is FULLY COMPLETE")
        else:
            print("\n❌ Some tests failed")
            return 1
