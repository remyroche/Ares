...
class MLTargetValidator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.max_reasonable_tp_multiplier = self.config.get(
            "max_reasonable_tp_multiplier", 10.0
        )
        self.min_reasonable_tp_multiplier = self.config.get(
            "min_reasonable_tp_multiplier", 0.1
        )
        self.max_reasonable_sl_multiplier = self.config.get(
            "max_reasonable_sl_multiplier", 5.0
        )
        self.min_reasonable_sl_multiplier = self.config.get(
        ...
        self.enable_automatic_fallback = self.config.get(
    ...
    def validate_prediction(
        ...
        try:
            ...
            consistency_issues = self._check_prediction_consistency(
    ...
    def _perform_sanity_checks(
        ...
        if current_atr > 0:
            ...
            if tp_mult > 0 and abs(take_profit - entry_price) > 0:
                ...
                if (
                    abs(implied_atr_tp - current_atr) / current_atr > 0.5
                ):  # 50% tolerance
                    issues.append("ATR inconsistency in TP calculation")

            if sl_mult > 0 and abs(stop_loss - entry_price) > 0:
                implied_atr_sl = abs(stop_loss - entry_price) / sl_mult
                if (
                    abs(implied_atr_sl - current_atr) / current_atr > 0.5
                ):  # 50% tolerance
                    issues.append("ATR inconsistency in SL calculation")
    ...
    def _is_multiplier_reasonable(self, multiplier: float, mult_type: str) -> bool:
        ...
        if mult_type == "tp":
            return (
                self.min_reasonable_tp_multiplier
                <= multiplier
                <= self.max_reasonable_tp_multiplier
            )
        elif mult_type == "sl":
            return (
                self.min_reasonable_sl_multiplier
                <= multiplier
    ...
    def _check_prediction_consistency(
        self, prediction: Dict[str, Any], signal_type: str
    ) -> List[str]:
    ...
    def _apply_fallback_correction(
        self,
        prediction: Dict[str, Any],
        signal_type: str,
        current_atr: float,
        ...
        if "LONG" in signal_type:
            corrected["take_profit"] = entry_price + (
                current_atr * self.fallback_tp_multiplier
            )
            corrected["stop_loss"] = entry_price - (
                current_atr * self.fallback_sl_multiplier
            )
        else:  # SHORT
            corrected["take_profit"] = entry_price - (
                current_atr * self.fallback_tp_multiplier
    ...
    def _get_emergency_fallback(
        self, signal_type: str, current_atr: float, market_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def get_health_report(self) -> Dict[str, Any]:
        ...
        if not self.prediction_history:
            return {"status": "unknown", "message": "No prediction history available"}

        recent_records = self.prediction_history[
            -min(50, len(self.prediction_history)) :
        ]
        ...
        return {
            ...
            "statistics": {
                ...
                "validation_rate": valid_predictions / total_predictions
                if total_predictions > 0
                else 0,
                "fallback_usage": fallback_usage,
                "fallback_rate": fallback_usage / total_predictions
                if total_predictions > 0
                else 0,
    ...
    def should_trigger_retraining(self) -> bool:
        ...
        return (
            self.model_health_status in ["critical", "poor"]
            or validation_rate < 0.3
