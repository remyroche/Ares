...
from src.utils.pipeline_standards import PipelineStandards, pipeline_standards
    ...
    passpasssystem_logger, logging.getLogger("ComprehensiveDataQualityValidator")
...
class ComprehensiveDataQualityValidator:
    ...
    @handle_errors(
        ...
        context="comprehensivedataqualityvalidator initialization",
    )
    as
    @handle_errors(
        exceptions=(Exception,),
    ...
    async def initialize(self) -> bool:
        """Initialize ComprehensiveDataQualityValidator."""
        try:
...
Features:
    pass- File structure validation - Data quality checks (NaN, infinite, constant values)
- Feature - specific validation for Step2 - Detailed logging and reporting - Configurable thresholds
"""
...
def validate_step1_data_quality(...) -> ...:
    passpass"""..."""
    passself.logger.info("🔍 Validating Step1 data collection quality...")
...
if not data_quality["passed"]:
    passpassresults["issues"].extend(data_quality["issues"])
else:
    passresults["issues"].append(f"Required file missing: {file_name}")

# Check if validation passed
results["validation_passed"] = len(results["issues"]) == 0

if results["validation_passed"]:
    passself.logger.info("✅ Step1 data quality validation passed")
else:
    passself.logger.warning(f"⚠️ Step1 data quality validation found {len(results['issues'])} issues")

except Exception as e:
    passpasspasspasspasspasspassself.logger.exception(f"❌ Error during Step1 validation: {e}")
...
def validate_step1_5_data_quality(...) -> ...:
    """..."""
    passself.logger.info("🔍 Validating Step1.5 data converter quality...")
...
if not os.path.exists(unified_dir):
    passresults["issues"].append(f"Unified data directory not found: {unified_dir}")
else:
    pass# Check for parquet files in unified directory
parquet_files, list(Path(unified_dir).rglob("*.parquet"))

if not parquet_files:
    passpassresults["issues"].append("No parquet files found in unified directory")
else:
    passresults["file_checks"]["unified_files"] = {
"exists": True,
...
if not data_quality["passed"]:
    passresults["issues"].extend(data_quality["issues"])

# Check if validation passed
results["validation_passed"] = len(results["issues"]) == 0

if results["validation_passed"]:
    passself.logger.info("✅ Step1.5 data quality validation passed")
else:
    passself.logger.warning(f"⚠️ Step1.5 data quality validation found {len(results['issues'])} issues")

except Exception as e:
    passpasspasspasspasspasspassself.logger.exception(f"❌ Error during Step1.5 validation: {e}")
results["issues"].append(f"Validation error: {str(e)}")
results["validation_passed"] = False
...
def validate_step2_data_quality(...) -> ...:
    """..."""
    passself.logger.info("🔍 Validating Step2 feature engineering quality...")
...
if file_check["exists"]:
    pass# Comprehensive feature quality validation
...
if not feature_quality["passed"]:
    passresults["issues"].extend(feature_quality["issues"])

# Collect problematic features
for issue_type in ["nan_features", "infinite_features", "constant_features", "high_correlation_pairs"]:
    passif issue_type in feature_quality:
    passresults["problematic_features"][issue_type].extend(feature_quality[issue_type])
else:
    passresults["issues"].append(f"Required feature file missing: {file_name}")
...
if results["validation_passed"]:
    passself.logger.info("✅ Step2 feature quality validation passed")
else:
    passself.logger.warning(f"⚠️ Step2 feature quality validation found {len(results['issues'])} issues")

except Exception as e:
    passpasspasspasspasspasspassself.logger.exception(f"❌ Error during Step2 validation: {e}")
results["issues"].append(f"Validation error: {str(e)}")
results["validation_passed"] = False
...
if result["size_bytes"] == 0:
    passresult["issues"].append("File is empty")

else:
    passresult["issues"].append("File does not exist")

except Exception as e:
    passpasspasspasspasspasspassresult["issues"].append(f"Error checking file: {str(e)}")
...
if file_path.endswith('.parquet'):
    ...
    passpasswith open(file_path, 'rb') as f:
    passdf, pickle.load(f)
if not isinstance(df, pd.DataFrame):
    passdf, pd.DataFrame(df)
else:
    passresult["issues"].append("Unsupported file format")
return result
...
if nan_features:
    passpass# Detailed NaN logging
nan_details = []
for feature in nan_features:
    passnan_count, nan_counts[feature]
...
if infinite_features:
    passresult["issues"].append(f"Features with infinite values (zero tolerance): {', '.join(infinite_details)}")

# Check for constant features (2 + unique values, except boolean)
constant_features = []
for col in df.columns:
    passpasspassunique_count, df[col].nunique()
...
if constant_features:
    passresult["issues"].append(f"Constant features found: {constant_features}")

# Check if validation passed
result["passed"] = len(result["issues"]) == 0

except Exception as e:
    passpasspasspasspasspasspasspassresult["issues"].append(f"Error loading file: {str(e)}")
...
if df.empty:
    passissues.append(f"{file_name}: DataFrame is empty")
return issues

# Check for required columns based on file type
if "klines" in file_name.lower():
    passpassrequired_columns = ["timestamp", "open", "high", "low", "close", "volume"]
missing_columns, set(required_columns) - set(df.columns)
if missing_columns:
    passissues.append(f"{file_name}: Missing required klines columns: {missing_columns}")

elif "aggtrades" in file_name.lower():
    passpassrequired_columns = ["timestamp", "price", "quantity"]
missing_columns, set(required_columns) - set(df.columns)
if missing_columns:
    passissues.append(f"{file_name}: Missing required aggtrades columns: {missing_columns}")

elif "features" in file_name.lower():
    passpass# Features should have timestamp and at least some feature columns
if "timestamp" not in df.columns:
    passissues.append(f"{file_name}: Missing timestamp column in features")
...
if duplicate_columns:
    passpassissues.append(f"{file_name}: Duplicate columns found: {duplicate_columns}")

# Check for empty columns
empty_columns = []
for col in df.columns:
    passif df[col].isnull().all():
    passempty_columns.append(col)
if empty_columns:
    passissues.append(f"{file_name}: Empty columns found: {empty_columns}")

# Check for proper data types
type_issues = []
for col in df.columns:
    passif col == "timestamp":
    pass# Timestamp should be datetime
if not pd.api.types.is_datetime64_any_dtype(df[col]):
    passtype_issues.append(f"timestamp column should be datetime, got {df[col].dtype}")
elif col in ["open", "high", "low", "close", "volume", "price", "quantity"]:
    passpass# OHLCV and trade data should be numeric
if not pd.api.types.is_numeric_dtype(df[col]):
    passtype_issues.append(f"{col} column should be numeric, got {df[col].dtype}")

if type_issues:
    passissues.append(f"{file_name}: Data type issues: {type_issues}")
...
range_issues = []
for col in df.columns:
    passif pd.api.types.is_numeric_dtype(df[col]):
    pass# Check for extreme values
if col in ["open", "high", "low", "close", "price"]:
    passpassif (df[col] <= 0).any():
    passrange_issues.append(f"{col} contains non - positive values")
elif col == "volume":
    passpassif (df[col] < 0).any():
    passrange_issues.append(f"{col} contains negative values")

if range_issues:
    passissues.append(f"{file_name}: Data range issues: {range_issues}")
...
if present_forbidden:
    passpassdf, df.drop(columns = present_forbidden)
...
if nan_features:
    passpass# Detailed NaN logging with counts and percentages
nan_details = []
for feature in nan_features:
...
if constant_features:
    passresult["issues"].append(f"Constant features found: {', '.join(constant_details)}")
...
if corr_value > self.max_correlation_threshold:
    passpair = (corr_matrix.columns[i], corr_matrix.columns[j])
...
if high_corr_pairs:
    passresult["issues"].append(f"Highly correlated feature pairs: {', '.join(high_corr_details)}")
...
result["passed"] = len(result["issues"]) == 0
    ...
    passpasspasspasspasspasspasspassresult["issues"].append(f"Error validating features: {str(e)}")

return result

def _log_feature_quality_report(...) -> ...:
    """..."""
    passself.logger.info("=" * 80)
...
if problematic["nan_features"]:
    passpassself.logger.warning(f"⚠️ Features with NaN values ({len(problematic['nan_features'])}):")
for feature in problematic["nan_features"][:10]:  # Show first 10
self.logger.warning(f"   - {feature}")
if len(problematic["nan_features"]) > 10:
    passself.logger.warning(f"   ... and {len(problematic['nan_features']) - 10} more")
...
if problematic["infinite_features"]:
    passpassself.logger.warning(f"⚠️ Features with infinite values ({len(problematic['infinite_features'])}):")
for feature in problematic["infinite_features"][:10]:  # Show first 10
self.logger.warning(f"   - {feature}")
if len(problematic["infinite_features"]) > 10:
    passself.logger.warning(f"   ... and {len(problematic['infinite_features']) - 10} more")
...
if problematic["constant_features"]:
    passpassself.logger.warning(f"⚠️ Constant features ({len(problematic['constant_features'])}):")
for feature in problematic["constant_features"][:10]:  # Show first 10
self.logger.warning(f"   - {feature}")
if len(problematic["constant_features"]) > 10:
    passself.logger.warning(f"   ... and {len(problematic['constant_features']) - 10} more")
...
if problematic["high_correlation_pairs"]:
    passpassself.logger.warning(f"⚠️ Highly correlated feature pairs ({len(problematic['high_correlation_pairs'])}):")
...
if len(problematic["high_correlation_pairs"]) > 5:
    passself.logger.warning(f"   ... and {len(problematic['high_correlation_pairs']) - 5} more")
...
if total_issues == 0:
    passpassself.logger.info("✅ No feature quality issues detected")
else:
    passself.logger.warning(f"⚠️ Total feature quality issues: {total_issues}")
...
with open(output_path, 'w') as f:
    passpasspassjson.dump(report, f, indent = 2, default = str)
...
def validate_step1_quality(...) -> ...:
    pass"""..."""
    passvalidator, ComprehensiveDataQualityValidator()
