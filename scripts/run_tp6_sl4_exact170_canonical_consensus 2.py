...
def _read_downloaded_15m_decision_open(
    ...
    *,
    ...
    ``return_lineage`` exposes source and fallback-volume metadata to the
    target-free candidate materialiser.  These fields are execution audit
    metadata only; they are never model inputs.
    """
    ...
    def _load(path: Path) -> pd.Series | None:
        if not path.exists():
            return None
        try:
            raw = pd.read_parquet(
    ...
    shared_mirror = _load(HF_15M_ROOT / name)
    if raw_download is None:
        values = shared_mirror
        raw_present = pd.Series(False, index=index)
    elif shared_mirror is None:
        values = raw_download
        raw_present = raw_download.notna()
    else:
        values = raw_download.combine_first(shared_mirror)
        raw_present = raw_download.notna()
    if values is None:
        values = pd.Series(np.nan, index=index, dtype=np.float64)
        raw_present = pd.Series(False, index=index)
