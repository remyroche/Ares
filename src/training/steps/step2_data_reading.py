...
class DataReadingStep:
        ...
        try:
            ...
            if dataframes:
                ...
                if validation_result.passed:
                    self.logger.info(
                        f"✅ Successfully read unified data: {
                            len(unified_data)} rows (quality score: {
    ...
    async def save_validation_report(
        self, validation_results: Dict[str, Any], symbol: str, exchange: str, data_dir: str
    ) -> bool:
    ...
    async def _log_step2_artifacts_and_report(
        ...
        try:
            ...
            report_data = create_detailed_step_report(
                ...
                errors_encountered=(
                    [] if validation_results.get("passed", False) else validation_results.get("issues", [])
                ),
            )
