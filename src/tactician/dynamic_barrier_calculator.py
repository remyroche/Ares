...
class DynamicBarrierCalculator:
    ...
    calculates Tactician barriers as fractions of those values. It supports both 1m and
    5m timeframes with appropriate adjustments.
    """
                    ...
                    try:
                        ...
                        if "analyst" in config_data:
                            ...
                            if "triple_barrier" in analyst_section:
                                ...
                                analyst_config["profit_take_multiplier"] = (
                                    analyst_section["profit_take_multiplier"]
                                )
                            elif "stop_loss_multiplier" in analyst_section:
                                analyst_config["stop_loss_multiplier"] = (
                                    analyst_section["stop_loss_multiplier"]
                                )

                    except Exception as e:
                        self.logger.warning(
                            f"⚠️ Could not load Analyst config from {config_path}: {e}"
                        )
                        continue

            self.logger.info(f"📊 Loaded Analyst Configuration:")
            self.logger.info(
                f"   Profit Take: {analyst_config['profit_take_multiplier']:.4f} ({analyst_config['profit_take_multiplier']*100:.3f}%)"
            )
            self.logger.info(
                f"   Stop Loss: {analyst_config['stop_loss_multiplier']:.4f} ({analyst_config['stop_loss_multiplier']*100:.3f}%)"
            )
            self.logger.info(
                f"   Time Barrier: {analyst_config['time_barrier_minutes']} minutes"
            )
    ...
    def _initialize_dynamic_barriers(self) -> None:
        ...
        self.upper_barrier_25_fraction = fractions.get(
            "upper_barrier_25_fraction", 0.25
        )
        self.lower_barrier_25_fraction = fractions.get(
        ...
        self.secondary_timeframe = self.tactician_config.get(
            "secondary_timeframe", "5m"
        )

        self.logger.info(f"🔧 Dynamic Barrier Calculator Initialized:")
        self.logger.info(
            f"   50% Barriers - Upper: {self.upper_barrier_50_fraction:.2f}, Lower: {self.lower_barrier_50_fraction:.2f}"
        )
        self.logger.info(
            f"   25% Barriers - Upper: {self.upper_barrier_25_fraction:.2f}, Lower: {self.lower_barrier_25_fraction:.2f}"
        )
        self.logger.info(
            f"   Timeframes: {self.timeframes} (both equal, ML model decides usage)"
        )
        self.logger.info(
            f"   Primary: {self.primary_timeframe}, Secondary: {self.secondary_timeframe}"
        )
    ...
    def calculate_dynamic_barriers(
        ...
        try:
            ...
            if timeframe not in self.timeframes:
                self.logger.warning(
                    f"⚠️ Invalid timeframe {timeframe}, using primary timeframe {self.primary_timeframe}"
                )
                timeframe = self.primary_timeframe

            # Get Analyst base values
            analyst_upper = self.analyst_config[
                "profit_take_multiplier"
            ]  # Upper barrier (profit take)
            analyst_lower = self.analyst_config[
            ...
            self.logger.info(
                f"🎯 2 Dynamic Barrier Combinations Calculated for {timeframe}:"
            )
            self.logger.info(
                f"   Analyst Base - Upper: {analyst_upper:.4f}, Lower: {analyst_lower:.4f}"
            )
            for name, (upper, lower) in barriers.items():
                self.logger.info(f"   {name}: Upper={upper:.4f}, Lower={lower:.4f}")
            self.logger.info(
                f"   Note: Both barrier combinations will be used - position only opens if confidence is high enough for both"
            )
    ...
    def calculate_multi_timeframe_barriers(
        ...
        try:
            ...
            self.logger.info(
                f"📊 Multi-timeframe barriers calculated (2 combinations each):"
            )
            for tf, combinations in barriers.items():
                self.logger.info(f"   {tf}:")
                for name, (upper, lower) in combinations.items():
                    self.logger.info(
                        f"     {name}: Upper={upper:.4f}, Lower={lower:.4f}"
                    )
            self.logger.info(
                f"   Note: Both barrier combinations will be used for each timeframe"
            )
    ...
    def validate_barrier_calculation(self, timeframe: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for barrier_name, (upper, lower) in barriers.items():
                ...
                upper_fraction_error = abs(
                    actual_upper_fraction - expected_upper_fraction
                )
                lower_fraction_error = abs(
                    actual_lower_fraction - expected_lower_fraction
                )
                ...
                validation_results[barrier_name] = {
                    ...
                    "is_valid": upper_fraction_error < 0.1
                    and lower_fraction_error < 0.1,
                }
            ...
            return {
                ...
                "overall_valid": all(
                    result["is_valid"] for result in validation_results.values()
                ),
            }

        except Exception as e:
            self.logger.error(f"❌ Error validating barrier calculation: {e}")
            return {"timeframe": timeframe, "error": str(e), "is_valid": False}
