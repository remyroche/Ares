...
class EfficientFeaturesDatabase:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.storage_format = self.db_config.get('storage_format', 'pickle')
    ...
    @handles_errors(exceptions=(OSError, PermissionError, ValueError), default_return=[])
    async def _scan_existing_databases(self) -> list[str]:
        ...
        try:
            ...
            for filename in os.listdir(self.base_storage_dir):
                if filename.endswith(('.pkl', '.parquet', '.h5')) and 'precomputed_features' in filename:
                    ...
                    if len(parts) >= 6:
                        ...
                        metadata = {'symbol': symbol, 'exchange': exchange, 'date': date, 'timestamp': timestamp, 'file_path': db_path, 'file_size': file_stat.st_size, 'last_modified': datetime.fromtimestamp(file_stat.st_mtime)}
                        try:
                            data_info = await self._get_database_info(db_path)
                            metadata.update(data_info)
                        except Exception as e:
                            self.logger.warning(f'Could not read info from {filename}: {e}')
    ...
    async def find_existing_database(self, symbol: str, exchange: str, start_time: pd.Timestamp = None, end_time: pd.Timestamp = None) -> tuple[str | None, list[tuple[pd.Timestamp, pd.Timestamp]]]:
        ...
        try:
            ...
            for db_name, metadata in self.metadata_cache.items():
                if metadata.get('symbol', '').upper() == clean_symbol and metadata.get('exchange', '').upper() == clean_exchange:
            ...
            latest_db = max(matching_dbs, key=lambda x: x[1].get('last_modified', datetime.min))
    ...
    async def save_database(self, data: pd.DataFrame, symbol: str, exchange: str, database_name: str = None) -> bool:
        ...
        try:
            ...
            if database_name is None:
                ...
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                database_name = self._generate_database_name(symbol, exchange, start_date, timestamp)
            ...
            file_stat = os.stat(db_path)
            metadata = {'symbol': symbol.replace('/', '').replace('-', '').upper(), 'exchange': exchange.upper(), 'date': data.index.min().strftime('%Y-%m-%d'), 'timestamp': datetime.now().strftime('%Y%m%d_%H%M%S'), 'file_path': db_path, 'file_size': file_stat.st_size, 'last_modified': datetime.fromtimestamp(file_stat.st_mtime), 'start_time': data.index.min(), 'end_time': data.index.max(), 'num_records': len(data), 'num_features': len(data.columns), 'feature_categories': self._analyze_feature_categories(data.columns)}
    ...
    async def update_database(self, new_data: pd.DataFrame, existing_database_name: str) -> bool:
        ...
        try:
            ...
            if existing_data.empty:
                self.logger.warning(f'Could not load existing database: {existing_database_name}')
                return False
            ...
            success = await self._save_database_with_timestamp_update(combined_data, symbol, exchange, existing_database_name)
            if success:
                self.logger.info(f'✅ Database updated with {len(truly_new_data)} new records')
    ...
    async def _save_database_with_timestamp_update(self, data: pd.DataFrame, symbol: str, exchange: str, database_name: str) -> bool:
        ...
        try:
            ...
            file_stat = os.stat(db_path)
            metadata = {'symbol': symbol.replace('/', '').replace('-', '').upper(), 'exchange': exchange.upper(), 'date': data.index.min().strftime('%Y-%m-%d'), 'timestamp': current_time.strftime('%Y%m%d_%H%M%S'), 'file_path': db_path, 'file_size': file_stat.st_size, 'last_modified': current_time, 'start_time': data.index.min(), 'end_time': data.index.max(), 'num_records': len(data), 'num_features': len(data.columns), 'feature_categories': self._analyze_feature_categories(data.columns), 'last_update': current_time.isoformat()}
            self.metadata_cache[database_name] = metadata
            if len(data) < self.chunk_size * 10:
                self.database_cache[database_name] = data.copy()
    ...
    def get_database_stats(self) -> dict[str, Any]:
        ...
        total_size = sum((meta.get('file_size', 0) for meta in self.metadata_cache.values()))
        total_records = sum((meta.get('num_records', 0) for meta in self.metadata_cache.values()))
    ...
    async def cleanup_old_databases(self, keep_latest_n: int = 5) -> None:
        ...
        try:
            ...
            self.logger.info(f'Cleanup completed. Deleted {deleted_count} old databases')
        except Exception as e:
