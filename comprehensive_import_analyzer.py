#!/usr/bin/env python3
"""
Comprehensive Import Analyzer - Detects all import patterns including multi-line imports.
"""
...
class ComprehensiveImportAnalyzer:
    """Comprehensive analyzer that detects all import patterns."""
    ...
    def analyze_utils_directory(self) -> Dict[str, Any]:
        ...
        all_imports = {}
        for file_path in all_python_files:
            imports = self._extract_comprehensive_imports(file_path)
        ...
        for utils_file in utils_files:
            ...
            for other_file, other_imports in all_imports.items():
                if other_file != file_path_str:
                    if self._is_module_imported_comprehensive(module_name, other_imports):
    ...
    def _extract_comprehensive_imports(self, file_path: Path) -> Set[str]:
        """Extract all imports from a Python file, including multi-line imports."""
        imports = set()
                ...
                if node.module:
                    ...
                    for alias in node.names:
                        if alias.name != '*':
                            ...
                            # Also add just the module name for broader matching
                            imports.add(node.module)
        
        return imports
    
    def _is_module_imported_comprehensive(self, module_name: str, imports: Set[str]) -> bool:
        """
        Check if a module is imported with comprehensive logic.
        """
        # Direct exact match
        ...
        # e.g., if module is "src.utils.sr_clustering" and we see "src.utils.sr_clustering.some_function"
        for imp in imports:
            if imp.startswith(module_name + '.'):
                return True
        
        # Check if this module is imported as a submodule
        # e.g., if module is "src.utils.sr_clustering" and we see "from src.utils import sr_clustering"
        module_parts = module_name.split('.')
        if len(module_parts) > 1:
            # Check if the parent module is imported and this is a submodule
            parent_module = '.'.join(module_parts[:-1])
            module_name_only = module_parts[-1]
            
            # Check for "from parent_module import module_name_only"
            for imp in imports:
                if imp == parent_module:
                    # This means the parent module is imported, but we need to check
                    # if the specific submodule is imported
                    # This is tricky - we'd need to look at the actual import statement
                    # For now, we'll be conservative and not count this
                    pass
    ...
    def print_detailed_report(self, results: Dict[str, Any]) -> None:
        ...
        print("\n" + "="*80)
        print("COMPREHENSIVE DEAD CODE ANALYSIS REPORT - src/utils/")
        print("="*80)
...
def main():
    ...
    analyzer = ComprehensiveImportAnalyzer()
