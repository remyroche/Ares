...
class IntelligentParameterPruner:
    ...
    def _identify_borderline_parameters(self, sensitivity_scores: Dict[str, float]) -> List[str]:
        ...
        borderline = [
            param for param, score in sensitivity_scores.items() if borderline_range[0] <= score <= borderline_range[1]
        ]

        return borderline

    async def _cross_validation_sensitivity_analysis(
        self, data: pd.DataFrame, borderline_params: List[str], parameter_mapping: Dict[str, Dict[str, Any]]
    ) -> Dict[str, float]:
        ...
        for param_key in borderline_params:
            try:
                ...
                for fold in range(3):
                    ...
                    if param_config:
                        sensitivity = await self._detailed_sensitivity_test(
                            fold_data, step_name, param_name, param_config
                        )
    ...
    async def _detect_parameter_interactions(
        self, data: pd.DataFrame, sensitivity_scores: Dict[str, float], parameter_mapping: Dict[str, Dict[str, Any]]
        ...
        for i, param1 in enumerate(high_impact_params[:10]):  # Limit to top 10 for efficiency
            ...
            for param2 in high_impact_params[i + 1 : 11]:  # Test with next 10
                try:
                    interaction_strength = await self._test_parameter_interaction(
    ...
    async def _test_parameter_interaction(
        self, data: pd.DataFrame, param1: str, param2: str, parameter_mapping: Dict[str, Dict[str, Any]]
    ) -> float:
    ...
    def _boost_interaction_scores(
        self, sensitivity_scores: Dict[str, float], interaction_scores: Dict[str, Dict[str, float]]
    ) -> Dict[str, float]:
    ...
    def _get_param_config_from_mapping(
        self, parameter_mapping: Dict[str, Dict[str, Any]], step_name: str, param_name: str
    ) -> Any:
    ...
    async def _evaluate_parameter_combination(
        self, data: pd.DataFrame, param1: str, val1: Any, param2: str, val2: Any
    ) -> float:
...
class AdaptiveTrialAllocator:
    ...
    def allocate_trials_by_phase(
        self, phase_performance: Dict[str, float], phase_complexity: Dict[str, int]
    ) -> Dict[str, int]:
    ...
    def adjust_allocation_during_optimization(
        self, current_phase: str, performance_trend: float, current_trials: int
    ) -> int:
        ...
        if performance_trend > 0.1:  # Improving
            ...
            self.logger.info(
                f"Phase {current_phase} improving, increasing trials from {current_trials} to {new_allocation}"
            )
            ...
            self.logger.info(
                f"Phase {current_phase} declining, decreasing trials from {current_trials} to {new_allocation}"
            )
            return new_allocation
        else:
...
class HierarchicalOptimizer:
    ...
    def __init__(self, config: Dict[str, Any], training_manager=None):
        ...
        self.trial_allocator = AdaptiveTrialAllocator(
            total_trials=config.get("total_trials", 1000), min_trials_per_phase=config.get("min_trials_per_phase", 50)
        )
        ...
        self.performance_thresholds = config.get(
            "performance_thresholds", {"excellent": 0.9, "good": 0.8, "acceptable": 0.7}
        )
    ...
    async def run_hierarchical_optimization(
        ...
        for phase_idx, phase_name in enumerate(parameter_groups):
            ...
            if ensemble_params and self.ensemble_optimization_enabled:
                ...
                high_impact_phase_params = self._optimize_ensemble_parameter_order(
                    high_impact_phase_params, ensemble_params
                )
            ...
            if self.adaptive_learning_rate:
                ...
                if new_trials != trial_allocations.get(phase_name, 100):
                    self.logger.info(
                        f"🔄 Adjusted trials for {phase_name}: {
                            trial_allocations.get(
                                phase_name, 100)} → {new_trials}"
                    )
        ...
        for phase_name, result in results.items():
            ...
            self.logger.info(
                f"  {phase_name}: {result.best_value:.4f} ({performance_level}) - {result.n_trials} trials"
            )
    ...
    async def _optimize_parameter_group_advanced(
        self, group_name: str, parameters: List[str], n_trials: int, data: pd.DataFrame, phase_idx: int
    ) -> OptimizationResult:
        ...
        study.optimize(
            objective, n_trials=n_trials, timeout=self.config.get("timeout_per_phase", 1800), callbacks=callbacks
        )
    ...
    def _evaluate_multi_objective(
        self, data: pd.DataFrame, params: Dict[str, Any], parameters: List[str], phase_idx: int
    ) -> List[float]:
        ...
        return [final_score * 0.5, final_score * 0.8, final_score * 2.0]  # Total Profit  # Win Rate  # Sharpe Ratio

    def _evaluate_parameter_group_advanced(
        self, data: pd.DataFrame, params: Dict[str, Any], parameter_names: List[str], phase_idx: int
    ) -> float:
    ...
    def _calculate_comprehensive_metrics(
        self, best_value: float, phase_idx: int, parameters: List[str]
    ) -> Dict[str, float]:
        """Calculate comprehensive performance metrics."""
