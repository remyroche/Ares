"""
Unified Matrix Operations - M1 Optimized

This module provides a unified, optimized matrix operations system specifically
designed for Apple Silicon (M1/M2/M3) Macs. It consolidates matrix operations
from multiple sources and integrates with existing utility frameworks.

Key Features:
- Apple Silicon GPU acceleration with Metal Performance Shaders (MPS)
- Intelligent memory management using existing M1 memory optimizer
- CPU parallel processing with existing M1 CPU optimizer
- Vectorized processing using existing vectorized processing core
- Integration with existing common operations and math validation
- No CUDA dependencies (Mac M1 focused)
...
>>> correlation = ops.safe_correlation_matrix(data)
"""
