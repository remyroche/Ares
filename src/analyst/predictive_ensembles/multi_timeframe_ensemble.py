...
class MultiTimeframeEnsemble:
    ...
    def __init__(self, model_name: str, regime: str, config: dict[str, Any] | None = None) -> None:
        ...
        self.logger = system_logger.getChild(f'MultiTimeframeEnsemble_{model_name}_{regime}')
        self.timeframes = CONFIG.get('TIMEFRAMES', {})
        self.timeframe_set = CONFIG.get('DEFAULT_TIMEFRAME_SET', 'intraday')
        self.active_timeframes = CONFIG.get('TIMEFRAME_SETS', {}).get(self.timeframe_set, [])
        ...
        self.logger.info(f'🚀 Initializing MultiTimeframeEnsemble for {model_name} in {regime}')
        self.logger.info(f'📊 Active timeframes: {self.active_timeframes}')
        self.logger.info(f'⚙️ Configuration: {self.config}')

    def train_multi_timeframe_ensemble(self, prepared_data: dict[str, pd.DataFrame], model_type: str='xgboost') -> bool:
        """
        ...
        try:
            self.logger.info(f'🎯 Starting multi-timeframe ensemble training for {self.model_name} in {self.regime}')
            self.logger.info(f'📈 Model type: {model_type}')
            self.logger.info(f'⏰ Available timeframes: {list(prepared_data.keys())}')
            self.logger.info(f'📊 Data shapes: {[(tf, df.shape) for tf, df in prepared_data.items()]}')
            timeframe_predictions = {}
            timeframe_confidences = {}
            training_stats = {}
            for i, timeframe in enumerate(self.active_timeframes, 1):
                self.logger.info(f'🔄 [{i}/{len(self.active_timeframes)}] Training {timeframe} timeframe...')
                if timeframe not in prepared_data:
                    self.logger.warning(f'⚠️ No data for timeframe {timeframe}, skipping')
                    continue
                tf_start_time = time.time()
                success = self._train_single_timeframe(timeframe, prepared_data[timeframe], model_type)
                tf_training_time = time.time() - tf_start_time
                if success:
                    self.logger.info(f'✅ {timeframe} training completed in {tf_training_time:.2f}s')
                    self.logger.info(f'📊 Collecting predictions for {timeframe}...')
                    predictions, confidences = self._get_timeframe_predictions(timeframe, prepared_data[timeframe])
                    timeframe_predictions[timeframe] = predictions
                    timeframe_confidences[timeframe] = confidences
                    training_stats[timeframe] = {'training_time': tf_training_time, 'predictions_count': len(predictions), 'avg_confidence': np.mean(confidences) if confidences else 0.0, 'success': True}
                    self.logger.info(f'📈 {timeframe} stats: {len(predictions)} predictions, avg confidence: {np.mean(confidences):.3f}')
                else:
                    self.logger.error(failed('❌ {timeframe} training failed'))
                    training_stats[timeframe] = {'training_time': tf_training_time, 'success': False}
            if len(timeframe_predictions) > 1:
                self.logger.info(f'🧠 Training meta-learner with {len(timeframe_predictions)} timeframes...')
                meta_start_time = time.time()
                success = self._train_meta_learner(timeframe_predictions, timeframe_confidences, prepared_data)
                meta_training_time = time.time() - meta_start_time
                if success:
                    self.trained = True
                    total_time = time.time() - start_time
                    self.logger.info('✅ Multi-timeframe ensemble training completed successfully!')
                    self.logger.info(f'⏱️ Total training time: {total_time:.2f}s')
                    self.logger.info('📊 Training summary:')
                    self.logger.info(f'   - Model: {self.model_name}')
                    self.logger.info(f'   - Regime: {self.regime}')
                    self.logger.info(f'   - Timeframes trained: {len(timeframe_predictions)}')
                    self.logger.info(f'   - Meta-learner training time: {meta_training_time:.2f}s')
                    for tf, stats in training_stats.items():
                        if stats.get('success'):
                            self.logger.info(f"   - {tf}: {stats['training_time']:.2f}s, {stats['predictions_count']} predictions, avg confidence: {stats['avg_confidence']:.3f}")
                        else:
                            self.logger.error(failed('   - {tf}: FAILED'))
                    return True
                self.logger.error(failed('❌ Meta-learner training failed'))
                return False
            self.logger.error(f'❌ Insufficient timeframes ({len(timeframe_predictions)}) for meta-learner training')
            return False
        except Exception as e:
            self.logger.error(f'💥 Error in multi-timeframe ensemble training: {e}')
            return False

    def _train_single_timeframe(self, timeframe: str, data: pd.DataFrame, model_type: str) -> bool:
        ...
        try:
            ...
            if model is not None:
                self.timeframe_models[timeframe] = {'model': model, 'model_type': model_type, 'timeframe': timeframe, 'trained_at': datetime.now(), 'features_shape': X.shape, 'target_distribution': y.value_counts().to_dict()}
                self.logger.info(f'✅ {timeframe} {model_type} model trained successfully')
                return True
            return False
        except Exception as e:
    ...
    def _train_random_forest_model(self, X: pd.DataFrame, y: pd.Series) -> Any | None:
    ...
    def _prepare_features_target(self, data: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    ...
    def _get_timeframe_predictions(self, timeframe: str, data: pd.DataFrame) -> tuple[list[str], list[float]]:
        ...
        try:
            ...
            if hasattr(model, 'predict_proba'):
                ...
                self.logger.warning(f"⚠️ {timeframe}: Model doesn't support predict_proba, using default confidence")
            return (predictions, confidences)
        except Exception as e:
            self.logger.error(f'💥 Error getting predictions for {timeframe}: {e}')
            return ([], [])

    def _train_meta_learner(self, timeframe_predictions: dict[str, list[str]], timeframe_confidences: dict[str, list[float]], prepared_data: dict[str, pd.DataFrame]) -> bool:
        ...
        try:
            ...
            meta_data = self._prepare_meta_learner_data(timeframe_predictions, timeframe_confidences, prepared_data)
            if len(meta_data) == 0:
    ...
    def _prepare_meta_learner_data(self, timeframe_predictions: dict[str, list[str]], timeframe_confidences: dict[str, list[float]], prepared_data: dict[str, pd.DataFrame]) -> pd.DataFrame:
                ...
                for timeframe in self.active_timeframes:
                    if timeframe in timeframe_predictions:
                        ...
                        if pred_idx < len(timeframe_predictions[timeframe]):
                            row_data[f'{timeframe}_prediction'] = timeframe_predictions[timeframe][pred_idx]
                            row_data[f'{timeframe}_confidence'] = timeframe_confidences[timeframe][pred_idx]
                        else:
    ...
    def get_prediction(self, current_features: pd.DataFrame, **kwargs) -> dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.debug(f'🔮 Getting prediction for {self.model_name} in {self.regime}')
            timeframe_predictions = {}
            timeframe_confidences = {}
            for timeframe in self.active_timeframes:
                if timeframe in self.timeframe_models:
                    self.logger.debug(f'📊 Getting prediction for {timeframe}...')
                    pred, conf = self._get_single_prediction(timeframe, current_features)
                    timeframe_predictions[timeframe] = pred
                    timeframe_confidences[timeframe] = conf
                    self.logger.debug(f'📊 {timeframe}: {pred} (confidence: {conf:.3f})')
            if self.meta_learner and len(timeframe_predictions) > 0:
                self.logger.debug('🧠 Combining predictions with meta-learner...')
                final_prediction, final_confidence = self._combine_with_meta_learner(timeframe_predictions, timeframe_confidences, current_features)
            else:
                self.logger.warning('⚠️ Using simple prediction combination (no meta-learner)')
                final_prediction, final_confidence = self._simple_combine_predictions(timeframe_predictions, timeframe_confidences)
            self.logger.info(f'🎯 Final prediction: {final_prediction} (confidence: {final_confidence:.3f})')
            return {'prediction': final_prediction, 'confidence': final_confidence, 'timeframe_predictions': timeframe_predictions, 'timeframe_confidences': timeframe_confidences, 'model_name': self.model_name, 'regime': self.regime}
        except Exception as e:
            self.logger.error(f'💥 Error getting prediction: {e}')
            return {'prediction': 'HOLD', 'confidence': 0.0}

    def _get_single_prediction(self, timeframe: str, features: pd.DataFrame) -> tuple[str, float]:
    ...
    def _combine_with_meta_learner(self, timeframe_predictions: dict[str, str], timeframe_confidences: dict[str, float], current_features: pd.DataFrame) -> tuple[str, float]:
        ...
        try:
            ...
            for timeframe in self.active_timeframes:
                ...
                pred_encoded = [1.0 if pred == 'BUY' else 0.0, 1.0 if pred == 'SELL' else 0.0, 1.0 if pred == 'HOLD' else 0.0]
            ...
            prediction = self.meta_label_encoder.inverse_transform([prediction_encoded])[0]
            if hasattr(self.meta_learner, 'predict_proba'):
                probas = self.meta_learner.predict_proba(meta_features_scaled)
                confidence = np.max(probas[0])
            else:
                confidence = 0.5
            self.logger.debug(f'🎯 Meta-learner prediction: {prediction} (confidence: {confidence:.3f})')
            return (prediction, confidence)
        except Exception as e:
            self.logger.error(f'💥 Error combining with meta-learner: {e}')
            return ('HOLD', 0.0)

    def _simple_combine_predictions(self, timeframe_predictions: dict[str, str], timeframe_confidences: dict[str, float]) -> tuple[str, float]:
        ...
        try:
            ...
            final_confidence = total_confidence / len(timeframe_confidences) if timeframe_confidences else 0.0
            self.logger.debug(f'📊 Simple combination: {pred_counts}, final: {final_prediction} (confidence: {final_confidence:.3f})')
    ...
    def save_model(self, path: str) -> bool:
        ...
        try:
            ...
            ensemble_info = {'model_name': self.model_name, 'regime': self.regime, 'active_timeframes': self.active_timeframes, 'trained': self.trained, 'trained_at': datetime.now()}
    ...
    def load_model(self, path: str) -> bool:
        ...
        try:
            ...
            if os.path.exists(info_path):
                ...
                self.logger.info(f'📊 Loaded ensemble info: {self.model_name} in {self.regime}')
            for timeframe in self.active_timeframes:
                model_path = os.path.join(path, f'{timeframe}_model.joblib')
                if os.path.exists(model_path):
                    cached_model = joblib.load(model_path)
                    self.timeframe_models[timeframe] = {'model': cached_model, 'model_type': 'loaded', 'timeframe': timeframe, 'loaded_at': datetime.now()}
