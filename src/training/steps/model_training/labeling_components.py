...
class TripleBarrierLabeler:
    ...
    def __init__(self, barrier_config: Dict[str, Any]):
    ...
    async def label(
        ...
        # Use regime-aware barriers if regime info is provided
    ...
    def _standard_labeling(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Initialize arrays for labels and metadata
        labels = np.zeros(n_samples)
        label_metadata = {
            "exit_index": np.full(n_samples, -1, dtype=int),
            "holding_period": np.zeros(n_samples, dtype=int),
            "exit_return": np.zeros(n_samples),
            "exit_reason": [""] * n_samples,
            "max_profit": np.zeros(n_samples),
            "max_loss": np.zeros(n_samples)
        }
        
        close_prices = data["close"].values
        
        # Apply triple barrier for each sample
        ...
        # Add labels and metadata to dataframe
    ...
    async def _regime_aware_labeling(
        self, 
        data: pd.DataFrame, 
        ...
        label_metadata = {
            ...
            "exit_return": np.zeros(n_samples),
            "exit_reason": [""] * n_samples,
            "regime_at_entry": np.zeros(n_samples, dtype=int),
            "regime_at_exit": np.zeros(n_samples, dtype=int),
            "used_profit_barrier": np.zeros(n_samples),
            "used_loss_barrier": np.zeros(n_samples)
        }
        
        close_prices = data["close"].values
        
        # Apply regime-aware triple barrier
        for i in range(n_samples - self.min_holding_period):
            entry_regime = regime_labels[i] if i < len(regime_labels) else 0
            entry_price = close_prices[i]
            
            # Get regime-specific barriers
            barriers = regime_barriers.get(entry_regime, {
            ...
            for j in range(i + self.min_holding_period, max_j):
                ...
                # Check regime-specific barriers
    ...
    def _calculate_regime_barriers(
        self, 
        ...
        for regime_key, chars in regime_characteristics.items():
            if isinstance(chars, dict) and regime_key.startswith("regime_"):
                ...
                # Adjust barriers based on regime volatility
                # Higher volatility -> wider barriers
                volatility_multiplier = max(0.5, min(2.0, volatility / 0.01))
                
                regime_barriers[regime_id] = {
                    "profit_taking": self.profit_taking * volatility_multiplier,
                    "stop_loss": self.stop_loss * volatility_multiplier,
                    "max_holding": int(self.max_holding_period / volatility_multiplier)
                }
                
                self.logger.info(
                    f"Regime {regime_id} barriers: "
                    f"PT={regime_barriers[regime_id]['profit_taking']:.3f}, "
                    f"SL={regime_barriers[regime_id]['stop_loss']:.3f}, "
                    f"MH={regime_barriers[regime_id]['max_holding']}"
                )
    ...
    def _add_label_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if "label" in data.columns:
            # Add label confidence based on exit return magnitude
            if "label_exit_return" in data.columns:
                data["label_confidence"] = np.abs(data["label_exit_return"])
            
            # Add label quality score based on holding period
            if "label_holding_period" in data.columns:
                # Shorter holding periods for clear signals are better
            ...
            # Add potential profit/loss ratio
            if "label_max_profit" in data.columns and "label_max_loss" in data.columns:
                # Avoid division by zero
                data["label_profit_loss_ratio"] = np.where(
                    data["label_max_loss"] < -0.0001,
                    -data["label_max_profit"] / data["label_max_loss"],
                    np.inf
                )
