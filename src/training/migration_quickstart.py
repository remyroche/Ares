...
class MigrationHelper:
    ...
    def list_pending_steps(self) -> List[Dict[str, str]]:
        ...
        pending = []
        step_info = {'07': ('enhanced_matrix_operations', 'model_training'), '08': ('feature_selection', 'model_training'), '09': ('hmm_based_training', 'model_training'), '09_5': ('multi_timeframe_hmm_ensemble', 'model_training'), '10': ('unified_regime_intelligence', 'model_training'), '11': ('analyst_creation', 'model_training'), '12': ('analyst_enhancement', 'model_training'), '13': ('analyst_ensemble_creation', 'model_training'), '14': ('tactician_labeling', 'model_training'), '15': ('tactician_specialist_training', 'model_training'), '21': ('saving', 'model_training')}
        for step_num, (step_name, category) in step_info.items():
            ...
            if old_path.exists() and (not new_path.exists()):
                file_size = old_path.stat().st_size / 1024
                line_count = sum((1 for _ in open(old_path)))
                pending.append({'step': step_num, 'name': step_name, 'category': category, 'old_path': str(old_path), 'new_path': str(new_path), 'size_kb': file_size, 'lines': line_count})
    ...
    def _create_basic_migration(self, step_info: Dict[str, str]) -> None:
        ...
        template = create_step_template(step_info['step'], step_info['category'], step_info['name'])
        new_path = Path(step_info['new_path'])
    ...
    def analyze_file_structure(self, file_path: Path) -> Dict[str, any]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                classes.append({'name': node.name, 'line': node.lineno, 'methods': methods, 'method_count': len(methods)})
            elif isinstance(node, ast.FunctionDef) and node.col_offset == 0:
                functions.append({'name': node.name, 'line': node.lineno})
            elif isinstance(node, (ast.Import, ast.ImportFrom)):
                imports.append(node.lineno)
        return {'classes': classes, 'functions': functions, 'import_lines': len(imports), 'total_lines': len(content.splitlines())}
