...
def create_pnl_aware_loss(pnl_multiplier=0.1, liquidation_penalty=2.0, reward_boost=1.5):
    """
    ...
    def pnl_aware_loss(y_true, y_pred):
        ...
        # This part of the loss focuses on getting the prediction category correct.
        ...
        financial_loss = (
            (1.0 - true_class_probs) * (risk_potential * liquidation_penalty) * is_wrong -
            (true_class_probs * reward_potential * reward_boost) * (1.0 - is_wrong)
        )
        
        # --- 3. Combine the Losses ---
        # The final loss is the sum of the classification loss and the weighted financial loss.
