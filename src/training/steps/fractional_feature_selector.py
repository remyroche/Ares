...
class FractionalFeatureSelector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # Selection methods
        self.selection_methods = self.config.get(
            "selection_methods", ["correlation", "importance", "stability", "diversity", "label_alignment"]
        )

        # Method weights
        self.method_weights = self.config.get(
            "method_weights",
            {"correlation": 0.25, "importance": 0.25, "stability": 0.15, "diversity": 0.15, "label_alignment": 0.20},
        )
    ...
    @validate_feature_engineering_with_lookahead_bias_detection
    def select_features(
        self, features: pd.DataFrame, labels: pd.Series, hmm_regime: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if "label_alignment" in self.selection_methods:
                selection_scores["label_alignment"] = self._calculate_label_alignment_scores(
    ...
    def _select_final_features(self, features: pd.DataFrame, scores: pd.Series) -> pd.DataFrame:
        ...
        try:
            ...
            n_features = min(
                max(self.min_features, self.target_feature_count), min(self.max_features, len(features.columns))
            )
