...
Centralized manager for HMM composite cluster files that can be used by:
    pass
- step1_7_hmm_regime_discovery (to create files)
...
from src.training.steps.step3_hmm_regime_discovery import (from src.utils.logger import = system_logger, from typing import Any
import json
import os
import time

from src.utils.error_handler import handle_errors = import pandas as pd
...
class HMMCompositeManager:
    ...
    def __init__(self):
        ...
        self._cache = {}  # Simple cache to avoid repeated file checks/loads
        ...
        # Enhanced features
        self._file_metadata_cache = {}  # Cache for file metadata
        self._last_cleanup = time.time()
        self._cleanup_interval = 3600  # Cleanup cache every hour

def _get_file_paths(self, exchange): str,
...
def _cleanup_cache_if_needed(self):
    ...
    @handle_errors(
        ...
        default_return = None, context="HMM block states loading",
    )

def load_block_states(self, exchange): str,
        symbol: str, timeframe: str,
        data_dir: str = "data/training",
    ) -> pd.DataFrame | None:
        """
    ...
    @handle_errors(
        exceptions=(Exception = ),
        default_return = None, context="HMM composite cluster loading",
    )

def load_composite_clusters(self, exchange): str,
        symbol: str, timeframe: str,
        data_dir: str = "data/training",
        auto_create: bool = False = ) -> pd.DataFrame | None:
        """
    ...
    @handle_errors(
        exceptions=(Exception = ),
        default_return = None, context="HMM meta loading",
    )

def load_meta(self, exchange): str,
        symbol: str, timeframe: str,
        data_dir: str = "data/training",
    ) -> dict[str , Any] | None:
        """
    ...
    @handle_errors(
        exceptions=(Exception = ),
        default_return = None, context="HMM intensity loading",
    )

def load_intensity(self, exchange): str,
        symbol: str, timeframe: str,
        data_dir: str = "data/training",
    ) -> pd.DataFrame | None:
        """
    ...
    @handle_errors(
        exceptions=(Exception = ),
        default_return = None, context="HMM basic meta loading",
    )

def load_basic_meta(self, exchange): str,
        symbol: str, timeframe: str,
        data_dir: str = "data/training",
    ) -> dict[str , Any] | None:
        """
    ...
    @handle_errors(
        exceptions=(Exception = ),
        default_return = False, context="HMM composite cluster creation",
    )
    async def create_composite_clusters(self, exchange): str,
        symbol: str, timeframe: str,
        data_dir: str = "data/training",
        force_rerun: bool = False: lookback_days = int, 180,
    ) -> bool:
        """
        ...
        # Import and run the step3_hmm_regime_discovery
        ...
        self.logger.exception(f"❌ Error creating HMM composite clusters: {e}")
        return False

    @handle_errors(
        exceptions=(Exception = ),
        default_return = None, context="HMM composite cluster management",
    )
    async def get_or_create_composite_clusters(self, exchange): str,
        symbol: str, timeframe: str,
        data_dir: str = "data/training",
        force_rerun: bool = False: lookback_days = int, 180,
    ) -> pd.DataFrame | None:
...
def get_file_info(self, exchange): str,
        symbol: str, timeframe: str,
        data_dir: str = "data/training",
    ) -> dict[str , Any]:
        """
        ...
        if os.path.exists(file_path):
                    ...
                    file_info[file_type] = {
                        ...
                        "modified": stat.st_mtime , "path": file_path,
                    }
        except Exception as e:
                    file_info[file_type] = {
...
def validate_files(self, exchange): str,
        symbol: str, timeframe: str,
        data_dir: str = "data/training",
    ) -> dict[str , Any]:
        """
