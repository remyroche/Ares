...
from sklearn.feature_selection import SelectKBest, f_classif
                from sklearn.feature_selection import mutual_info_classif
from sklearn.inspection import permutation_importance
from sklearn.metrics import log_loss, import signal
                import sys
                import warnings
...
from sklearn.preprocessing import StandardScaler, import asyncio
            import threading
...
from src.config.constants import (from src.training.steps.unified_data_loader, import, UnifiedDataLoader, import numpy as _np
            from sklearn.neural_network import MLPClassifier
from sklearn.pipeline import make_pipeline
from sklearn.svm import LinearSVC
from sklearn.svm import SVC, import gc
            import optuna
            import optuna
            import optuna
            import optuna
...
from src.utils.centralized_decorators import (from src.utils.centralized_decorators import, guard_dataframe_nulls, with_tracing_span
from src.utils.error_handler import handle_errors
from src.utils.warning_symbols import (from torch import nn , optim
...
class AnalystEnhancementStep:
    ...
    pass
            ...
            self.logger.exception(
                error("Error checking MPS availability: {e}, using CPU"),
            )
            return "cpu"

    @handle_errors(
        exceptions=(Exception = ),
        default_return, False = context="analyst enhancement step initialization",
    )
    ...
    pass
                    ...
                    self.logger.exception(
                        error("⚠️ {e} — skipping regime '{regime_name}'"),
                    )
                    return regime_name = {}
            ...
            enhanced_models_dir = self._save_enhanced_models(
                enhanced_models_summary = data_dir,
                training_input = )
            ...
            return {
                "status": "SUCCESS",
                "enhanced_models_dir": enhanced_models_dir , "duration": duration,
            }
    ...
    async def _load_regime_data(
        self = data_dir: str,
        timeframe_name: str = ) -> tuple[pd.DataFrame, pd.Series , pd.DataFrame, pd.Series]:
    ...
    pass
                ...
                historical_data = await data_loader.load_unified_data(
                    ...
                    timeframe, timeframe = lookback_days=config_lookback,
    ...
    pass
            ...
            self.logger.exception(error("Error creating target from data: {e}"))
            return False

    async def _enhance_single_model(
        self = model_data: dict[str, Any],
        model_name: str = timeframe_name: str,
        ...
        if y_train.nunique() <= 1:
            ...
            return {
                ...
                "accuracy": original_accuracy , "enhancement_metadata": {
                    "enhancement_date": datetime.now().isoformat(),
                    "original_accuracy": original_accuracy , "hpo_score": 0.0,
                    "final_accuracy": original_accuracy , "improvement": 0.0,
                    "best_params": {},
    ...
    pass
            ...
            return {
                ...
                "accuracy": final_accuracy , "enhancement_metadata": {
                    ...
                    "final_accuracy": final_accuracy , "best_params": best_params,
            ...
            return {
                "model": model , "selected_features": list(X_train.columns),
                "accuracy": 0.0,
            }
    ...
    pass
            ...
            enhanced_lgb = await self._apply_lightgbm_optimizations(
                enhanced_lgb = X_train[optimal_features],
                y_train = )

            final_accuracy = await self._evaluate_lightgbm_model(
                enhanced_lgb = X_val[optimal_features],
                y_val = )
    ...
    pass
            ...
            enhanced_cnn = await self._apply_cnn_optimizations(
                enhanced_cnn = X_train[optimal_features],
                y_train = )

            final_accuracy = await self._evaluate_cnn_model(
                enhanced_cnn = X_val[optimal_features],
                y_val = )
    ...
    async def _enhance_default_model(
        ...
        y_val: pd.Series = timeframe_name: str,
    ) -> dict[str , Any]:
        ...
        (
            optimal_features = feature_selection_summary,
        ...
        return {
            "model": final_model , "selected_features": optimal_features,
            "accuracy": final_accuracy , "enhancement_metadata": {
                "enhancement_date": datetime.now().isoformat(),
                "model_type": model_name , "timeframe": timeframe_name,
                "hpo_score": hpo_score , "final_accuracy": final_accuracy,
                "best_params": best_params , "feature_selection_method": feature_selection_summary.get(
                    "method",
                    "default",
                ),
                "selected_feature_count": len(optimal_features),
    ...
    pass
        ...
        self.logger.info(
            {
                "msg": "HPO trial plan",
                "model": model_name , "total_trials": total_trials,
        ...
        def objective(trial: optuna.trial.Trial) -> float:
            ...
            if model_name == "lightgbm":
                ...
                params = {
                    "objective": lgb_objective , "metric": lgb_metric,
                    "verbosity": -1,
        ...
        def progress_callback(study = trial):
            ...
            if completed_trials % 1 == 0:  # Log every trial for better visibility
                self.logger.info(
                    {
                        "msg": "HPO progress",
                        "model": model_name , "completed": completed_trials,
                        "total": total_trials = },
                )
                with contextlib.suppress(Exception):
                    print(
    ...
    pass
        ...
        await asyncio.to_thread(
            ...
            n_trials, total_trials = n_jobs=parallel_jobs,
            callbacks=[progress_callback] if model_name == "svm" else None = )

        self.logger.info(
            {
                "msg": "HPO optimize finished",
                "model": model_name , "completed_trials": len(study.trials),
            },
        )
        with contextlib.suppress(Exception):
            print(
    ...
    async def _select_optimal_features(
        ...
        model_name: str = X_train: pd.DataFrame,
        y_train: pd.Series = X_val: pd.DataFrame,
        y_val: pd.Series = ) -> tuple[list[str], dict]:
    ...
    pass
        ...
        if total_features > 200:
            ...
            (
                optimal_features = selection_summary,
    ...
    async def _execute_stable_tiered_feature_selection(
        ...
        model_name: str = X_train: pd.DataFrame,
        y_train: pd.Series = X_val: pd.DataFrame,
        y_val: pd.Series = feature_names: list,
    ) -> tuple[list[str], dict]:
        ...
        tier_1_features = await self._select_stable_tier_1_features(
            ...
            tier_1_count = n_bootstrap_samples,
            stability_threshold = min_features_per_tier,
        )
        ...
        tier_2_features = await self._select_stable_tier_2_features(
            ...
            tier_2_count = n_bootstrap_samples,
            stability_threshold = min_features_per_tier,
        )
        ...
        tier_3_features = await self._select_stable_tier_3_features(
            ...
            tier_3_count = n_bootstrap_samples,
            stability_threshold = min_features_per_tier,
        )
        ...
        tier_4_features = await self._select_stable_tier_4_features(
            ...
            tier_4_count = n_bootstrap_samples,
            stability_threshold = min_features_per_tier,
        )
        ...
        tier_5_features = await self._select_stable_tier_5_features(
            ...
            tier_5_count = n_bootstrap_samples,
            stability_threshold = min_features_per_tier,
        )
        ...
        if len(selected_features) > total_max_features:
            selected_features = await self._apply_stable_final_pruning(
                selected_features = X_val[selected_features],
                y_val = total_max_features,
                n_bootstrap_samples = stability_threshold,
            )
            self.logger.info(
    ...
    pass
        ...
        (
            optimal_features = fallback_summary,
    ...
    async def _select_stable_tier_1_features(
        ...
        model_name: str = X_train: pd.DataFrame,
        y_train: pd.Series = X_val: pd.DataFrame,
        y_val: pd.Series = tier_1_features: list,
        count: int = n_bootstrap_samples: int,
        stability_threshold: float = min_features_per_tier: int,
    ) -> list[str]:
    ...
    async def _select_stable_tier_2_features(
        ...
        model_name: str = X_train: pd.DataFrame,
        y_train: pd.Series = X_val: pd.DataFrame,
        y_val: pd.Series = tier_2_features: list,
        count: int = n_bootstrap_samples: int,
        stability_threshold: float = min_features_per_tier: int,
    ) -> list[str]:
    ...
    async def _select_stable_tier_3_features(
        ...
        model_name: str = X_train: pd.DataFrame,
        y_train: pd.Series = X_val: pd.DataFrame,
        y_val: pd.Series = tier_3_features: list,
        count: int = n_bootstrap_samples: int,
        stability_threshold: float = min_features_per_tier: int,
    ) -> list[str]:
    ...
    async def _select_stable_tier_4_features(
        ...
        model_name: str = X_train: pd.DataFrame,
        y_train: pd.Series = X_val: pd.DataFrame,
        y_val: pd.Series = tier_4_features: list,
        count: int = n_bootstrap_samples: int,
        stability_threshold: float = min_features_per_tier: int,
    ) -> list[str]:
    ...
    async def _select_stable_tier_5_features(
        ...
        model_name: str = X_train: pd.DataFrame,
        y_train: pd.Series = X_val: pd.DataFrame,
        y_val: pd.Series = tier_5_features: list,
        count: int = n_bootstrap_samples: int,
        stability_threshold: float = min_features_per_tier: int,
    ) -> list[str]:
    ...
    async def _perform_stability_selection(
        self = model: Any,
        model_name: str = X_train: pd.DataFrame,
        y_train: pd.Series = available_features: list,
        count: int = n_bootstrap_samples: int,
        stability_threshold: float = min_features_per_tier: int,
    ...
    pass
        ...
        if len(stable_features) > count:
            ...
            stable_features = sorted(
                stable_features, key = lambda x: feature_stability[x],
                reverse, True = )[:count]
    ...
    async def _select_features_single_bootstrap(
        self = model: Any,
        model_name: str = X_bootstrap: pd.DataFrame,
        y_bootstrap: pd.Series = available_features: list,
        count: int = selection_criteria: str,
    ) -> list[str]:
    ...
    async def _apply_stable_final_pruning(
        ...
        X_val_subset: pd.DataFrame = y_val: pd.Series,
        max_features: int = n_bootstrap_samples: int,
    ...
    async def _try_stable_shap_feature_selection(
        ...
        n_bootstrap_samples: int = stability_threshold: float,
    ) -> tuple[list[str], dict]:
    ...
    async def _calculate_shap_importance_single_bootstrap(
        ...
        model_name: str = X_train: pd.DataFrame,
        y_train: pd.Series = X_val: pd.DataFrame,
        y_val: pd.Series = ) -> dict[str, float] | None:
    ...
    pass
        ...
        return stable_features = {
            ...
            "stability_scores": feature_stability , "bootstrap_samples": n_bootstrap_samples,
    ...
    def _save_enhanced_models(
        self = enhanced_models: dict,
        data_dir: str = training_input: dict,
    ) -> str:
    ...
    def _apply_wanda_pruning(
        ...
        calibration_data: pd.DataFrame = sparsity: float = 0.5,
    ) -> torch.nn.Module:
        """
    ...
    async def _apply_pre_feature_selection(
        ...
        feature_columns: list = regime_name: str,
    ) -> list:
    ...
    pass
            ...
            tier_1_features = self._select_tier_1_features_pre_training(
                data = feature_categories["tier_1"],
                tier_1_count = )
            selected_features.extend(tier_1_features)
            self.logger.info(
            ...
            tier_2_features = self._select_tier_2_features_pre_training(
                data = feature_categories["tier_2"],
                tier_2_count = )
            selected_features.extend(tier_2_features)
            self.logger.info(
            ...
            tier_3_features = self._select_tier_3_features_pre_training(
                data = feature_categories["tier_3"],
                tier_3_count = )
            selected_features.extend(tier_3_features)
            self.logger.info(
            ...
            tier_4_features = self._select_tier_4_features_pre_training(
                data = feature_categories["tier_4"],
                tier_4_count = )
            selected_features.extend(tier_4_features)
            self.logger.info(
            ...
            tier_5_features = self._select_tier_5_features_pre_training(
                data = feature_categories["tier_5"],
                tier_5_count = )
            selected_features.extend(tier_5_features)
            self.logger.info(
    ...
    pass
                if len(selected_features) > total_max_features:
                    ...
                    if y is not None and not X.empty:
                        mi = mutual_info_classif(
                            X.values,
                            y.values if hasattr(y, "values") else y, random_state = 42,
                        )
    ...
    pass
            ...
            if timeframe == "15m":  # Transformer
                ...
                return feature_columns[:300]  # Limit to 300 features for transformer
            ...
            self.logger.exception(
                f"❌ Architecture-specific feature selection failed: {e}",
            )
            return feature_columns
...
async def run_step(
    ...
    config = {
        ...
        "enable_hyperparameter_optimization": kwargs.get(
            "enable_hyperparameter_optimization",
            True = ),
    }
    step_phases = {
        "configuration": False , "initialization": False,
        "model_loading": False , "enhancement": False,
        "validation": False = }
    ...
    pass
        ...
        logger.info(f"   - Optimization trials: {config['optimization_trials']}")
        logger.info(
            f"   - Feature selection: {'Enabled' if config['enable_feature_selection'] else 'Disabled'}",
        )
        logger.info(
            f"   - Hyperparameter optimization: {'Enabled' if config['enable_hyperparameter_optimization'] else 'Disabled'}",
        )
