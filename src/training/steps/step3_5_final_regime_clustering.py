...
class FinalRegimeClusteringStep:
    ...
    @secure_step_execution
    @validate_pipeline_step
    @handle_errors(exceptions=(Exception,), default_return=False, context="regime_clustering_execution")
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=pd.DataFrame(), context="prepare_features_with_optimized_params"
    )
    @monitor_feature_engineering()
    @validate_data_structure
    async def _prepare_features_with_optimized_params(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.info(
                f"✅ Features prepared with optimized parameters: {len(clustering_features.columns)} features"
            )
    ...
    async def _analyze_regime_characteristics(
        self, clustering_results: dict[str, Any], data: pd.DataFrame
    ) -> dict[str, Any]:
        ...
        try:
            ...
            for cluster_id in unique_clusters:
                ...
                features[cluster_mask] if not features.empty else pd.DataFrame()
    ...
    async def _generate_comprehensive_reports(
        self, clustering_results: dict[str, Any], regime_analysis: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if clustering_results:
                reports["clustering_summary"] = {
                    ...
                    "clustering_score": (
                        getattr(clustering_results.get("model"), "inertia_", 0)
                        if clustering_results.get("model")
                        else 0
                    ),
                }
    ...
    async def _save_final_results(
        self, clustering_results: dict[str, Any], regime_analysis: dict[str, Any], reports: dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            with open(clustering_file, "w") as f:
                ...
                if "state_sequence" in serializable_results.get("hmm_results", {}):
                    serializable_results["hmm_results"]["state_sequence"] = serializable_results["hmm_results"][
                        "state_sequence"
                    ].tolist()
...
@handle_errors(exceptions=(Exception,), default_return=False, context="step3_5_final_regime_clustering")
@secure_step_execution
