...
meta_labeling_system = PipelineStandards.safe_import(
    "src.analyst.meta_labeling_system", None
)
...
class LabelingStep:
    ...
    def _initialize_components(self) -> None:
        ...
        self.auto_recalculate_hmm_barriers = bool(
            labeling_cfg.get("auto_recalculate_hmm_barriers", True)
        )
        self.regime_col = str(
            labeling_cfg.get("hmm_barrier_regime_column", "hmm_regime")
        )
        ...
        self.logger.info(
            f"   - Auto recalculate HMM barriers: {self.auto_recalculate_hmm_barriers}"
        )
        ...
        try:
            ...
            self.regime_barrier_optimizer = RegimeSpecificTripleBarrierOptimizer(
                self.config
            )
            self.logger.info(
                "✅ RegimeSpecificTripleBarrierOptimizer initialized successfully"
            )
        except Exception as e:
            self.logger.warning(
                f"⚠️ Could not initialize RegimeSpecificTripleBarrierOptimizer: {e}"
            )
            self.regime_barrier_optimizer = None
        ...
        if meta_labeling_system is not None:
            try:
                self.meta_labeling_system = meta_labeling_system.MetaLabelingSystem(
                    self.config
                )
    ...
    async def execute_labeling(
        ...
        try:
            ...
            triple_barrier_path = (
                Path(data_dir)
                / "training"
                / f"{exchange}_{symbol}_{timeframe}_triple_barrier_labels.parquet"
            )
            if not triple_barrier_path.exists():
                self.logger.error(
                    f"❌ Triple barrier labels not found at {triple_barrier_path}"
                )
                return False

            self.logger.info(
                f"📁 Loading triple barrier labels from {triple_barrier_path}"
            )
            ...
            labeled_data = await self._generate_comprehensive_labels(
            ...
            output_path = (
                labeled_dir / f"{exchange}_{symbol}_{timeframe}_labeled_data.parquet"
            )
            ...
            metadata_path = (
                labeled_dir / f"{exchange}_{symbol}_{timeframe}_labeling_metadata.json"
            )
            metadata = {
                "symbol": symbol,
                "exchange": exchange,
                ...
                "triple_barrier_distribution": labeled_data["triple_barrier_label"]
                .value_counts()
                .to_dict(),
    ...
    async def _log_step5_artifacts_and_report(
        ...
        try:
            ...
            metrics_calculated = {
                ...
                "labeled_samples": (
                    len(labeled_data[labeled_data["label"].notna()])
                ...
                "triple_barrier_distribution": (
                    ...
                    if labeled_data is not None
                    and "triple_barrier_label" in labeled_data.columns
                    else {}
                ),
            }
            ...
            step_data = {
                ...
                "data_shape": (
                    list(labeled_data.shape) if labeled_data is not None else []
                ),
                "label_columns": (
                    list(labeled_data.columns) if labeled_data is not None else []
                ),
            }
            ...
            if metadata_path.exists():
                ...
                self.logger.info(
                    f"✅ Logged labeling metadata: {metadata_artifact_name}"
                )
    ...
    async def _generate_comprehensive_labels(
        ...
        """Generate comprehensive labels combining multiple labeling strategies with
        regime-aware triple barrier method.
        ...
        try:
            ...
            if "triple_barrier_label" not in result_data.columns:
                self.logger.info(
                    "🔄 Triple barrier labels not found, generating them using regime-aware methods..."
                )

                # Primary Path: Attempt regime-aware labeling
                if (
                    self.regime_barrier_optimizer is not None
                    and self.auto_recalculate_hmm_barriers
                ):
                    try:
                        self.logger.info(
                            "🚀 Attempting regime-aware triple barrier labeling..."
                        )

                        # Check if we have regime information
                        if self.regime_col in result_data.columns:
                            self.logger.info(
                                f"✅ Found regime column: {self.regime_col}"
                            )
                            ...
                            if regime_labels is not None:
                                ...
                                self.logger.info(
                                    "✅ Generated regime-aware triple barrier labels"
                                )
                            else:
                                raise Exception("Regime-aware labeling failed")
                        else:
                            self.logger.warning(
                                f"⚠️ Regime column '{self.regime_col}' not found"
                            )
                    ...
                    if not self.auto_recalculate_hmm_barriers:
                        self.logger.error(
                            "❌ Auto-calculation disabled for regime-aware labeling"
                        )
                    if self.regime_barrier_optimizer is None:
                        self.logger.error("❌ Regime barrier optimizer not available")
                    self.logger.error(
                        "❌ Regime-aware labeling is required - no fallback available"
                    )
                    return None
            ...
            if self.meta_labeling_system:
                try:
                    ...
                    analyst_labels = (
                        await self.meta_labeling_system._generate_analyst_labels(
                            data, symbol, exchange, timeframe
                        )
                    )
            ...
            result_data["label_confidence"] = await self._calculate_label_confidence(
                result_data
            )
            result_data["label_source"] = await self._determine_label_source(
                result_data
            )

            self.logger.info(
                f"✅ Generated comprehensive labels with {len(result_data.columns)} columns"
            )
            self.logger.info(
                f"   - Label distribution: {result_data['label'].value_counts().to_dict()}"
            )
            self.logger.info(
                f"   - Labeling method used: {result_data.get('labeling_method', 'unknown')}"
            )
    ...
    async def _create_composite_label(self, data: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            if "analyst_label" in data.columns:
                ...
                analyst_override_mask = (data["analyst_label"] != 0) & (
                    data["triple_barrier_label"] == 0
                )
                composite_label[analyst_override_mask] = data["analyst_label"][
    ...
    async def _calculate_label_confidence(self, data: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            if "analyst_label" in data.columns:
                agreement_mask = (data["label"] == data["analyst_label"]) & (
        ...
        try:
            ...
            for idx in range(len(data)):
                if data["label"].iloc[idx] == data["triple_barrier_label"].iloc[idx]:
                    if (
                        "analyst_label" in data.columns
                        and data["label"].iloc[idx] == data["analyst_label"].iloc[idx]
                    ):
    ...
    async def _generate_regime_aware_labels(
        ...
        """Generate regime-aware triple barrier labels using
        RegimeSpecificTripleBarrierOptimizer."""
        try:
            if self.regime_barrier_optimizer is None:
                self.logger.error("❌ Regime barrier optimizer not available")
            ...
            if self.regime_col not in data.columns:
                self.logger.error(
                    f"❌ Regime column '{self.regime_col}' not found in data"
                )
                return None
            ...
            if missing_columns:
                self.logger.error(
                    f"❌ Missing required columns for triple barrier labeling: {missing_columns}"
                )
                return None
...
if __name__ == "__main__":
    ...
    async def test():
        success = await run_step(
            symbol="ETHUSDT", exchange="BINANCE", timeframe="1m", data_dir="data_cache"
        )
