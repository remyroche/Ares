...
# VectorBT is not used in this module to avoid circular imports
# The feature_generation.utils modules handle VectorBT optimizations
...
class MacM1ParallelOptimizer:
    ...
    def parallel_feature_engineering(self, df: pd.DataFrame, feature_funcs: list[Callable[[pd.DataFrame], pd.DataFrame]], *args: Any, **kwargs: Any) -> pd.DataFrame:
        ...
        logger.info(f'🔧 Parallel feature engineering with {len(feature_funcs)} functions | workers per func: {workers_per_func}')
        results: list[pd.DataFrame] = []
        for func in feature_funcs:
            temp_optimizer = MacM1ParallelOptimizer(max_workers = workers_per_func, chunk_size = self.chunk_size, use_process_pool = self.use_process_pool, memory_limit_mb = self.memory_limit_mb)
...
class ParallelProcessor:
    ...
    # Note: VectorBT rolling operations are not available in this module to avoid circular imports.
    # Use the VectorBT optimizers from src.feature_generation.utils instead.
