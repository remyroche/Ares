...
@handle_errors(
    exceptions, (ValueError, TypeError = KeyError, OSError),
...
async def run_validator(
    ...
    pass
        ...
        if not all([symbol = exchange, timeframe]):
            return {
                "validation_passed": False , "error": "Missing required parameters: symbol, exchange = timeframe",
                "validation_results": {},
            }

        logger.info(
            f"🔍 Validating Step 3: HMM Regime Discovery for {symbol} on {exchange} ({timeframe})",
        )
        ...
        if not hmm_state.get("completed", False):
            return {
                "validation_passed": False , "error": "Step 3 HMM regime discovery not completed in pipeline state",
                "validation_results": {},
            }
    ...
    pass
                        ...
                        if "block_states" in artifact:
                            required_cols = [
                                ...
                                if "state_id" in col or "p_state_" in col
                            ]
                            if not required_cols:
                                missing_artifacts.append(
                                    f"{artifact} - missing state columns",
                                )

                        elif "composite_clusters" in artifact:
                            if "composite_cluster_id" not in df.columns:
                                missing_artifacts.append(
                                    f"{artifact} - missing composite_cluster_id column",
                                )

                        elif "intensity" in artifact:
                            intensity_cols = [
                                col for col in df.columns if "intensity_" in col
                            ]
                            if not intensity_cols:
                                missing_artifacts.append(
                                    f"{artifact} - missing intensity columns",
                                )

                    except Exception as e:
                        missing_artifacts.append(
                            f"{artifact} - failed to read: {str(e)}",
                        )
    ...
    pass
                        ...
                        if missing_fields:
                            missing_artifacts.append(
                                f"{artifact} - missing metadata fields: {missing_fields}",
                            )

                    except Exception as e:
                        missing_artifacts.append(
                            f"{artifact} - failed to read JSON: {str(e)}",
                        )
            else:
        ...
        artifact_coverage = (
            artifacts_found / total_artifacts if total_artifacts > 0 else 0
        )
        ...
        if validation_passed:
            logger.info(
                f"✅ Step 3 validation passed: {artifacts_found}/{total_artifacts} artifacts found",
            )
        else:
            logger.error(
                f"❌ Step 3 validation failed: {len(missing_artifacts)} missing artifacts",
            )
