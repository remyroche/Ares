...
def process_ohlcv_entropy_bars(
    ...
    # Pre-calculate counts for the first window
    # Window range: [1, window_size] (indices)
    counts = np.zeros(alphabet_size, dtype=np.int32)
    # Start from index 1 because log_returns[0] is 0
    current_window_start = 1
    current_window_end = window_size + 1  # exclusive

    if current_window_end > n_minutes:
        return np.array(bar_indices)

    for k in range(current_window_start, current_window_end):
        counts[symbols[k]] += 1

    inv_n = 1.0 / window_size
    inv_log2 = 1.0 / np.log(2.0)

    # Core loop starting from window_size
    for i in range(window_size, n_minutes):
        # Calculate entropy from counts (O(alphabet_size) instead of O(window_size))
        entropy_val = 0.0
        ...
        # Update counts for next iteration (i -> i+1)
        # We are moving from window ending at i (indices [i-window_size+1, i])
        # to window ending at i+1 (indices [i-window_size+2, i+1])
        if i + 1 < n_minutes:
            old_idx = i - window_size + 1
            new_idx = i + 1

            counts[symbols[old_idx]] -= 1
            counts[symbols[new_idx]] += 1

    return np.array(bar_indices)


def get_15min_threshold(
    close_prices: np.ndarray, n_bins: int = 10, window_size: int = 100
...
def generate_entropy_bars_from_ohlcv(
    ...
    tprint_info(
        f"🔧 Entropy bar calibration: n_bins={n_bins}, threshold={bar_threshold:.4f}"
    )

    # Generate entropy bar indices
    bar_indices = process_ohlcv_entropy_bars(
        close_prices, thresholds, window_size, bar_threshold
    )
    ...
    tprint_info(
        f"📊 Average bars per hour: {len(result_df) / (len(result_df) * target_minutes / 60):.2f}"
    )
...
def calculate_specialized_entropy_features(
    ...
    if use_optimized and OPTIMIZED_AVAILABLE:
        ...
        return vectorized_entropy_features(
            df=entropy_bars, entropy_bars=entropy_bars, config=config, use_numba=True
        )
    ...
    if base_model_updates is not None and not base_model_updates.empty:
        ...
        features["staleness_seconds"] = (
            entropy_bars.index - last_update_time
        ).total_seconds()
    ...
    if specialist_prices is not None and len(specialist_prices) > 0:
        ...
        aligned_specialist = specialist_prices.reindex(
            entropy_bars.index, method="ffill"
        )
        if not aligned_specialist.isna().all():
            features["drift_proxy"] = (
                entropy_bars["close"] - aligned_specialist
            ) / aligned_specialist
        else:
    ...
    if OPTIMIZED_AVAILABLE:
        features["lz_complexity"] = lempel_ziv_complexity_numba(
            entropy_bars["close"].values
        )
    else:
    ...
    if "entropy_contribution" in entropy_bars.columns:
        ...
        time_diff = (
            pd.Series(entropy_bars.index).diff().dt.total_seconds().fillna(60)
        )  # Default 60 seconds
    ...
    if "entropy_contribution" in entropy_bars.columns:
        ...
        features["entropy_zscore"] = (
            entropy_bars["entropy_contribution"] - features["entropy_ma"]
        ) / (features["entropy_std"] + 1e-9)
