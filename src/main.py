...
async def initialize_system() -> bool:
    """Initialize the trading system components, forcing LIVE mode."""
    try:
        system_logger.info("🚀 Initializing Ares Trading Bot in LIVE mode...")

        # Force trading environment to LIVE for this execution path
        settings.trading_environment = "LIVE"
        system_logger.info(
        ...
        system_logger.info("✅ Model manager initialized")
...
@handle_errors(
    exceptions=(Exception,), default_return=False, context="start_supervisor"
)
...
async def main():
    """Main entry point for the Ares trading bot (LIVE mode)."""
    try:
