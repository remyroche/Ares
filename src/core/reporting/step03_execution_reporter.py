...
class Step03ExecutionReporter:
    ...
    def __init__(self,
        ...
        self.performance_thresholds = {
            'max_function_duration': 30.0,  # seconds
    ...
    def _analyze_function_calls(self, raw_calls: List[Dict[str, Any]]) -> List[FunctionCallSummary]:
        ...
        # Convert to FunctionCallSummary objects
        summaries = []
        for key, data in function_summaries.items():
            ...
            summary = FunctionCallSummary(
                ...
                success_rate=data['success_count'] / data['call_count'] if data['call_count'] > 0 else 0.0,
                total_memory_used=sum(memory_usage),
                avg_memory_per_call=np.mean(memory_usage) if memory_usage else 0.0,
                total_cpu_used=sum(cpu_usage),
                avg_cpu_per_call=np.mean(cpu_usage) if cpu_usage else 0.0,
                error_types=list(data['error_types']),
                performance_warnings=list(set(data['performance_warnings']))
            )
    ...
    def _calculate_performance_metrics(self, 
        ...
        # Memory and CPU efficiency scores
        ...
        return PerformanceMetrics(
            ...
            total_memory_peak=max((call.total_memory_used for call in function_calls), default=0.0),
            total_cpu_usage=sum(call.total_cpu_used for call in function_calls),
            function_calls_count=total_calls,
            nested_calls_count=nested_calls,
            max_call_depth=max_call_depth,
            avg_call_duration=total_duration / total_calls if total_calls > 0 else 0.0,
            slowest_function=slowest_function.function_name if slowest_function else "N/A",
            fastest_function=fastest_function.function_name if fastest_function else "N/A",
    ...
    def _calculate_memory_efficiency(self, function_calls: List[FunctionCallSummary]) -> float:
        ...
        # Penalize functions that use excessive memory
        efficiency_scores = []
        for call in function_calls:
            if call.avg_memory_per_call > self.performance_thresholds['max_memory_usage']:
                efficiency_scores.append(0.0)
            else:
                # Score based on memory usage relative to threshold
    ...
    def _calculate_cpu_efficiency(self, function_calls: List[FunctionCallSummary]) -> float:
        ...
        # Penalize functions that use excessive CPU
        efficiency_scores = []
        for call in function_calls:
            if call.avg_cpu_per_call > self.performance_thresholds['max_cpu_usage']:
                efficiency_scores.append(0.0)
            else:
                # Score based on CPU usage relative to threshold
    ...
    def _calculate_overall_performance_score(self, function_calls: List[FunctionCallSummary]) -> float:
        ...
        # Factors: success rate, duration efficiency, memory efficiency, CPU efficiency
        success_rate_score = np.mean([call.success_rate for call in function_calls]) * 100
        
        # Duration efficiency (penalize slow functions)
        duration_scores = []
        for call in function_calls:
        ...
        overall_score = (
            ...
            duration_efficiency_score * 0.3 +
            memory_efficiency_score * 0.15 +
            cpu_efficiency_score * 0.15
        )
    ...
    def _analyze_errors(self, raw_errors: List[Dict[str, Any]]) -> ErrorAnalysis:
        ...
        return ErrorAnalysis(
            ...
            critical_errors=[e.get('error_id', 'unknown') for e in critical_errors],
    ...
    def _detect_error_patterns(self, raw_errors: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for func_name, errors in function_errors.items():
            if len(errors) >= 3:  # At least 3 errors
                patterns.append({
                    'pattern_type': 'frequent_errors',
                    'function_name': func_name,
                    'error_count': len(errors),
                    'error_categories': [e.get('error_category', 'unknown') for e in errors],
                    'severity': max(e.get('severity', 'medium') for e in errors),
                    'recommendation': f"Function {func_name} has {len(errors)} recent errors"
                })
    ...
    def _calculate_quality_metrics(self, 
        ...
        # Algorithm performance score (based on performance metrics)
        algorithm_performance_score = performance_metrics.overall_performance_score
        
        # Error handling score (based on error recovery rate)
        error_handling_score = error_analysis.error_recovery_rate
        
        # Resource utilization score (based on memory and CPU efficiency)
        ...
        overall_quality_score = (
            ...
            algorithm_performance_score * 0.3 +
            error_handling_score * 0.2 +
            resource_utilization_score * 0.2
        )
        ...
        return QualityMetrics(
            ...
            algorithm_performance_score=algorithm_performance_score,
    ...
    async def generate_report(self,
                            ...
                            data_directory: str,
                            start_time: datetime,
                            end_time: datetime) -> Step03ExecutionReport:
    ...
    async def _save_json_report(self, report: Step03ExecutionReport, timestamp: str) -> Path:
        ...
        report_dict = {
            ...
            'function_calls': [
                {
                    ...
                    'avg_duration': call.avg_duration,
                    'min_duration': call.min_duration,
                    'max_duration': call.max_duration,
                    'success_count': call.success_count,
                    'failure_count': call.failure_count,
                    'success_rate': call.success_rate,
                    'total_memory_used': call.total_memory_used,
                    'avg_memory_per_call': call.avg_memory_per_call,
                    'total_cpu_used': call.total_cpu_used,
                    'avg_cpu_per_call': call.avg_cpu_per_call,
            ...
            'performance_metrics': {
                ...
                'total_memory_peak': report.performance_metrics.total_memory_peak,
                'total_cpu_usage': report.performance_metrics.total_cpu_usage,
                'function_calls_count': report.performance_metrics.function_calls_count,
                'nested_calls_count': report.performance_metrics.nested_calls_count,
                'max_call_depth': report.performance_metrics.max_call_depth,
                'avg_call_duration': report.performance_metrics.avg_call_duration,
                'slowest_function': report.performance_metrics.slowest_function,
                'fastest_function': report.performance_metrics.fastest_function,
                'memory_efficiency_score': report.performance_metrics.memory_efficiency_score,
                'cpu_efficiency_score': report.performance_metrics.cpu_efficiency_score,
                'overall_performance_score': report.performance_metrics.overall_performance_score
            },
            'error_analysis': {
                'total_errors': report.error_analysis.total_errors,
                'errors_by_category': report.error_analysis.errors_by_category,
                'errors_by_severity': report.error_analysis.errors_by_severity,
                'most_common_errors': report.error_analysis.most_common_errors,
                'error_recovery_rate': report.error_analysis.error_recovery_rate,
                'critical_errors': report.error_analysis.critical_errors,
                'error_patterns': report.error_analysis.error_patterns,
                'recommendations': report.error_analysis.recommendations
            },
            'quality_metrics': {
                'data_quality_score': report.quality_metrics.data_quality_score,
                'algorithm_performance_score': report.quality_metrics.algorithm_performance_score,
                'error_handling_score': report.quality_metrics.error_handling_score,
                'resource_utilization_score': report.quality_metrics.resource_utilization_score,
                'overall_quality_score': report.quality_metrics.overall_quality_score,
                'quality_issues': report.quality_metrics.quality_issues,
                'quality_improvements': report.quality_metrics.quality_improvements
            },
    ...
    async def _save_csv_report(self, report: Step03ExecutionReport, timestamp: str) -> Path:
        ...
        perf_df = pd.DataFrame([{
            ...
            'metric': 'total_memory_peak',
            'value': report.performance_metrics.total_memory_peak
        }, {
            'metric': 'total_cpu_usage',
            'value': report.performance_metrics.total_cpu_usage
        }, {
            'metric': 'function_calls_count',
            'value': report.performance_metrics.function_calls_count
        }, {
        ...
        quality_df = pd.DataFrame([{
            ...
            'metric': 'algorithm_performance_score',
            'value': report.quality_metrics.algorithm_performance_score
        }, {
            'metric': 'error_handling_score',
            'value': report.quality_metrics.error_handling_score
        }, {
            'metric': 'resource_utilization_score',
            'value': report.quality_metrics.resource_utilization_score
        }, {
