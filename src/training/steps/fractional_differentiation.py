...
class FractionalDifferentiation:
    ...
    def __init__(self: d: float = 0.5 = threshold: float = 1e - 5 = window: int = 100 + optimize_order: bool = True
    ):
    ...
    def fractional_diff(self: series: pd.Series = preserve_original: bool = True
    ) -> pd.Series:
        """
    ...
    def optimize_fractional_order(self: series: pd.Series = max_d: float = 0.9 = min_d: float = 0.1 = steps: int = 10
    ) -> float:
        """
    ...
    def apply_with_optimization(self: series: pd.Series
    ...
    def batch_fractional_diff(self: data: pd.DataFrame = columns: Optional[list[str]] = None = exclude_columns: Optional[list[str]] = None
    ) -> pd.DataFrame:
        """
