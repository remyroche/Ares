...
This module implements multi-output training for probability outputs, replacing the
post-training calculation approach with direct training on probability targets.
"""
...
class ProbabilityTargetGenerator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.magnitude_threshold_factor = self.config.get(
    ...
    @handle_errors(
        default_return=np.array([]), context="generate_triple_barrier_targets"
    )
    @comprehensive_validation()
    def generate_triple_barrier_targets(
        ...
        for i in range(len(X)):
            if i >= len(market_data) - self.look_ahead_periods:
                ...
                future_prices = market_data["close"].iloc[
                    i + 1 : i + self.look_ahead_periods + 1
                ]
    ...
    @handle_errors(
        default_return=np.array([]), context="generate_barrier_avoidance_targets"
    )
    @comprehensive_validation()
        ...
        for i in range(len(X)):
            if i >= len(market_data) - self.avoidance_look_ahead:
                ...
                future_returns = (
                    market_data["close"]
                    .pct_change()
                    .iloc[i + 1 : i + self.avoidance_look_ahead + 1]
                )
    ...
    def generate_all_targets(
        self, X: np.ndarray, y: np.ndarray, market_data: pd.DataFrame
    ) -> Dict[str, np.ndarray]:
        ...
        targets = {
            ...
            "barrier_avoidance": self.generate_barrier_avoidance_targets(
                X, y, market_data
            ),
        }
...
class MultiOutputModel:
    ...
    This class manages individual models for each probability type and provides ensemble
    capabilities with optimized weights.
    """
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.model_architectures = self.config.get(
            "model_architectures",
            {
    ...
    def _create_model(self, output_type: str):
        ...
        if model_type.lower() in ["lightgbm", "lgb"]:
            ...
            # Default to LightGBM
            self.logger.warning(
                f"Unknown model type '{model_type}', defaulting to LightGBM"
            )
            return lgb.LGBMClassifier(
                n_estimators=self.n_estimators,
                learning_rate=self.learning_rate,
    ...
    def fit(
        ...
        for output_type in [
            ...
            if output_type in ["triple_barrier", "barrier_avoidance"]:
                ...
                try:
                    class_weights = compute_class_weight(
                        "balanced", classes=np.unique(y_train_target), y=y_train_target
                    )
                    sample_weights = class_weights[y_train_target.astype(int)]
                except Exception as e:
                    self.logger.warning(
                        f"Could not compute class weights for {output_type}: {e}"
                    )
            ...
            try:
                if hasattr(model, "fit"):
                    ...
                    if isinstance(model, NeuralNetworkWrapper):
                        ...
                        if sample_weights is not None:
                            model.fit(
                                X_train, y_train_target, sample_weight=sample_weights
                            )
                        else:
                            model.fit(X_train, y_train_target)
                        ...
                        try:
                            ...
                            self.logger.warning(
                                f"Calibration failed for {output_type}, using original model: {e}"
                            )
                            trained_models[output_type] = model
                else:
                ...
                self.logger.warning(
                    f"Skipping {output_type} model due to training failure"
                )
                continue
        ...
        def objective(weights):
            ...
            for i, output_type in enumerate(
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Error calculating loss for {output_type}: {e}"
                    )
    ...
    def predict_probabilities(
        ...
        for output_type in [
            ...
            if output_type not in self.models or self.models[output_type] is None:
                self.logger.warning(
                    f"Model for {output_type} not available, using default probability"
                )
...
from src.core.decorators import handles_errors
    ...
    def _configure_models_for_timeframe(self):
        ...
        if self.timeframe in self.model_architectures:
            ...
            self.logger.info(
                f"Configuring models for {self.timeframe} timeframe using {model_type}"
            )
            ...
            if model_type in ["tcn", "cnn", "transformer", "lstm", "gru"]:
                self.neural_config[model_type] = NEURAL_MODEL_CONFIGS.get(
                    model_type, {}
                )
        else:
            self.logger.warning(
                f"No specific model configuration for timeframe {self.timeframe}, using defaults"
            )
