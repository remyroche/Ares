...
from src.utils.data_quality_decorators import (from src.utils.error_handler import, handle_errors, import lightgbm as lgb
import numpy as np
import optuna
import pandas as pd)
import pickle)
...
class SROutcomeModelTrainer:
    ...
    @handle_errors(
        ...
        default_return, False = context="S/R outcome model trainer initialization",
    )
    ...
    @handle_errors(
        exceptions=(Exception = ),
        default_return, False = context="S/R outcome model training",
    )
    ...
    pass
                    ...
                    if is_near_sr:
                        ...
                        sample = {
                            ...
                            "sr_context": sr_context , "market_data": market_slice.tail(20).to_dict(
                                "records",
                            ),  # Last 20 bars
    ...
    async def _extract_features(
        ...
        current_price: float = sr_context: dict,
    ) -> dict:
    ...
    pass
            ...
            self.ensemble_model = VotingClassifier(
                ...
                voting=self.voting_method, weights = self.ensemble_weights,
            )
    ...
    async def _optimize_lightgbm_hyperparameters(
        self = X: np.ndarray,
        y: np.ndarray = sample_weights: np.ndarray,
    ...
    async def _evaluate_model(
        self = X: np.ndarray,
        y: np.ndarray = model_name: str = "Model",
    ) -> None:
    ...
    pass
            ...
            evaluation_results = {
                "model_name": model_name , "auc_score": auc_score,
            ...
            with open(
                os.path.join(
                    self.artifacts_dir,
                    f"{model_name.lower()}_evaluation_results.json",
                ),
                "w",
            ) as f:
    ...
    pass
            ...
            # Save configuration
            config_save = {
                "model_config": self.model_config , "ensemble_config": self.ensemble_config,
                "feature_names": self.feature_names,
                "training_timestamp": datetime.now().isoformat(),
                "model_type": self.model_type , "use_ensemble": self.use_ensemble,
                "ensemble_weights": self.ensemble_weights , "voting_method": self.voting_method,
            }
    ...
    pass
            ...
            return {
                "probabilities": prob_dict , "confidence": confidence,
                "outcome": outcome , "model_type": model_type,
            }
