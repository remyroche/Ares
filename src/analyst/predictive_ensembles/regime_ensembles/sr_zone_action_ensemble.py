...
class SRZoneActionEnsemble(BaseEnsemble):
    ...
    def _train_base_models(self, aligned_data: pd.DataFrame, y_encoded: np.ndarray):
        ...
        self.models["sr_lgbm"] = self._train_with_smote(LGBMClassifier(**lgbm_params, random_state=42, verbose=-1), X_flat, y_encoded)
    ...
    def _get_meta_features(self, df: pd.DataFrame, is_live: bool = False, **kwargs) -> pd.DataFrame or dict:
        ...
        if is_live:
            ...
            if self.models.get("sr_lgbm"):
                meta_features['sr_lgbm_prob'] = np.max(self.models["sr_lgbm"].predict_proba(current_row_flat))
            if self.models.get("sr_tabnet"):
                meta_features['sr_tabnet_prob'] = np.max(self.models["sr_tabnet"].predict_proba(current_row_flat.values))
            if self.models.get("order_flow_lgbm"):
                meta_features['of_lgbm_prob'] = np.max(self.models["order_flow_lgbm"].predict_proba(current_row_of))
            ...
            if self.models.get("sr_lgbm"):
                meta_df['sr_lgbm_prob'] = np.max(self.models["sr_lgbm"].predict_proba(X_flat), axis=1)
            if self.models.get("sr_tabnet"):
                meta_df['sr_tabnet_prob'] = np.max(self.models["sr_tabnet"].predict_proba(X_flat.values), axis=1)
            if self.models.get("order_flow_lgbm"):
                meta_df['of_lgbm_prob'] = np.max(self.models["order_flow_lgbm"].predict_proba(X_of), axis=1)
