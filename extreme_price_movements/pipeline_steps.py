...
def _meta_feature_keys_union(cfg) -> set[str]:
    ...
    return {
        ...
        if isinstance(k, str)
        and k
        and k not in _DISABLED_PIPELINE_FEATURE_KEYS
    }
...
def _expected_feature_keys_from_cfg(cfg) -> set[str]:
    ...
    return {
        ...
        for k in keys
        if str(k)
        and str(k) not in _DISABLED_PIPELINE_FEATURE_KEYS
    }
    ...
    if use_chunked_backfill:
        ...
        for ci, start in enumerate(range(0, len(all_syms), chunk_size), start=1):
            ...
            for bi, key_batch in enumerate(key_batches, start=1):
                ...
                feats_chunk, feat_index, feat_columns = (
                ...
                if batch_backfill_keys and unresolved:
                    missing_critical = [
                        k
                        for k in unresolved
                        if k in FEATURE_HEALTH_CRITICAL_KEYS
                    ]
                    if missing_critical:
                        raise RuntimeError(
