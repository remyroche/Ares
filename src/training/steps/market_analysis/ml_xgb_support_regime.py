...
class MLBreakoutBounceRegimeStep(BaseStep):
    ...
    def _create_breakout_labels(self, df: pd.DataFrame, config: Dict[str, Any]) -> pd.Series:
        """Support-focused Break vs Bounce labels.

        0: Bounce / No Break (support holds)
        1: Break (support fails)

        Only interactions with support levels are labeled; all other bars
        (including resistance contexts, pure chop, traps, etc.) are left
        unlabeled and later dropped.
        """
        ...
        # Moves relative to support level
        ...
        # Optional touch filter: require that price is reasonably close to the
        # support level at decision time to be considered an interaction.
        # Default to a relatively loose band so we capture more interactions;
        # configs can still tighten this via support_touch_band_pct.
        touch_band_default = max(cross_buf, 0.0040)
        touch_band = float(config.get("support_touch_band_pct", touch_band_default))
        touch_dist = (primary - close).abs() / primary
        near_support = is_support & (touch_dist <= touch_band)

        # Break: support fails decisively
        sup_break = near_support & (down_move_cross >= cross_buf) & (down_move_hold >= hold_buf)

        # Bounce: price moves away from support without meeting break criteria
        sup_bounce = near_support & (up_move_cross >= bounce_move) & ~sup_break

        labels[sup_break] = 1.0  # Break
        labels[sup_bounce] = 0.0  # Bounce / Hold
    ...
    def _train_breakout_classifier(
        ...
        # ====================================================================
        # STEP 3 (disabled for support regime): label refinement using
        # event_min_move. For the support Break-vs-Bounce model, we keep all
        # labeled events to maximize density and let the model learn the
        # effective threshold.
        # ====================================================================
        # (Intentionally no-op for this step)
        ...
        # ====================================================================
        # STEP 4 (disabled for support regime): regime-based training filter.
        # For support Break-vs-Bounce we train on all available labeled
        # interactions to avoid starving the classifier of events.
        # ====================================================================
        # (Intentionally no-op for this step)
