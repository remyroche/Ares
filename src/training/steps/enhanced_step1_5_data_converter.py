...
try:
    ...
    from src.utils.enhanced_data_quality_validator import QualityThresholds, UnifiedDataQualityValidator
...
class EnhancedStep1_5DataConverter:
    ...
    async def _save_unified_data(self, unified_data: pd.DataFrame, symbol: str, exchange: str, timeframe: str) -> bool:
        ...
        try:
            ...
            partition_cols = (
                ["year", "month", "day"] if all(col in unified_data.columns for col in ["year", "month", "day"]) else []
            )
            ...
            try:
                ...
                unified_data.to_parquet(
                    os.path.join(output_dir, "data.parquet"), compression=self.config.compression, index=False
                )
...
async def run_enhanced_step1_5(
    training_input: Dict[str, Any], pipeline_state: Dict[str, Any], config: Optional[Step1_5Config] = None
) -> Dict[str, Any]:
    """
