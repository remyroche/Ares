...
## 2026-05-19 - Bulk Pandas DataFrame Updates with Index Alignment
Learning: Iterating over DataFrame rows sequentially using `.iterrows()` and updating an existing DataFrame row-by-row via `.loc` causes massive latency overhead in performance-critical sections like the Prediction Ledger's `mark_resolved` function. In benchmark testing, this pattern resulted in severe O(n²) scaling delays for incremental persistence.
Action: Replaced `.iterrows()` with a vectorized approach utilizing index alignment. Deduplicate incoming updates first, ensure columns match by filling `float("nan")` (to avoid unintended object casting), and locate overlapping rows using `upd_idx.index.intersection(old_idx.index)`. Update common values directly via column assignment, and append totally new records with `pd.concat` on `new_idx`. This provides multiple orders of magnitude speedups.
