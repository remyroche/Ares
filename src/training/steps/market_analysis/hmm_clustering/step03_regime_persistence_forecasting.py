...
class RegimePersistenceForecaster:
    ...
    def _analyze_regime_durations(self, regimes: np.ndarray) -> Dict[str, Any]:
        ...
        for i in range(1, len(regimes)):
            if regimes[i] == current_regime:
                ...
                regime_durations.append({'regime': current_regime, 'duration': current_duration, 'start_index': i - current_duration, 'end_index': i - 1})
                current_regime = regimes[i]
                current_duration = 1
        regime_durations.append({'regime': current_regime, 'duration': current_duration, 'start_index': len(regimes) - current_duration, 'end_index': len(regimes) - 1})
        durations_by_regime = {}
    ...
    def _build_persistence_models(self, duration_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for regime, stats in duration_stats.items():
            ...
            try:
                ...
                models['exponential'] = {'type': 'exponential', 'params': exp_params, 'aic': self._calculate_aic(durations, expon, exp_params)}
            except:
                pass
            try:
                gamma_params = gamma.fit(durations)
                models['gamma'] = {'type': 'gamma', 'params': gamma_params, 'aic': self._calculate_aic(durations, gamma, gamma_params)}
            except:
                pass
            try:
                weibull_params = weibull_min.fit(durations)
                models['weibull'] = {'type': 'weibull', 'params': weibull_params, 'aic': self._calculate_aic(durations, weibull_min, weibull_params)}
            except:
                pass
            if models:
    ...
    def _build_transition_models(self, regimes: np.ndarray) -> Dict[str, Any]:
        ...
        transition_models = {'transition_matrix': transition_matrix, 'regime_mapping': {i: regime for i, regime in enumerate(unique_regimes)}, 'reverse_mapping': {regime: i for i, regime in enumerate(unique_regimes)}}
        transition_stats = {}
    ...
    def _build_forecasting_models(self, data: pd.DataFrame, regimes: np.ndarray) -> Dict[str, Any]:
        ...
        if data_frequency_minutes <= 1:
            horizons = [5, 15, 30, 60]
        elif data_frequency_minutes <= 5:
            horizons = [3, 6, 12]
        elif data_frequency_minutes <= 15:
            horizons = [2, 4]
        elif data_frequency_minutes <= 30:
            horizons = [2]
        else:
            horizons = [1]
    ...
    def _prepare_forecasting_features(self, data: pd.DataFrame, regimes: np.ndarray) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if 'close' in data.columns:
                ...
                features.extend([returns.rolling(5).mean(), returns.rolling(10).mean(), returns.rolling(20).mean(), returns.rolling(5).std(), returns.rolling(10).std(), returns.rolling(20).std()])
            if 'volume' in data.columns:
                volume = data['volume']
                features.extend([volume.rolling(5).mean(), volume.rolling(10).mean(), volume.rolling(20).mean(), (volume / volume.rolling(20).mean()).rolling(5).mean()])
            if 'high' in data.columns and 'low' in data.columns:
                volatility = (data['high'] - data['low']) / data['close']
                features.extend([volatility.rolling(5).mean(), volatility.rolling(10).mean(), volatility.rolling(20).mean()])
...
import numpy as np
            ...
            analyst_predictions = analyst_predictor.predict_regime_changes(features, hmm_probs, regimes)
            return {'analyst_predictions': analyst_predictions, 'integration_success': True, 'prediction_count': len(analyst_predictions.get('predictions', [])), 'confidence_scores': analyst_predictions.get('confidence_scores', [])}
        except Exception as e:
    ...
    def forecast_regime_transitions(self, current_data: pd.DataFrame, current_regime: int, models: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _forecast_ml(self, current_data: pd.DataFrame, current_regime: int, forecast_models: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for horizon_name, model_info in forecast_models.items():
            try:
                ...
                predictions[horizon_name] = {'regime_change_probability': prediction, 'confidence': abs(prediction - 0.5) * 2}
            except Exception as e:
    ...
    def _combine_forecasts(self, forecasts: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not forecasts:
            return {'forecast': 'unknown', 'confidence': 0.0}
        weights = {'persistence': 0.3, 'transitions': 0.3, 'ml': 0.4}
    ...
    def _assess_model_quality(self, persistence_models: Dict, transition_models: Dict, forecast_models: Dict) -> Dict[str, Any]:
        ...
        if persistence_models:
            quality_scores['persistence'] = len(persistence_models) / 5.0
        else:
        ...
        if forecast_models:
            quality_scores['forecasting'] = len(forecast_models) / 5.0
        else:
