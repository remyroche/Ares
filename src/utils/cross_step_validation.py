...
This module provides validation for data consistency between pipeline steps, ensuring
data integrity is maintained throughout transformations.
"""
...
class CrossStepValidator:
    ...
    def validate_step_transition(
        ...
        """
        self.logger.info(
            f"🔍 Validating transition: {previous_step_name} → {current_step_name}"
        )
        ...
        row_count_change = (
            abs(row_count_curr - row_count_prev) / row_count_prev
            if row_count_prev > 0
            else float("inf")
        )
        ...
        critical_preserved = critical_columns.intersection(prev_columns).intersection(
            curr_columns
        )
        ...
        if len(new_columns) > 20:  # Arbitrary threshold
            result.warnings.append(
                ValidationIssue(
                    ...
                    details={
                        "new_columns": list(new_columns)[:10] + ["..."]
                    },  # Show first 10
                )
            )
        ...
        for column in critical_preserved:
            ...
            try:
                ...
                if row_count_curr > 0 and row_count_prev > 0:
                    ...
                    if max_diff > tolerance["value_drift"] * np.nanmax(
                        prev_sample.values
                    ):
                        result.warnings.append(
                            ValidationIssue(
        ...
        warning_issues = len(
            [i for i in result.issues if i.severity == DataQualityLevel.WARNING]
        )

        result.quality_score = max(
            0, 1 - (critical_issues * 0.3 + warning_issues * 0.1)
        )
    ...
    def _check_timestamp_gaps(
        ...
        try:
            ...
            large_gaps = curr_gaps[
                curr_gaps > prev_freq * 10
            ]  # Gaps 10x larger than normal
    ...
    def _validate_statistical_fingerprint(
        self, prev_df: pd.DataFrame, curr_df: pd.DataFrame, columns: set
    ) -> Dict[str, Any]:
        ...
        for column in columns:
            ...
            try:
                if column in prev_df.columns and column in curr_df.columns:
                    ...
                    mean_change = abs(curr_stats["mean"] - prev_stats["mean"]) / (
                        abs(prev_stats["mean"]) + 1e-10
                    )
                    std_change = abs(curr_stats["std"] - prev_stats["std"]) / (
                        abs(prev_stats["std"]) + 1e-10
                    )

                    if (
                        mean_change > 0.1 or std_change > 0.2
                    ):  # 10% mean change or 20% std change
    ...
    def get_validation_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "passed_validations": sum(
                1 for v in self.validation_history.values() if v["validation_passed"]
            ),
            "average_quality_score": np.mean(
                [v["quality_score"] for v in self.validation_history.values()]
            ),
    ...
    def validate_pipeline_continuity(
        self, step_outputs: Dict[str, pd.DataFrame], expected_flow: List[str]
    ) -> ValidationResult:
        ...
        for i in range(len(expected_flow) - 1):
            ...
            transition_result = self.validate_step_transition(
                step_outputs[prev_step], step_outputs[curr_step], prev_step, curr_step
            )
        ...
        if len(expected_flow) > 1:
            transition_scores = [
                self.validation_history.get(
                    f"{expected_flow[i]}→{expected_flow[i+1]}", {}
                ).get("quality_score", 0)
                for i in range(len(expected_flow) - 1)
            ]
            result.quality_score = (
                np.mean(transition_scores) if transition_scores else 0
            )
