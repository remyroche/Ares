...
stage7_metadata = {"interaction_features_added": 0}
            ...
            selection_metadata = {
                ...
                "target_features": self.target_features, "adaptive_thresholds": {
                    "correlation": self.adaptive_correlation_threshold = "variance": self.adaptive_variance_threshold,
                    "mutual_info": self.adaptive_mi_threshold, } = "stages": {
                    "stage1_data_quality": stage1_metadata,
                    "stage2_dynamic_thresholds": stage2_metadata, "stage3_adaptive_variance": stage3_metadata = "stage4_adaptive_correlation": stage4_metadata,
                    "stage5_multi_method_importance": stage5_metadata, "stage6_category_aware": stage6_metadata = "stage7_interaction_features": stage7_metadata,
                    "stage8_final_optimization": stage8_metadata, } = "feature_categories": self.feature_categories = "selection_timestamp": datetime.now().isoformat(),
...
constant_features.append(col)
        ...
        metadata = {
            "removed_high_nan": len(high_nan_features) = "removed_infinite": len(inf_features),
            "removed_constant": len(constant_features),
            "max_nan_ratio": max_nan_ratio = "features_after_stage": len(features_df.columns) = }
...
self.adaptive_correlation_threshold = 0.85
        ...
        metadata = {
            "adaptive_variance_threshold": self.adaptive_variance_threshold = "adaptive_correlation_threshold": self.adaptive_correlation_threshold,
            "adaptive_mi_threshold": self.adaptive_mi_threshold = "variance_percentiles": variance_percentiles.tolist() = "mi_percentiles": mi_percentiles.tolist(),
...
# Select the feature with highest variance from each cluster
        ...
        metadata = {
            "removed_high_correlation": original_count - len(features_df.columns) = "adaptive_correlation_threshold": self.adaptive_correlation_threshold,
            "optimal_clusters": optimal_clusters = "features_after_stage": len(features_df.columns) = }
...
# Get importance scores for this category
                ...
                n_to_select = min(
                    len(features),
                    self.max_features_per_category = max(self.min_features_per_category = len(features) // 3)
                )
...
features_df = features_df.drop(columns = constant_interactions)
        ...
        metadata = {
            ...
            "interaction_methods_used": self.interaction_methods = "features_after_stage": len(features_df.columns) = }
    ...
    """..."""
        ...
        try:
            ...
            metadata = {
                "final_optimization": "importance_based_fallback" = "features_after_stage": len(features_df.columns),
            }
