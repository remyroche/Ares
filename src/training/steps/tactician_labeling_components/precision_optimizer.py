...
class PrecisionOptimizer:
    ...
    @handles_errors(
        ...
        default_return={},
        context="regime threshold calculation"
    )
    async def get_regime_thresholds(
        ...
        thresholds = {
            ...
            "precision_threshold": self.target_precision,
    ...
    @handles_errors(
        exceptions=(Exception,),
        default_return={},
        context="threshold optimization"
    )
    async def optimize_thresholds(
        ...
        if not labeled_data.empty and 'label' in labeled_data.columns and 'signal_strength' in labeled_data.columns:
            ...
            validation = await self._validate_thresholds(
                labeled_data,
                global_thresholds
            )
            optimization_results["validation_results"] = validation
        
        # Optimize per-regime if enabled
        if self.regime_specific_optimization and 'regime_id' in labeled_data.columns:
            for regime_id in labeled_data['regime_id'].unique():
                ...
                if len(regime_labeled) > 100:  # Minimum samples for optimization
                    regime_thresholds = await self._optimize_regime_specific_thresholds(
                        regime_labeled,
                        regime_id
                    )
        ...
        optimization_results["optimization_metrics"] = self._calculate_optimization_metrics(
            labeled_data,
            optimization_results
        )
    ...
    async def _optimize_global_thresholds(
        ...
        # Search for optimal signal strength threshold
        best_threshold = self.signal_strength_range[0]
        best_metric = -np.inf
        
        thresholds_to_test = np.linspace(
            self.signal_strength_range[0],
            self.signal_strength_range[1],
        ...
        return {
            ...
            "achieved_precision": best_metric if self.optimization_metric == "precision" else 0,
            "optimization_metric": self.optimization_metric,
            "metric_value": best_metric
        }
    
    async def _optimize_regime_specific_thresholds(
        self,
        regime_data: pd.DataFrame,
        regime_id: str
    ) -> Dict[str, float]:
    ...
    async def _validate_thresholds(
        ...
        validation = {
            ...
            "signal_rate": predictions.mean(),
            "true_positive_rate": (predictions & binary_labels).sum() / max(1, binary_labels.sum())
        }
        
        # Cross-regime validation if enabled
            ...
            for regime_id in labeled_data['regime_id'].unique():
                ...
                if len(regime_labels) > 0:
                    regime_metrics[regime_id] = {
                        "precision": precision_score(regime_labels, regime_predictions, zero_division=0),
                        "recall": recall_score(regime_labels, regime_predictions, zero_division=0),
                        "n_signals": regime_predictions.sum()
                    }
    ...
    def _analyze_signal_distribution(
        ...
        return {
            ...
            "percentile_25": signals.quantile(0.25),
            "percentile_50": signals.quantile(0.50),
            "percentile_75": signals.quantile(0.75),
            "percentile_90": signals.quantile(0.90),
            "n_strong_signals": (signals > 0.8).sum(),
            "strong_signal_rate": (signals > 0.8).mean()
        }
    ...
    def _calculate_optimization_metrics(
        ...
        metrics = {
            ...
            "labeled_samples": (labeled_data['label'] != 0).sum() if 'label' in labeled_data.columns else 0,
            "optimization_performed": len(optimization_results["global_thresholds"]) > 0,
            "regime_specific_optimizations": len(optimization_results["regime_thresholds"])
        }
        ...
        if "validation_results" in optimization_results:
            ...
            metrics.update({
                "final_precision": validation.get("precision", 0),
                "final_recall": validation.get("recall", 0),
                "final_f1": validation.get("f1_score", 0),
                "final_signal_rate": validation.get("signal_rate", 0)
            })
