...
class FeatureInteractionEngine:
    ...
    async def optimize_lookback_periods(
        self, market_data: pd.DataFrame, target: pd.Series, regimes: Optional[pd.Series] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _validate_lookback_periods(self):
        ...
        periods_to_validate = (
            self.dynamic_lookback_periods if self.dynamic_lookback_periods else self.fallback_lookback_periods
        )
            ...
            for i in range(len(periods)):
                for j in range(i + 1, len(periods)):
                    ...
                    if ratio < 1.5:  # Periods should be at least 1.5x different
                        self.logger.warning(
                            f"⚠️ {indicator}: Periods {period1} and {period2} may be too similar (ratio: {
                                ratio:.2f})"
                        )
        ...
        if "MACD" in periods_to_use:
            ...
            if len(macd_periods) >= 2:
                macd, macd_signal, macd_hist = talib.MACD(
                    market_data["close"].values, fastperiod=macd_periods[0], slowperiod=macd_periods[1], signalperiod=9
                )
        ...
        if "Bollinger_Bands" in periods_to_use:
            ...
            for period in bb_periods:
                bb_upper, bb_middle, bb_lower = talib.BBANDS(
                    market_data["close"].values, timeperiod=period, nbdevup=2, nbdevdn=2
                )
    ...
    def extract_interaction_features(
        ...
        try:
            ...
            all_interactions = np.concatenate(
                [basic_interactions, pattern_interactions, regime_interactions, timeframe_interactions], axis=1
            )
    ...
    def _create_pattern_specific_interactions(
        self, features: np.ndarray, pattern_indices: List[int], pattern_name: str, weight: float
    ) -> List[np.ndarray]:
        """
        Create pattern-specific interactions.
        """
    ...
    def _identify_market_regime(self, market_data: pd.DataFrame) -> str:
        ...
        try:
            ...
            trend_strength = (
                abs(market_data["close"].rolling(20).mean().iloc[-1] - market_data["close"].rolling(50).mean().iloc[-1])
