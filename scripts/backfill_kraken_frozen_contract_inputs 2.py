...
OI_HISTORY_ROOT = (
    ...
    / 'data_perp/exchanges/krakenfutures/open_interest_hourly'
)
...
def _oi_continuity_start(
    symbol: str,
    requested_start: int,
    end: int,
) -> int:
    """Extend an incremental refresh across the historical-sidecar seam.

    The feature materialiser combines the long historical OI sidecar with this
    public-API cache.  A live refresh that starts after the sidecar's last hour
    can otherwise leave a multi-hour hole between the two stores.  The native
    feature contract intentionally carries OI for only three hours, so such a
    hole invalidates rolling deltas despite a valid current observation.

    Fetch from the first hour after the last causal sidecar observation when
    that precedes the requested live window.  This does not use future data and
    does not rewrite historical values: ``_one`` retains existing cells via
    ``combine_first``.  When no compatible sidecar exists, keep the caller's
    declared start unchanged.
    """
    base = symbol.split('/')[0]
    path = next(iter(sorted(OI_HISTORY_ROOT.glob(f'{base}_USD*.parquet'))), None)
    if path is None:
        return requested_start
    try:
        history = pd.read_parquet(path, columns=['open_interest'])
        history.index = pd.to_datetime(history.index, utc=True, errors='coerce')
        valid = history.index[
            history.index.notna()
            & pd.to_numeric(history['open_interest'], errors='coerce').gt(0.0).to_numpy()
        ]
    except Exception:
        return requested_start
    if len(valid) == 0:
        return requested_start
    first_missing = int((valid.max() + pd.Timedelta(hours=1)).timestamp())
    if first_missing >= end:
        return requested_start
    return min(requested_start, first_missing)
...
def _one(symbol: str, start: int, end: int, out_dir: Path, include_trade: bool = False, include_orderbook_analytics: bool = False, include_base_primitives: bool = True) -> dict[str, object]:
    ...
    if include_base_primitives:
        ...
        # A one-hour live refresh must require that one mark observation.
        # Flooring ``0.995 * 1`` to zero previously made an entirely missing
        # current mark candle look complete and caused the live feature gate
        # to fail on mark/perpetual dislocation fields.
        required_mark_hours = max(1, int(np.ceil(0.995 * expected_hours)))
        if existing_mark_hours < required_mark_hours:
            try:
                mark = _mark(tradeable, start, end)
            except Exception as exc:
                errors.append(f'mark:{exc}')
        try:
            oi_start = _oi_continuity_start(symbol, start, end)
            oi = _open_interest(tradeable, oi_start, end)
        except Exception as exc:
