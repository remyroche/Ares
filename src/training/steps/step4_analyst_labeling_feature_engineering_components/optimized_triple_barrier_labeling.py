...
class OptimizedTripleBarrierLabeling:
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=pd.DataFrame(), context="optimized_triple_barrier_labeling.vectorized"
    )
    ...
    def apply_triple_barrier_labeling_vectorized(
        ...
        use_numba = (
            "numba" in globals() and numba is not None and callable(globals().get("_numba_triple_barrier_labels"))
        )
        ...
        if len(labeled_data) > 0:
            ...
            self.logger.info(
                f"   LONG signals - Avg profit: {
                    long_profits.mean():.4f}, Max: {
                    long_profits.max():.4f}, Min: {
                    long_profits.min():.4f}"
            )
            self.logger.info(
                f"   SHORT signals - Avg profit: {
                    short_profits.mean():.4f}, Max: {
                    short_profits.max():.4f}, Min: {
                    short_profits.min():.4f}"
            )
            self.logger.info(
                f"   Overall - Avg profit: {
                    labeled_data['potential_profit_pct'].mean():.4f}, Std: {
                    labeled_data['potential_profit_pct'].std():.4f}"
            )
        ...
        overall_agree = float(
            np.mean(
                ((next_sign_filtered > 0) & long_mask) | ((next_sign_filtered < 0) & short_mask),
            ),
        )
        self.logger.info(
            {
                ...
                "long_nextbar_agree": round(long_agree, 4) if long_agree == long_agree else None,
                "short_nextbar_agree": round(short_agree, 4) if short_agree == short_agree else None,
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=pd.DataFrame(), context="optimized_triple_barrier_labeling.parallel"
    )
