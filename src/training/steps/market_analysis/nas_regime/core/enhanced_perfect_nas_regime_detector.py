...
This module provides a NAS-specific interface to the unified regime detector,
...
from src.utils.nas_tas.regime_detector import create_nas_regime_detector
...
class EnhancedPerfectNASRegimeDetector:
    ...
    Provides a NAS-specific interface to the unified regime detector,
    maintaining backward compatibility while using the unified implementation.
    """
    ...
    def __init__(self, config=None):
        ...
        Args:
            config: NAS configuration (optional, uses defaults if None)
        """
        tprint_info("🧠 Initializing Enhanced Perfect NAS Regime Detector")
        ...
        # Create NAS-specific regime detector
        self.detector = create_nas_regime_detector(
            n_regimes=n_regimes,
            primary_timeframe=primary_timeframe
        )
        
        tprint_success("✅ Enhanced Perfect NAS Regime Detector initialized")
        self.logger = logging.getLogger(self.__class__.__name__)
        self.logger.info("✅ Enhanced Perfect NAS Regime Detector initialized")
    ...
    def detect_regimes(self,
                      ...
                      learn_thresholds: bool = True) -> UnifiedRegimeResult:
        """
        Detect market regimes using NAS system.
        ...
        Args:
            ...
            learn_thresholds: Whether to learn thresholds (for compatibility)
            
        Returns:
            UnifiedRegimeResult with regime detection results
        """
        tprint_info("🧠 Starting NAS regime detection...")
        
        try:
            result = self.detector.detect_regimes(
                market_data=market_data,
                timestamps=timestamps,
                optimize_performance=optimize_architecture
            )
