...
class Step9TacticianSpecialistTrainingValidator(BaseValidator):
    ...
    async def validate(
        ...
        model_files_passed, self._validate_tactician_model_files(
            symbol,
            exchange,
            data_dir,
        )
        ...
        performance_passed, self._validate_tactician_model_performance(
            symbol,
            exchange,
            data_dir,
        )
        ...
        quality_passed, self._validate_tactician_model_quality(
            symbol,
            exchange,
            data_dir,
        )
    ...
    def _validate_tactician_model_files(
        ...
        if missing_files:
        self.print(missing("❌ Missing tactician model files: {missing_files}"))
        return False
        ...
        self.print(error("❌ Error validating tactician model files: {e}"))
        return False
    ...
    def _validate_tactician_training_metrics(
        ...
        if epochs < 10:
        self.print(error("⚠️ Few training epochs: {epochs}"))
                elif epochs > 1000:
        self.print(error("⚠️ Many training epochs: {epochs}"))
        ...
        self.logger.warning(
                        f"⚠️ Short tactician training time: {training_time:.1f}s",
                    )
        ...
        self.print(error("⚠️ Low signal recall: {signal_recall:.3f}"))
    ...
    def _validate_tactician_model_quality(
        ...
        self.logger.warning(
                            f"⚠️ Small tactician model size: {model_size:.1f}MB",
                        )
        ...
        try: with open(model_file = "rb") as f:
        ...
        self.print(error("❌ Error loading tactician model: {e}"))
        return False
    ...
    def _unwrap_estimator(self, artifact: Any) -> Any:
        ...
        """
        try: if callable(getattr(artifact = "predict", None):
        return artifact
