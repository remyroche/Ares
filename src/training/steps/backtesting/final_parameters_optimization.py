...
class FinalParametersOptimizer(BaseStep):
    ...
    async def execute(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # NEW: Load OHLCV data for hierarchical optimization
    ...
    def _compute_regime_and_meta_scalars(self, n_trades: int) -> Dict[str, np.ndarray]:
        """
        Compute dummy regime and meta scalars for simulation when real data is unavailable.

        Args:
            n_trades: Number of trades to generate scalars for

        Returns:
            Dictionary of scalar arrays
        """
        # Return neutral defaults (mostly 1.0 or 0.0 depending on usage in simulate_trading)
        # Weights are usually multiplied by:
        # meta_conf (clipped 0-1) -> 1.0 is neutral/good
        # 1.0 - 0.5 * disagreement -> disagreement 0.0 is neutral/good
        # 1.0 - 0.5 * uncertainty -> uncertainty 0.0 is neutral/good
        # 0.5 + 0.5 * (1.0 - risk) -> risk 0.0 is neutral/good

        return {
            'meta_confidence': np.ones(n_trades, dtype=float),
            'disagreement_scalar': np.zeros(n_trades, dtype=float),
            'uncertainty_scalar': np.zeros(n_trades, dtype=float),
            'risk_scalar': np.zeros(n_trades, dtype=float),
            'path_scalar': np.zeros(n_trades, dtype=float),
            'trend_scalar': np.ones(n_trades, dtype=float) * 0.5 # trend neutral at 0.5
        }
