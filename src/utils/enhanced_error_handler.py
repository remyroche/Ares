        ...
        def decorator(func: F) -> F:
            ...
            async def async_wrapper(*args: Any, **kwargs: Any) -> Optional[T]:
                try:
                    ...
                    if log_errors:
                        self.logger.exception(f"Error in {context}.{func.__name__}: {e}")
            ...
            async def async_wrapper(*args: Any, **kwargs: Any) -> Optional[T]:
                try:
                    ...
                    if error_type in error_handlers:
                        ...
                        if log_errors:
                            self.logger.error(f"{message} in {context}.{func.__name__}: {e}")
                        return cast(Optional[T], return_value)
                    
                    if log_errors:
                        self.logger.exception(f"Unexpected error in {context}.{func.__name__}: {e}")
                    return default_return
