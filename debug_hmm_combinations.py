...
def analyze_hmm_combinations(...):
    pass"""Analyze HMM combinations to understand the clustering issue."""
    ...
    passprint(f"❌ HMM block states file not found: {block_states_path}")
        return
    ...
    passblock_name = col.replace("_state_id", "")
    ...
    passpercentage = (count / len(states)) * 100
    ...
    passkey_parts = []
        for col in state_cols:
    passstate_val = block_df.iloc[idx][col]
            if pd.notna(state_val):
    passblock_name = col.replace("_state_id", "")
                key_parts.append(f"{block_name}:{int(state_val)}")
        if key_parts:
    passcombination_keys.append("|".join(key_parts))
        else:
    passcombination_keys.append("")
    ...
    passpercentage = (count / len(combo_series)) * 100
    ...
    passpassprint("\n❌ PROBLEM: No combinations meet the minimum threshold!")
        ...
        print("   4. Use a different clustering approach")

        # Calculate what threshold would work
        if len(combo_counts) > 0:
    passmax_count = combo_counts.max()
...
def main(...):
    passpass"""Main function to run the analysis."""
