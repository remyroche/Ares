...
def _compute_feature_metrics(
    ...
    if is_binary_target:
        # Binary-aware MI proxy: use fast binned MI
        mi_full_series = {}
        for col in X.columns:
            x_full = X[col].to_numpy(dtype=float)
            mi_full_series[col] = compute_binned_mi(x_full, y_arr)
        mi_full = pd.Series(mi_full_series, index=X.columns)

        # MI stability across TimeSeriesSplit folds using the same binned MI
        ...
        # Continuous/real-valued target: use binned MI proxy
        mi_full_series = {}
        for col in X.columns:
            x_full = X[col].to_numpy(dtype=float)
            mi_full_series[col] = compute_binned_mi(x_full, y_arr)
...
def _compute_feature_metrics_with_tv_var(
    ...
    try:
        ...
        regime_detector = EightFeatureRegimeDetector()
        ...
        # Add TV-VAR system metrics
        enhanced_metrics["_tv_var_system"] = {
            "stability_score": tv_var_results.stability_score,
            "n_features": len(tv_var_features.columns),
            "n_samples": len(tv_var_features),
            "regime_count": len(tv_var_results.regime_assignments.unique()),
        }
...
def _generate_tv_var_features_from_specialists(specialist_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        features["rv_z_short"] = (rv_short - rv_short.rolling(100).mean()) / rv_short.rolling(100).std()
        features["rv_z_long"] = (rv_long - rv_long.rolling(100).mean()) / rv_long.rolling(100).std()
        ...
        features["volume_z"] = (volume - volume.rolling(50).mean()) / volume.rolling(50).std()
        
        # Spread proxy (using specialist dispersion)
        specialist_std = specialist_df.std(axis=1)
        features["spread_proxy_z"] = (specialist_std - specialist_std.rolling(50).mean()) / specialist_std.rolling(50).std()
        
        # 3. Trend/Directional features
        # Use mean specialist output as trend proxy
        trend_proxy = specialist_df.mean(axis=1)
        trend_slope = trend_proxy.rolling(window=24).apply(lambda x: np.polyfit(range(len(x)), x, 1)[0])
        features["trend_slope_z"] = (trend_slope - trend_slope.rolling(100).mean()) / trend_slope.rolling(100).std()
        ...
        drawdown = (trend_proxy - rolling_max) / rolling_max
        features["drawdown_z"] = (drawdown - drawdown.rolling(100).mean()) / drawdown.rolling(100).std()
...
def _select_specialist_scalars(X: pd.DataFrame) -> pd.DataFrame:
    ...
    # ------------------------------------------------------------------
    remaining_specialists = ["spectral", "microstructure", "volatility_burst", "reversion", "volume_force", "macro", "meso", "candlestick"]
...
# Optionally collapse raw specialist outputs to canonical scalars. When
    ...
    feature_metrics = _compute_feature_metrics_with_tv_var(X=X, y=y, cv_folds=cv_folds, enable_tv_var=enable_tv_var, tv_var_window=tv_var_window, tv_var_regime_aware=tv_var_regime_aware)
