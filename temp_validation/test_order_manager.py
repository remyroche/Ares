...
class TestOrderManager:
    ...
    async def test_create_order_nominal(self):
        ...
        assert_order_status(result['status'], OrderStatus.OPEN, "Le statut doit être OPEN")
    ...
    async def test_cancel_order_nominal(self):
        ...
        assert_order_status(result['status'], OrderStatus.CANCELLED, "Le statut doit être CANCELLED")
    ...
    async def test_get_active_orders_nominal(self):
        ...
        assert isinstance(result['orders'], list), "Les ordres doivent être une liste"
        assert len(result['orders']) == 3, "Trois ordres devraient être actifs"
        
        # Vérifier que tous les ordres sont actifs
        for order in result['orders']:
            assert order['status'] in [OrderStatus.OPEN, OrderStatus.PARTIALLY_FILLED], \
                "Les ordres doivent avoir un statut actif"
