...
class ComprehensiveDataQualityValidator:
    ...
    def validate_step2_data_quality(
        ...
        """Validate Step2 feature engineering quality."""
    ...
    def _validate_data_structure(self, df: Any, file_name: str) -> List[str]:
        ...
        if "klines" in file_name.lower():
            ...
            if hasattr(df, 'columns') and "timestamp" not in df.columns:
        ...
        if hasattr(df, 'columns'):
            for col in df.columns:
                if hasattr(df[col], 'isnull') and df[col].isnull().all():
        ...
        if hasattr(df, 'columns'):
            for col in df.columns:
                if col == "timestamp":
                    ...
                    if PANDAS_AVAILABLE and hasattr(pd.api.types, 'is_datetime64_any_dtype'):
                    ...
                    if PANDAS_AVAILABLE and hasattr(pd.api.types, 'is_numeric_dtype'):
        ...
        if PANDAS_AVAILABLE and not isinstance(df.index, pd.DatetimeIndex) and "timestamp" in df.columns:
    ...
    def _is_boolean_feature(self, series: Any) -> bool:
        """Check if a series represents a boolean feature."""
        if not PANDAS_AVAILABLE:
            return False
            
        # Check if it's already boolean dtype
        if hasattr(pd.api.types, 'is_bool_dtype') and pd.api.types.is_bool_dtype(series):
            return True

        # Check if it has exactly 2 unique values that could be boolean
        if hasattr(series, 'dropna') and hasattr(series, 'unique'):
    ...
    def _validate_feature_quality(self, file_path: str, file_name: str) -> Dict[str, Any]:
        """Validate feature-specific quality metrics."""
        result = {
            "passed": False,
            "shape": None,
            "feature_count": 0,
...
def validate_step1_quality(symbol: str, exchange: str, data_dir: str) -> Dict[str, Any]:
...
def validate_step1_5_quality(symbol: str, exchange: str, data_dir: str) -> Dict[str, Any]:
...
def validate_step2_quality(symbol: str, exchange: str, data_dir: str) -> Dict[str, Any]:
