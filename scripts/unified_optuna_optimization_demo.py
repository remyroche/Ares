...
class UnifiedOptunaDemo:
    ...
    def prepare_sample_data(self, data_type: str, n_samples: int = 2000) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        Returns:
            Tuple of (X, y) data
        """
        self.logger.info(f"Preparing {data_type} sample data with {n_samples} samples...")
    ...
    def optimize_sr_parameters(self, X: pd.DataFrame, y: pd.Series, n_trials: int = 100) -> OptimizationResult:
        ...
        try:
            ...
            self.logger.info(f"✅ S/R optimization completed: {result.validation_score:.4f}")
            return result
            
        except Exception as e:
            self.logger.error(f"❌ S/R optimization failed: {e}")
            return None
    ...
    def optimize_autoencoder(self, X: pd.DataFrame, y: pd.Series, n_trials: int = 75) -> OptimizationResult:
        ...
        try:
            ...
            self.logger.info(f"✅ Autoencoder optimization completed: {result.validation_score:.4f}")
            return result
            
        except Exception as e:
            self.logger.error(f"❌ Autoencoder optimization failed: {e}")
            return None
    ...
    def optimize_order_execution(self, X: pd.DataFrame, y: pd.Series, n_trials: int = 50) -> OptimizationResult:
        ...
        try:
            ...
            self.logger.info(f"✅ Order execution optimization completed: {result.validation_score:.4f}")
            return result
            
        except Exception as e:
            self.logger.error(f"❌ Order execution optimization failed: {e}")
            return None
    ...
    def custom_optimization_example(self, X: pd.DataFrame, y: pd.Series, n_trials: int = 50) -> OptimizationResult:
        ...
        def custom_objective(trial: optuna.Trial, X: pd.DataFrame, y: pd.Series) -> float:
            ...
            try:
                ...
                n_est_factor = min(1.0, n_estimators / 500)  # More estimators = better score
        ...
        try:
            ...
            self.logger.info(f"✅ Custom optimization completed: {result.validation_score:.4f}")
            return result
            
        except Exception as e:
            self.logger.error(f"❌ Custom optimization failed: {e}")
            return None
    ...
    def print_optimization_summary(self, results: Dict[str, OptimizationResult]):
        ...
        for optimization_type, result in results.items():
            ...
            if result.sr_performance_metrics:
                print(f"   S/R Performance Metrics:")
        ...
        try:
            ...
            for optimization_type, result in results.items():
                ...
                try:
                    ...
                    plots_created += 2
                    self.logger.info(f"📊 Created visualizations for {optimization_type}")
                    
                except Exception as e:
                    self.logger.warning(f"Could not create visualizations for {optimization_type}: {e}")
    ...
    def run_comprehensive_demo(self, optimization_type: str = "all", n_trials: int = 50):
        ...
        if optimization_type in ["all", "sr_parameters"]:
            ...
            sr_result = self.optimize_sr_parameters(X_sr, y_sr, n_trials * 2)  # More trials for S/R
        ...
        if optimization_type in ["all", "custom"]:
            ...
            custom_result = self.custom_optimization_example(X_custom, y_custom, n_trials)
...
async def main():
    ...
    parser.add_argument(
        "--optimization-type", 
        default="all",
        choices=["all", "ml_models", "sr_parameters", "autoencoder", "order_execution", "custom"],
        help="Type of optimization to run"
    )
    parser.add_argument("--n-trials", type=int, default=50, help="Number of trials per optimization")
