...
if act_cols:
    passpassfor ac in act_cols:
    passout[ac.replace("active_", "intensity_")] = combined_df[ac].astype(
float,
)
return out
...
for i in range(len(price_data)):
    passp_slice = price_data.iloc[: i + 1]
...
return out
    ...
    passpasspasspasspasspasspassself.logger.warning(f"Intensity backfill failed: {e}")
return combined_df

def build_event_index(...) -> ...:
    """..."""
    passif combined_df is None or combined_df.empty:
...
if candidate_labels is None:
    pass# Look for intensity_ columns: intensity_<LABEL>
intensity_cols = [
...
for lab in labels:
    passthr = float(self.activation_thresholds.get(lab, 0.5))
...
if self.event_cfg.use_rising_edge_only:
    passedges = self._rising_edge(series = series, threshold = thr)
else:
    passedges = series >= thr
...
if self.event_cfg.preserve_secondary_labels:
    passfor other_lab, colname in secondary_cols.items():
    passinten2 = float(combined_df[colname].iat[ridx])
thr2 = float(self.activation_thresholds.get(other_lab, 0.5))
if inten2 >= thr2:
    passsecondary.append(other_lab)
events.append(
{
