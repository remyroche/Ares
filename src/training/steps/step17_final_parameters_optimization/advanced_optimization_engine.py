...
class MultiObjectiveParetoOptimizer:
    ...
    async def create_multi_objective_study(self, study_name: str, n_trials: int, population_size: Optional[int] = None):
        ...
        study = optuna.create_study(
            ...
            sampler=NSGAIISampler(population_size=population_size, crossover_prob=0.8, mutation_prob=0.1, seed=42),
...
class CrossValidationPruner:
    ...
    async def analyze_parameter_sensitivity_cv(
        ...
        for step_idx, (step_name, step_params) in enumerate(parameter_mapping.items()):
            for param_idx, (param_name, param_config) in enumerate(step_params.items()):
                ...
                self.logger.info(
                    f"Analyzing {param_key} ({
                        step_idx * len(step_params) + param_idx + 1}/{total_params})"
                )
                ...
                try:
                    ...
                    if cv_result.is_significant:
                        ...
                        self.logger.debug(
                            f"⚠️ {param_key}: Not significant (sensitivity: {
                                cv_result.mean_sensitivity:.6f})"
                        )
    ...
    async def _analyze_single_parameter_cv(
        self, data: pd.DataFrame, step_name: str, param_name: str, param_config: Any
    ...
    async def _evaluate_parameter_sensitivity_fold(
        self, train_data: pd.DataFrame, val_data: pd.DataFrame, step_name: str, param_name: str, param_config: Any
    ) -> float:
    ...
    async def _evaluate_parameter_value(
        self, train_data: pd.DataFrame, val_data: pd.DataFrame, step_name: str, param_name: str, value: Any
    ) -> float:
...
class EnsembleParameterOptimizer:
        ...
        for param in ensemble_params:
            ...
            if "meta_learner" in param.lower():
                dependencies[param].extend(
                    [p for p in ensemble_params if "stacking_enabled" in p.lower() or "ensemble_size" in p.lower()]
                )
    ...
    def _get_constraint_rules(self, ensemble_params: List[str]) -> List[Dict[str, Any]]:
        ...
        if any("ensemble_size" in p.lower() for p in ensemble_params):
            constraints.append(
                {"parameter": "ensemble_size", "constraint": "ensemble_size >= 2", "type": "lower_bound"}
            )

        if any("stacking_cv_folds" in p.lower() for p in ensemble_params):
            constraints.append(
                {"parameter": "stacking_cv_folds", "constraint": "stacking_cv_folds >= 3", "type": "lower_bound"}
            )
...
class ParameterInteractionDetector:
    ...
    def __init__(self, interaction_threshold: float = 0.01, max_interactions: int = 50):
        ...
        self.logger.info(
            f"Parameter interaction detector initialized: threshold={interaction_threshold}, max={max_interactions}"
        )

    async def detect_parameter_interactions(
        self, data: pd.DataFrame, parameters: List[str], parameter_mapping: Dict[str, Dict[str, Any]]
            ...
            for j, param2 in enumerate(parameters[i + 1 :], i + 1):
                ...
                try:
                    ...
                    if interaction and interaction.interaction_strength > self.interaction_threshold:
                        ...
                        self.logger.debug(
                            f"Strong interaction detected: {param1} ↔ {param2} (strength: {
                                interaction.interaction_strength:.6f})"
                        )
    ...
    def get_interaction_summary(self, interactions: List[ParameterInteraction]) -> Dict[str, Any]:
        ...
        return {
            ...
            "interactions_by_type": {
                interaction_type: len(interactions) for interaction_type, interactions in by_type.items()
            },
            "strength_statistics": {
...
def create_interaction_detector(
    interaction_threshold: float = 0.01, max_interactions: int = 50
