...
from src.tactician.sr_weight_optimizer import setup_sr_weight_optimizer, WeightOptimizationResult
...
class SRWeightOptimizationBacktest:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.optimization_periods = self.backtest_config.get("optimization_periods", [
            "bull_market", "bear_market", "sideways_market", "volatile_market"
        ])
        self.min_period_length = self.backtest_config.get("min_period_length", 90)  # days
        self.validation_split = self.backtest_config.get("validation_split", 0.2)
        
        # Performance thresholds
        self.performance_thresholds = self.backtest_config.get("performance_thresholds", {
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.info("✅ SR Weight Optimization Backtest initialized successfully")
            return True
            
        except Exception as e:
            self.logger.error(f"❌ Error initializing backtest framework: {e}")
            return False
    ...
    async def run_comprehensive_backtest(
        ...
        """
        try:
            self.logger.info(f"🎯 Starting comprehensive backtest for {symbol} on {exchange}")
            ...
            recommendations = self._generate_optimization_recommendations(cross_period_analysis)
    ...
    def _prepare_backtest_data(self, price_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"✅ Prepared backtest data with {len(period_data)} market conditions")
            return {
    ...
    def _identify_market_conditions(self, price_data: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        try:
            ...
            bull_mask = (rolling_return > 0.001) & (rolling_volatility < rolling_volatility.quantile(0.6))
            conditions["bull_market"] = bull_mask
            
            # Bear market: negative returns, high volatility
            bear_mask = (rolling_return < -0.001) & (rolling_volatility > rolling_volatility.quantile(0.4))
            conditions["bear_market"] = bear_mask
            
            # Sideways market: low returns, low volatility
            sideways_mask = (abs(rolling_return) < 0.0005) & (rolling_volatility < rolling_volatility.quantile(0.5))
            ...
            self.logger.error(f"❌ Error identifying market conditions: {e}")
            return {}
        ...
        try:
            ...
            for period_name, period_data in prepared_data["period_data"].items():
                if len(period_data) < self.min_period_length:
                    self.logger.warning(f"⚠️ Period {period_name} too short: {len(period_data)} < {self.min_period_length}")
                    continue
                
                self.logger.info(f"🔍 Optimizing weights for {period_name} period ({len(period_data)} data points)")
                
                # Run optimization for this period
                result = await self.weight_optimizer.optimize_weights(
                    period_data,
                    period_data["target_returns"].dropna(),
                    [period_name]
                )
                
                if result:
                    period_results[period_name] = result
                    self.logger.info(f"✅ {period_name} optimization completed: score={result.optimization_score:.4f}")
                else:
            ...
            self.logger.error(f"❌ Error in multi-period optimization: {e}")
            return {}
    
    def _analyze_cross_period_results(
        self, 
        period_results: Dict[str, WeightOptimizationResult]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            weight_components = ["touch_count", "total_volume", "level_age", "bounce_rate", "isolation_score"]
            
            for component in weight_components:
                weights = [result.weights.get(component, 0) for result in period_results.values()]
            ...
            performance_metrics = ["sharpe_ratio", "win_rate", "profit_factor", "max_drawdown", "total_return"]
            
            for metric in performance_metrics:
                values = [getattr(result, metric, 0) for result in period_results.values()]
            ...
            analysis["recommended_weights"] = self._generate_recommended_weights(period_results)
            
            return analysis
            
        except Exception as e:
            self.logger.error(f"❌ Error analyzing cross-period results: {e}")
            return {}
    ...
    def _generate_recommended_weights(
        ...
        try:
            ...
            total_score = sum(result.optimization_score for result in period_results.values())
            if total_score > 0:
                for component in weight_components:
                    weighted_sum = sum(
            ...
            best_period = max(period_results.items(), key=lambda x: x[1].optimization_score)
            ...
            recommendations["market_adaptive"] = {
                "bull_market": period_results.get("bull_market", best_period[1]).weights.copy(),
                "bear_market": period_results.get("bear_market", best_period[1]).weights.copy(),
                "sideways_market": period_results.get("sideways_market", best_period[1]).weights.copy(),
                "volatile_market": period_results.get("volatile_market", best_period[1]).weights.copy()
            }
            
            return recommendations
            
        except Exception as e:
            self.logger.error(f"❌ Error generating recommended weights: {e}")
            return {}
    
    def _generate_optimization_recommendations(
        self, 
        cross_period_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for component, stats in weight_stability.items():
                cv = stats.get("cv", 0)
                if cv < 0.2:
                    recommendations["weight_recommendations"][component] = "STABLE - Use balanced weights"
                elif cv < 0.4:
                    recommendations["weight_recommendations"][component] = "MODERATE - Use conservative weights"
                else:
                    recommendations["weight_recommendations"][component] = "VOLATILE - Use market-adaptive weights"
            
            # Performance insights
            performance_consistency = cross_period_analysis.get("performance_consistency", {})
            for metric, stats in performance_consistency.items():
                ...
                if metric == "sharpe_ratio":
                    if mean_value > 1.0:
                        recommendations["performance_insights"][metric] = "EXCELLENT - Strong risk-adjusted returns"
                    elif mean_value > 0.5:
                        recommendations["performance_insights"][metric] = "GOOD - Acceptable risk-adjusted returns"
                    else:
                        recommendations["performance_insights"][metric] = "POOR - Consider re-optimization"
                
                elif metric == "win_rate":
                    if mean_value > 0.6:
                        recommendations["performance_insights"][metric] = "EXCELLENT - High win rate"
                    elif mean_value > 0.5:
                        recommendations["performance_insights"][metric] = "GOOD - Balanced win rate"
                    else:
                        recommendations["performance_insights"][metric] = "POOR - Low win rate"
    ...
    def _create_summary(self, recommendations: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            stable_components = [comp for comp, rec in weight_recommendations.items() if "STABLE" in rec]
            if stable_components:
                summary["key_findings"].append(f"Stable weight components: {', '.join(stable_components)}")
            
            excellent_metrics = [metric for metric, insight in performance_insights.items() if "EXCELLENT" in insight]
            if excellent_metrics:
                summary["key_findings"].append(f"Excellent performance in: {', '.join(excellent_metrics)}")
            
            # Recommended action
            if len(stable_components) >= 3 and len(excellent_metrics) >= 2:
                summary["recommended_action"] = "IMPLEMENT - Strong optimization results with stable weights"
                summary["confidence_level"] = "HIGH"
            elif len(stable_components) >= 2:
                summary["recommended_action"] = "IMPLEMENT WITH MONITORING - Good results, monitor performance"
                summary["confidence_level"] = "MEDIUM"
            else:
                summary["recommended_action"] = "RE-OPTIMIZE - Insufficient stability or performance"
...
async def main():
    ...
    print(f"🚀 Starting SR Weight Optimization Backtest for {args.symbol} on {args.exchange}")
    
    # Load configuration
    config = {
        "sr_weight_optimization": {
            "method": "grid_search",
            "backtest_lookback_days": args.period,
    ...
    results = await backtest.run_comprehensive_backtest(price_data, args.symbol, args.exchange)
