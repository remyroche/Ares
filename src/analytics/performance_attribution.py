...
class PerformanceAttributionSystem:
    ...
    def _analyze_single_regime(self, regime: str, window: str) -> Dict[str, Any]:
        ...
        if not trades:
            return {'regime': regime, 'status': 'no_data', 'trade_count': 0}
        performance_metrics = self._calculate_performance_metrics(trades, window)
        return {'regime': regime, 'status': 'analyzed', 'window': window, 'trade_count': len(trades), 'performance_metrics': performance_metrics, 'last_updated': regime_metrics['last_updated']}

    def _analyze_all_regimes(self, window: str) -> Dict[str, Any]:
        """Analyze performance for all regimes"""
        regime_analyses = {}
        overall_metrics = {'total_trades': 0, 'total_pnl': 0.0, 'regime_count': 0, 'best_regime': None, 'worst_regime': None}
        regime_performances = []
        for regime in self.regime_names:
    ...
    def _analyze_single_timeframe(self, timeframe: str, window: str) -> Dict[str, Any]:
        ...
        if not trades:
            return {'timeframe': timeframe, 'status': 'no_data', 'trade_count': 0}
        performance_metrics = self._calculate_performance_metrics(trades, window)
        return {'timeframe': timeframe, 'status': 'analyzed', 'window': window, 'trade_count': len(trades), 'performance_metrics': performance_metrics, 'last_updated': timeframe_metrics['last_updated']}

    def _analyze_all_timeframes(self, window: str) -> Dict[str, Any]:
        """Analyze performance for all timeframes"""
        timeframe_analyses = {}
        overall_metrics = {'total_trades': 0, 'total_pnl': 0.0, 'timeframe_count': 0, 'best_timeframe': None, 'worst_timeframe': None}
    ...
    async def analyze_combined_performance(self, window: str='medium') -> Optional[Dict[str, Any]]:
        ...
        try:
            if window not in self.rolling_windows:
                ...
                return None
            combined_analyses = {}
            overall_metrics = {'total_combinations': 0, 'total_trades': 0, 'total_pnl': 0.0, 'best_combination': None, 'worst_combination': None}
            ...
            for regime in self.regime_names:
                for timeframe in self.timeframes:
                    ...
                    if trades:
                        performance_metrics = self._calculate_performance_metrics(trades, window)
                        combined_analyses[key] = {'regime': regime, 'timeframe': timeframe, 'status': 'analyzed', 'window': window, 'trade_count': len(trades), 'performance_metrics': performance_metrics, 'last_updated': combined_metrics['last_updated']}
                        ...
                        combination_performances.append({'combination': key, 'regime': regime, 'timeframe': timeframe, 'total_pnl': performance_metrics['total_pnl'], 'sharpe_ratio': performance_metrics['sharpe_ratio'], 'win_rate': performance_metrics['win_rate']})
                    else:
    ...
    def _calculate_performance_metrics(self, trades: List[float], window: str) -> Dict[str, Any]:
        ...
        if not trades:
            return {'total_pnl': 0.0, 'avg_pnl': 0.0, 'std_pnl': 0.0, 'win_rate': 0.0, 'sharpe_ratio': 0.0, 'max_drawdown': 0.0, 'profit_factor': 0.0, 'total_trades': 0}
        ...
        if std_pnl > 0:
            sharpe_ratio = avg_pnl / std_pnl * np.sqrt(252 * 24 * 4)
        else:
            sharpe_ratio = 0.0
        ...
        return {'total_pnl': total_pnl, 'avg_pnl': avg_pnl, 'std_pnl': std_pnl, 'win_rate': win_rate, 'sharpe_ratio': sharpe_ratio, 'max_drawdown': max_drawdown, 'profit_factor': profit_factor, 'total_trades': len(trades_array), 'gross_profit': gross_profit, 'gross_loss': gross_loss}
    ...
    def get_attribution_summary(self) -> Dict[str, Any]:
        ...
        for regime, metrics in self.regime_metrics.items():
            regime_summary[regime] = {'trade_count': metrics['trade_count'], 'total_pnl': metrics['total_pnl'], 'last_updated': metrics['last_updated']}
        timeframe_summary = {}
        for timeframe, metrics in self.timeframe_metrics.items():
            timeframe_summary[timeframe] = {'trade_count': metrics['trade_count'], 'total_pnl': metrics['total_pnl'], 'last_updated': metrics['last_updated']}
        return {'system_status': 'active', 'total_trades_tracked': total_trades, 'regime_count': len(self.regime_names), 'timeframe_count': len(self.timeframes), 'leverage_multiplier': self.leverage_multiplier, 'rolling_windows': self.rolling_windows, 'regime_summary': regime_summary, 'timeframe_summary': timeframe_summary, 'last_updated': datetime.now()}