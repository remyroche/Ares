"""
Enhanced Multi-Horizon Integration

This module provides seamless integration between the enhanced data and labels system
and the existing multi-horizon profit labeler, ensuring no duplication of functionality
while providing the best of both systems.

Key Features:
1. Enhanced Data & Labels System Integration
2. Preserves existing multi-horizon functionality
3. Adds trading-aware label definitions
4. Comprehensive data cleaning and stability monitoring
...
# Import existing multi-horizon labeler
from src.training.steps.pre_training.multi_horizon_profit_labeler import (
    MultiHorizonProfitLabeler, MultiHorizonConfig, LabelingResult
)
...
class EnhancedMultiHorizonConfig(MultiHorizonConfig):
    """Enhanced configuration that extends MultiHorizonConfig with enhanced data and labels features."""
...
class EnhancedMultiHorizonProfitLabeler(MultiHorizonProfitLabeler):
    """
    Enhanced Multi-Horizon Profit Labeler that integrates the enhanced data and labels system.
    
    This class extends the existing MultiHorizonProfitLabeler with:
    ...
    def __init__(self, config: Optional[EnhancedMultiHorizonConfig] = None):
        ...
        base_config = MultiHorizonConfig(
            timeframe=self.enhanced_config.timeframe,
            base_period_minutes=self.enhanced_config.base_period_minutes,
            enable_volatility_normalization=self.enhanced_config.enable_volatility_normalization,
            enable_noise_gating=self.enhanced_config.enable_noise_gating,
            enable_quality_scoring=self.enhanced_config.enable_quality_scoring,
            enable_multi_target_scheme=self.enhanced_config.enable_multi_target_scheme,
            enable_regime_aware_labeling=self.enhanced_config.enable_regime_aware_labeling,
            regime_column=self.enhanced_config.regime_column,
            enable_enhanced_labels=self.enhanced_config.enable_enhanced_labels,
            label_definition_type=self.enhanced_config.label_definition_type,
            min_data_points=self.enhanced_config.min_data_points,
            save_intermediate_results=self.enhanced_config.save_intermediate_results,
            generate_reports=self.enhanced_config.generate_reports,
            min_auc_threshold=self.enhanced_config.min_auc_threshold,
            max_auc_std_threshold=self.enhanced_config.max_auc_std_threshold,
            min_psi_threshold=self.enhanced_config.min_psi_threshold,
            max_flip_rate_threshold=self.enhanced_config.max_flip_rate_threshold,
            min_balance_threshold=self.enhanced_config.min_balance_threshold,
            max_balance_threshold=self.enhanced_config.max_balance_threshold,
            enable_label_balancing=self.enhanced_config.enable_label_balancing,
            enable_sample_weighting=self.enhanced_config.enable_sample_weighting,
            enable_regime_balancing=self.enhanced_config.enable_regime_balancing,
            balancing_config=self.enhanced_config.balancing_config,
            weighting_config=self.enhanced_config.weighting_config,
            regime_config=self.enhanced_config.regime_config,
            fairness_config=self.enhanced_config.fairness_config
        )
    ...
    async def execute_enhanced_labeling(
        self,
        symbol: str,
        exchange: str,
        timeframe: str,
        ...
        """
        Execute enhanced multi-horizon profit labeling with full data and labels system integration.
        ...
        Args:
            ...
            force_enhanced_processing: Force enhanced processing even if cached
            
        Returns:
            Dictionary containing enhanced labeling results and metadata
        """
        try:
            ...
            if force_enhanced_processing or self.enhanced_config.enable_enhanced_data_cleaning:
                ...
                enhanced_result = self.enhanced_labels_system.process_market_data(
                    ...
                    force_recompute=force_enhanced_processing
                )
                ...
                if 'error' in enhanced_result:
                    ...
                    return await self.execute_labeling(symbol, exchange, timeframe, data_dir, regime_data)
                ...
                enhanced_labeling_result = await self.execute_labeling(symbol, exchange, timeframe, data_dir, regime_data)
            
            # Apply regime-aware processing if enabled and regime data is available
            if self.enhanced_config.enable_regime_aware_labeling and regime_series is not None:
                tprint_info("🎭 Applying regime-aware processing...")
