...
## 2026-05-19 - Replace pandas iterrows with numpy arrays in run_inference.py
Learning: `iterrows()` in pandas is extremely slow due to box/unbox overhead and index checking. Using it in inference hot paths (like `_evaluate_oco_policy` in `run_inference.py`) delays order evaluation. Simple iteration is better performed by converting dataframe columns to numpy arrays and using `zip()` to iterate, or vectorizing the operations entirely.
Action: Replaced `iterrows()` with numpy iteration using a fallback helper function to handle missing columns safely.
