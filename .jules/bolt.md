...
## 2024-05-24 - [Iterrows with Zip Numpy Array] Learning: When iterating over Pandas DataFrames in performance-critical sections (like inference loops), avoid `iterrows()`. Action: Use `zip()` on the underlying NumPy arrays (e.g., `df['col'].values`) to bypass Pandas object creation overhead and achieve significant speedups.
