...
## 2026-07-05 - [Vectorized Ledger Bulk Update]
Learning: The `PredictionLedger.mark_resolved` method previously used `.iterrows()` paired with `.loc` assignments and row-by-row `pd.concat` calls to update tracking ledgers, which had an O(N²) time complexity that became extremely slow. This can be easily reduced to O(1) via pandas set operations and vectorized indexing.
Action: For bulk updates (upsert) to DataFrames based on index matching, never use `.iterrows()`. Instead, identify `common_idx = upd_idx.index.intersection(old_idx.index)` to execute `old_idx.loc[common_idx, upd_idx.columns] = upd_idx.loc[common_idx]` and find `new_idx = upd_idx.index.difference(old_idx.index)` to append in a single `pd.concat` block.
