...
class EnsembleClusteringRegimeDetector:
    ...
    def __init__(self, config: Dict[str, Any]=None) -> None:
        ...
        self.hmm_params = self.config.get('hmm_params', {'n_components_range': [2, 8], 'covariance_types': ['full', 'tied', 'diag', 'spherical']})
        self.kmeans_params = self.config.get('kmeans_params', {'n_clusters_range': [10, 30], 'n_init': 10, 'max_iter': 300})
        self.dbscan_params = self.config.get('dbscan_params', {'eps_range': [0.1, 2.0], 'min_samples_range': [5, 50]})
        self.ensemble_weights = self.config.get('ensemble_weights', {'hmm': 0.4, 'kmeans': 0.3, 'dbscan': 0.3})
    ...
    def ensemble_regime_detection(self, features: np.ndarray, method_weights: Optional[Dict[str, float]]=None) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        consensus_regimes = self._optimized_consensus_voting(ensemble_results, quality_weights, method_weights)
    ...
    def _dbscan_clustering_optimized(self, features: np.ndarray) -> Dict[str, Any]:
        ...
        best_model = None
        best_params = None
        eps_values = [0.2, 0.3, 0.5, 0.7, 0.9]
        min_samples_values = [5, 8, 10, 15, 20]
    ...
    def _dbscan_clustering_vectorized(self, features: np.ndarray) -> Dict[str, Any]:
        ...
        best_model = None
        best_params = None
        eps_values = np.linspace(self.dbscan_params['eps_range'][0], self.dbscan_params['eps_range'][1], 10)
        min_samples_values = range(self.dbscan_params['min_samples_range'][0], self.dbscan_params['min_samples_range'][1] + 1, 5)
        for eps in eps_values:
            for min_samples in min_samples_values:
                try:
                    model = DBSCAN(eps=eps, min_samples=min_samples, n_jobs=self.n_jobs)
    ...
    def _calculate_dbscan_quality_score(self, features: np.ndarray, regimes: np.ndarray, model: DBSCAN) -> float:
        ...
        try:
            ...
            cluster_score = 1 / (1 + abs(n_clusters - 10) / 10)
    ...
    def optimize_ensemble_parameters(self, features: np.ndarray, target_n_regimes: Optional[int]=None) -> Dict[str, Any]:
        ...
        if target_n_regimes is not None:
            self.kmeans_params['n_clusters_range'] = [max(5, target_n_regimes - 5), min(50, target_n_regimes + 10)]
        return {'hmm_params': self.hmm_params, 'kmeans_params': self.kmeans_params, 'dbscan_params': self.dbscan_params}
if __name__ == '__main__':
    ...
    config = {'n_jobs': -1, 'random_state': 42, 'ensemble_weights': {'hmm': 0.4, 'kmeans': 0.3, 'dbscan': 0.3}}
