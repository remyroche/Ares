...
from src.training.steps.labeling.label_generation_utils import (
    UnifiedPriceMixin,
    BaseEventGenerator,
    OutputGeometry,
    ...
    average_uniqueness,
    ROLLING_Q_WINDOW,
    ROLLING_Q_THRESHOLDS,
    _quantiles_from_threshold,
    _quantiles_from_quantile,
    RAW_METRIC_FIELDS,
...
# ==========================================
# (Moved to label_generation_utils.py)
...
def get_specialist_event_matrix(df: pd.DataFrame, family: str) -> pd.Series:
    ...
    # Use dynamic threshold based on family
    thresh = 2.7
    if family == 'INFORMATION_SPECIALIST': thresh = 2.0
    elif family == 'MOMENTUM_DECAY_SPECIALIST': thresh = 2.0

    q1, q2 = _quantiles_from_threshold(thresh)
    details = detect_rolling_quantile_surprises(
...
def _score_single_candidate_worker(cand, df_full, price, vol_series, X_probe, transaction_cost, pt, sl, horizon, risk_budget, high, low):
    ...
    try:
        ...
        # Compute TBM Labels
        from src.training.steps.labeling.label_generation_utils import compute_dominance_labels, check_label_quality
        ...
        logger.warning(f"Scoring failed for candidate {cand.get('family', 'unknown')}: {e}")
        return None
...
def score_candidates(
    ...
    tprint_info(f"🚀 Scoring {len(candidates)} candidates in parallel (n_jobs={n_jobs})...")
...
def orthogonal_label_generation(
    ...
    import os
    n_jobs = os.cpu_count() or 4
    tprint_info(f"🚀 Starting Main Parameter Sweep in Parallel (n_jobs={n_jobs}) across {len(generator_configs)} configurations...")
