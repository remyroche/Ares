...
def create_meta_features(
    ...
    # Geometry-Specific Lag Features from Signals (Outside MTF Loop)
    ...
    # ===== MTF LOOP =====
    for w in windows:
        # --- 0. ORTHOGONAL CUSUM & EFFICIENCY FEATURES (MTF) ---
        # Dual CUSUM Continuous Stats (Window-Specific)
        # Using w as volatility window, and scaled ER window
        ...
        features[f'cusum_trend_pos_w{w}'] = _align_to_features(_norm(cusum_stats['S_trend_pos'], f'cusum_trend_pos_w{w}'), n_features)
        features[f'cusum_trend_neg_w{w}'] = _align_to_features(_norm(cusum_stats['S_trend_neg'], f'cusum_trend_neg_w{w}'), n_features)
        features[f'cusum_rev_pos_w{w}'] = _align_to_features(_norm(cusum_stats['S_rev_pos'], f'cusum_rev_pos_w{w}'), n_features)
        features[f'cusum_rev_neg_w{w}'] = _align_to_features(_norm(cusum_stats['S_rev_neg'], f'cusum_rev_neg_w{w}'), n_features)
        features[f'smoothed_return_w{w}'] = _align_to_features(_norm(cusum_stats['smoothed_return'], f'smoothed_return_w{w}'), n_features)
        features[f'residual_return_w{w}'] = _align_to_features(_norm(cusum_stats['residual_return'], f'residual_return_w{w}'), n_features)

        # Rolling Efficiency Ratios (MTF)
        er_w = get_efficiency_ratio(close, window=w)
        # Rolling mean of efficiency ratio over window w
        features[f'rolling_mean_er_w{w}'] = _align_to_features(_norm(er_w.rolling(w).mean(), f'rolling_mean_er_w{w}'), n_features)
        # Rolling std of efficiency ratio over window w (volatility of efficiency)
        features[f'rolling_std_er_w{w}'] = _align_to_features(_norm(er_w.rolling(w).std(), f'rolling_std_er_w{w}'), n_features)
