#!/usr/bin/env python3
"""Resume the top-two stage1 base run after the dist_rolling kill.

The previous run saved the loc_ema native model/OOF but died before writing the
base_models_intermediate.pkl bundle.  Seed that bundle from the saved loc_ema
native artifact, then rerun only the missing dist_rolling base head using the
source native preset feature/parameter contract.
"""
...
def seed_loc_ema_intermediate() -> None:
    native_dir = DATA_ROOT / "artifacts" / STAGE1_RUN_ID / "models" / "native"
    loc_model = native_dir / f"short_{LOC_EMA}_H10" / "model.joblib"
    _require_file(loc_model, "stage1 loc_ema native model")
    ...
    state.setdefault("ts_trained", STAGE1_RUN_ID)
    _atomic_pickle(trained_state_path, state)
    _append(
        "Seeded stage1 intermediate/trained_state from saved loc_ema native "
        f"artifact with features={len(loc_conf.get('feat_cols') or [])}."
    )


def run_dist_rolling_base_resume() -> None:
    ...
    slice_plan_path = _build_recent_tail_slice_plan(POLICY_SLICE_SOURCE_RUN_ID, STAGE1_RUN_ID)
    env = _train_env(
    ...
    _append(
        "Resume settings: dist_rolling only, native preset features+best_params, "
        "merge_existing_base_models=1."
    )
    _run_step(
        "resume_stage1_train_base_dist_rolling_native_preset",
        _pipeline_cmd("train_base", STAGE1_RUN_ID),
        env,
    )
    run_root = DATA_ROOT / "artifacts" / STAGE1_RUN_ID
    _require_file(
        run_root / "models" / "native" / f"short_{DIST_ROLLING}_H10" / "model.joblib",
        "stage1 dist_rolling native model",
    )
...
def main() -> int:
    _append(
        "Resume top2 stage1 missing base artifact: preserving loc_ema and "
        "rerunning dist_rolling with saved native preset contract."
    )
    seed_loc_ema_intermediate()
    run_dist_rolling_base_resume()
    _append("Resume top2 stage1 dist_rolling base completed.")
    return 0
