...
def main() -> None:
    ...
    for directory in bundle_dirs:
        ...
        reserve_days_match = (
            int(upstream_manifest.get("calibration_reserve_days", -1)) == 42
            and int(conversion_manifest.get("calibration_reserve_days", -1)) == 42
        )
        reserve_start_match = (
    ...
    check("shared_42d_reserve_excluded_from_all_active_fits", all(
    ...
    check("final_score_uses_same_model_prior42", conversion_audit["final_reference"].eq("same_conversion_model_prior42").all(), conversion_audit["final_reference"].unique().tolist())
    ...
    check("each_reserve_precedes_activation", reserve_start.lt(cutoff).all() and pd.to_numeric(block_audit["reserve_days"], errors="coerce").eq(42).all(), int(len(block_audit)))
