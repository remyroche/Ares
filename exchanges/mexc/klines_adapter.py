"""
MEXC Klines Data Adapter

This module provides a unified adapter for MEXC klines data that ensures
...
try:
    ...
    MEXC_EXCHANGE_AVAILABLE = True
except ImportError:
    MEXC_EXCHANGE_AVAILABLE = False


class MexcKlinesAdapter:
    """Unified adapter for MEXC klines data that ensures complete equivalency with other exchanges."""
    
    def __init__(self, api_key: str = None, secret_key: str = None, data_dir: str = "historical_data"):
        """Initialize the MEXC klines adapter.
        
        Args:
            api_key: MEXC API key (optional for public data)
            secret_key: MEXC secret key (optional for public data)
            data_dir: Directory for data storage
        """
        self.exchange = "mexc"
        self.data_dir = data_dir
        
        # Initialize MEXC exchange for API calls (if available)
        if MEXC_EXCHANGE_AVAILABLE:
            self.mexc_exchange = MexcExchange(api_key, secret_key)
        else:
    ...
    async def get_klines_data(
        ...
        """Get standardized klines data from MEXC API.
        ...
        try:
            if not self.unified_adapter:
                print("❌ MEXC exchange not available - cannot fetch data")
                return pd.DataFrame()
            ...
            tprint_data_format(
                standardized_data, 
                f"mexc_klines_{symbol}_{interval}", 
            ...
            if enable_data_preview and not standardized_data.empty:
                tprint_data_preview(
                    standardized_data, 
                    name=f"MEXC klines data for {symbol} ({interval})",
                    max_rows=5,
                    level="INFO"
                )
            ...
            print(f"❌ Error getting MEXC klines data: {e}")
            return pd.DataFrame()
    ...
    def _format_klines_data(self, data: List[Dict], symbol: str, interval: str) -> pd.DataFrame:
        """Format MEXC klines data to standard format.
        
        Args:
            data: Raw klines data from MEXC API
    ...
    async def download_and_process_klines(
        ...
        try:
            ...
            if enable_data_preview and not processed_data.empty:
                tprint_data_preview(
                    processed_data, 
                    name=f"Processed MEXC klines data for {symbol} ({interval})",
                    max_rows=5,
                    level="INFO"
                )
...
def create_mexc_klines_adapter(api_key: str = None, secret_key: str = None, data_dir: str = "historical_data") -> MexcKlinesAdapter:
    """Create a MEXC klines adapter instance.
    ...
    Returns:
        MexcKlinesAdapter instance
    """
    return MexcKlinesAdapter(api_key, secret_key, data_dir)