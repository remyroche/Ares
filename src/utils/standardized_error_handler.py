...
class ErrorRecord:
    ...
    def __init__(self, error: Exception, context: ErrorContext, severity: ErrorSeverity):
...
class StandardizedErrorHandler:
    ...
    def handle_step_error(self, error: Exception, step_name: str, 
                         severity: ErrorSeverity = ErrorSeverity.ERROR,
                         context: Optional[Dict[str, Any]] = None) -> ErrorRecord:
        """Handle error in a specific step."""
    ...
    def categorize_error(self, error: Exception) -> ErrorCategory:
        """Categorize an error."""
        error_record = ErrorRecord(error, ErrorContext("unknown", "unknown"), ErrorSeverity.ERROR)
        return error_record.category
    
    def get_recovery_strategy(self, error_type: Union[Exception, ErrorCategory]) -> Dict[str, Any]:
        """Get recovery strategy for an error type or category."""
        if isinstance(error_type, Exception):
            error_record = ErrorRecord(error_type, ErrorContext("unknown", "unknown"), ErrorSeverity.ERROR)
            return error_record.recovery_strategy
        else:
            # Direct category lookup
            error_record = ErrorRecord(Exception("dummy"), ErrorContext("unknown", "unknown"), ErrorSeverity.ERROR)
            error_record.category = error_type
            error_record.recovery_strategy = error_record._get_recovery_strategy()
            return error_record.recovery_strategy
    
    def log_error_with_context(self, error: Exception, step_name: str, 
        ...
        error_record = self.handle_step_error(error, step_name, context=context)
        self._log_error_with_context(error_record)
    
    def _log_error_with_context(self, error_record: ErrorRecord):
