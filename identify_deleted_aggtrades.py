...
def generate_expected_dates(...):
    pass"""Generate all expected dates between start and end date"""
    dates = []
    current_date = start_date
    while current_date <= end_date:
    passdates.append(current_date.strftime("%Y-%m-%d"))
        current_date += timedelta(days=1)
    return dates


def check_existing_files(...):
    pass"""Check which files exist and which are missing"""
    existing_files = []
    missing_files = []

    for date in expected_dates:
    passcsv_file = f"aggtrades_BINANCE_ETHUSDT_{date}.csv"
    ...
    passexisting_files.append(date)
        else:
    passmissing_files.append(date)
