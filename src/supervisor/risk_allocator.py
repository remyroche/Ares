...
class RiskAllocator:
    def __init__(self, config=CONFIG, portfolio_risk_pct=None):
        ...
        self.allocated_capital_multiplier = self.config.get(
            "initial_allocated_capital_multiplier", 1.0
        )
        self.portfolio_risk_pct = (
            portfolio_risk_pct
            if portfolio_risk_pct is not None
            else CONFIG["risk_allocator"].get("portfolio_risk_pct", 0.01)
        )
    ...
    def calculate_dynamic_capital_allocation(self, historical_pnl_data: pd.DataFrame):
        ...
        if historical_pnl_data.empty or len(historical_pnl_data) < lookback_days:
            self.logger.warning(
                "Insufficient historical P&L data. Keeping current allocation."
            )
            return
        ...
        current_effective_capital = (
            self.initial_equity * self.allocated_capital_multiplier
        )
        if current_effective_capital == 0:
            return

        avg_daily_pnl_pct = total_pnl_over_lookback / (
            lookback_days * current_effective_capital
        )
        ...
        min_allowed_multiplier = 1.0 - self.config.get(
            "max_capital_allocation_decrease_pct", 0.75
        )
        max_allowed_multiplier = 1.0 + self.config.get(
            "max_capital_allocation_increase_pct", 1.0
        )

        self.allocated_capital_multiplier = np.clip(
            new_multiplier, min_allowed_multiplier, max_allowed_multiplier
        )

        self.logger.info(
            f"New Allocated Capital Multiplier: {self.allocated_capital_multiplier:.2f}x"
        )
