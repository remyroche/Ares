...
TOP10_CUT = 0.90
DEFAULT_ROLLING_CALIBRATION_OUT = (
    DEFAULT_OUT.parent / "meta_oos_regime_calibration_rolling60d_oos15_20260708"
)

ARCHETYPE_PREFIX_ALIASES = [
    {"prefix": "long__long_mixed_gmm_", "alias": "long_mixed_wideslow_tentative"},
    {
        "prefix": "long__compression_release",
        "alias": "long_volcompression_wideslow_candidate",
    },
    {
        "prefix": "long__loud_breakout_impulse",
        "alias": "long_breakout_diagnostic_candidate",
    },
    {
        "prefix": "long__dirty_shock_avoid",
        "alias": "long_dirtyavoid_sparse_questionable",
    },
    {
        "prefix": "long__volatile_mean_reversion",
        "alias": "long_mixed_wideslow_tentative",
    },
...
def _risk_target(frame: pd.DataFrame) -> pd.Series:
    """Signed execution-risk target.

    Positive values should push a meta score down; negative values indicate
    clean executable opportunity pressure and may push a score up.
    """
...
def _apply_score_calibration(frame: pd.DataFrame, params: dict[str, Any]) -> pd.Series:
    base = _safe_numeric(frame[SCORE_COL]).fillna(0.0).clip(0.0, 1.0)
    risk = _safe_numeric(frame["risk_score"]).fillna(0.0)
    scale = float(params.get("score_scale", 1.0))
    max_up = float(params.get("max_upscore", 0.04))
    max_down = float(params.get("max_downscore", 0.04))
    delta = (-scale * risk).clip(lower=-max_down, upper=max_up)
    mode = str(params.get("score_application_mode", "additive"))
...
def _loss_sequence_metrics(frame: pd.DataFrame) -> dict[str, float]:
    ...
    if (
        loss.size >= 3
        and np.unique(loss).size >= 2
        and np.unique(loss[:-1]).size >= 2
        and np.unique(loss[1:]).size >= 2
    ):
        autocorr = float(
            np.corrcoef(loss[:-1].astype("float64"), loss[1:].astype("float64"))[0, 1]
        )
    else:
        autocorr = float("nan")
    streaks: list[int] = []
...
def _metric_rows(frame: pd.DataFrame, score_col: str, arm: str) -> list[dict[str, Any]]:
    ...
    for scope, cut in {"top10": 0.90, "top20": 0.80, "top30": 0.70}.items():
        ...
        for col in ["month", "week_start", "side_name", ARCH_COL]:
            groups.extend(
                (col, str(k), g)
                for k, g in top.groupby(col, dropna=False, observed=True)
            )
            ...
            rows.append(
                {
                    ...
                    "mean_ev_after_1pct": float(
                        _safe_numeric(g["ev_after_1pct"]).mean()
                    )
                    if len(g)
                    else np.nan,
                    "sum_ev_after_1pct": float(_safe_numeric(g["ev_after_1pct"]).sum())
                    if len(g)
                    else np.nan,
                    "clean_exec_rate": float(_safe_numeric(g["clean_exec"]).mean())
                    if len(g)
                    else np.nan,
                    "dirty_positive_rate": float(
                        _safe_numeric(g["dirty_positive"]).mean()
                    )
                    if len(g)
                    else np.nan,
                    "full_path_bad_mae_rate": float(
                        _safe_numeric(g["full_path_bad_mae_1r"]).mean()
                    )
                    if len(g)
                    else np.nan,
                    "timeout_rate": float(_safe_numeric(g["timeout"]).mean())
                    if len(g)
                    else np.nan,
                    **loss_metrics,
                }
...
def _select_feature_cols(
    ...
    *,
    ...
    for col in feature_cols:
        x = (
            _safe_numeric(sample[col])
            if col in sample.columns
            else pd.Series(np.nan, index=sample.index)
        )
...
class _NativeShallowLGBMRegressor:
    ...
    @staticmethod
    def _matrix(
        frame: pd.DataFrame, columns: list[str], fill_values: np.ndarray | None = None
    ) -> tuple[np.ndarray, np.ndarray]:
        x = frame.reindex(columns=columns).to_numpy(dtype="float32", copy=True)
        x[~np.isfinite(x)] = np.nan
        if fill_values is None:
            fill_values = np.nanmedian(x, axis=0).astype("float32", copy=False)
            fill_values[~np.isfinite(fill_values)] = 0.0
        missing = ~np.isfinite(x)
        if missing.any():
            x[missing] = np.take(fill_values, np.where(missing)[1])
    ...
    def fit(
        ...
        if lgb is None:
            raise RuntimeError(
                "lightgbm is unavailable; cannot fit shallow LGBM calibration"
            )
        self.columns_ = list(x.columns)
        arr, fill_values = self._matrix(x, self.columns_)
        self.fill_values_ = fill_values
        y_arr = np.asarray(y, dtype="float32")
        max_depth = int(self.params.get("max_depth", 2))
        max_leaves = max(2, 2**max_depth)
        ...
        native_params = {
            ...
            "max_depth": max_depth,
            "num_leaves": int(
                np.clip(int(self.params.get("num_leaves", max_leaves)), 2, max_leaves)
            ),
            "min_data_in_leaf": int(self.params.get("min_child_samples", 120)),
            "min_gain_to_split": float(self.params.get("min_split_gain", 0.0)),
            "lambda_l1": float(self.params.get("reg_alpha", 0.0)),
            "lambda_l2": float(self.params.get("reg_lambda", 1.0)),
            "bagging_fraction": float(self.params.get("subsample", 0.85)),
            "feature_fraction": float(self.params.get("colsample_bytree", 0.85)),
            "bagging_freq": 1,
            "seed": 42,
        ...
        self.booster_ = lgb.train(
            native_params,
            train_set,
            num_boost_round=int(self.params.get("n_estimators", 160)),
        )
        return self

    def predict(self, x: pd.DataFrame) -> np.ndarray:
        if self.booster_ is None or self.fill_values_ is None:
            raise RuntimeError("model is not fitted")
        arr, _ = self._matrix(x, self.columns_, self.fill_values_)
        return np.asarray(self.booster_.predict(arr), dtype="float32")


def _make_lgbm_shallow_model(params: dict[str, Any]) -> _NativeShallowLGBMRegressor:
    if lgb is None:
        raise RuntimeError(
            "lightgbm is unavailable; cannot fit shallow LGBM calibration"
        )
    return _NativeShallowLGBMRegressor(params)


def _make_model(family: str, params: dict[str, Any]) -> Any:
    if family == "bucket":
        return _make_spline_model(
            {
                "variant": "two_sided",
                "alpha": 10.0,
                "l1_ratio": 0.9,
    ...
    if family == "lgbm_shallow":
        return _make_lgbm_shallow_model(params)
    raise ValueError(f"unknown calibration family: {family}")


def _fit_kwargs(family: str, frame: pd.DataFrame) -> dict[str, Any]:
    if family == "lgbm_shallow":
        return {"sample_weight": _sample_weight(frame)}
    if family in {"gam", "ebm"}:
        return {"sample_weight": _sample_weight(frame)}
    return {}
...
def _safe_name(value: Any) -> str:
    return (
        str(value).replace("/", "_").replace(" ", "_").replace(":", "").replace("+", "")
    )
...
def _fit_predict_window(
    ...
    *,
    ...
    train_all = frame.loc[ts.ge(window["train_start"]) & ts.lt(window["train_end"])]
    eval_all = frame.loc[ts.ge(window["valid_from"]) & ts.lt(window["valid_to"])].copy()
    eval_all["risk_score"] = 0.0
    eval_all["risk_effect_count"] = 0
    effects: list[dict[str, Any]] = []
    if train_all.empty or eval_all.empty:
        return eval_all, effects
    for (side, arch), eval_group in eval_all.groupby(
        ["side_name", ARCH_COL], dropna=False, observed=True
    ):
        train = train_all.loc[
        ...
        try:
            model.fit(x_train, y_train, **_fit_kwargs(family, train))
            ...
            effects.append(
                {
                    ...
                    "fill_values": {
                        col: float(_safe_numeric(train[col]).median())
    ...
    eval_all["risk_score"] = _safe_numeric(eval_all["risk_score"]).clip(
        -cap_neg, cap_pos
    )
    eval_all["score_regime_calibrated"] = _apply_score_calibration(eval_all, params)
    return eval_all, effects
...
def _train_latest_effects(
    ...
    *,
    ...
    if train_all.empty:
        return effects
    for (side, arch), group in train_all.groupby(
        ["side_name", ARCH_COL], dropna=False, observed=True
    ):
        if len(group) < 250:
            continue
        ...
        model = _make_model(family, params)
        try:
            model.fit(x, y, **_fit_kwargs(family, group))
        except Exception:
            continue
        save_models_dir.mkdir(parents=True, exist_ok=True)
        safe_side = _safe_name(side)
        ...
        effects.append(
            {
                ...
                "candidate_feature_quota": int(candidate_feature_quota),
                "fill_values": {
                    col: float(_safe_numeric(group[col]).median())
...
def _suggest(
    ...
    allowed = families or ["bucket", "spline", "gam", "ebm", "lgbm_shallow"]
    family = trial.suggest_categorical("family", allowed)
    params: dict[str, Any] = {
        "risk_cap_negative": trial.suggest_categorical(
            "risk_cap_negative", [0.04, 0.06, 0.08, 0.10]
        ),
        "risk_cap_positive": trial.suggest_categorical(
            "risk_cap_positive", [0.02, 0.04, 0.06]
        ),
        "max_features": trial.suggest_categorical("max_features", [6, 8, 10]),
        "score_application_mode": trial.suggest_categorical(
            "score_application_mode", ["additive", "multiplicative"]
        ),
        "score_scale": trial.suggest_categorical(
            "score_scale", [0.5, 1.0, 2.0, 4.0, 8.0, 12.0, 16.0]
        ),
        "max_upscore": trial.suggest_categorical(
            "max_upscore", [0.005, 0.01, 0.02, 0.04, 0.06, 0.10]
        ),
        "max_downscore": trial.suggest_categorical(
            "max_downscore", [0.005, 0.01, 0.02, 0.04, 0.06, 0.10]
        ),
    }
    if family == "spline":
        params.update(
            {
                ...
                "n_splines": trial.suggest_categorical(
                    "spline_n_splines", [5, 6, 7, 8]
                ),
        ...
        params.update(
            {
                ...
                "interactions": trial.suggest_categorical(
                    "ebm_interactions", [0, 1, 2, 3]
                ),
                "max_leaves": trial.suggest_categorical("ebm_max_leaves", [2, 3, 4]),
                "max_rounds": trial.suggest_categorical(
                    "ebm_max_rounds", [160, 240, 320]
                ),
                "min_samples_leaf": trial.suggest_categorical(
                    "ebm_min_samples_leaf", [120, 200, 360]
                ),
                "outer_bags": trial.suggest_categorical("ebm_outer_bags", [1, 2]),
            }
        )
    elif family == "lgbm_shallow":
        max_depth = trial.suggest_categorical("lgbm_max_depth", [2, 3])
        params.update(
            {
                "max_depth": max_depth,
                "num_leaves": trial.suggest_int(
                    "lgbm_num_leaves", 3, 2 ** int(max_depth)
                ),
                "n_estimators": trial.suggest_categorical(
                    "lgbm_n_estimators", [80, 120, 180, 260]
                ),
                "learning_rate": trial.suggest_categorical(
                    "lgbm_learning_rate", [0.015, 0.025, 0.04, 0.06]
                ),
                "min_child_samples": trial.suggest_categorical(
                    "lgbm_min_child_samples", [80, 120, 180, 260, 360]
                ),
                "min_split_gain": trial.suggest_categorical(
                    "lgbm_min_split_gain", [0.0, 1e-4, 5e-4, 1e-3, 5e-3]
                ),
                "reg_alpha": trial.suggest_categorical(
                    "lgbm_reg_alpha", [0.0, 0.05, 0.2, 0.5, 1.0, 2.0]
                ),
                "reg_lambda": trial.suggest_categorical(
                    "lgbm_reg_lambda", [0.1, 0.5, 1.0, 2.0, 5.0, 10.0]
                ),
                "subsample": trial.suggest_categorical(
                    "lgbm_subsample", [0.65, 0.80, 0.95, 1.0]
                ),
                "colsample_bytree": trial.suggest_categorical(
                    "lgbm_colsample_bytree", [0.65, 0.80, 0.95, 1.0]
                ),
            }
        )
...
def _load_merged(
    ...
    pred = _derive_prediction_features(
        _load_predictions(args.meta_run, args.all_months)
    )
    ...
    features = (
        _load_feature_slice(args.handoff, pred, candidate_cols)
        if candidate_cols
        else pd.DataFrame(columns=KEYS)
    )
    merged = (
        pred.merge(features, on=KEYS, how="left", validate="many_to_one")
        if len(features)
        else pred
    )
    if getattr(args, "additional_feature_frame", None):
        ...
        if extra_path.exists():
            ...
            join_cols = [
                col for col in KEYS if col in extra.columns and col in merged.columns
            ]
            if len(join_cols) == len(KEYS):
                extra["__ts__"] = pd.to_datetime(
                    extra["__ts__"], utc=True, errors="coerce"
                )
    ...
    feature_map = {
        name: col for name, col in chosen.items() if col and col in merged.columns
    }
    ...
    if manifest_path is not None and manifest_path.exists():
        try:
            ...
            payload = {}
        for group_key, features_for_group in dict(
            payload.get("feature_map") or {}
        ).items():
            if isinstance(features_for_group, list):
                cols = [
                    str(col) for col in features_for_group if str(col) in merged.columns
                ]
                if cols:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        / "meta_oos_regime_calibration_model_ablation_rolling60d_oos15_20260708",
    )
    parser.add_argument(
        "--artifact-output-dir", type=Path, default=DEFAULT_ROLLING_CALIBRATION_OUT
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--families",
        nargs="+",
        default=["bucket", "spline", "gam", "ebm", "lgbm_shallow"],
        choices=["bucket", "spline", "gam", "ebm", "lgbm_shallow"],
        help=(
            "Calibration model families to search. Use '--families ebm lgbm_shallow' "
            "for the EBM-vs-shallow-LGBM comparison."
        ),
    )
    parser.add_argument(
    ...
    if "lgbm_shallow" in families and lgb is None:
        print(
            "[warn] lightgbm unavailable; dropping lgbm_shallow from calibration families"
        )
        families = [family for family in families if family != "lgbm_shallow"]
    if not families:
        raise RuntimeError("No calibration families are available")
    ...
    study.optimize(
        ...
        callbacks=[NoImprovementStopper(args.patience)],
    )
    ...
    (args.output_dir / "best_hpo.json").write_text(
    ...
    metrics = pd.DataFrame(
        [
            ...
            *_metric_rows(
                winner_adjusted, "score_regime_calibrated", f"winner_{best_family}"
            ),
        ]
    )
    base_overall = metrics.loc[
        (metrics["arm"].eq("baseline_meta")) & (metrics["group"].eq("overall"))
    ]
    win_overall = metrics.loc[
        (metrics["arm"].eq(f"winner_{best_family}")) & (metrics["group"].eq("overall"))
    ]
    delta = win_overall.merge(
        base_overall,
        on=["top_scope", "group", "group_value"],
        suffixes=("_winner", "_baseline"),
    )
    delta_cols = [
        "mean_ev_after_1pct",
        "clean_exec_rate",
    ...
    month_base = metrics.loc[
        (metrics["arm"].eq("baseline_meta")) & (metrics["group"].eq("month"))
    ]
    month_win = metrics.loc[
        (metrics["arm"].eq(f"winner_{best_family}")) & (metrics["group"].eq("month"))
    ]
    month_delta = month_win.merge(
        month_base,
        on=["top_scope", "group", "group_value"],
        suffixes=("_winner", "_baseline"),
    )
    for col in delta_cols:
        month_delta[f"delta_{col}"] = (
            month_delta[f"{col}_winner"] - month_delta[f"{col}_baseline"]
        )
    month_delta.to_csv(
        args.output_dir / "winner_month_delta_vs_baseline.csv", index=False
    )
    ...
    latest_valid_to = max(
        (pd.Timestamp(w["valid_to"]) for w in rolling_windows), default=None
    )
    if latest_valid_to is not None:
        latest_effects = _train_latest_effects(
            ...
            candidate_feature_map=candidate_feature_map,
            candidate_feature_quota=int(args.candidate_feature_quota),
            train_start=latest_valid_to
            - pd.Timedelta(days=int(args.rolling_train_days)),
            train_end=latest_valid_to,
            valid_from=latest_valid_to,
            save_models_dir=args.artifact_output_dir
            / "models"
            / "latest_live_fallback",
            model_base_dir=args.artifact_output_dir,
        )
    ...
    artifact = {
        ...
        "score_application": {
            "mode": str(best_params.get("score_application_mode", "additive")),
            "scale": float(best_params.get("score_scale", 1.0)),
            "max_upscore": float(best_params.get("max_upscore", 0.04)),
            "max_downscore": float(best_params.get("max_downscore", 0.04)),
        },
        "hpo": best_payload,
        "time_windowed_effects": True,
        "latest_valid_to": latest_valid_to.isoformat()
        if latest_valid_to is not None
        else None,
    ...
    (args.output_dir / "artifact_manifest.json").write_text(
        json.dumps(
            {
                "artifact_path": str(
                    args.artifact_output_dir / "regime_ev_calibration.json"
                ),
                **best_payload,
            },
            indent=2,
