...
class PositionSizer:
    ...
    async def calculate_position_size(
        ...
        """Calculate position size using ML intelligence and Kelly criterion."""
    ...
    def _calculate_kelly_position_size(
        ...
        try:
            ...
            if not price_target_confidences or not adversarial_confidences:
                return self.min_position_size

            # Calculate average confidence and risk
            avg_confidence = sum(price_target_confidences.values()) / len(price_target_confidences)
            avg_risk = sum(adversarial_confidences.values()) / len(adversarial_confidences)

            # Simplified Kelly calculation
            kelly_fraction = max(0, avg_confidence - avg_risk)
            kelly_multiplier = min(1.0, kelly_fraction * self.kelly_multiplier)
    ...
    def _calculate_weighted_position_size(
        ...
        """Calculate weighted position size combining Kelly and ML approaches."""
        try:
            # Calculate weighted position size
    ...
    def _apply_position_size_modifiers(
        ...
        """Apply market health and risk modifiers to position size."""
        try:
            adjusted = base_size
    ...
    def _generate_sizing_reason(
        ...
        """Generate human-readable reason for position sizing decision."""
        try:
    ...
    def _get_historical_performance(self) -> tuple[float, float]:
        """Get historical performance metrics for Kelly criterion."""
        try:
    ...
    def get_position_sizing_history(self, limit: Optional[int] = None) -> list[Dict[str, Any]]:
        """Get position sizing history with optional limit."""
        if limit:
...
async def setup_position_sizer(config: Optional[Dict[str, Any]] = None) -> Optional[PositionSizer]:
    """Setup and return a configured position sizer."""
    try:
        if config is None:
            config = {}
