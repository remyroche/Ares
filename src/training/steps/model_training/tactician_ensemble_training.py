...
class TacticianEnsembleTrainingStep(EnsembleTrainingStep, BaseStep):
    """
    ...
    async def execute(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.execute(
                ...
                base_model_outputs=base_model_outputs.values if base_model_outputs is not None else None
            )
            
            if result.get('success', False):
                # Save ensemble model
                if 'ensemble_model' in result:
                ...
                training_summary = {
                    'component_name': 'tactician_ensemble_training',
                    'status': 'completed',
                    'ensemble_model': 'ensemble_model' in result,
                    'individual_models': list(result.get('individual_models', {}).keys()),
                    'training_time': result.get('training_time', 0),
                    'performance_metrics': result.get('ensemble_metrics', {}),
                ...
                return {
                    ...
                    'base_models_count': len(result.get('individual_models', {})),
                    'training_time': result.get('training_time', 0)
                }
            else:
                return {
                    'success': False,
                    'error': f"Tactician ensemble training failed: {result.get('error', 'Unknown error')}",
                    'artifacts': [],
                    'metrics': {}
                }
