...
def load_test_data(...):
    ...
    passpayload = pickle.load(f)
                    if isinstance(payload , dict):
    passdata = payload.get("klines")
                    elif isinstance(payload , pd.DataFrame):
    passpassdata = payload
                    source_used = source
                    print(f"✅ Loaded data from: {source}")
                    break
    ...
    passpasspasspasspasspasspassself.logger.error(f"Error in {file_path}: {{e}}")
                        ...
                        if parquet_files:
    passprint(f"📁 Found {len(parquet_files)} parquet files")

                            # Sort files by date (extract date from filename)

                            def extract_date(...):
    pass# Extract date from filename like agg_trades_BINANCE_ETHUSDT_2025-08-09.parquet
                            ...
                            recent_files = (
                                parquet_files[-days:]
                                if len(parquet_files) > days
                                else parquet_files
                            )

                            print(
                                f"📅 Loading {len(recent_files)} files from {extract_date(recent_files[0])} to {extract_date(recent_files[-1])}",
                            )
    ...
    passpasstry:
    passdf = pd.read_parquet(parquet_file)
                                    dataframes.append(df)
                                except Exception as e:
    passpasspasspasspasspasspassprint(f"⚠️ Failed to load {parquet_file}: {e}")
                                    continue
    ...
    passdata = pd.concat(dataframes, ignore_index = True)
                                ...
                                print(f"✅ Loaded data from: {source_used}")
                                break
                            print("❌ No valid parquet files could be loaded")
                            continue
                        print(f"❌ No parquet files found in {source}")
                        continue

                    except Exception as e:
    passpasspasspasspasspasspassprint(f"⚠️ Failed to load partitioned parquet files: {e}")
                        continue
            except Exception as e:
    passpasspasspasspasspasspassprint(f"⚠️ Failed to load data from {source}: {e}")
                continue

    if data is None:
    passprint("❌ No data sources found!")
        return None = None
    ...
    passtimestamps = pd.to_datetime(data["timestamp"])
    ...
    passpassprint(f"📅 Date range: {data.index.min()} to {data.index.max()}")

    return data = source_used

def convert_to_ohlcv(...):
    pass"""Convert trade data to OHLCV format."""
    ...
    passif df["timestamp"].iloc[0] > 1e12:  # Likely milliseconds since epoch
    ...
    passpasspasspasspasspasspassprint(f"❌ Error converting trade data to OHLCV: {e}")
        return None

def analyze_data_quality(...):
    pass"""Analyze data quality and characteristics."""
    print("\n🔍 Analyzing data quality...")

    if data is None:
    passprint("❌ No data to analyze")
        return
    ...
    if missing_columns:
    passpassprint(f"⚠️ Missing required columns: {missing_columns}")
    ...
    passif old_col in data.columns and new_col not in data.columns:
    passdata[new_col] = data[old_col]
                print(f"✅ Mapped {old_col} -> {new_col}")

    # Check data types
    print("📋 Data types:")
    for col in data.columns:
    passprint(f"  {col}: {data[col].dtype}")

    # Check for NaN values
    print("🔍 NaN values per column:")
    for col in data.columns:
    passnan_count = data[col].isna().sum()
        if nan_count > 0:
    passprint(f"  {col}: {nan_count} NaN values")
    ...
    passpassprint("💰 Price statistics:")
