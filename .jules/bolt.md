...
## 2026-05-19 - Vectorized Prediction Ledger Resolution Updates
Learning: Looping row-by-row using `iterrows()` to apply resolution updates in `PredictionLedger.mark_resolved` is a severe performance bottleneck. Processing a few thousand updates this way incurs substantial Pandas object setup overhead, compounding execution time, particularly when row schema differs requiring `to_frame().T` instantiation.
Action: Replace `.iterrows()` nested `.loc` logic with a vectorized index intersection block (`upd_idx.index.intersection(old_idx.index)`). Deduplicate updates (`keep='last'`), align schemas by natively assigning `float("nan")` to missing columns instead of using `pd.NA` to prevent introducing 'object' dtypes, and then do bulk assignment `old_idx.loc[common_idx] = upd_idx.loc[common_idx]`.
