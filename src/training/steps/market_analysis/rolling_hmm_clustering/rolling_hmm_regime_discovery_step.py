...
class RollingHMMRegimeDiscoveryStep(BaseStep):
    ...
    def __init__(self, step_name: str = "rolling_hmm_regime_discovery"):
        ...
        tprint(f"â Initialized {step_name} step", "SUCCESS")

    @property
    def quality_assessor(self) -> ClusterQualityAssessor:
        """Lazy property for quality assessor."""
        tprint_debug("đ Accessing quality assessor instance")
        if self._quality_assessor is None:
            tprint_info("  â Initializing ClusterQualityAssessor")
    ...
    async def execute(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if timeframe != regime_timeframe:
            tprint(
                f"â° Using regime_timeframe={regime_timeframe} for Rolling HMM "
        ...
        tprint(
            f"đ Starting Rolling HMM Regime Discovery for {symbol} on {exchange} "
            f"(timeframe: {timeframe})",
            "INFO"
        )
        tprint_debug("đ§  Enhanced Features: EWMA rolling (8+16 to 12+24), Returns/Vol/Trend/Volume, PCA (3-5), Sticky HMM, VectorBT+M1")
        ...
        try:
            ...
            # Load market data
            tprint("đĽ Loading market data...", "INFO")

            market_data = self._load_market_data(symbol, exchange, timeframe, config)

            if market_data is None or market_data.empty:
                tprint_error(f"â No market data available for {symbol} on {timeframe}")
                raise ValueError(f"No market data available for {symbol} on {timeframe}")

            tprint(f"đ DEBUG: [STEP 1] market_data after _load_market_data: {market_data.shape}", "INFO")
            tprint(f"đ DEBUG: [STEP 1] Index range: {market_data.index.min()} to {market_data.index.max()}", "INFO")
            tprint(f"â Loaded {len(market_data)} samples of market data", "SUCCESS")
            ...
            try:
                if self.hardware_manager is not None and hasattr(self.hardware_manager, 'memory_optimizer'):
                    ...
                    if hasattr(optimizer, 'optimize_dataframe_memory'):
                        ...
                        tprint_debug("đ§  Applied M1 memory optimization to market_data")
            except Exception as e:
                tprint_debug(f"â ď¸ M1 memory optimization skipped due to error: {e}")
            ...
            tprint_info(f"đ§ Configuration check: execution_mode={execution_mode}, enable_auto_tuning={enable_auto_tuning}")
            ...
            if has_manual_params:
                tprint_info(f"đ Manual params found: {list(config['rolling_hmm_params'].keys())}")
            ...
            tprint(f"đ DEBUG: [STEP 2] market_data after _apply_execution_mode_filter: {market_data.shape}", "INFO")
            tprint(f"   â After execution mode filter ({execution_mode}): {len(market_data)} samples")
            ...
            if enable_auto_tuning:
                tprint("")
                tprint("đ Pre-computing features for HPO efficiency")
                all_cached_features = feature_engineer.precompute_all_features(market_data)
                tprint(f"â Cached features for {len(all_cached_features)} EWMA configurations")
                tprint("")
            ...
            if 'rolling_hmm_params' in config and config['rolling_hmm_params']:
                if config.get('enable_auto_tuning') is False:
                    ...
                    tprint_info("âšď¸  Manual params provided with enable_auto_tuning=False, skipping HPO")
                else:
                    tprint_info("âšď¸  Manual params provided but enable_auto_tuning=True (default), running HPO")
            
            # Show execution mode info
            if enable_auto_tuning and execution_mode in ['light', 'blank']:
                tprint_info(f"âšď¸  HPO enabled in '{execution_mode}' mode (will use reduced trials for speed)")
            
            if enable_auto_tuning:
                tprint("", "INFO")
                tprint("đŻ HPO Enabled - Finding Optimal Hyperparameters", "INFO")
                tprint("=" * 80, "INFO")
                ...
                if hpo_results and best_params:
                    tprint("", "INFO")
                    tprint("â HPO Complete - Using Optimal Parameters", "SUCCESS")
                    ...
                    tprint_warning("â ď¸  HPO did not complete, using default parameters")
                
                tprint("=" * 80, "INFO")
                tprint("", "INFO")
            
            # Run clustering
            tprint("đ Running Rolling HMM clustering...", "INFO")
            ...
            tprint("", "SUCCESS")
            tprint(f"â Rolling HMM Regime Discovery completed in {execution_time:.2f}s", "SUCCESS")
    ...
    def _load_market_data(
        ...
        if 'market_data' in config and config['market_data'] is not None:
            ...
            tprint(f"đ [REGIME_DISCOVERY] Validating config market_data:", "INFO")
            tprint(f"   â Execution mode: {execution_mode}", "INFO")
            tprint(f"   â Timeframe: {timeframe}", "INFO")
            tprint(f"   â Lookback days for mode: {lookback_days}", "INFO")
            tprint(f"   â Expected min samples ({lookback_days} days): {expected_min_samples:,}", "INFO")
            tprint(f"   â Actual samples in config: {actual_samples:,}", "INFO")

            if actual_samples < expected_min_samples * 0.3:  # Allow 30% tolerance
                tprint(
                    f"â [REGIME_DISCOVERY] CRITICAL: config['market_data'] has only {actual_samples:,} samples!\n"
                ...
                tprint(f"â Using market data from config ({len(external_data)} samples)", "SUCCESS")
                return external_data

        # Load using KlinesParquetManager (same method as regime_models_training)
        tprint(f"đĽ [REGIME_DISCOVERY] Loading fresh data for {symbol} from historical storage", "INFO")
        ...
        try:
            ...
            tprint(f"đĽ [REGIME_DISCOVERY] Loading last {lookback_days} days from latest available data (mode: {execution_mode})", "INFO")
            ...
            if fresh_data is None or len(fresh_data) == 0:
                tprint(f"â [REGIME_DISCOVERY] No data found in historical storage", "ERROR")
                raise ValueError(f"No historical data found for {symbol} {exchange} {timeframe}")

            tprint(f"đ DEBUG: KlinesParquetManager returned data: {fresh_data.shape if fresh_data is not None else 'None'}", "INFO")

            if fresh_data is not None and len(fresh_data) > 0:
                tprint(f"â [REGIME_DISCOVERY] Loaded {len(fresh_data):,} rows from historical storage (last {lookback_days} days)", "SUCCESS")
                ...
                if fresh_data.index.duplicated().any():
                    ...
                    tprint(f"â ď¸ [REGIME_DISCOVERY] Found {n_duplicates} duplicate timestamps, removing duplicates", "WARNING")
                    fresh_data = fresh_data[~fresh_data.index.duplicated(keep='first')]
                    tprint(f"â [REGIME_DISCOVERY] After deduplication: {len(fresh_data):,} rows", "SUCCESS")
                ...
                tprint(f"đ [REGIME_DISCOVERY] Data validation: Expected ~{expected_samples:,} samples for {lookback_days} days of {timeframe} data", "INFO")
                tprint(f"đ [REGIME_DISCOVERY] Data validation: Actual samples: {actual_samples:,}", "INFO")

                if actual_samples < expected_samples * 0.5:
                    tprint(f"â ď¸ [REGIME_DISCOVERY] WARNING: Only {actual_samples:,} samples available (expected ~{expected_samples:,})", "WARNING")

                tprint(f"đ [REGIME_DISCOVERY] Date range: {fresh_data.index.min()} to {fresh_data.index.max()}", "INFO")
                return fresh_data

        except Exception as e:
            tprint(f"â [REGIME_DISCOVERY] Failed to load from KlinesParquetManager: {e}", "ERROR")
            import traceback
            tprint(f"â [REGIME_DISCOVERY] Traceback: {traceback.format_exc()}", "ERROR")
        ...
        try:
            for step_name, artifact_name in artifact_sources:
                try:
                    ...
                    tprint(f"đ DEBUG: Artifact {step_name}/{artifact_name} returned: {market_data.shape if market_data is not None and hasattr(market_data, 'shape') else 'None/Invalid'}", "INFO")

                    if market_data is not None and not market_data.empty:
                        # Log data size but don't reject it - let the pipeline handle insufficient data
                        actual_samples = len(market_data)
                        tprint(f"â Loaded market data from {step_name}/{artifact_name} ({actual_samples:,} samples)", "SUCCESS")
                        ...
                        if actual_samples < expected_min_samples * 0.5:
                            tprint(
                                f"â ď¸ [REGIME_DISCOVERY] WARNING: Only {actual_samples:,} samples available\n"
        ...
        tprint(
            f"â ď¸ Could not load market data for {symbol} on {timeframe} from artifacts",
            "WARNING",
        )
    ...
    def _initialize_hardware_optimization(self):
        ...
        tprint_info("âĄ Initializing hardware optimization for M1")
        ...
        try:
            ...
            try:
                ...
                if cap < total_samples and cap > 0:
                    ...
                    tprint_info(f"đ§ HPO using subsample of {len(hpo_market_data)} rows out of {total_samples} (rolling_hmm_params cap)")
            except Exception as e:
                tprint_debug(f"â ď¸ HPO subsampling disabled due to error: {e}")
            ...
            if result and result['best_params']:
                ...
                tprint_warning("â ď¸  HPO returned no results")
                return None, None

        except Exception as e:
            tprint_error(f"â HPO failed: {e}")
    ...
    async def _run_clustering(
        ...
        try:
            ...
            tprint_info(f"  â Using EWMA config: {ewma_config.name}")

            # Generate features
            tprint(f"đ DEBUG: [STEP 3] market_data before feature generation: {market_data.shape}", "INFO")
            features = feature_engineer.generate_features(market_data, ewma_config)
            tprint(f"đ DEBUG: [STEP 4] features after generation: {features.shape}", "INFO")
            tprint(f"đ DEBUG: [STEP 4] features index range: {features.index.min()} to {features.index.max()}", "INFO")
            ...
            tprint(f"đ DEBUG: [STEP 5] features_economic after extraction: {features_economic.shape}", "INFO")
            tprint(f"đ DEBUG: [STEP 5] features_economic index range: {features_economic.index.min()} to {features_economic.index.max()}", "INFO")
            tprint(f"đ DEBUG: [STEP 5] Economic features: {list(features_economic.columns)}", "INFO")
            ...
            tprint_info(f"  â PCA explained variance (economic features): {pca_explained:.2%}")
            ...
            # OPTIMIZED: Reduced n_iter from 200 to 50 (5-10x faster convergence)
            ...
            hmm_config = StickyHMMConfig(
                ...
                kappa=float(kappa),
                n_iter=params.get('n_iter', 50),  # Reduced from 200 for faster training
            ...
            tprint_info(f"  â HMM config: n_components={n_components}, kappa={kappa}, min_covar={min_covar}")
            ...
            tprint(f"đ DEBUG: [STEP 6] regime_labels after HMM predict: shape={regime_labels.shape}, unique={np.unique(regime_labels)}", "INFO")
            tprint(f"đ DEBUG: [STEP 6] regime_probs after HMM predict: {regime_probs.shape}", "INFO")
            ...
            # Assess quality
            tprint_info("  â Assessing regime quality")

            # Diagnostic: Check regime transitions
            unique_regimes = np.unique(regime_labels)
            regime_transitions = np.sum(regime_labels[1:] != regime_labels[:-1])
            tprint_info(f"  â Found {len(unique_regimes)} unique regimes, {regime_transitions} transitions in {len(regime_labels)} samples")
            ...
            tprint_error(f"â Clustering failed: {e}")
    ...
    async def _save_results(
        ...
        try:
            tprint(f"đ DEBUG: [STEP 7] _save_results called", "INFO")
            tprint(f"đ DEBUG: [STEP 7] result['timestamps'] length: {len(result['timestamps'])}", "INFO")
            tprint(f"đ DEBUG: [STEP 7] result['regime_labels'] length: {len(result['regime_labels'])}", "INFO")
            tprint(f"đ DEBUG: [STEP 7] result['regime_probs'] shape: {result['regime_probs'].shape}", "INFO")
            ...
            tprint(f"đ DEBUG: [STEP 8] labels_df shape after creation: {labels_df.shape}", "INFO")
            tprint(f"đ DEBUG: [STEP 8] labels_df index range: {labels_df.index.min()} to {labels_df.index.max()}", "INFO")
            ...
            tprint_info(f"  â Creating probs_df with {len(timestamps)} timestamps")
            tprint_info(f"  â Timestamp type: {type(timestamps)}, dtype: {timestamps.dtype if hasattr(timestamps, 'dtype') else 'N/A'}")
            tprint_info(f"  â Timestamp range: {timestamps.min()} to {timestamps.max()}")
            tprint_info(f"  â Regime probs shape: {result['regime_probs'].shape}")
            ...
            tprint_info(f"  â probs_df shape: {probs_df.shape}")
            tprint_info(f"  â probs_df index type: {type(probs_df.index)}")
            tprint_info(f"  â probs_df index range: {probs_df.index.min()} to {probs_df.index.max()}")
            ...
            tprint(f"đ DEBUG: [STEP 9] About to save labels_df to HDF5: {labels_df_to_save.shape}", "INFO")
            tprint(f"đ DEBUG: [STEP 9] labels_df columns: {list(labels_df_to_save.columns)}", "INFO")
            self._save_artifact(
            ...
            tprint(f"â DEBUG: [STEP 9] Successfully saved rolling_hmm_regime_labels", "SUCCESS")
            ...
            tprint_info(f"  â Saving probs_df with {len(probs_df_to_save)} rows")
            tprint_info(f"  â Columns: {list(probs_df_to_save.columns)}")
            ...
            tprint_error(f"â Failed to save results: {e}")
    ...
    async def _generate_reports(
        ...
        try:
            ...
            tprint("", "INFO")
            tprint("=" * 80, "INFO")
            tprint("đ Rolling HMM Clustering Quality Report", "INFO")
            tprint("=" * 80, "INFO")
            ...
            if hpo_results is None:
                tprint("â ď¸ No HPO results found in result or config", "WARNING")
            elif not hpo_results:
                tprint("â ď¸ HPO results found but empty dictionary", "WARNING")
            elif not isinstance(hpo_results, dict):
                tprint(f"â ď¸ HPO results found but wrong type: {type(hpo_results)}", "WARNING")
                hpo_results = None  # Reset to None to avoid further issues
            
            if hpo_results:
                tprint(f"đ Found HPO results with keys: {list(hpo_results.keys())}", "INFO")
                trial_keys = ['coarse_results', 'fine_results', 'refinement_results', 'second_round_results']
                all_trials = []
                for key in trial_keys:
                    trials = hpo_results.get(key)
                    if isinstance(trials, list):
                        tprint(f"đ Processing {len(trials)} trials from {key}", "INFO")
                        for trial in trials:
                        ...
                        tprint(f"đ No trials found for key {key} (type: {type(trials)})", "INFO")
            else:
                tprint("â ď¸ No HPO results found in result or config", "WARNING")
                # Debug: check what keys are available
                available_keys = list(result.keys()) if result else []
                if available_keys:
                    tprint(f"đ Available result keys: {available_keys}", "INFO")
                config_keys = list(config.keys()) if config else []
                if config_keys:
                    tprint(f"đ Available config keys: {config_keys}", "INFO")

            if all_trials:
                tprint(f"đ Passing {len(all_trials)} trials to comprehensive CSV report generation", "INFO")
            else:
                tprint("â ď¸ No trials available for all-trials CSV export", "WARNING")
            ...
            tprint("", "INFO")
            tprint("=" * 80, "INFO")
            tprint("đ° Generating Economic Relevance Analysis", "INFO")
            tprint("=" * 80, "INFO")
            ...
            try:
                ...
                if isinstance(metrics_dict, dict):
                    ...
                    if not regime_types:
                        ...
                        if fallback_types:
                            ...
                            tprint_info(
                                f"  â Fallback regime classification inferred from returns: {regime_types}"
                            )
                        else:
                            detected_types = None
                    if regime_types:
                        tprint_info(
                            f"  â Using regime type mapping for economic analysis: {regime_types}"
                        )
                ...
                tprint_info("  â Evaluating trading strategies based on regimes")
                ...
                tprint_warning(f"â ď¸  Economic relevance analysis failed: {e}")
                self.logger.warning(f"Economic relevance analysis failed: {e}", exc_info=True)

        except Exception as e:
            tprint_warning(f"â ď¸  Failed to generate reports: {e}")
    ...
    def _infer_regime_types_from_metrics(self, metrics_dict: Dict[str, Any]) -> Dict[int, str]:
        ...
        for r in regime_data_list:
            ...
            tprint_debug(
                "  [RegimeType] Regime {} â mean={:.6f}, vol={:.6f}, sharpe={:.4f}".format(
    ...
    def _handle_execution_error(
        ...
        tprint(f"â Rolling HMM Regime Discovery failed: {error_msg}", "ERROR")
    ...
    def _apply_execution_mode_filter(
        ...
        if exec_mode == 'full':
            tprint_info("  â Full mode: Using all available data (no filtering)")
            return data
        ...
        if days_limit is None:
            tprint_info(f"  â {exec_mode.capitalize()} mode: No explicit day limit configured, using all available data")
            return data

        if exec_mode == 'blank':
            tprint_info(f"  â Blank mode: Using {days_limit} days of data (centralized config)")
        elif exec_mode == 'light':
            tprint_info(f"  â Light mode: Using {days_limit} days of data (centralized config)")
        else:
            tprint_info(f"  â {exec_mode.capitalize()} mode: Using {days_limit} days of data (centralized config)")

        # Calculate sample limit
        samples_per_day = samples_per_day_map.get(timeframe, 24)
        limit = days_limit * samples_per_day
        
        tprint_info(f"  â Data filtering: {days_limit} days Ă {samples_per_day} samples/day = {limit} samples limit")

        # Apply filter
        if len(data) > limit:
            filtered = data.tail(limit).copy()  # Keep most recent data
            tprint_info(f"  â {execution_mode.capitalize()} mode: Filtered to {days_limit} days ({limit} samples)")
            tprint_info(f"  â Filtered data range: {filtered.index.min()} to {filtered.index.max()}")
            return filtered

        tprint_info(f"  â {execution_mode.capitalize()} mode: Data size ({len(data)}) within limit ({limit} samples)")
        tprint_info(f"  â Data range: {data.index.min()} to {data.index.max()}")
        return data
