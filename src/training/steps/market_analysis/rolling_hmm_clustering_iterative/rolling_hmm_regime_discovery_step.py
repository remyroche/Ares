...
class RollingHMMRegimeDiscoveryStep(BaseStep):
    ...
    def _get_hpo_config(self, config: Dict[str, Any]) -> HPOConfig:
        ...
        if execution_mode == 'light':
            ...
            hpo_config['cv_folds'] = hpo_config.get('cv_folds', 3)  # 3 folds en mode light (amélioration demandée)
            # 🔍 LOGGING: Configuration de validation croisée en mode light
            tprint(f"🔍 [LIGHT_MODE] Configuration CV: 3 folds (réduction de 5 à 3 pour accélérer le traitement)", color="blue")
            tprint(f"   → Impact: Réduction du temps de validation croisée de ~40%", color="blue")
            tprint(f"   → Conséquence: Moins de robustesse statistique mais exécution plus rapide", color="blue")
        elif execution_mode == 'blank':
            hpo_config['final_refinement_trials'] = hpo_config.get('final_refinement_trials', 5)
            hpo_config['cv_folds'] = hpo_config.get('cv_folds', 3)  # 3 folds en mode blank
