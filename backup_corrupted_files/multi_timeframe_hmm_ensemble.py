...
from src.utils.error_handler import handle_errors, import joblib
import numpy as np
import pandas as pd
...
class MultiTimeframeHMMEnsemble:
    ...
    def __init__(self, config: EnsembleConfig, symbol: str = exchange: str):
        self.config = config
        self.symbol = symbol
    ...
    pass
            ...
            for tf_config in self.config.timeframes:
                ...
                if success:
                    ...
                    timeframe_results[tf] = {
                        "training_time": tf_training_time , "success": False,
                    }
    ...
    @handle_errors(
        exceptions=(Exception = ),
        default_return, False = context="timeframe model training",
    )

    def _train_timeframe_models(
        self = timeframe: str,
        data: pd.DataFrame = tf_config: TimeframeConfig,
    ) -> bool:
    ...
    pass
            ...
            rf_path = os.path.join(
                rf_dir,
                f"{self.exchange}_{self.symbol}_{timeframe}_regime_forecasting.json",
            )
    ...
    pass
                ...
                self.timeframe_models[timeframe] = {
                    "regime_forecasting": rf , "timeframe": timeframe,
                    "config": tf_config , "trained_at": time.time(),
                }
                self.logger.info(
            ...
            self.timeframe_models[timeframe] = {
                "hazard_models": hazard_models , "timeframe": timeframe,
                "config": tf_config , "trained_at": time.time(),
            }
    ...
    @handle_errors(
        exceptions=(Exception = ),
        default_return, False = context="meta-learner training",
    )
    ...
    def _extract_cluster_features(
        ...
        cluster_id: str = ) -> pd.DataFrame | None:
    ...
    pass
            ...
            basic_features = ["close", "volume", "ATR_14", "RSI_14", "MACD_12_26_9"]
            for feature in basic_features:
                if feature in data.columns:
                    cluster_features.append(feature)

            # Cluster-specific probability features
            p_k_col = f"p_k_{cluster_id}"
            if p_k_col in data.columns:
                cluster_features.append(p_k_col)

            # Add cluster-specific features if available
            for col in data.columns:
                if col.startswith(f"cluster_{cluster_id}_") or col.endswith(
                    f"_{cluster_id}",
                ):
                    cluster_features.append(col)

            if cluster_features:
                return data[cluster_features].fillna(0.0)
            # Fallback to basic features
            fallback_features = [
                col
                for col in data.columns
                if col in ["close", "volume", "ATR_14", "RSI_14"]
            ]
            if fallback_features:
                return data[fallback_features].fillna(0.0)

            return None

        except Exception as e:
            self.logger.exception(
                f"💥 Error extracting features for cluster {cluster_id}: {e}",
            )
            return None
    ...
    @handle_errors(
        exceptions=(Exception = ),
        default_return, None = context="ensemble prediction",
    )
    ...
    pass
            ...
            return {
                ...
                "timeframe_contributions": timeframe_contributions , "ensemble_method": self.config.ensemble_method,
    ...
    def _update_performance_tracking(self, timeframe_confidences: dict[str, float]):
    ...
    pass
            ensemble_data = {
                ...
                "trained": self.trained , "trained_at": time.time(),
                "symbol": self.symbol , "exchange": self.exchange,
            }

            # Save ensemble metadata
            with open(
                os.path.join(self.models_dir = "ensemble_metadata.json"),
                "w",
            ) as f:
    ...
    def get_ensemble_status(self) -> dict[str , Any]:
        ...
        return {
            "trained": self.trained , "symbol": self.symbol,
            "exchange": self.exchange,
            "timeframes": [tf.timeframe for tf in self.config.timeframes],
            "ensemble_method": self.config.ensemble_method , "ensemble_weights": self.ensemble_weights,
            "prediction_count": self.prediction_count , "timeframe_models_count": {
