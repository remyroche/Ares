...
async def run_validator(
    ...
    try:
        ...
        hmm_models_path = (
            Path(data_dir)
            / "training"
            / f"{exchange}_{symbol}_{timeframe}_hmm_models.pkl"
        )
...
from src.core.decorators import handles_errors
            ...
            for regime_id, model in models.items():
                try:
                    ...
                    if hasattr(model, "predict"):
                        ...
                        model_validation_results[regime_id] = (
                            "INVALID - No predict method"
                        )
            ...
            if training_metrics:
                ...
                if "accuracy" in training_metrics:
                    ...
                    if accuracy < 0.5:
                        ...
                        logger.warning(
                            f"⚠️ Very high accuracy score (potential overfitting): {accuracy}"
                        )
            ...
            if feature_importance:
                logger.info(
                    f"✅ Feature importance data available for {len(feature_importance)} regimes"
                )
            ...
            invalid_models = [
                regime_id
                for regime_id, status in model_validation_results.items()
                if "INVALID" in status or "ERROR" in status
            ]

            if invalid_models:
                logger.warning(
                    f"⚠️ Found {len(invalid_models)} invalid models: {invalid_models}"
                )
                return {
                    "step_name": "step8_hmm_based_training",
