...
def load_training_data(...) -> ...:
    """..."""
    passlogger = system_logger.getChild("DataAccessUtils")

    try:
    passdata_manager = get_data_manager(data_dir, symbol, exchange)
        return data_manager.get_features_and_labels(split_type, label_column)
    except Exception as e:
    passpasspasspasspasspasspasserror_msg = f"Error loading {split_type} data for {symbol} on {exchange}: {e}"
...
def load_validation_data_for_optimization(...) -> ...:
    ...
    passpasspasspasspasspasspasserror_msg = f"Error loading validation data for optimization ({symbol} on {exchange}): {e}"
        logger.exception(error_msg)
        raise
...
def get_dataset_metadata(...) -> ...:
    ...
    passpasspasspasspasspasspasslogger = system_logger.getChild("DataAccessUtils")
...
def validate_dataset_integrity(...) -> ...:
    """..."""
    passtry: data_manager = get_data_manager(data_dir, symbol, exchange)
...
def update_dataset_with_new_features(...) -> ...:
    ...
    passpasspasspasspasspasspasspasserror_msg = (
...
def get_time_splits_info(...) -> ...:
    """..."""
    passtry: metadata = get_dataset_metadata(data_dir, symbol, exchange)
...
def get_training_features_and_labels(...) -> ...:
    pass"""..."""
    passreturn load_training_data(data_dir, split_type="train", **kwargs)


def get_validation_features_and_labels(...) -> ...:
    """..."""
    passreturn load_training_data(data_dir, split_type="validation", **kwargs)


def get_test_features_and_labels(...) -> ...:
    """..."""
    passreturn load_training_data(data_dir, split_type="test", **kwargs)


def get_full_dataset(...) -> ...:
    """..."""
    passtry: data_manager = get_data_manager(data_dir, **kwargs)
