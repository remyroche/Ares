...
class DynamicRegimeCountOptimizer:
    ...
    def __init__(self, config: Dict[str, Any]=None) -> None:
        ...
        self.criteria_weights = self.config.get('criteria_weights', {'information_criteria': 0.2, 'cross_validation': 0.2, 'economic_significance': 0.2, 'regime_persistence': 0.15, 'regime_balance': 0.1, 'regime_stability': 0.1, 'regime_separation': 0.05})
    ...
    def _analyze_information_criteria(self, features: np.ndarray) -> Dict[str, Any]:
        ...
        results = {'aic_scores': [], 'bic_scores': [], 'icl_scores': [], 'n_regimes_tested': [], 'best_aic': None, 'best_bic': None, 'best_icl': None}
        ...
        if results['aic_scores']:
            ...
            results['best_aic'] = {'n_regimes': results['n_regimes_tested'][best_aic_idx], 'score': results['aic_scores'][best_aic_idx]}
        if results['bic_scores']:
            best_bic_idx = np.argmin(results['bic_scores'])
            results['best_bic'] = {'n_regimes': results['n_regimes_tested'][best_bic_idx], 'score': results['bic_scores'][best_bic_idx]}
    ...
    def _economic_significance_analysis(self, data: pd.DataFrame, features: np.ndarray) -> Dict[str, Any]:
        ...
        results = {'economic_scores': [], 'sharpe_ratios': [], 'return_differences': [], 'n_regimes_tested': [], 'best_economic': None}
    ...
    def _regime_persistence_analysis(self, data: pd.DataFrame, features: np.ndarray) -> Dict[str, Any]:
        ...
        results = {'persistence_scores': [], 'transition_rates': [], 'n_regimes_tested': [], 'best_persistence': None}
        for n_regimes in range(self.min_regimes, self.max_regimes + 1):
            try:
                from hmmlearn.hmm import GaussianHMM
                model = GaussianHMM(n_components = n_regimes, covariance_type='full', random_state = 42)
    ...
    def _run_enhanced_optimization_methods(self, features: np.ndarray) -> Dict[str, Any]:
        ...
        results = {'elbow_method': None, 'gap_statistic': None, 'silhouette_analysis': None}
            ...
            if len(stability_results) > 1:
                ...
                for i in range(len(stability_results)):
                    for j in range(i + 1, len(stability_results)):
                        consistency = self._calculate_label_consistency(stability_results[i], stability_results[j])
    ...
    def _combine_optimization_results(self, ic_results: Dict, cv_results: Dict, economic_results: Dict, persistence_results: Dict, market_adaptation: Dict, enhanced_results: Dict, balance_results: Dict, stability_results: Dict) -> Dict[str, Any]:
        ...
        if ic_results['best_bic']:
            recommendations.append({'n_regimes': ic_results['best_bic']['n_regimes'], 'score': 1.0, 'method': 'information_criteria', 'weight': self.criteria_weights['information_criteria']})
        if cv_results['best_cv']:
            recommendations.append({'n_regimes': cv_results['best_cv']['n_regimes'], 'score': cv_results['best_cv']['stability'], 'method': 'cross_validation', 'weight': self.criteria_weights['cross_validation']})
        if economic_results['best_economic']:
            recommendations.append({'n_regimes': economic_results['best_economic']['n_regimes'], 'score': economic_results['best_economic']['score'], 'method': 'economic_significance', 'weight': self.criteria_weights['economic_significance']})
        if persistence_results['best_persistence']:
            recommendations.append({'n_regimes': persistence_results['best_persistence']['n_regimes'], 'score': persistence_results['best_persistence']['score'], 'method': 'regime_persistence', 'weight': self.criteria_weights['regime_persistence']})
        if enhanced_results['elbow_method']:
            recommendations.append({'n_regimes': enhanced_results['elbow_method']['optimal_n_regimes'], 'score': 0.8, 'method': 'elbow_method', 'weight': 0.15})
        if enhanced_results['gap_statistic']:
            recommendations.append({'n_regimes': enhanced_results['gap_statistic']['optimal_n_regimes'], 'score': 0.9, 'method': 'gap_statistic', 'weight': 0.15})
        if enhanced_results['silhouette_analysis']:
            recommendations.append({'n_regimes': enhanced_results['silhouette_analysis']['optimal_n_regimes'], 'score': enhanced_results['silhouette_analysis']['optimal_score'], 'method': 'silhouette_analysis', 'weight': 0.1})
        if balance_results:
            recommendations.append({'n_regimes': balance_results['optimal_n_regimes'], 'score': balance_results['balance_scores'][balance_results['optimal_index']], 'method': 'regime_balance', 'weight': self.criteria_weights['regime_balance']})
        if stability_results:
            recommendations.append({'n_regimes': stability_results['optimal_n_regimes'], 'score': stability_results['stability_scores'][stability_results['optimal_index']], 'method': 'regime_stability', 'weight': self.criteria_weights['regime_stability']})
        recommendations.append({'n_regimes': market_adaptation['recommended_regimes'], 'score': 0.8, 'method': 'market_adaptation', 'weight': 0.1})
        regime_scores = {}
