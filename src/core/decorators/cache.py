...
class MemoryCacheBackend(CacheBackend):
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {'size': len(self.cache), 'max_size': self.max_size, 'total_hits': total_hits, 'keys': list(self.cache.keys())}
_memory_cache = MemoryCacheBackend()
_cache_backends: dict[CachePolicy, CacheBackend] = {CachePolicy.CROSS_REQUEST: _memory_cache}
...
def cache_stats() -> dict[str, Any]:
    ...
    if request_cache:
        stats['per_request'] = {'size': len(request_cache), 'keys': list(request_cache.keys())}
