...
def main() -> None:
    ...
    source_side = str(
        source_manifest.get("side", source_manifest.get("side_name", ""))
    ).strip().lower()
    if source_side != args.side and not (args.side == "long" and not source_side):
        raise ValueError(
            "Adaptive Exit source manifest side must match --side; "
            "legacy side-less sources are not valid for short training"
        )
    ...
    if "side_name" not in states:
        if args.side != "long":
            raise ValueError("short Adaptive Exit training requires explicit side_name lineage")
        states["side_name"] = "long"
    states["side_name"] = states["side_name"].astype(str).str.strip().str.lower()
    if not states["side_name"].isin(("long", "short")).all():
        raise ValueError("Adaptive Exit source has noncanonical side values")
    states = states.loc[states["side_name"].eq(args.side)].copy()
    if states.empty:
        raise RuntimeError(f"no {args.side} Adaptive Exit states in source")
    ...
    if str(policy_document.get("side") or "").strip().lower() != args.side:
        raise ValueError("Adaptive Exit policy JSON side must match --side")
    policy = policy_document["winner"]
    payload = {
    ...
    manifest = {
        "schema": SCHEMA, "bundle_id": bundle_id, "side": args.side, "controller": CONTROLLER,
    ...
    reference = (
        ...
        .tail(512)
        .loc[:, ["candidate_id", "decision_ts", "path_bar", "side_name", *dict.fromkeys([*f1, *f4])]]
        .copy()
    )
