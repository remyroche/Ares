...
class FunctionSignature:
    ...
    var_args: str | None = None
    var_kwargs: str | None = None
    ...
    def validate_project(self) -> dict[str, Any]:
        ...
        for file_path in python_files:
            try:
                ...
                logger.exception(f"Error collecting signatures from {file_path}: {e}")
...
class SignatureCollector(ast.NodeVisitor):
    ...
    def _process_function(self, node: ast.FunctionDef | ast.AsyncFunctionDef) -> None:
        ...
        for kw_default, kw_arg in zip(args.kw_defaults, keyword_args, strict=False):
            if kw_default:
...
class ArgumentAndAccessValidator(ast.NodeVisitor):
    ...
    def _validate_subscript_access(self, node: ast.Subscript) -> None:
        ...
        if not is_safe:
            self.validator._add_issue(
                ...
                "Use .get() for dicts or check bounds/existence first",
            )
    ...
    def _find_method_signature(self, method_name: str) -> FunctionSignature | None:
    ...
    def _is_safe_access(self, target: str, line: int) -> bool:
        ...
        if target in {"self", "cls", "True", "False"} or target.startswith(('"', "'")):
            return True
        ...
        return bool(target in self.safe_checks and self.safe_checks[target] < line)
