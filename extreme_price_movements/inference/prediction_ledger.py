...
class PredictionLedger:
    ...
    def mark_resolved(self, updates: pd.DataFrame) -> None:
        ...
        # Ensure schemas match before update to prevent dtype casting to object
        ...
        if not common_idx.empty:
            # Reassign columns from upd_idx to old_idx for common indices
            for col in upd_idx.columns:
                if col not in key_cols:
                    if old_idx[col].dtype != upd_idx[col].dtype:
                        # Align dtypes first to avoid incompatible dtype warnings
                        old_idx[col] = old_idx[col].astype(object)
                    old_idx.loc[common_idx, col] = upd_idx.loc[common_idx, col].values

        # Append new records
        new_idx = upd_idx.index.difference(old_idx.index)
        if not new_idx.empty:
            old_idx = pd.concat([old_idx, upd_idx.loc[new_idx]], sort=False)
