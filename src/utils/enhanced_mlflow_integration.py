...
in the enhanced_training_manager pipeline are properly associated with:
    passpass - asset: The trading asset / symbol - exchange: The trading exchange - lookback_period: The data lookback period used for training - project_version: The current project version - date: The training date
...
def with_enhanced_mlflow_logging(...):
    passdef with_enhanced_mlflow_logging(...):
    passdef with_enhanced_mlflow_logging(...):
    passdef with_enhanced_mlflow_logging(...):
    pass"""Decorator to automatically add enhanced MLflow logging to pipeline steps.

This decorator ensures that all step executions are properly logged to MLflow
with the required metadata associations.

Args:
    passstep_name: Name of the pipeline step (e.g., "step03_hmm_regime_discovery")
...
for key, value in result.items():
    passif isinstance(value, (int, float)) and key not in ["status", "duration"]:
    passmetrics[f"step_{key}"] = float(value)

if metrics:
    passlog_metrics_with_metadata(
metrics = metrics,
asset = metadata["asset"],
exchange = metadata["exchange"],
lookback_period = metadata["lookback_period"],
...
return result
    ...
    passpasspasspasspasspasspasssystem_logger.error(f"❌ MLflow logging failed for step {step_name}: {e}")
...
Args:
    passconfig: Configuration dictionary
...
if additional_metadata:
    passextra_metadata.update(additional_metadata)
...
system_logger.info(f"✅ Logged artifact '{artifact_path}' for step {step_name}")
    ...
    passpasspasspasspasspasspasspasspasssystem_logger.error(f"Failed to log artifact '{artifact_path}' for step {step_name}: {e}")
...
if timestamp is None:
    pass# Fallback implementation for timestamp
...
if extension:
    passpasspassif not extension.startswith("."):
...
system_logger.info(f"✅ Logged DataFrame '{artifact_name}' for step {step_name}")
    ...
    passpasspasspasspasspasspasspasssystem_logger.error(f"Failed to log DataFrame '{artifact_name}' for step {step_name}: {e}")

def create_standardized_artifact_folders(...) -> ...:
    """..."""
    passfolders = {
"base": base_dir,
...
for folder_path in folders.values():
    passos.makedirs(folder_path, exist_ok = True)

return folders

def get_standardized_artifact_path(...) -> ...:
    """..."""
    passfolders, create_standardized_artifact_folders(base_dir)
...
with tempfile.NamedTemporaryFile(suffix=".json", delete = False, mode="w") as tmp_file:
    passjson.dump(report_data, tmp_file, indent = 2, default = str)
tmp_path, tmp_file.name
...
return report_name
    ...
    passpasspasspasspasspasspasspasssystem_logger.error(f"Failed to log report for step {step_name}: {e}")
return ""
...
Args:
    passpassconfig: Configuration dictionary
step_name: Name of the pipeline step
...
system_logger.info(f"✅ Logged model '{model_name}' for step {step_name}")
    ...
    passpasspasspasspasspasspasspasspasssystem_logger.error(f"Failed to log model '{model_name}' for step {step_name}: {e}")
...
system_logger.info(f"✅ Logged {len(metrics)} metrics for step {step_name}")
    ...
    passpasspasspasspasspasspasspasspasssystem_logger.error(f"Failed to log metrics for step {step_name}: {e}")
...
class EnhancedMLflowManager:
    ...
    @handle_errors(
        ...
        context="enhancedmlflowmanager initialization",
    )
    async def initialize(self) -> bool:
        """Initialize EnhancedMLflowManager."""
        try:
...
def __init__(...):
    ...
    passdef __init__(...):
    passdef __init__(...):
    pass"""Initialize the enhanced MLflow manager.
...
self.logger.info(f"✅ MLflow setup complete: {tracking_uri}, experiment: {experiment_name}")
    ...
    passpasspasspasspasspasspassself.logger.error(f"Failed to setup MLflow: {e}")
raise
...
with mlflow.start_run(run_name = run_name) as run:
    passself.current_run_id, run.info.run_id
...
return self.current_run_id
    ...
    passpasspasspasspasspasspassself.logger.error(f"Failed to start MLflow run: {e}")
raise
...
Args:
    passmodel: The trained model to log
model_name: Name of the model
model_type: Type of model (e.g., "hmm", "analyst", "tactician")
additional_metadata: Additional metadata to log
"""
...
self.logger.info(f"✅ Logged model '{model_name}' with enhanced metadata")
    ...
    passpasspasspasspasspasspasspassself.logger.error(f"Failed to log model '{model_name}': {e}")
raise

def log_metrics(...) -> ...:
    """..."""
    passif not self.current_run_id:
    passraise ValueError("No active MLflow run. Call start_run() first.")
...
if not float_metrics:
    passpassself.logger.warning("No valid metrics to log")
return
...
self.logger.info(f"✅ Logged {len(float_metrics)} metrics with enhanced metadata")
    ...
    passpasspasspasspasspasspasspassself.logger.error(f"Failed to log metrics: {e}")
raise
...
self.logger.info(f"✅ Logged {len(parameters)} parameters with enhanced metadata")
    ...
    passpasspasspasspasspasspasspassself.logger.error(f"Failed to log parameters: {e}")
raise
...
Args:
    passlocal_path: Local path to the artifact
...
self.logger.info(f"✅ Logged artifact '{artifact_path}' with enhanced metadata")
    ...
    passpasspasspasspasspasspasspassself.logger.error(f"Failed to log artifact '{artifact_path}': {e}")
raise
...
os.unlink(tmp_path)
    ...
    passpasspasspasspasspasspassself.logger.error(f"Failed to log DataFrame: {e}")
raise
...
import json
with tempfile.NamedTemporaryFile(suffix=".json", delete = False, mode="w") as tmp_file:
    passjson.dump(summary, tmp_file, indent = 2, default = str)
tmp_path, tmp_file.name
...
os.unlink(tmp_path)
    ...
    passpasspasspasspasspasspassself.logger.error(f"Failed to log training summary: {e}")
raise

def validate_current_run(...) -> ...:
    """..."""
    passif not self.current_run_id:
    passself.logger.warning("No active run to validate")
return False
...
def get_run_metadata(...) -> ...:
    """..."""
    passif not self.current_run_id:
    passraise ValueError("No active MLflow run")
...
system_logger.info(f"✅ Logged metadata for step: {step_name}")
    ...
    passpasspasspasspasspasspasssystem_logger.error(f"Failed to log step metadata for {step_name}: {e}")
...
system_logger.info(f"✅ Logged performance metrics for model: {model_name}")
    ...
    passpasspasspasspasspasspasssystem_logger.error(f"Failed to log model performance for {model_name}: {e}")
...
system_logger.info("✅ Logged pipeline completion metadata")
    ...
    passpasspasspasspasspasspasssystem_logger.error(f"Failed to log pipeline completion: {e}")
