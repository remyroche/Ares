...
class Step2MarketRegimeClassificationValidator(BaseValidator):
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.max_regime_dominance: float = 0.85  # Increased from 0.8 to allow more dominant regimes
        self.min_regime_frequency: float = 0.03  # Reduced from 0.05 to allow rare regimes
        self.max_regime_switching: float = 0.6  # Increased from 0.5 to allow more regime changes
        self.max_stuck_ratio: float = 0.4  # Increased from 0.3 to allow longer regime periods
        self.probability_tolerance: float = 0.02  # Increased from 0.01 for probability validation
    ...
    async def validate(
        ...
        step_result: dict[str, Any] = pipeline_state.get("regime_classification", {})
        ...
        try:
            ...
            if regime_file_path.endswith(".parquet"):
                ...
                # Try to read minimal columns when possible
                try:
                    from src.utils.data_optimizer import regime_columns  # type: ignore
    ...
    def _validate_regime_classification(
        ...
        Returns: Tuple of (passed, metrics_dict)
        """
        try:
            ...
            if isinstance(confidence_scores, dict) and len(confidence_scores) == 0:
                ...
                if not confidence_scores_valid:
                    confidence_scores_message = (
                        "Confidence scores are not valid (should be between 0.0 and 1.0) "
                        f"- {invalid_confidence_count} invalid scores found"
                    )
                else:
                    confidence_scores_message = (
                        f"Valid confidence scores array with {len(confidence_scores)} scores"
                    )
            else:
                confidence_scores_valid = False
                confidence_scores_message = (
                    f"Invalid confidence_scores format: {type(confidence_scores)} (expected dict or list)"
                )
