...
from src.core.decorators import handles_errors
        ...
        self.upper_barrier_pct, self.lower_barrier_pct = (
            self.barrier_calculator.calculate_dynamic_barriers(timeframe="1m")
        )
        ...
        self.analyst_signal_requirement = self.config.get(
            "analyst_signal_requirement", True
        )
        self.direction_agreement_required = self.config.get(
            "direction_agreement_required", True
        )
        self.confidence_boost_threshold = self.config.get(
        ...
        self.logger.info(
            f"   Dynamic Upper Barrier: {self.upper_barrier_pct:.4f} ({self.upper_barrier_pct*100:.3f}%)"
        )
        self.logger.info(
            f"   Dynamic Lower Barrier: {self.lower_barrier_pct:.4f} ({self.lower_barrier_pct*100:.3f}%)"
        )
        self.logger.info(f"   Precision Threshold: {self.precision_threshold}")
        self.logger.info(
            f"   Position Size Multiplier: {self.position_size_multiplier}"
        )
    ...
    def validate_analyst_predictions(
        self, analyst_predictions: Dict[str, Any], tactician_predictions: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            analyst_price_pred = analyst_predictions.get("price_prediction", {}).get(
                "prediction", 0.0
            )
            analyst_confidence = analyst_predictions.get(
                "confidence_prediction", {}
            ).get("prediction", 0.5)
            analyst_regime = analyst_predictions.get("regime_prediction", {}).get(
                "prediction", "unknown"
            )

            # Extract key predictions from Tactician
            tactician_price_pred = tactician_predictions.get(
                "price_prediction", {}
            ).get("prediction", 0.0)
            tactician_confidence = tactician_predictions.get(
                "confidence_prediction", {}
            ).get("prediction", 0.5)
            tactician_regime = tactician_predictions.get("regime_prediction", {}).get(
                "prediction", "unknown"
            )
    ...
    def _directions_agree(
        self, analyst_direction: str, tactician_direction: str
    ) -> bool:
    ...
    def calculate_execution_parameters(
        ...
        based on multi-outcome predictions."""
        try:
            # Validate predictions first
            validation = self.validate_analyst_predictions(
            ...
            self.logger.info(
                f"   Upper Barrier: {execution_params['upper_barrier_price']:.4f}"
            )
            self.logger.info(
                f"   Lower Barrier: {execution_params['lower_barrier_price']:.4f}"
            )
            self.logger.info(
                f"   Position Size: {execution_params['position_size']:.4f}"
            )
            self.logger.info(
                f"   Precision Score: {execution_params['precision_score']:.3f}"
            )
    ...
    def _calculate_risk_adjusted_size(
        self, base_size: float, stop_loss_price: float, current_price: float
    ) -> float:
    ...
    def _calculate_entry_timing(
        self, market_data: pd.DataFrame, confidence: float
    ) -> Dict[str, Any]:
    ...
    def _calculate_precision_score(
        self, combined_confidence: float, volatility: float, market_data: pd.DataFrame
    ) -> float:
    ...
    async def execute_trade(
        self, execution_params: Dict[str, Any], market_data: pd.DataFrame
    ) -> Dict[str, Any]:
