...
class IntegratedDataQualityPipeline:
    ...
    def _initialize_components(self) -> None:
        ...
        try:
            ...
            self.enhanced_quality_manager = EnhancedDataQualityManager(
    ...
    async def run_comprehensive_quality_pipeline(
        ...
        try:
            ...
            initial_quality = await self._run_initial_quality_check(
            ...
            if run_step1:
                ...
                step1_result = await self._run_step1_data_collection(
                    symbol, exchange, timeframe, force_rerun
                )
            ...
            if run_step1_5:
                ...
                step1_5_result = await self._run_step1_5_data_conversion(
                    symbol, exchange, timeframe, force_rerun
                )
            ...
            if run_step3:
                ...
                step3_result = await self._run_step3_hmm_discovery(
                    symbol, exchange, timeframe, force_rerun
                )

                if step3_result.get("success", False):
                    logger.info(
                        "✅ Step 3: HMM Regime Discovery completed successfully"
                    )
                    results["steps_completed"].append("step3_hmm_discovery")
                    results["quality_metrics"]["hmm_results"] = step3_result.get(
                        "metrics", {}
                    )
                else:
            ...
            if run_step4:
                ...
                step4_result = await self._run_step4_labeling(
                    symbol, exchange, timeframe, force_rerun
                )
            ...
            final_quality = await self._run_final_quality_check(
                symbol, exchange, timeframe
            )
            ...
            if results["success"]:
                logger.info(
                    "🎉 Integrated Data Quality Pipeline completed successfully!"
                )
    ...
    async def _run_step1_data_collection(
        self, symbol: str, exchange: str, timeframe: str, force_rerun: bool
    ) -> Dict[str, Any]:
    ...
    async def _run_step4_labeling(
        ...
        try:
            ...
            if self.enhanced_quality_manager:
                data_ready = (
                    await self.enhanced_quality_manager.get_data_for_step3_step4(
    ...
    def generate_quality_report(self, results: Dict[str, Any]) -> str:
        ...
        if "initial_check" in quality_metrics:
            ...
            report.append(
                f"   🔍 Initial Check: {'✅ Passed' if initial.get('success') else '❌ Failed'}"
            )
            if initial.get("gaps_detected"):
                report.append(
                    f"      📊 Gaps detected: {len(initial['gaps_detected'])}"
                )
        ...
        if "final_check" in quality_metrics:
            ...
            report.append(
                f"   🔍 Final Check: {'✅ Passed' if final.get('success') else '❌ Failed'}"
            )

        if "hmm_results" in quality_metrics:
            hmm = quality_metrics["hmm_results"]
            report.append(
                f"   🔍 HMM Results: {hmm.get('unique_regimes', 0)} regimes discovered"
            )
...
if __name__ == "__main__":
    ...
    async def main() -> None:
        ...
        if len(sys.argv) >= 4:
            ...
            print(
                "Usage: python integrated_data_quality_pipeline.py <symbol> <exchange> <timeframe> [data_cache_path] [force_rerun]"
            )
            print(
                "Example: python integrated_data_quality_pipeline.py ETHUSDT BINANCE 1m data_cache true"
            )
            return
