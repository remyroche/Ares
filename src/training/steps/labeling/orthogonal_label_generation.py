...
def generate_dual_cusum_signals(
    ...
    # 4. Optimized Numba CUSUM logic
    diff = close.diff()
    
    diff_arr = diff.fillna(0).values
    h_arr = h.values
    er_arr = er.values
    
    signals = generate_dual_cusum_numba(diff_arr, h_arr, er_arr, er_min)

    return pd.DataFrame({
        'trend_signal': signals[:, 0],
        'reversal_signal': signals[:, 1]
    }, index=close.index)
