...
try:
    ...
    from src.utils.enhanced_data_quality_validator import EnhancedDataQualityValidator, QualityThresholds
...
class EnhancedStep1DataCollection:
    ...
    async def _process_and_validate_data(self, training_input: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            klines_file = os.path.join(
                self.config.data_dir, f"klines_{exchange}_{symbol}_{timeframe}_consolidated.parquet"
            )
            ...
            for file_path, file_type in [(klines_file, "klines"), (aggtrades_file, "aggtrades")]:
                if os.path.exists(file_path):
                    try:
                        ...
                        files_info.append(
                            {"type": file_type, "path": file_path, "size_mb": file_size, "columns": columns}
                        )
...
async def run_enhanced_step1(
    training_input: Dict[str, Any], pipeline_state: Dict[str, Any], config: Optional[Step1Config] = None
) -> Dict[str, Any]:
    """
