...
## $(date +%Y-%m-%d) - Bulk DataFrame Updates
Learning: Iterating over a DataFrame with `iterrows()` and updating another DataFrame row-by-row with `.loc` assignments or `pd.concat` is a severe performance bottleneck.
Action: Replace iterative `.loc` and `pd.concat` operations with vectorized `.loc[index, columns] = ...` (to unconditionally preserve nulls, unlike `.update()`) and a single bulk `pd.concat()` operation for new rows. Ensure you deduplicate updates with `drop_duplicates(keep='last')` (or `~duplicated(keep='last')`) to prevent `cannot reindex` errors. Handle schema evolution by explicitly adding missing columns using `pd.NA` or `float('nan')` (depending on the context and types) before the update.
