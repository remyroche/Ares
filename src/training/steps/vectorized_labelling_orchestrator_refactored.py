    ...
    async def orchestrate_labeling_and_feature_engineering(
        ...
        try:
            ...
            return self._create_success_response(
                combined_data, stage_timings, metadata, total_execution_time
            )
    ...
    async def _execute_feature_selection_stage(self, data: pd.DataFrame) -> StageResult:
        ...
        try:
            ...
            selected = await self.feature_selector.select_optimal_features(
                selection_input, data.get("label") if "label" in data.columns else None
            )
    ...
    async def _execute_validation_stage(self, data: pd.DataFrame) -> StageResult:
        ...
        try:
            ...
            if not validation_results["is_valid"]:
                raise ValueError(
                    f"Final validation failed: {validation_results['errors']}"
                )
