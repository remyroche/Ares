...
class RegimeSpecificTripleBarrierOptimizer:
    ...
    async def _optimize_single_regime(
        self, regime_name: str, regime_data: pd.DataFrame, study: optuna.Study, optimization_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        study.optimize(
            ...
            timeout=timeout,
            callbacks=[
                optuna.callbacks.EarlyStoppingCallback(patience=optimization_config.get("early_stopping_patience", 20))
            ],
        )
    ...
    def _create_regime_objective(self, regime_name: str, regime_data: pd.DataFrame, regime_config: Dict[str, Any]):
        ...
        def objective(trial):
            ...
            try:
                performance_score = self._evaluate_regime_parameters(regime_name, regime_data, params)
                    ...
                    if len(param_config) == 2:
                        if param_name in ["barrier_timeout", "n_estimators", "max_depth"]:
                            ...
                            params[category][param_name] = trial.suggest_int(
                                f"{category}_{param_name}", param_config[0], param_config[1]
                            )
                        else:
                            # Float parameters
                            params[category][param_name] = trial.suggest_float(
                                f"{category}_{param_name}", param_config[0], param_config[1], log=True
                            )
