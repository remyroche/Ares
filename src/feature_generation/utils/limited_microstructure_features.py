...
class LimitedMicrostructureFeatures:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            for data_type in self.available_data_types:
                self.feature_statistics[data_type] = {'mean': 0.0, 'std': 0.0, 'min': float('inf'), 'max': float('-inf'), 'count': 0}
    ...
    async def extract_features(self, market_data: Dict[str, Any], historical_data: Optional[pd.DataFrame]=None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            all_features = {**basic_features, **enhanced_features, **time_features, **volume_features, **price_features}
    ...
    def _extract_basic_features(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if spread > 0:
            ...
            price_position = 0.5
        return {'bid': bid, 'ask': ask, 'last_price': last_price, 'spread': spread, 'mid_price': mid_price, 'spread_percentage': spread_percentage, 'price_position': price_position, 'spread_tightness': 1.0 / (1.0 + spread_percentage)}

    def _extract_enhanced_features(self, market_data: Dict[str, Any], historical_data: Optional[pd.DataFrame]) -> Dict[str, Any]:
        ...
        if 'bid_volume' in market_data and 'ask_volume' in market_data:
            ...
            if bid_volume + ask_volume > 0:
                ...
                volume_imbalance = 0
            enhanced_features.update({'bid_volume': bid_volume, 'ask_volume': ask_volume, 'volume_imbalance': volume_imbalance, 'total_volume': bid_volume + ask_volume})
        if 'vwap' in market_data:
            ...
            if vwap > 0:
                ...
                vwap_deviation = 0
            enhanced_features.update({'vwap': vwap, 'vwap_deviation': vwap_deviation, 'price_vs_vwap': last_price / vwap if vwap > 0 else 1.0})
        if 'high' in market_data and 'low' in market_data:
            ...
            enhanced_features.update({'high': high, 'low': low, 'price_range': price_range, 'price_position_in_range': price_position_in_range, 'range_percentage': price_range / last_price * 100 if last_price > 0 else 0})
    ...
    def _extract_time_features(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        time_features = {'timestamp': current_time, 'time_since_last_trade': time_since_last_trade, 'hour_of_day': current_time.hour, 'day_of_week': current_time.weekday(), 'is_market_hours': True, 'is_weekend': current_time.weekday() >= 5, 'trading_session': self._get_trading_session(current_time)}
        return time_features

    def _extract_volume_features(self, market_data: Dict[str, Any], historical_data: Optional[pd.DataFrame]) -> Dict[str, Any]:
        ...
        if 'volume' in market_data:
            ...
            if historical_data is not None and 'volume' in historical_data.columns:
                ...
                if len(recent_volumes) > 1:
                    ...
                    volume_features.update({'volume_velocity': volume_velocity, 'volume_trend': volume_trend, 'avg_volume': avg_volume, 'volume_anomaly': 1.0 if volume_velocity > 2.0 else 0.0})
        return volume_features

    def _extract_price_features(self, market_data: Dict[str, Any], historical_data: Optional[pd.DataFrame]) -> Dict[str, Any]:
        ...
        if historical_data is not None and 'last_price' in historical_data.columns:
            ...
            if len(recent_prices) > 1:
                ...
                price_features.update({'price_volatility': price_volatility, 'price_trend': price_trend, 'price_momentum': price_momentum, 'volatility_percentage': price_volatility / last_price * 100 if last_price > 0 else 0})
        if historical_data is not None and 'last_price' in historical_data.columns:
            if len(historical_data) > 0:
                ...
                price_features.update({'price_change': price_change, 'price_change_percentage': price_change_percentage, 'price_direction': 1 if price_change > 0 else -1 if price_change < 0 else 0})
        ...
        for feature_name, value in features.items():
            if isinstance(value, (int, float)) and (not np.isnan(value)):
                if feature_name not in self.feature_statistics:
                    self.feature_statistics[feature_name] = {'mean': 0.0, 'std': 0.0, 'min': float('inf'), 'max': float('-inf'), 'count': 0}
    ...
    async def get_timeframe_specific_features(self, market_data: Dict[str, Any], timeframe: str, historical_data: Optional[pd.DataFrame]=None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if timeframe == '5m':
                ...
                if historical_data is not None and 'last_price' in historical_data.columns:
                    recent_prices = historical_data['last_price'].tail(12)
                    if len(recent_prices) > 1:
    ...
    def _create_timeframe_specific_signals(self, features: Dict[str, Any], timeframe: str) -> Dict[str, int]:
        ...
        if timeframe == '5m':
            if 'spread_percentage' in features:
                if features['spread_percentage'] < 0.005:
                    signals['ultra_tight_spread'] = 1
                elif features['spread_percentage'] > 0.02:
                    signals['wide_spread_5m'] = 1
            if 'volume_velocity' in features:
                if features['volume_velocity'] > 2.0:
                    signals['high_volume_5m'] = 1
                elif features['volume_velocity'] < 0.3:
                    signals['low_volume_5m'] = 1
            if 'volatility_5m' in features:
                if features['volatility_5m'] > 0.001:
                    signals['high_volatility_5m'] = 1
                elif features['volatility_5m'] < 0.0001:
