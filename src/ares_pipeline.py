...
class AresPipeline:
    ...
    async def start(self):
        ...
        while True:
            try:
                ...
                self.logger.error(
                    f"An unhandled error occurred in the AresPipeline loop: {e}",
                    exc_info=True,
                )
    ...
    async def _initial_setup(self):
        ...
        if self.supervisor.data.empty:
            self.logger.warning(
                "No historical data available from supervisor for initial setup."
            )
        else:
    ...
    async def _execute_main_cycle(self):
        ...
        if global_status == "PAUSED" or is_locally_paused:
            status_origin = (
                "Global Manager" if global_status == "PAUSED" else "Local Supervisor"
            )
            self.logger.warning(
                f"Trading is PAUSED by {status_origin}. Checking for open positions to close."
            )
        ...
        try:
            ...
            if klines.empty:
                self.logger.warning(
                    "Received empty klines data, skipping analysis cycle."
                )
                return
        except Exception as e:
        ...
        self.sentinel.run_checks(
            intelligence,
            self.state_manager.get_state("active_position"),
            champion_signal,
        )
        if self.sentinel.is_trade_halted():
            self.logger.critical(
                f"SENTINEL HALT (Champion): {self.sentinel.get_halt_reason()}."
            )
        elif champion_signal:
            await self._execute_trade_action(
                champion_signal, klines, self.exchange, self.tactician
            )

        # --- A/B Testing Logic for Challenger Model ---
        if (
            hasattr(self.supervisor, "ab_tester")
            and self.supervisor.ab_tester.ab_test_active
        ):
            ...
            self.sentinel.run_checks(
                intelligence, paper_trader.get_position(), challenger_signal
            )
            if self.sentinel.is_trade_halted():
                self.logger.warning(
                    f"SENTINEL HALT (Challenger): {self.sentinel.get_halt_reason()}."
                )
            elif challenger_signal:
                await self._execute_trade_action(
                    challenger_signal, klines, paper_trader, challenger_tactician
                )
    ...
    async def _execute_trade_action(self, signal, klines, client, tactician_instance):
        ...
        active_position = (
            client.get_position(self.exchange.symbol)
            if hasattr(client, "get_position")
            else self.state_manager.get_state("active_position")
        )

        if active_position and signal["action"] == "ENTRY":
            self.logger.info(
                f"Client is already in a position for {self.exchange.symbol}. Skipping new entry signal."
            )
            return
        ...
        if trade_size_usd > 0 and signal["action"] == "ENTRY":
            self.logger.info(
                f"Placing trade for {self.exchange.symbol} with size ${trade_size_usd:,.2f} on client {type(client).__name__}"
            )
            try:
                ...
                self.logger.error(
                    f"Error placing trade for {self.exchange.symbol}: {e}",
                    exc_info=True,
                )
    ...
    async def _handle_pause_state(self):
        ...
        if active_position:
            self.logger.critical(
                f"System is paused. Closing open position for {active_position['symbol']}."
            )
            try:
                await self.exchange.close_position(symbol=active_position["symbol"])
                self.logger.info(
                    f"Close order for {active_position['symbol']} placed successfully."
                )
                await self.supervisor._synchronize_exchange_state()
            except Exception as e:
                self.logger.error(
                    f"Failed to close position for {active_position['symbol']} during pause state: {e}",
                    exc_info=True,
                )
    ...
    async def _check_for_model_promotion(self):
        ...
        if flag_file and os.path.exists(flag_file):
            self.logger.critical(
                "Promote challenger flag detected! Initiating model hot-swap..."
            )
            if self.model_manager.promote_challenger_to_champion():
                self.logger.info("Model promotion successful. New champion is live.")
                send_email(
                    "Ares Alert: Model Promoted",
                    f"Challenger for {self.exchange.symbol} is now champion.",
                )
                self.analyst = self.model_manager.get_analyst()
                self.strategist = self.model_manager.get_strategist()
                self.tactician = self.model_manager.get_tactician()
                ...
                send_email(
                    "Ares Alert: Model Promotion FAILED",
                    f"Failed for {self.exchange.symbol}.",
                )
            os.remove(flag_file)

    def _write_pid_file(self):
        """Writes the process ID to a file for monitoring."""
        pid_file = self.config.get(
            "PIPELINE_PID_FILE", f"ares_{self.exchange.symbol}.pid"
        )
        try:
