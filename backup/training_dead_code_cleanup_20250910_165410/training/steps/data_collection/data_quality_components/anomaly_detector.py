...
class AnomalyDetector:
    ...
    def _detect_time_based_anomalies(self, data: pd.DataFrame, results: dict[str, Any]) -> None:
        ...
        if unusual_hours.any():
            time_anomalies['unusual_hours'] = {'count': unusual_hours.sum(), 'percentage': float(unusual_hours.sum() / len(data) * 100), 'sample_times': data.index[unusual_hours][:10].tolist()}
        ...
        if pd.notna(expected_interval):
            ...
            if significant_gaps.any():
                ...
                time_anomalies['time_gaps'] = {'count': len(gap_indices), 'gap_times': [{'start': str(data.index[i - 1]), 'end': str(data.index[i]), 'duration_hours': float(time_diffs.iloc[i].total_seconds() / 3600)} for i in range(len(data)) if significant_gaps.iloc[i] and i > 0][:10]}
        if time_anomalies:
    ...
    def detect_volume_anomalies(self, data: pd.DataFrame, volume_col: str='volume') -> dict[str, Any]:
        ...
        results['statistics'] = {'mean_volume': float(volume.mean()), 'std_volume': float(volume.std()), 'min_volume': float(volume.min()), 'max_volume': float(volume.max()), 'spike_count': len(results['volume_spikes']), 'drop_count': len(results['volume_drops']), 'zero_count': len(results['zero_volume_periods']), 'anomaly_rate': float((len(results['volume_spikes']) + len(results['volume_drops']) + len(results['zero_volume_periods'])) / len(data))}
        return results
