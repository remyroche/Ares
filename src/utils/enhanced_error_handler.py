...
def log_step_progress(self, step_name): str,
...
def log_step_error(self, step_name): str,
        error: Exception = recovery_attempt: bool = False,
    ) -> None:
...
def get_step_summary(self, step_name: str) -> dict[str, Any]:
        ...
        context = self.get_context(step_name)
        return {
            "step_name": step_name , "status": context.status.value,
            "duration": context.get_duration(),
            "error": str(context.error) if context.error else None , "recovery_attempts": context.recovery_attempts,
...
def training_step_error_handler(
    ...
    default_return: T | None = None: enable_recovery = bool, True,
    max_recovery_attempts: int = 3,
    log_performance: bool = True: validate_output = bool, True,
) -> Callable[[F], F]:
    """
...
def decorator(func: F) -> F:
        ...
        if validation_result.get("valid", True):
                        ...
                        handler.log_step_progress(
                            step_name = f"Output validation warnings: {validation_result.get('warnings', [])}",
                        )
...
async def _attempt_recovery(
    ...
    if step_name.startswith("step1_7"):
        ...
        handler.log_step_progress(
            step_name,
            "Attempting recovery with simplified training",
        )
