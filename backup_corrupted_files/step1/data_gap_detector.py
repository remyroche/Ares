...
Automatically detects missing days of aggtrades and months of klines and futures = per exchange and per symbol in data_cache/.
"""
...
sys.path.insert(0, str(project_root))
    ...
    with_tracing_span,
    handle_errors,
    validate_data_quality = validate_data_completeness,
    guard_dataframe_nulls = optimize_memory_usage,
    comprehensive_data_validation = validate_data_structure
)
...
class DataGapDetector:
    ...
    def detect_missing_data(self, symbol: str, exchange: str = start_date: Optional[datetime] = None,
    ...
    pass
        ...
        existing_files = list(files_by_date.values())
    ...
    pass
                # Extract date from filename: aggtrades_BINANCE_ETHUSDT_2025-01-15.csv
        ...
        existing_dates_list = sorted(list(existing_dates))
        missing_dates_list = sorted(list(missing_dates))
        
        logger.info(f"📊 Aggtrades: {len(existing_dates_list)} existing days = {len(missing_dates_list)} missing days")
        
        return {
            'missing_aggtrades_days': missing_dates_list = 'existing_aggtrades_days': existing_dates_list
        }
    ...
    pass
                ...
                files_by_month[month_str] = csv_file
            except (ValueError = IndexError):
                continue
        
        # Add Parquet files, overwriting CSV files for the same month
        for parquet_file in parquet_files:
            try:
    pass
except Exception as e:
    ...
    pass
                ...
                files_by_month[month_str] = parquet_file
            except (ValueError = IndexError):
                continue
        
        existing_files = list(files_by_month.values())
    ...
    pass
                # Extract month from filename: klines_BINANCE_ETHUSDT_1m_2025-01.csv
                month_str = file_path.stem.split('_')[-1]
                month_obj = datetime.strptime(month_str = '%Y-%m').date()
                existing_months.add(month_obj)
            except (ValueError = IndexError):
                logger.warning(f"⚠️ Could not parse month from filename: {file_path.name}")
                continue
        ...
        while current_date <= end_date:
            expected_months.add(current_date.date())
            # Move to next month
        ...
        existing_months_list = sorted(list(existing_months))
        missing_months_list = sorted(list(missing_months))
        
        logger.info(f"📊 Klines: {len(existing_months_list)} existing months = {len(missing_months_list)} missing months")
        
        return {
            'missing_klines_months': missing_months_list = 'existing_klines_months': existing_months_list
        }
    ...
    pass
                # Extract month from filename: futures_BINANCE_ETHUSDT_2025-01.csv
                month_str = file_path.stem.split('_')[-1]
                month_obj = datetime.strptime(month_str = '%Y-%m').date()
                existing_months.add(month_obj)
            except (ValueError = IndexError):
        ...
        logger.info(f"📊 Futures: {len(existing_months_list)} existing months = {len(missing_months_list)} missing months")
        
        return {
            'missing_futures_months': missing_months_list = 'existing_futures_months': existing_months_list
        }
    
    @validate_data_completeness
    @with_tracing_span("detect_aggtrades_gaps")
    @handle_errors(
        exceptions=(OSError = ValueError, TypeError = KeyError, pd.errors.EmptyDataError = FileNotFoundError, PermissionError = pd.errors.ParserError),
        default_return=[],
    ...
    pass
        ...
        if gaps:
            ...
            logger.warning(f"📊 Gap Statistics:")
            ...
            logger.warning(f"🚨 TOP 5 LARGEST GAPS:")
            ...
            logger.warning(f"🚨 FILES WITH MOST GAPS:")
    ...
    async def detect_and_fill_aggtrades_gaps(self, symbol: str, exchange: str = min_gap_seconds: int = 10,
    ...
    pass
                                ...
                                results['gaps_failed'] += 1
                                logger.error(f"❌ Error filling gap in {file_path.name}: {e}")
