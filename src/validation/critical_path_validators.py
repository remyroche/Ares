    ...
    def validate_trade_decision(decision: Any) -> TradeDecision:
        ...
        try:
            ...
            if 'stop_loss' in validated_decision and 'price' in validated_decision:
                ...
                elif validated_decision['action'] in ['open_short'] and validated_decision['stop_loss'] and (validated_decision['stop_loss'] <= validated_decision['price']):
...
class TypeSafetyMonitor:
    ...
    def record_violation(self, violation: RuntimeTypeError) -> None:
        ...
        self.violations.append({'timestamp': datetime.utcnow().isoformat() + 'Z', 'expected_type': str(violation.expected_type), 'actual_type': str(type(violation.actual_value)), 'context': violation.context, 'message': str(violation), 'correlation_id': get_correlation_id()})
    ...
    def get_violation_summary(self) -> dict:
        ...
        return {'total_violations': len(self.violations), 'violation_counts': self.violation_counts.copy(), 'recent_violations': self.violations[-10:] if self.violations else []}
