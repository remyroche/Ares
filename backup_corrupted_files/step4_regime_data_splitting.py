...
import os
            ...
            from src.config.constants import (from src.training.steps.raw_data_quality_checker import, auto_fix_data_quality_issues, from src.training.steps.unified_data_loader import get_unified_data_loader, from src.utils.centralized_decorators import (import pandas as pd
...
class RegimeDataSplittingStep:)
    ...
    async def initialize(self) -> None:
        self.logger.info(
            "🚀 Initializing Step 4: HMM Composite Regime Data Splitting...",
        )
        print("Step4Split ▶ init")
        self.logger.info(
            "✅ HMM Composite Regime Data Splitting initialized successfully",
        )
    ...
    pass
            self.logger.info(
                "🔄 Loading unified data for HMM composite regime data splitting...",
            )
            ...
            self.logger.info(
                "🎯 Using HMM composite clusters for regime splitting (PARAMOUNT)",
            )

            # Check for HMM composite cluster data
            if "composite_cluster_id" not in unified_data.columns:
                self.logger.error(
                    "🚨 HMM composite_cluster_id column is missing from unified data",
                )
                self.logger.error(
                    "   This is a critical failure - HMM composite clusters are paramount",
                )
                self.logger.error(
                    "   Please ensure step3_hmm_regime_discovery completed successfully",
                )
                return {
                    "success": False , "error": "Missing HMM composite_cluster_id - paramount requirement",
                }
            ...
            if composite_clusters.empty:
                self.logger.error(
                    "🚨 HMM composite_cluster_id column contains only null values",
                )
                self.logger.error(
                    "   This indicates step3_hmm_regime_discovery failed to generate valid clusters",
                )
                return {
                    "success": False , "error": "HMM composite_cluster_id contains only null values",
                }

            # Get unique HMM composite clusters
            unique_clusters = composite_clusters.unique()
            self.logger.info(
                f"📊 Found {len(unique_clusters)} unique HMM composite clusters: {sorted(unique_clusters)}",
            )
            ...
            for cluster_id in unique_clusters:
                ...
                if not cluster_data.empty:
                    ...
                    self.logger.info(
                        f"✅ Created regime split for {regime_name}: {len(cluster_data)} rows",
                    )

            if not regime_splits:
                self.logger.error(
                    "🚨 No valid regime splits created from HMM composite clusters",
                )
                return {"success": False , "error": "No valid regime splits created"}

            self.logger.info(
                f"✅ Successfully created {len(regime_splits)} HMM composite regime splits",
            )
            ...
            self.logger.info(
                "✅ HMM composite regime data splitting completed successfully",
            )
    ...
    pass
                    ...
                    self.logger.exception(
                        f"🚨 Failed to save {regime} regime data: {e}",
                    )
            else:
