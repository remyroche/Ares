...
async def test_futures_download():
    ...
    logger.info(f"Running test for {symbol} on {exchange_name} with a lookback of {lookback_years:.3f} years.")
    ...
    if success:
        logger.info(f"Check the 'data_cache' directory for files like 'futures_{exchange_name}_{symbol}_YYYY-MM.csv'")
