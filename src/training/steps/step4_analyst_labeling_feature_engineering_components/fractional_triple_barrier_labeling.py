...
Implements continuous labeling instead of binary classification for better gradient flow
and more nuanced risk management.
"""
...
class FractionalTripleBarrierLabeling:
    ...
    def apply_fractional_triple_barrier_labeling(
        ...
        self.logger.info(
            f"Fractional labeling complete: {len(filtered_data)}/{len(labeled_data)} samples retained"
        )
    ...
    def _calculate_time_decay_scores(self, labeled_data: pd.DataFrame) -> np.ndarray:
        ...
        if profit_hits.any():
            ...
            profit_pcts = labeled_data.loc[profit_hits, "potential_profit_pct"]
            scores[profit_hits] = np.clip(
                profit_pcts / self.base_labeler.profit_take_multiplier, 0, 1
            )

        if stop_hits.any():
            # Quick stop hits get lower scores (worse performance)
            stop_pcts = labeled_data.loc[stop_hits, "potential_profit_pct"]
            scores[stop_hits] = np.clip(
                stop_pcts / self.base_labeler.stop_loss_multiplier, 0, 1
            )
    ...
    def get_fractional_label_statistics(
