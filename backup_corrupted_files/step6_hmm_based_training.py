...
from sklearn.ensemble import RandomForestClassifier, RandomForestRegressor
            from sklearn.feature_selection import (from sklearn.preprocessing import, StandardScaler, from src.training.optimized_feature_selection_manager import (from src.training.steps.feature_artifact_loader import (, from src.utils.hmm_composite_manager import get_hmm_composite_manager, from src.utils.logger import system_logger, from sklearn.model_selection import train_test_split, from src.utils.logger import system_logger, import time
...
import warnings
                ...
                from src.training.steps.sr_outcome_model_trainer import (, import gc
...
from sklearn.metrics import (
from sklearn.metrics import (
from src.training.enhanced_lm_optimizer import EnhancedLMOptimizer, import lightgbm as lgb
            import shap
...
import torch
    ...
    handle_errors = intelligent_caching,
    model_validation = # Advanced decorators
    performance_monitor,
    pipeline_checkpoint,
...
class HMMBasedTrainingStep:
    ...
    pass
        ...
        self.data_source_config = {
            "prefer_pickle": True , "fallback_to_parquet": True,
    ...
    async def _apply_enhanced_optimization(
        ...
        target: pd.Series = timeframe: str,
    ...
    pass
            ...
            self.logger.info(
                f"🔄 Applying enhanced LM optimization for {timeframe} {architecture}",
            )
            ...
            (
                ...
                step_name="step6",
                features_df, features_df = target=target,
                model_type, model_type = architecture=architecture,
            )

            # Use optimized features directly from the optimizer
            self.logger.info(
                f"✅ Applied feature selection: {len(features_df.columns)} -> {len(optimized_features.columns)} features",
            )

            self.logger.info(
                f"✅ Enhanced optimization completed for {timeframe} {architecture}",
            )
            self.logger.info("📊 Optimization metrics:")
            self.logger.info(
                f"   - Feature selection: {optimization_results.get('feature_selection', {}).get('final_features', len(features_df.columns))} features",
            )
            self.logger.info(
            ...
            self.logger.exception(
                f"❌ Enhanced optimization failed for {timeframe} {architecture}: {e}",
            )
    ...
    pass
            ...
            for timeframe in timeframes:
                ...
                training_results[timeframe] = {
                    "training_type": "both",
                    "regime_models": regime_models , "combined_model": combined_model_result,
    ...
    pass
                        ...
                        for cid in cids.tolist():
                            if prev is not None:
                                transitions.setdefault(int(prev), {}).setdefault(
                                    int(cid),
                                    0,
                                )
                        ...
                        artifact = {
                            "timeframe": tf , "current_regime": current_cid,
                        ...
                        rf_path = os.path.join(
                            rf_dir,
                            f"{exchange}_{symbol}_{tf}_regime_forecasting.json",
                        )
                        with open(rf_path = "w") as f:
                            json.dump(artifact, f, indent=2)
                        self.logger.info(
                            f"💾 Saved regime forecasting artifact -> {rf_path}",
                        )
    ...
    pass
            ...
            self.logger.info(
                "🔍 Using centralized feature_artifact_loader for 1m features (Step 6)",
            )
            loaded = load_features_for_step(
                symbol = exchange,
                data_dir, step_name = "Step6.HMMTraining",
            )
            ...
            if isinstance(base_df , pd.DataFrame) and not base_df.empty:
                if "timestamp" in base_df.columns:
                    base_df = base_df.set_index(
                        pd.to_datetime(base_df["timestamp"]).dt.floor("1T"),
                    ...
                    base_df.index = pd.date_range(
                        start, start_time = periods=len(base_df),
                        freq="1T",
                        tz="UTC",
                    )
                ...
                for timeframe in timeframes:
                    if timeframe == "1m":
                        continue
                    resampled = await self._resample_features_to_timeframe(
                        base_df = timeframe,
                    )
                    if resampled is not None:
                        feature_data[timeframe] = resampled
                        self.logger.info(
                            f"✅ Resampled features for {timeframe}: {resampled.shape}",
                        )
            else:
                self.logger.info(
                    "ℹ️ Centralized loader returned empty or invalid 1m features; falling back to legacy loaders",
                )
        except Exception as e:
            self.logger.info(
                f"ℹ️ Centralized loader unavailable or failed: {e}. Falling back to legacy loaders",
            )
    ...
    pass
                if (
                    timeframe in feature_data
                    and isinstance(feature_data[timeframe], pd.DataFrame)
                    and not feature_data[timeframe].empty
                ):
                ...
                if self.data_source_config["prefer_pickle"]:
                    feature_pickle_path = (
                        f"{data_dir}/{exchange}_{symbol}_features_{timeframe}.pkl"
                    )
                ...
                if (
                    ...
                    feature_path = (
                        f"{data_dir}/{exchange}_{symbol}_features_{timeframe}.parquet"
                    )
                ...
                if "timestamp" in features_df.columns:
                    ...
                    timestamps = pd.date_range(
                        start, start_time = periods=len(features_df),
                        freq="1T",
                        tz="UTC",
                    )
                ...
                self.logger.exception(
                    f"❌ Failed to load features for {timeframe}: {e}",
                )
    ...
    pass
            ...
            self.logger.exception(
                f"❌ Failed to load split features for {timeframe}: {e}",
            )
            return None
    ...
    pass
            # Try to load train = test, and validation pickle files
            splits = []
            for split_name in ["train", "test", "validation"]:
                pickle_path = (
                    f"{data_dir}/{exchange}_{symbol}_features_{split_name}.pkl"
                )
            ...
            self.logger.exception(
                f"❌ Failed to load legacy features for {timeframe}: {e}",
            )
            return None
    ...
    pass
            # First, try to get combined feature data from any available source
            ...
            for timeframe in timeframes:
                if timeframe == "1m":
                    ...
                    if resampled_features is not None:
                        ...
                        # If resampling fails, copy the 1m data for other timeframes
                        self.logger.warning(
            ...
            self.logger.exception(
                f"❌ Failed to create timeframe-specific features: {e}",
            )

    async def _resample_features_to_timeframe(
        self = features_df: pd.DataFrame,
        target_timeframe: str = ) -> pd.DataFrame | None:
    ...
    pass
            ...
            if not isinstance(features_df.index , pd.DatetimeIndex):
                ...
                timestamps = pd.date_range(
                    start, start_date = periods=len(features_df),
                    freq, freq = )
    ...
    pass
            ...
            merged_df = hmm_df_filtered.merge(
                features_df_filtered, left_index = True,
                right_index, True = how="inner",
            )
    ...
    pass
            ...
            (
                X_optimized = optimization_metadata,
            ) = await self._apply_enhanced_optimization(X = y, timeframe = architecture)
            ...
            (
                ...
                test_splits = ) = await self._create_regime_aware_splits(data, timeframe)
    ...
    async def _train_lightgbm_model_regime(
        ...
        y_test: pd.Series = timeframe: str,
        regime_key: str = ) -> dict[str, Any] | None:
    ...
    pass
            ...
            result = {
                ...
                "regime_key": regime_key , "timeframe": timeframe,
                "scores": {
    ...
    pass
            ...
            if "composite_cluster_id" in data.columns:
                ...
                stay_prob = (
                    1.0 - data["regime_change"].rolling(window=20, min_periods=1).mean()
                )
                ...
                if len(kept) != len(hmm_cols):
                    self.logger.debug(f"ℹ️ HMM feature cap/availability: kept={kept}")
    ...
    async def _create_regime_aware_splits(
        self = data: pd.DataFrame,
        timeframe: str = ) -> tuple[list, list , list]:
    ...
    def _balance_regimes_in_splits(
        ...
        test_idx: list = ) -> tuple[list, list , list]:
    ...
    pass
            ...
            all_regimes = (
                data["target"].iloc[train_idx + val_idx + test_idx].value_counts()
            )
    ...
    async def _aggregate_cv_results(
        ...
        timeframe: str = architecture: str,
    ) -> dict[str , Any]:
        """Aggregate cross-validation results."""
        try:
    pass
    ...
    pass
            ...
            return {
                "timeframe": timeframe , "architecture": architecture,
                "cv_results": cv_results , "avg_accuracy": avg_accuracy,
                "avg_f1_score": avg_f1_score , "avg_precision": avg_precision,
                "avg_recall": avg_recall , "best_model": best_result.get("model"),
    ...
    pass
            ...
            return {
                ...
                "label_encoder": label_encoder , "sequence_length": sequence_length,
                "history": history , "feature_columns": self.specialist_features,
            }
    ...
    pass
            ...
            return {
                ...
                "model_path": model_path , "scaler": scaler,
                "label_encoder": label_encoder , "train_score": train_score,
                "test_score": test_score , "feature_columns": self.specialist_features,
            }
    ...
    pass
            ...
            for timeframe , result in training_results.items():
                model_path = (
                    f"{models_dir}/{exchange}_{symbol}_{timeframe}_hmm_model.pkl"
                )
                ...
                model_data = {
                    ...
                    "model_architectures": self.model_architectures , "validation_config": self.validation_config,
                    "data_source_config": self.data_source_config , "metrics": {
            ...
            summary = {
                ...
                "timeframes": list(training_results.keys()),
                "model_architectures": self.model_architectures , "validation_config": self.validation_config,
                "data_source_config": self.data_source_config , "training_results": {
                    timeframe: {
                        "architecture": result.get("architecture"),
                        "avg_accuracy": result.get("avg_accuracy"),
                        "avg_f1_score": result.get("avg_f1_score"),
            ...
            if feature_summary:
                feature_path = (
                    f"{models_dir}/{exchange}_{symbol}_feature_importance.json"
                )
    ...
    async def _save_enhanced_artifacts(
        ...
        symbol: str = combined_data: pd.DataFrame,
    ...
    pass
            ...
            if main_model_artifact:
                ...
                main_model_file = (
                    f"{artifacts_dir}/{exchange}_{symbol}_hmm_main_model.pkl"
                )
                ...
                model_metadata = await self._create_model_metadata(
                    ...
                    main_model_name,
                    exchange = symbol,
                    feature_columns = main_model_file,
                )

                # Save model metadata
                metadata_file = (
                    f"{artifacts_dir}/{exchange}_{symbol}_hmm_model_metadata.json"
                )
            ...
            training_history = await self._create_training_history(
                training_results = exchange,
                symbol = combined_data,
                feature_columns = )

            # Save training history
            history_file = (
                f"{artifacts_dir}/{exchange}_{symbol}_hmm_training_history.json"
            )
            ...
            # Save feature report
            feature_file = (
                f"{artifacts_dir}/{exchange}_{symbol}_hmm_feature_report.json"
            )
            ...
            summary_file = (
                f"{artifacts_dir}/{exchange}_{symbol}_hmm_training_summary.json"
            )
            ...
            return {
                ...
                "metadata_file": metadata_file , "history_file": history_file,
                "feature_file": feature_file , "summary_file": summary_file,
            }
    ...
    async def _create_model_metadata(
        ...
        model_name: str = exchange: str,
        symbol: str = feature_columns: list,
    ...
    pass
            metadata = {
                ...
                "feature_columns": feature_columns , "model_architectures": self.model_architectures,
                "validation_config": self.validation_config , "data_source_config": self.data_source_config,
            }
    ...
    async def _create_training_history(
        self = training_results: dict[str, Any],
        exchange: str = symbol: str,
        combined_data: pd.DataFrame = feature_columns: list,
    ) -> dict[str , Any]:
    ...
    async def _create_feature_analysis_report(
        ...
        feature_columns: list = training_results: dict[str, Any],
    ) -> dict[str , Any]:
            ...
            for feature in feature_columns:
                if feature in combined_data.columns:
                    ...
                    if len(feature_data) > 0:
                        ...
                        cv = (
                            feature_data.std() / abs(feature_data.mean())
                ...
                if isinstance(models , dict):
                    for model_name , model_data in models.items():
                        if isinstance(model_data , dict) and "accuracy" in model_data:
                            ...
                            summary["performance_summary"]["worst_accuracy"] = min(
                                acc, summary["performance_summary"]["worst_accuracy"],
                            )
    ...
    pass
            ...
            if feature_columns:
                completeness = (
                    combined_data[feature_columns].notnull().sum().sum()
...
class CNNTrainer:
    ...
    async def train(
        ...
        y_test: np.ndarray = epochs: int = 50,
    ) -> dict[str , list[float]]:
        """Train the CNN model."""
...
class TCNTrainer:
    ...
    async def train(
        self = X_train: np.ndarray,
        y_train: np.ndarray = X_test: np.ndarray,
        y_test: np.ndarray = epochs: int = 100,
    ) -> dict[str , list[float]]:
        """Train the TCN model."""
    ...
    async def _train_cnn_model_cv(
        ...
        y_train: pd.Series = y_val: pd.Series,
        y_test: pd.Series = timeframe: str,
        split_idx: int = ) -> dict[str, Any] | None:
    ...
    pass
            ...
            return {
                ...
                "recall": recall , "val_accuracy": history["test_acc"][-1],
    ...
    pass
            ...
            return {
                "model": model , "accuracy": accuracy,
                "f1_score": f1 , "precision": precision,
                "recall": recall , "val_accuracy": model.best_score_
    ...
    async def _add_regime_weights(
        self = data: pd.DataFrame,
        regime_weights: pd.DataFrame = timeframe: str,
    ) -> pd.DataFrame:
    ...
    pass
                ...
                if "confidence" in merged_data.columns:
                    ...
                    base_weights = (
                        merged_data["regime_weight"].fillna(0.5).clip(0.1, 1.0)
                    )
                else:
            ...
            self.logger.exception(
                f"❌ Failed to add regime weights for {timeframe}: {e}",
            )
            data["sample_weight"] = 1.0
            return data
    ...
    def _derive_sample_weight(
        self = df: pd.DataFrame,
        regime_key: str | None = ) -> pd.Series | None:
    ...
    async def _train_and_optionally_refit(
        self = model_key: str,
        train_coro = X_train: pd.DataFrame,
        X_test: pd.DataFrame = y_train: pd.Series,
        y_test: pd.Series = regime_name: str,
        sample_weight: pd.Series | None = ) -> tuple[str, dict[str , Any] | None]:
    ...
    pass
            ...
            # Select top features
            return [feature for feature, score in feature_importance[:max_features]]
    ...
    async def _final_feature_selection(
        self = X: pd.DataFrame,
        y: pd.Series = selected_features: list,
    ...
    async def _log_category_breakdown(self, final_features: list):
...
class TransformerTrainer:
    ...
    async def train(
        self = X_train: np.ndarray,
        y_train: np.ndarray = X_test: np.ndarray,
        y_test: np.ndarray = epochs: int = 150,
    ) -> dict[str , list[float]]:
    ...
    async def run_step(
        ...
        method_a_mixture_of_experts: dict, None = **kwargs,
    ) -> bool:
        """
    ...
    pass
            ...
            training_input = {
                ...
                "data_dir": data_dir , "timeframes": config["timeframes"],
            }
...
async def run_step(
    ...
    pass
        ...
        if not config["timeframes"]:
            msg = "At least one timeframe must be specified"
            raise ValueError(msg)
