#!/usr/bin/env python3
"""
VectorBT Integration Validation Script
...
Usage:
    python3 validate_vectorbt_integration.py
"""
...
class VectorBTValidator:
    ...
    def validate_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            vectorbt_import_found = any(re.search(pattern, content) for pattern in self.vectorbt_patterns[:3])
            ...
            vectorbt_ops = sum(len(re.findall(pattern, content)) for pattern in self.vectorbt_patterns[3:])
            ...
            pandas_ops = sum(len(re.findall(pattern, content)) for pattern in self.pandas_patterns)
    ...
    def generate_validation_report(self, results: List[Dict[str, Any]]) -> str:
        ...
        if files_without_vectorbt:
            ...
            for result in files_without_vectorbt:
        ...
        if self.validation_results['pandas_fallbacks'] > self.validation_results['vectorbt_operations']:
            report.append(f"  3. Replace pandas operations with VectorBT operations where possible")
        
        report.append(f"  4. Ensure all rolling operations use VectorBT for datasets > 1000 samples")
