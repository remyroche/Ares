#!/usr/bin/env python3
"""
Test Script for Interactive Feature Generation Fixes
...
async def test_feature_generators():
    """Test the feature generators directly."""
    tprint("🧪 Testing Feature Generators...")
    ...
    try:
        ...
        tprint_info(f"📊 Interaction features shape: {interaction_features.shape}")
        
        if len(interaction_features.columns) > 0:
            tprint_info(f"📊 Sample interaction features: {list(interaction_features.columns[:10])}")
        else:
        ...
        tprint_success(f"✅ Generated {len(ctf_features.columns)} cross-timeframe features")
        tprint_info(f"📊 Cross-timeframe features shape: {ctf_features.shape}")
        
        if len(ctf_features.columns) > 0:
            tprint_info(f"📊 Sample cross-timeframe features: {list(ctf_features.columns[:10])}")
        else:
        ...
        tprint_error(f"❌ Feature generator test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
async def main():
    """Run all tests."""
    tprint("🚀 Starting Interactive Feature Generation Fix Tests")
    tprint("=" * 60)
