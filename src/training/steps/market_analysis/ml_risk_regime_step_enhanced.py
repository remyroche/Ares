...
class EnhancedMLRiskRegimeStep(SpecialistDiagnosticsMixinEnhancedV2, BaseStep):
    ...
    def __init__(self, step_name: str = "enhanced_ml_risk_regime_step"):
        """Initialize the enhanced risk regime step."""
        super().__init__()
    ...
    def _create_manual_risk_enhanced_features(self, df: pd.DataFrame, enhanced_features: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in ['close', 'high', 'low', 'volume']):
            ...
            # 1. Enhanced multi-dimensional volatility features
            ...
            # Volatility regime classification (3-state)
            vol_zscore_20 = (vol_20 - vol_20.rolling(100).mean()) / (vol_20.rolling(100).std() + 1e-8)
            manual_features['enhanced_volatility_regime'] = np.where(vol_zscore_20 > 1.5, 2, np.where(vol_zscore_20 < -1.5, 0, 1))
            
            # Volatility momentum (change in volatility)
            vol_momentum = vol_20.diff().rolling(5).mean()
            manual_features['enhanced_volatility_momentum'] = vol_momentum
            
            # Volatility acceleration (second derivative)
            vol_acceleration = vol_20.diff().diff()
            manual_features['enhanced_volatility_acceleration'] = vol_acceleration
            
            # 2. Advanced drawdown-based risk features
            # Rolling maximum drawdown
            rolling_max = close.rolling(20).max()
            drawdown = (close - rolling_max) / rolling_max
            max_drawdown = drawdown.rolling(20).min()
            manual_features['enhanced_max_drawdown'] = max_drawdown
            
            # Drawdown velocity (speed of decline)
            drawdown_velocity = drawdown.diff().rolling(5).mean()
            manual_features['enhanced_drawdown_velocity'] = drawdown_velocity
            
            # Drawdown duration (consecutive periods in drawdown)
            drawdown_duration = (drawdown < 0).astype(int).rolling(20).sum()
            manual_features['enhanced_drawdown_duration'] = drawdown_duration
            
            # 3. Volume-adjusted risk features
            # Volume-weighted volatility
            volume_ma = volume.rolling(20).mean()
            volume_ratio = volume / (volume_ma + 1e-8)
            volume_weighted_vol = vol_20 * (1 + np.log(volume_ratio + 1))
            manual_features['enhanced_volume_weighted_volatility'] = volume_weighted_vol
            
            # Volume-volatility divergence (anomaly detection)
            ...
            manual_features['enhanced_volume_vol_divergence'] = volume_vol_divergence
            
            # 4. Price-based risk features
            # Range-based volatility
            range_ratio = (high - low) / close
            range_vol = range_ratio.rolling(20).std()
            manual_features['enhanced_range_volatility'] = range_vol
            
            # Price efficiency (how efficiently price moves)
            price_efficiency = abs(returns.rolling(10).mean()) / (vol_10 + 1e-8)
            manual_features['enhanced_price_efficiency'] = price_efficiency
            
            # Trend strength (persistency of moves)
            trend_strength = abs(returns.rolling(20).mean()) / (returns.rolling(20).std() + 1e-8)
            manual_features['enhanced_trend_strength'] = trend_strength
            
            # 5. Market microstructure risk features
            # Bid-ask spread proxy (using range)
            spread_proxy = range_ratio
            spread_volatility = spread_proxy.rolling(20).std()
            manual_features['enhanced_spread_volatility'] = spread_volatility
            
            # Order flow imbalance proxy
            price_volume_imbalance = (returns * volume).rolling(10).sum()
            manual_features['enhanced_order_flow_imbalance'] = price_volume_imbalance
            
            # Market depth proxy
            market_depth = volume / (range_ratio + 1e-8)
            manual_features['enhanced_market_depth'] = market_depth
            
            # 6. Downside risk focus
            # Downside volatility (negative returns only)
            downside_returns = returns.copy()
            downside_returns[downside_returns > 0] = 0
            downside_vol = downside_returns.rolling(20).std()
            manual_features['enhanced_downside_volatility'] = downside_vol
            
            # Semi-variance (downside risk focus)
            semi_variance = ((downside_returns) ** 2).rolling(20).mean()
            manual_features['enhanced_semi_variance'] = semi_variance
            
            # Value at Risk proxy (5th percentile)
            var_proxy = returns.rolling(20).quantile(0.05)
            manual_features['enhanced_var_proxy'] = var_proxy
            
            # 7. Risk regime transitions
            # Risk regime changes
            risk_regime_changes = vol_regime.diff().abs()
            manual_features['enhanced_risk_regime_changes'] = risk_regime_changes
            
            # Risk regime persistence
            risk_regime_persistence = (vol_regime == 1).rolling(10).sum()
            manual_features['enhanced_risk_regime_persistence'] = risk_regime_persistence
            
            # 8. Composite risk indicators
            # Risk stress index (combines multiple risk factors)
            risk_stress_index = (
            ...
            manual_features['enhanced_risk_stress_index'] = risk_stress_index
            
            # Risk appetite indicator (inverse of risk stress)
            risk_appetite = 1 - risk_stress_index
            manual_features['enhanced_risk_appetite'] = risk_appetite
    ...
    def _train_enhanced_risk_model(self, features: pd.DataFrame, labels: pd.Series, 
        ...
        """Train enhanced risk model with MI optimization."""
        ...
        temporal_config = create_temporal_split_config_for_pipeline(
            ...
            n_splits=config.get("risk_n_splits", 5),
            walk_forward_type="rolling",
            test_size_ratio=config.get("risk_test_size_ratio", 0.2),
            min_train_samples=config.get("risk_min_train_samples", 500),
        )
    ...
    async def execute(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            feature_df = self._generate_enhanced_risk_features(market_data, config)
            
            tprint_info(f"✅ Enhanced Risk Regime features: {len(feature_df.columns)} columns")
            ...
            if not config.get("is_batch_run", False):
                ...
                features_path = self._save_artifact(
                    ...
                    artifact_name="enhanced_ml_risk_features",
                    artifact_type="data",
            ...
            # 3. Generate Labels
            tprint_info("🎯 Generating Enhanced Risk Regime labels...")
            labels = self._create_risk_labels(market_data)
            ...
            tprint_info("🤖 Training Enhanced Risk Regime model with MI optimization...")
            model, model_metrics = self._train_enhanced_risk_model(X, y, config)
            ...
            # 7. Save Artifacts
            artifact_name = f"enhanced_ml_risk_predictions_{timeframe}"
            ...
            print(f"🐛 DEBUG: Output df shape: {standardized_output.shape}")
    ...
    def _load_market_data_with_cache(self, config: Dict[str, Any], timeframe: str) -> Tuple[pd.DataFrame, str]:
        """Load market data with caching and extended historical coverage."""
        symbol = config.get("symbol", "ETHUSDT")
        exchange = config.get("exchange", "binance")
        ...
        try:
            # Load available data
            from src.data_sources.klines_adapter import KlinesAdapter
            adapter = KlinesAdapter()
            
            # Load maximum available historical data
            end_date = datetime.now()
            start_date = end_date - timedelta(days=1000)  # ~3 years of data
            
            market_data = adapter.fetch_klines(
                symbol=symbol,
                exchange=exchange,
                interval=timeframe,
            ...
            if market_data.empty:
                ...
                self.logger.warning(f"No market data available for {symbol}, generating synthetic data")
                market_data = self._generate_synthetic_market_data(symbol, timeframe, end_date)
                source = "synthetic"
            else:
                source = "klines"
                
                # Extend coverage with synthetic data if needed
                if len(market_data) < 10000:
                    self.logger.info(f"Extending coverage with synthetic data for {symbol}")
                    synthetic_data = self._generate_synthetic_market_data(symbol, timeframe, end_date)
                    
                    # Combine real and synthetic data
                    synthetic_data = synthetic_data[~synthetic_data.index.isin(market_data.index)]
                    market_data = pd.concat([synthetic_data, market_data]).sort_index()
                    source = "hybrid"
            
            # Cache the data
            self._market_data_cache[cache_key] = market_data
            
            self.logger.info(f"✅ Loaded {len(market_data)} rows of market data for {symbol} from {source}")
            return market_data, source
            
        except Exception as e:
            self.logger.error(f"❌ Failed to load market data: {e}")
            # Generate synthetic data as fallback
            market_data = self._generate_synthetic_market_data(symbol, timeframe, datetime.now())
            self._market_data_cache[cache_key] = market_data
            return market_data, "synthetic_fallback"
    ...
    def _generate_synthetic_market_data(self, symbol: str, timeframe: str, end_date: datetime) -> pd.DataFrame:
        ...
        for i in range(total_periods):
            ...
            mean_reversion = -mean_reversion_strength * current_return
            
            # Random shock
            random_shock = np.random.normal(0, volatility)
            
            # Total return
            total_return = drift + mean_reversion + random_shock
