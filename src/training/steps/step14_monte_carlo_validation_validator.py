...
class Step14MonteCarloValidationValidator(BaseValidator):
    ...
    async def validate(self, training_input: Dict[str, Any], pipeline_state: Dict[str, Any]) -> bool:
        ...
        if not error_passed:
            self.logger.error("❌ Monte Carlo validation step had errors")
            return False
        
        # 2. Validate Monte Carlo validation files existence
        validation_files_passed = self._validate_monte_carlo_files(symbol, exchange, data_dir)
        if not validation_files_passed:
            self.logger.error("❌ Monte Carlo validation files validation failed")
            return False
        
        # 3. Validate Monte Carlo statistical significance
        significance_passed = self._validate_statistical_significance(symbol, exchange, data_dir)
        if not significance_passed:
            self.logger.error("❌ Statistical significance validation failed")
            return False
        
        # 4. Validate Monte Carlo performance distribution
        distribution_passed = self._validate_performance_distribution(symbol, exchange, data_dir)
        if not distribution_passed:
            self.logger.error("❌ Performance distribution validation failed")
            return False
        
        # 5. Validate Monte Carlo robustness
        robustness_passed = self._validate_monte_carlo_robustness(symbol, exchange, data_dir)
        if not robustness_passed:
            self.logger.error("❌ Monte Carlo robustness validation failed")
            return False
        ...
        if not outcome_passed:
            self.logger.warning("⚠️ Monte Carlo validation outcome is not favorable")
            return False
        
        self.logger.info("✅ Monte Carlo validation validation passed")
        return True
    
    def _validate_monte_carlo_files(self, symbol: str, exchange: str, data_dir: str) -> bool:
        """
        ...
        try:
            ...
            for file_path in expected_files:
                file_passed, file_metrics = self.validate_file_exists(file_path, "monte_carlo_files")
                if not file_passed:
                    missing_files.append(file_path)
            
            if missing_files:
                self.logger.error(f"❌ Missing Monte Carlo validation files: {missing_files}")
                return False
            ...
            self.logger.error(f"❌ Error validating Monte Carlo validation files: {e}")
            return False
    
    def _validate_statistical_significance(self, symbol: str, exchange: str, data_dir: str) -> bool:
        """
        ...
        try:
            ...
            if os.path.exists(results_file):
                ...
                if "simulation_count" in results:
                    ...
                    if sim_count < 1000:
                        self.logger.warning(f"⚠️ Few Monte Carlo simulations: {sim_count}")
                    elif sim_count > 100000:
                        self.logger.warning(f"⚠️ Many Monte Carlo simulations: {sim_count}")
            ...
            self.logger.error(f"❌ Error during statistical significance validation: {e}")
            return False
    
    def _validate_performance_distribution(self, symbol: str, exchange: str, data_dir: str) -> bool:
        """
            ...
            if os.path.exists(performance_file):
                ...
                if "distribution_stats" in performance:
                    ...
                    if "mean" in stats:
                        ...
                        if mean_perf < 0.5:
                            self.logger.warning(f"⚠️ Low mean Monte Carlo performance: {mean_perf:.3f}")
                    
                    # Check standard deviation
                    if "std" in stats:
                        std_perf = stats["std"]
                        if std_perf > 0.2:
                            self.logger.warning(f"⚠️ High Monte Carlo performance variance: {std_perf:.3f}")
                        elif std_perf < 0.01:
                            self.logger.warning(f"⚠️ Very low Monte Carlo performance variance: {std_perf:.3f}")
                    
                    # Check skewness
                    if "skewness" in stats:
                        skewness = stats["skewness"]
                        if abs(skewness) > 2:
                            self.logger.warning(f"⚠️ Highly skewed Monte Carlo performance: {skewness:.3f}")
                    
                    # Check kurtosis
                    if "kurtosis" in stats:
                        kurtosis = stats["kurtosis"]
                        if kurtosis > 10:
                            self.logger.warning(f"⚠️ High kurtosis in Monte Carlo performance: {kurtosis:.3f}")
                ...
                if "percentiles" in performance:
                    ...
                    if "5th" in percentiles:
                        p5 = percentiles["5th"]
                        if p5 < 0.3:
                            self.logger.warning(f"⚠️ Poor 5th percentile performance: {p5:.3f}")
                    
                    # Check 95th percentile (best case)
                    if "95th" in percentiles:
                        p95 = percentiles["95th"]
                        if p95 < 0.6:
                            self.logger.warning(f"⚠️ Poor 95th percentile performance: {p95:.3f}")
                ...
                if "stability_metrics" in performance:
                    ...
                    if "coefficient_of_variation" in stability:
                        ...
                        if cv > 0.5:
                            self.logger.warning(f"⚠️ High coefficient of variation: {cv:.3f}")
                    
                    if "interquartile_range" in stability:
                        iqr = stability["interquartile_range"]
                        if iqr > 0.3:
                            self.logger.warning(f"⚠️ High interquartile range: {iqr:.3f}")
            
            self.logger.info("✅ Performance distribution validation passed")
            return True
            
        except Exception as e:
            self.logger.error(f"❌ Error during performance distribution validation: {e}")
            return False
    
    def _validate_monte_carlo_robustness(self, symbol: str, exchange: str, data_dir: str) -> bool:
        """
            ...
            if os.path.exists(metadata_file):
                ...
                if "simulation_parameters" in metadata:
                    ...
                    if "random_seed" in params:
                        ...
                        if seed == 0 or seed == 42:  # Common default seeds
                            self.logger.warning(f"⚠️ Using common random seed: {seed}")
                    
                    # Check sample size
                    if "sample_size" in params:
                        sample_size = params["sample_size"]
                        if sample_size < 100:
                            self.logger.warning(f"⚠️ Small Monte Carlo sample size: {sample_size}")
                        elif sample_size > 10000:
                            self.logger.warning(f"⚠️ Large Monte Carlo sample size: {sample_size}")
                ...
                if "convergence_metrics" in metadata:
                    ...
                    if "converged" in convergence:
                        ...
                        if not converged:
                            self.logger.warning("⚠️ Monte Carlo simulations did not converge")
                    
                    if "convergence_iterations" in convergence:
                        iterations = convergence["convergence_iterations"]
                        if iterations > 1000:
                            self.logger.warning(f"⚠️ Many convergence iterations: {iterations}")
                ...
                if "robustness_metrics" in metadata:
                    ...
                    if "sensitivity_score" in robustness:
                        ...
                        if sensitivity > 0.8:
                            self.logger.warning(f"⚠️ High sensitivity to parameters: {sensitivity:.3f}")
                    
                    if "stability_score" in robustness:
                        stability = robustness["stability_score"]
                        if stability < 0.7:
                            self.logger.warning(f"⚠️ Low Monte Carlo stability: {stability:.3f}")
            
            self.logger.info("✅ Monte Carlo robustness validation passed")
            return True
            
        except Exception as e:
            self.logger.error(f"❌ Error during Monte Carlo robustness validation: {e}")
            return False
