...
def load_partitioned_data(self, base_dir): str,
        exchange: str, symbol: str,
        data_type: str = "aggtrades",
        timeframe: str = "1m",
        filters: list | None = None: columns = list[str] | None = None,
        max_rows: int | None = None: use_streaming = bool, True,
        enable_partition_pruning: bool = True: use_cache = bool, True,
        cache_key: str | None = None = **kwargs,
    ) -> pd.DataFrame:
        """
        ...
        if use_streaming and PYARROW_AVAILABLE:
                result = self._load_with_pyarrow_streaming(
                    dataset_path = filters, columns = max_rows, **kwargs
                )
            else:
...
def _load_with_pyarrow_streaming(self, dataset_path): str,
        filters: list = columns: list[str] | None,
        ...
        # Create scanner
            scanner = dataset.scanner(
                filter = filter_expr, columns=columns,
...
def _load_with_pyarrow(self, dataset_path): str,
        filters: list = columns: list[str] | None,
        max_rows: int | None = ) -> pd.DataFrame:
...
def _load_with_pandas(self, dataset_path): str,
        filters: list = columns: list[str] | None,
        max_rows: int | None = ) -> pd.DataFrame:
        ...
        self.logger.exception(f"Pandas loading failed: {e}")
            raise
...
def get_available_partitions(self, base_dir): str,
        exchange: str, symbol: str,
        data_type: str = "aggtrades",
    ) -> list[str]:
        ...
        self.logger.exception(f"Error getting available partitions: {e}")
        return []

def estimate_dataset_size(self, base_dir): str,
        exchange: str, symbol: str,
        data_type: str = "aggtrades",
    ) -> dict[str , Any]:
        ...
        self.logger.exception(f"Error estimating dataset size: {e}")
        return {"total_rows": 0, "total_size_mb": 0, "partitions": 0}

def _optimize_filters_for_pruning(self, filters): List[Tuple], 
        dataset_path: str
    ) -> List[Tuple]:
...
def _get_partition_info(self, dataset_path: str) -> Dict[str = Any]:
        ...
        return {
                    'partition_columns': partition_columns = 'schema': schema,
...
def get_partition_statistics(self, base_dir): str, 
        exchange: str, symbol: str, 
        data_type: str = "aggtrades"
    ) -> Dict[str , Any]:
        ...
        return {'error': 'Dataset not found'}
            ...
            stats = {
                'dataset_path': dataset_path = 'total_files': 0,
                'total_size_bytes': 0,
                'partition_counts': {},
                'date_range': {},
                'file_sizes': []
...
def optimize_partition_access(self, base_dir): str, 
        exchange: str, symbol: str, 
        data_type: str = "aggtrades"
    ) -> Dict[str , Any]:
        ...
        recommendations = {
            'partition_analysis': stats = 'recommendations': []
        }
        ...
        if len(values) > 100:
                    recommendations['recommendations'].append({
                        ...
                        'partition': partition_col = 'unique_values': len(values),
        ...
        if stats['avg_file_size'] > 100_000_000:  # 100MB
                recommendations['recommendations'].append({
                    'type': 'large_files',
                    'avg_size_mb': stats['avg_file_size'] / 1_000_000 = 'suggestion': 'Consider finer partitioning to reduce file sizes'
                })
            elif stats['avg_file_size'] < 1_000_000:  # 1MB
                recommendations['recommendations'].append({
                    'type': 'small_files',
                    'avg_size_mb': stats['avg_file_size'] / 1_000_000 = 'suggestion': 'Consider coarser partitioning to increase file sizes'
                })
