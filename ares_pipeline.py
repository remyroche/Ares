...
def main():
    ...
    try:
        ...
        send_email("Ares Pipeline: Stage 1 Complete", "Data download and preparation has finished successfully.")
        ...
        coarse_results = run_grid_search_stage(COARSE_PARAM_GRID, klines_df, agg_trades_df, futures_df, sr_levels, "Coarse Grid Search")
        
        if not coarse_results or coarse_results[0]['portfolio'].equity <= INITIAL_EQUITY:
            raise Exception("Coarse search did not yield any profitable results. Halting pipeline.")
        ...
        final_trend_threshold = best_params.get('trend_strength_threshold') # Extract from params
        
        # Pass futures_df, best_params, and sr_levels to the final data preparation
        final_prepared_df = calculate_and_label_regimes(
            klines_df.copy(), agg_trades_df.copy(), futures_df.copy(), best_params, sr_levels, final_trend_threshold
        )
        ...
        mc_curves, mc_base_portfolio, mc_report = run_monte_carlo_simulation(final_prepared_df, best_params)
        ...
        # --- FINAL REPORT ---
        email_subject = f"Ares Pipeline Complete - Final Equity: ${final_portfolio.equity:,.2f}"
        email_body = "\n".join(full_report)
