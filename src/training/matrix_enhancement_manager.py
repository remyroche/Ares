...
class MatrixEnhancementManager:
    ...
    def enhance_features_with_svd(self, features_df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """
        ...
        try:
            ...
            n_components = np.argmax(cumulative_variance >= self.config.min_explained_variance) + 1
            ...
            self.logger.info(f"✅ SVD enhancement completed: {n_components} components, {metadata['cumulative_variance']:.3f} variance explained")
            return enhanced_df, metadata
            
        except Exception as e:
            self.logger.error(f"❌ SVD enhancement failed: {e}")
            return features_df, {"error": str(e)}
    
    @handle_errors(exceptions=(ValueError, np.linalg.LinAlgError), default_return=None)
    def enhance_features_with_nmf(self, features_df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """
        ...
        try:
            ...
            nmf_feature_names = [f"nmf_component_{i+1}" for i in range(self.config.nmf_n_components)]
            ...
            self.logger.info(f"✅ NMF enhancement completed: {self.config.nmf_n_components} components, reconstruction error: {metadata['reconstruction_error']:.6f}")
            return enhanced_df, metadata
            
        except Exception as e:
            self.logger.error(f"❌ NMF enhancement failed: {e}")
            return features_df, {"error": str(e)}
    
    @handle_errors(exceptions=(ValueError, np.linalg.LinAlgError), default_return=None)
    def apply_spectral_clustering_features(self, features_df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """
        ...
        try:
            ...
            distance_feature_names = [f"distance_to_cluster_{i+1}" for i in range(self.config.spectral_n_clusters)]
            ...
            enhanced_df = pd.concat([features_df, cluster_features, distance_df], axis=1)
            
            # Metadata
            metadata = {
                "n_clusters": self.config.spectral_n_clusters,
                "cluster_sizes": [np.sum(cluster_labels == i) for i in range(self.config.spectral_n_clusters)],
                "processing_time": time.time() - start_time
            }
            
            self.logger.info(f"✅ Spectral clustering enhancement completed: {self.config.spectral_n_clusters} clusters")
            return enhanced_df, metadata
            
        except Exception as e:
            self.logger.error(f"❌ Spectral clustering enhancement failed: {e}")
            return features_df, {"error": str(e)}
    
    @handle_errors(exceptions=(ValueError, np.linalg.LinAlgError), default_return=None)
    def apply_tensor_decomposition(self, features_df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """
        ...
        try:
            ...
            if n_windows > 0:
                ...
                unfolded = tensor.reshape(n_windows, -1)
                ...
                # Create feature names
                tensor_feature_names = [f"tensor_component_{i+1}" for i in range(n_components)]
                
                # Create DataFrame (pad with zeros for samples that don't have full windows)
                tensor_df = pd.DataFrame(
                    np.vstack([tensor_features, np.zeros((n_samples - n_windows, n_components))]),
            ...
            self.logger.info(f"✅ Tensor decomposition enhancement completed")
            return enhanced_df, metadata
            
        except Exception as e:
            self.logger.error(f"❌ Tensor decomposition enhancement failed: {e}")
            return features_df, {"error": str(e)}
    ...
    def analyze_matrix_condition(self, features_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis = {
                ...
                "numerically_stable": condition_number < self.config.condition_number_threshold,
                "well_conditioned": min_eigenval > 1e-6
            }
            
            self.logger.info(f"📊 Matrix analysis: condition_number={condition_number:.2e}, rank={rank}/{X.shape[1]}")
            return analysis
            
        except Exception as e:
            self.logger.error(f"❌ Matrix condition analysis failed: {e}")
            return {"error": str(e)}
    
    @handle_errors(exceptions=(ValueError, np.linalg.LinAlgError), default_return=None)
    def apply_sparse_matrix_optimizations(self, features_df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.info(f"✅ Sparse matrix optimization completed: sparsity={metadata['sparsity']:.3f}")
            return enhanced_df, metadata
    ...
    def enhance_training_features(self, features_df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """
        ...
        try:
            ...
            if self.config.enable_spectral_clustering:
                enhanced_df, spectral_metadata = self.apply_spectral_clustering_features(enhanced_df)
                all_metadata["spectral_clustering"] = spectral_metadata
            
            # 5. Tensor decomposition
            if self.config.enable_tensor_decomposition:
                enhanced_df, tensor_metadata = self.apply_tensor_decomposition(enhanced_df)
                all_metadata["tensor_decomposition"] = tensor_metadata
            
            # 6. Sparse optimizations
            if self.config.enable_sparse_operations:
                enhanced_df, sparse_metadata = self.apply_sparse_matrix_optimizations(enhanced_df)
