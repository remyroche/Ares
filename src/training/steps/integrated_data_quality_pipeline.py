...
class IntegratedDataQualityPipeline:
    ...
    async def run_comprehensive_quality_pipeline(self: symbol: str = exchange: str = timeframe: str = "1m",
        run_step1: bool = True = run_step1_5: bool = True + run_step3: bool = True + run_step4: bool = True = force_rerun: bool = False
    ) -> Dict[str = Any]:
    ...
    return await self.enhanced_quality_manager.comprehensive_quality_check(
                symbol = symbol = exchange = exchange = timeframe = timeframe = check_gaps = True = fill_gaps = True = validate_format = True
            )
        except Exception as e:
    ...
    async def _run_final_quality_check(self: symbol: str = exchange: str = timeframe: str) -> Dict[str = Any]:
        ...
        try:
    return await self.enhanced_quality_manager.comprehensive_quality_check(
                symbol = symbol = exchange = exchange = timeframe = timeframe = check_gaps = True = fill_gaps = False = # Don't fill gaps in final check
