...
class RegimeModelsTrainingComponent(BaseMarketAnalysisComponent):
    ...
    def _load_artifacts_from_outcome_files(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # TODO: Implement the rest of artifact loading logic
            return artifacts
        except Exception as e:
    ...
    def get_hpo_config_for_model(self, model_type: str) -> Dict[str, Any]:
        """
        Obtenir la configuration HPO pour un modèle spécifique
        
        Args:
            model_type: Type de modèle
            
        Returns:
            Configuration HPO du modèle
        """
        if not hasattr(self, 'config_manager') or not hasattr(self, 'config'):
            # Fallback à la configuration hardcodée
            model_config = self._get_hardcoded_model_config(model_type)
            return model_config.get('hpo', {"enabled": True, "n_trials": 50, "timeout_seconds": 300})
        
        try:
            return self.config_manager.get_hpo_config(self.config, model_type)
        except Exception as e:
            tprint(f"⚠️ [REGIME_MODELS] Erreur lors de l'obtention de la configuration HPO pour {model_type}: {e}", color="yellow")
            model_config = self._get_hardcoded_model_config(model_type)
            return model_config.get('hpo', {"enabled": True, "n_trials": 50, "timeout_seconds": 300})
    
    def save_custom_config(self, overrides: Dict[str, Any], config_name: str = "custom") -> str:
        """
        Sauvegarder une configuration personnalisée
        
        Args:
            overrides: Modifications à appliquer à la configuration par défaut
            config_name: Nom de la configuration personnalisée
            
        Returns:
            Chemin vers la configuration sauvegardée
        """
        if not hasattr(self, 'config_manager'):
            raise RuntimeError("ConfigManager non initialisé")
        ...
        try:
            ...
            config_file = self.config_manager.save_config(custom_config, config_name, "yaml")
            tprint(f"💾 [REGIME_MODELS] Configuration personnalisée sauvegardée: {config_file}", color="green")
            
            return str(config_file)
            
        except Exception as e:
            tprint(f"❌ [REGIME_MODELS] Erreur lors de la sauvegarde de la configuration personnalisée: {e}", color="red")
            raise
    ...
    def list_available_configs(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            tprint(f"⚠️ [REGIME_MODELS] Erreur lors de la liste des configurations: {e}", color="yellow")
            return []
    
    def validate_config_for_training(self, config: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Valider une configuration pour l'entraînement
        
        Args:
            config: Configuration à valider (utilise la configuration actuelle si None)
            
        Returns:
            Résultat de la validation
        """
        config_to_validate = config or getattr(self, 'config', {})
        
        if not hasattr(self, 'config_manager'):
            return {"valid": True, "warnings": [], "suggestions": []}
        
        try:
            return self.config_manager.validate_for_training(config_to_validate)
        except Exception as e:
            tprint(f"⚠️ [REGIME_MODELS] Erreur lors de la validation de la configuration: {e}", color="yellow")
