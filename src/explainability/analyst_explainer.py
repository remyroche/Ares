...
class AnalystExplainer(BaseExplainer):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.feature_groups = {
            ...
            'location_features': ['location_probability', 'location_confidence', 'location_stability'],
            'ensemble_features': ['ensemble_agreement', 'ensemble_confidence', 'ensemble_diversity'],
            'market_health': ['market_health_score', 'liquidation_risk', 'correlation_risk']
        }
        ...
        try:
            ...
            if self.lime_explainer is not None:
                lime_explanation = self._calculate_lime_explanation(
                    self.lime_explainer, analyst_model, features, feature_names
                )
            ...
            explanation = ExplanationResult(
                model_name="Analyst_RegimeClassification",
    ...
    async def explain_location_classification(
        self,
        analyst_model: Any,
        market_data: pd.DataFrame,
        features: np.ndarray,
        feature_names: List[str],
        location_prediction: Optional[Union[str, int]] = None
    ) -> ExplanationResult:
        ...
        try:
            ...
            # Get location prediction if not provided
            ...
            # Create explanation result
            explanation = ExplanationResult(
                model_name="Analyst_LocationClassification",
                ...
                metadata={
                    ...
                    'explanation_type': 'location_classification'
                }
            )
    ...
    async def explain_ensemble_prediction(
        ...
        try:
            ...
            # Get ensemble prediction if not provided
            ...
            # Calculate SHAP values for ensemble
            shap_values = None
            if self.shap_explainer is not None:
            ...
            # Create explanation result
            explanation = ExplanationResult(
                model_name="Analyst_EnsemblePrediction",
                ...
                metadata={
                    ...
                    'regime_types': self.regime_types,
                    'location_types': self.location_types,
                    'explanation_type': 'ensemble_prediction'
                }
            )
    ...
    async def explain_confidence_prediction(
        self,
        analyst_model: Any,
        market_data: pd.DataFrame,
        features: np.ndarray,
        feature_names: List[str],
        confidence_prediction: Optional[float] = None
    ) -> ExplanationResult:
        ...
        try:
            ...
            # Get confidence prediction if not provided
            ...
            # Create explanation result
            explanation = ExplanationResult(
                model_name="Analyst_ConfidencePrediction",
                prediction=confidence_prediction,
                feature_names=feature_names,
                feature_values=features,
                shap_values=shap_values,
                lime_explanation=lime_explanation,
                ...
                metadata={
                    ...
                    'feature_groups': self.feature_groups,
                    'explanation_type': 'confidence_prediction'
                }
            )
    ...
    async def explain_prediction(
        ...
        try:
            ...
            if model_type == 'regime_classifier':
                ...
                return await self.explain_location_classification(
                    model, pd.DataFrame(), features, feature_names, prediction
                )
            elif model_type == 'ensemble_prediction':
                return await self.explain_ensemble_prediction(
                    model, pd.DataFrame(), features, feature_names, prediction
                )
            elif model_type == 'confidence_prediction':
                return await self.explain_confidence_prediction(
                    model, pd.DataFrame(), features, feature_names, prediction
                )
            else:
                # Generic explanation
                return await self._explain_generic_analyst_model(
    ...
    async def _explain_generic_analyst_model(
        self,
        model: Any,
        features: np.ndarray,
        feature_names: List[str],
    ...
    def _calculate_ensemble_shap_values(
        self,
        analyst_model: Any,
        features: np.ndarray
        ...
        try:
            ...
            # Try to get SHAP values for each component model
            ...
            # If no component models, use the main model
    ...
    def _extract_ensemble_feature_importance(self, analyst_model: Any) -> Optional[Dict[str, Dict[str, float]]]:
        ...
        try:
            ...
            # Extract from regime classifier
            if hasattr(analyst_model, 'regime_classifier') and analyst_model.regime_classifier is not None:
            ...
            # Extract from location classifier
            if hasattr(analyst_model, 'location_classifier') and analyst_model.location_classifier is not None:
    ...
    def _calculate_location_confidence(
        ...
        analyst_model: Any,
        features: np.ndarray,
        location_prediction: Union[str, int]
    ) -> float:
        """Calculate confidence for location classification."""
        try:
            confidence = 0.5  # Base confidence
            
            # Try to get location probabilities
            if hasattr(analyst_model, 'predict_proba'):
                probas = analyst_model.predict_proba(features.reshape(1, -1))[0]
    ...
    def _calculate_ensemble_confidence(
        ...
        shap_values: Optional[Dict[str, np.ndarray]]
    ) -> float:
        ...
        try:
            ...
            if isinstance(ensemble_prediction, dict) and 'regime' in ensemble_prediction and 'location' in ensemble_prediction:
                # Both predictions available - higher confidence
                confidence += 0.1
            
            # Adjust based on SHAP values consistency across ensemble
            if isinstance(shap_values, dict) and len(shap_values) > 1:
                # Calculate consistency across ensemble components
    ...
    def _calculate_confidence_confidence(
        self,
        ...
        try:
            ...
            # Adjust based on confidence prediction magnitude
            if isinstance(confidence_prediction, (int, float)):
                # Higher confidence predictions indicate more certainty
            ...
            # Adjust based on SHAP values consistency
    ...
    def _calculate_ensemble_shap_consistency(self, shap_values: Dict[str, np.ndarray]) -> float:
        ...
        try:
            ...
            # Calculate correlation between SHAP values of different components
