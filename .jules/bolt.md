...
## 2026-07-08 - [Live Replay Table] Learning: Iterating over DataFrame rows sequentially using `iterrows()` to construct the inference replay table (`_base_replay_table`) or to handle OOS fallback joins (`_join_oos`) is extremely slow. Action: Replace row-wise iteration with fully vectorized operations using Pandas `combine_first()`, `.where()`, and `pd.merge_asof()`. Ensure that conditionally gated math operations (like return calculation) explicitly use `.loc[valid_mask]` assignments to avoid division-by-zero or NaNs.
