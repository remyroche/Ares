...
class EnsembleCreator:
    ...
    @handles_errors(exceptions=(Exception,), default_return={}, context='ensemble creation')
    async def create_ensembles(self, enhanced_models: Dict[str, Dict[str, Any]], regime_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for regime_id, regime_models in enhanced_models.items():
            if len(regime_models) < self.min_models_for_ensemble:
                self.logger.warning(f'Regime {regime_id} has only {len(regime_models)} models, skipping ensemble creation')
                continue
            regime_ensembles = await self._create_regime_ensembles(regime_id, regime_models, regime_data.get(regime_id, {}))
            ensemble_models[regime_id] = regime_ensembles
        if len(enhanced_models) > 1:
            cross_regime_ensemble = await self._create_cross_regime_ensemble(enhanced_models, regime_data)
            ensemble_models['cross_regime'] = cross_regime_ensemble
        self.logger.info(f'Created {len(ensemble_models)} ensemble models')
        return ensemble_models

    async def _create_regime_ensembles(self, regime_id: str, regime_models: Dict[str, Any], regime_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'voting' in self.ensemble_methods:
            voting_ensemble = await self._create_voting_ensemble(base_models, regime_id)
            if voting_ensemble is not None:
                ensembles['voting'] = {'model': voting_ensemble, 'base_models': model_names, 'method': 'voting', 'voting_type': self.voting_type}
        if 'stacking' in self.ensemble_methods:
            stacking_ensemble = await self._create_stacking_ensemble(base_models, regime_id)
            if stacking_ensemble is not None:
                ensembles['stacking'] = {'model': stacking_ensemble, 'base_models': model_names, 'method': 'stacking', 'meta_learner': self.meta_learner}
        if regime_models:
            weighted_ensemble = await self._create_weighted_ensemble(regime_models, regime_id)
    ...
    async def _create_stacking_ensemble(self, base_models: List[tuple], regime_id: str) -> Optional[StackingClassifier]:
    ...
    async def _create_weighted_ensemble(self, regime_models: Dict[str, Any], regime_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            weighted_ensemble = {'models': [(name, model) for name, model, _ in models_with_scores], 'weights': weights.tolist(), 'method': 'weighted', 'base_scores': scores.tolist()}
            self.logger.info(f'Created weighted ensemble with weights: {dict(zip([m[0] for m in models_with_scores], weights))}')
            return weighted_ensemble
        except Exception as e:
            self.logger.error(f'Failed to create weighted ensemble: {str(e)}')
            return None

    async def _create_cross_regime_ensemble(self, enhanced_models: Dict[str, Dict[str, Any]], regime_data: Dict[str, Any]) -> Dict[str, Any]:
