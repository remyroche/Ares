...
def main() -> None:
    ...
    parser.add_argument(
        "--reset-to-sealed-base", action="store_true",
        help=(
            "Start a new same-day calibration carry from the hash-sealed base "
            "ledger only. Used only when a successor explicitly rebinds the "
            "parent-policy identity; no current labels are appended."
        ),
    )
    ...
    if args.reset_to_sealed_base:
        if args.intraday_frozen_ledger is not None or args.current_policy_labels is not None:
            raise ValueError("base reset cannot be combined with intraday or current labels")
        base["policy_label_available_ts"] = pd.to_datetime(
            base["policy_label_available_ts"], utc=True, errors="raise",
        )
        if not base["policy_label_available_ts"].lt(day).all():
            raise AssertionError("sealed base contains labels unavailable at reset cutoff")
        args.out_dir.mkdir(parents=True)
        output_path = args.out_dir / "walkforward_scored_label_ledger.parquet"
        shutil.copyfile(args.base_resolved_ledger, output_path)
        if _sha(output_path) != _sha(args.base_resolved_ledger):
            raise AssertionError("base-reset ledger copy is not byte exact")
        manifest = {
            ...
            "rows": int(len(base)),
            "max_label_available_ts": base["policy_label_available_ts"].max().isoformat(),
            "strictly_prior_to_utc_day": True,
            "current_decision_ids_appended": 0,
            "intraday_state_frozen": True,
            "intraday_source_decision_ts": None,
            "intraday_source_ledger_sha256": None,
            "calibration_reset_to_sealed_base": True,
            "policy_json": str(args.policy_json),
            "policy_json_sha256": _sha(args.policy_json),
            "sources": {"sealed_base_resolved_ledger": _sha(args.base_resolved_ledger)},
