...
class RegimeFeatureIntegration(VectorizedFeatureGenerator):
    ...
    def generate_features(self, data: pd.DataFrame, **kwargs) -> Dict[str, np.ndarray]:
        ...
        try:
            ...
            enhanced_features = self._generate_enhanced_regime_features(data)
            ...
            tprint(f"📊 Total features after enhancement: {len(features)}", "INFO")
...
class AnalystRegimeStabilityGenerator(VectorizedFeatureGenerator):
    ...
    def _generate_enhanced_regime_features(self, data: pd.DataFrame) -> Dict[str, np.ndarray]:
        """
        Generate enhanced regime-specific features for better HDBSCAN clustering.
        ...
        try:
            ...
            features.update(self._generate_regime_transition_features(data))
            
            # Regime persistence features
            features.update(self._generate_regime_persistence_features(data))
            
            # Economic regime features
            features.update(self._generate_economic_regime_features(data))
            
            # Volatility regime features
            features.update(self._generate_volatility_regime_features(data))
            
            # Volume regime features
            features.update(self._generate_volume_regime_features(data))
            
        except Exception as e:
            tprint(f"⚠️ Enhanced regime feature generation failed: {e}")
        
        return features
    
    def _generate_regime_transition_features(self, data: pd.DataFrame) -> Dict[str, np.ndarray]:
        """Generate regime transition detection features."""
        features = {}
        ...
        if 'close' in data.columns:
            ...
            features['price_regime_transition'] = self._detect_price_regime_transitions(close_prices, window, transition_threshold)
            
            # 2. Volatility regime transitions
            returns = np.diff(np.log(close_prices))
            features['volatility_regime_transition'] = self._detect_volatility_regime_transitions(returns, window, transition_threshold)
            
            # 3. Trend regime transitions
            features['trend_regime_transition'] = self._detect_trend_regime_transitions(close_prices, window, transition_threshold)
            
            # 4. Regime change frequency
            features['regime_change_frequency'] = self._calculate_regime_change_frequency(close_prices, window)
            
            # 5. Regime transition strength
            features['regime_transition_strength'] = self._calculate_regime_transition_strength(close_prices, window)
    ...
    def _generate_regime_persistence_features(self, data: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        if 'close' in data.columns:
            ...
            features['regime_persistence_score'] = self._calculate_regime_persistence(close_prices, window)
            
            # 2. Regime stability index
            features['regime_stability_index'] = self._calculate_regime_stability(close_prices, window)
            
            # 3. Regime duration features
            features['avg_regime_duration'] = self._calculate_avg_regime_duration(close_prices, window)
            features['max_regime_duration'] = self._calculate_max_regime_duration(close_prices, window)
            
            # 4. Regime consistency
            features['regime_consistency'] = self._calculate_regime_consistency(close_prices, window)
        
        return features
    
    def _generate_economic_regime_features(self, data: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        if 'close' in data.columns:
            ...
            features['economic_regime'] = self._classify_economic_regime(returns, window)
            
            # 2. Economic regime strength
            features['economic_regime_strength'] = self._calculate_economic_regime_strength(returns, window)
            
            # 3. Economic regime transitions
            features['economic_regime_transitions'] = self._detect_economic_regime_transitions(returns, window)
            
            # 4. Economic regime persistence
            features['economic_regime_persistence'] = self._calculate_economic_regime_persistence(returns, window)
        
        return features
    
    def _generate_volatility_regime_features(self, data: pd.DataFrame) -> Dict[str, np.ndarray]:
        """Generate volatility regime features."""
        features = {}
        ...
        if 'close' in data.columns:
            ...
            features['volatility_regime'] = self._classify_volatility_regime(returns, window)
            
            # 2. Volatility clustering strength
            features['volatility_clustering_strength'] = self._calculate_volatility_clustering_strength(returns, window)
            
            # 3. Volatility regime transitions
            features['volatility_regime_transitions'] = self._detect_volatility_regime_transitions(returns, window)
            
            # 4. Volatility regime persistence
            features['volatility_regime_persistence'] = self._calculate_volatility_regime_persistence(returns, window)
        
        return features
    
    def _generate_volume_regime_features(self, data: pd.DataFrame) -> Dict[str, np.ndarray]:
        """Generate volume regime features."""
        features = {}
        ...
        if 'volume' in data.columns and 'close' in data.columns:
            ...
            features['volume_regime'] = self._classify_volume_regime(volume, window)
            
            # 2. Volume-price relationship regime
            features['volume_price_regime'] = self._classify_volume_price_regime(volume, close_prices, window)
            
            # 3. Volume regime transitions
            features['volume_regime_transitions'] = self._detect_volume_regime_transitions(volume, window)
            
            # 4. Volume regime persistence
            features['volume_regime_persistence'] = self._calculate_volume_regime_persistence(volume, window)
        
        return features
    
    # Helper methods for regime detection
    def _detect_price_regime_transitions(self, prices: np.ndarray, window: int, threshold: float) -> np.ndarray:
        ...
        for i in range(window, len(prices)):
            ...
            std_change = abs(np.std(current_window) - recent_std) / (recent_std + 1e-8)
    ...
    def _detect_volatility_regime_transitions(self, returns: np.ndarray, window: int, threshold: float) -> np.ndarray:
    ...
    def _detect_trend_regime_transitions(self, prices: np.ndarray, window: int, threshold: float) -> np.ndarray:
    ...
    def _calculate_regime_change_frequency(self, prices: np.ndarray, window: int) -> np.ndarray:
        """Calculate regime change frequency."""
        frequency = np.zeros(len(prices))
        
        for i in range(window, len(prices)):
            recent_transitions = self._detect_price_regime_transitions(prices[i-window:i], window//2, 0.1)
            frequency[i] = np.sum(recent_transitions) / window
        
        return frequency
    
    def _calculate_regime_transition_strength(self, prices: np.ndarray, window: int) -> np.ndarray:
    ...
    def _calculate_regime_stability(self, prices: np.ndarray, window: int) -> np.ndarray:
    ...
    def _calculate_avg_regime_duration(self, prices: np.ndarray, window: int) -> np.ndarray:
    ...
    def _calculate_max_regime_duration(self, prices: np.ndarray, window: int) -> np.ndarray:
    ...
    def _classify_economic_regime(self, returns: np.ndarray, window: int) -> np.ndarray:
    ...
    def _calculate_economic_regime_strength(self, returns: np.ndarray, window: int) -> np.ndarray:
    ...
    def _detect_economic_regime_transitions(self, returns: np.ndarray, window: int) -> np.ndarray:
        """Detect economic regime transitions."""
        transitions = np.zeros(len(returns) + 1)
        
        for i in range(window, len(returns)):
            recent_regime = self._classify_economic_regime(returns[i-window:i], window//2)
            current_regime = self._classify_economic_regime(returns[i-window//2:i], window//2)
    ...
    def _calculate_economic_regime_persistence(self, returns: np.ndarray, window: int) -> np.ndarray:
        """Calculate economic regime persistence."""
        persistence = np.zeros(len(returns) + 1)
        
        for i in range(window, len(returns)):
            recent_returns = returns[i-window:i]
            regimes = self._classify_economic_regime(recent_returns, window//2)
    ...
    def _calculate_volatility_clustering_strength(self, returns: np.ndarray, window: int) -> np.ndarray:
    ...
    def _detect_volatility_regime_transitions(self, returns: np.ndarray, window: int) -> np.ndarray:
        """Detect volatility regime transitions."""
        transitions = np.zeros(len(returns) + 1)
    ...
    def _calculate_volatility_regime_persistence(self, returns: np.ndarray, window: int) -> np.ndarray:
        ...
        for i in range(window, len(returns)):
            recent_returns = returns[i-window:i]
            regimes = self._classify_volatility_regime(recent_returns, window//2)
    ...
    def _classify_volume_regime(self, volume: np.ndarray, window: int) -> np.ndarray:
    ...
    def _classify_volume_price_regime(self, volume: np.ndarray, prices: np.ndarray, window: int) -> np.ndarray:
    ...
    def _detect_volume_regime_transitions(self, volume: np.ndarray, window: int) -> np.ndarray:
    ...
    def _calculate_volume_regime_persistence(self, volume: np.ndarray, window: int) -> np.ndarray:
        """Calculate volume regime persistence."""
        persistence = np.zeros(len(volume))
        
        for i in range(window, len(volume)):
            recent_volume = volume[i-window:i]
            regimes = self._classify_volume_regime(recent_volume, window//2)
