...
class RegimePredictiveEnsembles:
    ...
    def __init__(self, config):
        ...
        self.global_meta_learner_path = os.path.join(
            self.model_storage_dir, "global_meta_learner.joblib"
        )
        self.global_meta_scaler_path = os.path.join(
            self.model_storage_dir, "global_meta_scaler.joblib"
        )
        self.global_meta_label_encoder_path = os.path.join(
            self.model_storage_dir, "global_meta_label_encoder.joblib"
        )
    ...
    def train_all_models(
        ...
        for regime_key, ensemble_instance in self.regime_ensembles.items():
            ...
            historical_features = regime_data.drop(
                columns=["target", "Market_Regime_Label"], errors="ignore"
            )
            ...
            if model_path_prefix:
                ...
                full_model_path = os.path.join(
                    self.model_storage_dir, f"final_{model_file_name}"
                )
            ...
            if os.path.exists(full_model_path):
                ...
                if ensemble_instance.load_model(full_model_path):
                    ...
                    ensemble_instance.train_ensemble(
                        historical_features, historical_targets
                    )
            else:
    ...
    def get_all_predictions(
        self, asset: str, current_features: pd.DataFrame, **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    def _train_global_meta_learner(self, meta_learner_raw_data: List[Dict[str, Any]]):
        ...
        self.global_meta_learner = LGBMClassifier(
            **self.global_meta_config, random_state=42
        )
    ...
    def _predict_with_global_meta_learner(
        ...
        meta_input_df = pd.get_dummies(
            meta_input_df, columns=["regime"], prefix="regime"
        )
    ...
    def _save_global_meta_learner(self):
        ...
        try:
            ...
            self.logger.error(
                f"Error saving global meta-learner components: {e}", exc_info=True
            )
    ...
    def _load_global_meta_learner(self):
        ...
        if (
            ...
            try:
                ...
                self.logger.error(
                    f"Error loading global meta-learner components: {e}", exc_info=True
                )
    ...
    def save_model(self, ensemble_instance: Any, path: str):
        ...
        try:
            ...
            self.logger.error(
                f"Error saving ensemble model to {path}: {e}", exc_info=True
            )
    ...
    def load_model(self, ensemble_instance: Any, path: str) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Error loading ensemble model from {path}: {e}", exc_info=True
            )
            return False
