...
class DynamicRegimeMapper:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.archetype_descriptions: dict[str, dict[int, str]] = {}
        self.cluster_centroids: dict[str, dict[int, list[float]]] = {}
    ...
    async def _discover_regimes_from_step1_7(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f'Discovered regimes for {len(self.regime_mappings)} timeframes')
            return True
        except Exception as e:
        ...
        try:
            ...
            for cluster_id_str, centroid in cluster_centroids.items():
                try:
                    ...
                    continue
            regime_mapping = self._generate_regime_mapping_from_archetypes(archetype_descriptions_int, cluster_centroids_int)
            ...
            self.logger.info(f'Processed {timeframe}: {len(regime_mapping)} regimes discovered')
            for cluster_id, regime_name in regime_mapping.items():
                description = archetype_descriptions_int.get(cluster_id, 'No description')
                self.logger.info(f'  Cluster {cluster_id} -> {regime_name}: {description}')
            return True
        except Exception as e:
            self.logger.exception(f'Error processing meta file {meta_file}: {e}')
            return False

    def _generate_regime_mapping_from_archetypes(self, archetype_descriptions: dict[int, str], cluster_centroids: dict[int, list[float]]) -> dict[int, str]:
        """Generate regime names from archetype descriptions."""
        regime_mapping = {}
        for cluster_id, description in archetype_descriptions.items():
            regime_name = self._classify_archetype_to_regime(cluster_id, description, cluster_centroids.get(cluster_id, []))
            regime_mapping[cluster_id] = regime_name
        return regime_mapping

    def _classify_archetype_to_regime(self, cluster_id: int, description: str, centroid: list[float]) -> str:
        ...
        if 'limited liquidity' in description_lower or 'low liquidity' in description_lower:
            return 'LOW_LIQUIDITY_REGIME'
        if 'abundant liquidity' in description_lower or 'high liquidity' in description_lower:
    ...
    def get_regime_clusters(self, regime_name: str, timeframe: str='1m') -> list[int]:
        ...
        mapping = self.get_regime_mapping(timeframe)
        return [cluster_id for cluster_id, name in mapping.items() if name == regime_name]
    ...
    def get_regime_summary(self, timeframe: str='1m') -> dict[str, Any]:
        ...
        for cluster_id, regime_name in mapping.items():
            if regime_name not in regime_summary:
                regime_summary[regime_name] = {'clusters': [], 'descriptions': [], 'cluster_count': 0}
            regime_summary[regime_name]['clusters'].append(cluster_id)
            regime_summary[regime_name]['descriptions'].append(descriptions.get(cluster_id, f'Cluster {cluster_id}'))
    ...
    def load_regime_mapping(self, input_path: str) -> bool:
        ...
        try:
            ...
            self.archetype_descriptions[timeframe] = mapping_data.get('archetype_descriptions', {})
