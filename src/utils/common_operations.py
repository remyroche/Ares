...
This module provides commonly used operations that were identified as undefined in the
codebase analysis. It serves as a central location for these utilities.
"""
...
def list_files(
    directory: Union[str, Path], pattern: str = "*", suffix: Optional[str] = None
) -> List[Path]:
...
def validate_data_quality(
    ...
    if not high_nan_cols.empty:
        quality_report["issues"].append(
            {"type": "high_nan_ratio", "columns": high_nan_cols.to_dict()}
        )
...
def safe_resample(
    df: pd.DataFrame, rule: str, agg_dict: Optional[Dict[str, str]] = None
) -> pd.DataFrame:
...
def safe_deque(
    items: Optional[List[Any]] = None, maxlen: Optional[int] = None
) -> deque:
