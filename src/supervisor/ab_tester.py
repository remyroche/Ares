...
class ABTester:
    ...
    async def evaluate_and_conclude_ab_test(self, champion_trade_logs):
        ...
        self.logger.info("--- CONCLUDING A/B TEST & GENERATING REPORT ---")
        self.ab_test_active = False
        
        # --- Performance Calculation ---
        # NOTE: Assumes the reporter's generate_performance_report is updated to return Sharpe Ratio
        ...
        champion_sharpe = champion_report['daily_summary'].get('Sharpe Ratio', 0.0) # Get Sharpe, default to 0
        ...
        challenger_sharpe = challenger_report['daily_summary'].get('Sharpe Ratio', 0.0) # Get Sharpe, default to 0

        # --- Parameter Difference Highlighting ---
        param_diffs = {}
        for key, challenger_value in self.challenger_params.items():
            champion_value = self.champion_params_snapshot.get(key)
