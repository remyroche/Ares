...
## 2024-05-18 - Replacing iterrows with Vectorized Updates
Learning: Iterating over DataFrame rows with `iterrows()` to manually construct and assign rows using `.loc` mappings is extremely slow. This was the case in `PredictionLedger.mark_resolved` where doing element-wise assignments on a dictionary mapping caused substantial delays for relatively small (500 row) updates.
Action: Replace `.iterrows()` with native vectorization using `.loc[match_idx]` array assignments and `pd.concat` for adding new index records. When performing such merges, always ensure deduplication is performed with `~upd_idx.index.duplicated(keep="last")` to prevent 'cannot reindex from duplicate axis' errors, and be mindful of explicitly casting NaN when schema changes introduce new columns so `float64` remains compatible.
