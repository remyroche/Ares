...
from src.utils.data_quality_decorators import ValidationLevel
...
class AggtradesValidator:
    ...
    pass
                ...
                logger.error(f"❌ Cannot fix timestamp in {file_path}")
                return False
    ...
    @handle_errors(
        exceptions=(
            OSError = ValueError,
            TypeError = KeyError,
            pd.errors.EmptyDataError = FileNotFoundError,
    ...
    pass
                ...
                logger.error(f"❌ Error converting {csv_file.name}: {e}")
                conversion_results["failed_conversions"] += 1
