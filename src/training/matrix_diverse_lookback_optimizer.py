...
class MatrixDiverseLookbackOptimizer:
    ...
    async def find_diverse_lookback_periods_matrix(
        ...
        """
        self.logger.info(
            f"🎯 Finding diverse lookback periods for {symbol} on {exchange}"
        )
        ...
        matrix_results = await self._analyze_matrix_optimization(
            data, target, diverse_periods
        )
        ...
        self.logger.info(
            "✅ Matrix-based diverse lookback period optimization completed"
        )
        return results
    ...
    async def _matrix_optimize_diverse_periods(
        ...
        for feature_name, lookback_config in self.matrix_config[
            "lookback_ranges"
        ].items():
    ...
    async def _matrix_optimize_feature_periods(
        ...
        info_scores = await self._calculate_vectorized_info_scores(
        ...
        diversity_metrics = self._calculate_matrix_diversity_metrics(
            selected_features,
            correlation_matrix[selected_indices][:, selected_indices],
        )
        ...
        return {
            ...
            "period_scores": [
                {
                    "period": periods[i],
                    "information_score": info_scores[i],
            ...
            "all_period_scores": [
                ...
                for p, s in zip(periods, info_scores, strict=False)
            ],
    ...
    def _calculate_feature_matrix(
        ...
        for i, period in enumerate(periods):
            feature_values = self._calculate_feature_with_period(
    ...
    def _matrix_optimize_period_selection(
        ...
        if np.sum(meaningful_mask) < self.matrix_config["min_periods_per_feature"]:
            ...
            top_indices = np.argsort(info_scores)[
                -self.matrix_config["min_periods_per_feature"] :
            ]
        ...
        meaningful_correlations = correlation_matrix[meaningful_mask][
            :, meaningful_mask
        ]
        ...
        if target_count == 3 and len(meaningful_indices) >= 3:
            ...
            if self._check_quality_thresholds(
                selected_indices, meaningful_scores, meaningful_correlations
            ):
                return selected_indices
            self.logger.info(
                "   ⚠️ 3-period solution doesn't meet quality thresholds, trying 2 periods"
            )

        # Fallback to 2 periods
        target_count = 2
        return self._try_2_period_optimization(
            meaningful_scores,
            meaningful_correlations,
            meaningful_indices,
        )
    ...
    def _check_quality_thresholds(
        ...
        if len(selected_indices) >= 2:
            selected_corr = meaningful_correlations[selected_indices][
                :, selected_indices
            ]
        ...
        diversity_score = self._calculate_diversity_score(
            selected_indices, meaningful_correlations
        )
    ...
    def _optuna_matrix_optimization(
        ...
        def objective(trial):
            ...
            selected_indices = trial.suggest_categorical(
                ...
                [
                    ...
                    for combo in itertools.combinations(
                        range(len(info_scores)), target_count
                    )
                ],
            )
            ...
            selected_correlations = correlation_matrix[selected_indices][
                :, selected_indices
            ]
    ...
    def _greedy_matrix_optimization(
        ...
        while len(selected_indices) < target_count:
            ...
            for i in range(len(info_scores)):
                ...
                # Diversity component
                candidate_correlations = correlation_matrix[candidate_set][
                    :, candidate_set
                ]
    ...
    async def _save_matrix_optimization_results(
        ...
        main_filename = (
            f"{exchange}_{symbol}_{timeframe}_matrix_diverse_lookback_periods.json"
        )
        ...
        summary_filename = (
            f"{exchange}_{symbol}_{timeframe}_diverse_periods_summary.json"
        )
        ...
        matrix_filename = (
            f"{exchange}_{symbol}_{timeframe}_matrix_optimization_details.json"
        )
    ...
    def _generate_optimized_feature_parameters(
        ...
        for feature_name, feature_data in diverse_periods.items():
            ...
            for period in selected_periods:
                ...
                elif feature_name in ["SMA_short", "SMA_long"] or feature_name in [
                    "EMA_short",
                    "EMA_long",
                ]:
                    param = {
    ...
    async def _save_optimized_parameters(
        ...
        params_filename = (
            f"{exchange}_{symbol}_{timeframe}_optimized_feature_parameters.json"
        )
        ...
        self.logger.info(
            f"💾 Saved optimized parameters to: {params_filepath.absolute()}"
        )
        ...
        self.logger.info(
            f"💾 Saved step parameters to: {step_params_filepath.absolute()}"
        )
    ...
    def get_optimized_feature_parameters(
        ...
        step_params_filepath = Path(
            f"data/optimized_feature_parameters/{exchange}_{symbol}_{timeframe}_optimized_feature_parameters.json"
        )

        if not step_params_filepath.exists():
            # Try main optimization directory
            main_params_filepath = Path(
                f"data/matrix_diverse_lookback_optimization/{exchange}_{symbol}_{timeframe}_optimized_feature_parameters.json"
            )

            if not main_params_filepath.exists():
                self.logger.warning(
                    f"⚠️ No optimized parameters found for {symbol} on {exchange}"
                )
                return {}
        ...
        try:
            ...
            self.logger.info(
                f"📂 Loaded optimized parameters from: {step_params_filepath.absolute()}"
            )
            return optimized_params
    ...
    def _calculate_feature_with_period(
        ...
        try:
            ...
            self.logger.warning(
                f"⚠️ Error calculating {feature_name} with period {period}: {e}"
            )
            return None
    ...
    def _calculate_keltner_channels(self, data: pd.DataFrame, period: int) -> pd.Series:
        ...
        return (data["close"] - keltner_lower) / (keltner_upper - keltner_lower)
    ...
    def _calculate_volume_rsi(self, data: pd.DataFrame, period: int) -> pd.Series:
        ...
        loss = (
            (-volume_change.where(volume_change < 0, 0)).rolling(window=period).mean()
        )
        rs = gain / loss
        return 100 - (100 / (1 + rs))
    ...
    def _calculate_volume_stochastic(
        ...
        return 100 * ((data["volume"] - volume_low) / (volume_high - volume_low))
    ...
    def _calculate_volume_price_confirmation(
        ...
        return confirmation.rolling(window=period).mean()
    ...
    def _calculate_volume_price_trend_indicator(
        ...
        return vpt.rolling(window=period).mean()
    ...
    def _calculate_volume_price_oscillator_signal(
        ...
        vpo = ((typical_price - vwap) / vwap) * 100
        return vpo.rolling(window=period // 2).mean()
    ...
    def _calculate_volume_price_oscillator_trigger(
        ...
        vpo = ((typical_price - vwap) / vwap) * 100
        return vpo.rolling(window=period // 3).mean()
    ...
    def _calculate_volume_price_oscillator_upper_band(
        ...
        vpo = ((typical_price - vwap) / vwap) * 100
        return vpo.rolling(window=period).std() * 2
    ...
    def _calculate_vwap_acceleration(
        ...
        return vwap_momentum - vwap_momentum.shift(period)
    ...
    def _calculate_vwap_volatility(self, data: pd.DataFrame, period: int) -> pd.Series:
        ...
        return vwap_returns.rolling(window=period).std()
    ...
    def _calculate_vwap_momentum_volatility(
        ...
        vwap_momentum = self._calculate_vwap_momentum(data, period)
        return vwap_momentum.rolling(window=period).std()
    ...
    async def _analyze_matrix_optimization(
        ...
        analysis["performance_metrics"] = {
            "total_periods_tested": total_periods_tested,
            "total_periods_selected": total_periods_selected,
            "reduction_ratio": (
                total_periods_selected / total_periods_tested
                if total_periods_tested > 0
                else 0.0
            ),
            "avg_diversity_score": avg_diversity_score,
            "n_features_optimized": n_features,
        }
