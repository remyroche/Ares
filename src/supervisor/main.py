...
class Supervisor:
    ...
    async def orchestrate_supervision(self, current_date: datetime.date, total_equity: float, 
        ...
        if self.ab_tester.ab_test_active and (datetime.datetime.now() - self.ab_tester.ab_test_start_date).days >= 7:
            self.ab_tester.evaluate_and_conclude_ab_test(daily_trade_logs)
