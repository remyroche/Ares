...
def requires_permission(*permissions: str, require_all: bool=False) -> Callable[[Callable[P, R]], Callable[P, R]]:
    ...
    def sync_handler(func: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
        ...
        if not user:
            msg = 'Authentication required'
            raise AuthenticationError(msg)
        has_required_perms = user.has_all_permissions(list(permissions)) if require_all else user.has_any_permission(list(permissions))
        if not has_required_perms:
            msg = f"Missing required permission(s): {', '.join(permissions)}"
            raise AuthorizationError(msg, required_permission=','.join(permissions), details={'required_permissions': list(permissions), 'user_permissions': list(user.permissions), 'require_all': require_all})
        return func(*args, **kwargs)
...
def owner_only(owner_field: str='user_id', param_name: str=None) -> Callable[[Callable[P, R]], Callable[P, R]]:
    ...
    def sync_handler(func: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
        ...
        if owner_id != user.id:
            ...
            raise AuthorizationError(msg, details={'owner_field': owner_field, 'owner_id': owner_id, 'user_id': user.id})
        return func(*args, **kwargs)
