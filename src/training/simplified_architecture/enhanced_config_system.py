...
class ConfigurationManager:
    ...
    def __init__(self, config_dir: Optional[Path]=None, logger: Optional[logging.Logger]=None) -> None:
    ...
    def create_config_template(self, template_name: str, output_path: Union[str, Path]) -> None:
        ...
        templates = {'basic_ml_pipeline': self._create_basic_ml_template(), 'advanced_trading_pipeline': self._create_advanced_trading_template(), 'hmm_regime_pipeline': self._create_hmm_regime_template(), 'ensemble_training_pipeline': self._create_ensemble_training_template()}
    ...
    def get_available_templates(self) -> List[str]:
        ...
        return ['basic_ml_pipeline', 'advanced_trading_pipeline', 'hmm_regime_pipeline', 'ensemble_training_pipeline']
    ...
    def _serialize_configuration(self, config: PipelineConfiguration) -> Dict[str, Any]:
        ...
        return {'name': config.name, 'version': config.version, 'description': config.description, 'environment': config.environment.value, 'global_settings': {'data_source': config.global_settings.data_source, 'model': config.global_settings.model, 'logging': config.global_settings.logging, 'performance': config.global_settings.performance, 'security': config.global_settings.security, 'monitoring': config.global_settings.monitoring}, 'steps': [{'name': step.name, 'class_name': step.class_name, 'enabled': step.enabled, 'timeout_seconds': step.timeout_seconds, 'retry_count': step.retry_count, 'retry_delay_seconds': step.retry_delay_seconds, 'fail_fast': step.fail_fast, 'priority': step.priority, 'parameters': step.parameters, 'dependencies': step.dependencies, 'output_schema': step.output_schema, 'validation_rules': step.validation_rules, 'resource_limits': step.resource_limits, 'metadata': step.metadata} for step in config.steps], 'metadata': config.metadata, 'created_at': config.created_at.isoformat() if config.created_at else None, 'updated_at': config.updated_at.isoformat() if config.updated_at else None}
