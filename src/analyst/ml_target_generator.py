...
class MLTargetGenerator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.stop_loss_threshold = self.target_config.get("stop_loss_threshold", -0.01)  # -1%
        self.take_profit_threshold = self.target_config.get("take_profit_threshold", 0.03)  # 3%
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.info("✅ ML Target Generator initialization completed successfully")
            return True
    ...
    def _generate_basic_targets(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            if 'close' not in data.columns:
                self.logger.warning("No 'close' column found, skipping basic target generation")
                return data
            ...
            data.loc[data['price_change'] > self.take_profit_threshold, 'target_multi'] = 1  # Buy
            data.loc[data['price_change'] < self.stop_loss_threshold, 'target_multi'] = -1  # Sell
    ...
    def _generate_advanced_targets(self, data: pd.DataFrame, current_price: float) -> pd.DataFrame:
        """
        ...
        try:
            if 'close' not in data.columns:
                self.logger.warning("No 'close' column found, skipping advanced target generation")
                return data
            ...
            data['dynamic_tp_threshold'] = volatility * 2  # 2x volatility for take profit
            data['dynamic_sl_threshold'] = -volatility * 1.5  # 1.5x volatility for stop loss
            ...
            data.loc[volatility > volatility.quantile(0.75), 'volatility_regime'] = 1  # High
            data.loc[volatility < volatility.quantile(0.25), 'volatility_regime'] = -1  # Low
    ...
    def get_target_statistics(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'target' in data.columns:
                stats['binary_target_distribution'] = data['target'].value_counts().to_dict()
                stats['binary_target_ratio'] = data['target'].mean()
            
            if 'target_multi' in data.columns:
                stats['multi_target_distribution'] = data['target_multi'].value_counts().to_dict()
            ...
            if 'volatility_regime' in data.columns:
                stats['volatility_regime_distribution'] = data['volatility_regime'].value_counts().to_dict()
