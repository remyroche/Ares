...
try:
    ...
    try:
        from src.utils.decorators.errors import handles_errors as _handles_errors
    except Exception:

        def _handles_errors(*_args, **_kwargs) -> None:

            def _dec(fn: Any) -> None:
                return fn
            return _dec
...
class AdvancedFeatureSelectionStep(BaseStep):
    ...
    @_handles_errors(exceptions=(Exception,), default_return={'success': False}, context='advanced feature selection execution')
