#!/usr/bin/env python3
"""Run fixed-contract meta recency-HPO for the top loc_ema_stack head."""
...
def main() -> int:
    ...
    log_path = LOG_DIR / f"recency_hpo_meta_top3_{STRATEGY_ID}_{run_id}.log"
    with log_path.open("ab", buffering=0) as log_fp:
        log_fp.write(b"\n=== START recency_hpo_meta_loc_ema_stack ===\n")
        log_fp.write(("RUN_ID " + run_id + "\n").encode())
        log_fp.write(("STRATEGY_ID " + STRATEGY_ID + "\n").encode())
        log_fp.write(("GRID_PAIRS " + TOP3_GRID_PAIRS + "\n").encode())
        log_fp.write(("WINNER_PATH " + env["EPM_RECENCY_HPO_META_WINNER_PATH"] + "\n").encode())
        ...
        ret = proc.wait()
        log_fp.write(f"\n=== END recency_hpo_meta_loc_ema_stack ret={ret} ===\n".encode())
    return int(ret)
