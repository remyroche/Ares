...
class DiverseLookbackOptimizer:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.diverse_config = config.get('diverse_lookback_optimization', {'target_periods_per_feature': 3, 'min_periods_per_feature': 2, 'max_periods_per_feature': 3, 'diversity_threshold': 0.3, 'meaningful_threshold': 0.1, 'correlation_threshold': 0.7, 'information_diversity_weight': 0.4, 'signal_strength_weight': 0.4, 'correlation_penalty_weight': 0.2, 'lookback_ranges': {'RSI': {'min': 5, 'max': 50, 'step': 2}, 'MACD_fast': {'min': 5, 'max': 25, 'step': 1}, 'MACD_slow': {'min': 20, 'max': 40, 'step': 2}, 'Bollinger_Bands': {'min': 10, 'max': 50, 'step': 2}, 'SMA_short': {'min': 3, 'max': 20, 'step': 1}, 'SMA_long': {'min': 20, 'max': 100, 'step': 5}, 'EMA_short': {'min': 3, 'max': 20, 'step': 1}, 'EMA_long': {'min': 20, 'max': 100, 'step': 5}, 'ATR': {'min': 5, 'max': 30, 'step': 1}, 'Stochastic_k': {'min': 5, 'max': 30, 'step': 1}, 'Stochastic_d': {'min': 3, 'max': 10, 'step': 1}, 'ADX': {'min': 5, 'max': 30, 'step': 1}, 'CCI': {'min': 5, 'max': 30, 'step': 1}}})
        self.logger.info('🎯 Diverse Lookback Optimizer initialized')

    @handles_errors(fallback={})
    async def find_diverse_lookback_periods(self, data: pd.DataFrame, target: pd.Series, regimes: pd.Series | None=None, symbol: str='UNKNOWN', exchange: str='UNKNOWN', timeframe: str='1m') -> dict[str, Any]:
        """
        ...
        results = {'optimization_timestamp': datetime.now().isoformat(), 'symbol': symbol, 'exchange': exchange, 'timeframe': timeframe, 'diverse_lookback_periods': {}, 'diversity_analysis': {}, 'information_content_analysis': {}, 'regime_specific_periods': {}}
        ...
        if regimes is not None and len(regimes.unique()) > 1:
            ...
            regime_periods = await self._find_regime_specific_diverse_periods(data, target, regimes, diverse_periods)
    ...
    async def _find_diverse_periods_for_all_features(self, data: pd.DataFrame, target: pd.Series) -> dict[str, Any]:
        ...
        for feature_name, lookback_config in self.diverse_config['lookback_ranges'].items():
            ...
            feature_periods = await self._find_diverse_periods_for_feature(data, target, feature_name, periods)
            diverse_periods[feature_name] = feature_periods
        return diverse_periods

    async def _find_diverse_periods_for_feature(self, data: pd.DataFrame, target: pd.Series, feature_name: str, periods: list[int]) -> dict[str, Any]:
        ...
        for period in periods:
            ...
            if feature_values is not None:
                ...
                period_scores.append({'period': period, 'feature_values': feature_values, 'information_score': info_score})
        meaningful_periods = [score for score in period_scores if score['information_score'] >= self.diverse_config['meaningful_threshold']]
        ...
        return {'selected_periods': selected_periods, 'period_scores': diverse_subset, 'diversity_metrics': diversity_metrics, 'all_period_scores': period_scores, 'meaningful_periods': len(meaningful_periods), 'total_periods_tested': len(periods)}
    ...
    def _select_diverse_subset(self, meaningful_periods: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        target_count = min(self.diverse_config['target_periods_per_feature'], len(meaningful_periods))
        if target_count == 0:
            return []
    ...
    def _calculate_set_diversity_score(self, period_set: list[dict[str, Any]]) -> float:
        ...
        return diversity_weight * avg_diversity + info_weight * avg_info_score
    ...
    def _calculate_feature_with_period(self, data: pd.DataFrame, feature_name: str, period: int) -> pd.Series | None:
        ...
        try:
            ...
            if feature_name in {'MACD_fast', 'MACD_slow'}:
            ...
            if feature_name in {'SMA_short', 'SMA_long'}:
                return self._calculate_sma(data['close'], period)
            if feature_name in {'EMA_short', 'EMA_long'}:
                return self._calculate_ema(data['close'], period)
            if feature_name == 'ATR':
                return self._calculate_atr(data, period)
            if feature_name == 'Stochastic_k':
                return self._calculate_stochastic_k(data, period)
            if feature_name == 'Stochastic_d':
    ...
    def _calculate_period_diversity_metrics(self, selected_features: dict[int, pd.Series], target: pd.Series) -> dict[str, float]:
        ...
        return {'diversity_score': diversity_score, 'avg_correlation': avg_correlation, 'n_periods': n_periods}

    async def _analyze_diversity_and_information(self, data: pd.DataFrame, target: pd.Series, diverse_periods: dict[str, Any]) -> dict[str, Any]:
        """Analyze diversity and information content across all features."""
        analysis = {'overall_diversity_score': 0.0, 'feature_diversity_scores': {}, 'information_content_summary': {}, 'correlation_analysis': {}, 'complementarity_analysis': {}}
        total_diversity = 0.0
        feature_count = 0
        for feature_name, feature_data in diverse_periods.items():
            ...
            feature_count += 1
            avg_info_score = np.mean([item['information_score'] for item in feature_data['period_scores']])
            analysis['information_content_summary'][feature_name] = {'avg_information_score': avg_info_score, 'n_periods': len(feature_data['selected_periods']), 'periods': feature_data['selected_periods']}
            if len(feature_data['selected_periods']) > 1:
                correlation_analysis = self._analyze_period_correlations(data, target, feature_name, feature_data['selected_periods'])
    ...
    def _analyze_period_correlations(self, data: pd.DataFrame, target: pd.Series, feature_name: str, periods: list[int]) -> dict[str, Any]:
        """Analyze correlations between different periods of the same feature."""
        correlations = {}
        for i, period1 in enumerate(periods):
            for _j, period2 in enumerate(periods[i + 1:], i + 1):
                ...
                if feature1 is not None and feature2 is not None:
                    ...
                    correlations[f'{period1}_vs_{period2}'] = {'correlation': correlation, 'abs_correlation': abs(correlation), 'diversity': 1.0 - abs(correlation)}
        return correlations

    async def _analyze_information_content(self, data: pd.DataFrame, target: pd.Series, diverse_periods: dict[str, Any]) -> dict[str, Any]:
        ...
        for feature_name, feature_data in diverse_periods.items():
            ...
            for period_score in feature_data['period_scores']:
                ...
                info_analysis = await self._analyze_period_information(feature_values, target, feature_name, period)
                feature_analysis[period] = info_analysis
            analysis[feature_name] = feature_analysis
        return analysis

    async def _analyze_period_information(self, feature_values: pd.Series, target: pd.Series, feature_name: str, period: int) -> dict[str, Any]:
        """Analyze information content for a specific period."""
        analysis = {'period': period, 'feature_name': feature_name, 'information_score': 0.0, 'signal_strength': 0.0, 'predictive_power': 0.0, 'market_insight': ''}
        try:
            info_score = await self._calculate_information_score(feature_values, target)
    ...
    async def _find_regime_specific_diverse_periods(self, data: pd.DataFrame, target: pd.Series, regimes: pd.Series, global_periods: dict[str, Any]) -> dict[str, Any]:
        ...
        for regime in regimes.unique():
            ...
            if len(regime_data) >= 100:
                ...
                regime_specific = await self._find_diverse_periods_for_all_features(regime_data, regime_target)
                regime_periods[f'regime_{regime}'] = regime_specific
        return regime_periods

    async def _save_diverse_lookback_results(self, results: dict[str, Any], symbol: str, exchange: str, timeframe: str) -> None:
    ...
    def get_diverse_lookback_periods(self, symbol: str, exchange: str, timeframe: str) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.exception(f'❌ Error loading diverse lookback results: {e}')
            return {}