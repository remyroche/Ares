...
## 2026-05-04 - Fast KlineRecord object instantiating from DataFrame
Learning: Looping via `iterrows()` to instantiate `KlineRecord` objects for each row of historical data is terribly slow due to Pandas iterating overhead. Using `.to_numpy()` and extracting column arrays to iterate using `zip()` drastically reduces this time.
Action: Whenever reading sequential datasets or hydrating simple dataclasses, bypass `iterrows` entirely, fetch individual column `.to_numpy()` arrays, providing `np.full()` fallbacks for missing columns, and iterate using standard Python `zip()`. Remember to preserve the inner exception catching mechanism when applying to large loops so failures in single lines do not abort the process.
