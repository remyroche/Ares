...
class OptimizedResampler:
    ...
    @staticmethod
    @njit(parallel=True)
    def _resample_ohlcv_numba(timestamps: np.ndarray, open_prices: np.ndarray, high_prices: np.ndarray, low_prices: np.ndarray, close_prices: np.ndarray, volumes: np.ndarray, bin_edges: np.ndarray) -> tuple:
    ...
    def resample_data(self, data: pd.DataFrame, source_timeframe: str, target_timeframe: str) -> pd.DataFrame:
        ...
        freq_map = {'1m': 60, '5m': 300, '15m': 900, '30m': 1800, '1h': 3600, '4h': 14400, '1d': 86400}
        bin_size = freq_map.get(target_timeframe, 300)
        bin_edges = np.arange(timestamps[0] - timestamps[0] % bin_size, timestamps[-1] + bin_size, bin_size)
        out_open, out_high, out_low, out_close, out_volume, out_count = self._resample_ohlcv_numba(timestamps, data['open'].values, data['high'].values, data['low'].values, data['close'].values, data['volume'].values, bin_edges)
        result_timestamps = pd.to_datetime(bin_edges[:-1], unit='s')
        result = pd.DataFrame({'open': out_open, 'high': out_high, 'low': out_low, 'close': out_close, 'volume': out_volume, 'count': out_count}, index=result_timestamps)
        result = result[result['count'] > 0].drop(columns=['count'])
        result = result.fillna(method='ffill')
        self.logger.info(f'Resampled {len(data)} rows to {len(result)} rows ({target_timeframe} timeframe)')
        return result

    def create_multi_timeframe_features(self, data: pd.DataFrame, base_timeframe: str, target_timeframes: List[str]) -> Dict[str, pd.DataFrame]:
