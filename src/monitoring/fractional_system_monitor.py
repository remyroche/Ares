...
class FractionalSystemMonitor:
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.alert_output_dir = Path(
            self.config.get("alert_output_dir", "data/monitoring/alerts")
        )
    ...
    def stop_monitoring(self):
        ...
        self.logger.info(
            f"⏹️ Stopped fractional system monitoring (duration: {monitoring_duration})"
        )
    ...
    def track_performance(
        ...
        try:
            ...
            self.logger.debug(
                f"📊 Performance tracked: feature_quality={metrics['feature_quality']:.3f}, "
        ...
        try:
            ...
            if not features.empty:
                ...
                for col in features.columns:
                    ...
                    if len(feature_series) > 0:
                        ...
                        non_zero_ratio = (feature_series != 0).sum() / len(
                            feature_series
                        )
                        quality_score = min(1.0, variance * 100) * non_zero_ratio
                        feature_qualities.append(quality_score)

                metrics["feature_quality"] = (
                    np.mean(feature_qualities) if feature_qualities else 0.0
                )
                metrics["feature_quality_std"] = (
                    np.std(feature_qualities) if feature_qualities else 0.0
                )
            else:
            ...
            if hmm_regime and not features.empty:
                regime_features = [
                    col
                    for col in features.columns
                    if col.startswith(f"regime_{hmm_regime}")
                ]
                if regime_features:
                    regime_qualities = []
                    ...
                    metrics["hmm_integration_quality"] = (
                        np.mean(regime_qualities) if regime_qualities else 0.0
                    )
                else:
    ...
    def _calculate_synergy_score(
        ...
        try:
            ...
            if correlations:
                ...
                return min(1.0, avg_correlation * 2)  # Scale to 0-1
            return 0.5
    ...
    def _store_metrics(self, metrics: dict[str, Any]):
        ...
        try:
            ...
            self.metrics["hmm_integration_quality"].append(
                metrics.get("hmm_integration_quality", 0.0)
            )
            ...
            recent_errors = sum(
                1
                for m in self.metrics["feature_quality"][-self.monitoring_window :]
                if m == 0.0
            )
            error_rate = recent_errors / min(
                len(self.metrics["feature_quality"]), self.monitoring_window
            )
    ...
    def _check_alerts(self, metrics: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if (
                metrics.get("feature_quality", 0.0)
                < self.alert_thresholds["feature_quality_min"]
            ):
                alerts.append(
                    {
            ...
            # Label quality alert
            if (
                metrics.get("label_quality", 0.0)
                < self.alert_thresholds["label_quality_min"]
            ):
                alerts.append(
                    {
            ...
            if (
                metrics.get("hmm_integration_quality", 0.0)
                < self.alert_thresholds["regime_quality_min"]
            ):
                alerts.append(
                    {
    ...
    def _trigger_alerts(self, alerts: list[dict[str, Any]], metrics: dict[str, Any]):
        ...
        try:
            for alert in alerts:
                ...
                self.logger.warning(
                    f"🚨 Alert triggered: {alert['type']} - {alert['message']}"
                )
    ...
    def _update_regime_performance(self, hmm_regime: str, metrics: dict[str, Any]):
        ...
        try:
            ...
            if recent_metrics:
                regime_data["avg_feature_quality"] = np.mean(
                    [m.get("feature_quality", 0.0) for m in recent_metrics]
                )
                regime_data["avg_label_quality"] = np.mean(
                    [m.get("label_quality", 0.0) for m in recent_metrics]
                )
                regime_data["avg_processing_time"] = np.mean(
                    [m.get("processing_time", 0.0) for m in recent_metrics]
                )

            # Keep only recent metrics
            if len(regime_data["metrics"]) > self.monitoring_window:
                regime_data["metrics"] = regime_data["metrics"][
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            summary["critical_alerts"] = len(
                [a for a in self.alert_history if a.get("severity") == "critical"]
            )
            summary["warning_alerts"] = len(
                [a for a in self.alert_history if a.get("severity") == "warning"]
            )
    ...
    def export_monitoring_report(
        self, output_dir: str = "data/monitoring/fractional_system"
    ) -> str:
