...
class PredictionLedger:
    ...
    def mark_resolved(self, updates: pd.DataFrame) -> None:
        ...
        # ⚡ Bolt Optimization: Replace O(N) iterrows with bulk vectorized updates
        # Deduplicate updates to prevent "cannot reindex" errors
        upd_idx = upd_idx[~upd_idx.index.duplicated(keep="last")]

        # Handle schema evolution by explicitly adding missing columns using float('nan')
        ...
        # Update existing records in bulk (unconditional update to preserve nulls)
        if not common_idx.empty:
            old_idx.loc[common_idx, upd_idx.columns] = upd_idx.loc[common_idx]

        # Append new records in bulk
