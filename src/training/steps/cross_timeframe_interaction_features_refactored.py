"""Refactored cross-timeframe and interaction feature generation with reduced
complexity.
...
class CrossTimeframeFeatureGenerator:
    ...
    def _generate_momentum_features(
        self, price_components: Dict[str, pd.Series]
    ) -> Dict[str, pd.Series]:
    ...
    def _calculate_hl_momentum(
        self, high: pd.Series, low: pd.Series, close: pd.Series, tf1: int, tf2: int
    ) -> Dict[str, pd.Series]:
    ...
    def _calculate_volatility_pair(
        self, returns: pd.Series, tf1: int, tf2: int
    ) -> Dict[str, pd.Series]:
    ...
    def _generate_volume_features(
        self, price_components: Dict[str, pd.Series], volume_data: pd.DataFrame
    ) -> Dict[str, pd.Series]:
    ...
    def _calculate_volume_pair(
        self, volume: pd.Series, tf1: int, tf2: int
    ) -> Dict[str, pd.Series]:
    ...
    def _calculate_macd(
        self, prices: pd.Series, fast_period: int, slow_period: int
    ) -> pd.Series:
    ...
    def _calculate_bollinger_position(
        self, prices: pd.Series, window: int, num_std: float
    ) -> Optional[pd.Series]:
...
class InteractionFeatureGenerator:
    ...
    def _select_top_features(self, features: pd.DataFrame) -> List[str]:
        ...
        top_features = valid_features.nlargest(
