"""
Analyst Ensemble Trainer - Unified Training Architecture

This module provides the ensemble trainer class for Analyst model training,
...
from .analyst_base_trainer import AnalystBaseTrainer, AnalystTrainingConfig, AnalystModelType
...
@dataclass
class AnalystEnsembleTrainingConfig(AnalystTrainingConfig):
...
class AnalystEnsembleTrainer(BaseTrainer):
    """
    Ensemble trainer for Analyst models.
    
    This class provides ensemble training capabilities for combining multiple
    Analyst base models for enhanced performance.
    """
    
    def __init__(self, config: AnalystEnsembleTrainingConfig, logger: Optional[logging.Logger] = None):
        """
        Initialize the Analyst ensemble trainer.
        
        Args:
            config: Analyst ensemble training configuration
        ...
        tprint_info(f"🔧 Initialized AnalystEnsembleTrainer with {config.ensemble_method.value}")
        self.logger.info(f"Initialized AnalystEnsembleTrainer with {config.ensemble_method.value}")
    
    @handles_errors(
        exceptions=(ValueError, RuntimeError, MemoryError),
        default_return=TrainingResult(
            success=False,
            error_message="Analyst ensemble training failed"
        ),
    ...
    async def train(self, data: pd.DataFrame, targets: Optional[pd.Series] = None) -> TrainingResult:
        ...
        try:
            ...
            self.logger.info(f"Analyst ensemble training completed in {training_time:.2f}s")
    ...
    async def validate(self, data: pd.DataFrame, targets: Optional[pd.Series] = None) -> ValidationResult:
        """
        Validate the trained Analyst ensemble.
        ...
        try:
            ...
            self.logger.info("Validating Analyst ensemble...")
            ...
            tprint_success(f"✅ Analyst ensemble validation completed in {validation_time:.2f}s")
            return result
    ...
    async def predict(self, data: pd.DataFrame) -> PredictionResult:
        """
        Make predictions with the trained Analyst ensemble.
        ...
        try:
            ...
            self.logger.info("Making Analyst ensemble predictions...")
            ...
            tprint_success(f"✅ Analyst ensemble predictions completed in {prediction_time:.2f}s")
            return result
    ...
    async def _train_base_models(self, data: pd.DataFrame, targets: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            for model_type in self.config.base_models:
                ...
                # Create base trainer
                base_config = AnalystTrainingConfig(
                ...
                base_trainer = AnalystBaseTrainer(base_config, self.logger)
            ...
            for model_name, trainer in self._base_trainers.items():
                ...
                if pred_result.success:
                    ...
                    if pred_result.probabilities:
                        model_probs = list(pred_result.probabilities.values())[0]
                        base_predictions[f"{model_name}_prob"] = model_probs
                else:
        ...
        try:
            ...
            return {'success': False, 'error': str(e)}
                ...
                def _create_meta_learner(self):
                    ...
                    try:
                        if self.config.meta_learner_type == ModelType.LIGHTGBM:
                            ...
                            return LogisticRegression(**self.config.meta_learner_params)
                ...
                def _create_weighted_model(self):
                    ...
                    class WeightedAveraging:
                        ...
                        def fit(self, X, y):
                            ...
                            self.weights_ = LogisticRegression(
                                solver='liblinear',
                                random_state=42,
                                **self.config.meta_learner_params
                            )
                ...
                async def _predict_ensemble(self, base_predictions: pd.DataFrame) -> np.ndarray:
                    ...
                    try:
                        if self.config.ensemble_method == EnsembleMethod.AVERAGING:
    ...
    async def _calculate_ensemble_metrics(self, data: pd.DataFrame, targets: pd.Series) -> Dict[str, float]:
        ...
        try:
            ...
            metrics = {
                'accuracy': accuracy_score(targets, ensemble_predictions),
                'precision': precision_score(targets, ensemble_predictions, average='weighted'),
                'recall': recall_score(targets, ensemble_predictions, average='weighted'),
                'f1': f1_score(targets, ensemble_predictions, average='weighted')
            }
