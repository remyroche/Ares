...
class LabelingStrategy:
    ...
    async def apply_triple_barrier_labeling(
        self,
        data: pd.DataFrame,
        ...
        # Process each barrier configuration
        all_labels = []
        for barrier_name, (upper_barrier, lower_barrier) in barriers.items():
            self.logger.info(
                f"  Processing barrier '{barrier_name}': "
                f"upper={upper_barrier:.4f}, lower={lower_barrier:.4f}"
            )
            
            barrier_labels = await self._apply_single_barrier(
                labeled_data,
                upper_barrier,
                lower_barrier,
                barrier_name,
        ...
        # Combine labels from different barriers
        if all_labels:
        ...
        labeled_data = self._apply_precision_filtering(
            labeled_data,
            precision_thresholds
        )
    ...
    async def _apply_single_barrier(
        self,
        data: pd.DataFrame,
        upper_barrier: float,
        lower_barrier: float,
        ...
        for i in range(n_samples - 1):
            # Skip if no analyst signal when required
            ...
            for j in range(1, max_lookahead + 1):
                ...
                if price_change >= upper_barrier:
                    labels.iloc[i] = {
                        ...
                        'exit_time': j,
                        'potential_profit_pct': price_change * 100,
                ...
                # Time barrier (max lookahead)
                elif j == max_lookahead:
                    # Classify based on final price change
                    if price_change > 0:
    ...
    def _calculate_signal_strength(
        ...
        # Base strength from analyst signal
        if 'analyst_signal' in row:
            strength = float(row['analyst_signal'])
        
        # Boost for direction agreement
        ...
        # e.g., volume confirmation, technical indicators, etc.
    ...
    def _combine_barrier_labels(
        ...
        # Start with the first set of labels
        combined = all_labels[0].copy()
        
        # Combine with other barrier results
        for labels in all_labels[1:]:
            # Update where current label is 0 (no signal)
            no_signal_mask = combined['label'] == 0
            
            # Take new labels where we don't have a signal yet
            for col in ['label', 'barrier_type', 'exit_time', 'potential_profit_pct', 'signal_strength']:
                if col in labels.columns:
                    combined.loc[no_signal_mask, col] = labels.loc[no_signal_mask, col]
            
            # For existing signals, take the one with higher signal strength
    ...
    def _apply_precision_filtering(
        self,
    ...
    @handles_errors(
        exceptions=(Exception,),
        default_return=pd.DataFrame(),
        context="multi-outcome labeling"
    )
    async def apply_multi_outcome_labeling(
        self,
        data: pd.DataFrame,
        barriers: Dict[str, Tuple[float, float]],
        analyst_predictions: pd.DataFrame,
        ...
        labeled_data = await self.apply_triple_barrier_labeling(
            data, barriers, analyst_predictions, {}, regime_id
        )
        
        # Convert to multi-class based on profit potential
        if 'potential_profit_pct' in labeled_data.columns:
            # Define class boundaries
            profit_bins = [-np.inf, -2.0, -0.5, 0.5, 2.0, np.inf]
            profit_labels = [0, 1, 2, 3, 4]  # Strong loss, loss, neutral, profit, strong profit
            
            labeled_data['multi_label'] = pd.cut(
                labeled_data['potential_profit_pct'],
                bins=profit_bins,
