...
class FractionalTripleBarrierLabeling:
    ...
    @handle_errors(
        exceptions=(Exception,), default_return=pd.DataFrame(), context="fractional_triple_barrier_labeling.apply"
    )
    ...
    def apply_fractional_triple_barrier_labeling(
        ...
        fractional_components = self._calculate_fractional_components(labeled_data, regime_labels, volatility_series)
        ...
        confidence_scores = self._calculate_confidence_scores(labeled_data, fractional_components)
    ...
    def _calculate_fractional_components(
        ...
        if self.fractional_config["enable_volatility_normalization"]:
            components["volatility_score"] = self._calculate_volatility_scores(labeled_data, volatility_series)
    ...
    def _combine_fractional_components(self, components: Dict[str, np.ndarray]) -> np.ndarray:
    ...
    def _calculate_confidence_scores(self, labeled_data: pd.DataFrame, components: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        component_std = np.std(
            [components["distance_score"], components["time_score"], components["volatility_score"]], axis=0
        )
