...
class EnhancedPipelineOrchestrator:
    ...
    async def validate_pipeline_prerequisites(self) -> bool:
        ...
        try:
            if not self.config.symbol or not self.config.exchange:
                ...
                return False
            data_dir_path = PipelineStandards.build_path('raw_data', self.config.exchange, self.config.symbol)
            ...
            dependency_check = PipelineStandards.validate_environment_dependencies(required_modules, self.logger)
    ...
    async def save_checkpoint(self, pipeline_name: str, result: PipelineResult) -> None:
        ...
        try:
            checkpoint_data = {'timestamp': format_datetime(get_current_datetime()), 'pipeline_name': pipeline_name, 'result': {'name': result.name, 'status': result.status.value, 'execution_time': result.execution_time, 'error': result.error, 'data_quality_score': result.data_quality_score}, 'pipeline_state': self.pipeline_state, 'config': {'symbol': self.config.symbol, 'exchange': self.config.exchange, 'timeframe': self.config.timeframe, 'data_dir': self.config.data_dir}}
    ...
    async def run_all_pipelines(self) -> bool:
        ...
        for result in self.pipeline_results:
            status_emoji = {PipelineStatus.COMPLETED: '✅ SUCCESS', PipelineStatus.FAILED: '❌ FAILED', PipelineStatus.ROLLED_BACK: '🔄 ROLLED_BACK'}.get(result.status, '❓ UNKNOWN')
