...
class ConfidenceCalibrationStep(BaseValidationStep):
    ...
    def _initialize_step(self) -> None:
        ...
        self.calibration_config = {
            ...
            "cv_folds": self.config.get("calibration_cv_folds", 3),
            "ensemble_calibration": self.config.get("ensemble_calibration", True),
            "regime_specific_calibration": self.config.get("regime_specific_calibration", True),
            "min_samples_for_calibration": self.config.get("min_samples_for_calibration", 100)
        }
    ...
    def _validate_step_specific_inputs(
        ...
        # Check for models that support probability predictions
    ...
    @handles_errors(
        ...
        context="confidence calibration logic"
    )
    async def execute_logic(
        ...
        # Apply ensemble calibration if configured
        if self.calibration_config["ensemble_calibration"]:
            ensemble_calibration = await self._apply_ensemble_calibration(
                self.calibrated_models, X_val, y_val
            )
        ...
        if len(calibrated_models) > 1:
            try:
                ...
                if predictions:
                    ...
                    # Create temperature-scaled ensemble
                    ensemble_calibrated["ensemble_temperature_scaled"] = {
    ...
    def _calculate_overall_calibration_metrics(self) -> Dict[str, float]:
        ...
        metrics = {
            ...
            "avg_log_loss_improvement": [],
            "n_calibrated": len(self.calibrated_models)
        }
    ...
    def _add_step_specific_summary(
        ...
        if overall.get("avg_brier_improvement", 0) > 0:
            summary["key_findings"].append(
                f"Average Brier score improvement: {overall['avg_brier_improvement']:.2%}"
            )
        
        if overall.get("avg_log_loss_improvement", 0) > 0:
            summary["key_findings"].append(
                f"Average log loss improvement: {overall['avg_log_loss_improvement']:.2%}"
            )
        ...
        if overall.get("avg_brier_improvement", 0) < 0:
            summary["recommendations"].append(
                "Calibration worsened performance - consider using uncalibrated models"
            )
