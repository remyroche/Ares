...
Provides decorators for validating function inputs using various validation strategies
(pydantic, dataclasses, custom validators).
"""
    ...
    def sync_handler(func: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
        ...
        if errors:
            raise ValidationError(
                f"Validation failed for {func.__name__}", details={"errors": errors}
            )
    ...
    *,
    ...
    def sync_handler(func: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
        ...
        if param_name:
            ...
            if not args:
                raise ValidationError("No arguments provided to validate")
            param_value = args[0]
            param_name_used = params[0] if params else "arg0"

        # Validate against schema
        validated_value = _validate_against_schema(
            param_value, schema, param_name or param_name_used, allow_extra
        )
    ...
    *,
    ...
    def sync_handler(func: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
        ...
        if param_name not in bound.arguments:
            raise ValidationError(f"Parameter {param_name} not found")
...
def _validate_param(
    name: str, value: Any, expected_type: Type, strict: bool, coerce: bool
) -> None:
...
def _validate_against_schema(
    value: Any, schema: Union[Type, Dict[str, Type]], param_name: str, allow_extra: bool
) -> Any:
    ...
    # Handle dict schemas
        ...
        if not isinstance(value, dict):
            raise ValidationError(
                f"Expected dict for {param_name}, got {type(value).__name__}",
                field=param_name,
            )
        ...
        if errors:
            raise ValidationError(
                "Dict validation failed", field=param_name, details={"errors": errors}
            )
