...
"""
Financial metrics logging for Step17_Financial.
...
class Step17FinancialloggingFinancialLogger:
    """Independent financial metrics logger for Step17_Financial with enhanced regime logging."""
    ...
    def log_step_execution(self, *args, data: Optional[pd.DataFrame] = None, **kwargs) -> bool:
        """
        Log comprehensive financial metrics for Step17_Financial execution with enhanced regime validation.
        ...
        try:
            ...
            if validate_and_log_regime_data:
                validation_success = validate_and_log_regime_data(
                    ...
                    step_name="Step17_Financial",
                    data=data,
                    regime_column='composite_cluster_id'
                )
                
                if not validation_success:
                    logger.error("🚨 Regime validation failed for Step17_Financial")
                    return False
            
            # Log step start
            self.financial_logger.log_step_start("Step17_Financial", self.symbol, self.exchange, self.timeframe)
    ...
    def _log_with_standard_method(self, *args, **kwargs) -> bool:
        ...
        with financial_metrics_context(
            ...
            try:
                ...
                self.financial_logger.log_step_end("Step17_Financial", self.symbol, self.exchange, self.timeframe, success=True)
                
                return True
                
            except Exception as e:
                self.financial_logger.log_step_end("Step17_Financial", self.symbol, self.exchange, self.timeframe, success=False, error_message=str(e))
                logger.error(f"Failed to log financial metrics: {e}")
                return False
    ...
    def _log_created_file_paths(self) -> None:
        ...
        try:
            ...
            logger.info("📁 File paths logged for Step17_Financial")
        except Exception as e:
            logger.warning(f"Could not log file paths: {e}")

# Enhanced Step17Financiallogging Financial Logger with Regime-Aware Decorator Support
class EnhancedStep17FinancialloggingFinancialLogger(Step17FinancialloggingFinancialLogger):
    """Enhanced Step17Financiallogging Financial Logger with automatic regime-aware logging decorator support."""
