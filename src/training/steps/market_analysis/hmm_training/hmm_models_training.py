...
This module handles the training of base models for HMM regime prediction.
It includes base model training, hyperparameter optimization, model saving, and metrics calculation.

Base Models:
- Logistic Regression
- LightGBM  
- GRU (LSTM alternative)
...
try:
    ...
    raise ImportError("TensorFlow not available. GRU models require TensorFlow.")
...
class HMMModelsTraining:
    ...
    def train_base_models(self, X: pd.DataFrame, y: np.ndarray, 
    ...
    def save_models(self, models: Dict[str, Any], symbol: str, exchange: str, 
                   timeframe: str, data_dir: str) -> List[str]:
        """Save trained base models."""
        import pickle
        from pathlib import Path
        
        models_dir = Path(data_dir) / 'models' / 'hmm' / 'base_models'
        models_dir.mkdir(parents=True, exist_ok=True)
        
        model_paths = []
        
        for name, model in models.items():
            model_path = models_dir / f'hmm_base_{name}_{symbol}_{exchange}_{timeframe}.pkl'
            with open(model_path, 'wb') as f:
                pickle.dump(model, f)
            model_paths.append(str(model_path))
        
        print(f"✅ Saved {len(model_paths)} base models")
        return model_paths