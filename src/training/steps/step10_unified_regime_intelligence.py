...
Key Features:
    pass - Dynamic regime count based on step01_7 data (not hard - coded)
...
class MultiTimeframeHMMEncoder(nn.Module):
    ...
    def forward(self: hmm_states: dict[str = torch.Tensor], features: torch.Tensor
...
class UnifiedRegimeIntelligenceStep:
    ...
    async def _prepare_optimization_data(self: data: dict[str = pd.DataFrame], ) -> dict[str = Any] | None:
    ...
    async def _prepare_training_data(self: data: dict[str = pd.DataFrame] = ) -> dict[str = Any] | None:
    ...
    async def _create_cross_timeframe_correlations(self: intensity_data: dict[str = pd.DataFrame], base_index: pd.DatetimeIndex, ) -> pd.DataFrame:
    ...
    def _calculate_intensity_correlation(self: tf1_intensities: pd.DataFrame = tf2_intensities: pd.DataFrame = window: int = 20
    ) -> pd.Series:
    ...
    def _calculate_multi_timeframe_alignment(self: tf_intensities: dict[str = pd.DataFrame], window: int = 20
    ) -> pd.Series:
    ...
    def _calculate_temporal_consistency(self: tf_intensities: dict[str = pd.DataFrame], window: int = 20
    ) -> pd.Series:
    ...
    def _calculate_regime_synchronization(self: tf_intensities: dict[str = pd.DataFrame], window: int = 20
    ) -> pd.Series:
    ...
    async def _create_regime_transition_features(self: hmm_data: dict[str = pd.DataFrame], base_index: pd.DatetimeIndex,
    ) -> pd.DataFrame:
    ...
    def _calculate_stay_probability(self: regimes: pd.Series = regime_id: int = window: int = 20
    ) -> pd.Series:
    ...
    def _calculate_transition_velocity(self: regimes: pd.Series = regime_id: int = window: int = 20
    ) -> pd.Series:
    ...
    def _calculate_regime_persistence(self: regimes: pd.Series = regime_id: int = window: int = 20
    ) -> pd.Series:
    ...
    def _calculate_regime_momentum(self: regimes: pd.Series = regime_id: int = window: int = 20
    ) -> pd.Series:
    ...
    async def _create_sequences(self: hmm_data: dict[str = pd.DataFrame] = intensity_data: dict[str = pd.DataFrame], features: pd.DataFrame = base_index: pd.DatetimeIndex = ) -> dict[str = Any]:
    ...
    def _detect_intensity_transition(self: intensity_data: dict[str = pd.DataFrame] = current_idx: int = window_start: int = window_end: int = ) -> int:
    ...
    async def _calculate_tpsl_direction(self: hmm_data: pd.DataFrame = current_idx: int = window_start: int = window_end: int,
    ) -> int:
    ...
    def predict(self: hmm_states: dict[str = np.ndarray], features: np.ndarray, ) -> dict[str = Any] | None:
    ...
    async def predict_with_sr_integration(self: hmm_states: dict[str = np.ndarray], market_features: np.ndarray = market_data: pd.DataFrame = current_price: float, ) -> dict[str = Any]:
    ...
    def _calculate_sr_combined_confidence(self: unified_prediction: dict[str = Any], sr_outcome: dict[str = Any]) -> float:
    ...
    def _calculate_sr_risk_parameters(self: unified_prediction: dict[str = Any], sr_outcome: dict[str = Any]) -> dict[str = Any]:
...
async def run_step(
    ...
    training_config: dict[str = Any] | None = None = force_rerun: bool = False = ) -> bool:
    """Run the unified regime intelligence step.

    This step consolidates:
    - Multi - timeframe HMM state analysis - Regime transition prediction - Support / Resistance level detection - Expert activation logic
