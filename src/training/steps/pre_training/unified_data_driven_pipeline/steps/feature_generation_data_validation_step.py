...
## Executive Summary
- **Overall Quality Score**: {quality_report.get('overall_quality_score', 0.0):.3f}
- **Data Shape**: {market_data.shape[0]} rows × {market_data.shape[1]} columns
- **Memory Usage**: {market_data.memory_usage(deep=True).sum() / 1024 / 1024:.2f} MB

## Schema Validation
- **Status**: {'✅ PASSED' if quality_report.get('schema_validation', {}).get('valid', False) else '❌ FAILED'}
- **Issues**: {len(quality_report.get('schema_validation', {}).get('issues', []))}
- **Warnings**: {len(quality_report.get('schema_validation', {}).get('warnings', []))}

## Statistical Validation
- **Status**: {'✅ PASSED' if quality_report.get('statistical_validation', {}).get('valid', False) else '❌ FAILED'}
- **Quality Score**: {quality_report.get('statistical_validation', {}).get('quality_score', 0.0):.3f}
- **Missing Data**: {quality_report.get('statistical_validation', {}).get('missing_data_percentage', 0.0):.2f}%
- **Outliers**: {quality_report.get('statistical_validation', {}).get('outlier_percentage', 0.0):.2f}%

## Input Validation
- **Status**: {'✅ PASSED' if quality_report.get('input_validation', {}).get('valid', False) else '❌ FAILED'}
- **Issues**: {len(quality_report.get('input_validation', {}).get('issues', []))}
- **Warnings**: {len(quality_report.get('input_validation', {}).get('warnings', []))}

## Data Overview
- **Columns**: {', '.join(market_data.columns[:10])}{'...' if len(market_data.columns) > 10 else ''}
- **Data Types**: {dict(market_data.dtypes.value_counts())}
- **Missing Values**: {market_data.isnull().sum().sum()}
...
"""
        ...
        if recommendations:
            ...
            report_content += "- No specific recommendations at this time.\n"
        
        report_content += f"""
## Next Steps
1. Review any failed validations
2. Address data quality issues if present
3. Proceed to feature generation step

---
*Report generated by Feature Generation Data Validation Step*
"""
