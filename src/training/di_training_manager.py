...
class DITrainingManager(InjectableBase):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to initialize training manager: {e}")
            return False
    ...
    async def _initialize_training_steps(self) -> None:
        ...
        for step_name in step_classes:
            try:
                ...
                class_name = "".join([word.capitalize() for word in step_name.split("_")])
                ...
                self.logger.warning(f"Failed to initialize training step {step_name}: {e}")
    ...
    def _validate_training_configuration(self) -> bool:
        ...
        try:
            ...
            for dir_name in required_dirs:
                ...
                if not dir_path:
                    self.logger.error(f"Missing {dir_name} directory configuration")
                    return False
    ...
    async def run_training_pipeline(
        ...
        try:
            ...
            self.logger.info(f"Starting {training_type} training pipeline for {symbol} on {exchange}")
            ...
            self.logger.info(f"Training pipeline {'completed' if success else 'failed'}")
            return success
    ...
    async def _run_full_training_pipeline(self, context: dict[str, Any]) -> bool:
        ...
        try:
            if not self.training_pipeline:
                self.logger.error("Training pipeline not initialized")
                return False
            ...
            for step_name in pipeline_steps:
                ...
                if not step:
                    self.logger.warning(f"Training step {step_name} not available")
                    continue
                ...
                if not success:
                    self.logger.error(f"Training step {step_name} failed")
                    return False
            ...
            self.logger.error(f"Full training pipeline failed: {e}")
            return False
    ...
    async def _run_incremental_training(self, context: dict[str, Any]) -> bool:
        ...
        try:
            ...
            for step_name in incremental_steps:
                ...
                if not success:
                    self.logger.error(f"Incremental step {step_name} failed")
                    return False
    ...
    async def _run_hyperparameter_optimization(self, context: dict[str, Any]) -> bool:
        ...
        try:
            ...
            for step_name in optimization_steps:
                ...
                if not success:
                    self.logger.error(f"Optimization step {step_name} failed")
                    return False
    ...
    async def _record_training_result(self, context: dict[str, Any], success: bool) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to record training result: {e}")
