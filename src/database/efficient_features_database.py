...
def _generate_database_name(...) -> ...:
    """..."""
    passif start_date is None:
    passstart_date = datetime.now().strftime("%Y-%m-%d")
...
def _get_database_path(...) -> ...:
    """..."""
    passif self.storage_format == "pickle":
    passextension = ".pkl"
elif self.storage_format == "parquet":
    passpassextension = ".parquet"
...
and "precomputed_features" in filename
    ...
    pass# Parse database metadata from filename
...
return databases
    ...
    passpasspasspasspasspasspassself.print(error("Error scanning existing databases: {e}"))
return []
...
if isinstance(cols, list) and len(cols) > 0:
    passdata = pd.read_parquet(db_path, columns=cols)
else:
    passdata = pd.read_parquet(db_path)
except Exception:
    passpassdata = pd.read_parquet(db_path)
elif self.storage_format == "hdf5":
    passpassdata = pd.read_hdf(db_path, key="features")
else:
    passreturn {}
...
return {}
    ...
    passpasspasspasspasspasspassself.print(warning("Error reading database info: {e}"))
return {}

def _analyze_feature_categories(...) -> ...:
    """..."""
    passcategories = {}
for col in columns:
    passif "_" in col:
    passcategory = col.split("_")[0]
...
and metadata.get("exchange", "").upper() == clean_exchange
    ...
    passmatching_dbs.append((db_name, metadata))
...
if start_time and end_time:
    passdb_start = metadata.get("start_time")
db_end = metadata.get("end_time")

if db_start is None or db_end is None:
    pass# No time info available, assume we need to process everything
missing_ranges = [(start_time, end_time)]
else:
    pass# Convert to pandas timestamps if needed
if not isinstance(db_start, pd.Timestamp):
    passdb_start = pd.Timestamp(db_start)
if not isinstance(db_end, pd.Timestamp):
    passdb_end = pd.Timestamp(db_end)

# Check for gaps
if start_time < db_start:
    passpassmissing_ranges.append((start_time, db_start))
if end_time > db_end:
    passmissing_ranges.append((db_end, end_time))

return db_name, missing_ranges

except Exception as e:
    passpasspasspasspasspasspassself.print(error("Error finding existing database: {e}"))
...
if database_name not in self.metadata_cache:
    passself.print(missing("Database not found: {database_name}"))
return pd.DataFrame()
...
return data
    ...
    passpasspasspasspasspasspasspasspassself.print(error("Error loading database {database_name}: {e}"))
return pd.DataFrame()
...
if database_name is None:
    passstart_date = data.index.min().strftime("%Y-%m-%d")
...
if not isinstance(data.index, pd.DatetimeIndex):
    passdata.index = pd.to_datetime(data.index, unit="ms")

# Save based on format
if self.storage_format == "pickle":
    passwith open(db_path, "wb") as f:
    passpickle.dump(data, f, protocol=pickle.HIGHEST_PROTOCOL)
elif self.storage_format == "parquet":
    passpassif self.compression:
    passdata.to_parquet(db_path, compression="snappy")
else:
    passdata.to_parquet(db_path)
elif self.storage_format == "hdf5":
    passpassif self.compression:
    passdata.to_hdf(
db_path,
...
complib="zlib",
    ...
    passdata.to_hdf(db_path, key="features", mode="w")
...
if len(data) < self.chunk_size * 10:
    passself.database_cache[database_name] = data.copy()

self.logger.info(f"✅ Database saved successfully: {db_path}")
return True

except Exception as e:
    passpasspasspasspasspasspassself.logger.error(f"❌ Error saving database: {e}")
return False
...
if not isinstance(new_data.index, pd.DatetimeIndex):
    passnew_data.index = pd.to_datetime(new_data.index, unit="ms")
if not isinstance(existing_data.index, pd.DatetimeIndex):
    passexisting_data.index = pd.to_datetime(existing_data.index, unit="ms")
...
if truly_new_data.empty:
    passself.logger.info("No new timestamps found, database already up-to-date")
return True
...
return success
    ...
    passpasspasspasspasspasspassself.print(error("❌ Error updating database: {e}"))
return False
...
return True
    ...
    passpasspasspasspasspasspassself.logger.error(f"❌ Error saving database with timestamp: {e}")
return False
...
for db_name, metadata in self.metadata_cache.items():
    passif symbol and metadata.get("symbol", "").upper() != symbol.upper():
    passcontinue
...
def get_database_stats(...) -> ...:
    """..."""
    passtotal_databases = len(self.metadata_cache)
total_size = sum(
...
if db_name in self.metadata_cache:
    passdel self.metadata_cache[db_name]
if db_name in self.database_cache:
    passdel self.database_cache[db_name]

except Exception as e:
    passpasspasspasspasspasspassself.logger.error(f"Error deleting database {db_name}: {e}")
...
f"Cleanup completed. Deleted {deleted_count} old databases",
    ...
    passpasspasspasspasspasspassself.logger.error(f"Error during database cleanup: {e}")
