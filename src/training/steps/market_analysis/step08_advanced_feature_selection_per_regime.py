...
class PerRegimeAdvancedFeatureSelectionStep(Step08AdvancedFeatureSelection):
    ...
    def _get_regime_feature_selection_config(self, regime_id: int) -> Dict[str, Any]:
        ...
        base_config = {'enable_correlation_filtering': True, 'enable_variance_filtering': True, 'enable_mutual_information': True, 'enable_recursive_feature_elimination': True, 'enable_permutation_importance': True}
        if regime_id <= 2:
            return {**base_config, 'selection_strategy': {'max_features': 50, 'correlation_threshold': 0.8, 'variance_threshold': 0.01, 'mutual_info_threshold': 0.05, 'emphasis': 'trend_features'}, 'feature_types': ['trend_indicators', 'momentum_features', 'volume_trend_features', 'price_trend_features']}
        elif regime_id >= 5:
            return {**base_config, 'selection_strategy': {'max_features': 40, 'correlation_threshold': 0.7, 'variance_threshold': 0.02, 'mutual_info_threshold': 0.03, 'emphasis': 'volatility_features'}, 'feature_types': ['volatility_indicators', 'mean_reversion_features', 'oscillator_features', 'range_features']}
        else:
            return {**base_config, 'selection_strategy': {'max_features': 45, 'correlation_threshold': 0.75, 'variance_threshold': 0.015, 'mutual_info_threshold': 0.04, 'emphasis': 'balanced_features'}, 'feature_types': ['mixed_indicators', 'balanced_features', 'adaptive_features', 'composite_features']}
    ...
    async def _apply_regime_feature_selection(self, matrix_data: Dict[str, Any], regime_config: Dict[str, Any], regime_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not feature_columns:
                ...
                return None
            results = {'regime_id': regime_id, 'total_features': len(feature_columns), 'selection_strategy': regime_config.get('selection_strategy', {}), 'selected_features': [], 'feature_scores': {}, 'selection_metadata': {}}
    ...
    def _apply_correlation_filtering(self, matrix_data: Dict[str, Any], regime_config: Dict[str, Any], feature_columns: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'threshold': correlation_threshold, 'high_correlations': len(high_correlations), 'features_removed': len(features_to_remove), 'features_remaining': len(remaining_features), 'removed_features': list(features_to_remove)}
        except Exception as e:
    ...
    def _apply_variance_filtering(self, matrix_data: Dict[str, Any], regime_config: Dict[str, Any], feature_columns: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if feature_importance:
                high_variance_features = [feature for feature, importance in zip(feature_columns, feature_importance) if importance >= variance_threshold]
                return {'threshold': variance_threshold, 'total_features': len(feature_columns), 'high_variance_features': len(high_variance_features), 'features_removed': len(feature_columns) - len(high_variance_features), 'selected_features': high_variance_features}
            else:
    ...
    def _apply_mutual_information_filtering(self, matrix_data: Dict[str, Any], regime_config: Dict[str, Any], feature_columns: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            feature_importance = balanced_ops.get('feature_importance', [])
            if feature_importance:
                high_mi_features = [feature for feature, importance in zip(feature_columns, feature_importance) if importance >= mi_threshold]
                return {'threshold': mi_threshold, 'total_features': len(feature_columns), 'high_mi_features': len(high_mi_features), 'features_removed': len(feature_columns) - len(high_mi_features), 'selected_features': high_mi_features}
            else:
    ...
    def _apply_recursive_feature_elimination(self, matrix_data: Dict[str, Any], regime_config: Dict[str, Any], feature_columns: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if explained_variance:
                ...
                return {'max_features': max_features, 'selected_features': len(top_features), 'features_removed': len(feature_columns) - len(top_features), 'selected_features': top_features}
            else:
    ...
    def _apply_permutation_importance(self, matrix_data: Dict[str, Any], regime_config: Dict[str, Any], feature_columns: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if clustering_ops:
                ...
                return {'total_features': len(feature_columns), 'important_features': len(important_features), 'features_removed': len(feature_columns) - len(important_features), 'selected_features': important_features}
            else:
