...
def _minute_path_pruned(
    ...
    tables = [pq.ParquetFile(path).read(columns=["ts", "open", "high", "low", "close"]) for path in fragments]
    if tables:
...
def _load_packb_candidates(source: Path, month: pd.Timestamp, side: str) -> pd.DataFrame:
    ...
    return _validate_candidate_frame(
        pd.read_parquet(path, columns=list(IDENTITY_COLUMNS)),
...
def _load_candidates(source: Path, month: pd.Timestamp, side: str, *, source_kind: str) -> pd.DataFrame:
    if source_kind == "packb":
        return _load_packb_candidates(source, month, side)
...
def _label_candidates_with_minute(
    ...
    invalid_targets = [
        "t2_tp6_sl4_event", "t2_tp6_sl4_exit_minute", "t4_tp6_sl4_exit_pnl_atr", "t4_tp6_sl4_terminal_pnl_atr",
...
def _write_status(
    ...
    start: pd.Timestamp, end: pd.Timestamp, required_months: list[pd.Timestamp], source_kind: str = "packb",
) -> None:
    ...
    expected_cells = len(required_months) * len(SIDES)
    complete = len(coverage) == expected_cells and coverage["status"].isin(["materialised", "reused"]).all()
    manifest = {
        "schema": SCHEMA,
        ...
        "cells": {"months": [m.strftime("%Y-%m") for m in required_months], "sides": list(SIDES), "expected": expected_cells, "complete": int(len(coverage))},
        "contract": {
...
def _args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--source-kind", choices=("packb", "historical", "stage_i_common30"),
        default="packb",
    )
...
def main() -> None:
    ...
    for month in selected_months:
        ...
        for side in selected_sides:
            candidates = _load_candidates(source, month, side, source_kind=args.source_kind)
            destination = args.out / "parts" / f"month={month:%Y-%m}" / f"side={side}.parquet"
        ...
        if pending and args.symbol_batch_size:
            ...
            if materialised is None:
                _write_status(
                    ...
                    start=start, end=end, required_months=all_months, source_kind=args.source_kind,
                )
                print(json.dumps({
