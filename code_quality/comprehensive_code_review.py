            ...
            if call.name not in defined_functions and call.name not in imported_functions:
                ...
                if not hasattr(__builtins__, call.name):
                    self._add_issue(
                        ...
                        f"Define the function or import it from the appropriate module"
                    )
...
class CodeQualityASTVisitor(ast.NodeVisitor):
    ...
    def _check_unused_parameters(self, node: Union[ast.FunctionDef, ast.AsyncFunctionDef]) -> None:
    ...
    def _is_magic_number_acceptable(self, value: Union[int, float]) -> bool:
